...
int32_t main(int32_t argc, char * argv[]){
	...
	if(io.initialize()){
		...
		tls_t::id_t cts = tls.context(event::node_t::SERVER, event::protocol_t::SCTP);
		...
		tls.on(cts, [&log](const tls_t::id_t id, const tls_t::error_t error, const string & message) noexcept -> void {
			/**
		...
		if(io.address(eid, event::address_t::IPV4, "127.0.0.1")){
			...
			io.on(eid, static_cast <event::callback::accept_t> ([cts, &sctp, &tls, &io, &log](const event::id_t eid, const event::id_t cid) noexcept -> void {
