...
int32_t main(int32_t argc, char * argv[]){
	...
	if(io.initialize()){
		...
		tls.on(tid, [eid, &tls, &io, &log](const tls_t::id_t id) noexcept -> void {
		...
		tls.on(tid, [eid, &tls, &io, &log](const tls_t::id_t id, const tls_t::event_t event, const uint8_t * buffer, const size_t size) noexcept -> void {
			/**
			 * Обрабатываем тип события DTLS
			 */
			switch(static_cast <uint8_t> (event)){
		...
		if(io.address(eid, event::address_t::IPV4, "127.0.0.1")){
			...
			io.on(eid, [tid, &tls, &io, &log](const event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
				...
				cout << "Client: " << io.address(eid, event::address_t::IPV4) << ":" << io.port(eid) << endl;
				// Устанавливаем клиента DTLS для события
				tls.peer(tid, io.address(eid, event::address_t::IPV4), io.port(eid));
