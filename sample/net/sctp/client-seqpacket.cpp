 ...
 *
 * @brief Пример клиента SCTP в режиме последовательных пакетов —
 *        демонстрация обмена сообщениями с сохранением их границ и распределения по исходящим потокам ассоциации
 *
	...
	if(io.initialize()){
		...
		if(io.setAddress(eid, event::address_t::IPV4, "0.0.0.0")){
			...
			if(io.setTarget(eid, "127.0.0.1")){
				...
				io.on(eid, [&sctp, &io](const event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
