 ...
 *
 * @brief Пример сервера SCTP поверх TLS —
 *        демонстрация приёма защищённых потоковых ассоциаций и обслуживания подключённых клиентов
 *
...
int32_t main(int32_t argc, char * argv[]){
	...
	if(io.initialize()){
		...
		if(io.setAddress(eid, event::address_t::IPV4, "127.0.0.1")){
			...
			io.on(eid, static_cast <engine::callback::accept_t> ([cts, &sctp, &tls, &io](const event::id_t sid, const event::id_t cid) noexcept -> void {
