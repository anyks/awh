 ...
 *
 * @brief Пример сервера SCTP в режиме последовательных пакетов —
 *        демонстрация приёма сообщений с сохранением их границ и ответа клиентам по указанным потокам
 *
...
int32_t main(int32_t argc, char * argv[]){
	...
	if(io.initialize()){
		...
		if(io.setAddress(eid, event::address_t::IPV4, "127.0.0.1")){
			...
			io.on(eid, static_cast <engine::callback::accept_t> ([&sctp, &io](const event::id_t sid, const event::id_t cid) noexcept -> void {
