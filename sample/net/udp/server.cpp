 ...
 *
 * @brief Пример сервера UDP на низкоуровневом движке ввода-вывода — демонстрация привязки датаграммного сокета,
 *        приёма датаграмм и ответа отправителю
 *
...
int32_t main(){
	...
	if(io.initialize()){
		...
		if(io.setAddress(eid, event::address_t::IPV4, "127.0.0.1")){
			...
			io.on(eid, static_cast <engine::callback::accept_t> ([&io](const event::id_t eid, const event::id_t cid) noexcept -> void {
				...
				io.on(cid, [&io](const event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
					...
					if(io.send(eid, reinterpret_cast <const char *> (data), size))
						...
						log::print("Отправлено: ID=%u, %zu байт", log::flag_t::INFO, eid, size);
			...
			io.setTimeout(eid, event::action_t::READ, 6000);
