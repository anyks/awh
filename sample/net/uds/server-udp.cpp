 ...
 *
 * @brief Пример сервера Unix Domain Socket в датаграммном режиме на низкоуровневом движке ввода-вывода —
 *        демонстрация приёма сообщений через файл сокета и ответа отправителю
 *
	...
	if(io.initialize()){
		...
		if(io.setAddress(eid, event::address_t::UDS, "/tmp/awh.sock")){
			...
			io.on(eid, static_cast <engine::callback::accept_t> ([&io](const event::id_t eid, const event::id_t cid) noexcept -> void {
				// Записываем в лог сообщение о принятии события
				log::print("Событие принято: ID=%u, Клиентский ID=%u, UDS=%s", log::flag_t::INFO, eid, cid, io.getAddress(cid, event::address_t::UDS).c_str());
				// Устанавливаем функцию обратного вызова на изменение статуса события
