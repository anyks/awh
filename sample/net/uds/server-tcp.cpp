 ...
 *
 * @brief Пример сервера Unix Domain Socket в потоковом режиме на низкоуровневом движке ввода-вывода —
 *        демонстрация создания файла сокета и обслуживания локальных подключений
 *
...
int32_t main(){
	...
	if(io.initialize()){
		...
		if(io.setAddress(eid, event::address_t::UDS, "/tmp/awh.sock")){
			...
			io.on(eid, static_cast <engine::callback::accept_t> ([&io](const event::id_t sid, const event::id_t cid) noexcept -> void {
				// Записываем в лог сообщение о принятии события
