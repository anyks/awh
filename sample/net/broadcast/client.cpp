 ...
 *
 * @brief Пример клиента широковещательной рассылки —
 *        демонстрация отправки датаграмм на широковещательный адрес подсети средствами движка ввода-вывода
 *
...
int32_t main(){
	...
	if(io.initialize()){
		...
		if(io.setOptions(eid, event::options::NO_SIGILL | event::options::NO_SIGPIPE | event::options::REUSE_ADDR | event::options::NO_IO_BLOCK | event::options::CLOSE_ON_EXEC | event::options::TCP_NO_DELAY | event::options::AUTO_RECONNECT | event::options::BROADCAST))
		...
		if(io.setAddress(eid, event::address_t::IPV4, "0.0.0.0")){
			...
			if(io.setTarget(eid, "192.168.3.255")){
