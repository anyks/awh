...
int32_t main(){
	...
	*/
	...
	engine::io_t io;
	// Добавляем новое событие файла
	event::id_t fid = io.event(event::node_t::FILE, event::family_t::FSYS);
	...
	if(io.initialize()){
		...
		// Устанавливаем IP-адрес и путь к файлу для отслеживания
		if(io.setAddress(eid, event::address_t::IPV4, "0.0.0.0") && io.setAddress(fid, event::address_t::FS, "../README.md")){
			// Устанавливаем адрес сервера назначения
			if(io.setTarget(eid, "127.0.0.1") && io.splice(fid, eid)){
				...
				io.on(eid, static_cast <engine::callback::connect_t> ([fid, &io](const event::id_t eid, const bool ok) noexcept -> void {
					// Записываем в лог сообщение о принятии события
					log::print("Событие подключения: ID=%u, результат: %s", log::flag_t::INFO, eid, ok ? "YES" : "NO");
					// Выполняем фиксацию настроек события сервера
					if(io.commit(fid)){
						// Устананавливаем опции события
						if(io.setOptions(fid, event::options::AUTO_FOLLOW))
							// Записываем в лог сообщение об успешной установке опций события
							cout << " Успешно установлены опции события!" << endl;
						// Записываем ошибку в лог установки опций события
						else cout << " Ошибка установки опций события!" << endl;
