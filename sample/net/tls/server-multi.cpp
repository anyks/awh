 ...
 *        запрошенному клиентом в расширении SNI, и обслуживания защищённых подключений
 *
...
int32_t main(){
	...
	if(io.initialize()){
		...
		if(io.setAddress(eid, event::address_t::IPV4, "127.0.0.1")){
			...
			io.on(eid, static_cast <engine::callback::accept_t> ([cts1, &tls, &io](const event::id_t sid, const event::id_t cid) noexcept -> void {
				// Записываем в лог сообщение о принятии события
				...
				if(io.setOptions(cid, event::options::NO_SIGILL | event::options::NO_SIGPIPE | event::options::REUSE_ADDR | event::options::NO_IO_BLOCK | event::options::CLOSE_ON_EXEC | event::options::TCP_NO_DELAY | event::options::AUTO_RECONNECT)){
					...
					tls.on(ctl, [cid, &tls, &io](const tls::coder_t::id_t id, const tls::coder_t::state_t state) noexcept -> void {
						/**
