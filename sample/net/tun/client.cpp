...
typedef struct Params {
	...
	explicit Params() noexcept :
	 ...
	 host{"127.0.0.1"}, net{"10.0.0.0"}, iface{""}, port(2222),
			...
			if(io.setTarget(eid, params.host) && io.setTarget(mid, params.peer) && io.setTarget(tid, params.peer) && io.splice(mid, eid)){
				...
				io.on(eid, [&log](const event::id_t eid, const event::status_t status) noexcept -> void {
					...
					switch(static_cast <uint8_t> (status)){
						...
						case static_cast <uint8_t> (event::status_t::ACCEPTED):
							...
							log.print("Событие клиента принято: ID=%u", log_t::flag_t::INFO, eid);
						break;
						// Если статус уничтожения
						case static_cast <uint8_t> (event::status_t::DESTROYED):
							// Записываем в лог сообщение об уничтожении события
							log.print("Событие клиента подлежит уничтожению: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::RESUMED):
							...
							log.print("Событие клиента возобновлено: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::SUCCESS):
							...
							log.print("Событие клиента успешно выполнено: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::FAILURE):
							...
							log.print("Событие клиента выполнено с ошибкой: ID=%u", log_t::flag_t::CRITICAL, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::PENDING):
							...
							log.print("Событие клиента в ожидании: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::CONNECTED):
							...
							log.print("Событие клиента подключено: ID=%u", log_t::flag_t::INFO, eid);
						break;
						// Если статус отмены события
						case static_cast <uint8_t> (event::status_t::CANCELLED):
							// Записываем в лог сообщение об отмене события
							log.print("Событие клиента отменено: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::RECONNECTED):
							...
							log.print("Событие клиента переподключено: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::LISTENING):
							...
							log.print("Событие клиента прослушивается: ID=%u", log_t::flag_t::INFO, eid);
						break;
					}
				});
				...
				io.on(mid, [&io, &log](const event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
					...
					if(io.send(eid, reinterpret_cast <const char *> (data), size))
						// Если данные успешно отправлены
						log.print("Отправлено в клиента: ID=%u, %zu байт", log_t::flag_t::INFO, eid, size);
					// Если данные не отправлены
					else log.print("Ошибка отправки в клиента: ID=%u", log_t::flag_t::CRITICAL, eid);
				});
				...
				io.on(eid, [&log](const event::id_t eid, const event::error_t error, const string & description) noexcept -> void {
					...
					switch(static_cast <uint8_t> (error)){
						...
						case static_cast <uint8_t> (event::error_t::UNKNOWN):
							...
							log.print("Неизвестная ошибка события клиента: ID=%u, Описание=%s", log_t::flag_t::CRITICAL, eid, description.c_str());
						break;
						...
						case static_cast <uint8_t> (event::error_t::INVALID):
							...
							log.print("Недопустимая операция события клиента: ID=%u, Описание=%s", log_t::flag_t::CRITICAL, eid, description.c_str());
						break;
						...
						case static_cast <uint8_t> (event::error_t::ACCESS_DENIED):
							...
							log.print("Доступ к событию клиента запрещён: ID=%u, Описание=%s", log_t::flag_t::CRITICAL, eid, description.c_str());
						break;
						...
						case static_cast <uint8_t> (event::error_t::ALREADY_EXISTS):
							...
							log.print("Объект события клиента уже существует: ID=%u, Описание=%s", log_t::flag_t::CRITICAL, eid, description.c_str());
						break;
						...
						case static_cast <uint8_t> (event::error_t::INVALID_SOCKET):
							...
							log.print("Ошибка доступа к сокету события клиента: ID=%u, Описание=%s", log_t::flag_t::CRITICAL, eid, description.c_str());
						break;
						...
						case static_cast <uint8_t> (event::error_t::INVALID_ADDRESS):
							...
							log.print("Некорректный адрес события клиента: ID=%u, Описание=%s", log_t::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка ошибки подключения
						case static_cast <uint8_t> (event::error_t::CONNECTION_FAIL):
							// Записываем ошибку в лог подключения
							log.print("Ошибка подключения события клиента: ID=%u, Описание=%s", log_t::flag_t::CRITICAL, eid, description.c_str());
						break;
						...
						case static_cast <uint8_t> (event::error_t::INSUFFICIENT_RES):
							...
							log.print("Недостаточно ресурсов для события клиента: ID=%u, Описание=%s", log_t::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка события
						case static_cast <uint8_t> (event::error_t::EVENT_FAIL):
							// Записываем ошибку в лог события
							log.print("Ошибка события клиента: ID=%u, Описание=%s", log_t::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если объект не найден
						case static_cast <uint8_t> (event::error_t::NOT_FOUND):
							// Записываем ошибку в лог события
							log.print("Объект события клиента не найден: ID=%u, Описание=%s", log_t::flag_t::CRITICAL, eid, description.c_str());
						break;
					}
				});
				...
				io.on(eid, [&log](const event::id_t eid, const event::action_t action) noexcept -> void {
					...
					switch(static_cast <uint8_t> (action)){
						...
						case static_cast <uint8_t> (event::action_t::DISCONNECT):
							...
							log.print("Событие клиента на отключение: ID=%u", log_t::flag_t::INFO, eid);
						break;
						// Если действие является переподключением
						case static_cast <uint8_t> (event::action_t::RECONNECT):
							// Записываем в лог сообщение о переподключении события
							log.print("Событие клиента на переподключение: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::action_t::CLOSE):
							...
							log.print("Событие клиента на закрытие подключения: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::action_t::CHANGE):
							...
							log.print("Событие клиента на изменение: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::action_t::DELETE):
							...
							log.print("Событие клиента на удаление: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::action_t::RENAME):
							...
							log.print("Событие клиента на переименование: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::action_t::ATTRIB):
							...
							log.print("Событие клиента на изменение атрибутов: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::action_t::REVOKE):
							...
							log.print("Событие клиента на отзыв доступа: ID=%u", log_t::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::action_t::HDLINK):
							...
							log.print("Событие клиента на изменение счётчика жёстких ссылок: ID=%u", log_t::flag_t::INFO, eid);
						break;
					}
				});
