 ...
 *
 * @note Подключается ПОСЛЕ стандартных модулей и до заголовков проекта: обработчик
 *       управляющих событий консоли объявлен видами самой системы
 */
#if _WIN32 || _WIN64
	#include <winsock2.h>
	#include <windows.h>
 ...
 *
 * @details Подключается ПОСЛЕ всех прочих подключений, как и предписано договором
 *          `sys/macro_push.hpp`: заголовок `windows.h` объявляет `INVALID_SOCKET`
 *          макросом, и запись `event::error_t::INVALID_SOCKET` без этой защиты
 *          столкнулась бы с ним. Пара снимается `sys/macro_pop.hpp` в конце файла
 *
 * @note Прежде образец обходился без неё лишь потому, что `windows.h` здесь не
 *       подключался вовсе. С приходом обработчика управляющих событий консоли он
 *       понадобился, и защита стала обязательной
 *
 */
...
#if _WIN32 || _WIN64
	/**
	 * @brief Обработчик управляющих событий консоли
	 *
	 * @details Сигналов у MS Windows нет вовсе - `::signal` там лишь подобие, и ни
	 *          закрытие окна консоли, ни завершение сеанса до него не доходят. Сервер
	 *          оттого снимался силой, а адрес туннеля оставался на устройстве до
	 *          перезагрузки машины: снять его успевает лишь тот, кому дали завершиться
	 *
	 * @note Событию закрытия система отводит считанные секунды и снимает процесс сама.
	 *       Оттого обработчик не просто ставит признак, а ДОЖИДАЕТСЯ окончания работы:
	 *       вернись он сразу, процесс погиб бы ровно там же, где и прежде
	 *
	 * @param type вид управляющего события консоли
	 * @return     признак обработанного события
	 *
	 */
	static BOOL WINAPI onConsole(DWORD type) noexcept {
		/**
		 * Определяем вид управляющего события консоли
		 */
		switch(type){
			// Если заказана остановка работы
			case CTRL_C_EVENT:
			...
			case CTRL_SHUTDOWN_EVENT: {
				// Отмечаем работу подлежащей остановке
				__awh_stop__.store(true);
				// Ожидаем завершения работы сервера, но не дольше отведённого системой срока
				for(uint16_t i = 0; (i < 100) && __awh_stop__.load(); i++)
...
typedef struct Params {
	...
	explicit Params() noexcept :
	 ...
	 net{"10.0.0.0"}, port(2222), prefix(24), iface{""}, driver{"auto"}, up(true), route(true) {}
} params_t;
...
int32_t main(int32_t argc, char * argv[]){
	...
	#if _WIN32 || _WIN64
		/**
		 * Устанавливаем обработчик управляющих событий консоли
		 *
		 * @note Без него сервер штатно не останавливался вовсе: закрытие окна консоли и
