...
/**
 * @brief Признак заказанного пересоздания устройства туннеля
 *
 * @details Заказывается пересоздание сроком, доводом запуска `--rebuild`, а не
 *          сигналом, как было прежде: сигнал `SIGHUP` есть у систем POSIX и у MS
 *          Windows его нет вовсе, отчего путь этот там не проверялся ни разу.
 *          Сроком же он проверяется у всякой системы одинаково
 *
 * @note Иначе путь пересоздания проверить нечем: при обычном запуске устройство
 *       заводится ровно один раз
 *
 */
static std::atomic_bool __awh_rebuild__{false};
...
typedef struct Params {
	...
	string driver;
	/**
	 * Срок до пересоздания устройства туннеля в секундах
	 *
	 * @note Нулевой срок означает, что пересоздания не заказано вовсе
	 */
	uint16_t rebuild;
	...
	explicit Params() noexcept :
	 ...
	 net{"10.0.0.0"}, port(2222), prefix(24), iface{""},
	 driver{"auto"}, rebuild(0), up(true), route(true) {}
} params_t;
...
static void usage(const char * name) noexcept {
	...
	cout << "  --rebuild <срок>  пересоздать устройство туннеля через столько секунд" << endl;
	cout << "                    (по умолчанию 0 - не пересоздавать)" << endl;
...
static bool parse(int32_t argc, char * argv[], params_t & params) noexcept {
	...
	for(int32_t i = 1; i < argc; i++){
		...
		// Иначе довод требует значения
			...
			else if(arg.compare("--rebuild") == 0) params.rebuild = static_cast <uint16_t> (::stoi(value));
...
int32_t main(int32_t argc, char * argv[]){
	...
	if(!parse(argc, argv, params))
		...
		return 1;
	/**
	 * @brief Отслеживание сигналов остановки средствами фреймворка
	 *
	 * @details Ставится оно единым обращением на всех системах: своей разницы у систем
	 *          здесь нет вовсе, и образцу знать о ней незачем - тем фреймворк и занят
	 *
	 * @note Из отклика движок не зовётся: отклик приходит не в потоке цикла событий.
	 *       Ставится один лишь признак, а разбирает его сам цикл
	 *
	 */
	signals_t signals;
	// Устанавливаем отклик на приход сигнала
	signals.on([]([[maybe_unused]] const int32_t signal) noexcept -> void {
	...
	if(io.initialize()){
		...
		if(io.setAddress(eid, netAddress, params.bind) && io.setAddress(tid, tunAddress, params.tun)){
			 ...
			 *
			 * @note Адрес называется и посреднику, и самому туннелю: без него движку
			 *       нечего задать другим концом точки-точки, и у IPv6 обратный путь
			 *       остаётся без маршрута вовсе
			 */
			if(io.setTarget(mid, params.peer) && io.setTarget(tid, params.peer)){
				...
				io.on(eid, [](const event::id_t eid, const event::status_t status) noexcept -> void {
					...
					switch(static_cast <uint8_t> (status)){
						...
						case static_cast <uint8_t> (event::status_t::ACCEPTED):
							...
							log::print("Событие принято: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус уничтожения
						case static_cast <uint8_t> (event::status_t::DESTROYED):
							// Записываем в лог сообщение об уничтожении события
							log::print("Событие подлежит уничтожению: ID=%u", log::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::RESUMED):
							...
							log::print("Событие возобновлено: ID=%u", log::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::SUCCESS):
							...
							log::print("Событие успешно выполнено: ID=%u", log::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::FAILURE):
							...
							log::print("Событие выполнено с ошибкой: ID=%u", log::flag_t::CRITICAL, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::PENDING):
							...
							log::print("Событие в ожидании: ID=%u", log::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::RECONNECTED):
							...
							log::print("Событие переподключено: ID=%u", log::flag_t::INFO, eid);
						break;
						...
						case static_cast <uint8_t> (event::status_t::LISTENING):
							...
							log::print("Событие прослушивается: ID=%u", log::flag_t::INFO, eid);
						break;
					}
				});
				// Устанавливаем функцию обратного вызова на событие туннеля
				io.on(tid, [](const event::id_t eid, const event::status_t status) noexcept -> void {
					/**
					 * Обрабатываем статус события
					 */
					switch(static_cast <uint8_t> (status)){
						...
						case static_cast <uint8_t> (event::status_t::ACCEPTED):
							// Записываем в лог сообщение о принятии события
							log::print("Событие туннеля принято: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус уничтожения
						case static_cast <uint8_t> (event::status_t::DESTROYED):
							// Записываем в лог сообщение об уничтожении события
							log::print("Событие туннеля подлежит уничтожению: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус инициализации
						case static_cast <uint8_t> (event::status_t::INITIAL):
							// Записываем в лог сообщение об инициализации события
							log::print("Событие туннеля инициализировано: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус запуска события
						case static_cast <uint8_t> (event::status_t::LAUNCHED):
							// Записываем в лог сообщение о запуске события
							log::print("Событие туннеля запущено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус паузы события
						case static_cast <uint8_t> (event::status_t::PAUSED):
							// Записываем в лог сообщение о паузе события
							log::print("Событие туннеля на паузе: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус возобновления события
						case static_cast <uint8_t> (event::status_t::RESUMED):
							// Записываем в лог сообщение о возобновлении события
							log::print("Событие туннеля возобновлено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус успешного выполнения события
						case static_cast <uint8_t> (event::status_t::SUCCESS):
							// Записываем в лог сообщение о успешном выполнении события
							log::print("Событие туннеля успешно выполнено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус неудачного выполнения события
						case static_cast <uint8_t> (event::status_t::FAILURE):
							// Записываем в лог сообщение о неудачном выполнении события
							log::print("Событие туннеля выполнено с ошибкой: ID=%u", log::flag_t::CRITICAL, eid);
						break;
						// Если статус выполнения события в ожидании
						case static_cast <uint8_t> (event::status_t::PENDING):
							// Записываем в лог сообщение о выполнении события в ожидании
							log::print("Событие туннеля в ожидании: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус подключения события
						case static_cast <uint8_t> (event::status_t::CONNECTED):
							// Записываем в лог сообщение о подключении события
							log::print("Событие туннеля подключено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус отмены события
						case static_cast <uint8_t> (event::status_t::CANCELLED):
							// Записываем в лог сообщение об отмене события
							log::print("Событие туннеля отменено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус переподключения события
						case static_cast <uint8_t> (event::status_t::RECONNECTED):
							// Записываем в лог сообщение о переподключении события
							log::print("Событие туннеля переподключено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус прослушивания события
						case static_cast <uint8_t> (event::status_t::LISTENING):
							// Записываем в лог сообщение о прослушивании события
							log::print("Событие туннеля прослушивается: ID=%u", log::flag_t::INFO, eid);
						break;
					}
				});
				// Устанавливаем функцию обратного вызова на событие посредника
				io.on(mid, [](const event::id_t eid, const event::status_t status) noexcept -> void {
					/**
					 * Обрабатываем статус события
					 */
					switch(static_cast <uint8_t> (status)){
						...
						case static_cast <uint8_t> (event::status_t::ACCEPTED):
							// Записываем в лог сообщение о принятии события
							log::print("Событие посредника принято: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус уничтожения
						case static_cast <uint8_t> (event::status_t::DESTROYED):
							// Записываем в лог сообщение об уничтожении события
							log::print("Событие посредника подлежит уничтожению: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус инициализации
						case static_cast <uint8_t> (event::status_t::INITIAL):
							// Записываем в лог сообщение об инициализации события
							log::print("Событие посредника инициализировано: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус запуска события
						case static_cast <uint8_t> (event::status_t::LAUNCHED):
							// Записываем в лог сообщение о запуске события
							log::print("Событие посредника запущено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус паузы события
						case static_cast <uint8_t> (event::status_t::PAUSED):
							// Записываем в лог сообщение о паузе события
							log::print("Событие посредника на паузе: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус возобновления события
						case static_cast <uint8_t> (event::status_t::RESUMED):
							// Записываем в лог сообщение о возобновлении события
							log::print("Событие посредника возобновлено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус успешного выполнения события
						case static_cast <uint8_t> (event::status_t::SUCCESS):
							// Записываем в лог сообщение о успешном выполнении события
							log::print("Событие посредника успешно выполнено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус неудачного выполнения события
						case static_cast <uint8_t> (event::status_t::FAILURE):
							// Записываем в лог сообщение о неудачном выполнении события
							log::print("Событие посредника выполнено с ошибкой: ID=%u", log::flag_t::CRITICAL, eid);
						break;
						// Если статус выполнения события в ожидании
						case static_cast <uint8_t> (event::status_t::PENDING):
							// Записываем в лог сообщение о выполнении события в ожидании
							log::print("Событие посредника в ожидании: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус подключения события
						case static_cast <uint8_t> (event::status_t::CONNECTED):
							// Записываем в лог сообщение о подключении события
							log::print("Событие посредника подключено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус отмены события
						case static_cast <uint8_t> (event::status_t::CANCELLED):
							// Записываем в лог сообщение об отмене события
							log::print("Событие посредника отменено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус переподключения события
						case static_cast <uint8_t> (event::status_t::RECONNECTED):
							// Записываем в лог сообщение о переподключении события
							log::print("Событие посредника переподключено: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если статус прослушивания события
						case static_cast <uint8_t> (event::status_t::LISTENING):
							// Записываем в лог сообщение о прослушивании события
							log::print("Событие посредника прослушивается: ID=%u", log::flag_t::INFO, eid);
						break;
					}
				});
				// Устанавливаем функцию обратного вызова на подключение нового клиента
				io.on(eid, static_cast <engine::callback::accept_t> ([tid, mid, &io](const event::id_t eid, const event::id_t cid) noexcept -> void {
					// Объединяем с пиром
					io.splice(mid, cid);
					// Записываем в лог сообщение о принятии события
					log::print("Событие принято: ID=%u, Клиентский ID=%u, IP=%s, PORT=%d", log::flag_t::INFO, eid, cid, io.getAddress(cid, event::address_t::IPV4).c_str(), io.getSourcePort(cid));
					...
					io.on(cid, static_cast <engine::callback::write_t> ([](const event::id_t eid, const size_t size) noexcept -> void {
						// Записываем в лог сообщение о записи данных
						log::print("Записано: ID=%u, %zu байт", log::flag_t::INFO, eid, size);
					}));
					// Устанавливаем функцию обратного вызова на чтение из события
					io.on(cid, [tid, &io](const event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
						// Текст входящего сообщения
						const string message(reinterpret_cast <const char *> (data), size);
						// Записываем в лог сообщение о чтении данных
						log::print("Прочитано: ID=%u, %zu байт", log::flag_t::INFO, eid, size);
						// Отправляем данные в туннель
						if(io.send(tid, reinterpret_cast <const char *> (data), size))
							// Если данные успешно отправлены
							log::print("Отправлено в туннель: ID=%u, %zu байт", log::flag_t::INFO, tid, size);
						// Если данные не отправлены
						else log::print("Ошибка отправки в туннель: ID=%u", log::flag_t::CRITICAL, tid);
					});
					// Устанавливаем функцию обратного вызова на ошибку события
					io.on(cid, [](const event::id_t eid, const event::error_t error, const string & description) noexcept -> void {
						/**
						 * Обрабатываем статус события
						 */
						switch(static_cast <uint8_t> (error)){
							...
							case static_cast <uint8_t> (event::error_t::UNKNOWN):
								...
								log::print("Неизвестная ошибка события: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
							break;
							...
							case static_cast <uint8_t> (event::error_t::INVALID):
								...
								log::print("Недопустимая операция события: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
							break;
							...
							case static_cast <uint8_t> (event::error_t::ACCESS_DENIED):
								...
								log::print("Доступ к событию запрещён: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
							break;
							...
							case static_cast <uint8_t> (event::error_t::ALREADY_EXISTS):
								...
								log::print("Объект события уже существует: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
							break;
							...
							case static_cast <uint8_t> (event::error_t::INVALID_SOCKET):
								...
								log::print("Ошибка доступа к сокету события: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
							break;
							...
							case static_cast <uint8_t> (event::error_t::INVALID_ADDRESS):
								...
								log::print("Некорректный адрес события: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
							break;
							// Если ошибка ошибки подключения
							case static_cast <uint8_t> (event::error_t::CONNECTION_FAIL):
								// Записываем ошибку в лог подключения
								log::print("Ошибка подключения события: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
							break;
							...
							case static_cast <uint8_t> (event::error_t::INSUFFICIENT_RES):
								...
								log::print("Недостаточно ресурсов для события: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
							break;
							// Если ошибка события
							case static_cast <uint8_t> (event::error_t::EVENT_FAIL):
								// Записываем ошибку в лог события
								log::print("Ошибка события: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
							break;
							// Если объект не найден
							case static_cast <uint8_t> (event::error_t::NOT_FOUND):
								// Записываем ошибку в лог события
								log::print("Объект события не найден: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
							break;
						}
					});
					...
					io.on(cid, [](const event::id_t eid, const event::action_t action) noexcept -> void {
						...
						switch(static_cast <uint8_t> (action)){
							...
							case static_cast <uint8_t> (event::action_t::DISCONNECT):
								...
								log::print("Событие на отключение: ID=%u", log::flag_t::INFO, eid);
							break;
							// Если действие является переподключением
							case static_cast <uint8_t> (event::action_t::RECONNECT):
								// Записываем в лог сообщение о переподключении события
								log::print("Событие на переподключение: ID=%u", log::flag_t::INFO, eid);
							break;
							...
							case static_cast <uint8_t> (event::action_t::CLOSE):
								...
								log::print("Событие на закрытие подключения: ID=%u", log::flag_t::INFO, eid);
							break;
							...
							case static_cast <uint8_t> (event::action_t::CHANGE):
								...
								log::print("Событие на изменение: ID=%u", log::flag_t::INFO, eid);
							break;
							...
							case static_cast <uint8_t> (event::action_t::DELETE):
								...
								log::print("Событие на удаление: ID=%u", log::flag_t::INFO, eid);
							break;
							...
							case static_cast <uint8_t> (event::action_t::RENAME):
								...
								log::print("Событие на переименование: ID=%u", log::flag_t::INFO, eid);
							break;
							...
							case static_cast <uint8_t> (event::action_t::ATTRIB):
								...
								log::print("Событие на изменение атрибутов: ID=%u", log::flag_t::INFO, eid);
							break;
							...
							case static_cast <uint8_t> (event::action_t::REVOKE):
								...
								log::print("Событие на отзыв доступа: ID=%u", log::flag_t::INFO, eid);
							break;
							...
							case static_cast <uint8_t> (event::action_t::HDLINK):
								...
								log::print("Событие на изменение счётчика жёстких ссылок: ID=%u", log::flag_t::INFO, eid);
							break;
						}
					});
				}));
				// Устанавливаем функцию обратного вызова на чтение из события посредника
				io.on(mid, [&io](const event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
					// Текст входящего сообщения
					const string message(reinterpret_cast <const char *> (data), size);
					// Записываем в лог сообщение о чтении данных
					log::print("Прочитано из посредника: ID=%u, %zu байт", log::flag_t::INFO, eid, size);
					// Отправляем данные обратно клиенту
					if(io.send(eid, reinterpret_cast <const char *> (data), size))
						// Если данные успешно отправлены
						log::print("Отправлено в сервер: ID=%u, %zu байт", log::flag_t::INFO, eid, size);
					// Если данные не отправлены
					else log::print("Ошибка отправки в сервер: ID=%u", log::flag_t::CRITICAL, eid);
				});
				...
				io.on(eid, [](const event::id_t eid, const event::error_t error, const string & description) noexcept -> void {
					...
					switch(static_cast <uint8_t> (error)){
						...
						case static_cast <uint8_t> (event::error_t::UNKNOWN):
							// Записываем ошибку в лог неизвестного события
							log::print("Неизвестная ошибка события сервера: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка недопустимой операции
						case static_cast <uint8_t> (event::error_t::INVALID):
							// Записываем ошибку в лог недопустимой операции
							log::print("Недопустимая операция события сервера: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка доступа запрещёния
						case static_cast <uint8_t> (event::error_t::ACCESS_DENIED):
							// Записываем ошибку в лог доступа запрещёния
							log::print("Доступ к событию сервера запрещён: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка уже существующего объекта
						case static_cast <uint8_t> (event::error_t::ALREADY_EXISTS):
							// Записываем ошибку в лог уже существующего объекта
							log::print("Объект события сервера уже существует: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка доступа к сокету
						case static_cast <uint8_t> (event::error_t::INVALID_SOCKET):
							// Записываем ошибку в лог доступа к сокету
							log::print("Ошибка доступа к сокету события сервера: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка некорректного адреса
						case static_cast <uint8_t> (event::error_t::INVALID_ADDRESS):
							// Записываем ошибку в лог некорректного адреса
							log::print("Некорректный адрес события сервера: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка ошибки подключения
						case static_cast <uint8_t> (event::error_t::CONNECTION_FAIL):
							// Записываем ошибку в лог подключения
							log::print("Ошибка подключения события сервера: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка недостаточно ресурсов
						case static_cast <uint8_t> (event::error_t::INSUFFICIENT_RES):
							// Записываем ошибку в лог недостаточно ресурсов
							log::print("Недостаточно ресурсов для события сервера: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка события
						case static_cast <uint8_t> (event::error_t::EVENT_FAIL):
							// Записываем ошибку в лог события
							log::print("Ошибка события сервера: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если объект не найден
						case static_cast <uint8_t> (event::error_t::NOT_FOUND):
							// Записываем ошибку в лог события
							log::print("Объект события сервера не найден: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
					}
				});
				// Устанавливаем функцию обратного вызова на ошибку события туннеля
				io.on(tid, [](const event::id_t eid, const event::error_t error, const string & description) noexcept -> void {
					/**
					 * Обрабатываем статус события
					 */
					switch(static_cast <uint8_t> (error)){
						...
						case static_cast <uint8_t> (event::error_t::UNKNOWN):
							// Записываем ошибку в лог неизвестного события
							log::print("Неизвестная ошибка события туннеля: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка недопустимой операции
						case static_cast <uint8_t> (event::error_t::INVALID):
							// Записываем ошибку в лог недопустимой операции
							log::print("Недопустимая операция события туннеля: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка доступа запрещёния
						case static_cast <uint8_t> (event::error_t::ACCESS_DENIED):
							// Записываем ошибку в лог доступа запрещёния
							log::print("Доступ к событию туннеля запрещён: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка уже существующего объекта
						case static_cast <uint8_t> (event::error_t::ALREADY_EXISTS):
							// Записываем ошибку в лог уже существующего объекта
							log::print("Объект события туннеля уже существует: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка доступа к сокету
						case static_cast <uint8_t> (event::error_t::INVALID_SOCKET):
							// Записываем ошибку в лог доступа к сокету
							log::print("Ошибка доступа к сокету события туннеля: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка некорректного адреса
						case static_cast <uint8_t> (event::error_t::INVALID_ADDRESS):
							// Записываем ошибку в лог некорректного адреса
							log::print("Некорректный адрес события туннеля: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка ошибки подключения
						case static_cast <uint8_t> (event::error_t::CONNECTION_FAIL):
							// Записываем ошибку в лог подключения
							log::print("Ошибка подключения события туннеля: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка недостаточно ресурсов
						case static_cast <uint8_t> (event::error_t::INSUFFICIENT_RES):
							// Записываем ошибку в лог недостаточно ресурсов
							log::print("Недостаточно ресурсов для события туннеля: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка события
						case static_cast <uint8_t> (event::error_t::EVENT_FAIL):
							// Записываем ошибку в лог события
							log::print("Ошибка события туннеля: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если объект не найден
						case static_cast <uint8_t> (event::error_t::NOT_FOUND):
							// Записываем ошибку в лог события
							log::print("Объект события туннеля не найден: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
					}
				});
				// Устанавливаем функцию обратного вызова на получение информации пакетов в туннеле
				io.on(tid, [&addr](const event::id_t tid, const event::id_t mid, const event::action_t action, const net::tun_info_t & info) noexcept -> void {
				...
				io.on(mid, [](const event::id_t eid, const event::error_t error, const string & description) noexcept -> void {
					/**
					 * Обрабатываем статус события
					 */
					switch(static_cast <uint8_t> (error)){
						...
						case static_cast <uint8_t> (event::error_t::UNKNOWN):
							// Записываем ошибку в лог неизвестного события
							log::print("Неизвестная ошибка события посредника: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка недопустимой операции
						case static_cast <uint8_t> (event::error_t::INVALID):
							// Записываем ошибку в лог недопустимой операции
							log::print("Недопустимая операция события посредника: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка доступа запрещёния
						case static_cast <uint8_t> (event::error_t::ACCESS_DENIED):
							// Записываем ошибку в лог доступа запрещёния
							log::print("Доступ к событию посредника запрещён: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка уже существующего объекта
						case static_cast <uint8_t> (event::error_t::ALREADY_EXISTS):
							// Записываем ошибку в лог уже существующего объекта
							log::print("Объект события посредника уже существует: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка доступа к сокету
						case static_cast <uint8_t> (event::error_t::INVALID_SOCKET):
							// Записываем ошибку в лог доступа к сокету
							log::print("Ошибка доступа к сокету события посредника: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка некорректного адреса
						case static_cast <uint8_t> (event::error_t::INVALID_ADDRESS):
							// Записываем ошибку в лог некорректного адреса
							log::print("Некорректный адрес события посредника: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка ошибки подключения
						case static_cast <uint8_t> (event::error_t::CONNECTION_FAIL):
							// Записываем ошибку в лог подключения
							log::print("Ошибка подключения события посредника: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка недостаточно ресурсов
						case static_cast <uint8_t> (event::error_t::INSUFFICIENT_RES):
							// Записываем ошибку в лог недостаточно ресурсов
							log::print("Недостаточно ресурсов для события посредника: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если ошибка события
						case static_cast <uint8_t> (event::error_t::EVENT_FAIL):
							// Записываем ошибку в лог события
							log::print("Ошибка события посредника: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
						// Если объект не найден
						case static_cast <uint8_t> (event::error_t::NOT_FOUND):
							// Записываем ошибку в лог события
							log::print("Объект события посредника не найден: ID=%u, Описание=%s", log::flag_t::CRITICAL, eid, description.c_str());
						break;
					}
				});
				...
				io.on(eid, [](const event::id_t eid, const event::action_t action) noexcept -> void {
					...
					switch(static_cast <uint8_t> (action)){
						...
						case static_cast <uint8_t> (event::action_t::READ):
							// Записываем в лог сообщение о чтении события
							log::print("Событие сервера на чтение: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является записью
						case static_cast <uint8_t> (event::action_t::WRITE):
							// Записываем в лог сообщение о записи события
							log::print("Событие сервера на запись: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является подключением
						case static_cast <uint8_t> (event::action_t::CONNECT):
							// Записываем в лог сообщение о подключении события
							log::print("Событие сервера на подключение: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является отключением
						case static_cast <uint8_t> (event::action_t::DISCONNECT):
							// Записываем в лог сообщение об отключении события
							log::print("Событие сервера на отключение: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является переподключением
						case static_cast <uint8_t> (event::action_t::RECONNECT):
							// Записываем в лог сообщение о переподключении события
							log::print("Событие сервера на переподключение: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является закрытием
						case static_cast <uint8_t> (event::action_t::CLOSE):
							// Записываем в лог сообщение о закрытии события
							log::print("Событие сервера на закрытие подключения: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является изменением
						case static_cast <uint8_t> (event::action_t::CHANGE):
							// Записываем в лог сообщение об изменении события
							log::print("Событие сервера на изменение: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является удалением
						case static_cast <uint8_t> (event::action_t::DELETE):
							// Записываем в лог сообщение об удалении события
							log::print("Событие сервера на удаление: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является переименованием
						case static_cast <uint8_t> (event::action_t::RENAME):
							// Записываем в лог сообщение о переименовании события
							log::print("Событие сервера на переименование: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является изменением атрибутов
						case static_cast <uint8_t> (event::action_t::ATTRIB):
							// Записываем в лог сообщение об изменении атрибутов события
							log::print("Событие сервера на изменение атрибутов: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является отзывом доступа
						case static_cast <uint8_t> (event::action_t::REVOKE):
							// Записываем в лог сообщение об отзыве доступа события
							log::print("Событие сервера на отзыв доступа: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является изменением счётчика жёстких ссылок
						case static_cast <uint8_t> (event::action_t::HDLINK):
							// Записываем в лог сообщение о изменении счётчика жёстких ссылок события
							log::print("Событие сервера на изменение счётчика жёстких ссылок: ID=%u", log::flag_t::INFO, eid);
						break;
					}
				});
				// Устанавливаем функцию обратного вызова на общее событие посредника
				io.on(mid, [](const event::id_t eid, const event::action_t action) noexcept -> void {
					/**
					 * Обрабатываем действие события
					 */
					switch(static_cast <uint8_t> (action)){
						...
						case static_cast <uint8_t> (event::action_t::READ):
							// Записываем в лог сообщение о чтении события
							log::print("Событие посредника на чтение: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является записью
						case static_cast <uint8_t> (event::action_t::WRITE):
							// Записываем в лог сообщение о записи события
							log::print("Событие посредника на запись: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является подключением
						case static_cast <uint8_t> (event::action_t::CONNECT):
							// Записываем в лог сообщение о подключении события
							log::print("Событие посредника на подключение: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является отключением
						case static_cast <uint8_t> (event::action_t::DISCONNECT):
							// Записываем в лог сообщение об отключении события
							log::print("Событие посредника на отключение: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является переподключением
						case static_cast <uint8_t> (event::action_t::RECONNECT):
							// Записываем в лог сообщение о переподключении события
							log::print("Событие посредника на переподключение: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является закрытием
						case static_cast <uint8_t> (event::action_t::CLOSE):
							// Записываем в лог сообщение о закрытии события
							log::print("Событие посредника на закрытие подключения: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является изменением
						case static_cast <uint8_t> (event::action_t::CHANGE):
							// Записываем в лог сообщение об изменении события
							log::print("Событие посредника на изменение: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является удалением
						case static_cast <uint8_t> (event::action_t::DELETE):
							// Записываем в лог сообщение об удалении события
							log::print("Событие посредника на удаление: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является переименованием
						case static_cast <uint8_t> (event::action_t::RENAME):
							// Записываем в лог сообщение о переименовании события
							log::print("Событие посредника на переименование: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является изменением атрибутов
						case static_cast <uint8_t> (event::action_t::ATTRIB):
							// Записываем в лог сообщение об изменении атрибутов события
							log::print("Событие посредника на изменение атрибутов: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является отзывом доступа
						case static_cast <uint8_t> (event::action_t::REVOKE):
							// Записываем в лог сообщение об отзыве доступа события
							log::print("Событие посредника на отзыв доступа: ID=%u", log::flag_t::INFO, eid);
						break;
						// Если действие является изменением счётчика жёстких ссылок
						case static_cast <uint8_t> (event::action_t::HDLINK):
							// Записываем в лог сообщение о изменении счётчика жёстких ссылок события
							log::print("Событие посредника на изменение счётчика жёстких ссылок: ID=%u", log::flag_t::INFO, eid);
						break;
					}
				});
				...
				if(io.commit(eid) && io.commit(tid) && io.commit(mid)){
					...
					if(io.launch(eid) && io.launch(tid)){
						...
						/**
						 * Приставку сети берём заказанную, а для IPv6 при умолчании - шестьдесят четыре
						 *
						 * @note Умолчание довода писано под IPv4: сеть IPv6 приставкой в двадцать четыре
						 *       не описывают вовсе, и маршрут вышел бы заведомо не тот
						 */
						route.prefix = (((params.prefix == 24) && (params.net.find(':') != string::npos)) ? 64 : params.prefix);
						...
						/**
						 * Срок, по истечении какого заказывается пересоздание устройства
						 *
						 * @note Отсчёт ведётся часами ровного хода: часы стенные вправе
						 *       переводиться, и перевод их сдвинул бы срок
						 */
						const std::chrono::steady_clock::time_point deadline = (
							std::chrono::steady_clock::now() + std::chrono::seconds(params.rebuild)
						);
						// Признак того, что пересоздание уже заказывалось
						bool ordered = false;
						/**
						...
						while(!__awh_stop__.load() && io.poll(250)){
							/**
							 * Если срок до пересоздания задан и вышел - заказываем пересоздание
							 *
							 * @note Заказывается оно единожды: срок отмеряет один заход, а не
							 *       череду их без конца
							 */
							if((params.rebuild > 0) && !ordered && (std::chrono::steady_clock::now() >= deadline)){
								// Отмечаем пересоздание заказанным
								ordered = true;
								// Отмечаем устройство подлежащим пересозданию
								__awh_rebuild__.store(true);
							}
							// Если пересоздание устройства не заказано
							if(!__awh_rebuild__.exchange(false))
								// Переходим к следующему обороту опроса
								continue;
							// Записываем в лог сообщение о заказанном пересоздании
							cout << " Заказано пересоздание устройства туннеля..." << endl;
							/**
							 * Пересоздаём устройство туннеля
							 *
							 * @note Ни фиксации настроек, ни запуска тут звать НЕ нужно:
							 *       пересоздание переигрывает обе стадии само, а повторный
							 *       запуск поверх уже запущенного отвечает отказом
							 */
							if(!io.rebuild(tid))
								// Записываем ошибку в лог пересоздания устройства
								cout << " Ошибка пересоздания устройства туннеля!" << endl;
							// Если устройство пересоздано
							else {
								// Получаем название заведённого устройства
								const string iface = io.getIface(tid);
								// Записываем в лог название заведённого устройства
								cout << " Устройство туннеля пересоздано: " << iface << endl;
								// Если устройство требуется поднять
								if(params.up && !iface.empty()){
									// Создаём объект работы с сетью
									eth_t eth;
									// Поднимаем устройство туннеля
