					...
					switch(static_cast <uint8_t> (state)){
						...
						case static_cast <uint8_t> (tls::coder_t::state_t::HANDSHAKED): {
							...
							if(tls.retransmit(id))
								...
								awh::log::print("Успешно выполнена повторная передача данных TLS: ID=%" PRIu64 "", awh::log::flag_t::INFO, id);
							// Записываем ошибку в лог повторной передачи данных TLS
							else awh::log::print("Ошибка повторной передачи данных TLS: ID=%" PRIu64 "", awh::log::flag_t::CRITICAL, id);
							*/
						} break;
					}
				});
