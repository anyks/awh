...
int32_t main(){
	...
	if(io.initialize()){
		...
		if(io.setAddress(eid, event::address_t::IPV4, "127.0.0.1")){
			...
			io.on(eid, static_cast <engine::callback::accept_t> ([cts, &tls, &io](const event::id_t eid, const event::id_t cid) noexcept -> void {
				// Записываем в лог сообщение о принятии события
					...
					switch(static_cast <uint8_t> (state)){
						...
						case static_cast <uint8_t> (tls::coder_t::state_t::HANDSHAKED): {
							...
							if(tls.retransmit(id))
								...
								log::print("Успешно выполнена повторная передача данных TLS: ID=%" PRIu64 "", log::flag_t::INFO, id);
							// Записываем ошибку в лог повторной передачи данных TLS
							else log::print("Ошибка повторной передачи данных TLS: ID=%" PRIu64 "", log::flag_t::CRITICAL, id);
							*/
						} break;
					}
				});
