 ...
 *
 * @brief Пример работы с сетевыми адресами — демонстрация разбора, нормализации и сравнения IPv4- и IPv6-адресов,
 *        вычисления масок и префиксов сети и определения принадлежности адреса зарезервированным диапазонам
 *
...
int32_t main(int32_t argc, char * argv[]){
	...
	/**
	 * Проверка спрашивает, является ли **хранимое** зеркалом IPv6 => IPv4, и обычный
	 * адрес IPv4 таковым не является: зеркало - это адрес IPv6 вида ::FFFF:a.b.c.d.
	 * Отрицательный ответ здесь ожидаем и верен, тогда как для ::ffff:192.0.2.1 выше
	 * та же проверка отвечает согласием
	 */
	cout << "46.39.230.51 BROADCAST IPv4 => IPv6 (обычный IPv4 зеркалом не является): " << addr.broadcastIPv6ToIPv4() << endl << endl;
	...
	cout << boolalpha;
	// Выполняем проверку маппинга адреса
	cout << "Выполняем проверку соответствия адреса 2001:1234:abcd:5678:9877:3322:5541:aabb хосту 53/::678:9877:3322:5541:AABB == " << addr.mapping("::678:9877:3322:5541:AABB", 53, net_addr_t::addr_t::HOST) << endl << endl;
	...
	cout << boolalpha;
	// Выполняем проверку маппинга адреса
	cout << "Выполняем проверку соответствия адреса 2001:1234:abcd:5678:9877:3322:5541:aabb хосту FFFF:FFFF:FFFF:F800::/::678:9877:3322:5541:AABB == " << addr.mapping("::678:9877:3322:5541:AABB", "FFFF:FFFF:FFFF:F800::", net_addr_t::addr_t::HOST) << endl << endl;
	 ...
	 * Выполняем тесты IP-адресов
	 *
	 * @note Локальным берётся адрес местного назначения (ULA) из блока FC00::/7, а не
	 *       петля [::1]: петля относится к зарезервированным, как и её сородич
	 *       127.0.0.1 у IPv4
	 */
	addr = "fd00::1";
	// Разрешаем вывод булевых переменных
	cout << boolalpha;
	// Выполняем проверку является ли IP-адрес локальным
	cout << "Выполняем проверку является ли IP-адрес локальным [fd00::1] == " << (addr.own() == net_addr_t::own_t::LAN) << endl << endl;
	...
	cout << boolalpha;
	// Выполняем проверку является ли IP-адрес зарезервированным
	cout << "Выполняем проверку является ли IP-адрес зарезервированным [::1] == " << (addr.own() == net_addr_t::own_t::SYS) << endl << endl;
