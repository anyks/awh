	...
	if(io.initialize()){
		...
		if(io.delivery(eid, event::delivery_mode_t::MULTICAST)){
			...
			if(io.hops(eid, event::family_t::IPV4, event::hops_t::NETWORK)){
				...
				if(io.options(eid, event::options::NO_SIGILL | event::options::NO_SIGPIPE | event::options::REUSE_ADDR | event::options::REUSE_PORT | awh::event::options::NO_IO_BLOCK | awh::event::options::CLOSEONEXEC | awh::event::options::MULTICAST_LOOPBACK))
