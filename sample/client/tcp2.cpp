 ...
 *
 * @brief Пример клиента TCP с внешним DNS-резолвером — демонстрация подключения к серверу по доменному имени с
		...
		void status(const event::status_t status, client_t * client) noexcept {
			...
			switch(static_cast <uint8_t> (status)){
				...
				case static_cast <uint8_t> (event::status_t::DESTROYED):
					...
					this->_log->print("TCP client destroyed", log_t::flag_t::INFO);
				break;
			}
		}
		/**
