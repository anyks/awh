 ...
 *
 * @brief Пример клиента Unix Domain Socket в датаграммном режиме —
 *        демонстрация обмена сообщениями с локальным сервером через файл сокета с сохранением границ пакетов
 *
...
int32_t main(){
	...
	[[maybe_unused]] const event::id_t eid = client.init(event::family_t::UDS, event::type_t::DATAGRAM);
