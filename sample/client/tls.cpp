 ...
 *
 * @brief Пример клиента TLS — демонстрация установки защищённого потокового соединения через фасад клиента,
 *        настройки контекста безопасности и обмена данными с сервером
 *
		...
		void status(const event::status_t status, client_t * client) noexcept {
			...
			switch(static_cast <uint8_t> (status)){
				...
				case static_cast <uint8_t> (event::status_t::DESTROYED):
					...
					awh::log::print("TLS client destroyed", awh::log::flag_t::INFO);
				break;
			}
		}
		/**
