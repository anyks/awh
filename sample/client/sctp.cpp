 ...
 *
 * @brief Пример клиента SCTP поверх TLS — демонстрация подключения в потоковом режиме с защитой транспортного уровня
 *        и разрешением имени сервера через DNS-резолвер
 *
		...
		void status(const event::status_t status, client_t * client) noexcept {
			...
			switch(static_cast <uint8_t> (status)){
				...
				case static_cast <uint8_t> (event::status_t::DESTROYED):
					...
					log::print("SCTP client destroyed", log::flag_t::INFO);
				break;
			}
		}
		/**
		...
		void state(const event::id_t eid, const event::status_t status, engine::sctp_t * sctp) noexcept {
			...
			switch(static_cast <uint8_t> (status)){
				...
				case static_cast <uint8_t> (event::status_t::REBIRTHED): {
					...
					log::print("SCTP client rebirthed: ID=%u", log::flag_t::INFO, eid);
