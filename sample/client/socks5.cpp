	...
	public:
		...
		void connect(const bool ok, client::socks5_t * client) noexcept {
			...
			if(ok){
				...
				else log::print("Sent data to remote server", log::flag_t::INFO);
...
int32_t main(){
	...
	client::socks5_t client(tls.transport(cts), &tls, &dns);
	...
	if(client.setTargetPort(2222) && client.setTarget("localhost") && client.endpoint(host, 443)){
		...
		client.on <void (const event::error_t, const string &)> ("error", &Executor::error, &executor, _1, _2, &client);
		...
		client.on <void (const tls::coder_t::error_t, const string &)> ("error_tls", &Executor::errorTLS, &executor, _1, _2, &client);
