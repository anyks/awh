 ...
 *
 * @brief Пример клиента DTLS с внешним DNS-резолвером — демонстрация подключения к серверу по доменному имени с
 *        предварительным разрешением адреса через собственный список DNS-серверов
 *
...
class Executor {
	...
	public:
		...
		/**
		 * @brief Метод обработки событий запуска клиента
		 *
...
int32_t main(){
	...
	if(client.setTarget("localhost") && client.setTargetPort(2222)){
		...
		client.on <void (const string &, const uint16_t)> ("launch", &Executor::launch, &executor, _1, _2, &client);
