 ...
 *
 * @brief Пример клиента Unix Domain Socket в потоковом режиме —
 *        демонстрация подключения к локальному серверу через файл сокета и обмена данными без участия сетевого стека
 *
...
class Executor {
	...
	public:
		...
		void status(const event::status_t status, client_t * client) noexcept {
			...
			switch(static_cast <uint8_t> (status)){
				...
				case static_cast <uint8_t> (event::status_t::DESTROYED):
					...
					this->_log->print("UDS client destroyed", log_t::flag_t::INFO);
				break;
			}
		}
		/**
		...
		void connect(const bool ok, client_t * client) noexcept {
			...
			if(ok){
				...
				if(client->send(request.c_str(), request.size()) == 0)
					...
					this->_log->print("Failed to send data to local server", log_t::flag_t::WARNING);
			// Если подключение не выполнено, то выводим сообщение об ошибке подключения клиента к локальному серверу
			} else this->_log->print("Failed to connect to local server", log_t::flag_t::WARNING);
		}
		/**
		...
		void ready([[maybe_unused]] const event::family_t family, const string & domain, const string & ip) noexcept {
			...
			this->_log->print("Client is ready to connect to local server: %s (%s)", log_t::flag_t::INFO, domain.c_str(), ip.c_str());
		}
		/**
...
int32_t main(){
	...
	[[maybe_unused]] const event::id_t eid = client.init(event::family_t::UDS, event::type_t::STREAM);
	...
	if(client.setTarget("/tmp/awh.sock")){
