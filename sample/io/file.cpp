 ...
 *
 * @brief Пример наблюдения за файлом средствами движка ввода-вывода —
 *        демонстрация подписки на события изменения содержимого, атрибутов и удаления отдельного файла
 *
...
int32_t main(){
	...
	if(io.initialize()){
		...
		io.on(eid, [&io](const event::id_t eid, const event::status_t status) noexcept -> void {
			/**
		...
		if(io.setAddress(eid, event::address_t::FS, "../README2.md")){
			...
			if(io.commit(eid)){
				...
				if(io.setOptions(eid, event::options::AUTO_FOLLOW))
