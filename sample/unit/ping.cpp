...
int32_t main(){
	...
	icmp.on <void (const unit::icmp_t::id_t, const unit::icmp_t::response_t &)> ("ping", [&addr, &chrono]([[maybe_unused]] const unit::icmp_t::id_t identifier, const unit::icmp_t::response_t & response) noexcept -> void {
		...
		// log::print("Ответ от %s: icmp_seq=%d time=%dms (ID: %d)", log::flag_t::INFO, static_cast <string> (addr).c_str(), sequence, elapsed, identifier);
		log::print("%zu bytes from %s: icmp_seq=%u ttl=%u time=%.1f %s", log::flag_t::INFO, response.size, static_cast <string> (addr).c_str(), response.sequence, response.timeToLive, abbr.second, label.c_str());
	...
	if(icmp.ping(icmp.issue(), 10, unit::icmp_t::mode_t::SYNC)){
		...
		if(icmp.init(event::family_t::IPV4)){
			...
			icmp.on <void (const event::id_t, const event::error_t, const string &)> ("error", [](const event::id_t, const event::error_t error, const string & description) noexcept -> void {
				...
				log::print("ICMP error: %s (code: %d)", log::flag_t::CRITICAL, description.c_str(), static_cast <uint16_t> (error));
			}, placeholders::_1, placeholders::_2, placeholders::_3);
			// Запускаем ICMP-клиент
			icmp.start();
		// Записываем ошибку в лог
		} else log::print("Не удалось запустить событие ICMP-клиента", log::flag_t::CRITICAL);
	}
