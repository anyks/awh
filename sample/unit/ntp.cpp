...
int32_t main(){
	...
	if(ntp.init(event::family_t::IPV4)){
		...
		ntp.on <void (const uint64_t)> ("timestamp", [&chrono](const uint64_t timestamp) noexcept -> void {
			...
			log::print("Получено дата от NTP-сервера: %s", log::flag_t::INFO, chrono.format(timestamp, "%H:%M:%S %d.%m.%Y").c_str());
		}, placeholders::_1);
		...
		ntp.on <void (const uint8_t)> ("attempts", [&ntp](const uint8_t attempts) noexcept -> void {
			...
			log::print("Количество попыток запроса времени к NTP-серверу attempts=%d", log::flag_t::WARNING, attempts);
		}, placeholders::_1);
		...
		ntp.on <void (const event::status_t)> ("status", [&ntp](const event::status_t status) noexcept -> void {
			...
			switch(static_cast <uint8_t> (status)){
				...
				case static_cast <uint8_t> (event::status_t::LAUNCHED): {
					...
					log::print("Событие NTP-клиента было запущено", log::flag_t::INFO);
					// Выполняем синхронизацию времени с NTP-сервером
					if(!ntp.sync(unit::ntp_t::version_t::V4))
						// Записываем ошибку в лог
						log::print("Не удалось выполнить синхронизацию времени с NTP-сервером", log::flag_t::CRITICAL);
				} break;
				// Если событие NTP-клиента остановлено
				case static_cast <uint8_t> (event::status_t::DESTROYED):
					// Записываем в лог сообщение об остановке события NTP-клиента
					log::print("Событие NTP-клиента было остановлено", log::flag_t::INFO);
				break;
			}
		}, placeholders::_1);
		...
		ntp.on <void (const event::id_t, const event::error_t, const string &)> ("error", [](const event::id_t, const event::error_t error, const string & description) noexcept -> void {
			...
			log::print("NTP error: %s (code: %d)", log::flag_t::CRITICAL, description.c_str(), static_cast <uint16_t> (error));
		}, placeholders::_1, placeholders::_2, placeholders::_3);
		// Запускаем NTP-клиент
		ntp.start();
	// Записываем ошибку в лог
	} else log::print("Не удалось запустить событие NTP-клиента", log::flag_t::CRITICAL);
