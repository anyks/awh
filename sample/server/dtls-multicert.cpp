	...
	public:
		...
		void read([[maybe_unused]] const event::id_t eid, const uint8_t * data, const size_t size, [[maybe_unused]] void * ctx, server_t * server) noexcept {
			...
			if(size > 0)
				...
				log::print("PID=%zu\n%s", log::flag_t::INFO, ::getpid(), string(reinterpret_cast <const char *> (data), size).c_str());
		...
		void status(const event::status_t status, server_t * server) noexcept {
			...
			switch(static_cast <uint8_t> (status)){
				...
				case static_cast <uint8_t> (event::status_t::LAUNCHED):
					...
					log::print("Server launched on port %d", log::flag_t::INFO, server->getPort());
				break;
		...
		void accept([[maybe_unused]] const event::id_t eid, const event::id_t cid, const tls::coder_t::id_t tid, server_t * server, tls::coder_t * tls) noexcept {
			...
			// Записываем в лог сообщение об успешном завершении рукопожатия TLS и выводим выбранный ALPN протокол
			log::print("TLS handshake completed: ID=%" PRIu64 ", ALPN protocol=%d", log::flag_t::INFO, tid, tls->alpn(tid));
		}
		/**
...
int32_t main(){
	...
	const tls::coder_t::id_t cts1 = tls.context(event::node_t::SERVER, event::protocol_t::UDP);
	// Создаём объект сервера
	server_t server(cts1, &tls, &dns);
	...
	const tls::coder_t::id_t cts2 = tls.context(event::node_t::SERVER, event::protocol_t::UDP);
