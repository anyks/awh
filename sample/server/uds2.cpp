...
class Executor {
	...
	public:
		 ...
		 *
		 */
		void status(const event::status_t status, [[maybe_unused]] server_t * server) noexcept {
			/**
			...
			switch(static_cast <uint8_t> (status)){
				...
				case static_cast <uint8_t> (event::status_t::LAUNCHED):
					// Записываем в лог сообщение об успешном прослушивании
					awh::log::print("Successfully listening", awh::log::flag_t::INFO);
				break;
...
int32_t main(){
	...
	const event::id_t eid = server.init(event::family_t::UDS, event::type_t::DATAGRAM);
