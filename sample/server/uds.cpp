 ...
 *
 * @brief Пример сервера Unix Domain Socket —
 *        демонстрация создания файла сокета через фасад сервера и обслуживания локальных подключений
 *
	...
	public:
			...
			switch(static_cast <uint8_t> (status)){
				...
				case static_cast <uint8_t> (event::status_t::LAUNCHED): {
					...
					if(!server->listen(100))
						...
						this->_log->print("Failed to listen", log_t::flag_t::WARNING);
					// Если подключение выполнено, то выводим сообщение об успешном прослушивании
					else this->_log->print("Successfully listening", log_t::flag_t::INFO);
				} break;
		...
		void accept([[maybe_unused]] const event::id_t eid, const event::id_t cid, const tls::coder_t::id_t tid, server_t * server) noexcept {
			...
			if(server->setOptions(cid, event::options::NO_SIGILL | event::options::NO_SIGPIPE | event::options::REUSE_ADDR | event::options::NO_IO_BLOCK | event::options::CLOSE_ON_EXEC | event::options::TCP_NO_DELAY | event::options::AUTO_RECONNECT))
				...
				cout << " Connection established: " << server->getAddress(cid, event::address_t::UDS) << endl;
		}
		/**
...
int32_t main(int32_t argc, char * argv[]){
	...
	const event::id_t eid = server.init(event::family_t::UDS, event::type_t::STREAM);
	// Устананавливаем опции события
	if(server.setOptions(eid, event::options::NO_SIGILL | event::options::NO_SIGPIPE | event::options::REUSE_ADDR | event::options::NO_IO_BLOCK | event::options::CLOSE_ON_EXEC | event::options::TCP_NO_DELAY))
		// Записываем в лог сообщение об успешной установке опций события
