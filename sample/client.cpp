...
int32_t main(int32_t argc, char * argv[]){
	...
	*/
	...
	// Устанавливаем тип ноды
	io.node(eid, event::node_t::CLIENT);
	// Устанавливаем порт события
	io.port(eid, 2222);
	// Инициализируем асинхронный движок ввода-вывода
	if(io.initialize()){
		// Устананавливаем опции события
		// if(io.options(eid, event::options::NOSIGILL | event::options::NOSIGPIPE | event::options::REUSEADDR | event::options::NOIOBLOCK | event::options::CLOSEONEXEC | event::options::TCPNODELAY | event::options::KEEPALIVE | event::options::ONSHOT))
