...
#
# Вызов:
#   benchmark/regex/stand.sh [корень дерева] [ключи программы замеров]
#
...
##
# Ключи программы замеров передаются ВТОРЫМ доводом и далее
#
# Первый довод - корень дерева, и передавать его программе нельзя. Ключи нужны
# машинам без порождения кода: там у обеих сторон работают толкователи, отдельные
# сценарии стоят эталону по 13 сопоставлений в секунду, и полный прогон на
# Эльбрусе идёт часами. Выборка «--filter=» сужает его до нужной группы
##
[ $# -gt 0 ] && shift
CXX=${CXX:-c++}
