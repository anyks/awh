...
namespace awh {
	...
	namespace benchmark {
		...
		namespace matching {
				...
				/**
				 * @brief Количество положений стека, при каких меряется всякий проход
				 *
				 * @details Скорость эталона зависит от положения стека процесса: PCRE2
				 *          держит на стеке буферы в десятки килобайтов, и при иных
				 *          положениях его машинный код и разбор программы проседают
				 *          втрое. Положение задаёт длина среды запуска, и прогоны
				 *          разных сеансов ловили эталон то в одном режиме, то в другом:
				 *          25.09.2026 машинный код эталона на «alternate-short» дал
				 *          107 миллионов совпадений в секунду днём и 53 вечером при том
				 *          же двоичном файле. Проходы оттого ведутся при нескольких
				 *          положениях стека, и принимается лучший - эталону всякий раз
				 *          достаётся лучший его режим, а сличение перестаёт зависеть
				 *          от среды запуска. Наша сторона меряется так же.
				 *
				 */
				static constexpr size_t SHIFTS = 4;

				/**
				 * @brief Шаг сдвига стека между положениями в октетах
				 *
				 * @details Режимы эталона повторяются с шагом в 256 октетов: машинный
				 *          код эталона на «literal-short» медлен при сдвиге 112-128
				 *          и 176-224 октетов и быстр при прочих. Из четырёх положений
				 *          через 64 октета при всяком начальном в медленные ложатся
				 *          одно-два, а в быстрые - два-три
				 *
				 */
				static constexpr size_t SHIFT = 64;
				 ...
				 *          измеренных ближе прочих к времени самой операции. Проходы
				 *          делятся поровну между положениями стека, по два на каждое
				 *
				 */
				static constexpr size_t ATTEMPTS = (SHIFTS * 2);
				...
				static const std::vector <scenario_t> SCENARIOS = {
					 ...
					 *          Ячейки состояния размещаются за ячейками захвата, и
					 *          проверка применимости прежде принимала их за выход
					 *          за пределы набора границ - отчего порождение кода
					 *          отменялось у всякого такого выражения. Набор устройства
					 *          этого не мерил вовсе, оттого изъян и жил незамеченным.
					 *
					 *          Пороги сняты правилом стенда: медленнейшими вышли FreeBSD
					 *          на первой строке (2245990) и OpenBSD на второй (3944120),
					 *          перевод отношением измеренным - 0.242 и 0.290 к показателю
					 *          стенда на машине рабочей, - запас в полтора раза.
					 *
					 *          Утраты порождения кода порог этот не ловит и ловить
					 *          не может: базою его стоит исполнение программы, каким
					 *          сборка сценария и ведётся намеренно. Возврат изъяна
					 *          ловится проверкой `Regex.CodegenProgress`, порождение
					 *          кода утверждающей, а доля кода к разбору выводится
					 *          строкою замера отдельно.
					 *
					 */
					{"region-empty-long",  "(?:[a-z]* ?)*dog",                        kind_t::LONG,  true, 360000.0},
					{"region-empty-lazy",  "(?:[a-z]* ?)*?dog",                       kind_t::LONG,  true, 760000.0},
