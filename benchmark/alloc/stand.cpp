...
/**
 * @brief Метод заведения модуля ядра
 *
 * @note Тело пустое намеренно: стенд собирается БЕЗ библиотеки, где заведение и живёт,
 *       а связывание требует определения. Из трёх дел заведения стенду нужно лишь одно -
 *       захват выдачи, - и его `main` делает сам, притом только у сборки с нашим
 *       распределителем: сборке с системным захват запрещён по устройству сличения.
 *       Предел снимка памяти и локаль на замеры не влияют
 *
 */
...
int main(){
	 ...
	 */
	/**
	 * Если стенд собран с соперником
	 *
	 * Имя соперника печатается ЯВНО: ярлык «системный» у столбца соперника однажды уже
	 * прятал, что выдачу обслуживал вовсе не он (COMPARISON.md, раздел «ОТЗЫВ»)
	 */
	#elif defined(AWH_BENCH_RIVAL)
		// Печатаем название соперника
		printf("распределитель: СОПЕРНИК %s (сборка из подмодуля AWH)\n\n", AWH_BENCH_RIVAL);
	#else
		// Печатаем название распределителя
