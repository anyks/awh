...
namespace {
	 ...
	 *          разброс между прогонами, доходящий на стендах до трети. Самым
	 *          медленным стендом всюду оказался OpenBSD amd64, отстающий от рабочей
	 *          машины вчетверо-впятеро; прогнаны также FreeBSD amd64, NetBSD amd64,
	 *          Fedora amd64 и Fedora ARM64
	 *
	 ...
	 *
	 */
	static constexpr double READ_ALLOCATIONS_THRESHOLD = 6.0;
	/**
	 ...
	 *
	 */
	static uint64_t reading(const string & text, const awh::codec::ini::reader_t::settings_t & settings) noexcept {
	...
	static uint64_t feed(const string & text) noexcept {
		// Объект потокового чтения текста настроек
		awh::codec::ini::reader_t reader(::logger());
		// Количество полученных событий разбора
		uint64_t result = 0;
