...
namespace {
	 ...
	 *       машины: подбор упирается в обратное чтение числа средствами библиотеки.
	 *       Стенды glibc дают 3.4-3.7 против 4.8-7.2 у musl и систем BSD - вдвое
	 *       меньше при вдвое более быстрой машине, - а меньше всех даёт `msvcrt` у MS
	 *       Windows: 2.03. Держит порог именно она, а не система: та же MS Windows с
	 *       `UCRT` даёт 16.85, в восемь раз больше. Порог, снятый на рабочей машине,
	 *       здесь оказывался ЗАВЫШЕН вчетверо
	 *
	 */
	static constexpr double WRITE_NUMBER_THRESHOLD = 1.00;
	/**
