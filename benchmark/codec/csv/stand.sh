...
#          машине: порог, снятый на быстрой, отказывает на медленной без всякой
#          регрессии. Полная же сборка библиотеки на стендах занимает десятки минут и
#          тянет за собою третью сторону, тогда как замерить требуется один кодек
#
# @warning Прежняя редакция строки этой велела равняться на «самую медленную машину».
#          Правило это НЕВЕРНО, и правится оно здесь по замерам (05.09.2026): одной
#          самой медленной машины не существует - дно у разных сценариев приходится на
#          разные машины, и порог всякого сценария берётся у своего дна. Пороги-отношения
#          от машины зависят тем более: отношение сокращает машину лишь настолько,
#          насколько обеим долям она одинаково дорога
#
...
#
# @note Часть «replace» изъята 15.09.2026: модуль «codec/replace» удалён как
