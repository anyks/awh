...
namespace {
	 ...
	 *       текста целиком, а не пропускная способность сама по себе
	 *
	 * @warning Порог этот выставлен ОЖИДАНИЕМ, а не замером по машинам, и в том его
	 *          отличие от однородных ему порогов разметки и документа: те сняты по семи
	 *          отладочным стендам (наихудшее 1.51 и 1.75 у netbsd при порогах 3.0 и 3.5),
	 *          а этот - ни разу
	 *
	 * @warning Довод, здесь прежде стоявший, - «отношение двух прогонов на одной машине
	 *          от быстродействия её не зависит, и порог ему можно назначить впритык», -
	 *          НЕВЕРЕН как общее правило. Отношение сокращает машину лишь настолько,
	 *          насколько обеим его долям машина одинаково дорога, а здесь доли разнородны:
	 *          подача кусками заводит перекладывание недочитанного, то есть работу с
	 *          памятью, а подача целиком её почти не заводит. Замер соседа по кодеку CEF
	 *          05.09.2026: такая же мера держалась 2.56-2.93 на трёх машинах и дала 8.71
	 *          на OpenBSD - раздатчик памяти там дорог и вошёл в одну долю сильнее, чем в
	 *          другую. Проверять такие меры надлежит машиной с ИНЫМ раздатчиком, а не
	 *          несколькими похожими
	 *
	 */
	 ...
	 *
	 * @warning Порог выставлен ОЖИДАНИЕМ, а не замером по машинам, и доли его разнородны
	 *          тем же образом, что и у порога подачи кусками: откладывание выдачи заводит
	 *          работу с памятью, а разбор без него - нет. Довод общий смотри при том пороге
	 *
	 */
