...
namespace {
	...
	/**
	 * @note ПОРОГИ РАЗМЕЧЕНЫ ПО ДНУ КАЖДОГО СЦЕНАРИЯ 21.08.2026: раскладка по тринадцати
	 *       стендам (macOS, FreeBSD, NetBSD, OpenBSD, Solaris, OpenIndiana, DragonFly,
	 *       Debian, Alpine, Alt, Ubuntu, OpenSuse, Fedora) тремя прогонами отдельными
	 *       процессами, показатель берётся серединою трёх
	 *
	 * @note Дно всякого сценария скорости взяла ОДНА машина - OpenBSD, и просадка её
	 *       архитектурная, а не свойство кодека. Дно расхода выделений памяти берут
	 *       иные машины, и расходятся они на единицу: 26 против 27 на сборке
	 *
	 */
	/**
	 ...
	 *
	 * @note Сценарий этот - САМЫЙ ЧУВСТВИТЕЛЬНЫЙ к машине из всего набора: между дном
	 *       и рабочей машиной шестикратный разрыв (OpenBSD 9.8 МБ/с против 64.0), тогда
	 *       как прочие сценарии расходятся вчетверо-впятеро. Порог, снятый по рабочей
	 *       машине, отвергал бы работающий кодек на СЕМИ стендах из тринадцати
	 *
	 * @note Снято дважды и порознь: по тринадцати стендам дно 9.8 МБ/с у OpenBSD, по
	 *       двенадцати - 10.38 МБ/с у неё же. Числа сошлись, и разница есть разброс
	 *       между обходами. Порог взят вдвое ниже худшего из двух, как то заведено по
	 *       всему набору: запас в треть, каким порог стоял прежде, отказал бы при
	 *       просадке машины на четверть - а машина между обходами гуляет на шестую часть
	 *
	 */
	static constexpr double BUILD_SERVICE_THRESHOLD = 5.19;
	/**
