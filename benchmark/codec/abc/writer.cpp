...
namespace {
	 ...
	 *
	 * @note Порог ослаблен с 4.0 до 5.5 23.08.2026, когда к карте стендов прибавился
	 *       MS Windows на x86-64 (MSYS2 MINGW64, 8 ГБ, четыре ядра). Показатель ходит
	 *       там через прежний порог от прогона к прогону: 3.52, 3.53, 4.19, 4.64 -
	 *       дно 4.64 при одиночном прогоне, без соседей по машине. Разброс этот
	 *       свойство машины, а не кодека: прочие двенадцать стендов держатся ниже
	 *       четырёх, и ни один сценарий сборки там не сдвинулся
	 *
	 */
	static constexpr double WRITE_SERVICE_LATENCY_THRESHOLD = 5.5;
	/**
