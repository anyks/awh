...
namespace {
	 ...
	 *
	 */
	static constexpr double TREE_SERVICE_THRESHOLD = 4.4;
	/**
	 ...
	 *
	 */
	static constexpr double TREE_ANCHORS_THRESHOLD = 3.15;
	/**
	 * @brief Порог пропускной способности перезаписи собранного дерева
	 *
	 */
	static constexpr double TREE_REWRITE_THRESHOLD = 12.94;
	/**
	 ...
	 *
	 */
	static constexpr double TREE_VERBATIM_THRESHOLD = 386.27;
	/**
	 ...
	 *
	 * @note Величина назначена по съёму 21.08.2026: двенадцать машин, по три прогона
	 *       ОТДЕЛЬНЫМИ процессами, 36 образцов. Худшая надбавка 1.04 у ALT Linux -
	 *       перезапись стоит ровно как копия, что устройство и обещает, - и порог взят
	 *       вдвое выше неё. Держится он этим запасом, а не переносимостью отношения
	 *
	 * @note Прежний съём вёлся по ОДНОМУ прогону на машину с семи машин и давал разброс
	 *       0.97...1.07; настоящий, по трём отдельным процессам с двенадцати, дал
	 *       0.93...1.04. Опасение, что одиночный образец прячет худший прогон,
	 *       подтвердилось лишь на сотые доли: показатель этот оказался устойчивее, чем
	 *       ожидалось, и порог опущен втрое - с 3.00 до 2.08
	 *
	 */
	static constexpr double TREE_OVERHEAD_THRESHOLD = 2.08;
	/**
	 ...
	 *
	 * @note Величина назначена по съёму 21.08.2026: двенадцать машин, по три прогона
	 *       отдельными процессами. Худшее отношение 0.19 у OpenIndiana, и порог взят
	 *       вдвое выше него. Держится он этим запасом, а не переносимостью отношения -
	 *       смотри оговорку у соседнего порога
	 *
	 */
	static constexpr double TREE_PATCHED_THRESHOLD = 0.38;
	/**
	 ...
	 *
	 */
	static constexpr double TREE_EDIT_LATENCY_THRESHOLD = 216.94;
	/**
