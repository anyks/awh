...
BENCH="yaml reader writer document value"
CODEC_PRESENT=$(ls "$ROOT/src/codec/yaml"/*.cpp | while read -r FILE; do basename "$FILE" .cpp; done | sort | tr '\n' ' ')
CODEC_LISTED=$(for PART in $PARTS; do echo "$PART"; done | sort | tr '\n' ' ')
BENCH_PRESENT=$(ls "$ROOT/benchmark/codec/yaml"/*.cpp | while read -r FILE; do basename "$FILE" .cpp; done | sort | tr '\n' ' ')
...
#
# Части эти лежат в «src/codec» и делятся между кодеками: «numeric.cpp» несёт приведение
# чисел, вынесенное из шести кодеков 01.09.2026, а «replace.cpp» - переносимую подмену
# целевого файла временным, вынесенную 07.09.2026. Перенос тела подмены из заголовка в
# исходник оборвал связывание сразу четырёх видов сборки, ведущих списки вручную:
# стендов проверок, ворошителей, стендов замеров и стендов чужих кодеков
#
# @note Перечень держится вручную по тому же доводу, что и перечень частей кодека:
#       маска подхватила бы «bridge.cpp» - перекладку между кодеками, зовущую все семь
##
SHARED="numeric replace"
...
#       повтором имён
$COMPILER $OPTIONS $OBJECTS -pthread $SYSTEM_LIBS -lz -o "$OUTPUT/yaml-bench"
