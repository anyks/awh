...
namespace {
	...
	/**
	 * @warning ПОРОГИ ЗДЕСЬ ВРЕМЕННЫЕ И НЕ СТОРОЖАТ НИЧЕГО. Сняты они по одной рабочей
	 *          машине и назначены с запасом, взятым на глаз: у обхода владеющего
	 *          значения он доходит до сорока крат, и просадку вдесятеро такой порог
	 *          пропустит молча. Разметить их надлежит по дну КАЖДОГО СЦЕНАРИЯ, снятому
	 *          раскладкой по двенадцати стендам тремя прогонами отдельными процессами, -
	 *          работа отложенная, а не отменённая: стенды заняты соседним прогоном
	 *
	 * @warning Числа рабочей машины (Release, libc++, 20.08.2026) для будущей разметки:
	 *          снятие с дерева 57, снятие настроек службы 136, потоковая сборка 1416,
	 *          запись в текст 256, обход 6006 МБ/с, выделения на снятие 139 на документ
	 *
	 */
	/**
