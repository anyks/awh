...
namespace {
	 ...
	 *          по устройству выносится за пределы кодека. Порог привязан к ASSEMBLY_COUNT: изменив
	 *          его, пороговое число надлежит переснять
	 *
	 */
	static constexpr double ASSEMBLY_THRESHOLD = 2786.89;
	/**
	 ...
	 *
	 */
	static constexpr double LOOKUP_THRESHOLD = 2793.05;
	...
	static awh::benchmark::result_t assembly() noexcept {
		...
		const outcome_t outcome = measure(0, 1, []() noexcept {
			...
			for(size_t i = 0; i < ASSEMBLY_COUNT; i++){
				...
				value["поле" + to_string(i)] = awh::codec::toml::value_t(static_cast <int64_t> (i));
	...
	static const bool ASSEMBLY_REGISTERED = awh::benchmark::add(
		"codec/toml: сборка значения вызовами", "уст./с", ASSEMBLY_THRESHOLD,
	...
	static const bool LOOKUP_REGISTERED = awh::benchmark::add(
		"codec/toml: поиск значения по имени", "обр./с", LOOKUP_THRESHOLD,
