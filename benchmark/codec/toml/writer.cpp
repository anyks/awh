...
namespace {
	...
	/**
	 * @brief Порог количества выделений памяти на собранный файл настроек
	 *
	 * @details Запись ведёт сборку на одном хранилище собираемого текста, и
	 *          количество выделений на файл определяется его ростом, а не числом
	 *          записанных пар: рост показателя означает, что какое-то хранилище
	 *          заводится заново на каждую запись
	 *
	 * @note Счётчик замера ловит всякое обращение к malloc, а не только выделения
	 *       самой записи, и оттого зависит от устройства стандартной библиотеки
	 *       системы. Запись дробного числа ведётся подбором кратчайшей точности,
	 *       а всякая проба идёт через snprintf: у libc NetBSD преобразование числа
	 *       выделяет память внутри себя, и одно дробное значение обходится в 15
	 *       обращений против нуля у macOS и FreeBSD. Замерено пробой на стенде
	 *       NetBSD 10.1/amd64 (13.08.2026): дробное 15.09 на значение, целое,
	 *       логическое и строковое - ноль; отсюда 970 на файл против 9. Порог взят
	 *       с двукратным запасом к самому расточительному из известных стендов -
	 *       наравне с задержкой чтения, где такой же запас брался по FreeBSD
	 *
	 */
	static constexpr double WRITE_ALLOCATIONS_THRESHOLD = 2000.0;
