...
namespace {
	 ...
	 */
	/**
	 * @note Запас взят двукратным к самому медленному из известных стендов: на MSYS2
	 *       MinGW64 сценарий даёт 6.1-7.0 МБ/с против 66.20 у рабочей машины
	 *       (13.08.2026). Просадка эта принадлежит машине: проба, строящая то же
	 *       дерево вызовом разбора напрямую, даёт там же отношение к кодеку INI
	 *       2.5 раза - ровно как и на рабочей машине, где оно 2.9. Первый прогон
	 *       выдал 4.91 и порога не удержал, но три следующих на незанятой машине
	 *       уложились: сценарий этот чуток к посторонней нагрузке
	 *
	 */
	 ...
	 *
	 * @note Запас взят двукратным к самому медленному из известных стендов: на
	 *       DragonFly 6.4 сценарий даёт 3.27 МБ/с против 30.54 у рабочей машины
	 *       (13.08.2026), и прежний порог держался лишь на девяти сотых
	 *
	 */
	static constexpr double TREE_TABLES_THRESHOLD = 1.5;
	/**
