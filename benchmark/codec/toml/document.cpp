...
namespace {
	 ...
	 *
	 * @details Пороги размечены по дну КАЖДОГО СЦЕНАРИЯ, снятому раскладкой по
	 *          двенадцати стендам 21.08.2026 тремя прогонами отдельными процессами, с
	 *          двукратным запасом от него. Дно берут разные машины: сборку дерева
	 *          крупного файла и множества таблиц держит DragonFly, прочее - OpenBSD
	 *
	 */
	/**
	 ...
	 *
	 */
	static constexpr double TREE_SERVICE_THRESHOLD = 3.2;
	/**
	 ...
	 *
	 */
	static constexpr double TREE_LARGE_THRESHOLD = 1.3;
	/**
	 ...
	 *
	 */
	static constexpr double TREE_TABLES_THRESHOLD = 1.3;
	/**
	 * @brief Порог пропускной способности перезаписи дерева настроек
	 *
	 * @details Дно 31.76 МБ/с держит OpenBSD
	 *
	 */
	static constexpr double TREE_REWRITE_THRESHOLD = 15.0;
	/**
	 ...
	 *
	 * @note Дно 1.73 мкс держит OpenBSD, и правило дало бы порог 3.4 - но порог оставлен
	 *       прежним, 4.0, с запасом 2.3 крат. Снят показатель по ОДИННАДЦАТИ машинам:
	 *       число NetBSD пришлось выбросить, ибо снято оно было с занятой машины и дало
	 *       11.07, тогда как повтор на тихой той же машине дал 1.16 / 0.99 / 0.87.
	 *       Ужесточать порог по неполному съёму не стал
	 *
	 */
	 ...
	 */
	static constexpr double TREE_LASTING_THRESHOLD = 3.6;
