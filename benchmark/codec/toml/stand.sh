...
#
# @brief Отдельный стенд замеров кодека TOML — сборка набора замеров без библиотеки
...
#
# Вызов:
#   benchmark/codec/toml/stand.sh [корень дерева] [каталог сборки]
#
...
rm -f "$OUTPUT/toml-bench" "$OUTPUT/toml-bench.exe"
...
echo "Собираем стенд замеров TOML: $COMPILER"
...
BENCH="toml reader writer document value"
CODEC_PRESENT=$(ls "$ROOT/src/codec/toml"/*.cpp | while read -r FILE; do basename "$FILE" .cpp; done | sort | tr '\n' ' ')
CODEC_LISTED=$(for PART in $PARTS; do echo "$PART"; done | sort | tr '\n' ' ')
BENCH_PRESENT=$(ls "$ROOT/benchmark/codec/toml"/*.cpp | while read -r FILE; do basename "$FILE" .cpp; done | sort | tr '\n' ' ')
...
#       повтором имён
$COMPILER $OPTIONS $OBJECTS -pthread $SYSTEM_LIBS -lz -o "$OUTPUT/toml-bench"

# Выводим сообщение об окончании сборки стенда
echo "Стенд собран: $OUTPUT/toml-bench"
