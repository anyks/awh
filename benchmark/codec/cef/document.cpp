...
namespace {
	 ...
	 *          Порог выставлен ОЖИДАНИЕМ ниже на порядок, а не дном по стендам
	 *
	 */
	constexpr double PARSE_DETECTION_THRESHOLD = 5.0;
	 ...
	 *          на macOS ARM64 в Release: 38 МБ/с. Порог выставлен ОЖИДАНИЕМ
	 *
	 */
	constexpr double PARSE_AUDIT_THRESHOLD = 5.0;
	 ...
	 *          macOS ARM64 в Release: 1.6-1.7 мкс на запись. Порог выставлен ОЖИДАНИЕМ
	 *
	 ...
	 *
	 * @details Мера: время прогона, делённое на число кругов. Замерено 04.09.2026 на
	 *          macOS ARM64 в Release: 15.5-16.5 мкс на запись. Порог выставлен
	 *          ОЖИДАНИЕМ на порядок выше, а не потолком по стендам
	 *
	 */
