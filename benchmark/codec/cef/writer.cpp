...
namespace {
	 ...
	 *          время прогона. Замерено 04.09.2026 на macOS ARM64 в Release: 140-167
	 *          МБ/с, 05.09.2026 на FreeBSD amd64: 41.19 МБ/с, на Debian 12 amd64: 70.66
	 *          МБ/с. Порог взят по ДНУ трёх проверенных на тот час машин с запасом вчетверо
	 *
	 */
	 ...
	 *
	 * @details Мера: длина эталонной записи, помноженная на число кругов, делённая на
	 *          время прогона. Замерено 04.09.2026 на macOS ARM64 в Release: 139-159
	 *          МБ/с, 05.09.2026 на FreeBSD amd64: 40.94 МБ/с, на Debian 12 amd64: 72.23
	 *          МБ/с. Порог взят по ДНУ трёх проверенных на тот час машин с запасом вчетверо
	 *
	 */
	 ...
	 *          запись, 05.09.2026 на FreeBSD amd64, Debian 12 (libstdc++) и Alpine
	 *          (musl): 3 - число в число, стандартной библиотеке вопреки: сборка строк
	 *          коротких не заводит вовсе, оттого запас её и не задевает.
	 *
	 * @warning У NetBSD 10.1 то же самое стоит 119 выделений (замерено 06.09.2026), и
	 *          это НЕ свойство кодека: там сама libc выделяет память внутри обращения
	 *          чисел в текст, около пятнадцати выделений на преобразование. Писатель
	 *          обращает в текст редакцию, важность и числовые значения - отсюда и
	 *          сорокакратная разница при одном и том же коде. Порог оттого и держится
	 *          широким: запас его на NetBSD вышел всего в 1.7 раза, и сужение порога по
	 *          машинам с дешёвым форматированием дало бы там ложную тревогу Запас держится широким, покуда стенд на libstdc++ не пройден:
