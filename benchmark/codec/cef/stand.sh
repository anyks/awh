...
#
# @brief Отдельный стенд замеров кодека CEF — сборка набора замеров без библиотеки
...
#
# Вызов:
#   benchmark/codec/cef/stand.sh [корень дерева] [каталог сборки]
#
...
rm -f "$OUTPUT/cef-bench" "$OUTPUT/cef-bench.exe"

# Собираем перечень объектных файлов стенда
OBJECTS="$OUTPUT/lexical-table.o $OUTPUT/main.o $OUTPUT/sys-log.o $OUTPUT/sys-fs.o $OUTPUT/sys-os.o $OUTPUT/sys-chrono.o $OUTPUT/sys-fmk.o $OUTPUT/net-nwt.o $OUTPUT/uni-normalize.o $OUTPUT/uni-table.o $OUTPUT/uni-unicode.o $OUTPUT/uni-utf8.o $OUTPUT/alloc-alloc.o $OUTPUT/alloc-cache.o $OUTPUT/alloc-central.o $OUTPUT/alloc-classes.o $OUTPUT/alloc-guard.o $OUTPUT/alloc-huge.o $OUTPUT/alloc-link.o $OUTPUT/alloc-pages.o $OUTPUT/alloc-profile.o $OUTPUT/alloc-source.o $OUTPUT/alloc-spin.o $OUTPUT/alloc-trace.o $OUTPUT/alloc-vessel.o $OUTPUT/alloc-elf.o $OUTPUT/alloc-mach.o $OUTPUT/alloc-pe.o $OUTPUT/charset.o $OUTPUT/charset-table.o"
...
case "$(uname -s)" in
	...
	#
	# @warning Системам Sun нужны «libsocket» и «libnsl» отдельно: `if_indextoname` и
	#          `if_nametoindex`, какие зовёт «src/net/addr.cpp», живут там, а не в libc.
...
echo "Собираем стенд замеров CEF: $COMPILER"
...
$COMPILER $OPTIONS -Wno-c++11-narrowing -c "$ROOT/src/alloc/vessel.cpp" -o "$OUTPUT/alloc-vessel.o"
...
#
# Собираем опорные части «abc» вместе с замыканием их
#
# @details Дерево записи журнала кодек держит на «abc::value_t», своего значения не
#          заводя, а ход `absorb` тянет за собою значение документа «abc». Перечень
#          восьми частей ниже и есть наименьшее замыкание, связывающееся БЕЗ третьей
#          стороны: прочие части «abc» тянут шифрование и сжатие. То же самое замыкание
#          несёт и стенд проверок, «tests/codec/cef/stand.sh»
#
...
#
# Собираем части работы с адресами, каких требует кодек CEF
#
# @details Сличение значения ключа расширения со словарём разбирает адреса: ключ,
#          адресом объявленный, а адресом не являющийся, отвечается отказом. Тянет это
#          «src/net/addr.cpp» и «src/net/net.cpp», третьей стороны в них нет
#
for PART in "$ROOT/src/net/addr.cpp" "$ROOT/src/net/net.cpp"; do
	# Выполняем сборку очередной части работы с адресами
	$COMPILER $OPTIONS -c "$PART" -o "$OUTPUT/net-$(basename "$PART" .cpp).o"
	# Добавляем собранное к перечню объектных файлов стенда
...
# Выполняем перебор всех частей набора замеров кодека CEF
for PART in cef reader writer document; do
	# Выполняем сборку очередной части набора замеров
	$COMPILER $OPTIONS -c "$ROOT/benchmark/codec/cef/$PART.cpp" -o "$OUTPUT/bench-$PART.o"
	# Добавляем собранное к перечню объектных файлов стенда
...
#       повтором имён
$COMPILER $OPTIONS $OBJECTS -pthread $SYSTEM_LIBS -lz -o "$OUTPUT/cef-bench"

# Выводим сообщение об окончании сборки стенда
echo "Стенд собран: $OUTPUT/cef-bench"
