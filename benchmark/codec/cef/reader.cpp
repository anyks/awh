 ...
 *        выделений памяти
 *
 * @note Пороги, здесь выставленные, проверены НА ДЕСЯТИ МАШИНАХ к 06.09.2026 - macOS,
 *       FreeBSD, OpenBSD, Debian (glibc), Alpine (musl), NetBSD, Solaris, OpenIndiana,
 *       DragonFly и Windows (MinGW). Пять стендов, добавленных вслед за первыми
 *       четырьмя, дна ни у одного сценария не сдвинули - числа их легли серединою
 *       ряда, - а Windows оказалась самой медленной из всех и всё же уложилась во
 *       всякий порог. Сводная таблица по всем десяти - в README.md рядом
 *
...
namespace {
	 ...
	 *          МБ/с, 05.09.2026 на FreeBSD amd64: 33.25 МБ/с, на Debian 12 amd64: 58.74
	 *          МБ/с, на OpenBSD 7.9 amd64: 27.82 МБ/с. Порог взят по ДНУ четырёх
	 *          проверенных на тот час машин с запасом
	 *
	 */
	 ...
	 *          МБ/с, 05.09.2026 на FreeBSD amd64: 30.28 МБ/с, на Debian 12 amd64: 51.38
	 *          МБ/с, на OpenBSD 7.9 amd64: 26.65 МБ/с. Порог взят по ДНУ четырёх
	 *          проверенных на тот час машин с запасом
	 *
	 */
	 ...
	 *          записей в секунду, FreeBSD amd64 (clang 18): 21 480-21 519, Debian 12
	 *          amd64 (g++ 12.2, glibc): 12 530, OpenBSD 7.9 amd64: 24 838. Порог взят по
	 *          ДНУ четырёх проверенных на тот час машин с запасом втрое
	 *
	 * @note Дно здесь у Debian, а НЕ у OpenBSD: последняя, медленнейшая на прочих
	 *       сценариях, поток записей разбирает вдвое быстрее Debian. Машина
	 *       медленнейшая вообще - выдумка: у всякого сценария она своя
	 *
	 * @warning Прежнее его значение, 20 000, выставленное ожиданием, дна FreeBSD не
	 *          выдерживало вовсе: запас выходил семь процентов, и машина медленнее
	 *          валила бы сценарий при исправном модуле. Замерено 05.09.2026 - первым
	 *          же прогоном по стендам. Показатель этот от машины ЗАВИСИТ сильнее
	 *          прочих: разница ARM64 и amd64 здесь двукратна
	 *
	 */
	constexpr double READ_EVENTS_THRESHOLD = 4000.0;
	 ...
	 *          на macOS ARM64 в Release: 2.79-2.93 раза, на FreeBSD amd64: 2.74 раза, на
	 *          Debian 12 amd64: 2.56 раза, на OpenBSD 7.9 amd64: 8.71 раза
	 *
	 * @warning Мера отношением от машины ЗАВИСИТ, вопреки первому впечатлению. Три
	 *          машины уложились в тринадцать процентов, и по ним выходило, что
	 *          отношение машину сокращает; четвёртая дала ВТРОЕ больше. Причина в том,
	 *          что подача кусками работу не просто дробит, а заводит перекладывание
	 *          недочитанного, а раздатчик памяти OpenBSD дорог: сокращается делением
	 *          лишь то, что обеим долям отношения ОДИНАКОВО дорого, а здесь это не так.
	 *          Замерено 05.09.2026. Порог оттого держится вдвое выше дна OpenBSD, а не
	 *          по трём быстрым машинам
	 *
	 */
	 ...
	 *          запись, 05.09.2026 на FreeBSD amd64: 15 и на OpenBSD 7.9 amd64: 15 - число
	 *          в число, а на Debian 12 amd64 (libstdc++): 22
	 *
	 * @warning Показатель этот от СКОРОСТИ машины не зависит, но от стандартной
	 *          библиотеки ЗАВИСИТ: запас короткой строки у libc++ и libstdc++ разный,
	 *          и одна и та же работа даёт 15 выделений на первой и 22 на второй.
	 *          Замерено 05.09.2026. Оттого порог и держится широким: сузь его по
	 *          libc++ - и всякий стенд на glibc отвечал бы ложной тревогой
	 *
