...
#
# @brief Отдельный стенд замеров кодека syslog — сборка набора замеров без библиотеки
...
#
# Вызов:
#   benchmark/codec/syslog/stand.sh [корень дерева] [каталог сборки]
#
...
rm -f "$OUTPUT/syslog-bench" "$OUTPUT/syslog-bench.exe"
...
echo "Собираем стенд замеров syslog: $COMPILER"
...
# Выполняем перебор всех частей кодека syslog
#
...
#          несёт и стенд проверок, «tests/codec/syslog/stand.sh»
#
...
for PART in common reader writer document dictionary; do
	# Выполняем сборку очередной части кодека syslog
	$COMPILER $OPTIONS -c "$ROOT/src/codec/syslog/$PART.cpp" -o "$OUTPUT/codec-$PART.o"
	# Добавляем собранное к перечню объектных файлов стенда
	OBJECTS="$OBJECTS $OUTPUT/codec-$PART.o"
done

# Выполняем перебор всех частей набора замеров кодека syslog
for PART in syslog reader writer document; do
	# Выполняем сборку очередной части набора замеров
	$COMPILER $OPTIONS -c "$ROOT/benchmark/codec/syslog/$PART.cpp" -o "$OUTPUT/bench-$PART.o"
	# Добавляем собранное к перечню объектных файлов стенда
...
#       повтором имён
$COMPILER $OPTIONS $OBJECTS -pthread $SYSTEM_LIBS -lz -o "$OUTPUT/syslog-bench"

# Выводим сообщение об окончании сборки стенда
echo "Стенд собран: $OUTPUT/syslog-bench"
