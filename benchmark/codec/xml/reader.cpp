...
namespace {
	...
	/**
	 * Наименьшая допустимая скорость чтения документа с описанием типа
	 *
	 * @details Путь этот несёт розыск объявлений на всякий узел, приведение значений
	 * атрибутов по объявленному виду и подстановку объявленных умолчаний - работу, какой
	 * прочие сценарии не берут ВОВСЕ: без описания типа отображение объявлений пусто
	 *
	 * @note Порог выведен по соразмерности с сценарием чтения атрибутов: там 21 при
	 *       измеренных 225, запас десятикратный, и здесь взят такой же от измеренных 196.
	 *       По стендам он НЕ СНЯТ - машина одна, а пороги-отношения от машины зависят
	 */
	static constexpr double READ_DECLARED_THRESHOLD = 18.0;
	/**
	 ...
	 *       текста целиком, а не пропускная способность сама по себе. Порог ему можно
	 *       назначить много ближе, чем мере по мегабайтам в секунду: та, заданная с
	 *       запасом на медленные машины, пропустила бы просадку впятеро
	 *
	 * @warning Независимость отношения от машины НЕ ОБЩЕЕ ПРАВИЛО, а замеренное свойство
	 *          ЭТОЙ меры, и полагаться на неё без замера нельзя. Отношение сокращает
	 *          машину лишь настолько, насколько обеим его долям машина одинаково дорога.
	 *          Здесь доли РАЗНОРОДНЫ: подача кусками заводит перекладывание недочитанного,
	 *          то есть работу с памятью, а подача целиком её почти не заводит, - и машина
	 *          с дорогим раздатчиком памяти входит в одну долю сильнее, чем в другую.
	 *          Наблюдение соседа по кодеку CEF 05.09.2026: его такая же мера держалась
	 *          2.56-2.93 на трёх машинах и дала 8.71 на OpenBSD - втрое больше, при пороге
	 *          12, то есть запас упал до 1.38. Здесь беды нет лишь оттого, что среди семи
	 *          стендов OpenBSD БЫЛ и наихудшим не оказался; проверять такие меры надо
	...
	/**
	 * @brief Функция прогона сценария чтения документа с описанием типа
	 *
	 * @details Сценарий этот меряет путь, какого прочие не берут ВОВСЕ: розыск объявлений
	 * атрибутов на всякий узел, приведение значений по объявленному виду и подстановку
	 * значений, объявленных по умолчанию. Без него цена этого хода остаётся неизмеренной,
	 * а «замеры в порогах» означают лишь, что правка не задела мерившееся
	 *
	 * @return результат измерения
	 *
	 */
	...
	/**
	 * Выполняем регистрацию сценария чтения документа с описанием типа
	 */
	static const bool DECLARED_REGISTERED = awh::benchmark::add(
		"codec/xml: чтение с описанием типа", "МБ/с", READ_DECLARED_THRESHOLD,
		awh::benchmark::bound_t::MINIMUM, readDeclared
	);
	/**
