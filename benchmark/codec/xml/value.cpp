...
namespace {
	 ...
	 *
	 * @warning Порог ВРЕМЕННЫЙ: снят он на рабочей машине, а не обходом по стендам. Взят
	 *          вычислением: 0.134 мкс, девятикратный разброс по машинам и двукратный запас
	 *          сверх того дают 2400, округлённые до 3000
	 *
	 ...
	 */
	static constexpr double WIDE_BUILD_THRESHOLD = 3000.0;
	/**
	 ...
	 *
	 * @warning Порог ВРЕМЕННЫЙ по той же причине: 0.029 мкс, девятикратный разброс и
	 *          двукратный запас дают 520, округлённые до 1000
	 *
	 * @note Показатель выражен в НАНОСЕКУНДАХ, а не в микросекундах, и это не прихоть:
	 *       каркас замеров печатает измеренное с двумя знаками после запятой, и доля
	 *       микросекунды теряла в нём разрешение вовсе - 0.021 и 0.029 печатались одним и
	 *       тем же «0.02». Порог по двенадцати машинам по такому числу не снять
	 */
	static constexpr double WIDE_LOOKUP_THRESHOLD = 1000.0;
