...
#
# @brief Отдельный стенд замеров кодека XML — сборка набора замеров без библиотеки
...
#
# Вызов:
#   benchmark/codec/xml/stand.sh [корень дерева] [каталог сборки]
#
...
rm -f "$OUTPUT/xml-bench" "$OUTPUT/xml-bench.exe"
...
echo "Собираем стенд замеров XML: $COMPILER"
...
# Выполняем перебор всех частей набора замеров кодека XML
for PART in xml reader writer document value; do
	# Выполняем сборку очередной части набора замеров
	$COMPILER $OPTIONS -c "$ROOT/benchmark/codec/xml/$PART.cpp" -o "$OUTPUT/bench-$PART.o"
	# Добавляем собранное к перечню объектных файлов стенда
...
#       повтором имён
$COMPILER $OPTIONS $OBJECTS -pthread $SYSTEM_LIBS -lz -o "$OUTPUT/xml-bench"

# Выводим сообщение об окончании сборки стенда
echo "Стенд собран: $OUTPUT/xml-bench"
