...
/**
 * @brief Учёт выдач памяти перегрузкой операторов языка
 *
 * @note Перехватчика распределителя здесь нет намеренно. TcMalloc, дававший его
 *       своим механизмом `MallocHook`, снят из дерева, а собственный распределитель
 *       ведёт свой учёт - но учёт этот считает ВСЯКУЮ выдачу процесса, включая
 *       нужды самой библиотеки языка. Замерам нужно иное: выдачи, заказанные
 *       замеряемым кодом, - их и считает перегрузка операторов
 */
/**
...
int32_t main(int32_t argc, char ** argv){
	...
	bool relaxed = false;
	/**
	 * Пути записи и сличения ряда измерений этой же машины
	 *
	 * @details Порог - это ДНО ОКРУЖЕНИЯ: одно число на все машины, снятое по самой
	 *          медленной из них. Оно ловит грубую поломку сценария и сборку без
	 *          оптимизации, но просадку вдвое на быстрой машине пропускает, потому
	 *          что там запас до дна многократный. Регрессию ловит только сличение
	 *          машины С САМОЙ СОБОЙ: у каждой свой ряд, и допущений о машине оно
	 *          не делает вовсе. Оба пути необязательны, и без них набор ведёт себя
	 *          в точности как прежде
	 */
	const char * baseline = nullptr;
	// Путь записи ряда измерений
	const char * record = nullptr;
	/**
	 * Доля просадки, признаваемая регрессией
	 *
	 * @note Четверть взята оттого, что разброс соседних прогонов на одной машине
	 *       доходит до десятой доли, а двукратный запас порога до дна оставляет
	 *       регрессии вдвое проскочить незамеченной. Четверть лежит между ними
	 */
	double slack = 0.25;
	/**
	...
	for(int32_t i = 1; i < argc; i++){
		...
		// Если задано количество повторов измерения
			...
			/**
			 * Если количество повторов измерения задано негодным
			 *
			 * @warning Негодное значение подставлялось единицей МОЛЧА до 07.09.2026.
			 *          Опечатка в числе обращала усреднение по нескольким кругам в
			 *          одиночный замер, а из одиночного значения вывода о просадке
			 *          делать нельзя вовсе: соседние прогоны одного и того же кода
			 *          перекрываются. Молчание здесь портило не работу, а ВЫВОД
			 */
			if(value <= 0){
				// Сообщаем о негодном количестве повторов измерения
				::printf("Количество повторов измерения задано негодным: «%s»\n", argv[i] + 9);
				// Выводим код выхода с ошибкой
				return 1;
			}
			// Устанавливаем количество повторов измерения
			repeat = static_cast <size_t> (value);
		// Если задано игнорирование порогов
		// Если задан ряд измерений для сличения
		} else if((::strncmp(argv[i], "--baseline=", 11) == 0))
			// Устанавливаем путь ряда измерений для сличения
			baseline = (argv[i] + 11);
		// Если задан путь записи ряда измерений
		else if((::strncmp(argv[i], "--record=", 9) == 0))
			// Устанавливаем путь записи ряда измерений
			record = (argv[i] + 9);
		// Если задана доля просадки, признаваемая регрессией
		else if((::strncmp(argv[i], "--slack=", 8) == 0)){
			// Получаем долю просадки, признаваемую регрессией
			const double value = ::atof(argv[i] + 8);
			/**
			 * Если доля просадки задана вне границ
			 *
			 * @warning Значение вне границ подставлялось четвертью МОЛЧА до 07.09.2026.
			 *          Доля эта - именно ДОЛЯ, а не проценты и не разы: «--slack=25»
			 *          читается как двадцать пять процентов, а оборачивалось той же
			 *          четвертью, и ошибка ничем себя не выдавала
			 */
			if((value <= 0.0) || (value >= 1.0)){
				// Сообщаем о негодной доле просадки
				::printf("Доля просадки задана вне границ: «%s» (ожидается доля от 0 до 1, например 0.25)\n", argv[i] + 8);
				// Выводим код выхода с ошибкой
				return 1;
			}
			// Устанавливаем долю просадки, признаваемую регрессией
			slack = value;
		// Если задано игнорирование порогов
		...
		// Если запрошена справка по параметрам запуска
			...
			::printf(
				...
				"  --record=ПУТЬ       записать измеренные значения в файл\n"
				"  --baseline=ПУТЬ     сличить с прежним рядом ЭТОЙ ЖЕ машины и\n"
				"                      сообщить о просадке (порог такого не ловит:\n"
				"                      он одно число на все машины, снятое по дну)\n"
				"  --slack=ДОЛЯ        доля просадки, признаваемая регрессией (0.25)\n"
			...
			return 0;
		/**
		 * Довод, ни одному ключу не отвечающий, есть ОШИБКА запуска
		 *
		 * @warning Ветви этой здесь не было до 07.09.2026, и неизвестный довод
		 *          проглатывался МОЛЧА. Опечатка в ключе - «--baselien=» вместо
		 *          «--baseline=» - обращала сличение с прежним рядом в ничто, а
		 *          прогон отвечал бодрым «все уложились в пороги» и кодом ноль.
		 *          Отказ здесь дешевле молчания: замеры гоняются оснасткой, и
		 *          опечатку в её строке заметить иначе нечем
		 */
		} else {
			// Сообщаем о неизвестном параметре запуска
			::printf("Неизвестный параметр запуска: «%s»\nСправка по параметрам: %s --help\n", argv[i], argv[0]);
			// Выводим код выхода с ошибкой
			return 1;
		}
	}
	// Ряд измерений прежнего прогона этой же машины
	std::map <std::string, double> previous;
	/**
	 * Если задано сличение с прежним рядом измерений
	 */
	if(baseline != nullptr){
		// Открываем файл ряда измерений
		std::ifstream file(baseline);
		/**
		 * Если файл ряда измерений открыт
		 */
		if(file.is_open()){
			// Строка файла ряда измерений
			std::string line;
			/**
			 * Читаем файл ряда измерений построчно
			 */
			while(std::getline(file, line)){
				/**
				 * Отделяем значение по ПОСЛЕДНЕМУ пробелу строки
				 *
				 * @details Название сценария само несёт пробелы («codec/xml: дерево ответа
				 * SOAP»), и разбор его как одного слова забирал бы лишь «codec/xml:», а
				 * значением пытался прочесть следующее слово названия
				 *
				 * @warning Разбор одним словом (`stream >> name >> value`) стоял здесь до
				 *          07.09.2026 и обращал сличение в НИЧТО: ни одна строка ряда не
				 *          разбиралась, перечень прежних измерений оставался пуст, и
				 *          просадка не находилась НИКОГДА. Отказа при этом не было -
				 *          файл открывался, и сообщение «сличать не с чем» не выводилось.
				 *          Доказано подложным рядом: против эталона, лучшего в 345 раз,
				 *          сличение отвечало «OK». Молчание сторожа, сломанного нацело,
				 *          неотличимо от сторожа, не нашедшего просадки
				 */
				const size_t space = line.rfind(' ');
				/**
				 * Если пробел-разделитель в строке ряда обнаружен
				 */
				if(space != std::string::npos){
					// Название сценария
					const std::string name = line.substr(0, space);
					/**
					 * Выполняем разбор значения характеристики
					 *
					 * @note Значение читается потоком, а не «stod»: тот отвечает на негодную
					 *       запись исключением, а строка ряда приходит из файла и негодной
					 *       быть вправе
					 */
					std::istringstream stream(line.substr(space + 1));
					// Измеренное значение характеристики
					double value = 0.0;
					// Если значение характеристики разобрано
					if(stream >> value)
						// Запоминаем измеренное значение сценария
						previous.emplace(name, value);
				}
			}
		/**
		 * Отсутствие файла - это не отказ, а первый прогон на машине: сличать
		 * не с чем, и говорить об этом надо прямо, иначе молчание неотличимо
		 * от сличения, не нашедшего просадки
		 */
		} else ::printf("Ряда измерений «%s» нет - сличать не с чем, это первый прогон\n", baseline);
	}
	// Ряд измерений текущего прогона
	std::map <std::string, double> current;
	// Количество сценариев, просевших против прежнего прогона
	size_t dropped = 0;
	...
	for(auto & scenario : awh::benchmark::scenarios()){
		...
		// Запоминаем измеренное значение для записи ряда
		current.emplace(scenario.name, result.value);
		/**
		 * Сличаем измеренное значение с прежним прогоном ЭТОЙ ЖЕ машины
		 */
		if(!previous.empty()){
			// Ищем сценарий в ряде прежнего прогона
			auto i = previous.find(scenario.name);
			/**
			 * Если сценарий в ряде прежнего прогона найден
			 */
			if((i != previous.end()) && (i->second > 0.0) && !result.invalid){
				/**
				 * Считаем просадку с оглядкой на направление сравнения: у границы
				 * MINIMUM хуже - меньше, у границы MAXIMUM хуже - больше
				 */
				const double drift = ((scenario.bound == awh::benchmark::bound_t::MINIMUM) ?
				 ((i->second - result.value) / i->second) : ((result.value - i->second) / i->second));
				// Если просадка превышает допустимую долю
				if(drift > slack){
					// Считаем просевший сценарий
					dropped++;
					// Выводим сведения о просадке против прежнего прогона
					::printf(
						"%34sПРОСАДКА против прежнего прогона: было %.2f, стало %.2f (%.1f %%)\n",
						"", i->second, result.value, (drift * 100.0)
					);
				}
			}
		}
	...
	// Если сличение с прежним прогоном выполнялось
	if(!previous.empty())
		// Выводим итог сличения с прежним прогоном
		::printf((dropped == 0) ?
		 "Просадки против прежнего прогона нет\n" :
		 "Просели против прежнего прогона: %zu\n", dropped);
	/**
	 * Если задана запись ряда измерений
	 */
	if(record != nullptr){
		// Открываем файл записи ряда измерений
		std::ofstream file(record, std::ios::trunc);
		/**
		 * Если файл записи ряда измерений открыт
		 */
		if(file.is_open()){
			// Перебираем ряд измерений текущего прогона
			for(auto & item : current)
				// Записываем измеренное значение сценария
				file << item.first << " " << item.second << "\n";
			// Сообщаем о записи ряда измерений
			::printf("Ряд измерений записан: %s (%zu сценариев)\n", record, current.size());
		/**
		 * Невозможность записи ряда - это ОТКАЗ, а не примечание
		 *
		 * @warning Код выхода здесь оставался нулевым до 07.09.2026, и оснастка,
		 *          снимающая эталон в цикле по стендам, шла дальше как ни в чём не
		 *          бывало. Обнаруживалось это позже и в другом месте: сличение с
		 *          ненаписанным рядом отвечало «это первый прогон»
		 */
		} else {
			// Сообщаем о невозможности записи ряда измерений
			::printf("Ряд измерений записать не удалось: %s\n", record);
			// Выводим код выхода с ошибкой
			return 1;
		}
	}
	...
	/**
	 * Выводим код выхода по результатам проверки порогов и сличения
	 *
	 * @note Просадка влияет на код выхода лишь тогда, когда сличение запрошено
	 *       явно: без «--baseline» набор ведёт себя в точности как прежде
	 */
