...
namespace {
	 ...
	 *
	 * @note Значение обязано совпадать с `DATAGRAM_BUFFER` эталонных стендов в
	 *       `tools/benchmark/common.hpp`: расхождение здесь обесценивает сличение
	 *
	 */
	static constexpr size_t DATAGRAM_BUFFER = (4 * 1024 * 1024);
	...
	static outcome_t exchange(const size_t senders, const size_t rounds) noexcept {
		...
		/**
		 * Расширяем приёмный буфер события сервера
		 *
		 * @warning Запас нужен не ради скорости, а ради того, чтобы прогон вообще
		 *          завершился: переспроса у датаграмм нет, и потерянный на
		 *          переполнении приёмной очереди обмен оставил бы отправителя
		 *          ждать ответа, которого уже не будет
		 *
		 * @warning Тот же запас ставят себе и эталонные стенды конкурентов
		 *          (`DATAGRAM_BUFFER` в `tools/benchmark/common.hpp`). Не
		 *          поставить его здесь значило бы записать движку в отставание
		 *          разницу, созданную постановкой замера
		 */
		io.setBufferSize(server, awh::event::action_t::READ, DATAGRAM_BUFFER);
		...
		for(size_t i = 0; i < senders; i++){
			...
			// Расширяем приёмный буфер события отправителя
			io.setBufferSize(sender->id, awh::event::action_t::READ, DATAGRAM_BUFFER);
			/**
