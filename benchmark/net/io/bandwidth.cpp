...
namespace {
	 ...
	 *
	 * @details Сценарий задаёт предел настолько высокий, насколько тот вообще
	 *          выражается: поле предела в узле объявлено как `uint32_t`, и
	 *          34.4 Гбит/с - последнее значение, которое в него влезает. Всё,
	 *          что выше, движок берёт по модулю и молча занижает, поэтому
	 *          задавать здесь "100Gbps" нельзя - это обернулось бы пределом
	 *          в 3.9 ГБ/с, а "1000Gbps" пределом в 425 МиБ/с.
	 *
	 *          Заданный предел петлевой интерфейс выдаёт без труда, поэтому
	 *          показатель измеряет не предел, а сам ограничитель: с заданным
	 *          пределом и чтение, и запись обрезаются до одного кадра MTU за
	 *          операцию, и достигается около двенадцати процентов разрешённого.
	 *          Порог снят с измеренного значения
	 *
	 */
	static constexpr double OVERHEAD_THRESHOLD = 250.0;
	/**
