...
bool awh::benchmark::io::ports(const size_t required, std::string & reason) noexcept {
	 ...
	 * Если сборка производится под операционную систему MS Windows
	 *
	 * @details Диапазон там ЕСТЬ и конечен, вопреки расхожему мнению: с Windows Vista
	 *          он начинается портом 49152 и содержит 16384 порта. Проверено на стенде
	 *          обращением `netsh int ipv4 show dynamicport tcp`
	 *
	 * @note Сама система границ этих ни одним описанным вызовом НЕ отдаёт: `netsh`
	 *       добывает их закрытым слоем NSI. Звать `netsh` из прогона замеров нельзя -
	 *       замер зависел бы и от наличия программы в пути, и от разбора её вывода, а
	 *       вывод этот переводится на язык системы. Оттого границы берутся иначе:
	 *       устаревшая настройка спрашивается у реестра, а при её отсутствии берутся
	 *       описанные умолчания системы
	 *
	 * @note Настройка `MaxUserPort` живёт с давних выпусков и на нынешних системах по
	 *       умолчанию ОТСУТСТВУЕТ. Толкуется она выпусками по-разному: одни её чтут,
	 *       другие пропускают мимо. Оттого выставленная настройка не заменяет
	 *       умолчаний, а лишь СУЖАЕТ запас, если она уже: проверка, ошибившись в
	 *       меньшую сторону, отдаст лишний пропуск, а ошибившись в большую - соврёт
	 *       молча, ровно так, как и заводилась не делать
	 *
	 * @warning Диапазон, суженный обращением `netsh`, не виден ни одному описанному
	 *          вызову: проверка его НЕ узнает и будет считать по умолчанию. Разбирать
	 *          ради него переведённый вывод чужой программы внутри замера - хуже
	 *
	 */
	#if defined(_WIN32) || defined(_WIN64)
		// Начальный порт диапазона по умолчанию
		size_t first = 49152;
		// Конечный порт диапазона по умолчанию
		size_t last = 65535;
		// Описатель ветви реестра с настройками стека
		HKEY key = nullptr;
		// Если ветвь реестра с настройками стека открыта
		if(::RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters", 0, KEY_QUERY_VALUE, &key) == ERROR_SUCCESS){
			// Значение устаревшей настройки верхней границы диапазона
			DWORD value = 0;
			// Размер значения настройки
			DWORD length = sizeof(value);
			// Вид значения настройки
			DWORD kind = 0;
			/**
			 * Если устаревшая настройка верхней границы выставлена
			 *
			 * @note Значения ниже 5000 стек не принимает, и такая настройка означала бы
			 *       не сужение диапазона, а порчу настройки - её мы не берём
			 */
			if((::RegQueryValueExW(key, L"MaxUserPort", nullptr, &kind, reinterpret_cast <LPBYTE> (&value), &length) == ERROR_SUCCESS) && (kind == REG_DWORD) && (value >= 5000)){
				// Запас портов, задаваемый устаревшей настройкой
				const size_t legacy = (static_cast <size_t> (value) - 1024 + 1);
				// Если устаревшая настройка сужает запас портов, берём её границы
				if(legacy < ((last - first) + 1)){
					// Устанавливаем начальный порт диапазона устаревшей настройки
					first = 1024;
					// Устанавливаем конечный порт диапазона устаревшей настройки
					last = static_cast <size_t> (value);
				}
			}
			// Закрываем ветвь реестра с настройками стека
			::RegCloseKey(key);
		}
		// Выводим признак достаточности диапазона эфемерных портов
