...
namespace {
	...
	static awh::benchmark::result_t exchanges() noexcept {
		...
		/**
		 * Если эфемерных портов на прогон не хватает, измерение не выполняется
		 *
		 * @note Всякое наблюдаемое подключение занимает исходящий порт из диапазона,
		 *       отведённого системой, и диапазоны эти разнятся сильно: у DragonFly
		 *       по умолчанию 1024-5000, то есть 3976 портов на всё, тогда как прогону
		 *       нужно 4000. Там подключения не устанавливались вовсе, и сценарий
		 *       отдавал ноль операций - что читалось как провал движка, хотя движок
		 *       ни при чём. Проверено опытом: с расширенным диапазоном тот же прогон
		 *       на той же машине даёт 73 780 обменов в секунду
		 */
		if(!ports(IDLE_WATCHED, reason)){
			// Отмечаем измерение как не выполнявшееся
			result.skipped = true;
			// Устанавливаем причину, по которой измерение не выполнялось
			result.reason = reason;
