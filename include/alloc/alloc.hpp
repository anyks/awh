	...
	namespace alloc {
		...
		typedef class __AWH_SHARED_EXPORT__ Allocator {
			...
			public:
				 ...
				 *       случиться в процессе. У систем ELF подмена именами работает с
				 *       самого запуска, и куча там обыкновенно заводится ещё при заведении
				 *       переменных процесса - прежде, чем прикладной код получит
				 *       управление; тогда подмена отвечает отказом. Но зависит это от
				 *       библиотеки времени исполнения: у OpenBSD, скажем, до `main` не
				 *       выделяется ничего, и подмена там проходит. Оттого ответ метода
				 *       обязан быть ПРОВЕРЕН, а не предположен: отказ - обычный исход
				 *
