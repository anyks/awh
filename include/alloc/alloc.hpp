...
namespace awh {
	...
	namespace alloc {
		...
		typedef struct Options {
			 ...
			 * себя слой прежде
			 *
			 * Умолчанием придержка НЕ ограничена, и это парно к умолчанию `purgeMode`
			 *
			 * Тот гласит `MANUAL` - «отдавать лишь по просьбе», - а слой крупных выдач
			 * прежде отдавал системе всё сверх шестнадцати мегабайт, никого не спрашивая:
			 * два слоя одного распределителя жили по противоположным укладам, и обещание
			 * настройки нарушалось молча. Просьба же опустошает придержку и сегодня -
			 * `Allocator::purge()` зовёт `Huge::drain()` первым делом
			 *
			 * Цена прежнего расхождения измерена на наборе замеров ABC (macOS ARM64):
			 * работа брала у системы 44 897 МБ крупными областями и отдавала их обратно,
			 * платя 2 422 763 отказами страниц против 119 841 у системного распределителя -
			 * в двадцать раз больше, а времени в ядре 2.24 с против 0.19. Разбор шёл при
			 * этом в полтора-два раза медленнее системного. С неограниченной придержкой
			 * из 3240 крупных выдач 3224 достаются готовыми, отказов остаётся 66 578, и
			 * разрыв закрывается целиком: 1.60x → 1.00x на чтении крупной записи
			 *
			 * Плата - расход памяти: на том же наборе 1071 МБ против 489 при потолке в
			 * шестнадцать мегабайт и 717 у системного распределителя. Приложению, какому
			 * память дороже скорости, потолок задаётся числом, а вместе с ним стоит
			 * выбирать и `purgeMode`: уклады эти парны
			 *
			...
			Options() noexcept :
			 ...
			 heapLimit(0), cacheLimit(0), classLimit(0), hugeCache(static_cast <size_t> (-1)), reportLarge(0), profileRate(0),
