	...
	namespace codec {
		...
		namespace csv {
			...
			enum class error_t : uint8_t {
				...
				SEPARATOR_CONFLICT   = 0x0E, // Разделитель не задан вовсе либо совпадает со знаком кавычек либо с переводом строки
				EMPTY_HEADER         = 0x0F, // Заголовок объявлен пустым именем поля
				DUPLICATE_HEADER     = 0x10, // Имя поля в заголовке объявлено повторно
				NO_HEADER            = 0x11, // Заголовок затребован, а текст пуст
				OVERFLOW_LIMIT       = 0x12, // Превышен предел, заданный настройками разбора
				FILE_NOT_OPENED      = 0x13, // Файл таблицы открыть не удалось
				TEXT_ALREADY_ENDED   = 0x14, // Подача продолжена после объявленного конца текста
				FILE_NOT_WRITTEN     = 0x15  // Текст таблицы записать в файл не удалось
			};
