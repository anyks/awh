			...
			typedef class __AWH_SHARED_EXPORT__ Reader {
				public:
					...
					typedef struct __AWH_SHARED_EXPORT__ Settings {
						 ...
						 * проверяется он и по ходу накопления, где разбор дожидается следующего
						 * куска, и по содержимому уже собранного события. Иначе предел зависел бы
						 * от устройства сети, а не от настроек вызывающего: текст, пришедший
						 * целиком, копить не приходится вовсе
						 */
						uint64_t maxEvent;
