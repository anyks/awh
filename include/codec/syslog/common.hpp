...
/**
 * Освобождаем имя «mode_t», занятое макросом системы
 *
 * @details NetBSD в «sys/types.h» заводит не только сам тип, но и макрос
 *          «#define mode_t __mode_t», и объявление перечисления вида записи ниже
 *          получало имя «__mode_t», тогда как всякое употребление его в разборе
 *          шло до подключения системных заголовков и оставалось «mode_t» - имена
 *          расходились, и сборка отказывала на девяти местах разом
 *
 * @note Стоит ЗДЕСЬ, а не в «reader.hpp»: снимать макрос надобно ДО объявления
 *       перечисления, а объявлено оно в этом файле. Снятие тип не трогает - он
 *       объявлен в «sys/types.h» отдельным «typedef» и остаётся доступен
 *
 * @warning Снимается БЕЗВОЗВРАТНО, а не парно через «push_macro»/«pop_macro», как
 *          устроен «suppress.hpp»: части кодека употребляют имя и в своих «.cpp», то
 *          есть ВНЕ этого заголовка, и возвращённый у его конца макрос ломал бы разбор
 *          ровно там же. Утраты нет: сам тип объявлен в «sys/types.h» отдельным
 *          «typedef», и всякий, кто пишет «mode_t», получает его же
 *
 */
#undef mode_t
