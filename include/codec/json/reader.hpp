...
namespace awh {
	...
	namespace codec {
		...
		namespace json {
			/**
			 * \~russian
			 * @brief Состояние чтения текста JSON
			 *
			 * @note Договор общий у семи кодеков рамки, и члены его значат ровно то же:
			 *       разниться толкованиям нельзя, иначе цикл, написанный по образцу
			 *       одного кодека, окажется верным по виду и неверным по условию выхода
			 *
			 * \~english
			 * @brief State of the reading of a JSON text
			 *
			 * \~
			 */
			...
			typedef class __AWH_SHARED_EXPORT__ Reader {
				public:
					...
					typedef struct __AWH_SHARED_EXPORT__ Settings {
						...
						bool emitComments;
						// Наибольшая допустимая длина строкового значения в байтах, ноль - без предела; отказ кодом STRING_TOO_LONG
						uint32_t maxString;
						// Наибольшая допустимая длина записи числа в байтах, ноль - без предела; отказ кодом NUMBER_TOO_LONG
						uint32_t maxNumber;
						/**
						 * \~russian
				...
				private:
					/**
					 * \~russian
					 * @brief Фаза разбора текста
					 *
					 * @note Звалась `state_t` до 07.09.2026 и переименована ради согласия
					 *       семи кодеков: имя `state_t` несёт у них ОТКРЫТОЕ состояние
					 *       потокового чтения (`READY`, `HUNGRY`, `FINISHED`, `FAILED`),
					 *       тогда как здесь тип описывает закрытый посимвольный автомат.
					 *       Два разных смысла под одним именем внутри одного кодека
					 *       развести было нельзя, а договор общий главнее внутреннего имени
					 *
					 * @details Фаза хранит и то, что обыкновенно достаётся взглядом на
				...
				public:
					 ...
					 *
					 * @details Состояние вычисляется по имеющемуся, а не хранится полем:
					 * события собираются в очередь при подаче куска, и хранимое состояние
					 * расходилось бы с нею при всякой выдаче события
					 *
					 * @note Прекращение разбора по требованию потребителя отражается членом
					 *       `FINISHED`: событий разбор больше не даст и продолжения не ждёт,
					 *       а причина прекращения известна тому, кто его затребовал, — своей
					 *       причины у разбора здесь нет, потому и `FAILED` неверен
					 *
					 * @return текущее состояние чтения текста JSON
					 *
					 * \~english
					 * @brief Method of getting the current state of the reading
					 * @return current state of the reading of a JSON text
					 *
					 * \~
					 */
