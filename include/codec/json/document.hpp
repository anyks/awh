		...
		namespace json {
			...
			typedef class __AWH_SHARED_EXPORT__ Document {
				...
				public:
					...
					typedef class __AWH_SHARED_EXPORT__ Value {
						...
						public:
							 ...
							 *
							 * @details Проверяются два условия: ссылка указывает на существующий узел и клеймо
							 * поколения её отвечает клейму дерева. Первое ловит ссылку пустую, второе - ссылку,
							 * пережившую перестроение дерева
							 *
							 * @warning Проверка эта предполагает документ ЖИВЫМ и обнаружить его уничтожение
							 *          НЕ МОЖЕТ: клеймо дерева лежит внутри самого документа, и спросить его у
							 *          снесённого объекта значит читать освобождённую память. Замер 31.08.2026:
							 *          ссылка, снятая внутри области видимости и опрошенная `valid()` за её
							 *          пределами, отвергается надзором как `stack-use-after-scope`. Ссылка жизнь
							 *          документа не продлевает и продлевать не должна - иначе всякая снятая
							 *          ручка держала бы дерево целиком
							 *
							 * @note Сказано это здесь нарочно: `valid()` СОБОЮ приглашает думать, будто спрашивать
						...
						private:
							 ...
							 * @return        признак успешности извлечения
							 *
							 * @note Переменная-приёмник при ОТКАЗЕ не трогается вовсе и сохраняет
							 *       то, что несла до вызова: уклад этот един у всех трёх кодеков и
							 *       снят замером 01.09.2026 по двадцати путям отказа
							 *
							 * \~english
						...
						public:
							 ...
							 *       `1.5`, а `1e2` - как `100.0`. Значение при этом то же самое
							 *
							 * @note Дробность записи при том СОХРАНЯЕТСЯ: запись с показателем
							 *       степени разбирается видом дробным, и кратчайшая запись,
							 *       читаемая тем же числом, обязана остаться дробной, - иначе
							 *       `1e2` вернулось бы целым, видом с исходным не совпадающим.
							 *       Прежде здесь стояло `100`, и замер 01.09.2026 это опроверг
							 *
							 * @note Настроек записи выдача эта НЕ слушает, и слушать ей нечего:
							 *       ни уклад экранирования, ни вид оформления, ни обращение с
							 *       негодной последовательностью числа не касаются. Замер того
							 *       же дня по четырём укладам: выдача неизменна
							 *
				...
				private:
					 ...
					 *
					 * @note Изменяемость его намеренна: сохранение документа объявлено НЕИЗМЕНЯЮЩИМ
					 *       - оно документа не трогает, - а подмена целевого файла временным
					 *       константным ходом у файловой системы не является
					 *
					 * \~english
				...
				public:
					 ...
					 *          а не от единообразия: RFC 8259 §2 велит тексту JSON нести ровно
					 *          одно значение, и документа без корня не бывает. У кодеков, чей
					 *          стандарт пустой документ допускает - YAML, INI, TOML, - снос корня
					 *          вправе быть дозволен, и приводить их к нашему укладу НЕ НАДО:
					 *          при столкновении согласия API со стандартом старше стандарт
					 *          (решение владельца от 03.09.2026)
					 *
