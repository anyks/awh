...
namespace awh {
	...
	namespace codec {
		...
		namespace json {
			...
			/**
			 * \~russian
			 * @brief Встраиваемый посредник разбора последовательности UTF-8
			 *
			 * @details Разбор зовётся на КАЖДЫЙ знак записываемого содержимого, а работы в
			 * нём - одна арифметика. Оставленный вызовом через границу единиц трансляции,
			 * он стоит дороже самой работы: замер 05.09.2026 дал 567 МБ/с записи отменяемых
			 * знаков против 843 у среза от 29.08.2026, где разбор жил прямо в писателе
			 *
			 * @warning Определение ОДНО: открытый `decode()` сведён к этому посреднику и
			 *          своего тела не несёт. Прежде тел было два - своё у писателя и своё
			 *          у разбора, - и 30.08.2026 их справедливо свели в одно; беда была
			 *          лишь в том, что вместе со вторым телом ушло и встраивание
			 *
			 * @note Звать посредника внутри `encoding.cpp` не надо: там суждение и так
			 *       лежит в одной единице трансляции, и собиратель встраивает его сам.
			 *       Принудительная подстановка там ВРЕДИТ - проверено на кодеке CSV
			 *
			 * @param text   разбираемое содержимое
			 * @param offset положение разбираемого байта в содержимом
			 * @param code   полученная кодовая точка
			 ...
			 *
			 * \~english
			 * @brief Inline mediator of the parsing of a UTF-8 sequence
			 * @param text   content being parsed
			 * @param offset position of the byte being parsed in the content
			 * @param code   obtained code point
			 * @param valid  sign of the validity of the parsed sequence
			 * @return       number of the parsed bytes of the content
			 *
			 * \~
			 */
			AWH_JSON_INLINE size_t readSequence(const string & text, const size_t offset, uint32_t & code, bool & valid) noexcept {
			...
			typedef class __AWH_SHARED_EXPORT__ Decoder {
				...
				private:
					 ...
					 * через него, и запись в журнал ведётся в одном месте на всё приведение
					 *
