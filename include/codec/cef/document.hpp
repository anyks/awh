...
namespace awh {
	...
	namespace codec {
		...
		namespace cef {
			 ...
			 * а не `FILE_NOT_OPENED`.** Каталог на POSIX открывается потоком УСПЕШНО и
			 * читается признаками конца - теми же, какими отзывается пустой файл, -
			 * отчего до заслона подача его отвечалась ИСТИНОЙ с пустым деревом. Код
			 * назван по существу беды: «открыть не удалось» слало бы потребителя
			 * искать права доступа либо отсутствующий путь, тогда как подан не файл
			 * вовсе. Распознаётся каталог ДО открытия потока - у MS Windows он не
			 * открывается, и проверка после открытия была бы там мертва. Закреплено
			 * `CodecContract.DirectoryFedInsteadOfAFileIsRefused`, где код сличается
			 * числом у восьми кодеков разом
			 *
			 * \~english
