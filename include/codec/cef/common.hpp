...
namespace awh {
	...
	namespace codec {
		...
		namespace cef {
			 ...
			 *       оканчиваются «%Z» и пояса требуют. Отвергались тем самым 33 поля
			 *       словаря разом - все, вид метки времени несущие
			 *
			 * @warning Метка без пояса разбирается как МЕСТНАЯ, а не как всемирная:
			 *          описание толкует непоставленный пояс поясом самого устройства, и
			 *          так же поступает кодек системного журнала с датами RFC 3164.
			 *          Разбор её всемирной сместил бы момент на величину пояса - молча
			 *          и правдоподобно
			 *
			 * \~english
			 ...
			 * \~
			 */
			constexpr string_view TIMESTAMP_ZONELESS_FRACTION_FORMAT = "%b %d %Y %H:%M:%S.%s";
			 ...
			 *       от настройки кодировки, снятой прежде (стандарт требует UTF-8, и
			 *       приведения кодек не ведёт); INVALID_CHARACTER и INVALID_ESCAPE
			 *       противоречат решению принимать всякий октет и оставлять косую перед
			 *       неведомым знаком как есть; MISSING_SEPARATOR и INVALID_KEY недостижимы,
			 *       ибо пара без знака равенства и ключ с недопустимым знаком отвергаются
			 *       сличением как UNKNOWN_KEY, а без сличения принимаются по договору;
			 *       TYPE_MISMATCH покрыт кодами INVALID_NUMBER, INVALID_ADDRESS и
			 *       INVALID_TIMESTAMP; UNEXPECTED_EOF покрыт кодом INCOMPLETE_HEADER, ибо
			 *       расширение оборванным не бывает - пара либо разобрана, либо нет;
			 *       STORAGE_EXHAUSTED недостижим при разрядности размера; INTERNAL и
			 *       CONFLICTING_SETTINGS не выставлялись ни на одном пути вовсе
			 *
			 * \~english
			...
			enum class error_t : uint8_t {
				...
				MISSING_SIGNATURE     = 0x01, // Запись не содержит слова «CEF:»
				INVALID_VERSION       = 0x02, // Номер редакции записи построен ошибочно
				UNSUPPORTED_VERSION   = 0x03, // Номер редакции записи не поддерживается
				INCOMPLETE_HEADER     = 0x04, // Полей заголовка меньше положенного
				EMPTY_HEADER_FIELD    = 0x05, // Обязательное поле заголовка пусто
				INVALID_SEVERITY      = 0x06, // Важность события построена ошибочно либо выходит за предел
				EMPTY_KEY             = 0x07, // Имя ключа расширения пусто
				UNKNOWN_KEY           = 0x08, // Ключ расширения словарю неизвестен
				NAME_TOO_LONG         = 0x09, // Длина имени превышает допустимую
				FIELD_TOO_LONG        = 0x0A, // Длина поля заголовка превышает допустимую
				RECORD_TOO_LONG       = 0x0B, // Длина записи превышает допустимую
				INVALID_ADDRESS       = 0x0C, // Значение не является адресом сети
				INVALID_TIMESTAMP     = 0x0D, // Значение не является меткой времени
				INVALID_NUMBER        = 0x0E, // Значение не является числом
				DANGLING_LABEL        = 0x0F, // Имя поля задано меткой, а самого поля в записи нет
				OVERFLOW_LIMIT        = 0x10, // Превышен предел, ЗАДАННЫЙ НАСТРОЙКАМИ разбора
