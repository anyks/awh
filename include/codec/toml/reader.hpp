		...
		namespace toml {
			...
			typedef class __AWH_SHARED_EXPORT__ Reader {
				...
				private:
					...
					string _table;
					/**
					 * Перечень уже объявленных ключей и таблиц
					 *
					 * @note Заполняется лишь при включённой проверке повторов: в прочих
					 * случаях удерживать имена незачем, а на большом тексте настроек
					 * перечень этот памяти стоит
					 */
					unordered_set <string_view> _declared;
					// Перечень уже объявленных наборов таблиц
					unordered_set <string_view> _arrays;
					/**
					 * Перечень таблиц, объявленных исподволь
					 *
					 * @details Таблица объявляется исподволь всяким объемлющим именем:
					 * заголовок «[a.b]» и составное имя ключа «a.b = 1» заводят таблицу
					 * «a», собственного объявления не имеющую. Описание запрещает
					 * дополнять такую таблицу набором таблиц наравне с объявленной
					 */
					unordered_set <string_view> _implicit;
					/**
				...
				private:
					 ...
					 *
					 */
					bool content(const bool end, const uint32_t depth) noexcept;
					/**
					 ...
					 *
					 */
					bool inlined(const bool end, const uint32_t depth) noexcept;
					/**
				...
				private:
					 ...
					 *
					 */
					bool declare(const string & name, const error_t error) noexcept;
					/**
