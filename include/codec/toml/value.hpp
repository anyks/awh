		...
		namespace toml {
			...
			typedef class __AWH_SHARED_EXPORT__ Value {
				...
				public:
					 ...
					 *
					 * @warning Ход этот отвечает ЛИШЬ у значений строковых: числу, логическому
					 *          значению и отметке времени он отдаёт пустую строку, от строки
					 *          пустой неотличимую, и кода отказа при том не ставит. Начертание
					 *          их берётся ходом `record()`. Договор здешний тем и разнится с
					 *          YAML и INI, где начертание всякого простого значения отдаёт сам
					 *          `text()`, - разница намеренная и от устройства: здешнее дерево
					 *          хранит величины видом родным, а не записью их
					 *
					 ...
					 *
					 * @details Ход этот отдаёт значение тем видом, каким оно ложится в текст:
					 * число, логическое значение и отметку времени - записью их, а строку -
					 * содержимым, оградою уже снятою
					 *
					 * @warning Начертание нестрокового значения брать надлежит ЭТИМ ходом, а не
					 *          ходом `text()`: тот отвечает лишь у значений строковых, а у прочих
					 *          отдаёт пустую строку, от строки пустой неотличимую и никакого кода
					 *          отказа не ставящую. Договор здешний тем и разнится с YAML и INI,
					 *          где начертание всякого простого значения отдаёт сам `text()`
					 *
					 * @note Перечень и таблица начертания одного, вне текста стоящего, не имеют:
					 *       им отвечает отказом с кодом `INVALID_VALUE`, а записывает их `dump()`
					 *
					 * @return начертание простого значения
					 *
					 * \~english
					 * @brief Method of the obtaining of the record of a simple value
					 * @return record of the simple value
					 *
					 * \~
					 */
					string record() const noexcept;
					/**
					 * \~russian
