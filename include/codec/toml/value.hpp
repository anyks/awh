...
namespace awh {
	...
	namespace codec {
		...
		namespace toml {
			 ...
			 *
			 * @note Правило это общее у INI и TOML, а найдено оно у кодеков JSON и
			...
			typedef class __AWH_SHARED_EXPORT__ Value {
				...
				public:
					 ...
					 *          пустой неотличимую, и кода отказа при том не ставит. Спрашивать
					 *          их надлежит по виду: `value()` - число и логическое значение,
					 *          `stamp()` - отметку времени, а вид даёт `type()`. Договор здешний
					 *          тем и разнится с YAML и INI, где начертание всякого простого
					 *          значения отдаёт сам `text()`, - разница намеренная и от
					 *          устройства: здешнее дерево хранит величины видом родным, а не
					 *          записью их, и начертания одного, вне текста стоящего, у них нет
					 *
					 * @note Ловушка эта не выдумана: мост, значения между кодеками переносящий,
					 *       на ней терял отметку времени - она уходила пустою строкою без
					 *       единого отказа, тогда как соседи отдавали её числом
					 *
