...
namespace awh {
	...
	namespace codec {
		...
		namespace toml {
			 ...
			 *          лишь таблица сообщений. Таковых ОДИН из сорока двух: OVERFLOW_LIMIT
			 *
			 * @note Перемерено 17.09.2026 тем же розыском: с прошлого замера ожил
			 *       CONFLICTING_SETTINGS - выдаётся он ныне судом о разладе настроек записи
			 *       с чтением (`document.cpp`), - и двое обратились одним. Перечень при том
			 *       вырос с тридцати семи имён до сорока двух
			 *
			 * @warning Запись, замера пережившая, здесь опаснее отсутствия: потребитель по
			 *          ней счёл бы отказ недостижимым и обхода ему не написал
			 *
			 ...
			 *
			 * @note Оттого часть кодов кодеком TOML не выдаётся
			...
			enum class error_t : uint8_t {
				...
				FILE_NOT_OPENED       = 0x23, // Файл настроек открыть не удалось
				FILE_NOT_WRITTEN      = 0x24, // Текст настроек записать в файл не удалось
				FILE_NOT_READ         = 0x25, // Текст настроек из файла прочитать не удалось
				TEXT_ALREADY_ENDED    = 0x26, // Подача текста продолжена после объявленного конца его
				ENCODING_ALREADY_CHOSEN = 0x27, // Кодировка сменена посреди подачи текста
				SETTINGS_ALREADY_APPLIED = 0x28, // Настройки сменены посреди подачи текста
				/**
				 * Значение такого вида корнем текста TOML быть не может
				 *
				 * @note Имя общее с кодеком INI намеренно: там тот же случай и тот же
				 *       смысл. Числа же у кодеков расходятся, и сличать коды между
				 *       кодеками надлежит по ИМЕНИ, а не по значению
				 *
				 * @warning Прежде здесь ставился `INVALID_VALUE` - «значение искажено», - и
				 *          то было ложно по существу: исправное целое либо запись искажены
				 *          не были, корнем текста их не сделать по устройству наречия.
				 *          Потребитель шёл искать порчу там, где её нет. Замерено 15.09.2026
				 *          сличением с кодеками рамки: JSON, YAML и XML начертание простого
				 *          значения ОТДАЮТ, INI отказывает и причину называет верно
				 */
				UNREPRESENTABLE_VALUE = 0x29
			};
			 ...
			 *
			 * @note Описание TOML велит документу быть текстом в годной кодировке UTF-8, и
			 *       правило `PASS` даёт текст, документом TOML НЕ являющийся. Правило это
			 *       оставлено потребителю намеренно - тем же составом, что у прочих
			 *       кодеков, - но выбор его есть выход за пределы описания, и умолчанием
			 *       оно не служит
			 *
			 * \~english
