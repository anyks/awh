...
namespace awh {
	...
	namespace codec {
		...
		namespace yaml {
			 ...
			 *
			 * @note Ширина отступа задаётся пробелами, и ноль ею быть не может: знак
			 ...
			 *
			 * @note Ноль, поданный настройкою, обращается шириною УМОЛЧАЛЬНОЮ, отказом при
			 *       том не отвечая: текст выходит тот же, что и при двойке. Отказ тут был бы
			 *       вреднее подмены - собранный текст верен всегда, а звавший, ноль подавший
			 *       по невниманию, получил бы отказ записи вместо годного текста
			 *
			 * @warning Прежде записано было «нуля она не принимает», и слово это обещало
			 *          отказ, какого нет: звавший, слову поверивший, ждал бы признака
			 *          неудачи, а получал молчаливую подмену. Расхождение слова с делом
			 *          замерено 17.09.2026 сличением ширин 0, 1, 2 и 3 на одном дереве
			 *
			 * @note Соседи INI да TOML одноимённое поле держат ПРИЗНАКОМ (`bool`), а не
			 *       шириною, и ноль у них значит «отступа не ставить» - сличать их с этим
			 *       полем на одном значении нельзя вовсе. Шириною поле это держат трое:
			 *       YAML, JSON да XML
			 *
			 * @note Устройство XML тут стоит помнить как образец иной: ширина отвечает у
			 *       него за ширину, а ЗНАК отступа вынесен отдельным полем (`separator_t`:
			 *       NONE, TABS, SPACES), и ноль ширины значит там буквально ноль знаков.
			 *       Приём «ноль значит подачу», каким JSON экономит поле, стоит того, что
			 *       настройка перестаёт значить то, как зовётся; нам же приём этот закрыт
			 *       описанием, и оттого ноль у нас несёт смысл третий - умолчание
			 *
			 * \~english
			...
			typedef class __AWH_SHARED_EXPORT__ Writer {
				...
				public:
					...
					/**
					 * @note Приём записи ЗНАКА - `value(const char)` - снят 09.09.2026. Он писал
					 *       знак строкою из одного знака, и довод его гласил, что без него
					 *       `value('z')` молча стало бы числом 122. Довод тот верен, но неполон:
					 *       у систем Sun вид `int8_t` объявлен КАК `char`, а не как `signed char`
					 *       (замерено на 10.100.1.105 через `is_same`), и запись `value(int8_t(-128))`
					 *       уходила туда же - байт `0x80` знаком UTF-8 не является, и правило
					 *       `malformed_t::REPLACE` подменяло его знаком `U+FFFD`. Число обращалось
					 *       в порчу МОЛЧА, и только на двух машинах из восьми
					 *
					 * @note Различить намерение кодек не может в принципе: на тех машинах `int8_t`
					 *       и `char` суть ОДИН вид, и перегрузки по нему неразличимы. Выбор потому
					 *       не между «знаком» и «числом», а между неожиданностью платформенно
					 *       НЕЗАВИСИМОЙ и порчею платформенно ЗАВИСИМОЙ
					 *
					 * @note Взято по кодеку JSON, у какого договор типов наречия перенят ДОСЛОВНО:
					 *       приёма записи знака у его писателя нет вовсе, и `value('z')` пишет там
					 *       122 на всех машинах разом. Расходился здесь YAML, а не JSON
					 *
					 * @note Записать знак по-прежнему можно строкою: `value("z")` либо
					 *       `value(string(1, letter))`. Заведение знака ВЛАДЕЮЩИМ значением
					 *       оставлено как есть - оно у JSON тоже имеется, и трогать его значило бы
					 *       разойтись с образцом
					 */
					/**
					 * \~russian
				...
				public:
					 ...
					 *
					 * @note Договор этот один у INI, TOML и YAML: расхождение их сведено к одному
					 * общим решением. Закреплён он проверкой RefusalLocksWriter, перебирающей ВСЕ
					 ...
					 *
					 * @warning ОТСТУПЛЕНИЕ отказом не является и кода не ставит: verbatim() и
					 * block() отвечают ложью там, где содержимое этим построением не передаётся -
					 * блочное значение внутри скобок, дословные строки за именем пары, знак
					 * возврата каретки в блочном значении, - и сборка берёт иное оформление тем
					 * же содержимым. Писателя такая ложь НЕ заклинивает. Закреплено проверкой
					 * BlockRetreatLeavesNoReason. Обратив отступление в отказ, роняешь четыре
					 * проверки дерева и одну записи - замерено
					 *
