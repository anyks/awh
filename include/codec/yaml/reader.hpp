...
namespace awh {
	...
	namespace codec {
		...
		namespace yaml {
			 ...
			 */
			/**
			 * @warning Имя `PARSING` при значении 0x01 расходится с шестью прочими кодеками рамки,
			 *          где то же значение зовётся `HUNGRY` - «разбору требуется продолжение текста».
			 *          Смысл один, имя разное. Сличены ВСЕ шесть поимённо: CSV, INI, JSON, TOML,
			 *          XML и CEF; у ABC состояния этого нет вовсе
			 *
			 * @note Вопрос ВЫНЕСЕН ВЛАДЕЛЬЦУ 07.09.2026 и решения не имеет. Запись эта стоит здесь
			 *       затем, чтобы следующий разбор не открывал расхождение заново: оно замечено,
			 *       а не просмотрено
			 *
			 * @warning Разрешись вопрос в пользу общего имени - переименование надлежит вести
			 *          ВМЕСТЕ со снятием этой записи, а не после неё
			 */
