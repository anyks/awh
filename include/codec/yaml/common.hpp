...
namespace awh {
	...
	namespace codec {
		...
		namespace yaml {
			 ...
			 * каждого кодека свой перечень. Оттого часть кодов кодеком YAML не выдаётся:
			 *
			 * @note - `STORAGE_EXHAUSTED` мест выдачи не имеет вовсе, и это НЕ свойство
			 * наречия, а пробел кодека: дерево YAML держит смещения в хранилище знаков
			 * четырьмя октетами наравне с INI, TOML, XML, CSV и JSON, однако поверки
			 * ширины у переноса записи (`deposit`) не ведёт вовсе - переполнение усекло бы
			 * смещение молча. Заведение поверки требует переменить договор `deposit` у
			 * восьми зовущих мест и вынесено владельцу отдельным докладом
			 *
