...
namespace awh {
	...
	namespace codec {
		...
		namespace yaml {
			 ...
			 *          лишь таблица сообщений. Таковых ТРИНАДЦАТЬ из пятидесяти, сиречь четверть с лишним: UNEXPECTED_EOF, INVALID_NUMBER,
			 *       NUMBER_OUT_OF_RANGE, INVALID_BINARY, INVALID_STAMP, EXPECTED_KEY, EXPECTED_COLON,
			 *       DUPLICATE_ANCHOR, UNEXPECTED_DOCUMENT, NUMBER_TOO_LONG, OVERFLOW_LIMIT,
			 *       CONFLICTING_SETTINGS и STORAGE_EXHAUSTED
			 *
			 * @note Тринадцать эти разбираются на ДВА разряда, и сличено это 08.09.2026 с
			 *       перечнем кодека JSON. СЕМЬ из них у JSON объявлены и ЖИВЫ - UNEXPECTED_EOF,
			 *       INVALID_NUMBER, NUMBER_OUT_OF_RANGE, EXPECTED_KEY, EXPECTED_COLON,
			 *       NUMBER_TOO_LONG и STORAGE_EXHAUSTED: перечень наш строился по образцу
			 *       JSON, и коды эти переехали вместе с образцом, а к местам выдачи привязаны
			 *       не были. ШЕСТЬ прочих - свои: INVALID_BINARY, INVALID_STAMP,
			 *       DUPLICATE_ANCHOR, UNEXPECTED_DOCUMENT, CONFLICTING_SETTINGS, OVERFLOW_LIMIT
			 *
			 * @note У одного из шести причина УСТАНОВЛЕНА опытом: `INVALID_BINARY` мёртв по
			 *       УСТРОЙСТВУ, а не по недосмотру. Содержимое метки `!!binary` кодек хранит
			 *       дословно и base64 не разбирает вовсе - решение это записано при слое
			 *       значений, - и оттого негодной записи не бывает: подачи `YW!j`, `YWJ`,
			 *       `YW=j` и `====` разбираются успехом и выдаются обратно теми же знаками.
			 *       Причины прочих пяти не устанавливались
			 *
			 ...
			 */
			/**
			 * @warning Настройка эта заведена лишь у ДВУХ кодеков рамки из семи - YAML и JSON.
			 *          Сличены все семь поимённо: у TOML, INI, CSV, XML и CEF её нет вовсе, и
			 *          обхождение с негодной последовательностью там ничем не правится
			 *
			 * @note Довод выше гласит, что правило это владельцем положено ОДИНАКОВЫМ для всех
			 *       кодеков; настройка же, правило это меняющая, есть у двоих. Расхождение
			 *       ВЫНЕСЕНО ВЛАДЕЛЬЦУ 07.09.2026 и решения не имеет
			 *
			 * @warning Разрешись вопрос в пользу общей настройки - заведение её у прочих кодеков
			 *          надлежит вести ВМЕСТЕ со снятием этой записи, а не после него
			 */
