...
namespace awh {
	...
	namespace codec {
		...
		namespace abc {
			...
			/**
			 * \~russian
			 * @note Кодеком ABC осуществлены ДВА правила из четырёх: `REFUSE` и `KEEP`.
			 * `FIRST` и `LAST` ведут себя как `KEEP` и оставлены словарём: выбор одного из
			 * двух значений требует видеть отображение целиком, тогда как потоковый разбор
			 * выдаёт события по одному и назад не ходит, а дерево держит узлы сплошным
			 * перечнем, где изъятие узла двигает все последующие. Осуществление их — работа
			 * отдельная, и объявлять её сделанной прежде времени значило бы лгать настройкой
			 *
			 * \~english
			 * @note TWO rules of the four are realized by the ABC codec: `REFUSE` and `KEEP`.
			 * `FIRST` and `LAST` behave as `KEEP` and are left as a vocabulary
			 *
			 * \~
			 */
