...
namespace awh {
	...
	namespace codec {
		...
		namespace abc {
			 ...
			 * причин 06.09.2026 прошёл все шестнадцать мест отказа хранилища порознь.
			 * Десять под сторожем, а шесть молчат, и молчание их разбирается так:
			 * - **запись и сброс** - `fwrite` и `fflush`: слепыми БЫЛИ, а быть ими перестали.
			 *   Довод «нужна отказывающая файловая система» оказался шире правды: ломать
			 *   носитель не надо, довольно предела `RLIMIT_FSIZE`, какой процесс ставит себе
			 *   сам, а `EFBIG` приходит от ядра и потому отказ этот НАСТОЯЩИЙ. Различает
			 *   два места РАЗМЕР содержимого: крупнее буфера `stdio` - отказывает запись
			 *   (`WriteBeyondTheFileSizeLimitIsRefused`), мельче - оседает в буфере и
			 *   отказывает сброс (`FlushBeyondTheFileSizeLimitIsRefused`). Закрыто 06.09.2026;
			 * - **отказ носителя** - `fsync` и `ferror`, да чтение в `fetch`: наводятся лишь
			 *   отказом самой файловой системы, поддельной же у набора нет. Слепота законна,
			 *   и закрывать её пришлось бы образом диска, цена какого несоразмерна
			 *   получаемому. Осталось таких мест ЧЕТЫРЕ, и всякое поверено порознь
			 *   06.09.2026; тогда же щуп пути показал, что дорога к каждому ХОЖЕНА, и потому
			 *   недостижимо здесь условие, а не путь. Про `ferror` особо: нулевое чтение значит либо конец файла, либо
			 *   отказ, и различает их один `ferror`. Каталог, поданный вместо файла, дверью
			 *   НЕ СЛУЖИТ: открытие идёт видом `r+b`, и каталог отсекается РАНЬШЕ того места,
			 *   ради какого проверка писалась бы, - она проходила бы по неверной причине.
			 *   Замерено 07.09.2026 на двух системах, знак в знак: `fopen` каталога видом
			 *   `rb` УСПЕШЕН у обеих (macOS 15 arm64 и Astra Linux, glibc 2.36), а `EISDIR`
			 *   приходит лишь на чтении; видами же `r+b` и `wb` отказ приходит сразу, при
			 *   самом открытии. Раннего отказа `fopen` у Linux, вопреки ходячему мнению,
			 *   НЕТ - мнение это было названо мною по памяти и опровергнуто замером. Перемотка в `load` - последняя рука: дойти до неё значит открыться,
			 *   а открытие перемоткою длину уже сняло;
			 * - **замер длины при открытии** слепым БЫЛ, а быть им перестал: длины требует
			 *   не файловая система, а сам поток, и поток бывает НЕПЕРЕМАТЫВАЕМЫЙ. Таков
			 *   именованный канал: перемотка его отказывает всегда, и подделки не надо
			 *   вовсе. Закреплено `CodecAbcStorage.UnseekableStreamIsRefused`. Порядок
			 *   открытия канала обратный привычному - сперва читатель, затем писатель;
			 * - **перенос причины снимателя при подаче** - ПОСЛЕДНЯЯ РУКА, а не пробел
			 *   проверок. `Loader::feed` отказывает единственно на нулевом буфере при
			 *   ненулевой длине, хранилище же подаёт указатель собственного вектора, и
			 *   нулевым он не бывает. Разобрано чтением зовомого 06.09.2026, когда щуп
			 *   переноса назвал место незакреплённым: проверка на порченом контейнере
			 *   писалась и была снята - подача его проходит УСПЕХОМ, ибо `feed` лишь
			 *   копит октеты, а разбор их ведёт `next`;
			 * - **усечение файла под ногами** - `truncate` мимо хранилища: файл живёт своей
			 *   жизнью, и чтение вернёт меньше затребованного. Закреплено
			 *   `CodecAbcStorage.TruncationUnderfootIsRefused`;
			 * - **работы до открытия файла** - привязка правки и выборки: закреплено
			 *   `CodecAbcStorage.EveryWorkBeforeOpeningNamesItsRefusal`;
			 * - прочие места закрыты проверками, и в том числе `CodecAbcStorage.Refusals`,
			 *   `.RefusalsWithoutMedium`, `.RefusedStoringLeavesThePreviousContainerWhole`
			 *
			 * @note **О заслонах, ни разу не сработавших.** Вопрос этот ИНОЙ, нежели у
			 * записи выше: та спрашивает, читают ли причину отказа, а этот - ловил ли
			 * заслон хоть что-нибудь. Сплошной щуп нужности 07.09.2026 обесточил порознь
			 * все 45 заслонов хранилища, сохраняя условие вычисляемым (`if((X) && false)`,
			 * иначе отнимается не заслон, а само действие). Под сторожем 31 место, молчат
			 * 13, и одно молчание ЛОЖНОЕ:
			 * - `measure` о двух ветвях систем: подмена ветви `_fseeki64` под
			 *   `#if _WIN32 || _WIN64` до компилятора на POSIX НЕ ДОХОДИТ вовсе, и щуп
			 *   молчит о себе, а не о заслоне. Молчание же парной ветви `fseeko` под
			 *   `#else` НАСТОЯЩЕЕ: отказ перемотки в конец не наводится ничем. Судить о
			 *   местах под условной сборкой по молчанию нельзя, а сличать двоичные файлы
			 *   негодно - сборка с покрытием НЕ детерминирована, замерено двумя
			 *   пересборками одного исходника;
			 * - прочие двенадцать суть последние руки носителя (`ferror`, короткое
			 *   `fread`, отказы перемотки, отказ открытия) и заслон журнала, разобранные
			 *   порознь записью выше: наводятся они отказом самой файловой системы;
			 * - подача кусков (`if(size == 0)`) прогон ПОДВЕСИЛА, и это ответ, а не сбой:
			 *   заслон здесь несёт выход из цикла чтения, и без него цикл вечен. Судить
			 *   по повисшему прогону нельзя, разобрано чтением
			 *
