...
namespace awh {
	...
	namespace codec {
		...
		namespace abc {
			 ...
			 *   НЕ СЛУЖИТ, хотя чтение своё и отвергает `EISDIR`: открытие идёт видом `r+b`,
			 *   и каталог отсекается раньше - проверка на нём проходила бы по неверной
			 *   причине. Перемотка в `load` - последняя рука: дойти до неё значит открыться,
