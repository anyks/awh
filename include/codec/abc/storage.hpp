...
namespace awh {
	...
	namespace codec {
		...
		namespace abc {
			 ...
			 * причин 05.09.2026 прошёл все шестнадцать мест отказа хранилища порознь.
			 * Двенадцать под сторожем, а семь молчат, и молчание их разбирается так:
			 * - **отказ носителя** - `fwrite`, `fflush`, `fsync`, `fseek`, `ferror` и замер
			 *   длины: наводятся лишь отказом самой файловой системы, поддельной же у
			 *   набора нет. Слепота законна, и закрывать её пришлось бы образом диска, цена
			 *   какого несоразмерна получаемому;
