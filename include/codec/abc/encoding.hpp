...
namespace awh {
	...
	namespace codec {
		...
		namespace abc {
			 ...
			 *
			 * @note Закреплено широко: сдвиг смещения расширения на лишний октет краснит
			 *       двадцать одну проверку - от `CodecAbcEncoding` до сборки, разбора и обоих
			 *       взглядов; у одиночного значения место сдвига закреплено прямо
			 *       (`CodecAbcEncoding.RealRoundtrip` сличает смещение с единицей). Проверено
			 *       подменою 04.09.2026
			 *
