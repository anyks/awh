...
namespace awh {
	...
	typedef class AWH_SHARED_EXPORT Watch {
		...
		public:
			 ...
			 *
			 * Выключенный таймер остаётся зарегистрированным в базе событий, поэтому его
			 * уведомитель закрывать нельзя: номер дескриптора достался бы новому сокету
			 *
			 * @param sock файловый дескриптор таймера
			 */
			void cancel(const SOCKET sock) noexcept;
			/**
