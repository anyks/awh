...
namespace awh {
	...
	typedef class AWH_SHARED_EXPORT Base {
		...
		private:
			...
			watch_t _watch;
		private:
			// Флаг отложенной очистки базы событий потоком опроса
			std::atomic_bool _defer;
			// Сокет уведомителя пробуждения опроса
			SOCKET _wakeup;
			/**
			 * Уведомитель пробуждения опроса: остановка из другого потока будит поток,
			 * ожидающий событий без таймаута, а очистку выполняет сам поток опроса
			 */
			notifier_t _wake;
		private:
			// Мютекс для блокировки потока
