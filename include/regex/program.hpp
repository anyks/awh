...
namespace awh {
	...
	namespace regex {
		...
		typedef struct __AWH_SHARED_EXPORT__ Program {
			...
			/**
			 * Набор адресов тел повторений одиночного символа
			 *
			 * @details Повторение одиночного символа либо класса символов компилируется
			 *          в переход по двум ветвям, тело повторения и переход к его началу,
			 *          отчего проход ряда подходящих символов обходится в три инструкции
			 *          на каждый символ. Набор помечает переход по двум ветвям, ветвь
			 *          повторения которого устроена именно так, адресом тела повторения,
			 *          благодаря чему исполнение проходит ряд одним ходом взамен
			 *          исполнения трёх инструкций на символ. Прочие переходы помечены
			 *          недействительным адресом.
			 *
			 */
			vector <address_t> runs;
			/**
			 * Набор адресов тел ленивых повторений одиночного символа
			 *
			 * @details Ленивое повторение компилируется тем же переходом по двум
			 *          ветвям, но ветви его переставлены: сопоставление продолжается
			 *          за повторением, а тело повторяется лишь по отказу продолжения.
			 *          Набор заведён отдельно от набора повторений жадных намеренно:
			 *          проход ряда одним ходом ленивому повторению неприменим, и
			 *          пометка в наборе общем прошла бы ряд жадно.
			 *
			 */
			vector <address_t> lazy;
			...
			void clear() noexcept {
				...
				this->strings.clear();
				// Выполняем очистку набора адресов тел повторений одиночного символа
				this->runs.clear();
				// Выполняем очистку набора адресов тел ленивых повторений
				this->lazy.clear();
