	...
	namespace regex {
		...
		typedef class __AWH_SHARED_EXPORT__ Assembly {
			...
			public:
				 ...
				 *
				 * @details Кодогенерация требует порождения машинного кода набора команд
				 *          выполняющего процессора, поэтому поддерживается не всякой
				 *          сборкой библиотеки. Отсутствие поддержки изъяном не является:
