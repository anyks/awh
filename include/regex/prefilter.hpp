...
namespace awh {
	...
	namespace regex {
		 ...
		 *
		 *          **Порог проверяется в точке перехода, а не на входе.** Прежде
		 *          он стоял входным условием, и текст короче его проходился поиском
		 *          обычным целиком - без окна, без счёта отвергнутых отборов
		 *          и без поиска якорного. Замером изъян этот вышел крупным:
		 *          «needle» по тексту в 4000 байтов стоило 533 наносекунды против
		 *          98 у эталона, отставание в 5.4 раза, и то же на тысяче
		 *          и на двух тысячах. Ныне окно проходится всегда, а порог решает
		 *          лишь одно - окупает ли **остаток** перехода на поиск якорный.
		 *
		 *          Величина порога от того сменилась с 4096 на 1024: прежняя
		 *          сличала пробу с проходом текста целиком, нынешняя - с проходом
		 *          остатка. Взята она замером: при 512 и 256 тексты около тысячи
		 *          байтов дорожают со 152 наносекунд до 195 - переход случается
		 *          там, где остатка на его окупаемость не хватает, - а при 2048
		 *          перехода не случается вовсе на двух тысячах. При 1024
		 *          «needle» по 4000 байтам стоит 281 наносекунду против 533,
		 *          по 2000 - 228 против 272, а тексты короче тысячи не меняются.
		 *
		 *          **Отдельно установлено и не взято:** проба, замененная парою
		 *          первого и последнего байтов без перебора вовсе, даёт ещё
		 *          228 наносекунд на 4000 байтах и 174 на 2000. Правка эта
		 *          не взята намеренно: она отменяет устройство намеренное, а
		 *          довода, что перебор пар своей платы не окупает, замером
		 *          не добыто - нужен текст, на каком пара первая вырождается,
		 *          а выбранная пробой - нет.
		 *
		 * \~english
		...
		AWH_REGEX_INLINE static size_t seek(string_view text, string_view what, const size_t pos) noexcept {
			...
			const size_t found = windowed(text.substr(0, (((bound + what.size()) - 1) < text.size()) ?
			 ((bound + what.size()) - 1) : text.size()), what, pos);
			/**
