...
namespace awh {
	...
	namespace regex {
		...
		/**
		 * \~russian
		 * @brief Количество отвергнутых отборов, окно поиска вырожденным признающее
		 *
		 * @details Отбор отвергнутый стоит поиска байта да сличения остатка -
		 *          единицы наносекунд, - тогда как переход на поиск якорный стоит
		 *          пробы в шестьдесят четыре байта. Предел потому и взят малым:
		 *          окно, отвергнувшее восемь отборов подряд, вырождено почти
		 *          наверняка, а плата за ошибку не превышает самой пробы.
		 *
		 *          Замером: «HTTP/» по ста тысячам байтов «H» стоило 8.1 микросекунды
		 *          против 1.7 на тексте постороннем, а с переходом по счёту - 1.77
		 *          при обоих текстах, тогда как эталон PCRE2 держится на 2.0.
		 *          Совпадение, окном покрытое, при этом не подорожало: замер
		 *          по длинному тексту дал 11.3 наносекунды против 11.6 прежних.
		 *          Закреплено проверкой «Regex.PrefilterDegenerate».
		 *
		 * \~english
		 * @brief Number of rejected selections at which the search window is deemed degenerate
		 *
		 * \~
		 */
		constexpr size_t REJECTS = 8;
		...
		AWH_REGEX_INLINE static size_t seek(string_view text, string_view what, const size_t pos) noexcept {
			...
			// Количество отборов отвергнутых, окно вырожденным признающее
			size_t rejected = 0;
			// Положение поиска байта первого искомой последовательности
			size_t at = pos;
			/**
			 * Выполняем поиск последовательности в окне по байту первому
			 *
			 * @details Окно проходится поиском байта первого с проверкой остатка
			 *          в каждой находке. Порядок этот дёшев, покуда байт первый
			 *          в тексте редок: совпадение, окном покрытое, обходится
			 *          в единицы наносекунд при всякой длине текста, ради чего
			 *          окно и заведено. На тексте же, байтом первым насыщенном,
			 *          он вырождается: сличение остатка платится в каждой позиции,
			 *          и замером «HTTP/» по ста тысячам байтов «H» стоило
			 *          8.1 микросекунды против 1.7 на тексте постороннем.
			 *
			 *          Вырождение опознаётся счётом отвергнутых отборов, а не
			 *          предсказанием: расположение байтов известно тексту, а не
			 *          выражению. Окно, счёт превысившее, уступает поиску якорному
			 *          немедля - тот отбирает байт, в пробе текста не встреченный,
			 *          и вырождения не имеет вовсе.
			 *
			 */
			while(at < bound) {
				// Выполняем поиск байта первого искомой последовательности
				const void * found = ::memchr((text.data() + at), what.front(), (bound - at));
				/**
				 * Если байт первый в окне поиска отсутствует
				 */
				if(found == nullptr)
					// Выходим из поиска последовательности в окне
					break;
				// Получаем положение найденного байта первого
				at = static_cast <size_t> (reinterpret_cast <const char *> (found) - text.data());
				/**
				 * Если искомая последовательность в тексте обнаружена
				 */
				if(::memcmp((text.data() + at), what.data(), what.size()) == 0)
					// Выводим положение обнаруженной последовательности
					return at;
				// Переходим к положению, за находкою следующему
				at++;
				/**
				 * Если окно поиска признано вырожденным
				 */
				if(++rejected > REJECTS)
					// Выводим результат поиска последовательности поиском якорным
					return anchored(text, what, at);
			}
