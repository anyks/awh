...
namespace awh {
	...
	namespace unit {
		 ...
		 *
		 * @warning Шаткость обмена по договору UPnP на длинном перечне вызвана устройством,
		 * а не модулем: маршрутизатор отвечает `Connection: close`, требуя подключения на
		 * каждое перенаправление, и часть подключений роняет. Повторы ведёт само событие
		 *
		 */
