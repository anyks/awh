...
namespace awh {
	...
	namespace unit {
		 ...
		 *
		 * - **Срок ожидания ответа обращается пределом простоя.** Движок держит его
		 *   ожиданием ответа: отправили - пошёл отсчёт, прочли - отсчёт снят, и заводится
		 *   он заново лишь следующей отправкой. Модулю такое не годится: и на общий порт
		 *   дейтаграммного договора, и на групповую рассылку приходят чужие ответы, а они
		 *   отбрасываются - отправлять же повторно на этом шаге нечего, и снятый чужой
		 *   дейтаграммой срок оставил бы обмен без итога вовсе. Оттого всем событиям обмена
		 *   выставляется режим `usage_t::REUSABLE`.
		 *
		...
		typedef class __AWH_SHARED_EXPORT__ Portmap : public unit_t {
			...
			public:
				...
				typedef struct __AWH_SHARED_EXPORT__ Mapping {
					 ...
					 *
					 * @warning Проверить работу с опознавателем на живом устройстве не
					 * удалось: маршрутизатор, на котором отлаживался модуль, службы заслона
					 * IPv6 не выдаёт вовсе
					 */
					uint16_t pinhole;
					/**
