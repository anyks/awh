		...
		typedef class __AWH_SHARED_EXPORT__ Server : public unit_t {
			 ...
			 */
				...
				private:
					...
					/**
					 * Придержанная передача слушающих событий, пришедшая от мастера
					 *
					 * @details Передача приходит работнику раньше, чем тот заводит свои
					 *          события: заведение отложено разрешением имени узла, а
					 *          мастер отвечает сразу. Оттого пришедшее придерживается
					 *          здесь, покуда за ним не придут - и приходят за ним оба
					 *          пути: и приход передачи, и отсрочка фиксации
					 *
					 * @note Запись, доставшаяся событию, из списка убирается: снимок
					 *       годен одному подъёму, и второму событию того же устройства
					 *       он не достанется
					 */
					vector <pair <vector <uint8_t>, vector <uint8_t>>> _handoverParts;
					/**
					 * Список слушающих событий работника, ждущих запуска работы
					 *
					 * @details Запуск работы события у работника откладывается тем же
					 *          доводом, что фиксация и прослушивание: отклик о запуске
					 *          юнита приходит задолго до снимка, и запускать тогда
					 *          нечего - сокета у события ещё нет
					 *
					 * @note Запуск случается следом за успешным прослушиванием, а не
					 *       прямо по приходу снимка: движку он отдаёт событие уже
					 *       слушающим, тем же порядком, каким его отдаёт мастер
					 */
					unordered_set <event::id_t> _handoverLaunch;
			#endif
			private:
			 ...
			 */
				...
				private:
					 ...
					 *
					 * @details Зовётся с обоих путей: приходом передачи от мастера и
					 *          отсрочкой фиксации своего события. Который из них случится
					 *          первым, заранее не известно - заведение событий отложено
					 *          разрешением имени узла, - оттого раздача и не привязана ни
					 *          к одному из них
					 *
					 * \~english
					 * @brief Method of the distribution of the held handover to the events awaiting it
					 *
					 * \~
					 */
					void handoverApply() noexcept;
					/**
					 * \~russian
