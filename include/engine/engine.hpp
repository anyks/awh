...
namespace awh {
	...
	typedef class Engine {
		public:
			 ...
			 */
			// Обратный вызов при закрытии события
			using closeCallback = function <void (const event::id_t)>;
			// Обратный вызов события пользователя
			using userEventCallback = function <void (const uint32_t)>;
			// Обратный вызов при создании события
			using createCallback = function <void (const event::id_t)>;
			// Обратный вызов при удалении события
			using destroyCallback = function <void (const event::id_t)>;
		...
		public:
			 ...
			 */
			virtual void acceptDepth(const event::id_t id, const uint32_t depth, const bool adaptive = false) noexcept = 0;
		public:
			/**
		...
		public:
			...
			/**
			 * @brief Методы установки функции обратного вызова на создание события
			 *
			 * @param id идентификатор события
			 * @param cb объект обратного вызова события
			 */
			virtual void on(const event::id_t id, const createCallback & cb) noexcept = 0;
			/**
			 * @brief Методы установки функции обратного вызова на удаление события
			 *
			 * @param id идентификатор события
			 * @param cb объект обратного вызова события
			 */
			virtual void on(const event::id_t id, const destroyCallback & cb) noexcept = 0;
			/**
			 * @brief Методы установки функции обратного вызова на закрытие события
			 *
			 * @param id идентификатор события
			 * @param cb объект обратного вызова события
			 */
			virtual void on(const event::id_t id, const closeCallback & cb) noexcept = 0;
			/**
		...
		public:
			 ...
			 */
			Engine(const fmk_t * fmk, const log_t * log) noexcept : _net(fmk, log), _fmk(fmk), _log(log) {}
			/**
			 * @brief Деструктор
			 *
			 */
