...
namespace awh {
	...
	namespace event {
		...
		enum class node_t : uint8_t {
			...
			CLIENT = 0x01, // Клиентский узел
			SERVER = 0x02  // Серверный узел
		};
		/**
		...
		enum class status_t : uint8_t {
			SUCCESS   = 0x00, // Операция выполнена успешно
			FAILURE   = 0x01, // Операция завершилась неудачей
			PENDING   = 0x02, // Операция в ожидании
			CANCELLED = 0x03  // Операция отменена
		};
		/**
		...
		enum class family_t : uint8_t {
			...
			DIR      = 0x04, // Семейство директорий
			FILE     = 0x05, // Файловое семейство сокетов
			IPV4     = 0x06, // Семейство сокетов IPv4
			IPV6     = 0x07, // Семейство сокетов IPv6
			TIMER    = 0x08, // Семейство таймеров
			INTERVAL = 0x09  // Семейство интервалов
		};
		/**
		 * @brief Флаги сокетов
		 *
