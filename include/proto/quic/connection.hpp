...
namespace awh {
	...
	namespace quic {
		...
		typedef class __AWH_SHARED_EXPORT__ Connection {
			...
			private:
				// Количество неудачных снятий защиты пакетов (лимит целостности AEAD, RFC 9001 §6.6)
				uint64_t _aeadFailures;
				// Предельное число пакетов на одном ключе (лимит конфиденциальности AEAD, RFC 9001 §6.6)
				uint64_t _confidentialityLimit;
				// Предельное число неудачных снятий защиты (лимит целостности AEAD, RFC 9001 §6.6)
				uint64_t _integrityLimit;
			private:
				...
				bool _handshakeDone;
				/**
				 * Флаг обработанного отказа удалённого узла в ранних данных. Хендшейк-машина
				 * держит признак отказа до конца соединения, а возврат отправленных ранних
				 * данных в очереди выполняется однократно (RFC 9001 §4.6.2)
				 */
				bool _restored;
			private:
			...
			private:
				...
				uint64_t _idleTime;
				/**
				 * Флаг отправки ack-eliciting пакета после последнего принятого. Отсчёт
				 * таймаута простоя перезапускается такой отправкой лишь однократно:
				 * иначе односторонняя отправка удерживала бы соединение живым сколь
				 * угодно долго при молчащем удалённом эндпоинте (RFC 9000 §10.1)
				 */
				bool _idleElicited;
			private:
