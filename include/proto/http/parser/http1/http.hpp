	...
	namespace http {
		...
		typedef class __AWH_SHARED_EXPORT__ Parser_HTTP : public parser_t {
			...
			public:
				 ...
				 *
				 * @note Режим прокси ужесточает кадрирование принимаемых ответов: узел,
				 *       передающий сообщение дальше по цепочке, обязан отвергать то,
				 *       что следующее звено может истолковать иначе. Подробности -
				 *       в описании limits_t
				 *
