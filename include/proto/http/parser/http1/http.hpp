		...
		typedef class __AWH_SHARED_EXPORT__ Parser_HTTP : public parser_t {
			...
			public:
				...
				enum class error_t : uint8_t {
					...
					MISSING_HOST              = 0x15  // У запроса HTTP/1.1 отсутствует либо продублирован заголовок Host
				};
			public:
				/**
			...
			public:
				 ...
				 *       уже нечего. Попытка отправить блок трейлеров после исчерпания
				 *       источника отбрасывается с записью в лог. Если трейлеры нужны -
				 *       выдавайте тело методом sendData, не закрывая сообщение флагом
				 *       endStream, и завершайте его блоком трейлеров.
				 *
			...
			public:
				 ...
				 *          (лишний заголовок вычищается с записью в лог), а при кадрировании
				 *          chunked заголовок Transfer-Encoding гарантированно заканчивается
				 *          токеном chunked. Это исключает генерацию кадров, которые принимающая
				 *          сторона обязана отвергнуть как request smuggling.
				 *
