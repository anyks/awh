	...
	namespace http {
		...
		typedef class __AWH_SHARED_EXPORT__ Parser_HTTP3 : public parser_t {
			...
			private:
				...
				/**
				 * Идентификаторы отменённых push, поток которых ещё не пришёл
				 *
				 * @details Отмена обгоняет поток: кадр CANCEL_PUSH идёт управляющим потоком,
				 *          а сам push - своим, и порядок между ними не задан. Множество,
				 *          а не список: повторять отмену одного обещания пиру не запрещено,
				 *          и списком она наращивала бы память соединения без предела
				 *
				 */
				unordered_set <uint64_t> _cancelledPush;
			private:
