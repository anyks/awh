		...
		typedef class __AWH_SHARED_EXPORT__ Connection {
			...
			private:
				...
				typedef struct __AWH_SHARED_EXPORT__ Streams {
					...
					/**
					 * Идентификаторы потоков с данными к отправке. В отличие от списков
					 * `readable` и `control`, порядок здесь значим - обход идёт круговым
					 * `lower_bound` по курсору, поэтому нужен упорядоченный набор, а не
					 * вектор. Пополняется при постановке данных (send) и при разблокировке
					 * потока лимитом (MAX_STREAM_DATA), а устаревшие, слитые и заблокированные
					 * записи отсеиваются по месту при обходе. Обход всего списка потоков на
					 * каждый пакет не окупается: у соединения без активных передач он пуст
					 */
					set <uint64_t> writable;
					/**
