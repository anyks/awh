 ...
 *
 * @brief Заголовочный файл конечного автомата соединения QUIC — класс quic2::Connection,
...
#ifndef __AWH_PROTO_QUIC2_CONNECTION__
#define __AWH_PROTO_QUIC2_CONNECTION__
...
namespace awh {
	...
	namespace quic2 {
		...
		typedef class __AWH_SHARED_EXPORT__ Connection {
			...
			private:
				/**
				 * Предел несобранного буфера отправки одного потока в октетах (backpressure).
				 * Данные сверх этого объёма от приложения не принимаются - send() возвращает
				 * BLOCKED. По умолчанию рассчитан на реальную сеть; на синтетических стендах с
				 * RTT≈0 меньший предел резко снижает удержание памяти без потери пропускной
				 * способности. Настраивается методом sendBuffer() (см. [[awh-framework-prefer-flags]])
				 */
				size_t _sendBuffer = (256 * 1024);
			private:
				/**
			...
			public:
				 ...
				 *
				 * @note Ограничивает объём несобранных данных, которые приложение может
				 *       поставить в очередь одного потока: сверх предела send() возвращает
				 *       BLOCKED, и приложение дописывает данные по мере дренирования. Больше,
				 ...
				 *       Значение ноль снимает ограничение (буфер растёт по мере поступления)
				 *
				 * @param size предел буфера отправки потока в октетах
				 *
				 */
				void sendBuffer(const size_t size) noexcept;
			private:
				/**
			...
			public:
				 ...
				 *
				 */
				void params(const quic2::params::params_t & params) noexcept;
				/**
...
#endif // __AWH_PROTO_QUIC2_CONNECTION__
