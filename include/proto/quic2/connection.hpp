	...
	namespace quic2 {
		...
		typedef class __AWH_SHARED_EXPORT__ Connection {
			...
			private:
				 ...
				 * число принятых байт (частичный приём); остаток дописывает приложение. Ноль
				 * снимает ограничение (буфер растёт по мере поступления). По умолчанию рассчитан
				 * на реальную сеть; на синтетических стендах с RTT≈0 меньшая метка резко снижает
				 * удержание памяти без потери пропускной способности. Настраивается sendWaterMarks()
				 */
				size_t _sendHigh = (256 * 1024);
				/**
				 ...
				 * половина верхней метки: приложение возобновляет выдачу, не дожидаясь полного
				 * опустошения буфера, что сохраняет непрерывность передачи. Настраивается sendWaterMarks()
				 */
				size_t _sendLow = (128 * 1024);
			private:
				/**
