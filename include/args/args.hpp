	...
	namespace args {
		...
		typedef class __AWH_SHARED_EXPORT__ Args {
			...
			private:
				 ...
				 *
				 * @warning Объявлен НЕконстантным намеренно: ход подмены целевого файла
				 *          временным `replaceAddress()` состояние объекта меняет, и у
				 *          константного поля он недоступен вовсе. Прочие потребные ходы
				 *          - `type()`, `read()`, `write()`, `size()` - константны, и до
				 *          заведения подмены поле стояло константным
				 */
				fs_t _fs;
			private:
