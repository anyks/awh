	...
	namespace args {
		...
		typedef class __AWH_SHARED_EXPORT__ Lexer {
			...
			public:
				 ...
				 *
				 * @note Правила взяты у оболочек POSIX: внутри кавычки ОДИНАРНОЙ косая
				 *       особого значения не имеет и берётся знаком как есть, ибо кавычку
				 *       эту ставят именно затем, чтобы уберечь косые - пути MS Windows
				 *       и выражения регулярные. Внутри кавычки ДВОЙНОЙ косая работает,
				 *       иначе саму кавычку двойную внести было бы нечем
				 *
				 * @note Последовательностей особых косая не разбирает вовсе: запись
				 *       «\n» даёт знак «n», а не перевод строки. Снимается лишь особое
				 *       значение знака, и ничего не подставляется взамен
				 *
				 * @note Кавычки пустые дают слово ПУСТОЕ, а не отсутствие слова: иначе
				 *       подать довод пустой было бы нечем
				 *
