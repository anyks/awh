...
namespace awh {
	...
	namespace server {
		...
		typedef class AWH_SHARED_EXPORT Websocket1 : public web_t {
			...
			private:
				/**
				 * Байты фреймов брокеров, ожидающие места в буфере отправки сетевого ядра. Сообщение
				 * ставится в поток отправки целиком или не ставится вовсе, поэтому на провод никогда
				 * не уходит оборванная последовательность фрагментов
				 */
				std::map <uint64_t, std::unique_ptr <awh::buffer_t>> _outgoing;
			private:
				/**
			...
			private:
				 ...
				 */
				size_t available(const uint64_t bid) const noexcept;
				/**
				 * @brief Метод постановки байт в поток отправки брокера
				 *
				 * Буфер ставится целиком или не ставится вовсе: то, что не помещается в буфер
				 * сетевого ядра, ожидает в очереди модуля и досылается по событию записи
				 *
				 * @param bid    идентификатор брокера
				 ...
				 */
				bool transmit(const uint64_t bid, const char * buffer, const size_t size) noexcept;
				/**
				 ...
				 */
				void flush(const uint64_t bid, const size_t size) noexcept;
			private:
				/**
