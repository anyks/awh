...
namespace awh {
	...
	namespace event {
		...
		enum class status_t : uint8_t {
			...
			LAUNCHED	    = 0x04, // Статус выполнения
			...
			GARBAGE         = 0x0A, // Статус мусора
			LISTENING       = 0x0B, // Статус прослушивания
			CONNECTED       = 0x0C, // Статус подключено
			CANCELLED       = 0x0D, // Операция отменена
			RECONNECTED     = 0x0E, // Статус переподключения
			QUEUE_OVERFLOW  = 0x0F, // Статус переполнения очереди
			QUEUE_AVAILABLE = 0x10  // Статус доступности очереди
		};
		/**
