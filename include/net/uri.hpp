...
namespace awh {
	...
	typedef class __AWH_SHARED_EXPORT__ Uniform_Resource_Identifier {
		...
		private:
			/**
			 * Признак наличия авторити у URI
			 *
			 * @details Авторити отделяется двумя косыми чертами, и признаком её наличия
			 *          служили заведённые атрибуты адреса. Авторити, однако, бывает и
			 *          пустой - записи "file:///etc/hosts" и "http:///path" несут пустой
			 *          хост с путём от корня, - и атрибутов ей не заводится. Наличие её
			 *          поэтому запоминается отдельно (RFC 3986 5.3)
			 *
			 */
			bool _authority;
		private:
