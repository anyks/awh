...
namespace awh {
	...
	typedef class __AWH_SHARED_EXPORT__ Engine {
		...
		public:
			 ...
			 */
				...
				public:
					 ...
					 * \~
					 */
					virtual eth::iface_t::driver_t getDriver() const noexcept = 0;
					/**
					 * \~russian
					 ...
					 *
					 * @details There are two drivers at MS Windows, and they carry different things: Wintun transfers
					 *          the packets of the network level by a ring in the memory shared with the driver, and
					 *          tap-windows6 — the frames of the channel level through a file descriptor.
					 *          The choice by default (`AUTO`) is given to the module: for the packets of the network level Wintun is preferred, if it is available.
					 *
					 * @note The driver is to be set BEFORE the starting of the device: the choice is made once, at the creation
					 *
					 * @param driver driver of the tunnel devices to set
					 *
					 * \~
					 */
					virtual void setDriver(const eth::iface_t::driver_t driver) noexcept = 0;
			#endif
		public:
			/**
			 * \~russian
		...
		public:
			 ...
			 *          есть путь
			 *
			 * @warning СНЯТИЕ СНИМКА ОТЧУЖДАЕТ ОПИСАТЕЛЬ, и передумать после него нельзя
			 *          НИГДЕ. Передающее событие отвязывается от очереди оповещений и
			 *          сбрасывается в исходное состояние: оповещений по нему больше не
			 *          придёт, и всякий иной порядок работы с ним становится незаконным.
			 *          Захотите вернуть его в дело - проходите обычный путь заведения,
			 *          `commit` и `launch`, как для только что созданного события.
			 *
			 *          Насколько отчуждение окончательно, у наречий РАЗНИТСЯ, и полагаться
			 *          на обратимость нельзя: у наречий POSIX снятие лишь ПОМЕЧАЕТ передачу,
			 *          и не пошли вызывающая сторона ни одного сообщения - описатель уедет
			 *          не успеет; у наречия MS Windows снятие её СОВЕРШАЕТ, и описатель к
			 *          этому мгновению уже поднят у получателя. Код, писанный в расчёте на
			 *          обратимость, работает у одних и молча теряет описатель у других
			 *
			 * @warning ПОРЯДОК ОБЯЗАТЕЛЕН: сперва снятие снимка, затем посылка сообщения
			 *          по событию `dest`. У наречий POSIX описатель уносит БЛИЖАЙШЕЕ
			 *          исходящее сообщение события-переносчика, и посылка, случившаяся
			 *          прежде снятия, не унесёт ничего; у наречия MS Windows описатель
			 *          едет внутри самих октетов снимка, и порядок там безразличен.
			 *          Записан строгий порядок: соблюдающий его код работает у всех
			 *
