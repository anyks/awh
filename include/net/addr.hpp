...
namespace awh {
	 ...
	 *
	 * @warning Копирующего конструктора у класса **нет**, и завести объект копией
	 *          другого нельзя. Перенести адрес позволяет присваивание, но переносит
	 *          оно только сам адрес: зона IPv6 и настройки объекта остаются
	 *          прежними. Хранить адреса в наборах, требующих копирования элементов,
	 *          поэтому не выйдет - подойдут `std::vector` и умный указатель
	 *
	 ...
	 *     awh::net_addr_t network(&fmk, &log);
	 *     // Переносим адрес присваиванием - копией объект завести нельзя
	 *     network = addr;
	...
	typedef class __AWH_SHARED_EXPORT__ Network_Address {
		public:
			 ...
			 *          в числовом виде. `BIG` - сетевой порядок, тот, в котором адрес
			 *          идёт по проводу и в котором его ждут системные вызовы. `LITTLE`
			 *          - порядок, принятый на большинстве машин
			 *
		...
		public:
			 ...
			 *
			 * @details Опустошает объект: сбрасывается сам адрес и пометка о его виде
			 *
			 * @note Настройки объекта очистка не трогает - строгий режим остаётся
			 *       выставленным, и объект годится для повторного разбора без
			 *       перенастройки
			 *
			 * @warning Зона IPv6 очисткой **не сбрасывается** и переживает её. Разбор
			 *          адреса IPv6 зону перезаписывает, но адрес иного вида, разобранный
			 *          после очистки, унаследует зону от прежнего содержимого
			 *
			 */
			void clear() noexcept;
		public:
		...
		public:
			 ...
			 *          `type()`, а не по возвращённому числу
			 *
		...
		public:
			 ...
			 *
			 * @warning Неудача **очищает** объект, а не сохраняет прежнее содержимое.
			 *          Разбор поверх уже заполненного объекта отрицательным итогом
			 *          оставит его пустым
			 *
		...
		public:
			 ...
			 *       запись пуста, не нужно: таких адресов не бывает
			 *
		...
		public:
			 ...
			 *
			 * @details Переносит адрес из другого объекта. Поскольку копирующего
			 *          конструктора у класса нет, присваивание - единственный способ
			 *          завести второй объект с тем же адресом
			 *
			 * @warning Переносится **только сам адрес** и его вид. Зона IPv6 и
			 *          строгий режим остаются у получателя прежними, а не берутся у
			 *          источника: адрес локальной связи, перенесённый присваиванием,
			 *          зону свою потеряет
			 *
