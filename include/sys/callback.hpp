...
namespace awh {
	...
	class Callback {
		...
		private:
			 ...
			 *
			 * @note Блокировки захватываются в детерминированном порядке (по адресу объекта),
			 * @note что исключает взаимоблокировку (deadlock) при встречных операциях над двумя контейнерами.
			 *
