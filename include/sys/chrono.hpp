...
namespace awh {
	...
	typedef class __AWH_SHARED_EXPORT__ Chrono {
		private:
			 ...
			 *          записывает, и разбор подставляет текущий, а если получившаяся
			 *          дата оказывается в будущем - предыдущий. Действующему RFC 5424
		...
		public:
			 ...
			 *          попадали бы в 1970-й год. Если подстановка текущего года относит запись
			 *          в будущее, берётся предыдущий год - так же поступают rsyslog и
			 *          syslog-ng. Вторая половина обеспечивает устойчивость: один и тот же
			 *          довод даёт один и тот же результат вне зависимости от момента вызова.
			 *
			 ...
			 *       и даёт 14 февраля 2026 года. Дата вне поддерживаемого диапазона 1970-2100
			 *       годов даёт бессмысленный штамп: год до 1970-го уходит за ноль при
			 *       вычитании начала эпохи
			 *
