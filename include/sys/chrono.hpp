...
namespace awh {
	 ...
	 *
	 * @details Смещение зоны окружения при формировании записи по штампу времени и при
	 *          разборе записи без зоны берётся на момент самой даты (как localtime), а не
	 *          на текущий момент: зимняя дата, записанная летом, иначе уезжает на час
	 *
	 */
	typedef class AWH_SHARED_EXPORT Chrono {
		...
		public:
			 ...
			 *          текущему моменту системных часов и сбрасывает временную зону,
			 *          выставленную методом setTimeZone. Реестр временных зон не
		...
		public:
			 ...
			 *          меньше единицы; отрицательная записывается со знаком. Дробная часть
			 *          округляется до сотых (30 суток дают 4.29w), величины мельче сотой доли
			 *          единицы выводятся без округления. Месяц берётся средней длительности
			 *          (30.436875 суток) наравне с обратным разбором
			 *
			 ...
			 *
			 * @details Обозначение занимает запись целиком: число обязано стоять в её начале,
			 ...
			 *          Составные записи вида «1h30m» не предусмотрены и дают ноль
			 *
		...
		public:
			 ...
			 *
			 * @details Запись, проверку пригодности (validate) не прошедшая, даёт пустую
			 *          строку. Нулевой штамп времени - законная дата 1 января 1970 года
			 *
