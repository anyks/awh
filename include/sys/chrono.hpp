 ...
 *          переменных в записи произволен.
	...
	typedef class __AWH_SHARED_EXPORT__ Chrono {
		...
		private:
			/**
			 * @brief Запомненное смещение временной зоны окружения
			 *
			 * @details Смещение это меняется лишь при переходе на летнее время либо при
			 *          правке переменной окружения TZ: первое происходит на границе
			 *          секунды, второе ловится сличением самой переменной, поэтому
			 *          запомненное смещение годно ровно в пределах той секунды, в которую
			 *          снято. Спрос его у системы обходится дороже, чем всё остальное
			 *          формирование записи вместе взятое
			 *
			 */
			// Смещение временной зоны окружения, снятое в последний раз
			mutable int32_t _zoneOffset;
			// Секунда, в которую снято смещение временной зоны окружения
			mutable time_t _zoneStamp;
			// Признак того, что смещение временной зоны окружения снято хотя бы раз
			mutable bool _zoneCached;
			// Значение переменной окружения TZ, при котором снято смещение
			mutable string _zoneName;
		private:
