...
namespace awh {
	...
	typedef class AWH_SHARED_EXPORT Hash {
		...
		private:
			// Максимальный размер извлекаемых данных при декомпрессии
			size_t _limitDecompress;
		private:
		...
		public:
			 ...
			 *
			 * По умолчанию установлен размер AWH_MAX_BODY_SIZE. Если при декомпрессии любым
			 * методом объём извлечённых данных превышает этот размер, декомпрессия считается
			 * неудачной и возвращается пустой результат (защита от архивных бомб).
			 * Значение 0 снимает ограничение.
			 *
			 * @param size максимальный размер извлекаемых данных в байтах
			 */
			void limitDecompress(const size_t size) noexcept;
		public:
			/**
			 * @brief Конструктор
			 *
