...
void awh::server::Core::createTimeout(const uint16_t sid, const uint32_t bid, const uint32_t msec, const mode_t mode) noexcept {
	...
	if((bid > 0) && (mode != mode_t::NONE) && (msec > 0)){
		...
		if(this->_timer == nullptr){
			// Выполняем блокировку потока
			const lock_guard lock1(this->_mtx.receive);
			// Выполняем блокировку потока
			const lock_guard lock2(this->_mtx.timeout);
...
void awh::server::Core::clusterEventsCallback(const uint16_t sid, const pid_t pid, const cluster_t::event_t event) noexcept {
	...
	if(i != this->_schemes.end()){
		...
		if(this->_callback.is("clusterEvents"))
			// Выполняем функцию обратного вызова
			this->_callback.call <void (const cluster_t::family_t, const uint16_t, const pid_t, const cluster_t::event_t)> ("clusterEvents", family, sid, pid, event);
	}
}
/**
