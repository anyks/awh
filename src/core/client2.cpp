		...
		if((shm->status.real == scheme_t::mode_t::DISCONNECT) && (shm->status.work == scheme_t::work_t::ALLOW)){
			...
			try {
				...
				if(family == scheme_t::family_t::IPC){
					...
					broker->addr.init(this->_fmk->format("%s/%s.sock", this->_settings.sockpath.c_str(), this->_settings.sockname.c_str()), engine_t::type_t::CLIENT);
				// Если unix-сокет не используется, выполняем инициализацию сокета
				} else broker->addr.init(url.ip, url.port, (family == scheme_t::family_t::IPV6 ? AF_INET6 : AF_INET), engine_t::type_t::CLIENT);
					...
					// Если подключение выполняется не по защищённому каналу DTLS
						...
						if(host != nullptr){
							...
							if(!shm->isProxy() && this->_callback.is("ssl"))
								...
								this->_engine.encrypted(this->_callback.call <bool (const uri_t::url_t &, const uint32_t, const uint16_t)> ("ssl", url, broker->id(), sid), broker->ectx);
						...
						// Если хост сервера не получен
							...
							if(this->_callback.is("disconnect"))
								// Выполняем функцию обратного вызова
								this->_callback.call <void (const uint32_t, const uint16_t)> ("disconnect", 0, sid);
							// Выходим из функции
							return;
						}
					}
					...
					if(shm->status.real == scheme_t::mode_t::PRECONNECT){
						...
						if(this->_info){
							...
							if(family == scheme_t::family_t::IPC)
								...
								this->_log->print("Server host [%s/%s.sock] SOCKET=%d is good", log_t::flag_t::INFO, this->_settings.sockpath.c_str(), this->_settings.sockname.c_str(), ret.first->second->addr.sock);
							// Если доменное имя используется
								...
								this->_log->print("Server host %s [%s:%d] SOCKET=%d is good", log_t::flag_t::INFO, url.domain.c_str(), url.ip.c_str(), url.port, ret.first->second->addr.sock);
							// Выводим ионформацию об удачном подключении к серверу по хосту и порту
							else this->_log->print("Server host [%s:%d] SOCKET=%d is good", log_t::flag_t::INFO, url.ip.c_str(), url.port, ret.first->second->addr.sock);
						}
					}
					// Выходим из функции
					return;
				// Если сокет не создан, выводим в консоль информацию
					...
					if(family == scheme_t::family_t::IPC){
						...
						this->_log->print("Server host %s/%s.sock is bad", log_t::flag_t::CRITICAL, this->_settings.sockpath.c_str(), this->_settings.sockname.c_str());
						// Если функция обратного вызова установлена
						if(this->_callback.is("error"))
							// Выполняем функцию обратного вызова
							this->_callback.call <void (const log_t::flag_t, const error_t, const string &)> ("error", log_t::flag_t::CRITICAL, error_t::CONNECT, this->_fmk->format("Server host %s/%s.sock is bad", this->_settings.sockpath.c_str(), this->_settings.sockname.c_str()));
					// Если доменное имя используется
					} else if(!url.domain.empty()) {
						// Выводим ионформацию об неудачном подключении к серверу по хосту и порту
						this->_log->print("Server host %s [%s:%u] is bad", log_t::flag_t::CRITICAL, url.domain.c_str(), url.ip.c_str(), url.port);
						// Если функция обратного вызова установлена
						if(this->_callback.is("error"))
							// Выполняем функцию обратного вызова
							this->_callback.call <void (const log_t::flag_t, const error_t, const string &)> ("error", log_t::flag_t::CRITICAL, error_t::CONNECT, this->_fmk->format("Server host %s [%s:%u] is bad", url.domain.c_str(), url.ip.c_str(), url.port));
					// Если доменное имя не используется
					} else {
						// Выводим ионформацию об неудачном подключении к серверу по хосту и порту
						this->_log->print("Server host [%s:%u] is bad", log_t::flag_t::CRITICAL, url.ip.c_str(), url.port);
						// Если функция обратного вызова установлена
						if(this->_callback.is("error"))
							// Выполняем функцию обратного вызова
							this->_callback.call <void (const log_t::flag_t, const error_t, const string &)> ("error", log_t::flag_t::CRITICAL, error_t::CONNECT, this->_fmk->format("Server host [%s:%u] is bad", url.ip.c_str(), url.port));
					}
				}
 ...
 */
void awh::client::Core::createTimeout(const uint32_t bid, const uint32_t msec) noexcept {
	...
	if(this->_busy.find(bid) == this->_busy.end()){
		...
		if(this->has(bid)){
			...
			if(i != this->_schemes.end()){
				...
				if(this->_callback.is("disconnect"))
					// Устанавливаем полученную функцию обратного вызова
					callback.on <void ()> ("disconnect", this->_callback.get <void (const uint32_t, const uint16_t)> ("disconnect"), bid, i->first);
 ...
 * @param bid идентификатор брокера
 */
void awh::client::Core::switchProxy(const uint32_t bid) noexcept {
	/**
	...
	if(this->has(bid)){
		...
		if(i != this->_schemes.end()){
			...
			if((shm->proxy.type != proxy_t::type_t::NONE) && shm->isProxy()){
				...
				const char * host = nullptr;
				// Если unix-сокет используется
				if(shm->proxy.family == scheme_t::family_t::IPC)
					// Выполняем получение хост сервера
				...
				if(host != nullptr){
					...
					if(this->_callback.is("ssl"))
						// Выполняем активацию шифрованного SSL канала
						this->_engine.encrypted(this->_callback.call <bool (const uri_t::url_t &, const uint32_t, const uint16_t)> ("ssl", shm->url, bid, i->first), broker->ectx);
					// Выполняем получение контекста сертификата
 ...
 * @param bid идентификатор брокера
 */
void awh::client::Core::connected(const uint32_t bid) noexcept {
	// Если идентификатор брокера подключений существует
	if(this->has(bid)){
		// Создаём бъект активного брокера подключения
		awh::scheme_t::broker_t * broker = const_cast <awh::scheme_t::broker_t *> (this->broker(bid));
			...
			if(shm->status.work == scheme_t::work_t::ALLOW){
				...
				if(shm->isProxy()){
					...
					if(this->_callback.is("connectProxy"))
						// Выполняем функцию обратного вызова
						this->_callback.call <void (const uint32_t, const uint16_t)> ("connectProxy", bid, i->first);
				// Если функция обратного вызова установлена
				} else if(this->_callback.is("connect"))
					// Выполняем функцию обратного вызова
					this->_callback.call <void (const uint32_t, const uint16_t)> ("connect", bid, i->first);
				// Выходим из функции
				return;
			}
			// Выполняем отключение от сервера
 ...
 */
bool awh::client::Core::send(const char * buffer, const size_t size, const uint32_t bid) noexcept {
 ...
 * @param bid идентификатор брокера
 */
void awh::client::Core::read(const uint32_t bid) noexcept {
	// Если данные переданы
	if(this->working() && this->has(bid)){
		// Создаём бъект активного брокера подключения
		awh::scheme_t::broker_t * broker = const_cast <awh::scheme_t::broker_t *> (this->broker(bid));
				...
				if((shm->receiving = (shm->status.real == scheme_t::mode_t::CONNECT))){
					...
					do {
						...
						if((broker->payload.size > 0) && (shm->status.real == scheme_t::mode_t::CONNECT)){
							...
							if(bytes > 0){
								// Если таймер ожидания получения данных установлен
								if((broker->timeouts.wait > 0) || (broker->timeouts.read > 0))
									// Выполняем удаление таймаута
									this->clearTimeout(bid);
								// Если подключение производится через, прокси-сервер
								if(shm->isProxy()){
									...
									if(this->_callback.is("readProxy"))
										...
										this->_callback.call <void (const char *, const size_t, const uint32_t, const uint16_t)> ("readProxy", broker->payload.buffer.get(), static_cast <size_t> (bytes), bid, i->first);
					...
					if(this->has(bid)){
						...
						if(broker->timeouts.wait > 0)
							...
							this->createTimeout(bid, static_cast <uint32_t> (broker->timeouts.wait) * 1000);
 ...
 * @param bid идентификатор брокера
 */
void awh::client::Core::write(const uint32_t bid) noexcept {
	// Если данные переданы
	if(this->working() && this->has(bid)){
		// Создаём бъект активного брокера подключения
		awh::scheme_t::broker_t * broker = const_cast <awh::scheme_t::broker_t *> (this->broker(bid));
 ...
 */
size_t awh::client::Core::write(const char * buffer, const size_t size, const uint32_t bid) noexcept {
	// Результат работы функции
	size_t result = 0;
	// Если данные переданы
				...
				if((shm->receiving = (shm->status.real == scheme_t::mode_t::CONNECT))){
					...
					if(max > 0){
						...
						if(bytes > 0){
							...
							if(broker->timeouts.read > 0)
								// Выполняем создание таймаута ожидания получения данных
								this->createTimeout(bid, static_cast <uint32_t> (broker->timeouts.read) * 1000);
						...
						if(bytes > 0){
							...
							if(shm->isProxy()){
								...
								if(this->_callback.is("writeProxy"))
									// Выводим функцию обратного вызова
									this->_callback.call <void (const char *, const size_t, const uint32_t, const uint16_t)> ("writeProxy", buffer, static_cast <size_t> (bytes), bid, i->first);
