 ...
 */
awh::buffer_t & awh::Web::dump() noexcept {
	// Выполняем сброс данных временного буфера
	this->_buffer.clear();
	// Устанавливаем идентификатор HTTP-модуля
	this->_buffer.push(this->_id);
	// Устанавливаем тип используемого HTTP-модуля
	this->_buffer.push(static_cast <uint8_t> (this->_hid));
	// Устанавливаем объект рабочих параметров
	this->_buffer.push(&this->_work, sizeof(this->_work));
	// Устанавливаем код ответа на HTTP-ответа
	this->_buffer.push(this->_response.code);
	// Устанавливаем версию протокола HTTP-ответа
	this->_buffer.push(this->_response.version);
	// Устанавливаем метод HTTP-запроса
	this->_buffer.push(static_cast <uint8_t> (this->_request.method));
	// Устанавливаем версию протокола HTTP-запроса
	this->_buffer.push(this->_request.version);
	...
	if(!this->_request.url.empty()){
		...
		this->_buffer.push(url.size());
		// Устанавливаем параметры HTTP-запроса
		this->_buffer.push(url.data(), url.size());
	// Устанавливаем размер записи параметров HTTP-запроса
	} else this->_buffer.push(static_cast <size_t> (0));
	// Если текст ответа установлен
	if(!this->_response.message.empty()){
		// Устанавливаем размер сообщения HTTP-ответа
		this->_buffer.push(this->_response.message.length());
		// Устанавливаем данные сообщения HTTP-ответа
		this->_buffer.push(this->_response.message);
	// Устанавливаем размер записи параметров HTTP-запроса
	} else this->_buffer.push(static_cast <size_t> (0));
	// Устанавливаем размер тела сообщения
	this->_buffer.push(this->_body.size);
	// Устанавливаем размер тела сообщения
	this->_buffer.push(this->_body.data.size());
	// Устанавливаем данные тела сообщения
	this->_buffer.push(this->_body.data);
	// Устанавливаем количество HTTP-заголовков
	this->_buffer.push(this->_headers.count());
	...
	for(auto & header : const_cast <web_t *> (this)->_headers){
		...
		this->_buffer.push(header.first.size());
		// Устанавливаем данные названия HTTP заголовка
		this->_buffer.push(header.first);
		// Устанавливаем размер значения HTTP заголовка
		this->_buffer.push(header.second.size());
		// Устанавливаем данные значения HTTP заголовка
		this->_buffer.push(header.second);
	}
	// Выводим результат
	return this->_buffer;
}
...
awh::Web::Web(const fmk_t * fmk, const log_t * log) noexcept :
 ...
 _buffer(fmk, log), _headers(fmk, log), _callback(log), _fmk(fmk), _log(log) {
