					...
					case static_cast <uint8_t> (ws::frame_t::opcode_t::BINARY): {
						...
						// Если сообщение является последним
							...
							if(this->_thr.initialized())
								...
								this->_thr.push(std::bind(&ws2_t::extraction, this, payload.data(), payload.size(), (this->_frame.opcode == ws::frame_t::opcode_t::TEXT)));
					...
					case static_cast <uint8_t> (ws::frame_t::opcode_t::CONTINUATION): {
						...
						if(!this->_buffer.fragments.empty()){
							...
							// Если сообщение является последним
								...
								if(this->_thr.initialized())
									// Добавляем в тредпул новую задачу на извлечение полученных сообщений
									this->_thr.push(std::bind(&ws2_t::extraction, this, static_cast <const char *> (this->_buffer.fragments), static_cast <size_t> (this->_buffer.fragments), (this->_frame.opcode == ws::frame_t::opcode_t::TEXT)));
								// Если тредпул не активирован, выполняем извлечение полученных сообщений
...
awh::client::Websocket2::Websocket2(const fmk_t * fmk, const log_t * log) noexcept :
 ...
 _freeze(false), _crypted(false), _inflate(false), _threads(0), _waitPong(_pingInterval * 2), _respPong(0),
