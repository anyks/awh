 ...
 */
#include <client/web/http2_2.hpp>
	...
	// Если мы работаем с сервером напрямую
		...
		if(i != this->_workers.end()){
			...
			if(web2_t::_callback.is("chunking")){
				...
				this->_chunk.push(buffer, size);
				// Выполняем функцию обратного вызова
				web2_t::_callback.call <void (const uint32_t, const buffer_t &, const awh::http_t *)> ("chunking", i->second->id, this->_chunk, &i->second->http);
				...
				switch(static_cast <uint8_t> (i->second->agent)){
					...
					case static_cast <uint8_t> (web_t::agent_t::HTTP): {
						...
						if(web2_t::_callback.is("chunks")){
							...
							this->_chunk.push(buffer, size);
							// Выполняем функцию обратного вызова
							web2_t::_callback.call <void (const int32_t, const uint32_t, const buffer_t &)> ("chunks", sid, i->second->id, this->_chunk);
						}
					} break;
					// Если агент является клиентом Websocket
					case static_cast <uint8_t> (web_t::agent_t::WEBSOCKET):
					...
					if(this->_http2.initialized()){
						...
						else this->_http.compressors(this->_compressors);
						{
							...
							if(!request.entity.empty()){
								 ...
								 */
								while(!(chunk = ::move(this->_http.chunk())).empty()){
									/**
									 * Если включён режим отладки
									 */
									#if DEBUG_MODE
										// Выводим сообщение о выводе чанка тела
										std::cout << this->_fmk->format("<chunk %zu>", chunk.size()) << std::endl << std::endl << std::flush;
									#endif
									...
									if(!web2_t::send(result, static_cast <const char *> (chunk), static_cast <size_t> (chunk), flag))
										// Выходим из функции
										return -1;
								}
							}
		...
		if((result = ((buffer != nullptr) && (size > 0)))){
			...
			if(this->_http2.initialized()){
				...
				while(!(chunk = ::move(this->_http.chunk())).empty()){
					...
					result = web2_t::send(sid, static_cast <const char *> (chunk), static_cast <size_t> (chunk), flag);
				}
