 ...
 */
#include <client/web/http1_2.hpp>
		...
		if((this->_core != nullptr) && (this->_bid > 0)){
			...
			if(!buffer.empty()){
				...
				while(!(chunk = ::move(this->_http.chunk())).empty()){
					...
					core->send(static_cast <const char *> (chunk), static_cast <size_t> (chunk), this->_bid);
				}
			}
			// Если установлена функция отлова завершения запроса
			if(web_t::_callback.is("end"))
	...
	if(hold.access({event_t::READ, event_t::CONNECT}, event_t::SEND)){
		...
		if((result = ((this->_core != nullptr) && (buffer != nullptr) && (size > 0)))){
			...
			while(!(chunk = ::move(this->_http.chunk())).empty()){
				...
				// Выполняем отправку тела запроса на сервер
				const_cast <client::core_t *> (this->_core)->send(static_cast <const char *> (chunk), static_cast <size_t> (chunk), this->_bid);
			}
		}
	}
