 ...
 * @param core объект сетевого ядра
 */
void awh::client::Web::proxyConnectCallback(const uint64_t bid, const uint16_t sid, awh::core_t * core) noexcept {
	// Если данные переданы верные
								...
								if(!(this->_stopped = (this->_attempt >= this->_attempts))){
									...
									if(!this->_scheme.proxy.url.empty()){
										...
										if(this->_scheme.proxy.http.is(http_t::state_t::ALIVE)){
											...
											this->proxyConnectCallback(bid, sid, core);
 ...
 * @param callback функция обратного вызова
 */
void awh::client::Web::on(function <void (const char *, const size_t)> callback) noexcept {
	// Устанавливаем функцию обратного вызова
	this->_callback.set <void (const char *, const size_t)> ("raw", callback);
}
/**
