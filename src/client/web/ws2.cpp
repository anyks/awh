 ...
 * @param core объект сетевого ядра
 */
void awh::client::Websocket2::connectEvent(const uint64_t bid, const uint16_t sid, awh::core_t * core) noexcept {
	...
	if(hold.access({event_t::OPEN, event_t::READ, event_t::PROXY_READ, event_t::PROXY_CONNECT}, event_t::CONNECT)){
		...
		web2_t::connectEvent(bid, sid, core);
		...
		// Если активирован режим работы с HTTP/1.1 протоколом
			...
			this->_ws1.connectEvent(bid, sid, core);
		}
 ...
 * @param core объект сетевого ядра
 */
void awh::client::Websocket2::disconnectEvent(const uint64_t bid, const uint16_t sid, awh::core_t * core) noexcept {
 ...
 * @param core   объект сетевого ядра
 */
void awh::client::Websocket2::readEvent(const char * buffer, const size_t size, const uint64_t bid, const uint16_t sid, awh::core_t * core) noexcept {
	...
	if((buffer != nullptr) && (size > 0) && (bid > 0) && (sid > 0)){
		...
		if(process){
			...
			} else this->_ws1.readEvent(buffer, size, bid, sid, core);
		}
	}
}
/**
	...
	// Если мы работаем с сервером напрямую
		...
		if(this->_sid == sid){
			...
			if(this->_callback.is("chunking"))
				// Выполняем функцию обратного вызова
				this->_callback.call <void (const uint64_t, const vector <char> &, const awh::http_t *)> ("chunking", this->_rid, vector <char> (buffer, buffer + size), &this->_http);
			// Если функция перехвата полученных чанков не установлена
				...
				if(this->_callback.is("chunks"))
					// Выполняем функцию обратного вызова
					this->_callback.call <void (const int32_t, const uint64_t, const vector <char> &)> ("chunks", sid, this->_rid, vector <char> (buffer, buffer + size));
			}
		}
	}
	// Выводим результат
 ...
 */
void awh::client::Websocket2::callback(const fn_t & callback) noexcept {
	...
	this->_callback.set("messageWebsocket", callback);
	{
		...
		if(this->_callback.is("error"))
			...
			callback.set <void (const uint64_t, const log_t::flag_t, const http::error_t, const string &)> ("error", std::bind(&ws2_t::errors, this, _1, _2, _3, _4));
		// Если функции обратного вызова установлены
		if(!callback.empty())
