 ...
 */
void awh::scheme_t::broker_t::read(ev::io & watcher, int revents) noexcept {
 ...
 * @param revents идентификатор события
 */
void awh::scheme_t::broker_t::connect(ev::io & watcher, int revents) noexcept {
 ...
 * @param revents идентификатор события
 */
void awh::scheme_t::broker_t::timeout(ev::timer & timer, int revents) noexcept {
			...
			if(this->_init){
				 ...
				 */
					 ...
					 */
						...
						// Если функция обратного вызова установлена
						if(this->_core->_callbacks.is("error"))
							// Выполняем функцию обратного вызова
							this->_core->_callbacks.call <void (const log_t::flag_t, const error_t, const string &)> ("error", log_t::flag_t::WARNING, error_t::START, error.what());
					#endif
				}
			}
					...
					case static_cast <uint8_t> (engine_t::method_t::READ): {
						...
						switch(static_cast <uint8_t> (mode)){
							...
							case static_cast <uint8_t> (mode_t::ENABLED): {
								...
								ev_set_priority(&adj->_bev.event.read, -2);
								// Устанавливаем базу событий
								adj->_bev.event.read.set(this->_dispatch.base);
								// Устанавливаем сокет для чтения
								adj->_bev.event.read.set(adj->_addr.fd, ev::READ);
								// Устанавливаем событие на чтение данных подключения
								adj->_bev.event.read.set <awh::scheme_t::broker_t, &awh::scheme_t::broker_t::read> (adj);
					...
					case static_cast <uint8_t> (engine_t::method_t::CONNECT): {
						...
						switch(static_cast <uint8_t> (mode)){
							...
							case static_cast <uint8_t> (mode_t::ENABLED): {
								...
								ev_set_priority(&adj->_bev.event.connect, -2);
								// Устанавливаем базу событий
								adj->_bev.event.connect.set(this->_dispatch.base);
								// Устанавливаем сокет для записи
								adj->_bev.event.connect.set(adj->_addr.fd, ev::WRITE);
								// Устанавливаем событие подключения
								adj->_bev.event.connect.set <awh::scheme_t::broker_t, &awh::scheme_t::broker_t::connect> (adj);
								...
								if(adj->_timeouts.connect > 0){
									...
									adj->_bev.timer.connect.set <awh::scheme_t::broker_t, &awh::scheme_t::broker_t::timeout> (adj);
									// Запускаем запись данных на сервер
									adj->_bev.timer.connect.start(adj->_timeouts.connect);
								}
							} break;
							// Если установлен сигнал деактивации сокета
							case static_cast <uint8_t> (mode_t::DISABLED): {
								// Останавливаем ожидание подключения
								adj->_bev.timer.connect.stop();
								// Останавливаем подключение
								adj->_bev.event.connect.stop();
							} break;
						}
					} break;
				}
