...
#include <lib/event2/net/dns.hpp>
 ...
 */
void awh::DNS::base(struct event_base * base) noexcept {
	// Выполняем блокировку потока
	const lock_guard <recursive_mutex> lock(this->_mtx.hold);
	// Если база передана
	if(base != nullptr)
 ...
 */
void awh::DNS::timeout(const uint16_t sec) noexcept {
	// Выполняем блокировку потока
	const lock_guard <recursive_mutex> lock(this->_mtx.hold);
 ...
 */
awh::DNS::DNS(const fmk_t * fmk, const log_t * log, const network_t * nwk, struct event_base * base) noexcept : _domain(""), _timeout(30), _fn(nullptr), _fmk(fmk), _log(log), _nwk(nwk), _base(base) {
	// Устанавливаем список серверов IPv4
	this->replace(AF_INET);
	// Устанавливаем список серверов IPv6
	this->replace(AF_INET6);
