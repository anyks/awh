 ...
 */
#include <compressor/stateless.hpp>
...
namespace driver {
	 ...
	 */
	static void lzma(const void * buffer, const size_t size, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		...
		if((buffer != nullptr) && (size > 0)){
			...
			try {
				...
				switch(static_cast <uint8_t> (event)){
					...
					case static_cast <uint8_t> (awh::compressor::stateless_t::event_t::ENCODE): {
					...
					case static_cast <uint8_t> (awh::compressor::stateless_t::event_t::DECODE): {
	 ...
	 * @param log    объект для работы с логами
	 */
	static void bzip2(const void * buffer, const size_t size, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		// Если буфер данных передан
		if((buffer != nullptr) && (size > 0)){
			/**
			 * Выполняем отлов ошибок
					...
					case static_cast <uint8_t> (awh::compressor::stateless_t::event_t::DECODE): {
						...
						do {
							...
							if((ret == BZ_OK) && (stream.avail_in == 0) && (stream.avail_out > 0)){
								...
								#if DEBUG_MODE
									// Записываем ошибку в лог
									log->debug("Bzip2: %s", __PRETTY_FUNCTION__, make_tuple(buffer, size, static_cast <uint16_t> (event)), awh::log_t::flag_t::WARNING, "Truncated or corrupted data");
								/**
								 * Если режим отладки не включён
								 */
								#else
									// Записываем ошибку в лог в лог
									log->print("Bzip2: %s", awh::log_t::flag_t::WARNING, "Truncated or corrupted data");
								#endif
								// Выходим из функции
								return;
							}
	 ...
	 * @param log    объект для работы с логами
	 */
	static void brotli(const void * buffer, const size_t size, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		// Если буфер данных передан
		if((buffer != nullptr) && (size > 0)){
			/**
			 * Выполняем отлов ошибок
	 ...
	 * @param log    объект для работы с логами
	 */
	static void snappy(const void * buffer, const size_t size, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		// Если буфер данных передан
		if((buffer != nullptr) && (size > 0)){
			/**
			 * Выполняем отлов ошибок
	 ...
	 * @param result строка куда следует положить результат
	 */
	static void density(const void * buffer, const size_t size, const uint32_t level, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		// Если буфер данных передан
		if((buffer != nullptr) && (size > 0)){
			/**
			 * Выполняем отлов ошибок
	 ...
	 * @param result строка куда следует положить результат
	 */
	static void lizard(const void * buffer, const size_t size, const uint32_t level, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		// Если буфер данных передан
		if((buffer != nullptr) && (size > 0)){
			/**
			 * Выполняем отлов ошибок
	 ...
	 * @param log    объект для работы с логами
	 */
	static void lz4(const void * buffer, const size_t size, const uint32_t level, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		// Если буфер данных передан
		if((buffer != nullptr) && (size > 0)){
			/**
			 * Выполняем отлов ошибок
	 ...
	 * @param log    объект для работы с логами
	 */
	static void zstd(const void * buffer, const size_t size, const uint32_t level, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		// Если буфер данных передан
		if((buffer != nullptr) && (size > 0)){
			/**
			 * Выполняем отлов ошибок
	 ...
	 * @param log    объект для работы с логами
	 */
	static void gzip(const void * buffer, const size_t size, const uint32_t level, const int16_t wbits, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		// Если буфер данных передан
		if((buffer != nullptr) && (size > 0)){
			/**
			 * Выполняем отлов ошибок
	 ...
	 * @param log    объект для работы с логами
	 */
	static void zlib(const void * buffer, const size_t size, const uint32_t level, const int16_t wbits, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		// Если буфер данных передан
		if((buffer != nullptr) && (size > 0)){
			/**
			 * Выполняем отлов ошибок
	 ...
	 * @param log    объект для работы с логами
	 */
	static void deflate(const void * buffer, const size_t size, const uint32_t level, const int16_t wbits, const awh::compressor::stateless_t::event_t event, T & result, const awh::log_t * log) noexcept {
		// Если буфер данных передан
		if((buffer != nullptr) && (size > 0)){
			/**
			 * Выполняем отлов ошибок
				...
				switch(static_cast <uint8_t> (event)){
					...
					case static_cast <uint8_t> (awh::compressor::stateless_t::event_t::ENCODE): {
						...
						if(ret != Z_OK){
							...
							#if DEBUG_MODE
								// Записываем ошибку в лог
								log->debug("Deflate: %s", __PRETTY_FUNCTION__, make_tuple(buffer, size, level, wbits, static_cast <uint16_t> (event)), awh::log_t::flag_t::WARNING, "Error initializing compression stream");
							/**
							 * Если режим отладки не включён
							 */
							#else
						...
						// Гарантируем освобождение локального потока при любом выходе (в т.ч. при исключении)
						auto guard = driver::makeScopeExit([&local]() noexcept {
						...
						while(zs->avail_in > 0){
							...
							if(ret != Z_OK){
								...
								#if DEBUG_MODE
									// Записываем ошибку в лог
									log->debug("Deflate: %s", __PRETTY_FUNCTION__, make_tuple(buffer, size, level, wbits, static_cast <uint16_t> (event)), awh::log_t::flag_t::WARNING, "Error during data compression");
								/**
								 * Если режим отладки не включён
								 */
								#else
					...
					case static_cast <uint8_t> (awh::compressor::stateless_t::event_t::DECODE): {
						...
						if(ret != Z_OK){
							...
							#if DEBUG_MODE
								// Записываем ошибку в лог
								log->debug("Deflate: %s", __PRETTY_FUNCTION__, make_tuple(buffer, size, level, wbits, static_cast <uint16_t> (event)), awh::log_t::flag_t::WARNING, "Error initializing decompression stream");
							/**
							 * Если режим отладки не включён
							 */
							#else
						...
						do {
							...
							if((ret != Z_OK) && (ret != Z_STREAM_END)){
								...
								#if DEBUG_MODE
									// Записываем ошибку в лог
									log->debug("Deflate: %s", __PRETTY_FUNCTION__, make_tuple(buffer, size, level, wbits, static_cast <uint16_t> (event)), awh::log_t::flag_t::WARNING, "Error during data decompression");
								/**
								 * Если режим отладки не включён
								 */
								#else
			 ...
			 */
				...
				#if DEBUG_MODE
					// Записываем ошибку в лог
					log->debug("Deflate: %s", __PRETTY_FUNCTION__, make_tuple(buffer, size, level, wbits, static_cast <uint16_t> (event)), awh::log_t::flag_t::CRITICAL, error.what());
				/**
				 * Если режим отладки не включён
				 */
				#else
 ...
 * @param level уровень компрессии
 */
void awh::compressor::Stateless::level(const level_t level) noexcept {
 ...
 */
void awh::compressor::Stateless::threadSafety(const bool mode) noexcept {
 ...
 * @param wbits размер скользящего окна
 */
void awh::compressor::Stateless::wbitsGZip(const int16_t wbits) noexcept {
	// Выполняем блокировку потоков
	const locker_t <> lock(this->_mtx);
 ...
 */
auto awh::compressor::Stateless::compress(string_view buffer, const method_t method) const noexcept -> T {
	// Переменная результата
	T result;
	// Если буфер данных передан
	if(!buffer.empty())
 ...
 *
 */
template string awh::compressor::Stateless::compress(string_view, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных из строки с выводом результата в буфер
 *
 */
template vector <char> awh::compressor::Stateless::compress(string_view, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных из строки с выводом результата в бинарный буфер
 *
 */
template vector <uint8_t> awh::compressor::Stateless::compress(string_view, const method_t) const noexcept;
/**
 * @brief Шаблон метода компрессии данных
 *
 ...
 * @return       результат компрессии
 */
auto awh::compressor::Stateless::compress(const B & buffer, const method_t method) const noexcept -> A {
	// Переменная результата
	A result;
	// Если буфер данных передан
	if(!buffer.empty())
 ...
 *
 */
template string awh::compressor::Stateless::compress(const string &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных из буфера с выводом результата в строку
 *
 */
template string awh::compressor::Stateless::compress(const vector <char> &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных из бинарного буфера с выводом результата в строку
 *
 */
template string awh::compressor::Stateless::compress(const vector <uint8_t> &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных из строки с выводом результата в буфер
 *
 */
template vector <char> awh::compressor::Stateless::compress(const string &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных из буфера с выводом результата в буфер
 *
 */
template vector <char> awh::compressor::Stateless::compress(const vector <char> &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных из бинарного буфера с выводом результата в буфер
 *
 */
template vector <char> awh::compressor::Stateless::compress(const vector <uint8_t> &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных из строки с выводом результата в бинарный буфер
 *
 */
template vector <uint8_t> awh::compressor::Stateless::compress(const string &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных из буфера с выводом результата в бинарный буфер
 *
 */
template vector <uint8_t> awh::compressor::Stateless::compress(const vector <char> &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных из бинарного буфера с выводом результата в бинарный буфер
 *
 */
template vector <uint8_t> awh::compressor::Stateless::compress(const vector <uint8_t> &, const method_t) const noexcept;
/**
 * @brief Шаблон метода компрессии данных
 *
 * @tparam T тип возвращаемого результата
 ...
 * @return       результат компрессии
 */
auto awh::compressor::Stateless::compress(const void * buffer, const size_t size, const method_t method) const noexcept -> T {
	// Переменная результата
	T result;
	// Если буфер данных передан
	if((buffer != nullptr) && (size > 0))
 ...
 *
 */
template string awh::compressor::Stateless::compress(const void *, const size_t, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных с выводом результата в буфер
 *
 */
template vector <char> awh::compressor::Stateless::compress(const void *, const size_t, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных с выводом результата в бинарный буфер
 *
 */
template vector <uint8_t> awh::compressor::Stateless::compress(const void *, const size_t, const method_t) const noexcept;
/**
 * @brief Шаблон метода компрессии данных
 *
 * @tparam T тип возвращаемого результата
 ...
 */
void awh::compressor::Stateless::compress(const void * buffer, const size_t size, const method_t method, T & result) const noexcept {
	// Если буфер данных передан
	if((buffer != nullptr) && (size > 0)){
		/**
		...
		switch(static_cast <uint8_t> (method)){
			...
			case static_cast <uint8_t> (method_t::GZIP): {
				...
				driver::gzip(buffer, size, this->_level[1], this->_wbits, event_t::ENCODE, result, this->_log);
				// Если результат операции пустой - значит произошла ошибка
				if(result.empty()){
					/**
					 * Если включён режим отладки
			...
			case static_cast <uint8_t> (method_t::DEFLATE): {
				...
				driver::deflate(buffer, size, this->_level[1], this->_wbits, event_t::ENCODE, result, this->_log);
				// Если результат операции пустой - значит произошла ошибка
				if(result.empty()){
					/**
					 * Если включён режим отладки
			...
			case static_cast <uint8_t> (method_t::ZLIB): {
				...
				driver::zlib(buffer, size, this->_level[1], this->_wbits, event_t::ENCODE, result, this->_log);
				// Если результат операции пустой - значит произошла ошибка
				if(result.empty()){
					/**
					 * Если включён режим отладки
 ...
 *
 */
template void awh::compressor::Stateless::compress(const void *, const size_t, const method_t, string &) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных с выводом результата в буфер
 *
 */
template void awh::compressor::Stateless::compress(const void *, const size_t, const method_t, vector <char> &) const noexcept;
/**
 * @brief Явный специализированный шаблон метода компрессии данных с выводом результата в бинарный буфер
 *
 */
template void awh::compressor::Stateless::compress(const void *, const size_t, const method_t, vector <uint8_t> &) const noexcept;
/**
 ...
 */
auto awh::compressor::Stateless::decompress(string_view buffer, const method_t method) const noexcept -> T {
	// Переменная результата
	T result;
	// Если буфер данных передан
	if(!buffer.empty())
 ...
 *
 */
template string awh::compressor::Stateless::decompress(string_view, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных из строки с выводом результата в буфер
 *
 */
template vector <char> awh::compressor::Stateless::decompress(string_view, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных из строки с выводом результата в бинарный буфер
 *
 */
template vector <uint8_t> awh::compressor::Stateless::decompress(string_view, const method_t) const noexcept;
/**
 * @brief Шаблон метода декомпрессии данных
 *
 * @tparam A тип возвращаемого результата
 ...
 * @return       результат декомпрессии
 */
auto awh::compressor::Stateless::decompress(const B & buffer, const method_t method) const noexcept -> A {
	// Переменная результата
	A result;
	// Если буфер данных передан
	if(!buffer.empty())
 ...
 *
 */
template string awh::compressor::Stateless::decompress(const string &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных из буфера с выводом результата в строку
 *
 */
template string awh::compressor::Stateless::decompress(const vector <char> &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных из бинарного буфера с выводом результата в строку
 *
 */
template string awh::compressor::Stateless::decompress(const vector <uint8_t> &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных из строки с выводом результата в буфер
 *
 */
template vector <char> awh::compressor::Stateless::decompress(const string &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных из буфера с выводом результата в буфер
 *
 */
template vector <char> awh::compressor::Stateless::decompress(const vector <char> &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных из бинарного буфера с выводом результата в буфер
 *
 */
template vector <char> awh::compressor::Stateless::decompress(const vector <uint8_t> &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных из строки с выводом результата в бинарный буфер
 *
 */
template vector <uint8_t> awh::compressor::Stateless::decompress(const string &, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных из буфера с выводом результата в бинарный буфер
 *
 */
template vector <uint8_t> awh::compressor::Stateless::decompress(const vector <char> &, const method_t) const noexcept;
/**
 ...
 *
 */
template vector <uint8_t> awh::compressor::Stateless::decompress(const vector <uint8_t> &, const method_t) const noexcept;
/**
 * @brief Шаблон метода декомпрессии данных
 *
 * @tparam T тип возвращаемого результата
 ...
 * @return       результат декомпрессии
 */
auto awh::compressor::Stateless::decompress(const void * buffer, const size_t size, const method_t method) const noexcept -> T {
	// Переменная результата
	T result;
	// Если буфер данных передан
	if((buffer != nullptr) && (size > 0))
 ...
 *
 */
template string awh::compressor::Stateless::decompress(const void *, const size_t, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных с выводом результата в буфер
 *
 */
template vector <char> awh::compressor::Stateless::decompress(const void *, const size_t, const method_t) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных с выводом результата в бинарный буфер
 *
 */
template vector <uint8_t> awh::compressor::Stateless::decompress(const void *, const size_t, const method_t) const noexcept;
/**
 * @brief Шаблон метода декомпрессии данных
 *
 * @tparam T тип возвращаемого результата
 ...
 * @param result контейнер куда следует положить результат
 */
void awh::compressor::Stateless::decompress(const void * buffer, const size_t size, const method_t method, T & result) const noexcept {
	// Если буфер данных передан
	if((buffer != nullptr) && (size > 0)){
		/**
		...
		switch(static_cast <uint8_t> (method)){
			...
			case static_cast <uint8_t> (method_t::GZIP): {
				...
				driver::gzip(buffer, size, this->_level[1], this->_wbits, event_t::DECODE, result, this->_log);
				// Если результат операции пустой - значит произошла ошибка
				if(result.empty()){
					/**
					 * Если включён режим отладки
			...
			case static_cast <uint8_t> (method_t::DEFLATE): {
				...
				driver::deflate(buffer, size, this->_level[1], this->_wbits, event_t::DECODE, result, this->_log);
				// Если результат операции пустой - значит произошла ошибка
				if(result.empty()){
					/**
					 * Если включён режим отладки
			...
			case static_cast <uint8_t> (method_t::ZLIB): {
				...
				driver::zlib(buffer, size, this->_level[1], this->_wbits, event_t::DECODE, result, this->_log);
				// Если результат операции пустой - значит произошла ошибка
				if(result.empty()){
					/**
					 * Если включён режим отладки
 ...
 *
 */
template void awh::compressor::Stateless::decompress(const void *, const size_t, const method_t, string &) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных с выводом результата в буфер
 *
 */
template void awh::compressor::Stateless::decompress(const void *, const size_t, const method_t, vector <char> &) const noexcept;
/**
 * @brief Явный специализированный шаблон метода декомпрессии данных с выводом результата в бинарный буфер
 *
 */
template void awh::compressor::Stateless::decompress(const void *, const size_t, const method_t, vector <uint8_t> &) const noexcept;
/**
 * @brief Конструктор
 *
 * @param log объект для работы с логами
 */
awh::compressor::Stateless::Stateless(const log_t * log) noexcept :
 _wbits(0),
 _level{
	1,
	...
	this->_wbits = static_cast <int16_t> (MAX_WBITS);
}
/**
 * @brief Деструктор
 *
 */
awh::compressor::Stateless::~Stateless() noexcept {}
