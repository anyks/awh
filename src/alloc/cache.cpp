...
/**
 * @brief Метод накопления освобождённого до шага проверки порога
 *
 * @note Запись эта прежде описывала совсем иной метод: говорила о «занятом прикладным
 *       кодом», звала второй довод `batch` при имени `step` и обещала возврат
 *       накопленного числом, тогда как метод отвечает признаком. Верное описание стоит
 *       у объявления, в заголовочном файле
 *
...
awh::alloc::cache_t * awh::alloc::Caches::create() noexcept {
	...
	if(!result->init(this->_central, this->_classes, result->_kept.load(std::memory_order_relaxed))){
		/**
		 * Метим кэш свободным ПОД ЗАМКОМ, как и всякую правку признака занятости
		 *
		 * Признак этот - обычное поле, а не неделимое: читает его перебор в `create`,
		 * и читает под замком общего списка. Правка же его здесь шла мимо замка, и
		 * выходило состязание по стандарту - запись без замка против чтения под ним.
		 * Следствие у него не только теоретическое: два потока, дошедшие сюда разом,
		 * вправе оба увидеть кэш свободным и оба забрать его себе, а кэш потока
		 * замком не защищён вовсе - в том весь его смысл
		 */
		// Захватываем замок общего списка кэшей
		hold_t hold(this->_lock);
