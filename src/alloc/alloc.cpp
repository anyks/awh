...
namespace {
	...
	static void * reserve(const size_t wanted) noexcept {
		...
		// Возможности выключены у подавляющего большинства приложений
		/**
		 ...
		 */
		if(AWH_ALLOC_UNLIKELY(!bare) && revive.load(std::memory_order_relaxed) && revive.exchange(false, std::memory_order_acq_rel)){
		 ...
		 */
		if(AWH_ALLOC_UNLIKELY(!bare) && machinery->guard.wanted(size)){
