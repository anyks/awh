...
AWH_ALLOC_LINKAGE void * AWH_ALLOC_HOOK(malloc)(size_t size) {
	...
	/**
	 * Спрашиваем засев через признак голой выдачи
	 *
	 * Строка признака читается всякой выдачей и лежит в кэше горячей, а поле настроек -
	 * своя строка, читаемая ради одного сравнения
	 */
	if(!::plain.load(std::memory_order_relaxed) && (result != nullptr) && (::settings.fill == awh::alloc::fill_t::ZERO))
		// Обнуляем выдаваемое
