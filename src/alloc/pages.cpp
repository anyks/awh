...
void * awh::alloc::Pages::meta(const size_t size) noexcept {
	...
	if(this->_metaLeft < need){
		...
		if(block == nullptr)
			...
			return nullptr;
		/**
		 * Вносим кусок в общий список: в начале куска лежит его заголовок из указателя
		 * на предыдущий и РАЗМЕРА выданного, оттого списку не нужно собственной памяти
		 *
		 * Размер хранится, а не берётся постоянной величиной при отдаче: источник вправе
		 * выдать больше запрошенного, округлив запрос по своему зерну (суперстраницы,
		 * подменный источник), и отдай мы такой кусок жёсткими 256 КБ - на POSIX `munmap`
		 * снял бы лишь голову отображения, а хвост потёк бы навсегда
		 */
...
bool awh::alloc::Pages::rehash(const size_t length) noexcept {
	 ...
	 */
	/**
	 * Куски переносятся ПРЕЖДЕ, чем новая таблица становится действующей
	 *
	 * Читатель разбирает адрес без замка и волен прийти в любой миг. Прежде новая
	 * таблица объявлялась действующей ПУСТОЙ, а куски переносились в неё следом:
	 * читатель, попавший в это окно, куска не находил и признавал наш же адрес чужим.
	 * Освобождение такого блока уходило прежнему распределителю, а тот на чужой адрес
	 * отвечает падением
	 *
	 * Перенос идёт напрямую в новую таблицу, а не через `enroll`: тот пишет в
	 * ДЕЙСТВУЮЩУЮ, каковой новая ещё не стала
	 */
	// Число кусков, перенесённых в новую таблицу
	size_t enrolled = 0;
	if(previous != nullptr){
		/**
		...
		for(size_t i = 0; i < previous->length; i++){
			...
			// Определяем место куска в новой таблице
			size_t index = static_cast <size_t> ((key * 0x9E3779B97F4A7C15ull) & (length - 1));
			/**
	...
	// Запоминаем число внесённых кусков
	this->_enrolled = enrolled;
	// Объявляем заполненную таблицу действующей
...
void awh::alloc::Pages::mark(span_t * span) noexcept {
	 ...
	 */
	/**
	 * Запись отдаётся с отпусканием - ОДНИМ барьером на всю область
	 *
	 * Читатель разбирает адрес без замка кучи, и увидь он указатель прежде, чем
	 * записанные в область поля, - прочёл бы недописанное. Отпускание ставится
	 * барьером перед перебором, а сами записи идут без упорядочивания: барьер
	 * отпускания, стоящий прежде атомарной записи, синхронизируется с захватом,
	 * прочитавшим записанное ею ([atomics.fences]), - обещание то же самое, что у
	 * отпускания на каждой записи
	 *
	 * Поштучное отпускание дорого у Эльбруса (e2k): там это команда `wait` -
	 * ожидание всех незавершённых загрузок и записей, - по две на страницу, то есть
	 * шестнадцать на область в 64 КБ. Замерено 18.09.2026 (E8C2, LCC 1.27): крупные
	 * блоки 357.5 -> 325.9 нс, рост перевыдачей 585.0 -> 530.9 нс; прочие сценарии
	 * без изменений. У x86-64 код метода тождествен до команды, у ARM64 (macOS) не
	 * хуже: один `dmb` вместо `stlr` на каждую запись
	 */
	/**
	 * Поля области берутся в местные ПРЕЖДЕ перебора
	 *
	 * Метка разряда - неделимый БАЙТ, а байт по правилам языка вправе совпасть с любой
	 * памятью: после записи в него собиратель обязан считать поля области изменившимися
	 * и перечитывать их на каждой странице. Поля эти под замком кучи неизменны, и
	 * перечитывать их незачем
	 *
	 * У Эльбруса (e2k) цикл перечитывал пять полей на страницу и не конвейеризовался:
	 * одиннадцать широких команд на страницу против трёх ныне. Замерено 18.09.2026
	 * (E8C2, LCC 1.27), вдобавок к барьеру: крупные блоки 325.9 -> 287.9 нс, рост
	 * перевыдачей 530.1 -> 471.2 нс; прочие сценарии без изменений. У x86-64 цикл был
	 * с теми же перечитываниями и ветвлением, ныне развёрнут вчетверо без единой
	 * загрузки в теле; у ARM64 (macOS) замер нейтрален
	 */
	// Места указателей областей, начиная с первой страницы
	std::atomic <span_t *> * const index = (span->chunk->index + first);
	// Места меток разряда, начиная с первой страницы
	std::atomic <uint8_t> * const tags = (span->chunk->tags + first);
	// Число страниц области
	const size_t pages = span->pages;
	// Метка разряда: у свободной области её нет, у выданной она берётся из самой области
	const uint8_t tag = (span->released ? 0 : static_cast <uint8_t> (span->tag));
	// Отпускаем записанное в область прежде записей по её страницам
	std::atomic_thread_fence(std::memory_order_release);
	for(size_t i = 0; i < pages; i++){
		// Записываем область по очередной странице
		index[i].store(span, std::memory_order_relaxed);
		/**
		 ...
		 */
		tags[i].store(tag, std::memory_order_relaxed);
	}
}
/**
...
awh::alloc::Pages::chunk_t * awh::alloc::Pages::grow() noexcept {
	...
	if(base == nullptr){
		/**
		 * Отказ источника при заданном потолке - тоже упор в потолок
		 *
		 * Потолок сторожат ДВА места, а не одно: куча считает своё взятое, а источник -
		 * всё взятое у системы, включая крупные выдачи и страницы заслонов, идущие мимо
		 * кучи. Приложение задаёт одно число, и оба места получают его. Какое из двух
		 * упрётся первым, зависит от того, много ли взято мимо кучи, - то есть от возраста
		 * процесса, а не от настройки
		 *
		 * Прежде отметку ставила лишь куча, и отказ источника уходил наверх МОЛЧА:
		 * приложение получало пустоту от `malloc`, а отклика об упоре не получало вовсе.
		 * Проверено опытом - `HeapLimitCallbackIsCalled` проходила в свежем процессе, где
		 * первой упиралась куча, и падала в общем прогоне, где мимо кучи было взято
		 * довольно, чтобы первым упёрся источник
		 *
		 * Отметка ставится лишь при ЗАДАННОМ потолке: без него отказ источника означает
		 * нехватку памяти у системы, а это не упор в потолок и докладывать так о нём
		 * значило бы лгать
		 */
		// Если потолок кучи задан
		if(this->_limit > 0)
			// Отмечаем куче отказ в росте из-за потолка
			this->_jammed = true;
		// Отвечаем отказом
	...
	/**
	 * Размер куска сверяется с ЗАПРОШЕННЫМ, а не берётся у источника на веру
	 *
	 * Разметка куска лежит в массивах постоянной длины - по указателю области и метке
	 * разряда на каждую его страницу, - и рассчитаны они ровно на кусок. Отдай источник
	 * БОЛЬШЕ запрошенного, и адрес из хвоста попадал бы за конец обоих массивов: разбор
	 * его признавал бы адрес нашим (тот лежит внутри куска по размеру) и читал бы
	 * разметку за границей. Отдай МЕНЬШЕ - область накрыла бы страницы, каких у нас нет
	 *
	 * Системный источник округляет запрос вверх до зернистости, а кусок берётся кратным
	 * ей у всех заявленных систем, - но источник подменяем приложением, и обещания его
	 * нам неведомы. Недостача отвергается отказом, излишек - забывается: вернуть его
	 * источнику нечем, зато размечать его никто не станет
	 */
	/**
	 * Размечается ровно столько, сколько источник И ВПРАВДУ дал
	 *
	 * Разметка куска лежит в массивах постоянной длины - по указателю области и метке
	 * разряда на каждую его страницу, - и рассчитаны они ровно на кусок. Прежде куску
	 * верили на слово: размер брался у источника, а размечался он всегда целиком.
	 * Отдай источник БОЛЬШЕ запрошенного, и адрес из хвоста попадал бы за конец обоих
	 * массивов - разбор признавал бы его нашим и читал разметку за границей; отдай
	 * МЕНЬШЕ - область накрыла бы страницы, каких у нас нет
	 *
	 * Системный источник округляет запрос вверх до зернистости, а кусок берётся кратным
	 * ей у всех заявленных систем, - но источник подменяем приложением, и обещания его
	 * нам неведомы. Оттого кусок принимается ЛЮБОЙ, а размечается по меньшему из двух:
	 * отказывать скупому источнику нельзя - следующий запрос принёс бы такой же кусок, и
	 * куча ходила бы за ними без конца
	 */
	// Определяем число страниц, какие и вправду размечаются
	const size_t served = (((actual < CHUNK) ? actual : CHUNK) / PAGE);
	// Если куска не хватает даже на одну страницу
	if(served == 0){
		// Отдаём источнику негодный кусок
		this->_source->release(base, actual);
		// Возвращаем учётную запись в список повторно используемых
		span->next = this->_spare;
		// Запоминаем запись повторно используемой
		this->_spare = span;
		// Отвечаем отказом: размечать нечего
		return nullptr;
	}
	// Запоминаем адрес начала куска
	chunk->base = base;
	// Запоминаем размер куска, каким он размечен
	chunk->size = (served * PAGE);
	// Запоминаем размер куска, взятый у источника
	chunk->served = actual;
	...
	span->base = base;
	// Область накрывает размеченную часть куска
	span->pages = served;
	// Область свободна
	span->released = true;
	...
	// Увеличиваем свободное на размеченную часть куска, а не на полный CHUNK: при
	// короткой выдаче источника размечено лишь served страниц, и полный PAGES завысил
	// бы свободное сверх взятого (free > total)
	this->_state.free += (served * PAGE);
...
void awh::alloc::Pages::destroy() noexcept {
	...
	for(chunk_t * chunk = this->_chunks; chunk != nullptr;){
		...
		this->_source->release(chunk->base, chunk->served);
	...
	for(void * block = this->_metaChunks; block != nullptr;){
		...
		size_t span = 0;
		// Извлекаем размер куска из его заголовка (следом за указателем на предыдущий)
		::memcpy(&span, (reinterpret_cast <uint8_t *> (block) + sizeof(void *)), sizeof(size_t));
		// Отдаём кусок источнику ровно тем размером, каким он был взят
...
void * awh::alloc::Pages::alloc(const size_t pages, size_t * served) noexcept {
	...
	if(span->pages > pages){
		...
		if(rest == nullptr){
			...
			span->tag = 0;
			/**
			 * Снимаем область с учёта отданного системе ТЕМ ЖЕ порядком, что и ниже
			 *
			 * Прежде здесь стояло голое `span->purged = false` - без вычитания байтов из
			 * учёта. Учёт этот тем самым застревал навсегда: страницы уходили наружу, а
			 * числились отданными системе, и вычесть их было уже некому - признак-то снят.
			 * Щупом поймано на самой ветви: куча доложила 171 страницу отданной при 31
			 * свободной, то есть больше, чем у неё вообще было свободного
			 */
...
bool awh::alloc::Pages::free(void * addr, const uint64_t now) noexcept {
	...
	const uintptr_t value = reinterpret_cast <uintptr_t> (addr);
	/**
	 * Кусок ищется ПО ТАБЛИЦЕ, а не перебором общего списка
	 *
	 * Перебор шёл по всем кускам кучи на КАЖДОМ возврате области, а куски берутся у
	 * системы по четыре мегабайта: у приложения на семьсот мегабайт их набирается под
	 * две сотни, и всякий возврат обходил их подряд. Таблица же отвечает за одно
	 * обращение, и заведена она тут же рядом - тем самым `discover`, каким пользуется
	 * разбор адреса
	 *
	 * Всякий кусок общего списка есть и в таблице: `grow` вносит его туда ПРЕЖДЕ
	 * списка, а при отказе внесения не заводит вовсе
	 */
	{
		// Ищем кусок, которому принадлежит адрес
		...
		span->released = true;
		/**
		 * Снимаем со страниц метку разряда, если она на них стояла
		 *
		 * Метка эта - быстрый путь освобождения: `classify` отвечает по ней одним чтением,
		 * а нуль означает «страница разряду не отдана». Освобождение метки не снимало, и
		 * договор этот держался лишь тем, что нарезанные разрядам области куче не
		 * возвращаются вовсе - единственный путь сюда идёт от выдач СВЕРХ разрядов, а у
		 * тех метки нет. Держался, но заслонки на то не было ни одной: заведись такой
		 * возврат - и `classify` относила бы свободную страницу к прежнему её разряду, а
		 * освобождение по ней уводило бы чужой блок в чужой список
		 *
		 * Сличение здесь холостое: у выдач сверх разрядов метка нулевая, и обход не идёт
		 */
		if(span->tag != 0){
			// Определяем номер первой страницы области в куске
			const size_t first = static_cast <size_t> ((span->base - chunk->base) / PAGE);
			/**
			 * Перебираем страницы области
			 */
			// Места меток разряда, начиная с первой страницы (поля берутся до перебора - довод у `mark`)
			std::atomic <uint8_t> * const tags = (chunk->tags + first);
			// Число страниц области
			const size_t pages = span->pages;
			// Отпускаем записанное прежде ОДНИМ барьером на всю область (довод у `mark`)
			std::atomic_thread_fence(std::memory_order_release);
			for(size_t i = 0; i < pages; i++)
				// Снимаем метку разряда очередной страницы
				tags[i].store(0, std::memory_order_relaxed);
			// Снимаем метку владельца и с самой области
			span->tag = 0;
		}
		/**
...
bool awh::alloc::Pages::tag(void * addr, const uint32_t tag) noexcept {
	...
	for(size_t i = 0; i < pages; i++)
		...
		tags[i].store(static_cast <uint8_t> (tag), std::memory_order_relaxed);
	// Отвечаем успехом
	return true;
}
/**
