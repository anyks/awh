...
void awh::alloc::Huge::reset() noexcept {
	...
	this->_spare = nullptr;
	/**
	 * Придержанные области отдаём здесь, а не обходом таблицы поиска
	 *
	 * Записи их в таблице не значатся - придержка снимает запись с учёта и заводит
	 * связь лишь в своём списке, - оттого обход таблицы выше их и не тронул. Отдай мы
	 * их там же, отдача прошла бы дважды; не отдай нигде - область утекла бы к системе
	 * неотданной. Оттого им и отведён свой обход, идущий ДО сброса списка
	 */
	{
...
bool awh::alloc::Huge::retain(uint8_t * base, const size_t span) noexcept {
	...
	// Захватываем замок слоя
	hold_t hold(this->_lock);
	// Если придерживать не велено либо область не вмещается в потолок ЦЕЛИКОМ
	if((this->_ceiling == 0) || (span > this->_ceiling))
		// Область придержать нельзя
		return false;
	/**
	 * Вытесняем СТАРЕЙШИЕ придержанные, покуда новая не поместится
	 *
	 * Прежде здесь стоял отказ: не помещается - и не придерживаем. Уклад этот обращал
	 * придержку в кладбище: работа, растящая одно вместилище удвоением, отдаёт области
	 * по возрастанию, и первые - мелкие - занимали потолок навсегда, а всякая следующая,
	 * более крупная и более ценная, отвергалась. Замером набора ABC при потолке в 16 МБ
	 * отвергалось 516 областей из 1088 - ровно все крупные
	 *
	 * Вытесняем старейшие, а не первые попавшиеся: при росте удвоением старейшая - она
	 * же и самая мелкая, а ценна как раз крупная, только что отданная
	 */
	while((this->_cached != nullptr) && ((this->_cachedBytes + span) > this->_ceiling)){
		// Предшественник старейшей придержанной области
		record_t * previous = nullptr;
		// Старейшая придержанная область
		record_t * oldest = this->_cached;
		/**
		 * Ищем хвост списка придержанных: он и есть старейшая область
		 *
		 * Перебор здесь недорог: список ограничен потолком, а области в нём по своей
		 * природе крупны - десяток мест на весь потолок
		 */
		while(oldest->spare != nullptr){
			// Запоминаем предшественника
			previous = oldest;
			// Переходим к следующей области
			oldest = oldest->spare;
		}
		// Если у старейшей области есть предшественник
		if(previous != nullptr)
			// Обрываем список на предшественнике
			previous->spare = nullptr;
		// Иначе список опустел
		else this->_cached = nullptr;
		// Уменьшаем объём придержанного
		this->_cachedBytes -= ((this->_cachedBytes < oldest->span) ? this->_cachedBytes : oldest->span);
		// Уменьшаем взятое у источника
		this->_state.taken -= ((this->_state.taken < oldest->span) ? this->_state.taken : oldest->span);
		// Отдаём источнику вытесненную область
		this->_source->release(oldest->base, oldest->span);
		// Возвращаем запись в список повторно используемых
		oldest->spare = this->_spare;
		// Запоминаем список повторно используемых записей
		this->_spare = oldest;
	}
 ...
 *
 */
bool awh::alloc::Huge::shield(const void * addr, const bool opened) noexcept {
	// Если менять доступность нечему
	if((addr == nullptr) || (this->_source == nullptr))
		// Отвечаем отказом
		return false;
	// Получаем зернистость выдачи источника
	const size_t grain = this->_source->granularity();
	...
	// Захватываем замок слоя
	hold_t hold(this->_lock);
	// Ищем запись, чья область накрывает заданный адрес
	record_t * record = this->lookup(addr, false);
	// Если запись не найдена
	if(record == nullptr)
		// Адрес не наш
		return false;
	/**
	 * Раскрытия СЧИТАЮТСЯ, а не отмечаются признаком
	 *
	 * При общем хранилище на процесс два потока раскрывают одну область, и тот, кто
	 * закончит первым, закрыл бы память под ногами второго - отказ доступа посреди
	 * чужой работы, то есть падение, и притом плавающее и редкое. Область запирается
	 * на нуле, и только на нуле
	 */
	// Если блок открывается
	if(opened){
		// Поднимаем счёт держателей раскрытия
		record->opened++;
		// Если блок и так доступен
		if(!record->sealed)
			/**
			 * Открывать доступное незачем
			 *
			 * Вызов системы ничего бы не изменил, а счёт держателей поднят - выход
			 * последнего из них блок закроет
			 */
			return true;
	// Если блок закрывается
	} else {
		// Если блок держит кто-то ещё
		if(record->opened > 0){
			// Опускаем счёт держателей раскрытия
			record->opened--;
			// Если держатели ещё остались
			if(record->opened > 0)
				// Закрывать рано
				return true;
		}
		/**
		 * Закрытие блока БЕЗ держателей законно
		 *
		 * Выданный блок доступен, а держателей раскрытия у него нет вовсе: первое
		 * закрытие приводит его в закрытое состояние, а не снимает чьё-то раскрытие.
		 * Судить об этом по одному лишь счёту нельзя - оттого при записи стоит и
		 * признак закрытости
		 */
		if(record->sealed)
			// Блок и так закрыт
	...
	/**
	 * Границы правим САМИ, а не полагаемся на систему
	 *
	 * Смена доступности идёт страницами: начало опускается к границе страницы, а длина
	 * поднимается к ней же. Невыровненное начало система вправе отвергнуть, и отказ
	 * этот читался бы звавшим как «не умею»
	 */
	// Получаем начало блока, опущенное к границе страницы
	uint8_t * const base = reinterpret_cast <uint8_t *> (
	 reinterpret_cast <uintptr_t> (record->block) & ~static_cast <uintptr_t> (grain - 1u));
	// Получаем длину блока, поднятую к границе страницы
	const size_t length = (((reinterpret_cast <uintptr_t> (record->block) -
	 reinterpret_cast <uintptr_t> (base)) + record->size + (grain - 1u)) & ~static_cast <uintptr_t> (grain - 1u));
	// Меняем доступность блока
	const bool result = this->_source->protect(base, length, opened);
	// Если доступность блока изменена
	if(result)
		// Запоминаем состояние блока
		record->sealed = !opened;
	/**
	 * Не сумев сменить доступность, счёт возвращаем на место
	 *
	 * Иначе блок числился бы закрытым, оставаясь доступным, и следующее раскрытие
	 * поверило бы счёту, а не действительности
	 */
	else if(opened && (record->opened > 0))
		// Возвращаем счёт держателей раскрытия на место
		record->opened--;
