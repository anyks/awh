...
namespace {
	...
	static void priorFree(void * ptr) noexcept {
		...
		if(ptr == nullptr)
			...
			return;
		/**
		 * @warning Звать здесь `::free` НЕЛЬЗЯ - это бесконечная возвратность
		 *
		 * Прежде здесь стояло именно оно, и довод был верен ровно до тех пор, пока
		 * захват у macOS шёл ОДНОЙ зоной: имя `free` принадлежало тогда системе, та
		 * разбирала блок по зонам и отдавала его владельцу. Двойной захват отдал это
		 * имя НАМ - подмена имён связыванием идёт теперь и у macOS, - и круг замкнулся:
		 * наш `free` не узнаёт чужого блока, зовёт прежнее освобождение, а прежнее зовёт
		 * `::free`, то есть снова нас. Щупом поймано срывом стека на первом же
		 * освобождении блока чужой зоны
		 *
		 * Тот же круг и по той же причине разорван у измерения блока - см. предупреждение
		 * при `priorMeasure`. Разница одна: там он вскрылся сразу, у ветвления процесса,
		 * а здесь пролежал незамеченным - блоки, выданные мимо нас, в наборе проверок
		 * не освобождаются
		 */
		/**
		 * Отдаём блок прежней основной зоне, если она его узнаёт
		 *
		 * Спрашиваем зону НАПРЯМУЮ, а не обходом: обход зон идёт и через нашу, а нам тут
		 * делать нечего - блок сюда попал именно потому, что нашим не оказался
		 */
		if((::__awh_zone_previous__ != nullptr) && (::__awh_zone_previous__->size != nullptr) &&
		   ((* ::__awh_zone_previous__->size)(::__awh_zone_previous__, ptr) != 0)){
			// Отдаём блок прежней основной зоне
			::malloc_zone_free(::__awh_zone_previous__, ptr);
			// Освобождать больше нечего
			return;
		}
		/**
		 * Ищем зону блока разбором адреса
		 *
		 * Указатель мог быть выдан и ТРЕТЬЕЙ зоной - скажем, заведённой кем-то ещё, - и
		 * отдать его прежней означало бы порчу чужого учёта. Свою зону здесь отвергаем:
		 * ответь разбор ею, и мы вернулись бы к тому же кругу
		 */
		::malloc_zone_t * zone = ::malloc_zone_from_ptr(ptr);
		// Если зона блока найдена и она не наша
		if((zone != nullptr) && (zone != &::__awh_zone__)){
			// Отдаём блок выдавшей его зоне
		 ...
		 * Отдаём блок прежней основной зоне
		 *
		 * Владельца не нашлось: ни прежняя зона, ни разбор адреса блока не признали. До
		 * захвата такой указатель ушёл бы в ту же самую зону и получил бы от неё тот же
		 * ответ - оттого поведение здесь и не меняется ни на волос
		 */
		if(::__awh_zone_previous__ != nullptr)
			// Отдаём блок прежней основной зоне
			::malloc_zone_free(::__awh_zone_previous__, ptr);
...
void awh::alloc::ZoneCapture::release() noexcept {
	...
	/**
	 * Вносим нашу зону ОБРАТНО, за прежней основной
	 *
	 * Снятие захвата означает «не обслуживать НОВЫЕ выдачи», а не «забыть выданное».
	 * Память, розданную прикладному коду до снятия, освобождать по-прежнему обязаны мы:
	 * договор захвата так и записан - распределитель обязан пережить снятие.
	 *
	 * Изъятая же зона делает выданные ею блоки НИЧЬИМИ. Освобождение таких блоков идёт
	 * обходом зон, ни одна из них блока не признаёт, и библиотека времени исполнения
	 * валит процесс: `pointer being freed was not allocated`. Доказано образцом
	 ...
	 *
	 * Внесённая обратно, зона основной НЕ становится - основная теперь прежняя, и новые
	 * выдачи идут мимо нас, - но обход зон её находит, и выданное нами освобождается
	 * нами же. Чужой блок с нею не спутается: `size` нашей зоны отвечает о нём нулём,
	 * и обход идёт дальше
	 */
	::malloc_zone_register(&::__awh_zone__);
