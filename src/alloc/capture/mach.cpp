...
/**
 * Подключаем заголовочный файл распределителя памяти
 *
 * Нужен он ради опроса расхода: сведения о зоне система читает средствами разбора
 * памяти, а величины эти знает распределитель. Зависимость идёт в ОДНУ сторону -
 * захват знает о распределителе, распределитель о захвате не знает вовсе
 */
...
namespace {
	...
	static void collect(::malloc_zone_t * zone, ::malloc_statistics_t * stats) noexcept {
		...
		/**
		 * Отвечаем настоящими величинами, а не нулями
		 *
		 * Нулями отвечать нельзя: сведения эти читают средства разбора памяти системы -
		 * `leaks`, `heap`, Instruments, - и зона, отчитавшаяся пустотой, для них пуста.
		 * Утечка в ней остаётся невидимой, а весь расход процесса выглядит вдвое меньше
		 * настоящего
		 *
		 * @note Опрос идёт СЧЁТЧИКАМИ, без замка кучи: средства разбора зовут сведения и
		 *       у остановленного процесса, где замок вправе оказаться захваченным чужим
		 *       потоком, - обращение к замку повесило бы разбор намертво
		 *
		 * @note Число блоков остаётся нулевым: посчитать его можно лишь обходом
		 *       удерживаемого, а тот ведётся только при включённом съёме мест выдачи.
		 *       Средства разбора числом этим лишь украшают печать, тогда как объём
		 *       читают всерьёз
		 */
		// Задаём занятое прикладным кодом прямо сейчас
		stats->size_in_use = awh::alloc::Allocator::property(awh::alloc::property_t::ALLOCATED);
		// Задаём наибольшее занятое за время работы
		stats->max_size_in_use = awh::alloc::Allocator::property(awh::alloc::property_t::PEAK);
		// Задаём взятое у системы под кучу
		stats->size_allocated = awh::alloc::Allocator::property(awh::alloc::property_t::HEAP);
	}
	/**
	...
	/**
	 * @brief Метод измерения блока прежним распределителем
	 *
	 * @warning Спрашивать `malloc_size` здесь НЕЛЬЗЯ: имя это обходит ВСЕ зоны процесса,
	 *          нашу в том числе, а наша зона отвечает на вопрос о размере нашим же
	 *          `msize`, - и тот, не узнав чужой блок, вновь зовёт `malloc_size`.
	 *          Возвратность эта срывала стек потомка после ветвления: падение выходило
	 *          в `Pages::discover` - самой глубокой точке пути, - и на слой страниц
	 *          указывало ложно
	 *
	 * @note Блок, выданный ТРЕТЬЕЙ зоной, здесь остаётся неопознанным: спросить о нём
	 *       можно лишь обходом зон, то есть той же возвратностью. Ответ нулём для него
	 *       совпадает с тем, что отвечала система до захвата
	 *
	 * @param ptr разбираемый адрес
	 * @return    размер блока, либо нуль если блок не прежней зоны
	 *
	 */
	static size_t priorMeasure(const void * ptr) noexcept {
		// Если измерять нечего либо нечем
		if((ptr == nullptr) || (::__awh_zone_previous__ == nullptr) || (::__awh_zone_previous__->size == nullptr))
			// Измерять нечего
			return 0;
		// Выводим размер блока, спрошенный у САМОЙ прежней зоны, а не у обхода зон
		return (* ::__awh_zone_previous__->size)(::__awh_zone_previous__, ptr);
	}
	/**
...
bool awh::alloc::ZoneCapture::acquire(const functions_t & hooks, functions_t & originals) noexcept {
	...
	this->_originals.msize = &::priorMeasure;
