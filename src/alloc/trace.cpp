...
namespace {
	 ...
	 *
	 * Хранится ключом системы, а НЕ поток-локальным местом: ленивое заведение того само
	 * зовёт выделение памяти, а мы стоим внутри выдачи. Ключ же заводится однажды, при
	 * заведении съёма, и обращение к нему памяти не просит
	 */
	#if defined(_WIN32) || defined(_WIN64)
