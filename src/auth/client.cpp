					...
					for(auto & param : params){
						...
						if((pos = param.find(L"=")) != wstring::npos){
							...
							// Если параметр является алгоритмом
								...
								if(value.compare(L"md5") == 0) this->digest.alg = alg_t::MD5;
								// Если алгоритм является SHA1
								else if(value.compare(L"sha1") == 0) this->digest.alg = alg_t::SHA1;
								// Если алгоритм является SHA256
								else if(value.compare(L"sha256") == 0) this->digest.alg = alg_t::SHA256;
								// Если алгоритм является SHA512
								else if(value.compare(L"sha512") == 0) this->digest.alg = alg_t::SHA512;
