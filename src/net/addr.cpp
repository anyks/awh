 ...
 *
 * @note Подключается он прежде разбора порядка октетов: запасная проба порядка ведётся
 *       числами заданной ширины, а примет порядка у оснастки MSVC нет вовсе - разбор
...
/**
 * Если оснастка примет порядка не заводит вовсе
 *
 * @note У оснастки MSVC примет порядка октетов нет ни одной, а порядок этот берётся
 *       у набора команд: все наборы, какие она собирает, - обратного порядка
 */
...
namespace {
	...
	void convertEndian(const uint8_t src[T], uint8_t dst[T]) noexcept {
		/**
		 * Порядок октетов спрашивается условием языка, а не условием препроцессора
		 *
		 * @details Примета порядка не всегда число: там, где оснастка своих примет не
		 *          заводит, порядок опознаётся пробой на ходу, и имя её разворачивается
		 *          в ВЫЗОВ. Препроцессор о вызовах не знает: имя он считает нулём, а
		 *          скобки - лишними знаками за указанием, и ветвь обратного порядка
		 *          выбиралась бы на машине обратного порядка молча. Условие языка верно
		 *          при всяком виде приметы, а постоянную оснастка сворачивает сама
		 */
		if(IS_LITTLE_ENDIAN)
			// На little-endian: переворачиваем все T байт
			::reverse_copy(src, src + T, dst);
		// На big-endian: ничего не делаем
		else ::memcpy(dst, src, T);
	}
	/**
	...
	bool parseIPv4DecQuad(string_view ip, uint8_t (& result)[4]) noexcept {
		...
		for(uint8_t i = 0; i < 4; ++i){
			...
			if(!::all_of(octet.begin(), octet.end(), [](const char letter) noexcept -> bool { return ascii::isDigit(letter); }))
				...
				return false;
			/**
			 * Ведущий нуль разрешён, но записи это НЕ МЕНЯЕТ: она остаётся десятичной
			 *
			 * @warning Расходится с разбором самостоятельного адреса IPv4 намеренно.
			 *          Там ведущий нуль означает восьмеричное число по наследию
			 *          `inet_aton` («010.0.0.1» это 8.0.0.1), здесь же по RFC 4291 хвост
			 *          записывается точечно-ДЕСЯТИЧНО («::ffff:010.0.0.1» это
			 *          ...10.0.0.1). Одна запись - два разных узла, и оба ответа верны
			 *          порознь
			 *
			 * @note Закреплено проверкой `NetLeadingZeroDependsOnFormTest`: расхождение
			 *       опасно тем, что всякая правка «для единообразия» выглядит улучшением
			 *
			 * @warning Прежде довод этот стоял ПУСТЫМ блоком под условием на ведущий
			 *          нуль - условие вычислялось и не делало ничего
			 *
			 */
			// Сбрасываем значение результата парсинга
			value = 0;
