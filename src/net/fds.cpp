 ...
 */
	...
	/**
	 * Прагма подключения библиотеки сокетов понятна только сборщику MSVC.
	 * Сборщик MinGW её не понимает и сообщает предупреждением при каждом
	 * разборе файла, а библиотека ws2_32 подключается ему сборкой CMake.
	 */
	#if defined(_MSC_VER)
