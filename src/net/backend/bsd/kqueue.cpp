...
namespace {
	...
	typedef struct Dirname : public net::fs_t {
		...
		explicit Dirname(const fmk_t * fmk, const log_t * log) noexcept :
		 handle(nullptr), fd(net::invalid_socket_t), addr(fmk, log) {}
	} dir_t;
	/**
	...
	typedef struct Client : public net::client_t {
		 ...
		 * @param log объект работы с логами
		 */
		explicit Client(awh::io_t * io, const eth_t * eth, const fmk_t * fmk, const log_t * log) noexcept :
		 io(io), fd(net::invalid_socket_t),
		 timeout(event::action_t::NONE),
		 transfer(fmk, log), addr(fmk, log), eth(eth) {}
	} client_t;
	/**
...
/**
 * Инкапсулируем статические объекты в пространство имён
 */
namespace {
	/**
	 * Подписываемся на пространство имён AWH
	 ...
	 */
	vector <struct kevent> __awh_change__;
	/**
	 ...
	 */
	unordered_map <event::id_t, intmd_t> __awh_inters__;
	/**
...
namespace io {
		...
		try {
			...
			if((i != node->timeouts.end()) && (i->second > 0)){
				...
				if((node->state.options & event::options::NOIOBLOCK) || (node->state.options & event::options::SMIOBLOCK)){
					...
					EV_SET(&::__awh_change__.back(), node->id, EVFILT_TIMER, EV_ADD | EV_ONESHOT, NOTE_USECONDS, static_cast <int64_t> (i->second) * 1000, node);
					// Увеличиваем количество событий
					ret.first->second.count++;
				// Устанавливаем таймаут на получение данных
				...
				case static_cast <uint8_t> (event::type_t::SEQPACKET): {
					...
					switch(static_cast <uint8_t> (peer->state.family)){
						...
						case static_cast <uint8_t> (event::family_t::UDS): {
							...
							const string & address = awh_cast <net::addr_fs_t *> (awh_cast <net::attr_uds_t *> (node->host.get())->path.get())->address;
						...
						case static_cast <uint8_t> (event::family_t::IPV4): {
							...
							node->eth->fillsource(event::node_t::PEER, source);
							...
							if(!node->blacklist.empty() || !node->whitelist.empty()){
								...
								if(::memcmp(&awh_cast <net::addr_mac_t *> (source.mac.get())->address[0], (uint8_t[6]){0}, 6) != 0){
									...
									peer->addr.mac(awh_cast <net::addr_mac_t *> (source.mac.get())->address);
								...
								peer->addr.v4(reinterpret_cast <struct sockaddr_in *> (&node->endpoint.client)->sin_addr.s_addr, net_addr_t::endian_t::LITTLE);
						...
						case static_cast <uint8_t> (event::family_t::IPV6): {
							...
							if(!node->blacklist.empty() || !node->whitelist.empty()){
								...
								// Устанавливаем полученный IP-адрес
								peer->addr.v6(awh_cast <net::addr_net_ipv6_t *> (source.ip.get())->address, net_addr_t::endian_t::LITTLE);
								// Получаем IP-адрес для проверки
								string ip = ::move(static_cast <string> (peer->addr));
								// Если адрес находится в чёрном списке
								if(!node->blacklist.empty() && (node->blacklist.find(ip) != node->blacklist.end())){
					...
					if(peer->transfer.input.data == nullptr){
						...
						peer->transfer.input.size = node->eth->bufferSize(peer->fd, net::socket_event_t::READ);
					...
					if((::__awh_kq__ != net::invalid_socket_t) && (ret.first->second->state.status.load(std::memory_order_acquire) != event::status_t::DESTROYED)){
						...
						if(ret.second){
							...
							EV_SET(&::__awh_change__.back(), peer->fd, EVFILT_WRITE, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, peer);
							// Устанавливаем количество событий
							ret.first->second.count = 2;
							// Устанавливаем индекс текущего элемента
							ret.first->second.index = (::__awh_change__.size() - 2);
							...
							if((i != peer->timeouts.end()) && (i->second > 0)){
								...
								} else node->eth->timeout(peer->fd, net::socket_event_t::READ, static_cast <uint32_t> (i->second));
							}
								...
								case static_cast <uint8_t> (event::family_t::PIPE): {
									...
									if((ipc->state.options & event::options::NOIOBLOCK) || (ipc->state.options & event::options::SMIOBLOCK)){
										...
										for(;;){
											...
											bytes = ::read(ipc->fd, ipc->transfer.input.data.get(), ipc->transfer.input.size);
											// Если мы получили ошибку
											if(bytes < 0){
									...
									// Если событие является блокирующим
										...
										// Выполняем чтение данных из TCP/IP сокета
										const int32_t bytes = ::read(ipc->fd, ipc->transfer.input.data.get(), ipc->transfer.input.size);
										// Если мы получили ошибку
										if(bytes < 0){
											// Устанавливаем текст ошибки
											const string error = fmk->format("Failed to receive data for inter-process communication: %s", ::strerror(errno));
				...
				case static_cast <uint8_t> (event::node_t::SERVER): {
					...
					switch(static_cast <uint8_t> (node->state.type)){
						...
						case static_cast <uint8_t> (event::type_t::DATAGRAM): {
							...
							switch(static_cast <uint8_t> (server->state.family)){
								...
								case static_cast <uint8_t> (event::family_t::IPV4): {
									...
									if(!server->blacklist.empty() || !server->whitelist.empty()){
										...
										// Выполняем извлечение сетевых параметров
										server->eth->fillsource(event::node_t::PEER, source);
										// Если MAC-адрес успешно получен
										if(::memcmp(&awh_cast <net::addr_mac_t *> (source.mac.get())->address[0], (uint8_t[6]){0}, 6) != 0){
											// Устанавливаем полученный MAC-адрес в объект события
							...
							// Выполняем чтение данных из TCP/IP сокета
							else bytes = ::recv(server->fd, server->transfer.input.data.get(), server->transfer.input.size, 0);
							// Если мы получили ошибку
							if(bytes < 0){
								// Устанавливаем текст ошибки
								const string error = fmk->format("Failed to receive data for peer: %s", ::strerror(errno));
							...
							case static_cast <uint8_t> (event::type_t::NONE): {
								...
								switch(static_cast <uint8_t> (ipc->state.family)){
									...
									case static_cast <uint8_t> (event::family_t::PIPE): {
										...
										const int32_t bytes = ::write(ipc->fd, reinterpret_cast <const uint8_t *> (ipc->transfer.output.data()) + ipc->transfer.offset, size);
					...
					switch(static_cast <uint8_t> (server->state.type)){
						...
						case static_cast <uint8_t> (event::type_t::DATAGRAM): {
							...
							if(!server->transfer.output.empty()){
								...
								// Выполняем отправку данных в TCP/IP сокет
								else bytes = ::send(server->fd, reinterpret_cast <const uint8_t *> (server->transfer.output.data()), server->transfer.output.size(), 0);
								// Если данные отправлены успешно
								if(bytes > 0){
									// Если функция обратного вызова для вывода записанных данных установлена
	 ...
	 */
	static bool socket(client_t * node, const log_t * log) noexcept {
		/**
		 * Выполняем перехват ошибок
		 */
		try {
				...
				case static_cast <uint8_t> (event::family_t::UDS): {
					...
					switch(static_cast <uint8_t> (node->state.type)){
						...
						case static_cast <uint8_t> (event::type_t::STREAM):
							...
							node->fd = ::socket(AF_UNIX, SOCK_STREAM, 0);
						break;
						// Для типа сокета SEQPACKET
						case static_cast <uint8_t> (event::type_t::SEQPACKET): {
							/**
							 ...
							 */
								...
								// Создаём сокет подключения
								node->fd = ::socket(AF_UNIX, SOCK_DGRAM, 0);
							/**
							 * Для остальных операционных систем
							 */
							#else
								// Создаём сокет подключения
								node->fd = ::socket(AF_UNIX, SOCK_SEQPACKET, 0);
							#endif
						} break;
						// Для типа сокета DATAGRAM
						case static_cast <uint8_t> (event::type_t::DATAGRAM):
					...
					switch(static_cast <uint8_t> (node->state.type)){
						...
						case static_cast <uint8_t> (event::type_t::RAW): {
							...
							switch(static_cast <uint8_t> (node->state.family)){
								...
								case static_cast <uint8_t> (event::family_t::UDPV6): {
									...
									switch(static_cast <uint8_t> (node->state.protocol)){
										...
										case static_cast <uint8_t> (event::protocol_t::NONE):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET6, SOCK_RAW, 0);
										break;
										// Если протокол определён как UDP
										case static_cast <uint8_t> (event::protocol_t::UDP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET6, SOCK_RAW, IPPROTO_UDP);
										break;
										// Если протокол определён как ICMP
										case static_cast <uint8_t> (event::protocol_t::ICMP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET6, SOCK_RAW, IPPROTO_ICMPV6);
										break;
										// Если установлен другой протокол
										default: ok = false;
									}
							...
							if(!ok){
								...
								#else
									// Выводим сообщение об ошибке
									log->print("RAW socket type only supports UDP protocol or Unix family socket with empty protocol", log_t::flag_t::WARNING);
								#endif
							}
						} break;
						// Для типа сокета DATAGRAM
							...
							switch(static_cast <uint8_t> (node->state.family)){
								...
								case static_cast <uint8_t> (event::family_t::UDPV4): {
									...
									switch(static_cast <uint8_t> (node->state.protocol)){
										...
										case static_cast <uint8_t> (event::protocol_t::IGMP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET, SOCK_DGRAM, IPPROTO_IGMP);
										break;
										// Если протокол определён как ICMP
										case static_cast <uint8_t> (event::protocol_t::ICMP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET, SOCK_DGRAM, IPPROTO_ICMP);
										break;
										// Если установлен другой протокол
										default: ok = false;
									}
								...
								case static_cast <uint8_t> (event::family_t::UDPV6): {
									...
									switch(static_cast <uint8_t> (node->state.protocol)){
										...
										case static_cast <uint8_t> (event::protocol_t::NONE):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET6, SOCK_DGRAM, 0);
										break;
										// Если протокол определён как UDP
										case static_cast <uint8_t> (event::protocol_t::UDP):
										// Если протокол определён как DTLS
										case static_cast <uint8_t> (event::protocol_t::DTLS):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP);
										break;
										// Если протокол определён как ICMP
										case static_cast <uint8_t> (event::protocol_t::ICMP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET6, SOCK_DGRAM, IPPROTO_ICMPV6);
										break;
										// Если установлен другой протокол
										default: ok = false;
									}
						...
						default: {
							 ...
							 */
								...
								// Выводим сообщение об ошибке
								log->debug(
									"An socket for a UDP event cannot be created because it has an invalid initialization type",
									__PRETTY_FUNCTION__, std::make_tuple(node, (node != nullptr ? node->id : 0)), log_t::flag_t::WARNING
								);
							/**
							 * Если режим отладки не включён
							...
							#else
								// Выводим сообщение об ошибке
								log->print("An socket for a UDP event cannot be created because it has an invalid initialization type", log_t::flag_t::WARNING);
							#endif
						}
					}
				} break;
							...
							switch(static_cast <uint8_t> (node->state.family)){
								...
								case static_cast <uint8_t> (event::family_t::IPV4): {
									...
									switch(static_cast <uint8_t> (node->state.protocol)){
										...
										case static_cast <uint8_t> (event::protocol_t::TCP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
										break;
										// Если протокол определён как SCTP
										case static_cast <uint8_t> (event::protocol_t::SCTP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET, SOCK_STREAM, IPPROTO_SCTP);
										break;
										// Если установлен другой протокол
										default: ok = false;
									}
								...
								case static_cast <uint8_t> (event::family_t::IPV6): {
									...
									switch(static_cast <uint8_t> (node->state.protocol)){
										...
										case static_cast <uint8_t> (event::protocol_t::NONE):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET6, SOCK_STREAM, 0);
										break;
										// Если протокол определён как TCP
										case static_cast <uint8_t> (event::protocol_t::TCP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP);
										break;
										// Если протокол определён как SCTP
										case static_cast <uint8_t> (event::protocol_t::SCTP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET6, SOCK_STREAM, IPPROTO_SCTP);
										break;
										// Если установлен другой протокол
										default: ok = false;
									}
							...
							switch(static_cast <uint8_t> (node->state.family)){
								...
								case static_cast <uint8_t> (event::family_t::IPV4): {
									...
									switch(static_cast <uint8_t> (node->state.protocol)){
										...
										case static_cast <uint8_t> (event::protocol_t::SCTP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET, SOCK_SEQPACKET, IPPROTO_SCTP);
										break;
										// Если установлен другой протокол
										default: ok = false;
									}
								...
								case static_cast <uint8_t> (event::family_t::IPV6): {
									...
									switch(static_cast <uint8_t> (node->state.protocol)){
										...
										case static_cast <uint8_t> (event::protocol_t::SCTP):
											// Создаём сокет подключения
											node->fd = ::socket(AF_INET6, SOCK_SEQPACKET, IPPROTO_SCTP);
										break;
										// Если установлен другой протокол
										default: ok = false;
									}
			...
			return (node->fd != net::invalid_socket_t);
		/**
		 * Если возникает ошибка
		 */
		} catch(const exception & error) {
		...
		if((i != ::__awh_nodes__.end()) && (i->second->state.status.load(std::memory_order_acquire) == event::status_t::NONE)){
			...
			switch(static_cast <uint8_t> (i->second->state.node)){
					...
					if(node->fd != net::invalid_socket_t){
						...
						if(node->transfer.input.data == nullptr){
							...
							if(node->state.family == event::family_t::PIPE)
								...
								node->transfer.input.size = 0x1000;
							// Устанавливаем размер буфера на чтение
							else node->transfer.input.size = this->_eth.bufferSize(node->fd, net::socket_event_t::READ);
							// Выполняем создание нового буфера на чтение
							node->transfer.input.data = make_unique <uint8_t []> (node->transfer.input.size);
						}
						// Создаём объект промежуточного звена
						...
						if((result = ret.second)){
							...
							::__awh_change__.push_back((struct kevent){});
							// Устанавливаем событие на запись но отключаем его
							EV_SET(&::__awh_change__.back(), node->fd, EVFILT_WRITE, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, node);
							// Устанавливаем количество событий
							ret.first->second.count = 2;
							// Устанавливаем индекс текущего элемента
							ret.first->second.index = (::__awh_change__.size() - 2);
				...
				case static_cast <uint8_t> (event::node_t::CLIENT): {
					...
					if((node->fd != net::invalid_socket_t) && (node->target != nullptr)){
						// Если буфер на чтение не создан
						if(node->transfer.input.data == nullptr){
							// Устанавливаем размер буфера на чтение
							node->transfer.input.size = this->_eth.bufferSize(node->fd, net::socket_event_t::READ);
							// Выполняем создание нового буфера на чтение
							node->transfer.input.data = make_unique <uint8_t []> (node->transfer.input.size);
						}
						/**
						...
						switch(static_cast <uint8_t> (node->state.family)){
							...
							case static_cast <uint8_t> (event::family_t::UDS): {
								...
								if((result = ret.second)){
									...
									// Устанавливаем событие на чтение но отключаем его
									else EV_SET(&::__awh_change__.back(), node->fd, EVFILT_READ, EV_ADD | EV_DISABLE, 0, 0, node);
									// Добавляем новое событие в список изменений
									::__awh_change__.push_back((struct kevent){});
									// Устанавливаем событие на запись но отключаем его
									EV_SET(&::__awh_change__.back(), node->fd, EVFILT_WRITE, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, node);
						...
						switch(static_cast <uint8_t> (node->state.family)){
											...
											if(pos != string::npos){
												...
												if(::bind(node->fd, reinterpret_cast <struct sockaddr *> (&node->endpoint.server), size) < 0){
													...
													// Если функция обратного вызова для вывода события установлена
														...
														#if DEBUG_MODE
															// Выводим сообщение об ошибке
															this->_log->debug("%s", __PRETTY_FUNCTION__, std::make_tuple(node->fd, unixsocket, serverName), log_t::flag_t::CRITICAL, ::strerror(errno));
														/**
														 * Если режим отладки не включён
														 */
														#else
							...
							case static_cast <uint8_t> (event::family_t::UDPV4): {
								...
								if(node->host != nullptr){
									...
									// Если сокет принадлежит к семейству UDPv4
									if(node->state.family == event::family_t::UDPV4){
										// Если буфер на чтение не создан
										if(node->transfer.input.data == nullptr){
											// Устанавливаем размер буфера на чтение
											node->transfer.input.size = this->_eth.bufferSize(node->fd, net::socket_event_t::READ);
							...
							case static_cast <uint8_t> (event::family_t::UDPV6): {
								...
								if(node->host != nullptr){
									...
									// Если сокет принадлежит к семейству UDPv6
									if(node->state.family == event::family_t::UDPV6){
										// Если буфер на чтение не создан
										if(node->transfer.input.data == nullptr){
											// Устанавливаем размер буфера на чтение
											node->transfer.input.size = this->_eth.bufferSize(node->fd, net::socket_event_t::READ);
				...
				case static_cast <uint8_t> (event::node_t::PEER): {
					...
					switch(static_cast <uint8_t> (peer->state.family)){
						...
						case static_cast <uint8_t> (event::family_t::UDPV4): {
							...
							if(remote->ip->size == 4){
								// Устанавливаем полученный IP-адрес
								peer->addr.v4(awh_cast <net::addr_net_ipv4_t *> (remote->ip.get())->address, net_addr_t::endian_t::LITTLE);
								// Возвращаем результат работы функции
						...
						case static_cast <uint8_t> (event::family_t::UDPV6): {
							...
							if(remote->ip->size == 16){
								// Устанавливаем полученный IP-адрес
								peer->addr.v6(awh_cast <net::addr_net_ipv6_t *> (remote->ip.get())->address, net_addr_t::endian_t::LITTLE);
								// Возвращаем результат работы функции
								return static_cast <string> (peer->addr);
							// Если размер IP-адреса не соответствует IPv4
							} else {
				...
				case static_cast <uint8_t> (event::node_t::CLIENT): {
					...
					switch(static_cast <uint8_t> (client->state.family)){
						...
						case static_cast <uint8_t> (event::family_t::UDPV4): {
							...
							if(target->ip->size == 4){
								// Устанавливаем полученный IP-адрес
								client->addr.v4(awh_cast <net::addr_net_ipv4_t *> (target->ip.get())->address, net_addr_t::endian_t::LITTLE);
								// Возвращаем результат работы функции
				...
				case static_cast <uint8_t> (event::node_t::SERVER): {
					...
					switch(static_cast <uint8_t> (server->state.family)){
						...
						case static_cast <uint8_t> (event::family_t::UDPV4): {
							...
							if(host->ip->size == 4){
								// Устанавливаем полученный IP-адрес
								server->addr.v4(awh_cast <net::addr_net_ipv4_t *> (host->ip.get())->address, net_addr_t::endian_t::LITTLE);
								// Возвращаем результат работы функции
							...
							switch(static_cast <uint8_t> (client->state.family)){
								...
								case static_cast <uint8_t> (event::family_t::UDPV4): {
									...
									if((client->source != nullptr) && (client->source->size == 4)){
										...
										if(::memcmp(&awh_cast <net::addr_mac_t *> (source.mac.get())->address[0], (uint8_t[6]){0}, 6) != 0){
											// Устанавливаем полученный MAC-адрес в объект события
											client->addr.mac(awh_cast <net::addr_mac_t *> (source.mac.get())->address);
											// Выводим результат работы функции
											return static_cast <string> (client->addr);
										// Если MAC-адрес не получен
										} else {
					...
					if(address == i->second->state.address){
						...
						switch(static_cast <uint8_t> (i->second->state.node)){
							...
							case static_cast <uint8_t> (event::node_t::PEER): {
								...
								peer->addr.v4(awh_cast <net::addr_net_ipv4_t *> (awh_cast <net::attr_net_t *> (peer->remote.get())->ip.get())->address, net_addr_t::endian_t::LITTLE);
								// Выводим результат работы функции
								return static_cast <string> (peer->addr);
							}
							// Если узел является клиентом
							case static_cast <uint8_t> (event::node_t::CLIENT): {
								...
								if(client->source == nullptr)
									...
									break;
								// Устанавливаем полученный IPv4-адрес
								client->addr.v4(awh_cast <net::addr_net_ipv4_t *> (client->source.get())->address, net_addr_t::endian_t::LITTLE);
								// Выводим результат работы функции
								return static_cast <string> (client->addr);
							}
							// Если узел является сервером
					...
					if(address == i->second->state.address){
						...
						switch(static_cast <uint8_t> (i->second->state.node)){
							...
							case static_cast <uint8_t> (event::node_t::PEER): {
								...
								peer->addr.v6(awh_cast <net::addr_net_ipv6_t *> (awh_cast <net::attr_net_t *> (peer->remote.get())->ip.get())->address, net_addr_t::endian_t::LITTLE);
								// Выводим результат работы функции
								return static_cast <string> (peer->addr);
							}
							// Если узел является клиентом
							case static_cast <uint8_t> (event::node_t::CLIENT): {
								...
								if(client->source == nullptr)
									...
									break;
								// Устанавливаем полученный IPv6-адрес
								client->addr.v6(awh_cast <net::addr_net_ipv6_t *> (client->source.get())->address, net_addr_t::endian_t::LITTLE);
								// Выводим результат работы функции
								return static_cast <string> (client->addr);
							}
							// Если узел является сервером
				...
				case static_cast <uint8_t> (event::address_t::IPV4): {
					...
					switch(static_cast <uint8_t> (i->second->state.node)){
						...
						case static_cast <uint8_t> (event::node_t::CLIENT): {
							...
							auto client = awh_cast <client_t *> (i->second.get());
							// Если типы адресов соответствуют
							if((client->state.family == event::family_t::IPV4) || (client->state.family == event::family_t::UDPV4)){
								// Выполняем парсинг IPv4-адреса
						...
						case static_cast <uint8_t> (event::node_t::SERVER): {
							...
							auto server = awh_cast <server_t *> (i->second.get());
							// Если типы адресов соответствуют
							if((server->state.family == event::family_t::IPV4) || (server->state.family == event::family_t::UDPV4)){
								// Выполняем парсинг IPv4-адреса
				...
				case static_cast <uint8_t> (event::address_t::IPV6): {
					...
					switch(static_cast <uint8_t> (i->second->state.node)){
						...
						case static_cast <uint8_t> (event::node_t::CLIENT): {
							...
							auto client = awh_cast <client_t *> (i->second.get());
							// Если типы адресов соответствуют
							if((client->state.family == event::family_t::IPV6) || (client->state.family == event::family_t::UDPV6)){
								// Устанавливаем полученный IPv6-адрес
						...
						case static_cast <uint8_t> (event::node_t::SERVER): {
							...
							auto server = awh_cast <server_t *> (i->second.get());
							// Если типы адресов соответствуют
							if((server->state.family == event::family_t::IPV6) || (server->state.family == event::family_t::UDPV6)){
								// Устанавливаем полученный IPv6-адрес
				...
				switch(static_cast <uint8_t> (i->second->state.family)){
					...
					case static_cast <uint8_t> (event::family_t::UDS): {
						...
						switch(static_cast <uint8_t> (i->second->state.type)){
							...
							case static_cast <uint8_t> (event::type_t::STREAM): {
								// Выполняем создание события
								auto ret = ::__awh_nodes__.emplace(io::identifier(), make_unique <client_t> (this, &this->_eth, this->_fmk, this->_log));
								// Устанавливаем идентификатор события
								ret.first->second->id = ret.first->first;
								// Устанавливаем флаг протокола сокета
								ret.first->second->state.protocol = protocol;
								...
								switch(static_cast <uint8_t> (i->second->state.node)){
									...
									case static_cast <uint8_t> (event::node_t::CLIENT): {
										...
										// Создаём сокет подключения
										second->fd = ::socket(AF_UNIX, SOCK_STREAM, 0);
										// Запоминаем размер структуры
							...
							case static_cast <uint8_t> (event::type_t::SEQPACKET): {
								...
								switch(static_cast <uint8_t> (i->second->state.node)){
									...
									case static_cast <uint8_t> (event::node_t::CLIENT): {
										...
										#if __APPLE__ || __MACH__ || __NetBSD__ || __OpenBSD__
											// Создаём сокет подключения
											second->fd = ::socket(AF_UNIX, SOCK_DGRAM, 0);
										/**
										 * Для остальных операционных систем
										 */
										#else
											// Создаём сокет подключения
											second->fd = ::socket(AF_UNIX, SOCK_SEQPACKET, 0);
										#endif
										// Запоминаем размер структуры
										second->endpoint.size = sizeof(struct sockaddr_un);
										// Выполняем копирование объекта подключения клиента
									...
									case static_cast <uint8_t> (event::node_t::CLIENT): {
										...
										switch(static_cast <uint8_t> (first->state.family)){
											...
											case static_cast <uint8_t> (event::family_t::UDPV4): {
												...
												switch(static_cast <uint8_t> (protocol)){
													...
													case static_cast <uint8_t> (event::protocol_t::RAW):
														// Создаём сокет подключения
														second->fd = ::socket(AF_INET, SOCK_RAW, IPPROTO_RAW);
													break;
													// Если протокол определён как ICMP
													case static_cast <uint8_t> (event::protocol_t::ICMP):
														// Создаём сокет подключения
														second->fd = ::socket(AF_INET, SOCK_RAW, IPPROTO_ICMP);
													break;
													// Если протокол определён как IGMP
													case static_cast <uint8_t> (event::protocol_t::IGMP):
														// Создаём сокет подключения
														second->fd = ::socket(AF_INET, SOCK_RAW, IPPROTO_IGMP);
													break;
													// Если установлен другой протокол
													default: ok = false;
												}
											...
											case static_cast <uint8_t> (event::family_t::UDPV6): {
												...
												switch(static_cast <uint8_t> (protocol)){
													...
													case static_cast <uint8_t> (event::protocol_t::ICMP):
														// Создаём сокет подключения
														second->fd = ::socket(AF_INET6, SOCK_RAW, IPPROTO_ICMPV6);
													break;
													// Если установлен другой протокол
													default: ok = false;
												}
										...
										if(ok){
											...
											switch(static_cast <uint8_t> (protocol)){
												...
												case static_cast <uint8_t> (event::protocol_t::NONE):
													// Создаём сокет подключения
													second->fd = ::socket(first->endpoint.client.ss_family, SOCK_RAW, 0);
												break;
												// Если протокол определён как UDP
												case static_cast <uint8_t> (event::protocol_t::UDP):
													// Создаём сокет подключения
													second->fd = ::socket(first->endpoint.client.ss_family, SOCK_RAW, IPPROTO_UDP);
												break;
												// Если установлен другой протокол
												default: ok = false;
											}
									...
									case static_cast <uint8_t> (event::node_t::SERVER): {
										...
										if(ok){
											...
											switch(static_cast <uint8_t> (protocol)){
												...
												case static_cast <uint8_t> (event::protocol_t::NONE):
													// Создаём сокет подключения
													second->fd = ::socket(first->endpoint.server.ss_family, SOCK_RAW, 0);
												break;
												// Если протокол определён как UDP
												case static_cast <uint8_t> (event::protocol_t::UDP):
													// Создаём сокет подключения
													second->fd = ::socket(first->endpoint.server.ss_family, SOCK_RAW, IPPROTO_UDP);
												break;
												// Если установлен другой протокол
												default: ok = false;
											}
									...
									case static_cast <uint8_t> (event::node_t::CLIENT): {
										...
										switch(static_cast <uint8_t> (first->state.family)){
											...
											case static_cast <uint8_t> (event::family_t::UDPV4): {
												...
												switch(static_cast <uint8_t> (protocol)){
													...
													case static_cast <uint8_t> (event::protocol_t::ICMP):
														// Создаём сокет подключения
														second->fd = ::socket(AF_INET, SOCK_DGRAM, IPPROTO_ICMP);
													break;
													// Если протокол определён как IGMP
													case static_cast <uint8_t> (event::protocol_t::IGMP):
														// Создаём сокет подключения
														second->fd = ::socket(AF_INET, SOCK_DGRAM, IPPROTO_IGMP);
													break;
													// Если установлен другой протокол
													default: ok = false;
												}
											...
											case static_cast <uint8_t> (event::family_t::UDPV6): {
												...
												switch(static_cast <uint8_t> (protocol)){
													...
													case static_cast <uint8_t> (event::protocol_t::ICMP):
														// Создаём сокет подключения
														second->fd = ::socket(AF_INET6, SOCK_DGRAM, IPPROTO_ICMPV6);
													break;
													// Если установлен другой протокол
													default: ok = false;
												}
										...
										if(ok){
											...
											switch(static_cast <uint8_t> (protocol)){
												...
												case static_cast <uint8_t> (event::protocol_t::NONE):
													// Создаём сокет подключения
													second->fd = ::socket(first->endpoint.client.ss_family, SOCK_DGRAM, 0);
												break;
												// Если протокол определён как UDP
												case static_cast <uint8_t> (event::protocol_t::UDP):
												// Если протокол определён как DTLS
									...
									case static_cast <uint8_t> (event::node_t::SERVER): {
										...
										if(ok){
											...
											switch(static_cast <uint8_t> (protocol)){
												...
												case static_cast <uint8_t> (event::protocol_t::NONE):
													// Создаём сокет подключения
													second->fd = ::socket(first->endpoint.server.ss_family, SOCK_DGRAM, 0);
												break;
												// Если протокол определён как UDP
												case static_cast <uint8_t> (event::protocol_t::UDP):
												// Если протокол определён как DTLS
								...
								switch(static_cast <uint8_t> (i->second->state.node)){
									...
									case static_cast <uint8_t> (event::node_t::CLIENT): {
										...
										switch(static_cast <uint8_t> (protocol)){
											...
											case static_cast <uint8_t> (event::protocol_t::NONE):
												// Создаём сокет подключения
												second->fd = ::socket(first->endpoint.client.ss_family, SOCK_STREAM, 0);
											break;
											// Если протокол определён как TCP
											case static_cast <uint8_t> (event::protocol_t::TCP):
												// Создаём сокет подключения
												second->fd = ::socket(first->endpoint.client.ss_family, SOCK_STREAM, IPPROTO_TCP);
											break;
											// Если протокол определён как SCTP
											case static_cast <uint8_t> (event::protocol_t::SCTP):
												// Создаём сокет подключения
												second->fd = ::socket(first->endpoint.client.ss_family, SOCK_STREAM, IPPROTO_SCTP);
											break;
											// Если установлен другой протокол
											default: ok = false;
										}
									...
									case static_cast <uint8_t> (event::node_t::SERVER): {
										...
										switch(static_cast <uint8_t> (protocol)){
											...
											case static_cast <uint8_t> (event::protocol_t::NONE):
												// Создаём сокет подключения
												second->fd = ::socket(first->endpoint.server.ss_family, SOCK_STREAM, 0);
											break;
											// Если протокол определён как TCP
											case static_cast <uint8_t> (event::protocol_t::TCP):
												// Создаём сокет подключения
												second->fd = ::socket(first->endpoint.server.ss_family, SOCK_STREAM, IPPROTO_TCP);
											break;
											// Если протокол определён как SCTP
											case static_cast <uint8_t> (event::protocol_t::SCTP):
												// Создаём сокет подключения
												second->fd = ::socket(first->endpoint.server.ss_family, SOCK_STREAM, IPPROTO_SCTP);
											break;
											// Если установлен другой протокол
											default: ok = false;
										}
								...
								switch(static_cast <uint8_t> (i->second->state.node)){
									...
									case static_cast <uint8_t> (event::node_t::CLIENT): {
										...
										switch(static_cast <uint8_t> (protocol)){
											...
											case static_cast <uint8_t> (event::protocol_t::SCTP):
												// Создаём сокет подключения
												second->fd = ::socket(first->endpoint.client.ss_family, SOCK_SEQPACKET, IPPROTO_SCTP);
											break;
											// Если установлен другой протокол
											default: ok = false;
										}
									...
									case static_cast <uint8_t> (event::node_t::SERVER): {
										...
										switch(static_cast <uint8_t> (protocol)){
											...
											case static_cast <uint8_t> (event::protocol_t::SCTP):
												// Создаём сокет подключения
												second->fd = ::socket(first->endpoint.server.ss_family, SOCK_SEQPACKET, IPPROTO_SCTP);
											break;
											// Если установлен другой протокол
											default: ok = false;
										}
	...
	try {
		...
		if((i != ::__awh_nodes__.end()) && (i->second->state.status.load(std::memory_order_acquire) != event::status_t::DESTROYED)){
			...
			if((result = (fd != net::invalid_socket_t))){
							...
							if(i->second->state.status.load(std::memory_order_acquire) != event::status_t::NONE){
								...
								switch(static_cast <uint8_t> (i->second->state.node)){
									...
									case static_cast <uint8_t> (event::node_t::SERVER): {
										...
										EV_SET(&::__awh_change__.back(), fd, EVFILT_READ, EV_DELETE, 0, 0, i->second.get());
										// Добавляем новое событие в список изменений
										::__awh_change__.push_back((struct kevent){});
										// Если событие находится в состоянии паузы
										if(i->second->state.status.load(std::memory_order_acquire) == event::status_t::PAUSED){
											// Устанавливаем событие на чтение но отключаем его
											EV_SET(&::__awh_change__.back(), fd, EVFILT_READ, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, i->second.get());
											// Устанавливаем количество событий
											ret.first->second.count += 2;
											// Если событие успешно добавлено
											if(ret.second)
										...
										// Если событие находится в активном состоянии
											...
											EV_SET(&::__awh_change__.back(), fd, EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0, i->second.get());
											// Устанавливаем количество событий
											ret.first->second.count += 2;
											// Если событие успешно добавлено
											if(ret.second)
							...
							if(i->second->state.status.load(std::memory_order_acquire) != event::status_t::NONE){
								...
								switch(static_cast <uint8_t> (i->second->state.node)){
									...
									case static_cast <uint8_t> (event::node_t::SERVER): {
										...
										if(i->second->state.status.load(std::memory_order_acquire) == event::status_t::PAUSED){
											// Устанавливаем событие на чтение но отключаем его
											EV_SET(&::__awh_change__.back(), fd, EVFILT_READ, EV_ADD | EV_DISABLE, 0, 0, i->second.get());
											// Устанавливаем количество событий
											ret.first->second.count += 2;
											// Если событие успешно добавлено
											if(ret.second)
										...
										// Если событие находится в активном состоянии
											...
											// Устанавливаем событие на чтение
											EV_SET(&::__awh_change__.back(), fd, EVFILT_READ, EV_ADD, 0, 0, i->second.get());
											// Устанавливаем количество событий
											ret.first->second.count += 2;
											// Если событие успешно добавлено
											if(ret.second)
				...
				// Если опция не передана как KEEPALIVE
				else
								...
								switch(static_cast <uint8_t> (ipc->state.family)){
									...
									case static_cast <uint8_t> (event::family_t::PIPE): {
										...
										if(ipc->state.options & event::options::NOIOBLOCK){
											...
											if(ipc->transfer.output.empty()){
												// Выполняем отправку данных в TCP/IP сокет
												const int32_t bytes = ::write(ipc->fd, data, size);
												// Если данные отправлены успешно
												if((result = (bytes > 0))){
													// Если данные отправлены не полностью
													if(static_cast <size_t> (bytes) != size){
										...
										// Если сокет является полублокирующим
											...
											if(this->_eth.noblocking(ipc->fd, net::socket_mode_t::DISABLED)){
												...
												// Если произошла какая-то ошибка при отправке данных
													...
													this->_eth.noblocking(ipc->fd, net::socket_mode_t::ENABLED);
													// Устанавливаем текст ошибки
													const string error = this->_fmk->format("Failed to send data from inter-process communication: %s", ::strerror(errno));
													// Если установлена функция обратного вызова
													if(ipc->callbacks.error != nullptr)
							...
							case static_cast <uint8_t> (event::type_t::DATAGRAM): {
								...
								if(server->state.options & event::options::NOIOBLOCK){
									...
									if(server->transfer.output.empty()){
										...
										if(server->state.options & event::options::ONSHOT)
											// Выполняем отправку данных в UDP сокет
											bytes = ::sendto(server->fd, data, size, 0, reinterpret_cast <struct sockaddr *> (&server->endpoint.client), server->endpoint.size);
										// Если флаг однократного использования сокета не установлен, выполняем отправку данных в TCP/IP сокет
										else bytes = ::send(server->fd, data, size, 0);
										// Если данные отправлены успешно
										if((result = (bytes > 0))){
											// Если функция обратного вызова для вывода записанных данных установлена
											if(server->callbacks.write != nullptr)
			...
			if((i != ::__awh_nodes__.end()) && (i->second->state.status.load(std::memory_order_acquire) != event::status_t::DESTROYED)){
				...
				switch(static_cast <uint8_t> (i->second->state.node)){
					...
					case static_cast <uint8_t> (event::node_t::SERVER): {
						 ...
						 * Определяем тип адреса события
						 */
						switch(static_cast <uint8_t> (node->state.address)){
							// Если тип адреса принадлежит к Unix Domain Socket
							case static_cast <uint8_t> (event::address_t::UDS): {
								// Если адрес соответствует адресу файловой системы
								if(node->addr.check(value, net_addr_t::type_t::FS))
							...
							case static_cast <uint8_t> (event::address_t::IPV6): {
								 ...
								 */
								switch(static_cast <uint8_t> (node->addr.host(value))){
						...
						case static_cast <uint8_t> (event::family_t::UDPV6): {
							...
							switch(static_cast <uint8_t> (action)){
								...
								case static_cast <uint8_t> (event::action_t::READ): {
									...
									const int32_t length = this->_eth.bufferSize(ipc->fd, net::socket_event_t::READ);
								...
								case static_cast <uint8_t> (event::action_t::WRITE): {
									...
									const int32_t length = this->_eth.bufferSize(ipc->fd, net::socket_event_t::WRITE);
									// Если установка размера буфера прошла успешно
									if(length > 0)
					...
					switch(static_cast <uint8_t> (peer->state.family)){
						...
						case static_cast <uint8_t> (event::family_t::UDPV6): {
							...
							switch(static_cast <uint8_t> (action)){
								...
								case static_cast <uint8_t> (event::action_t::READ): {
									// Устанавливаем размер буфера для чтения
									const int32_t length = this->_eth.bufferSize(peer->fd, net::socket_event_t::READ);
									// Если установка размера буфера прошла успешно
									if(length > 0)
										// Выводим размер буфера на чтение
										return static_cast <size_t> (length);
								...
								case static_cast <uint8_t> (event::action_t::WRITE): {
									// Устанавливаем размер буфера для записи
									const int32_t length = this->_eth.bufferSize(peer->fd, net::socket_event_t::WRITE);
									// Если установка размера буфера прошла успешно
									if(length > 0)
										// Выводим размер буфера на запись
										return static_cast <size_t> (length);
					...
					switch(static_cast <uint8_t> (client->state.family)){
						...
						case static_cast <uint8_t> (event::family_t::UDPV6): {
							...
							switch(static_cast <uint8_t> (action)){
								...
								case static_cast <uint8_t> (event::action_t::READ): {
									// Устанавливаем размер буфера для чтения
									const int32_t length = this->_eth.bufferSize(client->fd, net::socket_event_t::READ);
									// Если установка размера буфера прошла успешно
									if(length > 0)
										// Выводим размер буфера на чтение
										return static_cast <size_t> (length);
								...
								case static_cast <uint8_t> (event::action_t::WRITE): {
									// Устанавливаем размер буфера для записи
									const int32_t length = this->_eth.bufferSize(client->fd, net::socket_event_t::WRITE);
									// Если установка размера буфера прошла успешно
									if(length > 0)
										// Выводим размер буфера на запись
										return static_cast <size_t> (length);
					...
					switch(static_cast <uint8_t> (ipc->state.family)){
						...
						case static_cast <uint8_t> (event::family_t::UDPV6): {
							...
							switch(static_cast <uint8_t> (action)){
								...
								case static_cast <uint8_t> (event::action_t::READ): {
									// Устанавливаем размер буфера для чтения
									const int32_t length = this->_eth.bufferSize(ipc->fd, net::socket_event_t::READ, static_cast <int32_t> (size));
									// Если установка размера буфера прошла успешно
									if((result = (length > 0))){
										// Устанавливаем размер буфера на чтение
								...
								case static_cast <uint8_t> (event::action_t::WRITE):
									// Устанавливаем размер буфера для записи
									result = (this->_eth.bufferSize(ipc->fd, net::socket_event_t::WRITE, static_cast <int32_t> (size)) > 0);
								break;
							}
						} break;
					}
					...
					switch(static_cast <uint8_t> (peer->state.family)){
						...
						case static_cast <uint8_t> (event::family_t::UDPV6): {
							...
							switch(static_cast <uint8_t> (action)){
								...
								case static_cast <uint8_t> (event::action_t::READ): {
									// Устанавливаем размер буфера для чтения
									const int32_t length = this->_eth.bufferSize(peer->fd, net::socket_event_t::READ, static_cast <int32_t> (size));
									// Если установка размера буфера прошла успешно
									if((result = (length > 0))){
										// Устанавливаем размер буфера на чтение
								...
								case static_cast <uint8_t> (event::action_t::WRITE):
									// Устанавливаем размер буфера для записи
									result = (this->_eth.bufferSize(peer->fd, net::socket_event_t::WRITE, static_cast <int32_t> (size)) > 0);
								break;
							}
						} break;
					}
					...
					switch(static_cast <uint8_t> (client->state.family)){
						...
						case static_cast <uint8_t> (event::family_t::PIPE): {
							...
							switch(static_cast <uint8_t> (action)){
								...
								case static_cast <uint8_t> (event::action_t::WRITE):
									...
									result = (this->_eth.bufferSize(client->fd, net::socket_event_t::WRITE) > 0);
								break;
							}
						} break;
						// Для семейства UNIX-доменных сокетов
						...
						case static_cast <uint8_t> (event::family_t::UDPV6): {
							...
							switch(static_cast <uint8_t> (action)){
								...
								case static_cast <uint8_t> (event::action_t::READ): {
									// Устанавливаем размер буфера для чтения
									const int32_t length = this->_eth.bufferSize(client->fd, net::socket_event_t::READ, static_cast <int32_t> (size));
									// Если установка размера буфера прошла успешно
									if((result = (length > 0))){
										// Устанавливаем размер буфера на чтение
										client->transfer.input.size = static_cast <size_t> (length);
								...
								case static_cast <uint8_t> (event::action_t::WRITE):
									// Устанавливаем размер буфера для записи
									result = (this->_eth.bufferSize(client->fd, net::socket_event_t::WRITE, static_cast <int32_t> (size)) > 0);
								break;
							}
						} break;
					}
					...
					switch(static_cast <uint8_t> (server->state.family)){
						...
						case static_cast <uint8_t> (event::family_t::UDPV6): {
							...
							switch(static_cast <uint8_t> (action)){
								...
								case static_cast <uint8_t> (event::action_t::READ): {
									// Устанавливаем размер буфера для чтения
									const int32_t length = this->_eth.bufferSize(server->fd, net::socket_event_t::READ, static_cast <int32_t> (size));
									// Если установка размера буфера прошла успешно
									if((result = (length > 0))){
										// Устанавливаем размер буфера на чтение
					   ...
					   (peer->state.status.load(std::memory_order_acquire) == event::status_t::RESUMED)){
						...
						if((j != peer->timeouts.end()) && (j->second > 0)){
							...
							if((peer->state.options & event::options::NOIOBLOCK) || (peer->state.options & event::options::SMIOBLOCK)){
								...
								::__awh_change__.push_back((struct kevent){});
								// Устанавливаем таймаут на получение данных
								EV_SET(&::__awh_change__.back(), peer->id, EVFILT_TIMER, EV_ADD | EV_ONESHOT, NOTE_USECONDS, static_cast <int64_t> (j->second) * 1000, peer);
								// Увеличиваем количество событий
								ret.first->second.count++;
							// Если сокет является блокирующим
							} else this->_eth.timeout(peer->fd, net::socket_event_t::READ, static_cast <uint32_t> (j->second));
						}
						// Увеличиваем значение итератора
						++i;
					// Если событие поставленно на паузу
						...
						if((peer->state.options & event::options::NOIOBLOCK) || (peer->state.options & event::options::SMIOBLOCK))
							// Устанавливаем событие на чтение но отключаем его
							EV_SET(&::__awh_change__.back(), peer->fd, EVFILT_READ, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, peer);
						// Устанавливаем событие на чтение но отключаем его
						else EV_SET(&::__awh_change__.back(), peer->fd, EVFILT_READ, EV_ADD | EV_DISABLE, 0, 0, peer);
						// Добавляем новое событие в список изменений
						::__awh_change__.push_back((struct kevent){});
						// Устанавливаем событие на запись но отключаем его
						EV_SET(&::__awh_change__.back(), peer->fd, EVFILT_WRITE, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, peer);
				...
				case static_cast <uint8_t> (event::node_t::CLIENT): {
					   ...
					   (client->state.status.load(std::memory_order_acquire) == event::status_t::PAUSED)){
						...
						switch(static_cast <uint8_t> (client->state.family)){
							...
							case static_cast <uint8_t> (event::family_t::UDS): {
								...
								if((client->state.options & event::options::NOIOBLOCK) || (client->state.options & event::options::SMIOBLOCK))
									// Устанавливаем событие на чтение но отключаем его
									EV_SET(&::__awh_change__.back(), client->fd, EVFILT_READ, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, client);
								// Устанавливаем событие на чтение но отключаем его
								else EV_SET(&::__awh_change__.back(), client->fd, EVFILT_READ, EV_ADD | EV_DISABLE, 0, 0, client);
								// Добавляем новое событие в список изменений
								::__awh_change__.push_back((struct kevent){});
								// Устанавливаем событие на запись но отключаем его
								EV_SET(&::__awh_change__.back(), client->fd, EVFILT_WRITE, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, client);
								// Создаём объект промежуточного звена
								auto ret = ::__awh_inters__.emplace(i->first, intmd_t{});
								// Устанавливаем количество событий
								ret.first->second.count = 2;
					...
					if((client->state.status.load(std::memory_order_acquire) == event::status_t::CONNECTED) ||
					   (client->state.status.load(std::memory_order_acquire) == event::status_t::RESUMED)){
						...
						::__awh_change__.push_back((struct kevent){});
						// Активируем событие на чтение для серверного сокета
						EV_SET(&::__awh_change__.back(), i->first, EVFILT_READ, EV_ENABLE, 0, 0, client);
					}
					// Увеличиваем значение итератора
					++i;
				} break;
				...
				case static_cast <uint8_t> (event::node_t::SERVER): {
						...
						switch(static_cast <uint8_t> (server->state.family)){
							...
							case static_cast <uint8_t> (event::family_t::UDS): {
								...
								if((server->state.options & event::options::NOIOBLOCK) || (server->state.options & event::options::SMIOBLOCK)){
									...
									::__awh_change__.push_back((struct kevent){});
									// Устанавливаем событие на чтение но отключаем его
									EV_SET(&::__awh_change__.back(), server->fd, EVFILT_READ, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, server);
								// Если сокет является блокирующим
								} else {
									// Добавляем новое событие в список изменений
									::__awh_change__.push_back((struct kevent){});
									// Устанавливаем событие на чтение но отключаем его
									EV_SET(&::__awh_change__.back(), server->fd, EVFILT_READ, EV_ADD | EV_DISABLE, 0, 0, server);
								}
								// Создаём объект промежуточного звена
								auto ret = ::__awh_inters__.emplace(i->first, intmd_t{});
								// Устанавливаем количество событий
								...
								if(server->state.type == event::type_t::DATAGRAM){
									...
									::__awh_change__.push_back((struct kevent){});
									// Устанавливаем событие на запись но отключаем его
									EV_SET(&::__awh_change__.back(), server->fd, EVFILT_WRITE, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, server);
								}
							} break;
							// Для семейства IPv4
							case static_cast <uint8_t> (event::family_t::IPV4):
							...
							case static_cast <uint8_t> (event::family_t::UDPV4): {
								...
								if((server->state.options & event::options::NOIOBLOCK) || (server->state.options & event::options::SMIOBLOCK))
									...
									EV_SET(&::__awh_change__.back(), server->fd, EVFILT_READ, EV_ADD | EV_CLEAR | EV_DISABLE, 0, 0, server);
								// Устанавливаем событие на чтение но отключаем его
								else EV_SET(&::__awh_change__.back(), server->fd, EVFILT_READ, EV_ADD | EV_DISABLE, 0, 0, server);
								// Создаём объект промежуточного звена
								auto ret = ::__awh_inters__.emplace(i->first, intmd_t{});
								// Устанавливаем количество событий
								ret.first->second.count = 1;
								...
								ret.first->second.index = (::__awh_change__.size() - 1);
								// Если событие является UDP-подключением
								if(server->state.family == event::family_t::UDPV4){
									// Устанавливаем количество событий
									ret.first->second.count++;
									// Добавляем новое событие в список изменений
									::__awh_change__.push_back((struct kevent){});
							...
							case static_cast <uint8_t> (event::family_t::UDPV6): {
								...
								ret.first->second.index = (::__awh_change__.size() - 1);
								// Если событие является UDP-подключением
								if(server->state.family == event::family_t::UDPV6){
									// Устанавливаем количество событий
									ret.first->second.count++;
									// Добавляем новое событие в список изменений
									::__awh_change__.push_back((struct kevent){});
					...
					if((server->state.status.load(std::memory_order_acquire) == event::status_t::RUNNING) ||
					   (server->state.status.load(std::memory_order_acquire) == event::status_t::RESUMED)){
						...
						::__awh_change__.push_back((struct kevent){});
						// Активируем событие на чтение для серверного сокета
						EV_SET(&::__awh_change__.back(), i->first, EVFILT_READ, EV_ENABLE, 0, 0, server);
					}
					// Увеличиваем значение итератора
					++i;
				} break;
