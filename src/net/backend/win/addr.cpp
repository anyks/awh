...
void awh::eth::Network_Address::fillSource(const net::addr_t * net, net::src_t & source) const noexcept {
	 ...
	 *
	 * @warning Прежде здесь стоял подбор устройства БЕЗ ОГЛЯДКИ на сеть, и он расходился
	 *          с наречиями POSIX: те при отказе опроса просто выходят, ничего не заполняя.
	 *          Потребитель получал имя устройства, подобранного по чему угодно, и принимал
	 *          его за ответ о СВОЕЙ сети - та же беда, что у несличимой пары выше, только
	 *          с иным поводом
	 *
	 * @note Отказ оглашается: соседнее место этого же файла при том же самом отказе пишет
	 *       предупреждение, и молчать здесь значило бы разводить один отказ на два разных
	 *       поведения
	 */
	if(adapters == nullptr){
		// Выводим в журнал сообщение о невозможности опроса устройств
