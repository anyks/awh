...
bool awh::eth::Network_Address::isInSubnet(const uint32_t ip, const uint32_t net, const uint8_t prefix) const noexcept {
	 ...
	 *
	 * @warning Верхний край проверяется наравне с нижним: длина префикса приходит полем
	 *          октета, и значение свыше 32 обращает сдвиг в сдвиг на ОТРИЦАТЕЛЬНОЕ число
	 *          разрядов - поведение неопределённое. Найдено надзирателем undefined у
	 *          наречий POSIX: «shift exponent -2 is negative». Довод приходит от
	 *          потребителя, ограничивать его выше по течению нечем
	 *
	 * @note Префиксу от 32 и выше отвечает маска из одних единиц, то есть сличение
	 *       адресов целиком, - тот же ответ, что и у прочих систем
	 */
	const uint32_t mask = ((prefix >= 32) ? 0xFFFFFFFFU : (~((1U << (32 - prefix)) - 1)));
...
uint16_t awh::eth::Network_Address::checksum(const event::family_t family, const event::protocol_t protocol, const void * src, const void * dst, const void * transport, const size_t length) const noexcept {
	...
	try {
		 ...
		 *
		 * @warning Порция короче своего поля суммы гашения НЕ получает, и сумма всё
		 *          равно выдаётся - посчитанная вместе со старым полем вызывающего.
		 *          Я счёл это дефектом и завёл отказ, но откатил: договор у обращения
		 *          шире целого заголовка - проверка `AddressChecksumUdpZeroIsFFFFTest`
		 *          намеренно подаёт двухоктетную порцию, - а правка развела бы
		 *          поведение одного обращения по системам. Место одинаково у всех
		 *          четырёх наречий и вынесено вопросом в `AUDIT-FINDINGS.md`
		 *
		 * @warning Заслон этот к тому же оберегает от записи ЗА КОНЕЦ копии: копия
		 *          длиной `pseudoSize + length`, а гашение пишет два октета по
		 *          смещению `checksumOffset`. Сняв заслон при откате, я получил падение
		 *          набора на двухоктетной порции - и это ответ на вопрос выше числом:
		 *          широкий договор без этого заслона попросту небезопасен
		 */
...
bool awh::eth::Network_Address::ipv6PrefixEqual(const uint8_t * first, const uint8_t * second, const uint8_t length) const noexcept {
	 ...
	 *       сличением вида адресов, а сам метод остался открытым
	 *
