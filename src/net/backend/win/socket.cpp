...
/**
 * @brief Средства настройки протоколов передачи
 *
 * @note Нужен ради структуры `tcp_keepalive`: настройка удержания связи задаётся у
 *       MS Windows не тремя настройками сокета, как у наречий POSIX, а одной посылкой
 *       управления, и описание её живёт именно здесь. Оснастка MinGW тянула этот
 *       заголовок попутно, MSVC - нет
 */
#include <mstcpip.h>
...
namespace {
	 ...
	 *
	 * @warning НАМЕРЕННОЕ РЕШЕНИЕ, принятое по опыту живых приложений:
...
bool awh::eth::Socket::setHops(const net::socket_t sock, const event::family_t family, const event::delivery_mode_t delivery, const uint8_t hops) const noexcept {
	 ...
	 *
	 * @note Договор этот общий на все системы: решён по наречиям POSIX 03.09.2026
	 *       и здесь приведён к тому же виду
	 */
...
awh::event::mtu_discover_t awh::eth::Socket::getMaximumTransmissionUnitDiscover(const net::socket_t sock, const event::family_t family) const noexcept {
	/**
	 * Обнаружение MTU есть у семейств сетевых и только у них
	 *
	 * @warning Разбор семейства стоял у НАСТРОЙКИ и отсутствовал у ОПРОСА: настройка
	 *          отвечает отказом всякому семейству, кроме IPv4 и IPv6, а опрос всякое
	 *          не-IPv6 семейство считал за IPv4 и шёл к системе. Описателю местного узла
	 *          система отвечала отказом 10038, и отказ этот уходил в журнал ПОТРЕБИТЕЛЯ
	 *
	 * @note Замерено проверкой `IoUdsEmulatedRebuildTest`: перестройка узла домена UNIX
	 *       снимает настройки, чтобы наложить их на новый описатель, и снимала в том
	 *       числе эту - у узла, где обнаружения MTU нет и быть не может
	 *
	 * @note Ответ здесь тот же, что и у отказа опроса, - `NONE`. Умножения значений это
	 *       не создаёт: у семейства местного узла настройки этой нет вовсе, и «не задано»
	 *       единственный верный ответ ему
	 */
	if((family != event::family_t::IPV4) && (family != event::family_t::IPV6))
		// Выводим неопределённый способ обнаружения MTU
...
bool awh::eth::Socket::setMaximumTransmissionUnitDiscover(const net::socket_t sock, const event::family_t family, const event::mtu_discover_t mode) const noexcept {
	 ...
	 * Обнаружение MTU есть у семейств СЕТЕВЫХ и только у них
	 *
	 * @warning Заслон этот я поставил сперва одному лишь ОПРОСУ, сочтя, что пара тем
	 *          самым сошлась. Не сошлась: у настройки разбор идёт по СПОСОБУ обнаружения,
	 *          а семейство разбирается тернарником - и всякое не-IPv6 семейство он считает
	 *          за IPv4 ровно так же, как считал опрос
	 *
	 * @note Признак разбора, каким это найдено: искать надлежит не расхождение пары
	 *       `get`/`set`, а РАЗБОР, У КОТОРОГО НЕТ ВЕТВИ ДЛЯ НЕУЧТЁННОГО ЗНАЧЕНИЯ.
	 *       Расхождение пары было лишь способом заметить первый такой разбор, и, приняв
	 *       способ за предмет, я закрыл половину места
	 */
	if((family != event::family_t::IPV4) && (family != event::family_t::IPV6))
...
awh::net::socket_t awh::eth::Socket::issue(const event::family_t family, const event::type_t type, const event::protocol_t proto, const uint16_t options) const noexcept {
	...
	switch(static_cast <uint8_t> (type)){
		 ...
		 *
		 * @note Отказ этот про СОКЕТ, а не про вид узла. Узел вида `SEQPACKET` движок
		 *       изображает именованным каналом строя сообщений - границы там держит сама
		 *       система, - и до этого места такой узел не доходит вовсе: заводится он
		 *       выше, разбором семейства. Читать отказ как «вида этого у движка нет»
		 *       неверно, и запись стоит здесь именно затем, чтобы так не прочли
		 *
		 */
	 ...
	 *
	 * @note Подлогом была бы подмена ПОТОКОМ, а не изображение вообще. Намеренно
	 *       узел домена UNIX вида `DATAGRAM` либо `SEQPACKET` движок
	 *       изображает именованным каналом строя сообщений: у канала есть и адрес, и
	 *       границы сообщений, оттого договор сходится со всеми прочими системами. До
	 *       этого места такой узел не доходит - заведение его лежит в движке, - и отказ
	 *       здесь остаётся верным лишь для того, кто просит у системы именно СОКЕТ
	 *
	 */
	if(domain == AF_UNIX){
	...
	switch(static_cast <uint8_t> (proto)){
		 ...
		 *       IPv4, у IPv6 её нет ни в одном - расхождение это доложено по наречиям POSIX, здесь же
		 *       семейства равноправны
		 *
		 */
...
awh::event::ecn_t awh::eth::Socket::getExplicitCongestionNotification(const net::socket_t sock, const event::family_t family) const noexcept {
	...
	/**
	 * Уведомление о перегрузке есть у семейств СЕТЕВЫХ и только у них
	 *
	 * @warning Разбора семейства здесь не было вовсе: всякое не-IPv6 семейство считалось
	 *          за IPv4, и обращение шло к системе. Описателю местного узла система
	 *          отвечает отказом 10038, и отказ этот уходил в журнал ПОТРЕБИТЕЛЯ
	 *
	 * @note Замерено проверкой `IoUdsEmulatedRebuildTest`: перестройка узла домена UNIX
	 *       снимает настройки и накладывает их на новый описатель, и обе половины -
	 *       опрос и настройка - шли к системе у узла, где уведомления о перегрузке нет
	 *       и быть не может. То же было найдено у обнаружения MTU, там разбор стоял у
	 *       настройки и отсутствовал у опроса; здесь его не было ни у той, ни у другой
	 */
	if((family != event::family_t::IPV4) && (family != event::family_t::IPV6))
		// Выводим отсутствие признака перегрузки
		return event::ecn_t::NOT_ECT;
 ...
 *          отказ развёл бы договор метода по системам
 *
 */
/**
	...
	if((family == event::family_t::PIPE) || ((family == event::family_t::UDS) && ::__awh_socket_pipe__(sock))){
		...
		switch(option){
			...
			case event::options::NO_SIGPIPE:
				 ...
				 *
				 * @note Оба признака обмена просят ОДНОГО И ТОГО ЖЕ: `NO_IO_BLOCK` -
				 *       неблокирующего обмена, `SM_IO_BLOCK` - умного неблокирующего.
				 *       Блокирующего среди них нет, и согласие здесь не покрывает двух
				 *       противоположных просьб одним ответом. Запись эта стоит затем, что
				 *       по имени `SM_IO_BLOCK` читается как «блокирующий», и разбор 09.09.2026
				 *       принял его за таковой: канал заведён наложенным и блокирующим быть не
				 *       может, отчего согласие выглядело успехом на невыполненное дело
				 */
				return true;
			/**
