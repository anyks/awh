...
namespace {
	 ...
	 *
	 * @details Столкновения с вызывающей стороной, поднявшей средства сама, здесь нет:
	 *          система ведёт СЧЁТ ССЫЛОК, а не признак поднятости. Всякий `WSAStartup`
	 *          счёт увеличивает, всякий `WSACleanup` уменьшает, и снимаются средства
	 *          на нуле - то есть последним, кто их отпустил. Библиотека берёт ровно
	 *          одну ссылку и ровно одну отдаёт, оттого подъём её вызывающей стороне не
	 *          мешает, а отпуск её средств у той не отнимает
	 *
	 * @note Оттого и не понадобилось проверять поднятость пробой - заведением сокета
	 *       на выброс, как это делалось прежде. Проба отвечает на вопрос «подняты ли
	 *       средства сейчас», тогда как нужен ответ на другой - «держит ли их кто-то,
	 *       когда мы уходим», - а его даёт только счёт ссылок. Хуже того, проба
	 *       уводила от парности: не подняв своей ссылки, слой не отпускал и чужую, и
	 *       средства оставались поднятыми до конца процесса всегда
	 *
	 ...
	 *          лишь возможности из многих. Отказ заносится в журнал, а наружу выходит
	 *          обычным отказом заведения сокета - там, где вызывающая сторона его и
	 *          ждёт
	 *
	...
	bool __awh_winsock__() noexcept {
		...
		std::call_once(::__awh_winsock_once__, []() noexcept -> void {
			...
			/**
			 * Сверяем выданную систему версию средств сокетов с запрошенной
			 *
			 ...
			 *       Вешается оно лишь тогда, когда ссылка ВЗЯТА: отпуск невзятой
			 *       ссылки отнял бы средства у вызывающей стороны
			 */
