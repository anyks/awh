...
namespace {
	...
	muted_state_t <std::shared_mutex> __awh_socket_mutex__;
...
void awh::eth::Socket::threadSafety(const bool mode) noexcept {
	...
	/**
	 * Приводим замки к установленному режиму
	 *
	 * @warning Прежде обращение это ставило один признак и не включало НИЧЕГО: замок
	 *          слоя не захватывался нигде вовсе, а замки подсистемы качества
	 *          обслуживания жили сами по себе, включённые по умолчанию. То есть
	 *          переключатель был, а переключать ему было нечего
	 *
	 */
	::__awh_socket_mutex__.enabled = (::__awh_socket_thread_safety__ == event::mode_t::ENABLED);
	// Приводим замки подсистемы качества обслуживания к установленному режиму
	win::qos::threadSafety(mode);
}
 ...
 *
 * @warning Срок приёма у MS Windows ТЕРЯЕТ дейтаграмму, пришедшую ровно в миг снятия
 *          блокирующего приёма по этому сроку. Стек засчитывает её отданной получателю -
 *          счётчик `dwInDatagrams` растёт, - но приём возвращает `WSAETIMEDOUT`, в очереди
 *          гнезда дейтаграммы уже нет, и потерю не отмечает ни один счётчик: ни `dwNoPorts`,
 *          ни `dwInErrors`, ни отброшенное на уровне IP.
 *
 *          Свойство доказано опытом в три плеча по 4000 посылок на петле при сроке 20 мс:
 *          при подгонке прихода к самому истечению срока теряется от 1 до 25 дейтаграмм
 *          на 4000, при приходе в середину промежутка - ни одной, а при том же приходе
 *          к истечению, но с ожиданием готовности через `select` - тоже ни одной. Число
 *          снятий по сроку у плеч подгонки совпадает, то есть разнятся они единственно
 *          способом ожидания.
 *
 *          Самого движка это не касается: свой приём он ведёт наложенными обращениями
 *          через порт завершений, и срок этот там не участвует. Но всякий, кто ведёт
 *          приём опросом на блокирующем гнезде, обязан отсчитывать срок ДО обращения
 *          к приёму и звать приём лишь по готовности - тогда снятие по сроку очереди
 *          гнезда не касается вовсе. Сама настройка при этом остаётся запасным пределом
 *
 */
 ...
 *
 * @details Свободное место считается так же, как у наречий POSIX: вместимость буфера
 *          за вычетом занятого в нём места
 *
 * @warning Метод отвечал прежде «неизвестно» ОБЕИМ сторонам, а доводом тому стояло, что
 *          «MS Windows занятое место буферов не сообщает». Про буфер ОТПРАВКИ это верно -
 *          ни `SO_NWRITE`, ни `SIOCOUTQ` у системы нет, - но про буфер ПРИЁМА неверно:
 *          запрос `FIONREAD` там есть и работает. Проверено щупом на стенде: по приходе
 *          16 октет запрос называет ровно 16
 *
 * @note Сторона отправки отвечает «неизвестно» честно, а не наугад - ровно так же
 *       поступает эталон у OpenBSD, где нет `FIONWRITE`
 *
...
int32_t awh::eth::Socket::getBufferAvailable(const net::socket_t sock, const net::socket_event_t event) const noexcept {
	// Если спрашивается место буфера отправки - оно системе неизвестно
	if(event != net::socket_event_t::READ)
	...
	return ((static_cast <u_long> (size) > pending) ? static_cast <int32_t> (static_cast <u_long> (size) - pending) : 0);
}
/**
...
bool awh::eth::Socket::setHops(const net::socket_t sock, const event::family_t family, const event::delivery_mode_t delivery, const uint8_t hops) const noexcept {
	/**
	 * Если способ доставки не определён - отвечаем отказом
	 *
	 * @details `NONE` - НЕ способ доставки, а признак его отсутствия: сами методы
	 *          движка возвращают его там, где способ установить не удалось. Выбирать
	 *          имя настройки нечему, и ответ здесь - отказ
	 *
	 * @warning Принимать `NONE` за `UNICAST` НЕЛЬЗЯ, и прежде здесь принималось
	 *          именно так: выбор имени настройки шёл вопросом «многоадресная ли
	 *          доставка», и всякий иной способ - в том числе НЕЗАДАННЫЙ - молча
	 *          получал одноадресное имя. Потребитель, забывший задать способ, получал
	 *          согласие на действие, о каком не просил
	 *
	 * @note Договор этот общий на все системы, решён владельцем движков POSIX
	 *       03.09.2026 и приведён здесь к тому же виду
	 */
	if(delivery == event::delivery_mode_t::NONE)
		// Возвращаем отрицательный результат установки
		return false;
...
awh::net::socket_t awh::eth::Socket::issue(const event::family_t family, const event::type_t type, const event::protocol_t proto, const uint16_t options) const noexcept {
	...
	switch(static_cast <uint8_t> (proto)){
		...
		break;
		/**
		 * Если протоколом передачи данных является неструктурированный
		 *
		 * @details Значение это - не то же самое, что «протокол не задан»: сокет с
		 *          `IPPROTO_RAW` отдаёт пакет вместе с СОБСТВЕННОЙ головой, тогда как
		 *          сокет с протоколом 0 голову собирает сам. Эталонные слои различают
		 *          их прямо (`bsd/socket.cpp:4037`, `gnu:3796`, `sun:4062`)
		 *
		 * @warning Ветви этой не было НИ ОДНОЙ, и довод `RAW` падал в ветвь по
		 *          умолчанию с протоколом 0. Потребитель, просивший собрать голову
		 *          пакета своими руками, получал сокет, какой её собирает сам, -
		 *          молча и с виду успешно
		 *
		 * @note Замерено щупом на стенде Windows ARM64 03.09.2026: сокет заводится с
		 *       `IPPROTO_RAW` у ОБОИХ семейств. Эталонные слои ветвь эту имеют лишь у
		 *       IPv4, у IPv6 её нет ни в одном - расхождение доложено владельцу
		 *       наречий POSIX, здесь же семейства равноправны
		 *
		 */
		case static_cast <uint8_t> (event::protocol_t::RAW): protocol = IPPROTO_RAW; break;
		/**
		 * Если протоколом передачи данных является IGMP
		 *
		 * @note Ветви этой тоже не было, и довод падал в умолчание. Эталонные слои её
		 *       имеют, система заведение принимает - замерено тем же щупом
		 *
		 */
		case static_cast <uint8_t> (event::protocol_t::IGMP): protocol = IPPROTO_IGMP; break;
...
awh::net::socket_t awh::eth::Socket::channel(const string & name) const noexcept {
	...
	/**
	 * Предельное число заходов на открытие экземпляра канала
	 *
	 * @note Заход ограничен числом, а не одним лишь сроком ожидания: срок отсчитывается
	 *       заново на КАЖДОМ заходе, и обмен, у какого экземпляр канала перехватывает
	 *       кто-то ещё, кружил бы здесь без конца. Столько же заходов отмерено и
	 *       встречной стороне - в юните кластера
	 */
	static constexpr uint32_t ROUNDS = 200;
	// Число сделанных заходов на открытие экземпляра канала
	uint32_t rounds = 0;
 ...
 */
/**
 * @brief Функция заведения пары связанных концов именованного канала
 *
 * @details Служит она двум запросам разом: паре типа `SEQPACKET` любого семейства и паре
 *          семейства `UDS` всякого вида. Домен UNIX несёт у MS Windows один лишь поток, и
 *          недостающие виды его изображаются каналом - устройством иным, но той же работы:
 *          у канала есть имя, к нему подключается чужой процесс, обмен идёт в обе стороны
 *
...
static array <awh::net::socket_t, 2> __awh_channel_pair__(const awh::log_t * log, const awh::fmk_t * fmk, string & name, const bool message) noexcept {
	...
	HANDLE server = ::CreateNamedPipeW(
		...
		(message ? (PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE) : (PIPE_TYPE_BYTE | PIPE_READMODE_BYTE)) | PIPE_WAIT,
	...
	DWORD mode = (message ? PIPE_READMODE_MESSAGE : PIPE_READMODE_BYTE);
	/**
	 ...
	 *
	 * @note Довод терпимости прежде опирался на замер 04.09.2026: сообщение
	 *       дробилось движком, и границы не сохранялись всё равно. Довод этот
	 *       ОТМЕНЁН 08.09.2026 - дробила выдача принятого, а не система, и теперь
	 *       узел `SEQPACKET` границы держит. Терпимость же оставлена по второму
	 *       доводу, какой и был главным: переключение идёт на только что открытом
	 *       описателе своего же канала, и достижимость отказа близка к нулю
	 *
	 * @note Достижимость отказа при этом близка к нулю: переключение идёт на
	 *       только что открытом описателе своего же канала
	 */
	...
	result[1] = reinterpret_cast <awh::net::socket_t> (client);
...
array <awh::net::socket_t, 2> awh::eth::Socket::ipc(const event::family_t family, const event::type_t type, const event::protocol_t proto, string & name) const noexcept {
	...
	/**
	 * Пары семейств канала и домена UNIX строятся именованным каналом целиком
	 *
	 * @details Семейство `PIPE` есть канал по самому своему смыслу, и строить его сокетной
	 *          парой значило бы лишить его имени - а имя у канала и есть точка встречи для
	 *          чужого процесса. Домен UNIX несёт у MS Windows один лишь поток: дейтаграммного сокета в нём
	 *          нет вовсе, сохраняющего границы - тоже. Изображать недостающие виды одним, а
	 *          имеющийся оставлять родным значило бы развести виды одного семейства: у
	 *          одного есть имя, у других нет, и встреча процессов у них разная. Оттого
	 *          каналом строится всё семейство разом - тогда API сходится и здесь, и у
	 *          систем POSIX
	 *
	 * @note Пути в файловой системе у такой пары не появляется: имя её - имя канала.
	 *       Разницы это не создаёт, покуда обе стороны обращаются к нему одинаково, а
	 *       чужой программе, ждущей узла по пути, пара не предназначена вовсе
	 *
	 * @note Строй канала выбирается видом: потоковому - октетов, прочим - сообщений.
	 *       Границы сообщений домен UNIX держит у обоих недостающих видов
	 */
	...
	if(kind == SOCK_DGRAM){
		...
		do {
			...
			if((one == INVALID_SOCKET) || (two == INVALID_SOCKET))
				...
				break;
			/**
			 * Отчёт о недоступности получателя у сторон пары НЕ отключается
			 *
			 * @details Отключается он у сокетов, заведённых обращением issue, и там
			 *          отключение обязательно: сокет там несвязанный, а MS Windows
			 *          сообщает ему о чужой беде - об ICMP, пришедшем в ответ на
			 *          прежнюю отправку кому-то одному, - обрывая приём от всех прочих.
			 *          Системы POSIX несвязанному сокету об этом не сообщают вовсе
			 *
			 * @note Стороны же пары СВЯЗАНЫ встречно, и вопрос у них другой: связанному
			 *       сокету об уходе своего единственного собеседника сообщают и системы
			 *       POSIX - отказом ECONNREFUSED на приёме. Отключи слой отчёт и здесь -
			 *       сторона, чей собеседник закрылся, ждала бы данных вечно вместо
			 *       честного отказа, и MS Windows разошлась бы с эталоном
			 *
			 * @warning Однородными места эти оттого НЕ являются, и правило «чинить все
			 *          однородные места разом» к ним не применимо: вопрос, на какой они
			 *          отвечают, у них разный
			 *
			 */
	...
	do {
		/**
		 * Признак повторного использования адреса слушателю НЕ ставится
		 *
		 * @details У систем POSIX признак этот безобиден: он позволяет занять адрес,
		 *          оставшийся в состоянии TIME_WAIT, но занять адрес ЖИВОГО слушателя
		 *          не даёт. MS Windows понимает его иначе - как право перехвата: сокет
		 *          с этим признаком встаёт на адрес, уже занятый чужим слушателем, и
		 *          подключения достаются перехватчику
		 *
		 * @warning Здесь это прямая брешь: слушатель поднимается на петлевом устройстве
		 *          ради сведения двух сторон пары, и всякий местный процесс, поставивший
		 *          признак на тот же порт, принял бы наше подключение вместо нас. Порт
		 *          же выбирается системой, и угадывать его перехватчику не приходится -
		 *          довольно перебрать несколько
		 *
		 * @note Нужды в признаке нет вовсе: порт слушателю не задаётся, и занятого
		 *       адреса ему не достаётся никогда
		 *
		 */
		...
		if(::connect(first, &address.addr, static_cast <int32_t> (sizeof(address.inaddr))) == SOCKET_ERROR)
			...
			break;
		/**
		 * Принимаем подключение, сличая собеседника со своей же стороной
		 *
		 * @details Слушатель стоит на петлевом устройстве, и порт его открыт всякому
		 *          местному процессу: подключиться к нему меж `listen` и нашим `accept`
		 *          вправе кто угодно. Сличение отвечает на вопрос, наша ли сторона
		 *          принята: у настоящей пары местное имя нашей стороны и имя собеседника
		 *          у принятой - одно и то же
		 *
		 * @warning Пояснение выше, о непоставленном признаке повторного использования
		 *          адреса, прикрывает ОДИН лишь перехват адреса и о постороннем
		 *          подключении не говорит ничего. Довод там был уже вреда: читающий его
		 *          вправе заключить, что слушатель на петле безопасен сам по себе
		 *
		 * @note Что при этом РЕАЛЬНО происходит - измерено щупом на стенде (ARM64,
		 *       сборка 10.0.26200), а не выведено рассуждением. Очередь слушателя здесь
		 *       длиной в одно подключение, и посторонний, поспевший первым, занимает её
		 *       целиком: наш собственный `connect` отвечает тогда отказом 10061
		 *       (`WSAECONNREFUSED`), и заведение пары обрывается выше по цепочке. То
		 *       есть чужая сторона в паре при нынешней длине очереди не оказывается
		 *
		 * @note Сличение оставлено, несмотря на это: длина очереди - свойство издания
		 *       системы, а не наше обещание, и `listen` вправе округлить её вверх.
		 *       Стоит очереди вместить двоих - и посторонний, поспевший первым,
		 *       достанется `accept` вместо нашей стороны, а обмен ушёл бы чужому
		 *       процессу молча. Заслон этот стоит одного сравнения имён
		 *
		 * @note Повторного приёма здесь НЕТ намеренно: он висел бы на `accept`, когда
		 *       принимать больше нечего - измерено тем же щупом, слушатель готовности
		 *       не даёт вовсе. Посторонний собеседник обрывает заведение пары отказом,
		 *       и отказ этот честен: обмениваться с чужим процессом хуже, чем не завести
		 *       пары
		 *
		 */
		{
			// Местный адрес нашей стороны пары
			union { struct sockaddr addr; struct sockaddr_in inaddr; } mine{};
			// Размер местного адреса нашей стороны пары
			int32_t size = static_cast <int32_t> (sizeof(mine));
			// Если узнать местный адрес нашей стороны не удалось
			if(::getsockname(first, &mine.addr, &size) == SOCKET_ERROR)
				// Выходим из цикла заведения пары
				break;
			// Адрес собеседника у принятого подключения
			union { struct sockaddr addr; struct sockaddr_in inaddr; } peer{};
			// Размер адреса собеседника у принятого подключения
			int32_t length = static_cast <int32_t> (sizeof(peer));
			// Принимаем подключение - оно и даёт вторую сторону пары
			second = ::accept(listener, &peer.addr, &length);
			// Если подключение принято, но принадлежит оно постороннему процессу
			if((second != INVALID_SOCKET) && !((peer.inaddr.sin_family == mine.inaddr.sin_family) &&
...
static LPFN_WSASENDMSG __awh_wsa_sendmsg__(const SOCKET sock) noexcept {
	 ...
	 *
	 * @note Держится он значением слаженным, а не простым указателем: спросить адрес
	 *       вправе любой поток, а запись обычного указателя разошлась бы с чтением
	 *
	 */
	static std::atomic <LPFN_WSASENDMSG> cache{nullptr};
	// Получаем запомненный адрес расширенного вызова
	LPFN_WSASENDMSG result = cache.load(std::memory_order_acquire);
	// Если адрес уже взят
	if(result != nullptr)
	...
	DWORD size = 0;
	/**
	 * Спрашиваем адрес расширенного вызова в СВОЁ значение, а не в запомненное
	 *
	 * @warning Прежде адрес спрашивался прямо в запомненное значение, и на время
	 *          обращения оно стояло заполненным наполовину: поток соседний, увидав его
	 *          непустым, ушёл бы вызывать мусор. Отказ же обращения оставляет там
	 *          значение неопределённое, а не пустое
	 *
	 */
	if(::WSAIoctl(sock, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid, sizeof(guid), &result, sizeof(result), &size, nullptr, nullptr) != 0)
		// Выводим отсутствие адреса расширенного вызова
		return nullptr;
	// Запоминаем взятый адрес расширенного вызова
 ...
 *
 * **Настройка эта у MS Windows ПРИНИМАЕТСЯ, но исходящие датаграммы НЕ МЕТИТ.**
 * Замерено щупом 03.09.2026, отправитель и получатель на одной машине, метка ECT(0):
 *
 * | способ пометки | принятый октет TOS |
 * |---|---|
 * | настройка сокета `IP_ECN` (этот метод) | **0** |
 ...
 * То есть дело не в свойстве петли: система соглашается с настройкой (`setsockopt`
 * отвечает нулём, `WSAGetLastError` пуст) и просто не применяет её к отправке.
 *
 * Оттого метка перегрузки у этого движка держится НА УЗЛЕ и ставится каждой датаграмме
 * управляющими данными - разбор при `iocp.cpp`, где заведено `event::ecn_t` узла. Путь
 * этот работает и проверен; работает и узловое обращение `io_t::setExplicitCongestionNotification`.
 *
 * Здесь же настройка оставлена, а не заменена отказом: у систем POSIX она есть и
 * действует, договор движков её знает, и отвечать отказом значило бы расходиться с ними
 * на том, что система принимает. Но полагаться на неё под MS Windows НЕЛЬЗЯ, и это
 * сказано прямо, а не оставлено на догадку.
 *
 * @warning Вскрыто проверкой `SocketEcnDeliveryTest`, какая до 03.09.2026 под MS Windows
 *          ПРОПУСКАЛАСЬ: посредника `msghdr` проверке не хватало, и путь этот не
 *          проверялся вовсе. Снятие пропуска и вскрыло расхождение
 *
 */
 ...
 */
/**
 * @brief Функция проверки того, что описатель является именованным каналом
 *
 * @details Спрашивается это у самих средств работы с сокетами: описатель, не являющийся
 *          сокетом, отвечает на запрос своего вида отказом 10038 (`WSAENOTSOCK`). Вид
 *          файла (`GetFileType`) здесь негоден - сокеты у MS Windows отвечают им тем же
 *          значением, что и каналы
 *
 * @param sock проверяемый описатель
 * @return     признак именованного канала
 *
 */
static bool __awh_socket_pipe__(const awh::net::socket_t sock) noexcept {
	...
	// Выводим признак того, что описатель сокетом не является
	return ((::getsockopt(static_cast <SOCKET> (sock), SOL_SOCKET, SO_TYPE, reinterpret_cast <char *> (&type), &length) != 0) && (::WSAGetLastError() == WSAENOTSOCK));
}
...
bool awh::eth::Socket::switchOption(const net::socket_t sock, const event::family_t family, const net::socket_mode_t mode, const uint16_t option, [[maybe_unused]] const event::protocol_t proto) const noexcept {
	 ...
	 *
	 * @note Домен UNIX спрашивается видом описателя, а не одним семейством: пара его
	 *       изображена каналом, а узлы клиента и сервера стоят на настоящем сокете
	 *
	 */
	if((family == event::family_t::PIPE) || ((family == event::family_t::UDS) && ::__awh_socket_pipe__(sock))){
		/**
	...
	switch(option){
		...
		case event::options::HDRINCL: {
			 ...
			 * Если сокет работает по протоколу IPv6
			 *
			 * @details Настройка ставится ПО-НАСТОЯЩЕМУ: `IPV6_HDRINCL` у MS Windows
			 *          объявлен (`ws2ipdef.h:72`) и работает, и ответ движка есть ответ
			 *          самой системы. Применима она лишь к сокету НЕСТРУКТУРИРОВАННОМУ -
			 *          ровно как `IP_HDRINCL` у IPv4
			 *
			 * @warning Прежде здесь стоял УСПЕХ на пустое действие, обоснованный
			 *          утверждением «своей головы у IPv6 не даёт ни одна система».
			 *          Утверждение неверно, и молчаливый успех на невыполненное дело
			 *          хуже отказа: потребитель, собирающий свою голову пакета,
			 *          получал согласие и неработающий сокет. Замерено щупом на стенде
			 *          Windows ARM64 03.09.2026:
			 *
			 ...
			 *
			 *          То есть настройка эта не отсутствует, а применима лишь к сокету
			 *          неструктурированному, - ровно как `IP_HDRINCL` у IPv4
			 *
			 * @note Договор поправлен разом у всех движков 03.09.2026: настройка
			 *       ставится по-настоящему там, где система её имеет, и отвечает
			 *       успехом на пустое действие лишь там, где её нет вовсе (пять систем
			 *       из семи). Проверка `SocketSwitchOptionIPv6Test` вела настройку на
			 *       сокете TCP, где она бессмысленна у обеих систем, её имеющих, -
			 *       оттуда она убрана, а договор закреплён `SocketHeaderInclusionIPv6Test`
			 *       на сокете неструктурированном
			 */
			// Устанавливаем уровень настройки по семейству адресов сокета
			level = ((family == event::family_t::IPV6) ? IPPROTO_IPV6 : IPPROTO_IP);
			// Устанавливаем название настройки по семейству адресов сокета
			name = ((family == event::family_t::IPV6) ? IPV6_HDRINCL : IP_HDRINCL);
		} break;
		 ...
		 *
		 * @note Наследование у MS Windows ЕСТЬ, и устроено оно иначе, чем у POSIX: нет
		 *       образа `exec`, при котором описатель переживает замену образа процесса,
		 *       - взамен него порождаемому процессу достаются описатели, помеченные
		 *       наследуемыми, и лишь когда само порождение просили вести с наследованием.
		 *       Снятие пометки и есть то самое, чего добиваются закрытием при запуске.
		 *
		 * @warning Прежде здесь стояло «наследования у MS Windows нет вовсе», и читать
		 *          это следовало как отсутствие образа POSIX, а не самого наследования.
		 *          Замер сессии 04.09.2026 показал, чем такое чтение оборачивается:
		 *          кластер порождал работников с наследованием, и тем доставались ВСЕ
		 *          сокеты мастера - порт не освобождался, а закрытые соединения не слали
		 *          `FIN`. Доказано двумя плечами: с наследованием повторная привязка
		 *          отвечает 10048, без него - успехом
		 *
		 */
		case event::options::CLOSE_ON_EXEC: {
