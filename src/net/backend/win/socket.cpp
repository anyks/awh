...
array <awh::net::socket_t, 2> awh::eth::Socket::ipc(const event::family_t family, const event::type_t type, const event::protocol_t proto, string & name) const noexcept {
	...
	switch(static_cast <uint8_t> (type)){
		...
		case static_cast <uint8_t> (event::type_t::SEQPACKET): {
			 ...
			 *
			 * @note Почему терпимо, а не только опасно: замером 04.09.2026 установлено, что
			 *       запись по каналу ДРОБИТСЯ движком, и сообщение в 98304 октета доходит
			 *       до потребителя двадцатью четырьмя откликами при полной сохранности
			 *       октетов. То есть границы сообщений не сохраняются и в самом строе
			 *       сообщений, едва посылка перерастает буфер, - и обмен на них не
			 *       опирается. Отсюда и терпимость к отказу переключения
			 *
