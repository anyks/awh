...
int64_t awh::win::message::receive(const net::socket_t sock, struct msghdr * msg, const int32_t flags, int32_t * error) noexcept {
	/**
	 * Признак подсматривания расширенный вызов ПРИНИМАЕТ, прочих не принимает
	 *
	 ...
	 *
	 * @warning Затем сюда встал отказ на ВСЯКИЙ ненулевой набор признаков, с доводом
	 *          «поле `dwFlags` у приёма служит лишь ответом, а `MSG_PEEK` расширенный
	 *          вызов не поддерживает вовсе». Довод этот НЕВЕРЕН, и заменял он потерю
	 *          дейтаграммы отказом там, где система справляется сама
	 *
	 * @note Измерено щупом на стенде (ARM64, сборка 10.0.26200) двумя плечами, одна
	 *       разница: дейтаграмма в 8 октетов, приём расширенным вызовом, затем опрос
	 *       очереди через `FIONREAD`.
	 *       С `dwFlags = MSG_PEEK` вызов отвечает успехом, отдаёт все 8 октетов, а в
	 *       очереди остаются те же 8 - дейтаграмма НА МЕСТЕ.
	 *       С `dwFlags = 0` вызов отдаёт те же 8 октетов, а в очереди остаётся 0 -
	 *       дейтаграмма вынута.
	 *       То есть входным признак этот именно что берётся
	 *
	 * @note Прочие признаки приёма по-прежнему отвергаются отказом: за них здесь никто
	 *       не ручается, а молчаливое их отбрасывание и есть тот изъян, с какого
	 *       разбор этого места начинался. Понадобится - откроются тем же порядком:
	 *       щупом на стенде, а не догадкой
	 *
	...
	/**
	 * Устанавливаем признаки приёма
	 *
	 * @note Ненулевым сюда доходит лишь `MSG_PEEK` - прочие отвергнуты выше
	 */
	message.dwFlags = static_cast <DWORD> (flags);
