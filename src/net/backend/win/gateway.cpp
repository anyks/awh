...
/**
 * @brief Средства разбора опознавателей COM
 *
 * @note Нужен ради `CLSIDFromString`: названия сетевых устройств MS Windows несёт
 *       опознавателем вида COM, и разбор строки в двоичный вид живёт именно здесь.
...
namespace {
	...
	/**
	 * @brief Функция поиска маршрута по умолчанию в таблице системы
	 *
	 * @details Маршрут по умолчанию - это запись с НУЛЕВОЙ длиной префикса, и ищется он
	 *          именно так. Из нескольких таких берётся запись наименьшей стоимости -
	 *          тем же правилом, каким выбирает её сама система
	 *
	 * @warning Вопрос «каков маршрут по умолчанию» подменялся прежде вопросом «каким
	 *          путём уйдёт пакет к адресу 0.0.0.0», и ответ на него давал `GetBestRoute2`.
	 *          Вопросы эти РАЗНЫЕ: система выбирает по длиннейшему совпадающему префиксу,
	 *          и всякая запись, накрывающая нулевой адрес уже нулевого префикса (скажем,
	 *          `0.0.0.0/24`), побивает настоящий маршрут по умолчанию. Ответом тогда
	 *          выходил чужой маршрут с пустым шлюзом, притом молча. Поймано набором `net`
	 *          на стенде, где такая запись в таблице есть
	 *
	 * @param family семейство адресов искомого маршрута
	 * @param row    место для найденной записи маршрута
	 * @return       результат поиска маршрута по умолчанию
	 *
	 */
	bool __awh_defroute__(const ADDRESS_FAMILY family, MIB_IPFORWARD_ROW2 & row) noexcept {
		// Таблица маршрутов системы
		PMIB_IPFORWARD_TABLE2 table = nullptr;
		// Если снять таблицу маршрутов не удалось
		if(::GetIpForwardTable2(family, &table) != NO_ERROR)
			// Выводим отрицательный результат поиска
			return false;
		// Признак того, что маршрут по умолчанию найден
		bool found = false;
		/**
		 * Выполняем обход всех записей таблицы маршрутов
		 */
		for(ULONG i = 0; i < table->NumEntries; i++){
			// Получаем очередную запись таблицы маршрутов
			const MIB_IPFORWARD_ROW2 & entry = table->Table[i];
			// Записи с ненулевым префиксом маршрутом по умолчанию не являются
			if(entry.DestinationPrefix.PrefixLength != 0)
				// Переходим к записи следующей
				continue;
			// Если запись семейству искомых не принадлежит
			if(entry.DestinationPrefix.Prefix.si_family != family)
				// Переходим к записи следующей
				continue;
			// Если маршрут ещё не найден либо найденный дороже
			if(!found || (entry.Metric < row.Metric)){
				// Запоминаем найденный маршрут по умолчанию
				row = entry;
				// Отмечаем маршрут по умолчанию найденным
				found = true;
			}
		}
		// Освобождаем снятую таблицу маршрутов
		::FreeMibTable(table);
		// Выводим результат поиска маршрута по умолчанию
		return found;
	}
	/**
...
bool awh::eth::Gateway::get(route_t & route) const noexcept {
	...
	bool emptyDestination = true;
	{
		...
		/**
		 * Наличие ЗАДАННОГО маршрута спрашивается поиском по таблице
		 *
		 * @warning Одного лишь названного назначения для этого НЕДОСТАТОЧНО, и прежде
		 *          сюда уходило оно тоже. Метод отвечает на вопрос «каким путём система
		 *          отправит пакет», а адрес, своей записи в таблице не имеющий,
		 *          разрешается маршрутом по умолчанию - точное же сличение объявляло
		 *          его недостижимым. Расхождение с эталоном вскрылось прогоном набора
		 *          на стенде: `GatewayGetSpecificDestinationNotFoundIPv4` отступала там
		 *          доводом «выхода наружу у машины нет», хотя маршрут по умолчанию у
		 *          машины был, а на macOS та же проверка проходила
		 *
		 * @note Условие взято у эталонного наречия BSD дословно: там таблица обходится
		 *       сличением, а при неназванном шлюзе и неназванном устройстве спрашивается
		 *       само ядро запросом `RTM_GET`, отвечающее подбором по наидлиннейшей
		 *       приставке. Отвечает ему здесь `GetBestRoute2` ниже
		 */
	...
	/**
	 * Маршрут по умолчанию берётся из таблицы, а не опросом пути к нулевому адресу
	 *
	 * @note Шаг этот берётся лишь при НЕНАЗВАННОМ назначении - тогда и спрашивается
	 *       маршрут по умолчанию, записью нулевого префикса. Названное же назначение
	 *       идёт мимо, к опросу пути ниже: ответить на него записью по умолчанию
	 *       нельзя, а разрешить его обязана сама система
	 *
	 */
	if(emptyDestination && ::__awh_defroute__(static_cast <ADDRESS_FAMILY> ((route.destination->size == 4) ? AF_INET : AF_INET6), row)){
	...
	/**
	 * Опрос пути к адресу назначения силами самой системы
	 *
	 * @details Сюда приходят два случая: названное назначение, какое разрешить обязана
	 *          система подбором по наидлиннейшей приставке, и неназванное - когда записи
	 *          нулевого префикса в таблице не нашлось вовсе. Ответ этот вернее пустого
	 *          в обоих
	 */
