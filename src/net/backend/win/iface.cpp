...
string awh::eth::Interface::name(const net::addr_t * addr) const noexcept {
	...
	for(PIP_ADAPTER_ADDRESSES adapter = adapters; adapter != nullptr; adapter = adapter->Next){
		...
		if(adapter->AdapterName == nullptr)
			...
			continue;
		/**
		 * Если разыскивается устройство по его аппаратному адресу
		 *
		 * @details Лежит он у MS Windows не среди адресов устройства, а отдельным полем
		 *          самого устройства, и в переборе адресов ему места нет вовсе: перебор
		 *          этот идёт по адресам сетевым
		 *
		 * @warning Ветви этой прежде не было НИ ОДНОЙ, тогда как проверка на пустоту
		 *          аппаратного адреса выше стояла - то есть довод такого рода метод
		 *          принимал, разбирал и молча не находил никогда, отвечая пустым
		 *          названием при живом устройстве. Эталон же (`bsd/iface.cpp:271`) по
		 *          аппаратному адресу устройство ищет
		 *
		 * @note Длина сличается прежде самого адреса: поле это несёт адреса устройств
		 *       всякого рода, и у иных длина его не шесть октетов, а иная вовсе
		 *
		 */
		if(addr->size == 6){
			// Если аппаратный адрес устройства отвечает длине разыскиваемого
			if(adapter->PhysicalAddressLength == 6){
				// Если аппаратные адреса совпали
				if(::memcmp(&awh_cast <const net::addr_mac_t *> (addr)->address[0], adapter->PhysicalAddress, 6) == 0)
					// Выводим название найденного устройства
					return string(adapter->AdapterName);
			}
			// Переходим к следующему устройству: адресов сетевых здесь не спрашивают
			continue;
		}
		/**
		 * Выполняем перебор всех адресов устройства
 ...
 *
 * @details Своего общего на процесс учёта у слоя нет: перечень устройств спрашивается у
 *          системы всякий раз заново, и оберегать здесь нечего. А вот у туннельных
 *          устройств учёт есть - реестр заведённых устройств живёт на весь процесс, -
 *          и режим этот доводится до него
 *
 * @note Тем слой этот отличается от bsd и gnu, где реализация пуста: у MS Windows
 *       туннельные устройства заводятся не описателем, а драйвером, и описатели их
 *       приходится держать модулю
 *
...
void awh::eth::Interface::threadSafety(const bool mode) noexcept {
	// Приводим замки туннельных устройств к установленному режиму
	win::tunnel::threadSafety(mode);
}
 ...
 *
 * Виды устройств, кроме TUN и TAP, отвергаются НАМЕРЕННО, и это ограничение системы,
 * а не пробел слоя. Эталонные наречия заводят девять видов - `NET`, `GIF`, `GRE`,
 * `WLAN`, `VLAN`, `BOND`, `BRIDGE` вдобавок к двум здешним - клонируемыми
 * устройствами BSD (`bsd/iface.cpp:1822-1852`), то есть обращением к сети вида
 * «создать gif0». Соответствия этому у MS Windows нет вовсе: устройства таких видов
 * приносит драйвер либо надзорная оснастка (мост - через общий доступ к подключению
 * либо Hyper-V, объединение сетевых карт - через NetLbfo, метка VLAN - свойством
 * самого адаптера), а обращения уровня сокета, каким их можно было бы завести, у
 * системы не имеется.
 *
 * Отказ потому верен и оставлен: слой не берётся изображать заведение того, чего
 * система из приложения завести не даёт. Понадобится такое - путь лежит через
 * надзорную оснастку, а не через этот слой, и решаться это должно отдельно.
 *
 * Записано вслед за сличением состава разборов с эталоном 03.09.2026: неразъяснённый
 * отказ аудиты открывают заново, принимая ограничение системы за недоделку.
 *
 */
awh::net::socket_t awh::eth::Interface::create(const event::eth_t type, string & name) const noexcept {
	 ...
	 *       незачем
	 *
	 * @warning Закрепление идёт ПОСЛЕ удачного заведения, а не до него. Прежде поле
	 *          переписывалось прежде обращения, и отказ заведения оставлял его
	 *          переписанным - а последствие тут хуже неверного ответа `driver()`:
	 *          поле перестаёт быть AUTO, переключатель выше уходит в ветвь явно
	 *          заданного драйвера, и годность драйверов не проверяется больше НИКОГДА.
	 *          То есть одно неудачное заведение навсегда прибивало модуль к драйверу,
	 *          который только что отказал, и запасной путь к соседнему пропадал -
	 *          при том, что ради этого перебора значение AUTO и заведено
	 *
	 */
	...
	// Выводим дескриптор заведённого туннельного устройства
	return result;
}
/**
...
bool awh::eth::Interface::setAddress(string_view name, const net::addr_t * ip, const uint8_t prefix) const noexcept {
	 ...
	 *
	 * @warning Длина уходила системе НЕПРОВЕРЕННОЙ, как это было и у прокладки пути
	 *          (`gateway.cpp`). Хранится она в `uint8_t` и принимает до 255 при пределе
	 *          32 у IPv4 и 128 у IPv6; система такую запись отвергает, но отвечает общим
	 *          отказом, по какому не понять, что не так именно с длиной
	 *
	 * @note Отказ, а не приведение к пределу: длина здесь ЗАДАЁТ сеть, к какой адрес
	 *       причисляется, и приведение обратило бы бессмысленную просьбу в осмысленную,
	 *       но ДРУГУЮ - причислило бы адрес к одному узлу вместо заказанной сети, - и
	 *       сделало бы это молча
	 *
	 * @note Стоит заслон ПЕРЕД поиском устройства намеренно. Длина негодна сама по себе,
	 *       безотносительно устройства, а поиск отвечает своим отказом - и на
	 *       несуществующем устройстве довод о длине не прозвучал бы вовсе. Проверка
	 *       `IfaceOversizedPrefixTest` разбирает именно ДОВОД, и порядок этот ей
	 *       существен
	 *
	 */
...
bool awh::eth::Interface::delAddress(string_view name, const net::addr_t * ip, const net::addr_t * peer) const noexcept {
	...
	switch(ip->size){
		...
		case 16: {
			...
			::memcpy(&row.Address.Ipv6.sin6_addr, &awh_cast <const net::addr_net_ipv6_t *> (ip)->address[0], sizeof(struct in6_addr));
			/**
			 * Зона адреса переносится и при снятии
			 *
			 * @note Записи адресов система метит адресом ЦЕЛИКОМ, а зона - его часть, и
			 *       установка её переносит. Снятие же оставляло зону нулевой, и запись
			 *       адреса канальной связи по такому ключу не находилась: `GetUnicastIpAddressEntry`
			 *       отвечал отказом, снятие не выполнялось вовсе, а вызывающему уходил
			 *       отрицательный ответ - при живом адресе на устройстве
			 *
			 * @warning Несовпадение это молчаливо: адрес остаётся на устройстве и переживает
			 *          прогон ровно так же, как описано у самого метода
			 *
			 */
			row.Address.Ipv6.sin6_scope_id = static_cast <ULONG> (awh_cast <const net::addr_net_ipv6_t *> (ip)->zone);
		} break;
