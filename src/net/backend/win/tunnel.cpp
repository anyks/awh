...
namespace {
	...
	static string __awh_title__(const wchar_t * guid) noexcept {
		...
		wchar_t path[256];
		/**
		 * Сборка ведётся приёмом с усечением
		 *
		 * @note Приём без буквы `s` окончания строки при усечении НЕ ставит: договор у
		 *       него иной, нежели у `snprintf`, и вслед за ним окончание досылали своей
		 *       рукой. Приём с усечением (`_TRUNCATE`) ставит его сам, потому досылка
		 *       убрана - два способа обеспечить одно и то же расходятся при первой же
		 *       правке одного из них
		 */
		// Выполняем сборку пути к ветви настроек соединения
		::_snwprintf_s(path, sizeof(path) / sizeof(path[0]), _TRUNCATE, L"SYSTEM\\CurrentControlSet\\Control\\Network\\{4D36E972-E325-11CE-BFC1-08002BE10318}\\%ls\\Connection", guid);
	...
	static HANDLE __awh_tap__(string & name) noexcept {
		...
		for(DWORD index = 0; result == INVALID_HANDLE_VALUE; index++){
			...
			if(::RegQueryValueExW(entry, L"NetCfgInstanceId", nullptr, nullptr, reinterpret_cast <LPBYTE> (guid), &length) == ERROR_SUCCESS){
				...
				::_snwprintf_s(path, sizeof(path) / sizeof(path[0]), _TRUNCATE, L"\\\\.\\Global\\%ls.tap", guid);
				// Сборка ведётся приёмом с усечением: окончание строки он ставит сам
				/**
...
int64_t awh::win::tunnel::read(const net::socket_t sock, void * buffer, const size_t size) noexcept {
	 ...
	 * Определяем размер переносимых данных
	 *
	 * @note Пакет, не уместившийся в буфер потребителя, переносится усечённым, а остаток
	 *       пропадает: упреждающий приём подаётся следом и место под него занимает заново.
	 *       Признака усечения наружу не идёт - и это НАМЕРЕННО, потому что ровно так же
	 *       ведёт себя чтение туннельного устройства у систем POSIX, строй которых модуль
	 *       и изображает: там усечение делает само ядро, отдавая помещающееся без отказа.
	 *
	 * @note Через движок случай недостижим: устройство кладёт пакет в буфер размером
	 *       0xFFFF, а приём движка подаётся буфером пула в 0x10000 - помещается всегда.
	 *       Достижим он лишь у потребителя, спросившего пакет буфером покороче
	 */
	const size_t count = (static_cast <size_t> (length) < size ? static_cast <size_t> (length) : size);
	// Выполняем перенос принятого пакета в буфер приёма
