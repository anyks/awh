...
namespace {
	 ...
	 *
	 * @warning Замок это ОТДЕЛЬНЫЙ от оберегающего реестр намеренно: открытие работы
	 *          спрашивается и из-под замка реестра, и без него, и общий замок дал бы
	 *          повторный захват
	 *
	 */
	...
	static const qwave_t * __awh_qwave__(const awh::log_t * log) noexcept {
		...
		/**
		 * Открытие ведётся под замком
		 *
		 * @warning Спрашивается открытие и из потока цикла разбора событий, и из
		 *          чужого: отметка сокета кладётся вызывающим откуда угодно. Без
		 *          замка два потока разом входили бы в подключение библиотеки и
		 *          открытие подсистемы, теряя один из двух открытых описателей, а
		 *          третий видел бы уже выставленный признак попытки при ещё пустом
...
bool awh::win::qos::mark(const net::socket_t sock, const event::dscp_t dscp, const log_t * log) noexcept {
	...
	const locker_t <std::shared_mutex> lock(::__awh_mutex__, locker_t <std::shared_mutex>::mode_t::EXCLUSIVE);
	/**
	 * Сличаем реестр заново: пока заводился поток, замок не держался
	 *
	 * @warning Занесение шло прежде безоглядной перезаписью, и поток, заведённый за это
	 *          время соседним потоком исполнения (обращением `apply` либо тем же
	 *          `mark`), затирался вместе со своим номером. Снять его после того было
	 *          нечем: снятие берёт номер из реестра, а там уже стоял наш. Поток
	 *          оставался у подсистемы до конца работы приложения - число их у неё
	 *          ограничено, и течь эта копится
	 *
	 * @note Заведение потока обращается к ядру и держать на нём замок реестра незачем -
	 *       оттого окно и возникает. Закрывается оно не удержанием замка, а сличением
	 *       по возвращении: у кого поток уже есть, тот и остаётся, а лишний снимается
	 *
	 */
	auto i = ::__awh_registry__.find(sock);
	// Если за время заведения потока сокету завели поток на стороне
...
bool awh::win::qos::apply(const net::socket_t sock, const log_t * log) noexcept {
	 ...
	 * Сличаем реестр заново: пока заводился поток, замок не держался
	 *
	 * @warning Запись номера шла прежде без оглядки, и поток, заведённый за это время
	 *          соседним потоком исполнения, затирался вместе со своим номером - снять
	 *          его после того было нечем. Изъятие же записи из реестра за это время
	 *          означает, что сокет освобождён, и заведённый поток остался бы висеть
	 *          на снятом сокете
	 *
	 */
	if((i == ::__awh_registry__.end()) || (i->second.flow != 0)){
		...
		if(qwave != nullptr)
			// Выполняем снятие лишнего потока
			qwave->remove(qwave->handle, static_cast <SOCKET> (sock), flow, 0);
		// Выводим отрицательный результат применения: применил не этот заход
		return false;
	}
	// Запоминаем номер заведённого потока
	i->second.flow = flow;
