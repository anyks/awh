 ...
 *        multicast и параметров переиспользования адреса под операционную систему Linux
 *
...
int32_t awh::eth::Socket::getBufferAvailable(const net::socket_t sock, const net::socket_event_t event) const noexcept {
	 ...
	 *
	 * @note У Linux занятое место сообщают не параметры сокета, а запросы ввода-вывода:
	 *       SIOCOUTQ - для буфера отправки, SIOCINQ - для буфера приёма
	 */
	if(::ioctl(sock, ((event == net::socket_event_t::WRITE) ? SIOCOUTQ : SIOCINQ), &pending) != 0)
		// Выводим признак того, что свободное место неизвестно
		return -1;
...
bool awh::eth::Socket::setKeepalive(const net::socket_t sock, int32_t cnt, int32_t idle, int32_t intvl) const noexcept {
	 ...
	 *
	 * @note Имя настройки у Linux своё: TCP_KEEPIDLE. У macOS то же самое зовётся
	 *       TCP_KEEPALIVE, отчего имена эти легко перепутать - у Linux TCP_KEEPALIVE
	 *       не существует вовсе
	 */
		...
		switch(static_cast <uint8_t> (family)){
			...
			case static_cast <uint8_t> (event::family_t::IPV4): {
				...
				if(ok2 && ((socktype == SOCK_RAW) || (socktype == SOCK_DGRAM))){
					 ...
					 *
					 * @note У Linux настройка эта есть, в отличие от NetBSD и OpenBSD,
					 *       где её нет вовсе и класс обслуживания принятых пакетов
					 *       остаётся невыданным
					 *
					 */
		...
		switch(option){
			...
			case event::options::HDRINCL: {
				...
				switch(static_cast <uint8_t> (family)){
					 ...
					 * Для семейства IPv6
					 *
					 * @details Настройка у Linux ЕСТЬ (IPV6_HDRINCL = 36, уровень
					 *          IPPROTO_IPV6) и применима лишь к сокету
					 *          НЕСТРУКТУРИРОВАННОМУ - ровно как IP_HDRINCL у IPv4.
					 *          Замерено 03.09.2026: сырой IPPROTO_RAW и сырой ICMPv6
					 *          отвечают успехом (под полномочиями), TCP и UDP -
					 *          отказом ENOPROTOOPT
					 *
					 * @warning Прежде здесь стоял ответ УСПЕХОМ без всякого действия, и
					 *          обоснован он был утверждением «своей головы у IPv6 не
					 *          даёт ни одна система». Утверждение неверно: настройку
					 *          имеют Linux и MS Windows. Молчаливый успех на
					 *          невыполненное дело хуже отказа - потребитель, собирающий
					 *          свою голову пакета, получал согласие и неработающий
					 *          сокет
					 *
					 * @note Условная сборка нужна старым заголовкам: настройка
					 *       появилась не сразу, и собираться мы обязаны и там, где её
					 *       объявления нет
					 */
					case static_cast <uint8_t> (event::family_t::IPV6): {
						/**
						...
						/**
						 * Если настройки своей головы IPv6 система не объявляет
						 */
						#else
							// Настройки у этой системы нет, отвечаем успехом на пустое действие
							result = true;
						#endif
					} break;
				}
			...
			case event::options::TCP_CORKING: {
				 ...
				 *
				 * @note Именем TCP_CORK у Linux занят системный макрос, потому опция
				 *       набора зовётся TCP_CORKING - расходится имя, а не смысл
				 *
				 */
				// Включаем/отключаем придержку отправки
				if(!(result = !static_cast <bool> (::setsockopt(sock, IPPROTO_TCP, TCP_CORK, &flags, sizeof(flags))))){
					/**
					 * Если включён режим отладки
					 */
					#if DEBUG_MODE
			...
			case event::options::TCP_NO_DELAY: {
				 ...
				 *          этот сокет сам и знает о нём больше, чем ядро сообщит настройкой.
				 *          Обращение к ядру ради уже известного стоило одного системного
				 *          вызова на каждую установку опции
				 *
				 * @note Неназванный протокол разыскивается у сокета по-прежнему: обращений
				 *       к методу много, и не всякому из них протокол известен
				 *
				 */
				int32_t protocol = 0;
			...
			case event::options::REUSE_PORT: {
				 ...
				 *
				 *          BSD-системы принимают её на таком сокете вхолостую, а ядро
				 *          Linux отвергает отказом о неподдерживаемом действии. Расхождение
				 *          это здесь и сглаживается: договор движка один на все системы, и
				 *          установка полного набора настроек не должна отказывать на одной
				 *          из них из-за настройки, ни на одной из них ничего не делающей
				 *
				 * @note Отказ бы не просто сообщался, а рушил бы установку всего набора:
				 *       настройки применяются одним вызовом, и отказ любой из них делает
				 *       отказом весь вызов
				 *
				 */
 ...
 *
 * @details У Linux обнаружение пути задаётся **режимом**, а не признаком запрета
 *          дробления: настройка принимает одно из состояний - не искать, полагаться на
 *          сведения маршрута, искать всегда, прощупывать вопреки сведениям ядра
 *
 * @note Расхождение это в пользу Linux: у BSD и macOS доступен лишь признак
 *       IP_DONTFRAG, и все деятельные режимы там сводятся к нему одному. Перечисление
 *       набора ложится сюда напрямую
 *
 * @param sock   сетевой сокет
 * @param family семейство протоколов (IPv4 или IPv6)
...
awh::event::mtu_discover_t awh::eth::Socket::getMaximumTransmissionUnitDiscover(const net::socket_t sock, const event::family_t family) const noexcept {
	...
	// Признак успешного чтения настройки
	bool received = false;
	/**
	 * Определяем семейство события
	 */
	switch(static_cast <uint8_t> (family)){
		// Для семейства IPv4
		case static_cast <uint8_t> (event::family_t::IPV4):
			// Получаем режим обнаружения MTU
			received = (::getsockopt(sock, IPPROTO_IP, IP_MTU_DISCOVER, &value, &length) == 0);
		break;
		// Для семейства IPv6
		case static_cast <uint8_t> (event::family_t::IPV6):
			// Получаем режим обнаружения MTU
			received = (::getsockopt(sock, IPPROTO_IPV6, IPV6_MTU_DISCOVER, &value, &length) == 0);
		break;
	}
	// Если настройка прочитана
	if(received){
		/**
		 * Определяем прочитанный режим обнаружения MTU
		 */
		switch(value){
			// Если обнаружение MTU не выполняется
			case IP_PMTUDISC_DONT:
				// Устанавливаем полученный результат
				result = event::mtu_discover_t::DONT;
			break;
			// Если обнаружение MTU ведётся по сведениям маршрута
			case IP_PMTUDISC_WANT:
				// Устанавливаем полученный результат
				result = event::mtu_discover_t::WANT;
			break;
			// Если обнаружение MTU выполняется всегда
			case IP_PMTUDISC_DO:
				// Устанавливаем полученный результат
				result = event::mtu_discover_t::DO;
			break;
			// Если обнаружение MTU ведётся прощупыванием
			case IP_PMTUDISC_PROBE:
				// Устанавливаем полученный результат
				result = event::mtu_discover_t::PROBE;
			break;
		}
	}
	// Возвращаем результат
 ...
 * @par Намеренные решения
 *
 *      **Режимы сверх четырёх ядром не поддержаны.** Перечисление набора несёт девять
 *      состояний, а ядро Linux знает четыре: не искать, по сведениям маршрута, искать
 *      всегда, прощупывать. Прочие - приспособительное, строгое, напористое,
 *      осмотрительное и разумное - средствами сокета не задаются ни у одной системы
 *
 *      Сведены они к ближайшему по смыслу: прощупывание для тех, что ищут предел
 *      настойчиво, и поиск всегда для осмотрительных. Отвечать отказом на них было бы
 *      неверно - запрошенное **выполняется**, пусть и огрублённо, - а молча
 *      подставлять умолчание значило бы скрыть огрубление
 *
 * @param sock   сетевой сокет
 * @param family семейство протоколов (IPv4 или IPv6)
...
bool awh::eth::Socket::setMaximumTransmissionUnitDiscover(const net::socket_t sock, const event::family_t family, const event::mtu_discover_t mode) const noexcept {
	...
	bool result = false;
	// Значение опции
	int32_t value = IP_PMTUDISC_WANT;
	/**
	...
	switch(static_cast <uint8_t> (mode)){
		...
		// Если установлен флаг - выполнять адаптивное обнаружение MTU
		case static_cast <uint8_t> (event::mtu_discover_t::ADAPT):
		// Если установлен флаг - выполнять строгое обнаружение MTU
		case static_cast <uint8_t> (event::mtu_discover_t::STRICT):
		// Если установлен флаг - выполнять интеллектуальное обнаружение MTU
		case static_cast <uint8_t> (event::mtu_discover_t::SMART):
			// Запрещаем дробление всегда
			value = IP_PMTUDISC_DO;
		break;
		// Если установлен флаг - выполнять обнаружение MTU отправкой пробных пакетов
		case static_cast <uint8_t> (event::mtu_discover_t::PROBE):
		// Если установлен флаг - выполнять агрессивное обнаружение MTU
		case static_cast <uint8_t> (event::mtu_discover_t::AGGRESSIVE):
			// Прощупываем предел вопреки сведениям ядра
	...
	switch(static_cast <uint8_t> (family)){
		// Для семейства IPv4
		case static_cast <uint8_t> (event::family_t::IPV4): {
			// Устанавливаем режим обнаружения MTU
			if(!(result = !static_cast <bool> (::setsockopt(sock, IPPROTO_IP, IP_MTU_DISCOVER, &value, sizeof(value))))){
				/**
				 * Если включён режим отладки
				 */
				#if DEBUG_MODE
		...
		case static_cast <uint8_t> (event::family_t::IPV6): {
			// Устанавливаем режим обнаружения MTU
			if(!(result = !static_cast <bool> (::setsockopt(sock, IPPROTO_IPV6, IPV6_MTU_DISCOVER, &value, sizeof(value))))){
				/**
				 * Если включён режим отладки
				 */
				#if DEBUG_MODE
	...
	switch(static_cast <uint8_t> (family)){
		...
		case static_cast <uint8_t> (event::family_t::IPV4): {
			...
			switch(static_cast <uint8_t> (delivery)){
				/**
				 * Если способ доставки не определён
				 *
				 * @details `NONE` - НЕ способ доставки, а признак его отсутствия: сами
				 *          методы движка возвращают его там, где способ установить не
				 *          удалось. Задавать предел переходов нечему, и ответ здесь -
				 *          отказ, а не молчаливое приравнивание к одноадресной доставке
				 *
				 * @warning Принимать `NONE` за `UNICAST` НЕЛЬЗЯ: довод «способ неизвестен»
				 *          превратился бы в тихую подмену его одним определённым, и
				 *          потребитель, забывший задать способ, получал бы согласие на
				 *          действие, о каком не просил
				 *
				 * @note Ветвь записана ЯВНО по разбору 03.09.2026: прежде `NONE` не
				 *       попадал никуда, и верный ответ выходил следствием ОТСУТСТВИЯ
				 *       ветви, а не решением. Неразъяснённое поведение аудиты открывают
				 *       заново
				 */
				case static_cast <uint8_t> (event::delivery_mode_t::NONE): break;
						...
						switch(static_cast <uint8_t> (family)){
							...
							case static_cast <uint8_t> (event::family_t::IPV6): {
								...
								switch(static_cast <uint8_t> (proto)){
									 ...
									 *
									 * @details Ветви этой у IPv6 не было, тогда как у IPv4 она
									 *          есть: довод `protocol_t::RAW` падал в умолчание,
									 *          и заведение сокета отвергалось вовсе
									 *
									 * @warning `IPPROTO_RAW` и «протокол не задан» - НЕ одно и
									 *          то же: с первым пакет уходит вместе с СОБСТВЕННОЙ
									 *          головой, со вторым голову собирает система.
									 *          Подменять одно другим нельзя
									 *
									 * @note Заведено замером 03.09.2026: сырой сокет IPv6 с этим
									 *       протоколом принимают ВСЕ пять систем - Astra Linux,
									 *       FreeBSD 14.1, NetBSD 10.1, OpenBSD 7.9 и Sun Solaris
									 *       11.4. То есть отсутствие ветви было пробелом наречий,
									 *       а не свойством систем
									 */
									case static_cast <uint8_t> (event::protocol_t::RAW):
										// Печатаем дескриптор созданного сокета
										return ::socket(AF_INET6, SOCK_RAW | mode, IPPROTO_RAW);
									// Если протокол определён как ICMP
									case static_cast <uint8_t> (event::protocol_t::TCP):
										// Печатаем дескриптор созданного сокета
