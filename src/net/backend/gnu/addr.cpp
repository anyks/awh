			...
			if((::strncmp("lo", source.iface.c_str(), 2) != 0) &&
			   (::memcmp(&awh_cast <net::addr_mac_t *> (source.mac.get())->address[0], __awh_zero_mac__, 6) == 0)){
				...
				for(struct ifaddrs * ifa = ptr; ifa != nullptr; ifa = ifa->ifa_next){
					 ...
					 *       пользуются AF_LINK и sockaddr_dl. Сведения те же, разнятся
					 *       лишь имена и разметка
					 */
					if((ifa->ifa_addr != nullptr) && (ifa->ifa_addr->sa_family == AF_PACKET)){
						...
						if((result = (sll->sll_halen == 6))){
							// Копируем MAC-адрес в результат
							const uint8_t * ptr = reinterpret_cast <const uint8_t *> (sll->sll_addr);
							// Копируем MAC-адрес в результат
	...
	try {
		...
		switch(static_cast <uint8_t> (node)){
			...
			case static_cast <uint8_t> (event::node_t::PEER): {
				/**
				 * Разрешение аппаратного адреса ведётся по таблице соседей канального
				 * уровня, а у Linux она читается не вызовом `sysctl`, как у BSD, а
				 * выборкой у ядра через netlink - запросом RTM_GETNEIGH
				 *
				 * @note Обход идёт в обе стороны, как и у BSD: задан адрес - ищется
				 *       аппаратный адрес соседа, задан аппаратный - ищется его адрес
				 *
				 */
				// Определяем семейство протоколов по размеру адреса
				const uint8_t family = ((source.ip->size == 4) ? AF_INET : AF_INET6);
				// Выполняем инициализацию объекта опроса ядра
				gnu::netlink_t netlink(this->_log);
				// Признак того, что искомая запись найдена
				bool found = false;
				/**
				 * Выполняем выборку таблицы соседей канального уровня
				 */
				netlink.dump(RTM_GETNEIGH, family, [&](const struct nlmsghdr * header) noexcept -> bool {
					// Если сообщение не является записью таблицы соседей
					if(header->nlmsg_type != RTM_NEWNEIGH)
						// Продолжаем обход
						return true;
					// Получаем описание записи таблицы соседей
					const struct ndmsg * neighbour = reinterpret_cast <const struct ndmsg *> (NLMSG_DATA(header));
					// Если семейство протоколов записи не совпадает с искомым
					if(neighbour->ndm_family != family)
						// Продолжаем обход
						return true;
					// Адрес соседа
					const void * address = nullptr;
					// Аппаратный адрес соседа
					const uint8_t * hardware = nullptr;
					// Получаем первый признак записи
					const struct rtattr * attribute = reinterpret_cast <const struct rtattr *> (reinterpret_cast <const char *> (neighbour) + NLMSG_ALIGN(sizeof(struct ndmsg)));
					// Получаем размер оставшихся признаков записи
					int32_t length = static_cast <int32_t> (header->nlmsg_len - NLMSG_LENGTH(sizeof(struct ndmsg)));
					/**
					...
					for(; RTA_OK(attribute, length); attribute = RTA_NEXT(attribute, length)){
						...
						switch(attribute->rta_type){
							// Если признак содержит адрес соседа
							case NDA_DST:
								// Запоминаем адрес соседа
								address = RTA_DATA(attribute);
							break;
							// Если признак содержит аппаратный адрес соседа
							case NDA_LLADDR:
								// Если длина аппаратного адреса соответствует MAC-адресу
								if(RTA_PAYLOAD(attribute) == 6)
									// Запоминаем аппаратный адрес соседа
									hardware = reinterpret_cast <const uint8_t *> (RTA_DATA(attribute));
							break;
						}
					}
					// Если запись неполна, продолжаем обход
					if((address == nullptr) || (hardware == nullptr))
						// Продолжаем обход
						return true;
					/**
					 * Определяем тип адреса
					...
					switch(source.ip->size){
						// Если адрес является IPv4
						case 4: {
							// Получаем числовое значение искомого IP-адреса
							...
							if(addr > 0){
								...
								if(addr != *reinterpret_cast <const uint32_t *> (address))
									// Продолжаем обход
									return true;
								// Копируем MAC-адрес в результат
								::memcpy(&awh_cast <net::addr_mac_t *> (source.mac.get())->address[0], hardware, 6);
								// Запоминаем, что запись найдена
								found = true;
							// Если MAC-адреса совпадают
							} else if(::memcmp(&awh_cast <net::addr_mac_t *> (source.mac.get())->address[0], hardware, 6) == 0){
								// Копируем IP-адрес в результат
								awh_cast <net::addr_net_ipv4_t *> (source.ip.get())->address = *reinterpret_cast <const uint32_t *> (address);
								// Запоминаем, что запись найдена
								found = true;
							}
						} break;
						...
						case 16: {
							...
							if(::memcmp(&awh_cast <net::addr_net_ipv6_t *> (source.ip.get())->address[0], __awh_zero_ipv6__, 16) != 0){
								// Если искомый IP-адрес не совпадает, пропускаем
								if(::memcmp(&awh_cast <net::addr_net_ipv6_t *> (source.ip.get())->address[0], address, 16) != 0)
									// Продолжаем обход
									return true;
								// Копируем MAC-адрес в результат
								::memcpy(&awh_cast <net::addr_mac_t *> (source.mac.get())->address[0], hardware, 6);
							...
							// Если MAC-адреса совпадают
								...
								if(::if_indextoname(static_cast <uint32_t> (neighbour->ndm_ifindex), iface) != nullptr){
									// Устанавливаем название сетевого интерфейса
									source.iface = iface;
								...
								if(::memcmp(&awh_cast <net::addr_net_ipv6_t *> (source.ip.get())->address[0], __awh_zero_ipv6__, 16) == 0){
									// Копируем IP-адрес в результат
									::memcpy(&awh_cast <net::addr_net_ipv6_t *> (source.ip.get())->address[0], address, 16);
									// Проставляем зону адресу, которому она нужна
									::scope(source);
								}
								// Запоминаем, что запись найдена
					...
					// Прекращаем обход, если запись найдена
					return !found;
				});
			} break;
						...
						for(struct ifaddrs * ifa = ptr; ifa != nullptr; ifa = ifa->ifa_next){
							...
							// Если IP-адрес не установлен
								...
								for(struct ifaddrs * link = ptr; link != nullptr; link = link->ifa_next){
									...
									if((link->ifa_addr != nullptr) && (link->ifa_addr->sa_family == AF_PACKET) && (link->ifa_name != nullptr) && (::strcmp(link->ifa_name, ifa->ifa_name) == 0)){
										// Получаем текущее значение аппаратного сетевого адреса
										struct sockaddr_ll * sll = reinterpret_cast <struct sockaddr_ll *> (link->ifa_addr);
