...
namespace routing {
	...
	static bool modify(const uint16_t type, const uint16_t flags, const awh::eth::Gateway::route_t & route, const awh::log_t * log) noexcept {
		...
		const size_t size = ((family == AF_INET) ? 4 : 16);
		/**
		 * @warning Несличимую пару видов надлежит ОТВЕРГАТЬ, а не тихо ронять адрес
		 *          назначения. Прежде разбор стоял лишь при СОСТАВЛЕНИИ сообщения
		 *          (условие совпадения размеров), и назначение иного вида, нежели шлюз,
		 *          в сообщение просто не попадало - а сообщение уходило ядру дальше,
		 *          уже БЕЗ адреса назначения. Под правами суперпользователя ядро такой
		 *          снос принимает и отвечает успехом: снести могло не тот маршрут, а
		 *          вызывающий получал истину
		 *
		 * @details Замерено на стенде Debian 12, ядро 6.1.0-50 (09.09.2026): вызов
		 *          `remove` по паре «шлюз IPv4, назначение IPv6» под root отвечал
		 *          истиной. Без прав отказ давало отсутствие прав, оттого проверка
		 *          `GatewayMismatchedKindTest` беды и не показывала - она никогда не
		 *          шла под правами
		 */
		 ...
		 *
		 * @note Неопределённого поведения здесь НЕТ: длина уходит ядру полем записи,
		 *       а не счётчиком сдвига, как у наречия BSD. Беда иная - ядро отвергает
		 *       такую запись общим `EINVAL`, по какому выходит, будто не так с самим
		 *       путём, а не с его длиной. Отказ обязан называть ПРИЧИНУ
		 *
		 * @note Здесь именно ОТКАЗ, а не приведение к пределу: приведение обратило бы
		 *       бессмысленную просьбу в осмысленную, но ДРУГУЮ - путь к одному узлу
		 *       вместо заказанной сети, - и сделало бы это молча
		 */
		...
		/**
		 * @warning Тем же свойством обладают ЕЩЁ ДВА поля этого сообщения: при сносе ядро
		 *          их СЛИЧАЕТ, а не принимает как описание намерения. `fib_table_delete`
		 *          сличает оба условно - `RT_SCOPE_NOWHERE` означает «область любая»,
		 *          `RTN_UNSPEC` означает «вид любой», - и `ip route del` не задаёт ни
		 *          одного из трёх полей вовсе
		 *
		 * @details Замер в отдельном пространстве имён сети на стенде Debian 12,
		 *          ядро 6.1.0-50 (09.09.2026). Одно и то же сообщение сноса, разница
		 *          РОВНО в этих двух полях:
		 *
		 *          | маршрут                    | UNIVERSE + UNICAST | NOWHERE + UNSPEC |
		 *          |----------------------------|--------------------|------------------|
		 ...
		 *          | `10.66.66.0/24 via ...`    | снесён             | -                |
		 *
		 *          Последняя строка - эталон: тем же щупом и тем же путём маршрут области
		 *          UNIVERSE сносится, то есть отказ давали поля, а не щуп. До правки снос
		 *          не находил маршрут напрямую подключённой подсети и маршруты видов
		 *          `blackhole`/`unreachable`/`prohibit` вовсе
		 */
		// Устанавливаем область действия маршрута
		message.route.rtm_scope = ((type == RTM_DELROUTE) ? RT_SCOPE_NOWHERE : RT_SCOPE_UNIVERSE);
		// Устанавливаем вид маршрута
		message.route.rtm_type = ((type == RTM_DELROUTE) ? RTN_UNSPEC : RTN_UNICAST);
 ...
 *
 *      **Выборка таблицы, а следом - запрос одного маршрута.** У Linux путей два, и
 *      здесь взяты ОБА, по очереди. Выборка целиком идёт первой затем, что метод
 *      отвечает не только на вопрос «куда пойдёт пакет», но и на вопрос «каков маршрут
 *      по умолчанию», а его точечным запросом не задать: у маршрута по умолчанию адреса
 *      назначения нет вовсе.
 *
 *      Точечный запрос RTM_GETROUTE идёт вторым проходом и лишь по неудаче сличения,
 *      когда назван ОДИН адрес назначения. Прежде его не было вовсе, и адрес, попадающий
 *      в сеть по её приставке, объявлялся недостижимым: своей записи в таблице у него
 *      нет, а сличение по полю назначения точное. Ядро же подбирает путь по
 *      наидлиннейшей приставке
 *
 *      Обход прерывается на первой подошедшей записи, так что лишним чтением это не
 *      оборачивается, а второй проход обычного хода не касается вовсе
 *
...
bool awh::eth::Gateway::get(route_t & route) const noexcept {
	...
	try {
		 ...
		 *
		 * @warning Заслон этот стоял лишь на ЧТЕНИИ адреса назначения, а на записи
		 *          найденного маршрута его не было вовсе: вид там берётся от шлюза, и
		 *          назначение иного вида переписывалось шестнадцатью октетами при
		 *          выделенных четырёх. Ворошитель `tools/fuzz/eth.cpp` вскрыл это
		 *          надзирателем `undefined`: «downcast of address ... which does not
		 *          point to an object of type Address_Network_IPv4». Запись хуже
		 *          чтения: порча памяти уходит за пределы объекта молча
		 */
		if((route.destination != nullptr) && (route.gateway->size != route.destination->size))
		...
		/**
		 * Если названный адрес своей записи в таблице не имеет - спрашиваем ЯДРО
		 *
		 * @details Метод отвечает на вопрос «каким путём пойдёт пакет», а таблица знает
		 *          лишь заведённые пути. Адрес, попадающий в сеть по её приставке,
		 *          своей записи не имеет вовсе, и сличение `memcmp` по полю назначения
		 *          его не находит - точное сличение отвечает лишь тем адресам, что в
		 *          таблице записаны дословно.
		 *
		 *          Ядро же подбирает путь по НАИДЛИННЕЙШЕЙ приставке и отвечает той
		 *          записью, которой воспользуется само. Запрос этот подаётся вторым
		 *          проходом - лишь по неудаче сличения, - оттого обычный ход не
		 *          дорожает ни на одно обращение
		 *
		 * @warning Прежде второго прохода не было, и адрес без своей записи объявлялся
		 *          недостижимым. Проверка `GatewayGetSpecificDestinationNotFoundIPv4`
		 *          на Linux от того ОТСТУПАЛА с доводом «выхода наружу у машины нет»,
		 *          приписывая среде свойство кода: на macOS та же проверка проходит.
		 *          Три наречия из четырёх спрашивают систему давно - `bsd` и `sun`
		 *          запросом `RTM_GET`, `win` через `GetBestRoute2`
		 *
		 * @note Условие второго прохода то же, что и у соседей: спрашивать систему
		 *       следует тогда, когда назван ОДИН лишь адрес назначения. При названном
		 *       устройстве спрашивают не «куда пойдёт пакет», а «что записано в
		 *       таблице», и подменять один вопрос другим нельзя
		 */
		if(!found && wanted && (index == 0)){
			/**
			 * @brief Структура сообщения запроса одного маршрута
			 *
			 */
			struct {
				// Заголовок сообщения
			...
			// Устанавливаем размер сообщения
			query.header.nlmsg_len = NLMSG_LENGTH(sizeof(struct rtmsg));
			// Устанавливаем тип запроса
			query.header.nlmsg_type = RTM_GETROUTE;
			// Устанавливаем признаки запроса
			query.header.nlmsg_flags = NLM_F_REQUEST;
			// Устанавливаем порядковый номер запроса
			query.header.nlmsg_seq = 1;
			// Устанавливаем семейство протоколов маршрута
			query.route.rtm_family = family;
			// Устанавливаем длину префикса адреса назначения по разрядности адреса
			query.route.rtm_dst_len = static_cast <uint8_t> (size * 8);
			// Добавляем адрес назначения, путь до которого спрашиваем
			::routing::attribute(&query.header, sizeof(query), RTA_DST, destination, size);
			/**
			 * Выполняем запрос разрешённого ядром маршрута
			 */
			netlink.request(&query, query.header.nlmsg_len, [&](const struct nlmsghdr * header) noexcept -> bool {
				// Разобранная запись ответа ядра
				::routing::entry_t current;
				// Если запись разобрать не удалось
				if(!::routing::parse(header, family, current))
					// Продолжаем обход
					return true;
				// Запоминаем разрешённую ядром запись
				entry = current;
				// Запоминаем, что запись найдена
				found = true;
				// Прекращаем обход
