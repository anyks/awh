...
namespace device {
	 ...
	 *
	 */
	static void routePeers(const string_view name) noexcept {
	...
	static bool applyAddress(const awh::net::socket_t sock, const string_view name, const awh::net::addr_t * ip, const awh::net::addr_t * peer, const uint8_t prefix) noexcept {
		...
		if(::device::oversized(name, ip, prefix))
	...
	if(!name.empty()){
		...
		try {
			...
			if(::getifaddrs(&ptr) == 0){
				...
				result = ::device::classify(ptr, name, false);
			}
		/**
		 * Если возникает ошибка
		 */
	...
	if(!name.empty()){
		...
		try {
			...
			if(::getifaddrs(&ptr) == 0){
				...
				result = ::device::classify(ptr, name, true);
			}
		/**
		 * Если возникает ошибка
		 */
	...
	try {
		...
		switch(static_cast <uint8_t> (type)){
			...
			case static_cast <uint8_t> (event::eth_t::VLAN):
				...
				result = ::device::clonable("vlan", name);
			break;
			...
			case static_cast <uint8_t> (event::eth_t::BOND):
				 ...
				 *          «lagg» отсутствует
				 */
				result = ::device::clonable("bond", name);
			break;
			...
			case static_cast <uint8_t> (event::eth_t::BRIDGE):
				...
				result = ::device::clonable("bridge", name);
			break;
...
bool awh::eth::Interface::setAddress(string_view name, const net::addr_t * ip, const uint8_t prefix) const noexcept {
	...
	if(!name.empty() && (ip != nullptr)){
		...
		try {
			...
			result = ::device::applyAddress(sock, name, ip, nullptr, prefix);
			// Закрываем сокет
			::close(sock);
		/**
		 * Если возникает ошибка
...
bool awh::eth::Interface::setAddress(string_view name, const net::addr_t * ip, const net::addr_t * peer, const uint8_t prefix) const noexcept {
	...
	if(!name.empty() && (ip != nullptr) && (peer != nullptr) && (ip->size == peer->size)){
		...
		try {
			...
			result = ::device::applyAddress(sock, name, ip, peer, prefix);
			// Закрываем сокет
			::close(sock);
		/**
		 * Если возникает ошибка
