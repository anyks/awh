...
namespace iface {
	...
	static bool applyAddress(const awh::net::socket_t sock, const string_view name, const awh::net::addr_t * ip, const awh::net::addr_t * peer, const uint8_t prefix, const awh::log_t * log) noexcept {
		...
		bool result = false;
		/**
		 * Сличаем длину префикса с разрядностью адреса
		 *
		 * @details Предел зависит от ВИДА адреса: 32 у IPv4, 128 у IPv6. Прежде негодная
		 *          длина здесь молча приводилась к маске одного узла, и ответов на одну
		 *          бессмыслицу выходило три разных по наречиям - отказ у прокладки пути,
		 *          приведение здесь, пропуск установки маски у IPv6. Ответ сведён к
		 *          одному: ОТКАЗ с доводом
		 *
		 * @note Ноль длиной негодной НЕ считается и отвергнут быть не должен: он значит
		 *       «длина не задана», и обработка его ниже своя. Прежде ноль и бессмыслица
		 *       были слиты в одно условие - в том и был корень разнобоя
		 *
		 * @warning Приведение к пределу здесь неверно: оно обращает бессмысленную просьбу
		 *          в осмысленную, но ДРУГУЮ - адрес одного узла вместо заказанной сети, -
		 *          и делает это молча
		 */
	 ...
	 *
	 */
	static awh::net::socket_t attach(const string & path, const awh::log_t * log, const bool verbose = true) noexcept {
	 ...
	 *
	 */
	static awh::net::socket_t tunnel(string & name, const awh::log_t * log) noexcept {
		/**
		...
		if(!name.empty())
			...
			return ::iface::attach(("/dev/net/" + name), log);
		/**
