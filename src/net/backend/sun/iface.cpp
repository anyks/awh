...
namespace iface {
	...
	/**
	 * @brief Функция проверки того, что за связью уже числится адрес
	 *
	 * @details Признак выявляет связи, поднятые ДРУГИМ процессом: свой учёт о них
	 *          не знает, а система своего учёта туннелей не ведёт вовсе
	 *
	 * @param name имя проверяемой связи
	 * @return     признак наличия назначенного адреса
	 *
	 */
	static bool assigned(const string & name) noexcept {
		// Создаём сокет управления сетевыми интерфейсами
		const awh::net::socket_t sock = ::socket(AF_INET, SOCK_DGRAM, 0);
		// Если сокет управления создать не удалось
		if(sock == awh::net::invalid_socket_t)
			/**
			 * Считаем связь свободной
			 *
			 * @note Отказ сокета управления - не свидетельство занятости. Объявив связь
			 *       занятой, мы отбросили бы её на пустом месте
			 */
			return false;
		// Объект запроса настройки логического интерфейса
		struct lifreq lifr;
		// Заполняем объект запроса нулями
		::memset(&lifr, 0, sizeof(lifr));
		// Копируем имя проверяемой связи
		::iface::copyName(lifr.lifr_name, name);
		// Спрашиваем у связи назначенный ей адрес
		const bool result = (::ioctl(sock, SIOCGLIFADDR, &lifr) == 0);
		// Закрываем сокет управления
		::close(sock);
		// Если адрес получить не удалось, связь свободна
		if(!result)
			// Выводим результат
			return false;
		// Получаем назначенный связи адрес
		const struct sockaddr_in * addr = reinterpret_cast <const struct sockaddr_in *> (&lifr.lifr_addr);
		/**
		 * Выводим признак того, что адрес назначен не нулевой
		 *
		 * @note Поднятая, но не настроенная связь отдаёт нулевой адрес, и занятой
		 *       она не является: именно такой её оставляет «ipadm create-ip»
		 */
	...
	static awh::net::socket_t tunnel(string & name, const awh::log_t * log) noexcept {
		 ...
		 *          занятость выясняется ДО открытия, двумя разными способами
		 */
		for(uint16_t unit = 0; unit < 0x100; unit++){
			...
			if(!::iface::occupy(candidate))
				...
				continue;
			/**
			 * Пропускаем связь, за которой уже числится адрес
			 *
			 * @details Признак этот выявляет связи, занятые ДРУГИМ процессом: свой
			 *          учёт о чужих туннелях не знает. Туннель без адреса нежизнеспособен,
			 *          и назначенный адрес означает, что связь кем-то поднята
			 *
			 * @note Признак не безупречен: между открытием связи и назначением ей адреса
			 *       есть промежуток, и два процесса, стартующих разом, могут выбрать одну
			 *       связь. Надёжнее было бы запереть её замком, но замки fcntl на этих
			 *       устройствах не держат - проверено щупом, два процесса взяли замок
			 *       на одном устройстве одновременно
			 */
			if(::iface::assigned(candidate)){
				// Освобождаем связь в своём учёте
