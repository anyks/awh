...
void awh::Network_Queue::clear() noexcept {
	...
	this->_bwrite = 0;
	// Сбрасываем смещение выданного участка прямой записи
	this->_offset = 0;
	// Сбрасываем ёмкость выданного участка прямой записи
	this->_capacity = 0;
	// Снимаем признак выданного участка прямой записи
	this->_reserved = false;
	// Снимаем признак открытия вторичного региона B выданным участком
	this->_opening = false;
 ...
 *
 */
bool awh::Network_Queue::tail(void ** data, size_t & size) noexcept {
	// Если параметры переданы неверно либо прежняя выдача ещё не подтверждена
	if((data == nullptr) || this->_reserved)
		// Возвращаем значение по умолчанию (false) - повторной выдачи не делаем
		return false;
	// Выделяем буфер при необходимости
	if(!this->reserve())
		// Места в памяти нет - участок не выдаётся
		return false;
	/**
	 * Определяем тип очереди для выдачи участка
	 */
	switch(static_cast <uint8_t> (this->_type)){
		// Если очередь для потоков данных (например, TCP, bip-режим)
		case static_cast <uint8_t> (type_t::TCP): {
			// Если активен вторичный регион B - участок выдаётся только в нём
			if(this->_bwrite > 0){
				// Устанавливаем ёмкость участка между концом B и началом региона A
				size = (this->_read - this->_bwrite);
				// Устанавливаем смещение участка на конец вторичного региона B
				this->_offset = this->_bwrite;
				// Отмечаем, что вторичный регион B уже открыт
				this->_opening = false;
			// Иначе выбираем больший из непрерывных свободных регионов (хвост за A или голова перед A)
			} else {
				// Размер свободного хвоста за регионом A
				...
				// Размер свободной головы перед регионом A (потенциальный регион B)
				const size_t head = this->_read;
				// Если хвост за регионом A не меньше головы перед ним - выдаём хвост (регион A)
				if(tail >= head){
					// Устанавливаем ёмкость участка равной свободному хвосту
					size = tail;
					// Устанавливаем смещение участка на конец региона A
					this->_offset = this->_write;
					// Отмечаем, что вторичный регион B не открывается
					this->_opening = false;
				// Иначе выдаём голову перед регионом A, открывая вторичный регион B
				} else {
					// Устанавливаем ёмкость участка равной свободной голове
					size = head;
					// Устанавливаем смещение участка на начало буфера
					this->_offset = 0;
					// Отмечаем, что выданный участок открывает вторичный регион B
					this->_opening = true;
				}
			}
		} break;
		// Если очередь для границ сообщений (например, UDP, линейный режим)
		case static_cast <uint8_t> (type_t::UDP): {
			// Если места в конце буфера не хватает даже под заголовок записи
			if((AWH_NETWORK_QUEUE_BUFFER_SIZE - this->_write) < sizeof(size_t))
				// Выполняем сжатие очереди сдвигом данных к началу буфера
				this->compact();
			// Вычисляем свободное место в конце буфера
			const size_t space = (AWH_NETWORK_QUEUE_BUFFER_SIZE - this->_write);
			// Если места не хватает даже под заголовок записи
			if(space < sizeof(size_t))
				// Возвращаем значение по умолчанию (false) - участок не выдаётся
				return false;
			// Устанавливаем ёмкость участка за вычетом места под заголовок записи
			size = (space - sizeof(size_t));
			// Устанавливаем смещение участка за заголовком записи
			this->_offset = (this->_write + sizeof(size_t));
			// Отмечаем, что вторичный регион B здесь не применяется
			this->_opening = false;
		} break;
		// Если тип очереди не установлен
		default: return false;
	}
	// Если свободного места в выбранном регионе не осталось
	if(size == 0)
		// Возвращаем значение по умолчанию (false) - выдавать нечего
		return false;
	// Запоминаем ёмкость выданного участка для проверки при подтверждении
	this->_capacity = size;
	// Устанавливаем указатель на начало выданного участка
	(* data) = (this->_buffer + this->_offset);
	// Отмечаем участок выданным до его подтверждения
	this->_reserved = true;
	// Выводим результат успешной выдачи участка
	return true;
}
/**
 ...
 *
 */
bool awh::Network_Queue::commit(const size_t size) noexcept {
	// Если выдачи участка не было
	if(!this->_reserved)
		// Возвращаем значение по умолчанию (false) - подтверждать нечего
		return false;
	// Снимаем признак выдачи участка при любом исходе подтверждения
	this->_reserved = false;
	/**
	 * Если записано больше, чем было выдано
	 *
	 * @warning Верить источнику на слово НЕЛЬЗЯ: подтверждение сверх выданной ёмкости
	 *          сдвинуло бы границы региона за пределы буфера и молча испортило очередь
	 */
	if(size > this->_capacity){
		// Записываем ошибку в лог
		this->_log->print("The source has confirmed more data than the given capacity of the region of the queue", log_t::flag_t::CRITICAL);
		// Выводим результат отказа подтверждения
		return false;
	}
	// Если записанного нет - выдача отменяется, содержимое очереди не меняется
	if(size == 0)
		// Выводим результат успешной отмены выдачи участка
		return true;
	/**
	 * Определяем тип очереди для подтверждения записанного
	 */
	switch(static_cast <uint8_t> (this->_type)){
		// Если очередь для потоков данных (например, TCP, bip-режим)
		case static_cast <uint8_t> (type_t::TCP): {
			// Если выданный участок открывает вторичный регион B
			if(this->_opening)
				// Устанавливаем конец вторичного региона B
				this->_bwrite = size;
			// Если вторичный регион B уже был открыт
			else if(this->_bwrite > 0)
				// Сдвигаем конец вторичного региона B
				this->_bwrite += size;
			// Иначе сдвигаем конец основного региона A
			else this->_write += size;
			// Увеличиваем счётчик записей в очереди (для TCP он отслеживает количество байт)
			this->_count += size;
		} break;
		// Если очередь для границ сообщений (например, UDP, линейный режим)
		case static_cast <uint8_t> (type_t::UDP): {
			// Устанавливаем размер записи в её заголовок (прямой доступ)
			this->recordSize((this->_offset - sizeof(size_t)), size);
			// Увеличиваем позицию записи на размер заголовка и полезных данных
			this->_write = (this->_offset + size);
			// Увеличиваем счётчик записей в очереди
			this->_count++;
		} break;
	}
	// Обновляем кэшированный размер полезных данных
	this->_total += size;
	// Выводим результат успешного подтверждения записанного
	return true;
}
/**
...
awh::Network_Queue::Network_Queue(const fmk_t * fmk, const log_t * log) noexcept :
 ...
 _count(0), _bwrite(0), _offset(0), _capacity(0), _reserved(false), _opening(false), _buffer(nullptr), _fmk(fmk), _log(log) {}
/**
 * @brief Деструктор
 *
 */
