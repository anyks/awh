...
namespace {
	...
	/**
	 * @brief Класс контейнера свободных блоков буфера с корректной очисткой при завершении потока
	 *
	 */
	typedef class BlockPool {
		public:
 ...
 *
 */
size_t awh::Network_Queue::push(const void * head, const size_t length, const void * data, const size_t size) noexcept {
	// Если голова записи не передана - кладём данные обычным порядком
	if((head == nullptr) || (length == 0))
		// Выполняем добавление данных в очередь
		return this->push(data, size);
	// Если размер данных для добавления в очередь передан неверно
	if((size == 0) || (data == nullptr))
		// Возвращаем результат, поместить нечего
		return 0;
	/**
	 * Если очередь заведена не с сохранением границ сообщений
	 *
	 * @note Потоковая очередь вправе принять запись частью, и голова оторвалась бы
	 *       от своих данных: получатель прочёл бы настройки одного сообщения вместе
	 *       с данными другого. Отказ здесь честнее молчаливой порчи
	 */
	if(this->_type == type_t::TCP){
		/**
		 * Если запись целиком в непрерывный свободный участок не умещается
		 *
		 * @note Правило «сколько влезет» здесь недопустимо: голова оторвалась бы от
		 *       своих данных либо легла бы частью, и потребитель прочёл бы настройки
		 *       одного сообщения вместе с данными другого. Вызывающий обязан спросить
		 *       свободное место заранее и уложиться в него вместе с головой
		 */
		if(this->available() < (length + size))
			// Возвращаем результат, запись не помещается целиком
			return 0;
		// Выделяем буфер при необходимости
		if(!this->reserve())
			// Места в памяти нет - запись не выполняется
			return 0;
		// Укладываем голову записи в очередь
		if(this->push(head, length) != length)
			// Возвращаем результат, поместить не удалось
			return 0;
		// Укладываем данные записи в очередь следом за головой
		const size_t result = this->push(data, size);
		/**
		 * Если данные записи легли не целиком
		 *
		 * @details Случиться этого не должно: свободное место спрошено заранее, а
		 *          голова забирает его из того же непрерывного участка, и остатка
		 *          заведомо хватает на данные. Проверка стоит на случай, если
		 *          устройство очереди изменится: разорванная запись портит разбор
		 *          молча, и узнать о ней иначе будет неоткуда
		 *
		 * @warning Отменить укладку головы нечем: очередь снимает записи с головы
		 *          чтения, а не записи. Здесь остаётся лишь доложить о порче
		 */
		if(result != size)
	...
	// Определяем размер записи = размер головы + размер данных + заголовок (size_t)
	const size_t record = (sizeof(size_t) + length + size);
	// Быстрая проверка на переполнение буфера
	if((AWH_NETWORK_QUEUE_BUFFER_SIZE - this->_write) < record){
		// Недостаточно места в конце - пробуем сжать
		this->compact();
	...
	if(!this->reserve())
		...
		return 0;
	// Устанавливаем размер записи (прямой доступ)
	this->recordSize(this->_write, (length + size));
	// Увеличиваем позицию записи на размер заголовка
	this->_write += sizeof(size_t);
	// Копируем голову записи в буфер очереди
	::memcpy(this->_buffer + this->_write, head, length);
	// Увеличиваем позицию записи на размер головы
	this->_write += length;
	// Копируем данные в буфер очереди
