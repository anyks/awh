...
void awh::Uniform_Resource_Identifier::appendQuery(string & result, const bool separator, const bool callback) const noexcept {
	...
	try {
		...
		if(count > 0){
			 ...
			 *
			 * Пары поэтому выстраиваются по ключу, а совпадающие по ключу - ещё и
			 * по значению. Одного ключа мало: пары с общим ключом хранилище держит
			 * подряд, но в каком порядке - не обещает вовсе, и порядок этот у разных
			 * реализаций стандартной библиотеки свой. Адрес "?tag=one&tag=two"
			 * оттого перекладывался на каждом обороте и ни к одному виду не сходился
			 * там, где хранилище держит совпадающие пары не в порядке занесения.
			 * Сличение по значению доводит выстраивание до полного и делает строку
			 * одной и той же при любом числе оборотов на любой реализации.
			 *
			 * @note Значения совпадающих ключей от этого переставляются, и порядок
			 *       их занесения не сохраняется. Так и задумано: повторяющийся ключ
			 *       несёт набор равноправных параметров, а не упорядоченный перечень,
			 *       и потребитель собирает их по ключу сам
			 *
			...
			::stable_sort(pairs, pairs + count, [](const pair <const string, string> * first, const pair <const string, string> * second) noexcept -> bool {
				// Если ключи пар различаются, сравниваем пары по их ключам
				if(first->first != second->first)
					// Выводим результат сравнения ключей пар
					return (first->first < second->first);
				// Выводим результат сравнения значений пар
