...
void awh::Uniform_Resource_Identifier::host(string_view host) noexcept {
	...
	try {
		...
		if(!network){
			 ...
			 */
			if(uri::hasControl(host) || (!filesystem && (host.find(' ') != string_view::npos))){
				// Сбрасываем атрибуты URI адреса
				this->_attr.reset(nullptr);
				// Выходим из метода
				return;
...
bool awh::Uniform_Resource_Identifier::resolve(string_view uri) noexcept {
	...
	try {
		...
		if(uri::parse(this->_fmk, uri, scheme, userinfo, host, port, path, query, fragment, form, guess)){
			 ...
			 *
			 * ВНИМАНИЕ: условие это повторяет условие отказа в установщике хоста и
			 * обязано меняться вместе с ним. Расширь установщик приметы отказа - к
			 * примеру, длиной метки имени, - и хост, отвергнутый по новой примете,
			 * снимка бы не получил: адрес снова оставался бы полусобранным
			 */
			// Признак того, что отвергнутый хост испортил бы разобранный прежде адрес
			const bool guarded = (!host.empty() && !this->empty() &&
				(uri::hasControl(host) || (host.find(' ') != string_view::npos)));
