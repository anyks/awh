 ...
 *
 * @warning Заголовок «limits» нужен пределу числа слов разреза. У систем POSIX он
 *          приходит попутно через иные включения, у MinGW же цепочка включений
 *          иная, и сборка библиотеки вставала на этом файле целиком. Замерено
 *          16.09.2026 на стенде MSYS2 с g++ 16.1.0
 */
#include <limits>
...
bool awh::args::Lexer::split(const string_view text, vector <string> & result, const failure_t & failure) const noexcept {
	...
	char quote = 0;
	/**
	 * Предел числа слов разреза, выведенный из предела числа лексем разбора
	 *
	 * @note Одна лексема забирает не более ДВУХ слов - имя и значение, поданное
	 *       отдельным доводом, - и потому слов сверх удвоенного предела в предел
	 *       лексем не уложится никак, каким бы ни был их состав
	 *
	 * @warning Предел лексем действовал лишь у разбора уже нарезанного набора, а
	 *          разрез текста шёл до конца всегда: текст в 400 000 слов при пределе
	 *          в 10 лексем нарезался целиком и стоил 82 мс, тогда как выдано было
	 *          ровно 10 лексем. Потребитель, оградивший себя пределом от подачи
	 *          недоброй, ограждён не был вовсе. Замерено 16.09.2026 аудитом
	 *
	 * @note Отказ этот выдачу лексем опережает: текстовая дорога отвечает отказом
	 *       ещё до разбора, тогда как дорога набора уже нарезанного выдаёт лексемы
	 *       до предела и лишь затем отказывает. Разнится лишь то, сколько выдано
	 *       прежде отказа; сам ответ у обеих дорог один - разбор НЕ принят
	 */
	const size_t limit = ((this->_settings.maxTokens > 0) && (this->_settings.maxTokens <= (numeric_limits <size_t>::max() / 2)) ? (this->_settings.maxTokens * 2) : 0);
	// Выполняем перебор всех знаков поданного текста
	for(size_t i = 0; i < text.length(); i++){
		// Если число собранных слов предел разреза превысило
		if((limit > 0) && (result.size() >= limit)){
			// Выполняем извещение об отказе разбора
			notify(failure, error_t::MANY_TOKENS, location_t(result.size(), i));
			// Выходим из метода, текст разрезан не весь
			return false;
		}
		// Получаем знак поданного текста
		const char letter = text.at(i);
		/**
		 * Если знаком является обратная косая, а кавычка одинарная не открыта
		 *
		 * @warning Внутри кавычки ОДИНАРНОЙ косая особого значения не имеет и берётся
		 *          знаком как есть - таково правило оболочек POSIX, и потребитель
		 *          ставит кавычку одинарную именно затем, чтобы уберечь косые. Прежде
		 *          косая снималась и там: запись «'C:\Program\dir'» давала
		 *          «C:Programdir», а «'\d+'» давало «d+», и урон был МОЛЧАЛИВ -
		 *          отказа разрез не выдавал. Замерено 16.09.2026 щупом
		 *
