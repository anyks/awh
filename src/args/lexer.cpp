...
bool awh::args::Lexer::parse(const vector <string> & items, const callback_t & callback, const failure_t & failure) const noexcept {
	...
	for(size_t i = 0; i < items.size(); i++){
		...
		/**
		 * Если доводом является одиночное тире
		 *
		 * @warning Одиночное тире есть ПОЗИЦИОННЫЙ ДОВОД, а не имя параметра:
		 *          описание POSIX отводит ему значение «поток ввода», и наречия
		 *          `cat -`, `tar -xf -`, `grep образец -` стоят на нём. Прежде оно
		 *          попадало в общую ветвь доводов из одних лишь тире и отвечалось
		 *          отказом `EMPTY_KEY` - приложение, такое обращение принимающее,
		 *          отвергало его вовсе. Замерено 15.09.2026 аудитом
		 *
		 * @note Двойное же тире отказом отвечается по-прежнему при снятой настройке
		 *       `terminus`: у него значения позиционного довода нет ни у одного
		 *       наречия, и пустым именем оно и является
		 */
		if(item.length() == 1){
			// Устанавливаем вид лексемы позиционным доводом
			lexeme.type = token_t::OPERAND;
		...
		/**
		 * Запоминаем число ведущих тире смещением имени внутри довода
		 *
		 * @warning Смещение это - ЕДИНСТВЕННЫЙ след того, одним тире подан параметр
		 *          или двумя: имя лексема несёт уже без тире. Прежде смещение стояло
		 *          нулём у всякой лексемы, и записи «-awh» с «--awh» становились
		 *          неразличимы вовсе - потребитель, пожелавший подать именно длинное
		 *          имя, сказать того не мог никак. Замерено 16.09.2026 аудитом
		 */
		lexeme.location = location_t(i, dashes);
