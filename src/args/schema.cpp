...
bool awh::args::Schema::add(const param_t & param) noexcept {
	...
	/**
	 * Если короткое имя подано, а знаком буквенно-цифровым не является
	 *
	 * @warning Указание 3 договора POSIX о наречии программ зовёт коротким именем
	 *          один знак буквенно-цифровой, и спрос этот стоит тому порукой: знак
	 *          «=» разбор наречия отрезает значением и до имени не доносит, «-»
	 *          сливается с двойным тире и одиночным доводом, пробел и перевод
	 *          строки оболочка до программы не доводит вовсе. Прежде описание такие
	 *          имена принимало и выводило их справкой, обещая наречие, подать
	 *          которое нельзя. Замерено 15.09.2026 аудитом
	 */
	if((param.letter != 0) && !(
		((param.letter >= '0') && (param.letter <= '9')) ||
		((param.letter >= 'A') && (param.letter <= 'Z')) ||
		((param.letter >= 'a') && (param.letter <= 'z'))
	)){
		// Выводим в лог сообщение о негодном коротком имени
		awh::log::print("Schema: short name is not alphanumeric", awh::log::flag_t::WARNING);
	...
	if(i != this->_names.end()){
		/**
		 * Если короткое имя поданного описания занято ИНЫМ описанием
		 *
		 * @warning Спрос этот стоит ДО всякой правки и обязателен: без него замена
		 *          описания с занятым коротким именем принималась успехом, розыск
		 *          по знаку вёл к ЧУЖОМУ параметру, а прежнее короткое имя
		 *          заменяемого пропадало вовсе - описание при этом уверяло, что
		 *          знак у него есть. Замерено 08.09.2026 аудитом: `-h` вело к
		 *          `host`, тогда как `port` объявлял тот же знак своим
		 */
		auto letter = this->_letters.find(param.letter);
