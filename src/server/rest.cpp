...
awh::server::Rest::Rest(const server::core_t * core, const fmk_t * fmk, const log_t * log) noexcept :
 _pid(getpid()), _port(SERVER_PORT), _host(""), _worker(fmk, log),
 _sid(AWH_SHORT_NAME), _ver(AWH_VERSION), _name(AWH_NAME), _realm(""),
 _opaque(""), _pass(""), _salt(""), _cipher(hash_t::cipher_t::AES128),
 ...
 _threadsCount(0), _threadsEnabled(false), _fmk(fmk), _log(log), _core(core) {
	...
	this->_worker.callback.open = std::bind(&awh::server::Rest::openCallback, this, _1, _2);
	// Устанавливаем функцию персистентного вызова
	this->_worker.callback.persist = std::bind(&awh::server::Rest::persistCallback, this, _1, _2, _3);
	// Устанавливаем событие подключения
	this->_worker.callback.connect = std::bind(&awh::server::Rest::connectCallback, this, _1, _2, _3);
	// Устанавливаем функцию чтения данных
	this->_worker.callback.read = std::bind(&awh::server::Rest::readCallback, this, _1, _2, _3, _4, _5);
	// Устанавливаем функцию записи данных
	this->_worker.callback.write = std::bind(&awh::server::Rest::writeCallback, this, _1, _2, _3, _4, _5);
	// Устанавливаем событие отключения
	this->_worker.callback.disconnect = std::bind(&awh::server::Rest::disconnectCallback, this, _1, _2, _3);
	// Добавляем событие аццепта адъютанта
	this->_worker.callback.accept = std::bind(&awh::server::Rest::acceptCallback, this, _1, _2, _3, _4, _5);
	// Добавляем событие на получение сообщений сервера
	this->_worker.callback.mess = std::bind(&awh::server::Rest::messageCallback, this, _1, _2, _3, _4, _5, _6);
	...
	if(this->core->working()){
		...
		if(adj != nullptr){
			...
			if(!this->_alive && !adj->_alive && adj->http.isAlive())
				...
				adj->http.addHeader("Keep-Alive", this->fmk->format("timeout=%d, max=%d", this->_timeAlive / 1000, this->_maxRequests));
			// Формируем запрос авторизации
