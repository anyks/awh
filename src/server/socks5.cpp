...
void awh::server::ProxySocks5::connectServerCallback(const size_t aid, const size_t sid, awh::core_t * core) noexcept {
	...
	if((aid > 0) && (sid > 0) && (core != nullptr)){
		...
		if(adj != nullptr){
			...
			adj->scheme.callback.read = std::bind(&ProxySocks5::readClientCallback, this, _1, _2, _3, _4, _5);
			// Устанавливаем событие подключения
			adj->scheme.callback.connect = std::bind(&ProxySocks5::connectClientCallback, this, _1, _2, _3);
			// Устанавливаем событие отключения
			adj->scheme.callback.disconnect = std::bind(&ProxySocks5::disconnectClientCallback, this, _1, _2, _3);
...
awh::server::ProxySocks5::ProxySocks5(const fmk_t * fmk, const log_t * log) noexcept : _port(SERVER_PORT), _host(""), _usock(""), _nwk(fmk), _core(fmk, log), _scheme(fmk, log), _fmk(fmk), _log(log) {
	...
	this->_scheme.callback.open = std::bind(&ProxySocks5::openServerCallback, this, _1, _2);
	// Устанавливаем событие подключения
	this->_scheme.callback.connect = std::bind(&ProxySocks5::connectServerCallback, this, _1, _2, _3);
	// Устанавливаем функцию чтения данных
	this->_scheme.callback.read = std::bind(&ProxySocks5::readServerCallback, this, _1, _2, _3, _4, _5);
	// Устанавливаем функцию записи данных
	this->_scheme.callback.write = std::bind(&ProxySocks5::writeServerCallback, this, _1, _2, _3, _4, _5);
	// Добавляем событие аццепта адъютанта
	this->_scheme.callback.accept = std::bind(&ProxySocks5::acceptServerCallback, this, _1, _2, _3, _4, _5);
	// Устанавливаем событие отключения
	this->_scheme.callback.disconnect = std::bind(&ProxySocks5::disconnectServerCallback, this, _1, _2, _3);
	// Добавляем схему сети в сетевое ядро
