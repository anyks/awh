	...
	if((bid > 0) && (sid > 0)){
		...
		if(options != nullptr){
			...
			if(!this->_callback.is("chunking"))
				...
				options->http.on <void (const uint64_t, const vector <char> &, const awh::http_t *)> ("chunking", &http1_t::chunking, this, _1, _2, _3);
			// Устанавливаем функцию обработки вызова для получения чанков
		...
		if(process){
			...
			// Иначе выполняем обработку входящих данных как Web-сервер
				...
				if(options != nullptr){
					...
					// Запоминаем брокера, запросы которого разбирались ранее
					const uint64_t busy = this->_busy;
					// Устанавливаем брокера, запросы которого разбираются
					this->_busy = bid;
					// Выполняем разбор накопленных данных брокера
					this->process(bid, sid);
					// Восстанавливаем брокера, запросы которого разбирались ранее
					this->_busy = busy;
				}
			}
		}
 ...
 * @param bid идентификатор брокера
 */
void awh::server::Http1::resume(const uint64_t bid) noexcept {
	// Снимаем брокера с ожидания продолжения разбора
	this->_resumes.erase(bid);
	// Если сетевое ядро установлено и брокер ещё подключён
	if((this->_core != nullptr) && this->_core->working()){
		...
		if((i == this->_agents.end()) || (i->second == agent_t::HTTP)){
			// Получаем параметры активного клиента
			scheme::web_t::options_t * options = const_cast <scheme::web_t::options_t *> (this->_scheme.get(bid));
			// Если в буфере остались следующие запросы конвейера, а подключение не закрывается
			if((options != nullptr) && !options->close && !options->stopped && !options->buffer.empty()){
				...
				// Если идентификатор схемы сети получен
				if(sid > 0){
					// Запоминаем брокера, запросы которого разбирались ранее
					const uint64_t busy = this->_busy;
					// Устанавливаем брокера, запросы которого разбираются
...
void awh::server::Http1::core(const server::core_t * core) noexcept {
	...
	if(core != nullptr){
		...
		const_cast <server::core_t *> (this->_core)->on <void (const uint64_t, const uint16_t)> ("connect", &http1_t::connectEvents, this, _1, _2);
		// Устанавливаем событие отключения
		const_cast <server::core_t *> (this->_core)->on <void (const uint64_t, const uint16_t)> ("disconnect", &http1_t::disconnectEvents, this, _1, _2);
		// Устанавливаем функцию чтения данных
		const_cast <server::core_t *> (this->_core)->on <void (const char *, const size_t, const uint64_t, const uint16_t)> ("read", &http1_t::readEvents, this, _1, _2, _3, _4);
		// Устанавливаем функцию записи данных
		const_cast <server::core_t *> (this->_core)->on <void (const char *, const size_t, const uint64_t, const uint16_t)> ("write", &http1_t::writeEvents, this, _1, _2, _3, _4);
		// Добавляем событие аццепта брокера
		const_cast <server::core_t *> (this->_core)->on <bool (const string &, const string &, const uint32_t, const uint64_t)> ("accept", &http1_t::acceptEvents, this, _1, _2, _3, _4);
...
awh::server::Http1::Http1(const fmk_t * fmk, const log_t * log) noexcept :
 ...
 _maxRequests(SERVER_MAX_REQUESTS), _identity(http_t::identity_t::HTTP), _ws1(fmk, log), _scheme(fmk, log), _busy(0), _resumer(fmk, log) {
	// Отключаем вывод лишних сообщений таймера
	this->_resumer.verbose(false);
}
/**
 * @brief Конструктор
 *
