 ...
 */
void awh::server::Sample::openCallback(const size_t wid, awh::core_t * core) noexcept {
 ...
 * @param core объект биндинга TCP/IP
 */
void awh::server::Sample::persistCallback(const size_t aid, const size_t wid, awh::core_t * core) noexcept {
	// Если данные существуют
	if((aid > 0) && (wid > 0) && (core != nullptr)){
		// Получаем параметры подключения адъютанта
		sample_worker_t::coffer_t * adj = const_cast <sample_worker_t::coffer_t *> (this->_worker.get(aid));
 ...
 * @param core объект биндинга TCP/IP
 */
void awh::server::Sample::connectCallback(const size_t aid, const size_t wid, awh::core_t * core) noexcept {
	...
	if((aid > 0) && (wid > 0) && (core != nullptr)){
		...
		if(adj != nullptr){
			...
			adj->action = sample_worker_t::action_t::CONNECT;
 ...
 * @param core объект биндинга TCP/IP
 */
void awh::server::Sample::disconnectCallback(const size_t aid, const size_t wid, awh::core_t * core) noexcept {
	// Если данные переданы верные
	if((aid > 0) && (wid > 0) && (core != nullptr)){
		// Получаем параметры подключения адъютанта
		sample_worker_t::coffer_t * adj = const_cast <sample_worker_t::coffer_t *> (this->_worker.get(aid));
		...
		if(adj != nullptr){
			// Устанавливаем экшен выполнения
			adj->action = sample_worker_t::action_t::DISCONNECT;
			// Выполняем запуск обработчика событий
			this->handler(aid);
		}
	}
 ...
 * @param core   объект биндинга TCP/IP
 */
void awh::server::Sample::readCallback(const char * buffer, const size_t size, const size_t aid, const size_t wid, awh::core_t * core) noexcept {
	// Если данные существуют
	if((buffer != nullptr) && (size > 0) && (aid > 0) && (wid > 0)){
		...
		if(adj != nullptr){
			...
			if((adj->action == sample_worker_t::action_t::NONE) || (adj->action == sample_worker_t::action_t::READ)){
				...
				if(adj->allow.receive){
					// Устанавливаем экшен выполнения
					adj->action = sample_worker_t::action_t::READ;
 ...
 * @param core   объект биндинга TCP/IP
 */
void awh::server::Sample::writeCallback(const char * buffer, const size_t size, const size_t aid, const size_t wid, awh::core_t * core) noexcept {
	// Если данные существуют
	if((aid > 0) && (wid > 0) && (core != nullptr)){
		// Получаем параметры подключения адъютанта
		sample_worker_t::coffer_t * adj = const_cast <sample_worker_t::coffer_t *> (this->_worker.get(aid));
 ...
 */
bool awh::server::Sample::acceptCallback(const string & ip, const string & mac, const u_int port, const size_t wid, awh::core_t * core) noexcept {
...
void awh::server::Sample::handler(const size_t aid) noexcept {
	...
	if(adj != nullptr){
		...
		if(!adj->locker.mode){
			...
			while(loop && (adj->action != sample_worker_t::action_t::NONE)){
				...
				switch((uint8_t) adj->action){
					...
					case (uint8_t) sample_worker_t::action_t::READ: this->actionRead(aid); break;
					// Если необходимо запустить экшен обработки подключения к серверу
					case (uint8_t) sample_worker_t::action_t::CONNECT: this->actionConnect(aid); break;
					// Если необходимо запустить экшен обработки отключения от сервера
					case (uint8_t) sample_worker_t::action_t::DISCONNECT: this->actionDisconnect(aid); break;
...
void awh::server::Sample::actionRead(const size_t aid) noexcept {
	...
	if(aid > 0){
		...
		if(adj != nullptr){
			...
			if(adj->action == sample_worker_t::action_t::READ)
				// Выполняем сброс экшена
				adj->action = sample_worker_t::action_t::NONE;
...
void awh::server::Sample::actionConnect(const size_t aid) noexcept {
	...
	if(aid > 0){
		...
		if(adj != nullptr){
			// Если экшен соответствует, выполняем его сброс
			if(adj->action == sample_worker_t::action_t::CONNECT)
				// Выполняем сброс экшена
				adj->action = sample_worker_t::action_t::NONE;
			// Если функция обратного вызова установлена
...
void awh::server::Sample::actionDisconnect(const size_t aid) noexcept {
	...
	if(aid > 0){
		...
		if(adj != nullptr){
			...
			// Если экшен соответствует, выполняем его сброс
			if(adj->action == sample_worker_t::action_t::DISCONNECT)
				// Выполняем сброс экшена
				adj->action = sample_worker_t::action_t::NONE;
 ...
 */
void awh::server::Sample::bytesDetect(const worker_t::mark_t read, const worker_t::mark_t write) noexcept {
...
awh::server::Sample::Sample(const server::core_t * core, const fmk_t * fmk, const log_t * log) noexcept :
 _pid(getpid()), _port(SERVER_PORT), _host(""), _nwk(fmk), _uri(fmk, &_nwk), _worker(fmk, log),
 _cipher(hash_t::cipher_t::AES128), _alive(false), _fmk(fmk), _log(log), _core(core) {
	...
	this->_worker.callback.persist = std::bind(&Sample::persistCallback, this, _1, _2, _3);
	...
	this->_worker.callback.write = std::bind(&Sample::writeCallback, this, _1, _2, _3, _4, _5);
	...
	this->_worker.callback.accept = std::bind(&Sample::acceptCallback, this, _1, _2, _3, _4, _5);
