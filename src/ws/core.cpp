	...
	try {
		...
		if(!this->is(suite_t::HEADER, "Sec-WebSocket-Extensions")){
			...
			if(this->_encryption)
				...
				extensions.push_back({this->_fmk->format("permessage-encrypt=%u", static_cast <uint16_t> (this->_cipher))});
...
const string awh::WCore::sha1() const noexcept {
	...
	if(!this->_key.empty()){
		...
		try {
			...
			const string & key = string(reinterpret_cast <const char *> (digest), sizeof(digest));
		 ...
		 */
			...
			// Выводим сообщение об ошибке
			this->_log->print("WebSocket sha1: %s", log_t::flag_t::WARNING, error.what());
			// Если функция обратного вызова на на вывод ошибок установлена
			if(this->_callbacks.is("error"))
				// Выполняем функцию обратного вызова
				this->_callbacks.call <void (const uint64_t, const log_t::flag_t, const http::error_t, const string &)> ("error", this->_web.id(), log_t::flag_t::CRITICAL, http::error_t::PROTOCOL, error.what());
...
bool awh::WCore::extractExtension(const string & extension) noexcept {
	...
	if(!extension.empty()){
		 ...
		 */
			...
			// Выводим сообщение об ошибке
			this->_log->print("WebSocket extractExtension: %s", log_t::flag_t::WARNING, error.what());
			// Если функция обратного вызова на на вывод ошибок установлена
			if(this->_callbacks.is("error"))
				// Выполняем функцию обратного вызова
				this->_callbacks.call <void (const uint64_t, const log_t::flag_t, const http::error_t, const string &)> ("error", this->_web.id(), log_t::flag_t::CRITICAL, http::error_t::PROTOCOL, error.what());
...
vector <char> awh::WCore::dump() const noexcept {
	 ...
	 */
		...
		// Выводим сообщение об ошибке
		this->_log->print("WebSocket dump: %s", log_t::flag_t::WARNING, error.what());
		// Если функция обратного вызова на на вывод ошибок установлена
		if(this->_callbacks.is("error"))
			// Выполняем функцию обратного вызова
			this->_callbacks.call <void (const uint64_t, const log_t::flag_t, const http::error_t, const string &)> ("error", this->_web.id(), log_t::flag_t::CRITICAL, http::error_t::PROTOCOL, error.what());
...
void awh::WCore::clean() noexcept {
	 ...
	 */
		...
		// Выводим сообщение об ошибке
		this->_log->print("WebSocket clear: %s", log_t::flag_t::WARNING, error.what());
		// Если функция обратного вызова на на вывод ошибок установлена
		if(this->_callbacks.is("error"))
			// Выполняем функцию обратного вызова
			this->_callbacks.call <void (const uint64_t, const log_t::flag_t, const http::error_t, const string &)> ("error", this->_web.id(), log_t::flag_t::CRITICAL, http::error_t::PROTOCOL, error.what());
...
void awh::WCore::compressors(const vector <compressor_t> & compressors) noexcept {
	...
	if(!compressors.empty()){
		 ...
		 */
			...
			// Выводим сообщение об ошибке
			this->_log->print("WebSocket compressors: %s", log_t::flag_t::WARNING, error.what());
			// Если функция обратного вызова на на вывод ошибок установлена
			if(this->_callbacks.is("error"))
				// Выполняем функцию обратного вызова
				this->_callbacks.call <void (const uint64_t, const log_t::flag_t, const http::error_t, const string &)> ("error", this->_web.id(), log_t::flag_t::CRITICAL, http::error_t::PROTOCOL, error.what());
...
bool awh::WCore::check(const flag_t flag) noexcept {
	 ...
	 */
		...
		// Выводим сообщение об ошибке
		this->_log->print("WebSocket check: %s", log_t::flag_t::WARNING, error.what());
		// Если функция обратного вызова на на вывод ошибок установлена
		if(this->_callbacks.is("error"))
			// Выполняем функцию обратного вызова
			this->_callbacks.call <void (const uint64_t, const log_t::flag_t, const http::error_t, const string &)> ("error", this->_web.id(), log_t::flag_t::CRITICAL, http::error_t::PROTOCOL, error.what());
...
vector <char> awh::WCore::process(const process_t flag, const web_t::provider_t & provider) const noexcept {
	...
	try {
		...
		switch(static_cast <uint8_t> (flag)){
			...
			case static_cast <uint8_t> (process_t::REQUEST): {
				...
				if(!req.url.empty() && (req.version >= 2.f ? req.method == web_t::method_t::CONNECT : req.method == web_t::method_t::GET)){
			...
			case static_cast <uint8_t> (process_t::RESPONSE): {
				...
				// Если параметры запроса получены
				if(res.version >= 2.f ? res.code != 101 : res.code != 200){
					...
					// Если версия протокола ниже 2.0
					if((res.version < 2.f) && (res.code == 101)){
	 ...
	 */
		...
		// Выводим сообщение об ошибке
		this->_log->print("WebSocket process: %s", log_t::flag_t::WARNING, error.what());
		// Если функция обратного вызова на на вывод ошибок установлена
		if(this->_callbacks.is("error"))
			// Выполняем функцию обратного вызова
			this->_callbacks.call <void (const uint64_t, const log_t::flag_t, const http::error_t, const string &)> ("error", this->_web.id(), log_t::flag_t::CRITICAL, http::error_t::PROTOCOL, error.what());
...
vector <std::pair <string, string>> awh::WCore::process2(const process_t flag, const web_t::provider_t & provider) const noexcept {
	 ...
	 */
		...
		// Выводим сообщение об ошибке
		this->_log->print("WebSocket process2: %s", log_t::flag_t::WARNING, error.what());
		// Если функция обратного вызова на на вывод ошибок установлена
		if(this->_callbacks.is("error"))
			// Выполняем функцию обратного вызова
			this->_callbacks.call <void (const uint64_t, const log_t::flag_t, const http::error_t, const string &)> ("error", this->_web.id(), log_t::flag_t::CRITICAL, http::error_t::PROTOCOL, error.what());
