...
bool awh::unit::Portmap::search(string_view group) noexcept {
	...
	try {
		...
		/**
		 * Обращаем срок ожидания ответа в предел простоя
		 *
		 * @details Движок держит этот срок ожиданием ответа: отправили - пошёл отсчёт,
		 *          прочли - отсчёт снят, и заводится он заново лишь следующей отправкой.
		 *          Рассылка же ведётся на групповой адрес, и приходят по ней ответы
		 *          посторонних устройств: непригодный ответ отбрасывается, а отправлять
		 *          повторно на этом шаге нечего - срок был бы снят чужой дейтаграммой, и
		 *          обмен остался бы без итога вовсе
		 *
		 * @note Пределом простоя срок ведёт себя должным образом: чужой ответ его лишь
		 *       отодвигает, а молчание по-прежнему завершает обмен
		 */
		this->_io->setUsageReadTimeout(this->_exchangeUPNP.eid, event::usage_t::REUSABLE);
...
bool awh::unit::Portmap::stream(string_view address, const uint16_t port) noexcept {
	...
	try {
		 ...
		 * Обращаем срок ожидания ответа в предел простоя
		 *
		 * @note Ответ устройства приходит кусками, и снятый первым же куском срок
		 *       оставил бы недочитанный ответ без предела вовсе: устройство, оборвавшее
		 *       передачу посередине, держало бы обмен молча и без конца
		 */
		this->_io->setUsageReadTimeout(this->_stream, event::usage_t::REUSABLE);
		/**
...
bool awh::unit::Portmap::datagram(const type_t type) noexcept {
	...
	try {
		 ...
		 * Обращаем срок ожидания ответа в предел простоя
		 *
		 * @details Ответ приходит на общий порт договора, и прийти он может на чужую
		 *          просьбу: такой ответ отбрасывается сличением с отправленной просьбой.
		 *          Держи движок этот срок ожиданием ответа, чужая дейтаграмма снимала бы
		 *          его насовсем - отправлять повторно на этом шаге нечего, и обмен
		 *          остался бы без итога вовсе
		 *
		 * @note Пределом простоя срок ведёт себя должным образом: чужой ответ его лишь
		 *       отодвигает, а молчание по-прежнему приводит к повтору просьбы
		 */
		this->_io->setUsageReadTimeout(exchange.eid, event::usage_t::REUSABLE);
		// Устанавливаем функцию обратного вызова на событие получения ошибок
