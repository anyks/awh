...
bool awh::regex::Emitter::preserved(const reg_t reg) noexcept {
	 ...
	 *
	 *          Исключение одно: регистр итога. Итог подпрограмма выдаёт первым
	 *          регистром области доводов, а порождение вызова переносит его
	 *          в регистр итога договора - тот же самый, каким приходит адрес
	 *          начала текста. Оттого текст объявляется затираемым и сохраняется
	 *          в кадре, как и на прочих наборах команд. Изъян этот вскрыт
	 *          исполнением: сопоставитель читал байт по нулевому адресу текста
	 *          сразу после первого же вызова подпрограммы обстановки.
	 *
	 */
	#elif defined(__e2k__)
		// Выводим сохранность регистра при вызове подпрограммы
		return (reg != reg_t::TEXT);
	/**
