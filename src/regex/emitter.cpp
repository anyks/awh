...
bool awh::regex::Emitter::preserved(const reg_t reg) noexcept {
	 ...
	 *
	 * @details Окно регистров вызываемого начинается ЗА регистрами договора:
	 *          вызов сдвигает окно на область доводов, лежащую хвостом окна
	 *          сопоставителя, и всякий регистр ниже неё переживает вызов сам -
	 *          заботою самого процессора, а не порождённого кода.
	 *
	 *          Исключение - регистры доводов, с нулевого по третий. Сохранность
	 *          при вызове и сохранность при ПОДГОТОВКЕ вызова - разные вещи:
	 *          подготовка сама укладывает доводы подпрограммы в эти регистры
	 *          («context» в границы, «move» в позицию), а итог вызова ложится
	 *          в регистр текста, он же регистр итога договора. Окно бережёт их
	 *          от вызываемого, но не от порождающего, оттого они объявляются
	 *          затираемыми и откладываются в кадр, как и на прочих наборах
	 *          команд. Изъян этот вскрыт исполнением дважды: сперва
	 *          сопоставитель читал байт по нулевому адресу текста сразу после
	 *          первого же вызова, а после - писал по адресу мусорному, приняв
	 *          за адрес границ значение обстановки, доводом переданное.
	 *
	 *          Регистр обстановки (четвёртый) подготовкой не затирается и окном
	 *          оберегается - его сохранение выпадает, и в том выигрыш окон.
	 *
	 */
	#elif defined(__e2k__)
		// Выводим сохранность регистра при вызове подпрограммы
		return ((reg != reg_t::TEXT) && (reg != reg_t::SIZE) &&
		 (reg != reg_t::START) && (reg != reg_t::BOUNDS));
	/**
...
void awh::regex::Emitter::place(const size_t label) noexcept {
		 ...
		 *
		 * @details Метка обозначает место, куда управление приходит со стороны,
		 *          и стоять она обязана на границе широкой команды: иначе
		 *          операция, к команде прирощенная, оказалась бы исполненной
		 *          и на пути, метку эту достигающем
		 *
		 */
		this->flush();
	#endif
...
#endif // !defined(__aarch64__) && !defined(_M_ARM64) && !defined(__x86_64__) && !defined(_M_X64) && !defined(__e2k__)
