...
bool awh::regex::Emitter::available() noexcept {
	/**
	 * Если сборка выполняется для набора команд x86-64 под Windows
	 *
	 * @details Отображение регистров набора x86-64 отвечает соглашению System V:
	 *          доводы вызова приходят в rdi, rsi, rdx, rcx и r8, а сохранность
	 *          rbx, r12 и r13 возложена на сторону вызываемую. Windows же
	 *          держится соглашения своего: доводы приходят в rcx, rdx, r8 и r9,
	 *          довод пятый передаётся кадром вызова, оберегаемыми вызываемым
	 *          числятся сверх того rsi и rdi, а всякий вызов подпрограммы
	 *          требует отвести под доводы тридцать два байта кадра прежде себя.
	 *          Порождённый код, соглашения этого не ведающий, читает доводы
	 *          не из тех регистров и затирает оберегаемые: проба на машине
	 *          Windows x86-64 выдала вместо размера текста 13 значение
	 *          468275164784 - содержимое rsi, к доводам отношения не имеющее.
	 *
	 *          Поэтому порождение под Windows x86-64 объявляется недоступным,
	 *          и выражения сопоставляются исполнением программы, как и на
	 *          всяком наборе команд, порождения не получившем. Отказ этот
	 *          снимается заведением отображения регистров под соглашение
	 *          Windows, а не ослаблением условия.
	 *
	 *          Условие сложено из набора команд и признака «_WIN32»: признак
	 *          этот означает не разрядность, а Windows API, и объявляется
	 *          и на тридцати двух разрядах, и на шестидесяти четырёх, тогда
	 *          как «_WIN64» объявляется вдобавок к нему. Разрядность задана
	 *          набором команд, отчего «_WIN64» условию ничего не прибавляет:
	 *          «_WIN32 && _WIN64» есть в точности «_WIN64», а с проверкой
	 *          набора команд - в точности «_WIN32».
	 *
	 */
