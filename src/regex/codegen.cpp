	...
	while(true) {
		...
		if((instruction.type == opcode_t::SPLIT) && (instruction.split.run == INVALID_ADDRESS) &&
		 (instruction.split.lazy == INVALID_ADDRESS)) {
			 ...
			 *          отвергнута: она сохраняет ведение отказа переходом
			 *          по адресу из памяти, а именно оно и платит. Строка
			 *          «\\((?:[^()]|\\([^()]*\\))*\\)» с нею дала 14.63 миллиона
			 *          совпадений в секунду против 14.63 без неё, тогда как
			 *          разбор меткой на цепочке прямой даёт 22.67 против
			 *          14.59 у перебора.
			 *
			 */
