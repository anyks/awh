...
namespace {
	 ...
	 *
	 * @details Место это порождённым кодом НЕ зовётся: проверка возможности
	 *          совпадения выполняется стороною C++ до передачи управления
	 *          сопоставителю - см. «Codegen::exec». Оставлено оно намеренно,
	 *          заделом под перенос проверки в порождённый код, и заполняется
	 *          наравне с прочими, дабы обстановка порождения и восстановления
	 *          не расходились. Номера мест вписаны в самый порождённый код
	 *          смещениями, отчего изъятие места обесценило бы всякую запись,
	 *          прежде сохранённую.
	 *
	 */
	...
	/**
	 * @brief Признак просеивания широкого набора окном с отбором подпрограммой
	 *
	 * @details У процессора e2k порождённый ход по тексту на порядок дороже
	 *          хода, собранного LCC: команды порождения идут по одной на широкую
	 *          команду и ждут друг друга, тогда как LCC прячет задержки
	 *          конвейеризацией цикла. Замер поиска байта на мегабайте текста
	 *          на Эльбрусе-8С2: порождённый цикл - 17.6 такта на байт, он же
	 *          с подготовкой переходов до цикла - 14.6, цикл LCC - 1.6.
	 *
	 *          Просеивание широкого набора ходит по тексту побайтно в самом
	 *          коде, и на e2k оно проверяет в коде окно в WIDE_SIFTING позиций,
	 *          а участок без кандидатов за окном пропускает подпрограмма отбора.
	 *          Прочим процессорам порождённый ход не дороже собранного, и
	 *          подпрограмма добавила бы им лишь плату за вызов. Смотрите
	 *          «Намеренные решения» в заголовке кодогенерации.
	 *
	 */
	#if defined(__e2k__)
		constexpr bool WIDE_SEEKING = true;
	#else
		constexpr bool WIDE_SEEKING = false;
	#endif

	/**
	 * @brief Размер окна просеивания широкого набора на месте
	 *
	 * @details Окно короче отдаёт подпрограмме текст, где кандидат - почти
	 *          всякая позиция, и вызов платится на всякую попытку: без окна
	 *          вовсе «(?<=@)\w+» терял сорок два процента, с окном в одну
	 *          позицию «(\w+)@(\w+)\.(\w+)» терял двадцать. Окно длиннее
	 *          проходит порождённым ходом участок, какой подпрограмма прошла бы
	 *          вдесятеро дешевле: «[0-9]{3,5}» по длинному тексту набора
	 *          выигрывал с окном в одну позицию 164 процента, а в восемь - 112.
	 *          Окно в четыре позиции отнимало у «(?:[a-z]+/)+v1» пять процентов
	 *          во всяком круге, окно в восемь не отняло ни у одного сценария
	 *          набора больше 0.7 процента.
	 *
	 */
	constexpr size_t WIDE_SIFTING = 8;
	...
	void invoke(awh::regex::Emitter & emitter, const size_t slot, const size_t spill, const awh::regex::Emitter::reg_t pos, const size_t argument) noexcept {
		...
		/**
		 * Сличаем количество сохраняемых регистров с размером области кадра
		 *
		 * @details Размер области сохранения задан «SPILLS» отдельно от набора,
		 *          и связи между ними не было никакой: регистр, к набору
		 *          добавленный, писался бы за кадр вызова молча. Сличение
		 *          обращает такое расхождение отказом сборки.
		 *
		 */
		static_assert((sizeof(SPILLED) / sizeof(SPILLED[0])) == SPILLS, "Область сохранения регистров расходится с набором сохраняемых");
		/**
		...
		while(true) {
			...
			switch(static_cast <uint8_t> (instruction.type)) {
				...
				case static_cast <uint8_t> (awh::regex::opcode_t::LOOK): {
					 ...
					 *
					 * @details Порождаемый код держит точки возврата тела проверки
					 *          в пределах тела: продолжение за проверкой их
					 *          не достигает. Проверка не отсекающая требует
					 *          обратного, отчего порождению не поддаётся.
					 *
					 */
...
bool awh::regex::Codegen::compile(const program_t & program) noexcept {
	...
	/**
	 * Получаем признак просеивания широкого набора окном с отбором подпрограммой
	 *
	 * @details Смотрите «WIDE_SEEKING»: окно у такого просеивания в WIDE_SIFTING
	 *          позиций, а подпрограмма - общий отбор по таблице, а не поиск
	 *          значений набора: поиск сличает за оборот не более SPARSE значений.
	 *
	 */
	const bool wide = (WIDE_SEEKING && (allowed > SPARSE));
	/**
	...
	if(sifting && ((allowed <= SPARSE) || wide)) {
		...
		const size_t number = (wide ? 0 : this->spreader());
		...
		/**
		 * Если набор широк и отбирается подпрограммой общей
		 */
		if(wide)
			// Выполняем вызов подпрограммы отбора позиции начала попытки
		...
		else invoke(emitter, SLOT_SCATTERING, spill, reg_t::KEEPER, number);
		// Выполняем установку отобранной позиции начала попытки
		emitter.move(reg_t::KEEPER, reg_t::SCRATCH);
		// Выполняем сравнение позиции начала попытки с размером текста
		emitter.compare(reg_t::KEEPER, reg_t::SIZE);
	...
	while(true) {
		...
		if(singular(instruction.type)) {
			...
			/**
			 * Получаем количество копий ряда одинаковых инструкций
			 *
			 * @details Счётное повторение одиночного символа компилируется копиями,
			 *          и ряд их помечен длиною на всякой копии. Копия порождалась
			 *          отдельным куском - сверкой с концом текста, чтением адреса
			 *          таблицы, чтением байта, сличением да продвижением, - восемью
			 *          командами на копию. Ряд же проверяет конец текста однажды
			 *          на всю свою длину и адрес таблицы читает однажды, и копии
			 *          остаётся пять команд, а литералу - четыре.
			 *
			 *          Пометке порождение не доверяет, а берёт её верхней оценкой
			 *          и сверяет копии сами: программа приходит к порождению
			 *          и мимо поверки записи - проверка «CodegenMutated» подаёт
			 *          её испорченной, - а пометка, копиям не отвечающая,
			 *          заставила бы код перешагнуть инструкции иные. Ряд не
			 *          переходит и границы области: копия за нею принадлежит
			 *          области иной, порождаемой своим чередом.
			 *
			 */
			size_t copies = 1;
			{
				// Получаем количество инструкций от разбираемой до конца программы
				size_t limit = (program.instructions.size() - static_cast <size_t> (pc));
				/**
				 * Если область инструкций кончается прежде конца программы
				 */
				if((to != INVALID_ADDRESS) && ((static_cast <size_t> (to) - static_cast <size_t> (pc)) < limit))
					// Выполняем ограничение ряда границей области
					limit = (static_cast <size_t> (to) - static_cast <size_t> (pc));
				/**
				 * Если пометка ряда короче остатка области
				 */
				if(static_cast <size_t> (instruction.repeat) < limit)
					// Выполняем ограничение ряда его пометкой
					limit = static_cast <size_t> (instruction.repeat);
				/**
				 * Выполняем сверку копий ряда с инструкцией разбираемой
				 */
				while((copies < limit) && identical(instruction, program.instructions.at(static_cast <size_t> (pc) + copies)))
					// Увеличиваем количество копий ряда
					copies++;
			}
			/**
			...
			if(copies > 1) {
				 ...
				 *
				 * @details Копии ряда обязательны все до единой, и отказ одной
				 *          есть отказ всего ряда: копия, за концом текста стоящая,
				 *          дала бы отказ тот же, что и сверка эта. Позиция отказа
				 *          обработчику его безразлична - отказ и прежде приходил
				 *          с позиции любой копии.
				 *
				 */
				emitter.add(reg_t::SPARE, reg_t::CURSOR, static_cast <uint32_t> (copies));
				// Выполняем сравнение конца ряда с размером текста
				emitter.compare(reg_t::SPARE, reg_t::SIZE);
				// Выполняем переход к отказу при выходе ряда за конец текста
				emitter.branch(cond_t::GREATER, failure);
				/**
				 * Если сопоставление ведётся таблицей принадлежности байтов
				 */
				if(!literal)
					// Выполняем чтение адреса таблицы принадлежности байтов однажды
					emitter.context(reg_t::SCRATCH, static_cast <uint32_t> (number));
				/**
				...
				for(size_t copy = 0; copy < copies; copy++) {
					 ...
					 *
					 * @details Готов бывает лишь байт копии первой: его прочёл
					 *          разбор цепочки ветвей по первому байту.
					 *
					 */
					if((copy > 0) || !prefetch)
						// Выполняем чтение байта текста в позиции сопоставления
				...
				pc += static_cast <address_t> (copies);
				// Продолжаем обход инструкций программы
				continue;
			}
			/**
		 ...
		 *          окружения атомарными и полагающему; проверка же не отсекающая
		 *          - «(?*...)» и «(?<*...)» - обратного и требует, отчего
...
namespace {
	 ...
	 *
	 */
	uint8_t instructionSet() noexcept {
		/**
		 ...
		 */
			...
			// Выводим опознание набора команд e2k
			return 0x03;
		/**
...
namespace {
	/**
	 * @brief Функция вычисления приметы порождённого машинного кода
	 *
	 * @param code   адрес порождённого машинного кода
	 * @param length размер порождённого машинного кода в байтах
	 * @return       вычисленная примета порождённого машинного кода
	 *
	 * @details Примета берётся с ТЕХ САМЫХ байтов, каким предстоит исполниться:
	 *          она снимается с записи при сохранении и с участка исполняемой
	 *          памяти при восстановлении, уже после наполнения его и ДО
	 *          разрешения исполнения. Проверка записи взамен участка оставляла
	 *          бы щель между сличением и переносом.
	 *
	 */
	uint64_t imprint(const void * code, const size_t length) noexcept {
		// Накопленная примета порождённого машинного кода
		uint64_t result = 0xCBF29CE484222325ull;
		// Получаем обзор порождённого машинного кода
		...
		// Выводим вычисленную примету порождённого машинного кода
		return result;
	}
};
namespace {
	 ...
	 *
	 */
	void writeSize(uint64_t value, std::string & result) noexcept {
		/**
...
namespace {
	 ...
	 *
	 */
	bool readSize(std::string_view data, size_t & offset, uint64_t & value) noexcept {
...
bool awh::regex::Codegen::save(string & result) const noexcept {
	...
	/**
	 * Выполняем запись приметы порождённого машинного кода
	 *
	 * @details Примета сличается восстановлением с участком исполняемой памяти
	 *          уже наполненным, отчего порча кода в записи обращается отказом
	 *          порождения, а не исполнением испорченных команд.
	 *
	 */
	writeSize(imprint(this->_assembly.entry(), this->_assembly.length()), result);
...
bool awh::regex::Codegen::restore(string_view data, size_t & offset, const program_t & program) noexcept {
	...
	// Примета порождённого машинного кода, записью хранимая
	uint64_t mark = 0;
	/**
	 * Если чтение приметы порождённого кода не выполнено
	 */
	if(!readSize(data, offset, mark))
		// Выводим результат восстановления сопоставителя
		return false;
	...
	/**
	 * Если примета наполненного участка памяти записи не отвечает
	 *
	 * @details Примета снимается с участка исполняемой памяти уже наполненного
	 *          и ДО разрешения исполнения ему: сличение выполняется по тем
	 *          самым байтам, каким предстоит исполниться, отчего щели между
