...
bool awh::regex::Codegen::compile(const program_t & program) noexcept {
	 ...
	 *
	 *          Просеивание набором байтов отменяет отбор этот, а не наоборот.
	 *          Отбор ведётся вызовом подпрограммы, а вызов идёт на всякую
	 *          попытку отвергнутую: девять хранений регистров в кадре, вызов,
	 *          девять чтений обратно и поиск литерала заново. Просеивание же
	 *          стоит двух обращений к памяти прямо в порождённом коде.
	 *          Пока литерал редок, отбор окупается пропуском участков; едва
	 *          он част - плата вызова берёт своё. Замером на «(?:aa|bb|cc|dd) »
	 *          по тексту «adx » в тысячу шестьсот знаков: отбор по литералу
	 *          обходился в 5.76 наносекунды на знак, просеивание - в 0.58,
	 *          то есть вдесятеро дешевле; против толкователя это 0.20x
	 *          и 1.94x соответственно. Набор же байтов отсутствует лишь там,
	 *          где совпадение начинается всяким байтом, - там отбор
	 *          по литералу и остаётся единственным ходом.
	 *
	 */
