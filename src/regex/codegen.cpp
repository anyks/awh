	...
	while(true) {
		...
		if((instruction.type == opcode_t::SPLIT) && (instruction.split.run == INVALID_ADDRESS) &&
		 (instruction.split.lazy == INVALID_ADDRESS)) {
			 ...
			 *
			 *          Известное неустранённое: разбор проверяет конец текста
			 *          и читает байт сам, а ветвь, им выбранная, делает и то
			 *          и другое заново. Довесок этот постоянен на проход - замер
			 *          даёт 0.37 наносекунды при длине тела ветви от двух байтов
			 *          до шестнадцати, - и оттого он тем заметнее, чем тело
			 *          короче: отношение к телу без цепочки идёт 1.76 при двух
			 *          байтах и 1.08 при шестнадцати. Строки набора замеров,
			 *          от эталона отстающие, стоят на телах в один байт, то есть
			 *          на самом краю. Устранение довеска - слияние чтения байта
			 *          разбором с ведущей проверкой набора у ветви.
			 *
			 */
			if(labelled) {
