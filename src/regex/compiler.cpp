 ...
 *          а байты остатка инструкция несёт сама. Обход ведётся с конца:
 *          остаток всякой инструкции есть остаток следующей, её байтом
 *          дополненный спереди.
 *
 * @note Пометка развёрнутой формы НЕ отменяет: исполнение без возврата,
 *       детерминированное исполнение и порождённый код проходят инструкции
 *       по одной, пометки не замечая.
 *
 */
void awh::regex::Compiler::literals() noexcept {
	// Получаем количество инструкций программы регулярного выражения
	const size_t count = this->_program->instructions.size();
	/**
	 * Если программа регулярного выражения пуста
	...
	for(size_t i = count; i > 0; i--) {
		 ...
		 *
		 * @details Прочие коды операций в разбор символов исполнения с возвратом
		 *          не приходят, и пометки их не касается никто.
		 *
		 */
		if((instruction.type != opcode_t::CHAR) && (instruction.type != opcode_t::CLASS) &&
		 (instruction.type != opcode_t::ANY) && (instruction.type != opcode_t::CODEUNIT) &&
		 (instruction.type != opcode_t::GRAPHEME))
			// Переходим к инструкции предыдущей
			continue;
		/**
		 * Выполняем установку отсутствия литерала
		 *
		 * @details Исполнение с возвратом читает пометку у всякого символьного кода,
		 *          не сверяя кода операции, - смотрите «Намеренные решения»
		 *          в заголовке исполнения с возвратом, - и у кодов прочих она
		 *          обязана быть нулевой. Конструктор инструкции её обнуляет, но
		 *          сборка ставится здесь явно: инструкция, код операции какой
		 *          переписан на месте, несла бы в зазоре операнды прежние.
		 *
		 */
		instruction.letter.length = 0;
		/**
		 * Выполняем обнуление байтов литерала
		 *
		 * @details Запись хранилища несёт образ инструкций, и байты, пометкой
		 *          не занятые, обязаны быть одинаковыми у всякой сборки выражения.
		 *
		 */
		for(size_t j = 0; j < MAX_LITERAL; j++)
			// Выполняем обнуление очередного байта литерала
			instruction.letter.bytes[j] = 0;
		/**
		...
		instruction.letter.bytes[0] = static_cast <char> (instruction.letter.code);
		// Длина литерала, заимствуемого у инструкции следующей
		size_t rest = 0;
		/**
		 * Если за инструкцией следует инструкция, байт дословно сопоставляющая
		 *
		 * @details Обход идёт с конца, и пометка инструкции следующей уже
		 *          поставлена: литерал её заимствуется целиком, насколько
		 *          пометка вмещает.
		 *
		 */
		if(((index + 1) < count) && verbatim(instructions[index + 1]))
			// Получаем длину литерала, заимствуемого у инструкции следующей
			rest = min(static_cast <size_t> (instructions[index + 1].letter.length), (MAX_LITERAL - 1));
		/**
		 * Выполняем перенос байтов литерала инструкции следующей
		 */
		for(size_t j = 0; j < rest; j++)
			// Выполняем перенос очередного байта литерала
			instruction.letter.bytes[j + 1] = instructions[index + 1].letter.bytes[j];
		// Выполняем установку длины литерала
		instruction.letter.length = static_cast <uint8_t> (rest + 1);
	}
}
/**
...
bool awh::regex::Compiler::build(const Parser & parser, program_t & program) noexcept {
	...
	this->series();
	// Выполняем пометку литералов, байтами дословно сличаемых
	this->literals();
