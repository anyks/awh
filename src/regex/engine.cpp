...
bool awh::regex::Engine::exec(const expression_t & expression, string_view text, const size_t start, vector <pair <size_t, size_t>> & captures) noexcept {
	 ...
	 *
	 * @details Проход детерминированного исполнения устанавливает наличие
	 *          совпадения и позицию его завершения, а границы устанавливает
	 *          исполнение с возвратом, проходящее текст заново. Когда совпадение
	 *          лежит близко к началу поиска, первый проход не окупается: он
	 *          стоит столько же, сколько и второй, а отвечает на половину
	 *          вопроса. Оттого исполнение с возвратом испытывается ПЕРВЫМ,
	 *          на голове текста - тем же порядком, каким испытывается выражение
	 *          с ведущим литералом.
	 *
	 *          Голова эта задаётся ДВУМЯ пределами, и оба обязательны: предел
	 *          позиций начала попытки отсекает поиск совпадения, лежащего
	 *          далеко, а предел объёма работы - попытку единственную, весь
	 *          текст проходящую повторением (таково «\w+?@\w+?\.»: первая же
	 *          попытка ищет собаку до конца текста). Замером без второго предела
	 *          получено падение `lazy-long` с 4.03 до 0.40.
	 *
	 *          Выражение, к позиции начала поиска привязанное, порядку этому
	 *          не подлежит: детерминированное исполнение отвечает ему отказом
	 *          немедля - останов привязкою стоит одного шага, - тогда как
	 *          единственная попытка исполнения с возвратом перебирает длины
	 *          повторений до конца. Замером на «^(GET|POST) (\S+) HTTP/(\d)\.(\d)$»
	 *          без совпадения получено замедление вчетверо.
	 *
	 *          Выражение, текст единственной попыткой проходящее, пробе не
	 *          подлежит тоже: исполнение с возвратом ему уже дано выше, и проба
	 *          повторяла бы отказ, объёмом работы вызванный. Замером на
	 *          «.*needle» получено падение с 14.28 до 11.31.
	 *
	 */
	if(!expression.forward.anchored && !expression.forward.sweeping) {
		...
		if(this->_backtrack.exec(expression.forward, text, start, captures)) {
			 ...
			 *          проба, к отказу пришедшая, работы не снимает вовсе, и
			 *          учёт обращения не отличал бы её от пробы отменённой.
			 *
			 */
		 ...
		 *
		 * @details Исполнение с возвратом прошло голову текста целиком и
		 *          совпадения в ней не нашло, а голова эта - весь текст:
		 *          отказ окончателен и повторного прохода не требует.
		 *
		 */
		 ...
		 *
		 * @details Признак ставится лишь тогда, когда объём был ПОЛНЫМ, то есть
		 *          весь текст уместился в голову: объём урезанный об исходе
		 *          прохода полного не говорит ничего.
		 *
		 */
