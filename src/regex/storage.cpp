...
bool awh::regex::Storage::verify(const program_t & program) const noexcept {
	...
	for(size_t index = 0; index < program.instructions.size(); index++) {
		...
		switch(static_cast <uint8_t> (instruction.type)) {
			 ...
			 *
			 * @details Исполнение с возвратом читает пометку литерала у всякого
			 *          символьного кода, не сверяя кода операции, и пометка у кода
			 *          иного, нежели одиночный символ, велела бы ему сличить байты,
			 *          каких он не несёт: класс символов отказал бы там, где символ
			 *          ему принадлежит, - совпадение пропало бы молча.
			 *
			 */
