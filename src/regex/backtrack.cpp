...
/**
 * Признак подстановки прохода ряда телом цикла исполнения
 *
 * @details Проход ряда класса символов по таблице принадлежности ведётся
 *          методом «consume», и подставлять ли его телом цикла исполнения,
 *          выбрано замером по архитектуре и собирателю. У Clang на x86-64
 *          подстановка выигрывает и в сборке, где всякий блок выровнен по
 *          шестидесяти четырём байтам и раскладка из опыта изъята: двадцати
 *          трём строкам набора до 15 процентов, теряя у одной 2.1. У Clang
 *          на ARM64 та же подстановка отнимает у строк исполнения с возвратом
 *          от 6 до 10 процентов и в такой сборке; у LCC тесный цикл ряда
 *          внутри цикла исполнения идёт втрое медленнее. GCC метод подставляет
 *          сам, и запрет подстановки стоил бы ему до 4 процентов. Смотрите
 *          «Намеренные решения» в заголовке исполнения с возвратом.
 *
 *          Заведён здесь, а не в заголовке: читает его лишь исполнение
 *          с возвратом. Сборка переопределяет его, «-DAWH_REGEX_SERIES_INLINE=0»
 *          либо «=1»; значим он лишь для Clang - прочие собиратели подставляют
 *          либо не подставляют метод своей оценкой, и принуждение GCC дало бы
 *          предупреждение. Сборка ARM64EC у Visual Studio ставит признак x86-64
 *          тоже, а код её машинный - ARM64, отчего она исключена.
 *
 */
#if !defined(AWH_REGEX_SERIES_INLINE)
	/**
	 ...
	 */
		...
		/**
		 * Проход ряда подставляется телом цикла исполнения
		 */
		#define AWH_REGEX_SERIES_INLINE 1
	/**
	 * Если сборка ведётся под прочие архитектуры либо прочими собирателями
	 */
	#else
		/**
		 * Проход ряда остаётся методом, подстановку решает собиратель
		 */
		#define AWH_REGEX_SERIES_INLINE 0
	#endif
#endif
/**
 * Если проход ряда подставляется телом цикла исполнения собирателем Clang
 */
#if AWH_REGEX_SERIES_INLINE && defined(__clang__)
	/**
	 * Принудительная подстановка прохода ряда средствами Clang
	 */
	#define AWH_REGEX_SERIES __attribute__((always_inline))
/**
 * Если подстановку прохода ряда решает собиратель
 */
#else
	/**
	 * Проход ряда без признаков подстановки
	 */
	#define AWH_REGEX_SERIES
#endif

/**
 * Признак запрета подстановки метода сличения литерала
 *
 * @details Метод сличения литерала «literal» подстановке телом цикла
 *          исполнения запрещён всюду, кроме LCC, и выбрано это замером
 *          по собирателю. У Clang на x86-64 подставленный метод не выигрывает
 *          и в сборке, где всякий блок выровнен по шестидесяти четырём байтам;
 *          у Clang на ARM64 в такой сборке он выигрывает строкам литерала,
 *          но в обычной девятнадцать строк исполнения с возвратом теряют
 *          от 4 до 10 процентов; у GCC выигрыш одних строк уравновешен потерей
 *          других. У LCC, чей код от раскладки не зависит, подставленный метод
 *          выигрывает двадцати пяти строкам набора до 2.9 процента, а у девяти
 *          теряет не более 0.4.
 *          Смотрите «Намеренные решения» в заголовке исполнения с возвратом.
 *
 *          Сборка переопределяет признак, «-DAWH_REGEX_LITERAL_NOINLINE=0»
 *          либо «=1».
 *
 */
#if !defined(AWH_REGEX_LITERAL_NOINLINE)
	/**
	 * Если сборка ведётся собирателем МЦСТ
	 */
	#if defined(__LCC__)
		/**
		 * Подстановку метода сличения литерала решает собиратель
		 */
		#define AWH_REGEX_LITERAL_NOINLINE 0
	/**
	 * Если сборка ведётся прочими собирателями
	 */
	#else
		/**
		 * Метод сличения литерала подстановке запрещён
		 */
		#define AWH_REGEX_LITERAL_NOINLINE 1
	#endif
#endif
/**
 * Если метод сличения литерала подстановке запрещён
 */
#if AWH_REGEX_LITERAL_NOINLINE
	/**
	 * Запрет подстановки метода сличения литерала
	 */
	#define AWH_REGEX_LITERAL AWH_REGEX_NOINLINE
/**
 * Если подстановку метода сличения литерала решает собиратель
 */
#else
	/**
	 * Метод сличения литерала без признаков подстановки
	 */
	#define AWH_REGEX_LITERAL
#endif
 ...
 *
 * @note Подставлять ли метод телом цикла исполнения, решает признак
 *       «AWH_REGEX_SERIES_INLINE», и выбран он замером по собирателю: у Clang
 *       на x86-64 метод подставляется, у Clang на ARM64 и у LCC остаётся
 *       вызовом, а GCC подставляет его сам. Прежний довод - просадка
 *       литеральных сценариев на четверть от роста тела цикла - принадлежал
 *       раскладке прохода парой байтов в «prefilter.cpp», ныне закреплённой:
 *       смотрите «Намеренные решения» в заголовке исполнения с возвратом.
 *
 ...
 *
 */
AWH_REGEX_SERIES size_t awh::regex::Backtrack::consume(const instruction_t & instruction, const size_t from, const size_t size, const uint16_t series) noexcept {
 ...
 *
 * @note Метод отделён от цикла исполнения, и подстановка его телом цикла
 *       запрещена всюду, кроме LCC, признаком «AWH_REGEX_LITERAL_NOINLINE»:
 *       собиратель подставлял его, и на x86-64 это отнимало у восьми строк
 *       от 1.6 до 6.9 процента, тогда как метод отдельный - ни у одной.
 *       Перемер - при признаке и в «Намеренных решениях» заголовка исполнения
 *       с возвратом.
 *
 ...
 *
 */
AWH_REGEX_LITERAL size_t awh::regex::Backtrack::literal(const instruction_t & instruction, const size_t from, const size_t size) const noexcept {
 ...
 *
 * @note Метод отделён от цикла исполнения намеренно, и отделён он замером.
 *       Подставленный телом цикла, он не выигрывает цепочке ничего ни на одной
 *       машине, а теряет: у LCC «[0-9]{0,512}» идёт 6192 нс против 3763, и
 *       строки с цепочками теряют до девяти процентов - «address-short» 9.3,
 *       «digits-long» 6.4; у Clang на x86-64 строкам исполнения с возвратом
 *       он отдаёт большую часть выигрыша подстановки прохода ряда -
 *       «recurse-heavy» 2.6 процента взамен 13.3. Прежний довод - просадка
 *       литеральных сценариев на четверть - принадлежал не соседству по единице
 *       трансляции, а раскладке прохода парой байтов в «prefilter.cpp», ныне
 *       закреплённой: смотрите «Намеренные решения» в заголовке исполнения
 *       с возвратом.
 *
		 ...
		 */
			...
			switch(static_cast <uint8_t> (instruction.type)) {
				...
				case static_cast <uint8_t> (opcode_t::SPLIT): {
					 ...
					 *          пометки да перенос позиции - подставленный сюда проход
					 *          цепочки не выигрывает ни на одной машине.
					 *
				...
				default: {
					 ...
					 */
						 ...
						 *
						 * @details Проход ведётся методом, а подставлять ли его сюда телом,
						 *          решает признак «AWH_REGEX_SERIES_INLINE»: смотрите
						 *          примечание при «Backtrack::consume».
						 *
						 */
