		 ...
		 */
			...
			switch(static_cast <uint8_t> (instruction.type)) {
				...
				default: {
					 ...
					 *
					 * @details Пометка проверяется без кода операции намеренно: её несёт
					 *          лишь одиночный символ, у прочих кодов, сюда приходящих, она
					 *          нулевая - так её ставит сборка и так требует поверка записи.
					 *          Проверка кода операции здесь сворачивалась собирателем
					 *          в самый разбор кода: таблица переходов вбирала коды символьные,
					 *          прежде уходившие сюда одним сравнением, и класс символов
					 *          пошёл косвенным переходом - строки, литерала не несущие вовсе,
					 *          проседали на ARM64 от пяти до тринадцати процентов. Смотрите
