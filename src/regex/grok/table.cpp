 ...
 *
 * @details Набор перенесён из прежней самостоятельной надстройки Grok и сверен
 *          с описанием шаблонов Grok сборщика данных StreamSets:
 *          https://docs.streamsets.com/platform-datacollector/latest/datacollector/UserGuide/Apx-GrokPatterns/GrokPatterns_title.html
 *
 *          При переносе исправлены четыре записи, собрать какие не в состоянии
 *          ни настоящий модуль, ни эталонная библиотека PCRE2, и удалена одна:
 *
 *          - «WINPATH» несла лишнюю открывающую скобку в начале, отчего
 *            выражение оставалось незакрытым; вместе с нею отказывала и «PATH»,
 *            на неё ссылающаяся;
 *          - «RAILS3HEA» названа с опиской, а ссылались на неё как на
 *            «RAILS3HEAD», отчего «RAILS3» разворачивалась в ссылку на
 *            шаблон отсутствующий; заодно снят пробел в начале текста;
 *          - «SHOREWALL» несла шесть повторов записи «.?*», сочетания
 *            недопустимого; принято прочтение «.*?»;
 *          - «#URIPARAM» удалена: текст её оборван на середине, а название,
...
/**
 * Подписываемся на пространство имён модуля Grok
 */
using namespace awh::grok;

/**
 * @brief Встроенный набор шаблонов
 *
 */
const awh::grok::entry_t awh::grok::PATTERNS[] = {
			...
			{"BACULA_LOG_VSS", "(?:Generate\\s+)?VSS (?:Writer)?"},
			...
			{"BACULA_LOG_FATAL_CONN", "Fatal error: bsock.c:\\d+ Unable to connect to (?:Client:\\s+%{BACULA_HOST:client}|Storage daemon) on %{HOSTNAME}:%{POSINT}. ERR=(?<berror>%{GREEDYDATA})"},
			{"BACULA_LOG_NO_CONNECT", "Warning: bsock.c:\\d+ Could not connect to (?:Client:\\s+%{BACULA_HOST:client}|Storage daemon) on %{HOSTNAME}:%{POSINT}. ERR=(?<berror>%{GREEDYDATA})"},
			...
			{"BACULA_LOG_JOB", "(?:Error:\\s+)?Bacula %{BACULA_HOST} %{BACULA_VERSION} \\(%{BACULA_VERSION}\\):"},
 ...
 *
 */
const size_t awh::grok::PATTERNS_COUNT = (sizeof(awh::grok::PATTERNS) / sizeof(awh::grok::PATTERNS[0]));
