...
namespace {
	 ...
	 *
	 */
	string cleanAuthority(string authority) noexcept {
		// Ищем начало якоря
		const size_t fragment = authority.find('#');
		...
		if(provider != nullptr){
			...
			switch(static_cast <uint8_t> (provider->direct)){
				...
				case static_cast <uint8_t> (http::direct_t::REQUEST): {
					 ...
					 */
					const bool extended = (!request->protocol.empty() && (request->method == http::method_t::CONNECT));
					/**
					...
					if((request->method == http::method_t::CONNECT) && !extended){
						...
						if(!request->uri.empty()){
							...
							const string authority = ::cleanAuthority(absolute ? string(uriAuthority) : request->uri);
					...
					if(awh::http::splitTarget(request->uri, uriScheme, uriAuthority, uriPath)){
						...
						authority = ::cleanAuthority(string(uriAuthority));
					...
					path.assign(awh::http::targetPath(path, buffer));
					/**
					...
					if(extended){
						...
						header.from(":protocol", request->protocol);
						// Если псевдозаголовок сформирован корректно - добавляем его в результат
						if(!header.name.empty())
...
awh::http::Headers::fields_t awh::http::Headers::_compose(const http::proto_t proto) const noexcept {
	...
	try {
		...
		/**
		 * Псевдозаголовки набора попадают в результат при отсутствующем провайдере,
		 * поэтому авторитет мог прийти и оттуда. Без этой проверки набор, где рядом
		 * лежат [:authority] и заголовок адресата, дал бы два псевдозаголовка
		 * авторитета - сообщение, которое принимающая сторона обязана отвергнуть
		 */
		if(!authority && (this->_provider == nullptr))
			// Определяем наличие псевдозаголовка авторитета среди установленных заголовков
			authority = (::findByName(this->_headers.begin(), this->_headers.end(), ":authority") != this->_headers.end());
		/**
		...
		const bool response = ((this->_provider != nullptr) ?
			...
			(::findByName(this->_headers.begin(), this->_headers.end(), ":status") != this->_headers.end())
		);
		...
		if(!authority && request){
			// Выполняем поиск заголовка адресата среди установленных
			const auto i = ::findByName(this->_headers.begin(), this->_headers.end(), "Host");
			/**
			 * Если заголовок адресата установлен и несёт значение: пустой авторитет
			 * псевдозаголовком не передаётся - он не указывает ни на какой узел,
			 * а принимающая сторона обязана считать такое сообщение некорректным
			 */
			if((i != this->_headers.end()) && !i->value.empty()){
				/**
				 ...
				 */
				const string authority = ::cleanAuthority(string(::trim(i->value)));
				// Если после очистки авторитет не опустел - формируем псевдозаголовок
				if(!authority.empty()){
					// Создаём псевдозаголовок авторитета
					header_t header{};
		...
		for(uint8_t pass = 0; pass < 2; pass++){
			...
			for(const auto & header : this->_headers){
				...
				if(pseudoName != (pass == 0))
					...
					continue;
				/**
				 * Псевдозаголовки, попавшие в набор при присваивании списка полей,
				 * пропускаем при установленном провайдере: они уже сформированы из него,
				 * а вывод обоих источников дал бы сообщение с дубликатами
				 */
				if((this->_provider != nullptr) && !header.name.empty() && (header.name.front() == ':'))
					// Переходим к следующему заголовку
					continue;
...
void awh::http::Headers::merge(const Headers & headers, const mode_t mode) noexcept {
	...
	try {
		...
		/**
		 * Выполняем отлов ошибок слияния
		 */
		try {
			/**
		...
		/**
		 * Если слияние не удалось - возвращаем набор к тому виду, что был до него
		 */
		} catch(const exception &) {
			...
			// Передаём отказ общему перехвату, чтобы он записал его в лог
			throw;
		}
	/**
	 * Если возникает ошибка
