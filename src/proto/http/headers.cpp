...
string awh::http::Headers::date(const uint64_t date) const noexcept {
	...
	try {
		...
		stringstream ss{};
		/**
		 * Закрепляем за потоком общепринятую местность
		 *
		 * @details Поток берёт местность из общей, а её задаёт уже приложение вызовом
		 *          std::locale::global. Под местностью, скажем, «de_DE» вывод названий
		 *          дня и месяца становится немецким - «Do., 06 Aug. 2026», - тогда как
		 *          RFC 9110 §5.6.7 требует в поле [Date] именно английских сокращений
		 *          вида IMF-fixdate. Приложение вправе ставить себе любую местность, а
		 *          запись на провод обязана оставаться одной и той же
		 *
		 * @note Обращение это к уже готовой местности, а не построение новой: стоит оно
		 *       наносекунду, тогда как построение std::locale обходится десятками
		 *       микросекунд
		 */
