				...
				case static_cast <uint8_t> (http::direct_t::REQUEST): {
					...
					if((request->method == http::method_t::CONNECT) && !extended){
						...
						if(!request->uri.empty()){
							...
							const string authority = ::cleanAuthority(request->uri);
					...
					if(schemeEnd != string::npos){
						 ...
						 * '/', '?' и '#' (RFC 3986 §3.2). Поиск одного лишь '/' уводил бы
						 * строку запроса внутрь авторитета, если путь в URI отсутствует
						 */
						const size_t pathStart = path.find_first_of("/?#", authorityStart);
						...
						if(pathStart != string::npos){
							...
							// Оставляем в пути только его часть (начиная с найденного разделителя)
							 ...
							 * псевдозаголовка [:path] уехал бы на провод
							 */
							...
							if(fragment != string::npos)
								...
								path.erase(fragment);
							/**
							 * Если путь в URI отсутствовал и остаток начинается со строки запроса
							 * либо якоря - подставляем корневой путь: псевдозаголовок [:path]
							 * обязан начинаться с '/' (RFC 9113 §8.3.1). Остаток бывает и пустым:
							 * в адресе вида [https://example.com#frag] после отбрасывания якоря
							 * от пути не остаётся ничего, и обращение к первому символу
							 * читало бы за границей строки
							 */
							if(path.empty() || (path.front() != '/'))
								// Дополняем путь корневым слешем
						...
						authority = ::cleanAuthority(::move(authority));
					}
					/**
		...
		for(uint8_t pass = 0; pass < 2; pass++){
			...
			for(const auto & header : this->_headers){
				...
				if((this->_provider != nullptr) && !header.name.empty() && (header.name.front() == ':'))
					// Переходим к следующему заголовку
					continue;
				// Если заголовок управляет соединением либо является адресатом - пропускаем его
				if(::isConnectionSpecific(header.name) || ::equals(header.name, "Host"))
					// Переходим к следующему заголовку
					continue;
				/**
