 ...
 *
 */
size_t awh::http::h2::hpack::DynamicTable::retained() const noexcept {
	// Выводим количество вытесненных, но ещё не снятых записей
	return (this->_entries.size() - this->_live);
}
/**
...
awh::http::h2::status_t awh::http::h2::hpack::Decoder::decode(string_view block, vector <field_view_t> & output, const uint64_t maxListSize, error_t & error) noexcept {
	...
	auto commit = [this](const slice_t & slice, const size_t mark) noexcept -> void {
		...
		if(this->_overflow){
			...
			/**
			 * Заголовки блока, превысившего лимит, наружу не пойдут, поэтому удерживать
			 * вытесненные записи таблицы больше незачем.
			 *
			 * Без этого снятия отложенное вытеснение обходило бы саму защиту от
			 * decompression bomb: удержание идёт по вставкам, а лимит списка считается
			 * по выданным полям, и блок из одних вставок со ссылкой на длинное название
			 * удерживал бы запись на каждую из них. Три октета входа порождают запись
			 * размером с таблицу - на блоке в 18 КБ это восемнадцать мегабайт
			 */
			this->_slices.clear();
			// Снимаем вытесненные записи динамической таблицы
			this->_table.release();
