...
void awh::http::Parser_HTTP::beginBody() noexcept {
	 ...
	 */
	if(noBody && ((this->_proto == proto_t::PROXY1) || (this->_proto == proto_t::WEBSOCKET1))){
		// Получаем статус-код ответа сервера
		const uint16_t code = static_cast <const response_t *> (this->_message.provider.get())->code;
		/**
		 * Определяем ответы, объявленное кадрирование которых недопустимо
		 *
		 * Узлу, передающему сообщение дальше, опасен любой ответ, границу которого
		 * следующее звено определит иначе, - это все 1xx и 204. Соединению же,
		 * которому предстоит переключение протокола, опасен именно ответ
		 * [101 Switching Protocols]: за его пустой строкой начинается поток нового
		 * протокола, и объявленный размер тела делает точку переключения предметом
		 * догадки - принятые следом байты окажутся либо телом, либо первым кадром
		 */
		const bool declared = ((this->_proto == proto_t::PROXY1)
		 ? (((code >= 100) && (code < 200)) || (code == 204)) : (code == 101));
	...
	/**
	 * Если соединению предстоит переключение протокола либо передача дальше по цепочке -
	 * запрос, требующий переключения, не вправе нести тело
	 *
	 * Запрос с заголовком Upgrade получает в ответ [101 Switching Protocols], за пустой
	 * строкой которого начинается поток нового протокола. Объявленное этим же запросом
	 * тело делает точку переключения предметом догадки: звено, уважившее объявленный
	 * размер, съест эти байты как тело, а звено, признавшее переключение состоявшимся, -
	 * прочитает их как начало нового протокола. Разошедшись, соседние узлы дадут
	 * атакующей стороне выбрать первый кадр чужого соединения. Рукопожатие WebSocket
	 * тела не определяет вовсе (RFC 6455 §4.1), и законного сочетания здесь нет.
	 * Прямому соединению это не опасно: оно переключаться не собирается и читает
	 * обычный запрос с телом, а заголовок Upgrade игнорирует
	 */
	if((this->_direct == direct_t::REQUEST) && this->_flags.upgradeSeen &&
	   ((this->_proto == proto_t::PROXY1) || (this->_proto == proto_t::WEBSOCKET1))){
		// Если объявлено транспортное кодирование
		if(this->_flags.transferEncodingSeen){
			// Фиксируем ошибку некорректного Transfer-Encoding
			this->_error = error_t::INVALID_TRANSFER_ENCODING;
		...
		if(this->_flags.contentLengthSeen && (this->_statsBody.contentLength > 0)){
			// Фиксируем ошибку некорректного Content-Length
			this->_error = error_t::INVALID_CONTENT_LENGTH;
			// Выходим из метода
			return;
...
void awh::http::Parser_HTTP::clear() noexcept {
	...
	/**
	 * Возвращаем протокол работы парсера к прямому соединению
	 *
	 * Очистка готовит объект к повторному использованию, а протокол описывает
	 * соединение: оставить его означало бы отдать объект под новое соединение с
	 * настройками предыдущего, тогда как лимиты к умолчанию уже вернулись
	 */
	this->_proto = proto_t::HTTP1;
...
unique_ptr <awh::http::parser_t> awh::http::Parser_HTTP::clone() const noexcept {
	...
	try {
		...
		/**
		 * Копируем протокол, с которым работает парсер
		 *
		 * Он описывает соединение, а не сообщение, и настраивается наравне с лимитами
		 * безопасности: фабрика, настроенная режимом прокси либо переключения протокола,
		 * обязана выпускать парсеры в том же режиме. Иначе все проверки кадрирования,
		 * которые режим включает, у клонов молча выключены - а именно клоны и обслуживают
		 * соединения
		 */
...
void awh::http::Parser_HTTP::sendHeaders(const headers_t & headers, const bool endStream) noexcept {
	...
	try {
		...
		/**
		 * Если соединению предстоит переключение протокола либо передача дальше по
		 * цепочке - запрос, требующий переключения, не собирается с объявленным телом
		 *
		 * Проверка повторяет ту, что выполняется на приёме: собранное сообщение обязано
		 * быть принято собственным приёмником, а он в этих режимах такой запрос
		 * отвергает. Тело делает точку переключения предметом догадки - принятые за
		 * блоком заголовков байты окажутся либо телом, либо началом нового протокола,
		 * смотря какое звено их читает.
		 * Сверяется объявленное на проводе кадрирование, а не выбранный нами способ:
		 * приёмник смотрит именно на заголовки. Объявленное вызывающей стороной
		 * кодирование chunked уходит на провод и при пустом теле - завершённым нулевым
		 * чанком, а собственное кадрирование при этом не выбирается вовсе. Объявленный
		 * нулевой Content-Length, напротив, приёмником допускается: тела он не описывает
		 */
		const bool encodingDeclared = (injectChunked ||
		 (hasEncoding && !dropEncoding && !dropLegacyEncoding && !dropFraming));
		// Признак объявленного на проводе ненулевого размера тела
		const bool lengthDeclared = (hasLength && !dropLength && usableLength && (this->_sender.remaining > 0));
		// Если запрос требует смены протокола и объявляет тело
		if((this->_direct == direct_t::REQUEST) && (encodingDeclared || lengthDeclared) &&
		   ((this->_proto == proto_t::PROXY1) || (this->_proto == proto_t::WEBSOCKET1)) && headers.has("Upgrade")){
			// Возвращаем кадрирование тела в исходное состояние
			this->_sender.framing = sender_t::framing_t::NONE;
