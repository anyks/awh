...
awh::http::h3::status_t awh::http::h3::frame::parser::settings(const uint8_t * payload, const size_t size, vector <setting_entry_t> & output, error_t & error) noexcept {
	...
	size_t offset = 0;
	/**
	 * Множество уже встреченных идентификаторов
	 *
	 * @details Пока параметров мало, поиск повтора идёт перебором набора: обычный
	 *          кадр несёт единицы параметров, и хеш-множество на них стоило бы
	 *          дороже самого перебора. Кадр предельного размера, набитый различными
	 *          идентификаторами, вмещает их тысячи, и перебор становится
	 *          квадратичным: 16 КБ на входе против трёх с половиной миллисекунд
	 *          процессорного времени, причём кадр в соединении единственный
	 *          и лимитом частоты не сдерживается
	 */
	...
	while(offset < size){
		...
		if(seen.empty()){
			/**
			 * Выполняем перебор уже разобранных параметров
			 */
			...
			// Если набор перерос порог перебора - переносим его в множество
			if(output.size() >= SETTINGS_LOOKUP_THRESHOLD){
				// Резервируем место под уже разобранные параметры и текущий
				seen.reserve(output.size() + 1);
				/**
				 * Выполняем перенос уже разобранных идентификаторов в множество
				 */
				for(auto & exists : output)
					// Дописываем очередной идентификатор в множество
					seen.emplace(exists.id);
				// Дописываем идентификатор текущего параметра в множество
				seen.emplace(item.id);
			}
		// Если поиск повтора идёт по множеству
		} else if(!seen.emplace(item.id).second) {
			// Устанавливаем код ошибки протокола
			error = error_t::H3_SETTINGS_ERROR;
			// Выводим результат разбора
			return status_t::ERROR;
