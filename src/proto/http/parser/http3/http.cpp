...
void awh::http::Parser_HTTP3::closeStream(const uint64_t sid, const error_t code) noexcept {
	...
	/**
	 * Снимаем ссылки потока в кодеке: подтверждения его секций уже не придут,
	 * а удерживаемые ими записи таблицы обязаны освободиться
	 */
	this->_encoder.cancel(sid);
	// Извещаем кодер пира об отмене потока
