 ...
 *
 */
awh::quic2::frame::Range::Range() noexcept : low(0), high(0) {}

/**
 * @brief Конструктор
 *
 */
awh::quic2::frame::Ack::Ack() noexcept :
 ...
 *
 */
awh::quic2::frame::Reset_Stream::Reset_Stream() noexcept :
 ...
 *
 */
awh::quic2::frame::Stop_Sending::Stop_Sending() noexcept :
 streamId(0), code(0) {}
 ...
 *
 */
awh::quic2::frame::Crypto::Crypto() noexcept : offset(0), data{""} {}

/**
 * @brief Конструктор
 *
 */
awh::quic2::frame::Stream::Stream() noexcept :
 ...
 *
 */
awh::quic2::frame::New_Connection_Id::New_Connection_Id() noexcept :
 ...
 *
 */
awh::quic2::frame::Connection_Close::Connection_Close() noexcept :
 ...
 *
 */
bool awh::quic2::frame::parser::type(const uint8_t * data, const size_t size, frame_t & output) noexcept {
	// Прочитанное значение типа фрейма
	uint64_t value = 0;
	// Читаем тип фрейма
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::padding(const uint8_t * data, const size_t size, size_t & consumed, error_t & error) noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::ping(const uint8_t * data, const size_t size, size_t & consumed, error_t & error) noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::ack(const uint8_t * data, const size_t size, ack_t & output, size_t & consumed, error_t & error) noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::resetStream(const uint8_t * data, const size_t size, reset_stream_t & output, size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
	// Смещение разбора
	size_t offset = 0;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::stopSending(const uint8_t * data, const size_t size, stop_sending_t & output, size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
	// Смещение разбора
	size_t offset = 0;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::crypto(const uint8_t * data, const size_t size, crypto_t & output, size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
	// Смещение разбора
	size_t offset = 0;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::newToken(const uint8_t * data, const size_t size, string_view & token, size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
	// Смещение разбора
	size_t offset = 0;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::datagram(const uint8_t * data, const size_t size, string_view & output, size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
	// Смещение разбора
	size_t offset = 0;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::stream(const uint8_t * data, const size_t size, stream_t & output, size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
	// Смещение разбора
	size_t offset = 0;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::single(const uint8_t * data, const size_t size, const frame_t type, uint64_t & value, size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
	// Смещение разбора
	size_t offset = 0;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::pair(const uint8_t * data, const size_t size, const frame_t type, uint64_t & streamId, uint64_t & value, size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
	// Смещение разбора
	size_t offset = 0;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::newConnectionId(const uint8_t * data, const size_t size, new_connection_id_t & output, size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
	// Смещение разбора
	size_t offset = 0;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::path(const uint8_t * data, const size_t size, const frame_t type, uint8_t output[proto::PATH_DATA_SIZE], size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::connectionClose(const uint8_t * data, const size_t size, connection_close_t & output, size_t & consumed, error_t & error) noexcept {
	// Устанавливаем код ошибки транспорта на случай неудачи разбора
	error = error_t::FRAME_ENCODING_ERROR;
	// Смещение разбора
	size_t offset = 0;
 ...
 *
 */
awh::quic2::status_t awh::quic2::frame::parser::handshakeDone(const uint8_t * data, const size_t size, size_t & consumed, error_t & error) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::padding(string & output, const size_t count) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::ping(string & output) noexcept {
 ...
 *
 */
bool awh::quic2::frame::serialize::ack(string & output, const ack_t & frame) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::resetStream(string & output, const uint64_t streamId, const uint64_t code, const uint64_t finalSize) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::stopSending(string & output, const uint64_t streamId, const uint64_t code) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::crypto(string & output, const uint64_t offset, string_view data) noexcept {
 ...
 *
 */
bool awh::quic2::frame::serialize::newToken(string & output, string_view token) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::datagram(string & output, string_view data) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::stream(string & output, const uint64_t streamId, const uint64_t offset, string_view data, const bool fin) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::single(string & output, const frame_t type, const uint64_t value) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::pair(string & output, const frame_t type, const uint64_t streamId, const uint64_t value) noexcept {
	// Дописываем тип фрейма
	varint::write(output, static_cast <uint64_t> (type));
	// Дописываем идентификатор потока
	varint::write(output, streamId);
 ...
 *
 */
bool awh::quic2::frame::serialize::newConnectionId(string & output, const new_connection_id_t & frame) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::path(string & output, const frame_t type, const uint8_t data[proto::PATH_DATA_SIZE]) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::connectionClose(string & output, const uint64_t code, const uint64_t frameType, string_view reason, const bool app) noexcept {
 ...
 *
 */
void awh::quic2::frame::serialize::handshakeDone(string & output) noexcept {
