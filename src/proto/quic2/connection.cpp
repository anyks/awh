 ...
 */
#include <proto/quic2/varint.hpp>
#include <proto/quic2/packet.hpp>
#include <proto/quic2/connection.hpp>
...
namespace {
	 ...
	 *
	 */
	static constexpr size_t CID_SIZE = awh::quic2::Connection::LOCAL_CID_SIZE;
	/**
	 ...
	 *
	 */
	static constexpr size_t NEW_CID_OVERHEAD = (1 + 8 + 8 + 1 + awh::quic2::proto::MAX_CID_SIZE + awh::quic2::proto::RESET_TOKEN_SIZE);
	/**
	 ...
	 *
	 */
	static bool makeCid(awh::quic2::cid_t & cid) noexcept {
 ...
 *
 */
awh::quic2::Connection::Chunk::Chunk() noexcept : sid(0), offset(0), size(0), fin(false), data{""} {}

/**
 * @brief Конструктор учётной записи отправленного пакета
 *
 */
awh::quic2::Connection::Sent::Sent() noexcept :
 ...
 *
 */
awh::quic2::Connection::Congestion::Congestion() noexcept :
 ...
 *
 */
awh::quic2::Connection::Rtt::Rtt() noexcept :
 ...
 *
 */
awh::quic2::Connection::Path::Path() noexcept :
 ...
 *
 */
awh::quic2::Connection::Close::Close() noexcept :
 ...
 *
 */
awh::quic2::Connection::Token::Token() noexcept :
 ...
 *
 */
awh::quic2::Connection::Phase::Phase() noexcept :
 ...
 *
 */
awh::quic2::Connection::Routing::Routing() noexcept :
 ...
 *
 */
awh::quic2::Connection::Marking::Marking() noexcept :
 ...
 *
 */
awh::quic2::Connection::Limits::Limits() noexcept :
 ...
 *
 */
awh::quic2::Connection::Flow::Flow() noexcept :
 ...
 *
 */
awh::quic2::Connection::Buffers::Buffers() noexcept :
 ...
 *
 */
awh::quic2::Connection::Dgram::Dgram() noexcept {}

/**
 * @brief Конструктор
 *
 */
awh::quic2::Connection::Identity::Identity() noexcept :
 ...
 *
 */
awh::quic2::Connection::Amplify::Amplify(const endpoint_t endpoint) noexcept :
 received(0), sent(0), validated(endpoint == endpoint_t::CLIENT) {}
/**
 * @brief Конструктор
 *
 */
awh::quic2::Connection::AEAD::AEAD() noexcept :
 ...
 *
 */
awh::quic2::Connection::Core::Core(const endpoint_t endpoint) noexcept :
 ...
 *
 */
awh::quic2::Connection::Times::Times() noexcept :
 now(0), idle(0) {}
/**
 * @brief Конструктор
 *
 */
awh::quic2::Connection::Transport::Transport() noexcept : resumed(false) {}
/**
 * @brief Конструктор
 *
 */
awh::quic2::Connection::Cids::Cids() noexcept {}
/**
 * @brief Конструктор
 *
 * @param endpoint роль локального эндпоинта на соединении
 ...
 *
 */
awh::quic2::Connection::Crypto::Crypto(const endpoint_t endpoint, const tls::coder_t::id_t ctx, const tls::coder_t & coder, const log_t * log) noexcept :
 ...
 *
 */
awh::quic2::Connection::Streams::Streams() noexcept : cursor(0) {}

/**
 * @brief Конструктор
 *
 */
awh::quic2::Connection::Pmtu::Pmtu() noexcept :
 ...
 *
 */
awh::quic2::Connection::RemoteCid::RemoteCid() noexcept : seq(0), used(false), hasToken(false), resetToken{0} {}
 ...
 *
 */
awh::quic2::Connection::Space::Space() noexcept :
 ...
 *
 */
awh::quic2::Connection::space_t awh::quic2::Connection::space(const level_t level) const noexcept {
	/**
 ...
 *
 */
void awh::quic2::Connection::record(const space_t space, const uint64_t pn) noexcept {
 ...
 *
 */
bool awh::quic2::Connection::duplicate(const space_t space, const uint64_t pn) const noexcept {
	// Получаем состояние пространства номеров пакетов
 ...
 *
 */
void awh::quic2::Connection::pull() noexcept {
 ...
 *
 */
void awh::quic2::Connection::fail(const error_t error) noexcept {
	/**
 ...
 *
 */
void awh::quic2::Connection::discard(const level_t level) noexcept {
 ...
 *
 */
void awh::quic2::Connection::rtt(const uint64_t sample, const uint64_t delay) noexcept {
 ...
 *
 */
void awh::quic2::Connection::requeue(const space_t space, const sent_t & packet) noexcept {
	// Получаем состояние пространства номеров пакетов
	auto & item = this->_spaces[static_cast <size_t> (space)];
	/**
 ...
 *
 */
void awh::quic2::Connection::settle(const chunk_t & chunk) noexcept {
	...
	if(end <= stream.txAcked)
		// Выходим из метода
		return;
	// Если блок примыкает к подтверждённому префиксу либо перекрывает его - продвигаем префикс
	...
	// Если блок подтверждён не по порядку - запоминаем его как примыкающий диапазон
		...
		// Добавляем диапазон в очередь, ключ - смещение начала блока
		auto res = stream.txPending.emplace(chunk.offset, end);
		// Если диапазон с таким смещением уже есть (пере-сегментация ретрансмиссии при PTO) - расширяем его концом
		if(!res.second && (end > res.first->second))
			// Запоминаем наибольший конец перекрывающихся диапазонов
			res.first->second = end;
	}
	/**
	 * Поглощаем примыкающие и перекрывающие префикс подтверждённые диапазоны: конец
	 * одного блока примыкает к началу следующего, поэтому префикс продвигается по
	 * цепочке. Диапазоны, целиком ушедшие под префикс (перекрытие при PTO и пере-
	 * сегментации ретрансмиссии), отбрасываются, не оставляя осиротевших записей
	 */
	auto next = stream.txPending.begin();
	// Пока в очереди есть диапазон, начинающийся на границе префикса либо под ней
	while((next != stream.txPending.end()) && (next->first <= stream.txAcked)){
		// Если конец диапазона выходит за префикс - продвигаем префикс до него
		if(next->second > stream.txAcked)
			// Продвигаем префикс до конца поглощённого диапазона
			stream.txAcked = next->second;
		// Удаляем поглощённый либо ушедший под префикс диапазон и переходим к следующему
		next = stream.txPending.erase(next);
	}
}
/**
 ...
 *
 */
void awh::quic2::Connection::schedule(const uint64_t sid) noexcept {
	// Ищем поток по идентификатору
 ...
 *
 */
void awh::quic2::Connection::restore() noexcept {
	/**
 ...
 *
 */
bool awh::quic2::Connection::validate(const space_t space, const frame::ack_t & frame, const uint64_t marked) noexcept {
	/**
 ...
 *
 */
void awh::quic2::Connection::deflate() noexcept {
 ...
 *
 */
void awh::quic2::Connection::discover() noexcept {
	/**
 ...
 *
 */
void awh::quic2::Connection::detect(const space_t space, const vector <uint64_t> & ackedTimes) noexcept {
	// Получаем состояние пространства номеров пакетов
	auto & item = this->_spaces[static_cast <size_t> (space)];
	// Сбрасываем флаг взведённого таймера детекта потерь
	item.hasLossTime = false;
	 ...
	 * только пакеты, отправленные после наиболее позднего подтверждённого прежними
	 * фреймами. Всё отправленное до него подтверждением закрыто. Дополнительно период
	 * разрывается подтверждением текущего фрейма, попавшим внутрь серии потерь: период
	 * устойчивой перегрузки требует отсутствия подтверждений внутри него (RFC 9002 §7.6)
	 */
	...
	// Флаг обнаруженного периода устойчивой перегрузки
	...
	for(auto i = item.sent.begin(); i != item.sent.end();){
		...
		if(((i->pn + PACKET_THRESHOLD) <= item.largestAcked) || ((i->time + lossDelay) <= this->_times.now)){
			...
			lost = true;
			// Если пакет отправлен после наиболее позднего подтверждённого прежними фреймами
			if(!this->_congestion.hasAcked || (i->time > this->_congestion.acked)){
				/**
				 * Проверяем разрыв серии подтверждением текущего фрейма: если этот фрейм
				 * ACK подтвердил пакет, отправленный между предыдущим и текущим потерянными,
				 * серия прерывается. Так переупорядочивание, когда пир несмежными
				 * диапазонами подтверждает пакет внутри полосы потерь, не образует ложного
				 * периода устойчивой перегрузки. Подтверждения этого фрейма к моменту детекта
				 * уже удалены из списка отправленных, поэтому их времена передаются отдельно
				 */
				bool interrupted = false;
				// Если период уже начат - ищем подтверждение внутри серии
				if(periodCount > 0){
					/**
					 * Перебираем времена отправки подтверждённых текущим фреймом пакетов
					 */
					for(const uint64_t acked : ackedTimes){
						// Если подтверждённый пакет отправлен строго между предыдущим и текущим потерянными
						if((acked > periodEnd) && (acked < i->time)){
							// Отмечаем серию прерванной подтверждением
							interrupted = true;
							// Прекращаем перебор
							break;
						}
				...
				// Если период ещё не начат либо серия прервана подтверждением - начинаем новый период
				if((periodCount == 0) || interrupted){
					// Запоминаем время отправки первого потерянного пакета нового периода
					periodStart = i->time;
					// Устанавливаем время отправки последнего потерянного пакета периода
					periodEnd = i->time;
					// Начинаем счёт потерянных пакетов периода заново
					periodCount = 1;
				// Иначе продолжаем текущий период
				} else {
				...
				/**
				 * Оцениваем период немедленно: разрыв серии обнуляет накопление, поэтому
	 ...
	 * периода превышает порог (RFC 9002 §7.6). Признак накоплен по ходу цикла с учётом
	 * разрывов серии подтверждениями текущего фрейма
	 */
	if(persistent){
 ...
 *
 */
void awh::quic2::Connection::probe(const space_t space) noexcept {
	// Получаем состояние пространства номеров пакетов
	auto & item = this->_spaces[static_cast <size_t> (space)];
 ...
 *
 */
uint64_t awh::quic2::Connection::interval(const space_t space) const noexcept {
 ...
 *
 */
uint64_t awh::quic2::Connection::deadline(const space_t space) const noexcept {
	// Получаем состояние пространства номеров пакетов
	const auto & item = this->_spaces[static_cast <size_t> (space)];
 ...
 *
 */
uint64_t awh::quic2::Connection::persistence() const noexcept {
 ...
 *
 */
uint64_t awh::quic2::Connection::idle() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::consume(stream_data_t & stream, const uint64_t target) noexcept {
 ...
 *
 */
bool awh::quic2::Connection::stateless(const uint8_t * data, const size_t size) const noexcept {
	/**
 ...
 *
 */
bool awh::quic2::Connection::token(const uint8_t mark, const cid_t & odcid, string & output) const noexcept {
	/**
	 * Отказываемся выдавать токен без адреса клиента: адрес входит в код аутентичности,
	 * но при неизвестном семействе адреса опаковое представление пусто, и код заверил бы
	 * нулевой адрес - такой токен проверку прошёл бы с любого IP, обходя защиту адреса
	 * (RFC 9000 §8.1). Выдаём токен только с достоверно установленным адресом
	 */
	if(this->_core.address.empty())
 ...
 *
 */
bool awh::quic2::Connection::validate(string_view token, cid_t & odcid, bool & retried) const noexcept {
	/**
	 * Без адреса клиента проверку не проходим: код аутентичности заверяет адрес, и с
	 * пустым опаковым представлением любой токен подтвердился бы с произвольного IP
	 * (RFC 9000 §8.1). Проверяем токен только при достоверно установленном адресе
	 */
	if(this->_core.address.empty())
 ...
 *
 */
size_t awh::quic2::Connection::allowance() const noexcept {
	/**
 ...
 *
 */
bool awh::quic2::Connection::stalled() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::collect() noexcept {
 ...
 *
 */
void awh::quic2::Connection::prepare() noexcept {
 ...
 *
 */
void awh::quic2::Connection::promote() noexcept {
	// Получаем текущие ключи снятия защиты пакетов уровня приложения
	const crypto::keys_t * read = this->_crypto.handshake.decryption(level_t::APPLICATION);
 ...
 *
 */
void awh::quic2::Connection::acked(const sent_t & packet) noexcept {
	/**
 ...
 *
 */
void awh::quic2::Connection::congestion(const uint64_t time) noexcept {
 ...
 *
 */
void awh::quic2::Connection::issue() noexcept {
	...
	while((this->_cids.routing.issued.size() + (this->_cids.routing.retired ? 0 : 1)) < target){
		 ...
		 */
		if(!quic2::resetToken(this->_token.reset, frame.cid, frame.resetToken)){
 ...
 *
 */
bool awh::quic2::Connection::reserved(const uint64_t seq) const noexcept {
 ...
 *
 */
bool awh::quic2::Connection::sendable(const uint64_t sid) const noexcept {
 ...
 *
 */
bool awh::quic2::Connection::receivable(const uint64_t sid) const noexcept {
 ...
 *
 */
uint64_t awh::quic2::Connection::rxWindow(const uint64_t sid) const noexcept {
 ...
 *
 */
uint64_t awh::quic2::Connection::txWindow(const uint64_t sid) const noexcept {
	// Определяем инициатора потока
	const bool local = ((sid & 0x01) == ((this->_core.endpoint == endpoint_t::SERVER) ? 0x01 : 0x00));
	// Если поток однонаправленный
	if((sid & 0x02) != 0)
 ...
 *
 */
awh::quic2::Connection::stream_data_t * awh::quic2::Connection::accept(const uint64_t sid, error_t & error) noexcept {
	// Ищем поток по идентификатору
	auto i = this->_stream.list.find(sid);
	// Если поток найден
	if(i != this->_stream.list.end())
 ...
 *
 */
bool awh::quic2::Connection::closed(const uint64_t sid) const noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::Connection::inputStream(const frame::stream_t & frame) noexcept {
	...
	uint64_t delta = 0;
	/**
	 * Если принято новое наибольшее смещение данных потока. Пустой фрейм без завершения
	 * (нет данных и нет FIN) наибольшее смещение не продвигает: он не несёт ни байт, ни
	 * финального размера, и продвижение по нему открыло бы фантомный разрыв, зачтя в
	 * flow control не отправленные байты (RFC 9000 §19.8)
	 */
	if((end > stream->rxHigh) && (!frame.data.empty() || frame.fin)){
 ...
 *
 */
bool awh::quic2::Connection::established() noexcept {
	// Код ошибки транспорта
	error_t error = error_t::NO_ERROR;
 ...
 *
 */
void awh::quic2::Connection::credit(const uint64_t sid, stream_data_t & stream) noexcept {
 ...
 *
 */
bool awh::quic2::Connection::ready(const stream_data_t & stream) const noexcept {
	/**
 ...
 *
 */
void awh::quic2::Connection::notify(const uint64_t sid, stream_data_t & stream) noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::Connection::input(const level_t level, const uint64_t offset, string_view data) noexcept {
	// Получаем состояние пространства номеров пакетов
	auto & item = this->_spaces[static_cast <size_t> (this->space(level))];
	// Вычисляем конечное смещение данных фрейма
 ...
 *
 */
awh::quic2::status_t awh::quic2::Connection::frames(const level_t level, const uint8_t * data, const size_t size, bool & nonProbing) noexcept {
		...
		switch(type){
			...
			case frame_t::ACK_ECN: {
				...
				if(status == status_t::OK){
					...
					/**
					 * Времена отправки пакетов, впервые подтверждённых текущим фреймом (по
					 * возрастанию номера, а значит и времени): передаются в детект потерь для
					 * разрыва периода устойчивой перегрузки подтверждением внутри серии.
					 * Пакеты прежнего пути (stale) не учитываются - их подтверждение не
					 * относится к текущему пути (RFC 9000 §9.4). Буфер переиспользуется между
					 * фреймами (только очищается), чтобы горячий путь не выделял память заново
					 */
					vector <uint64_t> & ackedTimes = this->_ackedTimes;
					// Очищаем буфер времён подтверждённых пакетов, сохраняя выделенную ёмкость
					ackedTimes.clear();
					...
					for(auto i = item.sent.begin(); i != item.sent.end(); ++i){
						...
						// Пакеты прежнего пути в оценку текущего пути не входят (RFC 9000 §9.4)
						if(!i->stale){
							// Запоминаем время отправки наиболее позднего подтверждаемого пакета
							newestAcked = ::max(newestAcked, i->time);
							// Запоминаем время отправки подтверждённого пакета для разрыва периода устойчивой перегрузки
							ackedTimes.push_back(i->time);
						}
					...
					if(acked){
						...
						// Выполняем детект потерянных пакетов пространства с разрывом периода по подтверждениям фрейма
						this->detect(space, ackedTimes);
 ...
 *
 */
bool awh::quic2::Connection::payload(const level_t level, const size_t budget, string & output, sent_t & meta, bool & elicit, const bool limited) noexcept {
	// Получаем состояние пространства номеров пакетов
	auto & item = this->_spaces[static_cast <size_t> (this->space(level))];
	/**
 ...
 *
 */
size_t awh::quic2::Connection::headerSize(const level_t level, const uint64_t length, const size_t pnSize, const cid_t & dcid) const noexcept {
 ...
 *
 */
bool awh::quic2::Connection::seal(string & output, const level_t level, string_view payload, const cid_t & dcid) noexcept {
 ...
 *
 */
awh::tls::coder_t::alpn_t awh::quic2::Connection::alpn() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::params(const quic2::params::params_t & params) noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::Connection::peer(quic2::params::params_t & params, error_t & error) const noexcept {
 ...
 *
 */
void awh::quic2::Connection::address(const net::addr_t * addr, const uint16_t port) noexcept {
 ...
 *
 */
void awh::quic2::Connection::address(const net::attr_t * attr) noexcept {
 ...
 *
 */
void awh::quic2::Connection::retry(const bool mode) noexcept {
 ...
 *
 */
void awh::quic2::Connection::token(string_view token) noexcept {
 ...
 *
 */
void awh::quic2::Connection::resetKey(string_view key) noexcept {
	// Если соединение уже начато
	if(this->_core.state != state_t::NONE)
 ...
 *
 */
const string & awh::quic2::Connection::token() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::ecn(const bool mode) noexcept {
 ...
 *
 */
void awh::quic2::Connection::roaming(const bool mode) noexcept {
 ...
 *
 */
awh::event::ecn_t awh::quic2::Connection::marking() const noexcept {
	/**
 ...
 *
 */
awh::quic2::status_t awh::quic2::Connection::connect() noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::Connection::read(const uint8_t * data, const size_t size, const uint64_t now, const event::ecn_t ecn) noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::Connection::read(const uint8_t * data, const size_t size, const uint64_t now) noexcept {
	...
	while(offset < buffer.size()){
		...
		if((this->_core.endpoint == endpoint_t::SERVER) && (this->_core.state == state_t::NONE)){
			...
			if(!this->_transport.local.hasResetToken)
				...
				this->_transport.local.hasResetToken = quic2::resetToken(this->_token.reset, this->_cids.identity.source, this->_transport.local.resetToken);
 ...
 *
 */
bool awh::quic2::Connection::write(string & output, const uint64_t now) noexcept {
 ...
 *
 */
uint64_t awh::quic2::Connection::timeout() const noexcept {
	/**
 ...
 *
 */
void awh::quic2::Connection::tick(const uint64_t now) noexcept {
	/**
 ...
 *
 */
awh::quic2::status_t awh::quic2::Connection::rekey(const uint64_t now) noexcept {
	// Обновляем текущее время последнего вызова
	this->_times.now = now;
 ...
 *
 */
bool awh::quic2::Connection::phase() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::aeadLimits(const uint64_t confidentiality, const uint64_t integrity) noexcept {
	/**
 ...
 *
 */
void awh::quic2::Connection::repath(const bool remote) noexcept {
	/**
	...
	this->_congestion.recovery = 0;
	/**
	 * Сбрасываем нижнюю границу периода устойчивой перегрузки: она хранит время
	 * отправки наиболее позднего подтверждённого пакета прежнего пути, которое
	 * предшествует любому пакету нового пути. Сохранённая, она отключила бы разрыв
	 * периода по подтверждениям на новом пути ровно тогда, когда потери там наиболее
	 * вероятны - на непроверенном канале (RFC 9002 §7.6)
	 */
	this->_congestion.acked = 0;
	// Сбрасываем флаг наличия подтверждённых пакетов для оценки устойчивой перегрузки
	this->_congestion.hasAcked = false;
 ...
 *
 */
bool awh::quic2::Connection::probe() noexcept {
 ...
 *
 */
void awh::quic2::Connection::abandon(const bool revert) noexcept {
	// Сбрасываем флаг ожидания ответа на проверку пути
	this->_path.pending = false;
	// Сбрасываем флаг необходимости отправки фрейма PATH_CHALLENGE
	this->_path.queued = false;
 ...
 *
 */
bool awh::quic2::Connection::validated() const noexcept {
 ...
 *
 */
bool awh::quic2::Connection::migrate() noexcept {
 ...
 *
 */
bool awh::quic2::Connection::relocatable() const noexcept {
	/**
 ...
 *
 */
bool awh::quic2::Connection::preferred(const bool ipv6, string & ip, uint16_t & port) const noexcept {
 ...
 *
 */
bool awh::quic2::Connection::relocate() noexcept {
 ...
 *
 */
void awh::quic2::Connection::settle() noexcept {
	// Сбрасываем флаг выполняемой проверки предпочтительного адреса
	this->_path.relocating = false;
 ...
 *
 */
const awh::string & awh::quic2::Connection::path() const noexcept {
 ...
 *
 */
bool awh::quic2::Connection::alternate() const noexcept {
 ...
 *
 */
size_t awh::quic2::Connection::pmtu() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::pmtu(const size_t limit) noexcept {
	/**
 ...
 *
 */
size_t awh::quic2::Connection::streams() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::streams(const uint64_t limit) noexcept {
	/**
 ...
 *
 */
uint64_t awh::quic2::Connection::migrations() const noexcept {
 ...
 *
 */
bool awh::quic2::Connection::rotate() noexcept {
	// Если соединение не установлено
	if(this->_core.state != state_t::CONNECTED)
		// Выводим отрицательный результат
		return false;
 ...
 *
 */
string awh::quic2::Connection::sessionKey() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::persist() noexcept {
 ...
 *
 */
string awh::quic2::Connection::session() const noexcept {
	 ...
	 */
	quic2::params::params_t remembered = this->_transport.remote;
	// Значения по умолчанию для сброса непереносимых скалярных параметров
	const quic2::params::params_t defaults;
	...
	if(!quic2::params::serialize::encode(params, remembered, sender))
		// Выводим пустой результат
		return string();
 ...
 *
 */
bool awh::quic2::Connection::session(string_view session) noexcept {
	// Если соединение уже начато
	if(this->_core.state != state_t::NONE)
		// Выводим отрицательный результат
		return false;
	...
	quic2::params::params_t params;
	// Определяем роль удалённого узла (противоположна локальной)
	const endpoint_t sender = ((this->_core.endpoint == endpoint_t::CLIENT) ? endpoint_t::SERVER : endpoint_t::CLIENT);
	// Если разбор транспортных параметров удалённого узла не выполнен
	if(quic2::params::parser::decode(reinterpret_cast <const uint8_t *> (session.data() + 4), size, sender, params, error) != status_t::OK)
		// Выводим отрицательный результат
		return false;
	/**
 ...
 *
 */
bool awh::quic2::Connection::early() const noexcept {
 ...
 *
 */
uint64_t awh::quic2::Connection::cwnd() const noexcept {
 ...
 *
 */
uint64_t awh::quic2::Connection::inflight() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::close(const uint64_t code, string_view reason) noexcept {
 ...
 *
 */
void awh::quic2::Connection::drop([[maybe_unused]] const char * reason) const noexcept {
	/**
 ...
 *
 */
bool awh::quic2::Connection::writable() const noexcept {
 ...
 *
 */
uint64_t awh::quic2::Connection::open(const bool unidirectional) noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::Connection::datagram(string_view data) noexcept {
	// Если соединение к отправке данных приложения не готово
	if(!this->writable())
		// Выводим отрицательный результат
		return status_t::ERROR;
 ...
 *
 */
bool awh::quic2::Connection::datagram(string & output) noexcept {
 ...
 *
 */
size_t awh::quic2::Connection::datagrams() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::readable(vector <uint64_t> & output) noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::Connection::receive(const uint64_t sid, string & output, bool & fin) noexcept {
 ...
 *
 */
void awh::quic2::Connection::reset(const uint64_t sid, const uint64_t code) noexcept {
	// Если отправка данных в поток локальным эндпоинтом недопустима
	if(!this->sendable(sid))
		// Выходим из метода
		return;
 ...
 *
 */
void awh::quic2::Connection::stop(const uint64_t sid, const uint64_t code) noexcept {
 ...
 *
 */
bool awh::quic2::Connection::aborted(const uint64_t sid, uint64_t & code) const noexcept {
	// Ищем поток по идентификатору
	auto i = this->_stream.list.find(sid);
 ...
 *
 */
awh::quic2::Connection::state_t awh::quic2::Connection::state() const noexcept {
 ...
 *
 */
awh::quic2::error_t awh::quic2::Connection::error() const noexcept {
 ...
 *
 */
const awh::quic2::cid_t & awh::quic2::Connection::scid() const noexcept {
 ...
 *
 */
void awh::quic2::Connection::issued(vector <cid_t> & added, vector <cid_t> & removed) noexcept {
 ...
 *
 */
const awh::quic2::cid_t & awh::quic2::Connection::dcid() const noexcept {
 ...
 *
 */
const awh::quic2::handshake_t & awh::quic2::Connection::handshake() const noexcept {
 ...
 *
 */
awh::quic2::Connection::Connection(const endpoint_t endpoint, const tls::coder_t::id_t ctx, const tls::coder_t & coder, const log_t * log) noexcept :
