 ...
 */
#include <proto/quic2/handshake.hpp>
...
namespace {
	 ...
	 *
	 */
	static awh::quic2::level_t levelFromSSL(const enum ssl_encryption_level_t level) noexcept {
		/**
		...
		switch(level){
			...
			case ssl_encryption_initial:
				...
				return awh::quic2::level_t::INITIAL;
			// Уровень ранних данных 0-RTT
			case ssl_encryption_early_data:
				// Выводим внутренний уровень шифрования
				return awh::quic2::level_t::EARLY_DATA;
			// Уровень хендшейка
			case ssl_encryption_handshake:
				// Выводим внутренний уровень шифрования
				return awh::quic2::level_t::HANDSHAKE;
			// Уровень приложения 1-RTT
			case ssl_encryption_application:
				// Выводим внутренний уровень шифрования
				return awh::quic2::level_t::APPLICATION;
		}
	 ...
	 *
	 */
	static enum ssl_encryption_level_t levelToSSL(const awh::quic2::level_t level) noexcept {
		/**
		 * Определяем внутренний уровень шифрования
		 */
		switch(level){
			// Уровень Initial
			case awh::quic2::level_t::INITIAL:
				// Выводим уровень шифрования BoringSSL
				return ssl_encryption_initial;
			// Уровень ранних данных 0-RTT
			case awh::quic2::level_t::EARLY_DATA:
				// Выводим уровень шифрования BoringSSL
				return ssl_encryption_early_data;
			// Уровень хендшейка
			case awh::quic2::level_t::HANDSHAKE:
				// Выводим уровень шифрования BoringSSL
				return ssl_encryption_handshake;
			// Уровень приложения 1-RTT
			case awh::quic2::level_t::APPLICATION:
				// Выводим уровень шифрования BoringSSL
	 ...
	 *
	 */
	static bool suiteFromCipher(const SSL_CIPHER * cipher, awh::quic2::crypto::suite_t & suite) noexcept {
		/**
		...
		switch(::SSL_CIPHER_get_protocol_id(cipher)){
			...
			case 0x1301: {
				...
				suite = awh::quic2::crypto::suite_t::AES_128_GCM_SHA256;
			...
			case 0x1302: {
				// Устанавливаем криптографический набор
				suite = awh::quic2::crypto::suite_t::AES_256_GCM_SHA384;
				// Выводим положительный результат
				return true;
			}
			...
			case 0x1303: {
				// Устанавливаем криптографический набор
				suite = awh::quic2::crypto::suite_t::CHACHA20_POLY1305_SHA256;
				// Выводим положительный результат
				return true;
			}
		}
...
namespace {
	...
	static int32_t newSession(SSL * ssl, SSL_SESSION * session) noexcept {
		...
		auto handshake = reinterpret_cast <awh::quic2::handshake_t *> (SSL_get_app_data(ssl));
		...
		awh::quic2::HandshakeHook::ticket(handshake, session);
	...
	static int32_t setReadSecret(SSL * ssl, enum ssl_encryption_level_t level, const SSL_CIPHER * cipher, const uint8_t * secret, size_t size) noexcept {
		...
		awh::quic2::handshake_t * handshake = reinterpret_cast <awh::quic2::handshake_t *> (SSL_get_app_data(ssl));
		// Устанавливаем секрет направления чтения и выводим результат
		return (awh::quic2::HandshakeHook::secret(handshake, ::levelFromSSL(level), false, cipher, secret, size) ? 1 : 0);
	}
	/**
	 * @brief Функция установки секрета записи уровня шифрования
	 *
	...
	static int32_t setWriteSecret(SSL * ssl, enum ssl_encryption_level_t level, const SSL_CIPHER * cipher, const uint8_t * secret, size_t size) noexcept {
		...
		return (awh::quic2::HandshakeHook::secret(handshake, ::levelFromSSL(level), true, cipher, secret, size) ? 1 : 0);
	}
	/**
	 * @brief Функция добавления исходящих данных хендшейка на уровне шифрования
	 *
	...
	static int32_t addHandshakeData(SSL * ssl, enum ssl_encryption_level_t level, const uint8_t * data, size_t size) noexcept {
		...
		awh::quic2::handshake_t * handshake = reinterpret_cast <awh::quic2::handshake_t *> (SSL_get_app_data(ssl));
		// Добавляем данные в очередь исходящих CRYPTO-данных уровня
		awh::quic2::HandshakeHook::data(handshake, ::levelFromSSL(level), data, size);
		// Выводим положительный результат
		return 1;
	}
	/**
	...
	static int32_t sendAlert(SSL * ssl, [[maybe_unused]] enum ssl_encryption_level_t level, uint8_t alert) noexcept {
		...
		awh::quic2::HandshakeHook::alert(handshake, alert);
		// Выводим положительный результат
		return 1;
	}
};
 ...
 *
 */
awh::quic2::Handshake::Level::Level() noexcept : hasRead(false), hasWrite(false), data{""} {}
 ...
 *
 */
awh::quic2::status_t awh::quic2::Handshake::process() noexcept {
 ...
 *
 */
awh::tls::coder_t::alpn_t awh::quic2::Handshake::alpn() const noexcept {
 ...
 *
 */
string awh::quic2::Handshake::session() const noexcept {
 ...
 *
 */
bool awh::quic2::Handshake::session(string_view session) noexcept {
 ...
 *
 */
bool awh::quic2::Handshake::early() const noexcept {
 ...
 *
 */
bool awh::quic2::Handshake::rejected() const noexcept {
 ...
 *
 */
bool awh::quic2::Handshake::params(const quic2::params::params_t & params) noexcept {
	...
	if(!quic2::params::serialize::encode(this->_params, params, this->_endpoint))
		// Выводим отрицательный результат
		return false;
	// Выполняем сборку контекста ранних данных и выводим результат
	return quic2::params::serialize::early(this->_early, params);
}
/**
 ...
 *
 */
awh::quic2::status_t awh::quic2::Handshake::peer(quic2::params::params_t & params, error_t & error) const noexcept {
	...
	return quic2::params::parser::decode(data, size, sender, params, error);
}
/**
 ...
 *
 */
bool awh::quic2::Handshake::initial(const cid_t & dcid) noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::Handshake::start() noexcept {
 ...
 *
 */
awh::quic2::status_t awh::quic2::Handshake::crypto(const level_t level, const uint8_t * data, const size_t size) noexcept {
 ...
 *
 */
bool awh::quic2::Handshake::pending(const level_t level) const noexcept {
 ...
 *
 */
string awh::quic2::Handshake::data(const level_t level) noexcept {
 ...
 *
 */
const awh::quic2::crypto::keys_t * awh::quic2::Handshake::encryption(const level_t level) const noexcept {
	// Получаем состояние уровня шифрования
 ...
 *
 */
const awh::quic2::crypto::keys_t * awh::quic2::Handshake::decryption(const level_t level) const noexcept {
	// Получаем состояние уровня шифрования
	const auto & item = this->_levels[static_cast <size_t> (level)];
 ...
 *
 */
void awh::quic2::Handshake::install(const level_t level, const crypto::keys_t & read, const crypto::keys_t & write) noexcept {
	// Получаем состояние уровня шифрования
 ...
 *
 */
void awh::quic2::Handshake::discard(const level_t level) noexcept {
	// Получаем состояние уровня шифрования
	auto & item = this->_levels[static_cast <size_t> (level)];
 ...
 *
 */
awh::quic2::Handshake::state_t awh::quic2::Handshake::state() const noexcept {
 ...
 *
 */
awh::quic2::error_t awh::quic2::Handshake::error() const noexcept {
 ...
 *
 */
awh::quic2::Handshake::Handshake(const endpoint_t endpoint, const tls::coder_t::id_t ctx, const tls::coder_t & coder, const log_t * log) noexcept :
 ...
 *
 */
awh::quic2::Handshake::~Handshake() noexcept {
