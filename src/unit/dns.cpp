...
void awh::unit::DNS::response(const event::id_t eid, const uint8_t * data, const size_t size) noexcept {
	...
	try {
		...
		if((data != nullptr) && (size > 0)){
			 ...
			 *
			 * @note Ожидание при этом продолжается с прерванного места: приход данных
			 *       снял срок ожидания ответа, а ответ оказался не наш - чужой, шумовой
			 *       либо запоздалый на вопрос прежний. Не продолжи ожидание здесь, и
			 *       заданный вопрос повис бы навсегда: срок снят, а взводить его заново
			 *       нечем. Продолжается оно именно остатком, а не сроком полным, иначе
			 *       всякий чужой ответ отодвигал бы отказ и поток таких ответов не дал
			 *       бы наступить ему вовсе
			 */
			 ...
			 *
			 * @note Ожидание здесь не продолжается: вопроса с таким номером на учёте
			 *       нет вовсе, а значит срок по нему уже истёк и отказ выдан. Ответ
			 *       этот запоздал, и ждать по нему больше нечего
			 */
