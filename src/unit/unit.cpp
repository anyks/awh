...
void awh::unit::Unit::stop() noexcept {
	...
	try {
		/**
		 * Если цикл событий ещё не запущен, просьбу запоминаем
		 *
		 * @details Обращение вправе отказать прежде входа в цикл - скажем, рассылка не
		 *          уходит в неисправную сеть, - и отказ приходит вызывающему сразу же,
		 *          из той функции, которой обращение и начато. Просьба остановиться
		 *          поступает тогда, когда останавливать ещё нечего, и без этой отметки
		 *          она терялась бы молча, а следующий за нею запуск уходил бы в
		 *          ожидание навсегда: будить его было бы уже некому
		 *
		 * @note Отметка снимается самим запуском, а не здесь: снимать её тут значило бы
		 *       забыть просьбу ровно тем действием, которым она запоминается
		 *
		 */
		if(this->_status != event::status_t::LAUNCHED)
			// Запоминаем просьбу об остановке, поступившую прежде запуска
			this->_stopped = true;
...
void awh::unit::Unit::start() noexcept {
	...
	try {
		/**
		 * Если остановиться просили прежде запуска, в цикл не входим вовсе
		 *
		 * @details Обращение уже завершилось отказом, и ожидать в цикле нечего: событий,
		 *          способных его прервать, не заведено ни одного. Отметка снимается
		 *          здесь же - запуск, следующий за новым обращением, ею связан быть не
		 *          должен
		 *
		 */
		if(this->_stopped){
			// Снимаем отметку просьбы: она обслужена этим самым возвратом
			this->_stopped = false;
			// Выходим, не заводя цикла событий
			return;
		}
