 ...
 */
const std::map <uint64_t, std::unique_ptr <awh::server::scheme::WEB2::options_t>> & awh::server::scheme::WEB2::get() const noexcept {
	// Выводим результат
...
void awh::server::scheme::WEB2::openStream(const int32_t sid, const uint64_t bid) noexcept {
	...
	if((bid > 0) && (sid > -1) && !this->_options.empty()){
		...
		if(i != this->_options.end()){
			// Создаём объект параметров активного клиента
			auto ret = i->second->streams.emplace(sid, std::unique_ptr <stream_t> (new stream_t(i->second->fmk, i->second->log)));
