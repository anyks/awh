 ...
 */
uint32_t awh::Scheme::Broker::id() const noexcept {
	...
	switch(static_cast <uint8_t> (event)){
		...
		case static_cast <uint8_t> (base_t::event_type_t::WRITE): {
			...
			if(this->_callback.is("write"))
				// Выполняем функцию обратного вызова
				this->_callback.call <void (const uint32_t)> ("write", this->_id);
		} break;
	}
}
/**
 ...
 */
SOCKET awh::Scheme::socket(const uint32_t bid) const noexcept {
 ...
 */
uint32_t awh::Scheme::port(const uint32_t bid) const noexcept {
	// Результат работы функции
	uint32_t result = 0;
	// Если идентификатор брокера передан
	if(bid > 0){
 ...
 */
const string & awh::Scheme::ip(const uint32_t bid) const noexcept {
	// Результат работы функции
 ...
 */
const string & awh::Scheme::mac(const uint32_t bid) const noexcept {
	// Результат работы функции
	static const string result = "";
	// Если идентификатор брокера передан
	if(bid > 0){
