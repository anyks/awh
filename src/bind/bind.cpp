...
// Если - это Windows
	 ...
	 */
	void awh::Bind::winSocketInit() const noexcept {
	 ...
	 */
	void awh::Bind::winSocketClean() const noexcept {
		// Очищаем сетевой контекст
		WSACleanup();
 ...
 */
void awh::Bind::delay(const size_t seconds) const noexcept {
 ...
 */
void awh::Bind::read(struct bufferevent * bev, void * ctx) noexcept {
	...
	if((bev != nullptr) && (ctx != nullptr)){
		...
		if(wrk->readFn != nullptr){
			...
			if((size > 0) && (wrk != nullptr)){
				...
				wrk->readFn(wrk->buffer, size, wrk->wid, const_cast <bind_t *> (wrk->bind));
 ...
 * @param ctx передаваемый контекст
 */
void awh::Bind::write(struct bufferevent * bev, void * ctx) noexcept {
	// Если подключение не передано
	if((bev != nullptr) && (ctx != nullptr)){
		// Получаем объект подключения
		worker_t * wrk = reinterpret_cast <worker_t *> (ctx);
		...
		if(wrk->writeFn != nullptr){
			...
			if((size > 0) && (wrk != nullptr)){
				...
				// Выводим функцию обратного вызова
				wrk->writeFn(wrk->buffer, size, wrk->wid, const_cast <bind_t *> (wrk->bind));
				// Заполняем нулями буфер полученных данных
				memset((void *) wrk->buffer, 0, BUFFER_CHUNK);
			}
			// Удаляем данные из буфера
 ...
 * @param ctx    передаваемый контекст
 */
void awh::Bind::event(struct bufferevent * bev, const short events, void * ctx) noexcept {
	// Если подключение не передано
	if((ctx != nullptr) && (bev != nullptr)){
		// Получаем объект подключения
		worker_t * wrk = reinterpret_cast <worker_t *> (ctx);
		// Если фреймворк получен
		if(wrk->bind->fmk != nullptr){
			...
			if(events & BEV_EVENT_CONNECTED){
				...
				wrk->bind->log->print("connect client to server [%s:%d]", log_t::flag_t::INFO, wrk->url.ip.c_str(), wrk->url.port);
				// Если функция обратного вызова запуска установлена
				if(wrk->openFn != nullptr) wrk->openFn(wrk->wid, const_cast <bind_t *> (wrk->bind));
			// Если это ошибка или завершение работы
			} else if(events & (BEV_EVENT_ERROR | BEV_EVENT_EOF | BEV_EVENT_TIMEOUT)) {
				// Если это ошибка
				if(events & BEV_EVENT_ERROR)
					// Выводим в лог сообщение
					wrk->bind->log->print("closing server [%s:%d] error: %s", log_t::flag_t::WARNING, wrk->url.ip.c_str(), wrk->url.port, evutil_socket_error_to_string(EVUTIL_SOCKET_ERROR()));
				// Если - это таймаут, выводим сообщение в лог
				else if(events & BEV_EVENT_TIMEOUT) wrk->bind->log->print("timeout server [%s:%d]", log_t::flag_t::WARNING, wrk->url.ip.c_str(), wrk->url.port);
				...
				if(wrk->alive && (wrk->attempts.first <= wrk->attempts.second)){
					...
					const_cast <bind_t *> (wrk->bind)->close(wrk);
					// Увеличиваем колпичество попыток
					wrk->attempts.first++;
					// Выдерживаем паузу в 3 секунды
					const_cast <bind_t *> (wrk->bind)->delay(3);
					// Выполняем новое подключение
					const_cast <bind_t *> (wrk->bind)->connect(wrk);
				// Если автоматическое подключение выполнять не нужно
				} else const_cast <bind_t *> (wrk->bind)->close(wrk);
			}
		}
	}
}
 ...
 */
const bool awh::Bind::connect(const worker_t * worker) noexcept {
	...
	if((this->fmk != nullptr) && (worker != nullptr)){
		...
		auto socket = this->socket(wrk->url.ip, wrk->url.port, this->net.family);
 ...
 */
const awh::Bind::socket_t awh::Bind::socket(const string & ip, const u_int port, const int family) const noexcept {
	// Результат работы функции
	socket_t result;
 ...
 */
void awh::Bind::close(const worker_t * worker) noexcept {
	// Если воркер получен
	if(worker != nullptr){
		// Получаем объект воркера
		worker_t * wrk = const_cast <worker_t *> (worker);
		...
		if(wrk->fd > -1){
			...
			// Выводим функцию обратного вызова
			if(wrk->closeFn != nullptr) wrk->closeFn(wrk->wid, this);
		}
	}
}
/**
			...
			if(!this->workers.empty()){
				...
				for(auto & worker : this->workers){
					...
					if(worker.second->startFn != nullptr)
						...
						worker.second->startFn(worker.first, this);
				}
			}
 ...
 */
size_t awh::Bind::add(const worker_t * worker) noexcept {
	// Результат работы функции
	size_t result = 0;
	// Если воркер передан и URL адрес существует
	if((worker != nullptr) && !worker->url.empty()){
		// Получаем объект воркера
		worker_t * wrk = const_cast <worker_t *> (worker);
 ...
 */
void awh::Bind::closeAll() noexcept {
 ...
 */
void awh::Bind::removeAll() noexcept {
	// Выполняем удаление всех воркеров
	this->workers.clear();
}
/**
 ...
 */
void awh::Bind::open(const size_t wid) noexcept {
 ...
 * @param wid идентификатор воркера
 */
void awh::Bind::close(const size_t wid) noexcept {
	// Если идентификатор воркера передан
	if(wid > 0){
		// Выполняем поиск воркера
		auto it = this->workers.find(wid);
 ...
 * @param wid идентификатор воркера
 */
void awh::Bind::remove(const size_t wid) noexcept {
	// Если идентификатор воркера передан
	if(wid > 0){
		// Выполняем поиск воркера
		auto it = this->workers.find(wid);
 ...
 * @param wid    идентификатор воркера
 */
void awh::Bind::write(const char * buffer, const size_t size, const size_t wid) noexcept {
 ...
 */
void awh::Bind::setVerifySSL(const bool mode) noexcept {
 ...
 * @param family тип протокола интернета AF_INET или AF_INET6
 */
void awh::Bind::setFamily(const int family) noexcept {
 ...
 */
void awh::Bind::setUserAgent(const string & userAgent) noexcept {
 ...
 */
void awh::Bind::setCA(const string & cafile, const string & capath) noexcept {
 ...
 */
void awh::Bind::setServ(const string & id, const string & name, const string & ver) noexcept {
 ...
 * @param family тип протокола интернета AF_INET или AF_INET6
 */
void awh::Bind::setNet(const vector <string> & ip, const vector <string> & ns, const int family) noexcept {
	// Устанавливаем тип активного интернет-подключения
	this->net.family = family;
 ...
 */
awh::Bind::Bind(const fmk_t * fmk, const log_t * log) noexcept {
	try {
