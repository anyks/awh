	...
	try {
		 ...
		 */
			...
			if(this->_socks[0] != INVALID_SOCKET){
				 ...
				 */
				while(size < 8){
					// Выполняем чтение данных (читаем только недостающую часть, чтобы не выйти за границу буфера)
					bytes = static_cast <int8_t> (::read(this->_socks[0], buffer + size, 8 - size));
					// Если данные прочитанны
					if(bytes > 0)
						// Увеличиваем количество прочитанных данных
						size += bytes;
