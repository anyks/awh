 ...
 */
bool awh::Base::mode(const uint32_t id, const event_type_t type, const event_mode_t mode) noexcept {
	/**
	 * Выполняем обработку ошибки
	 */
	try {
		...
		if(i != this->_peers.end()){
			...
			switch(static_cast <uint8_t> (type)){
				...
				case static_cast <uint8_t> (event_type_t::READ): {
					...
					switch(static_cast <uint8_t> (mode)){
						...
						case static_cast <uint8_t> (event_mode_t::ENABLED):
							...
							i->second.events |= react_t::AWH_READ;
						break;
				...
				case static_cast <uint8_t> (event_type_t::WRITE): {
					...
					switch(static_cast <uint8_t> (mode)){
						...
						case static_cast <uint8_t> (event_mode_t::ENABLED):
							...
							i->second.events |= react_t::AWH_WRITE;
						break;
						// Если необходимо деактивировать событие
						case static_cast <uint8_t> (event_mode_t::DISABLED):
				...
				case static_cast <uint8_t> (event_type_t::TIMER): {
					...
					switch(static_cast <uint8_t> (mode)){
						...
						case static_cast <uint8_t> (event_mode_t::ENABLED):
							...
							i->second.events |= react_t::AWH_TIMER;
						break;
						// Если необходимо деактивировать событие
						case static_cast <uint8_t> (event_mode_t::DISABLED):
				...
				case static_cast <uint8_t> (event_type_t::INTERVAL): {
					/**
					 * Определяем флаг режима работы
					 */
					switch(static_cast <uint8_t> (mode)){
						...
						case static_cast <uint8_t> (event_mode_t::ENABLED):
							...
							i->second.events |= react_t::AWH_INTERVAL;
						break;
						// Если необходимо деактивировать событие
						case static_cast <uint8_t> (event_mode_t::DISABLED):
	 ...
	 */
		...
		#if DEBUG_MODE
			// Выводим сообщение об ошибке
			this->_log->debug("%s", __PRETTY_FUNCTION__, std::make_tuple(id, static_cast <uint16_t> (type), static_cast <uint16_t> (mode)), log_t::flag_t::CRITICAL, error.what());
		/**
		* Если режим отладки не включён
		*/
		#else
 ...
 */
uint32_t awh::Base::emplace(const SOCKET sock, const callback_t & callback, const uint32_t delay) noexcept {
...
void awh::Base::start() noexcept {
	...
	if(!this->_launched){
		...
		if((this->_works = this->_react.init())){
			/**
					...
					for(uint32_t i = 0; i < count; i++){
						...
						if(j != this->_peers.end()){
							...
							// Если событие принадлежит таймеру
								...
								// Выполняем функцию обратного вызова
								j->second.callback(j->second.sock, event_type_t::TIMER);
							// Если событие принадлежит интервалу
							else if(poller.events & react_t::AWH_INTERVAL)
								// Выполняем функцию обратного вызова
								j->second.callback(j->second.sock, event_type_t::INTERVAL);
							...
							// Если мы получили любое другое событие
								...
								if(poller.events & react_t::AWH_READ){
									...
									// Выполняем функцию обратного вызова
									callback(j->second.sock, event_type_t::READ);
								}
								...
								if(poller.events & react_t::AWH_WRITE){
									...
									if(i != this->_peers.end()){
										...
										// Выполняем функцию обратного вызова
										callback(i->second.sock, event_type_t::WRITE);
									}
								}
								...
								if((poller.events & react_t::AWH_CLOSE) || (poller.events & react_t::AWH_ERROR)){
									...
									if(i != this->_peers.end()){
										...
										callback = i->second.callback;
										// Выполняем функцию обратного вызова
										callback(i->second.sock, event_type_t::CLOSE);
									}
								}
							}
						}
