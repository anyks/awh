...
/**
 * @brief Инкапсулируем функции разбора чисел в пространство имён
 *
 */
namespace {
	/**
	 * @brief Шаблон функции разбора целого числа
	 *
	 * @tparam T тип данных с которым работает функция
	 */
	template <typename T>
	/**
	 * @brief Функция разбора целого числа из куска текста
	 *
	 * @details Запись разбирается целиком: запись, числом не являющаяся, равно как и
	 *          выходящая за пределы типа, выдаётся нулём. Разбор не зависит от
	 *          установленной локали
	 *
	 * @param value кусок текста с записью числа
	 * @return      разобранное число либо ноль
	 */
	T toNumber(string_view value) noexcept {
		// Переменная результата
		T result = static_cast <T> (0);
		// Если запись числа передана
		if(!value.empty()){
			// Выполняем разбор записи числа
			const auto answer = std::from_chars(value.data(), value.data() + value.size(), result);
			// Если разбор завершился ошибкой либо запись разобрана не целиком
	...
	/**
	 * @brief Функция разбора десятичной дроби из куска текста
	 *
	 * @details Дробная часть отделяется только точкой, разбор не зависит от
	 *          установленной локали. Запись, числом не являющаяся, выдаётся нулём
	 *
	 * @param value кусок текста с записью числа
	 * @return      разобранное число либо ноль
	 */
	double toDecimal(string_view value) noexcept {
		// Позиция в записи числа
		size_t pos = 0;
		// Признак отрицательного числа
		bool negative = false;
		// Если запись числа пустая
		if(value.empty())
			// Выводим нулевое значение
			return 0.;
		// Если запись начинается со знака числа
		if((value.front() == '-') || (value.front() == '+')){
			// Запоминаем знак числа
			negative = (value.front() == '-');
			// Пропускаем знак числа
			pos++;
		}
		// Целая часть и дробная часть числа
		double integer = 0., fraction = 0., scale = 1.;
		// Количество прочитанных цифр
		size_t count = 0;
		// Выполняем чтение целой части числа
		while((pos < value.size()) && (value[pos] >= '0') && (value[pos] <= '9')){
			// Накапливаем целую часть числа
			integer = ((integer * 10.) + static_cast <double> (value[pos] - '0'));
			// Увеличиваем количество прочитанных цифр
			count++;
			// Переходим к следующему символу
			pos++;
		}
		// Если найден разделитель дробной части
		if((pos < value.size()) && (value[pos] == '.')){
			// Пропускаем разделитель дробной части
			pos++;
			// Выполняем чтение дробной части числа
			while((pos < value.size()) && (value[pos] >= '0') && (value[pos] <= '9')){
				// Уменьшаем вес очередного разряда
				scale /= 10.;
				// Накапливаем дробную часть числа
				fraction += (static_cast <double> (value[pos] - '0') * scale);
				// Увеличиваем количество прочитанных цифр
				count++;
				// Переходим к следующему символу
				pos++;
		...
		// Если цифр не найдено либо запись разобрана не целиком
		if((count == 0) || (pos != value.size()))
			// Выводим нулевое значение
			return 0.;
		// Выводим разобранное число
		return (negative ? -(integer + fraction) : (integer + fraction));
	}
};
 ...
 *
 * @details Смещение зоны окружения зависит от самого момента: у зон с летним временем
 *          зимняя дата и летняя пишутся с разным смещением. Формирование записи по
 *          штампу времени прежде брало смещение текущего мгновения, и зимняя дата,
 *          записанная летом, уезжала на час, а до того - смещение стандартное, и на
 *          час уезжала уже летняя
 *
 * @param date штамп времени в миллисекундах
 * @return     смещение временной зоны окружения в секундах
 *
 */
int32_t awh::Chrono::zoneOf(const uint64_t date) const noexcept {
	// Переменная результата
	int32_t result = 0;
	/**
	 * Выполняем отлов ошибок
	 */
	try {
		// Получаем смещение зоны окружения текущего мгновения на случай отказа разложения
		result = this->getTimeZone(storage_t::GLOBAL);
		// Получаем проверяемый момент времени с точностью до секунды
		const time_t stamp = static_cast <time_t> (date / 1000);
		 ...
		 */
			...
			// Устанавливаем временную зону по умолчанию
			::_tzset();
			// Если момент времени разложен по местной временной зоне
			if(::localtime_s(&tm, &stamp) == 0){
				...
				// Если на этот момент действует летнее время
				if(tm.tm_isdst > 0){
 ...
 *
 * @details Поля записи заданы местным временем окружения, а смещение его зависит от
 *          самого момента, который из них ещё только собирается. Смещение подбирается
 *          так же, как это делает mktime: пробный момент берётся по полям, как если
 *          бы они были всемирным временем, и уточняется смещением, найденным на нём.
 *          Внутри часа перехода, где местное время повторяется либо отсутствует,
 *          ответ условен, но от текущего мгновения не зависит
 *
 * @param dt объект даты, поля которого записаны в зоне окружения
 * @return   смещение временной зоны окружения в секундах
 *
 */
int32_t awh::Chrono::zoneOf(const dt_t & dt) const noexcept {
	// Копия объекта даты для пробной сборки штампа времени
	dt_t probe = dt;
	// Собираем пробный момент, принимая поля записи за всемирное время
	probe.offset = 0;
	// Получаем смещение зоны окружения на пробный момент
	const int32_t first = this->zoneOf(this->makeStamp(probe));
	// Собираем момент по найденному смещению
	probe.offset = first;
	// Получаем смещение зоны окружения на уточнённый момент
	const int32_t second = this->zoneOf(this->makeStamp(probe));
	// Если смещение подтвердилось
	if(second == first)
		// Выводим найденное смещение
		return first;
	// Собираем момент по уточнённому смещению
	probe.offset = second;
	// Если уточнённое смещение подтвердилось, выводим его, иначе запись попала в разрыв перехода
	return ((this->zoneOf(this->makeStamp(probe)) == second) ? second : first);
}
/**
							...
							if(match[j].end > match[j].begin){
								...
								switch(static_cast <uint8_t> (fmt)){
									...
									case static_cast <uint8_t> (format_t::w): {
										...
										const uint8_t day = ::toNumber <uint8_t> (::fragment(text.data() + pos, match[j]));
										/**
									...
									case static_cast <uint8_t> (format_t::W):
										...
										dt.weeks = ::toNumber <uint8_t> (::fragment(text.data() + pos, match[j]));
									break;
									...
									case static_cast <uint8_t> (format_t::j): {
										...
										const uint16_t number = ::toNumber <uint16_t> (::fragment(text.data() + pos, match[j]));
										/**
									...
									case static_cast <uint8_t> (format_t::u):
										// Устанавливаем номер дня недели
										dt.day = ::toNumber <uint8_t> (::fragment(text.data() + pos, match[j]));
									break;
									// Если формат получен как %C
									case static_cast <uint8_t> (format_t::C):
										/**
										...
										dt.year = ::toNumber <uint16_t> (::fragment(text.data() + pos, match[j]));
									break;
									// Если формат получен как %y
									case static_cast <uint8_t> (format_t::y): {
										// Получаем значение указанного года
										const uint16_t num = ::toNumber <uint16_t> (::fragment(text.data() + pos, match[j]));
									...
									case static_cast <uint8_t> (format_t::d):
										...
										dt.date = ::toNumber <uint8_t> (::fragment(text.data() + pos, match[j]));
									break;
									// Если формат получен как %m
									case static_cast <uint8_t> (format_t::m):
										// Получаем значение номера месяца
										dt.month = ::toNumber <uint8_t> (::fragment(text.data() + pos, match[j]));
									break;
									// Если формат получен как %I
									case static_cast <uint8_t> (format_t::I):
										// Устанавливаем полученный час времени
										dt.hour = ::toNumber <uint8_t> (::fragment(text.data() + pos, match[j]));
									break;
									// Если формат получен как %H
									case static_cast <uint8_t> (format_t::H):
										// Устанавливаем полученный час времени
									...
									case static_cast <uint8_t> (format_t::M):
										...
										dt.minutes = ::toNumber <uint8_t> (::fragment(text.data() + pos, match[j]));
									break;
									// Если формат получен как %s
									case static_cast <uint8_t> (format_t::s):
									...
									case static_cast <uint8_t> (format_t::S):
										...
										dt.seconds = ::toNumber <uint8_t> (::fragment(text.data() + pos, match[j]));
									break;
									// Если формат получен как %a
									case static_cast <uint8_t> (format_t::a): {
...
double awh::Chrono::seconds(const string & value) const noexcept {
	...
	if(!text.empty()){
		...
		try {
			...
			const matches_t match = ::parseSeconds(text.data(), text.size());
			// Если совпадение получено
			if(!match.empty()){
				// Обрабатываем полученные группы совпадения
				{
						...
						if(match[j].end > match[j].begin){
							...
							switch(j){
								...
								case 1:
									...
									seconds = ::toDecimal(::fragment(text.data(), match[j]));
								break;
								// Если мы получили размерность числа
								case 2: {
									// Получаем обозначение размерности числа
									label.assign(text.data() + match[j].begin, match[j].end - match[j].begin);
									// Если мы получили секунды
									if(label.front() == 's')
		...
		try {
			...
			switch(static_cast <uint8_t> (unit)){
				...
				case static_cast <uint8_t> (unit_t::DAY): {
					...
					if(text){
						...
						if(!day->empty()){
							...
							if(this->_fmk->is(string(* day), fmk_t::check_t::NUMBER)){
								// День для установки
								const uint8_t num = ::toNumber <uint8_t> (string_view{day->c_str(), day->size()});
								// Если номер дня недели передан
								if((num > 0) && (num < 8))
									// Устанавливаем номер дня недели
									this->_dt.day = num;
				...
				case static_cast <uint8_t> (unit_t::DATE): {
					...
					if(text){
						// Дата для установки
						const uint8_t date = ::toNumber <uint8_t> (string_view{
				...
				case static_cast <uint8_t> (unit_t::YEAR): {
					...
					if(text){
						// Год для установки
						const uint16_t year = ::toNumber <uint16_t> (string_view{
							reinterpret_cast <const string *> (buffer)->c_str(),
							reinterpret_cast <const string *> (buffer)->length()
						});
						// Если год передан
				...
				case static_cast <uint8_t> (unit_t::HOUR): {
					...
					if(text){
						...
						const uint8_t hour = ::toNumber <uint8_t> (string_view{
							reinterpret_cast <const string *> (buffer)->c_str(),
							reinterpret_cast <const string *> (buffer)->length()
						});
				...
				case static_cast <uint8_t> (unit_t::DAYS): {
					...
					if(text){
						...
						const uint16_t days = ::toNumber <uint16_t> (string_view{
							reinterpret_cast <const string *> (buffer)->c_str(),
							reinterpret_cast <const string *> (buffer)->length()
						});
						/**
				...
				case static_cast <uint8_t> (unit_t::MONTH): {
					...
					if(text){
						...
						if(!month->empty()){
							...
							if(this->_fmk->is(string(* month), fmk_t::check_t::NUMBER)){
								// Месяц для установки
								const uint8_t num = ::toNumber <uint8_t> (string_view{month->c_str(), month->length()});
								// Если месяц передан
								if((num > 0) && (num < 13))
									// Устанавливаем месяц
									this->_dt.month = num;
				...
				case static_cast <uint8_t> (unit_t::WEEKS): {
					...
					if(text){
						...
						const uint8_t weeks = ::toNumber <uint8_t> (string_view{
							reinterpret_cast <const string *> (buffer)->c_str(),
							reinterpret_cast <const string *> (buffer)->length()
						});
				...
				case static_cast <uint8_t> (unit_t::OFFSET): {
					...
					if(text){
						...
						if((accepted = this->_fmk->is(string(* value), fmk_t::check_t::NUMBER)))
							// Получаем смещение временной зоны в секундах относительно UTC
							offset = ::toNumber <int32_t> (string_view{value->c_str(), value->length()});
					// Если данные переданы в виде числа
					} else {
						// Если устанавливаемые данные достаточны
						if(size >= sizeof(int64_t)){
				...
				case static_cast <uint8_t> (unit_t::MINUTES): {
					...
					if(text){
						...
						const uint8_t minutes = ::toNumber <uint8_t> (string_view{
							reinterpret_cast <const string *> (buffer)->c_str(),
							reinterpret_cast <const string *> (buffer)->length()
						});
				...
				case static_cast <uint8_t> (unit_t::SECONDS): {
					...
					if(text){
						...
						const uint8_t seconds = ::toNumber <uint8_t> (string_view{
							reinterpret_cast <const string *> (buffer)->c_str(),
							reinterpret_cast <const string *> (buffer)->length()
						});
						// Если количество секунд передано
				...
				case static_cast <uint8_t> (unit_t::NANOSECONDS): {
					...
					if(text){
						...
						const uint64_t nanoseconds = ::toNumber <uint64_t> (string_view{
							reinterpret_cast <const string *> (buffer)->c_str(),
							reinterpret_cast <const string *> (buffer)->length()
						});
						/**
				...
				case static_cast <uint8_t> (unit_t::MICROSECONDS): {
					...
					if(text){
						...
						const uint64_t microseconds = ::toNumber <uint64_t> (string_view{
							reinterpret_cast <const string *> (buffer)->c_str(),
							reinterpret_cast <const string *> (buffer)->length()
						});
						/**
				...
				case static_cast <uint8_t> (unit_t::MILLISECONDS): {
					...
					if(text){
						...
						const uint32_t milliseconds = ::toNumber <uint32_t> (string_view{
							reinterpret_cast <const string *> (buffer)->c_str(),
							reinterpret_cast <const string *> (buffer)->length()
						});
		...
		try {
			...
			if(!match.empty()){
				...
				// Если временная зона не найдена и название временной зоны получено
					...
					if(this->_fmk->is(string(name), fmk_t::check_t::NUMBER)){
						// Получаем количество часов смещения, заданное самим названием зоны
						const int64_t hours = ::toNumber <int64_t> (name);
						/**
	...
	try {
		...
		switch(static_cast <uint8_t> (storage)){
			...
			case static_cast <uint8_t> (storage_t::GLOBAL): {
				 ...
				 *       разложенного времени - тем сохраняется то же свойство, ради
				 *       которого выбрано поле tm_gmtoff: смещение отвечает мигу, а не
				 *       одному лишь стандартному положению зоны
				 *
				 ...
				 */
					...
					// Получаем значение текущего времени
					const time_t time = std::time(nullptr);
					// Выполняем разложение текущего времени по местной временной зоне
					::localtime_r(&time, &tm);
					// Получаем смещение временной зоны в секундах
					result = static_cast <int32_t> (-(tm.tm_isdst > 0 ? altzone : timezone));
				/**
...
void awh::Chrono::clearTimeZones() noexcept {
	// Выполняем блокировку потока
	const lock_guard <std::recursive_mutex> lock(this->_mtx.tz);
	/**
	 * Выполняем отлов ошибок
	 */
	try {
...
uint64_t awh::Chrono::parse(string_view date, string_view format, const storage_t storage, bool * valid) noexcept {
	...
	if(!date.empty() && !format.empty()){
		...
		uint16_t century = 0;
		// Признак записи, читаемой в зоне окружения
		...
		switch(static_cast <uint8_t> (storage)){
			...
			case static_cast <uint8_t> (storage_t::LOCAL): {
				...
				if(!flags[0]){
					...
					// Если временная зона объекту не выставлена
						...
						// Запись читается в зоне окружения
						environment = true;
					}
				/**
				...
				/**
				 * Смещение зоны окружения берётся на момент самой записи, а не текущий:
				 * у зон с летним временем зимняя запись, разобранная летом, иначе уезжала
				 * на час. Подбирается оно после подстановки полей - момент ими и задан
				 */
				if(environment)
					// Устанавливаем смещение зоны окружения на момент записи
					this->_dt.offset = offset = this->zoneOf(this->_dt);
				// Если признак пригодности записи запрошен
				if(valid != nullptr){
					/**
				...
				if(!flags[1] && (this->_yearRollback > 0) &&
				   (result > (current + (static_cast <uint64_t> (this->_yearRollback) * 1000)))){
					...
					// Устанавливаем флаг високосного года
					this->_dt.leap = this->leap(this->_dt.year);
					// Если запись читается в зоне окружения
					if(environment)
						// Устанавливаем смещение зоны окружения на момент новой даты
						this->_dt.offset = this->zoneOf(this->_dt);
					/**
			...
			case static_cast <uint8_t> (storage_t::GLOBAL): {
				...
				/**
				 * Смещение зоны окружения берётся на момент самой записи, а не текущий,
				 * по тем же основаниям, что и в местном хранилище
				 */
				if(environment)
					// Устанавливаем смещение зоны окружения на момент записи
					dt.offset = offset = this->zoneOf(dt);
				// Если признак пригодности записи запрошен
				if(valid != nullptr){
					/**
					 * Номер дня в году проверяется отдельно: сборка штампа времени опирается
 ...
 *
 */
uint64_t awh::Chrono::parse(const string & date, const string & format, bool & valid, const storage_t storage) noexcept {
...
string awh::Chrono::format(const uint64_t date, const standard_t standard) const noexcept {
	...
	if(standard == standard_t::ISO8601){
		// Получаем смещение временной зоны окружения на момент самой даты
		const int32_t offset = this->zoneOf(date);
 ...
 *
 */
uint64_t awh::Chrono::parse(const string & date, const standard_t standard, const storage_t storage) noexcept {
 ...
 *
 */
uint64_t awh::Chrono::parse(const string & date, const standard_t standard, bool & valid, const storage_t storage) noexcept {
	// Переменная результата
	uint64_t result = 0;
 ...
 *
 */
bool awh::Chrono::validate(const string & date, const standard_t standard) noexcept {
	/**
 ...
 *
 */
bool awh::Chrono::validate(const string & date, const string & format) noexcept {
 ...
 *
 */
bool awh::Chrono::validateTimeZone(const string & zone) const noexcept {
...
bool awh::Chrono::validateSeconds(const string & value) const noexcept {
	...
	try {
		...
		const string number(text.data() + match[1].begin, static_cast <size_t> (match[1].end - match[1].begin));
		/**
...
string awh::Chrono::format(const uint64_t date, const string & format) const noexcept {
	...
	if(!format.empty()){
		...
		dt_t dt;
		/**
		 * Смещение зоны окружения берётся на момент самой даты, а не текущий: зимняя
		 * дата, записанная летом, иначе уезжала на час
		 */
		this->makeDate(date, this->zoneOf(date), dt);
		...
		switch(static_cast <uint8_t> (storage)){
			...
			case static_cast <uint8_t> (storage_t::LOCAL): {
				...
				if((dt.offset == 0) && (dt.zone == zone_t::NONE))
					// Перекладываем объект даты во временную зону окружения на его момент
					this->shiftDate(dt, this->zoneOf(this->makeStamp(dt)));
				// Выполняем формирование формата даты
				return this->format(dt, format);
			}
			// Если хранилище глобальное
...
string awh::Chrono::strip(const string & date, const string & format1, const string & format2, const storage_t storage) const noexcept {
	...
	if(!date.empty() && !format1.empty()){
		...
		try {
			...
			bool valid = false;
			/**
			 * Метод в AWH 4 объявлен константным, а разбор с хранилищем storage_t::LOCAL
			 * пишет в объект, ровно как это делает parse. Подпись открытого метода
			 * менять нельзя, поэтому снятие константности сохранено таким, каким оно
			 * было: поведение определено, пока сам объект константным не объявлен
			 */
...
awh::Chrono::~Chrono() noexcept {
	// Регулярные выражения модулю больше не нужны, освобождать нечего
}
