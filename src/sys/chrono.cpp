	...
	inline bool matchLiterals(string_view text, const char * format) noexcept {
		...
		for(size_t i = 0; format[i] != '\0'; i++){
			...
			if((letter == ' ') || (letter == '\t'))
				...
				continue;
			/**
			 * Разделители сличаются без различения строчных и прописных: RFC 3339
			 * в примечании к §5.6 строчные разделители допускает наравне с прописными,
			 * и прежде запись «2003-10-11t22:14:15Z» отвергалась целиком, тогда как
			 * строчная «z» того же стандарта принималась - переменная зоны регистра
			 * не различает. Отвергать одну половину договора, принимая другую, нельзя
			 */
			/**
			 * Отыскиваем разделитель образца в записи, начиная с последнего найденного
			 */
			while((pos < text.length()) && (::toLowerChar(text[pos]) != ::toLowerChar(letter)))
