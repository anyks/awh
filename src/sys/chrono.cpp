...
uint64_t awh::Chrono::end(const uint64_t stamp, const type_t type) const noexcept {
	 ...
	 * периода вычисляется как начало следующего без единицы, и для последнего
	 * года, месяца, недели и суток начало следующего периода за предел выходило,
	 * отчего конец 9999 года выдавался мгновением, следующим за последним
	 */
	// Возвращаем результат
			...
			switch(static_cast <uint8_t> (unit)){
				...
				case static_cast <uint8_t> (unit_t::OFFSET): {
					...
					if(text)
						...
						this->shiftDate(this->_dt, ::clampZone(this->_fmk->atoi <int32_t> (
							reinterpret_cast <const string *> (buffer)->c_str(),
							reinterpret_cast <const string *> (buffer)->length()
						)));
					// Если данные переданы в виде числа
					...
					if(mode){
						...
						switch(letter){
							...
							case 'Z': {
								...
								else result.append(this->format(dt.zone, this->makeStamp(dt)));
							} break;
