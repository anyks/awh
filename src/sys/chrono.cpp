...
namespace {
	 ...
	 * @param caseless признак сличения разделителей без различения регистра
	 * @return        признак того, что разделители образца в записи присутствуют
	 *
	 */
	inline bool matchLiterals(string_view text, const char * format, const bool caseless = false) noexcept {
		...
		for(size_t i = 0; format[i] != '\0'; i++){
			...
			if((letter == ' ') || (letter == '\t'))
				...
				continue;
			/**
			 * Регистр разделителей сличается всегда, кроме записи по RFC 3339: приравнивать
			 * строчные к прописным общим правилом нельзя, ибо в самом модуле регистр значение
			 * несёт - переменная %Z даёт обозначение зоны, а %z её смещение числом. Строчные
			 * разделители разрешает один лишь RFC 3339 примечанием к §5.6, и разрешение это
			 * ему одному и оставлено: ISO 8601, записи с которым разбираются теми же образцами,
			 * строчных не допускает. Прежде запись «2003-10-11t22:14:15Z» отвергалась целиком,
			 * тогда как строчная «z» того же стандарта принималась - переменная зоны регистра
			 * не различает, - и одна половина договора отвергала то, что принимала другая
			 */
			const char sample = (caseless ? ::toLowerChar(letter) : letter);
			/**
			 * Отыскиваем разделитель образца в записи, начиная с последнего найденного
			 */
			while((pos < text.length()) && ((caseless ? ::toLowerChar(text[pos]) : text[pos]) != sample))
...
int32_t awh::Chrono::getTimeZone(const storage_t storage) const noexcept {
	...
	try {
		...
		switch(static_cast <uint8_t> (storage)){
			...
			case static_cast <uint8_t> (storage_t::GLOBAL):
				/**
				 * Смещение зоны окружения на текущий миг - частный случай смещения на
				 * заданный миг: спрос у системы и его запоминание ведутся в одном месте
				 */
				// Получаем смещение временной зоны окружения на текущий миг
				result = this->environZone(std::time(nullptr));
			break;
		}
	/**
	 * Если возникает ошибка
...
/**
 * Смещение временной зоны окружения запоминается в памяти потока, общей для
 * всех объектов модуля: величина эта принадлежит процессу, а не объекту, и память
 * потока снимает состязание за неё без всякой блокировки
 */
namespace {
	/**
	 * @brief Структура запомненных смещений временной зоны окружения
	 *
	 * @details Смещение запоминается по самому мигу, о котором спрошено, а не по
	 *          текущему: прежде единственная запись держала смещение «сейчас» и
	 *          прикладывала его ко всякой дате. Записей четыре, чтобы разбор, которому
	 *          смещение нужно по обе стороны местного времени, не вытеснял смещение,
	 *          нужное формированию. Запомненное годно в пределах текущей секунды и при
	 *          неизменной переменной окружения TZ: правка переменной ловится её
	 *          сличением и применяется немедленно, а смена зоны системы без неё - со
	 *          следующей секундой
	 *
	 */
	typedef struct AmbientCache {
		// Количество запоминаемых смещений временной зоны окружения
		static constexpr uint8_t SIZE = 4;
		// Миги, на которые сняты запомненные смещения
		time_t stamps[SIZE];
		// Запомненные смещения временной зоны окружения
		int32_t offsets[SIZE];
		// Количество запомненных смещений
		uint8_t count;
		// Место, в которое запоминается следующее смещение
		uint8_t next;
		// Секунда, в которую запомненные смещения сняты
		time_t second;
		// Значение переменной окружения TZ, при котором смещения сняты
		string name;
		/**
		 * @brief Конструктор
		 *
		 */
		AmbientCache() noexcept : stamps{}, offsets{}, count(0), next(0), second(0), name{} {}
	} ambient_cache_t;
	// Запомненные смещения временной зоны окружения текущего потока
	thread_local ambient_cache_t ambientCache;
	/**
	 * @brief Функция проверки годности запомненных смещений временной зоны окружения
	 *
	 * @details Проверка выполняется один раз на обращение к модулю, а не на каждый
	 *          спрос смещения: разбору записи без зоны смещение нужно по нескольким
	 *          мигам, и сличение переменной окружения с текущей секундой на каждом
	 *          из них стоило больше самого поиска
	 *
	 */
	void ambientFresh() noexcept {
		/**
		 * Выполняем отлов ошибок
		 */
		try {
			...
			// Приводим отсутствие переменной окружения к пустому значению
			const char * name = ((env != nullptr) ? env : "");
			// Если запомненные смещения сняты в эту же секунду и при той же зоне
			if((ambientCache.second == now) && (ambientCache.name.compare(name) == 0))
				// Запомненные смещения годны
				return;
			/**
			 * Для операционной системы MS Windows
			 */
			 ...
			 */
				...
				::tzset();
			#endif
			// Выполняем сброс запомненных смещений
			ambientCache.count = ambientCache.next = 0;
			// Запоминаем секунду, в которую снимаются смещения
			ambientCache.second = now;
			// Запоминаем значение переменной окружения, задающей временную зону
			ambientCache.name.assign(name);
		/**
		 * Если возникает ошибка
		 */
			...
			// Выполняем сброс запомненных смещений
			ambientCache.count = ambientCache.next = 0;
			// Сбрасываем секунду, чтобы следующее обращение повторило проверку
			ambientCache.second = 0;
		}
	}
	/**
	 * @brief Функция спроса у системы смещения временной зоны окружения на заданный миг
	 *
	 * @param stamp миг в секундах от начала эпохи
	 * @return      смещение временной зоны окружения в секундах
	 *
	 */
	int32_t systemZone(const time_t stamp) noexcept {
		// Переменная результата
		int32_t result = 0;
		/**
		 * Для операционной системы MS Windows
		 */
			...
			// Получаем стандартное смещение временной зоны окружения в секундах
			result = static_cast <int32_t> (offset * -1);
			// Если летнее время в зоне окружения бывает
			if(daylight != 0){
				...
				std::tm tm = {};
				/**
				 * Признак летнего времени снимается с разложения самого мига, а не
				 * текущего: миг за пределами разложения MS Windows (после 3000 года)
				 * получает стандартное смещение
				 */
				if((::localtime_s(&tm, &stamp) == 0) && (tm.tm_isdst > 0))
		 ...
		 *
		 * @details Поля tm_gmtoff у разложенного времени там нет вовсе: оно
		 *          заведено договорами BSD и GNU, а не общим. Взамен система
		 *          несёт пару глобальных переменных - timezone со стандартным
		 *          смещением зоны и altzone со смещением её летнего времени
		 *
		 * @note Выбор между ними ведётся по признаку действия летнего времени у
		 *       разложения самого мига, а не текущего
		 *
		 * @warning Знак у обеих переменных обратный принятому: они несут, сколько
		 *          прибавить к местному времени, чтобы выйти на всемирное, а нам
		 ...
		 */
			...
			// Создаем структуру времени
			std::tm tm = {};
			// Если миг разложен по местной временной зоне
			if(::localtime_r(&stamp, &tm) != nullptr)
				// Получаем смещение временной зоны в секундах
				result = static_cast <int32_t> (-(tm.tm_isdst > 0 ? altzone : timezone));
			// Иначе получаем стандартное смещение временной зоны
			else result = static_cast <int32_t> (-timezone);
		/**
		 ...
		 */
			...
			std::tm tm = {};
			/**
			 * Смещение снимается с разложения самого мига: поле tm_gmtoff несёт
			 * смещение, действовавшее в этот миг, с летним временем и с историей
			 * смены правил зоны, тогда как глобальная переменная timezone несёт
			 * одно лишь нынешнее стандартное смещение
			 */
			if(::localtime_r(&stamp, &tm) != nullptr)
				// Получаем смещение временной зоны в секундах
				result = static_cast <int32_t> (tm.tm_gmtoff);
			// Если миг разложить не удалось, берём смещение текущего мига
			else {
				// Получаем текущее время с точностью до секунды
				const time_t now = std::time(nullptr);
				// Если текущий миг разложен по местной временной зоне
	...
	/**
	 * @brief Функция получения смещения временной зоны окружения на заданный миг
	 *
	 * @note Годность запомненных смещений проверяется до вызова функцией ambientFresh
	 *
	 * @param stamp миг в секундах от начала эпохи
	 * @return      смещение временной зоны окружения в секундах
	 *
	 */
	int32_t ambientZone(const time_t stamp) noexcept {
		/**
		 * Выполняем перебор запомненных смещений
		 */
		for(uint8_t i = 0; i < ambientCache.count; i++){
			// Если смещение на этот миг уже запомнено
			if(ambientCache.stamps[i] == stamp)
				// Выводим запомненное смещение временной зоны окружения
				return ambientCache.offsets[i];
		}
		// Получаем смещение временной зоны окружения у системы
		const int32_t result = ::systemZone(stamp);
		// Запоминаем миг, на который снято смещение
		ambientCache.stamps[ambientCache.next] = stamp;
		// Запоминаем снятое смещение временной зоны окружения
		ambientCache.offsets[ambientCache.next] = result;
		// Переходим к следующему месту запоминания
		ambientCache.next = static_cast <uint8_t> ((ambientCache.next + 1) % ambient_cache_t::SIZE);
		// Если запомненные смещения ещё не заняли все места
		if(ambientCache.count < ambient_cache_t::SIZE)
			// Увеличиваем количество запомненных смещений
			ambientCache.count++;
		// Выводим результат
		return result;
	}
 ...
 *
 */
int32_t awh::Chrono::environZone(const time_t stamp) const noexcept {
	// Выполняем проверку годности запомненных смещений
	::ambientFresh();
	// Выводим смещение временной зоны окружения на заданный миг
	return ::ambientZone(stamp);
}
/**
 ...
 *
 */
int32_t awh::Chrono::wallZone(const dt_t & dt) const noexcept {
	// Выполняем копирование объекта даты
	dt_t local = dt;
	// Выполняем сброс смещения временной зоны
	local.offset = 0;
	// Выполняем сброс обозначения временной зоны
	local.zone = zone_t::NONE;
	// Собираем местное время записи так, как если бы оно было всемирным
	const time_t wall = static_cast <time_t> (this->makeStamp(local) / 1000);
	// Выполняем проверку годности запомненных смещений один раз на все спросы
	::ambientFresh();
	/**
	 * Смещения снимаются по разные стороны местного времени, дальше 14 часов от
	 * него: миг, записью описываемый, отстоит от местного времени не дальше чем на
	 * смещение зоны, и снятые смещения лежат по разные стороны всякого перехода,
	 * пришедшегося на запись. Точки снятия округляются до часа - тогда соседние
	 * записи одного часа делят их между собой и спрашивают систему однажды
	 */
	// Получаем начало часа местного времени записи
	const time_t hour = (wall - (((wall % 3600) + 3600) % 3600));
	// Получаем смещение зоны окружения до местного времени записи (за 24 часа)
	const int32_t before = ::ambientZone(hour - 86400);
	// Получаем смещение зоны окружения после местного времени записи (через 25 часов)
	const int32_t after = ::ambientZone(hour + 90000);
	// Если перехода рядом с записью нет, смещение у неё одно
	if(before == after)
		// Выводим смещение временной зоны окружения
		return before;
	/**
	 * Большее смещение пробуется первым: в складке перехода годны оба, и большее
	 * даёт первый из двух мигов - так же выбирает mktime при неизвестном признаке
	 * летнего времени и Python при fold=0
	 */
	// Получаем большее из смещений по обе стороны перехода
	const int32_t first = ((before > after) ? before : after);
	// Получаем меньшее из смещений по обе стороны перехода
	const int32_t second = ((before > after) ? after : before);
	// Если миг, выведенный большим смещением, даёт то же местное время
	if(::ambientZone(wall - first) == first)
		// Выводим большее смещение временной зоны окружения
		return first;
	// Если миг, выведенный меньшим смещением, даёт то же местное время
	if(::ambientZone(wall - second) == second)
		// Выводим меньшее смещение временной зоны окружения
		return second;
	/**
	 * Местного времени в провале перехода не существует: берётся смещение,
	 * действовавшее до перехода, и запись переносится вперёд на величину перевода
	 */
	return before;
}
/**
...
uint64_t awh::Chrono::parse(string_view date, string_view format, const storage_t storage, bool * valid) noexcept {
	...
	if(!date.empty() && !format.empty()){
		...
		/**
		 * Смещение зоны окружения зависит от мига записи, и разрешается оно по местным
		 * полям записи уже после того, как они достроены, а при откате года - заново
		 */
		// Признак чтения записи во временной зоне окружения
		bool ambient = false;
			...
			case static_cast <uint8_t> (storage_t::LOCAL): {
				...
				if(!flags[0]){
					...
					// Если временная зона объекту не выставлена
						...
						this->_dt.zone = zone_t::NONE;
						// Запись читается во временной зоне окружения
						ambient = true;
						/**
						 * Смещение зоны окружения зависит от мига записи и разрешается ниже,
						 * когда поля записи достроены: спрашивать его здесь незачем
						 */
						this->_dt.offset = 0;
					}
				/**
				 ...
				 */
				// Если запись читается во временной зоне окружения
				if(ambient)
					// Разрешаем смещение зоны окружения по местным полям записи
					this->_dt.offset = this->wallZone(this->_dt);
				...
				if(!flags[1] && (this->_yearRollback > 0) &&
				   (result > (current + (static_cast <uint64_t> (this->_yearRollback) * 1000)))){
					...
					// Если запись читается во временной зоне окружения
					if(ambient)
						// Разрешаем смещение зоны окружения по новой дате
						this->_dt.offset = this->wallZone(this->_dt);
					// Разрешаем смещение сводной временной зоны по новой дате
					this->resolveZone(this->_dt);
					// Запоминаем разрешённое смещение временной зоны
...
string awh::Chrono::format(const uint64_t date, const standard_t standard) const noexcept {
	...
	if(standard == standard_t::ISO8601){
		// Получаем смещение временной зоны окружения на саму записываемую дату
		const int32_t offset = this->environZone(static_cast <time_t> (date / 1000));
...
uint64_t awh::Chrono::parse(string_view date, const standard_t standard, bool & valid, const storage_t storage) noexcept {
	...
	for(uint8_t i = 0; i < 4; i++){
		...
		if(!::matchLiterals(date, format, (standard == standard_t::RFC3339)))
...
string awh::Chrono::format(const uint64_t date, string_view format) const noexcept {
	...
	if(!format.empty()){
		...
		dt_t dt;
		/**
		 * Смещение зоны окружения берётся на саму записываемую дату, а не на текущий
		 * миг: прежде январская запись, сформированная летом в зоне Europe/Berlin,
		 * выходила с летним смещением «+0200» и уезжала на час
		 */
		// Заполняем объект даты из штампа времени
		this->makeDate(date, this->environZone(static_cast <time_t> (date / 1000)), dt);
		...
		switch(static_cast <uint8_t> (storage)){
			...
			case static_cast <uint8_t> (storage_t::LOCAL): {
				...
				if((dt.offset == 0) && (dt.zone == zone_t::NONE))
					// Перекладываем объект даты во временную зону окружения на его же дату
					this->shiftDate(dt, this->environZone(static_cast <time_t> (this->makeStamp(dt) / 1000)));
				// Выполняем формирование формата даты
				return this->format(dt, format);
			}
			// Если хранилище глобальное
...
string awh::Chrono::strip(string_view date, string_view format1, string_view format2, const storage_t storage) noexcept {
	...
	if(!date.empty() && !format1.empty()){
		...
		try {
			...
			if(valid)
				/**
				 * Смещение зоны окружения берётся на саму разобранную дату, а не на
				 * текущий миг; местное хранилище несёт смещение, выставленное разбором
				 */
				return this->format(stamp, ((storage == storage_t::GLOBAL)
					? this->environZone(static_cast <time_t> (stamp / 1000))
					: this->getTimeZone(storage)), format2);
		/**
		 * Если возникает ошибка
		 */
		} catch(const exception & error) {
