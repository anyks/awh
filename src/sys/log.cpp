 ...
 */
#ifndef AWH_LOG_BUFFER_MAX
	#define AWH_LOG_BUFFER_MAX 0x1000000
#endif
...
string & awh::Log::cleaner(string & text) const noexcept {
	...
	while((pos = text.find("\x1B[", pos)) != string::npos){
		// Позиция текущего символа блока экранирования
		size_t i = (pos + 2);
		/**
		 * Пропускаем параметры блока экранирования (цифры и разделители ";")
		 */
		for(; i < text.length(); i++){
			// Выполняем получение текущего символа
			letter = text.at(i);
			// Если символ не является числом и не является разделителем, выходим
			if((letter != ';') && !this->_fmk->is(letter, fmk_t::check_t::NUMBER))
				// Выходим из цикла
				break;
		}
		/**
		 * Каждая ветка удаляет как минимум начало блока "\x1B[", поэтому поиск
		 * всегда продвигается вперёд, в том числе на незавершённом блоке в конце
		 * текста (иначе цикл крутится вечно, удерживая блокировку лога)
		 */
		// Если блок экранирования завершён символом "m"
		if((i < text.length()) && (text.at(i) == 'm'))
			// Выполняем удаление блока вместе с символом завершения
			text.erase(pos, (i + 1) - pos);
		// Если блок не завершён, удаляем его начало до текущего символа
		else text.erase(pos, i - pos);
	}
		...
		if((this->_level == level_t::ALL) ||
		  ...
		  ((this->_level == level_t::WARNING_CRITICAL) && ((flag == flag_t::WARNING) || (flag == flag_t::CRITICAL)))){
			...
			for(;;){
				 ...
				 */
				const int res = ::vsnprintf(buffer.data(), buffer.size(), format.c_str(), args2);
		...
		if((this->_level == level_t::ALL) ||
		  ...
		  ((this->_level == level_t::WARNING_CRITICAL) && ((flag == flag_t::WARNING) || (flag == flag_t::CRITICAL)))){
			...
			for(;;){
				 ...
				 * обнуляется и цикл крутится вечно
				 */
				const int res = ::vswprintf(buffer.data(), buffer.size(), format.c_str(), args2);
				// Если результат получен
				if((res >= 0) && (static_cast <size_t> (res) < buffer.size())){
					// Завершаем список аргументов
					va_end(args);
				...
				if(res < 0){
					...
					if(buffer.size() >= AWH_LOG_BUFFER_MAX){
						// Завершаем список локальных аргументов
						va_end(args2);
						// Выполняем сброс результата
						buffer.clear();
					...
					size = (buffer.size() * 2);
				// Увеличиваем размер буфера на один байт
				} else size = (static_cast <size_t> (res) + 1);
				// Очищаем буфер данных
				buffer.clear();
				// Выделяем память для буфера
				buffer.resize(size);
