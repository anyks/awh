 ...
 */
	...
	#include <syslog.h>
	/**
	 * Подключаем заголовочный файл проекта по обходу каталогов
	 */
...
namespace awh {
	...
	namespace log {
		...
		namespace {
			...
			class ConsoleSink : public Sink {
				...
				public:
					 ...
					 *
					 */
					explicit ConsoleSink() noexcept = default;
					/**
					 * @brief Деструктор
					 *
					 */
			...
			class SyslogSink : public Sink {
				...
				public:
					 ...
					 *
					 */
					explicit SyslogSink() noexcept = default;
					/**
					 * @brief Деструктор
					 *
					 */
			...
			class CallbackSink : public Sink {
				...
				public:
					 ...
					 *
					 */
					explicit CallbackSink() noexcept = default;
					/**
					 * @brief Деструктор
					 *
					 */
		...
		/**
		 * @brief Устройство вывода логов и служебных средств
		 *
		 */
		namespace {
			/**
			...
			void FileSink::rotate() const noexcept {
				 ...
				 */
					...
					if(file != INVALID_HANDLE_VALUE){
						...
						gzFile gz = ::gzopen_w(fmk::convert(archive).c_str(), "wb9h");
			...
			void FileSink::retention() const noexcept {
				 ...
				 */
					...
					if(find != INVALID_HANDLE_VALUE){
						...
						do {
							...
							const string name = fmk::convert(data.cFileName);
							...
							const string & full = fmk::format("%s%s", dir.c_str(), name.c_str());
				 ...
				 */
					...
					dir::DIR * directory = dir::opendir(dir.c_str());
					// Если каталог открыт
					if(directory != nullptr){
						// Объект записи каталога
						dir::dirent * entry = nullptr;
						/**
						 * Перебираем все записи каталога
						 */
						while((entry = dir::readdir(directory)) != nullptr){
							// Получаем имя файла
			...
			string FileSink::nextArchive() const noexcept {
				...
				string path = fmk::format("%s%s%s.gz", cmp.first.c_str(), cmp.second.c_str(), date.c_str());
				...
				if(exists(path)){
					...
					for(uint32_t i = 1; i > 0; i++){
						...
						const string candidate = fmk::format("%s%s%s_%u.gz", cmp.first.c_str(), cmp.second.c_str(), date.c_str(), i);
			 ...
			 *
			 */
			FileSink::FileSink() noexcept : Sink(), _pid(0), _fd(-1), _opened{""}, _size(0) {}
			/**
			 * @brief Деструктор
			 *
			 */
			...
			string & cleaner(string & text) noexcept {
				...
				while((pos = text.find("\x1B[", pos)) != string::npos){
					...
					for(size_t i = (pos + 2); i < text.length(); i++){
						...
						} else if(!fmk::is(letter, fmk::check_t::NUMBER) && (letter != ';')) {
			...
			string compose(const payload_t & payload, const bool colored) noexcept {
				...
				switch(static_cast <uint8_t> (payload.flag)){
					...
					case static_cast <uint8_t> (flag_t::NONE):
						...
						return fmk::format("%s%s", payload.text.c_str(), tail);
					// Печатаем информационное сообщение
					case static_cast <uint8_t> (flag_t::INFO):
						// Формируем текстовый вид лога
						return (
							colored ?
							fmk::format("\x1B[32m\x1B[1mInfo\x1B[0m \x1B[32m%s %s :\x1B[0m %s%s", payload.date.c_str(), state()._name.c_str(), payload.text.c_str(), tail) :
							fmk::format("Info %s %s : %s%s", payload.date.c_str(), state()._name.c_str(), payload.text.c_str(), tail)
						);
					// Записываем ошибку в лог
					case static_cast <uint8_t> (flag_t::CRITICAL):
						// Формируем текстовый вид лога
						return (
							colored ?
							fmk::format("\x1B[31m\x1B[1mError\x1B[0m \x1B[31m%s %s :\x1B[0m %s%s", payload.date.c_str(), state()._name.c_str(), payload.text.c_str(), tail) :
							fmk::format("Error %s %s : %s%s", payload.date.c_str(), state()._name.c_str(), payload.text.c_str(), tail)
						);
					// Записываем в лог сообщение предупреждения
					case static_cast <uint8_t> (flag_t::WARNING):
						// Формируем текстовый вид лога
						return (
							colored ?
							fmk::format("\x1B[33m\x1B[1mWarning\x1B[0m \x1B[33m%s %s :\x1B[0m %s%s", payload.date.c_str(), state()._name.c_str(), payload.text.c_str(), tail) :
							fmk::format("Warning %s %s : %s%s", payload.date.c_str(), state()._name.c_str(), payload.text.c_str(), tail)
						);
				}
		...
		namespace {
			...
			State::State() noexcept :
			 ...
			 _counter{1}, _screen(Screen <payload_t>::health_t::DEAD), _callback(nullptr) {
			 ...
			 *
			 */
			string serialization(std::initializer_list <log::arg_t> params) noexcept {
				 ...
				 */
				for(const log::arg_t & param : params){
