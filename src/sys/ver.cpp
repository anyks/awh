...
void awh::Version::set(const string & ver) noexcept {
	...
	if(!ver.empty()){
		...
		try {
			 ...
			 *
			 * Октет обязан состоять только из цифр [0-9] (как в AWH 5): хвосты вида
			 * "3-beta", пробелы и знаки отвергаются, прежнее значение версии сохраняется.
			 * Значения больше 255 также отвергаются: раньше они молча заворачивались
			 * (300 -> 44), а отрицательные давали 255
			 *
