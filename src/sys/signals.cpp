...
namespace signals {
	 ...
	 */
		...
		/**
		 * @brief Функция перехвата управляющих событий консоли
		 *
		 * @details Событий этих `::signal` не видит вовсе: подобие сигналов у MS Windows
		 *          несёт один лишь `SIGINT`, да и тот выставляет сама библиотека времени
		 *          выполнения по нажатию Ctrl+C. Ни закрытие окна консоли, ни выход из
		 *          сеанса, ни выключение машины до подобия сигналов не доходят, и
		 *          приложение снималось силой - не успев ни закрыть своих дескрипторов,
		 *          ни убрать за собой заведённого в системе
		 *
		 * @details Событие переводится в тот сигнал, какой отвечает ему по смыслу, и
		 *          уходит потребителю обычным путём - через ту же очередь. Потребитель
		 *          оттого пишет один и тот же разбор на всех системах и о разнице этой
		 *          не знает вовсе: тем фреймворк и занят
		 *
		 *          | Событие консоли        | Сигнал  |
		 *          |------------------------|---------|
		 *          | `CTRL_C_EVENT`         | SIGINT  |
		 *          | `CTRL_BREAK_EVENT`     | SIGINT  |
		 *          | `CTRL_CLOSE_EVENT`     | SIGTERM |
		 *          | `CTRL_LOGOFF_EVENT`    | SIGTERM |
		 *          | `CTRL_SHUTDOWN_EVENT`  | SIGTERM |
		 *
		 * @warning Событиям завершения система отводит считанные секунды и снимает
		 *          процесс, едва перехватчик вернёт управление. Оттого перехватчик не
		 *          просто ставит сигнал в очередь, а ДОЖИДАЕТСЯ, покуда приложение
		 *          свернёт свою работу - то есть покуда отслеживание сигналов не будет
		 *          остановлено. Вернись он сразу, приложение погибло бы ровно там же,
		 *          где и без перехватчика
		 *
		 * @note Ожидание ограничено сроком: приложение вправе и не свернуться вовсе, а
		 *       держать перехватчик дольше отведённого системой всё равно бесполезно -
		 *       она снимет процесс своей рукой
		 *
		 * @param type вид управляющего события консоли
		 * @return     признак того, что событие разобрано
		 *
		 */
		static BOOL WINAPI console(const DWORD type) noexcept {
			// Сигнал, отвечающий управляющему событию консоли
			int32_t sig = 0;
			// Признак того, что событие означает завершение работы приложения
			bool finish = false;
			/**
			 * Определяем вид управляющего события консоли
			 */
			switch(type){
				// Если заказано прерывание работы
				case CTRL_C_EVENT:
				case CTRL_BREAK_EVENT:
					// Устанавливаем сигнал прерывания работы
					sig = SIGINT;
				break;
				// Если заказано завершение работы
				case CTRL_CLOSE_EVENT:
				case CTRL_LOGOFF_EVENT:
				case CTRL_SHUTDOWN_EVENT: {
					// Устанавливаем сигнал завершения работы
					sig = SIGTERM;
					// Отмечаем событие означающим завершение работы приложения
					finish = true;
				} break;
				// Событие, разбора не имеющее, передаём дальше по цепочке
				default: return 0;
			}
			// Если перехват сигналов активен не - разбирать событие нечем
			if(!self.active.load(std::memory_order_acquire))
				// Передаём событие дальше по цепочке
				return 0;
			// Ставим отвечающий событию сигнал в очередь рабочего потока
			::signals::handler(sig);
			/**
			 * Если событие означает завершение работы - дожидаемся, пока приложение свернётся
			 *
			 * @note Свёрнутость узнаётся снятием перехвата сигналов: его снимает и явная
			 *       остановка отслеживания, и разрушение объекта отслеживания - то есть
			 *       всякий нормальный выход из приложения
			 */
			if(finish){
				/**
				 * Ждём сворачивания работы приложения, но не дольше отведённого системой срока
				 *
				 * @note Срок системы - около пяти секунд, и берётся здесь запас поменьше:
				 *       вернуть управление своей рукой лучше, чем быть снятым посреди дела
				 */
				for(uint16_t i = 0; (i < 90) && self.active.load(std::memory_order_acquire); i++)
					// Выдерживаем паузу, давая приложению свернуть работу
					std::this_thread::sleep_for(std::chrono::milliseconds(50));
			}
			// Выводим признак разобранного события
			return 1;
		}
		/**
...
void awh::Signals::disarm() noexcept {
	...
	if(this->_mode.exchange(false, std::memory_order_acq_rel)){
		 ...
		 */
			...
			/**
			 * Возвращаем прежнее расположение сигнала SIGPIPE
			 *
			 * @warning Возвращается ИМЕННО ПРЕЖНЕЕ, а не умолчание: до запуска сигнал
			 *          мог быть заглушён чужим кодом того же процесса, и умолчание
		 ...
		 */
			...
			for(auto sig : list)
				// Активируем обработчик по умолчанию
				::signal(sig, SIG_DFL);
			// Снимаем перехватчик управляющих событий консоли
			::SetConsoleCtrlHandler(&::signals::console, 0);
...
void awh::Signals::stop() noexcept {
	...
	if(worker.joinable()){
		 ...
		 *          прочитавший номер описателя, вправе быть вытеснен на любой
		 *          команде - и `disarm()` его не отзовёт, отозвать вошедший
		 *          обработчик невозможно вовсе. Закрой мы описатель, к мигу его
		 *          пробуждения номер принадлежал бы уже другому объекту, и
		 *          обработчик записал бы двадцать байт двоичной нагрузки в чужой
		 *          сокет либо файл
		 *
		 * @warning Условие это не редкое совпадение, а ПРАВИЛО: POSIX велит выдавать
		 *          наименьший свободный номер, оттого повторный подъём получает те же
		 *          номера ВСЕГДА. Замерено: четыре подъёма подряд дали 3 и 4 каждый
		 *
		 * @note Признак снят выше (`pipefd` = -1), и новых записей обработчик уже не
		 *       начнёт; описатель держится лишь ради тех, кто вошёл до снятия.
		 *       Закрытие достаётся деструктору - там объект уходит целиком, и номер
		 *       его больше никого не касается
		 *
		 */
	}
}
...
void awh::Signals::start() noexcept {
	 ...
	 *       остановку ради обработчиков, вошедших до снятия признака
	 *
	 */
	 ...
	 */
		...
		/**
		 * Иначе самопайп достался от прежнего подъёма, и его надлежит ОПУСТОШИТЬ
		 *
		 * @warning Опустошение обязательно. В самопайпе остаётся служебная нагрузка
		 *          пробуждения, которой останавливался прежний рабочий поток, а также
		 *          всё, что успели записать обработчики, вошедшие до снятия признака.
		 *          Не убери мы этого, новый рабочий поток первым же чтением выдал бы
		 *          потребителю сигнал ЧУЖОГО подъёма - и выглядело бы это не изъяном,
		 *          а самопроизвольным сигналом на ровном месте
		 *
		 * @warning Опустошение требуется ИМЕННО ТЕМ, что самопайп теперь переживает
		 *          остановку. Прежде он закрывался, и остаток пропадал вместе с ним;
		 *          сохраняя описатель, мы обязаны убрать остаток сами
		 *
		 * @note Проверкой `RestartNoPhantomTest` закрыта лишь служебная нагрузка
		 *       пробуждения, и держит её отбрасывание нуля рабочим потоком, а не
		 *       опустошение - проверено снятием заслонов поодиночке. Опустошение стоит
		 *       ради НАСТОЯЩИХ сигналов, записанных обработчиками, вошедшими до снятия
		 *       признака; воспроизвести их проверкой нечем - вход в обработчик не
		 *       управляем извне, - и заслон этот держится доводом устройства
		 *
		 */
		} else {
			// Объём вычитываемого за раз
			char trash[sizeof(::signals::payload_t) * 32];
			// Число прочитанных байт
			ssize_t bytes = 0;
			// Ставим чтение неблокирующим на время опустошения
			const int32_t flags = ::fcntl(this->_pipe[0], F_GETFL, 0);
			// Помечаем чтение неблокирующим
			::fcntl(this->_pipe[0], F_SETFL, flags | O_NONBLOCK);
			/**
			 * Вычитываем остаток прежнего подъёма до опустошения
			 */
			do {
				// Выполняем чтение остатка
				bytes = ::read(this->_pipe[0], trash, sizeof(trash));
			// Продолжаем, пока читается хоть что-то либо чтение прерывается сигналом
			} while((bytes > 0) || ((bytes < 0) && (errno == EINTR)));
			// Возвращаем чтению прежний вид
			::fcntl(this->_pipe[0], F_SETFL, flags);
		}
		 ...
		 *
		 * @warning Запасной стек подменяет лишь стек пользовательский. У процессора
		 *          Эльбрус есть ещё два аппаратных - процедурный и стек связующей
		 *          информации, со своими пределами RLIMIT_P_STACK и RLIMIT_PC_STACK, - и
		 *          возвратность без крупных местных переменных исчерпывает именно их.
		 *          Такой срыв до обработчика не доходит вовсе: сигнала ядро не шлёт, а
		 *          сорвавший поток снимает SIGKILL. Снимается один поток, а не процесс,
		 *          и процесс с рабочим потоком этого модуля остаётся жить без него -
		 *          без журнала, без функции обратного вызова и без завершения
		 */
		 ...
		 *
		 * @note Прежнее расположение сигнала СОХРАНЯЕТСЯ и возвращается снятием
		 *       обработчиков наравне с прочими семью сигналами: остановили сигналы -
		 *       значит модуль больше не работает, и оставлять за собой изменённое
		 *       поведение процесса незачем
		 *
		 * @note Настройка общая на весь процесс. Для сетевого кода заглушение
		 *       обыкновенно и желательно - разрыв встречной стороной обязан приходить
		 *       кодом отказа, а не убивать процесс, - но код ЧУЖОЙ, тем же процессом
		 *       исполняемый, получал бы изменённое поведение и о том не знал
		 *
		 * @note Движки на эту настройку не опираются: у них разрыв глушится своими
		 *       средствами - опцией сокета `event::options::NO_SIGPIPE` и признаком
		 *       `MSG_NOSIGNAL` при самой отправке. Здесь она служит вторым рубежом
		 */
		{
			// Формируем структуру заглушения сигнала
			struct sigaction ignore{};
			// Заполняем структуру нулями
			::memset(&ignore, 0, sizeof(ignore));
			// Устанавливаем заглушающий обработчик
			ignore.sa_handler = SIG_IGN;
			// Очищаем маску перехвата
			sigemptyset(&ignore.sa_mask);
			// Заглушаем сигнал, запоминая прежнее его расположение
			if(::sigaction(SIGPIPE, &ignore, &this->_pipesig) != 0)
				// Если заглушить не удалось, прежнее расположение считаем неизвестным
				::memset(&this->_pipesig, 0, sizeof(this->_pipesig));
		}
		/**
	 ...
	 */
		...
		/**
		 * Ставим перехватчик управляющих событий консоли
		 *
		 * @note Без него до приложения не доходят ни закрытие окна консоли, ни выход из
		 *       сеанса, ни выключение машины: подобие сигналов у MS Windows несёт один
		 *       лишь `SIGINT`. Разбор их устройство перехватчика приводит к общему виду -
		 *       к тем же сигналам, какие приложение разбирает у систем POSIX
		 */
		::SetConsoleCtrlHandler(&::signals::console, 1);
		// Запускаем рабочий поток обработки сигналов
		this->_worker = std::thread(&awh::Signals::worker, this);
		// Устанавливаем флаг запуска отслеживания сигналов
		this->_mode.store(true, std::memory_order_release);
...
awh::Signals::Signals() noexcept :
 _mode(false), _exit(false), _callback(nullptr) {
	 ...
	 */
		...
		/**
		 * Обнуляем прежнее расположение сигнала SIGPIPE
		 *
		 * @note Нулевая структура означает умолчание системы: таким расположение и
		 *       будет возвращено, если запуск не дошёл до сохранения настоящего
		 */
		::memset(&this->_pipesig, 0, sizeof(this->_pipesig));
	#endif
	/**
...
awh::Signals::~Signals() noexcept {
	 ...
	 */
		 ...
		 *
		 * @note Место закрытия единственное и выбрано намеренно: здесь объект уходит
		 *       целиком, повторных подъёмов у него больше не будет, и номер описателя
		 *       никого из его обработчиков уже не касается. Довод, по которому
		 *       закрытие изъято из `stop()`, изложен там же
		 *
		 */
