...
namespace signals {
	 ...
	 */
		 ...
		 *
		 * @note Признак нужен обеим сборкам, а не одному лишь релизу: по нему выбирается
		 *       годное поле объединения в структуре siginfo_t - адрес сбоя либо
		 *       идентификаторы отправителя
		 *
		 * @param signal номер проверяемого сигнала
		 * @return       признак сигнала, у которого осмыслен адрес обращения
		 *
		 */
		static bool faulty(const int32_t signal) noexcept {
			// Определяем принадлежность сигнала к сбоям обращения
			return (
				(signal == SIGILL) ||
				(signal == SIGFPE) ||
				(signal == SIGBUS) ||
		...
		static void handler(const int32_t sig, siginfo_t * info, [[maybe_unused]] void * ctx) noexcept {
			...
			if(fd >= 0){
				 ...
				 *
				 * Поля si_addr и si_pid лежат в одном объединении, и годно из них ровно
				 * одно - смотря какой сигнал пришёл. У сбоя обращения к памяти осмыслен
				 * адрес, а идентификаторы отправителя там - чужие байты того же места;
				 * у сигнала же, посланного вызовом kill, наоборот
				 */
				if(info != nullptr){
					// Если получен сигнал сбоя обращения
					if(faulty(sig))
						// Запоминаем адрес обращения, вызвавшего сбой
						payload.addr = info->si_addr;
					// Если сигнал получен от другого процесса
					else {
	 ...
	 */
		...
		static struct Self {
			 ...
			 *       `std::mutex`: голый мьютекс переживает ветвление процесса
 ...
 */
	...
	void awh::Signals::worker() noexcept {
		...
		while(!this->_exit.load(std::memory_order_acquire)){
			...
			this->process(payload.sig, payload.pid, payload.uid, payload.addr);
		}
	}
	/**
	 ...
	 *
	 */
	void awh::Signals::process(const int32_t sig, const pid_t pid, const uid_t uid, void * addr) noexcept {
		/**
		 * Если получен сигнал сбоя обращения и установлен объект логирования
		 *
		 * Адрес обращения - единственное, что система о сбое сообщает, и без него запись
		 * в журнале не отличает разыменование нулевого указателя от выхода за границу
	...
	void awh::Signals::worker() noexcept {
		...
		for(;;){
			...
			int32_t signal = 0;
			{
				...
				std::unique_lock <std::mutex> lock(static_cast <std::mutex &> (::signals::self.mtx));
...
void awh::Signals::stop() noexcept {
	/**
	 * Рабочий поток, изъятый из учёта для ожидания его завершения
	 *
	 * @warning Ожидание завершения потока вынесено ИЗ-ПОД замка намеренно. Рабочий
	 *          поток исполняет функцию обратного вызова потребителя, а та вправе
	 *          позвать `stop()` либо `on()` - оба берут этот самый замок. Дождись мы
	 *          завершения под замком, поток встал бы на замке, а ожидание - на потоке,
	 *          и остановка заклинивала бы намертво. Оттого под замком поток лишь
	 *          ИЗЫМАЕТСЯ из учёта, а ждут его уже снаружи
	 *
	 * @note Изъятие под замком заодно снимает состязание двух остановок разом: поток
	 *       достаётся ровно одной из них, второй не достаётся ничего
	 *
	 */
	std::thread worker;
	{
		 ...
		 */
			...
			if(this->_worker.joinable()){
				...
				/**
				 * Если остановка выполняется не из рабочего потока, изымаем поток из учёта
				 *
				 * @note Остановка ИЗ рабочего потока поток на месте и оставляет: присоединить
				 *       себя к себе нельзя. Завершение и закрытие дескрипторов достанутся
				 *       деструктору либо повторному вызову `stop()` из другого потока -
				 *       рабочий поток выйдет самостоятельно по признаку `_exit`
				 */
				if(this->_worker.get_id() != std::this_thread::get_id())
					// Изымаем рабочий поток из учёта: ждать его будем вне замка
					worker = ::move(this->_worker);
			}
		/**
		 * Для операционной системы MS Windows
		 */
			...
			if(this->_worker.joinable()){
				{
					// Формируем блокировку очереди сигналов
					const locker_t <> lk(::signals::self.mtx);
					// Устанавливаем флаг запроса остановки рабочего потока
					this->_exit.store(true, std::memory_order_release);
				}
				// Пробуждаем рабочий поток
	...
	// Если рабочий поток изъят из учёта
	if(worker.joinable()){
...
void awh::Signals::start() noexcept {
	/**
	 * Рабочий поток, оставшийся от остановки из функции обратного вызова
	 *
	 * @warning Ожидание его завершения вынесено ИЗ-ПОД замка по тому же доводу, что и
	 *          в `stop()`: поток этот доигрывает функцию обратного вызова потребителя,
	 *          а та вправе позвать `on()` либо `start()` - оба берут этот самый замок.
	 *          Дождись мы завершения под замком, поток встал бы на замке, а ожидание -
	 *          на потоке
	 *
	 */
	std::thread worker;
	{
		...
		if(this->_mode.load(std::memory_order_acquire))
			...
			return;
		// Если рабочий поток остался от остановки из функции обратного вызова
		if(this->_worker.joinable()){
			/**
			 * Для операционной системы не являющейся MS Windows
			 */
			...
			#endif
			/**
			 * Если запуск выполняется не из рабочего потока, изымаем поток из учёта
			 *
			 * @note Запуск ИЗ рабочего потока поток на месте и оставляет: присоединить
			 *       себя к себе нельзя
			 */
			if(this->_worker.get_id() != std::this_thread::get_id())
				// Изымаем рабочий поток из учёта: ждать его будем вне замка
	...
	// Формируем блокировку операций управления
	const locker_t <> lock(this->_mtx);
	// Если отслеживание сигналов успели запустить, пока мы ждали, выходим
	if(this->_mode.load(std::memory_order_acquire))
		// Прекращаем дальнейшую работу
		return;
	/**
	 ...
	 */
	#if !_WIN32 && !_WIN64
		// Закрываем дескриптор чтения самопайпа, оставшийся от прежнего запуска
		if(this->_pipe[0] >= 0){
			// Закрываем дескриптор
			::close(this->_pipe[0]);
			// Сбрасываем дескриптор
		...
		// Закрываем дескриптор записи самопайпа, оставшийся от прежнего запуска
		if(this->_pipe[1] >= 0){
			// Закрываем дескриптор
			::close(this->_pipe[1]);
			// Сбрасываем дескриптор
	 ...
	 */
		...
		/**
		 * Заводим запасной стек для обработчика сбоев
		 *
		 * Сбой обращения приходит и при срыве стека, а обработчику в этом случае встать
		 * некуда: место кончилось ровно тем, что его исчерпало. Без запасного стека
		 * такой сбой до обработчика не доходит вовсе - процесс гибнет молча, не позвав
		 * ни журнала, ни функции обратного вызова, ради которой модуль и заведён
		 *
		 * Память под стек берётся у системы один раз и живёт до разрушения объекта, а
		 * не до снятия перехватчиков: снятие идёт из разбора сбоя, когда сбойный поток
		 * на этом самом стеке и стоит, и освобождение памяти вышибло бы его из-под ног.
		 * Отказ заведения не считается препятствием: перехват без запасного стека
		 * работает для всех сбоев, кроме срыва стека
		 */
		if(this->_stack.ss_sp == nullptr){
			// Размер запасного стека: не меньше того, что требует система
			const size_t size = ((SIGSTKSZ > MINSIGSTKSZ) ? static_cast <size_t> (SIGSTKSZ) : static_cast <size_t> (MINSIGSTKSZ));
			// Выделяем память под запасной стек
			this->_stack.ss_sp = ::malloc(size);
			// Если память под запасной стек выделена
			if(this->_stack.ss_sp != nullptr){
				// Устанавливаем размер запасного стека
				this->_stack.ss_size = size;
				// Признаков поведения запасному стеку не требуется
				this->_stack.ss_flags = 0;
				// Устанавливаем запасной стек обработчику сигналов
				if(::sigaltstack(&this->_stack, nullptr) != 0){
					// Освобождаем выделенную память
					::free(this->_stack.ss_sp);
					// Помечаем запасной стек незаведённым
					this->_stack.ss_sp = nullptr;
				}
			}
		}
		 ...
		 */
			...
			int32_t flags = SA_SIGINFO;
		#endif
		/**
		 * Если запасной стек заведён, велим обработчику вставать на него
		 *
		 * Признак ставится лишь фатальным сигналам: прочие приходят при живом стеке,
		 * и уводить их на запасной незачем
		 */
		if(this->_stack.ss_sp != nullptr)
			// Отправляем обработчик фатальных сигналов на запасной стек
			flags |= SA_ONSTACK;
		/**
...
awh::Signals::~Signals() noexcept {
	 ...
	 */
	#if !_WIN32 && !_WIN64
		// Если запасной стек обработчика заводился
		if(this->_stack.ss_sp != nullptr){
			// Объект отключения запасного стека
			stack_t disable{};
			// Помечаем запасной стек отключаемым
			disable.ss_flags = SS_DISABLE;
			// Отключаем запасной стек у системы прежде освобождения памяти
			::sigaltstack(&disable, nullptr);
			// Освобождаем память запасного стека
			::free(this->_stack.ss_sp);
			// Помечаем запасной стек незаведённым
			this->_stack.ss_sp = nullptr;
		}
