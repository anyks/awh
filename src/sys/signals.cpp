...
void awh::Signals::start() noexcept {
	 ...
	 */
		 ...
		 *
		 * @warning Настройка эта НЕ ВОССТАНАВЛИВАЕТСЯ остановкой, в отличие от прочих
		 *          семи сигналов: `disarm()` возвращает по умолчанию `SIGINT`, `SIGFPE`,
		 *          `SIGILL`, `SIGBUS`, `SIGABRT`, `SIGTERM` и `SIGSEGV`, а `SIGPIPE`
		 *          остаётся заглушённым до конца жизни процесса. Прежнее расположение
		 *          его к тому же не сохраняется, так что вернуть его было бы нечем
		 *
		 * @note Настройка общая на весь процесс и переживает `stop()`. Для сетевого
		 *       кода это обыкновенно и желательно - разрыв встречной стороной обязан
		 *       приходить кодом отказа, а не убивать процесс, - но код ЧУЖОЙ, тем же
		 *       процессом исполняемый, получает изменённое поведение и о том не знает
		 *
