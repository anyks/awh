 ...
 */
void awh::Base::process(message_t message) noexcept {
	// Выполняем определение типа входящего сообщения
	switch(static_cast <uint8_t> (message.action)){
		// Если входящим сообщением является метод "del"
		case static_cast <uint8_t> (action_t::DEL): {
			// Если тип отслеживаемого события установлен
			if(message.type != event_type_t::NONE)
				// Выполняем удаление файлового дескриптора из базы событий для указанного события
				this->del(message.id, static_cast <SOCKET> (message.fd), message.type);
			// Выполняем удаление файлового дескриптора из базы событий для всех событий
			else this->del(message.id, static_cast <SOCKET> (message.fd));
		} break;
		// Если входящим сообщением является метод "add"
		case static_cast <uint8_t> (action_t::ADD):
			// Выполняем добавление файлового дескриптора в базу событий
			this->add(message.id, * static_cast <SOCKET *> (message.fd), message.callback, message.delay, message.series);
		break;
		// Если входящим сообщением является метод "mode"
		case static_cast <uint8_t> (action_t::MODE):
			// Выполняем установку режима работы модуля
			this->mode(message.id, static_cast <SOCKET> (message.fd), message.type, message.mode);
		break;
		// Если входящим сообщением является метод "kick"
		case static_cast <uint8_t> (action_t::KICK):
			// Выполняем отправку пинка
			this->kick();
		break;
		// Если входящим сообщением является метод "stop"
		case static_cast <uint8_t> (action_t::STOP):
			// Выполняем остановку работы базы событий
			this->stop();
		break;
		// Если входящим сообщением является метод "start"
		case static_cast <uint8_t> (action_t::START):
			// Выполняем запуск работы базы событий
			this->start();
		break;
		// Если входящим сообщением является метод "clear"
		case static_cast <uint8_t> (action_t::CLEAR):
			// Выполняем очистку всего содержимого базы событий
			this->clear();
		break;
		// Если входящим сообщением является метод "rebase"
		case static_cast <uint8_t> (action_t::REBASE):
...
void awh::Base::start() noexcept {
	...
	if((this->_mode == mode_t::CHILD) && !this->stream()){
		 ...
		 */
			...
			// Выводим сообщение об ошибке
			this->_log->print("Start event base: %s", log_t::flag_t::CRITICAL, error.what());
		}
	// Выполняем запуск базы событий
	} else {
		// Выполняем блокировку потока
 ...
 */
void awh::Base::mode(const mode_t mode) noexcept {
	/**
	 * Выполняем обработку ошибки
	 */
	try {
	 ...
	 */
		...
		// Выводим сообщение об ошибке
		this->_log->print("Mode event base: %s", log_t::flag_t::CRITICAL, error.what());
	}
}
/**
...
awh::Base::Base(const fmk_t * fmk, const log_t * log, const uint32_t count) noexcept :
 ...
 _screen(screen_t <message_t>::health_t::DEAD), _fmk(fmk), _log(log) {
	...
	this->_screen = static_cast <function <void (const message_t)>> (std::bind(&base_t::process, this, _1));
}
/**
 * ~Base Деструктор
 */
