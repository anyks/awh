 ...
 */
	...
	/**
	 * У OpenBSD ucontext удалён из системы: заголовка нет вовсе, и переключать
	 * стек приходится самим
	 */
	#define AWH_FIBER_ASM 1
#else
...
static void __awh_fiber_body__(awh::fiber::Fiber * fiber) noexcept {
	...
	fiber->state = awh::fiber::state_t::FINISHED;
...
awh::fiber::Fiber * awh::fiber::spawn(task_t task, const size_t size, const log_t * log) noexcept {
	 ...
	 */
		...
		result->sp = __awh_fiber_prepare__(top);
	#endif
...
bool awh::fiber::resume(Fiber * fiber) noexcept {
	 ...
	 */
		...
		__awh_fiber_swap__(&fiber->caller, fiber->sp);
	#endif
...
void awh::fiber::yield() noexcept {
	...
	#elif AWH_FIBER_ASM
		// Возвращаем управление разбудившей стороне
		__awh_fiber_swap__(&fiber->sp, fiber->caller);
	#endif
