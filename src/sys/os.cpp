 ...
 */
	...
	bool awh::OS::chown(const uid_t uid, const gid_t gid) const noexcept {
		/**
		 * Группу меняем до пользователя: после сброса прав root сменить группу уже нельзя,
		 * а дополнительные группы root сбрасываем, чтобы процесс их не унаследовал
		 */
		if((::setgroups(1, &gid) == 0) && (::setgid(gid) == 0)){
			...
			return false;
		// Если группа пользователя не установлена
		} else {
			/**
			 * Если включён режим отладки
			 */
