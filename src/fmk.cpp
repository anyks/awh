...
string awh::Framework::noexp(const double number, const bool onlyNum) const noexcept {
	...
	if(count > 0){
		// Флаг остановки перебора
		bool stop = false;
		// Запускаем бесконечный цикл удаления лишних нулей
		while((result.back() == '0') || stop){
			// Удаляем лишние нули
			result.erase(result.end() - 1, result.end());
			// Если нашли точку дробной части, выходим
			if(stop) break;
			// Определя точку дробной части
			stop = (result.back() == '.');
		}
	}
 ...
 */
void awh::Framework::log(const string & format, log_t flag, const char * filename, ...) const noexcept {
	// Если формат передан
	if(!format.empty()){
		// Создаем список аргументов
		va_list args;
		...
		if(size > 0){
			...
			// Выполняем проверку на наличие адреса файла логов
			const bool isFile = ((filename != nullptr) && (strlen(filename) > 0));
			// Формируем файловый поток
			ostream * file = (!isFile ? nullptr : new ofstream(filename, ios::app));
			// Формируем выходной поток
			ostream & out = (isFile ? * file : cout);
			/*
			// Если запись производится в консоль а не в файл
			if(!isFile && (this->plot != nullptr)){
 ...
 */
void awh::Framework::log(const string & format, log_t flag, const char * filename, const vector <string> & items) const noexcept {
	// Если формат передан
	if(!format.empty() && !items.empty()){
		// Создаем буфер для хранения даты
		char date[80];
		...
		strftime(date, sizeof(date), dateFormat.c_str(), timeinfo);
		// Создаём строку для проверки
		const string & str = this->format(format, items);
		// Если размер буфера меньше 3-х байт
		if(items.size() < 3)
			// Проверяем является ли это переводом строки
			isEnd = ((str.compare("\r\n") == 0) || (str.compare("\n") == 0));
