...
awh::codec::xml::node_id_t awh::codec::xml::Document::locate(const string & path) const noexcept {
	...
	if(this->_nodes.empty())
		...
		return INVALID_NODE;
	/**
	 * Если разыскиваемый путь пуст
	 *
	 * @note Корень дерева узлом разметки не является вовсе, и путём к нему не ведут:
	 *       пустой путь означал бы дерево целиком, а не узел его
	 */
	if(parts.empty())
		// Выводим признак того, что узел не разыскан
		return INVALID_NODE;
