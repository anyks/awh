...
bool awh::codec::xml::Writer::prefix(const string_view uri, const bool attribute, string & result) noexcept {
	 ...
	 *
	 * @warning Снимать отказ нельзя: он последний на дороге, и послабление подбора либо
	 *          заведение имени `xmlns:X` в обход связывания сделали бы его достижимым, а
	 *          следствием была бы метка с двумя объявлениями одного префикса
	 */
	...
	while(true){
		...
		if(!current.valid()){
			...
			if(!next.valid()){
				...
				/**
				 * @note Перенос отказа этот НЕДОСТИЖИМ и оттого не покрыт: записываемое дерево
				 *       получено разбором, а разбор отвергает всё, что отвергла бы запись, - имена
				 *       недопустимые, последовательности байтов негодные, построения ошибочные. Ни
				 *       набор, ни ворошитель на шестидесяти миллионах подач сюда не попали
				 *
				 * @warning Снимать перенос нельзя: он стоит ровно на страже того, что договор разбора
				 *          не разойдётся с договором записи. Разойдись они - и без него наружу ушёл бы
				 *          обрывок текста под видом успеха, а узел остался бы незакрытым
				 */
