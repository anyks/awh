...
bool awh::codec::xml::Writer::prefix(const string_view uri, const bool attribute, string & result) noexcept {
		...
		for(size_t i = this->_bindings; i > 0; i--){
			...
			if(this->_scopes[i - 1].prefix.empty()){
				...
				if(!this->_scopes[i - 1].uri.empty() && !attribute){
					 ...
					 *       отменять родительское не станет - оно перекрыто, а не отменено,
					 *       - и до отведения дело не дойдёт вовсе. Проверено обоими
					 *       построениями: и своим непустым умолчанием, и своим пустым
					 *
					 * @warning Снимать отказ нельзя: он единственный оберегает от записи
					 *          двух объявлений по умолчанию у одной метки - текста,
					 *          договором не дозволенного
					 */
	 ...
	 *       объявление префикса заводит и то, и другое разом. Счётчик же растёт при каждом
	 *       подборе, и одно имя дважды не порождается
	 *
	 * @warning Довод этот ЛОЖЕН, и дорога, какую он звал лишь возможной, открыта уже ныне.
	 *          Заведение имени `xmlns:X` в обход связывания доступно снаружи: запись свойства
	 *          ДОСЛОВНЫМ именем (`verbatim`) кладёт его в перечень имён узла, связывания при
	 *          том не заводя, - а подбор ниже смотрит лишь на действующие связывания и о
	 *          записанном имени не знает
	 *
	 * @note Доказано подачею 08.09.2026: `open("root")`, затем свойство `xmlns:n1` дословным
	 *       именем, затем свойство с обозначением пространства имён, префикса не имеющим, -
	 *       подбор выдаёт `n1`, и отказ срабатывает кодом `DUPLICATE_ATTRIBUTE`
	 *
	 * @note Поведение при этом ВЕРНО: отказ назван, текст наружу не выходит, дерево не
	 *       портится. Лгал один довод, а не код, и оттого правка здесь одна - правда в
	 *       записи. Подбор же, смотрящий и на записанные имена узла, выбрал бы `n2` и отказа
	 *       избежал; ход этот отложен намеренно - обход связывания есть ошибка звучащего, и
	 *       отвечать на неё названным отказом вернее, чем молча её обходить
	 *
	 * @warning Снимать отказ нельзя: он последний на дороге, и следствием была бы метка с
	 *          двумя объявлениями одного префикса
	 */
...
bool awh::codec::xml::Writer::attribute(const string_view local, const string_view value, const string_view uri, const bool verbatim) noexcept {
	...
	if(!verbatim && !this->prefix(uri, true, prefix))
		...
		return false;
	/**
	 * Если атрибуту префикс пространства имён не назначен
	 *
	 * @note Имя атрибута тут равно местному, и собирать его во временную строку
	 *       незачем: занятие имени берёт срез и заводит свою запись само
	 * @warning Правка эта снимает КОПИЮ, а не выделение памяти, и замером не
	 *          подтверждена: расход выделений на записи не изменился ни у libc++,
	 *          ни у libstdc++ - имена атрибутов короче запаса строки обеих, и сборка
	 *          укладывалась в него. Держится правка на том лишь, что работа эта
	 *          заведомо лишняя; выигрыша в замерах ей не приписывать
	 */
	if(prefix.empty()){
		/**
		 * Если имя атрибута узлу уже записано
		 */
		if(!this->occupy(local))
			// Выводим отрицательный результат выполнения операции
			return false;
	/**
	 ...
	 */
		...
		name.assign(prefix).append(1, ':').append(local);
		/**
		 * Если имя атрибута узлу уже записано
		 */
		if(!this->occupy(name))
...
bool awh::codec::xml::Writer::element(const node_t & node, const bool preserve) noexcept {
	 ...
	 *       записи. Заведение клейма условие это отменило, а довод сторожа пережил
	 *       собственную неверность. Отсюда правило: довод недостижимости обязан называть
	 *       условие, каким держится, - иначе перемена условия его не касается
	 */
	if(!node.valid()){
		// Выполняем отказ записи с сообщением о нём в журнал
		return this->refuse(error_t::INVALID_NODE);
	}
	/**
	...
	while(true){
		...
		if(!current.valid()){
			...
			if(!next.valid()){
				 ...
				 *       прививкою значения, собранного руками, - `set()`, `graft()`, `transplant()`, -
				 *       а разбора там нет вовсе
				 *
				 * @note Достижимость была ДОКАЗАНА печатью в теле 07.09.2026: значение с именами
				 *       `1bad`, `a b`, `<x>` и `a\x01b` прививалось признаком успеха, а запись дерева
				 *       выдавала пустую строку, и перенос отказа срабатывал всякий раз
				 *
				 * @note Ныне дорога закрыта у ИСТОКА, а не здесь: перенос значения в арену дерева судит
				 *       имя узла тем же судьёй `nameable()` и негодное отвергает кодом `INVALID_NAME`.
				 *       Молчание карты покрытия здесь означает закрытую дорогу, а не недостижимость по
				 *       устройству: сними судью у переноса - и перенос отказа оживёт в тот же день
				 *
				 * @warning Снимать перенос нельзя: он стоит ровно на страже того, что договор разбора и
				 *          прививки не разойдётся с договором записи. Разойдись они - и без него наружу
				 *          ушёл бы обрывок текста под видом успеха, а узел остался бы незакрытым
				 */
...
string awh::codec::xml::Writer::take() noexcept {
	 ...
	 * Если запись прекращена ошибкой
	 *
	 * @note Условие то же, каким закрыта выдача текста: изъятие обходило бы её стороною
	 *       и отдавало наружу ту самую негодную разметку, какую выдача не отдаёт
	 */
	if(this->_error != error_t::NONE)
		// Выводим пустой текст разметки
		return string();
