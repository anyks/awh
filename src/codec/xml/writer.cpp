...
bool awh::codec::xml::Writer::open(const string_view local, const string_view uri) noexcept {
	...
	if(!prefix.empty())
		...
		this->_text.insert(begin, prefix + ":");
	// Запоминаем имя узла в записи, принятой в тексте
	this->_opened.back().name.assign(prefix.empty() ? string(local) : (prefix + ":" + string(local)));
	// Выводим положительный результат выполнения операции
	return true;
}
/**
