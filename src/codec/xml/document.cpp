...
bool awh::codec::xml::Document::parse(const string_view text, const reader_t::settings_t & settings) noexcept {
	 ...
	 *
	 */
	auto store = [this](const string_view value) noexcept -> span_t {
	 ...
	 *
	 */
	auto title = [&store](const name_t & name) noexcept -> title_t {
		// Собираемая запись имени
		title_t result;
		// Выполняем размещение префикса имени
		result.prefix = store(name.prefix);
		// Выполняем размещение местного имени
		result.local = store(name.local);
		// Выполняем размещение обозначения пространства имён
		result.uri = store(name.uri);
	 ...
	 *
	 */
	auto append = [this](const node_id_t parent, const kind_t kind) noexcept -> node_id_t {
	...
	/**
	 * Выполняем отвод места под дерево разметки наперёд
	 *
	 * @details Дерево собирается тремя растущими хранилищами, и рост их идёт удвоением:
	 *          на разборе крупного текста половина занятой памяти приходится не на само
	 *          дерево, а на запас, отведённый последним удвоением, да на прежнее
	 *          хранилище, живущее до конца перекладывания. Разбирается здесь текст,
	 *          полученный целиком, и размер его известен заранее: место отводится по
	 *          нему единожды
	 *
	 * @note Оценка ведётся по числу начал разметки, отысканных средствами обработки
	 *       памяти: проход этот против самого разбора ничего не стоит, а завышает оценку
	 *       он не более чем вдвое - ровно настолько же завысил бы её и рост удвоением
	 */
	{
		...
		this->_nodes.reserve(tags + 1);
		// Выполняем отвод места под атрибуты узлов дерева разметки
		this->_attributes.reserve(pairs);
	...
	while(reader.next()){
		/**
		 * Если объём хранилища знаков превысил допустимый
		 */
		if(this->_storage.size() >= STORAGE_LIMIT){
			// Запоминаем положение обнаруженной ошибки
	...
	/**
	 * Если разбор текста разметки прекращён ошибкой
	 */
	if(reader.state() != state_t::FINISHED){
		// Запоминаем код ошибки разбора
		const error_t error = reader.error();
		// Запоминаем положение обнаруженной ошибки
		const location_t location = reader.errorLocation();
