...
namespace {
	 ...
	 *
	 * @details Запись числа средствами языка знак десятичной точки берёт из локали,
	 * а текст настроек - запись протокольная: под локалью de_DE «0.1» вышло бы как
	 * «0,1» и обратно уже не прочиталось бы
	 *
...
bool awh::codec::ini::Writer::number(const string_view key, const T value) noexcept {
	 ...
	 */
		...
		/**
		 * Если запись числа выполнена показательным видом
		 *
		 * @note Показательный вид кратчайшей записи выходит уже на числе в тысячу:
		 *       «-1.25e+03» вместо «-1250». Файл настроек пишется для человека, и
		 *       обычный вид ему понятнее - показательный оставляется лишь тогда,
		 *       когда обычный вышел бы непомерно длинным
		 */
		if((length > 0) && (::memchr(buffer, 'e', static_cast <size_t> (length)) != nullptr)){
			// Запись числа обычным видом
			char plain[64];
			/**
			 * Выполняем подбор кратчайшей записи числа обычным видом
			 */
			for(int32_t decimals = 0; decimals <= static_cast <int32_t> (numeric_limits <T>::max_digits10); decimals++){
				 ...
				 *
				 * @note Обычный вид берётся, лишь когда он не длиннее показательного:
				 *       «1250» короче «1.25e+03» и берётся, а «100000002004087734272»
				 *       длиннее «1e+20» - там показательный вид и короче, и понятнее
				 */
				if((size <= 0) || (static_cast <size_t> (size) >= sizeof(plain)) || (size > length))
					// Выполняем прекращение подбора точности записи
					break;
				// Выполняем замену знака десятичной точки локали точкой
				::pointed(plain, size);
				...
				T back = 0;
				/**
				 * Если запись обычным видом читается обратно тем же самым числом
				 */
				if(numeric(string_view(plain, static_cast <size_t> (size)), back) && (back == value)){
					// Выполняем перенос записи числа обычным видом
					::memcpy(buffer, plain, static_cast <size_t> (size));
					// Запоминаем длину записи числа обычным видом
					length = size;
					// Выполняем прекращение подбора точности записи
					break;
				}
