	...
	static bool unescape(const string_view text, const size_t offset, size_t & length, string & result, bool & hexed) noexcept {
		...
		switch(text[offset + 1]){
			...
			case 'U': {
				 ...
				 *
				 * @note Отчего замер молчал прежде: покрытие числило заход нетронутым оттого, что
				 *       подачи такой не было ни в наборе, ни у ворошителя, - а снятие захода
				 *       НЕРАЗЛИЧИМО быть и не могло, ибо иной поверки нет. Печать в теле
				 *       единственная и отвечает на срабатывание
				 *
