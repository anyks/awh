...
awh::codec::ini::Value & awh::codec::ini::Value::operator [] (const size_t index) noexcept {
	...
	if(index >= this->_items.size()){
		...
		if(this->_type == type_t::TABLE){
			 ...
			 * Выполняем снос указателя поиска
			 *
			 * @warning Снос обязателен: имена, ростом заведённые, указателю неведомы, и
			 *          розыск по ним отвечал бы отсутствием, тогда как перебор их находит.
			 *          Ответ тем разнился бы по числу пар - до порога заведения указателя
			 *          пара с именем пустым находилась, а за порогом пропадала
			 */
			this->unindex();
		}
...
bool awh::codec::ini::Value::erase(const string & name) noexcept {
	 ...
	 * Если значение вместилищем пар не является
	 *
	 * @warning Сторож этот равнозначен: у значения не-таблицы имён пар нет вовсе, и снос
	 *          отвечает отказом ниже - пары с таким именем не найдено. Порча захода
	 *          проверок не роняет, замерено. Сносу заход не подлежит: он называет отказ
	 *          видом значения, а не отсутствием имени
	 */
	if(this->_type != type_t::TABLE)
...
bool awh::codec::ini::Value::value(bool & result) const noexcept {
	 ...
	 * Если значение простым не является
	 *
	 * @warning Сторож этот равнозначен: у значения не-записи разбираемой записи нет, и
	 *          извлечение отвергается ниже - разбирать нечего. Порча захода проверок не
	 *          роняет, замерено. Сносу заход не подлежит: он снимает разбор заведомо
	 *          неподходящего значения
	 */
	if(this->_type != type_t::STRING)
		// Выводим признак неудачного извлечения
		return false;
...
void awh::codec::ini::Value::absorb(const Document & document) noexcept {
	...
	const auto gather = [&document](Value & holder, const string & section, const string & subsection) noexcept -> void {
		...
		for(auto & key : names){
			 ...
			 *
			 * @warning Заход этот НЕДОСТИЖИМ, покуда держится условие: имена свойств взяты
			 *          у самого дерева, и свойство, деревом названное, хотя бы одно значение
			 *          несёт всегда. Замерено покрытием: заход не задет ни разу
			 *
			 * @note Довод проверен ПО СУЩЕСТВУ чтением 07.09.2026, и цепь его такова: имена
			 *       выдаёт `keys()` из перечня ПЕРВЫХ объявлений `_order`, а значения - из
			 *       указателя `_properties`. Наполняются оба разом (`Document::set`: перечень
			 *       заводится и запись в него кладётся без промежутка, пустой доли не остаётся)
			 *       и опустошаются разом же (`Document::remove`: объявление изымается из
			 *       `_order`, а свойство - из `_properties`). Разойтись им негде
			 */
			if(values.empty())
	...
	for(auto & name : sections){
		 ...
		 *
		 * @warning Заход этот НЕДОСТИЖИМ, покуда держится условие: перечень разделов
		 *          дерево выдаёт без верхнего уровня - имя пустое в него не попадает.
		 *          Замерено покрытием: заход не задет ни разу
		 *
		 * @note Довод проверен ПО СУЩЕСТВУ чтением 07.09.2026: `sections()` выдаёт лишь
		 *       разделы с признаком `declared`, а ставится признак этот ровно в двух местах,
		 *       и оба пустое имя отвергают - разбор текста тремя заставами `EMPTY_SECTION`
		 *       у читающего, правка же заставою `acceptable()`, тем же кодом отвечающей.
		 *       Раздел без имени заводится лишь корнем и `declared` не получает никогда
		 *
		 * @note Сносу заход не подлежит: без него свойства верхнего уровня снялись бы
		 *       дважды - разделом с пустым именем поверх корня
		 */
		if(name.first.empty())
		...
		/**
		 * Если вместилище раздела вида ещё не имеет
		 *
		 * @note Раздел объявленный есть вместилище пар, пусть и пустое: без назначения
		 *       вида пустой раздел снимался бы значением неопределённым, а перенос такого
		 *       значения обратно в дерево отвечал бы отказом - объявление раздела теряло
		 *       бы себя круговым ходом. Нашёл это ворошитель круговым переносом
		 */
		if(section._type == type_t::NONE)
			// Назначаем вместилищу раздела вид вместилища пар
			section._type = type_t::TABLE;
		/**
		...
		/**
		 * Если вместилище подраздела вида ещё не имеет
		 */
		if(subsection._type == type_t::NONE)
			// Назначаем вместилищу подраздела вид вместилища пар
			subsection._type = type_t::TABLE;
...
bool awh::codec::ini::Value::load(const string & filename) noexcept {
	...
	string text;
	// Выполняем чтение содержимого файла настроек
	fs.read(filename, text);
	/**
	 * Если содержимое прочитать не удалось
	 */
	if(text.empty() && (fs.size(filename) > 0)){
		// Запоминаем код отказа чтения файла настроек
...
string awh::codec::ini::Value::dump(const writer_t::settings_t & settings) const noexcept {
	...
	const auto compose = [&writer](const Value & holder) noexcept -> bool {
		...
		for(size_t i = 0; i < holder._items.size(); i++){
			 ...
			 *
			 * @note Условие стоит ОДНОЮ половиною намеренно, в отличие от такого же
			 *       условия у наречия TOML, где оно стоит двумя. Имени пустого наречие INI
			 *       не знает вовсе: чтение отвергает строку `= значение` по отсутствию
			 *       имени, а запись `"" = значение` даёт имя длиною в два знака - кавычки
			 *       имя не ограждают, а входят в него. Проверено щупом на всех трёх путях,
			 *       а не выведено чтением; правило это одно на все шесть обходов файла
			 */
	...
	for(size_t i = 0; i < this->_items.size(); i++){
		...
		if(section._type != type_t::TABLE)
			...
			continue;
		/**
		 * @brief Признак раздела, свои свойства несущего
		 *
		 * @details Разделы наречия плоски: `[a "b"]` есть ОДИН раздел о двух именах, а
		 *          не два вложенных. Владеющее значение держит его вложенным ради
		 *          обхода по имени, и звено `a` там - устройство обхода, а не
		 *          объявление из текста. Объяви его запись - и в тексте завёлся бы
		 *          раздел, какого человек не писал
		 *
		 * @note Звено, свойств не несущее и подразделы несущее, объявлением потому не
		 *       выдаётся. Звено пустое ВОВСЕ - ни свойств, ни подразделов - выдаётся:
		 *       оно и есть раздел, человеком объявленный пустым. Нашло это сличение
		 *       двух путей перезаписи на корпусе соответствия: текст `[a "b"]` через
		 *       владеющее значение получал лишнюю строку `[a]`
		 */
		bool owned = section._items.empty();
		/**
		 * Выполняем розыск свойств раздела, подразделами не являющихся
		 */
		for(size_t j = 0; !owned && (j < section._items.size()); j++)
			// Запоминаем признак раздела, свои свойства несущего
			owned = (section._items.at(j)._type != type_t::TABLE);
		/**
		 * Если раздел свои свойства несёт
		 */
		if(owned){
			/**
 ...
 *
 * @note Имена, руками собранные, судятся у ИСТОКА - переносом в дерево, а не записью.
 *       Замерено щупом 07.09.2026: ключ `bad = key`, ключ с переводом строки и ключ с
 *       пробельною обвязкою отвергаются кодом INVALID_KEY (11), имя раздела со скобкой
 *       либо переводом - кодом INVALID_SECTION (6), и дерево при том остаётся прежним.
 *       Разряд беды, найденный Василием у кодека XML, - «привито успехом, а записи не
 *       подлежит» - здесь ОТСУТСТВУЕТ
 *
 */
bool awh::codec::ini::Value::graft(Document & document) const noexcept {
	 ...
	 * Если корень вместилищем пар не является
	 *
	 * @note Отказ называет причину кодом `UNREPRESENTABLE_VALUE`: запись INI вида
	 *       этого не выражает вовсе. Того же рода ещё два отказа укладки ниже -
	 *       значение перечня и значение пары, строковыми не являющиеся. Прежде все
	 *       три молчали, и `error()` отвечал «ошибок не обнаружено», что есть ложь
	 *       худшая молчания
	 *
	 * @note Ворошителем заходы эти не достигаются вовсе - значения он снимает с
	 *       дерева, а дерево иных видов не несёт, - и правило ворошителя «отказ
	 *       обязан назвать причину» на них молчит по этой причине, а не по их
	 *       отсутствию. Закреплены они проверкой GraftRefusalNamesItsCause
	 */
	if(this->_type != type_t::TABLE){
		// Запоминаем код отказа переноса исходному дереву
		document._error = error_t::UNREPRESENTABLE_VALUE;
			...
			if(item._type == type_t::ARRAY){
				...
				for(size_t j = 0; j < item._items.size(); j++){
					...
					if(entry._type != type_t::STRING){
						// Запоминаем код отказа переноса дереву-копии
						staging._error = error_t::UNREPRESENTABLE_VALUE;
						// Выводим признак неудачного переноса
						return false;
					}
					/**
					 ...
					 *
					 * @warning Заход этот НЕДОСТИЖИМ, покуда держится условие: объявления свойства
					 *          снесены вызовом `staging.erase(...)`, стоящим ПЕРЕД перебором значений
					 *          перечня, а раздел с подразделом объявлены прежде захода сюда -
					 *          дописывать значение некуда, кроме как в них. Замерено покрытием:
					 *          заход не задет ни разу
					 *
					 * @warning Прежняя запись гласила «снесены строкою выше», и расстояние это было
					 *          ЛОЖНО при верном существе: строкою выше стоит отсев значения не
					 *          строкового, а снос объявлений - вне перебора вовсе. Расстояние
					 *          основанием не годится: оно ветшает молча, тогда как имя вызова
					 *          устареет громко, вместе со сборкой
					 *
	...
	for(size_t i = 0; i < this->_items.size(); i++){
		...
		for(size_t j = 0; j < section._items.size(); j++){
			...
			for(auto & item : subsection._items){
				 ...
				 *
				 * @warning Заход этот НЕДОСТИЖИМ, покуда держится условие: вместилище третьего
				 *          уровня дерево отвергает раньше - построением имени подраздела, кодом
				 *          INVALID_SUBSECTION, - и до этой поверки перенос не доходит. Замерено
				 *          проверкой `RefusalsOfImpossibleShapes`, код отказа в ней закреплён
				 *
				 * @note Сносу заход не подлежит: он держит договор переноса на случай, если
				 *       дерево имя третьего уровня однажды примет
				 *
...
bool awh::codec::ini::Builder::expand(Value && value) noexcept {
	...
	if(this->_path.empty() && !this->_root.valid()){
		 ...
		 * Если имя свойства назначено
		 *
		 * @warning Заход этот НЕДОСТИЖИМ: назначение имени требует ОТКРЫТОГО вместилища
		 *          вида пар - `key()` отвергает всякое имя, покуда `opened()` вместилищем
		 *          пар не является, - а заход этот берётся лишь при вместилище корневом
		 *          ещё не открытом. Условия взаимно исключаются. Замерено
		 *
		 * @note Довод проверен ПО СУЩЕСТВУ чтением 07.09.2026: при пустом пути `opened()`
		 *       отдаёт сам корень, а корень неоткрытый вместилищем пар не является, и `key()`
		 *       отвечает отказом. Возврат же в состояние «корень не открыт» даёт один лишь
		 *       `reset()` (он же изнутри `finish()`), а тот признак назначенного имени сбрасывает
		 *
		 * @note Сносу заход не подлежит: он есть застава последнего рубежа. Ослабление
		 *       поверки у назначения имени вернёт сюда управление, и корень открылся бы
		 *       с именем, которому ложиться некуда
		 *
		 * @note Снятие стража этого НЕРАЗЛИЧИМО: замерено сличением двух сборок, одним лишь им
		 *       и различных. Наборы трёх кодеков зелены (397, 278, 276), ворошители на трёх
		 *       зёрнах по 20 000 текстов дают выдачу побайтно ту же
