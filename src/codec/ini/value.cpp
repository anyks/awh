...
bool awh::codec::ini::Value::graft(Document & document) const noexcept {
	 ...
	 * Если корень вместилищем пар не является
	 *
	 * @warning Отказ этот причины не называет, и `error()` отвечает «ошибок не
	 *          обнаружено»: кода, означающего «значение такого вида запись INI выразить
	 *          не может», перечень отказов не несёт, а заводить его - правка, потребителю
	 *          видная. Того же рода ещё два отказа укладки ниже: значение перечня и
	 *          значение пары, строковыми не являющиеся. Ворошителем заходы эти не
	 *          достигаются вовсе - значения он снимает с дерева, а дерево иных видов не
	 *          несёт, - и правило ворошителя «отказ обязан назвать причину» на них
	 *          молчит по этой причине, а не по их отсутствию
	 */
	if(this->_type != type_t::TABLE)
