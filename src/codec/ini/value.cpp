...
bool awh::codec::ini::Value::graft(Document & document) const noexcept {
	...
	const auto transfer = [&document](const Value & holder, const string_view section, const string_view subsection) noexcept -> bool {
		...
		for(size_t i = 0; i < holder._items.size(); i++){
			 ...
			 * Если значение пары является перечнем одноимённых значений
			 *
			 * @warning Перенос перечня в дерево настроек НЕ поддержан: договор правки
			 *          дерева кладёт одно значение свойства и повтора имени не строит.
			 *          Отказ намеренный - молчаливый пропуск оставил бы потребителя с
			 *          деревом без части его значения
			 */
			if(item._type == type_t::ARRAY)
				// Выводим признак неудачного переноса
				return false;
