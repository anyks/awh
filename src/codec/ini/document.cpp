...
bool awh::codec::ini::Document::graft(const string & path, const ::awh::codec::ini::Value & value) noexcept {
	...
	::awh::codec::ini::Value & target = whole.place(path);
	/**
	 * Если завести значение по пути не удалось
	 */
	if(!target.valid()){
	...
	// Выполняем постановку значения по пути
	target = value;
...
size_t awh::codec::ini::Document::size() const noexcept {
	...
	return ::awh::codec::ini::Value(* this).size();
}
/**
