...
bool awh::codec::ini::Document::set(const string_view key, const string_view value, const string_view section, const string_view subsection) noexcept {
	 ...
	 *          узнавал о беде вдали от места, где её завёл, и дерево оставалось незаписываемым
	 *          при коде отказа нулевом. Разряд беды тот же, что у подачи каталога: успех,
	 *          отданный за работу, какая не сделана. Найден сличением с кодеком XML, где ту же
	 *          дорогу закрыл Василий
	 *
	 * @note Судится ЛИШЬ годность последовательности, и это граница намеренная: она от настроек
	 *       записи не зависит вовсе. Прочие отказы записи - управляющий знак при переводе строки
	 *       да пробельная обвязка - зависят от настроек `escapes` и `indents`, а те вправе
	 *       смениться между правкой и записью. Судить о них здесь значило бы отвергать
	 *       содержимое, при иных настройках вполне записываемое: «о написанном нельзя судить
	 *       по настройке»
	 *
	 * @note Код тот же, каким отвечает запись, - INVALID_ENCODING: причина одна, и потребителю
	 *       незачем разбирать два кода на одну беду
	 */
