...
bool awh::codec::ini::Document::graft(const string & path, const ::awh::codec::ini::Value & value) noexcept {
	 ...
	 *
	 * @note Звенья пути, кроме последнего, обязаны быть НАЛИЦО: вместилищ по дороге не
	 *       заводится. Правило это взято у YAML дословно, где оно записано намеренным
	 *       решением - заведение недостающего звена обращает опечатку в пути в новую
	 *       ветвь дерева молча. TOML в этом расходится с обоими, и расхождение вынесено
	 *       владельцу отдельно, ибо касается общего договора, а не одного наречия
	 */
