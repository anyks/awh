...
bool awh::codec::json::Reader::feed(const void * buffer, const size_t size, const bool end) noexcept {
	 ...
	 *
	 * @warning Отказ здесь верен по существу - сменить кодировку посреди текста нельзя, - а
	 *          вот КОД его чужого смысла: `UNSUPPORTED_ENCODING` говорит «вид не поддержан»,
	 *          тогда как `UTF16LE` поддержан вполне, и беда в мгновении смены, а не в виде.
	 *          Своего кода такому случаю в перечне нет, а заведение члена перечня меняет
	 *          договор наружу: вынесено владельцу 08.09.2026 вместе с тем же расхождением у
	 *          кодеков INI, YAML и TOML
	 *
	 * @note Кодеки XML и CSV на ту же просьбу отвечают ИНАЧЕ и намеренно: несбывшееся
	 *       указание они снимают из настроек молча, дабы настройки не лгали о том, чего на
	 *       деле не происходит. Здесь же оно не теряется, а стоит текста - отказом при
	 *       ближайшей подаче. Оба ответа предписаны и записаны у обоих; расхождение внесено
	 *       владельцу в общий перечень «где договор, а где небрежность»
	 *
