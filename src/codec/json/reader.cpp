...
size_t awh::codec::json::Reader::bulk(const char * buffer, const size_t size) noexcept {
	...
	switch(static_cast <uint8_t> (this->_state)){
		...
		case static_cast <uint8_t> (state_t::STRING): {
			 ...
			 *
			 * @note Проход начинается словом, а не байтом, в отличие от прохода по
			 *       цифрам: строки в обиходе длиннее восьми байтов, и мерить их сперва
			 *       побайтно значило бы терять слово на всякой строке
			 */
