...
bool awh::codec::json::Value::compose(writer_t & writer) const noexcept {
	...
	switch(this->_kind){
		...
		case kind_t::NUMBER: {
			 ...
			 *
			 * @note Довод проверен ПЕРЕЧИСЛЕНИЕМ 07.09.2026: у `type_t` двадцать два члена -
			 *       одиннадцать числовых и одиннадцать прочих, - и все двадцать два стоят
			 *       ветвями выбора. Сличено и присвоение: `_type` получает лишь ПОИМЕНОВАННЫЕ
			 *       члены, сочетаний разрядов ему не присваивает ни одно место
			 *
		...
		case kind_t::OBJECT: {
			 ...
			 */
			/**
			 * @warning Довод, здесь прежде стоявший, звал отказ НЕДОСТИЖИМЫМ, ибо «отделитель
			 *          отвергает лишь второе значение верхнего уровня», и был НЕПОЛОН: открытие
			 *          вместилища отвергает ДВУМЯ дорогами, а довод знал одну. Вторая - предел
			 *          глубины `MAX_DEPTH`, и по ней отказ ДОСТИЖИМ был: значение, собранное
			 *          руками глубиною 1100, прививалось признаком успеха, а запись дерева
			 *          выдавала пустую строку. Замерено 08.09.2026
			 *
			 * @note Ныне дорога эта закрыта У ИСТОКА: перенос значения в дерево судит глубину
			 *       тем же пределом и отвергает кодом `DEPTH_EXCEEDED`. Молчание карты
			 *       покрытия означает здесь закрытую дорогу, а не недостижимость по
			 *       устройству - сними судью у переноса, и отказ оживёт в тот же день
			 *
			 * @note Отделитель же и вправду отвергает лишь второе значение верхнего уровня, а
			 *       запись дерева начинает поток с чистого места - эта половина довода верна
			 *
	 ...
	 *
	 * @note Довод проверен ПЕРЕЧИСЛЕНИЕМ 07.09.2026 тем же порядком, что и у записи числа:
	 *       члены перечня сличены с ветвями выбора поимённо
	 *
...
bool awh::codec::json::Value::parse(const string & text, const document_t::settings_t & settings) noexcept {
	 ...
	 */
	/**
	 * Раздаём журнал вглубь наравне с фреймворком
	 *
	 * @warning Без этого узлы, разбором заведённые, журнала не имеют вовсе, и отказ на
	 *          вложенном узле разыменовывает пустоту: ход записи заводит объект работы с
	 *          файловой системой журналом своего узла, а тот пустого указателя не терпит.
	 *          Прежде беду скрывал ручной зов назначения журнала в проверках - замерено
	 *          падением 13.09.2026, `Logging::allowed(this=0x0)`
	 */
	// Выводим признак успешного разбора
	return true;
}
...
bool awh::codec::json::Value::save(const string & filename, const writer_t::settings_t & settings) const noexcept {
	...
	if(text.empty()){
		 ...
		 *
		 * @note СЕТЬ БЕЗОПАСНОСТИ. Не покрыта набором ПОСТАНОВКА КОДА - она одна: заход
		 *       по пустоте текста берётся 15 раз, сторож кода отказа - однажды, и лишь
		 *       присвоение стоит нулём. Замерено 07.09.2026; прежняя редакция довода звала
		 *       непокрытой ВЕТВЬ, чем приписывала счёт тела заходу, а счета эти разные
		 *
	...
	/**
	 * Выполняем запись текста значения во временный файл
	 *
	...
	if(!::writable(fs, temporary)){
		...
		awh::log::print("JSON value failed: %s", awh::log::flag_t::CRITICAL, awh::codec::json::message(this->_error));
		// Выводим признак неудачной записи
		return false;
	}
	/**
...
bool awh::codec::json::Value::operator == (const Value & value) const noexcept {
	 ...
	 *       прежде
	 *
	 * @note Довод проверен ПЕРЕЧИСЛЕНИЕМ 08.09.2026: у `kind_t` семь членов - `NONE`, `NUL`,
	 *       `BOOL`, `NUMBER`, `STRING`, `ARRAY` и `OBJECT`, - и все семь стоят ветвями выбора
	 *       выше. Тождество это и есть доказательство: выход берётся лишь видом, ветви не
	 *       имеющим, а такого вида у значения нет
	 *
 ...
 *
 */
awh::codec::json::Builder::Builder() noexcept :
 _keyed(false), _appended(false) {}
/**
...
awh::codec::json::value_t * awh::codec::json::Builder::attach(const value_t & value) noexcept {
	 ...
	 *          пустому указанию
	 *
	 * @note Замерено 07.09.2026 сличением двух сборок: снятие каждого из отказов порознь
	 *       выдачи ворошителя на 20 000 текстов не меняет ни на байт. Доказана тем
	 *       НЕРАЗЛИЧИМОСТЬ снятия, а не недостижимость отказа: первая говорит «ни одна
	 *       подача не отличит», вторая — «ни одна подача не дойдёт», и вторая не доказуема
	 *       вовсе
	 *
	 * @warning Неразличимость снятия сносу НЕ ПОВОД: отказы эти держат договор между
	 *          сборщиком и вместилищем, и разойдись слои — станут видимы немедля
	 *
	 * @note Порча для такого замера ставится ОСОБО: условие вида `if(!parent->push(value))`
	 *       несёт в себе ДЕЙСТВИЕ, и замена его лжою убирает не проверку, а самоё действие.
	 *       Первый замер оттого показал различимость ложно; верный ход — вызвать действие
	 *       отдельной строкою, а условие обратить в ложь
	 *
	 * @note Само рассуждение проверено ЧТЕНИЕМ обоих принимающих 07.09.2026, а не замером:
	 *       `push()` отвергает единственным условием `_kind != ARRAY`, обращая неопределённое
	 *       значение массивом сам; `insert()` - единственным `_kind != OBJECT`, а поле с
	 *       занятым именем ПЕРЕЗАПИСЫВАЕТ, отказом не отвечая. Иных дорог к отказу у них
	 *       нет вовсе, и довод «отвергает лишь вместилище, значений принять не могущее»
	 *       держится не на замере, а на устройстве обоих
	 *
	 * @warning Основание у пяти отказов ниже ОБЩЕЕ: заведи `push()` либо `insert()` вторую
	 *          дорогу к отказу - хоть предел числа членов, хоть отказ на занятое имя при
	 *          строгих настройках, - и достижимыми станут они все пятеро разом. Правку
	 *          принимающих вести с оглядкою СЮДА
	 */
	/**
	...
	if(parent->kind() == kind_t::OBJECT){
		...
		if(this->_appended){
			 ...
			 *
			 * @note Отказ НЕДОСТИЖИМ - довод ССЫЛОЧНЫЙ, общее основание стоит у начала
			 *       помещения: вместилищем здесь служит узел, открытый последним, а тот
			 *       приходит открытием объекта либо массива и значений принять не может не
			 *       быть в состоянии. Там же и замер сличением двух сборок от 07.09.2026
			 */
		 ...
		 */
		/**
		 * @note Отказ этот НЕДОСТИЖИМ - довод изложен у начала помещения, и там же записано,
		 *       чем он проверен: чтением обоих принимающих 08.09.2026. Ссылочный довод своего
		 *       замера не имеет и иметь не должен - у общего основания и запись общая, - но и
		 *       переносить чужой замер на себя ему нельзя: он лишь указывает, где искать
		 */
 ...
 *
 */
uint32_t awh::codec::json::Document::transplant(const json::Value & value, const string * name, vector <node_t> & nodes, const uint32_t depth) noexcept {
	/**
	 * Если глубина вложенности переносимого значения превышает допустимую
	 *
	 ...
	 * него отказом `DEPTH_EXCEEDED`. Дерево же, собранное руками, до 08.09.2026 предела
	 * этого не знало вовсе
	 *
	 * @note Найдено 08.09.2026 чтением довода недостижимости у записи объекта: довод звал
	 *       непроходимым лишь отделитель, а `object()` отвергает ДВУМЯ дорогами, и вторая -
	 *       предел глубины. Замер: значение, собранное руками глубиною 1100, прививалось
	 *       признаком УСПЕХА при коде нулевом, а запись дерева выдавала пустую строку при
	 *       коде `DEPTH_EXCEEDED` - потребитель узнавал о беде далеко от места, где её завёл
	 *
	 * @note Судить глубину при правке МОЖНО, ибо предел этот от настроек не зависит: правило
	 *       «о написанном нельзя судить по настройке, какая вправе смениться» здесь не
	 *       мешает, в отличие от знаков, чья запись настройками и решается
	 *
	 */
	if(depth >= MAX_DEPTH){
		// Запоминаем код отказа переноса значения глубже допустимого
	...
	switch(value.kind()){
		...
		case kind_t::STRING: {
			 ...
			 *
			 * @note Довод ССЫЛОЧНЫЙ, и своего замера у него нет намеренно: запись ведётся
			 *       ОДИН раз при общем основании - у сторожа имени поля выше, - а здесь
			 *       лишь указано, где её искать. Там же стоит и замер щупом по местам
			 *       отказа от 04.09.2026
			 */
		...
		case kind_t::NUMBER: {
			...
			if(value.type() == type_t::EXTENDED){
				 ...
				 *
				 * @warning Порог этот набором НЕ покрыт и покрыт быть не может по той же
				 *          причине, что и два его собрата: четыре гигабайта - разрядность
				 *          смещения. Доказанная подмена кода (06.09.2026) не красит ни
				 *          одной проверки
				 */
		...
		case kind_t::OBJECT: {
			...
			for(size_t i = 0; i < value.size(); i++)
				...
				extent += this->transplant(value[i], (keyed ? &value.key(i) : nullptr), nodes, (depth + 1));
...
bool awh::codec::json::Document::set(const string & pointer, const json::Value & value) noexcept {
	...
	this->transplant(value, (keyed ? &name : nullptr), nodes, 0);
	/**
	 * Если перенос прививаемого значения завершился отказом
	 *
	 * @details Отказ переноса оставляет перечень узлов НЕПОЛНЫМ, и вставка такого перечня в
	 * дерево обратила бы его порчею: размахи поддеревьев разошлись бы с числом узлов
	 *
	 * @warning Проверка эта стоит ДО правки дерева, и порядок существен: прежде она
	 *          отсутствовала вовсе, и неполное поддерево вставлялось молча, а прививка
	 *          отвечала успехом. Найдено 08.09.2026 вместе с пределом глубины
	 *
	 * @note Дерево при отказе остаётся нетронутым: ни один узел его к этому мигу не снят и
	 *       не переставлен
	 */
	if(this->_error != error_t::NONE)
		// Выводим признак неудачной прививки
