...
namespace {
	 ...
	 *       20.08.2026, а запись эта отстала и говорила про усечение десять дней спустя.
	 *       Правило закреплено проверкой `Rounding`
	 *
	 * @note Запись числа ответа не меняет: `300` и `300.0` дают 44 в виде на один байт,
	 *       `-1` и `-1.0` дают 18446744073709551615 в беззнаковом виде на восемь. Прежде
	 *       записи расходились (44 против 255), и снято это решением владельца от
	 *       30.08.2026; правило закреплено проверкой `BothSpellingsOfANumberExtractAlike`
	 *
