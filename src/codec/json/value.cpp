 ...
 *
 */
void awh::codec::json::Value::setFramework(const fmk_t * fmk) noexcept {
	// Устанавливаем объект фреймворка
	this->_fmk = fmk;
	/**
	 * Выполняем перебор всех вложенных значений
	 *
	 * @note Объект уходит вглубь наравне с журналом: значение владеет вложенными целиком,
	 *       и работа с файловой системой у них общая
	 */
	for(auto & item : this->_items)
		// Выполняем установку объекта фреймворка вложенному значению
 ...
 *
 */
awh::codec::json::Value::Value(const fmk_t * fmk, const log_t * log) noexcept :
 _kind(kind_t::NONE), _type(type_t::UNDEFINED), _indexed(false) {
	// Выполняем сброс хранимого числа
	this->_number.natural = 0;
	// Устанавливаем объект ведения журнала работы
	this->_log = log;
