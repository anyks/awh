...
awh::codec::toml::Value & awh::codec::toml::Value::operator [] (const size_t index) noexcept {
	...
	if(index >= this->_items.size()){
		...
		if(this->_type == type_t::TABLE){
			 ...
			 *          пара с именем пустым находилась, а за порогом пропадала, - и
			 *          установка по тому же имени клала бы вторую пару рядом с первой,
...
bool awh::codec::toml::Value::extract(T & result) const noexcept {
	...
	switch(static_cast <uint8_t> (this->_type)){
		...
		case static_cast <uint8_t> (type_t::FLOAT):
			...
			result = ::awh::codec::convert <T> (this->_real);
		break;
		/**
 ...
 *
 * @note Отказов у связки этой РОВНО ДВА, и сочтены они чтением тела 07.09.2026: неудача
 *       `document.get(path, content)` да ветвь `default` выбора по виду значения, отметкою
 *       времени не оказавшегося. На этом основании стоят обе метки недостижимости при
 *       вызовах `absorb` - у таблиц набора и у пар таблицы, - и записано оно здесь, а не
 *       повторами при них: рухнет основание - падут обе разом
 *
 * @note Вторая дорога отказа доводами при вызовах НЕ НАЗЫВАЛАСЬ вовсе, и потому проверена
 *       перечислением: у перечня `type_t` пятнадцать членов; шесть разобраны ветвями
 *       (STRING, INTEGER, FLOAT, BOOLEAN, ARRAY, TABLE), четыре ловит `stamped()`
 *       (OFFSET_DATETIME, LOCAL_DATETIME, LOCAL_DATE, LOCAL_TIME), а прочих пять узлу
 *       дерева не присваиваются нигде: NONE отдаётся лишь при узле отсутствующем, а
 *       BASIC, LITERAL, MULTILINE_BASIC и MULTILINE_LITERAL суть ФОРМЫ ЗАПИСИ строки и
 *       живут при записи, а не при виде узла. Сличены грепом все присвоения `type =`
 *       у дерева и у читающего
 *
 */
...
bool awh::codec::toml::Value::save(const string & filename) const noexcept {
	 ...
	 *
	 * @warning Заход этот - СЕТЬ БЕЗОПАСНОСТИ, и он ДОСТИЖИМ: выдача возвращает пустой
	 *          текст при непустом содержимом, когда содержимое состоит из значений
	 *          неопределённых, и заход тогда получает управление. Замерено родным
	 *          покрытием clang: за набором кодека он берётся однажды, проверкою
	 *          `CodecTomlValue.DumpSkipsHolesAndRefusesUndefined`
	 *
	 * @warning Прежде тут стояло «объединённое покрытие числит его непройденным», и
	 *          это было ЛОЖНО: непройденною числилась лишь выдача сообщения внутри,
	 *          а сам заход брался. Метка о непройденности ставилась по строке тела, а
	 *          не по строке захода - счёт у них разный
	 *
	 * @note Выдача сообщения об отказе внутри и вправду не берётся, но не оттого, что
	 *       заход недостижим, а оттого, что значение в той проверке заведено БЕЗ
	 *       объекта ведения журнала. Разница эта существенна: недостижимость захода и
	 *       ненастроенность журнала - разные причины, и лечатся они разным
	 *
	 * @note Сносу заход не подлежит: без него запись пустоты легла бы на цель и
	 *       затёрла прежнее её содержимое, отчитавшись успехом. Довод тот же, каким
	 *       держится такой же заход у кодека JSON
	 */
...
bool awh::codec::toml::Value::inflate(Document & document, const vector <string_view> & path) const noexcept {
	...
	for(size_t i = 0; i < this->_items.size(); i++){
		...
		if(this->_type == type_t::ARRAY){
			 ...
			 *
			 * @note Довод проверен ПО СУЩЕСТВУ чтением 07.09.2026: отказов у `Document::push`
			 *       ровно два - место, перечнем не являющееся (его и закрывает довод выше), да
			 *       нехватка места в хранилище знаков по `spacious()`. Вторую дорогу довод не
			 *       называл вовсе, и стоит она на том же основании, что и прочие заставы
			 *       хранилища: предел его четыре гигабайта, и ни набором, ни ворошителем его
			 *       не набрать. Смена предела вернёт сюда управление
			 *
		...
		/**
		 * Если номер очередной пары за перечень имён встроенной таблицы выходит
		 *
		 * @warning Заголовок довода прежде звучал «пара дырою является» и описывал заставу
		 *          СОСЕДНЮЮ, стоящую строкою ниже: вставка легла в чужой блок и с чужим
		 *          отступом. Исправлено 07.09.2026 чтением по существу
		 *
 ...
 *       Замерено щупом 07.09.2026: имя с переводом строки и имя со знаком управляющим
 *       отвергаются кодом INVALID_KEY (9) при дереве нетронутом, тогда как имя с пробелом
 *       и имя пустое описанием ДОЗВОЛЕНЫ и записываются оградою - `"bad key" = "v"` и
 *       `"" = "v"`. Разряд беды «привито успехом, а записи не подлежит» здесь ОТСУТСТВУЕТ
 *
 */
bool awh::codec::toml::Value::graft(Document & document, const vector <string_view> & path) const noexcept {
	 ...
	 *
	 * @details Дерево, собранное руками, предела глубины не знало вовсе: разбор текста
	 * огранивает настройку `maxDepth` потолком `MAX_DEPTH` перед работою, а прививка
	 * значения шла мимо всякого суждения о глубине. Оттого дерево принимало значение,
	 * какого запись принять не может: вложенность в семьдесят массивов прививалась
	 * УСПЕХОМ, а `dump()` затем отдавал ПУСТУЮ строку с кодом `DEPTH_EXCEEDED`, и
	 * документ становился незаписуемым без всякого признака отказа при правке
	 *
	 * @note Судить здесь потолком `MAX_DEPTH`, а не настройкою записи, ВЕРНО: настройка
	 *       эта у записи потолком не огранена, но дерево глубже потолка не примет и
	 *       обратный разбор - его настройка огранивается всегда. Стало быть, потолок
	 *       есть общая граница записуемости и читаемости, и она от настроек не зависит
	 *
	 ...
	 *
	 * @note Правка эта родня находке о негодной кодировке при прививке: там дерево тоже
	 *       принимало то, чего запись принять не может. Разряд один - прививка судила
	 *       годность значения не тем же мерилом, каким судит её запись
	 */
	{
		// Наибольшая встреченная глубина вложенности значения
		size_t deepness = 0;
		...
		while(!stack.empty()){
			...
			stack.pop_back();
			/**
			 * Если глубина залегания узла превышает встреченную прежде
			 */
			if(entry.second > deepness)
				// Запоминаем наибольшую встреченную глубину вложенности
				deepness = entry.second;
			/**
			 * Если глубина залегания узла превышает потолок допустимого
			 */
			if(entry.second > static_cast <size_t> (MAX_DEPTH)){
				// Запоминаем код ошибки переноса значения
