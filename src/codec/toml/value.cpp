...
bool awh::codec::toml::Value::save(const string & filename) const noexcept {
	 ...
	 *
	 * @warning Заход этот - СЕТЬ БЕЗОПАСНОСТИ, и объединённое покрытие числит его
	 *          непройденным: выдача текста при значении с содержимым пустоты не
	 *          отдаёт, и до захода дело не доходит. Достижим он стал бы лишь тогда,
	 *          когда выдача вернула пустой текст при непустом содержимом, - то есть
	 *          при собственном изъяне выдачи
	 *
