...
awh::codec::toml::utf8_t awh::codec::toml::inspect(const string_view text, const size_t offset, uint32_t & code, size_t & length) noexcept {
	 ...
	 *
	 * @warning Заход этот есть застава последнего рубежа, ничем не задетая: текст
	 *          доходит до разбора лишь по приведении к UTF-8, а приведение байт,
	 *          ведущим не бывающий, отвергает прежде - кодом INVALID_ENCODING.
	 *          Замерено охватом: подача «k\xFF = 1» сюда управления не доводит.
