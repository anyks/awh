...
awh::codec::toml::utf8_t awh::codec::toml::inspect(const string_view text, const size_t offset, uint32_t & code, size_t & length) noexcept {
	 ...
	 */
	/**
	 * Если разбор начат за пределом переданного текста
	 *
	 * @warning Заход этот есть застава последнего рубежа, ничем не задетая: всякий зовущий
	 *          держит положение внутри текста и за предел его не заводит. Сносу заход не
	 *          подлежит: чтение за пределом текста иначе обратилось бы обращением к памяти чужой
	 *
	 * @note Зовущих ТРИ, и сочтены они грепом по `inspect(` во всём кодеке (07.09.2026),
	 *       за вычетом самого определения; прочтён каждый:
	 *       1. `encoding.cpp` у `decode(buffer, size, length)` - положение ноль, а пустой
	 *          кусок отсеян заставою `(buffer == nullptr) || (size == 0)` строкою выше;
	 *       2. `common.cpp` у `decode(text, offset, code)` - положение передаётся насквозь,
	 *          и зовут его двое: `encoding.cpp` разбором удержанного знака, где длина не
	 *          меньше числа байт последовательности, да `writer.cpp` перебором имени с
	 *          условием `i < part.name.length()`;
	 *       3. `reader.cpp` разбором знака Юникода имени - положение держится перебором
	 *          внутри накопленного текста
	 *
	 * @warning Довод прежде стоял в ЕДИНСТВЕННОМ числе - «зовущий перебирает знаки», - и тем
	 *          ручался за одного, покрывая собою троих. Разряд этот уже давал ложную метку
	 *          у `yaml/common.cpp` при `floating`, где зовущих оказалось трое и один доходил
	 *
	 ...
	 */
	/**
	 * Если длина последовательности по ведущему байту не разобрана
	 *
	 * @warning Заход этот ДОСТИЖИМ и СРАБАТЫВАЕТ. Доказано печатью, в условие
	 *          поставленной: за набором проверок TOML заход берётся. Прежде здесь
	 *          стояло «застава последнего рубежа, ничем не задетая», и довод при том
	 *          был ложен: приведение к UTF-8 отвергает не всякий байт, ведущим не
	 *          бывающий, и часть их доходит до разбора
	 *
	 * @warning Замер прежний - «подача „k\xFF = 1“ сюда управления не доводит» - был
	 *          ВЕРЕН и БЕСПОЛЕЗЕН: он закрыл дорогу известную, а не всякую. Разряд этот
	 *          пал сегодня семь раз у двух кодеков разом
	 *
	 * @note Отчего замер молчал прежде: сличение двух сборок отвечает на РАЗЛИЧИЕ снятия,
	 *       а не на срабатывание. Страж может браться и быть при том неразличимым, если
	 *       поверка ниже выдаёт тот же отказ. Достижимость доказывает лишь ПЕЧАТЬ в теле
	 *          Сносу заход не подлежит: разбор, зовущий это тело помимо приведения,
	 *          шагал бы по байтам вслепую и собрал бы имя, знаков не составляющее
	 */
	if(count == 0){
 ...
 *
 */
awh::codec::toml::encoding_t awh::codec::toml::encoding(const string_view text) noexcept {
	/**
