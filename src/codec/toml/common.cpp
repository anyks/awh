...
size_t awh::codec::toml::decode(const string_view text, const size_t offset, uint32_t & code) noexcept {
	 ...
	 */
		...
		// Запоминаем количество байт знака Юникода
		length = 2;
	...
	if((offset + length) > text.length())
		// Выводим количество байт оборванного знака
		return 0;
	/**
	...
	for(size_t i = 1; i < length; i++){
		...
		code = ((code << 6) | (letter & 0x3F));
	}
	/**
	 * Если знак записан длиннее необходимого
	 *
	 ...
	 */
	if((code < MINIMAL[length]) || (code > 0x10FFFF) || ((code >= 0xD800) && (code <= 0xDFFF)))
		// Выводим количество байт битого знака
		return 0;
	// Выводим количество байт знака Юникода
	return length;
