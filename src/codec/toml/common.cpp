...
awh::codec::toml::utf8_t awh::codec::toml::inspect(const string_view text, const size_t offset, uint32_t & code, size_t & length) noexcept {
	 ...
	 *          перебирает знаки, покуда положение внутри текста, и за предел его не
	 *          заводит. Сносу заход не подлежит: чтение за пределом текста иначе
	 *          обратилось бы обращением к памяти чужой
	 *
