...
size_t awh::codec::toml::sequence(const uint8_t leading) noexcept {
	 ...
	 *       всей последовательности - поверкою готового значения ЗА перебором. Поверка та
	 *       снята вместе с введением пределов по месту, и без сего отсева запись `C0 80`
	 *       прошла бы годною со знаком нуль. Замерено исчерпывающим сличением с телом
	 *       наречия YAML по всем записям до трёх байт: 82 245 расхождений из 16 843 008
	 */
...
awh::codec::toml::utf8_t awh::codec::toml::inspect(const string_view text, const size_t offset, uint32_t & code, size_t & length) noexcept {
	...
	for(size_t i = 1; i < count; i++){
		 ...
		 *
		 * @note Поверка эта стоит **внутри** перебора, а не до него: прежде она стояла
		 ...
		 *       наибольшую часть последовательности. Прежде длина оставалась НУЛЁМ, и
		 *       зовущий, желавший перешагнуть оборванную последовательность целиком, шагал
		 *       по одному байту
		 */
		 ...
		 *
		 * @note Пределы эти введены ВМЕСТО поверки готового значения по наименьшему
		 *       кодовому да по пределам Юникода, что стояла ЗА перебором: та отмеряла
		 *       негодной последовательности длину ЦЕЛИКОМ, тогда как наречие YAML тем же
		 *       телом отмеряет её по первому негодному байту. Расхождение видно подменою
		 *       знаком замены: на записи `E0 80 80` одно тело давало ОДИН знак, другое ТРИ
		 */
