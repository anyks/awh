...
bool awh::codec::toml::Writer::ready() noexcept {
	 ...
	 * Если своего кода отказа запись ещё не несёт
	 *
	 * @warning Код выбирается ПО ВИДУ незакрытого окружения, а не двоичным выбором
	 *          «имя без значения либо изъян кодека». Прежде стоял выбор из двух, и всякое
	 *          окружение, кроме ожидания значения, получало `INTERNAL` - обвинение кодеку
	 *          в собственном сбое за обычный недосмотр зовущего, забывшего закрыть
	 *          перечень либо встроенную таблицу. Потребитель, такое прочтя, идёт слать
	 *          донесение о пороке рамки вместо того, чтобы закрыть скобку
	 *
	 * @note Достижимость замерена 15.09.2026 пометкою самой ветви: ворошитель на трёх
	 *       тысячах текстов брал её 1364 раза. То есть «состояние невозможное» было
	 *       состоянием ОБЫЧНЫМ, и слово о невозможности лгало
	 *
	 * @note `INTERNAL` оставлен одному лишь `ROOT`: окружение это ставится при ПУСТОМ
	 *       перечне уровней, а сюда ход доходит лишь при непустом, - вот там оно и вправду
	 *       невозможно
	 */
	if(this->_error == error_t::NONE){
		/**
		 * Выбираем код отказа по виду незакрытого окружения
		 */
		switch(static_cast <uint8_t> (this->_levels.back().context)){
			// Если записано имя ключа, а значение его не записано
			case static_cast <uint8_t> (context_t::KEYED):
				// Запоминаем код отказа пропущенного значения
				this->_error = error_t::MISSING_VALUE;
			break;
			// Если запись ведётся внутри перечня значений
			case static_cast <uint8_t> (context_t::ARRAY):
				// Запоминаем код отказа незакрытого перечня
				this->_error = error_t::UNCLOSED_ARRAY;
			break;
			// Если запись ведётся внутри встроенной таблицы
			case static_cast <uint8_t> (context_t::INLINE):
				// Запоминаем код отказа незакрытой встроенной таблицы
				this->_error = error_t::UNCLOSED_INLINE_TABLE;
			break;
			// Если вид окружения иной
			default:
				// Запоминаем код отказа изъяна кодека
...
bool awh::codec::toml::Writer::declare(const part_t * parts, const size_t count, const bool array) noexcept {
	...
	if(this->_settings.separated && !this->_text.empty()){
		...
		if((this->_text.length() < length) || (this->_text.compare(this->_text.length() - length, length, string(newline).append(newline)) != 0)){
			 ...
			 *          `maxLine`, меньшем её, заход стал бы достижим
			 *
			 * @note Звено это ЗАКРЫТО замером 09.09.2026, и мерилось само спорное количество,
			 *       а не исход: печать выведена на КАЖДОЕ вычисление захода и печатала `_length`.
			 *       За набором кодека и щупом из 480 подач - вид перевода строки LF и CRLF на
			 *       пределы `maxLine` от 0 до 59, четырьмя очертаниями записи, среди коих и
			 *       многострочное значение, зовущее `fold()`, - заход вычислялся 253 раза, и
			 *       длина при нём была НОЛЬ у всех 253. При нулевой длине `newline()` отказать
			 *       не может вовсе: сличение его есть `_length > maxLine`
			 *
			 * @warning Прежний опыт бил мимо: он менял предел, но НЕ менял вида перевода строки,
			 *          а опасение стояло именно на длине перевода. Перебирать надлежит ту самую
			 *          величину, на какой стоит опасение, а не соседнюю с нею
			 *
			 * @warning Замер длины сильнее замера исхода, но доказательством по-прежнему не
			 *          является: он говорит, что подачи с ненулевой длиной среди испытанных
			 *          нет, а не что её нет вовсе
			 *
...
bool awh::codec::toml::Writer::text(const string_view text, const string_t quoting) noexcept {
	 ...
	 *       общего правила: оно одинаково у ВСЕХ кодеков, а умолчанием ему замена. Два
...
bool awh::codec::toml::Writer::nestable() noexcept {
	 ...
	 *          подниматься, - а решения такие выносятся за пределы кодека
	 */
 ...
 */
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Writer::number <char> (const string_view, const char) noexcept;
