...
bool awh::codec::toml::Writer::text(const string_view text, const string_t quoting) noexcept {
	...
	if(this->_error != error_t::NONE)
		...
		return false;
	/**
	 * Если строковое значение годным UTF-8 не является
	 *
	 ...
	 *
	 * @note Отказ здесь ЛУЧШЕ подмены: наречие YAML в том же случае подставляло знак
	 *       замены, и данные потребителя менялись молча, без отказа вовсе. Отказ же
	 *       называет причину и оставляет решение потребителю
	 */
	if(!::awh::utf8::valid(text)){
		// Запоминаем код отказа записи значения
