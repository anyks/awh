...
bool awh::codec::toml::Document::set(const vector <string_view> & path, const string_view value, const string_t quoting) noexcept {
	...
	this->fault(error_t::NONE);
	/**
	 * Если содержимое значения кодировке UTF-8 не отвечает
	 *
	 * @details Дерево принимало такое значение УСПЕХОМ, а отказывала лишь запись кодом
	 *          INVALID_ENCODING - потребитель узнавал о беде вдали от места, где её завёл, и
	 *          дерево оставалось незаписываемым при коде отказа нулевом. Замерено щупом
	 *          08.09.2026 подачею `bad\xC3\x28ok` да оборванной последовательности `\xE2\x82`
	 *
	 * @note Судится ЛИШЬ годность последовательности, и граница эта намеренна: описание TOML
	 *       требует UTF-8 от всего текста, и требование это от настроек записи не зависит
	 *       вовсе. Знак же управляющий у TOML отказа НЕ ВЫЗЫВАЕТ: запись ограждает его
	 *       последовательностью `\u00XX` - замерено тем же щупом, - и судить его здесь
	 *       значило бы отвергать вполне записываемое
	 *
	 * @note Разряд беды найден Василием у кодека XML и сличением перенесён сюда: успех,
	 *       отданный за работу, какая не сделана. Тот же разряд, что у подачи каталога
	 */
	if(!::awh::utf8::valid(value)){
		// Запоминаем код ошибки правки дерева настроек
		this->fault(error_t::INVALID_ENCODING);
		// Выполняем вывод сообщения об отказе в лог
		this->report();
		// Выводим отрицательный результат выполнения операции
		return false;
...
string awh::codec::toml::Document::text(const writer_t::settings_t & settings) const noexcept {
	 ...
	 *       не велось. У кодека INI суд этот был, у TOML и YAML - нет, и расхождение то
	 *       случайно
	 *
