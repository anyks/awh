...
namespace {
	 ...
	 * перечнем видов РАЗРЯДНЫХ, потому извлечение видом языка ведётся через вид
	 * разрядный той же ширины и знаковости - перенос этот точен и разряда не теряет
	 *
	 */
	template <typename T>
...
bool awh::codec::toml::Document::spacious(const size_t length) noexcept {
	 ...
	 *          заняло бы четыре гигабайта памяти. Сносу заход не подлежит: смещения
	 *          отрезков хранятся четырьмя байтами, и переполнение обратило бы их в
	 *          указатели на чужие места хранилища
	 *
	 * @warning Прежде здесь стояло «от него же идут и ОБА возврата пустого указателя у
	 *          зовущих», и число это ЛОЖНО. Сверено 07.09.2026: зовущих ПЯТЕРО, и пустой
	 *          указатель возвращает из них ОДИН - `reach()`. Второй же возврат пустого
	 *          указателя, стоящий в `reach()` строкою ниже, идёт вовсе не отсюда, а от
	 *          `acceptable()`, и он ДОСТИЖИМ: негодное составное имя отвергается им обычным
	 *          порядком. Прочие четверо зовущих отвечают признаком `false`
	 *
	 * @note Разряд ложности здесь - ПРОМАХ С РОЖДЕНИЯ, а не износ записи, и это ЗАМЕРЕНО,
	 *       а не выведено: `git show <снимок>:src/codec/toml/document.cpp | grep -A3 "spacious("`
	 *       по трём снимкам подряд даёт одно и то же - ОДИН `return nullptr` и ЧЕТЫРЕ
	 *       `return false`. Стало быть, «оба возврата» не были верны ни при одном состоянии
	 *       кода. Лечение у промаха и у износа РАЗНОЕ: износ лечит способ пересчёта, промах
	 *       же - лишь пересчёт при написании, и одно другого не заменяет
	 *
	 * @warning Причину ложности надлежит проверять наравне с самой ложностью: довод «это
	 *          износ» есть такой же довод при коде и такому же сличению подлежит. Причина
	 *          БЛАГОВИДНАЯ соблазняет принять её без проверки - износ никого не винит, промах
	 *          винит писавшего, - а стоит проверка ровно столько же. Урок принесён от
	 *          кодека ABC, где износом были объявлены два числа, а сличение по пяти
	 *          снимкам нашло оба промахами
	 *
	 * @note СПОСОБ ПЕРЕСЧЁТА, при числе оставленный нарочно: `grep -rn "spacious(" src/codec/toml/`
	 *       за вычетом строки определения даёт места вызова, а `sed` по трём строкам ниже
	 *       каждого - вид его отдачи (`return nullptr` либо `return false`). Число, способа
	 *       пересчёта при себе не несущее, стареет МОЛЧА: оно верно в миг написания и
	 *       расходится потом от чужой правки, тогда как довод о существовании поверки
	 *       стареет громко, вместе со сборкой. Мерка принесена от кодека ABC,
	 *       где два числа разошлись именно так - износом записи, а не промахом счёта
	 */
...
bool awh::codec::toml::Document::parse(const string_view text) noexcept {
	 ...
	 *
	 * @details Разбор вложенности ведётся стопою вызовов, а работы по готовому дереву -
	 * снятие владеющим значением да запись - идут возвратом, и глубина ложится прямо на
	 * стек. Настройка, поднятая потребителем выше потолка, обходила его вовсе, и текст
	 * из одних открывающих скобок срывал стек: замерено 04.09.2026 щупом - при
	 * `maxDepth = 200000` глубина 100000 валила процесс по SIGSEGV, а при настройке по
	 * умолчанию тот же текст отвечал честным отказом
	 *
	 * @note Ноль огранивается НАРАВНЕ с числом завышенным: ноль значит «без предела» у
	 *       всех кодеков рамки, а дерево без предела срывает стек. Запрет вложенности
	 *       выражается ныне признаком `nesting`, а не нулём предела
	 *
	 * @note Огранение идёт молча и до разбора: отказ выдаёт сам разбор - ровно там, где
	 *       текст предел и превысил, и с местом в тексте
	 *
	 * @note Устроено это тем же порядком, что и у контейнера JSON с деревом разметки
	 */
	if((settings.maxDepth == 0) || (settings.maxDepth > MAX_DEPTH))
		// Выполняем огранение предела вложенности потолком допустимого
 ...
 *
 * @details Ход этот общий у кодеков рамки: пустое значение от снятия узла отлично - пара
 * остаётся налицо, а значения при ней нет. Недоставало его здесь одному наречию из трёх,
 * и найдено это сличением общего договора по кодекам, а не отказом
 *
 ...
 *
 */
bool awh::codec::toml::Document::reset(const string & path) noexcept {
	// Выводим результат постановки пустого значения по указанному пути
	return this->graft(string_view(path), ::awh::codec::toml::Value(string()));
}
/**
 ...
 * звено)` неминуемо ведёт к тому самому ребёнку. У таблицы звеном служит имя пары, у
 * перечня значений и у набора одноимённых таблиц - порядковый номер
 *
 ...
 * выдаётся ЗАПИСАННЫМ, а не как есть. Отдай мы имя как есть - путь разомкнулся бы
 * молча, уведя обход не туда
 *
 * @note Набор одноимённых таблиц `[[имя]]` есть у наречия перечень, и потому звенья к
 *       его таблицам ЧИСЛОВЫЕ. Прежний перечень дочерних имён на нём законно молчит -
 *       набор таблицею не является, - и обход на нём разомкнулся бы; здесь он замкнут.
 *       Закреплено проверкою `CodecTomlDocument.ArrayOfTablesKeepsTraversalClosed`
 *
 * @param path путь до значения
 * @return     перечень звеньев пути к детям значения
 *
 */
vector <string> awh::codec::toml::Document::children(const string_view path) const noexcept {
	// Перечень звеньев пути к детям значения
	vector <string> result;
	// Выполняем розыск значения по указанному пути
	const ::awh::codec::toml::Value value = this->at(string(path));
	/**
	 * Если значение по пути не разыскано
	 *
	 * @note Заход этот есть СОКРАЩЕНИЕ, а не застава: значение негодное ни таблицею, ни
	 *       перечнем не является, и оба разбора ниже его пропускают - перечень звеньев
	 *       выходит пустым и без этого захода. Доказано порчею на копии дерева: заход,
	 *       обращённый в `if(false)`, проверки не валит, а щуп на всех трёх негодных
	 *       путях отдаёт нуль детей по-прежнему
	 *
	 * @note Сносу заход при том не подлежит: он избавляет от розыска вида да выделения
	 *       места под звенья там, где выдавать нечего
	 */
	if(!value.valid())
	 ...
	 */
	if(value.is(type_t::TABLE)){
		/**
		 * Выполняем перебор всех пар таблицы
		 */
		for(size_t i = 0; i < value.size(); i++)
			// Выполняем добавление имени пары, записанного звеном пути
			result.push_back(::escape(value.key(i)));
	/**
	 * Если значение является перечнем значений либо набором одноимённых таблиц
	 */
	} else if(value.is(type_t::ARRAY)) {
		/**
		 * Выполняем перебор всех значений перечня
		 */
		for(size_t i = 0; i < value.size(); i++)
			// Выполняем добавление порядкового номера, звеном пути служащего
...
bool awh::codec::toml::Document::has(const vector <string_view> & path) const noexcept {
	/**
	 * @note Граница этой связки ЗАМЕРЕНА 09.09.2026, ибо по имени она не очевидна и
	 *       выводилась мною заново. Связка отвечает о ПАРЕ, и значение ВНУТРИ пары
	 *       считает за неё, а таблицу - нет:
	 *
	 *         таблица `[т]`          has=0  present=1
	 *         пара `k` в таблице     has=1  present=1
	 *         набор таблиц `[[н]]`   has=0  present=1
	 *         элемент набора `н/0`   has=0  present=1
	 *         перечень `с` парою     has=1  present=1
	 *         элемент перечня `с/0`  has=1  present=1
	 *
	 * @note Отсюда `present()` есть общий розыск наличия по пути, а `has()` - розыск
	 *       ПАРЫ. Связки эти НЕ равнозначны, и подменять одну другою нельзя
	 *
	 * @warning Свод «has отрицает элемент набора, но подтверждает его ребёнка» выглядит
	 *          противоречием, а противоречия нет: ребёнок `н/0/a` есть пара своею
	 *          записью, а не находка спуском сквозь `н/0`. Розыск начинается с записи
	 *          пары и лишь затем спускается в значение её
	 *
	 * @warning Отказ `erase()` на `с/0` при `has()` истинном тоже НЕ противоречие:
	 *          `erase()` живёт над записями СТРОК, а элемент перечня строкою не является.
	 *          Снос его ведётся `prune()`, и дерево тем самым для правки не замкнуто
	 */
...
bool awh::codec::toml::Document::prune(const string & path) noexcept {
	 ...
	 *          свойство: снос его опустошает настройки целиком. Дозволен он оттого, что
	 *          пустой файл настроек TOML законен вполне, - в отличие от JSON и XML, где
	 *          описание велит нести ровно одно значение либо один корневой узел, и снос
	 *          корня там отвергается ПО СТАНДАРТУ
	 *
	 * @note Прежде здесь стоял отказ с примечанием, будто правило это согласовано с
	 *       с прочими кодеками рамки. Примечание лгало: согласовано было обратное -
	 *       снос корня решается стандартом кодека, и у YAML, INI, TOML он успешен. Найдено
	 *       это у кодека ABC сличением erase("") по пяти кодекам разом
	 */
	if(route.empty()){
		// Выполняем очистку дерева настроек целиком
		this->clear();
		// Выводим признак успешного снятия значения
		return true;
	}
	// Разыскиваем последний разделитель кусков пути
	const size_t separator = route.rfind('/');
	// Получаем имя снимаемого значения
	string name((separator == string::npos) ? route : route.substr(separator + 1));
	/**
	 * Если снять отменяющие записи последнего звена пути не удалось
	 *
	 * @warning Голова пути идёт через `place()`, где снятие стоит, а хвост брался сырым:
	 *          путь `/~0` разбирался тогда именем `~0` вместо имени `~`, и снятие било
	 *          мимо - молча и правдоподобно
	 */
	if(!::unescape(name)){
	...
	/**
	 * Если вместилище снимаемого значения не разыскано
	 */
	if(!owner.valid()){
		// Запоминаем код отказа снятия значения
		this->_error = error_t::INVALID_KEY;
	...
	// Признак успешности снятия значения с вместилища
	bool removed = false;
	/**
	 * Если вместилище является перечнем значений, а последнее звено пути номером
	 *
	 * @details Звено числовое принимается наравне с именем: перечень значений и набор
	 *          одноимённых таблиц `[[имя]]` суть у наречия перечни, и звенья к их детям
	 *          перечень звеньев выдаёт ЧИСЛАМИ. Не прими снос номера - обход был бы
	 *          замкнут для чтения и разомкнут для правки: звено, деревом выданное и для
	 *          розыска годное, сносу не поддавалось бы вовсе
	 *
	 * @note Порок этот найден сличением одного вызова по трём кодекам: YAML и INI звено
	 *       числовое сносили, а наречие это отвечало отказом
	 *
	 * @note Вид вместилища решает раньше вида записи: таблица, пару числовую несущую,
	 *       правится именем, а не номером
	 */
	if(owner.is(type_t::ARRAY) && !name.empty() &&
	 (name.find_first_not_of("0123456789") == string::npos)){
		/**
		 * Номер значения перечня, звеном пути заданный
		 *
		 * @note Зовётся он `number`, а не `index`: имя `index` занято работою POSIX
		 *       `index()` из `strings.h`, и разбор брал её вместо переменной - сборка
		 *       валилась несовпадением видов вдали от места
		 */
		uint64_t number = 0;
		/**
		 * Выполняем снятие значения перечня по номеру его
		 *
		 * @warning Номер разбирается ОБЩИМ разбором, а не `stoull`: тот при переполнении
		 *          кидает `std::out_of_range`, а метод объявлен `noexcept` - исключение
		 *          сносило приложение целиком. Путь `/fruit/99999999999999999999999999`
		 *          валил процесс с кодом 134, и приходил он от ПОТРЕБИТЕЛЯ, а не из
		 *          разбора текста: поверка выше дозволяет одни цифры, но длины им не
		 *          назначает вовсе
		 *
		 * @note Общий разбор при переполнении усекает пределом вида, и номер, за край
		 *       перечня вышедший, снятием отвергается обычным своим порядком
		 *
		 * @note Нашло это объединённое покрытие набора с ворошителем: заход лежал
		 *       непокрытым, а щуп по нему уронил приложение
		 */
		removed = (::awh::codec::numeric <uint64_t> (name, number) &&
		 owner.erase(static_cast <size_t> (number)));
	}
	// Выполняем снятие значения вместилища по имени его
	else removed = owner.erase(name);
	/**
...
bool awh::codec::toml::Document::graft(const string_view path, const ::awh::codec::toml::Value & value) noexcept {
	...
	/**
	 * Хранилище звеньев пути со снятыми отменяющими записями
	 *
	 * @details Держится оно отдельно от видов: снятие записи звено УКОРАЧИВАЕТ, и вид в
	 * исходный путь после того негоден - он показывал бы на записанное, а не на снятое
	 *
	 * @note Место под звенья выделяется наперёд: перераспределение хранилища оборвало бы
	 *       виды, прежде взятые, и перечень указывал бы в память освобождённую
	 */
	vector <string> store;
	...
	if(!route.empty()){
		...
		size_t links = 1;
		/**
		 * Выполняем подсчёт звеньев пути
		 */
		...
		while(start <= route.size()){
			...
			// Получаем очередной кусок пути
			string part(route, start, ((separator == string::npos) ? route.size() : separator) - start);
			/**
			...
			if(!::unescape(part)){
				...
				this->_error = error_t::INVALID_KEY;
				// Выводим признак неудачного переноса значения
				return false;
			}
			// Выполняем добавление снятого звена в хранилище
			store.push_back(::std::move(part));
			// Выполняем добавление вида на снятое звено в перечень кусков пути
			parts.push_back(string_view(store.back()));
			/**
...
bool awh::codec::toml::Document::set(const vector <string_view> & path, const string_view value, const string_t quoting) noexcept {
	...
	this->fault(error_t::NONE);
	/**
	 * @note Годность СОДЕРЖИМОГО к записи здесь НЕ судится, и граница эта намеренна:
	 *       дерево отвергает лишь то, чего не сделает записываемым НИ ОДНА настройка.
	 *       Имя свойства с разделителем таково при любых настройках, и застава его стоит
	 *       ниже; негодная же последовательность UTF-8 записываема при правилах `REPLACE`
	 *       и `PASS`, и отвергать её здесь значило бы отвергать вполне записываемое
	 *
	 * @details Застава `INVALID_ENCODING` стояла здесь и была ВЕРНА, покуда писатель
	 *          отвергал негодную кодировку БЕЗУСЛОВНО: тогда содержимое это не
	 *          записывалось ни при каких настройках, и дерево, его принявшее, обращалось
	 *          в незаписываемое целиком. Правилом `malformed_t`, заведённым 08.09.2026,
	 *          основание то снято: умолчанием негодный байт заменяется знаком `U+FFFD`, и
	 *          дерево остаётся записываемым. Замерено щупом: правка отвергала содержимое,
	 *          какое запись умолчанием приняла бы, - строгость сверх записи
	 *
	 * @note При правиле `REFUSE` потребитель узнаёт о беде у записи, а не у правки, - тем
	 *       же порядком, каким узнаёт о всяком ином незаписываемом содержимом: `text()`
	 *       отвечает пустотою и ставит код. Замерено на подаче перевода строки при
	 *       выключенных продолжениях: правка проходит без кода, запись даёт
	 *       `INVALID_CHARACTER`. Договор один, и исключения у него более нет
	 */
	/**
...
bool awh::codec::toml::Document::set(const vector <string_view> & path, const stamp_t & value, const type_t type) noexcept {
	...
	/**
	 * Если смещение часового пояса устанавливаемой отметки за отрезок значений выходит
	 *
	 * @details Довод тот же, каким отвергается дата несуществующая: дерево, смещение
	 *          негодное принявшее, собрать текст УЖЕ НЕ МОЖЕТ - писатель отрезок этот
	 *          поверяет своею заставой и отказывает записи, - и потребитель, правкою
	 *          обнадёженный, узнаёт о беде много позже
	 *
	 * @warning Беда та шире одного узла: запись отказывает ЦЕЛИКОМ, и dump() отдаёт
	 *          пустоту вместо всего документа - пары, правкою не тронутые, пропадают
	 *          вместе с нею. Замерено щупом: дерево о двух парах, смещением 9999
	 *          дополненное, отдало записью нуль байтов при коде отказа INVALID_DATETIME
	 *
	 * @note Отрезок берётся тот же, каким поверяет смещение писатель, - иначе застава
	 *       правки и застава записи разошлись бы, и одна пропускала бы отвергаемое второй
	 *
	 * @note Величина NO_TIMEZONE отрезком не поверяется вовсе: ею обозначено отсутствие
	 *       смещения, и писатель её отдельным заходом принимает прежде поверки отрезка
	 *
	 * @note Нашёл это объединённое покрытие набора с ворошителем: поверки даты и времени
	 *       стоят обе, а смещение, третьим полем отметки идущее, поверки не имело
	 */
	if((type == type_t::OFFSET_DATETIME) && (value.offset != NO_TIMEZONE) &&
	   ((value.offset < -((23 * 60) + 59)) || (value.offset > ((23 * 60) + 59)))){
		// Запоминаем код ошибки правки дерева настроек
		this->fault(error_t::INVALID_DATETIME);
		// Выполняем вывод сообщения об отказе в лог
		this->report();
 ...
 *          зовётся `tables()`
 *
 * @note Смена смысла закреплена проверкою `CodecTomlDocument.SizeCountsChildrenOfTheRoot`: под
 *       одним именем лежали три разных величины, и общего договора из них не выходило
 *       вовсе. Довод берётся у проверки, а не у разговора: разговор перескажешь как
 *       угодно, а проверка либо стоит, либо падает
 *
...
size_t awh::codec::toml::Document::size() const noexcept {
	 ...
	 *
	 * @details Счёт ведётся перечнем дочерних имён корня, а НЕ снятием владеющего
	 *          значения: снятие строит полную копию дерева ради одного числа, и спрос
	 *          размера на файле в двадцать пять килобайт стоил 314 микросекунд против
	 *          1.07 у перечня - в двести девяносто раз дороже. Замерено, а не выведено:
	 *          сама правка значения стоит 0.26 микросекунды, то есть спрос размера был
	 *          дороже правки в тысячу раз
	 *
	 * @note Смысл у обоих счётов ОДИН, и это проверено щупом по десяти построениям:
	 *       пустое дерево, одни пары, таблица, вложенные таблицы, набор одноимённых
	 *       таблиц, набор с парою, встроенная таблица, перечень, таблица без пар и
	 *       глубокое составное имя. Набор одноимённых таблиц считается ОДНИМ ребёнком у
	 *       обоих - он есть перечень, а не несколько детей
	 *
	 * @note Нашёл это разбор замера, а не сам замер: сценарий правки дерева зовёт размер
	 *       на каждом круге, и порог его не проходился никогда - мерился не ход правки,
	 *       а спрос размера
	 */
...
string awh::codec::toml::Document::text(const writer_t::settings_t & settings) const noexcept {
	...
	this->fault(error_t::NONE);
	/**
	 * Если запись дозволяет Юникод в голом имени, а чтение его не признаёт
	 *
	 * @details Настройки эти друг другу противоречат ПО ПОСЛЕДСТВИЮ: запись, признаком
	 * `unicode` наделённая, кладёт имя с кириллицей голым, а чтение без того же признака
	 * такое имя ОТВЕРГАЕТ, - круг рвётся. Замерено: запись даёт `ключ = "значение"`,
	 * обратный разбор отвечает отказом `INVALID_KEY`
	 *
	 * @note Разлад судится ПО ПОСЛЕДСТВИЮ, а не по самому факту расхождения: разлад с
	 *       законным употреблением дозволяется - так у кодека JSON запись бесконечности
	 *       при чтении, её отвергающем, годна стороннему потребителю, - а разлад, круг
	 *       рвущий либо искажающий само значение, отвергается. Здесь текст выходит негодным
	 *       для читающего, ОБЪЯВЛЕННОГО ЭТИМИ ЖЕ настройками, и годного употребления у
	 *       послабления нет
	 *
	 * @note Код `CONFLICTING_SETTINGS` был у кодека МЁРТВ: он обещал суд о разладе, какого
	 *       не велось. У кодека INI суд этот был (ограда при читающем, её не снимающем), у
	 *       TOML не было, и расхождение то случайно; заслон, здесь стоящий, его и снял
	 *
	 * @note Кодек YAML в один ряд с ними НЕ становится, и прежняя запись, ставившая его
	 *       туда, сваливала два разных случая в один: у TOML суда не было при НАЛИЧИИ пары,
	 *       круг рвущей, а у YAML самой пары нет вовсе - настройки `unicode` там ни у
	 *       записи, ни у чтения не заведено. Код у него мёртв ЗА ОТСУТСТВИЕМ ПРЕДМЕТА, и
	 *       довод тот записан при самом коде в `yaml/common.hpp` вместе с замером по
	 *       сочетаниям схем разрешения
	 *
	 * @warning Обратное сочетание - чтение Юникод признаёт, запись нет - разладом НЕ
	 *          является: имя обносится оградою и читается тем же чтением вполне. Судить
	 *          надлежит одну лишь пару, круг рвущую
	 */
	if(settings.unicode && !this->_settings.reader.unicode){
		// Запоминаем код отказа противоречивых настроек
	...
	for(auto & record : this->_records){
		...
		if(!result){
			 ...
			 *
			 * @note Заход ДОСТИЖИМ и берётся. Замерено родным покрытием clang: набор
			 *       кодека берёт его 7 раз, ворошитель за 60 000 проходов - 147, общий
			 *       набор - 7. Прежде тут стояло «путём открытым заход ныне НЕДОСТИЖИМ»,
			 *       и это было ЛОЖНО: довод снят был после правки, узел вида
			 *       неопределённого закрывшей, но иных дорог сюда никто не искал
			 *
			 * @warning Ошибка эта родом из замера МАЛЫМ ЧИСЛОМ подач: закрыв одну дорогу,
			 *          довод объявил недостижимым весь заход. Недостижимость доказывается
			 *          отсутствием ВСЯКОЙ дороги, а закрытие известной - лишь тем, что
			 *          известная закрыта
			 */
...
bool awh::codec::toml::Document::value(T & result, const vector <string_view> & path) const noexcept {
	/**
	 * Если извлечение ведётся видом ЯЗЫКА, разрядному не тождественным
	 *
	 * @note Перенос ведётся видом разрядным той же ширины и той же знаковости, потому
	 *       разряда тут не теряется вовсе. Прямое извлечение видом языка невозможно:
	...
	if constexpr(is_integral <T>::value && !is_same <T, bool>::value && !is_same <T, fixed_t <T>>::value){
		// Извлекаемое число видом разрядным
		fixed_t <T> number = 0;
		// Если извлечение числа не выполнено
		if(!this->value(number, path))
			// Выводим отрицательный результат выполнения операции
			return false;
		...
		// Выводим положительный результат выполнения операции
		return true;
	// Если извлечение ведётся видом разрядным
	} else {
		// Выполняем поиск узла значения по составному имени
		const node_t * node = this->seek(path);
		/**
		 ...
		 */
			 ...
			 *       решением 20.08.2026 доводом о том, что приведение языка не отказывает
			...
			if(node->type == type_t::FLOAT){
				...
				result = ::awh::codec::convert <T> (node->real);
				// Выводим положительный результат выполнения операции
				return true;
			}
			// Выводим отрицательный результат выполнения операции
 ...
 *          size_t - unsigned long; у Linux int64_t есть long, а size_t - тоже unsigned
 *          long. Перечень одних разрядных видов негоден на ОБЕИХ системах, только
 *          по-разному: у macOS не связывалось извлечение видом long и size_t, у Linux
 *          не связалось бы видом long long. Перечень видов языка покрывает обе
 *          расстановки, ибо разрядные виды суть его же псевдонимы
 *
 * @note Вид char голым намеренно не порождён: число им не извлекают, ему отвечает
 *       извлечение строковое
 *
 */
 ...
 */
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <char> (char &, const vector <string_view> &) const noexcept;
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <signed char> (signed char &, const vector <string_view> &) const noexcept;
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <unsigned char> (unsigned char &, const vector <string_view> &) const noexcept;
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <short> (short &, const vector <string_view> &) const noexcept;
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <unsigned short> (unsigned short &, const vector <string_view> &) const noexcept;
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <int> (int &, const vector <string_view> &) const noexcept;
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <unsigned int> (unsigned int &, const vector <string_view> &) const noexcept;
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <long> (long &, const vector <string_view> &) const noexcept;
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <unsigned long> (unsigned long &, const vector <string_view> &) const noexcept;
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <long long> (long long &, const vector <string_view> &) const noexcept;
template __AWH_SHARED_EXPORT__ bool awh::codec::toml::Document::value <unsigned long long> (unsigned long long &, const vector <string_view> &) const noexcept;
