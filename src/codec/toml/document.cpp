	...
	if(i == this->_children.end()){
		...
		for(size_t j = 0; j < node->items.size(); j++){
			...
			if((j >= node->names.size()) || (node->names.at(j).length == 0))
				...
				continue;
			/**
			 * Получаем первую составную часть имени очередной пары
			 *
			 * @note Выдаётся первая часть, а не имя целиком: перечень дочерний
			 *       перечисляет один уровень, и пара «a.b = 1» есть дочернее имя «a»
			 */
			const string_view name = this->get(this->_keys.at(node->names.at(j).offset).name);
