...
bool awh::codec::Bridge::absorbXML(const xml::document_t & document, const string & path, abc::value_t & result, const uint32_t depth) noexcept {
	...
	for(auto & attribute : attributes)
		...
		result[string(attribute.name.local)] = this->infer(string(attribute.value));
	/**
	...
	if(value.is(abc::type_t::MAP)){
		...
		for(size_t i = 0; i < value.size(); i++){
			...
			if(!nested){
				 ...
				 */
				string local = name;
				// Выполняем снятие приставок с имени поля
				while(!local.empty() && (local.front() == '_'))
					// Выполняем снятие одной приставки
					local.erase(local.begin());
				// Если поле несёт текст узла разметки
				if(local == this->_settings.text){
			...
			if(item.is(abc::type_t::ARRAY)){
				...
				for(size_t j = 0; j < item.size(); j++){
					 ...
					 *
					 * @warning Обнос этот не украшение: перечень у разметки имени не
					 *          имеет и выражается повтором одноимённых узлов. Подай
					 *          вложенный перечень тем же ходом - звенья двух перечней
					 *          легли бы вперемешку одноимёнными узлами, и границы
					 *          вложенного пропали бы. Замер 04.09.2026 показал хуже
					 *          того: вложенный перечень уходил в правило сужения и
					 *          ложился ПУСТЫМ узлом `<a/>`, теряя содержимое молча.
					 *          Обнос узлом взят у эталонного моста, там он зовётся
					 *          `item`
					 */
					if(item[j].is(abc::type_t::ARRAY)){
						// Выполняем перебор всех звеньев вложенного перечня
						for(size_t k = 0; k < item[j].size(); k++){
							// Собираемый узел звена вложенного перечня
							xml::Value nested(this->_settings.item);
							// Выполняем подачу звена вложенного перечня узлу разметки
							if(!this->feedXML(item[j][k], nested, depth + 2))
								// Выходим из метода, подача отвечена отказом
								return false;
					...
					// Если звеном перечня оказалось значение обычное
					} else if(!this->feedXML(item[j], node, depth + 1))
						// Выходим из метода, подача отвечена отказом
						return false;
					/**
...
bool awh::codec::Bridge::encodeXML(const abc::value_t & value, string & result) noexcept {
	...
	if(value.is(abc::type_t::MAP) && (value.size() == 1)){
		...
		if(value.key(0).value(local) && value[local].is(abc::type_t::MAP)){
 ...
 * @return         результат укладки
 *
 * @warning Путь передаётся ходам кодека ТОЛЬКО видом `string`, и это не
 *          придирка: застава общего хода у этого кодека стоит по ТОЧНОМУ виду,
 *          а не по преобразуемости, как у TOML. Строковый литерал уходит там в
 *          ход ПРЕЖНИЙ и означает имя РАЗДЕЛА, а не путь, отвечая пустым
 *          перечнем МОЛЧА - отказа не будет, будет неверный ответ. Оттого всюду
 *          здесь стоит `string`, а не литерал
 *
...
bool awh::codec::Bridge::absorbINI(const ini::document_t & document, const string & path, abc::value_t & result, const uint32_t depth) noexcept {
	...
	if(links.empty()){
		// Выполняем укладку значения последовательностью знаков
		result = abc::value_t(string(document.at(path).text()));
		// Выводим результат укладки
		return true;
	}
	/**
...
bool awh::codec::Bridge::encode(const abc::value_t & value, string & result, const format_t format) noexcept {
	...
	auto guardFn = [this, &value, &result](const bool ok) noexcept -> bool {
		// Если перевод отвечен отказом либо запись собрана непустою
		if(!ok || !result.empty())
