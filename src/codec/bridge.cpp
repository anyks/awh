...
awh::codec::abc::value_t awh::codec::Bridge::infer(const string & text) const noexcept {
	...
	if(!zeroed){
		 ...
		 *          самой проверки каркаса доложено владельцу отдельно, подпирать
		 *          его здесь нельзя
		 */
		/**
...
bool awh::codec::Bridge::encodeYAML(const abc::value_t & value, string & result) noexcept {
	 ...
	 *          `codec/yaml/document.hpp`, где расходился с телом; владелец кодека
	 *          своё правило исправил, мост был третьим списком.
	 *
...
bool awh::codec::Bridge::encodeXML(const abc::value_t & value, string & result) noexcept {
	 ...
	 *
	 * @note Правило пустого узла не трогается вовсе: умолчание его - решение
	 *       владельца, и у записи, ЧУЖИМ средством писанной, узел `<k/>` по-прежнему
...
bool awh::codec::Bridge::encodeINI(const abc::value_t & value, string & result) noexcept {
	 ...
	 *          тогда как выразить было ЧЕМ. Разобрано 15.09.2026 с владельцем
