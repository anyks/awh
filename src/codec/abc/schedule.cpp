...
void awh::codec::abc::Schedule::stop() noexcept {
	...
	/**
	 * Выполняем ожидание конца потоков, ушедших в отставку
	 *
	 * @note Отставной, оказавшийся НЫНЕШНИМ потоком, ожидания не берёт и остаётся в
	...
	for(size_t i = this->_retired.size(); i > 0; i--){
		...
		/**
		 * Если отставной поток ожидания ещё не взял и нынешним не является
		 */
		if(retired.joinable() && (retired.get_id() != this_thread::get_id()))
			// Выполняем ожидание конца отставного потока отбоя срока
			retired.join();
