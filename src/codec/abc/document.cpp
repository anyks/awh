...
static bool room(const size_t held, const size_t size) noexcept {
	/**
	 * @note Все ПЯТЬ мест, зовущих эту поверку, щуп нужности 09.09.2026 нашёл
	 *       МОЛЧАЩИМИ, и проверкою они не берутся вовсе: отказ требует настоящих
	 *       четырёх гигабайт накопленного хранилища октетов. Заслон тот же, что и у
	 *       обёртки записи кадром, у отрезка события разбора и у длины собираемой
	 *       записи, - смещение узла несёт ТРИДЦАТЬ ДВА разряда, и содержимое, за них
	 *       выходящее, легло бы в узел УСЕЧЁННЫМ молча: значение выходило годным по
	 *       виду, а отдавалось бы ЧУЖОЕ
	 */
...
const unordered_map <string_view, uint32_t> & awh::codec::abc::Document::naming(const uint32_t index) const noexcept {
	...
	while(child < bound){
		 ...
		 *
		 * @note Щуп нужности 09.09.2026 нашёл заслон МОЛЧАЩИМ, и место НЕДОСТИЖИМО по
		 *       устройству: отображение, где у имени нет значения, разбор отвергает сам,
		 *       а дерево заводится единственно разбором. Заслон держится оттого, что
		 *       обход этот читает узлы ПОДРЯД, и оборванная пара увела бы его в узлы
		 *       соседнего вместимого - молча и с чужим содержимым
		 */
		if(value >= bound)
...
bool awh::codec::abc::Document::identical(const uint32_t left, const uint32_t right) const noexcept {
	 ...
	 *
	 * @note Заслон этот и следующий за ним щуп нужности 09.09.2026 нашёл МОЛЧАЩИМИ, и
	 *       молчание их того же рода, что у ветви вместимого ниже: сличение зовётся
	 *       ныне лишь ИМЕНАМИ полей отображения, а имя есть одиночное значение о едином
	 *       узле - размахи таких поддеревьев равны всегда, и выйти за дерево они не
	 *       могут. Оставлены намеренно: сличение объявлено ПОДДЕРЕВЬЯМИ, а не именами
	 */
	...
	for(uint32_t i = 0; i < extent; i++){
		 ...
		 *
		 * @note Закреплено `CodecAbcDocument.KeysOfDifferentKindsAreNotDuplicates`: щуп
		 *       нужности 09.09.2026 нашёл поверку МОЛЧАЩЕЙ. Случай настоящий, а не
		 *       умозрительный: пустая строка и пустые двоичные данные держат длину
		 *       нулевую обе, содержимое обеих лежит в хранилище октетов, и сличение
		 *       содержимого их проходит по НУЛЮ октетов - то есть совпадает всегда.
		 *       Различает их единственно поверка вида, и без неё правило `FIRST` отсеяло
		 *       бы пару, повтором не являющуюся, а документ молча потерял бы поле
		 */
...
void awh::codec::abc::Document::resolve(const uint32_t index, const duplicate_t rule) noexcept {
	...
	/**
	 * @note **Отсев повторов молчит ВЕСЬ, и молчит законно.** Щуп нужности 08.09.2026
	 *       обесточил порознь все шесть заслонов этой работы - пропуск выбывшей пары
	 *       при обходе внешнем и внутреннем, прекращение обхода при правиле `LAST` и
	 *       ранний выход при пустом отсеве, - и не покрасил ни одной проверки. Разбор
	 *       показал, что иначе и быть не могло: заслоны эти стерегут не ПРАВИЛЬНОСТЬ, а
	 *       ЦЕНУ. Выбывшая пара, сличённая вторично, выбывает вторично же - значение
	 *       признака от того не меняется; обход, не прекращённый при `LAST`, роняет уже
	 *       уронённое; а перенос при пустом отсеве отдаёт то же дерево, каким оно и было
	 *
	 * @warning Отсюда НЕ следует, что заслоны эти лишние: работа растёт квадратом числа
	 *          пар, и без них квадрат этот проходится целиком на всяком отображении. Мера
	 *          их - замер, а не проверка, и заводить им сторожа проверкою значило бы
	 *          стеречь стоимость утверждением о правильности
	 */
...
bool awh::codec::abc::Document::fail(const error_t error) noexcept {
	/**
	 * @note Воронка эта чёрным ящиком НЕДОСТИЖИМА, и это измерено: щуп нужности
	 *       08.09.2026 обесточил объявление журналу, и не покраснело ничего. Зовётся она
	 *       всего из ДВУХ мест разбора, и оба объявлены последними руками - внутренний
	 *       отказ да пустая запись, какую разбиратель отвергает раньше сам. Донесение,
	 *       какое видит подписчик журнала при отвергнутом разборе, приходит НЕ ОТСЮДА, а
	 *       от `Reader::fail`
	 *
	 * @warning Проверка, писанная было под эту воронку, оказалась зелена и слепа - она
	 *          стояла на чужом донесении. Ныне она честно объявляет, что стережёт
	 *          разбирателя: `CodecAbcDocument.TheRefusedParseReachesTheJournal`
	 */
...
void awh::codec::abc::Document::assemble(void * context, reader_t & reader, const event_t event) noexcept {
	 ...
	 *
	 * @note Щуп нужности 09.09.2026 нашёл распространение это МОЛЧАЩИМ, и место
	 *       НЕДОСТИЖИМО: сборка дерева отвечает отказом лишь исчерпанием запаса
	 *       хранилища (четыре гигабайта, см. `::room` выше) да закрытием вместимого при
	 *       пустом стеке - а вместимые разбор уравновешивает сам, и незакрытое до дерева
	 *       не доходит
	 */
...
bool awh::codec::abc::Document::build(writer_t & writer) const noexcept {
	...
	for(size_t index = 0; index < this->_nodes.size(); index++){
		 ...
		 *
		 * @note Щуп нужности 09.09.2026 нашёл распространение это МОЛЧАЩИМ, и место
		 *       НЕДОСТИЖИМО по устройству: дерево заводится единственно разбором -
		 *       работ, правящих его, у документа нет вовсе, - а всякое значение,
		 *       разбор прошедшее, сборщиком принимается. Пробовано опытом: дерево со
		 *       строкою, кодировке не отвечающей, завести НЕЛЬЗЯ - разбор поверяет
		 *       кодировку всегда и отвергает такую запись сам (`INVALID_ENCODING`)
		 *
		 * @note Собрат его, распространяющий отказ укладки ИМЕНИ поля, достижим вполне
		 *       и закреплён `CodecAbcDocument.TheRefusedBuildingYieldsAnEmptyRecord`:
		 *       порядок имён поверяется настройками СБОРКИ, каких у разбора не было
		 */
		if(!result){
		...
		if(node.container() && (node.length() == 0)){
			 ...
			 *
			 * @note Щуп нужности 09.09.2026 нашёл заслон этот и близнеца его МОЛЧАЩИМИ, и
			 *       место НЕДОСТИЖИМО по устройству: конец вместимого сборщик отвергает
			 *       лишь при несходстве счёта значений с объявленным, а счёт этот ведёт
			 *       обход дерева, самим же деревом и объявленный. Дерево же заводится
			 *       единственно разбором, уравновешивающим вместимые сам
			 */
...
vector <uint8_t> awh::codec::abc::Document::dump() const noexcept {
	 ...
	 *
	 * @note Закреплено `CodecAbcDocument.TheRefusedBuildingYieldsAnEmptyRecord` вместе с
	 *       близнецом своим у сборки затребованными настройками: щуп нужности 09.09.2026
	 *       нашёл оба МОЛЧАЩИМИ - набор собирал записи лишь из деревьев, сборщиком
	 *       принимаемых. Собранная наполовину запись есть запись негодная, и отличить её
	 *       от годной зовущему нечем
	 */
...
awh::codec::abc::Document::value_t awh::codec::abc::Document::root() const noexcept {
	 ...
	 *       `CodecAbcDocument.TheEmptyTreeAnswersEmptinessToEveryDemand`, нарочно для
	 *       него писанной, и молчание это ЗАКОННО: поверка действительности сличает
	 *       номер узла с размером дерева, и у пустого дерева всякий номер её не проходит.
	 *       Проверка держит ДОГОВОР - пустое дерево отвечает пустотою, - а не этот заслон
	 */
	if(this->_nodes.empty())
...
awh::codec::abc::Document::value_t awh::codec::abc::Document::at(const string & path) const noexcept {
	...
	while(abc::segment(path, offset, segment, unescaped, invalid)){
		 ...
		 *       спрос значения либо поля у недействительной ссылки отвечает
		 *       недействительной же, и обход, заслона не имеющий, доходит до конца пути
		 *       с тем же итогом. Заслон бережёт работу, а не правильность
		 */
		if(!current.valid())
			// Выводим недействительную ссылку на значение
			return value_t();
 ...
 *
 */
awh::codec::abc::Document::Document() noexcept :
 _error(error_t::NONE) {}
/**
...
awh::codec::abc::Document::value_t awh::codec::abc::Document::Value::at(const size_t index) const noexcept {
	 ...
	 * Если узел вместимым не является
	 *
	 * @note Заслон этот и сосед его ниже ЗАТЕНЯЮТ ДРУГ ДРУГА и закреплены СОВМЕСТНО, а не
	 *       порознь: у невместимого узла число значений нулевое, оттого сосед отвергает
	 *       спрос и без него, а при пустом вместимом - наоборот. Щуп нужности 08.09.2026
	 *       обесточил каждый порознь и не покрасил ничего; обесточенные РАЗОМ, они красят
	 *       `CodecAbcDocument.TheDemandAgainstTheKindIsAnsweredWithEmptiness`
	 *
	 * @warning Приговор о паре СЛАБЕЕ одиночного: он говорит лишь, что вдвоём они нужны, а
	 *          порознь не говорит ничего. Сносить ни один нельзя - затенение держится на
	 *          том, что `size()` у невместимого нулевой, и правка этого свойства разом
	 *          обнажила бы оба
	 */
	if(!node.container())
		// Выводим недействительную ссылку на значение
...
awh::codec::abc::Document::value_t awh::codec::abc::Document::Value::key(const size_t index) const noexcept {
	 ...
	 *
	 * @note Закреплено `CodecAbcDocument.TheDemandBeyondTheContainerDoesNotYieldTheNeighbour`:
	 *       щуп нужности 09.09.2026 нашёл заслон МОЛЧАЩИМ. Затенения здесь НЕТ, и это
	 *       отличает место от собратьев: за концом отображения стоит узел НАСТОЯЩИЙ -
	 *       соседнее значение, - и обход предела отдал бы именно его под видом имени
	 *       поля, а поверка действительности того не заметила бы вовсе. Набор молчал
	 *       оттого, что вместимое стояло в проверках ПОСЛЕДНИМ значением, и за концом
	 *       его лежал конец дерева
	 */
	if(index >= this->size())
		// Выводим недействительную ссылку на значение
		return value_t();
...
awh::codec::abc::Document::value_t awh::codec::abc::Document::Value::begin() const noexcept {
	 ...
	 *
	 * @note Закреплено `CodecAbcDocument.TheDemandBeyondTheContainerDoesNotYieldTheNeighbour`:
	 *       щуп нужности 09.09.2026 нашёл заслон МОЛЧАЩИМ. Довод тот же, что и у спроса
	 *       имени за пределом: обход пустого вместимого без заслона отдал бы СОСЕДНИЙ
	 *       узел первым значением его
	 */
	if(!node.container() || (node.length() == 0))
		// Выводим недействительную ссылку на значение
		return value_t();
...
awh::codec::abc::Document::value_t awh::codec::abc::Document::Value::get(const string_view name) const noexcept {
	 ...
	 *
	 * @note Закреплено `CodecAbcDocument.TheDemandBeyondTheContainerDoesNotYieldTheNeighbour`:
	 *       щуп нужности 09.09.2026 нашёл заслон МОЛЧАЩИМ. Затенения нет и здесь:
	 *       указатель имён полей строится обходом пар «имя-значение», и перечень СТРОК,
	 *       пройденный тем же обходом, дал бы имени `a` значением соседнюю строку `b`
	 */
