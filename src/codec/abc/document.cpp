...
bool awh::codec::abc::Document::identical(const uint32_t left, const uint32_t right) const noexcept {
	...
	for(uint32_t i = 0; i < extent; i++){
		 ...
		 *
		 * @note Недостижимо УСЛОВИЕ, а не сам путь, и разница эта ИЗМЕРЕНА 05.09.2026
		 *       щупом пути: заслон, принуждённый срабатывать всегда, краснит 2 проверок.
		 *       Число это есть цена ошибки следующего - снос места, объявленного
		 *       недостижимым, валит именно столько, - а не мера ширины дороги
		 */
		if(first.container()){
...
bool awh::codec::abc::Document::parse(const void * buffer, const size_t size, const reader_t::settings_t & settings) noexcept {
	 ...
	 *
	 * @note Недостижимо УСЛОВИЕ, а не сам путь, и разница эта ИЗМЕРЕНА 05.09.2026
	 *       щупом пути: заслон, принуждённый срабатывать всегда, краснит 68 проверок.
	 *       Число это есть цена ошибки следующего - снос места, объявленного
	 *       недостижимым, валит именно столько, - а не мера ширины дороги
	 */
...
bool awh::codec::abc::Document::build(writer_t & writer) const noexcept {
	 ...
	 *
	 * @note Недостижимо УСЛОВИЕ, а не сам путь, и разница эта ИЗМЕРЕНА 05.09.2026
	 *       щупом пути: заслон, принуждённый срабатывать всегда, краснит 8 проверок.
	 *       Число это есть цена ошибки следующего - снос места, объявленного
	 *       недостижимым, валит именно столько, - а не мера ширины дороги
	 */
	if(!stack.empty()){
