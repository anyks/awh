...
namespace {
	 ...
	 *
	 * @details Ниже порога сличение идёт обходом перечня: он растёт квадратом числа
	 *          имён, но множитель его - одно сличение свёрток, - а указатель требует
	 *          выделения памяти на всякое отображение. Порог этот и есть место, где
	 *          квадрат обхода перевешивает плату за выделение
	 *
	 * @note Величина взята степенью двойки не ради счёта, а ради ровности: замер
	 *       30.08.2026 показал разницу между 16 и 64 в пределах разброса, и брать её
	 *       точнее оснований нет
	 *
	 */
	 ...
	 *
	 * @details Свёртка эта СВОЯ, а не взятая у модуля шифрования, и тому две причины.
	 *          Первая: ядро разбора держат лёгким намеренно - отдельные стенды собирают
	 *          его без третьей стороны вовсе, - а зависимость от модуля шифрования
	 *          потянула бы туда и его. Вторая: свёртка эта наружу НЕ выходит ни единым
	 *          октетом, живёт одним разбором и с чужими нигде не встречается, - согласовывать
	 *          её с кем бы то ни было незачем
	 *
	 * @note Взята FNV-1a: сличение имён ею не решается, а лишь ускоряется, и ложное
	 *       совпадение стоит одного лишнего сличения октетов, а не ложного отказа. Требовать
	 *       от неё стойкости оттого не приходится
	 *
	...
	if(error != error_t::NONE){
		 ...
		 */
			...
			awh::log::debug("ABC: %s", __PRETTY_FUNCTION__,
			 {static_cast <uint16_t> (error), this->_location.offset, this->_location.depth},
 ...
 *
 */
void awh::codec::abc::Reader::rehash(const size_t depth, frame_t & frame, const size_t needed) noexcept {
	...
	for(size_t i = frame.base; i < this->_spans.size(); i++){
		...
		size_t slot = (static_cast <size_t> (this->_spans.at(i).digest) & mask);
		/**
 ...
 *
 * @note Щуп нужности 09.09.2026 нашёл здесь три МОЛЧАЩИХ заслона, и молчание их
 *       ЗАКОННО, а не слепота. Порог `HASHING_NAMES` и перестройка указателя стерегут
 *       ЦЕНУ, а не правильность: обесточенный порог отдаёт сличение указателю, и повтор
 *       находится тот же самый, - оттого набор и остаётся зелен. Отвечать за них должен
 *       замер, а не проверка, и предмет замера тут наличен - число сличений свёрток
 *
 * @note Третий заслон - предел перечня в четыре миллиарда мест - достижим лишь записью
 *       в гигабайты и проверкою НЕ берётся вовсе: сличение обходом верно при всяком
 *       размахе, и заслон этот бережёт от молчаливого пропуска повтора там, куда
 *       указателю уже не дотянуться
 *
 */
bool awh::codec::abc::Reader::duplicated(frame_t & frame, const size_t offset, const size_t length, const uint64_t digest) noexcept {
	// Выполняем получение числа имён полей, уложенных вместимым
	const size_t count = (this->_spans.size() - frame.base);
	/**
	...
	if(frame.hashed == 0){
		...
		if(count < HASHING_NAMES){
			...
			for(size_t i = frame.base; i < this->_spans.size(); i++){
				...
				const naming_t & span = this->_spans.at(i);
				// Если свёртки либо длины записей имён не совпадают, имена различны заведомо
				if((span.digest != digest) || (static_cast <size_t> (span.length) != length))
					// Переходим к следующему отрезку записи имени поля
					continue;
				// Если записи имён совпали октет в октет
				if((length == 0) || (::memcmp(this->_pool.data() + static_cast <size_t> (span.offset),
		 ...
		 * того указателю неподвластен. Достижим предел этот лишь записью в гигабайты,
		 * но молчаливого пропуска повтора он стоил бы, а обход верен при всяком размахе
		 */
		if(this->_spans.size() >= static_cast <size_t> (numeric_limits <uint32_t>::max())){
			/**
			 * Выполняем перебор отрезков записей прежних имён полей вместимого
			 */
			for(size_t i = frame.base; i < this->_spans.size(); i++){
	...
	while(table.at(slot) != 0){
		// Выполняем получение отрезка записи прежнего имени поля
		const naming_t & span = this->_spans.at(static_cast <size_t> (table.at(slot)) - 1);
		/**
		 * Если свёртки и длины записей имён совпали, сличаем их октет в октет
		 */
		if((span.digest == digest) && (static_cast <size_t> (span.length) == length) &&
		   ((length == 0) || (::memcmp(this->_pool.data() + static_cast <size_t> (span.offset),
		    this->_pool.data() + offset, length) == 0)))
			// Сообщаем, что имя поля в этом вместимом уже было
			return true;
		// Выполняем переход к следующему гнезду указателя
	 ...
	 * по возврату отсюда, и разрывать поверку с укладкою надвое незачем
	 */
	table.at(slot) = static_cast <uint32_t> (this->_spans.size() + 1);
	// Сообщаем, что имя поля в этом вместимом ещё не встречалось
	return false;
}
/**
		...
		if(frame.mapping && frame.expectKey){
			...
			if(this->_settings.canonical){
				...
				if(frame.marked){
					 ...
					 *
					 * @note Щуп нужности 09.09.2026 нашёл заслон МОЛЧАЩИМ, и молчание его
					 *       ЗАКОННО: запись значения САМОРАЗМЕРНА - длина её определяется
					 *       собственными ведущими октетами, - оттого запись одного имени
					 *       строгим началом записи другого стоять не может, и общая часть
					 *       совпадает лишь у имён РАВНЫХ. У равных же и длины равны, и
					 *       уточнение оставляет ноль нулём, отчего отказ объявляется всё
					 *       тем же заслоном ниже. Сличение писано видом общим, и уточнение
					 *       это - его часть, а не отдельный сторож
					 */
					if(compare == 0)
			 ...
			 */
				...
				if(record.span.length > 0)
					...
					this->_pool.insert(this->_pool.end(),
					 ...
					 this->_buffer.data() + static_cast <size_t> (record.span.offset) + static_cast <size_t> (record.span.length));
				/**
				 * Если вместилище октетов имён шире поля отрезка, разбор отвергается
				 *
				 * @details Отрезок записи имени несёт смещение и длину ТРИДЦАТЬЮ ДВУМЯ
				 * разрядами, тогда как вместилище растёт всеми именами открытых
				 * отображений разом и предела своего не знает. Усечение смещения обращало
				 * бы сличение к чужому месту вместилища, и повтор имени ПРОПУСКАЛСЯ бы
				 * молча - отказа при том не случилось бы ни единого
				 *
				 * @note Установлено щупом 31.08.2026: поле смещения, суженное до
				 * шестнадцати разрядов, на семидесяти именах по тысяче октетов отвечало
				 * согласием там, где повтор был настоящий. Поверка одна на смещение и на
				 * длину: длина отрезка вместилища не превышает, и укладываются они вместе
				 *
				 * @note Одна поверка длины у самого значения (`INVALID_LENGTH` выше) тут
				 * не спасает: она стережёт имя ОДНО, а переполняется вместилище их суммою
				 *
				 * @note Место НЕДОСТИЖИМО проверкою: требует настоящих четырёх гигабайт
				 * накопленных имён. Заслоны той же величины у величины неограниченной
				 * ширины и у открытого расширения достижимы вполне - там сличается
				 * ОБЪЯВЛЕННАЯ длина, - и закреплены проверкою
				 * WideNumbersAndExtensionsNameTheirCauses. Установлено щупом по местам
				 * отказа 04.09.2026
				 */
				if(this->_pool.size() > static_cast <size_t> (numeric_limits <uint32_t>::max())){
				 ...
				 * квадратом их количества, и полное сличение октетов на всякой паре обходилось
				 * дорого: замер 30.08.2026 дал выигрыш от пропуска груза 26 крат против 55 при
				 * снятой поверке, а со снятым одним лишь обходом - 49. Плата сидела в обходе,
				 * а не в переносе имён, - свёртка обращает его в числовое сличение
				 *
				 * @note Свёртка отказа НЕ решает: совпадение её ведёт к полному сличению
				 * октетов, и повтор объявляется лишь им. Ложное совпадение свёртки стоит
				 * одного лишнего сличения, а не ложного отказа
				 */
				const uint64_t digest = ::digest(this->_pool.data() + offset, length);
				// Если имя поля отображения в этом вместимом уже было
				if(this->duplicated(frame, offset, length, digest)){
					// Выполняем возврат вместилища октетов к прежнему виду
					this->_pool.resize(offset);
					// Выполняем объявление отказа разбора
				...
				this->_spans.emplace_back(static_cast <uint32_t> (offset), static_cast <uint32_t> (length), digest);
			}
...
bool awh::codec::abc::Reader::unwind() noexcept {
	...
	while(!this->_stack.empty()){
		 ...
		 *
		 * @note Щуп нужности 09.09.2026 нашёл заслон МОЛЧАЩИМ, и молчание его ЗАКОННО:
		 *       звено собираемого кусками заводится с признаком неопределённой длины
		 *       (место заведения единственное), и следующий же заслон отвечает за него.
		 *       Заслон этот держится ради ясности намерения, а не ради его действия
		 */
	...
	for(;;){
		...
		switch(static_cast <uint8_t> (this->_state)){
			...
			case static_cast <uint8_t> (state_t::ITEM): {
				 ...
				 *
				 * @note Молчащий выход, затенённый состоянием `FAILED`, - см. пояснение у
				 *       первого такого выхода при выдаче события ниже
				 */
				if(!this->item(done))
					// Сообщаем, что разбор отвечен отказом
					return false;
			...
			case static_cast <uint8_t> (state_t::SEGMENT): {
				 ...
				 *
				 * @note Щуп нужности 09.09.2026 нашёл этот выход и трёх его собратьев (по
				 *       одному на разбор октетов значения, одиночного значения, величины и
				 *       открытого расширения) МОЛЧАЩИМИ, и молчание их ЗАКОННО: `settle`
				 *       отвечает отказом лишь через `fail`, а тот переводит разбор в состояние
				 *       `FAILED`, какое следующим же оборотом цикла отвечает отказом само.
				 *       Выход этот экономит один оборот и держит намерение на виду, но
				 *       последнею рукою не является, и проверкою порознь не берётся
				 */
			...
			case static_cast <uint8_t> (state_t::SINGLE): {
				...
				this->_state = state_t::ITEM;
				/**
				 * Одиночное значение куском собираемого стоять НЕ вправе, и выдача его отдана
				 * общему пути: кусками собираются лишь строка да двоичные данные, а
				 * `settle` отвергает всякое иное значение внутри собираемого кусками
				 *
				 * @note Прежде здесь стояла ветвь выдачи куска - точная копия такой же у
				 *       разбора октетов значения, - и она обходила `settle` стороной.
				 *       Опознаватель либо отметка времени внутри строки, собираемой кусками,
				 *       принимались КУСКОМ ЕЁ и выдавались наружу, тогда как число в том же
				 *       месте отвергалось `INVALID_SEGMENT`, а вместимое - им же. Найдено
				 *       09.09.2026 разбором молчащих заслонов, закреплено проверкой
				 *       `CodecAbcReader.SingleValueInsideSegmentIsRefused`
				 */
				/**
				 * Если выдача события завершённого значения отвечена отказом
				 *
				 * @note Молчащий выход, затенённый состоянием `FAILED`, - см. пояснение у
				 *       первого такого выхода в разборе октетов значения выше
				 */
				if(!this->settle(record))
					// Сообщаем, что разбор отвечен отказом
					return false;
			...
			case static_cast <uint8_t> (state_t::EXTEND_EXPONENT): {
				...
				if(!abc::take(this->_buffer.data(), this->_buffer.size(), offset, unit, error)){
					 ...
					 * Если октетов единицы недостаёт
					 *
					 * @note Закреплено `CodecAbcReader.ChunkIndependenceOfExtensions`: щуп нужности
					 *       09.09.2026 нашёл эту ветвь и пятерых её собратьев по разбору расширений
					 *       МОЛЧАЩИМИ. Прежняя проверка независимости от нарезки расширений не несла
					 *       вовсе, и всякое расширение, разорванное между кусками подачи, было бы
					 *       объявлено повреждением записи, чего не заметил бы никто
					 */
			...
			case static_cast <uint8_t> (state_t::SPANNED_WIDTH): {
				 ...
				 */
				/**
				 * @note Закреплено `CodecAbcReader.TheSpanBeyondTheCountOfPlacesIsRefused`:
				 *       граница взята последним годным размахом и первым негодным, а не
				 *       круглым числом
				 */
			...
			case static_cast <uint8_t> (state_t::EXTEND_LENGTH): {
				...
				if(!abc::take(this->_buffer.data(), this->_buffer.size(), offset, unit, error)){
					 ...
					 * Если октетов единицы недостаёт
					 *
					 * @note Закреплено `CodecAbcReader.ChunkIndependenceOfExtensions`, см. пояснение
					 *       у такой же ветви разбора десятичного порядка величины выше
					 */
					if(error == error_t::UNEXPECTED_EOF)
						// Сообщаем, что разбор успешен
						return true;
				 ...
				 *
				 * @note Закреплено `CodecAbcReader.TheWideNumberRefusesTheLengthOfAForeignKind`:
				 *       щуп нужности 09.09.2026 нашёл заслон МОЛЧАЩИМ - у открытого
				 *       расширения оба таких места были закреплены, а у величины
				 *       неограниченной ширины не было закреплено ни одного
				 */
				if(unit.group != group_t::UNSIGNED)
					// Выполняем объявление отказа разбора
					return this->fail(error_t::INVALID_BIGNUM);
				...
				/**
				 * Если объявленная длина октетов величины шире отрезка события
				 *
				 * @note Сторож тот же, что и у строки с двоичным значением, и по той же
				 * причине: октеты величины ложатся в отрезок события, а он несёт длину
				 * тридцатью двумя разрядами. Кусками величина не кладётся, оттого предел
				 * этот ей и настоящий - но величина в четыре гигабайта есть число о
				 * тридцати четырёх миллиардах разрядов, и тесным предел назвать нельзя
				 */
				if(unit.value > static_cast <uint64_t> (numeric_limits <uint32_t>::max()))
					// Выполняем объявление отказа разбора
					return this->fail(error_t::INVALID_LENGTH);
				// Выполняем сдвиг смещения разбора
				this->_offset = offset;
			...
			case static_cast <uint8_t> (state_t::EXTEND_DATA): {
				 ...
				 *
				 * @note Закреплено `CodecAbcReader.ChunkIndependenceOfExtensions`: щуп нужности
				 *       09.09.2026 нашёл заслон МОЛЧАЩИМ, а обесточенный он ВАЛИТ прогон - чтение
				 *       уходит за конец накопленного. Привязка падения к этой проверке доказана
				 *       щупом привязки: проверка одна, и валит её этот заслон
				 */
				if((this->_buffer.size() - this->_offset) < this->_pending)
					// Сообщаем, что разбор успешен
					return true;
			...
			case static_cast <uint8_t> (state_t::FINISHED): {
				 ...
				 *
				 * @note Закреплено проверкой `CodecAbcReader.NodeLimitIsDeclaredPerDocument`,
				 *       заведённой 04.09.2026: прежде исправление это не стерёг никто
				 */
				this->_nodes = 0;
				// Выполняем перевод разбора в ожидание очередной единицы
				this->_state = state_t::ITEM;
...
bool awh::codec::abc::Reader::item(bool & done) noexcept {
	 ...
	 *
	 * @note Запоминание ведётся ЛИШЬ у имени поля, а не у всякой единицы. Первая сборка
	 * поверки повторов запоминала ведущие октеты подряд, и перенос этот платился на
	 * КАЖДОМ значении записи, тогда как нужен он одним именам: замер 30.08.2026 дал
	 * выигрыш от пропуска груза 25…30 крат против 55…61 при снятой поверке - половину
	 * меры съедала работа, выбрасываемая тут же. Ныне поверка стоит рядом с той, какою
	 * имя опознаётся и дальше
	 *
	 * @note Признак ожидания имени снимается УЧЁТОМ уложенного значения, а учёт идёт
	 * позже, - оттого признак этот и годен здесь опознавателем имени
	 *
	 * @note Запоминание годно ЛИШЬ покуда имя есть ОДНА единица: собирайся имя кусками,
	 * между запоминанием и употреблением легли бы куски, звенья стека да метка конца, и
	 * запомненное разошлось бы с употребляемым молча. Держится это отказом `INVALID_KEY`
	 * на имени неопределённой длины, закреплённым проверкой
	 * `CodecAbcReader.ChunkedKeyRefused`, - отступление от него сломало бы сличение имён
	 */
	if((this->_settings.canonical || (this->_settings.duplicates == duplicate_t::REFUSE)) &&
	   !this->_stack.empty() && this->_stack.back().mapping && this->_stack.back().expectKey)
	...
	switch(static_cast <uint8_t> (unit.group)){
		...
		case static_cast <uint8_t> (group_t::UNSIGNED): {
			 ...
			 *
			 * @note Закреплено `CodecAbcReader.TheUnsignedNumberCarriesItsSignedViewWhenItFits`:
			 *       щуп нужности 09.09.2026 нашёл заслон МОЛЧАЩИМ - у целого без знака
			 *       спрашивали поле `number`, а поля `integer` не спрашивал никто
			 */
			if(unit.value <= static_cast <uint64_t> (numeric_limits <int64_t>::max()))
		...
		case static_cast <uint8_t> (group_t::BLOB): {
			...
			if(unit.indefinite){
				...
				if(!this->_stack.empty()){
					 ...
					 *
					 * @warning Довод, названный выше, УЖЕ самого отказа, и разница эта
					 * опасна: строгий вид записи умолчанием СНЯТ, и читающий довод вправе
					 * счесть заслон излишним вне его. Настоящих доводов три, и держатся они
					 * во ВСЯКОМ виде разбора:
					 * - сборка дерева кладёт кусками собираемое узлом, признака имени НЕ
					 *   несущим (`keyed` там остаётся ложью), - имя вошло бы в дерево
					 *   обыкновенным значением, и отображение разъехалось бы молча;
					 * - поверка повторов запоминает ведущие октеты имени, а годно
					 *   запоминание лишь покуда имя есть ОДНА единица (см. довод при
					 *   запоминании выше);
					 * - строгий вид записи требует одному значению отвечать одной записи
					 *
					 * @note Разобрано 05.09.2026 по случаю: владелец сетевых движков нашёл у
					 * себя отказ, взятый ШИРЕ вреда и оправданный доводом ложным. Здесь
					 * обратное - отказ верен, а довод УЖЕ его, - и это тот же изъян с иной
					 * стороны: следующий, поверив доводу, ослабил бы заслон вне строгого
					 * вида и сломал бы сборку дерева
					 */
					if(frame.mapping && frame.expectKey)
						// Выполняем объявление отказа разбора
			 ...
			 *
			 * @note Закреплено проверкой `CodecAbcReader.ChunkedValueRespectsLimit`: она
			 *       кладёт куски, всякий из каких в предел УМЕЩАЕТСЯ, а сумма их за предел
			 *       выходит, - вплоть до дробления по одному октету
			 */
			...
			/**
			 * Если объявленная длина значения шире отрезка события
			 *
			 * @details Отрезок события несёт смещение и длину ТРИДЦАТЬЮ ДВУМЯ разрядами -
			 * события копятся очередью, и восемьдесят разрядов на событие стоили бы памяти
			 * на всякой записи ради невиданного. Но длина, за предел выходящая, ложилась бы
			 * в отрезок УСЕЧЁННОЙ молча: событие вышло бы годным по виду, а содержимое его -
			 * оборванным, и вскрылось бы это лишь у потребителя
			 *
			 * @note Возможности записать значение длиннее четырёх гигабайт отказ этот НЕ
			 * отнимает: значение такой длины кладётся кусками, а кусок каждый в отрезок
			 * умещается. Сумма кусков сличается с пределом выше и тридцатью двумя разрядами
			 * не стеснена вовсе
			 *
			 * @note Сторож равняется на два таких же, стоящих в кодеке: обёртка записи
			 * кадром отвергает запись шире четырёх гигабайт, а дерево документа - содержимое
			 * шире хранилища. Молчаливое усечение здесь было бы третьим случаем одного вида
			 */
			if(unit.value > static_cast <uint64_t> (numeric_limits <uint32_t>::max()))
				// Выполняем объявление отказа разбора
				return this->fail(error_t::INVALID_LENGTH);
		...
		case static_cast <uint8_t> (group_t::MAP): {
			 ...
			 * Если количество узлов документа превышает допустимое
			 *
			 * @note Закреплено `CodecAbcReader.TheNodeLimitCountsTheContainersThemselves`:
			 *       щуп нужности 09.09.2026 нашёл учёт МОЛЧАЩИМ. Прежняя проверка предела
			 *       клала перечень СО ЗНАЧЕНИЯМИ, и отказ объявлялся учётом их, затеняя
			 *       собою учёт вместимых, - а запись из одних лишь вложенных перечней
			 *       прошла бы предел насквозь
			 */
			...
			if(this->_skipping){
				...
				frame.remain = 0;
				/**
				 * Выполняем снятие признака неопределённой длины у пропущенного вместимого
				 *
				 * @details Вместимое неопределённой длины закрывается МЕТКОЮ конца, а не
				 * исчерпанием значений, - и сматывание, на исчерпание опирающееся, звено
				 * его со стека не снимало вовсе. Разбор при том уходил ЗА метку конца, и
				 * звено оставалось висеть открытым: значения, стоящие следом, приписывались
				 * пропущенному вместимому, а корневое пропущенное обрывало документ
				 * недоразобранным - `UNEXPECTED_EOF` там, где запись цела
				 *
				 * @note Своя сборка размаха неопределённому вместимому не объявляет вовсе
				 * (`Writer::open`: метка размаха кладётся лишь при `!indefinite`), но запись
				 * приходит ИЗВНЕ, и метка размаха перед неопределённым вместимым законна
				 * по виду записи. Закреплено проверкой
				 * `CodecAbcReader.SkippedIndefiniteContainerCloses`
				 *
				 * @note Признак снимается ЛИШЬ у звена, а не у выданного события: событие
				 * начала объявляет длину такою, какова она в записи, и потребитель вправе
				 * судить о ней по нему
				 */
		...
		case static_cast <uint8_t> (group_t::SINGLE): {
			...
			switch(unit.detail){
				...
				case static_cast <uint8_t> (single_t::YES): {
					...
					record.boolean = (unit.detail == static_cast <uint8_t> (single_t::YES));
					// Выполняем выдачу события завершённого значения
					return this->settle(record);
				}
				/**
				...
				case static_cast <uint8_t> (single_t::BREAK): {
					 ...
					 * Если длина вместившего определена
					 *
					 * @note Закреплено `CodecAbcReader.TheIndefiniteContainerIsCountedByItsParent`:
					 *       щуп нужности 09.09.2026 нашёл учёт МОЛЧАЩИМ - вместимое
					 *       неопределённой длины клали КОРНЕМ записи, где учитывать его
					 *       некому, а внутрь вместимого определённой длины не клал никто
					 */
					if(!parent.indefinite)
						// Выполняем учёт закрытого вместимого значением вместившего
						parent.remain--;
			 ...
			 * Выполняем объявление отказа разбора
			 *
			 * @note Место это ПЕРЕКРЫТО заслоном, стоящим раньше: отложенные подробности
			 *       метки отвергает само снятие единицы проволочной записи, и сюда разбор
			 *       с ними не доходит. Держится оно на случай, если подробность будет
			 *       снятию разрешена, а разбору - ещё нет
			 *
			 * @note Установлено щупом по местам отказа 04.09.2026: подмена причины здесь
			 *       не красит ничего, при том что сама причина закреплена проверкою
			 *       EveryRefusalNamesItsCauseNotOnlyItsFact - краснеет она от подмены у
			 *       снятия единицы, а не здесь
			 *
			 */
	 ...
	 * Выполняем объявление отказа разбора
	 *
	 * @note Место это НЕДОСТИЖИМО по устройству, и оттого проверкою не стережётся: все
	 *       восемь крупных видов записи разобраны ветвями выше, а девятого не бывает -
	 *       вид укладывается тремя старшими разрядами ведущего октета. Держится оно
	 *       заслоном последней руки на случай, если ветвь выше будет снята правкой
	 *
	 * @note Недостижимо УСЛОВИЕ, а не сам путь, и разница эта ИЗМЕРЕНА 05.09.2026
	 *       щупом пути: заслон, принуждённый срабатывать всегда, краснит 119 проверок.
	 *       Число это есть цена ошибки следующего - снос места, объявленного
	 *       недостижимым, валит именно столько, - а не мера ширины дороги
	 *
	 * @note Установлено щупом по местам отказа 04.09.2026: подмена причины здесь не
	 *       красит ни единой проверки, и это ЗАКОННО. Прочие одиннадцать молчавших мест
	 *       того же захода оказались достижимы и закреплены
	 *
	 */
...
void awh::codec::abc::Reader::trim() noexcept {
	 ...
	 *
	 * @note Щуп нужности 09.09.2026 назвал заслон МОЛЧАЩИМ, и приговор его тут НЕ К
	 *       ДЕЛУ: щуп отнимает условие целиком, а без условия усечения не бывает вовсе -
	 *       буфер лишь растёт, и всё остаётся верным. Обещание заслона иное: «не усекать,
	 *       покуда события не сняты», и поверяется оно ИЗВРАЩЕНИЕМ, а не снятием
	 *
	 * @note Извращён 09.09.2026 отнятием поверки очереди (`if(this->_offset > 0)`), и
	 *       покраснели ДВЕ проверки - `CodecAbcReader.DeferredEvents` и
	 *       `CodecAbcReader.ChunkIndependenceOfStatefulSettings`. Заслон стережётся, и
	 *       молчание сплошного щупа его слепотою не является
	 */
...
bool awh::codec::abc::Reader::feed(const void * buffer, const size_t size, const bool last) noexcept {
	 ...
	 *
	 * @note Закреплено `CodecAbcReader.TheAbsentBufferWithDeclaredOctetsIsRefusedByTheReader`:
	 *       щуп нужности 09.09.2026 нашёл заслон МОЛЧАЩИМ, а обесточенный он ВАЛИТ
	 *       прогон - накопление читает по нулевому указателю. Привязка падения к этой
	 *       проверке доказана щупом привязки: проверка одна, и валит её этот заслон
	 */
	...
	this->trim();
	/**
	 * Если накопленное вместе с подаваемым выводит буфер за поле смещения отрезка события
	 *
	 * @details Отрезок события несёт смещение ТРИДЦАТЬЮ ДВУМЯ разрядами, и смещение шире
	 *          ложилось бы в него УСЕЧЁННЫМ молча: событие выходило годным по виду, число
	 *          событий сходилось, а содержимое отдавалось ЧУЖОЕ. Усечённое смещение
	 *          остаётся ВНУТРИ буфера, оттого ни санитайзер, ни падение того неказали бы
	 *
	 * @details Усечение разобранной части возможно лишь при пустой очереди событий -
	 *          отрезки ссылаются в буфер смещением, - оттого потребитель, подающий без
	 *          снятия событий, растил буфер безо всякой границы. Замер 31.08.2026: 4.10 ГиБ
	 *          в 67134 записях по 64 КиБ, и запись, поданная за перевалом, возвращалась
	 *          содержимым чужой
	 *
	 * @note Отказ этот - обратное давление, а не поломка, ровно как и предел числа событий
	 *       выше: разбор цел, а по снятии событий усечение сдвигает смещение к нулю, и
	 *       подача продолжается с того же места. Возможности разобрать запись длиннее
	 *       четырёх гигабайт он НЕ отнимает - отнимает лишь право не снимать события
	 *
	 * @note Сторож безусловен, а не настраиваем: четыре гигабайта здесь не пожелание, а
	 *       ШИРИНА ПОЛЯ. Настройка тут обещала бы то, чего вместилище не удержит
	 *
	 * @note Закреплено проверкой `CodecAbcReader.BufferSpanWidthGuardOnFeeding`. Прежде
	 *       примечание указывало на `BufferSpanWidthGuard`, стерегущую ВЫДАЧУ МЕСТА, -
	 *       место иное, и подмена причины здесь не красила ничего. Довод был шире
	 *       проверенного; установлено щупом по местам отказа 04.09.2026
	 */
	if((this->_buffer.size() + size) > static_cast <size_t> (numeric_limits <uint32_t>::max())){
		// Выполняем установку кода отказа подачи
		this->_error = error_t::OVERFLOW_LIMIT;
		// Выводим признак неудачной подачи
		return false;
...
void * awh::codec::abc::Reader::reserve(const size_t size) noexcept {
	 ...
	 * место обращается вспять: содержимым записи хвост его не является вовсе
	 *
	 * @details Возврат тот же, что и у подачи, и заведён он здесь по той же причине.
	 * Прежде выдача места сбрасывала лишь РАЗМЕР выданного, октетов же буферу не
	 * возвращала: висящая выдача, за какою шла выдача новая либо отказ от неё, оставляла
	 * в буфере разбора хвост НЕОПРЕДЕЛЁННЫХ октетов, и разбор принимал их содержимым
	 * записи
	 *
	 * @note Найдено щупом нужности 09.09.2026: `reserve(8)` с последующим `reserve(0)` и
	 * подачей годной записи отвечал отказом `octets after the end of the document` -
	 * восемь неопределённых октетов стояли ПЕРЕД записью. Закреплено проверкой
	 * `CodecAbcReader.TheAbandonedPlaceIsReturnedToTheBuffer`
	 */
	if(this->_reserved > 0){
		// Выполняем возврат выданного места под приём октетов
		this->_buffer.resize(this->_buffer.size() - this->_reserved);
	...
	if(size == 0)
		...
		return nullptr;
	/**
	 * Если очередь собранных событий уже полна
	 *
	 * @details Сторож равняется на такой же у подачи копированием, и по той же причине,
	 * какою заведён сторож ширины поля ниже: путь этот отдаёт место под приём напрямую,
	 * и без сторожа обратное давление обходилось бы одним лишь ВЫБОРОМ способа подачи.
	 * Настройка `maxEvents` стерегла очередь у `feed` и молчала у `reserve`, то есть
	 * не работала вовсе у того потребителя, ради какого путь этот и заведён, - у
	 * поточного, принимающего в выданное место
	 *
	 * @note Замер 02.09.2026 на записи в 178 октетов при пределе в четыре события:
	 * `feed` отвечал отказом на пятом октете, `reserve` принимал все 178 и отказа не
	 * давал ни разу. Закреплено проверкой
	 * `CodecAbcReader.EventLimitHoldsBothIntakePaths`
	 *
	 * @note Места при отказе не выдаётся вовсе, и принимать потребителю некуда: ровно
	 * так же ведёт себя сторож ширины поля ниже
	 *
	 * @note Разбор при таком отказе НЕ портится: состояние его цело, и выдача места
	 * после снятия событий продолжается с того же места
	 */
	if((this->_settings.maxEvents > 0) && (this->_events.size() >= this->_settings.maxEvents)){
		// Выполняем установку кода отказа выдачи места
		this->_error = error_t::OVERFLOW_LIMIT;
		// Выполняем сброс размера выданного места
		this->_reserved = 0;
		// Сообщаем, что места под приём октетов нет
	...
	/**
	 * Если выдача места выводит буфер за поле смещения отрезка события
	 *
	 * @note Сторож равняется на такой же у подачи копированием: путь этот отдаёт место под
	 *       приём напрямую, и без сторожа здесь обратное давление обходилось бы выбором
	 *       способа подачи. Места при отказе не выдаётся вовсе, и принимать потребителю
	 *       некуда
	 */
	if((offset + size) > static_cast <size_t> (numeric_limits <uint32_t>::max())){
		// Выполняем установку кода отказа выдачи места
		this->_error = error_t::OVERFLOW_LIMIT;
		// Выполняем сброс размера выданного места
		this->_reserved = 0;
...
bool awh::codec::abc::Reader::skip() noexcept {
	...
	if(this->_beyond == 0)
		 ...
		 *       `CodecAbcReader.TheSkipRefusesWhatItCannotSkip`, нарочно для него
		 *       писанной: обесточенный, он проваливается на СОСЕДА - «место за вместимым
		 *       лежит прежде нынешнего места разбора», - а при нулевом размахе тот
		 *       истинен всегда, ибо метку вместимого разбор к тому времени уже прошёл.
		 *       Заслон затенён соседом; проверка же держит ДЕЙСТВИЕ - отказ пропуска, -
		 *       и остаётся верной
		 */
		return false;
	/**
	...
	if((this->_beyond - this->_origin) > static_cast <uint64_t> (this->_buffer.size()))
		 ...
		 * Выводим признак неудачного пропуска
		 *
		 * @note Закреплено `CodecAbcReader.TheSkipRefusesWhatItCannotSkip`: щуп нужности
		 *       09.09.2026 нашёл заслон МОЛЧАЩИМ - пропуск звали лишь там, где он удаётся
		 */
		return false;
