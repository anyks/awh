...
void awh::codec::abc::Value::clone(const Value & value) noexcept {
	...
	auto own = [](const Value & from, Value & into) noexcept -> void {
		...
		/**
		 * Перенимаем объект фреймворка и объект журнала.
		 *
		 * @note Ребёнок берёт их у родителя, а копия - у образца: взять их иначе значению
		 *       НЕГДЕ, конструктор их не принимает. Заведённое НЕ перезаписывается -
		 *       своё старше перенятого, как то сделано у `awh::Buffer`
		 */
	};
...
void awh::codec::abc::Value::absorb(const Document::value_t & value) noexcept {
	...
	while(!pending.empty()){
		...
		pending.pop_back();
		/**
		 * Перенимаем объект фреймворка и объект журнала у дерева, откуда идёт значение.
		 *
		 * @note Стоит это ВНУТРИ обхода намеренно: им кроется не корень один, а всякий
		 *       ребёнок. Прежде перенимание стояло при входе и метило РОВНО ОДИН узел -
		 *       ровно тем же и был порочен снятый владельцем назначатель `setLogger`,
		 *       и повторять его беду на новый лад было бы вдвойне глупо
		 *
		 * @note Иначе взять пару значению НЕГДЕ: конструктор её не принимает - прими он
		 *       пару указателей, неявное приведение вида `value_t v = "текст"` стало бы
		 *       невозможным. Устройство взято у `awh::Buffer`, где решён тот же вопрос:
		 *       объекты перенимаются встречей. Заведённое НЕ перезаписывается - своё
		 *       старше перенятого
		 */
		if(task.source._doc != nullptr){
		}
...
awh::codec::abc::Value::Value(Value && value) noexcept :
 _kind(value._kind), _type(value._type),
 _number(value._number), _text(std::move(value._text)), _exponent(value._exponent),
 _negative(value._negative), _keys(std::move(value._keys)),
 ...
 *
 */
awh::codec::abc::Builder::Builder() noexcept :
 _keyed(false), _done(false) {}
