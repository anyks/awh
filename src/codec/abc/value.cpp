...
/**
 * Подключаем заголовочный файл общего для кодеков приведения чисел
 */
...
namespace {
	 ...
	 *
	 * @details Приведение это ОБЩЕЕ у всех кодеков рамки и живёт в `awh::codec::convert`.
	 *          Здесь оно звалось СВОЕЙ копией, и копия была равна общей - сличено 07.09.2026
	 *          замером по тридцати двум краевым значениям на одиннадцати видах: расхождений
	 *          ноль. Оттого копия и снята: равная копия хуже отсутствия копии, ибо расходится
	 *          она молча и не сразу
	 *
	 * @note Наблюдение о двух копиях пришло от Николая через Василия 07.09.2026
	 *
	 * @note Своё у ABC тут одно, и оно сохранено ради истории: прежде кодек отвечал на
	 *       такое извлечение ОТКАЗОМ, соблюдая вид хранения, - отменено владельцем
	 *       20.08.2026 вместе с прочими кодеками рамки. Прочие подробности договора
	 *       (нуль от `NaN`, предел вида, увод половины от нуля) живут при общей работе
	 *
	...
	static T convert(const double value) noexcept {
		// Выполняем приведение числа общим для кодеков способом
		return awh::codec::convert <T> (value);
	}
	/**
