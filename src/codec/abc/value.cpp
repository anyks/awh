...
awh::codec::abc::Value & awh::codec::abc::Value::place(const string & path) noexcept {
	 ...
	 * пути завело бы значение НЕ ТАМ и отчиталось успехом. Ссылка выдаётся на само
	 * обходимое значение, а не на заведённое, - иного при негодном пути и быть не может
	 */
	if(invalid)
		// Выводим ссылку на обходимое значение
		return (* current);
