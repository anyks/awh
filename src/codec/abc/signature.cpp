...
bool awh::codec::abc::Merkle::add(const void * buffer, const size_t size) noexcept {
	 ...
	 *
	 * @note Заслон ПОСЛЕДНЕЙ РУКИ наравне с близнецом своим у свёртки узла: вид свёртки
	 *       задан постоянной `Digest` и равен SHA256, а та выдаёт ровно `DIGEST_LENGTH`
	 *       октетов всегда. Условие достижимо лишь отказом самой подсистемы шифрования,
	 *       поддельной у набора нет, - оттого сплошной щуп 17.09.2026 нашёл молчащими оба.
	 *       Убирать нельзя: свёртка иной длины легла бы в дерево, и корень сошёлся бы не с
	 *       тем, а беду искали бы в подписи
	 */
...
bool awh::codec::abc::unpack(const void * buffer, const size_t size, sign_t & sign, error_t & error) noexcept {
	 ...
	 */
	/**
	 * @warning **Проверкою заслон этот БОЛЕЕ НЕ ЗАКРЕПИТЬ, и виною тому правка 16.09.2026.**
	 *          Прежде он стерёг поля: без него вид подписи и вид хэш-суммы успевали лечь из
	 *          октетов, каковых у короткого буфера нет, - и `TheShortSignatureRecordLeavesTheFieldsUntouched`
	 *          ловила это. Ныне поля ложатся ПОСЛЕ всех сличений, наблюдаемой разницы не
	 *          осталось, и обесточенный заслон проверку зелёной оставляет: сосед ниже,
	 *          сличающий длину с объявленной записью, отвечает тою же причиною
	 *
	 * @note Стережёт он ныне ОДНО - чтение за концом поданного буфера, - и доказать это
	 *       можно лишь адресным надзирателем. Замер 17.09.2026 на буфере РОВНО в четыре
	 *       октета: обесточенный заслон даёт `heap-buffer-overflow` на снятии длины корня,
	 *       целый - отказ `TRUNCATED_SIGNATURE` и молчание надзирателя. Сам надзиратель при
	 *       том поверен нарочным чтением за концом в щупе: без такой поверки молчание его
	 *       ничего не значит
	 *
	 * @warning Прогон под адресным надзирателем требует КЛЮЧА `CMAKE_BUILD_ALLOCATOR=OFF`:
	 *          покуда выдачу памяти ведёт распределитель AWH, надзиратель кучи не видит
	 *          вовсе - перехват его подменён. Замерено тем же щупом: связанный с объектами
	 *          AWH, он не ловит даже заведомого чтения за концом. Разобрано это в
	 *          `CMakeLists.txt` 15.09.2026, где и предписаны ДВА прогона под надзирателями
	 */
