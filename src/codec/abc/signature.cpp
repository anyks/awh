...
void awh::codec::abc::pack(const sign_t & sign, vector <uint8_t> & result) noexcept {
	 ...
	 *          не там. Работа отказа объявить не вправе - выдачи у неё нет вовсе, - и
	 *          сторож стоит у зовущего, а здесь запись остаётся ПУСТОЙ: пустую подпись
	 *          снятие отвергает заведомо
	 *
	...
	if((sign.signature.size() > 0xFFFF) || (sign.root.size() > 0xFFFF))
		// Выходим из работы, ничего не уложив
		return;
