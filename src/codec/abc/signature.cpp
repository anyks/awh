...
/**
 * Подключаем заголовочный файл кадра контейнера: свёртка листа снимает учётный
 * разряд мусора с признаков кадра, и знать о них дереву приходится
 */
...
bool awh::codec::abc::Merkle::fail(const char * message) const noexcept {
	 ...
	 *
	 * @note Закреплено `SignatureFixture.TheMerkleFunnelReportsItsTroubleToTheJournal`:
	 *       щуп нужности 08.09.2026 нашёл воронку МОЛЧАЩЕЙ. Здесь молчание дороже, чем у
	 *       близнецов: кода отказа у дерева нет вовсе, и журнал не второй путь наружу, а
	 *       ЕДИНСТВЕННЫЙ - зовущий видит одну ложь возврата и спросить причину не может
	 *       ничем. Воронки `Editor::fail` и `Fetcher::fail` молчали тем же молчанием и
...
bool awh::codec::abc::Merkle::add(const void * buffer, const size_t size) noexcept {
	...
	/**
	 * Выполняем снятие разряда мусора с признаков кадра перед выработкой свёртки
	 *
	 * @details Разряд этот УЧЁТНЫЙ, а не содержательный: им фиксация метит прежнее
	 *          оглавление, обращённое в мусор, правкою одного октета внутри уже
	 *          подписанного тела. Свёртка, следующая за ним, ломала подпись ПРЕЖНЕГО
	 *          поколения при целых записях: обрыв фиксации сразу после пометки оставлял
	 *          контейнер, где все записи читаются, а подпись не сходится
	 *
	 * @note Порядком записи это не лечится - испробовано и отвергнуто замером: пометка
	 *       последней спасает прежнее поколение, но ломает подпись у ФИКСАЦИИ УДАВШЕЙСЯ,
	 *       ибо свёртка снимается с кадра непомеченным, а на носителе он помечен
	 *
	 * @note Дерево это сводит КАДРЫ контейнера и ничего иного, оттого снятие разряда
	 *       стоит здесь, у единственного места выработки свёртки листа, а не у мест
	 *       внесения, каковых ВОСЕМЬ - сочтено 07.09.2026 розыском `merkle.add(` по
	 *       исходникам `src/codec/abc`: два у контейнера, пять у правщика и одно здесь.
	 *       Прежняя запись числила их шестью
	 */
	if(size > CHUNK_FLAGS)
		// Выполняем снятие разряда мусора с признаков вносимого кадра
		payload[1 + CHUNK_FLAGS] = static_cast <uint8_t> (payload[1 + CHUNK_FLAGS] & ~CHUNK_WASTE);
...
bool awh::codec::abc::Merkle::root(vector <uint8_t> & result, const void * buffer, const size_t size) const noexcept {
	 ...
	 *
	 * @note Заслон этот ИЗБЫТОЧЕН, и это измерено, а не выведено: щуп нужности 07.09.2026
	 *       обесточил его, и не покраснело ничего. Причина в том, что внесение кадра ниже
	 *       (`merkle.add`) отвергает ровно те же три условия само, и отказ приходит от
	 *       него. Держится заслон ради ВНЯТНОСТИ доклада: свой довод называет беду прямо,
	 *       а нижний говорит лишь о внесении кадра
	 *
	 * @note Договор трёхдоводного сведения закреплён проверкою
	 *       `SignatureFixture.MerkleRootWithAnAppendedChunkRefusesWithoutItsGear`. Она
	 *       стережёт ОТКАЗ, а не этот заслон: обесточение его проверку не красит, ибо
	 *       отказывает нижний. Прежде на дорогу эту ссылались проверкою внесения кадра,
	 *       и довод был шире правды - трёхдоводного сведения без оснастки не звал никто
	 */
	if((this->_crypto == nullptr) || (buffer == nullptr) || (size == 0))
...
awh::crypto_t::hash_t awh::codec::abc::digest(const crypto_t::signature_t kind,
 const crypto_t::hash_t hash) noexcept {
	...
	switch(static_cast <uint8_t> (kind)){
		...
		case static_cast <uint8_t> (crypto_t::signature_t::GOST512):
			/**
			 * @note Ветвь `GOST512` прибавлена к перечню 01.09.2026 и поведения НЕ
			 *       меняет: прежде вид этот уходил в выдачу по умолчанию, дающую то же
			 *       отсутствие хэша. Проверено снятием ветви - набор остаётся зелёным.
			 *       Стоит она здесь ради полноты перечня: вид, в перечне не названный,
			 *       читается забытым, и разбор кода к нему возвращается снова и снова
			 */
 ...
 *
 */
bool awh::codec::abc::pack(const sign_t & sign, vector <uint8_t> & result) noexcept {
	/**
	 * Если длина подписи либо длина корня шире шестнадцати разрядов, запись негодна
	 *
	 * @details Обе длины кладутся двумя октетами, и укладка обрезала бы их молча:
	 *          запись вышла бы разборной, а подпись - несходящейся, и повод искали бы
	 *          не там
	 *
	 * @note Предел этот не тесен: подпись RSA-16384 занимает 2048 октетов, а самая
	 *       широкая у схем послеквантовых - около 50 000. Сторож стоит впрок, и сегодня
	 *       ни одна из поддержанных схем его не достаёт
	 *
	 * @note Отказ объявляется ВЫДАЧЕЙ, а прежде объявлялся пустотою записи, и довод
	 *       ссылался на сторожа у зовущего. Сторожа того не было НИ У ОДНОГО из троих:
	 *       запись оставалась пустой, обёртка кадром принимала пустое содержимое
	...
	// Сообщаем, что запись подписи уложена
	return true;
}
/**
...
bool awh::codec::abc::unpack(const void * buffer, const size_t size, sign_t & sign, error_t & error) noexcept {
	 ...
	 */
	/**
	 * @note Закреплено `SignatureFixture.TheShortSignatureRecordLeavesTheFieldsUntouched`:
	 *       щуп нужности 08.09.2026 нашёл место МОЛЧАЩИМ. Причину утверждать здесь
	 *       нельзя - сосед ниже отвечает тою же самой, - оттого проверка утверждает
	 *       НЕПРИКОСНОВЕННОСТЬ полей: без заслона вид подписи и вид хэш-суммы успевают
	 *       лечь из октетов заголовка, каковых у короткого буфера нет
	 */
	...
	/**
	 * Если вид хэш-суммы не отвечает виду подписи.
	 *
	 * Судит об этом ТОТ ЖЕ посредник `digest`, каким выбирает хэш сама укладка: завести
	 * здесь свой список значило бы развести двух судей об одном, и разошлись бы они
	...
	/**
	 * Если хвостовые октеты заголовка записи несут неведомое.
	 *
	 * Два октета эти оставлены впрок, укладка кладёт их нулями, и требование нулей
	 * равняется на то же требование у строки оглавления: поле приходит с провода и
	 * обязано быть опознано. Без него занятые кем-то октеты проходили бы молча, а
	 * запись числилась бы понятой целиком
	 */
	if((static_cast <size_t> (octets[6]) | (static_cast <size_t> (octets[7]) << 8)) != 0){
		// Выполняем установку кода отказа повреждённой подписи
		error = error_t::INVALID_SIGNATURE;
		// Выводим признак неудачного снятия подписи
		return false;
...
bool awh::codec::abc::fingerprint(const crypto_t & crypto, const string & name, vector <uint8_t> & result) noexcept {
	 ...
	 */
	/**
	 * @note Закреплено `SignatureFixture.TheFingerprintOfAnAbsentKeyIsRefused`: щуп
	 *       нужности 08.09.2026 нашёл место МОЛЧАЩИМ. Без заслона усечение пустого
	 *       отпечатка дало бы пустой же итог, а работа объявила бы себя УДАВШЕЙСЯ:
	 *       пустой отпечаток лёг бы в заголовок, и беда вскрылась бы лишь сличением с
	 *       чужим - сиречь никогда, ибо пустой не сойдётся ни с чем и будет сочтён
	 *       чужою подписью
	 */
	if(full.empty())
