 ...
 *
 * @details Разбор пути держится здесь, в одном месте на всех. Зовущих его ТРОЕ, сочтено
 *          07.09.2026 и поимённо: `Value::at`, `Value::place` и `Document::at`. Опрос же
 *          наличия у дерева (`Document::has`) правил этих сам не зовёт вовсе - он идёт
 *          через `Document::at`, и прежняя запись, числившая зовущих четверыми, считала
 *          его отдельно. Пересчёт: розыск `abc::segment(` по исходникам `src/codec/abc`.
 *          Расписаны правила у объявления работы
 *
...
bool awh::codec::abc::segment(const string_view path, size_t & offset, string_view & result,
 string & buffer, bool & invalid) noexcept {
	...
	for(size_t i = buffer.find('~'); i != string::npos; i = buffer.find('~', i + 1)){
		 ...
		 *
		 * @note Заслон этот для ПРАВИЛЬНОСТИ избыточен, и это измерено, а не выведено:
		 *       щуп нужности 08.09.2026 обесточил его, и не покраснело ничего. Причина в
		 *       том, что `buffer[i + 1]` при `i + 1 == size()` есть обращение ЗАКОННОЕ -
		 *       стандарт обещает там нулевой знак, - а ветвь `default` разбора отвергает
		 *       его тем же порядком, объявляя путь недействительным. Держится заслон ради
		 *       ВНЯТНОСТИ: обрыв записи он называет прямо, а `default` говорит лишь о
		 *       неведомой отменяющей записи
		 *
		 * @warning Довод «без него идёт чтение за концом вместилища» НЕВЕРЕН и здесь не
		 *          записан намеренно: обращение законно, и опора эта развалилась бы при
		 *          первой же поверке. Сам договор - недействительность пути, кончающегося
		 *          знаком отмены, - закреплён `CodecAbcCommon.TheSegmentWithATruncatedEscapeIsRefused`,
		 *          и проверка та честно объявляет, что ЭТОГО заслона не стережёт
		 */
		if((i + 1) >= buffer.size()){
...
bool awh::codec::abc::indexed(const string_view segment, size_t & result) noexcept {
	 ...
	 *
	 * @note Закреплено `CodecAbcCommon.TheEmptySegmentIsNotANumber`: щуп нужности
	 *       08.09.2026 нашёл место МОЛЧАЩИМ. Без заслона перебор знаков пустого звена не
	 *       находит ни одного негодного, и работа объявляет звено ГОДНЫМ с номером НУЛЬ -
	 *       путь `//` указывал бы на нулевое значение перечня вместо того, чтобы быть
	 *       отвергнутым
	 */
	if(segment.empty())
