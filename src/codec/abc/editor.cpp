...
bool awh::codec::abc::Editor::commit() noexcept {
	...
	/**
	 * Если контейнер подпись объявлял, новые кадры отодвигаются ЗА прежнюю запись её
	 *
	 * @details За прежним оглавлением лежит запись подписи прежнего поколения, и новые
	 *          кадры ложились прямо на неё. Обрыв фиксации оставлял контейнер, где все
	 *          записи целы, головной заголовок прежний, а запись подписи, на какую он
	 *          указывает, затёрта наполовину: поверка отвечала «запись подписи
	 *          повреждена». Замер 26.08.2026: обрыв на любой из записей 2..5 губил
	 *          подпись при двадцати целых записях
	 *
	 ...
	 *
	 * @details Запись подписи обёрнута кадром, и место её затирать не приходится вовсе:
	 *          обход кадров подряд идёт по ней как по мусорному кадру. Прежнее поколение
	 *          от того переживает обрыв фиксации целиком - и записи, и подпись
	 *
	 * @note Прежде новые кадры ложились прямо на запись подписи, и обрыв оставлял
	 *       контейнер с целыми записями и затёртой подписью, на какую указывал головной
	 *       заголовок прежнего поколения
	 */
