...
bool awh::codec::abc::Editor::fail(const error_t error) noexcept {
	 ...
	 */
	/**
	 * @note Закреплено `TheRefusalFunnelReportsItsCauseToTheJournal`: щуп нужности
	 *       08.09.2026 нашёл воронку МОЛЧАЩЕЙ. Набор спрашивал `error()`, а журнала не
	 *       спрашивал никто, и обесточенное объявление оставило бы его зелёным при том,
	 *       что всякий отказ правщика стал бы для стороннего наблюдателя невидим.
...
bool awh::codec::abc::Editor::fetch(const uint64_t origin) noexcept {
	 ...
	 */
	/**
	 * @note Сторож этот бережёт от ЗАВОРОТА, а не от чтения за краем. Смещение прочтено
	 *       из строки оглавления и недоверенно; сумма `HEADER_LENGTH + origin` при
	 *       `origin` близ предела разрядной сетки заворачивается и даёт смещение МАЛОЕ -
	 *       на контейнере о трёх записях выходило 95, вполне внутри его. Правщик читал бы
	 *       тогда не за краем, а НЕ ТАМ, оставаясь с виду в границах
	 *
	 * @note `EditorFixture.ChunkOffsetBeyondTheBodyIsRefused` наводит этот сценарий, но
	 *       сторожа НЕ закрепляет: обесточь его - и отказ придёт ниже по течению, назвав
	 *       ту же причину `INVALID_CHUNK`. Прежняя запись здесь гласила «закреплено
	 *       проверкою» и была шире правды; установлено сплошным щупом нужности 07.09.2026.
	 *       Закрепляет сторожа `EditorFixture.RefusedChunkOffsetNeverReachesTheMedium`,
	 *       утверждающая ЧИСЛО обращений к носителю: с ним их ноль, без него - есть.
	 *       Утверждать место обращения негодно ровно из-за заворота
	 */
...
bool awh::codec::abc::Editor::harvest() noexcept {
	...
	for(uint64_t offset = 0; offset < this->_header.length;){
		 ...
		 *
		 * @note Заслон этот и все ЕМУ ПОДОБНЫЕ (`SIGNING_FAILED` у правки и у сборки
		 *       вместилища - двадцать мест) суть ПОСЛЕДНЯЯ РУКА, а не пробел охраны, и
...
bool awh::codec::abc::Editor::add(const void * buffer, const size_t size,
 const payload_t kind, const bool added, const uint64_t number) noexcept {
	 ...
	 *
	 * @warning Путь ПОСЛЕДНЕЙ РУКИ: сюда доходит лишь накопленное свыше четырёх гигабайт,
	 *          и проверкою такое не закрепить - буфер потребен настоящий, и всякий стенд
	 *          набора на нём встанет. Сплошной щуп по заслонам правщика 16.09.2026 нашёл
	 *          место молчащим, и молчание это означает НЕДОСТИЖИМОСТЬ УСЛОВИЯ ценою
	 *          прогона, а не мёртвый путь: укладка накопленного сама по себе хожена всякой
	 *          сменою вида содержимого, соседним заслоном выше
	 */
	if(!this->_pending.empty() &&
...
bool awh::codec::abc::Editor::open(source_t source, sink_t sink, const uint64_t length) noexcept {
	 ...
	 *       выборки записей и у обхода счёта мусора, что ниже в этой же `open` (сличение
	 *       `(bound - place) < whole`), - а у самого открытия его не было вовсе, хотя
	 *       открытие первым и встречает недоверенный контейнер
	 *
	 * @warning Прежде здесь стояло «ДЕСЯТЬЮ СТРОКАМИ НИЖЕ». Указание износилось молча:
	 * замерено 07.09.2026 - между записью и названным местом 136 строк, а не десять.
	 * Расстояние в записи стареет БЫСТРЕЕ числа и так же беззвучно: его разносит всякая
	 * правка соседнего кода, ни единого вызова не тронув. Оттого место указывается ИМЕНЕМ
	 * работы да видом сличения, каковые розыском и берутся, - разряд принесён со стороны
	 *
	 ...
	 *       пройдя сторож насквозь. От заворота вычитания бережёт ПЕРВОЕ слагаемое самого
	 *       сторожа - `index > _length`, - и ничто иное
	 *
	 * @warning Прежде здесь стояло «смещение поверено выше тем, что кадр его вычитан по
	 * нему успешно». Довод неверен по существу: успешное чтение есть отдача ИСТОЧНИКА, а
	 * источник даётся потребителем, и полагаться на разумность чужой работы нельзя - то
	 * же самое сказано при обходе счёта мусора ниже по файлу. Сторож на чужую отдачу и не
	 * опирается; опиралась одна лишь запись. Найдено 07.09.2026 сплошным ситом доводов: довод,
	 * ссылающийся на поверку «выше», сличается с телом ТОЙ ЖЕ работы
	 *
	 ...
	 *
	 * @note Причина ПЕРЕНОСИТСЯ от укладчика: разжиматель отдаёт зовущий, и сжатый
	 *       контейнер волен попасть правщику, сжатия не знающему, - тогда отказ настигает
	 *       уже открытие. Закреплено проверкою
	 *       `EditorFixture.OpeningCompressedWithoutTheCompressorCarriesThePackerCause`,
	 *       и подмена переносимой причины красит одну лишь её (2019 прогнано, 2018
	 *       прошло, 05.09.2026)
	 */
	 ...
	 */
	{
		...
		while(place < bound){
			 ...
			 */
			/**
			 * @note Закреплено `ChunkLengthBeyondTheBodyStopsTheWasteWalk`: щуп нужности
			 *       08.09.2026 нашёл это место МОЛЧАЩИМ. Порча кладётся на кадр МУСОРНЫЙ
			 *       намеренно - живой в счёт не идёт и при снятом заслоне, и проверка на
			 *       нём была бы зелена всегда
			 */
...
bool awh::codec::abc::Editor::erase(const uint64_t number) noexcept {
	 ...
	 *       щупом пути: заслон, принуждённый срабатывать всегда, краснит 17 проверок.
...
bool awh::codec::abc::Editor::record(const uint64_t number, vector <uint8_t> & result) noexcept {
	...
	if(found != numeric_limits <size_t>::max()){
		...
		if(mark.batch != numeric_limits <size_t>::max()){
			 ...
			 *
			 * @details Кадр этот УЛОЖЕН нами же, и порче взяться неоткуда - зато оснастка
			 *          правщику не принадлежит: сжатие и шифрование отдаёт зовущий, и отдать
			 *          он волен когда угодно, в том числе ПОСЛЕ укладки. Тогда кадр, уложенный
			 *          сжимающим правщиком, снимает правщик, сжатия уже не знающий
			 *
			 * @note Причина здесь не заводится своя, а ПЕРЕНОСИТСЯ от укладчика: слою выше
			 *       знать нечего о разжимателе. Закреплено проверкою
			 *       `EditorFixture.PendingUnpackFailureCarriesThePackerCause`, и подмена
			 *       переносимой причины красит одну лишь её (2014 прогнано, 2013 прошло).
			 *       До 05.09.2026 место было слепым: набор оснастку по ходу работы не менял
			 */
			 ...
			 *
			 * @note Заслон стережёт СВОЮ бухгалтерию, а не провод: кадр этот уложен самим
			 *       правщиком в нынешнем ходу, и строка правки посчитана им же. Оттого
			 *       навести отказ подделкой нельзя, и сплошной щуп 16.09.2026 нашёл место
			 *       молчащим законно. Близнец его, стерегущий строку оглавления С ПРОВОДА,
			 *       живёт у выборщика и закреплён порознь: там недоверенность настоящая
			 *
			 * @warning Убирать нельзя: без него порча собственного учёта - правка, что
			 *          собьёт смещение либо длину, - обратилась бы в чтение за концом
			 *          содержимого, а не в отказ
			 */
...
bool awh::codec::abc::Editor::commit() noexcept {
	...
	auto rollback = [this, &header, &garbage, &rows, &undo]() noexcept -> void {
		...
		if(this->_signer != nullptr){
			/**
			 * @note Пересборка закреплена `RefusedCommitRebuildsTheSigningTree`: щуп
			 *       нужности 08.09.2026 нашёл это место МОЛЧАЩИМ, ибо отказ и откат
			 *       поверялись лишь причиною да состоянием оглавления, а СЛЕДУЮЩАЯ
			 *       фиксация после отката не звучала нигде. Без пересборки она уложила
			 *       бы корень, считанный по лишним кадрам, и прошла бы УСПЕШНО - подпись
			 *       не сходилась бы на теле молча
			 */
			/**
...
bool awh::codec::abc::Editor::compact(sink_t target, const payload_t kind, uint64_t & length) noexcept {
	 ...
	 */
	/**
	 * @note Закреплено проверкою `EditorFixture.CompactingRefusesWithoutItsGear`: уборке
	 *       потребны открытый контейнер, откуда брать, и работа записи, куда класть, -
	 *       нет одного, работать не с чем (05.09.2026)
	 */
	...
	if(!this->commit())
		...
		return false;
	/**
	 * @note Отката у уборки нет, и это не упущение, а свойство её устройства: уборка
	 *       живого состояния правки НЕ ТРОГАЕТ ВОВСЕ - пишет она в поданный сток, а из
	 *       своих полей правит один лишь код отказа. Откатывать здесь попросту нечего,
	 *       тогда как фиксация выше состояние правит и откат оттого держит журналом
	 *
	 * @note Записано это 03.09.2026 по разбору всех дорог кодека на один порок: правка
	 *       состояния, отказом не откаченная. Разбор нашёл ровно одно вхождение - у
	 *       `Document::parse`, где отказ на середине оставлял обломок дерева, - и
	 *       уборка была вторым подозреваемым по числу отказов. Довод этот стоит здесь
	 *       затем, чтобы разбирающий после нас не искал отката, какому не быть
	 *
	 * @warning Довод держится на том, что уборка пишет ЛИШЬ в сток. Заведи кто-нибудь
	 *          здесь правку своих полей - и откат станет обязателен, а отсутствие его
	 *          обратится в тот самый порок
	 */
	...
	auto deliver = [&]() noexcept -> bool {
		...
		for(const uint64_t mark : marks){
			 ...
			 *
			 * @note Отсюда и молчание сплошного щупа 16.09.2026: условие недостижимо по
			 *       устройству - правка отвечает отказом одному лишь номеру за пределом
			 *       оглавления, а номер взят из него же. Заслон держится сторожем СВОЕГО
			 *       учёта, как и близнец его у выборки накопленной записи
			 */
	...
	if(this->_signer != nullptr){
		 ...
		 * Если отпечаток открытого ключа владельца выработан
		 *
		 * @note **Место это и близнец его у фиксации ЗАТЕНЯЮТ ДРУГ ДРУГА, и порознь ни
		 * одно проверкою не закрепить.** Уборка первым делом зовёт `commit()` безусловно,
		 * а та отпечаток кладёт сама, - оттого к сему месту в заголовке уже лежит верное
		 * значение, и обесточение любого ОДНОГО из двух не меняет ничего. Установлено
		 * щупом 08.09.2026: порознь молчат оба, обесточенные же РАЗОМ красят
		 * `EditorFixture.CompactionStoresTheFingerprintOfTheCurrentSigner`
		 *
		 * @warning Отсюда следует, что укладка эта ИЗБЫТОЧНА при нынешнем устройстве, - но
		 * снимать её нельзя: избыточность держится на том, что уборка зовёт фиксацию
		 * БЕЗУСЛОВНО. Пропади это условие правкой, отпечаток пропал бы молча, а проверка
		 * осталась бы зелёной, ибо закрепляет она договор («заголовок несёт отпечаток
		 * нынешнего подписанта»), а не место его исполнения
		 *
		 * @note Проверка ведёт уборку ключом ИНЫМ, нежели подписан прежний контейнер: при
		 * одном ключе новая выработка от унаследованного значения неотличима, и утверждение
		 * отпечатка не закрепляет ничего - щуп отверг такое написание
		 */
		if(abc::fingerprint(* this->_signer, this->_name, print))
			// Выполняем установку отпечатка открытого ключа владельца
