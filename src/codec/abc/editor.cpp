...
bool awh::codec::abc::Editor::fetch(const uint64_t origin) noexcept {
	 ...
	 */
	/**
	 * @note Закреплено проверкою `EditorFixture.ChunkOffsetBeyondTheBodyIsRefused`:
	 *       смещение кадра подделывается в строке оглавления непомерным числом, сумма
	 *       кадра оглавления пересчитывается. До 05.09.2026 место стояло слепым - подделку
	 *       строки у правщика не наводил никто
	 */
