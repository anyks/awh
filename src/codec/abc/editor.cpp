...
bool awh::codec::abc::Editor::erase(const uint64_t number) noexcept {
	 ...
	 *       бы мусор сосчитанным, а запись НЕ снесённой, и уборка гналась бы за долею
	 *       мусора, какой на носителе нет. Правка эта - `entry` тут копия строки, и до
	 *       `replace` оглавление не тронуто вовсе, - равняется на такой же откат у
	 *       фиксации правок. Отказ через открытый API недостижим (номер поверен выше),
	 *       и переставлено ради порядка, а не по случившемуся
	 */
