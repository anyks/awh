...
bool awh::codec::abc::Assembler::append(const void * buffer, const size_t size, const payload_t kind) noexcept {
	 ...
	 *          и проверкою такое не закрепить - буфер потребен настоящий. Сплошной щуп
	 *          17.09.2026 нашёл место молчащим, и молчание это означает недостижимость
	 *          условия ценою прогона, а не мёртвый путь: сама укладка накопленного хожена
	 *          соседним заслоном, что стоит выше и стережёт смену вида содержимого
	 */
...
bool awh::codec::abc::Loader::next(vector <uint8_t> & result, chunk_t & chunk) noexcept {
	...
	while(true){
		 ...
		 *
		 * @note Заслон ненулевого смещения ИЗБЫТОЧЕН для исхода, и это измерено щупом
		 *       17.09.2026: отбрасывание нуля октетов есть работа пустая, а не ошибочная.
		 *       Держится он затем, чтобы не звать `erase` и не трогать вместилище там, где
		 *       трогать нечего
		 */
		if(this->_offset > 0){
