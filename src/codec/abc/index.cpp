 ...
 *
 */
void awh::codec::abc::Index::truncate(const size_t size) noexcept {
	/**
	 * Если затребованное количество строк меньше нынешнего
	 *
	 * @note Количество, большее нынешнего, оглавления не трогает: работа усекает, а не
	 *       заводит строк из ничего
	 */
	if(size < this->_entries.size())
		// Выполняем усечение строк оглавления контейнера
		this->_entries.resize(size);
}
/**
...
bool awh::codec::abc::Index::unpack(const void * buffer, const size_t size, error_t & error) noexcept {
	...
	for(size_t i = 0; i < (size / ENTRY_LENGTH); i++){
		...
		/**
		 * Если хвостовые октеты строки несут неведомое.
		 *
		 * Четыре октета эти оставлены впрок, укладка кладёт их нулями, и требование
		 * нулей равняется на сличение разрядов свойств, стоящее ниже: оба поля приходят
		 * с провода, и оба обязаны быть опознаны. Без него занятые кем-то октеты
		 * проходили бы молча, а строка числилась бы понятой целиком
		 */
		if(abc::gather(record + 20, 4) != 0){
		 ...
		 *
		 * Сличение это равняется на сличение разрядов кадра: оба поля приходят с провода
		 * и оба обязаны быть опознаны. Порча старших разрядов без этой проверки уходила
		 * бы обратно в запись при перекладке контейнера.
		 *
		 * ИСПРАВЛЕНО 29.08.2026: прежний довод гласил, будто оглавление целостностью не
		 * защищено вовсе, ибо кадр контрольной суммы не несёт. Довод этот УСТАРЕЛ -
		 * сумма кадра заведена 25.08.2026 и покрывает заголовок вместе с содержимым, а
		 * снятие оглавления идёт ПОСЛЕ сличения её. Случайная порча сюда более не
		 * доходит. Сличение оттого стережёт не порчу, а ПОДДЕЛКУ: подделыватель сумму
		 * пересчитывает, и разряды свойств приходят к нам какими он положит.
		 *
...
bool awh::codec::abc::Fetcher::open(source_t source) noexcept {
	 ...
	 *
	 * @details Длина из кадра НЕДОВЕРЕННА: испорченные четыре октета её заставляли завести
	 *          до четырёх гигабайт памяти прежде, чем чтение ответит отказом по
...
bool awh::codec::abc::Fetcher::record(const uint64_t number, vector <uint8_t> & result) noexcept {
	...
	if(!this->_cached || (this->_origin != entry.chunk)){
		 ...
		 *
		 * @details Сторож этот стоит ПРЕЖДЕ первого же чтения, а не после него: смещение
		 *          приходит из оглавления, а оно недоверенно, и сложение `HEADER_LENGTH +
		 *          entry.chunk` при смещении близ предела разрядной сетки ЗАВЕРНУЛОСЬ бы,
		 *          подав источнику октетов смещение малое вместо непомерного
		 *
		 * @note Тело ограничено заголовком, а заголовок несёт контрольную сумму, и
		 *       объявленное им поверять можно
		 */
		if(entry.chunk >= this->_header.length){
			// Выполняем установку кода отказа повреждённой записи оглавления
			this->fail(error_t::INVALID_CHUNK);
			// Выводим признак неудачной выборки записи
			return false;
		}
		 ...
		 *
		 * @details Длина эта прочитана из САМОГО контейнера и недоверенна: испорченные
		 *          четыре октета её заставляли завести до 4 ГиБ прежде, чем чтение
		 *          ответит отказом по недочитанному. Тело же ограничено заголовком, и
		 *          кадр, за него выходящий, негоден заведомо - заводить место под него
		 *          не приходится вовсе
		 *
		 * @note Сличение ведётся ВЫЧИТАНИЕМ от предела тела, а не сложением смещения с
		 *       длиною: слагаемые оба недоверенны, и сумма их завернулась бы, пройдя
		 *       сторож насквозь. Смещение кадра поверено выше, оттого вычитание
		 *       заведомо не уходит ниже нуля
		 *
		 * @note Сторож этот стоит ЗДЕСЬ, а не в источнике октетов: источник даётся
		 *       потребителем, и полагаться на разумность чужой работы нельзя
		 */
		if((this->_header.length - entry.chunk) < (static_cast <uint64_t> (CHUNK_HEADER) + length)){
			// Выполняем установку кода отказа повреждённой записи оглавления
			this->fail(error_t::INVALID_CHUNK);
			// Выводим признак неудачной выборки записи
			return false;
