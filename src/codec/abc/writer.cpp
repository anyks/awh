 ...
 *
 * @note Предел ширины вида берётся ЗДЕСЬ, в теле, а не в объявлениях писаря, оттого и
 *       заголовок его нужен прямо: договор заголовка `numeric_limits` не поминает, и
 *       ждать его оттуда неоткуда. На libc++ он приходил попутно через иные заголовки,
 *       и на рабочей машине дефекта видно не было вовсе - сборка падала лишь там, где
 *       стоит libstdc++: Debian (g++ 12.2), Alpine (g++ 15.2) и DragonFly. Опознано
 *       раскладкой стендов кодеков по восьми машинам 13.09.2026
 */
#include <limits>
#include <cstring>
#include <sys/log.hpp>
...
bool awh::codec::abc::Writer::text(const string_view value) noexcept {
	 ...
	 *       звено собираемого кусками заводится с признаком неопределённой длины, а
	 *       `account` уменьшает счёт значений лишь у вместимого ОПРЕДЕЛЁННОЙ длины -
	 *       учёт куска оттого ничего не меняет. Выход этот держится скорым путём и
	 *       ясностью намерения, а не действием своим
	 *
	 * @note Договор же - «собранное кусками есть ОДНО значение вместившего» - закреплён
	 *       проверками `SegmentedBlobInsideContainer` и `SegmentedStringInsideContainer`;
	 *       держится он `account`, а не этим выходом
	 */
	if(chunk)
...
bool awh::codec::abc::Writer::blob(const void * buffer, const size_t size) noexcept {
	 ...
	 * Если уложен кусок собираемого значения, учитывать его вместившим нельзя
	 *
	 * @note Заслон молчащий и законно молчащий - см. пояснение у такого же выхода при
	 *       укладке строки выше
	 */
	if(chunk)
		// Сообщаем, что укладка куска успешна
