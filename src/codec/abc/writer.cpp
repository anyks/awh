...
namespace {
	/**
	 * @brief Порог числа имён поля, за каким заводится указатель имён
	 *
	 * @note Величина взята у разбора (`Reader`) знак в знак: поверка повторов там и
	 *       здесь решает один и тот же вопрос, и порог у них обязан совпадать
	 *
	 */
	constexpr size_t HASHING_NAMES = 32;
	/**
	 * @brief Функция выработки свёртки записи имени поля отображения
	 *
	 * @details Свёртка эта СВОЯ, а не взятая у модуля шифрования, и наружу не выходит ни
	 *          единым октетом. Взята FNV-1a: сличение имён ею не решается, а лишь
	 *          ускоряется, и ложное совпадение стоит одного лишнего сличения октетов, а
	 *          не ложного отказа
	 *
	 * @note Тело её повторяет тело такой же работы у разбора знак в знак. Расходиться им
	 *       нельзя лишь в одном - в скорости; итог же у них никуда не выходит и ни с чем
	 *       не сличается, оттого и согласовывать их незачем
	 *
	 * @param buffer буфер записи имени поля
	 * @param size   размер записи имени поля в октетах
	 * @return       выработанная свёртка записи имени поля
	 *
	 */
	uint64_t digest(const uint8_t * buffer, const size_t size) noexcept {
		// Собираемая свёртка записи имени поля
		uint64_t result = 0xCBF29CE484222325ull;
		/**
		 * Выполняем перебор всех октетов записи имени поля
		 */
		for(size_t i = 0; i < size; i++){
			// Выполняем внесение очередного октета в свёртку
			result ^= static_cast <uint64_t> (buffer[i]);
			// Выполняем перемешивание свёртки множителем
			result *= 0x100000001B3ull;
		}
		// Выводим выработанную свёртку записи имени поля
		return result;
	}
};
...
bool awh::codec::abc::Writer::prepare(const bool container) noexcept {
	/**
	 * @note Заслон `!prepare(false)` стоит у КАЖДОЙ работы укладки порознь - одиннадцать
	 *       мест по счёту щупа нужности 08.09.2026, - и все одиннадцать МОЛЧАЛИ. Причина
	 *       видна из набора: отказ у нас наводили, но следом за ним сборщика бросали, а
	 *       не пользовались им далее. Меж тем запись, собранная поверх отказа, вышла бы
	 *       наполовину годной - начало правильно, хвост лёг мимо объявленного, - и
	 *       негодность эта не читалась бы ничем до самого разбора. Закреплено
	 *       `CodecAbcWriter.TheFailedBuildingAcceptsNothingFurther`, перебирающей работы
	 *       ВСЕ до единой: заслоны эти друг друга не стерегут
	 */
 ...
 *
 */
void awh::codec::abc::Writer::rehash(const size_t depth, frame_t & frame, const size_t needed) noexcept {
	// Если запаса указателей на глубину звена ещё не хватает, расширяем его
	if(this->_tables.size() <= depth)
		// Выполняем расширение запаса указателей до глубины звена
		this->_tables.resize(depth + 1);
	// Выполняем получение указателя имён полей, отвечающего глубине звена
	vector <uint32_t> & table = this->_tables.at(depth);
	// Заводимая вместимость указателя имён полей вместимого
	size_t capacity = 64;
	/**
	 * Выполняем подбор вместимости указателя, вдвое превышающей число имён
	 *
	 * @details Загрузка держится ниже половины намеренно: открытая засылка вырождается
	 * в тот же обход по мере заполнения, и запас этот есть плата за постоянство поиска
	 */
	while(capacity < (needed * 2))
		// Выполняем удвоение вместимости указателя имён полей
		capacity <<= 1;
	/**
	 * Выполняем заведение указателя имён полей вместимого заново
	 *
	 * @details Обнуление идёт по всей вместимости, а не по прежней величине: гнёзда
	 * сверх неё могли остаться занятыми от прежнего вместимого той же глубины
	 */
	table.assign(capacity, 0);
	// Выполняем запоминание величины заведённого указателя имён полей
	frame.hashed = capacity;
	// Выполняем получение маски места гнезда указателя
	const size_t mask = (capacity - 1);
	/**
	 * Выполняем перебор всех имён полей, принадлежащих вместимому
	 */
	for(size_t i = frame.base; i < this->_keys.size(); i++){
		// Выполняем получение места гнезда указателя младшими разрядами свёртки
		size_t slot = (static_cast <size_t> (this->_keys.at(i).digest) & mask);
		/**
		 * Выполняем поиск свободного гнезда указателя имён полей
		 */
		while(table.at(slot) != 0)
			// Выполняем переход к следующему гнезду указателя
			slot = ((slot + 1) & mask);
		// Выполняем занятие гнезда указателя местом имени поля в перечне
		table.at(slot) = static_cast <uint32_t> (i + 1);
	}
}
/**
 ...
 *
 */
bool awh::codec::abc::Writer::duplicated(frame_t & frame, const size_t offset, const size_t length, const uint64_t digest) noexcept {
	// Выполняем получение числа имён полей, уложенных вместимым
	const size_t count = (this->_keys.size() - frame.base);
	/**
	 * Если указателя имён полей вместимое ещё не завело
	 */
	if(frame.hashed == 0){
		/**
		 * Если порога числа имён вместимое ещё не достигло, сличаем обходом перечня
		 *
		 * @details Обход растёт квадратом числа имён, но множитель его - одно сличение
		 * свёрток, - и на десятке имён он дешевле самого заведения указателя
		 */
		if(count < HASHING_NAMES){
			/**
			 * Выполняем перебор отрезков записей прежних имён полей вместимого
			 */
			for(size_t i = frame.base; i < this->_keys.size(); i++){
				// Выполняем получение отрезка записи прежнего имени поля
				const naming_t & key = this->_keys.at(i);
				// Если свёртки либо длины записей имён не совпадают, имена различны заведомо
				if((key.digest != digest) || (static_cast <size_t> (key.length) != length))
					// Переходим к следующему отрезку записи имени поля
					continue;
				// Если записи имён совпали октет в октет
				if((length == 0) || (::memcmp(this->_record.data() + static_cast <size_t> (key.offset),
				 this->_record.data() + offset, length) == 0))
					// Сообщаем, что имя поля в этом вместимом уже было
					return true;
			}
			// Сообщаем, что имя поля в этом вместимом ещё не встречалось
			return false;
		}
		/**
		 * Если мест перечня указателю уже не выразить, сличаем обходом перечня
		 *
		 * @details Гнездо держит место имени числом в четыре октета, и перечень шире
		 * того указателю неподвластен
		 */
		if(this->_keys.size() >= static_cast <size_t> (numeric_limits <uint32_t>::max())){
			/**
			 * Выполняем перебор отрезков записей прежних имён полей вместимого
			 */
			for(size_t i = frame.base; i < this->_keys.size(); i++){
		...
		// Выполняем заведение указателя имён полей вместимого
		this->rehash(this->_stack.size() - 1, frame, count + 1);
	/**
	 * Если загрузка указателя перевалила за половину, перестраиваем его вдвое шире
	 */
	} else if(((count + 1) * 2) > frame.hashed)
		// Выполняем перестроение указателя имён полей вместимого
		this->rehash(this->_stack.size() - 1, frame, count + 1);
	// Выполняем получение указателя имён полей, отвечающего глубине звена
	vector <uint32_t> & table = this->_tables.at(this->_stack.size() - 1);
	// Выполняем получение маски места гнезда указателя
	const size_t mask = (frame.hashed - 1);
	// Выполняем получение места гнезда указателя младшими разрядами свёртки
	size_t slot = (static_cast <size_t> (digest) & mask);
	/**
	 * Выполняем обход занятых гнёзд указателя имён полей вместимого
	 */
	while(table.at(slot) != 0){
		// Выполняем получение отрезка записи прежнего имени поля
		const naming_t & key = this->_keys.at(static_cast <size_t> (table.at(slot)) - 1);
		/**
		 * Если свёртки и длины записей имён совпали, сличаем их октет в октет
		 */
		if((key.digest == digest) && (static_cast <size_t> (key.length) == length) &&
		   ((length == 0) || (::memcmp(this->_record.data() + static_cast <size_t> (key.offset),
		    this->_record.data() + offset, length) == 0)))
			// Сообщаем, что имя поля в этом вместимом уже было
			return true;
		// Выполняем переход к следующему гнезду указателя
	...
	/**
	 * Выполняем занятие свободного гнезда местом имени поля, укладываемого зовущим
	 *
	 * @details Место берётся концом перечня: зовущий укладывает туда запись имени сразу
	 * по возврату отсюда
	 */
	table.at(slot) = static_cast <uint32_t> (this->_keys.size() + 1);
	// Сообщаем, что имя поля в этом вместимом ещё не встречалось
	return false;
}
/**
...
bool awh::codec::abc::Writer::account(const size_t start) noexcept {
	...
	if(frame.mapping && frame.expectKey){
		 ...
		 */
		/**
		 * @note Место это НЕДОСТИЖИМО проверкою: оно требует настоящих четырёх гигабайт
		 *       уложенных октетов, тогда как заслоны той же величины у разбора достижимы
		 *       вполне - там сличается ОБЪЯВЛЕННАЯ длина, а не отведённая, и закреплены
		 *       они проверкою WideNumbersAndExtensionsNameTheirCauses. Установлено щупом
		 *       по местам отказа 04.09.2026
		 */
		...
		if(this->_settings.duplicates && !this->_settings.canonical){
			/**
			 * Выполняем выработку свёртки записи имени поля отображения
			 *
			 * @details Сличение идёт со ВСЕМИ прежними именами вместимого, то есть
			 * квадратом их количества, и полное сличение октетов на всякой паре обходится
			 * дорого. Свёртка обращает перебор в числовое сличение, а указатель имён -
			 * в постоянный поиск
			 *
			 * @note Устройство взято у разбора (`Reader::duplicated`) знак в знак, и
			 * заведено оно по ЗАМЕРУ, а не по подобию. Замер 03.09.2026, сборка
			 * отображения на 16 000 имён: поверка повторов стоила 590 мс против 5.3 мс при
			 * снятой поверке - 112 крат, - и росла КВАДРАТОМ (1000 имён 3.1 мс, 2000 - 10.3,
			 * 4000 - 43.2, 8000 - 187.5). Разбор той же записи, у какого указатель уже был,
			 * шёл линейно и укладывался в 13.6 мс, то есть сборка была вчетверо с лишним
			 * медленнее разбора на тех же данных
			 *
			 * @note Свёртка отказа НЕ решает: совпадение её ведёт к полному сличению
			 * октетов, и повтор объявляется лишь им
			 */
			const uint64_t digest = ::digest(this->_record.data() + start, length);
			// Если имя поля отображения в этом вместимом уже было
			if(this->duplicated(frame, start, length, digest))
				// Выполняем объявление отказа сборки
				return this->fail(error_t::DUPLICATE_KEY);
			// Выполняем запоминание отрезка записи уложенного имени поля
			this->_keys.emplace_back(static_cast <uint32_t> (start), static_cast <uint32_t> (length), digest);
		}
		// Выполняем запоминание отрезка записи уложенного имени поля
...
bool awh::codec::abc::Writer::boolean(const bool value) noexcept {
	...
	abc::mark(this->_record, group_t::SINGLE,
	 static_cast <uint8_t> (value ? single_t::YES : single_t::NO));
	// Выполняем учёт уложенного значения
	return this->account(start);
}
/**
