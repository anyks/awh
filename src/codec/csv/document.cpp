 ...
 */
/**
 * @brief Метод проверки того, что запись числа означает не нуль
 *
 * @details Судится запись, а не разобранное значение: значение вправе обратиться нулём
 * при разборе, и тогда сличать с ним уже нечего. Нулём запись означает лишь тогда, когда
 * все цифры мантиссы её нулевые: показатель степени нуля ненулевым не сделает, а знак и
 * подавно
 *
 * @note Записи «inf» и «nan» цифр не содержат вовсе и нулевыми здесь не числятся: они и
 *       не нуль, и до этой проверки не доходят - нулю равными они не бывают
 *
 * @param text проверяемая запись числа
 * @return     признак того, что запись означает не нуль
 *
 */
static bool nonzero(const ::std::string_view text) noexcept {
	/**
	 * Выполняем перебор всех знаков записи числа до показателя степени
	 */
	for(const char letter : text){
		/**
		 * Если начался показатель степени
		 *
		 * @note Цифры показателя мантиссу нулевой не делают и ненулевой тоже: перебор
		 *       на них и обрывается
		 */
		if((letter == 'e') || (letter == 'E'))
			// Выходим из перебора знаков записи числа
			break;
		/**
		 * Если знак является ненулевой цифрой
		 */
		if((letter >= '1') && (letter <= '9'))
			// Выводим признак ненулевой записи числа
			return true;
	}
	// Выводим признак нулевой записи числа
	return false;
}
template <typename T>
bool awh::codec::csv::Document::numeric(const size_t row, const size_t col, T & result) const noexcept {
	 ...
	 */
		...
		/**
		 * Если число, нулём не бывшее, обратилось в затребованном виде нулём
		 *
		 * @details Сторож этот - второй конец той же пары, что и сличение с пределом
		 * выше. Переполнение обращает число бесконечностью, исчезновение обращает его
		 * нулём, и утрата в обоих случаях полная: правило кодека велит отвечать отказом
		 * там, где число теряется, а стоял сторож лишь у одного конца. Замер 31.08.2026:
		 * «1e-50» видом `float` отдавало нуль признаком УСПЕХА, и запись, число
		 * содержавшая, выходила пустым значением молча
		 *
		 * @note Отвергается лишь утрата ПОЛНАЯ, как и у переполнения: число, потерявшее
		 *       часть разрядов, но уцелевшее, принимается обоими концами равно. Правило
		 *       здесь одно, и оно не о точности, а о самом наличии числа
		 *
		 * @warning Сличается ИТОГ приведения, а не исходное число с наименьшим
		 *          значением вида: наименьшее нормальное - не предел, ниже него лежат
		 *          поднормальные, и число, в них попавшее, уцелело. Замер на рабочей
		 *          машине: «1e-45» видом `float` уцелевает поднормальным и принимается,
		 *          «1e-50» обращается нулём и отвергается
		 *
		 * @warning Сличается запись, а не разобранное значение, и это существенно. Довод
		 *          прежде говорил, что довольно сличить итог с исходным числом, - и довод
		 *          тот ОКАЗАЛСЯ НЕВЕРЕН. На системах, поднормальных не имеющих, разбор
		 *          обращает число нулём САМ, и сличать с ним уже нечего: исходное число
		 *          тоже нуль. Замер 31.08.2026 - машина без поднормальных воспроизведена
		 *          разрядом обнуления в `FPCR` архитектуры ARM64, стенда не понадобилось:
		 *          «1e-320» и «5e-324» видом `double` отдавали нуль признаком успеха.
		 *          Оттого проверка и судит запись: нулевой она бывает лишь тогда, когда
		 *          все цифры мантиссы нулевые
		 *
		 * @note Разбор числа ведёт здесь `lexical_t`, а не `strtod`: он вычисляет значение
		 *       сам, и обнуление поднормальных приходит к нему арифметикою машины, а не
		 *       библиотекою системы. Наблюдения над `strtod` сюда не переносятся
		 */
		if((result == static_cast <T> (0)) && ::nonzero(value))
			// Выводим признак неудачного приведения
			return false;
	 ...
	 */
		 ...
		 * Если приведение содержимого поля не удалось
		 *
		 * @note Целое, записанное дробно, извлекается тем же числом: «300.0» и «300»
		 *       суть одно число, и расходиться извлечению по одному лишь написанию
		 *       нечем. Решение это владельца от 30.08.2026, и проведено оно у всех
		 *       кодеков рамки, где число извлекается видом
		 *
		 * @note Написание с настоящею дробною частью отказ сохраняет: «300.5» видом
		 *       целым отвергается, ибо дробь при таком извлечении потерялась бы, а
		 *       правило кодека - отвечать отказом там, где число теряется. Кодеки JSON
		 *       и XML в том же месте округляют, следуя языку, и расхождение это
		 *       намеренно: там выдаётся само значение, отказать которым нечем
		 *
		 * @warning Сличение ведётся лишь у конечных чисел: «inf» и «nan» целым видом
		 *          отвергаются, ибо целого числа они не означают вовсе
		 */
		if(!integer(value, number)){
			// Полученное число с плавающей точкой
			double real = 0.;
			/**
			 * Если содержимое поля не является и числом с плавающей точкой
			 */
			if(!csv::real(value, real))
				// Выводим признак неудачного приведения
				return false;
			/**
			 * Если число, нулём не бывшее, разбором обратилось нулём
			 *
			 * @details Сторож этот - тот же, что стоит у ветви дробной, и стоять он
			 * обязан у ВСЕХ трёх: правило кодека велит отвечать отказом там, где число
			 * теряется, а вид приёмника к утрате отношения не имеет. Прежде сторож был
			 * поставлен лишь дробной ветви, и целые пропускали утрату молча
			 *
			 * @warning Сличается ЗАПИСЬ, а не разобранное значение: на машине, поднормальных
			 *          не имеющей, разбор обращает число нулём САМ, и сличать с исходным уже
			 *          нечего - оно тоже нуль. Замер 01.09.2026 подделкою такой машины
			 *          разрядом обнуления `FPCR.FZ`: «1e-320» видом `uint8_t` отдавало нуль
			 *          признаком УСПЕХА, и запись, число содержавшая, выходила нулём молча.
			 *          На машине поднормальные несущей путь этот недостижим - число уцелевает
			 *          и отвергается ниже сличением с усечением, - оттого дефект и прожил
			 *          незамеченным
			 */
			if((real == 0.) && ::nonzero(value))
				// Выводим признак неудачного приведения
				return false;
			/**
			 * Если число дробно либо целым видом не представимо
			 *
			 * @note Пределы сличаются степенями двойки, а не пределами вида: пределы
			 *       вида к `double` приводятся с округлением, и сличение с ними
			 *       пропускало бы значение, в вид не помещающееся
			 */
			if(!::isfinite(real) || (real != ::trunc(real)) ||
			   (real < -::ldexp(1., 63)) || (real >= ::ldexp(1., 63)))
				// Выводим признак неудачного приведения
				return false;
			/**
			 * Если число дробным написанием представлено неточно
			 *
			 * @details Двойная точность держит целые без утраты лишь до двух в
			 * пятьдесят третьей: выше того разбор дробного написания теряет разряды
			 * ЕЩЁ ДО возврата, и вернувшееся число написанному уже не равно. Сличить
			 * их нечем - написание к тому мигу разобрано, - и признать такое число
			 * значило бы отдать утрату признаком успеха. Замер 31.08.2026:
			 * «9007199254740993.0» видом `uint64_t` давало 9007199254740992, а
			 * «9223372036854775807.0» - 9223372036854775808
			 *
			 * @note Целое написание предела этого не знает: разбор целого читает
			 *       разряды сам и до двойной точности не доходит вовсе. Оттого
			 *       «18446744073709551615» принимается, а «18446744073709551615.0» -
			 *       нет, и расхождение это не написаний одного числа, а точного
			 *       написания с неточным
			 */
			if(::fabs(real) >= ::ldexp(1., 53))
				// Выводим признак неудачного приведения
				return false;
			// Запоминаем полученное целое число со знаком
			number = static_cast <int64_t> (real);
		}
		/**
		 * Если число в затребованный вид не помещается
		 *
	 ...
	 */
		...
		if(!integer(value, number)){
			...
			if((real == 0.) && ::nonzero(value))
				...
				return false;
			/**
			 * Если число дробно, отрицательно либо целым видом не представимо
			 *
			 * @note Отрицательное отвергается, а не заворачивается по кругу: тем же
			 *       отказом отвечает и запись «-1» видом без знака, и расходиться этим
			 *       двум написаниям нечем
			 */
			if(!::isfinite(real) || (real != ::trunc(real)) ||
			   (real < 0.) || (real >= ::ldexp(1., 64)))
				// Выводим признак неудачного приведения
				return false;
			/**
			 * Если число дробным написанием представлено неточно
			...
			if(::fabs(real) >= ::ldexp(1., 53))
				// Выводим признак неудачного приведения
				return false;
			// Запоминаем полученное целое число без знака
			number = static_cast <uint64_t> (real);
		}
		/**
		 * Если число в затребованный вид не помещается
		 *
