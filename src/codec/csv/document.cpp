...
bool awh::codec::csv::Document::load(const string & filename) noexcept {
	...
	while(file){
		...
		/**
		 * @warning Место это набором НЕ покрыто: доказанная подмена кода (05.09.2026) не
		 *          красит ни одной проверки. Признак негодности потока ставится лишь
		 *          отказом самого устройства чтения посреди уже открытого файла, а такой
		 *          отказ стендом не подделывается: каталог распознаётся ДО открытия, а
		 *          пустой и обрезанный файлы читаются до конца благополучно. Сторож этот -
		 *          сеть безопасности, и снимать его нельзя: без него отказ доступа
		 *          отвечал бы `EMPTY_TEXT` - доводом ложным
		 *
		 * @note Порядок здесь ЗДОРОВЫЙ: очистка стоит ПЕРЕД постановкой кода, и код
		 *       возврата переживает её. Разметка то же место имела с порядком обратным -
		 *       код ставился, а следом гиб в очистке, - и это чинилось 04.09.2026
		 */
		/**
	 ...
	 *
	 * @note Ветвь эта НЕДОСТИЖИМА проверкой и оттого не покрыта: признак `bad` ставится
	 *       отказом САМОГО УСТРОЙСТВА посреди чтения - обрывом носителя, отказом сетевой
	 *       файловой системы, - а переносимого способа устроить такой отказ у набора нет.
	 *       Способы, что есть, непереносимы: `/dev/full` даёт отказ ЗАПИСИ и живёт лишь у
	 *       Linux, обрыв канала неотличим от конца текста
	 *
	 * @warning Снимать ветвь нельзя, и она опаснее прочих: без неё усечённое чтение выдаётся
	 *          за таблицу ЦЕЛУЮ - вызов отвечает успехом, а записей недостаёт. То третий
	 *          разряд беды у потребителя, «принято молча, а прочитано не всё», и отличить
	 *          его от таблицы, вправду короткой, ему нечем
	 */
	/**
