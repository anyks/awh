...
bool awh::codec::csv::Document::save(const string & filename) const noexcept {
	 ...
	 *
	 * @warning Запись ведётся ДОПИСЫВАНИЕМ в хвост, а не ходом `write` со смещением, и
	 *          выбор этот замерен, а не взят из договора: у `write` ветви РАСХОДЯТСЯ по
	 *          платформам. У MS Windows он открывает файл признаком `OPEN_ALWAYS` и пишет
	 *          по смещению, а у POSIX заводит `ofstream` с признаком `ios::binary`, к
	 *          какому подмешивается `ios::out`, - и файл УСЕКАЕТСЯ каждым зовом. Замерено
	 *          09.09.2026 щупом: два куска, писанные со смещениями 0 и 7, дали содержимое
	 *          `             ВТОРОЙ` вместо `ПЕРВЫЙ-ВТОРОЙ` - первый кусок затёрт, а
	 *          недостача добита пустотою до нужной длины
	 *
	 * @warning Заслон по величине этой беды НЕ ЛОВИТ: длина выходит ровно ожидаемая - 25
	 *          байтов из 25, - и сличение размера отвечает согласием на испорченном файле.
	 *          Разряд беды тот самый, «принято молча». Дописывание же верно на обеих
	 *          ветвях: тот же щуп дал `ПЕРВЫЙ-ВТОРОЙ`
	 *
 ...
 *
 */
awh::codec::csv::Document::Document(const fmk_t * fmk, const log_t * log) noexcept :
 _log(log), _fmk(fmk), _fs(fmk, log), _error(error_t::NONE), _opened(false) {}
/**
 * @brief Конструктор
 *
 * @param fmk      объект фреймворка
 ...
 *
 */
awh::codec::csv::Document::Document(const fmk_t * fmk, const log_t * log, const settings_t & settings) noexcept :
 _log(log), _fmk(fmk), _fs(fmk, log),
