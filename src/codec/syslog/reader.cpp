...
bool awh::codec::syslog::Reader::legacy(const string_view record, size_t offset) noexcept {
	...
	while((i < record.size()) && (words < LEGACY_DATE_WORDS)){
		...
		if(timed){
			...
			while((next < record.size()) && (record[next] >= '0') && (record[next] <= '9'))
				...
				next++;
			/**
			 * Если за временем следует год четырьмя цифрами
			 *
			 * @note Проверка на разделитель нужна: без неё именем узла из четырёх цифр
			 *       дата прирастала бы молча
			 */
	 ...
	 *
	 * @note Метка оканчивается двоеточием либо открывающей скобкой опознавателя
	 *       работы, а пробел её обрывает: описание метку знаками не ограничивает вовсе,
	 *       но пробел там означает, что метки нет, а есть уже текст сообщения
	 */
	while((offset < record.size()) && (record[offset] != ':') && (record[offset] != '[') && (record[offset] != ' '))
		// Сдвигаем смещение разбора
		offset++;
