	...
	while(true){
		...
		if(letter == ':'){
			...
			if(sequence && !bracket.paired){
				 ...
				 *
				 * @warning Отказ этот НЕДОСТИЖИМ: место начала записи запоминается размахом накопителя
				 *          прежде выдачи хоть одного события её, а накопитель внутри разбора строки не
				 *          убывает - снятие с конца ниже тут же возвращает снятое вставкою, а опустошают
				 *          его лишь `clear`, `fail` и `commit`, и ни один из них внутри разбора строки
				 *          не зовётся. Замерено подменою кода отказа особым: ни одно из шести написаний
				 *          пары внутри перечня сюда не дошло. Довод тот же, каким недостижим страж
				 *          `keyed` - оба стерегут одно и то же место одного накопителя
				 *
