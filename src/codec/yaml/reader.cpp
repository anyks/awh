...
namespace {
	...
	/**
	 * @brief Функция проверки содержимого метки `!!binary` на запись base64
	 *
	 * @details Описание YAML 1.1 велит содержимому метки этой быть записью base64 по RFC
	 * 2045, где пробельные знаки дозволены между всяких четвёрок и значения не несут.
	 * Поверки этой у кодека не было ВОВСЕ: содержимое принималось дословно, и код
	 * `INVALID_BINARY` обещал сверку, какой нет. Довод при месте применения метки гласил,
	 * будто сверяет её извлечение содержимого, - и то было ложью: разбора base64 в кодеке
	 * не нашлось ни одного места
	 *
	 * @note Разбор здесь НЕ ведётся, ведётся одна поверка записи: содержимое хранится
	 *       дословно и разбирается потребителем, а дело кодека - не пропустить негодного
	 *
	 * @param text поверяемое содержимое метки типа
	 * @return     признак того, что содержимое отвечает записи base64
	 *
	 */
	static bool base64(const string_view text) noexcept {
		// Количество значащих знаков записи
		size_t count = 0;
		// Количество знаков заполнения в конце записи
		size_t padding = 0;
		/**
		 * Выполняем перебор знаков поверяемого содержимого
		 */
		for(const char letter : text){
			/**
			 * Если знак является пробельным
			 *
			 * @note Пробельные знаки дозволены где угодно и значения не несут: длинное
			 *       содержимое описанием велено разбивать на строки
			 */
			if((letter == ' ') || (letter == '\t') || (letter == '\n') || (letter == '\r'))
				// Выполняем переход к следующему знаку содержимого
				continue;
			/**
			 * Если знак является знаком заполнения
			 */
			if(letter == '='){
				// Выполняем учёт знака заполнения
				padding++;
				// Выполняем учёт знака записи
				count++;
				// Выполняем переход к следующему знаку содержимого
				continue;
			}
			/**
			 * Если знак заполнения уже встречен, а знак значащий стоит за ним
			 *
			 * @note Заполнение стоит лишь в конце записи: знак значащий за ним означает
			 *       запись негодную, а не длинную
			 */
			if(padding > 0)
				// Выводим признак негодности записи
				return false;
			/**
			 * Если знак не принадлежит словарю записи base64
			 */
		...
		/**
		 * Если знаков заполнения больше двух
		 */
		if(padding > 2)
			// Выводим признак негодности записи
			return false;
		// Выводим признак того, что длина записи кратна четырём
		return ((count % 4) == 0);
	}
	/**
	 * @brief Функция проверки содержимого метки `!!timestamp` на запись отметки времени
	 *
	 * @details Описание YAML 1.1 велит отметке времени быть записью вида `ГГГГ-ММ-ДД`
	 * либо полною - с часами, минутами, секундами, долею секунды и поясом. Поверки этой
	 * у кодека не было вовсе, и запись `не-дата` принималась наравне с годной
	 *
	 * @note Числа месяца и дня сверяются ПО СУЩЕСТВУ, а не одним лишь видом: запись
	 *       `2020-99-99` виду отвечает, а отметкою времени не является
	 *
	 * @param text поверяемое содержимое метки типа
	 * @return     признак того, что содержимое отвечает записи отметки времени
	 *
	 */
	static bool stamp(const string_view text) noexcept {
		// Положение разбираемого знака в записи
		size_t offset = 0;
		/**
		 * @brief Способ чтения числа заданной ширины из записи
		 *
		 * @param least  наименьшее количество знаков числа
		 * @param most   наибольшее количество знаков числа
		 * @param result читаемое число
		 * @return       признак успешного чтения числа
		 *
		 */
		const auto number = [&text, &offset](const size_t least, const size_t most, uint32_t & result) noexcept -> bool {
			// Количество прочитанных знаков числа
			size_t count = 0;
			// Выполняем сброс читаемого числа
			result = 0;
			/**
			 * Выполняем перебор знаков читаемого числа
			 */
			while((offset < text.size()) && (count < most)){
				// Получаем очередной знак читаемого числа
				const char letter = text.at(offset);
				/**
				 * Если знак числом не является
				 */
				if((letter < '0') || (letter > '9'))
					// Выполняем прекращение чтения числа
					break;
				// Выполняем накопление читаемого числа
				result = ((result * 10) + static_cast <uint32_t> (letter - '0'));
				// Выполняем переход к следующему знаку записи
				offset++;
				// Выполняем учёт прочитанного знака числа
				count++;
			}
			// Выводим признак того, что знаков числа прочитано довольно
			return (count >= least);
		};
		// Год, месяц и день отметки времени
		uint32_t year = 0, month = 0, day = 0;
		/**
		 * Если год прочесть не удалось либо за ним не стоит разделитель даты
		 */
		if(!number(4, 4, year) || (offset >= text.size()) || (text.at(offset++) != '-'))
			// Выводим признак негодности записи
			return false;
		/**
		 * Если месяц прочесть не удалось либо за ним не стоит разделитель даты
		 *
		 * @note Ширина месяца и дня описанием дозволена и однозначная: запись `2020-1-2`
		 *       отметкою времени является вполне
		 */
		if(!number(1, 2, month) || (offset >= text.size()) || (text.at(offset++) != '-'))
			// Выводим признак негодности записи
			return false;
		/**
		 * Если день прочесть не удалось
		 */
		if(!number(1, 2, day))
			// Выводим признак негодности записи
			return false;
		/**
		 * Если числа месяца либо дня за пределами возможного
		 *
		 * @note Числа сверяются ПО СУЩЕСТВУ, а не одним лишь видом: запись `2020-99-99`
		 *       виду отвечает, а отметкою времени не является
		 */
		if((month < 1) || (month > 12) || (day < 1) || (day > 31))
			// Выводим признак негодности записи
			return false;
		/**
		 * Если запись одною датою и оканчивается
		 */
		if(offset >= text.size())
			// Выводим признак годности записи
			return true;
		/**
		 * Если за датою стоит знак, разделителем времени не являющийся
		 *
		 * @note Разделителей описанием дозволено два: буква `T` и пробельные знаки
		 */
		if((text.at(offset) == 'T') || (text.at(offset) == 't'))
			// Выполняем переход к следующему знаку записи
			offset++;
		/**
		 * Если разделителем времени служат пробельные знаки
		 */
		else if((text.at(offset) == ' ') || (text.at(offset) == '\t')){
			/**
			 * Выполняем пропуск пробельных знаков разделителя времени
			 */
			while((offset < text.size()) && ((text.at(offset) == ' ') || (text.at(offset) == '\t')))
				// Выполняем переход к следующему знаку записи
				offset++;
		// Выводим признак негодности записи
		} else return false;
		// Часы, минуты и секунды отметки времени
		uint32_t hour = 0, minute = 0, second = 0;
		/**
		 * Если часы прочесть не удалось либо за ними не стоит разделитель времени
		 */
		if(!number(1, 2, hour) || (offset >= text.size()) || (text.at(offset++) != ':'))
			// Выводим признак негодности записи
			return false;
		/**
		 * Если минуты прочесть не удалось либо за ними не стоит разделитель времени
		 */
		if(!number(2, 2, minute) || (offset >= text.size()) || (text.at(offset++) != ':'))
			// Выводим признак негодности записи
			return false;
		/**
		 * Если секунды прочесть не удалось
		 */
		if(!number(2, 2, second))
			// Выводим признак негодности записи
			return false;
		/**
		 * Если числа времени за пределами возможного
		 *
		 * @note Секунда шестидесятая дозволена: она есть секунда високосная
		 */
		if((hour > 23) || (minute > 59) || (second > 60))
			// Выводим признак негодности записи
			return false;
		/**
		 * Если за секундами стоит доля секунды
		 */
		if((offset < text.size()) && (text.at(offset) == '.')){
			// Доля секунды отметки времени
			uint32_t fraction = 0;
			// Выполняем переход к следующему знаку записи
			offset++;
			/**
			 * Если доля секунды не прочитана
			 */
			if(!number(1, 9, fraction))
				// Выводим признак негодности записи
				return false;
		}
		/**
		 * Выполняем пропуск пробельных знаков перед указанием пояса
		 */
		while((offset < text.size()) && ((text.at(offset) == ' ') || (text.at(offset) == '\t')))
			// Выполняем переход к следующему знаку записи
			offset++;
		/**
		 * Если запись временем и оканчивается
		 */
		if(offset >= text.size())
			// Выводим признак годности записи
			return true;
		/**
		 * Если поясом объявлено всемирное время
		 */
		if((text.at(offset) == 'Z') || (text.at(offset) == 'z'))
			// Выводим признак того, что за поясом ничего не стоит
			return ((offset + 1) == text.size());
		/**
		 * Если знак пояса указан
		 */
		if((text.at(offset) != '+') && (text.at(offset) != '-'))
			// Выводим признак негодности записи
			return false;
		// Часы и минуты пояса отметки времени
		uint32_t shift = 0, minutes = 0;
		// Выполняем переход к следующему знаку записи
		offset++;
		/**
		 * Если часы пояса прочесть не удалось
		 */
		if(!number(1, 2, shift))
			// Выводим признак негодности записи
			return false;
		/**
		 * Если запись поясными часами и оканчивается
		 */
		if(offset >= text.size())
			// Выводим признак годности записи
			return true;
		/**
		 * Если за часами пояса стоит знак, разделителем времени не являющийся
		 */
		if(text.at(offset++) != ':')
			// Выводим признак негодности записи
			return false;
		/**
		 * Если минуты пояса прочесть не удалось
		 */
		if(!number(2, 2, minutes))
			// Выводим признак негодности записи
			return false;
		// Выводим признак того, что за поясом ничего не стоит
	 ...
	 * @param depth глубина поточных построений, строками прежними открытых
	 ...
	 *       прежнее поведение
	 */
	static bool unclosed(const string_view line, const size_t depth) noexcept {
		 ...
		 *
		 * @warning Отсчёт ведётся ОТ ГЛУБИНЫ, строками прежними набранной, а не от нуля:
		 *          проверка эта видит одну телесную строку, и построение, открытое строкою
		 *          выше, ей невидимо. Считая с нуля, она числила запятую в написании
		 *          `[\nx, "а\n б"\n]` знаком содержимого - ибо `[` осталось на строке
		 *          первой, - и ограда за нею оградою не признавалась вовсе: склейка строк
		 *          не давала, а значение огранённое обрывалось. Нашло это сличение с
		 *          набором yaml-test-suite, случай 8UDB
		 */
		size_t flowing = depth;
		/**
		...
		while(offset < line.size()){
			 ...
			 */
			/**
			 * @warning Запятая место значения открывает лишь ВНУТРИ поточного построения:
			 *          вне его она есть знак содержимого простого значения. Сбрасывая
			 *          признак безусловно, проверка числила кавычку в записи `a: ,"`
			 *          стоящей на месте значения, склейка глотала строку следующую, и
			 *          текст `a: ,"\nb\n` принимался значением `," b`, тогда как без
			 *          хвостового перевода отвергался - строка последняя склейке не
			 *          подлежит. Нашёл это ворошитель сличением исходов по хвостовому
			 *          переводу строки
			 */
			if(((flowing > 0) && (letter == ',')) || (!plained && ((letter == '[') || (letter == '{'))) ||
...
bool awh::codec::yaml::Reader::settings(const settings_t & settings) noexcept {
	 ...
	 *       прочих настроек берёт свой `SETTINGS_ALREADY_APPLIED`. Договор мой строже
	 *       соседского: они отвергают одну лишь кодировку, я же настройки ЦЕЛИКОМ
	 *
	 * @note Кодировка сличается с ОПРЕДЕЛЁННОЮ САМИМ ЧТЕНИЕМ, а не с признаком начатого
	 *       приведения: приведение из UTF-8 в UTF-8 не ведётся вовсе и признака такого не
	 *       поднимает, - заслон, на нём стоящий, пропускал бы смену молча
	 */
	if(this->_state == state_t::HUNGRY){
		// Запоминаем код отказа смены настроек посреди подачи текста
		this->_error = ((settings.encoding != this->_decoder.encoding()) ?
		 error_t::ENCODING_ALREADY_CHOSEN : error_t::SETTINGS_ALREADY_APPLIED);
 ...
 *          пустым (число сверено 07.09.2026 розыском `if(!this->scalar(string(), style_t::PLAIN,`
 *          по этому файлу - РОВНО ЧЕТЫРНАДЦАТЬ; всех же мест вызова связки двадцать три,
 *          и девять прочих подают значение непустое, доводом этим не прикрытое): предел длины пустоте не помеха, метка порядка байтов в пустоте не
 ...
 *
 * @note Довод сверен ЗАНОВО 07.09.2026: число мест - грепом по `if(!this->collapse(`,
 *       вышло РОВНО ДВА; отсутствие отказа - чтением тела, где выход один `return true`
 *
 * @warning Первый обзор числил связку эту отказывающей ЧЕТЫРЕЖДЫ: окно чтения захватило
 *          соседей `matched()` и `expand()`, чьи `fail()` и легли на её счёт. Тело
 *          читать надлежит ГРАНИЦАМИ связки, а не окном строк
 *
...
bool awh::codec::yaml::Reader::finish(const size_t column) noexcept {
	...
	if(this->_expected){
		...
		if(this->_asked){
			 ...
			 *
			 * @note Довод этот стоит не здесь одном: мест с ним ДЕСЯТЬ, и все они покоятся на
			 *       ОДНОМ основании: пустота объявляется там, где метки вида уже нет.
			 *       Пересчитываются розыском `событие пустоты` по этому файлу: он даёт
			 *       ОДИННАДЦАТЬ строк, из коих одна есть вот эта самая запись, - мест
			 *       остаётся десять. Оговорка нужна: запись о числе мест сама попадает
			 *       в розыск, каким это число велено считать. Родство
			 *       их не в повторении слога, а в общем основании, и оттого судьба у них ОДНА:
			 *       падёт основание - падут все десять разом, и поправлять их порознь незачем.
			 *       Здесь довод и записан целиком, у прочих девяти он повторён кратко
			 *
	...
	while(offset < inner.size()){
		...
		switch(escaped){
			...
			case 'N': {
				 ...
				 *
				 * @note Доказательство здесь ПОЛНОЕ и первого рода - разрядом значений, а не
				 *       замером: `encode` отказывает ровно двум разрядам - суррогатам
				 *       0xD800…0xDFFF да значениям свыше MAX_CODEPOINT, - а знак задан
				 *       постоянной, ни в один из них не попадающей. Замер неразличимости ниже
				 *       оставлен приметою дня и доказательства не составляет: он слабее
				 *       разряда значений и нужен лишь там, где значение приходит извне.
				 *       Постоянных таких в кодеке четыре, и берутся они розыском
				 *       `encode(0x` по файлу: 0x85, 0xA0, 0x2028 и 0x2029
				 *
				 * @note Снятие стража этого НЕРАЗЛИЧИМО: замерено сличением двух сборок, одним лишь им
				 *       и различных. Наборы трёх кодеков зелены (397, 278, 276), ворошители на трёх
				 *       зёрнах по 20 000 текстов дают выдачу побайтно ту же
...
bool awh::codec::yaml::Reader::bounds(const string_view line, const size_t offset, style_t & style, size_t & length) noexcept {
	 ...
	 *
	 * @note Довод проверен ПО СУЩЕСТВУ чтением 07.09.2026, обеими половинами. Число зовущих
	 *       пересчитывается розыском `this->bounds(` по этому файлу - их РОВНО ДВА, и оба
	 *       названы именами работ, а не строками: `Reader::content()`, чей вход открывается
	 *       заставою `if(offset >= line.size()) return true;` - остаток там непуст всегда, -
	 *       да разбор огранённого значения, входящий лишь при `leading` равном кавычке
	 *       одиночной либо двойной, а тот прочтён из `line[offset]` прежде вызова
	 *
...
bool awh::codec::yaml::Reader::typing(const string_view text, const style_t style, const size_t column, type_t & type) noexcept {
	 ...
	 *
	 * @warning Прежний довод здесь гласил: «запись base64 здесь не сверяется - сверяет её
	 *          извлечение содержимого, ибо там она и разбирается». Довод ЛОЖЕН первого
	 *          разряда: разбора base64 в кодеке не нашлось НИ ОДНОГО места, и код
	 *          `INVALID_BINARY` обещал сверку, какой нет вовсе. Замерено розыском `base64`
	 *          по всему кодеку - одни лишь описания кода да этот самый довод
	 */
	if(suffix == "binary"){
		/**
		 * Если содержимое записи base64 не отвечает
		 */
		if(!::base64(text))
			// Выводим отказ негодного содержимого метки двоичного значения
			return this->fail(error_t::INVALID_BINARY, column);
		// Запоминаем вид значения, меткою типа заданный
		type = type_t::BINARY;
		// Выводим признак успешного разрешения вида
		return true;
	 ...
	 *
	 * @note Поверки этой не было вовсе, и запись `не-дата` принималась наравне с годной.
	 *       Код `INVALID_STAMP` был мёртв по той же причине, что и `INVALID_BINARY`
	 */
	if(suffix == "timestamp"){
		/**
		 * Если содержимое записи отметки времени не отвечает
		 */
		if(!::stamp(text))
			// Выводим отказ негодного содержимого метки отметки времени
			return this->fail(error_t::INVALID_STAMP, column);
		// Запоминаем вид значения, меткою типа заданный
		type = type_t::STAMP;
		// Выводим признак успешного разрешения вида
		return true;
...
bool awh::codec::yaml::Reader::content(const string_view line, const size_t offset, const uint32_t indent) noexcept {
		...
		if(!answered){
			...
			if(this->_levels.empty() || (this->_levels.back().indent < indent) ||
			   (this->_levels.back().kind != nesting_t::MAPPING)){
				...
				string anchor, tag;
				/**
				 * Если метка узла разобрана в этой же строке
				 *
				 * @warning Свойства отделяются ПОРОЗНЬ, а не признаком `_propped` разом:
				 *          признак тот верен, когда в строке разобрано ХОТЬ ОДНО свойство, и
				 *          написание `port: &m` с записью `!!str k: v` строкою ниже отдавало
				 *          имени пары ОБА - метку типа, в этой строке стоящую, и метку узла,
				 *          строкою выше объявленную. Метка `&m` меткою отображения является,
				 *          а доставалась первому имени его: перезапись выдавала `port:` со
				 *          строкою `&m !!str k: v`, и второй круг записи от первого расходился
				 *
				 * @note Нашёл это ворошитель сличением устойчивости перезаписи - после того,
				 *       как набор его выучили строить свойства имён пар. Прежде метка типа
				 *       имени пары деревом отбрасывалась, и расхождение было незримо
				 */
				if(this->_anchoredHere)
					// Выполняем изъятие метки узла у открываемого отображения
					anchor.swap(this->_anchor);
				/**
				 * Если метка типа разобрана в этой же строке
				 */
				if(this->_taggedHere)
	...
	if((leading == '|') || (leading == '>')){
		/**
		 * Если заголовок блочного значения стоит на отступе открытого вместилища
		 *
		 * @details Правило это то же, каким отвергается скалярное значение на отступе
		 *          открытого построения, - и блочное значение ему подвластно наравне с
		 *          поточным: значение пары стоит либо в строке имени её, либо ГЛУБЖЕ
		 *          имени, а заголовок на отступе самого вместилища ни значением пары не
		 *          является, ни записью его. Развилка заголовка, однако, уходила
		 *          возвратом ПРЕЖДЕ того правила, и заслон её не касался вовсе
		 *
		 * @note Написание `a:` со строкою `>` под ним давало пару с именем пустым,
		 *       содержимым какой становилась строка следующая: текст `a:\n>\nb:`
		 *       читался деревом о двух парах, где второй в тексте нет вовсе. Написание
		 *       же `a:\n>\n b` роняло содержимое молча. Нашёл это ворошитель правкой
		 *       дерева
		 */
		if(!this->_expected && !this->_awaited && !this->_plaining && !this->_levels.empty() &&
		   (indent <= this->_levels.back().indent))
			// Выводим отказ значения, вместилищем не объявленного
		...
		if(this->_levels.empty() || (this->_levels.back().indent < indent) ||
		   (this->_levels.back().kind != nesting_t::MAPPING)){
			...
			if(this->_propped){
				 ...
				 *          строкою `&m !!str k: v`, и второй круг записи от первого расходился
				 *
				 * @note Откладывание ниже беды той не покрывало: оно ведёт лишь ссылки, а
				 *       имя пары обыкновенное сюда приходит с пустым `_delayed`, и вернуть
				 *       отображению было нечего
				 *
				 * @note Нашёл это ворошитель сличением устойчивости перезаписи - после того,
				 *       как набор его выучили строить свойства имён пар. Прежде метка типа
				 *       имени пары деревом отбрасывалась, и расхождение было незримо
...
bool awh::codec::yaml::Reader::closing(const size_t column) noexcept {
	...
	if(!this->scalar(result, this->_block, this->_opening))
		...
		return false;
	/**
	 * Если блочному значению метка типа не предпослана
	 *
	 * @details Содержимое блочного значения есть строка по построению своему: заголовок
	 *          `|` да `>` берут его дословно, и разрешение вида схемою обратило бы запись
	 *          `true` строкою в логическое значение. Оттого вид его назначается строковым
	 *
	 * @warning Метка типа, ЯВНО предпосланная, вид решает сама, и назначение это её
	 *          решение отменяло: запись `!!null |` с пустым содержимым выдавалась строкою,
	 *          тогда как то же пустое содержимое под оградой - `!!null ""` - выдавалось
	 *          пустотою. Перезапись поточным построением обращала первое во второе, и
	 *          значение при перезаписи МЕНЯЛОСЬ молча - строка обращалась пустотой. Метка,
	 *          содержимому противная, сюда не доходит вовсе: разрешение вида отвергает её
	 *          выше отказом `TAG_MISMATCH` - запись `!!int |` с содержимым `5` отвергается
	 *          там. Нашёл это ворошитель сличением деревьев перезаписи иного оформления
	 */
	if(this->_staged.back().tag.length == 0)
...
bool awh::codec::yaml::Reader::flowed(const string_view line, size_t & offset) noexcept {
	...
	const char leading = line[offset];
	/**
	 * Признак того, что строка простое значение ПРОДОЛЖАЕТ, а не открывает
	 *
	 * @details Описание строку продолжения берёт правилом `s-ns-plain-next-line`, а первый
	 *          знак её - правилом `ns-plain-char`, а НЕ `ns-plain-first`: указатели там
	 *          знаками содержимого и остаются. Изымаются одни лишь указатели построения
	 *          поточного - запятая да скобки, - и они значение обрывают выше, до захода сюда
	 *
	 * @warning Прежде указатели эти разбирались свойствами узла и оградою наравне с началом
	 *          значения, и записи выходили небывалые: `[a`, `&m`, `b]` давало значение `a b`
	 *          с меткою `&m` - запись метки из содержимого пропадала, - а `[a`, `"b"]` давало
	 *          ДВА значения, да ещё и в обратном порядке. Блочное построение того же кодека
	 *          держит их содержимым всюду, и расходиться поточному с ним нельзя
	 */
	const bool plained = this->_flowning;
	/**
	 ...
	 */
	if(!plained && (leading == '?') && (((offset + 1) >= line.size()) || spacing(line[offset + 1])))
		/**
...
bool awh::codec::yaml::Reader::flowing(const string_view line, size_t & offset) noexcept {
	...
	while(true){
		 ...
		 *
		 * @note Знак примечания обрывает собранное значение наравне с разделителем: описание
		 *       правилом `ns-plain-multi-line` строке продолжения примечания не отводит, и
		 *       примечание значение оканчивает. Прежде оно значение НЕ обрывало, и запись
		 *       `[a`, `# c`, `b]` склеивалась в одно значение `a b` - примечание пропадало,
		 *       а два куска, им разделённые, выдавались одним. То же примечание строкою
		 *       первой - запись `[a # c`, `b]` - отвергалось по праву, и выдача разнилась
		 *       по одному лишь месту примечания. Блочное построение обрывает значение
		 *       примечанием всюду, и расходиться поточному с ним нельзя
		 *
		...
		if(this->_flowning && ((letter == ',') || (letter == ']') || (letter == '}') ||
		   (letter == '[') || (letter == '{') ||
		   ((letter == '#') && ((offset == 0) || spacing(line[offset - 1]))) ||
		...
		if(this->_phase == flow_t::ENTRY){
			 ...
			 */
			if(!this->_flowning && (letter == '?') && (((offset + 1) >= line.size()) || spacing(line[offset + 1]) ||
			   (line[offset + 1] == ',') || (line[offset + 1] == ']') || (line[offset + 1] == '}'))){
				/**
			 ...
			 */
			if(!this->_flowning && ((letter == '&') || (letter == '!'))){
				/**
				 * Если разобрать свойства узла не удалось
				 */
				if(!this->property(line, offset))
		...
		if(letter == ':'){
			...
			if(sequence && !bracket.paired){
				 ...
				 *
				 * @warning Отказ этот ДОСТИЖИМ и берётся. Замерено родным покрытием clang:
				 *          ВОРОШИТЕЛЬ за 60 000 проходов берёт его 172 раза, тогда как набор
				 *          кодека и общий набор - ни разу
				 *
				 * @warning Прежде тут стояло «отказ НЕДОСТИЖИМ, замерено подменою кода отказа
				 *          особым: ни одно из шести написаний пары внутри перечня сюда не
				 *          дошло». Довод ложен, и ложь его родом из замера МАЛЫМ ЧИСЛОМ подач:
				 *          шесть написаний, вручную составленных, недостижимости не
				 *          доказывают. Ворошитель дошёл сюда с первого же прогона
				 *
				 * @note Довод «оба стерегут одно и то же место одного накопителя» роднил этот
				 *       заход со стражем `keyed`. Родство доводом более не является, и страж
				 *       тот перемерен ОСОБО: тело его даёт ноль и у набора кодека, и у
				 *       ворошителя, и у общего набора - метка при нём верна. Один и тот же
				 *       довод у двух стражей оправдал одного и не оправдал другого
				 *
				 * @note Сносу заход не подлежит: он есть застава последнего рубежа перед `at` да
				 *       `insert` по месту, накопителю не принадлежащему - а то есть порча памяти
				 */
 ...
 *
 * @warning Перебор строк исходных стоит здесь ЗАСТАВОЙ ПОСЛЕДНЕГО РУБЕЖА и ныне НЕДОСТИЖИМ:
 *          розыск простого значения через строку не переходит вовсе, и склеенная логическая
 *          строка приходит сюда одною строкою исходной. Замерено трижды - счётчиком,
 *          поставленным на приход строки с переводом внутри: за 361 проверкой набора, за
 *          120 000 текстами ворошителя и заново 09.09.2026 за 406 проверками набора, где
 *          свёртка бралась 3945 раз, а строка с переводом внутри - НИ РАЗУ. Ветви свёртки,
 *          вторую и далее строку разбирающие, порчею не роняются ничем
 *
 * @note Ветвь счёта доказана способной сработать: подмена искомого знака перевода на `e`
 *       дала 239 срабатываний из тех же 3945. Без такой подмены ноль неотличим от щупа,
 *       поставленного мимо места
 *
 * @note Перебор оставлен намеренно: писан он по замеренному пороку (`l\n- "\n-` клало
 *       перевод строки внутрь значения сырым), и недостижим лишь оттого, что склейка была
 *       после того обужена местом кавычки. Ослабнет обужение - свёртка окажется на месте
 *
...
bool awh::codec::yaml::Reader::record(const string_view line) noexcept {
	 ...
	 *
	 * @warning Отступ директиве описанием НЕ ДОЗВОЛЕН: правило `l-directive` ставит знак
	 *          `%` в начало строки, и отступа у него нет вовсе. Принимая директиву с
	 *          отступом, чтение расходилось с удержанием исходного текста, где начало
	 *          документа берётся вместе с директивами его И ТОЛЬКО с начала строки:
	 *          написание `...`, подача, `%YAML 1.2`, `---` читалось потоком о двух
	 *          документах, а перезапись правленого дерева ставила содержимое МЕЖДУ
	 *          директивой и чертою начала - и отвергалась своим же чтением. Нашла это
	 *          кампания ворошителя
	 *
	 * @note Строка с отступом уходит разбору содержимого, а простому значению знак `%`
	 *       описанием в начале заказан: негодное написание отвергается им же
	 */
	...
	if(this->_anchor.empty() && this->_tag.empty())
		...
		this->_awaited = false;
	/**
	 * Если свойства узла своего не дождались, а строка стоит отступом имени пары либо мельче
	 *
	 * @details Оговорка эта правилу выше необходима: ожидание держится ради узла, что
	 *          придёт СТРОКОЮ ГЛУБЖЕ, а строка отступом имени пары самой пару эту
	 *          закрывает - узла свойствам там уже не будет, и значением пары остаётся
	 *          пустота, свойства эти несущая. Обращая ожидание обратно ожиданием
	 *          значения, разбор отдаёт пустоту готовой выдачей ниже
	 *
	 * @warning Без оговорки свойства переживали пару свою и прилипали к имени пары
	 *          следующей: написание `k:`, `  !!str`, `j: v` читалось парою `k: !!str j`,
	 *          а значение `v` пропадало молча. В перечне же то же самое прикрывалось
	 *          отказом правила GT5M, и снятие того отказа порок обнажило
	 */
	else if(this->_awaited && (indent <= this->_pending)){
		// Выполняем сброс признака подачи значения пары, объявленной прежде
		this->_awaited = false;
		// Запоминаем признак ожидания значения пары, объявленной прежде
		this->_expected = true;
	}
	/**
 ...
 *
	 *
	 * @note ДОГОВОР ПОДАЧИ ПОСЛЕ КОНЦА ныне ЕДИН, и замерен он заново 09.09.2026 по
	 *       шести кодекам рамки разом - две подачи с признаком конца подряд:
	 *
	 *         INI 37, TOML 38, YAML 51, JSON 30, XML 41, CSV 17
	 *
	 *       Все шесть отвечают отказом при коде НЕнулевом и при названной причине;
	 *       имена кодов свои у каждого, но смысл один - подача продолжена после
	 *       объявленного её конца
	 *
	 * @warning Здесь стояло описание расхождения, замеренного 07.09.2026: YAML отвечал
	 *          `TRAILING_CHARACTERS` - кодом чужого смысла, TOML без вычерпывания событий
	 *          отдавал УСПЕХ, а INI отказывал при коде НУЛЕВОМ, то есть немо. Расхождение
	 *          это устранено, а описание его осталось стоять во ВСЕХ ТРЁХ кодеках сразу и
	 *          лгало трижды
	 *
	 * @warning Довод тот утверждал вдобавок, будто кода `TEXT_ALREADY_ENDED` у YAML, TOML
	 *          и INI в перечне нет вовсе и заведение его меняет договор наружу. Ныне код
	 *          этот есть у всех трёх и им же они и отвечают: запись пережила ровно ту
	 *          правку, невозможность которой утверждала
	 *
	 * @note Отсюда правило: запись, объявляющая нечто НЕВОЗМОЖНЫМ, обязана быть снесена
	 *       той самой правкой, какая это возможным делает. Иначе она обращается доводом
	 *       против работы уже сделанной
 */
bool awh::codec::yaml::Reader::feed(const void * buffer, const size_t size, const bool end) noexcept {
	 ...
	 *
	 * @details Код здесь стоял `TRAILING_CHARACTERS`, и он называл не ту причину: знаков
	 * за окончанием документа в тексте не было вовсе - подачу продолжил САМ потребитель,
	 * уже объявив её конец. Сообщение «знаки за окончанием документа» вело его искать
	 * лишнее в тексте, тогда как искать надлежало лишний вызов у себя
	 *
	 * @note Кодеки INI и TOML на ту же подачу отвечали иначе - первый отказом без кода,
	 *       второй успехом. Три кодека расходились в трёх местах на одном случае, и ныне
	 *       все три отвечают `TEXT_ALREADY_ENDED`
	 */
	if(this->_final)
		// Выводим отказ подачи после объявленного конца текста
		return this->fail(error_t::TEXT_ALREADY_ENDED, 0);
	...
	while(this->_offset < this->_buffer.size()){
		...
		if(position == string::npos){
			 ...
			 *
			 * @warning Снятие это НЕДОСТИЖИМО: возврат каретки, за которым перевода строки
			 *          не стоит, сам концом строки является - розыск конца берёт его, и
			 *          строкою последней, конца не имеющей, остаётся лишь та, что возврата
			 *          за собою не несёт. Замерено счётчиком, сюда поставленным: заход в
			 *          строку последнюю сработал 106 раз набором проверок и 67 701 раз
			 *          ворошителем, а снятие - ни разу
			 *
			 * @note Сносу снятие не подлежит: близнец его у строк обыкновенных работу
			 *       несёт (порча роняет 8 проверок), и расхождение двух этих мест выдало
			 *       бы возврат каретки внутрь значения у одной лишь последней строки
			 */
		...
		if(!this->_blocking && !continued){
			 ...
			 */
			while(unclosed(string_view(this->_buffer.data() + this->_offset, (ending - this->_offset)), this->_flow.size())){
				// Признак отложенного решения о возврате каретки последнем
				bool postponed = false;
				// Разыскиваем знак конца следующей строки накопителя
				size_t next = terminator(this->_buffer, (ending + 1), sealed, postponed);
				/**
				...
				if(postponed || (next == string::npos)){
					 ...
					 */
					/**
					 * Если за концом найденным стоит строка, переводом не закрытая
					 *
					 * @details Строка последняя перевода за собою не имеет, и розыск конца
					 *          её отвечает отсутствием - тем же, чем отвечает отсутствие
					 *          строк вовсе. Склейка обрывалась на ней, и ограда значения
					 *          оставалась незакрытой: текст `[[],"` со строкою `"]` под
					 *          ним без хвостового перевода отвергался, а с переводом
					 *          принимался. Концом такой строки берётся конец накопителя
					 *
					 * @note Нашёл это ворошитель сличением исходов разбора по хвостовому
					 *       переводу строки
					 */
					if((ending + 1) < this->_buffer.size())
						// Запоминаем концом присоединяемой строки конец накопителя
						next = this->_buffer.size();
					/**
					 * Иначе выходим из присоединения строк
					 */
					else break;
				}
				/**
	...
	if(end){
		...
		this->commit();
		/**
		 * @warning Состояние окончания ставится ПРЕЖДЕ вычерпывания очереди событий, тогда
		 *          как у прочих кодеков рамки оно ставится после. Потребитель, спросивший
		 *          состояние между подачею последнего куска и вычерпыванием, получит здесь
		 *          «разобрано до конца» при непустой очереди
		 *
		 * @note Вопрос ВЫНЕСЕН ВЛАДЕЛЬЦУ 07.09.2026 и решения не имеет. Запись стоит здесь
		 *       затем, чтобы следующий разбор не открывал расхождение заново
		 *
		 * @warning Разрешись вопрос в пользу общего порядка - перенос постановки надлежит
		 *          вести ВМЕСТЕ со снятием этой записи, а не после неё
		 */
...
bool awh::codec::yaml::Reader::next() noexcept {
	...
	if(this->_reading >= this->_events.size()){
		...
		if(this->_final){
			/**
			 * Запоминаем состояние окончания разбора текста
			 *
			 * @details Прежде состояние это ставилось при ПОДАЧЕ, тогда как у прочих кодеков
			 * рамки оно ставится по вычерпывании очереди. Потребитель, спросивший состояние
			 * между подачею последнего куска и вычерпыванием, получал «разобрано до конца»
			 * при непустой очереди. Событий при том не терялось - очередь вычерпывалась
			 * по-прежнему, - но ответ был ложен по существу
			 *
			 * @note Заслон подачи после объявленного конца перенесён вместе с этим на признак
			 *       `_final`: останься он на состоянии - подача после конца снова стала бы
			 *       приниматься, ибо состояние ныне наступает позже
			 */
