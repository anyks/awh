...
void awh::codec::yaml::Reader::clear() noexcept {
	...
	// Выполняем сброс признака опознания кодировки исходного текста
	this->_detected = false;
	// Выполняем сброс признака корня документа, разбором открытого
	this->_rooted = false;
	// Выполняем сброс признака черты записи перечня в разбираемой строке
	this->_dashed = false;
	// Выполняем сброс признака знака горизонтальной подачи в отступе
	this->_tabbed = false;
	// Выполняем сброс признака свойств узла, в разбираемой строке объявленных
	this->_propped = false;
	// Выполняем сброс признака метки узла, в разбираемой строке объявленной
	this->_anchoredHere = false;
	// Выполняем сброс признака метки типа, в разбираемой строке объявленной
	...
	this->_shallow = 0;
	// Выполняем сброс отступа, продолжению значения требуемого
	this->_required = 0;
	// Выполняем сброс отступа построения, значение объемлющего
	this->_outer = 0;
	// Выполняем сброс отступа, разбором открытого
	this->_opening = 0;
	// Выполняем сброс отступа, записью перечня занятого
	this->_margin = 0;
	// Выполняем сброс отступа, пометкою отмеченного
	this->_marked = NO_INDENT;
	// Выполняем сброс отступа имени, вопросом объявленного
	this->_questioned = 0;
	// Выполняем сброс количества уровней, вопросом открытых
	this->_asking = 0;
...
bool awh::codec::yaml::Reader::plaining(const string_view line, bool & attached) noexcept {
	 ...
	 * внутри простого значения оградою не является вовсе - кавычка значаща лишь на месте
	 * значения, - и присоединение склеенного дословно клало перевод строки внутрь
	 * значения сырым. Оттого написание `l\n- "\n-` без перевода хвостового давало
	 * значение `l - " -`, а с переводом - значение `l - "\n-`: хвостовой перевод менял
	 * содержимое значения, стоящего много выше него
	 *
	 * @note Свёртка ведётся тем же правилом, каким свёрнуты строки, склейке не
	 *       подвергшиеся: перевод один обращается пробелом, а каждый следующий остаётся
	 *       переводом. Строка исходная при этом обвязку свою теряет - отступ продолжения
	 *       содержимым не является
	 */
	{
		// Присоединяемое содержимое логической строки
		const string_view joined = line.substr(content, (length - content));
		...
		while(origin <= joined.size()){
			 ...
			 *
			 * @note Строка пустая переводом и остаётся: свёртка обращает пробелом лишь
			 *       перевод одиночный
			 */
			else if(piece.empty())
				// Выполняем присоединение перевода строки к собираемому значению
