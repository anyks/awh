...
namespace {
	/**
	 * @brief Функция чтения дробного числа, к местности нечувствительная
	 *
	 * @details Чтение ведётся приведением `awh::lexical_t::fromChars`, разряда `LC_NUMERIC`
	 * не знающим вовсе: запись читается по правилам местности «C» при любой действующей
	 * местности. Тем же приведением читают дробное кодеки INI и TOML
	 *
	 * @note Прежде здесь стоял `strtod` с ПОДМЕНОЮ десятичного знака: точка в записи
	 *       заменялась знаком действующей местности, ибо `strtod` местности ПОДЧИНЁН.
	 *       Подпорка та неверна в основе, и доказано это раскладкою 09.09.2026: у
	 *       OpenIndiana под местностью «ar_SA.UTF-8» десятичным знаком служит «٫» (U+066B),
	 *       подмена ставила его в запись, а `strtod` там его НЕ принимает - запись `0.1`
	 *       читалась НУЛЁМ, и значение терялось молча
	 *
	 * @warning Подпорка держалась на допущении, что `strtod` всюду следует за местностью
	 *          ОДИНАКОВО. Допущение это ложно: у OpenBSD разряда `LC_NUMERIC` нет вовсе, у
	 *          illumos он есть, а приведение за ним не идёт. Замер на рабочей машине и на
	 *          Debian того показать не мог - там подмена и приведение согласны между собою
	 *
	 * @note Довод прежней записи гласил, что `strtod` основания записи не знает вовсе и
	 *       читает всякую запись десятичную; довод этот сохраняется полностью - основание
	 *       выбирается зовущим выше, и сюда приходит запись уже десятичная
	 *
	 * @param text читаемая запись дробного числа
	 * @return     прочитанное дробное число
	 *
	 */
	/**
	 * @note Связки строковые зовутся через `std::`, а НЕ через `::`: заголовок `<cstring>`
	 *       обязан выложить имена в пространство `std`, а выкладывать ли их наравне в
	 *       глобальное - оставлено на усмотрение реализации. У libc++ и у MinGW они там
	 *       оказываются, и расхождение молчит; у libstdc++ с GCC - нет, и сборка встаёт
	 *       отказом «`::strlen` has not been declared». Донесено сессией соседа по замеру
	 *       на Debian 12 (g++ 12.2) и Solaris 11.4 09.09.2026
	 *
	 * @warning Разряд этот на рабочей машине НЕВИДИМ вовсе: там сборка идёт clang с libc++,
	 *          где глобальные имена налицо. Ловится он лишь стендом с GCC
	 */
	static double decimal(const char * text) noexcept {
		// Прочитанное дробное число
		double result = 0.;
		// Получаем длину читаемой записи дробного числа
		const size_t length = ((text != nullptr) ? std::strlen(text) : 0);
		/**
		 * Если читать нечего
		 */
		if(length == 0)
			// Выводим нулевое дробное число
			return 0.;
		// Выполняем чтение дробного числа записью, местности не знающей
		const awh::lexical_t::result_t <char> outcome = awh::lexical_t::fromChars(text, (text + length), result);
		/**
		 * Если число за предел дробного вида вышло
		 *
		 * @details Приведение отвечает на выход за предел кодом `OVERFLOW_RANGE`, тогда как
		 *          `strtod` отвечал на него разрядом `ERANGE`, - а зовущие судят по
		 *          `errno`, ибо запись `1e400` обязана выйти видом `EXTENDED`, а не
		 *          бесконечностью. Разряд ставится здесь, чтобы договор помощника остался
		 *          прежним и зовущих править не пришлось
		 *
		 * @note Само значение при том выдаётся как есть - бесконечностью при переполнении и
		 *       нулём при потере значимости, - ровно как выдавал его `strtod`. Замерено
		 *       щупом на семи краевых записях: `1e400` даёт бесконечность, `1e-400` нуль, а
		 *       поднормальное `1e-320` читается годным
		 */
		if(outcome.error == awh::lexical::error_t::OVERFLOW_RANGE){
			// Запоминаем разряд выхода за предел вида
			errno = ERANGE;
			// Выводим прочитанное дробное число
			return result;
		}
		/**
		 * Если прочитать дробное число не удалось
		 *
		 * @note Отказ здесь отвечает нулём наравне со `strtod`, каким чтение велось прежде:
		 *       зовущие судят о годности записи СОБСТВЕННЫМ разбором до вызова, и второй
		 *       свод правил здесь развёл бы толкования
		 */
	...
	static bool based(const string_view text, const uint8_t radix, const bool score) noexcept {
		 ...
		 * Если запись пуста
		 *
		 * @warning Заход этот есть застава последнего рубежа, ничем не задетая: все
		 *          четыре зовущих отсекают запись короткую проверкою длины прежде
		 *          вызова - «0x» без цифр до сюда не доходит. Сносу заход не подлежит:
		 *          правка проверки длины у любого из них пропустила бы сюда запись
		 *          пустую, и разбор её обратился бы числом нулевым вместо строки
		 *
		 * @note Рассуждение это проверено ПО СУЩЕСТВУ чтением, а не одним замером: трое
		 *       зовущих требуют `number.size() > 2` и подают `number.substr(2)`, четвёртый
		 *       требует `size() > 1` и подаёт `substr(1)`; во всех четырёх длина поданного
		 *       не меньше единицы. Число зовущих сверено грепом - их ровно четыре
		 *
		 * @warning Замер и рассуждение - РАЗНЫЕ работы: замер говорит, срабатывает ли страж,
		 *          а не верно ли рассуждение при нём. Ложное рассуждение и неразличимое
		 *          снятие уживаются прекрасно, и проверять надлежит оба
		 *
	...
	static bool floating(const string_view text, const bool score) noexcept {
		 ...
		 * Если запись пуста
		 *
		 * @warning Заход этот ДОСТИЖИМ и берётся. Доказано печатью, в заход поставленной:
		 *          написание `1:` при схеме LEGACY доходит сюда с записью ПУСТОЙ. Прежде
		 *          здесь стояло «застава последнего рубежа, ничем не задетая: зовущий
		 *          отсекает запись пустую проверкою длины прежде вызова», и довод тот
		 *          ложен дважды - зовущих ТРОЕ, а не один, и
		 *          первый из них подаёт часть пустую при написании, двоеточием оканчивающемся
		 *
		 * @note Зовущие названы ИМЕНАМИ, а не строками, и пересчитываются розыском `floating(`
		 *       по этому файлу: `sexagesimal()` при разборе части, двоеточиями отделённой
		 *       (он-то и подаёт пустую), да `resolve()` дважды - разбором записи числа и
		 *       разрешением вида. Прежде здесь стояли числа строк - 557, 926 и 1060, - и все
		 *       три ИЗНОСИЛИСЬ за один день, съехав на девять строк от правок ВЫШЕ них, тогда
		 *       как ни один вызов не двинулся. Координата годна лишь приметою дня: место
		 *       надлежит называть именем работы, розыском берущимся. Мерка принесена
		 *       Геннадием от кодека ABC. Само же число «один зовущий» было ПРОМАХОМ С РОЖДЕНИЯ,
		 *       а не износом, и это замерено: `git show <снимок>:src/codec/yaml/common.cpp |
		 *       grep -c "floating("` по трём снимкам подряд даёт ЧЕТЫРЕ - определение да трое
		 *       зовущих, - то есть трое их были всегда. Мерка принесена
		 *       Геннадием от кодека ABC, где указание РАССТОЯНИЕМ («десятью строками ниже»)
		 *       разошлось в тринадцать раз - расстояние копит всякую правку между двумя
		 *       точками, а номер строки лишь правки выше себя
		 *
		 * @note Отчего замер молчал: снятие захода НЕРАЗЛИЧИМО, ибо перебор по записи пустой
		 *       не выполняется вовсе, счёт цифр остаётся нулём, и поверка `count == 0` ниже
		 *       выдаёт тот же отказ. То есть страж берётся, а снятие его незаметно - вещи
		 *       разные, и первая доказывается печатью, вторая сличением
		 *
		 * @warning Метка эта пала от ЧТЕНИЯ, а не от опыта: три захода порчи числили её
		 *          неразличимой и были правы. Замер отвечает, срабатывает ли страж наружу;
		 *          верно ли рассуждение при нём - вопрос иной, и опытом он не берётся
		 *
		 * @note Сносу заход не подлежит: без него разбор записи пустой обратился бы нулём
		 *       вместо строки, случись поверка `count == 0` ниже ослабленной
		 */
		if(text.empty())
			// Выводим признак несоответствия записи
			return false;
	...
	/**
	 * @brief Функция дробного приближения записи числа заданным основанием
	 *
	 * @details Число, за предел целого вышедшее, хранится записью своей, а наружу
	 *          выдаётся приближением дробным. Собирается оно здесь по разрядам, а не
	 *          передаётся `strtod`: тот основания записи не знает вовсе и читает всякую
	 *          запись десятичной. Запись `0xFFFFFFFFFFFFFFFFFF` он читал нулём - разрядов
	 *          шестнадцатеричных без указателя `0x` он не признаёт, - а восьмеричную
	 *          запись наречия 1.1 читал десятичной, завышая число на два порядка
	 *
	 * @note Нашло это чтение кода с нуля
	 *
	 * @param text   запись числа без знака и без разделителей разрядов
	 * @param offset смещение начала разрядов записи
	 * @param radix  основание системы счисления записи
	 * @return       дробное приближение записанного числа
	 *
	 */
	static double approximate(const string & text, const size_t offset, const uint8_t radix) noexcept {
		// Собираемое дробное приближение числа
		double result = 0.;
		/**
		 * Выполняем перебор всех разрядов записи числа
		 */
		for(size_t i = offset; i < text.size(); i++){
			// Получаем очередной разряд записи числа
			 ...
			 *
			 * @note Разряды сверены разрешением вида ещё до разбора, и сюда знак иной не
			 *       доходит: сличение это стоит порукою устройству, а не проверкою входа
			 *
			 * @warning Заход этот НЕДОСТИЖИМ, покуда держится условие, названное выше:
			 *          замерено покрытием - заход не задет ни разу. Сносу он не подлежит:
			 *          правка разрешения вида пустила бы сюда знак иной, и разбор молча
			 *          принял бы его нулевым разрядом
			 *
			 * @note Довод проверен ПО СУЩЕСТВУ чтением 07.09.2026, и цепь его такова: зовущий
			 *       ОДИН (сверено грепом по `approximate(`), подаёт он запись `plain`, а та
			 *       собрана перебором с отсевом знака подчёркивания - «запись числа без знаков
			 *       подчёркивания», - и разряды её прежде сверены с основанием связкою `based`.
			 *       Подчёркивание есть единственный знак, какой `based` пропускает сверх
			 *       разрядов, и до сюда он не доходит: сними отсев - и заход стал бы достижим
			 */
			else break;
			// Выполняем накопление числа очередным разрядом
			result = ((result * static_cast <double> (radix)) + static_cast <double> (value));
		}
		// Выводим дробное приближение записанного числа
		return result;
	}
	/**
...
type_t awh::codec::yaml::resolve(const string_view text, const schema_t schema) noexcept {
	 ...
	 *
	 * @note Проверка эта стоит ПРЕЖДЕ схемы защитной намеренно: схема та объявляет
	 *       строкою всякое СКАЛЯРНОЕ значение, а пара без значения скаляром не является
	 *       вовсе. Разбор так и поступает - пара `ключ:` выдаётся им пустой при всякой
	 *       схеме, - а правка дерева поступала иначе и обращала пустое значение в пустую
	 *       строку. Нашёл расхождение это ворошитель круговым переносом
	 */
	if(text.empty())
	...
	/**
	 * Признак того, что запись открывается знаком числа
	 *
	 * @details Наречие 1.2 дозволяет знак ОДНОЙ ЛИШЬ записи десятичной: описание даёт
	 *          целому лексику `[-+]?[0-9]+ | 0o[0-7]+ | 0x[0-9a-fA-F]+`, и знак стоит
	 *          в ней только у первой из трёх. Наречие же 1.1 дозволяет знак всякой
	 *          записи, шестнадцатеричной да двоичной в том числе
	 *
	...
	// Получаем запись числа без знака
	const string_view number = (signed_ ? text.substr(1) : text);
	/**
...
type_t awh::codec::yaml::narrow(const string_view text, const schema_t schema, numeric_t & result) noexcept {
	...
	if((radix == 10) && (schema == schema_t::LEGACY) && (plain.find(':') != string::npos)){
		...
		while(position <= plain.size()){
			...
			if(part.find('.') != string::npos)
				...
				fractional = true;
			// Выполняем накопление числа очередным разрядом
			collected = ((collected * 60.) + ::decimal(part.c_str()));
			/**
	...
	if((radix == 10) && (plain.find_first_of(".eE") != string::npos)){
		...
		result.real = ::decimal(plain.c_str());
		/**
		 * Если запись несёт знак минус
		 */
		if(negative)
	...
	if(errno == ERANGE){
		 ...
		 * Запоминаем разобранное число дробным приближением его
		 *
		 * @note Приближение десятичной записи снимается разбором дробного числа, а записи
		 *       иного основания - сборкою по разрядам: разбор оснований не знает
		 */
		result.real = ((radix == 10) ? ::decimal(plain.c_str() + offset) :
		 approximate(plain, offset, radix));
		/**
		 * Если запись несёт знак минус
		 */
		if(negative)
...
const char * awh::codec::yaml::name(const encoding_t encoding) noexcept {
	...
	switch(static_cast <uint8_t> (encoding)){
		...
		case static_cast <uint8_t> (encoding_t::CP1252):
			...
			return "WINDOWS-1252";
		/**
		 * Если кодировка не определена вовсе
		 *
		 * @note Член NONE есть член перечня наравне с прочими, и имя ему положено своё:
		 *       «unknown» остаётся значению, членом перечня НЕ ЯВЛЯЮЩЕМУСЯ. Иначе имя
		 *       кодировки неопределённой неотличимо от имени мусора, и потребитель,
		 *       имя печатающий, о разнице не узнает
		 */
		case static_cast <uint8_t> (encoding_t::NONE):
			// Выводим название неопределённой кодировки
			return "none";
	}
	// Выводим название значения, членом перечня не являющегося
	return "unknown";
}
