...
style_t awh::codec::yaml::quoting(const string_view text, const schema_t schema, const bool key) noexcept {
	...
	for(size_t i = 0; i < text.size(); i++){
		...
		switch(letter){
			/**
			 * Если знак является управляющим либо переводом строки
			 *
			 * @note Управляющий знак записать без отмены нельзя вовсе, и оттого значение,
			 *       его несущее, получает двойную ограду
			 */
			case '\0':
			case '\a':
			case '\b':
