...
namespace {
	...
	static bool floating(const string_view text, const bool score) noexcept {
		 ...
		 * Если запись пуста
		 *
		 * @warning Заход этот есть застава последнего рубежа, ничем не задетая: зовущий
		 *          отсекает запись пустую проверкою длины прежде вызова. Сносу заход не
		 *          подлежит: правка той проверки пропустила бы сюда пустоту, и разбор её
		 *          обратился бы нулём вместо строки
		 */
		if(text.empty())
			// Выводим признак несоответствия записи
			return false;
