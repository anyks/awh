...
namespace {
	 ...
	 *
	 * @details Приведение `strtod` подчинено разряду `LC_NUMERIC`: под местностью с
	 * запятою вместо точки оно останавливается НА ТОЧКЕ и отдаёт то, что успело прочесть.
	 * Замерено 08.09.2026: запись «0.25» под местностью «de_DE.UTF-8» читалась НУЛЁМ, и
	 * значение терялось молча, без всякого признака отказа
	 *
	 * @note Местность здесь обходится подменою знака, а не заменою самого приведения:
	 *       довод при разборе стоит на том, что `strtod` основания записи не знает вовсе
	 *       и читает всякую запись десятичную, - и довод этот сохраняется полностью
	 *
	 * @note Десятичный знак берётся ЦЕЛИКОМ, а не первым байтом: у местностей «fa_IR» и
	 *       «ar_SA» знаком служит «٫» (U+066B), в UTF-8 двухбайтовый
	 *
	 * @note Кодеки INI и TOML к местности глухи ПО УСТРОЙСТВУ: разбор дробного у них свой,
	 *       и описан он как совпадающий со `strtod` в местности «C». Кодек YAML зовёт
	 *       `strtod` сам, и глухота его держится вот этой подменою
	 *
	...
	static double decimal(const char * text) noexcept {
		// Получаем десятичный знак действующей местности
		const char * point = ::localeconv()->decimal_point;
		/**
		 * Если десятичным знаком местности является точка
		 */
		if((point == nullptr) || (point[0] == '\0') || (::strcmp(point, ".") == 0))
			// Выводим прочитанное дробное число
			return ::strtod(text, nullptr);
		// Собираемая запись числа со знаком действующей местности
		string record(text);
		// Получаем положение точки в записи числа
		const size_t pos = record.find('.');
		/**
		 * Если точка в записи числа найдена
		 */
		if(pos != string::npos)
			// Выполняем замену точки десятичным знаком действующей местности
			record.replace(pos, 1, point);
		// Выводим прочитанное дробное число
		return ::strtod(record.c_str(), nullptr);
	}
	/**
