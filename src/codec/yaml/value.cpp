...
string awh::codec::yaml::Value::dump() const noexcept {
	// Выводим записанный текст YAML настройками записи обычными
...
bool awh::codec::yaml::Value::operator == (const Value & value) const noexcept {
	...
	if((this->_kind == kind_t::MAPPING) || (this->_kind == kind_t::SEQUENCE)){
		...
		for(size_t i = 0; i < this->_items.size(); i++){
			/**
			 * Если значения вместилищ расходятся
			 */
...
size_t awh::codec::yaml::Builder::deposit(Value && value) noexcept {
	...
	if(owner.kind() == kind_t::MAPPING){
		...
		const string name(this->_keyed ? this->_key : std::to_string(owner.size()));
 ...
 *
 */
void awh::codec::yaml::Builder::clear() noexcept {
 ...
 *
 */
awh::codec::yaml::Value awh::codec::yaml::Builder::take() noexcept {
 ...
 *
 */
awh::codec::yaml::Builder::Builder() noexcept : _keyed(false), _done(false) {}
