 ...
 *
 * @note Имя пары, косую черту несущее, переносу не подлежит и отвечает отказом: путь
 *       делится ею, и имя такое указывало бы на узел чужой. Молчаливый пропуск оставил
 *       бы потребителя с деревом без части его значения
 *
