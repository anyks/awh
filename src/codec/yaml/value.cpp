...
awh::codec::yaml::Value::Value(const double value) noexcept : Value() {
	 ...
	 */
		...
		if((length > 0) && (static_cast <size_t> (length) < sizeof(buffer))){
			...
			/**
			 * Выполняем замену десятичного знака местности точкою
			 *
			 * @details Сборка `snprintf` подчинена разряду `LC_NUMERIC`: под местностью
			 * с запятою число выходило с нею, запись видела его негодным, брала в кавычки
			 * и приписывала «.0», - и значение ЧИСЛОВОЕ обращалось в СТРОКУ. Замерено
			 * 08.09.2026 на близнеце этого места
			 *
			 * @note Десятичный знак берётся ЦЕЛИКОМ, а не первым байтом: у местностей
			 *       «fa_IR» и «ar_SA» знаком служит «٫» (U+066B), в UTF-8 двухбайтовый
			 *
			 * @note Мест сборки дробного у кодека ТРИ, и названы они ИМЕНАМИ работ:
			 *       заведение владеющего значения (здесь), установка значения дерева
			 *       (`Document::set` от двойной точности) и запись числа текстом
			 *       (`Writer::value` от двойной точности). Пересчитываются розыском
			 *       `%.17g` по каталогу `src/codec/yaml/` С ОТБОРОМ строк, несущих
			 *       `snprintf`: голый розыск даёт ШЕСТЬ строк, ибо доводы при местах
			 *       поминают вид записи дословно. Глухота нужна ВСЕМ трём:
			 *       двух из них хватило бы, чтобы значение потерялось молча
			 */
			if(const char * point = ::localeconv()->decimal_point){
				/**
				 * Если десятичным знаком местности точка не является
				 */
				if((point[0] != '\0') && (::strcmp(point, ".") != 0)){
					// Получаем положение десятичного знака местности в записи числа
					const size_t pos = this->_text.find(point);
					/**
					 * Если десятичный знак местности в записи числа найден
					 */
					if(pos != string::npos)
						// Выполняем замену десятичного знака местности точкою
						this->_text.replace(pos, ::strlen(point), 1, '.');
				}
			}
		}
		/**
