...
void awh::codec::yaml::Document::spread() noexcept {
	...
	for(size_t i = 1; i < this->_starts.size(); i++){
		...
		while(offset > 0){
			 ...
			 */
			while((letter < (offset - 1)) && ((this->_source.at(letter) == ' ') || (this->_source.at(letter) == '\t')))
				// Выполняем переход к следующему знаку строки
				letter++;
			/**
	...
	for(size_t i = 0; i < this->_roots.size(); i++){
		...
		while(closing > 0){
			 ...
			 */
			while((letter < (closing - 1)) && ((this->_source.at(letter) == ' ') || (this->_source.at(letter) == '\t')))
				// Выполняем переход к следующему знаку строки
				letter++;
			/**
			 * Если строка пуста либо несёт одно примечание
			 */
			if((letter >= (closing - 1)) || (this->_source.at(letter) == '#')){
