 ...
 *
 * @note Обрезаются одни пробелы: отступ описание языка складывает из них, и строка
 *       подачи содержимым блока не становится вовсе
 *
 * @note Нашёл это ворошитель сличением чтения правленого дерева с чтением перезаписи его
 *
...
static bool __awh_trimmed__(const string_view preamble, string & result) noexcept {
	...
	for(size_t i = 0; i < preamble.size();){
		...
		size_t letter = i;
		/**
		 * Выполняем пропуск пробелов очередной строки предисловия
		 */
		while((letter < finish) && (preamble.at(letter) == ' '))
			// Выполняем переход к следующему знаку строки
			letter++;
		/**
		 * Если строка из одних пробелов составлена
		 */
		if(letter >= finish){
...
void awh::codec::yaml::Document::spread() noexcept {
	...
	for(uint32_t i = 0; i < static_cast <uint32_t> (this->_nodes.size()); i++){
		...
		for(uint32_t child = (i + 1); child < bound; child += this->_nodes.at(child).extent()){
			 ...
			 */
			if((former != NO_ORIGIN) && this->hanging(former))
...
void awh::codec::yaml::Document::compose(writer_t & writer, const uint32_t index) const noexcept {
	...
	stack.push_back(frame_t{index, (index + 1), NO_ORIGIN});
	/**
	...
	while(!stack.empty()){
		...
		if(!this->_source.empty() && !this->_nodes.at(child).touched){
			...
			if(this->verbatim(writer, child, last,
			   (kind(this->_nodes.at(stack.back().index).type) == kind_t::SEQUENCE), gapped)){
		...
		this->preamble(writer, child, gapped);
		...
		if(this->enter(writer, child))
			...
			stack.push_back(frame_t{child, (child + 1), NO_ORIGIN});
	}
}
/**
