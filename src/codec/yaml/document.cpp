...
uint32_t awh::codec::yaml::Document::closing(const uint32_t index, const uint32_t boundary) const noexcept {
	...
	size_t position = this->_source.find('\n', latest);
	/**
	 * Если запись узла в строке своей умещается целиком
	 *
	 * @details Добор строк меряет глубину отступом строки начала, а в записи сжатой -
	 *          `- - значение` - узел стоит в колонке за чертами, тогда как отступ строки
	 *          его равен нулю. Строки соседей выходили тогда глубже начала своего, и
	 *          добор забирал весь текст ниже: граница ребёнка уезжала в конец документа,
	 *          перенос черты не состоялся, и правка выдавала лишнюю пустую запись перечня.
	 *          Записи, в строке своей умещающейся, добор не нужен вовсе
	 *
	 * @note Значение блочное да значение, пустою строкой разорванное, под правило это не
	 *       подпадают: длина записи их строки своей длиннее, и добор идёт своим чередом.
	 *       Нашёл это ворошитель сличением чтения правленого дерева с чтением перезаписи
	 */
	if((this->_nodes.at(index).extent() == 1) && (position != string::npos) &&
	   (this->_nodes.at(index).style != style_t::LITERAL) && (this->_nodes.at(index).style != style_t::FOLDED) &&
	   (static_cast <size_t> (this->_nodes.at(index).length()) <= (position - latest)) &&
	   (terminus <= (position + 1)))
		// Выводим конец строки, записью узла занятой
