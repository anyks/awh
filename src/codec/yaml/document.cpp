...
bool awh::codec::yaml::Document::deduplicate(const uint32_t parent, unordered_map <string, uint32_t> & anchors, const reader_t & reader) noexcept {
	...
	for(size_t i = removed.size(); i > 0; i--){
		...
		{
			/**
			 * Начало собственной строки снесённой пары в удержанном тексте
			 *
			 * @note Берётся начало строки, а не начало записи: пустые строки и примечания,
			 *       над парою стоявшие, писаны к ней и уходят вместе с нею
			 */
			const uint32_t started = this->leading(child);
			/**
			 * Выполняем перебор всех узлов, снесённой паре предшествующих
			 */
			for(uint32_t i = (parent + 1); i < child; i++){
				/**
				 * Если поддерево очередного узла в снесённую пару не упирается
				 *
				 * @note Упирающихся вообще-то несколько: сосед её, последний узел поддерева
				 *       соседа и всё, что между ними, - граница записи у каждого кладётся
				 *       началом записи следующего
				 */
				if((i + this->_nodes.at(i).extent()) != child)
					// Выполняем переход к следующему узлу
					continue;
				/**
				 * Если начало снесённой пары в удержанном тексте известно
				 *
				 * @note Граница подрезается, а не узел помечается: пометка отправила бы
				 *       узел сборке заново, и поточное построение вернулось бы блочным -
				 *       перезапись меняла бы вид записи там, где потребитель её не трогал
				 */
				if((started != NO_ORIGIN) && (this->_nodes.at(i).edge != NO_ORIGIN) &&
				   (this->_nodes.at(i).edge > started))
					...
					this->_nodes.at(i).edge = started;
				/**
				 * Если начало снесённой пары в удержанном тексте неизвестно
				 */
