...
void awh::codec::yaml::Document::spread() noexcept {
	...
	for(uint32_t i = 0; i < static_cast <uint32_t> (this->_nodes.size()); i++){
		...
		const uint32_t own = this->leading(i);
		...
		for(uint32_t child = (i + 1); child < bound; child += this->_nodes.at(child).extent()){
			...
			// Признак того, что промежуток до строки черты занят одними пробельными знаками
			bool solely = true;
			/**
			 * Выполняем перебор всех знаков промежутка до строки черты
			 *
			 ...
			 */
			for(size_t j = ((boundary < this->_prologue) ? this->_prologue : boundary); j < start; j++){
				/**
				 * Если знак пробельным не является
				 */
				if(!::blanking(this->_source.at(j)) && (this->_source.at(j) != '\n')){
					// Запоминаем занятость промежутка знаками иными
					solely = false;
					// Выходим из перебора знаков промежутка
					break;
				}
			}
	...
	switch(static_cast <uint8_t> (kind(node.type))){
		...
		case static_cast <uint8_t> (kind_t::STRING): {
			...
			if((node.style == style_t::LITERAL) || (node.style == style_t::FOLDED)){
				 ...
				 */
				if(writer.block(text, node.style, ((!text.empty() && (text.back() == '\n')) ? chomp_t::KEEP : chomp_t::STRIP)))
					// Выходим из сборки поддерева узла
					break;
			}
			/**
...
uint32_t awh::codec::yaml::Document::closing(const uint32_t index, const uint32_t boundary) const noexcept {
	...
	for(uint32_t i = index; (i < (index + this->_nodes.at(index).extent())) && (i < this->_nodes.size()); i++){
		...
		/**
		 * Если граница записи узла известна и начала записей превосходит
		 *
		 * @details Начала записей одних недостаточно: запись блочного значения начинается
		 *          строкою `- |`, а тянется на строки ниже, и конец её по началу не узнать.
		 *          Граница же записи стоит за последним знаком её
		 *
		 * @note Без того промежуток между блочным значением и чертою следующей записи
		 *       числился занятым содержимым блока, перенос черты не состоялся, и перезапись
		 *       правки выдавала лишнюю пустую запись перечня. Нашёл это ворошитель правкой
		 *       дерева
		 */
		if((this->_nodes.at(i).edge != NO_ORIGIN) && (this->_nodes.at(i).edge > 0) &&
		   ((this->_nodes.at(i).edge - 1) > latest))
			// Запоминаем границу записи узла наибольшею
			latest = (this->_nodes.at(i).edge - 1);
	}
	/**
	...
	while((letter < this->_source.size()) && ::blanking(this->_source.at(letter)))
		// Выполняем переход к следующему знаку строки
		letter++;
	// Получаем отступ строки, наибольшим началом записи занятой
	...
	while(position != string::npos){
		...
		while((next < this->_source.size()) && ::blanking(this->_source.at(next)))
			...
			next++;
		/**
		 * Если строка пуста либо глубже строки начала не стоит
		 */
		if((next >= this->_source.size()) || (this->_source.at(next) == '\n') || ((next - following) <= indent))
			// Выходим из добора строк записи
			break;
