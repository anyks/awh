...
bool awh::codec::yaml::Document::digest(reader_t & reader) noexcept {
	...
	const auto attach = [&](const type_t type) noexcept -> uint32_t {
		...
		if((node.origin != NO_ORIGIN) && !reader.value().flow){
			// Получаем начало собственной строки события, узел заводящего
			const uint32_t own = opening();
			/**
			 * Если собственная строка события известна
			 */
			 ...
			 */
			if((own != NO_ORIGIN) && (own < node.origin))
