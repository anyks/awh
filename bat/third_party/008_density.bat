@echo off
rem ============================================================================
rem  Density для оснастки Microsoft Visual Studio
rem
...
rem  в общей части: из версии 0.14.2 выходит метка density-0.14.2
rem ----------------------------------------------------------------------------
set "VPREFIX=density-"
set "VSUFFIX="
set "VDEFAULT=0.14.2"
...
if /i "%~1"=="--clean" (
	...
	if exist "%PREFIX%\lib\density.lib" del /f /q "%PREFIX%\lib\density.lib" > nul 2>&1
...
for /r "%SRC%\src" %%C in (*.c) do (
	cl.exe /nologo /c /O2 /DNDEBUG /W0 /I "%SRC%\src" /Fo:"%WORK%\!COUNT!_%%~nC.obj" "%%~fC" > nul || (
...
lib.exe /nologo /out:"%PREFIX%\lib\density.lib" %OBJECTS% || (
...
xcopy /s /y /q "%SRC%\src\*.h" "%PREFIX%\include\%NAME%\" > nul
