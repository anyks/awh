@echo off
rem ============================================================================
rem  LZ4 для оснастки Microsoft Visual Studio
rem
...
rem  в общей части: из версии 1.10.0 выходит метка v1.10.0
rem ----------------------------------------------------------------------------
set "VPREFIX=v"
set "VSUFFIX="
set "VDEFAULT=1.10.0"
...
if /i "%~1"=="--clean" (
	...
	if exist "%PREFIX%\lib\lz4.lib" del /f /q "%PREFIX%\lib\lz4.lib" > nul 2>&1
	exit /b 0
)
...
call "%~dp0_common.bat" :build "%NAME%" "%VERSION%" "-DBUILD_STATIC_LIBS=ON" "build\cmake"
if errorlevel 1 exit /b 1

rem ----------------------------------------------------------------------------
rem  Раскладка заголовков
rem
rem  AWH ищет заголовки зависимости в своём каталоге (include\lz4), а установка
...
rem ----------------------------------------------------------------------------
if not exist "%PREFIX%\include\lz4" mkdir "%PREFIX%\include\lz4" > nul 2>&1
for %%H in ("%PREFIX%\include\*.h" "%PREFIX%\include\*.hpp") do (
	if exist "%%~fH" move /y "%%~fH" "%PREFIX%\include\lz4\" > nul 2>&1
)
