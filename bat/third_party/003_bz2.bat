@echo off
rem ============================================================================
rem  BZ2 для оснастки Microsoft Visual Studio
rem
...
rem  в общей части: из версии 1.0.8 выходит метка bzip2-1.0.8
rem ----------------------------------------------------------------------------
set "VPREFIX=bzip2-"
set "VSUFFIX="
set "VDEFAULT=1.0.8"
...
if /i "%~1"=="--clean" (
	...
	if exist "%PREFIX%\lib\bz2.lib" del /f /q "%PREFIX%\lib\bz2.lib" > nul 2>&1
	exit /b 0
)
...
call "%~dp0_common.bat" :build "%NAME%" "%VERSION%" "-DENABLE_SHARED_LIB=OFF -DENABLE_STATIC_LIB=ON -DENABLE_EXAMPLES=OFF -DENABLE_DOCS=OFF -DENABLE_TESTS=OFF"
if errorlevel 1 exit /b 1

rem ----------------------------------------------------------------------------
rem  Раскладка заголовков
rem
rem  AWH ищет заголовки зависимости в своём каталоге (include\bz2), а установка
...
rem ----------------------------------------------------------------------------
if not exist "%PREFIX%\include\bz2" mkdir "%PREFIX%\include\bz2" > nul 2>&1
for %%H in ("%PREFIX%\include\*.h" "%PREFIX%\include\*.hpp") do (
	if exist "%%~fH" move /y "%%~fH" "%PREFIX%\include\bz2\" > nul 2>&1
)
