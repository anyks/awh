@echo off
rem ============================================================================
rem  ZSTD для оснастки Microsoft Visual Studio
rem
...
rem  в общей части: из версии 1.5.7 выходит метка v1.5.7
rem ----------------------------------------------------------------------------
set "VPREFIX=v"
set "VSUFFIX="
set "VDEFAULT=1.5.7"
...
if /i "%~1"=="--clean" (
	...
	if exist "%PREFIX%\lib\zstd.lib" del /f /q "%PREFIX%\lib\zstd.lib" > nul 2>&1
	exit /b 0
)
...
call "%~dp0_common.bat" :build "%NAME%" "%VERSION%" "-DZSTD_BUILD_TESTS=OFF -DZSTD_BUILD_STATIC=ON -DZSTD_BUILD_SHARED=OFF -DZSTD_BUILD_PROGRAMS=OFF" "build\cmake"
if errorlevel 1 exit /b 1

rem ----------------------------------------------------------------------------
rem  Раскладка заголовков
rem
rem  AWH ищет заголовки зависимости в своём каталоге (include\zstd), а установка
...
rem ----------------------------------------------------------------------------
if not exist "%PREFIX%\include\zstd" mkdir "%PREFIX%\include\zstd" > nul 2>&1
for %%H in ("%PREFIX%\include\*.h" "%PREFIX%\include\*.hpp") do (
	if exist "%%~fH" move /y "%%~fH" "%PREFIX%\include\zstd\" > nul 2>&1
)
