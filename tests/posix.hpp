 ...
 */
	...
	/**
	 * @brief Функция отключения отчёта о недоступности получателя на дейтаграммном гнезде
	 *
	 * @param fd гнездо, которому отключается отчёт
	 * @return   ноль всегда
	 *
	 * @details У систем POSIX отключать нечего: отказ ICMP там на приём не влияет.
	 *          Средство заведено ради единообразия вызова, см. ветвь MS Windows
	 *
	 */
	inline int32_t disableConnectionReset([[maybe_unused]] const int32_t fd) noexcept {
		// Выводим успешный результат: отключать нечего
		return 0;
	}
	/**
 ...
 */
	 ...
	 *
	 * @details Разбор ведётся подлинным разборщиком системы RtlIpv4StringToAddressA
	 *          из ntdll: правила приёма у него те же, что у inet_aton в POSIX, и
	 *          проверено это щупом на стенде Windows ARM64 - "9" даёт 0.0.0.9,
	 *          "127.1" даёт 127.0.0.1, восьмеричное "0177.0.0.1" даёт 127.0.0.1,
	 *          а "256.1.1.1" отвергается
	 *
	 * @details Объявление заводится своё, а сама функция берётся из библиотеки в миг
	 *          работы: заголовка ip2string.h у MinGW нет вовсе, хотя функция в ntdll
	 *          присутствует
	 *
	 * @warning Прежде здесь стоял inet_pton, и замена та была СТРОЖЕ подменяемого:
	 *          она принимает лишь запись из четырёх десятичных полей. Набор
	 *          NetSystemDifferentialAcceptanceTest сличает разборщик модуля с
	 *          системным на записях СЛУЧАЙНЫХ, среди которых есть и краткие, - и
	 *          падал на записи "9" всякий раз, обвиняя модуль в том, что он принял
	 *          законную запись. Строгий эталон обвинял верный разбор
	 *
	 * @warning Если функцию раздобыть не удалось, разбор ведётся прежним inet_pton:
	 *          набор тогда сличает на записях полных, о чём и предупреждает журнал
	 *
	 */
	inline int32_t inet_aton(const char * value, struct in_addr * result) noexcept {
		// Вид подлинного разборщика записи адреса IPv4
		typedef long (__stdcall * parser_t)(const char *, unsigned char, const char **, struct in_addr *);
		// Подлинный разборщик записи адреса IPv4, добываемый единожды
		static parser_t parser = reinterpret_cast <parser_t> (reinterpret_cast <void *> (
			::GetProcAddress(::LoadLibraryW(L"ntdll.dll"), "RtlIpv4StringToAddressA")
		));
		// Если подлинный разборщик раздобыть не удалось
		if(parser == nullptr)
			// Выполняем разбор записи адреса IPv4 строгим разборщиком
			return ((::inet_pton(AF_INET, value, result) == 1) ? 1 : 0);
		// Место, на каком разбор записи остановился
		const char * end = nullptr;
		// Выполняем разбор записи адреса IPv4 подлинным разборщиком
		const long code = parser(value, 0, &end, result);
		// Выводим успех, если разбор прошёл и запись прочитана до конца
		return (((code == 0) && (end != nullptr) && (* end == 0)) ? 1 : 0);
	}
	/**
	 * @brief Функция установки предела ожидания приёма на гнезде
	 *
	 ...
	 * @return   ноль при успехе, иначе -1
	 *
	 * @details Приняв отказ ICMP о недоступности получателя, MS Windows отдаёт его
	 *          **отправителю в его же приём** ошибкой WSAECONNRESET, а очередная
	 *          дейтаграмма при этом теряется. Поведение это ошибочно для всякого
	 *          дейтаграммного обмена, и движок отключает его при заведении сокета -
	 *          смотрите src/net/backend/win/socket.cpp. Гнёзда, заводимые проверками
	 *          напрямую, обязаны делать то же самое
	 *
	 * @note Проверено опытом на стенде Windows ARM64 (17.08.2026): поддельный
	 *       маршрутизатор терял повторную просьбу модуля, потому что тот заводит на
	 *       каждую попытку новое гнездо, а ответ на прежнее рождал отказ ICMP
	 *
	 */
	inline int32_t disableConnectionReset(const int32_t fd) noexcept {
		...
		return ((::WSAIoctl(static_cast <SOCKET> (fd), SIO_UDP_CONNRESET, &report, sizeof(report), nullptr, 0, &bytes, nullptr, nullptr) == 0) ? 0 : -1);
	}
	/**
...
/**
 * @brief Функция ожидания готовности гнезда к приёму
 *
 * @param fd      гнездо, готовности которого ожидаем
 * @param timeout предел ожидания в миллисекундах
 * @return        истина, если гнездо готово к приёму, ложь по истечении предела
 *
 * @details Средство это обязательно для всякого поддельного приёмника, ведущего приём
 *          опросом со сроком: у MS Windows дейтаграмма, пришедшая ровно в миг снятия
 *          блокирующего `recvfrom` по сроку `SO_RCVTIMEO`, ТЕРЯЕТСЯ. Стек засчитывает
 *          её отданной получателю (счётчик `dwInDatagrams` растёт), но приём возвращает
 *          10060, а в очереди гнезда её уже нет, и никакой счётчик потери не отмечает -
 *          ни `dwNoPorts`, ни `dwInErrors`, ни отброшенное на уровне IP.
 *
 *          Свойство доказано опытом в три плеча по 4000 посылок на петле при сроке
 *          в 20 мс: при подгонке прихода к самому истечению срока теряется от 1 до 25
 *          дейтаграмм на 4000, при приходе в середину промежутка - ни одной, а при том
 *          же приходе к истечению, но с ожиданием через `select` - тоже ни одной.
 *          Число снятий по сроку у плеч подгонки совпадает (4017 против 4018), то есть
 *          разнятся плечи одним лишь способом ожидания.
 *
 *          Отсюда правило: срок отсчитывается ДО обращения к приёму, а сам приём
 *          зовётся лишь по готовности - тогда снятие по сроку очереди гнезда не
 *          касается вовсе. Настройку `SO_RCVTIMEO` при этом снимать не нужно: она
 *          остаётся запасным пределом на случай ложной готовности
 *
 */
inline bool waitReadable(const int32_t fd, const uint32_t timeout) noexcept {
	// Если гнездо не заведено, ожидать нечего
	if(fd < 0) return false;
	// Набор гнёзд, готовности которых ожидаем
	fd_set set;
	...
	#else
		// У систем POSIX набор ожидания принимает обычный дескриптор
		FD_SET(fd, &set);
	#endif
	// Предел ожидания готовности
	struct timeval value;
	// Устанавливаем целые секунды предела ожидания
	value.tv_sec = static_cast <decltype(value.tv_sec)> (timeout / 1000);
	// Устанавливаем доли секунды предела ожидания
	...
	#else
		// У систем POSIX первым доводом идёт число, большее наибольшего дескриптора
