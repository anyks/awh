 ...
 *
 * @note Регрессионный тест: порт > 65535 не должен ни молча усекаться
 *       (99999 mod 65536 == 34463), ни молча сниматься с адреса - запись эта
 *       негодна целиком
 *
 */
TEST_F(UriFixture, ParsePortRangeOverflowTest){
	...
	ASSERT_EQ(awh::uri_t::type_t::NONE, this->_uri->parse("http://example.com:99999/"));
	// Проверяем, что негодная запись адреса собой не заняла
 ...
 *       номер за пределы диапазона портов. Негодное представление отменяет разбор
 *       записи целиком: порт входит в опознание ресурса, и снятый молча он уводил
 *       бы обращение на стандартный порт схемы - то есть на другой узел
 *
 */
TEST_F(UriFixture, ParsePortMalformedTest){
	...
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Номер порта за пределами диапазона отменяет разбор записи
	ASSERT_EQ(awh::uri_t::type_t::NONE, this->_uri->parse("http://example.com:65536/"));
	// Проверяем, что негодная запись адреса собой не заняла
	ASSERT_TRUE(this->_uri->empty());
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	...
	ASSERT_EQ(awh::uri_t::type_t::NONE, this->_uri->parse("http://example.com:+80/"));
	// Проверяем, что негодная запись адреса собой не заняла
	ASSERT_TRUE(this->_uri->empty());
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Нечисловое представление номера порта негодно
	ASSERT_EQ(awh::uri_t::type_t::NONE, this->_uri->parse("http://example.com:80a/"));
	// Проверяем, что негодная запись адреса собой не заняла
	ASSERT_TRUE(this->_uri->empty());
}
...
TEST_F(UriFixture, PrintKeepsObjectUnchangedTest){
	 ...
	 *          намеренно - чем наполнить цикл, решается отдельно; правка «по
	 *          догадке» сменила бы то, что проверка утверждает
	 */
...
TEST_F(UriFixture, QueryKeepsRepeatedKeysTest){
	 ...
	 *
	 * Сличается набор значений, а не их последовательность. Повторяющийся ключ
	 * несёт набор равноправных параметров, а не упорядоченный перечень: потребитель
	 * собирает их по ключу сам, и очерёдность ему безразлична. Хранилище параметров
	 * порядок совпадающих по ключу пар не обещает вовсе, и у разных реализаций
	 * стандартной библиотеки он свой - сличение последовательности проверяло бы
	 * реализацию, а не наш разбор
	 *
	 */
	// Ожидаемый набор значений повторяющегося ключа
	const std::multiset <std::string> expected = {"one", "two", "three"};
	// Полученный набор значений повторяющегося ключа
	...
	// Все значения повторяющегося ключа обязаны сохраниться
	ASSERT_EQ(expected, values);
	/**
	 * Строка собирается выстроенной по ключу и значению: вид её от реализации
	 * стандартной библиотеки не зависит, а набор параметров сохраняется полностью
	 */
...
/**
 * @brief Тест согласия объекта, собранного установщиками, со своей же строкой
 *
 * @details Установщики заводят состояние в обход разбора, и объект, собранный
 *          ими, обязан давать строку, разбор которой отдаёт его же. Прежде
 *          согласия не выходило по трём причинам: установщик пути точечные
 *          сегменты хранил, а разбор их снимал; наличие авторити выводилось из
 *          одного лишь вида записи, и путь у объекта с учётной записью от корня
 *          не вёлся, а печать его от корня выписывала; снятие схемы оставляло
 *          разновидность незнакомой схемы вместо неопределённой
 *
 */
TEST_F(UriFixture, BuiltObjectAgreesWithItsRecordTest){
	...
	this->_uri->parse("http://h/a/b/c");
	// Устанавливаем путь с точечными сегментами
	this->_uri->path(std::vector <std::string> {"..", "x"});
	// Точечные сегменты обязаны сняться
	ASSERT_EQ("http://h/x", this->_uri->print(awh::uri_t::item_t::URI));
	// Выполняем разбор собранной строки
	again.parse(this->_uri->print(awh::uri_t::item_t::URI));
	// Объект обязан совпасть со своей же строкой
	ASSERT_TRUE(again == (* this->_uri));
	/**
	 * Учётная запись заводит авторити наравне с хостом, и путь при ней ведётся от
	 * корня: печать его от корня выписывает, и объект обязан это знать
	 */
	// Выполняем очистку объектов работы с URI
	this->_uri->clear();
	again.clear();
	// Собираем адрес установщиками
	this->_uri->scheme("mailto");
	this->_uri->user("u", "p");
	this->_uri->path(std::vector <std::string> {"a"});
	// Выполняем разбор собранной строки
	again.parse(this->_uri->print(awh::uri_t::item_t::URI));
	// Объект обязан совпасть со своей же строкой
	ASSERT_TRUE(again == (* this->_uri)) << "строка: " << this->_uri->print(awh::uri_t::item_t::URI);
	/**
	 * Вид записи задаётся по схеме лишь тогда, когда задан не был: авторити,
	 * заведённая учётной записью либо атрибутами, его не задаёт
	 */
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Собираем адрес установщиками в порядке "схема, хост, учётная запись"
	this->_uri->scheme("https");
	this->_uri->host("example.com");
	this->_uri->user("u", "p");
	// Две косые черты обязаны сохраниться
	ASSERT_EQ("https://u:p@example.com", this->_uri->print(awh::uri_t::item_t::URI));
	/**
	 * Записи без схемы разновидность не положена: снятие схемы обязано её
	 * сбрасывать
	 */
	// Выполняем очистку объектов работы с URI
	this->_uri->clear();
	...
	this->_uri->parse("https://u:p@example.com:8443/x?y#z");
	// Снимаем схему адреса
	this->_uri->scheme("");
	// Разновидность обязана стать неопределённой
...
/**
 * @brief Тест равнозначности записей, различающихся подразумеваемым
 *
 * @details Записи, различающиеся лишь тем, что и без того подразумевается,
 *          равнозначны (RFC 3986 6.2.3): пустой путь равнозначен пути "/", а
 *          порт, принятый для схемы, - его отсутствию. Печать при этом хранит
 *          написание исходной записи, и строки таких записей различаются.
 *
 *          Оттого признак ведения пути от корня различает записи, которые
 *          сличение считает равными, - в ключ сличения его брать нельзя.
 *          Решения эти описаны в DECISIONS.md
 *
 */
TEST_F(UriFixture, ImpliedPartsDoNotDivideTest){
	...
	const std::vector <std::pair <std::string, std::string>> samples = {
		...
		{"https://a.com/x", "https://a.com:443/x"},
		{"//host:8080#f", "//host:8080/#f"}
	};
	/**
	...
	for(auto & sample : samples){
		...
		// Записи, различающиеся лишь подразумеваемым, обязаны быть равны
	...
	/**
	 * Печать написание исходной записи хранит: черта у пустого пути ставится,
	 * лишь когда за ней идут параметры или якорь, - иначе разделитель терялся бы
	 */
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор записи без пути
	this->_uri->parse("http://a.com");
	// Черта у записи без пути не ставится
	ASSERT_EQ("http://a.com", this->_uri->print(awh::uri_t::item_t::URI));
	// Путь от корня у записи без пути не ведётся
	ASSERT_FALSE(this->_uri->rooted());
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор записи с якорем
	this->_uri->parse("http://a.com#f");
	// Черта перед якорем ставится и при пустом пути
	ASSERT_EQ("http://a.com/#f", this->_uri->print(awh::uri_t::item_t::URI));
}
...
/**
 * @brief Тест угадывания односегментного хоста с портом
 *
 * @details Приметой хоста служит не одна лишь точка в имени: записи
 *          "localhost:8080" и "myhost:80/a" точки не несут, а хостом с портом
 *          являются - за двоеточием у них стоят одни цифры, а не путь. Прежде
 *          такая запись читалась как схема "localhost" с непрозрачным путём
 *          "8080", и обращение шло в никуда
 *
 */
TEST_F(UriFixture, SingleLabelHostWithPortTest){
	// Записи односегментного хоста с портом
	const std::vector <std::pair <std::string, uint32_t>> samples = {
		{"localhost:8080", 8080},
		{"example:8080", 8080},
		{"host-1:1234", 1234}
	};
	...
	for(auto & sample : samples){
		...
		this->_uri->parse(sample.first);
		// Имя обязано стать хостом, а не схемой
		ASSERT_EQ(sample.first.substr(0, sample.first.find(':')), this->_uri->host()) << "запись: " << sample.first;
		// Схемы у записи быть не должно
		ASSERT_TRUE(this->_uri->scheme().empty()) << "запись: " << sample.first;
		// Порт обязан разобраться
		ASSERT_EQ(sample.second, this->_uri->port()) << "запись: " << sample.first;
	}
	/**
	 * Схема, разбору знакомая, этому правилу не поддаётся: запись "mailto:123" -
	 * это схема с непрозрачным путём, а не хост с портом.
	 *
	 * Незнакомой схеме защитой служит состав хвоста: у настоящего адреса за
	 * двоеточием стоит не одно лишь число - "urn:isbn:0451450523" несёт второе
	 * двоеточие, - а число, за пределы разрядности порта вышедшее, приметой его
	 * не служит вовсе
	 */
	// Записи знакомых схем с числовым путём
	const std::vector <std::string> schemes = {
		"mailto:123", "redis:6379", "file:80",
		"urn:isbn:0451450523", "tel:79101234567", "custom:99999"
	};
	/**
	 * Перебираем все записи знакомых схем
	 */
	for(auto & sample : schemes){
		// Выполняем очистку объекта работы с URI
		this->_uri->clear();
		// Выполняем разбор записи
		this->_uri->parse(sample);
		// Схема обязана остаться схемой
		ASSERT_EQ(sample.substr(0, sample.find(':')), this->_uri->scheme()) << "запись: " << sample;
	}
	/**
	 * Запись без двоеточия хостом не становится: отличить её от относительного
	 * пути нечем, и "docs/index.html" ушло бы в хост "docs"
	 */
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор записи относительного пути
	this->_uri->parse("localhost/api");
	// Хоста у записи без двоеточия быть не должно
	ASSERT_TRUE(this->_uri->host().empty());
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор записи относительного пути
	this->_uri->parse("docs/index.html");
	// Хоста у записи относительного пути быть не должно
	ASSERT_TRUE(this->_uri->host().empty());
	/**
	 * Знак, цифрой не являющийся, правило отменяет: за двоеточием у хоста стоит
	 * номер порта, а всё прочее - непрозрачный путь схемы
	 */
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор записи со схемой и непрозрачным путём
	this->_uri->parse("custom:80a");
	// Имя обязано остаться схемой
	ASSERT_EQ("custom", this->_uri->scheme());
	// Хоста у такой записи быть не должно
	ASSERT_TRUE(this->_uri->host().empty());
}

/**
 * @brief Тест признака согласия с записью при разрешении
 *
 * @details Отказ от негодной записи по разновидности адреса не виден: у
 *          заполненного объекта она остаётся прежней, то есть настоящей.
 *          Переход по перенаправлению следует делать методом, отдающим сам
 *          признак, - иначе отвергнутая ссылка выглядела бы применённой, а
 *          работа пошла бы по прежнему адресу
 *
 */
TEST_F(UriFixture, ResolveReportsRefusalTest){
	// Выполняем разбор основы
	ASSERT_TRUE(this->_uri->resolve("https://example.com/a"));
	// Ссылка с негодным представлением порта обязана быть отвергнута
	ASSERT_FALSE(this->_uri->resolve("//other.com:99999/b"));
	// Прежний адрес обязан остаться нетронутым
	ASSERT_EQ("https://example.com/a", this->_uri->print(awh::uri_t::item_t::URI));
	// Годная ссылка обязана быть принята
	ASSERT_TRUE(this->_uri->resolve("//other.com:8080/b"));
	// Адрес обязан смениться
	ASSERT_EQ("https://other.com:8080/b", this->_uri->print(awh::uri_t::item_t::URI));
	/**
	 * Разбор разновидности отдаёт тот же итог: работу ведёт разрешение записи, а
	 * разбор лишь отдаёт разновидность поверх него
	 */
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор основы
	this->_uri->parse("https://example.com/a");
	// Разновидность отказом не меняется
	ASSERT_EQ(awh::uri_t::type_t::HTTPS, this->_uri->parse("//other.com:99999/b"));
	// Прежний адрес обязан остаться нетронутым
	ASSERT_EQ("https://example.com/a", this->_uri->print(awh::uri_t::item_t::URI));
}

/**
 * @brief Тест отмены разбора ссылки негодным представлением порта
 *
 * @details Негодное представление порта отменяет разбор целиком, а не снимается
 *          с адреса молча: запись "http://host:99999/" при отброшенном порте
 *          разбиралась бы как "http://host/", то есть уводила бы обращение на
 *          другой узел. Отвергнутая ссылка прежний адрес оставляет нетронутым.
 *
 *          Тип, разбором выводимый, признаком удачи при этом не служит: у
 *          заполненного объекта отвергнутая ссылка даёт тип прежнего адреса,
 *          неотличимый от типа разрешённой
 *
 */
TEST_F(UriFixture, BadPortRejectsReferenceTest){
	// Ссылки с негодным представлением порта
	const std::vector <std::string> samples = {
		"//other.com:99999/b", "//other.com:65536/b", "//other.com:80a/b", "//other.com:abc/b"
	};
	/**
	...
	for(auto & sample : samples){
		...
		// Выполняем разбор основы
		this->_uri->parse("https://example.com/a");
		// Выполняем разрешение ссылки с негодным представлением порта
		this->_uri->parse(sample);
		// Прежний адрес обязан остаться нетронутым
		ASSERT_EQ("https://example.com/a", this->_uri->print(awh::uri_t::item_t::URI)) << "ссылка: " << sample;
		// Хост прежнего адреса обязан остаться нетронутым
		ASSERT_EQ("example.com", this->_uri->host()) << "ссылка: " << sample;
	}
	/**
	 * Пустое представление порта задаёт его стандартным и разбору не мешает
	 * (RFC 3986 3.2.3)
	 */
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор записи с пустым представлением порта
	this->_uri->parse("http://example.com:/a");
	// Хост обязан разобраться
	ASSERT_EQ("example.com", this->_uri->host());
	// Порт обязан оказаться стандартным для схемы
	ASSERT_EQ(80, this->_uri->port());
	/**
	 * Тип, разбором выводимый, признаком удачи не служит: ловить отказ следует
	 * сличением адреса до и после разбора
	 */
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор основы
	this->_uri->parse("https://example.com/a");
	// Адрес объекта до разбора ссылки
	const std::string before = this->_uri->print(awh::uri_t::item_t::URI);
	// Тип, выведенный разбором негодной ссылки
	const awh::uri_t::type_t rejected = this->_uri->parse("//other.com:99999/b");
	// Тип прежнего адреса отказом не меняется
	ASSERT_EQ(awh::uri_t::type_t::HTTPS, rejected);
	// Отказ обнаруживается сличением адреса до и после разбора
	ASSERT_EQ(before, this->_uri->print(awh::uri_t::item_t::URI));
	// Выполняем разрешение годной ссылки
	ASSERT_EQ(awh::uri_t::type_t::HTTPS, this->_uri->parse("//other.com:8080/b"));
	// Годная ссылка адрес меняет
	ASSERT_NE(before, this->_uri->print(awh::uri_t::item_t::URI));
}

/**
 * @brief Тест сохранности порта при пустом хосте
 *
 * @details Порт принадлежит авторити, а не хосту, и записывается он в атрибуты
 *          адреса. Пустой хост атрибутов не заводил, и установка порта уходила в
 *          пустоту: запись "http://:8080/" собиралась как "http:///", а порт с
 *          неё снимался нулевым - отбор по порту на такой записи молча
 *          оказывался ни при чём
 *
 */
TEST_F(UriFixture, PortSurvivesEmptyHostTest){
	// Записи с пустым хостом и явно заданным портом
	const std::vector <std::pair <std::string, uint32_t>> samples = {
		{"http://:8080/", 8080},
		{"//:8080/x", 8080},
		{"custom://:1234/a", 1234}
	...
	for(auto & sample : samples){
		...
		// Выполняем разбор записи
		this->_uri->parse(sample.first);
		// Хост у записи пустой
		ASSERT_TRUE(this->_uri->host().empty()) << "запись: " << sample.first;
		// Порт обязан сохраниться
		ASSERT_EQ(sample.second, this->_uri->port()) << "запись: " << sample.first;
		// Собранная строка обязана нести порт
		ASSERT_EQ(sample.first, this->_uri->print(awh::uri_t::item_t::URI)) << "запись: " << sample.first;
	}
	/**
	 * Пустая авторити без порта атрибутов по-прежнему не заводит: записи
	 * "http:///path" и "file:///etc/hosts" - это пустой хост с путём от корня
	 */
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор записи с пустой авторити без порта
	this->_uri->parse("http:///path");
	// Атрибуты у записи без порта не заводятся
	ASSERT_EQ(nullptr, this->_uri->attr());
	// Собранная строка обязана совпасть с исходной
	ASSERT_EQ("http:///path", this->_uri->print(awh::uri_t::item_t::URI));
	/**
	 * Порт переживает и разрешение относительной ссылки: авторити берётся от
	 * ссылки целиком, вместе с портом
	 */
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор основы
	this->_uri->parse("http://example.com/a");
	// Выполняем разрешение ссылки с пустым хостом и портом
	this->_uri->parse("//:8080/x");
	// Порт обязан сохраниться
	ASSERT_EQ(8080, this->_uri->port());
	// Собранная строка обязана нести порт
	ASSERT_EQ("http://:8080/x", this->_uri->print(awh::uri_t::item_t::URI));
}

/**
 * @brief Тест копирования атрибутов доменного сокета с незаведённым путём
 *
 * @details Путь к сокету заводится отдельно от самих атрибутов и у них может
 *          отсутствовать вовсе: атрибуты, заведённые снаружи, приходят с пустым
 *          указателем. Копирование по нему падало обращением по нулевому адресу
 *
 */
TEST_F(UriFixture, SocketPathMayBeAbsentTest){
	// Атрибуты доменного сокета с незаведённым путём
	awh::net::attr_uds_t attr;
	// Устанавливаем схему доменного сокета
	this->_uri->scheme("unix");
	// Установка таких атрибутов падать не должна
	ASSERT_NO_THROW(this->_uri->attr(&attr));
...
TEST_F(UriFixture, SchemeIsGuessedOnlyWithHostTest){
	...
	ASSERT_EQ(awh::uri_t::type_t::NONE, this->_uri->type());
	/**
	 * Порт при этом сохраняется: он принадлежит авторити, а не хосту, и запись
	 * его несёт. Прежде он терялся - пустой хост атрибутов не заводил, и
	 * установка порта уходила в пустоту, - а отбор по порту на такой записи
	 * молча оказывался ни при чём
	 */
	// Порт обязан сохраниться при пустом хосте
	ASSERT_EQ(443, this->_uri->port());
	// Собранная строка обязана нести пустую авторити с портом и путём от корня
	ASSERT_EQ("//:443/x", this->_uri->print(awh::uri_t::item_t::URI));
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
...
TEST_F(UriFixture, PortIsTakenOnlyFromDigitsTest){
	 ...
	 * Записи такой негоден сам порт, а с ним и вся запись
	 */
	// Выполняем очистку объекта работы с URI
	this->_uri->clear();
	// Выполняем разбор записи с негодным номером порта
	ASSERT_EQ(awh::uri_t::type_t::NONE, this->_uri->parse("http://example.com:80a/x"));
	// Проверяем, что негодная запись адреса собой не заняла
	ASSERT_TRUE(this->_uri->empty());
}
