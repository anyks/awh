 ...
 * | `sun` | свободное место (заведено 08.09.2026) | -1, средства нет |
 *
...
TEST_F(EthFixture, SocketBufferAvailableTest){
	 ...
	 *       какие у всех названных систем есть всегда, и -1 означал бы не свойство
	 *       машины, а несделанную работу наречия. Solaris и illumos добавлены сюда
	 *       08.09.2026, когда чтение у наречия `sun` было заведено: прежде оно
	 *       отвечало -1 не по отсутствию средства, а потому, что сделано не было
	 */
	#if defined(_WIN32) || defined(_WIN64) || defined(__linux__) || defined(__sun)
...
/**
 * @brief Тест занятости буфера у описателя КАНАЛА
 *
 * @par Намеренные решения
 *
 * Договор тот же, что и у сокета: ответ либо -1 - «средства у системы нет», - либо
 * величина от нуля до размера буфера. Спрашивается он здесь у канала, а не у гнезда.
 *
 * Прежняя находка по этому же обращению звучала так: «половина верного довода закрывала
 * работу целиком» - про буфер ОТПРАВКИ у MS Windows средства и вправду нет, а про буфер
 * ПРИЁМА есть, `FIONREAD`. Здесь та же развилка этажом ниже: `FIONREAD` отвечает гнезду,
 * а описателю канала - отказом, и ответ терялся молча. Между тем средство есть и у
 * канала, своё: `PeekNamedPipe` называет, сколько октет в нём лежит.
 *
 * @note Утверждается не число, а НАЗВАННОСТЬ: канал заводится пустым, и «занято нуль,
 *       свободно всё» - единственный ответ, какой ему подобает. Ответ -1 означал бы не
 *       свойство системы, а несделанную работу наречия
 *
 */
#if defined(_WIN32) || defined(_WIN64)
	TEST_F(EthFixture, SocketChannelBufferAvailableTest){
		...
		/**
		 * Проверяем, что оба описателя пары заведены
		 */
		for(uint8_t i = 0; i < 2; i++)
			// Проверяем, что описатель пары заведён
			ASSERT_NE(pair[i], awh::net::invalid_socket_t);
		/**
		 * Переходим по обоим концам пары
		 */
		for(uint8_t i = 0; i < 2; i++){
			// Получаем размер приёмного буфера канала
			const int32_t size = this->_eth->socket.getBufferSize(pair[i], awh::net::socket_event_t::READ);
			// Размер приёмного буфера канала обязан быть положительным
			ASSERT_GT(size, 0) << "конец канала " << static_cast <uint32_t> (i) << " не назвал размера приёмного буфера";
			// Получаем занятость приёмного буфера канала
			const int32_t reading = this->_eth->socket.getBufferAvailable(pair[i], awh::net::socket_event_t::READ);
			// Занятость приёмного буфера канала обязана быть названа
			ASSERT_NE(reading, -1) << "конец канала " << static_cast <uint32_t> (i) << " не назвал занятости приёмного буфера, хотя средство у канала есть";
			// Свободное место у пустого канала обязано быть положительным
			ASSERT_GT(reading, 0) << "пустой канал объявлен без свободного места в приёмном буфере";
			// Свободное место не вправе превышать сам буфер
			ASSERT_LE(reading, size) << "свободного места объявлено больше самого буфера";
		}
		/**
 ...
 *       вовсе, и опрос прав там не годится
 *
 */
...
TEST_F(EthFixture, SocketEcnDeliveryTest){
	 ...
	 * именно ради снятия этого отступления
	 *
...
TEST_F(EthFixture, SocketRawProtocolIPv6Test){
	 ...
	 *          средствами опроса НЕ НАБЛЮДАЕМ вовсе. Замерено под MS Windows
	 *          03.09.2026, оба пути отрицательны: настройки `SO_PROTOCOL` там
