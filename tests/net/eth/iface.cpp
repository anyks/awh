...
/**
 * @brief Тест переключения защиты от одновременного доступа
 *
 * @details Метод `threadSafety` включает и снимает замки у работы с туннельными
 *          устройствами. Наружу состояние замка не выставлено, и утверждать его
 *          напрямую нельзя - утверждается наблюдаемое следствие: под включённой
 *          защитой одновременные обращения к перечислению устройств из нескольких
 *          потоков обязаны отвечать ОДНИМ И ТЕМ ЖЕ непустым перечнем
 *
 * @note У систем, где своих замков модулю не нужно, тело метода намеренно пусто -
 *       проверка и там обязана проходить, ничего при этом не утверждая ложно
 *
 */
TEST_F(EthFixture, IfaceThreadSafetyTest){
	// Снимаем образец перечня устройств до включения защиты
	const auto reference = this->_eth->iface.available();
	// Перечень устройств машины пустым быть не может: петлевое есть всегда
	ASSERT_FALSE(reference.empty()) << "Перечень доступных устройств машины пуст";
	// Включаем защиту от одновременного доступа
	this->_eth->iface.threadSafety(true);
	// Замок, оберегающий сбор итогов потоков
	std::mutex mtx;
	// Собранные потоками перечни устройств
	std::vector <std::unordered_set <std::string>> results;
	// Набор потоков опроса
	std::vector <std::thread> workers;
	/**
	 * Заводим потоки, одновременно опрашивающие перечень устройств
	 */
	for(uint8_t i = 0; i < 4; i++)
		// Заводим очередной поток опроса
		workers.emplace_back([this, &mtx, &results]() noexcept {
			/**
			 * Опрашиваем перечень устройств многократно, чтобы обращения наложились
			 */
			for(uint8_t j = 0; j < 25; j++){
				// Получаем перечень доступных устройств машины
				auto listing = this->_eth->iface.available();
				// Выполняем блокировку сбора итогов
				const std::lock_guard <std::mutex> lock(mtx);
				// Сохраняем полученный перечень
				results.push_back(std::move(listing));
			}
		});
	/**
	 * Дожидаемся завершения всех потоков опроса
	 */
	for(auto & worker : workers)
		// Дожидаемся завершения очередного потока опроса
		worker.join();
	// Снимаем защиту от одновременного доступа
	this->_eth->iface.threadSafety(false);
	// Все потоки обязаны были отработать полностью
	ASSERT_EQ(results.size(), static_cast <size_t> (4 * 25));
	/**
	 * Перебираем все собранные потоками перечни устройств
	 */
	for(size_t i = 0; i < results.size(); i++){
		// Ни один перечень пустым оказаться не смеет
		ASSERT_FALSE(results[i].empty()) << "Перечень устройств пуст у обращения №" << i;
		// Каждый перечень обязан совпасть с образцом
		ASSERT_EQ(results[i], reference) << "Перечень устройств разошёлся с образцом у обращения №" << i;
	}
	// Повторное переключение защиты обязано оставаться безвредным
	this->_eth->iface.threadSafety(true);
	// Снимаем защиту повторно
	this->_eth->iface.threadSafety(false);
	// Перечень устройств обязан уцелеть и после переключений
	ASSERT_EQ(this->_eth->iface.available(), reference);
}

/**
 * @brief Тест отказа установки адреса по длине префикса, разрядности адреса не отвечающей
 *
 * @par Намеренные решения
 *
 * Проверка заведена 03.09.2026, когда ответы на одну и ту же бессмыслицу оказались
 * ТРОЯКИМИ по слоям: прокладка пути отвечала отказом, установка адреса молча приводила
 * длину к маске одного узла, а ветвь IPv6 просто не ставила маску вовсе. Ответ сведён
 * к одному - отказ с доводом
 *
 * @note Зрячей проверку делает разбор ДОВОДА, а не признак отказа. Признак для заслона
 *       негоден по устройству: без надзорных полномочий установка адреса отвечает
 *       отказом и без всякого заслона, то есть утверждала бы нехватку прав
 *
 * @warning Ноль длиной негодной НЕ считается: он значит «длина не задана», обработка
 *          его своя, и заслон его пропускать обязан. Прежде ноль и бессмыслица были
 *          слиты в одно условие - в том и был корень разнобоя
 *
 */
TEST_F(EthFixture, IfaceOversizedPrefixTest){
	 ...
	 */
	for(const uint8_t prefix : {static_cast <uint8_t> (33), static_cast <uint8_t> (200), static_cast <uint8_t> (255)}){
		// Адрес, каким ведётся проверка
		std::unique_ptr <awh::net::addr_t> ip = std::make_unique <awh::net::addr_net_ipv4_t> ();
		// Установка адреса по негодной длине обязана отвечать отказом
		ASSERT_NO_THROW(static_cast <void> (this->_eth->iface.setAddress("lo0", ip.get(), prefix)));
		// Установка адреса по негодной длине обязана отвечать отказом
		ASSERT_FALSE(this->_eth->iface.setAddress("lo0", ip.get(), prefix)) << "Движок принял длину префикса " << static_cast <uint32_t> (prefix) << " у адреса IPv4, где предел равен 32";
	}
	/**
	 * Длины, разрядности адреса IPv6 не отвечающие
	 */
	for(const uint8_t prefix : {static_cast <uint8_t> (129), static_cast <uint8_t> (255)}){
		// Адрес, каким ведётся проверка
		std::unique_ptr <awh::net::addr_t> ip = std::make_unique <awh::net::addr_net_ipv6_t> ();
		// Установка адреса по негодной длине обязана отвечать отказом
		ASSERT_FALSE(this->_eth->iface.setAddress("lo0", ip.get(), prefix)) << "Движок принял длину префикса " << static_cast <uint32_t> (prefix) << " у адреса IPv6, где предел равен 128";
	}
	...
	/**
	 * Заслон обязан срабатывать ПРЕЖДЕ обращения к устройству
	 *
	 * @details Просьба негодна дважды: устройства с таким именем нет ни в одной
	 *          системе, и длина префикса предел превышает. Проверяется здесь ПОРЯДОК:
	 *          отказ обязан прийти от заслона по длине, а не от поиска устройства
	 *
	 * @warning Признак отказа тут не различает ничего: и с заслоном, и без него, и на
	 *          несуществующем устройстве обращение отвечает ложью. Различает только
	 *          ПЕРВЫЙ довод в журнале - он отвечает на вопрос, ЧЕЙ отказ пришёл.
	 *          Найдено Андреем на наречии MS Windows, где заслон стоял после поиска
	 *          устройства и на несуществующем устройстве молчал
	 */
	{
		...
		this->_log->subscribe([&first]([[maybe_unused]] const awh::log_t::flag_t flag, const std::string_view text) noexcept -> void {
			...
			if(first.empty())
				...
				first.assign(text.begin(), text.end());
		});
		// Адрес, каким ведётся проверка
		std::unique_ptr <awh::net::addr_t> ip = std::make_unique <awh::net::addr_net_ipv4_t> ();
