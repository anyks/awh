 ...
 */
	 ...
	 *
	 * @note Заголовок sched.h нужен ради unshare() и CLONE_NEWNET: проверка сноса
	 *       маршрута канальной области ставится в отдельном пространстве имён сети,
...
/**
 * @brief Тест сноса маршрута канальной области IPv4
 *
 * @details Снос обязан находить маршрут напрямую подключённой подсети - тот, что ядро
 *          заводит само при назначении адреса устройству. Поля `rtm_scope` и `rtm_type`
 *          при `RTM_DELROUTE` ядро СЛИЧАЕТ, а не принимает описанием намерения, поэтому
 *          безусловные `RT_SCOPE_UNIVERSE` и `RTN_UNICAST` такой маршрут не находили и
 *          отвечали ESRCH. Замер в отдельном пространстве имён сети, Debian 12,
 *          ядро 6.1.0-50 (09.09.2026)
 *
 * @note Опыт целиком идёт в ОТДЕЛЬНОМ пространстве имён сети дочернего процесса, поэтому
 *       таблица маршрутов машины не затрагивается вовсе. Объект работы с Ethernet
 *       заводится ПОСЛЕ `unshare`: сокет netlink, созданный до него, остался бы привязан
 *       к прежнему пространству имён и опыт был бы недоказателен
 *
 */
#if defined(__linux__)
TEST_F(EthFixture, GatewayRemoveLinkScopeRouteIPv4){
	// Если пользователь не является привилигированным
	if(::getuid() != 0)
		// Пропускаем проверку
		GTEST_SKIP() << "Для заведения пространства имён сети нужны права суперпользователя";
	// Если средство настройки сети недоступно
	if(::system("ip -V > /dev/null 2>&1") != 0)
		// Пропускаем проверку
		GTEST_SKIP() << "средство ip недоступно: завести устройство опыта нечем";
	...
	if(pid == 0){
		// Заводим отдельное пространство имён сети
		if(::unshare(CLONE_NEWNET) != 0)
			// Выходим с признаком пропуска
			::_exit(2);
		// Заводим устройство опыта и адрес на нём: маршрут подсети ядро заведёт само
		if(::system("ip link add awhdummy type dummy > /dev/null 2>&1") != 0)
			// Выходим с признаком пропуска
			::_exit(2);
		// Поднимаем устройство опыта
		if(::system("ip link set awhdummy up > /dev/null 2>&1") != 0)
			// Выходим с признаком пропуска
			::_exit(2);
		// Назначаем адрес устройству опыта
		...
		// Инициализируем объект адреса назначения в маршруте
		route.destination = std::make_unique <awh::net::addr_net_ipv4_t> ();
		// Выполняем парсинг адреса подсети устройства опыта
		addr = "10.55.55.0";
		// Устанавливаем адрес назначения маршрута
		awh_cast <awh::net::addr_net_ipv4_t *> (route.destination.get())->address = addr.v4(awh::net_addr_t::endian_t::LITTLE);
		// Устанавливаем префикс сети маршрута
		route.prefix = 24;
		// Устанавливаем название устройства опыта
		route.ifname = "awhdummy";
		// Если снести маршрут канальной области не удалось
		if(!eth.gateway.remove(route))
		...
		search.destination = std::make_unique <awh::net::addr_net_ipv4_t> ();
		// Устанавливаем адрес назначения маршрута
		awh_cast <awh::net::addr_net_ipv4_t *> (search.destination.get())->address = addr.v4(awh::net_addr_t::endian_t::LITTLE);
		// Устанавливаем название устройства опыта
		search.ifname = "awhdummy";
		// Снесённый маршрут находиться больше не должен
		::_exit(eth.gateway.get(search) ? 1 : 0);
	}
	...
	ASSERT_TRUE(WIFEXITED(status)) << "Процесс опыта завершился по сигналу";
	// Если опыт поставить не удалось
	if(WEXITSTATUS(status) == 2)
		// Пропускаем проверку
		GTEST_SKIP() << "завести пространство имён сети либо устройство опыта не удалось";
	// Снос маршрута канальной области обязан состояться
	ASSERT_EQ(WEXITSTATUS(status), 0)
	 << "Маршрут канальной области 10.55.55.0/24 снести не удалось: ядро сличает "
	    "поля rtm_scope и rtm_type, и безусловные UNIVERSE/UNICAST его не находят";
}
#endif
