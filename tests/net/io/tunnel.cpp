...
TEST_F(IoFixture, IoTunnelLifecycleTest){
	 ...
	 * Для операционных систем Sun
	 *
	 * @warning Договор движка там неисполним ПО УСТРОЙСТВУ, и это не свойство стенда:
	 *          устройство заводится в `event()`, а имя готовой связи передаётся
	 *          `setIface()` - то есть ПОСЛЕ заведения. Систему Sun устройство завести
	 *          не может вовсе (связи там заводятся надзорно через `dladm`), и без
	 *          имени `event()` отказывает сразу: «tunnel device name is required».
	 *          Назвать связь заранее нечем - `event()` имени не принимает
	 *
	 * @note Проверки пропускаются здесь ровно до тех пор, пока договор не позволит
	 *       называть устройство до его заведения. Стенд при этом подготовлен: связи
	 *       awhtun0 и awhtun1 поверх etherstub заведены, см. README
	 */
	#if __sun__
		// Пропускаем проверку
		GTEST_SKIP() << "договор движка требует имя устройства до заведения, а передать его можно лишь после: у систем Sun связь заводится надзорно";
	#endif
	...
	for(uint8_t round = 0; round < 2; round++){
		...
		// Устройство обязано завестись: годность окружения проверена выше пробой
		ASSERT_GT(tid, 0u) << sign << ": права есть, а устройство не заведено - нет драйвера туннельных устройств либо дефект движка";
		/**
...
TEST_F(IoFixture, IoTunnelCarriesPacketTest){
	...
	const awh::event::id_t tid = this->_io->event(awh::event::node_t::TUNNEL, awh::event::family_t::IPV4);
		...
		if(!tunnelDevice().empty())
			...
			ASSERT_TRUE(this->_io->setIface(tid, tunnelDevice()));
	// Устройство обязано завестись: годность окружения проверена выше пробой
	ASSERT_GT(tid, 0u) << "права есть, а устройство не заведено - нет драйвера туннельных устройств либо дефект движка";
...
TEST_F(IoFixture, IoTunnelIPv6HousekeepingTest){
	...
	for(uint8_t round = 0; round < 2; round++){
		 ...
		 *
		 * @note Устройство заводится ЗДЕСЬ, а не фиксацией
		 */
