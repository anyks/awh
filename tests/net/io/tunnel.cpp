...
TEST_F(IoFixture, IoTunnelLifecycleTest){
	...
	for(uint8_t round = 0; round < 2; round++){
		 ...
		 */
		{
			...
			/**
			 * Объекты адресов заводятся ЗАРАНЕЕ
			 *
			 * @details Опрос адреса ждёт готовых объектов нужного вида и по ним же
			 *          узнаёт, какого семейства адрес спрашивают. Пустые указатели он
			 *          отвергает сразу, отвечая отказом, - и отказ этот легко принять
			 *          за отсутствие адреса у устройства
			 */
			// Объект адреса самого устройства
			std::unique_ptr <awh::net::addr_t> address = std::make_unique <awh::net::addr_net_ipv4_t> ();
			// Объект адреса того конца связи
			std::unique_ptr <awh::net::addr_t> peer = std::make_unique <awh::net::addr_net_ipv4_t> ();
			// Размер префикса подсети устройства
			uint8_t prefix = 0;
			// Спрашиваем у системы адрес заведённого устройства
			ASSERT_TRUE(eth.iface.getAddress(iface, address, peer, prefix))
				<< sign << ": движок согласился поставить адрес, а система устройству его не назначила";
			// Устройство обязано нести адрес
			ASSERT_NE(address, nullptr) << sign << ": устройство заведено без адреса";
			// Создаём объект обращения адреса в текст
			awh::net_addr_t text(this->_fmk.get(), this->_log.get());
			// Переносим в него адрес, полученный у системы
			text.source(address.get(), awh::net_addr_t::endian_t::LITTLE);
			// Сличаем адрес устройства с запрошенным
			ASSERT_EQ(static_cast <std::string> (text), std::string(TUNNEL_LOCAL))
				<< sign << ": на устройстве стоит не тот адрес, что был запрошен";
		}
		/**
