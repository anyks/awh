...
/**
 * Заголовочный файл краткого имени библиотеки
 *
 * @note Подключается ЯВНО: имя это приходит из заголовка, а не из одной лишь
 *       настройки сборки, - у систем Sun проверка звала его при поиске связей
 *       распорядителя машины и без заголовка не собиралась вовсе. Тот же недосмотр
 *       был прежде найден у `win/tunnel.cpp`
 */
 ...
 */
/**
 * @brief Проверка снятия адреса при свёртывании движка БЕЗ сноса узла туннеля
 *
 * @details Прочие проверки туннеля держат `TunnelGuard` и сносят узел сами, оттого
 *          прибирание у них идёт помощником `destroy(node)` - единственным путём, где
 *          снятие адреса у macOS заведено. Путь же «свернуть движок при ЖИВОМ узле»
 *          ими не проходится вовсе, а он законен: потребитель вправе просто позвать
 *          `deinitialize()` либо дать движку уйти деструктором
 *
 * @warning Довод к снятию адреса записан у самого движка и ИЗМЕРЕН: у macOS `utun`
 *          исчезает не сразу, адрес всё это время числится за мертвецом, и следующий
 *          туннель получает свой адрес занятым. Проверка ставит ровно этот опыт:
 *          заводит туннель, сворачивает движок НЕ снося узел, и заводит туннель с ТЕМ
 *          ЖЕ адресом заново. Отказ второго захода означает, что адрес не снят
 *
 */
TEST_F(IoFixture, IoTunnelDeinitializeReleasesAddressTest){
	// Если надзорных прав у процесса нет
	if(!tunnelPrivileged())
		// Пропускаем проверку
		GTEST_SKIP() << "заведение туннельного устройства требует надзорных прав";
	...
	if(!tunnelReady(1))
		...
		GTEST_SKIP() << "связи под туннели не заведены распорядителем машины";
	/**
	 * Проходим два захода: первый оставляет узел живым, второй берёт тот же адрес
	 */
	for(uint8_t round = 0; round < 2; round++){
		// Роспись захода для сообщений об отказе
		const std::string sign = std::string("заход=") + std::to_string(round + 1);
		// Закрепляем драйвер туннельных устройств
		tunnelPinDriver(this->_io.get(), tunnel_driver_t::TAP);
		// Выполняем инициализацию движка
		ASSERT_TRUE(this->_io->initialize()) << sign;
		// Заводим узел туннеля
		const awh::event::id_t tid = this->_io->event(awh::event::node_t::TUNNEL, awh::event::family_t::IPV4);
		// Узел обязан завестись
		ASSERT_GT(tid, 0u) << sign << ": движок не записал узел туннеля";
		// Устанавливаем настройки узла
		ASSERT_TRUE(this->_io->setOptions(tid, awh::event::options::NO_IO_BLOCK | awh::event::options::CLOSE_ON_EXEC)) << sign;
		// Устанавливаем адреса концов туннеля
		ASSERT_TRUE(this->_io->setAddress(tid, awh::event::address_t::IPV4, TUNNEL_LOCAL)) << sign;
		ASSERT_TRUE(this->_io->setTarget(tid, TUNNEL_PEER)) << sign;
		/**
		 * Фиксируем настройки: здесь заводится устройство и ставится адрес
		 *
		 * @note Отказ ВТОРОГО захода и есть искомый признак: адрес остался числиться
		 *       за устройством прежнего захода, и система отдать его отказывается
		 */
		ASSERT_TRUE(this->_io->commit(tid))
			<< sign << ": устройство с адресом " << TUNNEL_LOCAL << " не заведено"
			<< ((round > 0) ? " - адрес не снят свёртыванием прошлого захода" : "");
		// Запускаем событие туннеля
		ASSERT_TRUE(this->_io->launch(tid)) << sign;
		/**
		 * Сличаем, за каким устройством система числит поставленный адрес
		 *
		 * @warning Утверждение это ставит саму проверку под сомнение раньше, чем
		 *          движок: пройди она за две миллисекунды и без единого устройства,
		 *          зелёный её цвет не значил бы ничего
		 */
		{
			...
			// Объект запрошенного адреса устройства
			std::unique_ptr <awh::net::addr_t> address = std::make_unique <awh::net::addr_net_ipv4_t> ();
			// Переносим разобранный адрес в объект запроса
			awh_cast <awh::net::addr_net_ipv4_t *> (address.get())->address = expected.v4(awh::net_addr_t::endian_t::LITTLE);
			// Спрашиваем, какому устройству принадлежит поставленный адрес
			const std::string owner = eth.iface.name(address.get());
			// Адрес обязан числиться за заведённым устройством
			ASSERT_EQ(owner, iface)
				<< sign << ": движок согласился поставить адрес " << TUNNEL_LOCAL
				<< ", а система числит его "
				<< (owner.empty() ? std::string("ни за одним устройством") : (std::string("за устройством \"") + owner + "\""));
		...
		/**
		 * Сворачиваем движок, НЕ снося узел туннеля
		 *
		 * @warning Сноса узла здесь нет НАМЕРЕННО - в нём вся суть проверки
		 */
		ASSERT_TRUE(this->_io->deinitialize()) << sign;
	}
}
...
TEST_F(IoFixture, IoTunnelCarriesPacketTest){
	 ...
	 *
	 * @note Установлено на стенде Windows ARM64 20.08.2026 сличением двух
 ...
 *
 * @note Установлено на стенде Windows ARM64 21.08.2026: путь этот исправен и
...
TEST_F(IoFixture, IoTunnelAcceptsWrittenPacketTest){
	...
	while((std::chrono::duration_cast <std::chrono::milliseconds> (std::chrono::steady_clock::now() - start).count() < 10000) && !accepted){
		 ...
		 *
		 * @note Замечено 21.08.2026: сторож стоял ПОСЛЕ полного ожидания, и на
	 ...
	 *
	 * @note Установлено на стенде FreeBSD 21.08.2026: правила заслона там
 ...
 *
 * @note Установлено 21.08.2026 на трёх межмашинных парах: в первом обороте
