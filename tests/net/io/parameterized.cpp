...
class IoPingParameterizedFixture : public IoFixture, public ::testing::WithParamInterface <IoPingTestParameter> {
	...
	public:
		...
		std::unique_ptr <awh::chrono_t> chrono = std::make_unique <awh::chrono_t> ();
		// Создаём объект работы с IP-адресами
		std::unique_ptr <awh::net_addr_t> addr = std::make_unique <awh::net_addr_t> ();
};
...
TEST_P(IoPingParameterizedFixture, IoPingTest){
	 ...
	 */
	/**
	 * @warning Постоянности здесь нет НАМЕРЕННО. У MS Windows признак задан
	 *          значением `true`, и будь он объявлен постоянной, чтение его в отклике
	 *          не считалось бы употреблением: собиратель счёл бы захват излишним и
	 *          предупредил (`-Wunused-lambda-capture`). Захват по ссылке, каким это
	 *          лечили прежде, не помогает - предупреждение остаётся тем же
	 *
	...
	/**
	 * Признак сырого сокета захватывается ССЫЛКОЙ
	 *
	 * @note Ссылка верна: отклик живёт не дольше объемлющей проверки. Довод о
	 *       постоянности признака перенесён к самому его объявлению - лечится
	 *       предупреждение там, а не здесь
	 */
	this->_io->on(eid, [&raw, identifier, &expected, &replies](const awh::event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
		...
		awh::log::print(
			...
			awh::log::flag_t::INFO, eid, size, offset,
	...
	for(uint8_t i = 0; i < 3; i++){
		...
		// Запоминаем счёт откликов до обмена
		const uint8_t before = replies;
		// Отправляем сообщение серверу
		ASSERT_TRUE(this->_io->send(eid, reinterpret_cast <char *> (&icmp), sizeof(icmp)));
		/**
		 * Выполняем чтение ответа, пока не придёт отклик именно на наш запрос
		 *
		 * @warning Одного чтения на запрос НЕ ХВАТАЕТ, и системы здесь расходятся. По
		 *          петле Linux отдаёт сырому сокету НАШ ЖЕ эхо-запрос (тип 8) раньше
		 *          отклика (тип 0): пакет достаётся сокету копией через входной путь.
		 *          Обработчик такой пакет отбрасывает по типу - и верно делает, - но
		 *          отклик остаётся в очереди, а следующая итерация ждёт уже другой
		 *          номер последовательности. Счёт откликов оставался нулевым при вполне
		 *          исправном обмене. У macOS свой запрос сырому сокету не возвращается
		 *          вовсе, оттого прежнее одно чтение там проходило, а на Linux по петле
		 *          валилось. Проверено щупом на стенде OpenWRT 01.09.2026: сырой сокет
		 *          принимает подряд тип 8, затем тип 0
		 *
		 * @note На внешнем узле своего запроса не возвращается, и отклик приходит
		 *       первым же чтением - предел ниже там не тратится
		 *
		 ...
		 */
		for(uint8_t attempt = 0; (attempt < 3) && (replies == before); attempt++)
			// Выполняем чтение очередного принятого пакета
...
TEST_P(IoTimerParameterizedFixture, IoTimerTest){
	...
	if(this->_parameter.node == awh::event::node_t::INTERVAL){
		...
		this->_io->on(eid, [&count, &stop, this](const awh::event::id_t eid, const awh::event::status_t status) noexcept -> void {
			...
			if(status == awh::event::status_t::SUCCESS){
				...
				awh::log::print("Интервал сработал: ID=%u, %u seconds", awh::log::flag_t::INFO, eid, std::chrono::duration_cast <std::chrono::seconds> (shift - ts).count());
				// Замеряем время начала работы для интервала времени
	...
	// Если таймер является таймаутом
		...
		this->_io->on(eid, [&count, &stop, this](const awh::event::id_t eid, const awh::event::status_t status) noexcept -> void {
			...
			if(status == awh::event::status_t::SUCCESS){
				...
				awh::log::print("Таймер сработал: ID=%u, %u seconds", awh::log::flag_t::INFO, eid, std::chrono::duration_cast <std::chrono::seconds> (shift - this->ts).count());
	 ...
	 */
	/**
	 * Срок опроса обязателен: не приди ожидаемое событие через устройство, опрос ждал бы
	 * его без конца, и проверка не отказала бы, а зависла. Зависшая проверка неотличима
	 * от медленной и валит весь прогон без объяснения
	 */
	{
		/**
		 * Срок берётся от самой задержки, а не круглым числом: интервал обязан сработать
		 * четырежды, то есть работа теста длится четыре задержки, и при задержке в пять
		 * секунд круглый срок в двадцать секунд обрубал бы проверку ровно на последнем
		 * срабатывании. Запас шестикратный плюс пять секунд на планировщик
		 */
...
TEST_P(IoIPCTestParameterizedFixture, IoIPCTest){
	 ...
	 */
		 ...
		 */
		{
			...
			ASSERT_TRUE(::SetEnvironmentVariableW(L"AWH_IO_IPC_PIPE", awh::fmk::convert(pipe).c_str()));
			/**
		...
		case static_cast <uint8_t> (awh::event::family_t::PIPE): {
			...
			switch((pid = ::fork())){
				...
				case 0: {
					...
					this->_io->on(cfds[0], [](const awh::event::id_t eid, const awh::event::status_t status) noexcept -> void {
						/**
						 * Обрабатываем статус события
						 */
					...
					// Устанавливаем функцию обратного вызова на запись в событие
					this->_io->on(mfds[1], static_cast <awh::engine::callback::write_t> ([](const awh::event::id_t eid, const size_t size) noexcept -> void {
						// Записываем в лог сообщение о переподключении события
						awh::log::print("Записано: ID=%u, %zu байт", awh::log::flag_t::INFO, eid, size);
					}));
					// Устанавливаем функцию обратного вызова на чтение из события
					this->_io->on(cfds[0], [mpid, &stop](const awh::event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
					...
					// Устанавливаем функцию обратного вызова на ошибку события
					this->_io->on(cfds[0], [](const awh::event::id_t eid, const awh::event::error_t error, const std::string & description) noexcept -> void {
						/**
						 * Обрабатываем статус события
						 */
						switch(static_cast <uint8_t> (error)){
					...
					// Устанавливаем функцию обратного вызова на общее событие
					this->_io->on(cfds[0], [](const awh::event::id_t eid, const awh::event::action_t action) noexcept -> void {
						/**
						 * Обрабатываем действие события
						 */
						switch(static_cast <uint8_t> (action)){
				...
				default: {
					...
					// Устанавливаем функцию обратного вызова на получение статуса события
					this->_io->on(mfds[0], [](const awh::event::id_t eid, const awh::event::status_t status) noexcept -> void {
						/**
						 * Обрабатываем статус события
						 */
						switch(static_cast <uint8_t> (status)){
					...
					// Устанавливаем функцию обратного вызова на запись в событие
					this->_io->on(cfds[1], static_cast <awh::engine::callback::write_t> ([](const awh::event::id_t eid, const size_t size) noexcept -> void {
						// Записываем в лог сообщение о переподключении события
						awh::log::print("Записано: ID=%u, %zu байт", awh::log::flag_t::INFO, eid, size);
					}));
					// Устанавливаем функцию обратного вызова на чтение из события
					this->_io->on(mfds[0], [mpid, &stop](const awh::event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
						// Текст входящего сообщения
						const std::string message(reinterpret_cast <const char *> (data), size);
						// Записываем в лог сообщение о переподключении события
						awh::log::print("Прочитано: ID=%u, MPID=%u, PID=%u, %zu байт, сообщение: %s", awh::log::flag_t::INFO, eid, mpid, ::getpid(), size, message.c_str());
					...
					// Устанавливаем функцию обратного вызова на ошибку события
					this->_io->on(mfds[0], [](const awh::event::id_t eid, const awh::event::error_t error, const std::string & description) noexcept -> void {
						/**
						 * Обрабатываем статус события
						 */
						switch(static_cast <uint8_t> (error)){
					...
					// Устанавливаем функцию обратного вызова на общее событие
					this->_io->on(mfds[0], [](const awh::event::id_t eid, const awh::event::action_t action) noexcept -> void {
						/**
						 * Обрабатываем действие события
						 */
						switch(static_cast <uint8_t> (action)){
		...
		case static_cast <uint8_t> (awh::event::family_t::UDS): {
			...
			switch((pid = ::fork())){
				...
				case 0: {
					...
					// Устанавливаем функцию обратного вызова на чтение из события
					this->_io->on(events[1], [mpid, &stop](const awh::event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
						// Текст входящего сообщения
						const std::string message(reinterpret_cast <const char *> (data), size);
						// Записываем в лог сообщение о переподключении события
						awh::log::print("Прочитано: ID=%u, MPID=%u, PID=%u, %zu байт, сообщение: %s", awh::log::flag_t::INFO, eid, mpid, ::getpid(), size, message.c_str());
				...
				default: {
					...
					// Устанавливаем функцию обратного вызова на чтение из события
					this->_io->on(events[0], [mpid, &stop](const awh::event::id_t eid, const uint8_t * data, const size_t size) noexcept -> void {
						// Текст входящего сообщения
						const std::string message(reinterpret_cast <const char *> (data), size);
						// Записываем в лог сообщение о переподключении события
						awh::log::print("Прочитано: ID=%u, MPID=%u, PID=%u, %zu байт, сообщение: %s", awh::log::flag_t::INFO, eid, mpid, ::getpid(), size, message.c_str());
