 ...
 *
 * @brief Параметризованные тесты сетевой очереди —
 *        прогон подготовленных наборов входных данных через методы модуля с проверкой буферизации данных,
 *        контроля ёмкости и корректности порядка извлечения записей
 *
