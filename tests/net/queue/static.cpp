...
TEST_F(NetworkQueueFixture, DISABLED_RssFootprintBenchmark){
	...
	::printf("=============================================\n");
	/**
	 * Недоступность замера памяти обязана быть ПРОПУСКОМ с доводом, а не тишиной
	 *
	 * @warning Прежде здесь стояло условие без ветви «иначе»: на системе, где
	 *          замер резидентной памяти не даётся, проверка не утверждала НИЧЕГО
	 *          и проходила зелёной. Отличить такой проход от настоящего было
	 *          нечем - ни отказа, ни пропуска, ни довода
	 */
	if((rssBase == 0) || (rssEmpty == 0))
		// Замер памяти системой не даётся - утверждать нечего
		GTEST_SKIP() << "замер резидентной памяти на этой системе недоступен: rssBase=" << rssBase << ", rssEmpty=" << rssEmpty;
	...
	/**
	 * Заявка замера утверждается, а не только печатается
	 *
	 * @details Смысл ленивого выделения в том, что деятельные буферы обходятся
	 *          КРАТНО дешевле сплошного отведения по 64 КБ на соединение. Прежде
	 *          это лишь печаталось строкой «Savings vs legacy», и замер прошёл бы
	 *          и при экономии в единицу, то есть при полной утрате смысла правки
	 *
	 * @note Порог взят с большим запасом - вдесятеро при измеренных 71.6 раза на
	 *       macOS. Отношение это от машины зависит, и жать его к измеренному
	 *       значило бы ловить не утрату свойства, а разницу машин
	 */
	if(rssActive > rssBase)
		// Экономия против сплошного отведения обязана быть кратной
		ASSERT_GT((legacy / (rssActive - rssBase)), 10u) << "экономия резидентной памяти против сплошного отведения пропала: ленивое выделение буфера больше не работает";
}
...
TEST_F(NetworkQueueFixture, DISABLED_LatencyBenchmark){
	 ...
	 * Заявка замера утверждается, а не только печатается
	 *
	 * @details Смысл кольцевой очереди TCP в том, что она обходится БЕЗ `memmove`,
	 *          тогда как линейная очередь UDP уплотняется переносом. Под
	 *          раздроблённостью это и меряется. Прежде утверждение здесь было одно -
	 *          `ASSERT_GT(sink, 0u)`, - и оно лишь не давало собирателю выбросить
	 *          измеряемый код. Замер прошёл бы и будь кольцевая очередь вдесятеро
...
/**
 * @brief Нагрузочный тест целостности потоковой очереди при случайных последовательностях
 *
 * @details Читать код глазами мало: устройство bip-буфера с двумя регионами A и B ломается
 *          не на простых случаях, а на переключении регионов под нагрузкой. Проверка гоняет
 *          десятки тысяч случайных чередований помещения и вычитывания, сличая поток ПОБАЙТНО
 *          с образцом. Всякая перестановка, потеря либо задвоение октета видны немедленно
 *
 * @note Помещение здесь ЧАСТИЧНОЕ: принято может быть меньше отданного, и продолжать надо с
 *       непринятого остатка. Именно на этом ловится подмена правила на «всё или ничего»
 *
 */
TEST_F(NetworkQueueFixture, TcpStreamIntegrityStressTest){
	// Очищаем очередь
	this->_queue->clear();
	// Устанавливаем тип очереди для потоков данных (например, TCP)
	this->_queue->type(awh::net_queue_t::type_t::TCP);
	// Образец потока, которым проверяется целостность
	std::vector <uint8_t> pattern(64 * 1024, 0);
	// Наполняем образец неповторяющейся последовательностью
	for(size_t i = 0; i < pattern.size(); i++)
		// Записываем очередной октет образца
		pattern[i] = static_cast <uint8_t> ((i * 7 + (i >> 8)) & 0xFF);
	// Смещение подачи в образце
	size_t sent = 0;
	// Смещение вычитывания в образце
	size_t taken = 0;
	// Генератор случайных величин с постоянным зерном для воспроизводимости прогона
	std::mt19937 generator(20260814);
	// Распределение размеров порций обмена
	std::uniform_int_distribution <size_t> sizes(1, 9000);
	/**
	 * Гоняем случайные чередования помещения и вычитывания
	 */
	for(size_t step = 0; step < 20000; step++){
		// Если очередь ещё способна принимать данные
		if((generator() & 1) || this->_queue->empty()){
			/**
			 * Определяем размер очередной порции подачи
			 *
			 * @note Каждая четвёртая порция берётся заведомо крупной, чтобы упереться в нехватку
			 *       места: без этого случай «отдано больше, чем влезает» почти не встречается, и
			 *       подмена частичного помещения правилом «всё или ничего» остаётся незамеченной
			 */
			const size_t size = std::min((((step & 3) == 0) ? (pattern.size() / 2) : sizes(generator)), (pattern.size() - sent));
			// Если образец исчерпан, начинаем его заново
			if(size == 0){
				// Сбрасываем смещение подачи в начало образца
				sent = 0;
				// Переходим к следующему шагу
				continue;
			}
			// Запоминаем свободное место в очереди до помещения
			const size_t available = this->_queue->available();
			// Помещаем очередную порцию в очередь
			const size_t accepted = this->_queue->push((pattern.data() + sent), size);
			// Проверяем что принято не больше отданного
			ASSERT_LE(accepted, size) << "очередь приняла больше отданного на шаге " << step;
			/**
			 * Проверяем что принято ровно столько, сколько влезало
			 *
			 * @warning Утверждение это отличает частичное помещение от правила «всё или ничего»:
			 *          без него подмена одного другим проходит незамеченной, ведь поток остаётся
			 *          целым в обоих случаях - отвергнутая порция просто подаётся заново. Проверено
			 *          опытом: сличение потока такую подмену НЕ ловит
			 */
			ASSERT_EQ(accepted, std::min(size, available)) << "принято не столько, сколько влезало, на шаге " << step;
			// Сдвигаем смещение подачи на действительно принятое
			sent += accepted;
			// Если образец исчерпан целиком
			if(sent >= pattern.size())
				// Сбрасываем смещение подачи в начало образца
				sent = 0;
		// Иначе вычитываем накопленное
		} else {
			// Указатель на непрерывный участок данных очереди
			const void * data = nullptr;
			// Размер непрерывного участка данных очереди
			size_t size = 0;
			// Получаем непрерывный участок данных из очереди
			ASSERT_TRUE(this->_queue->front(&data, size)) << "очередь не отдала данные при непустом состоянии на шаге " << step;
			// Проверяем что участок не пустой
			ASSERT_GT(size, 0u) << "очередь отдала пустой участок при непустом состоянии на шаге " << step;
			// Определяем сколько октетов вычитываем за этот шаг
			const size_t part = std::min(size, sizes(generator));
			/**
			 * Сличаем вычитанное с образцом побайтно
			 */
			for(size_t i = 0; i < part; i++){
				// Проверяем что очередной октет совпадает с образцом
				ASSERT_EQ(reinterpret_cast <const uint8_t *> (data)[i], pattern[(taken + i) % pattern.size()])
					<< "поток искажён на шаге " << step << ", смещение " << (taken + i);
			}
			// Сдвигаем смещение вычитывания
			taken += part;
			// Удаляем вычитанное из очереди
			ASSERT_TRUE(this->_queue->pop(part)) << "очередь не удалила вычитанное на шаге " << step;
		}
		// Проверяем что для потоковой очереди счётчик совпадает с размером данных
		ASSERT_EQ(this->_queue->count(), this->_queue->size()) << "счётчик разошёлся с размером на шаге " << step;
		// Проверяем что размер данных не превышает ёмкости буфера
		ASSERT_LE(this->_queue->size(), static_cast <size_t> (AWH_NETWORK_QUEUE_BUFFER_SIZE)) << "размер превысил ёмкость на шаге " << step;
		// Проверяем что свободное место не превышает ёмкости буфера
		ASSERT_LE(this->_queue->available(), static_cast <size_t> (AWH_NETWORK_QUEUE_BUFFER_SIZE)) << "свободное место превысило ёмкость на шаге " << step;
	}
	// Проверяем что обмен действительно состоялся
	ASSERT_GT(taken, static_cast <size_t> (AWH_NETWORK_QUEUE_BUFFER_SIZE)) << "через очередь прошло меньше её ёмкости, нагрузка не состоялась";
}

/**
 * @brief Нагрузочный тест сохранения границ сообщений при случайных последовательностях
 *
 * @details Дейтаграммная очередь обязана отдавать сообщения ЦЕЛИКОМ и в порядке помещения,
 *          не склеивая соседние и не дробя одно. Проверка гоняет случайные размеры сообщений
 *          вперемежку с вычитыванием, сличая каждое сообщение с тем, что клали
 *
 * @note Помещение здесь НЕДЕЛИМО: сообщение либо помещается целиком, либо не помещается вовсе
 *
 */
TEST_F(NetworkQueueFixture, UdpMessageBoundariesStressTest){
	// Очищаем очередь
	this->_queue->clear();
	// Устанавливаем тип очереди для границ сообщений (например, UDP)
	this->_queue->type(awh::net_queue_t::type_t::UDP);
	// Очередь помещённых сообщений для последующего сличения
	std::vector <std::vector <uint8_t>> expected;
	// Генератор случайных величин с постоянным зерном для воспроизводимости прогона
	std::mt19937 generator(20260814);
	// Распределение размеров сообщений
	std::uniform_int_distribution <size_t> sizes(1, 4000);
	// Количество сличённых сообщений
	size_t checked = 0;
	/**
	 * Гоняем случайные чередования помещения и вычитывания
	 */
	for(size_t step = 0; step < 20000; step++){
		// Если помещаем очередное сообщение
		if((generator() & 1) || expected.empty()){
			// Определяем размер очередного сообщения
			const size_t size = sizes(generator);
			// Собираем сообщение, помеченное собственным номером
			std::vector <uint8_t> message(size, static_cast <uint8_t> (step & 0xFF));
			// Помещаем сообщение в очередь
			const size_t accepted = this->_queue->push(message.data(), message.size());
			// Проверяем что сообщение принято целиком либо отвергнуто целиком
			ASSERT_TRUE((accepted == message.size()) || (accepted == 0)) << "сообщение принято частично на шаге " << step;
			// Если сообщение принято
			if(accepted > 0)
				// Запоминаем помещённое сообщение для сличения
				expected.push_back(std::move(message));
		// Иначе вычитываем верхнее сообщение
		} else {
			// Указатель на данные верхнего сообщения
			const void * data = nullptr;
			// Размер верхнего сообщения
			size_t size = 0;
			// Получаем верхнее сообщение из очереди
			ASSERT_TRUE(this->_queue->front(&data, size)) << "очередь не отдала сообщение при непустом состоянии на шаге " << step;
			// Проверяем что размер сообщения сохранён в точности
			ASSERT_EQ(size, expected.front().size()) << "граница сообщения нарушена на шаге " << step;
			// Проверяем что содержимое сообщения не искажено
			ASSERT_EQ(::memcmp(data, expected.front().data(), size), 0) << "содержимое сообщения искажено на шаге " << step;
			// Удаляем вычитанное сообщение из очереди
			ASSERT_TRUE(this->_queue->pop()) << "очередь не удалила сообщение на шаге " << step;
			// Удаляем сличённое сообщение из образца
			expected.erase(expected.begin());
			// Увеличиваем количество сличённых сообщений
			checked++;
		}
		// Проверяем что счётчик записей совпадает с количеством ожидаемых сообщений
		ASSERT_EQ(this->_queue->count(), expected.size()) << "счётчик записей разошёлся с образцом на шаге " << step;
	}
	// Проверяем что обмен действительно состоялся
	ASSERT_GT(checked, 1000u) << "через очередь прошло слишком мало сообщений, нагрузка не состоялась";
}
