...
/**
 * @brief Тест обрыва разбора на первом ненайденном поле
 *
 * @details Разбор обрывается первым полем образца, которого в записи нет,
 *          и остальные поля не разбираются: замер мягкого пропуска на
 *          200 000 записей дал 3992 спасённых разбора против 5667 грубо
 *          неверных, объявленных при этом пригодными
 *
 */
TEST_F(ChronoFixture, ExecutionParseStopsAtMissingFieldChronoTest){
	// Запись смещения зоны не несёт, а образец его требует первым полем
	ASSERT_FALSE(this->_chrono->validate("01.02.2003 04:05:06", "%z %d.%m.%Y %H:%M:%S"));
	// Признак разбора отказ подтверждает
	bool valid = true;
	// Выполняем разбор записи, смещения зоны не несущей
	this->_chrono->parse("01.02.2003 04:05:06", "%z %d.%m.%Y %H:%M:%S", valid);
	// Разбор пригодным не считается
	ASSERT_FALSE(valid);
	// Та же запись образцом, ей отвечающим, разбирается пригодной
	ASSERT_TRUE(this->_chrono->validate("01.02.2003 04:05:06", "%d.%m.%Y %H:%M:%S"));
