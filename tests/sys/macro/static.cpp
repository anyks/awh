 ...
 */
	 ...
	 *       пришедший следом, объявления его переопределяет. Система оглашает это
	 *       предупреждением `#warning Please include winsock2.h before windows.h`,
	 *       и оно у боевой сборки было единственным предупреждением всего дерева,
	 *       поднятым заголовками системы
	 *
	 * @note Замысел проверки порядком этим НЕ страдает: макросы `DELETE`, `ERROR`,
	 *       `STRICT` и прочие вводит `windows.h`, и к разбору заголовков AWH они
	 *       действуют по-прежнему - худший случай со стороны потребителя
	 *       воспроизводится ровно так же
	 */
	#include <winsock2.h>
	#include <windows.h>
#endif
