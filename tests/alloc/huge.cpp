...
TEST_F(AllocFixture, InPlaceGrowthAmendsHoldings){
	...
	ASSERT_NE(grown, nullptr);
	/**
	 * Оставшийся на месте блок проверяется отдельно
	 *
	 * @details Рост на месте есть дело соседства: место за блоком свободно - слой
	 *          растит его правкою отображения, занято - перевыдаёт. Утверждать
	 *          неподвижность нельзя, соседство складывается ходом прежних проверок
	 *
	 * @warning Прежде здесь стояло `ASSERT_EQ(grown, block)`, и проверка валилась
	 *          порядком набора, а не дефектом: одиночкою и парою она зелена, полным
	 *          прогоном - красна. Предмет её - не место блока, а ПРАВКА РАЗМЕРА в
	 *          учёте, и правка эта обязательна обоими путями
	 */
	if(grown == block)
		// Доносим о том, что рост прошёл именно на месте
		::testing::Test::RecordProperty("growth", "in-place");
