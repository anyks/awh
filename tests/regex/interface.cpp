...
TEST(Regex, InterfaceControlBacktracking) {
	 ...
	 *          совпадение давшего, а по отказу - отметка последняя, всем
	 *          сопоставлением встреченная. Глагол отсечения её НЕ снимает,
	 *          а сопоставление, отбором позиций отвергнутое, отметки
	 *          не даёт вовсе - попыток оно не делает.
	 *
	 *          Указание отмены отбора проставлено там, где отбор на отметку
	 *          и влияет: набор оптимизаций начала у нас и у эталона разный,
	 *          а отметка отказа от него ЗАВИСИТ - под отменою обе реализации
	 *          отвечают одинаково, и это доказано опытом.
	 *
	 */
	static const struct {
...
/**
 * @brief Проверка глаголов управления с именем отметки
 *
 * @details Разряд пятый описи глаголов управления. Правила сняты с эталонной
 *          реализации опытом матрицею на шестьдесят шесть образцов. Имя у глагола
 *          отсечения означает отметку, глаголу предшествующую: «(*COMMIT:имя)»
 *          равен «(*MARK:имя)(*COMMIT)». Глагол переноса имя толкует иначе -
 *          ключом розыска отметки ЖИВОЙ того имени: найдя её, он переносит
 *          попытку в положение её, а не найдя - не правит вовсе, будто
 *          глагола и не было. Имя пустое обращает глагол в безымянный.
 *
 */
TEST(Regex, InterfaceNamedControlVerbs) {
	...
	vector <pair <size_t, size_t>> bounds;
	/**
	 * @brief Ожидания сопоставления глаголов с именем отметки
	 *
	 * @details Указание отмены отбора позиций проставлено там, где отметка
	 *          отказа сличается: набор оптимизаций начала у нас и у эталона
	 *          разный, а отметка отказа от него ЗАВИСИТ - эталон под тем же
	 *          указанием отвечает ровно как мы, и это доказано опытом.
	 *
	 */
	static const struct {
		// Выражение с глаголом управления, имя несущим
		const char * pattern;
		// Текст сопоставления выражения
		const char * text;
		// Ожидаемый исход сопоставления
		bool matched;
		// Ожидаемые границы обнаруженного совпадения
		size_t begin;
		size_t finish;
		// Ожидаемое имя отметки сопоставления
		const char * marker;
	} SAMPLES[] = {
		// Имя глагола отсечения означает отметку, глаголу предшествующую
		{"a(*COMMIT:A)b", "ab", true, 0, 2, "A"},
		{"a(*PRUNE:A)b", "xab", true, 1, 3, "A"},
		{"a(*ACCEPT:A)b", "ab", true, 0, 1, "A"},
		{"a(*FAIL:A)b", "ab", false, 0, 0, "A"},
		{"(*NO_START_OPT)aa(*COMMIT:Z)b", "aaab", false, 0, 0, "Z"},
		{"(?:aa(*THEN:Z)b|aab)", "aaab", true, 1, 4, "Z"},
		{"aa(*PRUNE:Z)b", "aaab", true, 1, 4, "Z"},
		// Отметка глагола отсечения снимается возвратом наравне с захватами
		{"a(*THEN:A)b|ac", "ac", true, 0, 2, ""},
		// Глагол переноса отметки своей не заводит вовсе
		{"a(*SKIP:A)b", "xab", true, 1, 3, ""},
		{"a(*SKIP:A:B)b", "ab", true, 0, 2, ""},
		{"a(*SKIP:A B)b", "ab", true, 0, 2, ""},
		// Глагол переноса переносит попытку в положение отметки имени
		{"a(*MARK:A)a(*SKIP:A)b", "aaab", true, 1, 4, "A"},
		{"(*MARK:A)aa(*SKIP:A)b", "aaab", true, 1, 4, "A"},
		{"(*NO_START_OPT)(*MARK:A)a+(*SKIP:A)b", "aaac", false, 0, 0, "A"},
		/**
		 * Отметки имени не найдя, глагол переноса не правит вовсе
		 *
		 * @details Безымянный глагол на том же образце совпадения не даёт:
		 *          он переносит попытку в положение своё, а глагол с именем
		 *          неведомым обход позиций не правит и совпадение находит.
		 *
		 */
		{"aa(*SKIP:Z)b", "aaab", true, 1, 4, ""},
		{"aa(*SKIP)b", "aaab", false, 0, 0, ""},
		{"aa(*SKIP:A)b", "aaab", true, 1, 4, ""},
		/**
		 * Розыск находит отметку ЖИВУЮ, а не встреченную последнею
		 *
		 * @details Отметка, ветвью отвергнутой поставленная, возвратом снята
		 *          и розыском не находится: глагол переноса не правит вовсе.
		 *
		 */
		{"(?:(*MARK:A)xx|aa)(*SKIP:A)b", "aaab", true, 1, 4, ""},
		{"(?:(*MARK:A)xx|aa)(*SKIP)b", "aaab", false, 0, 0, "A"},
		// Имя пустое обращает глагол в безымянный
		{"a(*SKIP:)b", "aab", true, 1, 3, ""},
		{"a(*COMMIT:)b", "ab", true, 0, 2, ""},
		/**
		 * Отказ выводит отметку последнюю, сопоставлением встреченную
		 *
		 * @details Глагол отсечения её не снимает: правило снято с эталона
		 *          опытом на «a(*MARK:A)(*COMMIT)b» с текстом «aab».
		 *
		 */
		{"(*NO_START_OPT)a(*MARK:A)(*COMMIT)b", "aab", false, 0, 0, "A"},
		{"(*NO_START_OPT)x(*MARK:A)y", "xq", false, 0, 0, "A"},
		{"(*NO_START_OPT)(*MARK:A)x|(*MARK:B)y(*SKIP:A)z", "yq", false, 0, 0, "B"}
	};
	/**
	...
	for(auto & sample : SAMPLES) {
		...
		// Выполняем проверку имени отметки сопоставления
		EXPECT_EQ(regexp.marker(), string(sample.marker))
		 << sample.pattern << " на тексте «" << sample.text << "»";
	}
	// Выполняем проверку отказа квантора за глаголом управления с именем
	EXPECT_FALSE(static_cast <bool> (regexp.build("a(*SKIP:A)*b")));
	EXPECT_FALSE(static_cast <bool> (regexp.build("a(*COMMIT:A)+b")));
	// Выполняем проверку отказа глагола с именем, скобкой не завершённого
	EXPECT_FALSE(static_cast <bool> (regexp.build("a(*SKIP:A")));
	EXPECT_FALSE(static_cast <bool> (regexp.build("a(*FAIL:A")));
}

/**
 * @brief Проверка вызовов подпрограмм и условных выражений
 *
 * @details Правила сняты с эталонной реализации опытом. Условие ветвями делит
 *          сопоставление надвое, и отбор позиций обязан обе ветви обходить;
 *          повторение неограниченное проходом пустым завершается, а ограниченное
 *          проходы свои делает все; длина вызова подпрограммы берётся длиною
 *          вызываемой группы, отчего ретроспективная проверка вызов допускает.
 *
 */
TEST(Regex, InterfaceSubroutineCalls) {
	...
	vector <pair <size_t, size_t>> bounds;
	/**
	 * @brief Ожидания сопоставления вызовов и условий
	 *
	 */
	static const struct {
		// Выражение с вызовом подпрограммы либо условием
		const char * pattern;
		// Текст сопоставления выражения
		const char * text;
		// Ожидаемый исход сопоставления
		...
		/**
		 * Отбор позиций обязан обходить ветвь невыполнения условия
		 *
		 * @details Совпадение способно начаться байтами обеих ветвей: обход
		 *          одной лишь ветви выполнения отнимал у выражения позиции,
		 *          ветвью невыполнения начинающиеся.
		 *
		 */
		{"(?(?=a)b|c)", "c", true, 0, 1},
		{"(?(?=a)b|c)", "abc", true, 2, 3},
		...
		{"(?(?=a)ab|c)", "ab", true, 0, 2},
		/**
		 * Повторение неограниченное проходом пустым завершается
		 *
		 * @details Проход первый пуст, ибо группа захвата не завершила, и на том
		 *          повторение и кончается: прохода второго, условие уже
		 *          выполняющего, эталон не делает вовсе.
		 *
		 */
		{"((?(1)a))+", "aaa", true, 0, 0},
		{"((?(1)a))++", "aaa", true, 0, 0},
		...
		{"(?:(?(1)a)(b?))+", "abab", true, 0, 0},
		/**
		 * Повторение ограниченное проходы свои делает все
		 *
		 * @details Сторож продвижения стережёт бесконечность, а её у повторения
		 *          ограниченного нет: проход второй, условие уже выполняющий,
		 *          эталон делает.
		 *
		 */
		{"((?(1)a)){1,2}+", "aab", true, 0, 1},
		{"(x?)+", "aaa", true, 0, 0},
		{"(x?)+", "xaa", true, 0, 1},
		/**
		 * Длина вызова подпрограммы берётся длиною вызываемой группы
		 *
		 * @details Ретроспективная проверка вызов допускает, и группа вправе
		 *          объявляться позже проверки: длина вычисляется по завершении
		 *          разбора наравне со ссылками отложенными.
		 *
		 */
		{"(a)(?<=(?1))", "aa", true, 0, 1},
		{"(a)(?<=(?-1))", "aa", true, 0, 1},
	...
	/**
	 * Выполняем проверку отказа вызова неограниченной длины в проверке
	 *
	 * @details Длина вызова рекурсивного, равно как и вызова группы длины
	 *          неограниченной, неизвестна: ретроспективная проверка такова
	 *          отвергается наравне с эталоном.
	 *
	 */
	EXPECT_FALSE(static_cast <bool> (regexp.build("(a)(?<=(?R))")));
	EXPECT_FALSE(static_cast <bool> (regexp.build("(a+)(?<=(?1))")));
	EXPECT_FALSE(static_cast <bool> (regexp.build("(a)(?<=(?0))")));
}

/**
 * @brief Проверка ретроспективных проверок переменной длины
 *
 * @details Правила сняты с эталонной реализации опытом. Длина ссылки на захват
 *          берётся длиною группы, ею указанной, наравне с длиною вызова
 *          подпрограммы, а длины проверки перебираются от НАИБОЛЬШЕЙ:
 *          исход самой проверки порядок не правит - она нулевой ширины, -
 *          а захваты тела правит целиком.
 *
 */
/**
 * @brief Проверка сопоставления не отсекающих проверок окружения
 *
 * @details Проверка окружения обыкновенная атомарна: точки возврата, телом её
 *          накопленные, отсекаются выполнением проверки. Проверка же
 *          не отсекающая - «(?*...)» и «(?<*...)» - точки сохраняет,
 *          отчего отказ последующего текста продолжается перебором
 *          тела: правило снято с эталонной реализации опытом
 *          на «(?*(a|ab))\1c», где «(?=(a|ab))\1c» отказывает.
 *
 */
/**
 * @brief Проверка сопоставления прогонов письменности
 *
 * @details Прогон письменности - «(*sr:...)» - сводит письменности сопоставленного
 *          текста пересечением наборов расширений: набор пустой отказывает
 *          сопоставлению. Десятичные цифры сверх того обязаны принадлежать
 *          набору единому. Правила сняты с эталонной реализации опытом.
 *
 */
/**
 * @brief Проверка указаний начала выражения
 *
 * @details Указание «(*NOTEMPTY_ATSTART)» запрещает пустое совпадение одною лишь
 *          позицией начала поиска, тогда как «(*NOTEMPTY)» запрещает его всюду.
 *          Оба испытываются и путём порождения машинного кода: признаки
 *          эти ставит разбор выражения, а не заказчик сборки.
 *
 */
/**
 * @brief Проверка завершения комментария соглашением о переводе строки
 *
 * @details Комментарий режима расширенного - «#» до конца строки - завершается
 *          переводом строки СОГЛАШЕНИЯ действующего, а не переводом строки
 *          всяким: при соглашении «(*CRLF)» одиночный перевод строки
 *          комментария не рвёт, отчего «(*CRLF)(?x)(#c\n)a»
 *          отвергается несоответствием скобок.
 *
 */
TEST(Regex, InterfaceCommentConvention) {
	...
	vector <pair <size_t, size_t>> bounds;
	/**
	 * @brief Ожидания завершения комментария переводом строки
	 *
	 */
	static const struct {
		// Выражение с комментарием режима расширенного
		const char * pattern;
		// Ожидание сопоставления буквы, за комментарием стоящей
		bool matched;
	} SAMPLES[] = {
		/**
		 * Пару возврата каретки завершает лишь пара
		 */
		{"(*CRLF)(?x)#c\na", false},
		{"(*CRLF)(?x)#c\r\na", true},
		{"(*CRLF)(?x)#c\ra", false},
		/**
		 * Соглашение одиночного знака завершается им одним
		 */
		{"(*CR)(?x)#c\na", false},
		{"(*CR)(?x)#c\ra", true},
		{"(*LF)(?x)#c\na", true},
		...
		{"(*LF)(?x)#c\va", false},
		/**
		 * Соглашения всякого перевода строки завершаются всяким знаком его
		 */
		{"(*ANYCRLF)(?x)#c\na", true},
		{"(*ANYCRLF)(?x)#c\ra", true},
		{"(*ANY)(?x)#c\va", true},
		/**
		 * Соглашение умолчания есть перевод строки
		 */
		{"(?x)#c\na", true}
	};
	/**
	...
	for(auto & sample : SAMPLES) {
		...
		ASSERT_TRUE(static_cast <bool> (expression)) << sample.pattern;
		// Выполняем проверку обнаружения совпадения
		ASSERT_TRUE(regexp.match("a", expression, bounds)) << sample.pattern;
		/**
		 * Выполняем проверку длины обнаруженного совпадения
		 *
		 * @details Комментарий, переводом строки завершённый, оставляет
		 *          букву «a» частью выражения - совпадение длиною
		 *          в знак. Комментарий незавершённый поглощает
		 *          её, и совпадение выходит пустым.
		 *
		 */
		EXPECT_EQ(bounds.front().second, static_cast <size_t> (sample.matched ? 1 : 0)) << sample.pattern;
	}
	/**
	 * Выполняем проверку отказа выражения с комментарием, скобку поглотившим
	 */
	EXPECT_FALSE(static_cast <bool> (regexp.build("(*CRLF)(?x)(#c\na")));
	EXPECT_FALSE(static_cast <bool> (regexp.build("(*CRLF)(?x)(#c\n)a")));
}
TEST(Regex, InterfaceStartOptions) {
	...
	vector <pair <size_t, size_t>> bounds;
	/**
	 * @brief Ожидания сопоставления указаний начала выражения
	 *
	 */
	static const struct {
		// Выражение с указанием начала
		const char * pattern;
		// Текст сопоставления выражения
		const char * text;
		// Ожидание обнаружения совпадения
		bool matched;
		// Ожидаемые границы обнаруженного совпадения
		size_t begin;
		size_t finish;
		...
		/**
		 * Пустое совпадение запрещается одною лишь позицией начала поиска
		 *
		 * @details Совпадение пустое в позиции последующей допустимо: «(*NOTEMPTY)»
		 *          отказывает всюду, «(*NOTEMPTY_ATSTART)» - лишь в начале.
		 *
		 */
		{"(*NOTEMPTY_ATSTART)a*", "b", true, 1, 1},
		{"(*NOTEMPTY_ATSTART)a*", "", false, 0, 0},
		{"(*NOTEMPTY_ATSTART)a*", "ab", true, 0, 1},
		{"(*NOTEMPTY_ATSTART)b*", "ab", true, 1, 2},
		{"(*NOTEMPTY_ATSTART)x*", "ab", true, 1, 1},
		{"(*NOTEMPTY_ATSTART)(?:)", "ab", true, 1, 1},
		{"(*NOTEMPTY_ATSTART)||", "aa", true, 1, 1},
		{"(*NOTEMPTY)x*", "ab", false, 0, 0},
		{"(*NOTEMPTY)a*", "ab", true, 0, 1},
		{"(*NOTEMPTY)||", "aa", false, 0, 0},
		/**
		 * Указание отмены привязки ведущей точкой принимается
		 *
		 * @details Оптимизации, обращающей ведущее «.*» в привязку, у модуля
		 *          нет вовсе, отчего указание не правит ничего: отвергать
		 *          его нельзя - эталон выражения с ним собирает.
		 *
		 */
		{"(*NO_DOTSTAR_ANCHOR).*a", "ba", true, 0, 2},
		{"(*NO_DOTSTAR_ANCHOR)a", "ba", true, 1, 2}
	};
	/**
	 * Выполняем обход набора ожиданий сопоставления
	 */
	for(auto & sample : SAMPLES) {
		 ...
		 *
		 * @details Первый проход исполняет выражение толкователем, второй -
		 *          порождённым машинным кодом: признаки указаний ставит
		...
		for(size_t index = 0; index < 2; index++) {
			...
			if(!sample.matched) {
				...
				// Переходим к пути следующему
				continue;
			}
			// Выполняем проверку обнаружения совпадения
			ASSERT_TRUE(regexp.match(sample.text, expression, bounds))
			 << sample.pattern << " на тексте «" << sample.text << "», путь " << index;
			// Выполняем проверку границ обнаруженного совпадения
...
TEST(Regex, InterfaceScriptRuns) {
	...
	vector <pair <size_t, size_t>> bounds;
	/**
	 * @brief Ожидания сопоставления прогонов письменности
	 *
	 */
	static const struct {
		// Выражение с прогоном письменности
		const char * pattern;
		// Текст сопоставления выражения
		const char * text;
		// Ожидание обнаружения совпадения
		...
		/**
		 * Письменности прогона сводятся пересечением наборов
		 *
		 * @details Текст письменности единой прогоном проходится целиком,
		 *          а смена письменности прогон прерывает: «.+» отступает
		 *          до длины, прогоном допустимой.
		 *
		 */
		{"(*UTF)(*sr:.+)", "abc", true, 0, 3},
		{"(*UTF)(*sr:.+)", "\xce\xb1\xce\xb2\xce\xb3", true, 0, 6},
		{"(*UTF)(*sr:.+)", "a\xce\xb1", true, 0, 1},
		{"(*UTF)(*sr:.+)", "\xce\xb1\xd0\xb0", true, 0, 2},
		/**
		 * Символы письменности общей прогона не сужают
		 *
		 * @details Знаки препинания, пробелы и цифры латинские принадлежат
		 *          письменности «Common» и годны прогону всякому.
		 *
		 */
		{"(*UTF)(*sr:.+)", "a1", true, 0, 2},
		{"(*UTF)(*sr:.+)", "a_b", true, 0, 3},
		{"(*UTF)(*sr:.+)", "a b", true, 0, 3},
		{"(*UTF)(*sr:.+)", "\xd8\xb9" "1", true, 0, 3},
		/**
		 * Сочетания письменностей восточноазиатских допустимы
		 *
		 * @details Эталон допускает прогоном китайское «Han + Bopomofo»,
		 *          японское «Han + Hiragana + Katakana» и корейское
		 *          «Han + Hangul», а сочетания прочие - нет:
		 *          «Hiragana + Hangul» прогоном не является.
		 *
		 */
		{"(*UTF)(*sr:.+)", "\xe6\x97\xa5\xe3\x81\x94", true, 0, 6},
		{"(*UTF)(*sr:.+)", "\xe6\x97\xa5\xed\x95\x9c", true, 0, 6},
		{"(*UTF)(*sr:.+)", "\xe3\x81\x94\xed\x95\x9c", true, 0, 3},
		{"(*UTF)(*sr:.+)", "\xe6\x97\xa5\xe3\x83\xbb\xe3\x81\x94", true, 0, 9},
		/**
		 * Десятичные цифры обязаны принадлежать набору единому
		 *
		 * @details Цифра арабо-индийская с цифрой латинской прогона не дают,
		 *          хотя письменности их совместимы: «1٢» прерывается
		 *          на цифре второй.
		 *
		 */
		{"(*UTF)(*sr:.+)", "\xd9\xa1\xd9\xa2", true, 0, 4},
		{"(*UTF)(*sr:.+)", "12", true, 0, 2},
		{"(*UTF)(*sr:.+)", "1\xd9\xa2", true, 0, 1},
		/**
		 * Прогон атомарный отказом проверки тела не пересопоставляет
		 *
		 * @details Прогон «(*asr:...)» обёртке «(?>(*sr:...))» НЕ равен:
		 *          отказ проверки прогона прекращает попытку в позиции
		 *          текущей, тогда как обёртка отступает телом.
		 *
		 */
		{"(*UTF)(*asr:.*)", "\xce\xb1\xd0\xb0", true, 2, 4},
		{"(*UTF)(?>(*sr:.*))", "\xce\xb1\xd0\xb0", true, 0, 2},
		{"(*UTF)(*asr:.*)x", "\xce\xb1\xd0\xb0x", false, 0, 0},
		{"(*UTF)(*asr:a|ab)", "ab", true, 0, 1},
		/**
		 * Имена прогонов записываются и полностью
		 */
		{"(*UTF)(*script_run:.+)", "a\xce\xb1", true, 0, 1},
		{"(*UTF)(*atomic_script_run:.*)", "\xce\xb1\xd0\xb0", true, 2, 4}
	};
	/**
	 * Выполняем обход набора ожиданий сопоставления
	 */
	for(auto & sample : SAMPLES) {
		...
		if(!sample.matched) {
			...
			EXPECT_FALSE(regexp.match(sample.text, expression, bounds))
			 << sample.pattern << " на тексте «" << sample.text << "»";
			// Переходим к ожиданию следующему
			continue;
		}
		// Выполняем проверку обнаружения совпадения
		ASSERT_TRUE(regexp.match(sample.text, expression, bounds))
	...
	/**
	 * Выполняем проверку отказа прогона письменности проходом детерминированным
	 *
	 * @details Прогон письменности ведёт учёт письменностей сопоставленного
	 *          текста, а проход детерминированный такого не ведёт вовсе:
	 *          выражение с прогоном исполняется путём с возвратом.
	 *
	 */
	EXPECT_TRUE(static_cast <bool> (regexp.build("(*UTF)(*sr:\\w+)")));
	/**
	 * Выполняем проверку отказа имени прогона без двоеточия
	 */
	EXPECT_FALSE(static_cast <bool> (regexp.build("(*sr)a")));
	EXPECT_FALSE(static_cast <bool> (regexp.build("(*srx:a)")));
}
TEST(Regex, InterfaceNonAtomicLookarounds) {
	...
	vector <pair <size_t, size_t>> bounds;
	/**
	 * @brief Ожидания сопоставления не отсекающих проверок окружения
	 *
	 */
	static const struct {
		// Выражение с не отсекающей проверкой окружения
		const char * pattern;
		// Текст сопоставления выражения
		const char * text;
		// Ожидание обнаружения совпадения
		...
		size_t begin;
		size_t finish;
		// Ожидаемые границы захвата первой группы
		size_t first;
		size_t second;
	} SAMPLES[] = {
		/**
		 * Точки возврата тела переживают выполнение проверки
		 *
		 * @details Отказ хвоста «\1c» на захвате «a» продолжается ветвью
		 *          следующей тела, дающей захвату «ab», - проверка
		 *          атомарная того не позволяет и отказывает.
		 *
		 */
		{"(?*(a|ab))\\1c", "abc", true, 0, 3, 0, 2},
		{"(?=(a|ab))\\1c", "abc", false, 0, 0, 0, 0},
		{"(?*(a|ab))\\1c", "xxabc", true, 2, 5, 2, 4},
		{"x(?*(a|ab))\\1c", "xabc", true, 0, 4, 1, 3},
		{"(?*(a|ab))\\1", "ab", true, 0, 1, 0, 1},
		{"(?*(a+))\\1", "aaaa", true, 0, 4, 0, 4},
		{"(?*(a|ab))*\\1", "ab", true, 0, 1, 0, 1},
		{"(?*(a|ab)){2}\\1c", "abc", true, 0, 3, 0, 2},
		{"(?*(?*(a|ab)))\\1c", "abc", true, 0, 3, 0, 2},
		{"(?*(a|ab))\\1(?*(c|cd))\\2", "abcd", true, 0, 3, 0, 2},
		/**
		 * Проверка ретроспективная не отсекающая перебор длин держит живым
		 *
		 * @details Длины перебираются от наибольшей наравне с проверкой
		 *          обыкновенной, однако перебор переживает выполнение
		 *          проверки: отказ хвоста испытывает длину следующую.
		 *
		 */
		{"(?<*(a|ba))c\\1", "bacba", true, 2, 5, 0, 2},
		{"(?<*(a{1,2}))b\\1", "aabaa", true, 2, 5, 0, 2},
		{"(?<*(a|ba))\\1c", "bac", false, 0, 0, 0, 0},
		/**
		 * Глагол отсечения точки тела снимает
		 *
		 * @details Глагол «(*COMMIT)», за проверкою стоящий, отсекает точки,
		 *          телом её накопленные, отчего перебор тела прекращается
		 *          и совпадения не находится вовсе.
		 *
		 */
		{"(?*(a|ab))(*COMMIT)\\1c", "abc", false, 0, 0, 0, 0},
		/**
		 * Семья глаголов буквенных именует конструкции знаковые
		 *
		 * @details «(*napla:» есть «(?*», «(*naplb:» есть «(?<*», а «(*pla:»,
		 *          «(*nla:», «(*plb:», «(*nlb:» и «(*atomic:» - «(?=»,
		 *          «(?!», «(?<=», «(?<!» и «(?>» соответственно.
		 *
		 */
		{"(*napla:(a|ab))\\1c", "abc", true, 0, 3, 0, 2},
		{"(*naplb:(a|ba))c\\1", "bacba", true, 2, 5, 0, 2},
		{"(*non_atomic_positive_lookahead:(a|ab))\\1c", "abc", true, 0, 3, 0, 2},
		{"(*pla:(a))\\1", "aa", true, 0, 1, 0, 1},
		{"(*positive_lookahead:(a))\\1", "aa", true, 0, 1, 0, 1}
	};
	/**
	 * Выполняем обход набора ожиданий сопоставления
	 */
	for(auto & sample : SAMPLES) {
		...
		EXPECT_EQ(bounds.front().first, sample.begin) << sample.pattern;
		EXPECT_EQ(bounds.front().second, sample.finish) << sample.pattern;
		// Выполняем проверку наличия границ захвата первой группы
		ASSERT_GT(bounds.size(), static_cast <size_t> (1)) << sample.pattern;
		// Выполняем проверку границ захвата первой группы
		EXPECT_EQ(bounds.at(1).first, sample.first) << sample.pattern;
		EXPECT_EQ(bounds.at(1).second, sample.second) << sample.pattern;
	}
	/**
	 * Выполняем проверку отсутствия захвата у групп буквенных
	 *
	 * @details Глагол управления захвата не выполняет, и учёт его группой
	 *          захватывающей смещал нумерацию ссылок: «(*atomic:b)»
	 *          отнимал номер у группы, за ним стоящей.
	 *
	 */
	{
		// Выполняем сборку выражения с атомарной группой буквенной
		const auto expression = regexp.build("(a)(*atomic:b)(c)\\1");
		// Выполняем проверку выполнения сборки регулярного выражения
		ASSERT_TRUE(static_cast <bool> (expression));
		// Выполняем проверку обнаружения совпадения
		ASSERT_TRUE(regexp.match("abca", expression, bounds));
		// Выполняем проверку количества захваченных групп
		EXPECT_EQ(bounds.size(), static_cast <size_t> (3));
	}
	/**
	 * Выполняем проверку отказа проверки не отсекающей условием
	 *
	 * @details Проверка не отсекающая исполняется продолжением исполнения
	 *          текущего, и отказ тела её передаётся точкам возврата,
	 *          а не инструкции проверки: ветвь невыполненного условия
	 *          при том недостижима, отчего условием проверка такая
	 *          служить не вправе - отвергает её и эталон.
	 *
	 */
	EXPECT_FALSE(static_cast <bool> (regexp.build("(?(?*a)b|c)")));
	/**
	 * Выполняем проверку отказа проверки ретроспективной неограниченной длины
	 */
	EXPECT_FALSE(static_cast <bool> (regexp.build("(?<*a+)b")));
	/**
	 * Выполняем проверку отказа глагола буквенного без двоеточия
	 *
	 * @details Имя буквенное обязано завершаться двоеточием: «(*pla)»
	 *          эталонная реализация отвергает, а «(*plx:...)» -
	 *          имя неведомое - отвергает наравне с нею.
	 *
	 */
	EXPECT_FALSE(static_cast <bool> (regexp.build("(*pla)a")));
	EXPECT_FALSE(static_cast <bool> (regexp.build("(*plx:a)a")));
}
TEST(Regex, InterfaceVariableLookbehind) {
	...
	vector <pair <size_t, size_t>> bounds;
	/**
	 * @brief Ожидания сопоставления ретроспективных проверок
	 *
	 */
	static const struct {
		// Выражение с ретроспективной проверкой переменной длины
		const char * pattern;
		// Текст сопоставления выражения
		const char * text;
		// Ожидаемые границы обнаруженного совпадения
		...
		/**
		 * Длина ссылки на захват берётся длиною указанной группы
		 *
		 * @details Группа постоянной длины даёт ссылке длину постоянную,
		 *          и проверка такая допустима: эталон её принимает.
		 *
		 */
		{"(a)(?<=\\1)", "aa", 0, 1, 0, 1},
		{"(aa)(?<=\\1)", "aa", 0, 2, 0, 2},
		...
		{"(a)(?<=x\\1)", "xa", 1, 2, 1, 2},
		/**
		 * Длины проверки перебираются от наибольшей
		 *
		 * @details Захват группы даётся последовательностью длиннейшей:
		 *          «(?<=(a{1,2})b)c» на тексте «aabc» даёт группе «aa»,
		 *          а не «a», хотя проверка удаётся при обеих длинах.
		 *
		 */
		{"(?<=(a{1,2})b)c", "aabc", 3, 4, 0, 2},
		{"(?<=(a)?b)c", "abc", 2, 3, 0, 1},
		{"x(?<=(a)?x)", "ax", 1, 2, 0, 1},
		{"(?<=(a)b)c", "abc", 2, 3, 0, 1}
	};
	...
	/**
	 * Выполняем проверку отказа проверки неограниченной длины
	 *
	 * @details Группа неограниченной длины даёт неограниченную длину и ссылке
	 *          на неё: проверка такова отвергается наравне с эталоном.
	 *
	 */
	EXPECT_FALSE(static_cast <bool> (regexp.build("(a+)(?<=\\1)")));
	EXPECT_FALSE(static_cast <bool> (regexp.build("(a*)b(?<=\\1b)")));
}
...
TEST(Regex, InterfaceFailure) {
	 ...
	 *
	 * @details Код сличается поимённо, а не утверждением «не NONE»: последнему
	 *          удовлетворяет всякий отказ, и подмена кода несовпадения скобок
	 *          кодом иным прошла бы молча.
	 *
	 */
	EXPECT_EQ(regexp.error(), regex::error_t::UNMATCHED_PAREN);
...
TEST(Regex, InterfaceCache) {
	...
	/**
	 * Выполняем проверку слабости ссылок кэша собранных выражений
	 *
	 * @details Кэш держит собранные выражения ссылкою слабой, и решение это
	 *          записано намеренным: сильная ссылка обратила бы кэш в хранилище
	 *          вечное - всякое выражение, единожды собранное, жило бы до конца
	 *          работы, - а заметить это по совпадению указателей нельзя вовсе,
	 *          ибо совпадение то есть и признак попадания в кэш, и признак
	 *          удержания сильного.
	 *
	 *          Величина наблюдаемая здесь одна: собственная ссылка слабая.
	 *          Истекает она лишь тогда, когда ссылок сильных не осталось нигде,
	 *          в том числе и внутри самого модуля.
	 *
	 */
	{
		// Заводим слабую ссылку на собранное выражение
		weak_ptr <const regex::expression_t> watcher;
		/**
		 * Выполняем сборку выражения, вызывающей стороной не удерживаемого
		 */
		{
			// Выполняем сборку регулярного выражения
			const auto lonely = regexp.build("[0-9]+кэш[0-9]+");
			// Выполняем проверку выполнения сборки регулярного выражения
			ASSERT_TRUE(static_cast <bool> (lonely));
			// Выполняем установку наблюдения за собранным выражением
			watcher = lonely;
			// Выполняем проверку удержания выражения вызывающей стороной
			EXPECT_FALSE(watcher.expired());
		}
		// Выполняем проверку освобождения выражения по отпускании его стороной
...
/**
 * @brief Проверка сопоставления литералом за пределами базовой плоскости
 *
 * @details Выражение, групп не захватывающее и ветвлений не несущее,
 *          сопоставляется поиском последовательности байтов вместо
 *          исполнения программы, и последовательность ту собирает
 *          обратная перекодировка кодовых значений в UTF-8. Ветвь
 *          четырёхбайтовая перекодировки той покрыта не была вовсе:
 *          набор не нёс ни одного выражения из знаков за пределами
 *          базовой плоскости, отчего быстрый путь сопоставления им
 *          не проверялся ничем.
 *
 */
TEST(Regex, InterfaceLiteralAstral) {
	 ...
	 */
	struct sample_t {
		const char * pattern; // Текст регулярного выражения
		const char * text;    // Текст сопоставления выражения
		bool result;          // Ожидаемый результат сопоставления
	};
	// Набор образцов сопоставления литералом
	const vector <sample_t> samples = {
		{"\xF0\x9F\x98\x80", "a\xF0\x9F\x98\x80z", true},
		{"\xF0\x9F\x98\x80", "a\xF0\x9F\x98\x81z", false},
		{"\xF0\x9F\x98\x80", "a\xF0\x9F\x98\x80", true},
		{"\xF0\x9F\x98\x80\xF0\x9F\x98\x81", "x\xF0\x9F\x98\x80\xF0\x9F\x98\x81 y", true},
		{"\xF0\x9F\x98\x80\xF0\x9F\x98\x81", "x\xF0\x9F\x98\x80\xF0\x9F\x98\x82 y", false},
		{"a\xF0\x9F\x98\x80z", "qa\xF0\x9F\x98\x80zq", true},
		{"\xD0\xB6\xF0\x9F\x98\x80", "\xD0\xB6\xF0\x9F\x98\x80", true},
		{"\xF4\x8F\xBF\xBF", "\xF4\x8F\xBF\xBF", true}
	};
	/**
	...
	for(const auto & sample : samples) {
		...
		const auto expression = regexp.build(sample.pattern, {regexp_t::flag_t::UTF});
		// Выполняем проверку сборки выражения
		ASSERT_TRUE(!!expression) << "Выражение \"" << sample.pattern << "\" не собрано";
		// Выполняем проверку сопоставления выражения тексту
		EXPECT_EQ(regexp.test(sample.text, expression), sample.result)
		 << "Выражение \"" << sample.pattern << "\" на тексте \"" << sample.text << "\"";
	}
	/**
	 * Выполняем проверку границ совпадения литералом
	 *
	 * @details Границы обязаны указывать на самый знак, а не на его начало:
	 *          обратная перекодировка, разряды кодового значения смешавшая,
	 *          дала бы совпадение длиною иной либо в позиции иной. Границы
	 *          выдаются началом и завершением совпадения, а не началом и
	 *          длиною его, отчего знак четырёхбайтовый в позиции второй
	 *          завершается позицией шестой.
	 *
	 */
	const auto expression = regexp.build("\xF0\x9F\x98\x80", {regexp_t::flag_t::UTF});
	// Выполняем проверку сборки выражения
	ASSERT_TRUE(!!expression);
	// Выполняем сопоставление выражения тексту
	const auto bounds = regexp.match("ab\xF0\x9F\x98\x80" "cd", expression);
	// Выполняем проверку наличия совпадения в тексте
	ASSERT_FALSE(bounds.empty());
	// Выполняем проверку позиции начала совпадения
	EXPECT_EQ(bounds.front().first, static_cast <size_t> (2));
	// Выполняем проверку позиции завершения совпадения
	EXPECT_EQ(bounds.front().second, static_cast <size_t> (6));
}
