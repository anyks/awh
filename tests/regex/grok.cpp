...
TEST(Grok, StorageErrors) {
	...
	for(size_t i = 0; i < record.size(); i++) {
		 ...
		 *
		 * @details Обрыв обязан ловиться заслонами длины - пустотою записи,
		 *          её обрывом либо отказом вложенного хранилища, обрыв свой
		 *          отвергающего самого. Код разбора здесь означал бы, что
		 *          заслоны обрыв пропустили, а правильность вышла случайной.
		 *
		 */
		EXPECT_TRUE((grok.error() == Grok::error_t::STORAGE_EMPTY) ||
		 (grok.error() == Grok::error_t::STORAGE_TRUNCATED) ||
		 (grok.error() == Grok::error_t::STORAGE))
		 << "оборвано на " << i << " дало код " << static_cast <uint32_t> (grok.error());
	}
	/**
...
/**
 * @brief Проверка обращений надстройки, покрытием не достигнутых
 *
 * @details Карта покрытия показала четыре обращения надстройки, набором
 *          не звучащих вовсе: выдача границ совпадения по собранному
 *          шаблону - у неё исполнялся один лишь путь отказа, - выдача
 *          набора полей по шаблону несобранному, запись выражений
 *          с набором признаков вместо разрядов и установка доверия
 *          к записи. Обращения эти вынесены наружу, отчего потребитель
 *          зовёт их наравне с прочими.
 *
 */
TEST(Grok, UnsoundedApi) {
	 ...
	 *
	 * @details Границы выдаются наравне с выражением обычным: первою парою
	 *          границы совпадения целиком, а далее границы групп захвата
	 *          в порядке их объявления.
	 *
	 */
	{
		// Выполняем сборку шаблона надстройки
		const auto exp = grok.build("%{INT:number}-%{WORD:name}");
		// Выполняем проверку сборки шаблона надстройки
		ASSERT_TRUE(!!exp);
		// Выполняем извлечение границ совпадения и захваченных групп
		const auto bounds = grok.match("код 42-alpha конец", exp);
		// Выполняем проверку наличия совпадения в тексте
		ASSERT_FALSE(bounds.empty());
		// Выполняем проверку позиции начала совпадения целиком
		EXPECT_EQ(bounds.front().first, static_cast <size_t> (7));
		// Выполняем проверку позиции завершения совпадения целиком
		EXPECT_EQ(bounds.front().second, static_cast <size_t> (15));
		// Выполняем проверку количества выданных границ
		ASSERT_EQ(bounds.size(), static_cast <size_t> (3));
		// Выполняем проверку границ первой захваченной группы
		EXPECT_EQ(bounds.at(1).first, static_cast <size_t> (7));
		// Выполняем проверку завершения первой захваченной группы
		EXPECT_EQ(bounds.at(1).second, static_cast <size_t> (9));
		// Выполняем проверку границ второй захваченной группы
		EXPECT_EQ(bounds.at(2).first, static_cast <size_t> (10));
		// Выполняем проверку завершения второй захваченной группы
		EXPECT_EQ(bounds.at(2).second, static_cast <size_t> (15));
		// Выполняем проверку отсутствия совпадения в тексте ином
		EXPECT_TRUE(grok.match("буквы и знаки", exp).empty());
	}
	/**
	 * Выполняем проверку выдачи набора полей по шаблону несобранному
	 */
	{
		// Создаём несобранный шаблон надстройки
		grok_t::exp_t none;
		// Выполняем проверку пустоты набора полей шаблона несобранного
		EXPECT_TRUE(grok.fields(none).empty());
	}
	/**
	 * Выполняем проверку записи выражений с набором признаков
	 *
	 * @details Обращение с набором признаков сводит их к разрядам и зовёт
	 *          обращение с разрядами, отчего запись обязана выйти той же,
	 *          что и у обращения с разрядами теми.
	 *
	 */
	{
		// Набор записываемых шаблонов надстройки
		const vector <string> patterns = {"%{INT:number}", "%{WORD:name}-%{INT:code}"};
		// Запись выражений, признаками набора собранная
		string listed;
		// Выполняем проверку записи выражений с набором признаков
		ASSERT_TRUE(grok.save(patterns, listed, vector <grok_t::flag_t> ()))
		 << "код " << static_cast <uint32_t> (grok.error());
		// Запись выражений, разрядами собранная
		string bitted;
		// Выполняем проверку записи выражений с разрядами признаков
		ASSERT_TRUE(grok.save(patterns, bitted, static_cast <uint32_t> (0)))
		 << "код " << static_cast <uint32_t> (grok.error());
		/**
		 * Выполняем проверку восстановления записи, признаками набора собранной
		 *
		 * @details Записи сличаются восстановлением, а не побайтово: несут они
		 *          отметку времени порождения, отчего байты их расходятся
		 *          законно, а поведение восстановленных шаблонов - нет.
		 *
		 */
		{
			// Набор восстановленных шаблонов надстройки
			vector <grok_t::exp_t> restored;
			// Выполняем проверку восстановления выражений
			ASSERT_TRUE(grok.load(listed, restored))
			 << "код " << static_cast <uint32_t> (grok.error());
			// Выполняем проверку количества восстановленных шаблонов
			ASSERT_EQ(restored.size(), patterns.size());
			// Выполняем проверку сопоставления первого восстановленного шаблона
			EXPECT_TRUE(grok.test("значение 42", restored.at(0)));
			// Выполняем проверку сопоставления второго восстановленного шаблона
			EXPECT_TRUE(grok.test("alpha-42", restored.at(1)));
		}
		// Выполняем проверку совпадения размеров записей
		EXPECT_EQ(listed.size(), bitted.size());
	}
	/**
	 * Выполняем проверку установки доверия к записи
	 *
	 * @details Доверие снимает проверку правильности программы при разборе
	 *          записи, отчего запись своя обязана восстанавливаться при нём
	 *          наравне с разбором недоверенным.
	 *
	 */
	{
		// Набор записываемых шаблонов надстройки
		const vector <string> patterns = {"%{WORD:name}"};
		// Запись выражений надстройки
		string record;
		// Выполняем проверку записи выражений
		ASSERT_TRUE(grok.save(patterns, record))
		 << "код " << static_cast <uint32_t> (grok.error());
		// Выполняем установку доверия к записи
		grok.trusted(true);
		// Набор восстановленных шаблонов надстройки
		vector <grok_t::exp_t> restored;
		// Выполняем проверку восстановления выражений при доверии к записи
		ASSERT_TRUE(grok.load(record, restored))
		 << "код " << static_cast <uint32_t> (grok.error());
		// Выполняем проверку количества восстановленных шаблонов
		ASSERT_EQ(restored.size(), patterns.size());
		// Выполняем проверку сопоставления восстановленного шаблона
		EXPECT_TRUE(grok.test("alpha", restored.at(0)));
		// Выполняем снятие доверия к записи
		grok.trusted(false);
	}
	/**
	 * Выполняем проверку слабости ссылок кэша восстановленных шаблонов
	 *
	 * @details Кэш держит восстановленные шаблоны ссылкою слабой, и решение это
	 *          записано намеренным: сильная ссылка удерживала бы весь набор
	 *          до очистки реестра, а набор этот бывает в миллион шаблонов.
	 *          Наблюдаемая величина здесь одна - собственная ссылка слабая:
	 *          истекает она лишь тогда, когда ссылок сильных не осталось
	 *          нигде, в том числе и внутри самой надстройки.
	 *
	 */
	{
		// Заводим слабую ссылку на собранный шаблон
		weak_ptr <const awh::grok::expression_t> watcher;
		/**
		 * Выполняем сборку шаблона, вызывающей стороной не удерживаемого
		 */
		{
			// Выполняем сборку шаблона надстройки
			const auto lonely = grok.build("%{WORD:слово}-%{INT:число}");
			// Выполняем проверку сборки шаблона надстройки
			ASSERT_TRUE(!!lonely);
			// Выполняем установку наблюдения за собранным шаблоном
			watcher = lonely;
			// Выполняем проверку удержания шаблона вызывающей стороной
			EXPECT_FALSE(watcher.expired());
		}
		// Выполняем проверку освобождения шаблона по отпускании его стороной
		EXPECT_TRUE(watcher.expired());
	}
}
/**
 * @brief Проверка поведения встроенного набора шаблонов
 *
 * @details Набор сличался с эталонным по именам, и все имена эталона в нём
 *          нашлись. Того, ЧТО именно шаблоны сопоставляют, не сличал никто:
 *          имя, стоящее над телом иным, проверку именами проходит молча.
 *
 *          Проверка ведёт сличение двумя способами, эталонных определений
 *          не требующими. Первый - перечень шаблонов, сопоставляемых
 *          с пустым текстом: шаблон такой принимает строку всякую,
 *          и появление в перечне имени нового означает, что тело
 *          шаблона стало всеядным. Двенадцать имён перечня всеядны
 *          по собственным определениям своим, эталону отвечающим:
 *          «DATA» и «GREEDYDATA» суть «.*», «SPACE» - «\s*», у «EXIM_QT»
 *          все части необязательны, у «RAILS3PROFILE» и «URIQUERY» тело
 *          необязательно целиком, у строк запроса «S3» и «ELB» есть
 *          отступление на «DATA», а «CISCO_REASON» несёт отступление
 *          «(?:%{WORD}\s*)*».
 *
 *          Второй - сличение образцами годными и негодными под привязкой
 *          к границам текста: привязка нужна, ибо сопоставление ведётся
 *          поиском, и негодный образец совпал бы частью своей.
 *
 */
TEST(Grok, BuiltinBehaviour) {
	 ...
	 *
	 */
	const set <string> EMPTIES = {
		"CISCO_REASON", "DATA", "ELB_REQUEST_LINE", "EXIM_QT", "GREEDYDATA",
		"HAPROXYCAPTUREDREQUESTHEADERS", "HAPROXYCAPTUREDRESPONSEHEADERS",
		"JAVALOGMESSAGE", "RAILS3PROFILE", "S3_REQUEST_LINE", "SPACE", "URIQUERY"
	};
	// Получаем набор названий встроенных шаблонов
	const auto names = grok.patterns();
	// Выполняем проверку непустоты набора названий шаблонов
	...
	for(const auto & name : names) {
		...
		// Выполняем проверку сборки шаблона надстройки
		ASSERT_TRUE(!!exp) << "шаблон \"" << name << "\" не собран, код "
		 << static_cast <uint32_t> (grok.error());
		/**
		 * Выполняем проверку соответствия всеядности шаблона перечню
		 *
		 * @details Расхождение с перечнем означает одно из двух: тело шаблона
		 *          стало всеядным - и тогда правится тело, - либо всеядность
		 *          его намеренна, и тогда имя вносится в перечень решением,
		 *          а не молча.
		 *
		 */
		EXPECT_EQ(grok.test("", exp), (EMPTIES.count(name) > 0))
		 << "шаблон \"" << name << "\" на пустом тексте";
	}
	/**
	 * @brief Образец сличения поведения встроенного шаблона
	 *
	 */
	struct sample_t {
		const char * name;   // Название встроенного шаблона
		const char * text;   // Текст сопоставления шаблона
		bool result;         // Ожидаемый результат сопоставления
	};
	// Набор образцов сличения поведения встроенных шаблонов
	const vector <sample_t> samples = {
		...
		{"IPV6", "::1", true},
		{"IPV6", "12345::", false},
		{"MAC", "00:1a:2b:3c:4d:5e", true},
		{"MAC", "00:1a:2b:3c:4d", false},
		...
		{"POSINT", "80a", false},
		{"POSINT", "0", false},
		{"HOSTPORT", "example.com:8080", true},
		{"HOSTPORT", "example.com", false},
		{"HOSTNAME", "sub.example.com", true},
		{"HOSTNAME", "-bad-.com", false},
		{"EMAILADDRESS", "info@anyks.com", true},
		{"EMAILADDRESS", "info@", false},
		{"MONTH", "September", true},
		{"MONTH", "Septemper", false},
		{"LOGLEVEL", "WARN", true},
		{"LOGLEVEL", "WARNING", true},
		{"BASE16NUM", "0x1f", true},
		{"BASE16NUM", "0xzz", false},
		{"QUOTEDSTRING", "\"abc\"", true},
	...
	for(const auto & sample : samples) {
		...
		const auto exp = grok.build(pattern);
		// Выполняем проверку сборки шаблона надстройки
		ASSERT_TRUE(!!exp) << "шаблон \"" << pattern << "\" не собран, код "
		 << static_cast <uint32_t> (grok.error());
		// Выполняем проверку сопоставления шаблона тексту образца
		EXPECT_EQ(grok.test(sample.text, exp), sample.result)
		 << "шаблон \"" << sample.name << "\" на тексте \"" << sample.text << "\"";
	}
}
/**
 * @brief Проверка кодов отказа, договором объявленных, но не закреплённых
 *
 * @details Сличение трёх перечней - кодов объявленных, кодов, порождаемых
 *          телом модуля, и кодов, проверками ожидаемых, - показало четыре
 *          кода, которые тело порождает, а не ожидает никто:
 *          «PATTERN_TOO_LARGE», «STORAGE_CHECKSUM», «STORAGE_PACKING»
 *          и «STORAGE». Код, объявленный наружу и никем не закреплённый,
 *          есть обещание потребителю, ничем не стерегомое: подмена его
 *          кодом иным - скажем, порчи содержимого кодом обрыва, - прошла
 *          бы молча, а разбирать её потребителю пришлось бы на своей
 *          стороне.
 *
 */
TEST(Grok, RefusalCodes) {
	 ...
	 *
	 * @details Ссылки разворачиваются текстом, а не указателем, отчего цепочка
	 *          удвоений даёт рост показательный: двадцать три ступени обращают
	 *          восемь байтов в объём, предел превышающий, оставаясь при этом
	 *          много ниже предела вложенности.
	 *
	 */
	{
		// Выполняем добавление основания цепочки удвоений
		ASSERT_TRUE(grok.pattern("DOUBLE0", "abcdefgh"));
		/**
		...
		for(uint32_t i = 1; i < 24; i++) {
			...
			const string body = ("%{DOUBLE" + std::to_string(i - 1) + "}%{DOUBLE" + std::to_string(i - 1) + "}");
			// Выполняем добавление очередной ступени цепочки
			ASSERT_TRUE(grok.pattern(name, body)) << "ступень " << i;
		}
		// Выполняем проверку отказа сборки шаблона, предел размера превысившего
		EXPECT_FALSE(grok.build("%{DOUBLE23}"));
		// Выполняем проверку кода отказа сборки шаблона
		EXPECT_EQ(grok.error(), Grok::error_t::PATTERN_TOO_LARGE);
	}
	/**
	 * Выполняем проверку несовпадения контрольной суммы содержимого
	 *
	 * @details Порча содержимого обязана давать несовпадение суммы: сличается
	 *          она ДО разбора, отчего разбор порченого содержимого
	 *          не начинается вовсе.
	 *
	 */
	{
		...
		string record;
		// Выполняем запись собранных шаблонов
		ASSERT_TRUE(keeper.save({"%{IP:client}", "%{NUMBER:code:int}"}, record))
		 << "код " << static_cast <uint32_t> (keeper.error());
		/**
		 * Получаем позицию содержимого части записи, шаблоны описывающей
		 *
		 * @details Заголовок несёт опознание восемью байтами, издание устройства
		 *          записи, метод сжатия, два размера содержимого числами
		 *          переменной длины и контрольную сумму восемью байтами.
		 *          Числа переменной длины читаются здесь так же, как их пишет
		 *          сам модуль: по семь разрядов на байт, старший разряд -
		 *          признак продолжения.
		 *
		 */
		size_t offset = 10;
		...
		for(uint8_t number = 0; number < 2; number++) {
			 ...
			 */
			while((offset < record.size()) && ((static_cast <uint8_t> (record[offset++]) & 0x80) != 0))
				// Продолжаем чтение числа переменной длины
				continue;
		}
		// Выполняем переход за контрольную сумму содержимого части записи
		offset += 8;
		// Выполняем проверку размещения содержимого в пределах записи
		ASSERT_LT(offset, record.size());
		// Получаем запись с испорченным содержимым
		string damaged = record;
		// Выполняем порчу первого байта содержимого части записи
		damaged[offset] = static_cast <char> (damaged[offset] ^ 0x5A);
		// Набор восстановленных шаблонов Grok
		vector <Grok::exp_t> expressions;
		// Выполняем проверку отказа восстановления записи
		EXPECT_FALSE(keeper.load(damaged, expressions));
		// Выполняем проверку кода отказа восстановления записи
		EXPECT_EQ(keeper.error(), Grok::error_t::STORAGE_CHECKSUM);
	}
	/**
	 * Выполняем проверку невыполненного разбора сжатого содержимого
	 *
	 * @details Сумма сличается прежде разжатия, отчего запись сюда доходит
	 *          целой, и отказ принадлежит самому обработчику разжатия.
	 *
	 */
	{
		// Создаём объект разбора текста по шаблонам Grok
		...
		string record;
		// Выполняем запись собранных шаблонов со сжатием
		ASSERT_TRUE(keeper.save({"%{IP:client}"}, record))
		 << "код " << static_cast <uint32_t> (keeper.error());
		// Выполняем установку обработчика разжатия, отвечающего отказом
		keeper.packer(compressor::method_t::LZ4, &reversed,
		 [](string_view, string &) noexcept -> bool { return false; });
		// Набор восстановленных шаблонов Grok
		vector <Grok::exp_t> expressions;
		// Выполняем проверку отказа восстановления записи
		EXPECT_FALSE(keeper.load(record, expressions));
		// Выполняем проверку кода отказа восстановления записи
		EXPECT_EQ(keeper.error(), Grok::error_t::STORAGE_PACKING);
	}
	/**
	 * Выполняем проверку отказа вложенного хранилища выражений
	 *
	 * @details Запись хранилища собранных выражений приложена к записи шаблонов
	 *          в конце, и порча её отвергается самим хранилищем, а надстройка
	 *          выводит отказ тот наружу кодом своим.
	 *
	 */
	{
		// Создаём объект разбора текста по шаблонам Grok
		...
		ASSERT_TRUE(keeper.save({"%{IP:client}"}, record))
		 << "код " << static_cast <uint32_t> (keeper.error());
		// Получаем запись с испорченным хвостом
		string damaged = record;
		// Выполняем порчу последнего байта записи хранилища выражений
		...
		EXPECT_FALSE(keeper.load(damaged, expressions));
		// Выполняем проверку кода отказа восстановления записи
		EXPECT_EQ(keeper.error(), Grok::error_t::STORAGE);
	}
}

/**
 * @brief Проверка круга записи шаблонов через файловую систему
 *
 * @details Надстройка файловой системы не касается вовсе - ни при чтении
 *          шаблонов из каталога, ни при хранении записи: и то и другое
 *          возложено на вызывающую сторону, объект «awh::fs_t». Проверка
 *          закрепляет круг «сохранил в файл - прочитал - восстановил»,
 *          которым пользуется всякий потребитель.
 *
 *          Восстановленные шаблоны обязаны работать наравне с собранными
 *          из текста, отчего сличение ведётся не количеством, а извлечёнными
 *          значениями полей: количество совпадает и у записи, полей
 *          растерявшей.
 *
 */
TEST(Grok, StorageFilesystem) {
	...
	// Запись собранных шаблонов
	string record;
	// Выполняем запись набора собранных шаблонов
	ASSERT_TRUE(grok.save({"%{IP:client} %{WORD:method}", "%{TIMESTAMP_ISO8601:время}"}, record))
	 << "код " << static_cast <uint32_t> (grok.error());
	// Выполняем проверку непустоты записи
	ASSERT_FALSE(record.empty());
	// Выполняем запись набора собранных шаблонов в файл
	...
	// Прочитанная из файла запись собранных шаблонов
	string readed;
	// Выполняем чтение записи собранных шаблонов из файла
	...
	vector <grok_t::exp_t> restored;
	// Выполняем восстановление собранных шаблонов
	ASSERT_TRUE(grok.load(readed, restored)) << "код " << static_cast <uint32_t> (grok.error());
	// Выполняем проверку количества восстановленных шаблонов
	ASSERT_EQ(restored.size(), static_cast <size_t> (2));
	// Набор извлечённых значений полей
	unordered_map <string, string> values;
	// Выполняем извлечение полей восстановленным шаблоном
	ASSERT_TRUE(grok.exec("10.0.0.7 POST", restored.front(), values));
	// Выполняем проверку значений полей, восстановленным шаблоном извлечённых
	EXPECT_EQ(values["client"], "10.0.0.7");
	EXPECT_EQ(values["method"], "POST");
	// Выполняем очистку набора извлечённых значений полей
	values.clear();
	// Выполняем извлечение полей вторым восстановленным шаблоном
	ASSERT_TRUE(grok.exec("2026-09-09T12:00:00Z", restored.back(), values));
	// Выполняем проверку названия поля, письменностью русскою заданного
	EXPECT_EQ(values["время"], "2026-09-09T12:00:00Z");
	// Выполняем удаление файла записи собранных шаблонов
