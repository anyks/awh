...
/**
 * @brief Метод проверки отсутствия переаллокаций в установившемся потоковом режиме
 *
 * @details Инвариант, на который опирается сборка исходящих сообщений HTTP: после
 *          выхода на рабочий объём цикл prepare/commit/clear обязан переиспользовать
 *          уже выделенную память, не трогая аллокатор и не переинициализируя хвост
 *
 */
TEST_F(BufferFixture, SteadyStateNoReallocationBufferTest){
	...
	/**
	 * Выполняем прогрев буфера, чтобы он вышел на рабочий объём
	 */
	for(size_t i = 0; i < 4; i++){
		// Резервируем место под порцию в хвосте буфера
		void * area = buffer.prepare(PORTION);
		// Проверяем что резервирование выполнено
		ASSERT_NE(area, nullptr);
		// Заполняем зарезервированную область данными
		std::memset(area, static_cast <int> ('a' + i), PORTION);
		// Фиксируем записанную порцию
		ASSERT_EQ(buffer.commit(PORTION), PORTION);
		// Освобождаем отданную порцию
		buffer.clear();
	}
	// Запоминаем объём выделенной памяти после прогрева
	const size_t capacity = buffer.capacity();
	// Проверяем что рабочий объём выделен
	ASSERT_GE(capacity, PORTION);
	/**
	 * Прогоняем через буфер заведомо больший объём данных
	 */
	for(size_t i = 0; i < 1000; i++){
		// Резервируем место под порцию в хвосте буфера
		void * area = buffer.prepare(PORTION);
		...
		ASSERT_NE(area, nullptr);
		// Заполняем зарезервированную область данными
		std::memset(area, static_cast <int> ('x'), PORTION);
		// Фиксируем записанную порцию
		ASSERT_EQ(buffer.commit(PORTION), PORTION);
		// Проверяем что порция доступна целиком
		ASSERT_EQ(buffer.size(), PORTION);
		// Освобождаем отданную порцию
		buffer.clear();
	}
	// Проверяем что прокачка 16 МБ данных не потребовала ни одной переаллокации
	ASSERT_EQ(buffer.capacity(), capacity);
}

/**
 * @brief Метод проверки роста хранилища с запасом при потоковой дозаписи
 *
 * @details Рост ровно до требуемого объёма означал бы переаллокацию на каждой
 *          дозаписи - потоковая запись состоит из множества мелких дозаписей подряд
 *
 */
TEST_F(BufferFixture, GeometricGrowthBufferTest){
	...
	/**
	 * Выполняем множество мелких дозаписей подряд без извлечения данных
	 */
	for(size_t i = 0; i < 4096; i++){
		// Дописываем очередную мелкую порцию данных
		ASSERT_TRUE(buffer.push(std::string("0123456789")));
		// Если объём выделенной памяти вырос
		if(buffer.capacity() != previous){
			// Учитываем увеличение выделенной памяти
			++growths;
			// Запоминаем новый объём выделенной памяти
			previous = buffer.capacity();
		}
	}
	// Проверяем что все данные на месте
	ASSERT_EQ(buffer.size(), (4096u * 10u));
	/**
	 * Проверяем что рост оказался логарифмическим: при росте ровно до требуемого
	 * объёма увеличений было бы на порядки больше
	 */
	ASSERT_LT(growths, 32u);
}

/**
 * @brief Метод проверки ограничения фиксации зарезервированным объёмом
 *
 * @details Хранилище растёт с запасом, поэтому свободного места в хвосте больше
 *          запрошенного - фиксация сверх резервирования протащила бы в буфер
...
TEST_F(BufferFixture, CommitBeyondReservationBufferTest){
	...
	buffer.clear();
	// Резервируем место под небольшую порцию данных
	void * area = buffer.prepare(16);
	// Проверяем что резервирование выполнено
	ASSERT_NE(area, nullptr);
	// Заполняем зарезервированную область данными
	std::memset(area, static_cast <int> ('z'), 16);
	// Проверяем что фиксация сверх зарезервированного объёма ограничена резервированием
	ASSERT_EQ(buffer.commit(4096), 16u);
	// Проверяем что в буфер попала ровно зарезервированная порция
	ASSERT_EQ(buffer.size(), 16u);
	// Проверяем что повторная фиксация без резервирования ничего не добавляет
	ASSERT_EQ(buffer.commit(16), 0u);
	// Проверяем что размер данных буфера не изменился
	ASSERT_EQ(buffer.size(), 16u);
}
