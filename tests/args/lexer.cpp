/**
 * @file lexer.cpp
 * @date 2026-09-02
 *
 * @license{LicenseRef-AWH-1.0}
 *
 * @author Yuriy Lobarev
 *
 * @telegram{forman}
 * @phone{+7 (910) 983-95-90}
 *
 * @email forman@anyks.com
 * @site https://anyks.com
 *
 * @brief Автоматические тесты разборщика параметров запуска — записи параметров всех
 *        поддержанных видов, позиционные доводы, признак конца именованных параметров,
 *        разрез текстового потока на слова и коды отказов разбора
 *
 * @copyright Copyright © 2026
 *
 */

/**
 * Стандартные заголовочные файлы
 */
#include <string>
#include <vector>

/**
 * Подключаем заголовочные файлы проекта
 */
#include <args/lexer.hpp>

/**
 * Подключаем заголовочные файлы тестового окружения
 */
#include "../main.hpp"

/**
 * Используем стандартное пространство имён
 */
using namespace std;
using namespace awh;
using namespace awh::args;

/**
 * @brief Внутренние помощники набора проверок разборщика
 *
 */
namespace {
	/**
	 * @brief Снимок разобранной лексемы, содержимое копирующий
	 *
	 * @details Лексема разбора ссылается на поданный набор и живёт не дольше его,
	 *          оттого проверки хранят её снимком, а не самой лексемой
	 *
	 */
	typedef struct Shot {
		// Вид лексемы разбора
		token_t type;
		// Признак наличия значения у именованного параметра
		bool assigned;
		// Имя параметра без ведущих тире
		string key;
		// Значение параметра либо содержимое позиционного довода
		string value;
		// Порядковый номер довода в поданном наборе
		size_t index;
		/**
		 * @brief Конструктор
		 *
		 * @param lexeme лексема разбора для снятия снимка
		 *
		 */
		Shot(const lexeme_t & lexeme) noexcept :
		 type(lexeme.type), assigned(lexeme.assigned),
		 key(lexeme.key), value(lexeme.value), index(lexeme.location.index) {}
	} shot_t;

	/**
	 * @brief Метод разбора набора доводов со снятием снимков лексем
	 *
	 * @param lexer  разборщик параметров запуска
	 * @param items  набор доводов запуска
	 * @param shots  контейнер снимков разобранных лексем
	 * @param errors контейнер кодов отказов разбора
	 * @return       результат разбора
	 *
	 */
	bool collect(const lexer_t & lexer, const vector <string> & items, vector <shot_t> & shots, vector <args::error_t> & errors) noexcept {
		// Выполняем очистку контейнера снимков разобранных лексем
		shots.clear();
		// Выполняем очистку контейнера кодов отказов разбора
		errors.clear();
		// Выполняем разбор набора доводов запуска
		return lexer.parse(items, [&shots](const lexeme_t & lexeme) noexcept -> bool {
			// Выполняем снятие снимка разобранной лексемы
			shots.emplace_back(lexeme);
			// Сообщаем, что разбор следует продолжить
			return true;
		}, [&errors](const args::error_t error, const location_t &) noexcept -> bool {
			// Выполняем запоминание кода отказа разбора
			errors.push_back(error);
			// Сообщаем, что разбор следует продолжить
			return true;
		});
	}
}

/**
 * @brief Проверка описания всех кодов ошибок разбора
 *
 */
TEST(ArgsLexer, Messages) {
	/**
	 * Выполняем перебор всех кодов ошибок разбора
	 *
	 * @warning Верхний предел перебора берётся ПОСЛЕДНИМ членом перечня, а не тем, что
	 *          стоял последним в день написания
	 *
	 * @warning Предупреждение это словами своими не убереглось: предел стоял на
	 *          `FILESYSTEM`, а за ним успели встать `REQUIRED` и `CLUSTER` - два кода
	 *          не проверялись ВОВСЕ, и запись при том уверяла, будто предел взят
	 *          последним членом. Оттого ниже стоит утверждение, блюдущее себя само:
	 *          код ЗА последним членом обязан отвечать общим описанием, и заведение
	 *          нового кода роняет проверку в ту же минуту, покуда описание ему не
	 *          отведено. Приём принят от кодека TOML, где он завёлся тем же путём -
	 *          из находки, что предупреждение словами не блюдётся
	 */
	for(uint8_t i = 0; i <= static_cast <uint8_t> (args::error_t::CLUSTER); i++){
		// Получаем описание очередного кода ошибки разбора
		const char * text = args::message(static_cast <args::error_t> (i));
		// Выполняем проверку наличия описания кода ошибки
		ASSERT_NE(text, nullptr);
		// Выполняем проверку того, что описание кода ошибки не пусто
		ASSERT_FALSE(string(text).empty()) << static_cast <uint32_t> (i);
		/**
		 * Выполняем проверку того, что описание коду ОТВЕДЕНО, а не подставлено общим:
		 * выдача отвечает на код неизвестный строкою «unknown error», и она не пуста
		 */
		ASSERT_STRNE(text, "unknown error") << static_cast <uint32_t> (i);
	}
	// Выполняем проверку выдачи общего описания коду, отведённого не имеющему
	ASSERT_STREQ(args::message(static_cast <args::error_t> (0xFF)), "unknown error");
	/**
	 * Выполняем проверку того, что перечень кончился ИМЕННО ЗДЕСЬ
	 *
	 * @warning Утверждение это и есть застава от гниения предела: заведи кто новый код
	 *          за `CLUSTER` - и описание ему найдётся, а утверждение упадёт, требуя
	 *          подвинуть предел перебора. Без него новый код молча остался бы
	 *          непроверенным, как случилось с `REQUIRED` и `CLUSTER`
	 */
	ASSERT_STREQ(args::message(static_cast <args::error_t> (static_cast <uint8_t> (args::error_t::CLUSTER) + 1)), "unknown error");
}

/**
 * @brief Проверка разбора записи параметра со значением через знак равенства
 *
 */
TEST(ArgsLexer, AssignedForm) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер снимков разобранных лексем
	vector <shot_t> shots;
	// Контейнер кодов отказов разбора
	vector <args::error_t> errors;
	// Выполняем разбор записей параметра обоими видами тире
	ASSERT_TRUE(collect(lexer, {"--name=value", "-other=second"}, shots, errors));
	// Выполняем проверку отсутствия отказов разбора
	ASSERT_TRUE(errors.empty());
	// Выполняем проверку числа разобранных лексем
	ASSERT_EQ(shots.size(), 2);
	// Выполняем проверку вида первой разобранной лексемы
	ASSERT_EQ(shots.at(0).type, token_t::PARAM);
	// Выполняем проверку имени первого разобранного параметра
	ASSERT_EQ(shots.at(0).key, "name");
	// Выполняем проверку значения первого разобранного параметра
	ASSERT_EQ(shots.at(0).value, "value");
	// Выполняем проверку признака наличия значения у первого параметра
	ASSERT_TRUE(shots.at(0).assigned);
	// Выполняем проверку имени второго разобранного параметра
	ASSERT_EQ(shots.at(1).key, "other");
	// Выполняем проверку значения второго разобранного параметра
	ASSERT_EQ(shots.at(1).value, "second");
}

/**
 * @brief Проверка разбора записи параметра со значением следующим доводом
 *
 */
TEST(ArgsLexer, SeparatedForm) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер снимков разобранных лексем
	vector <shot_t> shots;
	// Контейнер кодов отказов разбора
	vector <args::error_t> errors;
	// Выполняем разбор записей параметра обоими видами тире
	ASSERT_TRUE(collect(lexer, {"--name", "value", "-other", "second"}, shots, errors));
	// Выполняем проверку отсутствия отказов разбора
	ASSERT_TRUE(errors.empty());
	// Выполняем проверку числа разобранных лексем
	ASSERT_EQ(shots.size(), 2);
	// Выполняем проверку имени первого разобранного параметра
	ASSERT_EQ(shots.at(0).key, "name");
	// Выполняем проверку значения первого разобранного параметра
	ASSERT_EQ(shots.at(0).value, "value");
	// Выполняем проверку имени второго разобранного параметра
	ASSERT_EQ(shots.at(1).key, "other");
	// Выполняем проверку значения второго разобранного параметра
	ASSERT_EQ(shots.at(1).value, "second");
}

/**
 * @brief Проверка независимости разбора от положения параметра в наборе
 *
 */
TEST(ArgsLexer, OrderIndependence) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер снимков разобранных лексем
	vector <shot_t> shots;
	// Контейнер кодов отказов разбора
	vector <args::error_t> errors;
	// Выполняем разбор набора, где записи перемешаны с позиционными доводами
	ASSERT_TRUE(collect(lexer, {"first", "--name=value", "second", "-other", "third"}, shots, errors));
	// Выполняем проверку отсутствия отказов разбора
	ASSERT_TRUE(errors.empty());
	// Выполняем проверку числа разобранных лексем
	ASSERT_EQ(shots.size(), 4);
	// Выполняем проверку вида первой разобранной лексемы
	ASSERT_EQ(shots.at(0).type, token_t::OPERAND);
	// Выполняем проверку содержимого первого позиционного довода
	ASSERT_EQ(shots.at(0).value, "first");
	// Выполняем проверку имени разобранного параметра
	ASSERT_EQ(shots.at(1).key, "name");
	// Выполняем проверку содержимого второго позиционного довода
	ASSERT_EQ(shots.at(2).value, "second");
	// Выполняем проверку имени второго разобранного параметра
	ASSERT_EQ(shots.at(3).key, "other");
	// Выполняем проверку взятия следующего довода значением параметра
	ASSERT_EQ(shots.at(3).value, "third");
}

/**
 * @brief Проверка разбора параметра, значения не имеющего
 *
 */
TEST(ArgsLexer, FlagForm) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер снимков разобранных лексем
	vector <shot_t> shots;
	// Контейнер кодов отказов разбора
	vector <args::error_t> errors;
	// Выполняем разбор набора двух признаков, идущих подряд
	ASSERT_TRUE(collect(lexer, {"--verbose", "--debug"}, shots, errors));
	// Выполняем проверку отсутствия отказов разбора
	ASSERT_TRUE(errors.empty());
	// Выполняем проверку числа разобранных лексем
	ASSERT_EQ(shots.size(), 2);
	/**
	 * Выполняем проверку того, что первый признак второго НЕ СЪЕЛ: иначе всякий
	 * признак съедал бы следующий за ним параметр, и набор менял бы смысл от
	 * перестановки
	 */
	ASSERT_FALSE(shots.at(0).assigned);
	// Выполняем проверку имени первого разобранного признака
	ASSERT_EQ(shots.at(0).key, "verbose");
	// Выполняем проверку отсутствия значения у второго признака
	ASSERT_FALSE(shots.at(1).assigned);
	// Выполняем проверку имени второго разобранного признака
	ASSERT_EQ(shots.at(1).key, "debug");
}

/**
 * @brief Проверка отличия поданного пустого значения от значения, не поданного вовсе
 *
 */
TEST(ArgsLexer, EmptyValue) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер снимков разобранных лексем
	vector <shot_t> shots;
	// Контейнер кодов отказов разбора
	vector <args::error_t> errors;
	// Выполняем разбор записи с пустым значением и записи без значения вовсе
	ASSERT_TRUE(collect(lexer, {"--name=", "--other"}, shots, errors));
	// Выполняем проверку отсутствия отказов разбора
	ASSERT_TRUE(errors.empty());
	// Выполняем проверку числа разобранных лексем
	ASSERT_EQ(shots.size(), 2);
	// Выполняем проверку того, что пустое значение поданным считается
	ASSERT_TRUE(shots.at(0).assigned);
	// Выполняем проверку пустоты поданного значения
	ASSERT_TRUE(shots.at(0).value.empty());
	// Выполняем проверку того, что значение не подано вовсе
	ASSERT_FALSE(shots.at(1).assigned);
}

/**
 * @brief Проверка взятия отрицательного числа значением параметра
 *
 */
TEST(ArgsLexer, NegativeNumber) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер снимков разобранных лексем
	vector <shot_t> shots;
	// Контейнер кодов отказов разбора
	vector <args::error_t> errors;
	// Выполняем разбор записи параметра с отрицательным числом значением
	ASSERT_TRUE(collect(lexer, {"--count", "-5", "--rate", "-1.5e-3"}, shots, errors));
	// Выполняем проверку отсутствия отказов разбора
	ASSERT_TRUE(errors.empty());
	// Выполняем проверку числа разобранных лексем
	ASSERT_EQ(shots.size(), 2);
	// Выполняем проверку взятия целого отрицательного числа значением
	ASSERT_EQ(shots.at(0).value, "-5");
	// Выполняем проверку взятия дробного отрицательного числа значением
	ASSERT_EQ(shots.at(1).value, "-1.5e-3");
	// Создаём настройки разбора параметров
	lexer_t::settings_t settings;
	// Снимаем признак взятия числа значением параметра
	settings.negative = false;
	// Создаём разборщик параметров запуска с изменёнными настройками
	lexer_t plain;
	// Устанавливаем настройки разбора параметров
	plain.settings(settings);
	// Выполняем разбор того же набора с отключённым признаком
	ASSERT_TRUE(collect(plain, {"--count", "-5"}, shots, errors));
	// Выполняем проверку того, что число значением более не берётся
	ASSERT_EQ(shots.size(), 2);
	// Выполняем проверку отсутствия значения у параметра
	ASSERT_FALSE(shots.at(0).assigned);
	// Выполняем проверку разбора числа именованным параметром
	ASSERT_EQ(shots.at(1).key, "5");
}

/**
 * @brief Проверка признака конца именованных параметров
 *
 */
TEST(ArgsLexer, Terminus) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер снимков разобранных лексем
	vector <shot_t> shots;
	// Контейнер кодов отказов разбора
	vector <args::error_t> errors;
	// Выполняем разбор набора с признаком конца именованных параметров
	ASSERT_TRUE(collect(lexer, {"--name=value", "--", "--other=second"}, shots, errors));
	// Выполняем проверку отсутствия отказов разбора
	ASSERT_TRUE(errors.empty());
	// Выполняем проверку числа разобранных лексем
	ASSERT_EQ(shots.size(), 3);
	// Выполняем проверку вида лексемы признака конца параметров
	ASSERT_EQ(shots.at(1).type, token_t::TERMINUS);
	/**
	 * Выполняем проверку того, что довод за признаком конца разобран позиционным
	 * ЦЕЛИКОМ, а не именованным параметром
	 */
	ASSERT_EQ(shots.at(2).type, token_t::OPERAND);
	// Выполняем проверку содержимого позиционного довода
	ASSERT_EQ(shots.at(2).value, "--other=second");
}

/**
 * @brief Проверка отказа разбора при пустом имени параметра
 *
 */
TEST(ArgsLexer, EmptyKeyFailure) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер снимков разобранных лексем
	vector <shot_t> shots;
	// Контейнер кодов отказов разбора
	vector <args::error_t> errors;
	// Выполняем разбор набора с пустым именем параметра
	ASSERT_TRUE(collect(lexer, {"--=value", "--name=value"}, shots, errors));
	// Выполняем проверку числа отказов разбора
	ASSERT_EQ(errors.size(), 1);
	// Выполняем проверку кода отказа разбора
	ASSERT_EQ(errors.at(0), args::error_t::EMPTY_KEY);
	/**
	 * Выполняем проверку того, что разбор отказом НЕ ПРЕРВАН: набор разбирается
	 * целиком, чтобы приложение показало сразу все огрехи набора
	 */
	ASSERT_EQ(shots.size(), 1);
	// Выполняем проверку имени разобранного параметра
	ASSERT_EQ(shots.at(0).key, "name");
}

/**
 * @brief Проверка разреза текстового потока на слова
 *
 */
TEST(ArgsLexer, TextSplit) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер слов, собранных разрезом текста
	vector <string> items;
	// Выполняем разрез текста с кавычками и обратной косой
	ASSERT_TRUE(lexer.split("  --name=\"first second\"   --other='third'  \\ fourth ", items));
	// Выполняем проверку числа собранных слов
	ASSERT_EQ(items.size(), 3);
	// Выполняем проверку объединения кавычками двойными
	ASSERT_EQ(items.at(0), "--name=first second");
	// Выполняем проверку объединения кавычками одинарными
	ASSERT_EQ(items.at(1), "--other=third");
	// Выполняем проверку снятия особого значения обратной косой
	ASSERT_EQ(items.at(2), " fourth");
}

/**
 * @brief Проверка совпадения разбора текстового потока с разбором набора запуска
 *
 */
TEST(ArgsLexer, TextMatchesArgv) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер снимков лексем, разобранных из набора запуска
	vector <shot_t> shots;
	// Контейнер кодов отказов разбора
	vector <args::error_t> errors;
	// Выполняем разбор набора доводов запуска
	ASSERT_TRUE(collect(lexer, {"--name", "first second", "-flag", "operand"}, shots, errors));
	// Контейнер снимков лексем, разобранных из текстового потока
	vector <shot_t> stream;
	// Выполняем разбор того же самого набора, поданного текстом
	ASSERT_TRUE(lexer.parse("--name \"first second\" -flag operand", [&stream](const lexeme_t & lexeme) noexcept -> bool {
		// Выполняем снятие снимка разобранной лексемы
		stream.emplace_back(lexeme);
		// Сообщаем, что разбор следует продолжить
		return true;
	}));
	// Выполняем проверку совпадения числа разобранных лексем
	ASSERT_EQ(shots.size(), stream.size());
	// Выполняем перебор всех разобранных лексем
	for(size_t i = 0; i < shots.size(); i++){
		// Выполняем проверку совпадения вида лексемы
		ASSERT_EQ(shots.at(i).type, stream.at(i).type) << i;
		// Выполняем проверку совпадения признака наличия значения
		ASSERT_EQ(shots.at(i).assigned, stream.at(i).assigned) << i;
		// Выполняем проверку совпадения имени параметра
		ASSERT_EQ(shots.at(i).key, stream.at(i).key) << i;
		// Выполняем проверку совпадения значения параметра
		ASSERT_EQ(shots.at(i).value, stream.at(i).value) << i;
	}
}

/**
 * @brief Проверка отказов разбора текстового потока
 *
 */
TEST(ArgsLexer, TextFailures) {
	// Создаём объект работы с логами
	// Создаём разборщик параметров запуска
	const lexer_t lexer;
	// Контейнер слов, собранных разрезом текста
	vector <string> items;
	// Контейнер кодов отказов разбора
	vector <args::error_t> errors;
	// Создаём отзыв извещения об отказе разбора
	const lexer_t::failure_t failure = [&errors](const args::error_t error, const location_t &) noexcept -> bool {
		// Выполняем запоминание кода отказа разбора
		errors.push_back(error);
		// Сообщаем, что разбор следует продолжить
		return true;
	};
	// Выполняем разрез текста с незакрытой кавычкой
	ASSERT_TRUE(lexer.split("--name=\"first", items, failure));
	// Выполняем проверку числа отказов разбора
	ASSERT_EQ(errors.size(), 1);
	// Выполняем проверку кода отказа разбора
	ASSERT_EQ(errors.at(0), args::error_t::UNPAIRED);
	// Выполняем очистку контейнера кодов отказов разбора
	errors.clear();
	// Выполняем разрез текста, оканчивающегося обратной косой
	ASSERT_TRUE(lexer.split("--name=value\\", items, failure));
	// Выполняем проверку числа отказов разбора
	ASSERT_EQ(errors.size(), 1);
	// Выполняем проверку кода отказа разбора
	ASSERT_EQ(errors.at(0), args::error_t::DANGLING);
}

/**
 * @brief Проверка одиночного тире позиционным доводом
 *
 * @warning Проверка эта ловит ОТКАЗ на законном обращении. Одиночное тире есть
 *          позиционный довод, а не имя параметра: описание POSIX отводит ему
 *          значение «поток ввода», и наречия `cat -`, `tar -xf -`, `grep образец
 *          -` стоят на нём. Прежде оно попадало в общую ветвь доводов из одних
 *          лишь тире и отвечалось отказом `EMPTY_KEY` - приложение, такое
 *          обращение принимающее, отвергало его вовсе. Замерено 15.09.2026
 *          аудитом
 *
 * @note Двойное тире тем же отказом отвечается по-прежнему при снятой настройке
 *       `terminus`: у него значения позиционного довода нет ни у одного наречия,
 *       и пустым именем оно и является - утверждение о том держит вторую половину
 *       договора
 *
 */
TEST(ArgsLexer, ASingleDashIsAnOperand) {
	// Создаём объект разбора доводов запуска
	lexer_t lexer;
	/**
	 * Набор доводов запуска
	 *
	 * @warning Набор держится ПЕРЕМЕННОЙ, а не подаётся временным: содержимое
	 *          лексемы берётся ссылками на поданный набор и живёт не дольше него -
	 *          договор этот объявлен при самой лексеме. Собери проверка лексемы и
	 *          посмотри на них после смерти временного набора - смотрела бы она на
	 *          мусор, и утверждение падало бы не на дефекте, а на себе самой
	 */
	const vector <string> items = {string("-"), string("--вход=-"), string("--"), string("-")};
	// Виды собранных лексем разбора
	vector <token_t> kinds;
	// Содержимое собранных лексем разбора
	vector <string> values;
	// Число извещений об отказе разбора
	size_t failures = 0;
	// Выполняем разбор набора доводов запуска
	const bool result = lexer.parse(items, [&kinds, &values](const lexeme_t & lexeme) -> bool {
		// Выполняем добавление вида разобранной лексемы
		kinds.push_back(lexeme.type);
		// Выполняем добавление содержимого разобранной лексемы КОПИЕЙ
		values.push_back(string(lexeme.value));
		// Продолжаем разбор набора дальше
		return true;
	}, [&failures](const args::error_t, const location_t) -> bool {
		// Увеличиваем число извещений об отказе разбора
		failures++;
		// Продолжаем разбор набора дальше
		return true;
	});
	// Выполняем проверку успешности разбора
	ASSERT_TRUE(result) << "разбор набора с одиночным тире отвечен отказом";
	// Выполняем проверку отсутствия извещений об отказе
	ASSERT_EQ(failures, static_cast <size_t> (0)) << "одиночное тире отвечено отказом разбора";
	// Выполняем проверку числа собранных лексем
	ASSERT_EQ(kinds.size(), static_cast <size_t> (4)) << "число собранных лексем не сошлось";
	// Выполняем проверку того, что первое тире стало позиционным доводом
	ASSERT_EQ(kinds.front(), token_t::OPERAND) << "одиночное тире не стало позиционным доводом";
	// Выполняем проверку содержимого позиционного довода
	ASSERT_EQ(values.front(), string("-")) << "содержимое позиционного довода утрачено";
	// Выполняем проверку того, что значением параметра тире тоже годно
	ASSERT_EQ(values.at(1), string("-")) << "одиночное тире значением параметра не принято";
	// Выполняем проверку того, что двойное тире осталось концом именованных
	ASSERT_EQ(kinds.at(2), token_t::TERMINUS) << "двойное тире концом именованных быть перестало";
	// Выполняем проверку того, что тире за концом именованных стало доводом
	ASSERT_EQ(kinds.at(3), token_t::OPERAND) << "тире за концом именованных доводом не стало";
	/**
	 * Выполняем проверку двойного тире при снятой настройке конца именованных
	 *
	 * @warning Утверждение это держит ВТОРУЮ половину договора: правка касается
	 *          лишь ОДИНОЧНОГО тире, а двойное пустым именем быть не перестало
	 */
	lexer_t other;
	// Получаем настройки разбора доводов запуска
	lexer_t::settings_t settings = other.settings();
	// Снимаем признак разбора конца именованных параметров
	settings.terminus = false;
	// Устанавливаем настройки разбора доводов запуска
	other.settings(settings);
	// Число извещений об отказе разбора
	size_t refusals = 0;
	// Набор доводов запуска из одного двойного тире
	const vector <string> second = {string("--")};
	// Выполняем разбор набора из одного двойного тире
	static_cast <void> (other.parse(second, [](const lexeme_t &) -> bool {
		// Продолжаем разбор набора дальше
		return true;
	}, [&refusals](const args::error_t error, const location_t) -> bool {
		// Если отказом оказалось пустое имя параметра
		if(error == args::error_t::EMPTY_KEY)
			// Увеличиваем число извещений об отказе разбора
			refusals++;
		// Продолжаем разбор набора дальше
		return true;
	}));
	// Выполняем проверку отказа на двойном тире
	ASSERT_EQ(refusals, static_cast <size_t> (1)) << "двойное тире при снятой настройке отказом не отвечено";
}

/**
 * @brief Проверка того, что предел лексем ограждает и текстовую дорогу
 *
 * @details Одна лексема забирает не более двух слов, и слов сверх удвоенного предела
 * в предел лексем не уложится никак. Прежде предел действовал лишь у разбора уже
 * нарезанного набора, а разрез текста шёл до конца всегда: потребитель, оградивший
 * себя пределом от подачи недоброй, ограждён не был вовсе
 *
 */
TEST(ArgsLexer, TheTokenLimitGuardsTheTextRoadToo){
	// Создаём разборщик параметров запуска
	lexer_t lexer;
	// Извлекаем настройки разбора
	lexer_t::settings_t settings = lexer.settings();
	// Устанавливаем предел числа лексем разбора
	settings.maxTokens = 4;
	// Устанавливаем настройки разбора
	lexer.settings(settings);
	// Собираемый текст подачи
	string text = "";
	// Собираем текст, предел лексем заведомо превышающий
	for(size_t i = 0; i < 2048; i++)
		// Добавляем к тексту очередной именованный параметр
		text.append("--key" + std::to_string(i) + " ");
	// Число выданных лексем разбора
	size_t count = 0;
	// Код отказа разбора, полученный извещением
	args::error_t error = args::error_t::NONE;
	// Выполняем разбор собранного текста
	const bool result = lexer.parse(string_view(text), [&count](const args::lexeme_t &) noexcept -> bool {
		// Увеличиваем число выданных лексем разбора
		count++;
		// Сообщаем, что разбор следует продолжить
		return true;
	}, [&error](const args::error_t code, const args::location_t &) noexcept -> bool {
		// Запоминаем код отказа разбора
		error = code;
		// Сообщаем, что разбор следует остановить
		return false;
	});
	// Выполняем проверку того, что разбор отвечен отказом
	ASSERT_FALSE(result) << "подача сверх предела лексем принята";
	// Выполняем проверку того, что отказ назван превышением числа лексем
	ASSERT_EQ(error, args::error_t::MANY_TOKENS) << "отказ назван причиной иною";
	/**
	 * Выполняем проверку того, что лексем не выдано вовсе
	 *
	 * @warning Проверка «лексем не более предела» СЛЕПА: столько же выдаётся и без
	 *          ограды разреза, ибо предел разбора отказывает следом. Отличает ограду
	 *          именно то, что текстовая дорога отвечает отказом ДО выдачи
	 */
	ASSERT_EQ(count, static_cast <size_t> (0)) << "разрез текста прошёл мимо предела лексем";
}

/**
 * @brief Проверка того, что кавычка одинарная бережёт обратную косую
 *
 * @details Таково правило оболочек POSIX: внутри кавычки одинарной косая особого
 * значения не имеет, и потребитель ставит её именно затем, чтобы уберечь косые.
 * Прежде косая снималась и там - запись пути MS Windows либо выражения
 * регулярного теряла косые МОЛЧА, отказа разрез не выдавал
 *
 */
TEST(ArgsLexer, TheSingleQuoteKeepsTheBackslash){
	// Создаём разборщик параметров запуска
	lexer_t lexer;
	// Контейнер собранных слов разреза
	vector <string> words;
	// Выполняем разрез текста с путём и выражением под кавычкой одинарной
	ASSERT_TRUE(lexer.split("--path 'C:\\Program\\dir' --mask '\\d+'", words));
	// Выполняем проверку числа собранных слов
	ASSERT_EQ(words.size(), static_cast <size_t> (4)) << "разрез собрал слова иначе";
	// Выполняем проверку того, что косые пути уцелели
	ASSERT_EQ(words.at(1), "C:\\Program\\dir") << "кавычка одинарная косые не уберегла";
	// Выполняем проверку того, что косая выражения уцелела
	ASSERT_EQ(words.at(3), "\\d+") << "кавычка одинарная косую выражения не уберегла";
	// Выполняем очистку контейнера собранных слов
	words.clear();
	/**
	 * Выполняем разрез текста с косою под кавычкой двойной
	 *
	 * @note Внутри кавычки двойной косая работает по-прежнему: ею вносится сама
	 *       кавычка двойная, иначе внести её было бы нечем
	 */
	ASSERT_TRUE(lexer.split("\"a\\\"b\" \"c\\\\d\"", words));
	// Выполняем проверку числа собранных слов
	ASSERT_EQ(words.size(), static_cast <size_t> (2)) << "разрез собрал слова иначе";
	// Выполняем проверку того, что кавычка двойная внесена косою
	ASSERT_EQ(words.at(0), "a\"b") << "косая под кавычкой двойной особого значения лишилась";
	// Выполняем проверку того, что косая внесена косою
	ASSERT_EQ(words.at(1), "c\\d") << "косая под кавычкой двойной внесена иначе";
	// Выполняем очистку контейнера собранных слов
	words.clear();
	/**
	 * Выполняем разрез записи, кавычку одинарную вносящей
	 *
	 * @note Оболочки POSIX кавычку одинарную внутрь себя внести не дают вовсе, и
	 *       запись `'a\'b'` есть сцепка трёх кусков - «a\», «b» и пустого хвоста
	 */
	ASSERT_TRUE(lexer.split("'a\\'b'", words));
	// Выполняем проверку числа собранных слов
	ASSERT_EQ(words.size(), static_cast <size_t> (1)) << "разрез собрал слова иначе";
	// Выполняем проверку того, что сцепка собрана правилом оболочек
	ASSERT_EQ(words.at(0), "a\\b") << "сцепка кусков собрана иначе";
}
