...
TEST(ArgsLexer, Messages) {
	 ...
	 *
	 * @warning Предупреждение это словами своими не убереглось: предел стоял на
	 *          `FILESYSTEM`, а за ним успели встать `REQUIRED` и `CLUSTER` - два кода
	 *          не проверялись ВОВСЕ, и запись при том уверяла, будто предел взят
	 *          последним членом. Оттого ниже стоит утверждение, блюдущее себя само:
	 *          код ЗА последним членом обязан отвечать общим описанием, и заведение
	 *          нового кода роняет проверку в ту же минуту, покуда описание ему не
