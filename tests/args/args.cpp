...
/**
 * @brief Проверка согласия числа значений с извлечением перечнем
 *
 * @warning Находка аудита 14.09.2026: число значений бралось прямо у дерева, и
 *          одиночное значение давало НУЛЬ, тогда как извлечение перечнем на том же
 *          значении давало одну запись. Потребитель, перебирающий значения счётом
...
TEST(Args, SizeAgreesWithArrayExtraction){
	...
	ASSERT_TRUE(args.parse({"--one=1", "--many=a", "--many=b", "--flag"}));
	// Выполняем проверку того, что одиночное значение считается одним, а не нулём
	ASSERT_EQ(args.size("one"), size_t(1)) << "одиночное значение сочтено нулём значений";
	// Выполняем проверку того, что взведённый признак считается одним значением
	ASSERT_EQ(args.size("flag"), size_t(1)) << "взведённый признак сочтён нулём значений";
	// Выполняем проверку числа значений вместимого
	ASSERT_EQ(args.size("many"), size_t(2)) << "число значений вместимого сочтено неверно";
	// Выполняем проверку того, что параметр, не поданный вовсе, даёт нуль
	ASSERT_EQ(args.size("неподанный"), size_t(0)) << "неподанный параметр сочтён поданным";
	// Выполняем проверку согласия числа значений с извлечением перечнем
	ASSERT_EQ(args.size("one"), args.arr <string> ("one").size()) << "число значений разошлось с извлечением перечнем";
	// Выполняем ту же проверку для вместимого параметра
	ASSERT_EQ(args.size("many"), args.arr <string> ("many").size()) << "число значений вместимого разошлось с извлечением перечнем";
}

/**
 * @brief Проверка извлечения записью значения, уложенного своим видом
 *
 * @details Вывод вида значения включён настройкою по умолчанию, оттого `--port=8080`
 *          ложится числом. Извлечение записью обязано такое значение выразить, а не
 *          отвечать пустотой, неотличимой от параметра, не поданного вовсе
 *
 * @warning Находка аудита 14.09.2026: запас этот стоял у извлечения логического
...
TEST(Args, StringExtractionRendersTypedValues){
	...
	ASSERT_TRUE(args.parse({"--num=8080", "--neg=-42", "--big=18446744073709551615",
		"--real=3.5", "--flag", "--off=false", "--nil=null", "--text=привет"}));
	// Выполняем проверку извлечения записью целого без знака
	ASSERT_EQ(args.get <string> ("num"), string("8080")) << "число записью не выражено";
	// Выполняем проверку извлечения записью целого со знаком
	ASSERT_EQ(args.get <string> ("neg"), string("-42")) << "число со знаком записью не выражено";
	// Выполняем проверку извлечения записью целого, разрядность знаковых превзошедшего
	ASSERT_EQ(args.get <string> ("big"), string("18446744073709551615")) << "большое число записью не выражено";
	// Выполняем проверку извлечения записью числа дробного
	ASSERT_EQ(args.get <string> ("real"), string("3.5")) << "дробное число записью не выражено";
	// Выполняем проверку извлечения записью взведённого признака
	ASSERT_EQ(args.get <string> ("flag"), string("true")) << "взведённый признак записью не выражен";
	// Выполняем проверку извлечения записью признака, сброшенного значением
	ASSERT_EQ(args.get <string> ("off"), string("false")) << "сброшенный признак записью не выражен";
	// Выполняем проверку того, что уложенная запись выдаётся как есть
	ASSERT_EQ(args.get <string> ("text"), string("привет")) << "уложенная запись искажена";
	/**
	 * Выполняем проверку того, что пустое значение записью НЕ выражается
	 *
	 * @note Пустое значение означает «значения нет», и запись «null» здесь означала
	 *       бы обратное - что значение есть и равно этому слову
	 */
	ASSERT_TRUE(args.get <string> ("nil").empty()) << "пустое значение выражено записью";
}
...
/**
 * @brief Проверка ограждения знаков, у пути хранения особых
 *
 * @warning Проверка эта ловит ПОДМЕНУ ИМЕНИ. Имя параметра уходило в путь оси
 *          хранения КАК ЕСТЬ, и два знака толковались осью по-своему: имя `a/b`
 *          давало лишний УРОВЕНЬ, хотя разделителем звеньев настройками задана
 *          точка, а имя `a~1b` ложилось полем, названным `a/b`, - отменяющая
 *          запись RFC 6901 срабатывала у потребителя, её не писавшего. Два
 *          разных имени вели к одному месту, и собранное дерево уходило в файл
 *          настроек с именами, коих потребитель не задавал. Замерено 15.09.2026
 *          аудитом
 *
 * @note Спрос ведётся ДЕРЕВОМ, а не ходами `has()` и `get()`: те переводят имя
 *       в путь тем же ходом, и подмена у них сокращалась бы - неверное имя
 *       разыскивалось бы по неверному же пути и отвечало бы истиной. Видна
 *       подмена лишь в собранном дереве, куда смотрит файл настроек
 *
 */
TEST(Args, KeyNamesAreEscapedForTheStorageAxis) {
	// Создаём объект сбора параметров запуска
	args_t args;
	// Выполняем установку значения с косою чертой в имени
	ASSERT_TRUE(args.fallback(string_view("a/b"), string_view("первое"))) << "установка имени с косою чертой отвечена отказом";
	// Выполняем установку значения с отменяющей записью в имени
	ASSERT_TRUE(args.fallback(string_view("a~1b"), string_view("второе"))) << "установка имени с тильдой отвечена отказом";
	// Выполняем проверку того, что оба имени разысканы
	ASSERT_TRUE(args.has(string_view("a/b"))) << "имя с косою чертой не разыскано";
	ASSERT_TRUE(args.has(string_view("a~1b"))) << "имя с тильдой не разыскано";
	// Выполняем проверку того, что значения не перепутаны
	ASSERT_EQ(args.get <string> (string_view("a/b")), string("первое")) << "значение имени с косою чертой подменено";
	ASSERT_EQ(args.get <string> (string_view("a~1b")), string("второе")) << "значение имени с тильдой подменено";
	// Собираемая запись дерева
	string text = "";
	// Выполняем перевод собранного дерева в запись JSON
	ASSERT_TRUE(args.dump(text, codec::Bridge::format_t::JSON)) << "перевод дерева в запись JSON отвечен отказом";
	/**
	 * Выполняем проверку того, что имена легли в дерево ДОСЛОВНО
	 *
	 * @warning Утверждение это ловит лишний уровень: прежде имя `a/b` давало
	 *          отображение `a` с полем `b` внутри
	 */
	ASSERT_NE(text.find("\"a/b\""), string::npos) << "имя с косою чертой в дерево дословно не легло; собрано: " << text;
	ASSERT_NE(text.find("\"a~1b\""), string::npos) << "имя с тильдой в дерево дословно не легло; собрано: " << text;
	// Выполняем проверку того, что лишнего уровня не завелось
	ASSERT_EQ(text.find("\"a\":"), string::npos) << "имя с косою чертой дало лишний уровень; собрано: " << text;
	// Создаём объект сбора параметров запуска для проверки вложенности
	args_t nested;
	// Выполняем установку значения с разделителем звеньев в имени
	ASSERT_TRUE(nested.fallback(string_view("сеть.порт"), string_view("8080"))) << "установка вложенного имени отвечена отказом";
	// Собираемая запись дерева вложенности
	string second = "";
	// Выполняем перевод собранного дерева в запись JSON
	ASSERT_TRUE(nested.dump(second, codec::Bridge::format_t::JSON)) << "перевод дерева вложенности отвечен отказом";
	/**
	 * Выполняем проверку того, что вложенность по разделителю уцелела
	 *
	 * @note Ограждение идёт звеньям, а не пути целиком: разделитель звеньев
	 *       обращается в косую черту ПОСЛЕ ограждения, иначе он был бы огорожен сам
	 */
	ASSERT_NE(second.find("\"сеть\""), string::npos) << "вложенность по разделителю утрачена; собрано: " << second;
	ASSERT_NE(second.find("\"порт\""), string::npos) << "звено вложенности утрачено; собрано: " << second;
}

/**
 * @brief Проверка сохранности имён полей файла настроек при слиянии
 *
 * @warning Проверка эта ловит РАЗРУШЕНИЕ ИМЁН НА ПУТИ ВНУТРЬ. Имя поля приходит
 *          из файла настроек, и тильда с косою чертою в нём законны - и JSON, и
 *          YAML имена такие несут. Прежде имя уходило в путь оси хранения как
 *          есть: поле `a/b` файла ложилось в дерево ДВУМЯ уровнями, а поле
 *          `a~1b` - полем `a/b`. Обратная же выдача уносила в файл имена, коих
 *          в нём не было. Замерено 15.09.2026 аудитом
 *
 * @note Изъян тот же, что у перевода имени параметра в путь, и чинен он в ОБЩЕМ
 *       месте: ход ограждения звена один на оба пути внутрь - и от потребителя, и
 *       из файла
 *
 */
TEST(Args, SettingsFieldNamesSurviveTheMerge) {
	// Создаём объект сбора параметров запуска
	args_t args;
	// Запись настроек с именами полей, особые знаки несущими
	const string text = "{\"a/b\": \"первое\", \"a~1b\": \"второе\", \"сеть\": {\"порт\": 8080}}";
	// Выполняем разбор записи настроек
	ASSERT_TRUE(args.config(string_view(text), codec::Bridge::format_t::JSON)) << "разбор записи настроек отвечен отказом";
	// Выполняем проверку того, что оба имени разысканы
	ASSERT_TRUE(args.has(string_view("a/b"))) << "поле с косою чертою не разыскано";
	ASSERT_TRUE(args.has(string_view("a~1b"))) << "поле с тильдой не разыскано";
	// Выполняем проверку того, что значения не перепутаны
	ASSERT_EQ(args.get <string> (string_view("a/b")), string("первое")) << "значение поля с косою чертою подменено";
	ASSERT_EQ(args.get <string> (string_view("a~1b")), string("второе")) << "значение поля с тильдой подменено";
	// Собираемая запись обратной выдачи
	string dump = "";
	// Выполняем перевод собранного дерева в запись JSON
	ASSERT_TRUE(args.dump(dump, codec::Bridge::format_t::JSON)) << "перевод дерева в запись JSON отвечен отказом";
	/**
	 * Выполняем проверку того, что имена файла уцелели ДОСЛОВНО
	 *
	 * @warning Утверждение это ловит лишний уровень: прежде поле `a/b` давало
	 *          отображение `a` с полем `b` внутри, и файл настроек, поданный
	 *          обратно, нёс имена иные, нежели поданный внутрь
	 */
	ASSERT_NE(dump.find("\"a/b\""), string::npos) << "имя поля с косою чертою утрачено; собрано: " << dump;
	ASSERT_NE(dump.find("\"a~1b\""), string::npos) << "имя поля с тильдой утрачено; собрано: " << dump;
	// Выполняем проверку того, что лишнего уровня не завелось
	ASSERT_EQ(dump.find("\"a\":"), string::npos) << "поле с косою чертою дало лишний уровень; собрано: " << dump;
	// Выполняем проверку того, что вложенность файла уцелела
	ASSERT_NE(dump.find("\"сеть\""), string::npos) << "вложенность файла утрачена; собрано: " << dump;
	// Выполняем проверку того, что значение вложенного поля разыскано
	ASSERT_EQ(args.get <string> (string_view("сеть.порт")), string("8080")) << "вложенное значение утрачено";
}

/**
 * @brief Проверка повтора параметра при настройке без сбора массивов
 *
 * @details Признаков кратности в модуле ДВА: настройка сбора говорит, собирается
 * ли повтор массивом, а описание параметра - дозволен ли повтор вовсе. Настройка
 * СТАРШЕ описания - решение владельца от 15.09.2026
 *
 * @warning Проверка эта закрепляет сочетание, прежде разбиравшееся молча:
 *          описание повтор дозволяет, настройка массивов не велит. Отказа тут
 *          нет - повтор дозволен, - но прежнее значение уходит, и уход этот
 *          обязан быть ОГЛАШЁН: прежде запись `--порт=1 --порт=2` оставляла
 *          двойку, а единица пропадала бесследно, без отказа и без строки в
 *          журнале
 *
 * @note Отказ `DUPLICATE` здесь был бы ложью: он означает «повтор запрещён», а
 *       запрещён лишь сбор массивом
 *
 */
TEST(Args, TheRepeatWithoutArraysKeepsTheLastValue) {
	// Создаём объект сбора параметров запуска
	args_t args;
	// Получаем настройки сбора параметров запуска
	args_t::settings_t settings = args.settings();
	// Устанавливаем запрет сбора повторов массивом
	...
	param.name = "порт";
	// Устанавливаем потребность значения параметра
	param.value = schema_t::value_t::REQUIRED;
	// Устанавливаем дозволенность повторной подачи параметра
	param.multiple = true;
	// Выполняем добавление описания ожидаемого параметра
	ASSERT_TRUE(args.schema().add(param)) << "добавление описания параметра отвечено отказом";
	// Набор доводов запуска с повторно поданным параметром
	const vector <string> items = {string("--порт=1"), string("--порт=2")};
	/**
	 * Выполняем разбор набора доводов запуска
	 *
	 * @warning Утверждение это ловит ЛОЖНЫЙ ОТКАЗ: повтор описанием дозволен, и
	 *          отвечать на него отказом нельзя
	 */
	ASSERT_TRUE(args.parse(items)) << "повтор, описанием дозволенный, отвечен отказом";
	// Выполняем проверку того, что отказов не накоплено
	ASSERT_TRUE(args.errors().empty()) << "повтор, описанием дозволенный, записан отказом";
	// Выполняем проверку того, что победило последнее поданное значение
	ASSERT_EQ(args.get <string> (string_view("порт")), string("2")) << "повтор без массивов оставил не последнее значение";
	// Выполняем проверку того, что массив не собран
	ASSERT_EQ(args.size(string_view("порт")), static_cast <size_t> (1)) << "массив собран вопреки настройке";
	// Создаём объект сбора параметров запуска с дозволенным сбором массивов
	args_t collected;
	// Выполняем добавление того же описания ожидаемого параметра
	ASSERT_TRUE(collected.schema().add(param)) << "добавление описания параметра отвечено отказом";
	// Выполняем разбор того же набора доводов запуска
	ASSERT_TRUE(collected.parse(items)) << "разбор при дозволенном сборе массивов отвечен отказом";
	/**
	 * Выполняем проверку сбора повтора массивом
	 *
	 * @note Утверждение это держит ВТОРУЮ половину договора: настройка старше
	 *       описания лишь в запрете, а при дозволении сбор идёт как прежде
	 */
...
/**
 * @brief Проверка того, что круг записи ABC замыкается и по имени файла
 *
 * @details Выдача `save(путь, ABC)` проходила исправно, а чтение того же файла по
 * имени отвечалось отказом «unknown format», тогда как всякий иной вид записи по
 * имени читался. Потребителю оставалось звать чтение с указанием вида - о чём ему
 * было знать неоткуда
 *
 */
TEST(Args, TheABCRecordClosesItsCircleByFileNameToo){
	...
	args_t args;
	// Выполняем разбор набора доводов запуска
	ASSERT_TRUE(args.parse({"--host", "localhost", "--port", "8080"}));
	// Получаем путь к файлу настроек записи ABC
	const string filename = "./probe-circle.abc";
	// Выполняем запись дерева настроек записью контейнера ABC
	ASSERT_TRUE(args.save(filename, codec::Bridge::format_t::ABC)) << "выдача записи ABC отвечена отказом";
	// Создаём объект сбора для чтения записанного файла
	args_t back;
	// Выполняем чтение файла настроек по имени его
	const bool result = back.filename(filename);
	// Выполняем снос файла настроек до всякой проверки
	::remove(filename.c_str());
	// Выполняем проверку того, что чтение по имени прошло
	ASSERT_TRUE(result) << "запись ABC по имени файла не прочтена";
	// Выполняем проверку того, что настройки прочтены теми же
	ASSERT_EQ(back.get <string> ("host"), "localhost") << "настройка круга не уцелела";
	ASSERT_EQ(back.get <string> ("port"), "8080") << "настройка круга не уцелела";
}

/**
 * @brief Проверка того, что строгость спрашивается всяким источником
 *
 * @details Строгость спрашивалась лишь у дорог набора запуска и текста, ибо обе идут
 * через укладку по описанию, тогда как окружение и файл настроек ложатся в дерево
 * напрямую. Одна и та же настройка давала два разных ответа: имя `unknown` из
 * набора отвечалось отказом, а то же имя переменной окружения либо поля файла
...
TEST(Args, TheStrictnessIsAskedByEverySource){
	 ...
	 *
	 * @note Дороги эти ходят разными путями - набор и текст через описание, а
	 *       окружение с файлом напрямую, - и договор у них обязан быть один
	 */
	for(uint8_t i = 0; i < 4; i++){
		...
		args.settings(settings);
		// Выполняем заведение единственного описания ожидаемого параметра
		ASSERT_TRUE(args.schema().add("host", 'h', schema_t::value_t::REQUIRED));
		// Признак успешности подачи имени, описанию неизвестного
		bool result = true;
		// Определяем дорогу подачи настроек
		switch(i){
			// Выполняем подачу набором доводов запуска
			case 0: result = args.parse({"--unknown", "1"}); break;
			// Выполняем подачу текстовым потоком
			case 1: result = args.text("--unknown 1"); break;
			// Выполняем подачу файлом настроек записью JSON
			case 2: result = args.config("{\"unknown\":1}", codec::Bridge::format_t::JSON); break;
			// Выполняем подачу значением по умолчанию
			case 3: result = args.fallback("unknown", "1"); break;
		}
		// Выполняем проверку того, что подача отвечена отказом
		ASSERT_FALSE(result) << "дорога " << static_cast <uint32_t> (i) << " приняла имя, описанию неизвестное";
		// Выполняем проверку того, что отказ назван неизвестностью имени
		ASSERT_FALSE(args.errors().empty()) << "дорога " << static_cast <uint32_t> (i) << " отказала без причины";
		ASSERT_EQ(args.errors().back().first, args::error_t::UNKNOWN) << "дорога " << static_cast <uint32_t> (i) << " назвала причину иною";
		// Выполняем проверку того, что настройка не заведена вовсе
		ASSERT_FALSE(args.has("unknown")) << "дорога " << static_cast <uint32_t> (i) << " завела настройку, описанию неизвестную";
		/**
		 * Выполняем подачу имени, описанию ведомого, тою же дорогой
		 *
		 * @note Строгость обязана отвергать неведомое, а не всякое: дорога, у которой
		 *       и ведомое имя не проходит, слепа так же, как и та, что пропускает всё
		 */
		switch(i){
			// Выполняем подачу набором доводов запуска
			case 0: result = args.parse({"--host", "localhost"}); break;
			// Выполняем подачу текстовым потоком
			case 1: result = args.text("--host localhost"); break;
			// Выполняем подачу файлом настроек записью JSON
			case 2: result = args.config("{\"host\":\"localhost\"}", codec::Bridge::format_t::JSON); break;
			// Выполняем подачу значением по умолчанию
			case 3: result = args.fallback("host", "localhost"); break;
		}
		// Выполняем проверку того, что имя, описанию ведомое, принято
		ASSERT_TRUE(result) << "дорога " << static_cast <uint32_t> (i) << " отвергла имя, описанию ведомое";
