...
/**
 * @brief Подставной сервер времени для проверки NTP-клиента
 *
 * @details Отвечает на вопросы сам, своей нитью. Отличие от сервера имён одно, но
 *          существенное: примета своего ответа у NTP - переписанная в ответ метка
 *          отправки из вопроса, и портится в подставном ответе именно она
 *
 */
class NTPStubServer {
	private:
		...
		uint16_t _port;
	private:
		// Признак того, что метку вопроса в ответ переписывать не следует
		bool _foreign;
	private:
		// Признак работы нити сервера
		std::atomic <bool> _running;
	...
	public:
		 ...
		 *
		 */
		bool start(const bool foreign) noexcept;
		/**
		 * @brief Метод остановки сервера
		 *
		 */
