...
bool DNSStubServer::start(const uint32_t delay, const bool silent, const uint32_t foreign) noexcept {
	...
	/**
	 * Отключаем отчёт о недоступности получателя
	 *
	 * @warning У MS Windows дейтаграммное гнездо, получившее отказ ICMP на СВОЙ прежний
	 *          ответ, отдаёт следующему приёму отказ `WSAECONNRESET`, и круг приёма
	 *          отбрасывает его наравне с истёкшим сроком - вопрос, стоявший в очереди,
	 *          при этом пропадает, а проверка недосчитывается его. Ровно этой бедой у
	 *          поддельных маршрутизаторов `portmap` терялась дейтаграмма, оттого вызов
	 *          там и появился; у подставных серверов имён его недоставало
	 *
	 * @note На системах POSIX вызов пуст и ничего не делает
	 */
	::disableConnectionReset(this->_fd);
