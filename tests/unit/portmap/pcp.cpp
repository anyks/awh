...
class PcpRouter {
	...
	public:
		...
		void stop() noexcept {
			 ...
			 *          проверка недосчитывалась её. Закрыть гнездо раньше значило бы и
			 *          оставить потоку приём по сброшенному дескриптору, а то и по
			 *          чужому, если система успеет выдать тот же номер новому гнезду
			 *
			 ...
			 *       же доводу устроены поддельный маршрутизатор NAT-PMP, молчащий
			 *       маршрутизатор в `retry.cpp` и подставные серверы в `dns/dns.cpp`
			 */
...
TEST_F(PortmapUnitFixture, PortmapPcpDamagedAnswer) {
	...
	for(const pair_t & pair : {
		...
		/**
		 * Испытание повторяется, покуда просьбы не дойдут до маршрутизатора
		 *
		 * @details Стенд Windows ARM64 теряет дейтаграммы по петле: примерно каждый пятый
		 *          прогон не досчитывался просьбы. Установлено щупами, а не догадкой -
		 *          модуль отправляет ОБЕ просьбы, движок обращается к `sendto` дважды,
		 *          система принимает по 60 октетов из 60 и отказа не даёт, получатель у
		 *          обеих `127.0.0.1:5351`, порты отправителя разные, ошибок приёма нет,
		 *          а до гнезда доходит одна. Эталон снимает вопрос о нашем коде: на стенде
		 *          x86-64 тот же двоичный код даёт 0 отказов из 40, на ARM64 - 7 из 40
		 *
		 * @warning Повтор проверку НЕ ослабляет: модуль, повторяющий просьбу, доходит до
		 *          нужного числа хотя бы с одного захода, а модуль, сдающийся после первой
		 *          попытки, не дойдёт ни с одного - и проверка откажет по-прежнему.
		 *          Отброшены замером: привязка маршрутизатора к петле вместо любого адреса
		 *          (стало ХУЖЕ - 12 отказов из 40), перехват порта, отсев коротких просьб,
		 *          потеря в очереди движка и снос узла с непустой очередью
		 */
		// Итог обращения к маршрутизатору
		outcome_t outcome;
		// Число принятых маршрутизатором просьб
		int32_t requests = 0;
		/**
		 * Выполняем испытание, повторяя его при потере дейтаграммы
		 */
		...
		::setup(portmap);
		// Выполняем ожидание итога обращения к маршрутизатору
		outcome = this->await(portmap, awh::unit::portmap_t::action_t::OPEN);
		/**
		...
		// Выполняем остановку поддельного маршрутизатора
		router.stop();
		// Запоминаем число принятых маршрутизатором просьб
		requests = router.requests();
		// Если просьбы дошли, повторять испытание незачем
		if(requests >= pair.requests) break;
		}
		 ...
		 */
		ASSERT_GE(requests, pair.requests) << "вид порчи " << static_cast <int32_t> (pair.damage) << ", потрачено " << outcome.spent << " мс";
		// Выполняем проверку того, что просьба сверх положенного не повторялась
		if(pair.requests == 1) ASSERT_EQ(requests, 1) << "вид порчи " << static_cast <int32_t> (pair.damage);
	}
}
