...
static void setup(awh::unit::portmap_t & portmap) noexcept {
	 ...
	 *
	 * @note Берётся оно у поддельного шлюза: тот встал на устройстве местной сети,
	 *       и рассылка обнаружения обязана уйти им же. Прежде обмен вёлся петлёй,
	 *       и проверки пропускались на системах, не доставляющих рассылку через неё
	 */
	portmap.setIface(FakeIGD::deviceName().empty() ? LOOPBACK_IFACE : FakeIGD::deviceName().c_str());
	/**
	 * Запрещаем рассылке обнаружения покидать машину
	 *
	 * @note Это и есть замена петли: нулевой предел числа переходов оставляет просьбу
	 *       внутри машины, и живой маршрутизатор сети её не увидит. Без этого испытание
	 *       завело бы на нём перенаправление порта, о котором никто не просил
	 */
