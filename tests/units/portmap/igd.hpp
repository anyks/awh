...
/**
 * @brief Название сетевого устройства петли
 *
 * @details Имя устройства петли у систем разное: BSD и macOS зовут его `lo0`, Linux -
 *          `lo`. Имя чужой системы приводит к тому, что устройство не находится вовсе,
 *          рассылка обнаружения уходит мимо него, а проверки, опирающиеся на неё,
 *          отказывают либо виснут в ожидании ответа, которому взяться неоткуда
 *
 * @note Имя заведено здесь единым для всех проверок обращения к устройству доступа в
 *       сеть: прежде оно стояло вписанным в каждой из них порознь, и расхождение по
