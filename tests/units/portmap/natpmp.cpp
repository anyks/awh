 ...
 *
 * @note Проверкой закрепляется и режим срока ожидания ответа: движок держит его
 *       ожиданием ответа, снимая первым же чтением, и отброшенная дейтаграмма
 *       оставила бы обмен без срока вовсе - ни ответа, ни отказа, ни повтора. Модуль
 *       обращает этот срок пределом простоя, и отказ приходит должным образом
 *
 */
