 ...
 */
#include "../../../include/units/timer.hpp"
...
TEST_F(DNSUnitFixture, DnsLateAnswerDiscarded) {
	 ...
	 *
	 * @note Задержка берётся вдвое больше срока ожидания у единственной попытки:
	 *       ответ обязан прийти заведомо после того, как резолвер отчитался отказом
	 */
	ASSERT_TRUE(server.start(TEST_DELAY * 2, false));
	// Создаём объект DNS-резолвера
	awh::unit::dns_t dns(awh::event::family_t::IPV4, this->_fmk.get(), this->_log.get());
	// Выполняем настройку резолвера на подставной сервер имён
	this->setup(dns, server, TEST_DELAY, 1);
	 ...
	 *
	 * @warning Проверка эта обязана в наборе идти **последней** из тех, что заводят
	 *          таймер: узел таймера, отработавший в прогоне предыдущем, оставляет учёт
	 *          сроков движка таким, что сроки ожидания чтения в прогоне следующем не
	 *          срабатывают вовсе. Резолвер тогда отказом не отчитывается, запоздалый
	 *          ответ застаёт вопрос ещё ожидающим и принимается за настоящий - то
	 *          есть проверка падает, показывая дефект движка, а не резолвера
	 */
...
TEST_F(DNSUnitFixture, DnsSilentServerAttempts) {
	...
	ASSERT_TRUE(result.failed);
	/**
	 * Выполняем проверку того, что обращений было столько, сколько задано попыток
	 *
	 * @note Сличается нижняя граница, а не точное число: повтор ведёт сам резолвер,
	 *       и обращений он вправе сделать не меньше заданного
	 */
	ASSERT_GE(server.received(), static_cast <uint32_t> (3));
}
