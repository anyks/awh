...
TEST_F(ParserFixture, SendDefaultHeadersTest){
	...
	/**
	 * Проверяем что перевод строки внутри идентификации сервиса на провод не попадает
	 *
	 ...
	 * поле в каждом собранном сообщении. Непригодная составляющая отвергается в точке
	 * входа данных в контейнер, поэтому сообщение собирается - но с прежней
	 * идентификацией, а не с подставленной
	 */
	{
		// Создаём объект парсера-отправителя ответа
		auto sender = this->make(direct_t::RESPONSE);
		...
		// Получаем собранные байты исходящего сообщения
		const std::string wire{sender->pending()};
		// Проверяем что сообщение собрано
		ASSERT_FALSE(wire.empty());
		// Проверяем что подставленное поле на провод не попало
		ASSERT_EQ(wire.find("X-Injected"), std::string::npos) << wire;
	}
}
/**
