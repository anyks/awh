 ...
 *
 */
TEST_F(Quic2Fixture, VarintReferenceTest){
	// Прочитанное число
	uint64_t value = 0;
 ...
 *
 */
TEST_F(Quic2Fixture, VarintWriteTest){
 ...
 *
 */
TEST_F(Quic2Fixture, DecodePacketNumberTest){
 ...
 *
 */
TEST_F(Quic2Fixture, PacketNumberSizeTest){
 ...
 *
 */
TEST_F(Quic2Fixture, ParseClientInitialTest){
 ...
 *
 */
TEST_F(Quic2Fixture, LongHeaderRoundTripTest){
 ...
 *
 */
TEST_F(Quic2Fixture, ShortHeaderRoundTripTest){
	// Выходной буфер сборки
	std::string output;
 ...
 *
 */
TEST_F(Quic2Fixture, VersionNegotiationRoundTripTest){
	// Выходной буфер сборки
	std::string output;
	// Идентификаторы соединения
	const cid_t dcid = this->makeCid(this->unhex("0102030405060708"));
 ...
 *
 */
TEST_F(Quic2Fixture, RetryRoundTripTest){
	// Выходной буфер сборки
	std::string output;
	// Идентификаторы соединения
 ...
 *
 */
TEST_F(Quic2Fixture, MalformedPacketTest){
	// Разобранный заголовок пакета
	packet::header_t header;
	// Код ошибки транспорта
	error_t error = error_t::NO_ERROR;
 ...
 *
 */
TEST_F(Quic2Fixture, AckFrameRoundTripTest){
	// Выходной буфер сборки
	std::string output;
 ...
 *
 */
TEST_F(Quic2Fixture, AckEcnFrameRoundTripTest){
	// Выходной буфер сборки
	std::string output;
	// Фрейм подтверждения приёма пакетов
	frame::ack_t source;
 ...
 *
 */
TEST_F(Quic2Fixture, AckFrameMalformedTest){
 ...
 *
 */
TEST_F(Quic2Fixture, StreamFrameFinalSizeTest){
	/**
 ...
 *
 */
TEST_F(Quic2Fixture, CryptoFrameRoundTripTest){
	// Выходной буфер сборки
	std::string output;
 ...
 *
 */
TEST_F(Quic2Fixture, NewConnectionIdFrameRoundTripTest){
	// Выходной буфер сборки
	std::string output;
 ...
 *
 */
TEST_F(Quic2Fixture, ConnectionCloseFrameRoundTripTest){
	// Выходной буфер сборки
	std::string output;
 ...
 *
 */
TEST_F(Quic2Fixture, FrameSequenceTest){
 ...
 *
 */
TEST_F(Quic2Fixture, RouteLongHeaderTest){
	...
	ASSERT_TRUE(awh::quic2::route(reinterpret_cast <const uint8_t *> (datagram.data()), datagram.size(), 8, key));
	...
	ASSERT_TRUE(awh::quic2::route(reinterpret_cast <const uint8_t *> (datagram.data()), datagram.size(), 20, other));
 ...
 *
 */
TEST_F(Quic2Fixture, RouteShortHeaderTest){
	// Ключ маршрутизации датаграммы
	awh::net::origin_key_t key;
	...
	ASSERT_TRUE(awh::quic2::route(reinterpret_cast <const uint8_t *> (datagram.data()), datagram.size(), 4, other));
 ...
 *
 */
TEST_F(Quic2Fixture, RouteMalformedTest){
	// Ключ маршрутизации датаграммы
	awh::net::origin_key_t key;
	// Проверяем отказ извлечения из непереданной датаграммы
	ASSERT_FALSE(awh::quic2::route(nullptr, 16, 8, key));
	// Формируем пустую датаграмму
	const std::string empty = "";
	// Проверяем отказ извлечения из пустой датаграммы
	ASSERT_FALSE(awh::quic2::route(reinterpret_cast <const uint8_t *> (empty.data()), empty.size(), 8, key));
	// Формируем усечённый длинный заголовок
	const std::string truncated = this->unhex("c0000000");
	// Проверяем отказ извлечения из усечённого длинного заголовка
	ASSERT_FALSE(awh::quic2::route(reinterpret_cast <const uint8_t *> (truncated.data()), truncated.size(), 8, key));
	...
	ASSERT_FALSE(awh::quic2::route(reinterpret_cast <const uint8_t *> (oversized.data()), oversized.size(), 8, key));
	...
	ASSERT_FALSE(awh::quic2::route(reinterpret_cast <const uint8_t *> (overflow.data()), overflow.size(), 8, key));
	...
	ASSERT_FALSE(awh::quic2::route(reinterpret_cast <const uint8_t *> (anonymous.data()), anonymous.size(), 8, key));
	...
	ASSERT_FALSE(awh::quic2::route(reinterpret_cast <const uint8_t *> (cropped.data()), cropped.size(), 8, key));
	// Проверяем отказ извлечения короткого заголовка при нулевой локальной политике
	ASSERT_FALSE(awh::quic2::route(reinterpret_cast <const uint8_t *> (cropped.data()), cropped.size(), 0, key));
}
 ...
 *
 */
TEST_F(Quic2Fixture, DiagnosticNamesTest){
	/**
	 * Перебираем известные типы фреймов, включая базовые типы диапазонов
	 */
	const awh::quic2::frame_t frames[] = {
		awh::quic2::frame_t::PADDING, awh::quic2::frame_t::PING,
		awh::quic2::frame_t::ACK, awh::quic2::frame_t::ACK_ECN,
		awh::quic2::frame_t::RESET_STREAM, awh::quic2::frame_t::STOP_SENDING,
		awh::quic2::frame_t::CRYPTO, awh::quic2::frame_t::NEW_TOKEN,
		awh::quic2::frame_t::STREAM, awh::quic2::frame_t::MAX_DATA,
		awh::quic2::frame_t::MAX_STREAM_DATA, awh::quic2::frame_t::MAX_STREAMS_BIDI,
		awh::quic2::frame_t::MAX_STREAMS_UNI, awh::quic2::frame_t::DATA_BLOCKED,
		awh::quic2::frame_t::STREAM_DATA_BLOCKED, awh::quic2::frame_t::STREAMS_BLOCKED_BIDI,
		awh::quic2::frame_t::STREAMS_BLOCKED_UNI, awh::quic2::frame_t::NEW_CONNECTION_ID,
		awh::quic2::frame_t::RETIRE_CONNECTION_ID, awh::quic2::frame_t::PATH_CHALLENGE,
		awh::quic2::frame_t::PATH_RESPONSE, awh::quic2::frame_t::CONNECTION_CLOSE,
		awh::quic2::frame_t::CONNECTION_CLOSE_APP, awh::quic2::frame_t::HANDSHAKE_DONE,
		awh::quic2::frame_t::DATAGRAM
	};
	...
	for(auto & frame : frames){
		...
		const std::string name(awh::quic2::frameName(frame));
	...
	ASSERT_EQ(awh::quic2::frameName(static_cast <awh::quic2::frame_t> (0x0F)), "STREAM");
	/**
	 ...
	 */
	ASSERT_EQ(awh::quic2::frameName(static_cast <awh::quic2::frame_t> (0x31)), "DATAGRAM");
	// Проверяем что неизвестный тип фрейма опознаётся как неизвестный
	ASSERT_EQ(awh::quic2::frameName(static_cast <awh::quic2::frame_t> (0x7F)), "UNKNOWN");
	/**
	 * Перебираем известные типы пакетов
	 */
	const awh::quic2::packet_t packets[] = {
		awh::quic2::packet_t::INITIAL, awh::quic2::packet_t::ZERO_RTT,
		awh::quic2::packet_t::HANDSHAKE, awh::quic2::packet_t::RETRY,
		awh::quic2::packet_t::VERSION_NEGOTIATION, awh::quic2::packet_t::ONE_RTT
	};
	...
	for(auto & packet : packets){
		...
		const std::string name(awh::quic2::packetName(packet));
	 ...
	 */
	const awh::quic2::error_t errors[] = {
		awh::quic2::error_t::NO_ERROR, awh::quic2::error_t::INTERNAL_ERROR,
		awh::quic2::error_t::CONNECTION_REFUSED, awh::quic2::error_t::FLOW_CONTROL_ERROR,
		awh::quic2::error_t::STREAM_LIMIT_ERROR, awh::quic2::error_t::STREAM_STATE_ERROR,
		awh::quic2::error_t::FINAL_SIZE_ERROR, awh::quic2::error_t::FRAME_ENCODING_ERROR,
		awh::quic2::error_t::TRANSPORT_PARAMETER_ERROR, awh::quic2::error_t::CONNECTION_ID_LIMIT_ERROR,
		awh::quic2::error_t::PROTOCOL_VIOLATION, awh::quic2::error_t::INVALID_TOKEN,
		awh::quic2::error_t::APPLICATION_ERROR, awh::quic2::error_t::CRYPTO_BUFFER_EXCEEDED,
		awh::quic2::error_t::KEY_UPDATE_ERROR, awh::quic2::error_t::AEAD_LIMIT_REACHED,
		awh::quic2::error_t::NO_VIABLE_PATH, awh::quic2::error_t::VERSION_NEGOTIATION_ERROR
	};
	// Очищаем список встреченных названий
	names.clear();
	/**
	...
	for(auto & error : errors){
		...
		const std::string name(awh::quic2::errorName(error));
	...
	ASSERT_EQ(awh::quic2::errorName(static_cast <awh::quic2::error_t> (0x7FFF)), "UNKNOWN_ERROR");
}
