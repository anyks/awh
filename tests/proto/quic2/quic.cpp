...
void Quic2Fixture::SetUp(){
	...
	this->_security = std::make_unique <Quic2Security> (this->_fmk.get(), this->_log.get());
}
 ...
 *
 */
std::string Quic2Fixture::makePath(const std::string & host, const uint16_t port) const noexcept {
 ...
 *
 */
std::string Quic2Fixture::unhex(const std::string & hex) const noexcept {
 ...
 *
 */
std::string Quic2Fixture::hex(const std::string & data) const noexcept {
 ...
 *
 */
awh::quic2::cid_t Quic2Fixture::makeCid(const std::string & data) const noexcept {
	// Результирующий идентификатор соединения
	awh::quic2::cid_t result;
 ...
 *
 */
awh::tls::Coder & Quic2Security::coder() noexcept {
 ...
 *
 */
awh::tls::Coder::id_t Quic2Security::context(const awh::quic2::endpoint_t endpoint) const noexcept {
	// Выводим шаблон контекста безопасности запрошенной роли
	return ((endpoint == awh::quic2::endpoint_t::CLIENT) ? this->_client : this->_server);
}
/**
 ...
 *
 */
awh::tls::Coder::id_t Quic2Security::make(const awh::quic2::endpoint_t endpoint, const std::vector <awh::tls::Coder::alpn_t> & protocols, const bool validate) noexcept {
	// Определяем роль узла кодера
	const awh::event::node_t node = ((endpoint == awh::quic2::endpoint_t::CLIENT) ? awh::event::node_t::CLIENT : awh::event::node_t::SERVER);
	...
	if(endpoint == awh::quic2::endpoint_t::SERVER){
 ...
 *
 */
Quic2Security::Quic2Security(const awh::fmk_t * fmk, const awh::log_t * log) noexcept :
 _client(0), _server(0), _coder(fmk, log) {
	...
	this->_client = this->make(awh::quic2::endpoint_t::CLIENT, {awh::tls::Coder::alpn_t{0, "h3"}});
	// Создаём шаблон контекста безопасности сервера
	this->_server = this->make(awh::quic2::endpoint_t::SERVER, {awh::tls::Coder::alpn_t{0, "h3"}});
}
/**
 * @brief Деструктор
 *
 */
Quic2Security::~Quic2Security() noexcept {
