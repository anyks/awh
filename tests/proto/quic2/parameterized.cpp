 ...
 *
 */
struct Quic2VarintBoundaryParameter {
	// Кодируемое число
	uint64_t value;
 ...
 *
 */
class Quic2VarintBoundaryParameterizedFixture : public Quic2Fixture, public ::testing::WithParamInterface <Quic2VarintBoundaryParameter> {
	public:
		// Параметры теста границ кодека
		Quic2VarintBoundaryParameter _parameter = GetParam();
};
 ...
 *
 */
TEST_P(Quic2VarintBoundaryParameterizedFixture, RoundTrip){
...
INSTANTIATE_TEST_SUITE_P(
	Quic2,
	Quic2VarintBoundaryParameterizedFixture,
	::testing::Values(
		// Нижняя граница одноктетного кодирования
		Quic2VarintBoundaryParameter{0, 1},
		// Верхняя граница одноктетного кодирования
		Quic2VarintBoundaryParameter{63, 1},
		// Нижняя граница двухоктетного кодирования
		Quic2VarintBoundaryParameter{64, 2},
		// Верхняя граница двухоктетного кодирования
		Quic2VarintBoundaryParameter{16383, 2},
		// Нижняя граница четырёхоктетного кодирования
		Quic2VarintBoundaryParameter{16384, 4},
		// Верхняя граница четырёхоктетного кодирования
		Quic2VarintBoundaryParameter{1073741823, 4},
		// Нижняя граница восьмиоктетного кодирования
		Quic2VarintBoundaryParameter{1073741824, 8},
		// Верхняя граница восьмиоктетного кодирования (2^62 - 1)
		Quic2VarintBoundaryParameter{0x3FFFFFFFFFFFFFFF, 8}
	)
);
 ...
 *
 */
struct Quic2StreamFrameParameter {
 ...
 *
 */
class Quic2StreamFrameParameterizedFixture : public Quic2Fixture, public ::testing::WithParamInterface <Quic2StreamFrameParameter> {
	public:
		// Параметры теста вариантов фрейма STREAM
		Quic2StreamFrameParameter _parameter = GetParam();
};
 ...
 *
 */
TEST_P(Quic2StreamFrameParameterizedFixture, RoundTrip){
...
INSTANTIATE_TEST_SUITE_P(
	Quic2,
	Quic2StreamFrameParameterizedFixture,
	::testing::Values(
		// Клиентский двунаправленный поток без смещения
		Quic2StreamFrameParameter{0, 0, false, "first-chunk"},
		// Поток со смещением данных
		Quic2StreamFrameParameter{4, 1024, false, "middle-chunk"},
		// Завершающий фрагмент потока
		Quic2StreamFrameParameter{8, 65536, true, "last-chunk"},
		// Пустой завершающий фрагмент (только FIN)
		Quic2StreamFrameParameter{12, 2048, true, ""},
		// Большой идентификатор потока с большим смещением
		Quic2StreamFrameParameter{0x3FFFFFFF, 0x3FFFFFFF, false, "big-ids"},
		// Серверный однонаправленный поток
		Quic2StreamFrameParameter{3, 0, true, "server-uni"}
	)
);
