...
/**
 * @brief Проверка согласия встраиваемых посредников с открытым суждением
 *
 * @details Горячие дороги записи и разбора зовут посредников `readCode()` и
 * `suitableChar()`, встроенных прямо в место вызова, тогда как потребитель снаружи
 * зовёт открытые `decode()` и `isChar()`. Определение у них ОДНО - открытые сведены к
 * посредникам, - и проверка эта стережёт, чтобы оно одним и оставалось: заведи кто-нибудь
 * второе тело ради скорости, и запись стала бы судить иначе, чем разбор, а круговой ход
 * рвался бы молча
 *
 * @note Сличаются все двести пятьдесят шесть значений байта и все последовательности
 *       длиною до трёх байтов из значащих начал
 *
 * @warning Проверка эта СЕГОДНЯ ТАВТОЛОГИЧНА, и знать это надо: открытые сведены к
 *          посредникам, тело у них одно, и порча этого тела красит обе стороны сличения
 *          разом - молча для неё. Доказано щупом 05.09.2026: подмена порога избыточно
 *          длинной записи в двухбайтовой дороге покрасила `DecoderAgreesWithExternalJudge`
 *          и НЕ покрасила эту. Сторожем правильности служит та проверка, а эта стережёт
 *          иное и на будущее: попытку завести ВТОРОЕ тело ради скорости. Заведи его
 *          кто-нибудь - и запись стала бы судить иначе, чем разбор, а круговой ход рвался
 *          бы молча; вот это она и поймает
 *
 */
TEST(CodecCsvEncoding, InlineMediatorsAgreeWithTheOpenJudgement) {
	/**
	 * Выполняем перебор всех кодовых значений, какие судит открытое суждение
	 */
	for(uint32_t code = 0; code <= 0x110001; code++){
		// Выполняем проверку согласия суждений о знаке
		ASSERT_EQ(csv::isChar(code), csv::suitableChar(code)) << "код " << code;
		// Выполняем переход через области, суждение в которых не меняется
		if((code > 0x100) && (code < 0xD700))
			// Выполняем переход к границе суррогатных значений
			code = 0xD7FE;
		else if((code > 0xE000) && (code < 0x10FFFE))
			// Выполняем переход к пределу Юникода
			code = 0x10FFFD;
	}
	/**
	 * Выполняем перебор всех значений первого байта последовательности
	 */
	for(uint32_t first = 0; first < 0x100; first++){
		/**
		 * Выполняем перебор всех значений второго байта последовательности
		 */
		for(uint32_t second = 0; second < 0x100; second++){
			...
			/**
			 * Выполняем перебор всех длин переданного текста
			 */
			for(size_t size = 1; size <= sizeof(sequence); size++){
				// Длина, прочитанная открытым способом
				size_t opened = 0;
				// Длина, прочитанная встраиваемым посредником
				size_t inlined = 0;
				// Выполняем чтение открытым способом
				const uint32_t taken = csv::decode(sequence, size, opened);
				// Выполняем чтение встраиваемым посредником
				const uint32_t fast = csv::readCode(sequence, size, inlined);
				// Выполняем проверку согласия прочитанных значений
				ASSERT_EQ(taken, fast) << "байты " << first << " " << second << " длина " << size;
				// Выполняем проверку согласия прочитанных длин
				ASSERT_EQ(opened, inlined) << "байты " << first << " " << second << " длина " << size;
			}
		}
	}
}
 ...
 *
 */
TEST(CodecCsvEncoding, ByteOrderMarkContradictingTheForcedEncoding) {
	{
