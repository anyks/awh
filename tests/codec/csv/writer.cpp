 ...
 * закрепляет и то и другое, а заодно предупреждение о смене разделителя посреди записи -
 * текст такой обратно не читается, и отказом запись на это не отвечает
 *
 */
TEST(CodecCsvWriter, SettingsApplyFromNextField) {
	...
	writer.record();
	/**
	 * Выполняем смену разделителя посреди записи таблицы
	 *
	 * @note Смена эта ЛОВУШКА, а не приём: разделитель принадлежит всему файлу, и
	 *       читатель определяет его по первой записи. Проверка закрепляет именно
	 *       поведение записи - записанное прежде остаётся нетронутым, - а не годность
	 *       выходящего текста
	 */
	settings.separator = ';';
	...
	/**
	 * Выполняем проверку того, что текст этот обратно не читается
	 */
	{
		// Собранный текст таблицы
		const string text(writer.text());
