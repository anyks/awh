...
TEST(CodecCsvDocument, BothEndsOfNumberLossRefuseAlike) {
	...
	{
		/**
		 * Проверяемое число, вида одинарной точности не достигающее
		 *
		 * @details Исчезает оно НЕ ВСЕГДА: режим округления ставит потребитель
		 * библиотеки, и при округлении ВВЕРХ приведение отдаёт наименьшее поднормальное
		 * число вместо нуля - утраты нет, и успех извлечения законен. Замер 01.09.2026
		 * подделкою такого потребителя через «fesetround(FE_UPWARD)»: строка эта краснела
		 * единственной из ста девяноста двух, закрепляя режим округления, а не кодек
		 *
		 * @warning Утверждают ОБЕ ветви: обещание кодека - отказ там, где число теряется,
		 *          и там, где оно уцелело, обещанием требуется успех. Пропуск вместо
		 *          второй ветви был бы молчаливым отключением проверки
		 */
		volatile double vanishing = 1e-50;
		/**
		 * Если число видом одинарной точности исчезает
		 */
		if(static_cast <float> (vanishing) == 0.f)
			// Выполняем проверку отказа извлечения числа, в виде исчезающего
			ASSERT_FALSE(extract("1e-50", single));
		/**
		 * Если число видом одинарной точности уцелело
		 */
		else {
			// Выполняем проверку успеха извлечения уцелевшего числа
			ASSERT_TRUE(extract("1e-50", single));
			// Выполняем проверку того, что уцелевшее число нулём не выходит
	 ...
	 */
	{
		 ...
		 * Проверяемое число, вида одинарной точности не достигающее
		 *
		 * @note Обведено тем же условием, что и близнец его выше: при округлении ВВЕРХ
		 *       приведение отдаёт наименьшее поднормальное вместо нуля, терять нечего, и
		 *       успех извлечения законен
		 */
		volatile double vanishing = 1.0e-50;
		/**
		 * Если число видом одинарной точности исчезает
		 */
		if(static_cast <float> (vanishing) == 0.f)
			// Выполняем проверку отказа извлечения записи ненулевой, обратившейся нулём
		 ...
		 */
			...
			// Выполняем проверку успеха извлечения уцелевшего числа
			ASSERT_TRUE(extract("1.0e-50", single));
			// Выполняем проверку того, что уцелевшее число нулём не выходит
			ASSERT_NE(single, 0.f);
		}
	}
...
TEST(CodecCsvDocument, ReceiverIsUntouchedOnEveryRefusal) {
	...
	for(size_t i = 0; i < document.rows(); i++){
		 ...
		 *
		 * @note Вид этот отвергает ВСЕ пять записей: числа в один октет не помещается
		 *       ни одно из них
		 */
...
/**
 * @brief Проверка отказа целым видом на машине, поднормальных чисел не имеющей
 *
 * @details Разбор отвечает отказом там, где число теряется, и вид приёмника к утрате
 * отношения не имеет. Правило это на рабочей машине целыми видами НЕ ПРОВЕРЯЕТСЯ вовсе:
 * запись «1e-320» уцелевает поднормальным числом и отвергается сличением с усечением,
 * до сторожа утраты не доходя. Машина же, поднормальных не несущая, обращает такое число
 * нулём в самом разборе, и путь тот делается достижим. Замер 01.09.2026: прежде правки
 * «1e-320» видом «uint8_t» отдавало нуль признаком УСПЕХА
 *
 * @note Машина такая подделывается разрядом обнуления «FPCR.FZ» у набора команд ARM64 и
 *       разрядами «FTZ»/«DAZ» у x86-64; отладочного стенда для этого не надо
 *
 * @warning Признак применения настройки ПЕЧАТАЕТСЯ и сличается: проверка, у которой
 *          настройка не легла, отчиталась бы тем же зелёным, ничего не измерив
 */
TEST(CodecCsvDocument, IntegerRefusesLossOnMachineWithoutSubnormals) {
	 ...
	 */
		...
		// Запоминаем прежнее значение управляющего регистра
		const uint32_t restore = control;
	...
	/**
	 * Выполняем проверку отказа извлечения целыми видами
	 */
	{
		// Документ таблицы для проверки извлечения чисел
		csv::document_t document(::logger());
		...
		int64_t big = -777;
		/**
		 * Если поднормальные числа машиною обнуляются
		 */
		if(!alive){
			// Выполняем проверку отказа извлечения видом без знака
			EXPECT_FALSE(document.numeric <uint8_t> (0, 0, small));
			// Выполняем проверку неприкосновенности приёмника при отказе
			EXPECT_EQ(small, 77u);
			// Выполняем проверку отказа извлечения видом со знаком
			EXPECT_FALSE(document.numeric <int64_t> (0, 0, big));
			// Выполняем проверку неприкосновенности приёмника при отказе
			EXPECT_EQ(big, -777);
		/**
		 * Если поднормальные числа настройкою не обнулились
		 */
		} else ADD_FAILURE() << "обнуление поднормальных объявлено применённым, но не легло";
	}
	/**
	 ...
	 */
		...
		__asm__ __volatile__("msr fpcr, %0" :: "r"(restore));
	#elif defined(__x86_64__) || defined(__i386__)
		// Выполняем запись прежнего значения управляющего регистра
		__asm__ __volatile__("ldmxcsr %0" :: "m"(restore));
	#endif
}
/**
