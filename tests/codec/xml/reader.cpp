...
TEST(CodecXmlReader, Malformed) {
	...
	const struct {
		...
		/**
		 * @note Запись издания знаком ВЫШЕ цифры добавлена 08.09.2026 по карте ветвей:
		 *       порядковый номер стерегут два предела - ниже нуля и выше девятки, - и
		 *       взят был лишь нижний. Точка у «1.0.1» стоит ниже нуля, и верхний предел
		 *       не проверялся НИ РАЗУ
		 */
		{"<?xml version='1.a'?><a/>",                            xml::error_t::UNSUPPORTED_VERSION},
		...
		/**
		 * Объявление разметки и описание типа документа ВНУТРИ узла
		 *
		 * @note Добавлено 08.09.2026 по карте ветвей: место у обоих стерегут составные
		 *       условия, и условие «разбор стоит внутри узла» истинным не бывало НИ РАЗУ -
		 *       оба отказа проверялись лишь вторым появлением на верхнем уровне
		 */
		{"<a><?xml version='1.0'?></a>",                         xml::error_t::RESERVED_PROCESSING},
		{"<a><!DOCTYPE d></a>",                                  xml::error_t::DOCTYPE_MISPLACED},
		{"<a xmlns:p='u' xmlns:q='u' p:x='1' q:x='2'/>",         xml::error_t::DUPLICATE_ATTRIBUTE},
		/**
		 * Неразбираемая сущность, объявленная ПАРАМЕТРИЧЕСКОЙ
		 *
		 * @note Договор дозволяет `NDATA` лишь у сущностей общих: параметрическая живёт
		 *       внутри самого описания типа, и внешнему неразбираемому содержимому там
		 *       места нет. Добавлено 08.09.2026 по карте ветвей: условие «сущность
		 *       параметрическая» у этого отказа истинным не бывало НИ РАЗУ
		 */
		{"<!DOCTYPE a [<!ENTITY % e SYSTEM 'x' NDATA n>]><a/>",  xml::error_t::INVALID_DOCTYPE},
		/**
		 * Числовые ссылки на суррогаты и на знак за пределом Unicode
		 *
		 * @note Договор XML знака с кодом из отрезка суррогатов не знает вовсе: суррогаты
		 *       живут лишь внутри UTF-16 парою, а ссылка называет знак, а не единицу
		 *       записи. Добавлено 09.09.2026 при сличении с кодеком JSON, где обе границы
		 *       младшего суррогата закрыты в тот же день
		 */
		{"<a>&#xD800;</a>",                                      xml::error_t::INVALID_CHAR_REFERENCE},
		{"<a>&#xDFFF;</a>",                                      xml::error_t::INVALID_CHAR_REFERENCE},
		{"<a>&#x110000;</a>",                                    xml::error_t::INVALID_CHAR_REFERENCE}
...
TEST(CodecXmlReader, Conformance) {
	...
	/**
	 * Выполняем проверку отклонения указания, за целью которого стоит знак вопроса,
	 * концом разметки не являющийся
	 *
	 * @note Знак этот проверкой конца разметки не покрыт: концом её делает лишь пара
	 *       «?>», а одиночный знак вопроса разделителем цели договором не назван. Без
	 *       этой подачи третье условие короткого замыкания не берётся вовсе, и подмена
	 *       проверки пары проверкою одного знака осталась бы незамеченной
	 */
	ASSERT_FALSE(parse("<?pi?x ?><a/>", defaults));
	// Выполняем проверку такого же отклонения внутри содержимого узла
	ASSERT_FALSE(parse("<a><?pi?x ?></a>", defaults));
}
/**
	...
	for(size_t offset = 0; offset < text.size(); offset++){
		...
		while(reader.next()){
			 ...
			 */
				 ...
				 *
				 * @warning Скобки в цепочке этой ЗНАЧАЩИЕ: утверждение разворачивается в
				 *          `if-else`, и без них `else if` привязывался бы квнутреннему макроса, а
				 *          не к условию выше. Найдено предупреждением `-Wdangling-else`
				 *          собирателем g++ на стенде Debian 06.09.2026; clang рабочей
				 *          машины о нём молчит
				 */
...
TEST(CodecXmlReader, EncodingImpositionTiming) {
	...
	/**
	 * Навязывание по завершении разбора НЕ снимается молча
	 *
	 * @note Правлено 08.09.2026 решением закрыть расхождения кодеков: прежде
	 *       несбывшееся указание снималось из настроек, и настройки лгали о том, чего на
	 *       деле не происходит, - а просьба, исполнить которую нельзя, пропадала бесследно.
	 *       Ныне указание остаётся заявленным и стоит текста: ближайшая подача отвечает
	 *       отказом с кодом `ENCODING_ALREADY_CHOSEN`, ровно как у кодеков JSON и CSV
	 */
	{
		// Объект потокового чтения текста разметки
		xml::reader_t reader;
		...
		// Выполняем задание настроек разбора
		reader.settings(settings);
		// Выполняем проверку того, что несбывшееся указание из настроек НЕ пропало
		ASSERT_EQ(reader.settings().encoding, xml::encoding_t::UTF8);
		/**
...
/**
 * @brief Проверка отказа при смене кодировки посреди подачи текста разметки
 *
 * @details Сменить кодировку посреди подачи нельзя: разобранное разобрано прежней, и склеить
 * с ним текст, приведённый иною, значило бы разбирать вслепую. Прежде указание это снималось
 * из настроек МОЛЧА - просьба принималась, а исполнить её было нельзя, и звучащий о том не
 * узнавал вовсе. То первый разряд беды у потребителя
 *
 * @note Заведено 08.09.2026 решением закрыть расхождения кодеков: все три кодека -
 *       JSON, XML и CSV - отвечают ныне отказом при ближайшей подаче, кодом
 *       `ENCODING_ALREADY_CHOSEN`. Прежде отказом отвечал один лишь JSON, и код у него был
 *       чужого смысла - `UNSUPPORTED_ENCODING`, говорящий «вид не поддержан», тогда как вид
 *       поддержан вполне
 *
 * @note Половина проверки отдана ОБРАТНОМУ: кодировка, распознанной отвечающая, подачи не
 *       рвёт. Без неё «отказ на смене» достигался бы отказом на всяком задании настроек
 *
 */
TEST(CodecXmlReader, EncodingCannotBeChangedInTheMiddleOfTheFeed) {
	/**
	 ...
	 */
	{
		 ...
		 *
		 * @note Кусок подаётся длиною НЕ МЕНЕЕ четырёх октетов намеренно: распознавание
		 *       кодировки у разметки ждёт четырёх октетов начала - метка порядка байтов
		 *       короче не опознаётся, - и на куске покороче кодировка остаётся НЕ выбранной
		 *       вовсе. Смена её тогда законна, и заслон не сработал бы по праву
		 */
		ASSERT_TRUE(reader.feed("<a>x", 4, false)) << xml::message(reader.error());
		// Выполняем разбор накопленных событий, дабы приведение началось делом
		while(reader.next());
		// Выполняем проверку отсутствия отказа у первого куска
		ASSERT_EQ(reader.error(), xml::error_t::NONE);
		// Выполняем проверку того, что кодировка исходного текста уже распознана
		...
		ASSERT_FALSE(reader.feed("</a>", 4, true));
		// Выполняем проверку кода отказа смены кодировки
		ASSERT_EQ(reader.error(), xml::error_t::ENCODING_ALREADY_CHOSEN);
	}
	/**
	 ...
	 */
	{
		...
		ASSERT_TRUE(reader.feed("</a>", 4, true)) << xml::message(reader.error());
...
/**
 * @brief Проверка того, что отказ доходит до журнала, единственного на процесс
 *
 * @details Журнал переделан в пространство имён и заведён единственным на
 * процесс: объекта его больше нет, ни конструктором, ни назначателем он не передаётся.
 * Оттого проверяется здесь одно - и чтение разметки, и приведение исходного текста об
 * отказе своём этому журналу докладывают
 *
 * @note Прежде проверка звалась `LoggerSetAfterCreation` и меряла снятый ныне ход
 *       `setLogger()`: она вела каждый путь ДВАЖДЫ - без журнала и с журналом. После
 *       перевода половины стали дословно одинаковы, и задвоение снято
 *
 */
TEST(CodecXmlReader, FailureReachesTheProcessWideJournal) {
	...
	const vector <string> & messages = journal.messages();
	{
		...
		ASSERT_FALSE(messages.empty())
		 << "отказ чтения не дошёл до журнала, единственного на процесс";
	...
	{
		// Объект приведения текста разметки
		...
		// Выполняем проверку того, что отказ дошёл до журнала, единственного на процесс
		ASSERT_FALSE(messages.empty())
		 << "отказ приведения не дошёл до журнала, единственного на процесс";
...
/**
 * @brief Проверка кодов отказа у написаний, ответ которых неочевиден
 *
 * @details Написания эти собраны тем, что ответ их угадать нельзя: одни отвечают кодом
 * НЕ ТЕМ, какого ждёшь, а два не отвечают отказом вовсе. Прежде на месте этом стоял щуп,
 * коды ПЕЧАТАВШИЙ и не утверждавший ничего, - он проходил бы и у разбора, переставшего
 * отказывать напрочь. Коды сняты замером 01.09.2026 и закреплены здесь литералами
 *
 * @note Два написания отвечают УСПЕХОМ намеренно, и в этом вся их ценность:
 *       «<a><![CDATA[]]]]></a>» несёт две закрывающих скобки внутри раздела дословных
 *       данных, и разделу они принадлежат законно; «<!DOCTYPE a><b/>» объявляет корень
 *       именем «a», а несёт «b» - сличение это принадлежит ПОВЕРКЕ по описанию, какой
 *       кодек не ведёт, и разбору отказывать нечем
 *
 * @warning Сличается ЛИТЕРАЛ, а не имя члена перечня: сличение имени с самим собою прошло
 *          бы и при смене значения
 */
TEST(CodecXmlReader, SurprisingCodesAreWrittenDown) {
	/**
	 * @brief Написание разметки и ожидаемый код ответа
	 */
	const struct {
		// Разбираемое написание разметки
		const char * text;
		// Ожидаемый код ответа разбора
		uint32_t code;
		// Довод, чем ответ этот неочевиден
		const char * reason;
	} cases[] = {
		{"<a></b>",                   0x0B, "закрывающая метка не отвечает открывающей"},
		{"<a>",                       0x0A, "метка не закрыта вовсе"},
		{"</a>",                      0x0C, "закрывающая метка без открывающей"},
		{"<a/><b/>",                  0x0D, "корень разметки не один"},
		{"<a x/>",                    0x11, "у свойства нет значения"},
		{"<a x=y/>",                  0x13, "значение свойства без кавычек - код СВОЙ, а не общий код свойства"},
		{"<a x='1' x='2'/>",          0x12, "имя свойства повторено"},
		{"<a>&#xZ;</a>",              0x1B, "в записи знака шестнадцатеричная цифра негодна"},
		{"<a>&#;</a>",                0x1B, "запись знака пуста - код ТОТ ЖЕ, что у негодной цифры"},
		{"<a><!-- -- --></a>",        0x1C, "две черты внутри примечания"},
		{"<a><![CDATA[]]]]></a>",     0x00, "две закрывающих скобки разделу дословных данных принадлежат ЗАКОННО"},
		{"<?xml version='2.0'?><a/>", 0x06, "издание разметки не поддержано"},
		{"<?XML version='1.0'?><a/>", 0x1F, "цель указания заглавными - имя заповедное, и код НЕ издания"},
		{"<a>]]></a>",                0x02, "закрывающая скобка раздела вне раздела запрещена самим договором"},
		{"text<a/>",                  0x0F, "знаки до корня разметки"},
		{"<a:b/>",                    0x23, "приставка имени не привязана"},
		{"<a xmlns:xml='x'/>",        0x24, "заповедная приставка переопределена"},
		{"<!DOCTYPE a><b/>",          0x00, "описание объявляет корень «a», а несёт «b»: поверки по описанию кодек не ведёт"},
		{"<a>&#x110000;</a>",         0x1B, "кодовое значение вне области - код ТОТ ЖЕ, что у негодной записи"}
	};
	/**
	 * Выполняем перебор всех проверяемых написаний разметки
	 */
	for(const auto & item : cases){
		...
		// Выполняем перебор всех событий разбора
		while(reader.next()) ;
		// Выполняем проверку кода ответа разбора
		EXPECT_EQ(static_cast <uint32_t> (reader.error()), item.code)
			<< "написание «" << item.text << "»: " << item.reason;
	}
}
 ...
 *          пятью утверждениями сразу, что и требуется: принятое решение меняется
...
/**
 * @brief Проверка точности предела объёма подстановки на самой границе
 *
 * @details Предел объёма подстановки сличается в ДВУХ местах разбора, и оба прежде границы
 * не имели: ослабление предела на единицу у обоих сразу не красило НИ ОДНОЙ проверки из
 * двухсот восьмидесяти двух. Держалось это тем, что проверки подавали превышение с запасом -
 * предел восемь при объёме двенадцать, - а такое ослабления на единицу не замечает
 *
 * @note Замер 01.09.2026 ослаблением «> maxExpansion» до «> (maxExpansion + 1)»
 *
 * @warning Обе половины обязательны: объём РОВНО предела принимается, и лишь на единицу
 *          больший отвергается. Проверка, стоящая на одном отказе, пропускает предел,
 *          ошибающийся на единицу в любую сторону
 */
TEST(CodecXmlReader, ExpansionLimitIsExactAtTheBoundary) {
	/**
	 * @brief Разбор разметки с заданным пределом объёма подстановки
	 *
	 * @param  text  разбираемый текст разметки
	 * @param  limit наибольший допустимый объём подстановки
	 * @return       код отказа разбора
	 *
	 */
	const auto probe = [](const string & text, const uint32_t limit) noexcept -> xml::error_t {
		// Настройки разбора с пределом объёма подстановки
		xml::reader_t::settings_t settings;
	...
	// Разбираемая разметка с сущностью, значение которой занимает ровно семь знаков
	const string text = "<!DOCTYPE a [<!ENTITY e \"<b></b>\">]><a>&e;</a>";
	/**
	 * Выполняем проверку границы предела объёма подстановки
	 */
	{
		// Выполняем проверку принятия подстановки объёмом ровно предела
		EXPECT_EQ(probe(text, 7), xml::error_t::NONE)
			<< "подстановка объёмом ровно предела обязана приниматься";
		// Выполняем проверку отказа подстановке объёмом на единицу больше предела
		EXPECT_EQ(probe(text, 6), xml::error_t::ENTITY_LIMIT_EXCEEDED)
			<< "подстановка объёмом на единицу больше предела обязана отвергаться";
	}
	/**
	 * Выполняем проверку границы у подстановки, вложенной в подстановку
	 *
	 * @note Место сличения там ВТОРОЕ, и граница ему нужна своя: объём считается суммою
	 *       вложенных подстановок, и ошибка на единицу вылезла бы лишь на сумме
	 */
	{
		// Разбираемая разметка с сущностью, вложенной в сущность
		const string nested = "<!DOCTYPE a [<!ENTITY x \"<b/>\"><!ENTITY e \"<c>&x;</c>\">]><a>&e;</a>";
		// Объём подстановки: значение внешней сущности плюс значение вложенной
		const uint32_t volume = 14;
		// Выполняем проверку принятия вложенной подстановки объёмом ровно предела
		EXPECT_EQ(probe(nested, volume), xml::error_t::NONE)
			<< "вложенная подстановка объёмом ровно предела обязана приниматься";
		// Выполняем проверку отказа вложенной подстановке объёмом на единицу больше предела
		EXPECT_EQ(probe(nested, volume - 1), xml::error_t::ENTITY_LIMIT_EXCEEDED)
			<< "вложенная подстановка объёмом на единицу больше предела обязана отвергаться";
	}
}
/**
 * @brief Проверка точности предела объёма события у всех видов разметки
 *
 * @details Предел объёма события сличается в ДВЕНАДЦАТИ местах разбора - по ветви на всякий
 * вид разметки, - а стерегла границу его одна проверка. Замер 01.09.2026 ослаблением
 * «> maxEvent» до «> (maxEvent + 1)» разом во всех двенадцати: покраснела ровно ОДНА из
 * двухсот восьмидесяти трёх
 *
 * @note Границы намерены ЩУПОМ, а не выведены счётом: объём меряется от начала события до
 *       конца накопленного и обвязку вида включает, оттого счёт пером тут обманчив. Замер
 *       дал по видам 42, 45, 48, 47, 41, 47 и 49
 *
 * @warning Обе половины обязательны у КАЖДОГО вида: объём ровно предела принимается, и лишь
 *          на единицу больший отвергается. Проверка, подающая превышение с запасом, ослабления
 *          на единицу не замечает - ровно так дыра и держалась
 */
TEST(CodecXmlReader, EventLimitIsExactForEveryKindOfMarkup) {
	/**
	 * @brief Разбор разметки с заданным пределом объёма события
	 *
	 * @param  text  разбираемый текст разметки
	 * @param  limit наибольший допустимый объём события
	 * @return       код отказа разбора
	 *
	 */
	const auto probe = [](const string & text, const uint32_t limit,
	                      const bool whole = true, const bool merge = false) noexcept -> xml::error_t {
		// Настройки разбора с пределом объёма события
		xml::reader_t::settings_t settings;
		// Задаём наибольший допустимый объём события
		settings.maxEvent = limit;
		 ...
		 *
		 * @note Подача по кускам ведёт к ИНЫМ местам сличения: разбор там доносит событие
		 *       через границы кусков, и предел сличается на каждом доборе
		 */
		else {
			/**
			...
			for(size_t i = 0; i < text.size(); i++)
				// Выполняем подачу очередного знака текста разметки
	...
	/**
	 * @brief Вид разметки и его граница объёма события
	 */
	const struct {
		// Название вида разметки
		const char * kind;
		// Разбираемый текст разметки
		const char * text;
		// Наименьший предел, при котором разбор проходит
		uint32_t bound;
	} kinds[] = {
		{"знаки текста",      "<a>ттттттттттттттттттттт</a>",        42},
		{"примечание",        "<a><!--ппппппппппппппппппп--></a>",   45},
		{"дословные данные",  "<a><![CDATA[дддддддддддддддддд]]></a>", 48},
		{"указание разбору",  "<a><?цель ссссссссссссссссс?></a>",   47},
		{"имя метки",         "<ммммммммммммммммммм/>",              41},
		{"значение свойства", "<a x='ззззззззззззззззззз'/>",        47},
		{"описание вида",     "<!DOCTYPE ооооооооооооооооооо><a/>",  49}
	};
	/**
	 * Выполняем перебор всех видов разметки
	 */
	for(const auto & item : kinds){
		// Выполняем проверку принятия события объёмом ровно предела
		EXPECT_EQ(probe(item.text, item.bound), xml::error_t::NONE)
			<< "вид «" << item.kind << "»: событие объёмом ровно предела обязано приниматься";
		// Выполняем проверку отказа событию объёмом на единицу больше предела
		EXPECT_EQ(probe(item.text, item.bound - 1), xml::error_t::OVERFLOW_LIMIT)
			<< "вид «" << item.kind << "»: событие объёмом на единицу больше предела обязано отвергаться";
		// Выполняем проверку отказа тому же событию при подаче ПО КУСКАМ
		EXPECT_EQ(probe(item.text, item.bound - 1, false), xml::error_t::OVERFLOW_LIMIT)
			<< "вид «" << item.kind << "»: подача по кускам обязана отвечать тем же отказом";
		// Выполняем проверку принятия того же события при подаче по кускам
		EXPECT_EQ(probe(item.text, item.bound, false), xml::error_t::NONE)
			<< "вид «" << item.kind << "»: подача по кускам обязана принимать объём ровно предела";
	}
	/**
	 * @note Место сличения при СКЛЕЙКЕ соседних кусков текста здесь НЕ покрыто, и это
	 *       сказано вслух, а не спрятано зелёным. Замер 01.09.2026 тремя образцами -
	 *       текст, разорванный подстановкой; текст, разорванный разделом дословных данных;
	 *       текст, поданный по одному знаку, - настройка склейки не изменила НИ числа
	 *       текстовых событий, ни границы объёма ни в одном из них. Стало быть, входа,
	 *       доводящего до того места, я не нашёл, и закреплять там нечего, пока он не
	 *       найден: проверка, поставленная на образце, где склейка ничего не меняет,
	 *       мерила бы пустоту
	 */

}
/**
 * @brief Проверка независимости кода отказа незакрытого раздела от нарезки и склейки
 *
 * @details Раздел дословного текста, не закрытый до конца текста и перебравший предел
 * объёма события, обязан отвергаться ОДНИМ кодом при всякой нарезке. Заслон предела
 * стоял под настройкою склейки текста (`mergeText`), а довод его к склейке отношения не
 * имеет вовсе: он о том, что предел обязан побеждать незавершённость при любой подаче.
 * С выключенной склейкой заслон не срабатывал, и один и тот же текст отвечал разными
 * кодами - при подаче целиком незавершённостью раздела, а кусками превышением предела
 *
 * @note Дефект найден 02.09.2026 розыском входа в место склейки: место это достижимо
 *       лишь незакрытым разделом, и оно же вскрыло расхождение у соседней настройки
 *
 * @warning Проверяются ОБА положения настройки склейки: закрепи здесь одну включённую,
 *          и дефект, живущий у выключенной, прошёл бы мимо
 *
 */
TEST(CodecXmlReader, UnclosedSectionRefusalIsIndependentOfChunkingAndMerging) {
	// Раздел дословного текста, не закрытый до конца текста и длиннее предела события
	const string text = ("<a><![CDATA[" + string(40, 'x'));
	/**
	 * Выполняем перебор обоих положений настройки склейки текста
	 */
	for(const bool merge : {true, false}){
		// Настройки разбора текста разметки
		xml::reader_t::settings_t settings;
		// Устанавливаем положение настройки склейки текста
		settings.mergeText = merge;
		// Устанавливаем наибольший допустимый объём события
		settings.maxEvent = 8;
		/**
		 * @brief Метод получения кода отказа разбора при заданной нарезке
		 *
		...
		// Получаем код отказа при подаче текста целиком
		const xml::error_t sample = code(0);
		/**
		 * Выполняем проверку того, что отказом является превышение предела
		 *
		 * @note Код закреплён литералом, а не сличением подач между собой: сличение
		 *       осталось бы зелёным и тогда, когда обе подачи разом сойдут с договора
		 */
		ASSERT_EQ(sample, xml::error_t::OVERFLOW_LIMIT) << "склейка: " << merge;
		/**
		 * Выполняем перебор размеров куска подачи
		 */
		for(size_t chunk = 1; chunk <= text.size(); chunk++)
			// Выполняем проверку совпадения кода отказа с кодом подачи целиком
			ASSERT_EQ(code(chunk), sample) << "склейка: " << merge << ", размер куска: " << chunk;
	}
}
/**
 * @brief Проверка предела объёма события у незакрытых построений разметки
 *
 * @details Построение, не закрытое до конца текста и перебравшее предел объёма события,
 * обязано отвергаться пределом, а не незавершённостью: предел сличается прежде проверки
 * конца текста, иначе код отказа выходит зависящим от нарезки. Правило это записано в
 * коде трижды - у раздела дословного текста, у примечания и у указания обработчику, - а
 * закреплено было не у всех
 *
 * @note Дыра найдена картой мутантов 02.09.2026 по двенадцати местам предела: снятие
 *       заслона у указания обработчику меняло код отказа с превышения предела на
 *       незавершённость построения и НЕ красило ни одной проверки набора
 *
 * @note Пробельное содержимое сверх предела проверяется здесь же: заслон его в наборе
 *       тоже не закреплён, хотя место достижимо - печать внутри ветви показала 63
 *       срабатывания. Снятие того заслона исхода не меняет, ибо случай ловится соседним;
 *       проверка стережёт ОБА, краснея, если снять их разом
 *
 * @warning Коды закреплены литералами, а не сличением подач между собой: сличение
 *          осталось бы зелёным и тогда, когда все подачи разом сойдут с договора
 *
 */
TEST(CodecXmlReader, UnclosedMarkupPrefersTheLimitOverIncompleteness) {
	/**
	 * @brief Ожидание отказа по одному построению разметки
	 *
	 */
	struct expectation_t {
		// Название проверяемого построения
		const char * name;
		// Проверяемый текст разметки
		string text;
		// Признак отделения пробельного содержимого
		bool spaces;
	};
	// Проверяемые построения разметки, не закрытые до конца текста
	const vector <expectation_t> expectations = {
		{"указание обработчику", ("<?" + string(40, 'x')), false},
		{"примечание", ("<!--" + string(40, 'x')), false},
		{"раздел дословного текста", ("<a><![CDATA[" + string(40, 'x')), false},
		{"метка узла", ("<a" + string(40, 'x')), false},
		{"пробельное содержимое", ("<a>" + string(40, ' ')), true}
	};
	// Выполняем перебор всех проверяемых построений разметки
	for(const expectation_t & expectation : expectations){
		// Настройки разбора текста разметки
		xml::reader_t::settings_t settings;
		// Устанавливаем наибольший допустимый объём события
		settings.maxEvent = 8;
		// Устанавливаем признак отделения пробельного содержимого
		settings.separateSpaces = expectation.spaces;
		/**
		 * @brief Метод получения кода отказа разбора при заданной нарезке
		 *
		 * @param chunk размер куска подачи, ноль - подача целиком
		...
		const auto code = [&expectation, &settings](const size_t chunk) noexcept -> xml::error_t {
			...
			do {
				// Получаем размер очередного куска подачи
				const size_t size = ((chunk == 0) ? (expectation.text.size() - offset) : min(chunk, expectation.text.size() - offset));
				// Выполняем подачу очередного куска текста
				reader.feed(expectation.text.data() + offset, size, ((offset + size) >= expectation.text.size()));
				// Выполняем изъятие всех собранных событий разбора
				while(reader.next()) continue;
				// Выполняем смещение по тексту
				offset += size;
			} while(offset < expectation.text.size());
			// Выводим код отказа разбора
			return reader.error();
		};
		// Выполняем проверку отказа по превышению предела при подаче целиком
		ASSERT_EQ(code(0), xml::error_t::OVERFLOW_LIMIT) << expectation.name;
		/**
		 * Выполняем перебор размеров куска подачи
		 */
		for(size_t chunk = 1; chunk <= expectation.text.size(); chunk++)
			// Выполняем проверку того, что код отказа от нарезки не зависит
			ASSERT_EQ(code(chunk), xml::error_t::OVERFLOW_LIMIT)
				<< expectation.name << ", размер куска: " << chunk;
	}
}

/**
 * @brief Проверка кодов отказа разбора у заслонов, дотоле не проверявшихся
 *
 * @details Заслоны эти держатся порознь и стоят на РАЗНЫХ дорогах разбора: описание типа
 * документа, указание обработчику, повтор свойства узла, обрыв метки и пересечение
 * границы подставленной сущности. Общего у них лишь то, что ни один не стерёгся кодом
 *
 * @note Замерено щупом по местам отказа 04.09.2026: из двадцати одного места читателя
 *       разметки одиннадцать не красились подменой кода вовсе. Часть из них несёт доводы
 *       недостижимости, а вот эти шесть достижимы прямым текстом - им недоставало лишь
 *       проверки
 *
 * @warning Пересечение границы сущности проверяется ДВАЖДЫ, комментарием и разделом
 *          CDATA: места эти разные, и подмена кода в одном другого не красит. Требование
 *          же общее и стандартом положенное - построение, начатое внутри подставленной
 *          сущности, обязано внутри неё и завершиться
 *
 */
TEST(CodecXmlReader, RefusalCodesOfUnwatchedGuards) {
	// Разбираемые тексты разметки и ожидаемые коды отказа их
	const struct {
		// Описание разбираемого случая
		const char * title;
		// Разбираемый текст разметки
		const char * text;
		...
		{
			"ссылка на параметрическую сущность без точки с запятой",
			"<!DOCTYPE r [<!ENTITY % p \"x\"> %p ]><r/>",
			xml::error_t::INVALID_DOCTYPE
		}, {
			/**
			 * @note Цель указания обязана отделяться от данных пробельным знаком либо
			 *       завершать указание сразу. Заслон ОТДЕЛЬНЫЙ от заслона незакрытого
			 *       указания: тот ловится выше и своим местом, а сюда ведёт лишь цель,
			 *       за которой сразу пошли данные
			 */
			"указание обработчику, цель которого не отделена от данных",
			"<?pi[d?><r/>",
			xml::error_t::INVALID_PROCESSING
		}, {
			/**
			 * @note Свойств взято СОРОК намеренно, и число это существенно: разбор
			 *       сличает имена попарно, пока свойств не больше тридцати двух, и лишь
			 *       за этим порогом переходит к упорядочению по свёрткам. Заслоны у двух
			 *       дорог РАЗНЫЕ, и повтор среди двадцати свойств проверял бы попарную
			 *       дорогу вторично, оставляя вторую немой. Замерено подменой 04.09.2026
			 */
			"повторённое имя свойства среди многих свойств узла",
			"<r a1=\"1\" a2=\"2\" a3=\"3\" a4=\"4\" a5=\"5\" a6=\"6\" a7=\"7\" a8=\"8\" a9=\"9\" a10=\"10\" a11=\"11\" a12=\"12\" a13=\"13\" a14=\"14\" a15=\"15\" a16=\"16\" a17=\"17\" a18=\"18\" a19=\"19\" a20=\"20\" a21=\"21\" a22=\"22\" a23=\"23\" a24=\"24\" a25=\"25\" a26=\"26\" a27=\"27\" a28=\"28\" a29=\"29\" a30=\"30\" a31=\"31\" a32=\"32\" a33=\"33\" a34=\"34\" a35=\"35\" a36=\"36\" a37=\"37\" a38=\"38\" a39=\"39\" a40=\"40\" a7=\"повтор\"/>",
			xml::error_t::DUPLICATE_ATTRIBUTE
		}, {
			"обрыв текста на открывающей скобке метки",
			"<r/><",
			xml::error_t::INVALID_TAG
		}, {
			"комментарий, начатый внутри подставленной сущности",
			"<!DOCTYPE r [<!ENTITY c \"<!-- \">]><r>&c;-->текст</r>",
			xml::error_t::ENTITY_BOUNDARY
		}, {
			/**
			 * @note Значение свойства по умолчанию ссылаться на внешнюю сущность не
			 *       вправе: подстановка её потребовала бы чтения из источника, а
			 *       значение обязано быть готовым к выдаче
			 */
			"значение свойства по умолчанию ссылается на внешнюю сущность",
			"<!DOCTYPE r [<!ENTITY e SYSTEM \"f.txt\"><!ATTLIST r a CDATA \"&e;\">]><r/>",
			xml::error_t::INVALID_REFERENCE
		}, {
			/**
			 * @note Поле берётся НЕОБЯЗАТЕЛЬНОЕ, и это существенно: обязательное поле,
			 *       построенное ошибочно, отвергается сразу и своим заслоном, а разбор
			 *       до конца объявления не доходит вовсе. Заслон же этот стоит последним
			 *       и ловит поломку, замеченную по дороге, но разбора не прервавшую, -
			 *       случай, возможный лишь у полей, каких объявление вправе не нести.
			 *       Замерено подменою 05.09.2026: шесть текстов порознь, и пять из них
			 *       вели к соседним заслонам, а не сюда
			 */
			"поле объявления НЕОБЯЗАТЕЛЬНОЕ, названное верно, но построенное ошибочно",
			"<?xml version=\"1.0\" encoding=UTF-8?><r/>",
			xml::error_t::INVALID_DECLARATION
		}, {
			/**
			 * @note Мусор стоит ВНУТРИ описания типа, за закрывающей скобкой, а не за
			 *       самим описанием: за описанием он был бы содержимым вне корня и
			 *       отвечал бы своим кодом, дорогою иной
			 */
			"мусор внутри описания типа документа за закрывающей скобкой",
			"<!DOCTYPE r [] мусор ><r/>",
			xml::error_t::INVALID_DOCTYPE
		}, {
			/**
			 * @note Случай закрепляет ДОГОВОР, а не место: ссылка на необъявленную
			 *       сущность отвергается при разборе самой ссылки, а заслон на подстановке
			 *       её - иное место и перекрыто. Замерено подменою 05.09.2026 - подмена
			 *       кода на подстановке эту половину не красит. Держать её всё равно
			 *       надо: код у ссылки на имя неизвестное обязан быть свой, а не общий с
			 *       кодом ссылки, построенной ошибочно
			 */
			"ссылка на сущность, нигде не объявленную",
			"<r>&неизвестная;</r>",
			xml::error_t::UNKNOWN_ENTITY
		}, {
			"раздел CDATA, начатый внутри подставленной сущности",
			"<!DOCTYPE r [<!ENTITY c \"<![CDATA[ \">]><r>&c;]]></r>",
			xml::error_t::ENTITY_BOUNDARY
		}
	};
	/**
	...
	for(const auto & item : items){
		 ...
		 * Выполняем перебор всех событий разбора
		 *
		 * @note Отказ приходит СОБЫТИЕМ, а не возвратом подачи: подача лишь принимает
		 *       байты, а разбор идёт перебором, и код отказа встаёт по ходу его
		 */
		while(reader.next());
		// Выполняем проверку прихода разбора к отказу
		ASSERT_EQ(reader.state(), xml::state_t::FAILED) << item.title;
		// Выполняем проверку кода отказа разбора
		ASSERT_EQ(reader.error(), item.code) << item.title;
	}
}

/**
 * @brief Проверка выдачи накопленного содержимого прежде отложенного отказа
 *
 * @details Отказ, наступивший посреди содержимого, ОТКЛАДЫВАЕТСЯ до выдачи накопленного:
 * иначе один и тот же текст давал бы разный набор событий в зависимости от того, каким
 * куском он подан, - разбор, успевший накопить содержимое, выдал бы его, а разбор,
 * дошедший до отказа тем же куском, выбросил бы накопленное целиком
 *
 * @note Проверка закрепляет ТРИ половины разом, и порознь ни одна из них поведения не
 *       описывает: накопленное содержимое выдано событием; выдано оно ЦЕЛИКОМ, до знака,
 *       предшествующего отказу; отказ следом наступил и причину назвал. Замерено
 *       доказанной подменою 05.09.2026 - снятие отложенного отказа не стереглось ничем,
 *       и подмена его проходила незамеченной
 *
 * @warning Половина «выдано целиком» существеннее прочих: отказ, обрезавший накопленное
 *          на середине, выглядел бы исправным - события есть, отказ есть, - а потребитель
 *          получил бы содержимое, молча усечённое
 *
 */
TEST(CodecXmlReader, ContentAccumulatedBeforeRefusalIsDeliveredWhole) {
	...
	while(reader.next()){
		 ...
		 */
		if(reader.event() == xml::event_t::TEXT)
			// Выполняем сбор выданного содержимого
			collected.append(reader.text());
		/**
		 * Выполняем проверку того, что выданное событие отказом НЕ ЗАПЯТНАНО
		 *
		 * @warning Половина эта стережёт снятие отложенного отказа: отказ занесён
		 *          РАНЬШЕ, чем выдано накопленное, и не будь он снят до поры, всякое
		 *          событие выдавалось бы вместе с кодом отказа, к нему не относящегося.
		 *          Потребитель, спрашивающий причину после каждого события, - а это
		 *          дорога законная, - принял бы годное содержимое за испорченное.
		 *          Замерено доказанной подменою 05.09.2026: без этой половины подмена
		 *          снятия проходила незамеченной
		 */
		ASSERT_EQ(reader.error(), xml::error_t::NONE) << static_cast <uint32_t> (reader.event());
	}
	// Выполняем проверку прихода разбора к отказу
	ASSERT_EQ(reader.state(), xml::state_t::FAILED);
	// Выполняем проверку названной причины отказа
	ASSERT_EQ(reader.error(), xml::error_t::UNKNOWN_ENTITY);
	/**
	 * Выполняем проверку выдачи накопленного содержимого ЦЕЛИКОМ
	 *
	 * @note Сличается точный текст, а не одна лишь непустота: непустота прошла бы и при
	 *       содержимом, обрезанном на первом знаке
	 */
	ASSERT_EQ(collected, "накопленное содержимое");
}
/**
 * @brief Проверка честности состояния чтения при поданном, но не разобранном тексте
 *
 * @details Разбор разметки ЛЕНИВ: событие добывается вызовом «next», а не подачей.
 * Оттого состояние, спрошенное сразу после подачи, отвечало «HUNGRY» и при тексте,
 * принятом ЦЕЛИКОМ вместе с признаком конца
 *
 * @warning Ложь эта заводила потребителя в бесконечный круг: договор велит при «HUNGRY»
 *          подавать следующий кусок, подача пустого куска отвечает успехом и состояния
 *          не меняет. Половина закрепляет именно ЭТО - состояние ДО первого «next»;
 *          прочие проверки состояния глядят на него ПОСЛЕ выборки событий и лжи не видят
 *
 */
TEST(CodecXmlReader, StateAfterFeedIsHonestBeforeTheFirstEvent) {
	 ...
	 *
	 * @note Голод здесь верен: разбирать нечего вовсе
	 */
	ASSERT_EQ(reader.state(), xml::state_t::HUNGRY);
	// Выполняем подачу исходного текста разметки целиком
	ASSERT_TRUE(reader.feed("<r><a>1</a></r>"));
	/**
	 * Выполняем проверку состояния ПОСЛЕ подачи, но ДО первого события
	 *
	 * @note Текст принят и ждёт разбора: верный ответ потребителю - «забирай событие»,
	 *       а не «подай ещё кусок». Кодек JSON отвечает здесь тем же членом
	 */
	ASSERT_EQ(reader.state(), xml::state_t::READY);
	...
	while(reader.next())
		...
		events++;
	// Выполняем проверку получения событий разбора
...
/**
 * @brief Проверка принятия пробельных знаков в общем опознавателе описания типа
 *
 * @details Договор XML 1.0 держит в наборе знаков общего опознавателя пробел, возврат
 *          каретки и перевод строки наравне с буквами и цифрами. Набор проверок подавал
 *          опознаватели лишь из знаков непробельных, и ветвь их разбора не бралась ни разу
 *
 * @warning Найдено картою СТЕНДА 07.09.2026, а не общего набора: приём разбора лежит в
 *          безымянном пространстве, а такие приёмы общий набор не видит вовсе - записи
 *          покрытия одноимённых приёмов разных кодеков сводятся по имени, и покрытие
 *          одного файла уходит к другому
 *
 * @note Пробел внутри опознавателя отличается от пробела РАЗДЕЛЯЮЩЕГО: первый есть часть
 *       значения, второй - граница полей заголовка, и смешение их обратило бы разбор
 *       заголовка в розыск полей по названию
 *
 */
TEST(CodecXmlReader, PublicIdentifierAcceptsWhitespaceCharacters) {
	...
	/**
	 * Выполняем проверку принятия опознавателя с ПЕРЕВОДОМ СТРОКИ внутри значения
	 */
	{
		// Выполняем создание объекта потокового чтения разметки
		xml::reader_t reader;
	...
	/**
	 * Выполняем проверку отказа на знаке, договором НЕ допущенном
	 *
	 * @note Половина эта обязательна: проверка, одно лишь принятие утверждающая, зеленела
	 *       бы и у разбора, принимающего всякий знак подряд
	 */
	{
		// Выполняем создание объекта потокового чтения разметки
		xml::reader_t reader;
		...
		// Выполняем перебор всех событий разбора
		while(reader.next());
		// Выполняем проверку того, что разбор ответил отказом
		ASSERT_EQ(reader.error(), xml::error_t::INVALID_DOCTYPE);
	}
}

/**
 * @brief Проверка отказа неопознанной разметки, поданной неполным куском
 *
 * @details Вид разметки, начатой знаками `<!`, опознаётся по девяти первым октетам:
 * примечание, раздел дословного текста и описание типа документа. Не совпав ни с одним,
 * разбор обязан ответить отказом - но лишь тогда, когда октетов ДОВОЛЬНО: пока их меньше
 * девяти, а текст не кончен, вид определять нечем, и разбор просит следующий кусок
 *
 * @note Заведено 08.09.2026 по карте ВЕТВЕЙ: у условия «октетов ещё мало» ложная половина
 *       не бралась НИ РАЗУ при незавершённой подаче. То есть неопознанная разметка длиною
 *       в девять октетов и более, поданная НЕ последним куском, разбору не встречалась, и
 *       отказ на ней стоял неизмеренным
 *
 * @note Сличаются подача кусками и подача целиком: код отказа обязан совпасть. Разбор,
 *       зависящий от нарезки, отвечал бы на один и тот же текст по-разному
 *
 */
TEST(CodecXmlReader, UnrecognisedMarkupIsRefusedOnAPartialFeed) {
	// Разбираемый текст с неопознанной разметкой
	const string text = "<!WRONGTAG/><a/>";
	// Код отказа разбора подачею кусками
	xml::error_t chunked = xml::error_t::NONE;
	// Выполняем разбор текста кусками, в первый из которых разметка не умещается
	::run(text, 12, chunked);
	// Выполняем проверку кода отказа построения метки
	ASSERT_EQ(chunked, xml::error_t::INVALID_TAG) << xml::message(chunked);
	// Код отказа разбора подачею целиком
	xml::error_t whole = xml::error_t::NONE;
	// Выполняем разбор того же текста целиком
	::run(text, 0, whole);
	// Выполняем проверку совпадения кода отказа у обеих подач
	ASSERT_EQ(whole, chunked) << xml::message(whole);
}

/**
 * @brief Проверка отказов ссылки на внешнюю сущность по каждому из трёх условий
 *
 * @details Внешних сущностей разбор не читает, и подставить такую ссылку нечем. Договор
 * разнит два места: в значении атрибута ссылка запрещена им самим, а в содержимом разбору
 * велено её распознать и пропустить. Сверх того отказ ставится ещё дважды - на сущность
 * НЕРАЗБИРАЕМУЮ, запрещённую договором и в содержимом, и по настройке строгости
 *
 * @note Заведено 08.09.2026 по карте ВЕТВЕЙ: из трёх условий отказа истинным бывало лишь
 *       первое - ссылка в значении атрибута. Ни неразбираемая сущность в содержимом, ни
 *       строгость настройкой не подавались НИ РАЗУ, и два условия из трёх стояли
 *       неизмеренными
 *
 * @note Проверяется и обратное: обычная внешняя сущность в содержимом при умолчании
 *       настроек пропускается, а текст принимается. Без него «отвергает троих» достигалось
 *       бы отказом на всякую внешнюю ссылку вовсе - ровно тем, чем разбор грешил прежде,
 *       отвергая 22 текста набора соответствия W3C
 *
 */
TEST(CodecXmlReader, ExternalEntityIsRefusedByEachOfTheThreeConditions) {
	// Описание типа документа с внешнею и неразбираемою сущностями
	const string doctype =
		"<!DOCTYPE a [<!NOTATION n SYSTEM 'y'>"
		"<!ENTITY e SYSTEM 'x'>"
		"<!ENTITY u SYSTEM 'x' NDATA n>]>";
	/**
	 * Ссылка на неразбираемую сущность в содержимом узла
	 */
	{
		// Код отказа разбора
		xml::error_t error = xml::error_t::NONE;
		// Выполняем разбор текста со ссылкой на неразбираемую сущность
		::run(doctype + "<a>&u;</a>", 0, error);
		// Выполняем проверку кода отказа внешней сущности
		ASSERT_EQ(error, xml::error_t::EXTERNAL_ENTITY) << xml::message(error);
	}
	/**
	 * Ссылка на внешнюю сущность в содержимом при затребованной строгости
	 */
	{
		// Настройки чтения со строгостью к внешним сущностям
		xml::reader_t::settings_t settings;
		// Затребуем отказ на всякую ссылку внешней сущности
		settings.externals = true;
		// Код отказа разбора
		xml::error_t error = xml::error_t::NONE;
		// Выполняем разбор текста со ссылкой на внешнюю сущность
		::run(doctype + "<a>&e;</a>", 0, error, settings);
		// Выполняем проверку кода отказа внешней сущности
		ASSERT_EQ(error, xml::error_t::EXTERNAL_ENTITY) << xml::message(error);
	}
	/**
	 * Обратное: та же ссылка при умолчании настроек пропускается
	 */
	{
		// Код отказа разбора
		xml::error_t error = xml::error_t::NONE;
		// Выполняем разбор того же текста при умолчании настроек
		::run(doctype + "<a>&e;</a>", 0, error);
		// Выполняем проверку отсутствия отказа разбора
		ASSERT_EQ(error, xml::error_t::NONE) << xml::message(error);
	}
}
...
/**
 * @brief Проверка разбора при выключенном разрешении пространств имён
 *
 * @details Без разрешения пространств имён двоеточие в имени знаком особым не является:
 * договор XML 1.0 сам по себе о пространствах имён не знает вовсе, их заводит отдельный
 * договор «Namespaces in XML». Оттого имя берётся целиком, объявление `xmlns` становится
 * обычным атрибутом, а имя с несвязанным префиксом отказа более не вызывает
 *
 * @note Заведена 17.09.2026 по карте ветвлений: ветвь раскладки атрибута при выключенном
 *       разрешении не бралась набором НИ РАЗУ. Единственное место, где разрешение
 *       выключалось, меряло предел объёма события, а не саму раскладку имён
 *
 */
TEST(CodecXmlReader, NamespacesDisabledKeepTheQualifiedNamesWhole) {
	/**
	 * @brief Метод снятия первого узла разметки при выключенном разрешении
	 *
	 * @param text  разбираемый текст разметки
	 * @param names собираемые имена атрибутов узла
	 * @return      имя узла целиком
	 *
	 */
	const auto element = [](const string & text, vector <string> & names) noexcept -> string {
		// Настройки разбора текста разметки
		xml::reader_t::settings_t settings;
		// Выполняем отключение разбора пространств имён
		settings.namespaces = false;
		// Чтение текста разметки
		xml::reader_t reader(settings);
		// Собираемое имя узла
		...
		while(reader.next()){
			/**
			 * Если событие началом узла не является
			 */
			if(reader.event() != xml::event_t::ELEMENT_OPEN)
				// Выполняем переход к следующему событию
				continue;
			/**
			 * Если имя узла ещё не собрано
			 */
			if(result.empty()){
				// Запоминаем имя узла целиком
				result = string(reader.name().local);
				/**
				 * Выполняем перебор всех атрибутов узла
				 */
				for(const auto & attribute : reader.attributes())
					// Выполняем сбор имени очередного атрибута
					names.push_back(string(attribute.name.local));
				/**
				 * Выполняем проверку того, что объявлений пространств имён не собрано
				 *
				 * @note Объявление `xmlns` ложится обычным атрибутом, и перечень объявлений
				 *       остаётся пуст: разрешать нечего
				 */
				EXPECT_TRUE(reader.bindings().empty()) << text;
			}
		}
		// Выполняем проверку того, что разбор отказа не выдал
		EXPECT_EQ(reader.error(), xml::error_t::NONE) << text;
		// Выводим собранное имя узла
		return result;
	};
	{
		// Собираемые имена атрибутов узла
		vector <string> names;
		// Выполняем проверку имени узла, снятого целиком
		ASSERT_EQ(element("<a xmlns='urn:x' y='1'/>", names), "a");
		// Выполняем проверку того, что объявление легло обычным атрибутом
		ASSERT_EQ(names, (vector <string> {"xmlns", "y"}));
	}
	{
		// Собираемые имена атрибутов узла
		vector <string> names;
		// Выполняем проверку имени узла, снятого целиком вместе с префиксом
		ASSERT_EQ(element("<p:a xmlns:p='urn:x' p:y='1'/>", names), "p:a");
		// Выполняем проверку того, что имена атрибутов сняты целиком
		ASSERT_EQ(names, (vector <string> {"xmlns:p", "p:y"}));
	}
	{
		// Собираемые имена атрибутов узла
		vector <string> names;
		/**
		 * Выполняем проверку разбора имени с несвязанным префиксом
		 *
		 * @note С разрешением пространств имён тот же текст отвечает отказом
		 *       `UNBOUND_PREFIX`: связывать префикс без объявления не с чем
		 */
		ASSERT_EQ(element("<a p:y='1'/>", names), "a");
		// Выполняем проверку того, что имя атрибута снято целиком
		ASSERT_EQ(names, (vector <string> {"p:y"}));
	}
}
