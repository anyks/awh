...
TEST(CodecXmlWriter, ErrorCodes) {
	 ...
	 *
	 * @note Код здесь ВНЕШНИЙ, а не внутренний: узел передан извне, и непригодным он
	 *       бывает не только заведением пустым, как здесь, но и удержанием через
	 *       перестроение дерева. Прежде проверка эта закрепляла код `INTERNAL` -
	 *       и тем закрепляла отправку потребителя искать дефект у нас
	 */
	{
		// Объект записи текста разметки
		xml::writer_t writer;
		...
		// Выполняем проверку выданного кода ошибки записи
		ASSERT_EQ(writer.error(), xml::error_t::INVALID_NODE);
	}
}
 ...
 * ради договора, общего всем классам кодека: проверяется здесь лишь то, что сборка
 * текста идёт своим ходом. Ветвь эта покрытием пройдена не была
 *
 * @note Прежде проверка звалась «установка журнала после создания» и ставила журнал
 *       ходом `setLogger`. Ход снят вместе с самим объектом журнала: журнал переделан
 *       владельцем в пространство имён и заведён единственным на процесс
 *
 */
TEST(CodecXmlWriter, LoggerDoesNotHinderTextAssembly) {
	...
	ASSERT_TRUE(writer.text("значение"));
	// Выполняем проверку закрытия узла разметки
	ASSERT_TRUE(writer.close());
	// Выполняем проверку собранного текста разметки
	ASSERT_EQ(writer.text(), "<корень>значение</корень>");
}
/**
 * @brief Проверка изъятия собранного текста разметки
 *
 * @details Изъятие переносит собранное звучащему и оставляет сборщик пустым, состояния
 *          записи не трогая: разметка подаётся наружу кусками без списывания текста
 *
 * @note Имя общее с записью документа и таблицы намеренно: у всех трёх кодеков изъятие
 *       зовётся `take()`, а прежде запись разметки его не имела вовсе, и подача кусками
...
TEST(CodecXmlWriter, TakeYieldsTextAndKeepsState){
	...
	// Выполняем запись вложенного узла
	ASSERT_TRUE(writer.element("первый", "раз")) << xml::message(writer.error());
	// Выполняем изъятие собранного начала разметки
	const string first = writer.take();
	// Выполняем проверку изъятого начала разметки
	ASSERT_EQ(first, "<корень><первый>раз</первый>");
	// Выполняем проверку опустошения сборщика
	ASSERT_TRUE(writer.text().empty());
	// Выполняем проверку того, что разметка законченной не считается
	ASSERT_FALSE(writer.complete());
	// Выполняем запись второго вложенного узла
	ASSERT_TRUE(writer.element("второй", "два")) << xml::message(writer.error());
	// Выполняем закрытие корневого узла
	ASSERT_TRUE(writer.close()) << xml::message(writer.error());
	// Выполняем проверку законченности разметки
	ASSERT_TRUE(writer.complete());
	// Выполняем изъятие остатка собранной разметки
 ...
 *
 * @details Выдача текста при отказе отдаёт пустоту намеренно, и изъятие обязано
 * поступать так же: иначе оно обходило бы эту защиту стороною и отдавало наружу ту
 * самую негодную разметку, какую выдача не отдаёт
 */
TEST(CodecXmlWriter, TakeClosedByRefusal){
	...
	ASSERT_FALSE(writer.open("<плохое>"));
	// Выполняем проверку кода отказа записи
	ASSERT_NE(writer.error(), xml::error_t::NONE);
	// Выполняем проверку пустоты выдаваемого текста
	ASSERT_TRUE(writer.text().empty());
	// Выполняем проверку пустоты изъятого текста
	ASSERT_TRUE(writer.take().empty());
	// Выполняем очистку объекта записи
	writer.clear();
	// Выполняем проверку снятия отказа очисткой
	ASSERT_EQ(writer.error(), xml::error_t::NONE);
}
/**
 * @brief Проверка того, что устаревший узел не выдаётся за внутренний изъян кодека
 *
 * @details Запись узла дерева отвергает узел непригодный. Прежде отказ шёл кодом
 * `INTERNAL`, и потребитель, передавший узел, удержанный через перестроение дерева,
 * отправлялся искать дефект у нас
 *
 * @note Сторож был верен ДО заведения клейма поколения: тогда узел становился непригодным
 *       лишь заведением пустым, и всякий иной путь сюда означал изъян записи. Заведение
 *       клейма условие это отменило, а довод сторожа пережил собственную неверность
 *
 * @note Отсюда правило: довод недостижимости обязан называть условие, каким держится, -
 *       иначе перемена условия его не касается. Найдено сличением с кодеками INI и TOML
 */
TEST(CodecXmlWriter, StaleNodeIsNotAnInternalDefect){
	...
	ASSERT_FALSE(writer.element(node));
	/**
	 * Выполняем проверку того, что отказ назван причиной внешней, а не внутренней
	 */
	ASSERT_EQ(writer.error(), xml::error_t::INVALID_NODE);
	/**
	 * Выполняем проверку того, что узел, снятый заново, записывается
	 */
	{
		// Запись текста разметки
		xml::writer_t writer;
...
/**
 * @brief Проверка отказа при подборе префикса, занятого дословным именем
 *
 * @details Свойство, записанное ДОСЛОВНЫМ именем, ложится в перечень имён узла, связывания
 * при том не заводя. Подбор же имени для нового объявления смотрит лишь на действующие
 * связывания и о записанном имени не знает - оттого он вправе выдать имя, уже занятое, и
 * запись отвечает названным отказом
 *
 * @note Заведена 08.09.2026 по находке: довод при этом отказе звал его недостижимым, а
 *       дорогу к нему - лишь возможной. Дорога открыта уже ныне, и доказано это подачею
 *
 * @note Проверка закрепляет ОТКАЗ, а не обход: заведение имени `xmlns:X` в обход связывания
 *       есть ошибка звучащего, и отвечать на неё названным отказом вернее, чем молча
 *       выбирать иное имя. Переменится решение - проверка о том скажет
 *
 */
TEST(CodecXmlWriter, GeneratedPrefixClashingWithAVerbatimNameIsRefused) {
	...
	ASSERT_TRUE(writer.attribute("xmlns:n1", "urn:вручную", "", true)) << xml::message(writer.error());
	// Выполняем проверку отказа записи свойства, требующего подбора префикса
	ASSERT_FALSE(writer.attribute("поле", "значение", "urn:иное"));
