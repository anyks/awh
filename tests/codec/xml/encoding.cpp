...
TEST(CodecXmlEncoding, SingleByteEncodingsFromCharset){
	...
	for(auto & item : vector <pair <string, string>> {
		...
		/**
		 * Выполняем проверку выдаваемого вида кодировки
		 *
		 * @details Сторона эта прежде не поверялась ничем: сличалось одно лишь
		 *          СОДЕРЖИМОЕ, а вид кодировки, потребителю выдаваемый, оставался
		 *          без сторожа. Между тем потребитель по нему и судит, какою
		 *          кодировкой пришёл текст
		 *
		 * @note Всякая однобайтовая кодировка отвечает ОДНИМ видом `SINGLE`, а не
		 *       своим именем: имён их у кодека нет вовсе, таблица берётся у общего
		 *       модуля `awh::charset`, и различать их дальше кодеку нечем
		 *
		 * @note Пробел вскрыт сличением состава перечня `encoding_t` с тем, что
		 *       поминается проверками: вид этот единственный из семи не поминался
		 *       нигде, при том что ставится он исправно
		 */
		ASSERT_EQ(document.encoding(), xml::encoding_t::SINGLE) << item.first;
	}
	/**
...
TEST(CodecXmlEncoding, DecodersAgreeWithExternalJudge) {
	 ...
	 *
	 * @note Приём взят у кодека ABC: там стенд портит судью сам
...
TEST(CodecXmlEncoding, NameableAgreesWithTheWriter) {
	 ...
	 *
	 * @note Последнее имя записано ДВУМЯ литералами подряд намеренно, и склеивать их в
	 *       один нельзя: шестнадцатеричная отмена `\xA0` не имеет предела длины, и
	 *       следующая за нею буква «b» была бы прочитана её же разрядом - вышло бы
	 *       `\xA0b`, а не неразрывный пробел с буквой. Разрыв литерала и есть предел
	 *
	 * @warning Собиратель поминал это место предупреждением о подозрительной склейке
	 *          литералов в заведении списка (`-Wstring-concatenation`). Предупреждение
	 *          здесь ЛОЖНО, и «исправление» его запятой завело бы тринадцатое имя
	 *          вместо двенадцатого, подменив проверяемое. Оттого склейка обведена
	 *          скобками - способом, какой советует сам собиратель: намерение он видит,
	 *          а состав списка от скобок не меняется вовсе
	 */
	const char * names[] = {
		"а", "a", "a-b", "a.b", "a:b", "1a", "-a", "a b", "", "xml", "a_b", ("a\xC2\xA0" "b")
	};
	/**
	 * Выполняем проверку числа проверяемых имён
	 *
	 * @note Число это записано литералом затем, чтобы склейка последних двух литералов,
	 *       разделённая запятою по совету собирателя, краснела здесь: список вырос бы до
	 *       тринадцати имён, а проверяемое имя подменилось бы двумя чужими
	 */
	ASSERT_EQ(sizeof(names) / sizeof(names[0]), static_cast <size_t> (12));
