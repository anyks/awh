...
TEST(CodecBridge, RoundTripINI){
	...
	// Выполняем проверку обоих объявлений повторённого свойства
	ASSERT_EQ(second["net"]["port"][0].text(), "80");
	ASSERT_EQ(second["net"]["port"][1].text(), "443");
}
...
TEST(CodecBridge, MarkupContainers){
	 ...
	 */
	ASSERT_NE(text.find("<a><item>1</item><item>2</item></a>"), string::npos) << "вложенный перечень обнесён не узлом; собрано: " << text;
	/**
	 ...
	 *
	 * @note Круг здесь НЕ замкнут намеренно: перечень перечней возвращается
	 *       отображением с полем обноса, а поле `3` - полем `Item3`. Замкнуть
	 *       его нечем - записей таких у разметки нет вовсе, - и утверждения
	 *       эти закрепляют именно то, во что они обращаются
	 */
	ASSERT_TRUE(repeat["config"]["a"]["item"].is(abc::type_t::ARRAY)) << "обнос вложенного перечня обратным чтением не признан; собрано: " << text;
	ASSERT_EQ(repeat["config"]["a"]["item"].size(), 2u);
