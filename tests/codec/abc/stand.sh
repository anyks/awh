...
#       успешном, по виду журнала, проходе. Journal лжёт, время не лжёт
for SOURCE in "$ROOT/src/codec/abc"/*.cpp "$ROOT/tests/codec/abc"/*.cpp; do
	# Если очередной исходный текст новее собранного стенда
	if [ "$SOURCE" -nt "$OUTPUT/abc-tests" ]; then
		# Выводим сообщение об устарелости собранного стенда
		echo "ОТКАЗ: стенд старше исходного текста $SOURCE" >&2
