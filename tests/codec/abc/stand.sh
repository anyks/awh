...
#
# @note Части эти лежат ВНЕ каталога кодека и потому легко забываются: перечень стенда
#       ведётся ВРУЧНУЮ, и вынос общего кода из кодека ломает сборку молча - CMake при том
#       остаётся зелёным, ибо свой перечень ведёт сам. Так и вышло 07.09.2026, когда
#       приведение чисел было сведено к общему `awh::codec::convert`: набор через CMake
#       прошёл, а стенд пал на связывании
#
# @note «src/codec/numeric.cpp» несёт приведение и разбор чисел, общие у всех кодеков
#       рамки. Третьей стороны он не тянет, и потому стенду годен
#
