 ...
 *       кодеков. Вскрыто 06.09.2026 на стенде MSYS2 MinGW64
 */
...
namespace {
	 ...
	 * виновно соседство ПРОЦЕССОВ. Установлено опытом 07.09.2026 на
	 * четырёх кодеках сразу и воспроизведено здесь двумя прогонами разом: четыре красных
 ...
 *       близнеца её нет, - и вскрыт он не чтением кода, а вопросом от соседнего кодека:
 *       «а есть ли чем взять голую запись обратно»
 *
 ...
 *
 * @note Находка пришла по следу кодеков JSON, XML и CSV 04.09.2026, у каких запись
...
TEST(CodecAbcStorage, RefusedStoringLeavesThePreviousContainerWhole) {
	 ...
	 *       кодека бьют прогон по сроку и снимают его силою. Найдено 07.09.2026
	 *       соседним набором кодеков: краснота всплывала у соседа, а порождалась здесь
	 *
	 ...
	 */
	{
		 ...
		 *       прогоном под Windows 07.09.2026, способ замены найден замером, и
...
/**
 * @brief Проверка того, что воронка отказа хранилища доносит причину журналу
 *
 * @details Воронка эта ПЯТАЯ в своём роду: близнецы её у правщика, выборщика, дерева свёрток
 *          и укладчика кадра закреплены каждый своею проверкою ещё 08.09.2026, а своя
 *          осталась незакреплённой. Сплошной щуп по заслонам хранилища 16.09.2026 нашёл её
 *          МОЛЧАЩЕЙ: набор спрашивал причину работою `error()`, а журнала не спрашивал никто
 *
 * @warning Поверке этой журнал НУЖЕН, и она обязана заявить нужду свою САМА: сторож гашения
 *          глушит журнал у всякой проверки набора, и полагаться на состояние, оставленное
 *          соседями, значило бы городить поверку, чей исход зависит от порядка запуска
 *
 */
TEST(CodecAbcStorage, TheStorageFunnelReportsItsCauseToTheJournal){
	...
	abc::storage_t storage;
	/**
	 * Выполняем открытие КАТАЛОГА вместо файла: работа обязана быть отвечена отказом
	 */
	ASSERT_FALSE(storage.open(".")) << "каталог открыт файлом контейнера, проверка негодна";
	// Выполняем получение объявленной причины отказа
	const abc::error_t cause = storage.error();
...
/**
 * @brief Проверка того, что работа чтения октетов стережёт свои пределы сама
 *
 * @details Заслоны эти сплошной щуп 16.09.2026 нашёл молчащими у СВОИХ проверок, а краснели
 *          на них проверки CSV и XML: кодеки те кладут файлы через это же хранилище. Охрана,
 *          пришедшая извне, уходит с первой правкой чужого кодека, и числить её своим
 *          сторожем нельзя
 *
 * @note Работа чтения отдаётся наружу и живёт своей жизнью: правщик и выборщик держат её
 *       откликом. Оттого поверяется она здесь ПРЯМО, а не через привязку
 *
 */
TEST(CodecAbcStorage, TheSourceWorkGuardsItsOwnBounds){
	// Выполняем отключение вывода журнала на приставку
	awh::log::mode({});
	// Временный файл проверки
	Temporary file("source-bounds");
	// Выполняем сборку контейнера с одною записью
	const vector <uint8_t> container = build({"запись"});
	// Хранилище контейнера на носителе
	abc::storage_t storage;
	// Буфер прочитанных октетов
	vector <uint8_t> result;
	/**
	 * Выполняем чтение НЕПРИВЯЗАННЫМ хранилищем: работа обязана ответить отказом
	 */
	ASSERT_FALSE(storage.source()(0, 1, result)) << "непривязанное хранилище отдало октеты";
	// Выполняем укладку собранного контейнера на носитель
	ASSERT_TRUE(storage.store(file.filename(), container.data(), container.size()))
		<< "код отказа: " << abc::message(storage.error());
	// Выполняем получение работы чтения октетов контейнера
	const abc::editor_t::source_t source = storage.source();
	/**
	 * Выполняем чтение НУЛЕВОЙ длины: оно законно и отвечается успехом с пустою выдачей
	 */
	ASSERT_TRUE(source(0, 0, result)) << "чтение нулевой длины отвечено отказом";
	// Выполняем проверку того, что выдача чтения нулевой длины пуста
	ASSERT_TRUE(result.empty()) << "чтение нулевой длины отдало октеты";
	/**
	 * Выполняем чтение с смещения ЗА концом контейнера
	 */
	ASSERT_FALSE(source(storage.length() + 1, 1, result))
		<< "чтение за концом контейнера отвечено успехом";
	/**
	 * Выполняем чтение длины, за конец контейнера выводящей
	 *
	 * @note Смещение здесь законно, а длина - нет: сличение ведётся ВЫЧИТАНИЕМ от полной
	 *       длины, и сложение смещения с длиною завернулось бы, пройдя сторож насквозь
	 */
	ASSERT_FALSE(source(storage.length() - 1, 2, result))
		<< "чтение сверх конца контейнера отвечено успехом";
	/**
	 * Выполняем чтение непомерной длины: заворот сложения ловится тем же вычитанием
	 */
	ASSERT_FALSE(source(1, static_cast <size_t> (numeric_limits <uint32_t>::max()), result))
		<< "чтение непомерной длины отвечено успехом";
	// Выполняем проверку того, что законное чтение проходит
	ASSERT_TRUE(source(0, static_cast <size_t> (storage.length()), result))
		<< "законное чтение контейнера отвечено отказом";
	// Выполняем проверку того, что прочитан контейнер целиком
	ASSERT_EQ(result, container) << "прочитанное расходится с уложенным";
	// Выполняем закрытие файла контейнера
	storage.close();
}
/**
 * @brief Проверка того, что работа записи октетов стережёт свои доводы сама
 *
 * @details Заслоны эти сплошной щуп 16.09.2026 нашёл молчащими у своих проверок наравне с
 *          заслонами чтения, и стерегли их те же чужие кодеки
 *
 */
TEST(CodecAbcStorage, TheSinkWorkGuardsItsOwnArguments){
	// Выполняем отключение вывода журнала на приставку
	awh::log::mode({});
	// Временный файл проверки
	Temporary file("sink-arguments");
	// Выполняем сборку контейнера с одною записью
	const vector <uint8_t> container = build({"запись"});
	// Хранилище контейнера на носителе
	abc::storage_t storage;
	// Октеты, подаваемые записи
	const vector <uint8_t> payload(4, 0x7F);
	/**
	 * Выполняем запись НЕПРИВЯЗАННЫМ хранилищем: работа обязана ответить отказом
	 */
	ASSERT_FALSE(storage.sink()(0, payload.data(), payload.size()))
		<< "непривязанное хранилище приняло октеты записи";
	// Выполняем укладку собранного контейнера на носитель
	ASSERT_TRUE(storage.store(file.filename(), container.data(), container.size()))
		<< "код отказа: " << abc::message(storage.error());
	// Выполняем получение работы записи октетов контейнера
	const abc::editor_t::sink_t sink = storage.sink();
	/**
	 * Выполняем запись ПУСТОГО буфера: она законна и отвечается успехом
	 */
	ASSERT_TRUE(sink(0, payload.data(), 0)) << "запись нулевой длины отвечена отказом";
	/**
	 * Выполняем запись из НЕСУЩЕСТВУЮЩЕГО буфера при объявленных октетах
	 */
	ASSERT_FALSE(sink(0, nullptr, payload.size()))
		<< "запись из несуществующего буфера отвечена успехом";
	// Выполняем проверку того, что полная длина контейнера отказами не тронута
	ASSERT_EQ(storage.length(), static_cast <uint64_t> (container.size()))
		<< "отвергнутая запись нарастила полную длину контейнера";
	// Выполняем закрытие файла контейнера
	storage.close();
}
/**
 * @brief Проверка того, что подача снимателю ловит усечение файла под ногами
 *
 * @details Заслон, сличающий полный счёт поданных октетов с длиною контейнера, сплошной щуп
 *          16.09.2026 нашёл молчащим. Он и есть то, что заменило собою `ferror`: нулевое
 *          чтение значило прежде либо конец файла, либо отказ, и без различения их отказ
 *          посреди файла докладывался УСПЕХОМ
 *
 * @warning Усечение идёт ПОСЛЕ открытия намеренно: длину хранилище снимает при открытии, и
 *          усечённый до открытия файл дал бы длину уже усечённую - счёт сошёлся бы, и заслон
 *          остался бы нехоженым. Проверка соседняя, `TruncationUnderfootIsRefused`, усекает
 *          ДО открытия и стережёт иной путь - выборку записи по оглавлению
 *
 * @note Отказ снимателя сюда НЕ входит: `feed` октеты лишь копит, а разбор ведёт `next`, и
 *       отвечает подача отказом одному лишь несуществующему буферу - тому, какого файловая
 *       система не подаёт никогда. Заслон, её отказ выносящий, есть заслон последней руки
 *
 */
TEST(CodecAbcStorage, TruncationUnderfootIsRefusedByTheFeeding){
	// Выполняем отключение вывода журнала на приставку
	awh::log::mode({});
	// Временный файл проверки
	Temporary file("feeding-truncated");
	// Выполняем сборку контейнера с двумя записями
	const vector <uint8_t> container = build({"первая", "вторая"});
	// Хранилище контейнера на носителе
	abc::storage_t storage;
	...
	ASSERT_TRUE(storage.open(file.filename())) << "код отказа: " << abc::message(storage.error());
	// Выполняем проверку того, что полная длина снята целою
	ASSERT_EQ(storage.length(), static_cast <uint64_t> (container.size()));
	/**
	 * Выполняем усечение файла ВДВОЕ, минуя хранилище: так поступает всякий, кто правит
	 * файл рядом с нами
	 */
	ASSERT_EQ(::truncate(file.filename().c_str(), static_cast <off_t> (container.size() / 2)), 0)
		<< "усечь файл не удалось";
	// Сниматель контейнера
	abc::loader_t loader;
	// Выполняем проверку того, что подача усечённого файла отвечена отказом
	ASSERT_FALSE(storage.load(loader, 16)) << "усечённый файл подан снимателю успехом";
	/**
	 * Выполняем проверку того, что причина названа невозможностью чтения: усечение видно
	 * лишь коротким счётом поданного, и опознать его иначе неоткуда
	 */
	ASSERT_EQ(storage.error(), abc::error_t::UNREADABLE_SOURCE) << abc::message(storage.error());
	// Выполняем закрытие файла контейнера
	storage.close();
...
/**
 * @brief Проверка того, что работа чтения ловит усечение файла под ногами
 *
 * @details Заслон, сличающий длину прочитанного с затребованной, стережёт не пределы - те
 *          поверены соседом выше, - а КОРОТКОЕ ЧТЕНИЕ в законных пределах. Наводится оно
 *          усечением файла ПОСЛЕ открытия: полная длина снята при открытии и осталась
 *          прежней, а октетов на носителе убыло
 *
 * @warning Проверка соседняя, `TheSourceWorkGuardsItsOwnBounds`, этого заслона НЕ стережёт, и
 *          это измерено щупом 16.09.2026: обесточенный, он оставляет её зелёной, ибо всякое
 *          её чтение отвергается сличением пределов выше
 *
 */
TEST(CodecAbcStorage, TheSourceWorkCatchesTheTruncationUnderfoot){
	// Выполняем отключение вывода журнала на приставку
	awh::log::mode({});
	// Временный файл проверки
	Temporary file("source-truncated");
	// Выполняем сборку контейнера с двумя записями
	const vector <uint8_t> container = build({"первая", "вторая"});
	// Хранилище контейнера на носителе
	abc::storage_t storage;
	...
	// Буфер прочитанных октетов
	vector <uint8_t> result;
	// Выполняем проверку того, что чтение целого файла проходит
	ASSERT_TRUE(source(0, static_cast <size_t> (storage.length()), result))
		<< "чтение целого контейнера отвечено отказом";
	/**
	 * Выполняем усечение файла ВДВОЕ, минуя хранилище: полная длина хранилища при том
	 * остаётся прежней, и чтение идёт в законных её пределах
	 */
	ASSERT_EQ(::truncate(file.filename().c_str(), static_cast <off_t> (container.size() / 2)), 0)
		<< "усечь файл не удалось";
	/**
	 * Выполняем то же самое чтение: пределы законны, а октетов на носителе недостаёт
	 */
	ASSERT_FALSE(source(0, static_cast <size_t> (storage.length()), result))
		<< "усечённый файл отдал чтение успехом";
	/**
	 * Выполняем проверку того, что выдача очищена: обломок прочитанного потребителю,
	 * признака не проверившему, неотличим от целого чтения
	 */
	ASSERT_TRUE(result.empty()) << "отвергнутое чтение оставило обломок прочитанного";
	// Выполняем закрытие файла контейнера
	storage.close();
}
