 ...
 *       значения не искажало. Отменено 20.08.2026 разом у всех кодеков
 *
 */
 ...
 *
 * @details Решение от 30.08.2026 общее на все кодеки рамки, а тело приведения у кодека
 * ABC ДВА: своё у владеющего значения и своё у дерева документа. Правка, внесённая в
 * одно из них, оставила бы слои расходящимися - запись, прочитанная деревом, давала бы
 * одно, а тою же записью собранное значение - другое
 *
...
TEST(CodecAbcDocument, LayersAgreeOnNumberEdges){
	...
	auto same = [](const auto & source, const string & title) noexcept -> void {
		 ...
		 *
		 * @note Признак выведен 31.08.2026: опора обязана лежать ВНЕ проверяемого. Тем
		 *       же признаком пал доклад «разбор - сборка - разбор, расхождений ноль»,
		 *       где опорою служило то же, что и проверялось
		 */
		{
	 ...
	 *
	 * @note Проверено щупом 31.08.2026: машина без поднормальных
	 ...
	 * поверяя. Установлено 31.08.2026
	 */
 ...
 *
 * @details Согласие договоров всех семи кодеков на уровне общего API затребовано
 * 02.09.2026. Поверка держит ту его часть, какая у ABC заведена ради согласия: `size`,
 ...
 *
 * @note Довод подан 03.09.2026: то же найдено на соседнем кодеке, и поверка заведена
...
/**
 * @brief Повторный разбор одним документом хранилища не наращивает
 *
 * @details Документ, разбирающий записи одну за другой, обязан снимать прежде собранное
 * ЦЕЛИКОМ, а не дописывать новое к старому. Порок роста молчалив вдвойне: разбор
 * отвечает успехом, содержимое выдаётся верное, а память прибывает от записи к записи -
 * и у потока без конца это отказ по памяти через часы работы
 *
 * @note Свойство это держится на снятии содержимого, заведённом 03.09.2026 вместе с
 *       починкой отказа: до неё разбор, отказавший на середине, оставлял обломок, и
 *       обломок этот ложился в основание следующего разбора
 *
 * @warning Круги здесь чередуются с ОТКАЗАМИ намеренно, и в том вся сила проверки. Рост
 *          мог бы копиться именно на пути отказа - там снятие зовётся иным местом, чем
 *          на пути успеха, - и перебор одних лишь удавшихся разборов такого роста не
 *          показал бы вовсе, оставаясь зелёным при живом пороке
 *
 * @note Замер до закрепления: 200 000 кругов вперемежку с отказами, прирост пика памяти
 *       НУЛЕВОЙ. Проверка держит меньше кругов - ей довольно показать, что счёт узлов и
 *       содержимое не зависят от числа разборов, - а полный замер снимался щупом
 *
 */
TEST(CodecAbcDocument, RepeatedParsingDoesNotGrowTheStorage){
	// Собираемое значение
	abc::value_t root;
	/**
	 * Выполняем набор записи из восьми полей
	 */
	for(size_t i = 0; i < 8; i++)
		ASSERT_TRUE(root.insert(abc::value_t("поле-" + to_string(i)),
		 abc::value_t(string("значение-") + to_string(i))));
	...
	size_t nodes = 0;
	/**
	 * Выполняем перебор кругов разбора
	 */
	for(size_t round = 0; round < 64; round++){
		// Выполняем разбор годной записи документа
		ASSERT_TRUE(document.parse(record.data(), record.size())) << round;
		/**
		 * Если круг первый, запоминаем счёт узлов арены
		 */
		if(round == 0)
			// Запоминаем счёт узлов арены первого круга
			nodes = document.nodes();
		// Выполняем проверку того, что счёт узлов арены не изменился
		ASSERT_EQ(document.nodes(), nodes) << round;
		// Выполняем проверку того, что счёт детей корня не изменился
		ASSERT_EQ(document.size(), static_cast <size_t> (8)) << round;
		// Выполняем проверку достижимости содержимого по пути
		ASSERT_TRUE(document.has("/поле-7")) << round;
		// Выполняем наведение отказа негодною записью
		ASSERT_FALSE(document.parse(&corrupted, sizeof(corrupted))) << round;
		// Выполняем проверку снятия содержимого отказом
		ASSERT_EQ(document.nodes(), static_cast <size_t> (0)) << round;
	}
}
/**
 * @brief Розыск по имени даёт один ответ по обе стороны порога указателя
 *
 * @details Дорог розыска по имени у документа ДВЕ, и выбирается она числом полей
 * отображения: покуда полей меньше `INDEX_THRESHOLD`, розыск идёт перебором, а начиная
 * с порога заводится указатель имён, и розыск идёт по нему. Дороги эти обязаны отвечать
 * ОДНО И ТО ЖЕ, ибо порог есть решение о расходе, а не о смысле
 *
 * @note Проверка эта заведена 04.09.2026 по правилу «подменять и то, что кажется
 *       заведомо верным». Подмена порога с шестнадцати на пятнадцать не покрасила НИ
 *       ОДНОЙ проверки во всём наборе, а выглядело это ровно как исправность
 *
 * @note Безвредная подмена говорит одно из двух, и различать их надлежит РАЗБОРОМ, а не
 *       повторной подменой: либо проверка слепа, либо договор держит не это место.
 *       Здесь второе: порог есть решение о РАСХОДЕ, и обе дороги отвечают одно по
 *       устройству, - оттого сдвиг его и безвреден. Дырою же было отсутствие проверки на
 *       СОГЛАСИЕ дорог: разойдись они - скажем, оброни указатель пустое имя, - и
 *       отображение о шестнадцати полях повело бы себя иначе, чем о пятнадцати, причём
 *       молча. Стерегла бы такое расхождение не эта постоянная, а вот эта проверка
 *
 * @note Из пяти постоянных вида записи, перебранных подменою в тот день, безвредной
 *       вышла одна эта: `SPAN_LENGTH` покрасил 2 проверки, `TIME_WIDTH` - 6, `UUID_WIDTH`
 *       и `MAX_NODES` - по 8
 *
 * @warning Отображения берутся ПО ОБЕ СТОРОНЫ порога и на самом пороге: пятнадцать
 *          полей - дорога перебором, шестнадцать - дорога указателем, семнадцать -
 *          она же с запасом. Возьми проверка одно лишь малое отображение - и вся
 *          дорога указателя осталась бы непройденной вовсе, а зелень её была бы
 *          неотличима от зелени пройденной
 *
 * @warning Пустое имя проверяется ОТДЕЛЬНО и по обе стороны порога: имя это законно у
 *          ABC, а указатель строит отображение по строкам, и пустая строка есть тот
 *          случай, где вместилище имён вправе повести себя иначе, чем перебор
 *
 */
TEST(CodecAbcDocument, LookupAgreesOnBothSidesOfTheIndexThreshold){
	/**
	 * Выполняем перебор чисел полей вокруг порога заведения указателя
	 */
	for(const size_t count : {static_cast <size_t> (abc::INDEX_THRESHOLD - 1),
	 static_cast <size_t> (abc::INDEX_THRESHOLD), static_cast <size_t> (abc::INDEX_THRESHOLD + 1)}){
		// Собираемое отображение
		abc::value_t root;
		/**
		 * Выполняем набор отображения затребованного размера
		 */
		for(size_t i = 0; i < count; i++)
			// Выполняем добавление очередного поля отображения
			ASSERT_TRUE(root.insert(abc::value_t("поле-" + to_string(i)),
			 abc::value_t(static_cast <int64_t> (i)))) << count;
		// Выполняем сборку двоичной записи
		const vector <uint8_t> record = root.dump();
		// Объект документа
		abc::document_t document;
		...
		ASSERT_EQ(document.size(), count);
		/**
		 * Выполняем перебор всех полей отображения
		 */
		for(size_t i = 0; i < document.root().size(); i++){
			// Получаем имя очередного поля отображения
			const abc::document_t::value_t name = document.root().key(i);
			// Выполняем проверку пригодности полученного имени
			ASSERT_TRUE(name.valid()) << count << ":" << i;
			// Собираем строку имени поля отображения
			const string named(name.data());
			/**
			 * Выполняем проверку того, что розыск по имени ведёт К ТОМУ ЖЕ узлу
			 *
			 * @note Сличается тождество, а не содержимое: поля здесь несут разные
			 *       значения, но сличение содержимого прошло бы и у дороги, отдающей
			 *       узел верный по случайности
			 */
	 ...
	 */
	for(const size_t count : {static_cast <size_t> (abc::INDEX_THRESHOLD - 1),
	 static_cast <size_t> (abc::INDEX_THRESHOLD)}){
		// Собираемое отображение
		abc::value_t root;
		// Выполняем добавление поля, названного пустой строкой
		ASSERT_TRUE(root.insert(abc::value_t(string()), abc::value_t(string("пустое-имя"))));
		/**
		 * Выполняем набор отображения до затребованного размера
		 */
		...
		const abc::document_t::value_t value = document.root().get("");
		// Выполняем проверку того, что поле с пустым именем разыскано
		ASSERT_TRUE(value.valid()) << count;
		// Выполняем проверку содержимого поля с пустым именем
		ASSERT_EQ(string(value.data()), string("пустое-имя")) << count;
	}
}

/**
 * @brief Проверка достижимости имён, косую черту и знак отмены несущих
 *
 * @details Имя поля отображения у ABC есть строка произвольная, и косая черта в ней
 *          законна вполне. Путь же звенья свои косою чертой и разделяет, - оттого без
 *          отменяющих записей поле с косою чертой в имени недостижимо путём НИКАК:
 *          звено рвётся надвое, и обе половины ведут в пустоту
 *
 * @note Записи `~1` и `~0` взяты у RFC 6901 и едины со всею рамкой: у указателя JSON
 *       они те же, и порядок снятия тот же - слева направо один раз, а не двумя
 *       проходами. Снятие `~0` прежде `~1` обратило бы записанное `~01` в косую черту
 *       вместо `~1`
 *
 * @note Находка пришла от моста, где померено то же у ABC, и подтвердилась у таблицы
 *       CSV независимо. Строгость (знак отмены, за каким не стоит нуля либо единицы,
 *       делает путь недействительным) взята единою с JSON: мягкость означала бы, что
 *       имя `t~x` достижимо двояко
 *
 */
TEST(CodecAbcDocument, EscapedNamesAreReachableByPath) {
	// Собираемое дерево документа
	abc::value_t root(abc::kind_t::MAP);
	// Выполняем заведение поля, косую черту в имени несущего
	ASSERT_TRUE(root.insert("a/b", abc::value_t(static_cast <int64_t> (7))));
	// Выполняем заведение поля, знак отмены в имени несущего
	ASSERT_TRUE(root.insert("t~x", abc::value_t(static_cast <int64_t> (9))));
	// Выполняем заведение поля, обе записи разом несущего
	ASSERT_TRUE(root.insert("~0/1", abc::value_t(static_cast <int64_t> (5))));
	// Собираемая двоичная запись дерева
	vector <uint8_t> record;
	...
	ASSERT_TRUE(document.parse(record.data(), record.size())) << abc::message(document.error());
	/**
	 * @brief Метод извлечения целого значения по пути
	 *
	 * @param path путь к извлекаемому значению
	 * @return     извлечённое значение, ноль при отсутствии его
	 *
	 */
	const auto number = [&document](const string & path) noexcept -> int64_t {
		// Извлекаемое значение дерева документа
		const abc::document_t::value_t value = document.at(path);
		// Если значения по пути нет
		if(!value.valid())
			// Выводим ноль признаком отсутствия
			return 0;
		// Извлекаемое целое значение
		int64_t result = 0;
		// Выполняем извлечение целого значения
		(void) value.value(result);
		// Выводим извлечённое целое значение
		return result;
	};
	// Выполняем проверку достижимости имени с косою чертой отменяющей записью
	ASSERT_EQ(number("/a~1b"), 7);
	// Выполняем проверку достижимости имени со знаком отмены отменяющей записью
	ASSERT_EQ(number("/t~0x"), 9);
	// Выполняем проверку достижимости имени, обе записи несущего
	ASSERT_EQ(number("/~00~11"), 5);
	/**
	 * Выполняем проверку того, что косая черта ДОСЛОВНАЯ имени не даёт
	 *
	 * @note Половина эта и делает проверку зрячей: без неё она была бы зелена и при
	 *       разборе, берущем звено дословно, - `a~1b` тогда просто не нашлось бы, а
	 *       `a/b` нашлось бы двумя звеньями
	 */
	ASSERT_FALSE(document.has("/a/b"));
	/**
	 * Выполняем проверку недействительности пути с негодной отменяющей записью
	 *
	 * @warning Половина эта стережёт РАЗЛИЧЕНИЕ двух исходов у разбора пути: посредник
	 * отвечает ложью и на исчерпание пути, и на недействительность его. Без различения
	 * путь `/t~x` отдавал бы последнее дошедшее значение, то есть КОРЕНЬ дерева, - и
	 * потребитель получал бы всё дерево на месте отсутствующего поля
	 */
	ASSERT_FALSE(document.has("/t~x"));
	// Выполняем проверку недействительности пути со знаком отмены на конце
	ASSERT_FALSE(document.has("/поле~"));
	/**
	 * Выполняем проверку того, что владеющее значение отвечает тем же
	 *
	 * @note Правила пути держит один посредник на четверых, и проверка сличает оба
	 *       взгляда: разойдись они - разошлись бы молча
	 */
	ASSERT_TRUE(root.at("/a~1b").valid());
	// Выполняем проверку того, что дословная косая черта имени не даёт и владеющему
	ASSERT_FALSE(root.at("/a/b").valid());
	// Выполняем проверку недействительности негодной записи у владеющего значения
	ASSERT_FALSE(root.at("/t~x").valid());
}

/**
 * @brief Проверка причины отказа у разбора записи, дерева не давшей
 *
 * @details Причина `EMPTY_RECORD` стоит у документа в ДВУХ местах - у разбора и у
 *          сборки, - и стерегомо было лишь второе, да и то не стендом, а общею
 *          проверкою договора CodecContract.EmptyDocumentWritingDivergesByStandard.
 *          Первое молчало
 *
 * @note Место найдено щупом по местам отказа 04.09.2026 и ПЕРЕСПРОШЕНО полным набором
 *       прежде, чем звать его пробелом: стенд общих проверок договора не несёт, и
 *       место, стерегомое лишь ими, показывает молчащим. Переспрос место сборки и
 *       выгородил
 *
 * @warning Проверка эта стережёт причину, но НЕ то место, ради какого заводилась.
 * Пустую запись отвергает сам разбиратель, и до заслона у документа разбор не доходит:
 * подмена причины у документа не красит ничего, подмена у разбирателя красит эту
 * проверку и две соседних. Заслон у документа есть последняя рука на случай, если
 * разбиратель пустое пропустит, - и недостижим, покуда тот его отвергает
 *
 * @warning Записано это здесь нарочно: проверка, названная по месту, а стерегущая
 * другое, есть та же западня, что и довод, ссылающийся не на ту проверку. За один день
 * 04.09.2026 таких случаев вышло два
 *
 */
TEST(CodecAbcDocument, EmptyRecordParsingNamesItsCause) {
	...
	ASSERT_EQ(document.error(), abc::error_t::EMPTY_RECORD) << abc::message(document.error());
	// Выполняем проверку того, что дерево документа осталось пустым
	ASSERT_FALSE(document.root().valid());
}

/**
 * @brief Признак составимости выражения равенства для вида
 *
 * @details Определяется подстановкою: частный вид годится лишь тогда, когда выражение
 *          `value == value` составляется, и тогда берётся он; иначе берётся общий
 *
 * @param value сличаемое значение
 * @return      истина, если выражение равенства составляется
 *
 */
template <typename T>
static auto comparable(const T & value) -> decltype(value == value, true) {
	// Выводим признак составимости выражения равенства
	return true;
}
/**
 * @brief Откат для видов, равенства не несущих
 *
 * @return ложь признаком отсутствия равенства
 *
 */
static bool comparable(...) noexcept {
	// Выводим признак отсутствия равенства
	return false;
}
/**
 * @brief Проверка того, что у двух взглядов разные вопросы о равенстве
 *
 * @details У ВЛАДЕЮЩЕГО значения `operator ==` значит равенство СОДЕРЖИМОГО, а у курсора
 *          в разобранную арену равенство спрашивается словом `same` и значит ТОЖДЕСТВО
 *          узла - тот же документ и то же место в нём. Вопросы это разные, и оператор у
 *          курсора не заведён намеренно: два смысла у одного знака в одном кодеке были бы
 *          западнёю молчаливой - сличение вышло бы не тем, каким его читают
 *
 * @note Решение записано доводом при самом ходе `same`, а стерегомо не было: отсутствие
 *       оператора проверкою поведения не стережётся вовсе - его нельзя ни позвать, ни
 *       увидеть. Закрепляется оно сборкою, статическим утверждением о том, что выражение
 *       `курсор == курсор` НЕ составляется
 *
 * @note Приём взят у закрепления запрета копирования, где подмена показала: стеречь
 *       надлежит СВОЙСТВО, а не запись его
 *
 */
TEST(CodecAbcDocument, EqualityIsAskedByDifferentWordsInTwoViews) {
	// Владеющее значение документа
	const abc::value_t owning(static_cast <int64_t> (1));
	...
	ASSERT_TRUE(document.parse(record.data(), record.size())) << abc::message(document.error());
	// Курсор в разобранную арену документа
	const abc::document_t::value_t cursor = document.root();
	/**
	 * Выполняем проверку того, что у курсора равенства НЕТ
	 *
	 * @warning Половина эта и есть весь договор: заведись у курсора оператор равенства -
	 * и он значил бы тождество узла, тогда как у соседнего вида тот же знак значит
	 * равенство содержимого. Читающий сличение не различил бы их ничем
	 */
	ASSERT_FALSE(comparable(cursor));
	// Выполняем проверку того, что тождество курсора спрашивается словом
	ASSERT_TRUE(cursor.same(document.root()));
	// Выполняем проверку того, что тождество с недействительным курсором не утверждается
	ASSERT_FALSE(cursor.same(abc::document_t::value_t()));
}

/**
 * @brief Проверка того, что две непригодные ссылки тождественными не объявляются
 *
 * @details Тождество курсора есть совпадение УЗЛА, а непригодность узла не несёт вовсе:
 *          отсутствие с отсутствием не совпадает, совпасть могут лишь два присутствия.
 *          Объяви мы их тождественными - и всякий путь в никуда равнялся бы всякому
 *          другому пути в никуда
 *
 * @note Заведена находкой 04.09.2026: подмена вывода на `(!valid() && !value.valid())`
 *       не красила НИ ОДНОЙ проверки стенда, - договор был назван доводом и не стерегом
 *       ничем. Цена молчания видна из самого довода: проверка замкнутости обхода
...
TEST(CodecAbcDocument, InvalidCursorsAreNotIdentical) {
	...
	/**
	 * Две ссылки в никуда, полученные РАЗНЫМИ путями: одна именем несуществующего поля,
	 * другая - пустым курсором
	 */
	const abc::document_t::value_t missing = document.at("/нет такого поля");
	// Пустой курсор, узла не несущий вовсе
	const abc::document_t::value_t empty;
	// Выполняем проверку непригодности обеих ссылок
	ASSERT_FALSE(missing.valid());
	// Выполняем проверку непригодности пустого курсора
	ASSERT_FALSE(empty.valid());
	// Выполняем проверку того, что две непригодные ссылки тождественными не объявляются
	ASSERT_FALSE(missing.same(empty));
	// Выполняем проверку того, что непригодная ссылка не тождественна и самой себе
	ASSERT_FALSE(missing.same(missing));
	// Выполняем проверку того, что пустой курсор не тождествен пустому курсору
	ASSERT_FALSE(empty.same(abc::document_t::value_t()));
	// Выполняем проверку того, что пригодная ссылка тождественна самой себе
	ASSERT_TRUE(document.root().same(document.root()));
...
/**
 * @brief Проверка того, что непригодная ссылка отвечает пустотою на всякий спрос
 *
 * @details Ссылка в никуда получается путём обычным - именем несуществующего поля, - и
 *          зовущий волен спросить у неё что угодно, не поверяя пригодности. Заслон
 *          `!valid()` стоит у КАЖДОЙ работы ссылки порознь, и всякий обесточенный обратился
 *          бы в обращение к узлу по хранимому в ссылке месту - месту, какого в арене узлов
 *          нет
 *
 * @note Заслонов таких одиннадцать по счёту щупа нужности 08.09.2026, и все они молчали.
 *       Причина видна из набора: непригодность ссылки проверяли работою `valid()`, а
 *       спрашивать у непригодной прочее не пробовал никто. Оттого проверка перебирает
 *       работы ВСЕ до единой: заслоны эти друг друга не стерегут
 *
 * @note Утверждается пустой ОТВЕТ каждой работы, а не «не упало»: не падает и работа,
 *       отдавшая мусор из чужого узла
 *
 */
TEST(CodecAbcDocument, TheInvalidCursorAnswersEmptinessToEverything) {
	...
	ASSERT_TRUE(document.parse(writer.record().data(), writer.record().size()))
		<< abc::message(document.error());
	/**
	 * Выполняем перебор ссылок в никуда, полученных РАЗНЫМИ путями: пустым курсором и
	 * именем несуществующего поля. Оба пути обязаны давать одну и ту же немоту
	 */
	for(const abc::document_t::value_t & cursor : {abc::document_t::value_t(),
	 document.at("/нет такого поля")}){
		// Выполняем проверку того, что ссылка непригодна
		ASSERT_FALSE(cursor.valid()) << "ссылка объявлена пригодной, проверка негодна";
		// Вид непригодной ссылки обязан быть неопределённым
		ASSERT_EQ(cursor.type(), abc::type_t::UNDEFINED) << "непригодная ссылка назвала вид";
		// Число значений непригодной ссылки обязано быть нулевым
		ASSERT_EQ(cursor.size(), 0ull) << "непригодная ссылка назвала число значений";
		// Обращение по номеру у непригодной ссылки обязано давать непригодную же
		ASSERT_FALSE(cursor.at(0).valid()) << "непригодная ссылка отдала значение по номеру";
		// Обращение по имени у непригодной ссылки обязано давать непригодную же
		ASSERT_FALSE(cursor.get("поле").valid()) << "непригодная ссылка отдала значение по имени";
		// Имя поля у непригодной ссылки обязано быть непригодным
		ASSERT_FALSE(cursor.key(0).valid()) << "непригодная ссылка отдала имя поля";
		// Начало обхода у непригодной ссылки обязано быть непригодным
		ASSERT_FALSE(cursor.begin().valid()) << "непригодная ссылка отдала начало обхода";
		// Следующее значение у непригодной ссылки обязано быть непригодным
		ASSERT_FALSE(cursor.next().valid()) << "непригодная ссылка отдала следующее значение";
		// Признак поименованности у непригодной ссылки обязан быть ложью
		ASSERT_FALSE(cursor.keyed()) << "непригодная ссылка объявлена поименованной";
		// Октеты непригодной ссылки обязаны быть пусты
		ASSERT_TRUE(cursor.data().empty()) << "непригодная ссылка отдала октеты";
		// Порядок непригодной ссылки обязан быть нулевым
		ASSERT_EQ(cursor.exponent(), 0ll) << "непригодная ссылка назвала порядок";
		// Свой вид записи у непригодной ссылки обязан быть нулевым
		ASSERT_EQ(cursor.subtype(), 0ull) << "непригодная ссылка назвала свой вид записи";
		// Признак отрицательности у непригодной ссылки обязан быть ложью
		ASSERT_FALSE(cursor.negative()) << "непригодная ссылка объявлена отрицательной";
	}
	/**
	 * Выполняем проверку того, что ПРИГОДНАЯ ссылка на те же работы отвечает иначе: без
	 * того проверка прошла бы и у работ, отвечающих пустотою всегда
	 */
	{
		// Пригодная ссылка на корень разобранного документа
		const abc::document_t::value_t root = document.root();
		// Выполняем проверку пригодности ссылки на корень
		ASSERT_TRUE(root.valid()) << "ссылка на корень непригодна";
		// Вид корня обязан быть отображением
		ASSERT_EQ(root.type(), abc::type_t::MAP);
		// Число пар отображения обязано быть единицею
		ASSERT_EQ(root.size(), 1ull);
		// Обращение по имени обязано давать пригодную ссылку
		ASSERT_TRUE(root.get("поле").valid()) << "обращение по имени отдало непригодную ссылку";
		// Имя первой пары отображения обязано быть пригодным
		ASSERT_TRUE(root.key(0).valid()) << "имя первой пары непригодно";
		// Начало обхода обязано быть пригодным
		ASSERT_TRUE(root.begin().valid()) << "начало обхода непригодно";
	}
}
/**
 * @brief Проверка того, что спрос не по виду значения отвечен пустотою
 *
 * @details Работы ссылки спрашиваются у значения ЛЮБОГО вида: зовущий волен спросить имя
 *          поля у массива, порядок у строки, свой вид записи у числа. Заслоны вида стоят
 *          у каждой такой работы порознь и обязаны отвечать пустотою, а не толковать
 *          октеты чужого вида как свои
 *
 * @note Заслоны эти молчали все до единого (щуп нужности 08.09.2026), и молчание их иного
 *       рода, нежели у непригодной ссылки: там спроса не было вовсе, а здесь спрос был, но
 *       ВСЕГДА по виду. Круговой обход, каким проверяют дерево, спрашивает у значения ровно
 *       то, что в него уложено, - и мимо этих заслонов проходит, оставаясь зелёным
 *
 * @note Утверждается пустой ОТВЕТ, а не отказ: работы эти отказа не имеют вовсе и отвечают
 *       значением, оттого пустота и есть весь их сказ
 *
 */
TEST(CodecAbcDocument, TheDemandAgainstTheKindIsAnsweredWithEmptiness) {
	...
	// Выполняем укладку имени поля массива
	ASSERT_TRUE(writer.text("массив"));
	// Выполняем укладку массива о двух значениях
	ASSERT_TRUE(writer.arrayBegin(2));
	// Выполняем укладку первого значения массива
	ASSERT_TRUE(writer.number(static_cast <int64_t> (1)));
	// Выполняем укладку второго значения массива
	ASSERT_TRUE(writer.number(static_cast <int64_t> (2)));
	// Выполняем закрытие массива
	ASSERT_TRUE(writer.arrayEnd());
	// Выполняем укладку имени поля строки
	ASSERT_TRUE(writer.text("строка"));
	// Выполняем укладку значения строки
	ASSERT_TRUE(writer.text("значение"));
	// Выполняем закрытие отображения
	ASSERT_TRUE(writer.mapEnd());
	// Выполняем разбор собранной записи
	ASSERT_TRUE(document.parse(writer.record().data(), writer.record().size()))
		<< abc::message(document.error());
	// Ссылка на массив разобранного документа
	const abc::document_t::value_t array = document.at("/массив");
	// Выполняем проверку пригодности ссылки на массив
	ASSERT_TRUE(array.valid()) << "ссылка на массив непригодна";
	// Ссылка на строку разобранного документа
	const abc::document_t::value_t text = document.at("/строка");
	// Выполняем проверку пригодности ссылки на строку
	ASSERT_TRUE(text.valid()) << "ссылка на строку непригодна";
	/**
	 * Имя поля у МАССИВА: имён у массива нет, и спрос обязан быть отвечен пустотою
	 */
	ASSERT_FALSE(array.key(0).valid()) << "массив отдал имя поля";
	/**
	 * Обращение по имени у МАССИВА: указателя имён у массива нет вовсе
	 */
	ASSERT_FALSE(array.get("массив").valid()) << "массив отдал значение по имени";
	/**
	 * Обращение по номеру у СТРОКИ: вместимым она не является, и детей у неё нет
	 */
	ASSERT_FALSE(text.at(0).valid()) << "строка отдала значение по номеру";
	/**
	 * Обращение по номеру СВЕРХ числа значений: у массива их два, третьего нет
	 */
	ASSERT_FALSE(array.at(2).valid()) << "массив отдал значение сверх числа своих";
	/**
	 * Имя пары СВЕРХ числа пар отображения: у корня их две, третьей нет
	 */
	ASSERT_FALSE(document.root().key(2).valid()) << "отображение отдало имя сверх числа пар";
	/**
	 * Порядок у СТРОКИ: десятичного порядка строка не несёт
	 */
	ASSERT_EQ(text.exponent(), 0ll) << "строка назвала десятичный порядок";
	/**
	 * Свой вид записи у СТРОКИ: открытым расширением она не является
	 */
	ASSERT_EQ(text.subtype(), 0ull) << "строка назвала свой вид записи";
	/**
	 * Обращение ПУТЁМ по номеру сверх числа значений: путь обязан обрываться пустотою,
	 * а не толковать номер как имя
	 */
	ASSERT_FALSE(document.at("/массив/2").valid()) << "путь отдал значение сверх числа своих";
	/**
	 * Выполняем проверку того, что спрос ПО ВИДУ отвечается по существу: без того
	 * проверка прошла бы и у работ, отвечающих пустотою всегда
	 */
	ASSERT_TRUE(document.root().key(0).valid()) << "отображение имени первой пары не отдало";
	// Выполняем проверку того, что обращение по номеру у массива даёт значение
	ASSERT_TRUE(array.at(1).valid()) << "массив второго значения не отдал";
	// Выполняем проверку того, что обращение путём по номеру даёт значение
	ASSERT_TRUE(document.at("/массив/1").valid()) << "путь второго значения не отдал";
}
/**
 * @brief Проверка того, что отвергнутый разбор доходит до журнала
 *
 * @details Отказ разбора обязан быть объявлен журналу: зовущий видит лишь ложь возврата, а
 *          причину спрашивает не всякий, и журнал есть единственный путь наружу для
 *          стороннего наблюдателя
 *
 * @warning Проверка эта стережёт воронку `Reader::fail`, а НЕ `Document::fail`, и сказано
 *          это прямо, дабы следующий аудит не счёл иначе. Первая редакция называлась
 *          «воронка отказа документа» и была ЗЕЛЕНА, ничего не стерегши: щуп нужности
 *          08.09.2026 обесточил `Document::fail` - и не покраснело ничего. Разбор показал
 *          почему: донесение приходит от РАЗБИРАТЕЛЯ («ABC: tag is reserved for the
 *          future», работа `awh::codec::abc::Reader::fail`), а воронка документа зовётся
 *          всего из двух мест, и оба объявлены последними руками. Опора на чужое донесение
 *          есть та же слепота, что и опора на чужой заслон
 *
 * @note Воронок этого рода в кодеке СЕМЬ, и щуп нашёл молчащими все, до каких дошёл. Число
 *       это не совпадение, а свойство набора: причину спрашивали работою `error()`, а
 *       журнала не спрашивал никто
 *
 * @note Журнал здесь заводится СВОЙ: общий у набора погашен намеренно, а погашенный
 *       донесений не разносит вовсе
 *
 */
TEST(CodecAbcDocument, TheRefusedParseReachesTheJournal) {
	/**
	...
	ASSERT_FALSE(document.parse(broken.data(), broken.size())) << "негодная запись разобрана, проверка негодна";
	// Выполняем получение объявленной причины отказа
	const abc::error_t cause = document.error();
	...
	/**
	 * Выполняем проверку того, что донесение несёт ИМЕННО объявленную причину
	 */
...
/**
 * @brief Проверка того, что имена полей разного ВИДА повтором не считаются
 *
 * @details Сличение имён идёт по узлам поддерева, и вид значения есть часть узла наравне
 * с содержимым его. Два имени, сходящиеся содержимым, но разные видом, суть имена
 * РАЗНЫЕ, и правило повтора отсеивать их не вправе
 *
 * @note Случай этот настоящий, а не умозрительный: пустая строка и пустые двоичные
 * данные держат длину нулевую оба, содержимое обоих лежит в хранилище октетов, и
 * сличение содержимого их проходит по НУЛЮ октетов - то есть совпадает всегда. Различает
 * их единственно поверка вида, и без неё правило `FIRST` отсеяло бы пару, повтором не
 * являющуюся
 *
 * @note Щуп нужности 09.09.2026 нашёл поверку вида МОЛЧАЩЕЙ: набор сличал имена одного
 * вида, а имён разного вида с общим содержимым не подавал никто
 *
 */
TEST(CodecAbcDocument, KeysOfDifferentKindsAreNotDuplicates){
	/**
	 * Работа разбора записи затребованным правилом повтора имени
	 *
	 * @param record разбираемая запись
	 ...
	 * @return       признак успешности разбора
	 */
	auto parse = [](const vector <uint8_t> & record, abc::document_t & digest) noexcept -> bool {
		// Настройки разбора записи
		abc::reader_t::settings_t settings;
		// Выполняем установку правила оставления первого из повторяющихся значений
		settings.duplicates = abc::duplicate_t::FIRST;
		// Выводим признак успешности разбора записи в дерево документа
		return digest.parse(record.data(), record.size(), settings);
	};
	/**
	 * Отображение о двух парах, имена каких сходятся содержимым, но разнятся видом:
	 * {"": 1, <пустые двоичные данные>: 2}
	 */
	{
		// Разбираемая запись
		...
		/**
		 * Обе пары обязаны уцелеть: имена их разного вида, и повтором они не являются
		 */
		ASSERT_EQ(document.root().size(), 2u) << "имена разного вида сочтены повтором";
	}
	/**
	 * То же отображение, но имена ОДНОГО вида: {"": 1, "": 2}. Без этого проверка прошла
	 * бы и у отсева, не работающего вовсе
	 */
	{
		// Разбираемая запись
		...
		ASSERT_EQ(document.root().size(), 1u) << "повтор имени одного вида не отсеян";
	}
}
/**
 * @brief Проверка того, что отказ сборки отдаёт запись ПУСТОЙ, а не собранной наполовину
 *
 * @details Сборка записи из дерева идёт сборщиком, и сборщик вправе отвергнуть дерево:
 * строгий вид записи требует имён по возрастанию, а дерево, разобранное вне строгого
 * вида, их порядка не блюдёт. Отказ обязан отдавать запись ПУСТОЙ - собранная наполовину
 * запись есть запись негодная, и отличить её от годной зовущему нечем
 *
 * @note Щуп нужности 09.09.2026 нашёл распространение отказа сборки МОЛЧАЩИМ: набор
 * собирал записи лишь из деревьев, сборщиком принимаемых, и отказа сборки не наводил
 * никто
 *
 */
TEST(CodecAbcDocument, TheRefusedBuildingYieldsAnEmptyRecord){
	/**
	 * Отображение о двух парах, имена каких идут по УБЫВАНИЮ: {"b": 1, "a": 2}
	 */
	...
	ASSERT_TRUE(document.parse(record.data(), record.size()))
		<< "код отказа: " << abc::message(document.error());
	/**
	 * Сборка вне строгого вида отдаёт запись ту же самую: без этого проверка прошла бы
	 * и у сборки, отвергающей всякое дерево
	 */
	ASSERT_EQ(document.dump(), record) << "сборка вне строгого вида отдала иную запись";
	/**
	 * Сборка строгим видом отвергается порядком имён, и запись обязана выйти ПУСТОЙ
	 */
	{
		...
		// Выполняем сборку записи строгим видом
		const vector <uint8_t> strict = document.dump(settings);
		// Запись обязана выйти пустой
		ASSERT_TRUE(strict.empty()) << "отказавшая сборка отдала запись из "
			<< strict.size() << " октетов";
	}
}
/**
 * @brief Проверка того, что пустое дерево отвечает пустотою на всякий спрос
 *
 * @details Дерево бывает пустым до разбора и после отказа его, и всякая работа над таким
 * деревом обязана отвечать недействительным значением, а не заглядывать в узлы, каких
 * нет. Корень, обход по пути и обход по звеньям спрашиваются здесь ВСЕ до единого:
 * заслоны эти друг друга не стерегут
 *
 * @note Щуп нужности 09.09.2026 нашёл заслон пустого дерева МОЛЧАЩИМ: набор спрашивал
 * корень лишь у дерева РАЗОБРАННОГО, а до разбора его не спрашивал никто
 *
 */
TEST(CodecAbcDocument, TheEmptyTreeAnswersEmptinessToEveryDemand){
	...
	ASSERT_FALSE(document.at("/поле").valid()) << "пустое дерево отдало значение по пути";
	// Обход по пустому пути обязан отдать недействительное значение
	ASSERT_FALSE(document.at("").valid()) << "пустое дерево отдало значение по пустому пути";
	/**
	 * То же дерево после разбора отдаёт корень действительный: без этого проверка прошла
	 * бы и у работы, отвечающей пустотою всегда
	 */
	{
		// Разбираемая запись одного числа
		const vector <uint8_t> record = {0x07};
		// Выполняем разбор записи в дерево документа
		ASSERT_TRUE(document.parse(record.data(), record.size()))
			<< "код отказа: " << abc::message(document.error());
		// Корень разобранного дерева обязан быть действителен
		ASSERT_TRUE(document.root().valid()) << "разобранное дерево отдало недействительный корень";
	}
}
/**
 * @brief Проверка того, что спрос за пределом вместимого не отдаёт СОСЕДА
 *
 * @details Значения дерева лежат подряд, и вместимое отделено от следующего за ним лишь
 * размахом своим. Спрос, за предел вместимого выходящий, обязан отвечать
 * недействительным значением - а обойди он предел, отдал бы узел СОСЕДНИЙ, вполне
 * действительный и вполне чужой
 *
 * @note Проверка эта заведена по разбору молчащих заслонов 09.09.2026. Собратья их у
 * пустого дерева и у недействительной ссылки затенены поверкой действительности: узел за
 * концом дерева ею и отсеивается. Здесь же затенения НЕТ - за концом вместимого стоит
 * узел настоящий, - и оттого случай этот поверяется соседством нарочно: вместимое взято
 * НЕ последним значением перечня
 *
 */
TEST(CodecAbcDocument, TheDemandBeyondTheContainerDoesNotYieldTheNeighbour){
	/**
	 * Перечень о двух значениях, первое из каких есть отображение об одной паре, а
	 * второе - число: [{"a": 1}, 42]
	 */
	{
		// Разбираемая запись
		...
		ASSERT_TRUE(document.parse(record.data(), record.size()))
			<< "код отказа: " << abc::message(document.error());
		// Выполняем получение отображения, стоящего первым значением перечня
		const abc::document_t::value_t mapping = document.root().at(0);
		// Отображение обязано быть действительным и нести ровно одну пару
		ASSERT_TRUE(mapping.valid());
		ASSERT_EQ(mapping.size(), 1u);
		// Имя первой пары обязано быть отдано
		ASSERT_TRUE(mapping.key(0).valid()) << "имя первой пары не отдано";
		/**
		 * Имя пары ЗА пределом отображения отдано быть не обязано: за концом его стоит
		 * число, и обход предела отдал бы именно его
		 */
		ASSERT_FALSE(mapping.key(1).valid()) << "спрос за пределом отдал соседний узел";
	}
	/**
	 * Перечень о двух значениях, первое из каких есть ПУСТОЙ перечень: [[], 42]
	 */
	{
		// Разбираемая запись
		...
		ASSERT_TRUE(document.parse(record.data(), record.size()))
			<< "код отказа: " << abc::message(document.error());
		// Выполняем получение пустого перечня, стоящего первым значением
		const abc::document_t::value_t empty = document.root().at(0);
		// Пустой перечень обязан быть действительным и не нести ни одного значения
		ASSERT_TRUE(empty.valid());
		ASSERT_EQ(empty.size(), 0u);
		/**
		 * Обход пустого вместимого обязан начинаться недействительным значением: за
		 * концом его стоит число, и обход предела отдал бы именно его
		 */
		ASSERT_FALSE(empty.begin().valid()) << "обход пустого вместимого отдал соседний узел";
	}
	/**
	 * Перечень строк, спрашиваемый ИМЕНЕМ поля: ["a", "b"]
	 *
	 * @note Случай этот того же рода и оттого стоит здесь же: указатель имён полей
	 * строится обходом пар «имя-значение», и перечень строк, пройденный тем же обходом,
	 * дал бы имени `a` значением СОСЕДНЮЮ строку `b`
	 */
	{
		// Разбираемая запись
		...
		// Выполняем получение корня дерева документа
		const abc::document_t::value_t root = document.root();
		// Корень обязан быть действительным перечнем о двух значениях
		ASSERT_TRUE(root.valid());
		ASSERT_EQ(root.size(), 2u);
		// Спрос имени поля у перечня отдан быть не обязан
		ASSERT_FALSE(root.get("a").valid()) << "перечень отдал значение по имени поля";
	}
}
