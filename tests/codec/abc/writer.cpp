...
/**
 * @brief Проверка объявления размаха у вместимого, стоящего первым в записи
 *
 * @details Смещение отведённого места записи размаха хранится числом, а ноль служит
 *          признаком того, что размах не объявлен. Годится ноль признаком по доводу:
 *          перед размахом лежит хотя бы метка расширения, и нулевым смещение его быть не
 *          может НИКОГДА. Довод этот поверяется лишь там, где метка расширения стоит на
 *          смещении НОЛЬ, - сиречь у вместимого, открывающего запись
 *
 * @note Проверки размаха, стоящие выше, открывают запись отображением, и метка
 *       расширения ложится у них на смещение не первое. Довод про ноль они не поверяют
 *       вовсе: смещение у них и без того велико
 *
 * @note Опора вне сборки: размах сличается ЧИСЛОМ, снятым с записи по объявленной
 *       раскладке, и разбором записи в дерево. Признак успеха сборки сошёлся бы и у
 *       записи, где размах объявлен нулём
 *
 */
TEST(CodecAbcWriter, SpannedAtRecordStart) {
	// Сборщик бинарной записи
	abc::writer_t writer(::logger());
	// Получаем настройки сборки записи
	abc::writer_t::settings_t settings = writer.settings();
	// Выполняем установку порога объявления размаха, вмещающего всякое вместимое
	settings.spanned = 1;
	// Выполняем установку настроек сборки записи
	writer.settings(settings);
	// Выполняем открытие массива из трёх значений ПЕРВЫМ в записи
	ASSERT_TRUE(writer.arrayBegin(3)) << "код отказа: " << abc::message(writer.error());
	/**
	 * Выполняем укладку значений массива
	 */
	for(uint64_t i = 0; i < 3; i++)
		// Выполняем укладку очередного значения массива
		ASSERT_TRUE(writer.number(i)) << "код отказа: " << abc::message(writer.error());
	// Выполняем закрытие массива
	ASSERT_TRUE(writer.arrayEnd()) << "код отказа: " << abc::message(writer.error());
	// Собранная запись
	const vector <uint8_t> & record = writer.record();
	// Запись обязана быть собрана целиком
	ASSERT_TRUE(writer.complete()) << "код отказа: " << abc::message(writer.error());
	// Метка расширения размаха обязана стоять ПЕРВЫМ октетом записи
	ASSERT_EQ(record.at(0), 0xE3);
	// Записи обязано достать на метку, размах и само вместимое
	ASSERT_GT(record.size(), 9ul);
	// Объявленный размах вместимого
	uint64_t spanned = 0;
	/**
	 * Выполняем снятие объявленного размаха: восемь октетов за меткою, от младшего
	 * октета к старшему
	 */
	for(size_t i = 0; i < 8; i++)
		// Выполняем внесение очередного октета объявленного размаха
		spanned |= (static_cast <uint64_t> (record.at(1 + i)) << (i * 8));
	/**
	 * Объявленный размах обязан отвечать длине вместимого: ноль означал бы, что
	 * смещение места размаха сочтено признаком отсутствия и размах не вписан вовсе
	 */
	...
	// Разобранный массив обязан нести три значения
	ASSERT_EQ(document.root().size(), 3u);
}
/**
 * @brief Проверка того, что строгий вид отвергает повтор имени и при снятой поверке
 *
 * @details Настройки сборки несут ДВА правила об одном и том же: отказ на повторяющееся
 *          имя поля и строгий вид, обязывающий вести имена по возрастанию. Правила эти
 *          независимы, и потребитель вправе снять первое, оставив второе. Повтор при том
 *          обязан отвергаться ПОРЯДКОМ - повторное имя возрастающим не бывает
 *
 * @note Настройки приходят от потребителя, сиречь со стороны такой же ЧУЖОЙ, как провод:
 *       сочетание их кодек не собирает, а принимает. Сочетание «строгий вид без поверки
 *       повторов» противоречиво лишь с виду, и поверено оно замером, а не доводом
 *
 * @note Коды отказа сличаются, а не признак: правила разные, и отказ обязан называть
 *       ВИНОВНОЕ - `DUPLICATE_KEY` при поверке повторов, `UNORDERED_KEY` при строгом виде
 *       без неё. Сличение признака не отличило бы одно правило от другого
 *
 */
TEST(CodecAbcWriter, CanonicalRefusesDuplicateByOrder) {
	/**
	 * @brief Функция укладки отображения с повторяющимся именем поля
	 *
	 * @param canonical  признак строгого вида записи
	 * @param duplicates признак отказа на повторяющееся имя поля
	 * @param error      код отказа сборки записи
	 * @return           признак успешности сборки
	 *
	 */
	auto assemble = [](const bool canonical, const bool duplicates, abc::error_t & error) noexcept -> bool {
		// Сборщик бинарной записи
		abc::writer_t writer(::logger());
		// Получаем настройки сборки записи
		abc::writer_t::settings_t settings = writer.settings();
		...
		// Выполняем установку признака отказа на повторяющееся имя поля
		settings.duplicates = duplicates;
		// Выполняем установку настроек сборки записи
		writer.settings(settings);
		// Признак успешности укладки отображения
		bool result = writer.mapBegin(2);
		// Выполняем укладку первого поля отображения
		result = (result && writer.text("а") && writer.number(static_cast <uint64_t> (1)));
		// Выполняем укладку второго поля отображения с тем же именем
		result = (result && writer.text("а") && writer.number(static_cast <uint64_t> (2)));
		// Выполняем закрытие отображения
		result = (result && writer.mapEnd());
		// Выполняем снятие кода отказа сборки записи
		error = writer.error();
		// Выводим признак успешности укладки отображения
		return result;
	};
	// Код отказа сборки записи
	abc::error_t error = abc::error_t::NONE;
	// При поверке повторов вне строгого вида отказ обязан быть о повторе имени
	EXPECT_FALSE(assemble(false, true, error));
	// Отказ обязан называть виновным повторяющееся имя поля
	EXPECT_EQ(error, abc::error_t::DUPLICATE_KEY);
	// Вне строгого вида и без поверки повторов запись обязана собираться
	EXPECT_TRUE(assemble(false, false, error)) << "код отказа: " << abc::message(error);
	// При строгом виде и поверке повторов отказ обязан быть о повторе имени
	EXPECT_FALSE(assemble(true, true, error));
	// Отказ обязан называть виновным повторяющееся имя поля
	EXPECT_EQ(error, abc::error_t::DUPLICATE_KEY);
	/**
	 * При строгом виде БЕЗ поверки повторов отказ обязан прийти от ПОРЯДКА: повторное
	 * имя возрастающим не бывает, и правило порядка ловит его само
	 */
	EXPECT_FALSE(assemble(true, false, error));
	// Отказ обязан называть виновным порядок имён, а не повтор
	EXPECT_EQ(error, abc::error_t::UNORDERED_KEY);
}
/**
 * @brief Проверка того, что вид числа записан, а не выведен из величины
 *
 * @details Решение это объявлено намеренным у самого перечня видов: вид числа несётся
 *          ЗАПИСЬЮ, а не выводится из величины. Край его - число ЦЕЛОЕ ПО ВЕЛИЧИНЕ,
 *          уложенное видом дробным: соблазн уложить `1.0` целым велик, запись выходит
 *          короче на семь октетов, и разбор вернул бы ту же величину. Вернул бы он её
 *          иным ВИДОМ, а вид есть часть записанного
 *
 * @note Проверки видов, стоящие в наборе, укладывают числа НЕЦЕЛЫЕ (`-2.25`), и решения
 *       этого не поверяют вовсе: у нецелого дробный вид вынужден, а не выбран
 *
 * @note Сличается и метка, и разобранный вид узла: метка стережёт укладку, вид узла -
 *       обратный путь. Одной метки мало - разбор волен свести дробное к целому сам
 *
 */
TEST(CodecAbcWriter, NumberKindIsRecordedNotDeduced) {
	/**
	 * @brief Структура поверяемого случая вида числа
	 *
	 */
	struct probe_t {
		// Ожидаемая метка записи числа
		uint8_t tag;
		// Ожидаемая длина записи в октетах
		size_t length;
		// Ожидаемый вид разобранного узла
		abc::type_t type;
		// Имя поверяемого случая
		const char * name;
	};
	// Сборщик записи целого числа
	abc::writer_t whole(::logger());
	// Выполняем укладку целого числа
	ASSERT_TRUE(whole.number(static_cast <uint64_t> (1)));
	// Сборщик записи дробного числа одинарной точности
	abc::writer_t single(::logger());
	// Выполняем укладку дробного числа одинарной точности, целого по величине
	ASSERT_TRUE(single.number(static_cast <float> (1.0f)));
	// Сборщик записи дробного числа двойной точности
	abc::writer_t twofold(::logger());
	// Выполняем укладку дробного числа двойной точности, целого по величине
	ASSERT_TRUE(twofold.number(static_cast <double> (1.0)));
	// Перечень поверяемых случаев вида числа
	const vector <pair <const vector <uint8_t> *, probe_t>> probes = {
		{&whole.record(), {0x01, 1, abc::type_t::UINT8, "1: целое остаётся целым"}},
		{&single.record(), {static_cast <uint8_t> ((static_cast <uint8_t> (abc::group_t::SINGLE) << 5) |
		 static_cast <uint8_t> (abc::single_t::FLOAT)), 5, abc::type_t::FLOAT,
		 "2: дробное одинарной точности остаётся дробным"}},
		{&twofold.record(), {static_cast <uint8_t> ((static_cast <uint8_t> (abc::group_t::SINGLE) << 5) |
		 static_cast <uint8_t> (abc::single_t::DOUBLE)), 9, abc::type_t::DOUBLE,
		 "3: дробное двойной точности остаётся дробным"}}
	};
	/**
	 * Выполняем перебор поверяемых случаев вида числа
	 */
	for(auto & probe : probes){
		// Собранная запись числа
		const vector <uint8_t> & record = (* probe.first);
		// Длина записи обязана отвечать виду числа
		EXPECT_EQ(record.size(), probe.second.length) << probe.second.name;
		// Запись обязана быть не пуста
		ASSERT_FALSE(record.empty()) << probe.second.name;
		...
		EXPECT_EQ(document.root().type(), probe.second.type) << probe.second.name;
	}
}
/**
 * @brief Проверка отсева повтора имени поля при большом их числе
 *
 * @details Поверка повторов у сборки шла ОБХОДОМ всех прежних имён с полным сличением
 * октетов на всякой паре и росла квадратом их количества, тогда как у разбора та же
 * поверка давно ведётся свёрткой и указателем имён. Замер 03.09.2026 на отображении в
 * 16 000 имён: сборка 590 мс против 5.3 мс при снятой поверке, и она была вчетверо с
 * лишним медленнее РАЗБОРА той же записи. После заведения свёртки с указателем - 6.9 мс
 *
 * @note Поверка сличает не скорость, а ПРАВИЛЬНОСТЬ на всех трёх дорогах устройства:
 * ниже порога заведения указателя (обход), сразу за порогом (указатель заведён) и за
 * перестроением его (указатель расширен вдвое). Скорость мерить проверкой нельзя -
 * она зависит от машины, - а вот дороги обязаны отвечать одинаково
 *
 * @note Повтор ставится ПЕРВЫМ именем, а не последним: последнее лежит в указателе
 * ближе всего, и повтор его отсеялся бы даже при негодном указателе
 *
 */
TEST(CodecAbcWriter, DuplicateKeyIsCaughtAtEveryScale){
	/**
	 * Выполняем перебор числа имён по всем трём дорогам устройства поверки
	 */
	for(const size_t count : {static_cast <size_t> (8), static_cast <size_t> (40),
	 static_cast <size_t> (200)}){
		// Сборка бинарной записи
		abc::writer_t writer(::logger());
		// Выполняем укладку начала отображения неопределённой длины
		ASSERT_TRUE(writer.mapBegin()) << "число имён: " << count;
		/**
		 * Выполняем укладку всех пар отображения
		 */
		for(size_t i = 0; i < count; i++){
			// Выполняем укладку имени очередной пары
			ASSERT_TRUE(writer.text("поле-" + to_string(i))) << "число имён: " << count;
			// Выполняем укладку значения очередной пары
			ASSERT_TRUE(writer.number(static_cast <uint64_t> (i))) << "число имён: " << count;
		}
		// Повтор ПЕРВОГО имени обязан быть отвергнут
		ASSERT_FALSE(writer.text(string{"поле-0"})) << "число имён: " << count;
		// Поводом отказа обязан стоять повтор имени поля
		ASSERT_EQ(writer.error(), abc::error_t::DUPLICATE_KEY) << "число имён: " << count;
	}
	/**
	 * Имя, не бывшее прежде, обязано быть принято при том же числе имён
	 */
	{
		// Сборка бинарной записи
		abc::writer_t writer(::logger());
		...
		ASSERT_TRUE(writer.mapBegin());
		/**
		 * Выполняем укладку двухсот пар отображения
		 */
		for(size_t i = 0; i < 200; i++){
			// Выполняем укладку имени очередной пары
			ASSERT_TRUE(writer.text("поле-" + to_string(i)));
			// Выполняем укладку значения очередной пары
			ASSERT_TRUE(writer.number(static_cast <uint64_t> (i)));
		}
		// Имя, прежде не бывшее, обязано быть принято
		ASSERT_TRUE(writer.text(string{"поле-иное"}));
		// Выполняем укладку значения принятой пары
		ASSERT_TRUE(writer.number(static_cast <uint64_t> (1)));
		// Выполняем укладку конца отображения
		ASSERT_TRUE(writer.mapEnd());
		...
		ASSERT_TRUE(document.parse(writer.record().data(), writer.record().size()))
			<< "код отказа: " << abc::message(document.error());
		// Пар у разобранного отображения обязано выйти двести одна
		ASSERT_EQ(document.root().size(), static_cast <size_t> (201));
	}
	/**
	 * Указатель имён обязан быть свой у ВСЯКОГО вместимого: имя, бывшее у внешнего,
	 * во вложенном повтором не является
	 */
	{
		// Сборка бинарной записи
		abc::writer_t writer(::logger());
		// Выполняем укладку начала внешнего отображения
		ASSERT_TRUE(writer.mapBegin());
		/**
		 * Выполняем укладку сорока пар внешнего отображения
		 */
		...
		// Выполняем укладку имени пары, несущей вложенное отображение
		ASSERT_TRUE(writer.text(string{"вложенное"}));
		// Выполняем укладку начала вложенного отображения
		ASSERT_TRUE(writer.mapBegin());
		// Имя, бывшее у внешнего отображения, во вложенном повтором не является
		ASSERT_TRUE(writer.text(string{"поле-0"}));
		// Выполняем укладку значения пары вложенного отображения
		ASSERT_TRUE(writer.number(static_cast <uint64_t> (1)));
		// Повтор же в самом вложенном обязан быть отвергнут
		ASSERT_FALSE(writer.text(string{"поле-0"}));
		// Поводом отказа обязан стоять повтор имени поля
		ASSERT_EQ(writer.error(), abc::error_t::DUPLICATE_KEY);
	}
}
/**
 * @brief Писатель не производит записи, какой читатель не примет
 *
 * @details Потолок вложенности стоит у обоих - у сборки и у разбора, - и стоять он
 * обязан НА ОДНОМ И ТОМ ЖЕ месте. Разойдись они на единицу в одну сторону - и писатель
 * стал бы выдавать запись, разбору не подлежащую, то есть кодек принимал бы от
 * потребителя то, чего сам прочесть не может; разойдись в другую - и запись, читателем
 * принимаемая, оказалась бы кодеку недоступна к сборке
 *
 * @warning Сличается КРАЙ, а не середина, и в том вся сила проверки. Глубина в сотню
 *          уровней прошла бы у обоих и при потолках, разъехавшихся на единицу: расхождение
 *          видно ровно на последнем допустимом уровне и на первом недопустимом. Оттого
 *          здесь и берутся четыре глубины подряд вокруг потолка, а не одна произвольная
 *
 * @note Проверка эта стережёт СОГЛАСИЕ двух постоянных, а не работу каждой порознь.
 *       Подними кто-нибудь потолок у одной половины, не тронув другой, - все проверки
 *       глубины у обеих останутся зелены, ибо всякая судит свою половину сама по себе
 *
 */
TEST(CodecAbcWriter, TheDepthCeilingAgreesWithTheReader){
	/**
	 * Работа сборки значения затребованной глубины вложенности
	 *
	 * @note Глубина здесь считается ВМЕСТИМЫМИ, а не уровнями вообще: лист цепи есть
	 *       число и вместимым не является. Счёт этот и есть то, чем меряется потолок
	 */
	const auto nested = [](const size_t depth) noexcept -> abc::value_t {
		// Собираемое значение, листом цепи вложенности являющееся
		abc::value_t result(static_cast <int64_t> (1));
		/**
		...
		for(size_t i = 0; i < depth; i++){
			...
			abc::value_t wrapper;
			/**
			 * Если обернуть собранное значение не вышло
			 */
			if(!wrapper.push(result))
				// Выводим непригодное значение признаком неудачи
				return abc::value_t(abc::kind_t::NONE);
	...
	/**
	 * Выполняем перебор глубин вокруг потолка вложенности
	 */
	for(const size_t depth : {static_cast <size_t> (1023), static_cast <size_t> (1024),
	 static_cast <size_t> (1025), static_cast <size_t> (1026)}){
		// Собираемое значение затребованной глубины
		const abc::value_t value = nested(depth);
		// Выполняем проверку того, что значение собрано владеющим деревом
		ASSERT_TRUE(value.valid()) << depth;
		// Собираемая двоичная запись
		vector <uint8_t> record;
		// Код отказа сборки записи
		abc::error_t error = abc::error_t::NONE;
		// Признак успешности сборки двоичной записи
		const bool written = value.dump(record, error);
		/**
		 * Если глубина лежит В ПРЕДЕЛАХ потолка
		 */
		if(depth <= static_cast <size_t> (abc::MAX_DEPTH)){
			// Выполняем проверку успешности сборки записи
			ASSERT_TRUE(written) << depth << ": " << abc::message(error);
			 ...
			 *
			 * @note Предел разбора снят нулём намеренно: сличается потолок МОДУЛЯ, а не
			 *       настройка, и настройка, поставленная тесной, скрыла бы расхождение
			 */
			...
			// Выполняем проверку успешности разбора собранной записи
			ASSERT_TRUE(document.parse(record.data(), record.size()))
				<< depth << ": " << abc::message(document.error());
			// Выполняем проверку пригодности корня разобранного дерева
			ASSERT_TRUE(document.root().valid()) << depth;
		/**
		 * Иначе глубина потолок перерастает
		 */
		} else {
			// Выполняем проверку отказа сборки записи чрезмерной глубины
			ASSERT_FALSE(written) << depth;
			// Выполняем проверку названной причины отказа сборки
			ASSERT_EQ(error, abc::error_t::DEPTH_EXCEEDED) << abc::message(error);
			// Выполняем проверку пустоты записи, сборка какой отвергнута
			ASSERT_TRUE(record.empty()) << depth;
		}
	}
	/**
	 * Половина вторая: потолок сличается ЧИСЛОМ, а не именем своим
	 *
	 * @warning Половина эта дописана 04.09.2026 по следам грубой подмены: потолок,
	 * сдвинутый с 1024 до ЧЕТЫРЁХ, не покрасил ни единой проверки стенда - включая
	 * половину первую, заведённую как раз ради него. Довод её был верен, а устройство
	 * слепо: ветвление её спрашивает `depth <= MAX_DEPTH`, то есть сличает потолок С
	 * НИМ ЖЕ САМИМ. Сдвинься он куда угодно - обе стороны уедут согласно, писатель с
	 * читателем сойдутся, и проверка останется зелена, тогда как всякий документ глубже
	 * четырёх перестанет читаться
	 *
	 * @note Довод, по какому потолок сличается ЛИТЕРАЛОМ, тот же, что у величин договора
	 * с носителем: смена его обращает прежде читаемые контейнеры в отвергаемые. Ссылка
	 * на имя стережёт лишь СОГЛАСИЕ двух дорог, а величину не стережёт ничто
	 *
	 * @note Грубая подмена - до заведомо достижимой малости, а не на единицу - есть щуп
	 * владельца разметки: сдвиг на единицу слеп ко всякому пределу, у какого нет проверки
	 * ровно на границе, и молчание его означает «на границе никто не стоит», а вовсе не
	 * «предел не стережётся»
	 *
	 */
	{
		// Выполняем проверку величины потолка вложенности
		ASSERT_EQ(abc::MAX_DEPTH, 0x400u);
		// Собираемое значение глубины, потолку заведомо подлежащей
		const abc::value_t value = nested(1000);
		// Выполняем проверку того, что значение собрано владеющим деревом
		...
		abc::error_t error = abc::error_t::NONE;
		/**
		 * Выполняем проверку того, что глубина тысячи потолком ПРИНИМАЕТСЯ
		 *
		 * @note Число здесь стоит записанным, а не выведенным из потолка, намеренно:
		 *       выведенное уехало бы вместе с ним
		 */
		...
		// Выполняем установку настроек документа
		document.settings(settings);
		// Выполняем проверку успешности разбора записи глубины тысячи
		ASSERT_TRUE(document.parse(record.data(), record.size()))
...
/**
 * @brief Проверка причин отказа у сборки значений, собираемых кусками
 *
 * @details Щуп по местам отказа показал у сборщика три молчащих места. Одно из них -
 *          заслон длины записи свыше четырёх гигабайт - требует настоящих четырёх
 *          гигабайт октетов и оттого стенду не подлежит; два прочих достижимы вполне и
 *          закрепляются здесь
 *
 * @note У причины «глубина» у сборщика ДВА места - одно при открытии вместимого, другое
 *       при начале сборки кусками, - и запись из вложенных перечней идёт первым, оставляя
 *       второе непокрытым. Довод общий: сличать надлежит число ПРОВЕРЕННЫХ мест с числом
 *       мест, причину наводящих
 *
 */
TEST(CodecAbcWriter, SegmentedBuildingNamesItsCauses) {
	/**
	 * Причина первая: строка внутри двоичных данных, собираемых кусками
	 */
	{
		// Сборка бинарной записи
		abc::writer_t writer(::logger());
		// Выполняем начало сборки двоичных данных кусками
		ASSERT_TRUE(writer.blobBegin()) << abc::message(writer.error());
		// Выполняем проверку отказа укладки строки куском двоичных данных
		ASSERT_FALSE(writer.text("кусок"));
		// Выполняем проверку названной причины отказа
		ASSERT_EQ(writer.error(), abc::error_t::INVALID_SEGMENT) << abc::message(writer.error());
	}
	/**
	 * Причина вторая: начало сборки кусками за пределом глубины вложенности
	 */
	{
		// Сборка бинарной записи
		abc::writer_t writer(::logger());
		...
		// Выполняем установку настроек сборки записи
		writer.settings(settings);
		// Выполняем открытие перечня неопределённой длины
		ASSERT_TRUE(writer.arrayBegin()) << abc::message(writer.error());
		// Выполняем проверку отказа начала сборки кусками за пределом глубины
		ASSERT_FALSE(writer.textBegin());
		// Выполняем проверку названной причины отказа
		ASSERT_EQ(writer.error(), abc::error_t::DEPTH_EXCEEDED) << abc::message(writer.error());
	}
}
/**
 * @brief Проверка того, что отказавшая сборка не принимает ни одной укладки
 *
 * @details Сборщик, однажды отвеченный отказом, обязан отвергать ВСЯКУЮ дальнейшую укладку:
 *          запись, собранная поверх отказа, вышла бы наполовину годной - начало её
 *          правильно, а хвост лёг мимо объявленного, - и негодность эта не читалась бы
 *          ничем до самого разбора
 *
 * @note Заслон этот стоит у КАЖДОЙ работы укладки порознь (одиннадцать мест по счёту щупа
 *       08.09.2026), и все одиннадцать молчали. Причина видна из набора: отказ у нас
 *       наводили, но следом за ним сборщика бросали, а не пользовались им далее. Оттого
 *       проверка перебирает работы ВСЕ до единой, а не берёт одну за образец: заслоны эти
 *       друг друга не стерегут, и пропуск любой работы оставил бы её слепой
 *
 * @note Утверждается ДЕЙСТВИЕ - отказ укладки, - а не причина: причину заводит то самое
 *       переполнение, каким отказ и наведён, и утверждать её значило бы стеречь чужой
 *       заслон
 *
 */
TEST(CodecAbcWriter, TheFailedBuildingAcceptsNothingFurther) {
	// Сборщик записи контейнера
	abc::writer_t writer(::logger());
	/**
	 * Выполняем объявление массива РОВНО об одном значении и укладку двух: вторая
	 * укладка переполняет объявленное и отвечается отказом
	 */
	ASSERT_TRUE(writer.arrayBegin(1));
	// Выполняем укладку единственного объявленного значения
	ASSERT_TRUE(writer.boolean(true));
	// Выполняем проверку того, что укладка сверх объявленного отвечена отказом
	ASSERT_FALSE(writer.boolean(false)) << "укладка сверх объявленного удалась, проверка негодна";
	/**
	 * Выполняем проверку того, что причина названа переполнением вместимого: без того
	 * отказ мог бы принадлежать иному заслону, и дальнейшее стерегло бы не то состояние
	 */
	ASSERT_EQ(writer.error(), abc::error_t::CONTAINER_OVERFLOW)
		<< "причина: " << abc::message(writer.error());
	// Октеты, подаваемые работам укладки двоичного вида
	const vector <uint8_t> octets(16, 0x41);
	/**
	 * Выполняем перебор ВСЕХ работ укладки значений: отказавший сборщик обязан отвергнуть
	 * каждую из них
	 */
	ASSERT_FALSE(writer.boolean(true)) << "отказавший сборщик принял истинность";
	ASSERT_FALSE(writer.number(static_cast <int64_t> (-1))) << "отказавший сборщик принял целое со знаком";
	ASSERT_FALSE(writer.number(static_cast <uint64_t> (1))) << "отказавший сборщик принял целое без знака";
	ASSERT_FALSE(writer.number(0.5)) << "отказавший сборщик принял дробное двойной точности";
	ASSERT_FALSE(writer.number(0.5f)) << "отказавший сборщик принял дробное одинарной точности";
	ASSERT_FALSE(writer.text("строка")) << "отказавший сборщик принял строку";
	ASSERT_FALSE(writer.blob(octets.data(), octets.size())) << "отказавший сборщик принял двоичные данные";
	ASSERT_FALSE(writer.timestamp(1755500000)) << "отказавший сборщик принял отметку времени";
	ASSERT_FALSE(writer.uuid(octets.data(), octets.size())) << "отказавший сборщик принял опознаватель";
	ASSERT_FALSE(writer.decimal(octets.data(), octets.size(), false, 0)) << "отказавший сборщик принял десятичное";
	ASSERT_FALSE(writer.custom(0x20, octets.data(), octets.size())) << "отказавший сборщик принял свой вид";
	ASSERT_FALSE(writer.textBegin()) << "отказавший сборщик принял начало строки кусками";
	ASSERT_FALSE(writer.blobBegin()) << "отказавший сборщик принял начало данных кусками";
	/**
	 * Выполняем проверку того, что и вместимые отказавшим сборщиком не принимаются
	 */
	ASSERT_FALSE(writer.arrayBegin()) << "отказавший сборщик принял начало массива";
	ASSERT_FALSE(writer.arrayBegin(2)) << "отказавший сборщик принял начало массива о двух значениях";
	ASSERT_FALSE(writer.mapBegin()) << "отказавший сборщик принял начало отображения";
	ASSERT_FALSE(writer.mapBegin(2)) << "отказавший сборщик принял начало отображения о двух парах";
	/**
	 * Выполняем проверку того, что и ЗАКРЫТИЕ вместимого отказавшим сборщиком не
	 * принимается: массив стоит открытым, и без заслона закрытие его удалось бы, отдав
	 * зовущему запись, объявленную ЗАВЕРШЁННОЙ поверх отказа
	 */
	ASSERT_FALSE(writer.arrayEnd()) << "отказавший сборщик закрыл вместимое";
	/**
	 * Выполняем проверку того, что причина отказа за всю череду НЕ подменилась: сборщик
	 * отвечает отказом прежней причиною, а новой не заводит
	 */
	ASSERT_EQ(writer.error(), abc::error_t::CONTAINER_OVERFLOW)
		<< "череда отказов подменила объявленную причину: " << abc::message(writer.error());
}
/**
 * @brief Проверка того, что отказавшая сборка не закрывает и значения, собираемого кусками
 *
 * @details Заслон отказа стоит и у конца значения, собираемого кусками, и стережёт он
 *          случай особый: стек несёт незакрытое звено куска, и без заслона конец его
 *          прошёл бы обычным путём, объявив кусками собранное значение готовым - поверх
 *          отказа, случившегося посреди сборки
 *
 * @note Проверка эта отделена от `TheFailedBuildingAcceptsNothingFurther` намеренно: там
 *       стек звена куска НЕ несёт, и конец куска отвергается СОСЕДНИМ заслоном - о
 *       несведённом вместимом. Заслон отказа она бы не стерегла, и щуп это показал
 *
 */
TEST(CodecAbcWriter, TheFailedBuildingDoesNotCloseASegment) {
	// Сборщик записи контейнера
	abc::writer_t writer(::logger());
	// Выполняем объявление строки, собираемой кусками
	ASSERT_TRUE(writer.textBegin());
	// Выполняем укладку первого куска строки
	ASSERT_TRUE(writer.text("начало"));
	/**
	 * Выполняем укладку числа внутрь строки, собираемой кусками: значение иного вида
	 * внутри куска негодно, и укладка отвечается отказом
	 */
	ASSERT_FALSE(writer.number(static_cast <int64_t> (1))) << "число внутри куска уложено, проверка негодна";
	/**
	 * Выполняем проверку того, что причина названа негодным звеном куска
	 */
	ASSERT_EQ(writer.error(), abc::error_t::INVALID_SEGMENT) << "причина: " << abc::message(writer.error());
	/**
	 * Выполняем проверку того, что конец строки, собираемой кусками, отвергнут: без
	 * заслона он прошёл бы обычным путём, ибо звено куска на стеке стоит и виду отвечает
	 */
	ASSERT_FALSE(writer.textEnd()) << "отказавший сборщик закрыл значение, собираемое кусками";
	/**
	 * Выполняем проверку того, что причина отказа НЕ подменилась
	 */
	ASSERT_EQ(writer.error(), abc::error_t::INVALID_SEGMENT)
		<< "конец куска подменил объявленную причину: " << abc::message(writer.error());
}
/**
 * @brief Проверка того, что воронка отказа сборщика доносит причину журналу
 *
 * @details Воронка `Writer::fail` заводит код отказа и объявляет его журналу. Объявление
 ...
 *
 * @note Воронка эта ПЯТАЯ в своём роду и закрепляется ПОРОЗНЬ, как и четыре прежние
 *       (`Editor::fail`, `Fetcher::fail`, `Merkle::fail`, `Packer::fail`). Все пять
 *       молчали об одном: набор спрашивал причину работою `error()`, а журнала не
 *       спрашивал никто
 *
 * @note Журнал здесь заводится СВОЙ, а не берётся общий: общий у набора погашен намеренно
 *       (проверок отказа тут большинство, и всякая писала бы записью), а погашенный
 *       журнал донесения не разносит вовсе. Гашение - настройка журнала, и подменять ею
 *       предмет проверки нельзя
 *
 */
TEST(CodecAbcWriter, TheWriterFunnelReportsItsCauseToTheJournal) {
	// Объект фреймворка проверки
	fmk_t fmk;
	// Объект журнала проверки, вывод какого не гасится
	log_t log(& fmk);
	// Донесения, снятые с журнала подпискою
	vector <string> journal;
	// Выполняем подписку на журнал ради разбора донесений об отказах
	log.subscribe([&journal](const log_t::flag_t, const string_view text) noexcept -> void {
		// Выполняем накопление очередного донесения журнала
		journal.emplace_back(text);
	});
	// Сборщик записи контейнера со своим журналом
	abc::writer_t writer(& log);
	// Выполняем объявление массива ровно об одном значении
	ASSERT_TRUE(writer.arrayBegin(1));
	// Выполняем укладку единственного объявленного значения
	ASSERT_TRUE(writer.boolean(true));
	// Выполняем проверку того, что журнал донесений об удавшейся укладке НЕ принял
	ASSERT_TRUE(journal.empty()) << "журнал принял донесение об удавшейся укладке: " << journal.front();
	// Выполняем проверку того, что укладка сверх объявленного отвечена отказом
	ASSERT_FALSE(writer.boolean(false)) << "укладка сверх объявленного удалась, проверка негодна";
	// Выполняем получение объявленной причины отказа
	const abc::error_t cause = writer.error();
...
/**
 * @brief Проверка того, что укладка пустого указателя при объявленных октетах отвергается
 *
 * @details Работы укладки двоичного вида берут указатель и длину порознь, и пара
 *          «пустой указатель при объявленной длине» есть беда УСТРОЙСТВА зовущего:
 *          октеты объявлены, а взять их неоткуда. Без заслона укладка перенесла бы в
 *          запись объявленное число октетов по пустому указателю
 *
 * @note Пара «пустой указатель при НУЛЕВОЙ длине» бедою НЕ является и здесь закрепляется
 *       обратной половиною: у пустого вместилища `data()` выдаёт ноль, и отвергать это
 *       значило бы запретить укладку пустых данных вовсе
 *
 * @note Опознаватель стоит особняком: у него длина закреплена видом записи, и пустой
 *       указатель отвергается им БЕЗ оглядки на длину
 *
 */
TEST(CodecAbcWriter, TheAbsentBufferWithDeclaredOctetsIsRefused) {
	/**
	 * Двоичные данные: пустой указатель при объявленных октетах отвергается
	 */
	{
		// Сборщик записи контейнера
		abc::writer_t writer(::logger());
		// Выполняем проверку того, что укладка отвечена отказом
		ASSERT_FALSE(writer.blob(nullptr, 16)) << "уложены двоичные данные по пустому указателю";
		// Выполняем проверку того, что причина названа внутренним отказом
		ASSERT_EQ(writer.error(), abc::error_t::INTERNAL) << "причина: " << abc::message(writer.error());
	}
	/**
	 * Двоичные данные: пустой указатель при НУЛЕВОЙ длине принимается
	 */
	{
		// Сборщик записи контейнера
		abc::writer_t writer(::logger());
		// Выполняем проверку того, что укладка пустых данных принята
		ASSERT_TRUE(writer.blob(nullptr, 0)) << "укладка пустых данных отвергнута: " << abc::message(writer.error());
		// Выполняем проверку завершённости собранной записи
		ASSERT_TRUE(writer.complete()) << "запись из пустых данных не завершена";
	}
	/**
	 * Десятичное число: пустой указатель при объявленных октетах отвергается
	 */
	{
		// Сборщик записи контейнера
		abc::writer_t writer(::logger());
		// Выполняем проверку того, что укладка отвечена отказом
		ASSERT_FALSE(writer.decimal(nullptr, 8, false, 0)) << "уложено десятичное по пустому указателю";
		// Выполняем проверку того, что причина названа внутренним отказом
		ASSERT_EQ(writer.error(), abc::error_t::INTERNAL) << "причина: " << abc::message(writer.error());
	}
	/**
	 * Свой вид записи: пустой указатель при объявленных октетах отвергается
	 */
	{
		// Сборщик записи контейнера
		abc::writer_t writer(::logger());
		// Выполняем проверку того, что укладка отвечена отказом
		ASSERT_FALSE(writer.custom(0x20, nullptr, 16)) << "уложен свой вид по пустому указателю";
		// Выполняем проверку того, что причина названа внутренним отказом
		ASSERT_EQ(writer.error(), abc::error_t::INTERNAL) << "причина: " << abc::message(writer.error());
	}
	/**
	 * Опознаватель: пустой указатель отвергается БЕЗ оглядки на длину, ибо длина его
	 * закреплена видом записи
	 */
	{
		// Сборщик записи контейнера
		abc::writer_t writer(::logger());
		// Выполняем проверку того, что укладка отвечена отказом
		ASSERT_FALSE(writer.uuid(nullptr, abc::UUID_WIDTH)) << "уложен опознаватель по пустому указателю";
		/**
		 * Выполняем проверку того, что причина названа ВНУТРЕННИМ отказом, а не негодною
		 * длиной: длина подана верная, и подмена причины означала бы, что отвечает
		 * соседний заслон
		 */
		ASSERT_EQ(writer.error(), abc::error_t::INTERNAL) << "причина: " << abc::message(writer.error());
	}
...
/**
 * @brief Проверка укладки строки, собираемой кусками, внутрь вместимого
 *
 * @details Значение, собранное кусками, есть ОДНО значение вместившего, а не череда их:
 * кусок есть часть значения, и учитывать его вместившим нельзя
 *
 * @note Проверка эта - близнец такой же у двоичных данных, и заведена она по перекосу,
 * найденному 09.09.2026: договор этот у ДВОИЧНЫХ данных внутри вместимого был закреплён
 * `SegmentedBlobInsideContainer`, а у СТРОКИ не был закреплён ничем - строка, собираемая
 * кусками, укладывалась в наборе лишь корнем записи, где учитывать её некому
 *
 * @warning Проверка эта закрепляет ДОГОВОР, а не скорый выход `if(chunk)` у укладки
 * строки: щуп нужности 09.09.2026 показал, что выход тот молчит и при ней, ибо затенён
 * самим учётом - `account` уменьшает счёт значений лишь у вместимого определённой длины,
 * а звено собираемого кусками заводится неопределённым. Держать проверку следует именно
 * за договор, и приписывать ей закрепление того выхода нельзя
 *
 */
TEST(CodecAbcWriter, SegmentedStringInsideContainer) {
	// Сборщик бинарной записи
	abc::writer_t writer(::logger());
	// Выполняем укладку начала массива из двух значений
	ASSERT_TRUE(writer.arrayBegin(2));
	...
	while(reader.next()){
		...
		if(reader.event() == abc::event_t::STRING)
			...
			content.append(reader.value().data);
		// Если событием является конец собранного значения либо число
		if((reader.event() == abc::event_t::STRING_END) || (reader.event() == abc::event_t::NUMBER))
			// Выполняем учёт снятого значения массива
			values++;
	}
	// Выполняем проверку количества снятых значений массива
...
/**
 * @brief Проверка сохранности отрезков имён полей при вклейке содержимого, уложенного ссылкой
 *
 * @details Содержимое крупных значений кладётся ССЫЛКОЙ и вклеивается в запись лишь при
 * извлечении её целиком. Отрезки же уложенных имён полей отсчитаны от начала буфера, и
 * вклейка сдвигает их молча: сличение имён после того читало бы ЧУЖИЕ октеты, и повтор
 * имени прошёл бы незамеченным
 *
 * @note Проверка эта заведена по трём МОЛЧАВШИМ местам сдвига (щуп нужности 09.09.2026):
 * набор клал содержимое ссылкой лишь в ПЕРЕЧЕНЬ, где имён полей нет вовсе, а извлечения
 * записи ПОСРЕДИ сборки не делал никто - меж тем работа эта неразрушающая, и звать её
 * посреди сборки потребитель вправе
 *
 * @note Сдвиг у всякого имени свой: врезка, вставшая ПОСЛЕ имени, его не двигает. Оттого
 * проверка кладёт имя и до врезки, и после неё
 *
 */
TEST(CodecAbcWriter, TheKeySegmentsSurviveTheSpliceOfTheReferencedContent) {
	// Сборщик бинарной записи
	abc::writer_t writer(::logger());
	// Выполняем получение настроек сборки записи
	abc::writer_t::settings_t settings = writer.settings();
	// Выполняем установку порога укладки содержимого ссылкой
	settings.reference = 16;
	// Выполняем установку отказа на повтор имени поля отображения
	settings.duplicates = true;
	// Выполняем установку настроек сборки записи
	writer.settings(settings);
	// Крупное значение, укладываемое ссылкой
	const string large(1024, 'a');
	// Выполняем укладку начала отображения из трёх пар
	ASSERT_TRUE(writer.mapBegin(static_cast <uint64_t> (3)));
	// Выполняем укладку имени первого поля, стоящего ПЕРЕД врезкой
	ASSERT_TRUE(writer.text("первое"));
	// Выполняем укладку крупного значения, ложащегося ссылкой
	ASSERT_TRUE(writer.text(large)) << "код отказа: " << abc::message(writer.error());
	/**
	 * Выполняем извлечение записи целиком ПОСРЕДИ сборки: вклейка врезки идёт именно
	 * здесь, и отрезки уложенных имён обязаны сдвинуться вместе с нею
	 */
	static_cast <void> (writer.record());
	// Выполняем укладку имени второго поля, стоящего ПОСЛЕ врезки
	ASSERT_TRUE(writer.text("второе"));
	// Выполняем укладку значения второго поля
	ASSERT_TRUE(writer.number(static_cast <uint64_t> (7)));
	/**
	 * Повтор имени, уложенного ДО врезки, обязан быть отвергнут: без сдвига отрезка
	 * сличение читало бы чужие октеты, и повтор прошёл бы незамеченным
	 */
	ASSERT_FALSE(writer.text("первое")) << "повтор имени, уложенного до врезки, принят";
	// Выполняем проверку названной причины отказа
	ASSERT_EQ(writer.error(), abc::error_t::DUPLICATE_KEY) << abc::message(writer.error());
	/**
	 * Повтор имени, уложенного ПОСЛЕ врезки, отвергается той же сборкой заново: сдвиг
	 * его нулевой, и сличение обязано остаться верным и для него
	 */
	{
		// Сборщик бинарной записи
		abc::writer_t second(::logger());
		// Выполняем установку тех же настроек сборки записи
		second.settings(settings);
		// Выполняем укладку начала отображения из трёх пар
		ASSERT_TRUE(second.mapBegin(static_cast <uint64_t> (3)));
		// Выполняем укладку имени первого поля
		ASSERT_TRUE(second.text("первое"));
		// Выполняем укладку крупного значения, ложащегося ссылкой
		ASSERT_TRUE(second.text(large));
		// Выполняем укладку имени второго поля, стоящего после врезки
		ASSERT_TRUE(second.text("второе"));
		// Выполняем укладку значения второго поля
		ASSERT_TRUE(second.number(static_cast <uint64_t> (7)));
		// Выполняем извлечение записи целиком посреди сборки
		static_cast <void> (second.record());
		// Повтор имени, уложенного после врезки, обязан быть отвергнут
		ASSERT_FALSE(second.text("второе")) << "повтор имени, уложенного после врезки, принят";
		// Выполняем проверку названной причины отказа
		ASSERT_EQ(second.error(), abc::error_t::DUPLICATE_KEY) << abc::message(second.error());
	}
	/**
	 * Тот же сдвиг у СТРОГОГО вида записи: имена там сличаются с ПРЕДЫДУЩИМ, а отрезок
	 * его хранится звеном стека, а не перечнем, - и сдвигается он своим путём
	 *
	 * @note Содержимое взято знаком `z` нарочно: пересдвинутый отрезок читал бы октеты
	 * самого содержимого, и совпади они с началом имени, ложь сличения осталась бы
	 * незамеченной
	 */
	{
		// Сборщик бинарной записи
		abc::writer_t strict(::logger());
		// Настройки сборки записи строгого вида
		abc::writer_t::settings_t canonical = settings;
		// Выполняем установку строгого вида записи
		canonical.canonical = true;
		// Выполняем установку настроек сборки записи
		strict.settings(canonical);
		// Крупное значение, укладываемое ссылкой
		const string content(1024, 'z');
		// Выполняем укладку начала отображения из трёх пар
		ASSERT_TRUE(strict.mapBegin(static_cast <uint64_t> (3)));
		// Выполняем укладку имени первого поля
		ASSERT_TRUE(strict.text("aaa"));
		// Выполняем укладку крупного значения, ложащегося ссылкой
		ASSERT_TRUE(strict.text(content)) << "код отказа: " << abc::message(strict.error());
		// Выполняем извлечение записи целиком посреди сборки
		static_cast <void> (strict.record());
		// Имя, идущее по возрастанию, обязано быть принято
		ASSERT_TRUE(strict.text("bbb")) << "код отказа: " << abc::message(strict.error());
		// Выполняем укладку значения второго поля
		ASSERT_TRUE(strict.number(static_cast <uint64_t> (7)));
		/**
		 * Имя, идущее НЕ по возрастанию, обязано быть отвергнуто: без сдвига отрезка
		 * предыдущего имени сличение читало бы октеты самого содержимого
		 */
		ASSERT_FALSE(strict.text("aaa")) << "имя не по возрастанию принято";
		// Выполняем проверку названной причины отказа
		ASSERT_EQ(strict.error(), abc::error_t::UNORDERED_KEY) << abc::message(strict.error());
	}
}
