...
/**
 * @brief Проверка соответствия заголовка кадра объявленной раскладке
 *
 * @details Раскладка заголовка кадра по октетам объявлена у `CHUNK_HEADER`, а вместе с
 *          нею и выработка контрольной суммы: заголовок с обнулённой суммой и снятым
 *          разрядом мусора, зерно `0x41424348554E4B01`, затем содержимое кадра тем же
 *          зерном. Проверка эта читает поля ПО СМЕЩЕНИЯМ и складывает сумму ПО ЗАПИСИ
 *
 * @note Опора здесь ВНЕ поверяемого: поля читаются не снятием кадра, а прямо из октетов,
 *       и сумма складывается зовом свёртки напрямую, а не через `abc::digest`. Круговой
 *       ход укладки со снятием расхождения такого не видит вовсе - сдвиг поля переживает
 *       его, покуда обе стороны сдвинуты разом
 *
 * @note Величины полей взяты РАЗЛИЧИМЫМИ намеренно: на совпадающих перестановка двух
 *       полей местами прошла бы зелёной
 *
 */
TEST_F(ChunkFixture, HeaderLayout) {
	// Укладчик кадров
	abc::packer_t packer;
	// Укладываемое содержимое кадра
	const string payload = "содержимое кадра";
	// Буфер уложенного кадра
	vector <uint8_t> record;
	// Выполняем укладку кадра с различимыми номером и поколением
	ASSERT_TRUE(packer.pack(payload.data(), payload.size(), abc::payload_t::TEXT, 0x1122334455667788ull, 0x09ABCDEFu, record))
		<< "код отказа: " << abc::message(packer.error());
	// Выполняем проверку длины уложенного кадра
	ASSERT_EQ(record.size(), abc::CHUNK_HEADER + payload.size());
	/**
	 * @brief Функция снятия числа с объявленного смещения заголовка кадра
	 *
	 * @param offset смещение снимаемого числа
	 * @param width  ширина снимаемого числа в октетах
	 * @return       снятое число
	 *
	 */
	auto taken = [&record](const size_t offset, const size_t width) noexcept -> uint64_t {
		// Снимаемое с заголовка кадра число
		uint64_t result = 0;
		/**
		 * Выполняем перебор октетов снимаемого числа от младшего к старшему
		 */
		for(size_t i = 0; i < width; i++)
			// Выполняем внесение очередного октета снимаемого числа
			result |= (static_cast <uint64_t> (record.at(offset + i)) << (i * 8));
		// Выводим снятое число
		return result;
	};
	// Метод сжатия содержимого кадра обязан стоять первым октетом
	ASSERT_EQ(record.at(0), static_cast <uint8_t> (compressor::method_t::NONE));
	// Разряды кадра обязаны быть пусты: содержимое не зашифровано и мусором не помечено
	ASSERT_EQ(record.at(abc::CHUNK_FLAGS), 0x00);
	// Место запаса обязано быть заполнено нулями
	ASSERT_EQ(taken(2, 2), 0ull);
	// Длина уложенного содержимого обязана стоять на смещении четыре
	ASSERT_EQ(taken(4, 4), static_cast <uint64_t> (payload.size()));
	// Длина исходного содержимого обязана стоять на смещении восемь
	ASSERT_EQ(taken(8, 4), static_cast <uint64_t> (payload.size()));
	// Порядковый номер кадра обязан стоять на смещении двенадцать
	ASSERT_EQ(taken(12, 8), 0x1122334455667788ull);
	// Поколение записи кадра обязано стоять на смещении двадцать
	ASSERT_EQ(taken(20, 4), 0x09ABCDEFull);
	// Заголовок кадра, приведённый к виду выработки суммы
	vector <uint8_t> head(record.begin(), record.begin() + abc::CHUNK_HEADER);
	// Выполняем снятие разряда мусора с признаков кадра
	head.at(abc::CHUNK_FLAGS) = static_cast <uint8_t> (head.at(abc::CHUNK_FLAGS) & ~abc::CHUNK_WASTE);
	/**
	 * Выполняем обнуление места самой контрольной суммы: в себя она не входит
	 */
	for(size_t i = 0; i < 8; i++)
		// Выполняем обнуление очередного октета контрольной суммы
		head.at(abc::CHUNK_DIGEST + i) = 0x00;
	// Выполняем выработку суммы по заголовку кадра с объявленным зерном
	uint64_t checksum = hashing::generate(head.data(), head.size(), 0x41424348554E4B01ull);
	// Выполняем выработку суммы по содержимому кадра с зерном заголовка
	checksum = hashing::generate(record.data() + abc::CHUNK_HEADER, record.size() - abc::CHUNK_HEADER, checksum);
	// Контрольная сумма обязана стоять на смещении двадцать четыре и отвечать сложенной
	ASSERT_EQ(taken(abc::CHUNK_DIGEST, 8), checksum);
}
/**
 * @brief Проверка отказа снятия кадра с занятыми октетами запаса
 *
 * @details Октеты `2…3` заголовка кадра оставлены впрок, укладка кладёт их нулями, и
 *          снятие нулевыми их требует. Требование равняется на сличение разрядов кадра
 *          и на то же требование у строки оглавления: поле приходит с провода и обязано
 *          быть опознано, иначе занятые кем-то октеты проходят молча
 *
 * @note Сумма кадра здесь пересобирается намеренно: без того отказ пришёл бы о
 *       несошедшейся сумме, и проверка закрепляла бы сумму, а не опознание запаса
 *
 */
TEST_F(ChunkFixture, ReservedOctetsRefused) {
	// Укладчик кадров
	abc::packer_t packer;
	// Укладываемое содержимое кадра
	const string payload = "содержимое кадра";
	/**
	 * Выполняем перебор обоих октетов запаса заголовка кадра
	 */
	for(size_t i = 2; i < 4; i++){
		...
		ASSERT_TRUE(packer.pack(payload.data(), payload.size(), abc::payload_t::TEXT, 7, 3, record))
			<< "код отказа: " << abc::message(packer.error());
		// Октет запаса обязан быть уложен нулём
		EXPECT_EQ(record.at(i), 0x00) << "смещение октета: " << i;
		// Выполняем занятие очередного октета запаса
		record.at(i) = 0x01;
		// Выполняем пересборку контрольной суммы кадра по правленому заголовку
		...
		// Снятые сведения о кадре
		abc::chunk_t chunk;
		// Занятый октет запаса обязан отвечаться отказом опознания кадра
		EXPECT_FALSE(packer.unpack(record.data(), record.size(), offset, content, chunk))
			<< "смещение октета: " << i;
		// Отказ обязан быть именно об опознании кадра
 ...
 *
 * @note Воронка эта ЧЕТВЁРТАЯ в своём роду и закрепляется ПОРОЗНЬ. Близнецы её -
 *       `Editor::fail`, `Fetcher::fail` и `Merkle::fail` - молчали тем же молчанием и
 *       закреплены каждая своею проверкою: работы разные, и закрепление одной другую не
 *       стережёт. Молчание всех четырёх об одном: набор спрашивал причину работою
 *       `error()`, а журнала не спрашивал никто
 *
 */
TEST_F(ChunkFixture, ThePackerFunnelReportsItsCauseToTheJournal) {
	/**
	...
	abc::chunk_t chunk;
	/**
	 * Выполняем снятие кадра из НЕДОСТАЮЩИХ октетов: работа обязана быть отвечена отказом
	 */
	const vector <uint8_t> shortage(4, 0x00);
	// Выполняем проверку того, что снятие короткого кадра отвечено отказом
	ASSERT_FALSE(packer.unpack(shortage.data(), shortage.size(), offset, payload, chunk))
		<< "короткий кадр снят, проверка негодна";
	// Выполняем получение объявленной причины отказа
	const abc::error_t cause = packer.error();
...
/**
 * @brief Проверка того, что сумма недостающих октетов кадра пуста
 *
 * @details Выработка суммы кадра отвечает не отказом, а ЧИСЛОМ, и потому заслон здесь -
 *          единственное, что стоит между коротким буфером и переносом тридцати двух октетов
 *          заголовка. Обесточенный, он обратил бы всякий короткий буфер в чтение за его
 *          пределом, а итогом отдал бы сумму, выработанную по чужой памяти: сумму
 *          НЕПОВТОРИМУЮ, отчего сличение её с уложенной отказывало бы то и дело
 *
 * @note Утверждается НУЛЬ намеренно: у работы, отвечающей числом, нуль и есть объявленный
 *       признак невыработки, иного она подать не может
 *
 */
TEST_F(ChunkFixture, TheDigestOfAShortChunkIsEmpty) {
	// Выполняем проверку того, что сумма пустого указателя пуста
	ASSERT_EQ(abc::digest(nullptr, abc::CHUNK_HEADER), 0ull) << "сумма пустого указателя выработана";
	/**
	 * Выполняем перебор всех длин буфера, заголовку кадра недостающих
	 */
	for(size_t length = 1; length < abc::CHUNK_HEADER; length++){
		/**
		 * Поданные октеты кадра: буфер отводится РОВНО по длине, оттого чтение за его
		 * пределом остаётся чтением за пределом, а не чтением запаса
		 */
		const vector <uint8_t> buffer(length, 0x5A);
		// Выполняем проверку того, что сумма недостающих октетов пуста
		ASSERT_EQ(abc::digest(buffer.data(), buffer.size()), 0ull)
			<< "сумма выработана по короткому кадру, длина: " << length;
	}
	/**
	 * Выполняем проверку того, что кадр ПОЛНОЙ длины сумму даёт: без того проверка
	 * прошла бы и у работы, отвечающей нулём всегда
	 */
	{
		// Поданные октеты кадра полной длины
		const vector <uint8_t> buffer(abc::CHUNK_HEADER, 0x5A);
		// Выполняем проверку того, что сумма кадра полной длины выработана
		ASSERT_NE(abc::digest(buffer.data(), buffer.size()), 0ull) << "сумма целого кадра не выработана";
	}
}
