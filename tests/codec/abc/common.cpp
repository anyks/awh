...
TEST(CodecAbcCommon, WireConstants) {
	 ...
	 * закреплено числами было одиннадцать. Приём этот взят у кодека разметки, и он
...
TEST(CodecAbcCommon, EveryCauseCarriesItsOwnExplanation) {
	 ...
	 *          предмета не имело. Поправлено 09.09.2026 по доводу от кодеков syslog и CEF,
	 *          у какого тот же сторож с тем же `0xFF` пропустил новый код и вскрылся
	 *          лишь падением карты покрытия со 100 до 97.22. Правило здесь такое:
