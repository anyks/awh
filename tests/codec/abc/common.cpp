...
/**
 * Подключаем общий глушитель вывода журнала проверок кодеков
 *
 * @details Подключается он ОДИН раз на кодек и гасит вывод у всякой проверки набора:
 *          отказы разбора проверки наводят намеренно, и записи о них засоряли бы выдачу -
 *          за один прогон кодека ABC их выходило две с половиной тысячи
 *
 * @note Место выбрано здесь, а не у отдельных наборов, нарочно: общий файл входит и в
 *       отдельные стенды, где `tests/main.cpp` со своим сторожем состояния журнала нет
 *       вовсе. Прежде у каждого набора ABC стоял свой сторож гашения - устройство то же,
 *       а средств двое, и читающему оставалось гадать, какое из них здесь главное
 *
 * @warning Проверке, какой журнал НУЖЕН, надлежит заявить о том САМОЙ - зовами
 *          `awh::log::level(awh::log::level_t::ALL)` и `awh::log::mode({...})` у себя в
 *          начале. Глушитель работает в `OnTestStart`, то есть ДО тела проверки, и
 *          заявленное телом остаётся за нею
 */
#include "../silent.hpp"
...
TEST(CodecAbcCommon, WireConstants) {
	...
	/**
	 * Величины ниже дописаны 04.09.2026 сличением состава ЗАКРЕПЛЁННОГО с составом
	 * ПОЛНЫМ: перебор всех постоянных заголовков кодека показал двадцать одну, а
	 * закреплено числами было одиннадцать. Приём этот взят у владельца разметки, и он
	 * ловит не то же, что подмена: подмена спрашивает «стережёт ли хоть кто-нибудь эту
	 * величину», а сличение состава - «не забыта ли величина вовсе»
	 */
	// Старший знак вида записи, ложащийся в заголовок опознания
	EXPECT_EQ(abc::VERSION_MAJOR, 1);
	// Младший знак вида записи, ложащийся в заголовок опознания
	EXPECT_EQ(abc::VERSION_MINOR, 0);
	// Ширина поля размаха у вместимого с объявленным размером
	EXPECT_EQ(abc::SPAN_LENGTH, 8u);
	// Ширина записи отметки времени в октетах
	EXPECT_EQ(abc::TIME_WIDTH, 8u);
	// Ширина записи опознавателя в октетах
	EXPECT_EQ(abc::UUID_WIDTH, 16u);
	// Признак отсутствия смещения, ложащийся в строку оглавления
	EXPECT_EQ(abc::NO_OFFSET, static_cast <uint64_t> (~0ull));
	// Признак отсутствия места узла
	EXPECT_EQ(abc::NO_INDEX, static_cast <uint32_t> (~0u));
	/**
	 * @note Старший знак вида записи есть НАИБОЛЕЕ опасная из всех: он и ложится в
	 *       заголовок, и сличается при снятии, - смена его обращает всякий прежний
	 *       контейнер в нечитаемый, а по набору проходит зелёной, ибо укладка со
	 *       снятием двигаются согласно. Закреплён не был вовсе
	 *
	 * @note Пределы MAX_DEPTH, MAX_NODES и INDEX_THRESHOLD числами здесь НЕ
	 *       закрепляются намеренно: договором с носителем они не являются - смена их
	 *       прежних контейнеров не портит, а первые два вдобавок перекрываются
	 *       настройками. Согласие двух дорог розыска, каким INDEX_THRESHOLD и опасен,
	 *       закреплено отдельно проверкою
	 *       CodecAbcDocument.LookupAgreesOnBothSidesOfTheIndexThreshold
	 *
	 */
}
...
TEST(CodecAbcCommon, EveryCauseCarriesItsOwnExplanation) {
	...
	/**
	 * Выполняем проверку того, что код ЗА ГРАНИЦЕЙ перечня объяснения не получает
	 *
	 ...
	 *
	 * @warning Берётся ИМЕННО «последний плюс один», а не круглое число вроде `0xFE`.
	 *          Прежняя редакция брала `0xFE`, и сторож такой при новом коде не падает
	 *          ВОВСЕ: код `0x2E`, заведённый видом записи и объяснения не получивший,
	 *          проходил бы молча, ибо далёкий `0xFE` остаётся безымянным при любом
	 *          пополнении перечня. Обещание «за перечнем кодов нет» было записано, а
	 *          предмета не имело. Поправлено 09.09.2026 по доводу Генри (syslog, CEF),
	 *          у какого тот же сторож с тем же `0xFF` пропустил новый код и вскрылся
	 *          лишь падением карты покрытия со 100 до 97.22. Правило родом от Василия:
	 *          перебрал до границы - следующий за нею обязан быть безымянным
	 */
	ASSERT_EQ(string(abc::message(static_cast <abc::error_t> (
	 static_cast <uint16_t> (abc::error_t::NON_MINIMAL_TAG) + 1))), "unknown error");
}
/**
 * @brief Проверка того, что пустое звено пути номером не является
 *
 * @details Разбор звена номером отвечает не отказом, а признаком годности, и пустое звено
 *          обязано быть отвергнуто. Без заслона перебор знаков пустого звена не находит
 *          ни одного негодного, и работа объявляет звено ГОДНЫМ с номером НУЛЬ: путь
 *          `//` указывал бы на нулевое значение перечня вместо того, чтобы быть
 *          отвергнутым
 *
 * @note Утверждается ПРИЗНАК, а не разобранный номер: нуль в номере лежит и при отказе,
 *       ибо работа сбрасывает его первым действием, и утверждение о нём стерегло бы то,
 *       что верно в обоих случаях
 *
 */
TEST(CodecAbcCommon, TheEmptySegmentIsNotANumber) {
	// Разобранный номер значения
	size_t number = 0x1000;
	/**
	 * Выполняем проверку того, что пустое звено пути номером не признано
	 */
	ASSERT_FALSE(abc::indexed(string_view{}, number)) << "пустое звено признано номером";
	/**
	 * Выполняем проверку того, что номер сброшен: работа сбрасывает его первым
	 * действием, и отказ обязан оставить его нулём
	 */
	ASSERT_EQ(number, 0ull) << "отказ оставил зовущему прежний номер";
	/**
	 * Выполняем проверку того, что звено НЕПУСТОЕ номером признаётся: без того проверка
	 * прошла бы и у работы, отвергающей всякое звено
	 */
	ASSERT_TRUE(abc::indexed(string_view{"12"}, number)) << "звено «12» номером не признано";
	// Выполняем проверку разобранного номера значения
	ASSERT_EQ(number, 12ull);
}
/**
 * @brief Проверка того, что звено с оборванной отменяющей записью отвергается
 *
 * @details Знак отмены `~` по RFC 6901 отменяет ЗНАК за собою, и звено, кончающееся им,
 *          отменяет пустоту. Такое звено обязано объявлять путь недействительным
 *
 * @note Утверждается признак НЕДЕЙСТВИТЕЛЬНОСТИ пути, а не одна лишь ложь выдачи: ложью
 *       работа отвечает и на исчерпанный путь, и различить их можно лишь этим признаком
 *
 * @warning Проверка эта заслона `(i + 1) >= buffer.size()` НЕ стережёт, и сказано это
 *          прямо, дабы следующий аудит не счёл её сторожем. Щуп нужности 08.09.2026
 *          обесточил тот заслон, и проверка осталась ЗЕЛЕНА - разбор показал почему:
 *          `buffer[i + 1]` при `i + 1 == size()` есть обращение ЗАКОННОЕ, стандарт
 *          обещает там нулевой знак, а разбор отменяющей записи всё неведомое отвергает
 *          ветвью `default` тем же порядком. Заслон оттого для правильности ИЗБЫТОЧЕН и
 *          держится ради внятности разбора. Первая редакция этой записки гласила, будто
 *          без него идёт чтение за концом вместилища, - довод благовидный и НЕВЕРНЫЙ,
 *          снят по приговору щупа
 *
 */
TEST(CodecAbcCommon, TheSegmentWithATruncatedEscapeIsRefused) {
	/**
	 * Выполняем перебор путей, кончающихся знаком отмены
	 */
	for(const string_view path : {string_view{"~"}, string_view{"поле~"}, string_view{"поле/звено~"}}){
		// Смещение разбора пути
		size_t offset = 0;
		// Выдаваемое звено пути
		string_view result;
		// Вместилище отменённого звена пути
		string buffer;
		// Признак недействительности пути
		bool invalid = false;
		/**
		 * Выполняем разбор пути до звена с оборванной отменяющей записью
		 */
		while(abc::segment(path, offset, result, buffer, invalid))
			// Продолжаем разбор пути
			;
		/**
		 * Выполняем проверку того, что путь объявлен недействительным: без заслона выдача
		 * прекратилась бы исчерпанием пути, а недействительным он объявлен не был бы
		 */
		ASSERT_TRUE(invalid) << "путь с оборванной отменяющей записью недействительным не объявлен: " << path;
	}
	/**
	 * Выполняем проверку того, что путь с ЦЕЛОЙ отменяющей записью действителен: без
	 * того проверка прошла бы и у работы, объявляющей недействительным всякий путь со
	 * знаком отмены
	 */
	{
		// Смещение разбора пути
		...
		// Признак недействительности пути
		bool invalid = false;
		// Выполняем разбор звена с отменяющей записью косой черты
		ASSERT_TRUE(abc::segment(string_view{"поле~1звено"}, offset, result, buffer, invalid))
			<< "звено с целой отменяющей записью не выдано";
		// Выполняем проверку того, что путь недействительным не объявлен
		ASSERT_FALSE(invalid) << "путь с целой отменяющей записью объявлен недействительным";
		// Выполняем проверку того, что отменяющая запись снята косой чертой
		ASSERT_EQ(result, string_view{"поле/звено"});
	}
}
