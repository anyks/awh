...
TEST_F(SignatureFixture, SignRecordRoundtrip) {
	 ...
	 *
	 * @warning Запись подаётся ПОДЛИННО короткая, а не длинная с укороченной объявленной
	 *          длиной. Разница здесь не в опрятности: заслон этот бережёт ПАМЯТЬ - без
	 *          него разбор дочитал бы заголовок за концом поданного. Указатель на полную
	 *          запись прятал бы это чтение, ибо октеты за объявленной длиной там свои же
	 *          и живые, и отказ приходил бы от СОСЕДНЕГО заслона, называющего ту же
	 *          причину. Вскрыто щупом нужности 07.09.2026: обесточение заслона не красило
	 *          проверку, хотя причина ею и утверждалась
	 */
	const vector <uint8_t> stub(abc::SIGNATURE_HEADER - 1, 0x00);
	ASSERT_FALSE(abc::unpack(stub.data(), stub.size(), taken, error));
...
/**
 * @brief Проверка того, что сведение дерева с приданным кадром требует своей оснастки
 *
 * @details Дерево свёрток сводится двумя работами: `root(result)` - по одним лишь листьям,
 *          и `root(result, buffer, size)` - с кадром, придаваемым на лету. Заслон второй
 *          стерёгся доводом, что «дорога стережётся `MerkleRefusesWithoutItsGear`», а тот
 *          довод был шире правды: названная проверка стережёт `add` и ОДНОДОВОДНЫЙ `root`,
 *          трёхдоводного же без оснастки не звал никто. Вскрыто щупом нужности 07.09.2026
 *
 * @warning Проверка эта стережёт ДОГОВОР, а не заслон: обесточение заслона в `root` её не
 *          красит, ибо тот избыточен - внесение кадра ниже отвергает те же три условия
 *          само. Ждать от неё покраснения при правке заслона нельзя, и знать это надо
 *          прежде, чем счесть её беззубой
 */
TEST_F(SignatureFixture, MerkleRootWithAnAppendedChunkRefusesWithoutItsGear) {
	...
	ASSERT_FALSE(bare.root(root, "кадр", 4)) << "дерево свелось без модуля шифрования";
	...
	merkle.crypto(this->_crypto.get());
	// Выполняем внесение кадра в дерево свёрток
	ASSERT_TRUE(merkle.add("первый", 6));
	// Выполняем проверку отказа сведения дерева с кадром, поданным пустым указателем
	ASSERT_FALSE(merkle.root(root, nullptr, 4)) << "дерево свелось с пустым указателем кадра";
	// Выполняем проверку отказа сведения дерева с кадром нулевой длины
	ASSERT_FALSE(merkle.root(root, "кадр", 0)) << "дерево свелось с кадром нулевой длины";
	// Выполняем проверку того, что корень при отказе очищен
	ASSERT_TRUE(root.empty()) << "корень при отказе не очищен";
	/**
	 * Выполняем проверку того, что сведение с ГОДНЫМ кадром работает: без этой половины
	 * проверка прошла бы и у дерева, не сводящегося ни при каких доводах
	 */
	ASSERT_TRUE(merkle.root(root, "кадр", 4)) << "дерево не свелось с годным кадром";
	// Выполняем проверку того, что корень сведённого дерева непуст
	ASSERT_FALSE(root.empty()) << "корень сведённого дерева пуст";
	/**
	 * Выполняем проверку того, что приданный кадр на корень ВЛИЯЕТ: иначе доводы его
	 * принимались бы, ничего не решая
	 */
	vector <uint8_t> other;
	ASSERT_TRUE(merkle.root(other, "иной", 4)) << "дерево не свелось со вторым кадром";
	ASSERT_NE(root, other) << "приданный кадр на корень не влияет";
}
/**
 * @brief Проверка того, что воронка отказа дерева свёрток доносит беду журналу
 *
 * @details Воронка `Merkle::fail` кода отказа не заводит вовсе - у дерева свёрток его нет,
 *          - и доносит она ТЕКСТОМ. Оттого журнал здесь не второй путь наружу, а
 *          ЕДИНСТВЕННЫЙ: зовущий видит одну лишь ложь возврата и причины спросить не может
 *          ничем. Обесточенная воронка обратила бы всякий отказ дерева в немой
 *
 * @note Воронка эта ТРЕТЬЯ в своём роду и закрепляется ПОРОЗНЬ. Близнецы её -
 *       `Editor::fail` и `Fetcher::fail` - молчали тем же молчанием и закреплены каждая
 *       своею проверкою: работы разные, и закрепление одной другую не стережёт
 *
 */
TEST_F(SignatureFixture, TheMerkleFunnelReportsItsTroubleToTheJournal) {
	...
	vector <uint8_t> root;
	/**
	 * Выполняем сведение дерева БЕЗ модуля шифрования и без единого кадра: работа
	 * обязана быть отвечена отказом
	 */
	ASSERT_FALSE(merkle.root(root)) << "сведение пустого дерева удалось, проверка негодна";
	// Выполняем проверку того, что журнал донесение о беде принял
	ASSERT_FALSE(journal.empty()) << "беда дерева журналу не объявлена вовсе";
	// Признак того, что донесение назвало беду дерева свёрток
	bool named = false;
	/**
	 * Выполняем перебор всех накопленных донесений журнала
	 */
	for(const string & line : journal)
		// Отмечаем, что очередное донесение назвало беду дерева свёрток
		named = (named || (line.find("Merkle tree") != string::npos));
	/**
	 * Выполняем проверку того, что донесение назвало ИМЕННО дерево свёрток: без того
 ...
 *
 * @details Выработка отпечатка отдаёт пустоту, когда ключа с поданным именем у модуля
 *          шифрования нет. Без заслона усечение пустого отпечатка дало бы пустой же
 *          результат, а работа объявила бы себя УДАВШЕЙСЯ: зовущий уложил бы в заголовок
 *          пустой отпечаток и узнал бы о беде лишь при сличении с чужим - то есть никогда,
 *          ибо пустой отпечаток не сойдётся ни с чем и будет сочтён чужою подписью
 *
 * @note Утверждается ДЕЙСТВИЕ - признак отказа, - а не пустота итога: пуст он в обоих
 *       случаях, и утверждение о нём стерегло бы то, что и без заслона верно
 *
 */
TEST_F(SignatureFixture, TheFingerprintOfAnAbsentKeyIsRefused) {
	// Выполняем заведение ключа владельца контейнера
	ASSERT_TRUE(this->_crypto->generateKey("владелец", crypto_t::signature_t::ED25519));
	// Буфер вырабатываемого отпечатка открытого ключа
	vector <uint8_t> print;
	/**
	 * Выполняем проверку того, что отпечаток заведённого ключа вырабатывается: без того
	 * проверка стерегла бы отказ, принадлежащий устройству, а не имени ключа
	 */
	ASSERT_TRUE(abc::fingerprint(* this->_crypto, "владелец", print)) << "отпечаток заведённого ключа не выработан";
	// Выполняем проверку того, что отпечаток заведённого ключа не пуст
	ASSERT_FALSE(print.empty()) << "отпечаток заведённого ключа пуст";
	/**
	 * Выполняем проверку того, что отпечаток ключа с ИНЫМ именем отвечен отказом
	 */
	ASSERT_FALSE(abc::fingerprint(* this->_crypto, "постороннее имя", print))
		<< "отпечаток несуществующего ключа объявлен выработанным";
	/**
	 * Выполняем проверку того, что буфер отпечатка при отказе оставлен пустым
	 */
	ASSERT_TRUE(print.empty()) << "отказ оставил в буфере октеты отпечатка";
}
/**
 * @brief Проверка того, что отказ по недостатку заголовка подписи полей НЕ трогает
 *
 * @details Снятие подписи читает вид подписи и вид хэш-суммы ПЕРЕД сличением поданной
 *          длины с объявленной. Заслон этот - единственное, что стоит между коротким
 *          буфером и чтением шести октетов его заголовка
 *
 * @note Утверждается неприкосновенность ПОЛЕЙ, а не причина отказа: причина у соседнего
 *       заслона та же самая (обрыв подписи), и оба на коротком буфере отвечают отказом.
 *       Разнятся они лишь тем, что без этого заслона вид подписи успевает лечь в
 *       разбираемую запись из октетов, до каких дела нет
 *
 */
TEST_F(SignatureFixture, TheShortSignatureRecordLeavesTheFieldsUntouched) {
	/**
	 * Выполняем перебор всех длин буфера, заголовку записи подписи недостающих
	 */
	/**
	 * Длина НУЛЕВАЯ сюда не берётся: пустой вектор отдаёт `data()` пустотою, и отказ
	 * приходит от заслона соседнего - о неверно поданных октетах, - а не от недостатка
	 * заголовка. Замерено: при нуле выходит «internal parsing error»
	 */
	for(size_t length = 1; length < abc::SIGNATURE_HEADER; length++){
		/**
		 * Поданные октеты записи подписи: буфер отводится РОВНО по длине, оттого чтение
		 * за его пределом остаётся чтением за пределом, а не чтением запаса
		 */
		vector <uint8_t> buffer(length, 0x7F);
		// Разбираемая запись подписи владельца контейнера
		abc::sign_t sign;
		/**
		 * Выполняем установку заведомого вида подписи: он обязан пережить отказ
		 * неприкосновенным
		 */
		sign.kind = crypto_t::signature_t::ED25519;
		// Выполняем установку заведомого вида хэш-суммы
		sign.hash = crypto_t::hash_t::SHA512;
		// Код отказа снятия записи подписи
		abc::error_t error = abc::error_t::NONE;
		// Выполняем проверку того, что снятие короткой записи отвечено отказом
		ASSERT_FALSE(abc::unpack(buffer.data(), buffer.size(), sign, error))
			<< "короткая запись подписи снята, длина: " << length;
		// Выполняем проверку того, что причина названа обрывом подписи
		ASSERT_EQ(error, abc::error_t::TRUNCATED_SIGNATURE)
			<< "длина: " << length << ", причина: " << abc::message(error);
		/**
		 * Выполняем проверку того, что вид подписи отказом НЕ тронут: без заслона он лёг
		 * бы из первого октета поданных - либо из октета за пределом буфера
		 */
		ASSERT_EQ(sign.kind, crypto_t::signature_t::ED25519)
			<< "вид подписи подменён отказом, длина: " << length;
		// Выполняем проверку того, что вид хэш-суммы отказом НЕ тронут
		ASSERT_EQ(sign.hash, crypto_t::hash_t::SHA512)
			<< "вид хэш-суммы подменён отказом, длина: " << length;
	}
}
/**
 * @brief Проверка того, что отвергнутое снятие записи подписи не трогает полей вовсе
 *
 * @note Проверка соседняя, `TheShortSignatureRecordLeavesTheFieldsUntouched`, стережёт ту же
 *       неприкосновенность, но лишь на ОДНОМ пути отказа - у короткого буфера, где до
 *       установки полей снятие не доходит вовсе. Прочие четыре пути отказа ею не покрыты, и
 *       на них поля ложились с провода
 *
 * @warning Поданные записи здесь отвергаются РАЗНЫМИ заслонами: неведомый вид подписи, пара
 *          вида и хэш-суммы, какой кодек не уложил бы, занятые хвостовые октеты заголовка и
 *          негодная длина корня. Всякий из них обязан оставить поля нетронутыми
 *
 */
TEST_F(SignatureFixture, TheRefusedSignatureRecordIsLeftIntact) {
	/**
	 * @brief Структура поверяемого случая отказа снятия записи подписи
	 *
	 */
	struct probe_t {
		// Вид подписи владельца контейнера
		...
		// Вид хэш-суммы, какой подпись выработана
		uint8_t hash;
		// Хвостовые октеты заголовка записи подписи
		uint8_t reserved;
		// Объявленная длина корня дерева свёрток
		uint8_t width;
		// Имя поверяемого случая
		const char * name;
	};
	// Перечень поверяемых случаев отказа снятия записи подписи
	const probe_t probes[] = {
		{0xFE, 0x00, 0x00, static_cast <uint8_t> (abc::DIGEST_LENGTH), "неведомый вид подписи"},
		{static_cast <uint8_t> (crypto_t::signature_t::ED25519), static_cast <uint8_t> (crypto_t::hash_t::SHA256),
		 0x00, static_cast <uint8_t> (abc::DIGEST_LENGTH), "пара вида подписи и хэш-суммы, кодеком не кладомая"},
		{static_cast <uint8_t> (crypto_t::signature_t::ED25519), 0x00, 0x7F,
		 static_cast <uint8_t> (abc::DIGEST_LENGTH), "занятые хвостовые октеты заголовка"},
		{static_cast <uint8_t> (crypto_t::signature_t::ED25519), 0x00, 0x00, 0x08, "негодная длина корня"}
	};
	/**
	 * Выполняем перебор всех поверяемых случаев отказа снятия записи подписи
	 */
	for(const probe_t & probe : probes){
		// Поданные октеты записи подписи
		vector <uint8_t> buffer(abc::SIGNATURE_HEADER + abc::DIGEST_LENGTH + 64, 0x11);
		// Выполняем установку вида подписи владельца контейнера
		buffer[0] = probe.kind;
		// Выполняем установку вида хэш-суммы, какой подпись выработана
		buffer[1] = probe.hash;
		// Выполняем установку длины подписи владельца контейнера
		buffer[2] = 64;
		// Выполняем установку старшего октета длины подписи владельца контейнера
		buffer[3] = 0;
		// Выполняем установку длины корня дерева свёрток
		buffer[4] = probe.width;
		// Выполняем установку старшего октета длины корня дерева свёрток
		buffer[5] = 0;
		// Выполняем установку хвостовых октетов заголовка записи подписи
		buffer[6] = probe.reserved;
		// Выполняем установку старшего хвостового октета заголовка записи подписи
		buffer[7] = 0;
		// Разбираемая запись подписи владельца контейнера
		abc::sign_t sign;
		/**
		 * Выполняем установку заведомых полей: они обязаны пережить отказ неприкосновенными
		 */
		sign.kind = crypto_t::signature_t::RSA;
		// Выполняем установку заведомого вида хэш-суммы
		sign.hash = crypto_t::hash_t::SHA512;
		// Код отказа снятия записи подписи
		abc::error_t error = abc::error_t::NONE;
		// Выполняем проверку того, что снятие записи отвечено отказом
		ASSERT_FALSE(abc::unpack(buffer.data(), buffer.size(), sign, error))
			<< "негодная запись подписи снята: " << probe.name;
		// Выполняем проверку того, что вид подписи отказом не тронут
		ASSERT_EQ(sign.kind, crypto_t::signature_t::RSA)
			<< "отвергнутое снятие положило вид подписи: " << probe.name;
		// Выполняем проверку того, что вид хэш-суммы отказом не тронут
		ASSERT_EQ(sign.hash, crypto_t::hash_t::SHA512)
			<< "отвергнутое снятие положило вид хэш-суммы: " << probe.name;
	}
}
