...
/**
 * @brief Проверка счёта мусора против длин снесённых записей
 *
 * @details Счёт мусора ведётся ПРИБАВЛЕНИЕМ при всяком сносе и всякой правке, и поверить
 *          его сличением с носителем нельзя: часть мусора - октеты записи, на какую
 *          строка оглавления уже не смотрит, и восстановить её длину по контейнеру нечем.
 *          Счётчик тем самым есть ЕДИНСТВЕННЫЙ свидетель части мусора, и поверять его
 *          надобно счётом СТОРОННИМ - тем, что вела сама проверка
 *
 * @note Проверки, стоящие выше, сличают счёт мусора С САМИМ СОБОЮ: больше нуля, не
 *       изменился после отката, равен прежнему. Сдвиг счёта на постоянную величину они
 *       переживают все до одной - сличать надобно с ВЕЛИЧИНОЙ, а не с прежним собою
 *
 * @note Счёт снимается ДО фиксации намеренно: фиксация прибавляет к мусору ещё и кадр
 *       прежнего оглавления целиком, и величина та зависит от сжатия. Сличать сумму
 *       длин записей надлежит там, где иных слагаемых нет
 *
 */
TEST_F(EditorFixture, GarbageCountsRemovedRecordLengths) {
	// Носитель, несущий правимый контейнер
	Medium medium;
	// Выполняем сборку контейнера с четырьмя записями
	this->build(medium, {"первая", "вторая", "третья", "четвёртая"});
	...
	// Выполняем открытие контейнера правщиком
	ASSERT_TRUE(this->open(editor, medium)) << "код отказа: " << abc::message(editor.error());
	// Свежий контейнер мусора нести не обязан
	ASSERT_EQ(editor.garbage(), 0ull);
	// Выполняем снятие длины второй записи по оглавлению
	const uint64_t second = static_cast <uint64_t> (editor.index().entries().at(1).length);
	...
	// Длины записей обязаны быть отличны от нуля
	ASSERT_GT(second, 0ull);
	// Длина третьей записи обязана быть отлична от нуля
	ASSERT_GT(third, 0ull);
	// Выполняем снос второй записи контейнера
	ASSERT_TRUE(editor.erase(1)) << "код отказа: " << abc::message(editor.error());
	// Мусор обязан вырасти ровно на длину снесённой записи
	ASSERT_EQ(editor.garbage(), second);
	...
	ASSERT_TRUE(editor.replace(2, item.data(), item.size(), abc::payload_t::TEXT))
		<< "код отказа: " << abc::message(editor.error());
	/**
	 * Мусор правкой НЕ растёт, покуда правка не закреплена: прежние октеты записи
	 * остаются живыми до тех пор, пока новые не легли на носитель
	 *
	 * @note Снос и правка считают мусор В РАЗНОЕ ВРЕМЯ: снос - при зове, правка - при
	 *       закреплении. Расхождение это ЗАМЕРЕНО, а не выведено, и здесь закреплено
	 *       таким, каково оно есть: счёт после сноса равен длине снесённой записи, а
	 *       после правки не меняется вовсе
	 */
	ASSERT_EQ(editor.garbage(), second);
	// Выполняем закрепление накопленных правок на носителе
	ASSERT_TRUE(editor.commit()) << "код отказа: " << abc::message(editor.error());
	/**
	 * Закрепление обязано досчитать и прежнюю третью запись, и кадр прежнего
	 * оглавления целиком: величина последнего зависит от сжатия и здесь не сличается
	 */
	ASSERT_GT(editor.garbage(), second + third);
}
