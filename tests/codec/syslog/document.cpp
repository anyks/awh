 ...
 *
 * @details Описание RFC 5424 требует неповторимости опознавателя блока и имени поля
 *          внутри блока, но живые службы пишут и повторы. При строгом сличении повтор
 *          опознавателя отвергается, а при вольном - запись берётся как есть, и в дереве
 *          остаётся ПОСЛЕДНЕЕ значение: дерево имён не повторяет
 *
...
TEST(CodecSysLogDocument, DuplicateNames) {
	...
	EXPECT_EQ(document.at("/structures/a@1/k").text(), string("второе"));
