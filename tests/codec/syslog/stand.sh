...
#
# @brief Отдельный стенд проверок кодека syslog — сборка набора проверок без библиотеки
#        целиком, ради прогона на отладочных стендах
#
# @details Кодек syslog опирается на разбор дат и на владеющее значение дерева, и собрать
...
#
# Вызов:
#   tests/codec/syslog/stand.sh [корень дерева] [каталог сборки]
#
...
echo "Собираем стенд проверок syslog: $COMPILER"
...
#       своим стендом. Взято отсюда лишь то, без чего дерево syslog не связывается
#
...
#       владеющее значение взято у «abc» выше. Зато есть «dictionary» - словарь имён
#       источников и важностей, какого у прочих кодеков нет вовсе
#
for PART in common reader writer document dictionary; do
	...
	# Выполняем сборку проверок очередной части кодека syslog
	$COMPILER $OPTIONS -c "$ROOT/tests/codec/syslog/$PART.cpp" -o "$OUTPUT/test-$PART.o"
	# Добавляем собранное к перечню объектных файлов стенда
