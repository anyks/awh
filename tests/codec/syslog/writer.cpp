...
TEST(CodecSysLogWriter, LocaleNumbers) {
	...
	for(const char * name : {"de_DE.UTF-8", "de_DE.utf8", "German_Germany.1252", "German_Germany", "fa_IR.UTF-8", "ar_SA.UTF-8"}){
		 ...
		 *          длина записи. Замерено 08.09.2026, донесено в «sys/fmk»
		 *
 ...
 *          доводу - непокрытая ветвь неотличима от недостижимой, покуда не
 *          замерена
 *
...
TEST(CodecSysLogWriter, ByteOrderMarkOnlyForValidUtf8) {
	/**
	 * @warning Строгость эта ЗАЁМНАЯ, и проверка эта - её сторож. Годность записи в UTF-8
	 *          решает `awh::utf8::valid` - ход сторонний, - и своей поверки кодек не ведёт
	 *          намеренно: UTF-8 есть единое описание, а не смешение описаний, и держать его
	 *          в двух местах значило бы заводить второй источник истины. Щуп 15.09.2026
	 *          показал прямо: обрати тот ход во «всегда годно», и метка встаёт перед
	 *          негодным UTF-8 - то самое молчаливое искажение, ради какого поверка и
	 *          заводилась. Ослабни ход - пусть лучше покраснеет здесь
	 *
	 * @note Разница с поверкою границ чисел даты намеренна: модуль дат держит СМЕШАННЫЕ
	 *       описания, и правка одной его части задевает прочие, отчего там кодек поверяет
	 *       сам. См. `strictTimestamp` в `src/codec/syslog/reader.cpp`
	 */
	// Настройки записи событий
	syslog::writer_t::settings_t settings;
	// Устанавливаем сборку записи нынешним описанием
 ...
 * парами «имя=значение» через запятую, и обход одинаков на всякой глубине
 *
 * @warning Прежде здесь стоял сбор ДВОИЧНОЙ записи контейнера ABC, и её октеты клались в
 *          поле записи как текст: имя узла обращалось в «\x82AaAb». Запись выходила
 *          негодной вовсе - обратно не читалась и набору знаков описания противоречила, -
 *          а настройка при том обещала знаки
 *
 * @note Найдено щупом вложенности 15.09.2026 по признаку, взятому у кодеков
 *       JSON: если то же значение при ИНОЙ настройке записывается, дело в настройке, а не
 *       в значении. Тот же дефект слово в слово лежал и в кодеке CEF
 */
TEST(CodecSysLogWriter, NestedValueBecomesReadableText) {
	...
	// Укладываем отображение в поле имени узла
	ASSERT_TRUE(document.set("/header/hostname", map));
	// Настройки записи событий
	syslog::writer_t::settings_t settings;
	// Устанавливаем обращение вложенного значения в знаки
	settings.nested = syslog::nested_t::TEXT;
	 ...
	 *       контейнера, каких набор знаков описания не допускает вовсе
	 */
	...
	// Объект документа системного журнала для поверки оборота
	syslog::document_t again;
...
/**
 * @brief Проверка отказа приоритетом, наполовину объявленным
 *
 * @details Приоритет собирается из источника и важности, и обе части у него равноправны.
 * Дерево, несущее лишь одну из них, второй не объявляет вовсе, и дописать её нулём
 * значило бы объявить источник с важностью, каких отправитель не объявлял: нуль источника
 * есть ядро системы, а нуль важности есть «система непригодна к работе»
 *
 * @note Найдено щупом словаря 15.09.2026: дерево, несущее лишь «severity», писалось
 *       приставкой «<3>» - сообщением ЯДРА; дерево, несущее лишь «facility», писалось
 *       приставкой «<128>» - важностью «emerg»
 *
 * @warning Проверка эта закрепляет ОБЕ половины договора: отказ половине и прежнюю
 *          работу целому. Вторая половина важнее первой - без неё следующая правка
...
TEST(CodecSysLogWriter, HalfDeclaredPriorityIsRefused){
	...
	/**
	 * @brief Метод сборки дерева события заданными частями приоритета
	 *
	 * @param facility источник сообщения либо пустота, если объявлять его не надлежит
	 * @param severity важность сообщения либо пустота, если объявлять её не надлежит
	 * @return         собранное дерево события
	 */
	auto make = [](const string & facility, const string & severity) noexcept -> abc::value_t {
		// Дерево события системного журнала
		abc::value_t tree(abc::kind_t::MAP);
		// Если источник сообщения объявлять надлежит
		if(!facility.empty())
			// Ставим источник сообщения в дерево события
			tree.place("/facility") = abc::value_t(facility);
		// Если важность сообщения объявлять надлежит
		if(!severity.empty())
			// Ставим важность сообщения в дерево события
			tree.place("/severity") = abc::value_t(severity);
		// Ставим дату сообщения в дерево события
		tree.place("/header/timestamp") = abc::value_t(string("Oct 22 12:34:56"));
		// Ставим имя узла в дерево события
		tree.place("/header/hostname") = abc::value_t(string("myhostname"));
		// Ставим текст сообщения в дерево события
		tree.place("/message") = abc::value_t(string("Sample message"));
		// Выводим собранное дерево события
		return tree;
	};
	// Собранная запись системного журнала
	string result = "";
	// Выполняем проверку отказа дереву, лишь источник сообщения несущему
	EXPECT_FALSE(build(make("local0", ""), settings, result)) << "собрано: " << result;
	// Обнуляем собранную запись системного журнала
	result.clear();
	// Выполняем проверку отказа дереву, лишь важность сообщения несущей
	EXPECT_FALSE(build(make("", "err"), settings, result)) << "собрано: " << result;
	// Обнуляем собранную запись системного журнала
	result.clear();
	/**
	 * Выполняем проверку того, что дерево, обе части несущее, пишется по-прежнему
	 *
	 * @note Половина эта договора важнее первой: отказ, на всякое дерево с именами
	 *       распространённый, сломал бы сборку приоритета из частей вовсе
	 */
	ASSERT_TRUE(build(make("local0", "err"), settings, result));
	// Выполняем проверку приставки приоритета, из частей собранной
	EXPECT_EQ(result.compare(0, 6, "<131>O"), 0) << "собрано: " << result;
	// Обнуляем собранную запись системного журнала
	result.clear();
	// Дерево события с приоритетом, целиком объявленным
	abc::value_t whole = make("", "");
	// Ставим приоритет записи в дерево события
	whole.place("/priority") = abc::value_t(static_cast <uint64_t> (13));
	// Выполняем проверку того, что приоритет целиком объявленный пишется по-прежнему
	ASSERT_TRUE(build(whole, settings, result));
	// Выполняем проверку приставки приоритета, деревом объявленного
	EXPECT_EQ(result.compare(0, 5, "<13>O"), 0) << "собрано: " << result;
	// Обнуляем собранную запись системного журнала
	result.clear();
	// Выполняем проверку того, что дерево без приоритета вовсе пишется по-прежнему
	ASSERT_TRUE(build(make("", ""), settings, result));
	// Выполняем проверку того, что приставка приоритета НЕ домыслена
	EXPECT_NE(result.front(), '<') << "собрано: " << result;
}

/**
 * @brief Проверка набора знаков у имён структурированных данных при записи
 *
 * @details Имя структуры либо поля отмены знаков не знает вовсе, и потому имя, набору
 * описания не отвечающее, записанным быть не может: повторный разбор такой записи взял
 * бы его иначе. Поверка велась перечнем знаков, из головы взятым - пробел, табуляция,
 * переводы строки, равенство, скобка, кавычка, - и знаки управления с октетами свыше
 * набора проходили сквозь неё свободно
 *
 * @note Найдено щупом 16.09.2026: имя со знаком `0x01` уходило в собранную запись СЫРЫМ
 *       октетом, и своё же строгое чтение такую запись отвергало. Кодек писал то, чего
 *       сам не читает
 *
 * @warning Проверка закрепляет ОБЕ половины договора: отказ имени, набору не
 *          отвечающему, и прежнюю запись имени законного - вместе с опознавателем,
...
TEST(CodecSysLogWriter, StructuredNamesFollowTheNameSet){
	 ...
	 *
	 * @param id     опознаватель блока структурированных данных
	 * @param result собранная запись системного журнала
	 * @return       признак успешности сборки записи
	 */
	auto build = [](const string & id, string & result) noexcept -> bool {
		// Дерево события системного журнала
		abc::value_t tree(abc::kind_t::MAP);
		// Ставим приоритет записи в дерево события
		tree.place("/priority") = abc::value_t(static_cast <uint64_t> (165));
		// Ставим дату сообщения в дерево события
		tree.place("/header/timestamp") = abc::value_t(string("2026-09-15T00:00:00Z"));
		// Ставим имя узла в дерево события
		tree.place("/header/hostname") = abc::value_t(string("myhostname"));
		// Ставим название приложения в дерево события
		tree.place("/header/application") = abc::value_t(string("myapp"));
		 ...
		 *
		 * @warning Блок заводится ИМЕНЕМ через `insert`, а НЕ ходом `place`: тот
		 *          разбирает довод строковым путём, и опознаватель, косую черту
		 *          несущий, рвался бы им на два звена - в дереве оказался бы блок
		 *          «NOwKW0omSiy» с вложенным «Ll» вместо одного блока. Первая
		 *          редакция этой проверки 16.09.2026 на том и споткнулась, показав
		 *          отказ там, где кодек работал верно
		 */
		abc::value_t params(abc::kind_t::MAP);
		// Ставим поле блока структурированных данных
		EXPECT_TRUE(params.insert(string("iut"), abc::value_t(string("3"))));
		// Заводим блок структурированных данных дерева события
		abc::value_t structures(abc::kind_t::MAP);
		// Ставим блок структурированных данных ИМЕНЕМ его
		const bool placed = structures.insert(id, params);
		// Ставим блоки структурированных данных в дерево события
		tree.place("/structures") = structures;
		// Если завести блок именем не удалось, записывать нечего
		if(!placed)
		...
		syslog::writer_t writer;
		// Выводим признак успешности сборки записи
		return writer.write(tree, result);
	};
	// Собранная запись системного журнала
	string result = "";
	// Выполняем проверку того, что законный опознаватель записывается по-прежнему
	EXPECT_TRUE(build("exampleSDID@32473", result));
	// Обнуляем собранную запись системного журнала
	result.clear();
	/**
	 * Выполняем проверку того, что опознаватель с косою чертою записывается по-прежнему
	 *
	 * @note Половина эта договора важнее первой: косая черта набору описания отвечает,
	 *       и живые журналы её пишут - отказ ей сломал бы записи настоящих устройств
	 */
	EXPECT_TRUE(build("NOwKW0omSiy/Ll", result));
	// Обнуляем собранную запись системного журнала
	result.clear();
	// Выполняем проверку отказа записи знаком управления в опознавателе блока
	EXPECT_FALSE(build(string("exa\x01mple@1"), result)) << "собрано: " << result;
	// Обнуляем собранную запись системного журнала
	result.clear();
	// Выполняем проверку отказа записи октетом свыше набора в опознавателе блока
	EXPECT_FALSE(build(string("exa\xC3\xA9mple@1"), result)) << "собрано: " << result;
	// Обнуляем собранную запись системного журнала
	result.clear();
	// Выполняем проверку отказа записи кавычкою в опознавателе блока
	EXPECT_FALSE(build(string("exa\"mple@1"), result)) << "собрано: " << result;
	// Обнуляем собранную запись системного журнала
	result.clear();
	// Выполняем проверку отказа записи пустым опознавателем блока
	EXPECT_FALSE(build(string(""), result)) << "собрано: " << result;
}

/**
 * @brief Проверка отказа именем источника, словарю неизвестным
 *
 * @details Источник сообщения принимается именем наравне с числом, и имя это ищется в
 * словаре. Имя, словарю неизвестное, записанным быть не может: приоритет из него не
 * собрать, а домыслить его значило бы объявить источник, отправителем не объявленный
 *
 * @note Заведена по итогу сплошного щупа заслонов 16.09.2026: заслон этот набором не
 *       достигался ни разу, будучи достижимым и осмысленным
 *
 * @warning Проверка закрепляет ОБЕ половины договора: отказ имени неизвестному и
...
TEST(CodecSysLogWriter, UnknownFacilityNameIsRefused){
	 ...
	 *
	 * @param facility источник сообщения именем
	 * @param result   собранная запись системного журнала
	 * @return         признак успешности сборки записи
	 */
	auto assemble = [](const string & facility, string & result) noexcept -> bool {
		// Настройки записи событий
		syslog::writer_t::settings_t settings;
		// Устанавливаем запись приставки приоритета
		settings.prefix = true;
		...
		tree.place("/facility") = abc::value_t(facility);
		// Ставим важность сообщения в дерево события
		tree.place("/severity") = abc::value_t(string("err"));
		// Ставим дату сообщения в дерево события
		tree.place("/header/timestamp") = abc::value_t(string("Oct 22 12:34:56"));
		// Ставим имя узла в дерево события
		tree.place("/header/hostname") = abc::value_t(string("myhostname"));
		...
		tree.place("/message") = abc::value_t(string("Sample message"));
		// Выводим признак успешности сборки записи
		return ::build(tree, settings, result);
	};
	// Собранная запись системного журнала
	string result = "";
	// Выполняем проверку того, что имя словаря записывается по-прежнему
	ASSERT_TRUE(assemble("local0", result));
	// Выполняем проверку приставки приоритета, из имён собранной
	EXPECT_EQ(result.compare(0, 6, "<131>O"), 0) << "собрано: " << result;
	// Обнуляем собранную запись системного журнала
	result.clear();
	/**
	 * Выполняем проверку того, что имя, наравне принятое, записывается по-прежнему
	 *
	 * @note Половина эта договора важнее первой: службы журналов пишут правила отбора
	 *       именами «error», «warn» и «panic» наравне с основными
	 */
	ASSERT_TRUE(assemble("AUTHPRIV", result));
	// Выполняем проверку приставки приоритета, именем прописными объявленной
	EXPECT_EQ(result.compare(0, 5, "<83>O"), 0) << "собрано: " << result;
	// Обнуляем собранную запись системного журнала
	result.clear();
	// Выполняем проверку отказа записи именем источника, словарю неизвестным
	EXPECT_FALSE(assemble("НЕТ ТАКОГО", result)) << "собрано: " << result;
	// Обнуляем собранную запись системного журнала
	result.clear();
	// Выполняем проверку отказа записи именем, словарю близким, но чужим
	EXPECT_FALSE(assemble("local8", result)) << "собрано: " << result;
}

/**
 * @brief Проверка пределов ДЛИНЫ полей заголовка при записи
 *
 * @details Читатель длины полей заголовка блюдёт строгим разбором - RFC 5424 назначает
 * их разделами 6.2.4 - 6.2.7, - а писатель выдавал поле любой длины, и своя собственная
 * запись своим же читателем отвергалась кодом FIELD_TOO_LONG. Оборот не замыкался
 *
 * @note Найдено кругом аудита 17.09.2026: щуп прогнал четыре поля из четырёх, и все
 *       четыре выдавались писателем, а обратным разбором отвергались
 *
 * @warning Проверка закрепляет и РОВНЫЙ предел: поле длиною ровно в предел записи
...
TEST(CodecSysLogWriter, HeaderFieldLengthLimitsAreEnforced){
	...
	const struct {
		...
		{"/header/hostname", 255}, {"/header/application", 48},
		{"/header/process", 128}, {"/header/messageId", 32}
	};
	/**
	 * Выполняем перебор всех полей заголовка, пределом ограниченных
	 */
	for(auto & field : FIELDS){
		/**
		 * Поле длиною РОВНО в предел записи подлежит
		 */
		{
			// Объект документа системного журнала
			syslog::document_t document;
			// Выполняем проверку успешности разбора образцовой записи
			ASSERT_TRUE(document.parse("<165>1 2026-09-15T00:00:00Z host app 1 mid - ТЕКСТ"));
			// Укладываем в поле заголовка значение длиною ровно в предел
			ASSERT_TRUE(document.set(field.path, abc::value_t(string(field.limit, 'x')))) << field.path;
			// Выполняем проверку того, что запись собрана
			EXPECT_FALSE(document.dump().empty()) << "поле длиною ровно в предел отвергнуто: " << field.path;
		}
		/**
		 * Поле, предел на единицу превышающее, записи не подлежит
		 */
		{
			// Объект документа системного журнала
			syslog::document_t document;
			// Выполняем проверку успешности разбора образцовой записи
			ASSERT_TRUE(document.parse("<165>1 2026-09-15T00:00:00Z host app 1 mid - ТЕКСТ"));
			// Укладываем в поле заголовка значение, предел на единицу превышающее
			ASSERT_TRUE(document.set(field.path, abc::value_t(string(field.limit + 1, 'x')))) << field.path;
			// Выполняем проверку того, что сборка записи отказом отвечена
			EXPECT_TRUE(document.dump().empty()) << "поле, предел превысившее, записано: " << field.path;
			// Выполняем проверку того, что отказ назвал причину слишком длинным полем
			EXPECT_EQ(
				static_cast <uint8_t> (document.error()),
				static_cast <uint8_t> (syslog::error_t::FIELD_TOO_LONG)
			) << field.path;
		}
	}
}
...
/**
 * @brief Проверка ОТСУТСТВИЯ пределов длины у описания RFC 3164
 *
 * @details Пределы длины полей заголовка назначены RFC 5424 и к прежнему описанию
 * неприменимы вовсе: RFC 3164 длин не ограничивает, и поверка, на него распространённая,
 * отвергала бы законные записи
 *
 * @note Проверка эта - вторая половина правки пределов: без неё поверку легко перенести
...
TEST(CodecSysLogWriter, LegacyStandardKnowsNoLengthLimits){
	...
	ASSERT_TRUE(document.parse("<34>Oct 11 22:14:15 mymachine su: ТЕКСТ"));
	// Выполняем проверку того, что запись прежним описанием опознана
	ASSERT_EQ(document.standard(), syslog::standard_t::RFC3164);
	// Укладываем в имя узла значение, предел нынешнего описания превышающее
	ASSERT_TRUE(document.set("/header/hostname", abc::value_t(string(300, 'x'))));
	// Выполняем проверку того, что запись собрана, пределом не стеснённая
	EXPECT_FALSE(document.dump().empty()) << "предел RFC 5424 наложен на запись RFC 3164";
}
