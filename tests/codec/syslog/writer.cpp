...
TEST(CodecSysLogWriter, LocaleNumbers) {
	...
	for(const char * name : {"de_DE.UTF-8", "de_DE.utf8", "German_Germany.1252", "German_Germany", "fa_IR.UTF-8", "ar_SA.UTF-8"}){
		...
		EXPECT_NE(result.find("k=\"0.1\""), string::npos) << name << ": " << result;
		// Выполняем проверку записи числа с большим количеством разрядов
		EXPECT_NE(result.find("m=\"2986.808299\""), string::npos) << name << ": " << result;
		// Выполняем проверку того, что запятая в запись не попала вовсе
		EXPECT_EQ(result.find(','), string::npos) << name << ": " << result;
