...
/**
 * Подключаем глушитель вывода журнала
 *
 * @details Подключается ЗДЕСЬ и единожды на кодек: слушатель заводится на процесс, и
 *          второй такой же ничего не прибавит. Часть «common» выбрана оттого, что она
 *          входит и в отдельный стенд кодека, где общего «tests/main.cpp» нет вовсе
 */
#include "../silent.hpp"
...
TEST(CodecSysLogCommon, Messages) {
	 ...
	 */
	for(uint8_t code = 0; code <= static_cast <uint8_t> (syslog::error_t::INVALID_MESSAGE_UTF8); code++){
	 ...
	 *
	 * @warning Заведено 09.09.2026 по образцу кодеков JSON, XML и CSV, где сторож этот стоит
	 *          во всех трёх наборах. Собственная беда того же дня: код FILE_NOT_READ (ныне граница - INVALID_HEADER_FIELD)
	 ...
	 */
	EXPECT_STREQ(syslog::message(static_cast <syslog::error_t> (static_cast <uint32_t> (syslog::error_t::INVALID_MESSAGE_UTF8) + 1)), "unknown error");
}
