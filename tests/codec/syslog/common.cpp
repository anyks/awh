...
TEST(CodecSysLogCommon, Messages) {
	 ...
	 */
	for(uint8_t code = 0; code <= static_cast <uint8_t> (syslog::error_t::MULTIPLE_RECORDS); code++){
	 ...
	 */
	EXPECT_STREQ(syslog::message(static_cast <syslog::error_t> (static_cast <uint32_t> (syslog::error_t::MULTIPLE_RECORDS) + 1)), "unknown error");
}
