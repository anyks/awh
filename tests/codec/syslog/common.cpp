...
TEST(CodecSysLogCommon, Messages) {
	 ...
	 */
	for(uint8_t code = 0; code <= static_cast <uint8_t> (syslog::error_t::FILE_NOT_OPENED); code++){
	...
	EXPECT_STREQ(syslog::message(static_cast <syslog::error_t> (0xFF)), "unknown error");
}
