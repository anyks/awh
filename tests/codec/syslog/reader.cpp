 ...
 *
 * @note Замечено 07.09.2026: видов даты много, и зависят они от системы.
...
TEST(CodecSysLogReader, TimestampStrictnessOfTheStandard) {
	...
	const vector <string> REFUSED = {
		 ...
		 *          так 15.09.2026: модуль дат держит несколько
	...
	const vector <string> TOLERATED = {
		 ...
		 *       дефекту сличения литералов с учётом регистра. Модуль дат дефект
 ...
 *
 * @note Заведена 15.09.2026 по разряду, взятому у кодеков JSON: проверка,
...
/**
 * @brief Проверка набора знаков у имён структурированных данных
 *
 * @details RFC 5424, раздел 6.3.3, задаёт именам структур и полей свой набор, от
 * заголовочных полей отличный: `SD-NAME = 1*32 PRINTUSASCII`, за вычетом знака
 * равенства, пробела, закрывающей скобки и кавычки. Строгое сличение поверяло у имён
 * ДЛИНУ и не поверяло набор знаков ничем
 *
 * @note Найдено щупом 16.09.2026: восемь имён, описанием запрещённых, принимались
 *       строгим сличением все восемь
 *
 * @warning Проверка закрепляет и то, что довод «разделители отсеяны разбором» ОПОРОЮ
 *          НЕ СЛУЖИТ: имя поля разбор обрывает знаком равенства, а имя БЛОКА — нет,
 *          и первая редакция поверки, на том доводе стоявшая, `[exa=mple@1 ...]`
...
TEST(CodecSysLogReader, StructuredNamesFollowTheNameSet){
	...
	/**
	 * @brief Метод разбора записи строгим сличением
	 *
	 * @param structured блок структурированных данных записи
	 * @return           признак успешности разбора записи
	 */
	auto strict = [&head](const string & structured) noexcept -> bool {
		// Объект документа системного журнала
		syslog::document_t document;
		...
		return document.parse(head + structured + " message");
	};
	// Выполняем проверку того, что законное имя принимается по-прежнему
	EXPECT_TRUE(strict("[exampleSDID@32473 iut=\"3\"]"));
	// Выполняем проверку отказа кавычкою в опознавателе блока
	EXPECT_FALSE(strict("[exa\"mple@1 iut=\"3\"]"));
	/**
	 * Выполняем проверку отказа знаком равенства в опознавателе блока
	 *
	 * @note Случай этот главный в наборе: разбор обрывает опознаватель блока лишь
	 *       пробелом и скобкой, отчего знак равенства проходит в него свободно
	 */
	EXPECT_FALSE(strict("[exa=mple@1 iut=\"3\"]"));
	// Выполняем проверку отказа знаком управления в опознавателе блока
	EXPECT_FALSE(strict(string("[exa\x01mple@1 iut=\"3\"]")));
	// Выполняем проверку отказа знаком 0x7F в опознавателе блока
	EXPECT_FALSE(strict(string("[exa\x7Fmple@1 iut=\"3\"]")));
	// Выполняем проверку отказа октетом свыше набора в опознавателе блока
	EXPECT_FALSE(strict(string("[exa\xC3\xA9mple@1 iut=\"3\"]")));
	// Выполняем проверку отказа кавычкою в имени поля
	EXPECT_FALSE(strict("[example@1 i\"ut=\"3\"]"));
	// Выполняем проверку отказа знаком управления в имени поля
	EXPECT_FALSE(strict(string("[example@1 i\x01ut=\"3\"]")));
	// Выполняем проверку отказа октетом свыше набора в имени поля
	EXPECT_FALSE(strict(string("[example@1 i\xD0\xBAut=\"3\"]")));
	/**
	 * Выполняем проверку того, что слабое сличение те же записи принимает
	 *
	 * @note Половина эта договора важнее первой: набор знаков есть требование
	 *       СТРОГОГО сличения, и записи живых устройств, его нарушающие, слабым
	 *       сличением обязаны читаться по-прежнему
	 */
	syslog::document_t document;
	// Настройки разбора записи
	syslog::reader_t::settings_t settings;
	// Устанавливаем слабое сличение записи
	settings.mode = syslog::mode_t::WEAK;
	// Устанавливаем настройки разбора записи
	document.settings(settings);
	// Выполняем проверку того, что запись с кавычкою в имени слабым сличением принята
	EXPECT_TRUE(document.parse(head + "[exa\"mple@1 iut=\"3\"] message"));
}

/**
 * @brief Проверка записи номера описания, а не одного его значения
 *
 * @details RFC 5424, раздел 6.2.2, задаёт номеру описания вид `VERSION = NONZERO-DIGIT
 * 0*2DIGIT`: первая цифра не нуль, и всего цифр не более трёх. Отсюда два запрета,
 * значением номера не выразимые: нуль номером не бывает вовсе, и ведущий нуль запрещён.
 * Число, из записи набранное, ведущих нулей не помнит, и судить надлежит ЗАПИСЬ поля
 *
 * @note Найдено щупом 16.09.2026: записи «<165>0 ...», «<165>01 ...» и «<165>001 ...»
 *       разбирались строгим сличением как RFC 5424 редакции ПЕРВОЙ - предел номера
 *       равен единице, и нуль в него укладывался. При этом редакция вторая, видом
 *       законная, отвергалась как неподдерживаемая: строгость спрашивала с годных
 *       записей и не спрашивала с негодных
 *
 * @warning Проверка закрепляет ОБЕ половины договора и ещё отличие двух отказов друг
 *          от друга: отказ номеру негодному (INVALID_VERSION) и отказ номеру годному,
 *          но неподдерживаемому (UNSUPPORTED_VERSION), суть разные ответы, и слить
 *          их в один значило бы потерять различие между «запись битая» и «запись
...
TEST(CodecSysLogReader, VersionFieldIsJudgedAsWritten){
	 ...
	 *
	 * @param version номер описания записи
	 * @param mode    строгость сличения записи
	 * @return        признак успешности разбора записи
	 */
	auto parse = [](const string & version, const syslog::mode_t mode) noexcept -> bool {
		// Объект документа системного журнала
		syslog::document_t document;
		// Настройки разбора записи
		syslog::reader_t::settings_t settings;
		...
		document.settings(settings);
		// Выводим признак успешности разбора записи
		return document.parse("<165>" + version + " 2026-09-15T00:00:00Z host app 1 - - hi");
	};
	// Выполняем проверку того, что номер описания первый принимается по-прежнему
	EXPECT_TRUE(parse("1", syslog::mode_t::STRONG));
	// Выполняем проверку отказа строгого сличения нулевым номером описания
	EXPECT_FALSE(parse("0", syslog::mode_t::STRONG));
	// Выполняем проверку отказа строгого сличения ведущим нулём номера описания
	EXPECT_FALSE(parse("01", syslog::mode_t::STRONG));
	// Выполняем проверку отказа строгого сличения двумя ведущими нулями
	EXPECT_FALSE(parse("001", syslog::mode_t::STRONG));
	/**
	 * Выполняем проверку отказа номеру описания, видом законному, но неподдерживаемому
	 *
	 * @note Отказ этот иного рода и наводится ИНЫМ кодом: запись такая не битая, а
	 *       описания, кодеку неведомого
	 */
	EXPECT_FALSE(parse("2", syslog::mode_t::STRONG));
	/**
	 * Выполняем проверку того, что слабое сличение те же записи принимает
	 *
	 * @note Половина эта договора важнее первой: вид номера есть требование СТРОГОГО
	 *       сличения, и записи живых устройств, его нарушающие, слабым сличением
	 *       обязаны читаться по-прежнему
	 */
	EXPECT_TRUE(parse("0", syslog::mode_t::WEAK));
	// Выполняем проверку того, что ведущий нуль слабым сличением принимается
	EXPECT_TRUE(parse("01", syslog::mode_t::WEAK));
}

/**
 * @brief Проверка годности текста сообщения кодировке, меткою объявленной
 *
 * @details RFC 5424, раздел 6.4, требует прямо: текст, меткою порядка байтов открытый,
 * ОБЯЗАН быть годным UTF-8. Метка есть заявление о кодировке, и заявление это поверяемо
 *
 * @note Найдено щупом 16.09.2026: пять текстов, меткою объявленных UTF-8 и годным UTF-8
 *       НЕ бывших - обрывок пары, одинокий хвост, октет 0xFE, сверхдлинный нуль и
 *       суррогат D800, - приняты строгим сличением все пять, тогда как рамка своим же
 *       ходом `awh::utf8::valid` звала их негодными. Кодек метку снимал и о годности
 *       не спрашивал
 *
 * @warning Проверка закрепляет ОБЕ половины договора, и вторая важнее первой: текст БЕЗ
 *          метки не поверяется вовсе и обязан читаться по-прежнему. Заявления о
 *          кодировке там нет, описание дозволяет ему быть любым набором октетов, и
 *          записи живых устройств сплошь такие - отказ им сломал бы чтение настоящих
 *          журналов
 *
 */
TEST(CodecSysLogReader, MarkedMessageMustBeValidUtf8){
	 ...
	 * @brief Метод разбора записи заданным сличением
	 *
	 * @param message текст сообщения записи
	 * @param mode    строгость сличения записи
	 * @return        признак успешности разбора записи
	 */
	auto parse = [](const string & message, const syslog::mode_t mode) noexcept -> bool {
		// Объект документа системного журнала
		syslog::document_t document;
		// Настройки разбора записи
		syslog::reader_t::settings_t settings;
		// Устанавливаем строгость сличения записи
		settings.mode = mode;
		// Устанавливаем снятие метки порядка байтов с текста сообщения
		settings.bom = true;
		// Устанавливаем настройки разбора записи
		document.settings(settings);
		// Выводим признак успешности разбора записи
		return document.parse("<165>1 2026-09-15T00:00:00Z host app 1 - - " + message);
	};
	// Выполняем проверку того, что текст с меткою и годным UTF-8 принимается
	EXPECT_TRUE(parse(mark + "hello", syslog::mode_t::STRONG));
	// Выполняем проверку того, что кириллица с меткою принимается
	EXPECT_TRUE(parse(mark + "\xD0\xBF\xD1\x80\xD0\xB8", syslog::mode_t::STRONG));
	// Выполняем проверку отказа обрывком пары октетов
	EXPECT_FALSE(parse(mark + "a\xD0", syslog::mode_t::STRONG));
	// Выполняем проверку отказа одиноким хвостовым октетом
	EXPECT_FALSE(parse(mark + "a\x80\x80", syslog::mode_t::STRONG));
	// Выполняем проверку отказа октетом 0xFE, кодировке чуждым вовсе
	EXPECT_FALSE(parse(mark + "a\xFE", syslog::mode_t::STRONG));
	// Выполняем проверку отказа сверхдлинной записью нуля
	EXPECT_FALSE(parse(mark + "a\xC0\x80", syslog::mode_t::STRONG));
	// Выполняем проверку отказа записью суррогатной пары
	EXPECT_FALSE(parse(mark + "a\xED\xA0\x80", syslog::mode_t::STRONG));
	/**
	 * Выполняем проверку того, что те же октеты БЕЗ метки принимаются
	 *
	 * @note Половина эта договора важнее первой: заявления о кодировке нет, и
	 *       спрашивать с текста годности UTF-8 не с чего
	 */
	EXPECT_TRUE(parse(string("a\xD0"), syslog::mode_t::STRONG));
	// Выполняем проверку того, что октет 0xFE без метки принимается строгим сличением
	EXPECT_TRUE(parse(string("a\xFE"), syslog::mode_t::STRONG));
	/**
	 * Выполняем проверку того, что слабое сличение негодную кодировку принимает
	 *
	 * @note Годность кодировке есть требование СТРОГОГО сличения
	 */
	EXPECT_TRUE(parse(mark + "a\xFE", syslog::mode_t::WEAK));
}

/**
 * @brief Проверка заслонов, набором прежде не достигавшихся
 *
 * @details Проверка эта заведена по итогу сплошного щупа заслонов 16.09.2026: стенд
 * собран с покрытием, набор прогнан, и заслоны, ни разу не сработавшие, выписаны
 * замером, а не догадкой. Молчание их разобрано на три природы, и здесь собраны те,
 * что относятся к ТРЕТЬЕЙ - достижимые и осмысленные, но набором не тронутые
 *
 * @note Две прочих природы правки не требуют и здесь не проверяются. Первая - стражи
 *       устройства, недостижимые по построению: ветвь `default` у исчерпывающего
 *       перебора описаний, извлечение значения после поверки его вида, отказ укладки
 *       в перечень контейнера. Вторая - ветви кода нового, набором ещё не достигнутые
 *
 * @warning Заслон, ни разу не сработавший, есть обещание непроверенное: он либо
 *          недостижим вовсе, либо достижим и не проверен - и различить это можно лишь
 *          щупом, а не чтением
 *
 */
TEST(CodecSysLogReader, GuardsUntouchedByTheSuite){
	 ...
	 *
	 * @note Дата пробуется четырьмя видами - RFC 3164, RFC 3339, asctime и запись с
	 *       пробелом вместо знака «T», - и заслон этот стоит за всеми четырьмя
	 */
	{
		// Объект документа системного журнала
		syslog::document_t document;
		...
		// Устанавливаем слабое сличение записи
		settings.mode = syslog::mode_t::WEAK;
		// Устанавливаем чтение записи устаревшим описанием
		settings.standard = syslog::standard_t::RFC3164;
		// Устанавливаем настройки разбора записи
		document.settings(settings);
		// Выполняем проверку отказа разбора записи с негодной датой
		EXPECT_FALSE(document.parse("<13>НЕ ДАТА ВОВСЕ host app: hi"));
		// Выполняем проверку кода отказа
		EXPECT_EQ(static_cast <uint8_t> (document.error()), static_cast <uint8_t> (syslog::error_t::INVALID_TIMESTAMP));
	}
	/**
	 * Выполняем проверку отказа незакрытой скобкой опознавателя работы
	 *
	 * @note Записи живых устройств рвутся посреди поля, и скобка при том остаётся
	 *       незакрытой: заслон этот стоит ровно на такой записи
	 */
	{
		// Объект документа системного журнала
		syslog::document_t document;
		...
		// Устанавливаем настройки разбора записи
		document.settings(settings);
		// Выполняем проверку отказа разбора записи с незакрытой скобкой
		EXPECT_FALSE(document.parse("<13>Oct 22 12:34:56 myhost myapp[1234"));
		// Выполняем проверку кода отказа
		EXPECT_EQ(static_cast <uint8_t> (document.error()), static_cast <uint8_t> (syslog::error_t::INVALID_PROCESS));
	}
	/**
	 * Выполняем проверку того, что скобка ЗАКРЫТАЯ принимается по-прежнему
	 *
	 * @note Половина эта договора важнее первой: заслон обязан ловить незакрытую
	 *       скобку и не трогать закрытую
	 */
	{
		// Объект документа системного журнала
		syslog::document_t document;
		...
		// Устанавливаем настройки разбора записи
		document.settings(settings);
		// Выполняем проверку успешности разбора записи с закрытой скобкой
		ASSERT_TRUE(document.parse("<13>Oct 22 12:34:56 myhost myapp[1234]: hi"));
...
/**
 * @brief Проверка отказа пустым полем заголовка
 *
 * @details Описание назначает разделителем полей РОВНО ОДИН пробел и задаёт каждому полю
 * заголовка длину не менее одного знака либо знак отсутствия значения. Два пробела
 * подряд давали поле пустое, и запись такая принималась строгим сличением наравне с
 * записью законной
 *
 * @note Найдено картою кодов отказа 16.09.2026: код `EMPTY_HEADER_FIELD` объявлен с
 *       описанием и не наводился НИ ОДНИМ местом кодека. Правило же его стояло запискою
 *       у самого разбора полей - обещание было записано и не выполнено
 *
 * @warning Проверка закрепляет ОБЕ половины договора: отказ полю пустому и прежний приём
 *          знака «-», каковым отсутствие значения и записывается. Слить их значило бы
...
TEST(CodecSysLogReader, EmptyHeaderFieldIsRefusedWhenStrict){
	 ...
	 * @return       признак успешности разбора записи
	 */
	auto parse = [](const string & record, const syslog::mode_t mode) noexcept -> bool {
		// Объект документа системного журнала
		syslog::document_t document;
		// Настройки разбора записи
		syslog::reader_t::settings_t settings;
		// Устанавливаем строгость сличения записи
		settings.mode = mode;
		// Устанавливаем чтение записи нынешним описанием
		settings.standard = syslog::standard_t::RFC5424;
		// Устанавливаем настройки разбора записи
		document.settings(settings);
		// Выводим признак успешности разбора записи
	...
	// Выполняем проверку того, что запись с полными полями принимается по-прежнему
	EXPECT_TRUE(parse("<165>1 2026-09-15T00:00:00Z host app 1 - - hi", syslog::mode_t::STRONG));
	/**
	 * Выполняем проверку того, что знак отсутствия значения принимается по-прежнему
	 *
	 * @note Половина эта договора важнее первой: знаком «-» отсутствие значения и
	 *       записывается, и отказ ему сломал бы законные записи
	 */
	EXPECT_TRUE(parse("<165>1 2026-09-15T00:00:00Z - - - - hi", syslog::mode_t::STRONG));
	// Выполняем проверку отказа пустым именем узла
	EXPECT_FALSE(parse("<165>1 2026-09-15T00:00:00Z  app 1 - - hi", syslog::mode_t::STRONG));
	// Выполняем проверку отказа пустым названием приложения
	EXPECT_FALSE(parse("<165>1 2026-09-15T00:00:00Z host  1 - - hi", syslog::mode_t::STRONG));
	// Выполняем проверку кода отказа пустым полем заголовка
	{
		...
		// Устанавливаем настройки разбора записи
		document.settings(settings);
		// Выполняем проверку отказа разбора записи с пустым полем
		ASSERT_FALSE(document.parse("<165>1 2026-09-15T00:00:00Z  app 1 - - hi"));
		// Выполняем проверку кода отказа
		EXPECT_EQ(static_cast <uint8_t> (document.error()), static_cast <uint8_t> (syslog::error_t::EMPTY_HEADER_FIELD));
	}
	/**
	 * Выполняем проверку того, что слабое сличение пустое поле принимает
	 *
	 * @note Слабое сличение отвечает отказом лишь неразбираемому, а поле пустое
	 *       разбирается
	 */
	EXPECT_TRUE(parse("<165>1 2026-09-15T00:00:00Z  app 1 - - hi", syslog::mode_t::WEAK));
}
...
TEST(CodecSysLogReader, ChunkedPositionsHoldTheirPlaces){
	...
	const string text =
		"<165>1 2023-04-11T23:29:33.003Z machine.example.com evntslog - ID47 "
		"[exampleSDID@32473 iut=\"3\" eventSource=\"App\\]ication\"][b@1 x=\"y\"] Текст события\n"
		"<165>Oct 22 10:52:01 host sched[222]: That's All Folks!\n"
		"<13>Sat Jan  8 20:07:41 2011 myhostname myapp[1234]: Sample.\n";
	/**
