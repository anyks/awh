 ...
 *
 */
TEST(CodecTomlCommon, MalformedSequenceIsMeasuredByItsLongestPart) {
	/**
	...
	for(const auto & item : {
		...
		ASSERT_NE(toml::inspect(item.first, 0, code, length), toml::utf8_t::VALID) << item.first.size();
	...
	ASSERT_EQ(toml::inspect(string_view("\xD0\x94", 2), 0, code, length), toml::utf8_t::VALID);
