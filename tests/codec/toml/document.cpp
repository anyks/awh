 ...
 *
 * @details Ход общий у всех семи кодеков рамки. Прежде снос звался у TOML двумя именами
 *          разом - `erase(перечень)` снимал пару, `remove(перечень)` таблицу, - а имена
 *          эти в языке суть синонимы, и потребитель угадать по ним, который что делает,
 *          не мог вовсе
 *
 * @note Проверка держит и то, что прежние ходы целы: доводы их и довод нового различаются
 *       лишь видом, и заведение нового сделало старые двусмысленными. Ограждение снято
 ...
 *
 */
TEST(CodecTomlDocument, EncodingIsReportedByTheReader){
	...
	ASSERT_EQ(document.encoding(), toml::encoding_t::NONE);
	...
	ASSERT_EQ(document.encoding(), toml::encoding_t::UTF8);
}
TEST(CodecTomlDocument, LoadAndSaveRoundTrip){
	// Путь до временного файла настроек
	const string filename = string(::testing::TempDir()) + ::unique("toml-roundtrip.conf");
	// Объект дерева настроек, текст разбирающий
	toml::document_t source(::framework(), ::logger());
	// Выполняем разбор текста настроек
	...
	toml::document_t loaded(::framework(), ::logger());
	...
	toml::document_t missing(::framework(), ::logger());
...
TEST(CodecTomlDocument, RemovingValueByPath){
	...
	// Выполняем проверку наличия обеих пар до снятия
	ASSERT_TRUE(document.has("/r/a"));
	ASSERT_TRUE(document.has("/r/b"));
	// Выполняем снятие значения по пути
	ASSERT_TRUE(document.erase("/r/a")) << toml::message(document.error());
	// Выполняем проверку того, что снятое значение из дерева ушло
	ASSERT_FALSE(document.has("/r/a"));
	// Выполняем проверку того, что соседнее значение снятия не заметило
	ASSERT_TRUE(document.has("/r/b"));
	/**
	 * Выполняем проверку того, что пустой путь сносит ВЕСЬ документ
	 *
	 * @details Пустой путь по описанию RFC 6901 означает весь документ, а не отсутствующее
	 *          свойство. Дозволен снос оттого, что пустой файл настроек TOML законен
	 *          вполне, - в отличие от JSON и XML, где описание велит нести ровно одно
	 *          значение либо один корневой узел, и снос корня отвергается ПО СТАНДАРТУ
	 *
	 * @note Прежде здесь стоял отказ, и примечание при нём ссылалось на согласие
	 *       владельцев прочих кодеков - согласия, какого не было: сговорено было обратное.
	 *       Нашлось сличением erase("") по пяти кодекам разом
	 */
	ASSERT_TRUE(document.erase("")) << toml::message(document.error());
	// Выполняем проверку того, что снос пустым путём опустошил настройки целиком
	ASSERT_TRUE(document.empty());
	ASSERT_EQ(document.size(), 0u);
	// Выполняем разбор текста настроек заново для проверки прежнего хода
	ASSERT_TRUE(document.parse("[r]\nb = 2\n")) << toml::message(document.error());
	// Выполняем проверку того, что прежний ход снятия пары перечнем цел
	ASSERT_TRUE(document.erase({"r", "b"})) << toml::message(document.error());
	// Выполняем проверку того, что пара, перечнем снятая, из дерева ушла
	ASSERT_FALSE(document.has("/r/b"));
}
...
TEST(CodecTomlDocument, Typed) {
	 ...
	 */
	{
		...
		toml::document_t halved(::framework(), ::logger());
		// Выполняем разбор текста настроек
...
TEST(CodecTomlDocument, Creating) {
	...
	toml::document_t reread(::framework(), ::logger());
...
TEST(CodecTomlDocument, EditReadable) {
	...
	for(const string & source : {
		...
		for(uint32_t kind = 0; kind < 4; kind++){
			...
			toml::document_t after(::framework(), ::logger());
...
TEST(CodecTomlDocument, StampEditRefusesMalformed) {
	...
	/**
	 * Выполняем проверку отказа правки временем, за пределы суток выходящим
	 *
	 * @note Застава эта стояла непроверенной: поверки даты да вида проверены были обе,
	 *       а поверка времени, третьей рядом с ними стоящая, не задета ни разу.
	 *       Открыло пробел объединённое покрытие набора с ворошителем
	 */
	{
		// Дерево настроек, отметку разбирающее
		toml::document_t document(::framework(), ::logger());
		...
		ASSERT_TRUE(document.parse(text));
		/**
		 * Проверяемые времена, за пределы суток выходящие
		 *
		 * @note Секунда шестидесятая сюда НЕ входит: описанием она дозволена - ею
		 *       записывается секунда координации, - и отвергать её нельзя
		 */
		const vector <array <uint32_t, 4>> beyond = {
			{{24, 0, 0, 0}}, {{0, 60, 0, 0}}, {{0, 0, 61, 0}}, {{0, 0, 0, 1000000000}}
		};
		/**
		 * Выполняем перебор всех времён, за пределы суток выходящих
		 */
		for(auto & moment : beyond){
			// Отметка времени, правкою назначаемая
			toml::stamp_t stamp;
			// Устанавливаем год отметки
			stamp.date.year = 2026;
			...
			stamp.time.hour = moment.at(0);
			// Устанавливаем минуту отметки
			stamp.time.minute = moment.at(1);
			// Устанавливаем секунду отметки
			stamp.time.second = moment.at(2);
			// Устанавливаем долю секунды отметки
			stamp.time.nanosecond = moment.at(3);
			// Выполняем проверку отказа правки временем негодным
			ASSERT_FALSE(document.set({"mark"}, stamp, toml::type_t::LOCAL_DATETIME)) << moment.at(0);
			// Выполняем проверку того, что отказ назван причиною своею
			ASSERT_EQ(document.error(), toml::error_t::INVALID_DATETIME) << moment.at(0);
			// Выполняем проверку того, что дерево отказом не тронуто
			ASSERT_EQ(document.text(), text) << moment.at(0);
		}
	}
	/**
	 * Выполняем проверку отказа правки смещением часового пояса негодным
	 *
	 * @details Поверки этой не было вовсе: дерево смещение принимало любое, а писатель
	 *          отрезок значений поверял своею заставою и отказывал ЗАПИСИ. Потребитель,
	 *          правкою обнадёженный, получал пустую запись вместо всего документа -
	 *          пары, правкою не тронутые, пропадали вместе с нею
	 *
	 * @note Отрезок берётся тот же, каким поверяет смещение писатель: 23 часа 59 минут
	 *       в обе стороны. Величина NO_TIMEZONE отрезком не поверяется - ею обозначено
	 *       отсутствие смещения
	 */
	{
		// Дерево настроек, отметку разбирающее
		...
		ASSERT_TRUE(document.parse(text));
		/**
		 * Проверяемые смещения, за отрезок значений выходящие
		 */
		const vector <int32_t> beyond = {9999, -9999, ((23 * 60) + 60), -((23 * 60) + 60)};
		/**
		 * Выполняем перебор всех смещений, за отрезок значений выходящих
		 */
		for(auto offset : beyond){
			// Отметка времени, правкою назначаемая
			toml::stamp_t stamp;
			// Устанавливаем год отметки
			stamp.date.year = 2026;
			...
			stamp.time.hour = 12;
			// Устанавливаем минуту отметки
			stamp.time.minute = 30;
			// Устанавливаем смещение часового пояса отметки
			stamp.offset = offset;
			// Выполняем проверку отказа правки смещением негодным
			ASSERT_FALSE(document.set({"mark"}, stamp, toml::type_t::OFFSET_DATETIME)) << offset;
			// Выполняем проверку того, что отказ назван причиною своею
			ASSERT_EQ(document.error(), toml::error_t::INVALID_DATETIME) << offset;
			/**
			 * Выполняем проверку того, что дерево отказом не тронуто
			 *
			 * @warning Половина эта и есть самая суть находки: прежде запись отдавала
			 *          НУЛЬ БАЙТОВ, и терялось всё дерево целиком, а не один узел
			 */
			ASSERT_EQ(document.text(), text) << offset;
		}
		/**
		 * Выполняем проверку того, что смещение крайнее правкою принято
		 *
		 * @note Половина эта стережёт ширину отказа: застава, всякое смещение
		 *       отвергающая, прошла бы перебор выше целиком
		 */
		{
			// Отметка времени, правкою назначаемая
			toml::stamp_t stamp;
			...
			// Устанавливаем минуту отметки
			stamp.time.minute = 30;
			// Устанавливаем смещение часового пояса крайнее допустимое
			stamp.offset = ((23 * 60) + 59);
			// Выполняем проверку успешности правки смещением крайним
			ASSERT_TRUE(document.set({"mark"}, stamp, toml::type_t::OFFSET_DATETIME)) << toml::message(document.error());
			// Выполняем проверку того, что запись правленого дерева удалась
			ASSERT_EQ(document.text(), "mark = 2026-08-27T12:30:00+23:59\n");
		}
	}
	/**
...
TEST(CodecTomlDocument, KeyWithNewlineSurvivesRoundtrip) {
	...
	toml::document_t other(::framework(), ::logger());
...
TEST(CodecTomlDocument, EditRefusalsNotCoveredBefore) {
	 ...
	 */
	{
		...
		toml::document_t empty(::framework(), ::logger());
 ...
 *
 * @note Имя с косою чертою у TOML законно: ключ в кавычках принимает всякий знак, а
 *       голый ключ ограничен знаками ASCII по стандарту. Замерено 03.09.2026 до правки:
 *       имя `a/b` в дереве стояло, а `at("/a/b")` отвечало недействительным значением
 *
 */
TEST(CodecTomlDocument, PathTokenCarriesEscapesOfTheStandard){
	 ...
	 */
	ASSERT_TRUE(document.present("/a~1b"));
	// Выполняем извлечение значения по записанному звену
	ASSERT_TRUE(document.at("/a~1b").value(number));
	 ...
	 *       паре `b` таблицы `a`, какой в дереве нет вовсе, и обход ушёл бы в никуда молча
	 */
	ASSERT_FALSE(document.present("/a/b"));
	// Выполняем проверку достижимости имени со знаком отмены отменяющей записью
	ASSERT_TRUE(document.present("/тильда~0тут"));
	// Выполняем извлечение значения по записанному звену со знаком отмены
	ASSERT_TRUE(document.at("/тильда~0тут").value(number));
	// Выполняем проверку того, что записанное звено ведёт к тому самому имени
	 ...
	 */
	ASSERT_FALSE(document.present("/тильда~тут"));
	// Выполняем проверку того, что имя отменяющих записей не требующее достижимо просто
	ASSERT_TRUE(document.present("/обычное"));
	// Выполняем проверку того, что знак отмены в конце звена отвергается
	ASSERT_FALSE(document.present("/обычное~"));
}
/**
 * @brief Проверка огранения поднятого предела вложенности потолком допустимого
 *
 * @details Разбор вложенности ведётся стопою вызовов, и настройка, поднятая потребителем
 * выше потолка, обходила его вовсе: текст из одних открывающих скобок срывал стек
 *
 * @note Замерено 04.09.2026 щупом до правки: при `maxDepth = 200000` глубина 100000
 *       валила процесс по SIGSEGV, а при настройке по умолчанию тот же текст отвечал
 *       честным отказом. Нашёл владелец кодека CSV своим щупом на этой же машине
 *
 * @warning Глубина здесь взята НАМНОГО выше потолка, но НИЖЕ той, что стек срывает:
 *          возьми мы глубину срывающую - проверка при возврате порока валила бы весь
 *          набор целиком вместо честного красного, и причина отказа стала бы невидима
 *
 */
TEST(CodecTomlDocument, RaisedDepthLimitIsBoundedByTheCeiling){
	// Объект дерева настроек, текст разбирающего
	toml::document_t document(::framework(), ::logger());
	// Получаем настройки дерева настроек
	toml::document_t::settings_t settings = document.settings();
	// Поднимаем предел вложенности много выше потолка допустимого
	settings.reader.maxDepth = 200000;
	// Выполняем установку настроек дерева настроек
	document.settings(settings);
	// Собираемый текст настроек с глубокой вложенностью перечней
	string text = "a = ";
	// Выполняем добавление открывающих скобок перечней значений
	text.append(4096, '[');
	// Выполняем добавление закрывающих скобок перечней значений
	text.append(4096, ']');
	/**
	...
	ASSERT_EQ(document.error(), toml::error_t::DEPTH_EXCEEDED);
	/**
	 * Выполняем проверку того, что вложенность в пределах потолка разбирается
	 *
	 * @note Половина эта важнее первой: огранение, поставленное слишком низко, отвергало
	 *       бы и то, что стандартом дозволено, и отказ был бы неотличим от починки
	 */
	{
		// Собираемый текст настроек со вложенностью в пределах потолка
		string shallow = "a = ";
		// Выполняем добавление открывающих скобок перечней значений
		shallow.append(8, '[');
		// Выполняем добавление закрывающих скобок перечней значений
		shallow.append(8, ']');
		// Выполняем проверку разбора текста со вложенностью в пределах потолка
		ASSERT_TRUE(document.parse(shallow)) << toml::message(document.error());
	}
}
/**
 * @brief Проверка уклада нуля и признака дозволения вложенности
 *
 * @details Уклад нуля сведён к общему у всех кодеков рамки: ноль значит «без предела».
 * Прежде ноль вложенность ЗАПРЕЩАЛ, и настройка, перенесённая потребителем с иного
 * кодека, срабатывала наоборот и молча. Запрет получил своё выражение - признак
 * `nesting`, - ибо одним числом две разные мысли выразить нельзя, не отняв одну из них
 *
 * @note Правило выбора: RFC про уклад нуля не говорит ничего, а где стандарт молчит,
 *       кодеки сводятся к общему API
 *
 */
TEST(CodecTomlDocument, ZeroMeansNoLimitAndNestingIsRefusedByItsOwnSign){
	{
		// Объект дерева настроек, текст разбирающего
		toml::document_t document(::framework(), ::logger());
		// Получаем настройки дерева настроек
		toml::document_t::settings_t settings = document.settings();
		// Снимаем предел вложенности значений нулём
		settings.reader.maxDepth = 0;
		// Выполняем установку настроек дерева настроек
		document.settings(settings);
		/**
		 * Выполняем проверку того, что нуль предел СНИМАЕТ
		 *
		 * @note Прежде тот же текст отвергался: нуль запрещал вложенность вовсе
		 */
		ASSERT_TRUE(document.parse("a = [[[[1]]]]\n")) << toml::message(document.error());
		/**
		 * Выполняем проверку того, что снятый нулём предел потолком всё же огранен
		 *
		 * @note Ноль огранивается наравне с числом завышенным: дерево без предела срывает
		 *       стек, а отказ приходит с местом в тексте
		 */
		{
			// Собираемый текст настроек с глубокой вложенностью перечней
			string deep = "b = ";
			...
			deep.append(4096, ']');
			// Выполняем проверку отказа разбора по превышению глубины вложенности
			ASSERT_FALSE(document.parse(deep));
			// Выполняем проверку того, что отказано именно по глубине вложенности
			ASSERT_EQ(document.error(), toml::error_t::DEPTH_EXCEEDED);
		}
	}
	{
		...
		// Получаем настройки дерева настроек
		toml::document_t::settings_t settings = document.settings();
		// Запрещаем вложенные значения признаком дозволения
		settings.reader.nesting = false;
		// Выполняем установку настроек дерева настроек
		document.settings(settings);
		// Выполняем проверку того, что перечень значений отвергается запретом
		ASSERT_FALSE(document.parse("a = [1]\n"));
		// Выполняем проверку того, что отказано именно по глубине вложенности
		ASSERT_EQ(document.error(), toml::error_t::DEPTH_EXCEEDED);
		// Выполняем проверку того, что встроенная таблица отвергается тем же запретом
		ASSERT_FALSE(document.parse("a = { b = 1 }\n"));
		/**
		 * Выполняем проверку того, что значения простые запретом принимаются
		 *
		 * @note Половина эта важнее первой: запрет, отвергающий всё подряд, прошёл бы обе
		 *       половины выше, а смысл его именно в том, чтобы простое оставить
		 */
		ASSERT_TRUE(document.parse("a = 1\nb = \"текст\"\n")) << toml::message(document.error());
	}
}
...
TEST(CodecTomlDocument, FailedSaveKeepsThePreviousContent){
	...
	const string filename(::unique("/tmp/awh-toml-failed-save.txt"));
	...
	toml::document_t document(::framework(), & watching);
	// Выполняем разбор текста, предел размера заведомо превосходящего
	{
		 ...
		 */
		for(size_t i = 0; i < 20000; i++)
			// Выполняем добавление очередной пары настроек
			bulky.append("k").append(std::to_string(i)).append(" = \"значение\"\n");
		// Выполняем разбор собранного текста
		ASSERT_TRUE(document.parse(bulky)) << toml::message(document.error());
	}
	/**
 ...
 *
 * @details Снятие отменяющих записей живёт у нескольких разборов пути, и пропуск его хотя
 * бы у одного означает, что правка заводит пару НОВУЮ вместо правки существующей: молча и
 * правдоподобно. У этого кодека пропущены были ДВА хода - перенос значения и снятие его
 *
 * @warning Сличается ЧИСЛО детей корня, а не одно лишь значение по пути: правка, заведшая
 *          пару новую, отдала бы по пути верное значение и прошла бы проверку, сличающую
 *          только его
 *
 * @note Имена здесь в кавычках намеренно: голый ключ ограничен знаками ASCII по описанию
 *       языка, и ни знак отмены, ни косая черта в него не входят
 *
 */
TEST(CodecTomlDocument, EditingByAnEscapedTokenReachesThatVeryField){
	// Объект дерева настроек, текст разбирающего
	toml::document_t document(::framework(), ::logger());
	// Выполняем разбор текста настроек с именами, отменяющих записей требующими
	...
	ASSERT_EQ(document.size(), static_cast <size_t> (3));
	// Выполняем правку пары по записанному звену знака отмены
	ASSERT_TRUE(document.set("/~0", toml::value_t(static_cast <int64_t> (10)))) << toml::message(document.error());
	// Число, путём разысканное
	int64_t number = 0;
	// Выполняем извлечение значения по записанному звену
	ASSERT_TRUE(document.at("/~0").value(number));
	// Выполняем проверку того, что правка легла по записанному звену
	ASSERT_EQ(number, static_cast <int64_t> (10));
	/**
	 * Выполняем проверку того, что пары НОВОЙ правка не завела
	 *
	 * @note Половина эта и есть весь порок: прежде тут заводилась пара, буквально
	 *       названная `~0`, а пара `~` оставалась нетронутой
	 */
	ASSERT_EQ(document.size(), static_cast <size_t> (3));
	// Выполняем правку пары по записанному звену косой черты
	ASSERT_TRUE(document.set("/a~1b", toml::value_t(static_cast <int64_t> (20)))) << toml::message(document.error());
	// Выполняем извлечение значения по записанному звену косой черты
	ASSERT_TRUE(document.at("/a~1b").value(number));
	// Выполняем проверку того, что правка легла по записанному звену
	ASSERT_EQ(number, static_cast <int64_t> (20));
	// Выполняем проверку того, что пары новой и эта правка не завела
	ASSERT_EQ(document.size(), static_cast <size_t> (3));
	// Выполняем снятие пары по записанному звену
	ASSERT_TRUE(document.erase("/~0")) << toml::message(document.error());
	// Выполняем проверку того, что снята именно та пара
	ASSERT_FALSE(document.present("/~0"));
	// Выполняем проверку того, что детей корня стало на одного меньше
	ASSERT_EQ(document.size(), static_cast <size_t> (2));
	// Выполняем проверку того, что соседняя пара снятием не задета
	ASSERT_TRUE(document.present("/a~1b"));
}
 ...
 *
 * @details Записи отменяющих у пути ровно две - `~0` знаком отмены да `~1` косой
 *          чертой, - и всякая иная отвергается, а не берётся дословно: имя `~x`,
 *          принятое как есть, стало бы ВТОРЫМ написанием того же места, и обход
 *          перестал бы быть замкнутым - `keys()` выдаёт всякую тильду записанной
 *
 * @warning Отвергать обязаны ОБА хода - и дерева, и владеющего значения: путь
 *          разбирается у них порознь, и поверка, поставленная на один лишь ход,
 *          второй пропустила бы молча
 *
 * @note Пробел этот открыло объединённое покрытие набора с ворошителем: успешные
 *       записи `~0` и `~1` проверены были обе, а два отказа снятия - тильда
...
TEST(CodecTomlDocument, MalformedEscapeInThePathIsRefused){
	 ...
	 */
	const vector <string> broken = {"/a~", "/~", "/a~x", "/~9", "/a~b/c", "/обычное~"};
	/**
	 * Выполняем перебор всех негодных звеньев пути
	 */
	for(auto & path : broken){
		// Выполняем проверку того, что розыск по негодному звену отвергнут
		ASSERT_FALSE(document.present(path)) << path;
	 ...
	 *
	 * @note Половина эта стережёт ширину отказа: поверка, тильду отвергающая
	 *       без разбора, прошла бы перебор выше целиком
	 */
	ASSERT_TRUE(document.present("/~0"));
	ASSERT_TRUE(document.present("/a~1b"));
	// Владеющее значение, тот же текст разбирающее
	toml::value_t value;
	// Выполняем разбор текста настроек владеющим значением
	ASSERT_TRUE(value.parse("\"~\" = 1\n\"a/b\" = 2\n\"обычное\" = 3\n")) << toml::message(value.error());
	/**
	...
	// Выполняем проверку того, что годные записи отказом не задеты и здесь
	ASSERT_TRUE(value.at("/~0").valid());
	ASSERT_TRUE(value.at("/a~1b").valid());
	/**
	 * Выполняем перебор всех негодных звеньев пути у обрезки и переноса
	 *
	 * @warning Ходы эти разбирают путь СВОИМ снятием, а не тем, каким разбирают его
	 *          `at()` с `present()`: те двое идут снятием владеющего значения, и
	 *          поверка, ими одними ведомая, снятие дерева не задевала вовсе.
	 *          Доказано порчею: отказ, у дерева снятый, перебора выше не валит
	 */
	for(auto & path : broken){
		...
		ASSERT_EQ(document.error(), toml::error_t::INVALID_KEY) << path;
		// Выполняем проверку того, что перенос по негодному звену отвергнут
		ASSERT_FALSE(document.graft(path, toml::value_t(static_cast <int64_t> (7)))) << path;
	 ...
	 *
	 * @note Половина эта стережёт целость: обрезка, до отказа успевшая снять пару,
...
/**
 * @brief Проверка замкнутости обхода дерева на наборе одноимённых таблиц
 *
 * @details Набор таблиц есть место, где обход мог бы РАЗОМКНУТЬСЯ молча: выдай дерево
 *          двух одноимённых потомков - и одно звено имени вело бы лишь к одной из
 *          таблиц, а вторая стала бы недостижимой ничем. Проверка закрепляет, что имя
 *          ведёт к НАБОРУ одним потомком, а внутрь набора ведут звенья числовые
 *
 * @note Своей грамматики звена наречию отсюда не требуется, хотя повтор имени таблицы
 *       законен по стандарту: звено остаётся именем, а различает таблицы номер
 *
 * @note Половина вторая закрепляет ЗАКОННОСТЬ пустого ответа перечня дочерних имён на
 *       наборе таблиц: ход этот обещает имена детей ТАБЛИЦЫ, а набор таблиц таблицею не
 *       является - числом таблиц ведает свой ход `count()`. Пустота эта потому не есть
 *       порок, и чинить её нечего; общий же вид перечня звеньев, когда имя это
 *       освободится, обязан будет выдавать здесь звенья ЧИСЛОВЫЕ
 */
TEST(CodecTomlDocument, ArrayOfTablesKeepsTraversalClosed){
	// Объект дерева документа, текст разбирающего
	toml::document_t document(::framework(), ::logger());
	// Выполняем разбор текста документа с набором одноимённых таблиц
	ASSERT_TRUE(document.parse(
		"[[fruit]]\n"
		"name = \"apple\"\n"
		"\n"
	...
	// Перечень дочерних имён корня дерева документа
	const vector <string_view> keys = document.keys();
	/**
	 * Выполняем проверку того, что набор таблиц выдан именем ОДНАЖДЫ
	 *
	 * @note Выдача имени дважды была бы ложью: потомок там один - сам набор
	 */
	ASSERT_EQ(keys.size(), static_cast <size_t> (1));
	// Выполняем проверку выданного имени набора таблиц
	ASSERT_EQ(keys.front(), "fruit");
	// Выполняем проверку количества таблиц набора
	ASSERT_EQ(document.count(vector <string_view> {"fruit"}), static_cast <size_t> (2));
	// Выполняем проверку количества таблиц набора, разысканного путём
	ASSERT_EQ(document.at("/fruit").size(), static_cast <size_t> (2));
	// Выполняем проверку достижимости первой таблицы набора числовым звеном
	ASSERT_EQ(document.at("/fruit/0/name").text(), "apple");
	// Выполняем проверку достижимости второй таблицы набора числовым звеном
	ASSERT_EQ(document.at("/fruit/1/name").text(), "pear");
	/**
	 * Выполняем проверку законной пустоты перечня дочерних имён на наборе таблиц
	 */
	ASSERT_TRUE(document.keys(vector <string_view> {"fruit"}).empty());
	/**
	 * Выполняем проверку того, что у самой таблицы набора имена детей выдаются
	 *
	 * @note Половина эта отделяет пустоту ЗАКОННУЮ от немоты: спроси перечень таблицу, а
	 *       не набор - и имена он выдаёт
	 */
	const vector <string_view> inner = document.keys(vector <string_view> {"fruit", "0"});
	// Выполняем проверку количества дочерних имён первой таблицы набора
	ASSERT_EQ(inner.size(), static_cast <size_t> (1));
	// Выполняем проверку выданного дочернего имени первой таблицы набора
	ASSERT_EQ(inner.front(), "name");
	/**
	 * Выполняем проверку того, что путь несуществующий выдаёт перечень пустой
	 *
	 * @details Обход обязан на таком пути МОЛЧАТЬ, а не выдавать детей чего-то иного:
	 *          перечень, путь потерявший да отдавший чужих детей, увёл бы обходящего в
	 *          другую ветвь дерева, и заметить подмену было бы нечем
	 *
	 * @warning Проверка эта поверяет ПОВЕДЕНИЕ, а не заход отсечения: сам заход есть
	 *          СОКРАЩЕНИЕ, и порча его проверки не валит - значение негодное ни
	 *          таблицею, ни перечнем не является, и перечень звеньев выходит пустым и
	 *          без него. Доказано порчею на копии дерева и щупом по трём путям.
	 *          Записано затем, чтобы читающий не принял одно за другое
	 */
	for(auto & path : {"/нет-такого", "/fruit/9", "/fruit/0/нет", "/fruit/0/name/глубже"}){
		// Выполняем проверку того, что перечень детей по такому пути пуст
		ASSERT_TRUE(document.keys(string(path)).empty()) << path;
	}
	/**
	 * Выполняем проверку того, что путь существующий отказом не задет
	 *
	 * @note Половина эта стережёт ширину: перечень, всякий путь теряющий, прошёл бы
	 *       перебор выше целиком
	 */
	ASSERT_FALSE(document.keys(string("/fruit/0")).empty());
}

/**
 * @brief Проверка полного обхода документа общим перечнем звеньев и розыском
 *
 ...
 *          их в дереве. Замкнутость держится отменяющими записями RFC 6901 - имя, косую
 *          черту либо тильду несущее, выдаётся записанным
 *
 * @note Ход поставлен ПЕРЕГРУЗКОЙ рядом с прежним перечнем дочерних имён таблицы:
 *       половина вторая проверки закрепляет, что прежний ход правкою не подменён и
 *       обещание своё несёт по-прежнему
 */
TEST(CodecTomlDocument, EnumerationAndLookupFormAClosedTraversal){
	// Объект дерева документа, текст разбирающего
	toml::document_t document(::framework(), ::logger());
	// Выполняем разбор текста документа с таблицами, перечнями и трудными именами
	ASSERT_TRUE(document.parse(
		"\"простое\" = 1\n"
		"\"перечень\" = [ \"раз\", \"два\" ]\n"
		"\"имя/с/чертой\" = 4\n"
	 ...
	 *
	 * @note Счёт берётся с корнем: шесть пар корня, два значения перечня, одна пара
	 *       вложенной таблицы, две таблицы набора с парою в каждой да сам корень -
	 *       четырнадцать
	 */
	ASSERT_EQ(walked, static_cast <size_t> (14));
	...
	ASSERT_EQ(keys.size(), static_cast <size_t> (6));
	/**
	 ...
	 */
	ASSERT_EQ(keys.at(2), "имя~1с~1чертой");
	...
	/**
	 * Выполняем проверку того, что прежний ход перечня имён правкою не подменён
	 *
	 * @note Ходы стоят перегрузкою и несут РАЗНЫЕ обещания: прежний берёт составное имя
	 *       перечнем кусков и выдаёт одни лишь имена детей таблицы, ничего не записывая
	 */
	const vector <string_view> named = document.keys(vector <string_view> {"вложенное"});
	// Выполняем проверку количества дочерних имён вложенной таблицы
	ASSERT_EQ(named.size(), static_cast <size_t> (1));
	// Выполняем проверку выданного дочернего имени вложенной таблицы
	ASSERT_EQ(named.front(), "внутри");
	// Выполняем проверку того, что прежний ход имя записанным НЕ выдаёт
	ASSERT_EQ(document.keys().at(2), "имя/с/чертой");
}

/**
 * @brief Проверка установки пустого значения по пути
 *
 * @details Пустое значение от снятия узла ОТЛИЧНО: пара остаётся налицо, а значения при
 *          ней нет. Ход этот общий у кодеков рамки, и здесь его недоставало одному
 *          наречию из трёх - найдено сличением общего договора по кодекам, а не отказом
 *
 * @note Пустотою наречию служит пустая СТРОКА: своего вида пустоты описание TOML не знает
 *       вовсе, и всякая пара обязана нести значение
 */
TEST(CodecTomlDocument, SettingEmptyValueByPath){
	// Объект дерева документа, текст разбирающего
	toml::document_t document(::framework(), ::logger());
	// Выполняем разбор текста документа с таблицей
	ASSERT_TRUE(document.parse("a = 1\n[srv]\nhost = \"anyks\"\n")) << toml::message(document.error());
	// Выполняем установку пустого значения по пути
	ASSERT_TRUE(document.reset("/a")) << toml::message(document.error());
	/**
	 * Выполняем проверку того, что пара осталась налицо
	 *
	 * @note Половина эта и отделяет пустое значение от снятия узла: снятый узел спрос
	 *       наличия не прошёл бы вовсе
	 */
	ASSERT_TRUE(document.has("/a"));
	// Выполняем проверку того, что значения при паре нет
	ASSERT_TRUE(document.at("/a").text().empty());
	// Выполняем проверку того, что счёт детей корня не изменился
	ASSERT_EQ(document.size(), static_cast <size_t> (2));
	// Выполняем установку пустого значения по пути внутри таблицы
	ASSERT_TRUE(document.reset("/srv/host")) << toml::message(document.error());
	// Выполняем проверку того, что пара таблицы осталась налицо
	ASSERT_TRUE(document.has("/srv/host"));
	// Выполняем проверку того, что значения при паре таблицы нет
	ASSERT_TRUE(document.at("/srv/host").text().empty());
	/**
	 * Выполняем проверку того, что записанное читается обратно пустою строкой
	 */
	toml::document_t back(::framework(), ::logger());
	// Выполняем разбор перезаписи правленого дерева
	ASSERT_TRUE(back.parse(document.dump())) << toml::message(back.error());
	// Выполняем проверку того, что пара пережила круг записи и чтения
	ASSERT_TRUE(back.has("/srv/host"));
	// Выполняем проверку того, что значение прочтено пустою строкой
	ASSERT_TRUE(back.at("/srv/host").text().empty());
	/**
	 * Выполняем проверку заведения недостающих звеньев пути
	 *
	 * @warning Наречие это звенья по дороге ЗАВОДИТ, тогда как YAML и INI их налицо
	 *          требуют и отвечают отказом. Расхождение это подлинное, замерено одним
	 *          вызовом по трём кодекам, и молчанием оно не закрывается: касается оно
	 *          общего договора кодеков, а не одного наречия, и вынесено владельцу
	 *
	 * @note Проверка закрепляет ЗДЕШНЮЮ правду, а не желаемую: перепиши поведение по
	 *       соседям без решения владельца - и потребитель, на заведении стоящий, получил
	 *       бы отказ молча
	 */
	ASSERT_TRUE(document.reset("/нет/внутри")) << toml::message(document.error());
	// Выполняем проверку достижимости заведённого пути
	ASSERT_TRUE(document.has("/нет/внутри"));
	// Выполняем проверку того, что значение при заведённой паре пусто
	ASSERT_TRUE(document.at("/нет/внутри").text().empty());
}

/**
 * @brief Проверка снятия значения перечня по числовому звену пути
 *
 * @details Звено числовое принимается сносом наравне с именем: перечень значений и набор
 *          одноимённых таблиц суть у наречия перечни, и перечень звеньев выдаёт к их
 *          детям ЧИСЛА. Не прими снос номера - обход был бы замкнут для чтения и разомкнут
 *          для правки: звено, деревом выданное и для розыска годное, сносу не поддавалось
 *          бы вовсе
 *
 * @note Порок найден сличением одного вызова по трём кодекам: YAML и INI звено числовое
 *       сносили, а наречие это отвечало отказом. Ворошителю он недоступен: тот сносит
 *       пути, деревом выданные, но исход сноса судит по ответу, а ответ был честным
 *       отказом
 */
TEST(CodecTomlDocument, RemovalByNumericLink){
	// Объект дерева документа, текст разбирающего
	toml::document_t document(::framework(), ::logger());
	// Выполняем разбор текста документа с перечнем и набором одноимённых таблиц
	ASSERT_TRUE(document.parse(
		"\"список\" = [ \"раз\", \"два\", \"три\" ]\n"
		"[[\"набор\"]]\n"
		"\"имя\" = \"первая\"\n"
	...
	ASSERT_TRUE(document.erase(string("/список/1"))) << toml::message(document.error());
	// Выполняем проверку того, что у перечня осталось два значения
	ASSERT_EQ(document.at("/список").size(), static_cast <size_t> (2));
	// Выполняем проверку того, что снято именно названное значение
	ASSERT_EQ(document.at("/список/1").text(), "три");
	// Выполняем проверку того, что соседнее значение перечня осталось на месте
	ASSERT_EQ(document.at("/список/0").text(), "раз");
	// Выполняем проверку отказа снятия по номеру за краем перечня
	ASSERT_FALSE(document.erase(string("/список/9")));
	/**
	 * Выполняем проверку снятия таблицы набора по числовому звену
	 */
	ASSERT_TRUE(document.erase(string("/набор/0"))) << toml::message(document.error());
	// Выполняем проверку того, что у набора осталась одна таблица
	ASSERT_EQ(document.at("/набор").size(), static_cast <size_t> (1));
	// Выполняем проверку того, что осталась именно вторая таблица набора
	ASSERT_EQ(document.at("/набор/0/имя").text(), "вторая");
	/**
	 * Выполняем проверку замкнутости правки: всякое выданное звено сносу поддаётся
	 */
	for(const string & key : document.keys(string("/список")))
		// Выполняем проверку наличия значения по выданному звену
		ASSERT_TRUE(document.has("/список/" + key)) << key;
	// Выполняем проверку сноса по последнему выданному звену перечня
	ASSERT_TRUE(document.erase("/список/" + document.keys(string("/список")).back())) << toml::message(document.error());
}
 ...
 *          ради одного числа. Замерено: спрос размера на файле в двадцать пять килобайт
 *          стоил 314 микросекунд против 0.26 у самой правки значения - в тысячу раз
 *          дороже правки. Ныне счёт ведётся перечнем дочерних имён корня
 *
 * @warning Проверка сторожит СМЫСЛ счёта, а не его скорость: скорость меряется набором
 *          замеров, а здесь закрепляется то, что дешёвый счёт даёт ровно те же числа на
 *          построениях, где счёты могли бы разойтись, - набор одноимённых таблиц есть
 *          ОДИН ребёнок корня, а не несколько
 *
 * @note Нашёл это разбор замера, а не сам замер: сценарий правки дерева зовёт размер на
 *       каждом круге, и порог его не проходился никогда - мерился не ход правки, а спрос
 *       размера. Порог, ни разу не проходившийся, читается по диагонали ровно так же,
 *       как проходящийся всегда
 */
TEST(CodecTomlDocument, SizeCountsChildrenWithoutBuildingTheTree){
	...
	for(auto & one : cases){
		...
		ASSERT_TRUE(document.parse(one.first)) << one.first << ": " << toml::message(document.error());
		// Выполняем проверку счёта детей корня дерева документа
		ASSERT_EQ(document.size(), one.second) << one.first;
		/**
		 * Выполняем проверку того, что счёт сходится со снятым значением
		 *
		 * @note Половина эта и сторожит смысл: дешёвый счёт обязан давать ровно то же,
		 *       что даёт снятие владеющего значения, - иначе он дешевле, но неверен
		 */
		ASSERT_EQ(document.size(), toml::value_t(document).size()) << one.first;
	}
}
 ...
 *
 */
TEST(CodecTomlDocument, EveryFailedSaveIsAnnounced){
	// Объект фреймворка проверок
	awh::fmk_t framework;
	// Объект журнала, отчёты собирающий
	awh::log_t watching(& framework);
	 ...
	 */
	const string folder = ::unique("./цель-записи-каталог-toml");
	/**
	...
	const string missing = "./нет-такого-каталога-toml/цель.toml";
	/**
...
TEST(CodecTomlDocument, LoggerSetAfterBuildingIsUsed){
	...
	toml::document_t broken(::framework(), nullptr);
 ...
 *
 */
TEST(CodecTomlDocument, HugeIndexInThePathIsRefusedNotFatal){
	// Объект дерева настроек, текст разбирающего
	toml::document_t document(::framework(), ::logger());
	// Выполняем разбор текста с перечнем одноимённых объявлений
	ASSERT_TRUE(document.parse("[[fruit]]\nname = \"a\"\n[[fruit]]\nname = \"b\"\n")) << toml::message(document.error());
	...
	const vector <string> huge = {
		"/fruit/99999999999999999999999999",
		"/fruit/18446744073709551616",
		"/fruit/999999999999999999999999999999999999999999"
	};
	/**
	...
	for(auto & path : huge){
		...
		ASSERT_FALSE(document.graft(path, toml::value_t(string("x")))) << path;
...
TEST(CodecTomlDocument, ForcedEncodingIsObeyed) {
	 ...
	 */
	{
		...
		// Собираемое дерево настроек
		toml::document_t document(::framework(), ::logger(), settings);
	 ...
	 */
	{
		...
		{
			// Собираемое дерево настроек умолчальных
			toml::document_t bare(::framework(), ::logger());
...
TEST(CodecTomlDocument, WritingSettingsReachTheWriter) {
	 ...
	 */
	{
		 ...
		 */
		{
			...
			// Собираемое дерево настроек
			toml::document_t document(::framework(), ::logger(), unbounded);
			// Выполняем разбор текста настроек
			ASSERT_TRUE(document.parse("a = 1\n")) << toml::message(document.error());
			// Выполняем постановку значения, строку записи удлиняющего
			ASSERT_TRUE(document.set("/имя", toml::value_t("значение подлиннее"))) << toml::message(document.error());
 ...
 *
 */
TEST(CodecTomlDocument, LanguageKindsExtractTheNumber){
	...
	ASSERT_TRUE(document.value(widest, {"server", "port"}));
	// Выполняем проверку извлечённого числа
	...
	ASSERT_TRUE(document.value(unsigned_, {"server", "port"}));
	// Выполняем проверку извлечённого числа
	ASSERT_EQ(unsigned_, 8080UL);
}

/**
 * @brief Проверка отказа извлечения числа видом ЯЗЫКА
 *
 * @details Отказ извлечения у видов языка идёт своею дорогою: извлечение ведётся видом
 ...
 *       извлечённого идёт лишь при успехе, и прежнее содержимое приёмника отказ
 *       оставляет нетронутым
 *
 */
TEST(CodecTomlDocument, LanguageKindsRefuseTheNonNumber){
	// Выполняем создание объекта документа
	toml::document_t document(::framework(), ::logger());
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse(SAMPLE)) << static_cast <uint32_t> (document.error());
	...
	long wide = 1250;
	// Выполняем проверку отказа извлечения числа видом языка со знаком
	ASSERT_FALSE(document.value(wide, {"server", "host"}));
	// Выполняем проверку сохранности прежнего содержимого приёмника
	ASSERT_EQ(wide, 1250);
	// Приёмник вида размера с прежним содержимым
	size_t size = 77;
	// Выполняем проверку отказа извлечения числа видом размера
	ASSERT_FALSE(document.value(size, {"server", "host"}));
	// Выполняем проверку сохранности прежнего содержимого приёмника
 ...
 *
 */
TEST(CodecTomlDocument, DirectoryIsRefusedNotLoaded){
	// Выполняем создание объекта документа
	toml::document_t document(::framework(), ::logger());
 ...
 *
 * @note Замерено 09.09.2026 на стенде MSYS2 при ACP=1251: до правки запись «настройки.toml»
 *       заводила на диске «РЅР°СЃС‚СЂРѕР№РєРё.toml», задуманного имени не было вовсе, а
 ...
 *
 */
TEST(CodecTomlDocument, TheCyrillicFileNameLandsOnTheDiskAsIntended) {
	// Собираемое дерево настроек
	toml::document_t document(::framework(), ::logger());
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse("[\"раздел\"]\n\"ключ\" = \"значение\"\n"));
	// Путь записи с кириллическим именем
	const string filename = string(::testing::TempDir()) + ::unique("настройки-кириллица.toml");
	// Выполняем запись дерева настроек в файл
	ASSERT_TRUE(document.save(filename)) << toml::message(document.error());
	/**
	 * Для операционной системы MS Windows
	 */
	#if defined(_WIN32) || defined(_WIN64)
	...
	ASSERT_TRUE(back.load(filename)) << toml::message(back.error());
	// Выполняем проверку того, что содержимое пережило круг
	ASSERT_TRUE(back.has({"раздел", "ключ"}));
