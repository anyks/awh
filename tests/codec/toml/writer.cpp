...
TEST(CodecTomlWriter, LocaleNumbers) {
	 ...
	 *
	 * @note То же и у OpenBSD, и это ЗАМЕРЕНО 08.09.2026 на стенде 10.100.1.145
	 *       (OpenBSD 7.9 amd64): перечень локалей там богат - 68 имён, `de_DE.UTF-8`
	 *       среди них есть, и `setlocale` его ПРИНИМАЕТ, - но `localeconv()` неизменно
	 *       отдаёт десятичным знаком точку, а `printf("%.1f", 0.1)` пишет «0.1».
	 *       Разряд `LC_NUMERIC` там не заведён, и проверка пропускается всегда
	 *
	 * @warning Пропуск этот однажды был прочтён ОТКАЗОМ и передан к кодеку как
	 *          падение проверки. Счёт стенда гласил «2129 из 2131», и разница в два
	 *          читалась провалом, тогда как отчёт говорил `SKIPPED 2`, `FAILED 0`, а
	 *          исход прогона был нулевым. Пропуск проверкою НЕ ПРОЙДЕН, но и НЕ ПРОВАЛЕН:
	 *          вычитать его из общего числа наравне с отказом нельзя
	 */
	if(checked == 0)
 ...
 *
 * @details Договор этот у трёх кодеков сведён к одному общим решением: отказ записи
 ...
 *
 * @note Довод «о написанном нельзя судить по настройке» принесён от кодека
...
/**
 * @brief Проверка того, что отказы записи называют СВОЮ причину
 *
 * @details Записывающий ход, позванный не в свой черёд, отвечает отказом - и код его
 * уходит потребителю причиною. Значение вне пары, закрытие без открытия, имя дважды
 * подряд: причины эти разные, и подмена любой из них увела бы потребителя править не то
 *
 * @note Заведено щупом слепоты: всякое место `refuse(error_t::X)` подменялось порознь на
 *       причину иную, и печатались места, которых набор не заметил. Места эти числились
 *       было недостижимыми - помета стояла у СОСЕДНЕГО захода выше, а разбор брал её по
 *       близости. Соседство - не принадлежность, и все девять берутся простым вызовом
 *
 */
TEST(CodecTomlWriter, RefusalsNameTheirOwnCause) {
	/**
	 * Выполняем проверку отказа записи значения вне пары
	 *
	 * @note Значение в тексте настроек всегда при имени: описание корнем текста полагает
	 *       перечень пар, и голого значения в нём не бывает вовсе
	 */
	{
	...
	/**
	 * Выполняем проверку отказа закрытия построения, не открытого вовсе
	 *
	 * @note Причина здесь иная, чем у значения вне пары: закрытие лишнее означает не
	 *       содержимое не в том месте, а построение, счёт которому разошёлся
	 */
	{
		// Объект потоковой записи текста настроек
		toml::writer_t writer;
	...
	/**
	 * Выполняем проверку отказа записи имени дважды подряд
	 *
	 * @note Причина здесь третья: имя, значения не дождавшееся, есть пара незавершённая
	 */
	{
		// Объект потоковой записи текста настроек
		toml::writer_t writer;
...
/**
 * @brief Проверка записи числа написаниями видов языка
 *
 * @details Порождение приёма записи числа велось видами заданной РАЗРЯДНОСТИ, а `size_t`
 * у macOS arm64 есть `unsigned long`, тогда как `uint64_t` есть `unsigned long long`:
 * виды это разные, и записи `number(size_t(9))` с `number(long(5))` не СВЯЗЫВАЛИСЬ вовсе.
 * Порождение переведено на виды языка, и проверка эта стережёт перечень порождения
 *
 * @note Проверка эта стережёт сборкою: вид, из перечня выпавший, даёт отказ связывания, и
 *       набор не собирается вовсе. Оттого важен не столько сличаемый текст, сколько сам
 *       перебор написаний
 *
 * @warning Голый `char` сюда не входит намеренно: приём зовётся записью ЧИСЛА, и знак, в
 *          него попавший, вышел бы числом 122 молча. Отказ сборки для знака оставлен
 *
 */
TEST(CodecTomlWriter, EveryLanguageIntegerRecordIsWritten) {
	...
	ASSERT_TRUE(writer.number("b", static_cast <long> (5)));
	// Выполняем запись числа написанием короткого целого
	ASSERT_TRUE(writer.number("c", static_cast <short> (3)));
	// Выполняем запись числа написанием целого простого
	ASSERT_TRUE(writer.number("d", 7));
	// Выполняем запись числа написанием знакового однобайтового
	ASSERT_TRUE(writer.number("e", static_cast <signed char> (-128)));
	// Выполняем запись числа написанием беззнакового однобайтового
	...
	ASSERT_EQ(writer.text(), "a = 9\nb = 5\nc = 3\nd = 7\ne = -128\nf = 255\ng = 4294967295\n"
		"h = 65535\ni = -9007199254740993\nj = 9007199254740993\n");
}

/**
 * @brief Проверка того, что первопричина отказа не подменяется следствием
 *
 * @details Спрос собранного текста поверяет готовность записи, и та писала код
 * `MISSING_VALUE` БЕЗУСЛОВНО. Отвергнутое значение оставляет запись в состоянии «имя
 * есть, значения нет», и оттого всякий спрос текста писал поверх причины её же
 * ПОСЛЕДСТВИЕ: потребитель шёл искать пропущенное значение вместо негодной кодировки.
 * Замерено: до спроса текста код 4, после спроса 16
 *
 */
TEST(CodecTomlWriter, TheFirstCauseIsNotOverwrittenByItsConsequence) {
	...
	ASSERT_FALSE(writer.text(string("\x80", 1)));
	// Выполняем проверку того, что причина отказа названа поимённо
	ASSERT_EQ(writer.error(), toml::error_t::INVALID_ENCODING) << toml::message(writer.error());
	// Выполняем проверку того, что собранный текст пуст
	ASSERT_TRUE(writer.text().empty());
	/**
	 * @note Потеря причины и подмена её - беды разной тяжести. Потеря видна: код `NONE` при
	 *       отказе противоречив сам по себе. Подмена не видна вовсе - код правдоподобен
	 */
	// Выполняем проверку того, что первопричина спрос текста ПЕРЕЖИЛА
	ASSERT_EQ(writer.error(), toml::error_t::INVALID_ENCODING) << toml::message(writer.error());
}

/**
 * @brief Проверка того, что незакрытое окружение называет свою породу
 *
 * @details Запись, оставленная с незакрытым перечнем либо встроенной таблицей, обязана
 *          назвать причиною ИМЕННО незакрытость, а не изъян кодека: скобку забыл
 *          зовущий, и починка тут в его руках
 *
 * @warning Прежде код выбирался двоичным выбором «имя без значения либо INTERNAL», и
 *          оба незакрытых окружения получали «internal parser error». Обвинение это
 *          ложно и дорого: потребитель шлёт донесение о пороке рамки вместо того, чтобы
 *          закрыть скобку. Достижимость замерена пометкою самой ветви - ворошитель брал
 *          её 1364 раза на трёх тысячах текстов, то есть «невозможное» состояние было
 *          обычным
 *
 * @note Третья половина проверки стережёт обратное: имя без значения обязано и впредь
 *       отвечать `MISSING_VALUE`. Без неё правка, все три случая свёдшая к одному коду,
 *       проверку бы не уронила
 *
 */
TEST(CodecTomlWriter, UnclosedContextNamesItsKind) {
	// Настройки записи текста настроек
	toml::writer_t::settings_t settings;
	/**
	 * Выполняем проверку отказа по незакрытому перечню значений
	 */
	{
		// Объект записи текста настроек
		toml::writer_t writer(settings);
		...
		// Выполняем проверку того, что собранный текст пуст
		ASSERT_TRUE(writer.text().empty());
		// Выполняем проверку того, что причина названа незакрытостью перечня
		ASSERT_EQ(writer.error(), toml::error_t::UNCLOSED_ARRAY);
	}
	/**
	 * Выполняем проверку отказа по незакрытой встроенной таблице
	 */
	{
		// Объект записи текста настроек
		toml::writer_t writer(settings);
		// Выполняем запись имени пары
		ASSERT_TRUE(writer.key("ключ"));
		// Выполняем открытие встроенной таблицы
		ASSERT_TRUE(writer.inlineOpen());
		// Выполняем проверку того, что собранный текст пуст
		ASSERT_TRUE(writer.text().empty());
		// Выполняем проверку того, что причина названа незакрытостью таблицы
		ASSERT_EQ(writer.error(), toml::error_t::UNCLOSED_INLINE_TABLE);
	}
	/**
	 * Выполняем проверку отказа по имени, значения не получившему
	 */
	{
		// Объект записи текста настроек
		toml::writer_t writer(settings);
		...
		// Выполняем проверку того, что собранный текст пуст
		ASSERT_TRUE(writer.text().empty());
		// Выполняем проверку того, что причина названа пропущенным значением
		ASSERT_EQ(writer.error(), toml::error_t::MISSING_VALUE);
	}
}
