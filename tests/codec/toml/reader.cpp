 ...
 *
 */
TEST(CodecTomlReader, MalformedSequenceOutcomeIsIndependentOfTheChunking) {
	/**
	...
	for(const string & sequence : {
		...
		for(const string & text : {"k = \"a" + sequence + "b\"\n", "k = \"a" + sequence}){
			...
			ASSERT_NE(whole, static_cast <uint8_t> (toml::error_t::NONE)) << text.size();
			/**
...
TEST(CodecTomlReader, ChunkIndependence) {
	...
	for(auto & text : texts){
		 ...
		 *
		 * @note Перебор ведётся до ДЛИНЫ текста, а не до семи: замер 17.09.2026 щупом по
		 *       всем размерам куска показал, что прежний предел оставлял непроверенными
		 *       все границы, дальше седьмого октета лежащие, - а там у этих текстов стоят
		 *       тройная ограда, отметка времени да разделитель разрядов. Сосед YAML
		 *       перебирал до длины текста с самого начала
		 */
 ...
 *       довод от соседних кодеков, проверенный замером и принятый
 *
 ...
 *
 * @note Довод принесён от кодеков JSON и XML, где виды хранятся указаниями в
 *       своё хранилище и копия перебазирует их сама собою. Здесь устройство иное, и
 *       ручательство обратное. Правка договора вынесена за пределы кодека
 *
 */
 ...
 *
 * @note Пробел открыт обездвиживанием настроек: настройка эта - ЕДИНСТВЕННАЯ из девяти
 *       логических у кодека, обездвиживание которой набор не заметил вовсе. Та же
 *       настройка и с тем же итогом молчала и у кодека INI
 *
 */
TEST(CodecTomlReader, CommentEmissionIsSwitchable){
	...
	for(const bool emit : {true, false}){
		 ...
		 */
		ASSERT_EQ(events, static_cast <size_t> (emit ? 4 : 3)) << emit;
