 ...
 *
 * @note Заведено 09.09.2026: починка записи дробного была приложена к
...
/**
 * @brief Проверка непредставимости имени ключа расширения знаком равенства
 *
 * @details Пары расширения разделяются пробелом, а имя ключа отделяется от значения
 * знаком равенства, и отмены ни того, ни другого имя ключа не знает. Ключ, знак такой
 * несущий, записанным быть не может: повторный разбор такой записи разделил бы его на
 * две пары либо взял бы за имя огрызок
 *
 * @note Найдено щупом 16.09.2026: писатель ставил знаку равенства отмену - «a\=b=3», -
 *       а читатель отмены в ИМЕНИ не признаёт вовсе и брал за имя «a\». Кодек писал то,
 *       чего сам не читает. Цена правки замерена ворошителем: 18 переписей из 12 417,
 *       и все восемнадцать несли запись, означавшую иное
 *
 * @warning Проверка закрепляет ОБЕ половины договора: отказ имени с равенством и
 *          прежнюю запись имени с косою чертой, каковая отменяется и оборот проходит
 *          дословно
 *
 */
TEST(CodecCefWriter, ExtensionKeyCannotHoldEquals){
	 ...
	 */
	auto build = [](const string & key, string & result) noexcept -> bool {
		// Дерево события записи CEF
		abc::value_t root(abc::kind_t::MAP);
		// Заводим заголовок записи
		root.place("/header") = abc::value_t(abc::kind_t::MAP);
		// Ставим номер редакции записи
		root.place("/header/version") = abc::value_t(static_cast <uint64_t> (0));
		// Ставим изготовителя записи
		root.place("/header/vendor") = abc::value_t(string("A"));
		// Ставим изделие записи
		root.place("/header/product") = abc::value_t(string("B"));
		// Ставим выпуск изделия записи
		root.place("/header/release") = abc::value_t(string("C"));
		// Ставим опознаватель события записи
		root.place("/header/signature") = abc::value_t(string("D"));
		// Ставим имя события записи
		root.place("/header/name") = abc::value_t(string("E"));
		// Ставим важность события записи
		root.place("/header/severity") = abc::value_t(static_cast <uint64_t> (1));
		/**
		 * Заводим расширение записи
		 *
		 * @warning Ключ ставится ИМЕНЕМ через `insert`, а НЕ ходом `place`: тот
		 *          разбирает довод строковым путём
		 */
		abc::value_t extension(abc::kind_t::MAP);
		// Ставим пару расширения именем ключа
		if(!extension.insert(key, abc::value_t(string("3"))))
			// Выводим отказ сборки записи
			return false;
		// Ставим расширение в дерево события
		root.place("/extension") = extension;
		// Объект записи событий
		cef::writer_t writer;
	...
	// Собранная запись CEF
	string result = "";
	// Выполняем проверку того, что законное имя ключа записывается по-прежнему
	EXPECT_TRUE(build("src", result));
	// Обнуляем собранную запись CEF
	result.clear();
	/**
	 * Выполняем проверку того, что имя с косою чертой записывается по-прежнему
	 *
	 * @note Половина эта договора важнее первой: косая черта в имени отменяется, и
	 *       оборот её проходит дословно - отказ ей был бы правкою сверх нужного
	 */
	ASSERT_TRUE(build("a\\b", result));
	// Выполняем проверку того, что косая черта в имени отменена
	EXPECT_NE(result.find("a\\\\b="), string::npos) << "собрано: " << result;
	// Обнуляем собранную запись CEF
	result.clear();
	// Выполняем проверку отказа записи именем ключа со знаком равенства
	EXPECT_FALSE(build("a=b", result)) << "собрано: " << result;
	// Обнуляем собранную запись CEF
	result.clear();
	// Выполняем проверку отказа записи именем ключа с пробелом
	EXPECT_FALSE(build("a b", result)) << "собрано: " << result;
}

/**
 * @brief Проверка непредставимости приставки журнала словом заголовка
 *
 * @details Заголовок записи отыскивается ПЕРВЫМ словом «CEF:», и отмены приставка не
 * знает вовсе - вид её задаёт описание syslog, а не CEF. Приставка, слово это несущая,
 * записанной быть не может: повторный разбор взял бы за начало заголовка её середину,
 * а за приставку - огрызок перед нею
 *
 * @note Найдено щупом 16.09.2026: приставка «prefix with CEF: inside» уходила в запись
 *       беспрепятственно, а своё же чтение отвечало ей отказом - разбор брал за номер
 *       редакции слово «inside»
 *
 * @note Ворошителем цена этой правки НУЛЕВАЯ, и тому есть причина: чтение такого дерева
 *       породить не может вовсе - запись с такою приставкой не разбирается, - и правка
 *       бережёт дерево, потребителем собранное. Замерено эталоном: 12417 переписей до
 *       правки и 12417 после неё, тогда как соседняя правка имени ключа стоила 18
 *
 * @warning Проверка закрепляет ОБЕ половины договора: отказ приставке со словом
 *          заголовка и прежнюю запись приставок обычных - и RFC 5424, и RFC 3164, и
...
TEST(CodecCefWriter, SyslogPrefixCannotHoldTheSignature){
	 ...
	 *
	 * @param prefix приставка системного журнала
	 * @param result собранная запись CEF
	 * @return       признак успешности сборки записи
	 */
	auto build = [](const string & prefix, string & result) noexcept -> bool {
		// Дерево события записи CEF
		abc::value_t root(abc::kind_t::MAP);
		// Ставим приставку системного журнала в дерево события
		root.place("/syslog") = abc::value_t(prefix);
		// Заводим заголовок записи
		root.place("/header") = abc::value_t(abc::kind_t::MAP);
		// Ставим номер редакции записи
		...
		// Настройки записи событий
		cef::writer_t::settings_t settings;
		// Устанавливаем запись приставки системного журнала
	...
	string result = "";
	/**
	 * Выполняем проверку того, что приставка RFC 5424 записывается по-прежнему
	 *
	 * @note Половина эта договора важнее первой: приставка есть обычай живых журналов,
	 *       и отказ ей сломал бы записи настоящих устройств
	 */
	ASSERT_TRUE(build("<165>1 2026-09-15T00:00:00Z host app - -", result));
	// Объект документа записи CEF для поверки оборота
	cef::document_t document;
	// Выполняем проверку того, что собранная запись читается обратно
	ASSERT_TRUE(document.parse(result)) << "собрано: " << result;
	// Выполняем проверку приставки, оборот прошедшей
	EXPECT_EQ(document.at("/syslog").text(), "<165>1 2026-09-15T00:00:00Z host app - -");
	// Обнуляем собранную запись CEF
	result.clear();
	// Выполняем проверку того, что приставка RFC 3164 записывается по-прежнему
	EXPECT_TRUE(build("Oct 22 12:34:56 myhost myapp:", result));
	// Обнуляем собранную запись CEF
	result.clear();
	/**
	 * Выполняем проверку того, что приставка с прямыми чертами записывается по-прежнему
	 *
	 * @note Прямая черта разделяет поля ЗАГОЛОВКА, а приставка стоит прежде него, и
	 *       черта в ней законна
	 */
	EXPECT_TRUE(build("a|b|c", result));
	// Обнуляем собранную запись CEF
	result.clear();
	// Выполняем проверку отказа приставке, слово заголовка внутри несущей
	EXPECT_FALSE(build("prefix with CEF: inside", result)) << "собрано: " << result;
	// Обнуляем собранную запись CEF
	result.clear();
