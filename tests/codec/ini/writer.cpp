...
/**
 * @brief Проверка записи нулевого байта управляющей последовательностью
 *
 * @details Нулевой байт в значении записывается как «\0» и обратным чтением возвращается
 * в содержимое как есть. Строку эту пересечение трёх прогонов числило слепой: перебор
 * управляющих знаков нулевой байт не спрашивал
 *
 */
TEST(CodecIniWriter, NullByteEscaped) {
	// Настройки записи текста настроек
	ini::writer_t::settings_t settings;
	// Устанавливаем запись управляющих последовательностей в значении
	settings.escapes = true;
	...
	// Выполняем запись объявления раздела
	ASSERT_TRUE(writer.section("a"));
	// Выполняем запись свойства со значением, нулевой байт несущим
	ASSERT_TRUE(writer.property("k", string("до\0после", 15)));
	// Выполняем проверку того, что нулевой байт записан управляющей последовательностью
	ASSERT_EQ(writer.text(), "[a]\nk = до\\0после\n");
}
/**
 * @brief Проверка записи примечания из нескольких строк
 *
 * @details Примечание, знаки конца строки несущее, записывается строкой на строку -
 * каждая со своим знаком начала примечания: иначе вторая и следующие ушли бы из
 * примечания вовсе и прочлись бы записями настроек. Возврат каретки при этом
 * отбрасывается: содержимое размечено как есть, а разметку строк ставит настройка
 *
 * @note Строки эти пересечение трёх прогонов числило слепыми - примечания подавались
 *       односрочными, а разметка CRLF в них не встречалась вовсе
 *
 */
TEST(CodecIniWriter, MultilineComment) {
	/**
	 * Выполняем проверку записи примечания из двух строк
	 */
	{
		// Объект записи текста настроек
		ini::writer_t writer;
		// Выполняем запись объявления раздела
		ASSERT_TRUE(writer.section("a"));
		// Выполняем запись примечания из двух строк
		ASSERT_TRUE(writer.comment("первая\nвторая"));
		// Выполняем проверку того, что каждая строка примечания получила свой знак
		ASSERT_EQ(writer.text(), "[a]\n; первая\n; вторая\n");
	}
	/**
	 * Выполняем проверку отбрасывания возврата каретки у строк примечания
	 */
	{
		// Объект записи текста настроек
		ini::writer_t writer;
		// Выполняем запись объявления раздела
		ASSERT_TRUE(writer.section("a"));
		// Выполняем запись примечания, размеченного возвратом каретки с переводом строки
		ASSERT_TRUE(writer.comment("первая\r\nвторая"));
		/**
		 * Выполняем проверку того, что возврат каретки в содержимое не попал
		 *
		 * @note Без отбрасывания строка несла бы возврат каретки перед знаком конца
		 *       строки, поставленным настройкой, и разметка вышла бы двойною
		 */
		ASSERT_EQ(writer.text(), "[a]\n; первая\n; вторая\n");
	}
}
/**
 * @brief Проверка отказа дописки примечания к записи, ничего не собравшей
 *
 * @details Примечание дописывается к строке уже записанной, и у записи пустой дописывать
 * его некуда. Отказ этот есть ошибка потребителя, а не изъян записи
 *
 * @warning Прежде отвечал он кодом INTERNAL, и это было неверно вдвойне: INTERNAL
 *          означает изъян самой записи, а тут ничего не сломано; сверх того TOML и YAML
 *          на том же входе отвечают UNEXPECTED_CONTENT. Замерено щупом на трёх кодеках
 *          разом, сведено решением о единообразии кодеков
 *
 * @note Настройка примечаний в строке обязательна: без неё дописка уходит примечанием
 *       отдельной строкой и отказа не даёт вовсе
 *
 */
TEST(CodecIniWriter, TrailingRefusedOnEmptyWriter) {
	// Настройки записи текста настроек
	ini::writer_t::settings_t settings;
	// Устанавливаем признание примечаний в конце строки
	settings.inlineComments = true;
	// Объект записи текста настроек
	ini::writer_t writer(settings);
	// Выполняем проверку отказа дописки примечания к записи, ничего не собравшей
	ASSERT_FALSE(writer.trailing("хвост"));
	// Выполняем проверку того, что код отказа отвечает содержимому не на своём месте
	ASSERT_EQ(writer.error(), ini::error_t::UNEXPECTED_CONTENT);
	/**
	 * Выполняем проверку того, что без настройки примечаний в строке отказа нет
	 *
	 * @note Заход этот стоит рядом нарочно: отказ, приходящий на всякую дописку, отвечал
	 *       бы тому же ожиданию, и порча условия осталась бы незамечена
	 */
	{
		// Объект записи текста настроек
		ini::writer_t plain;
		// Выполняем проверку того, что дописка уходит примечанием отдельной строкой
		ASSERT_TRUE(plain.trailing("хвост"));
		// Выполняем проверку собранного текста настроек
		ASSERT_EQ(plain.text(), "; хвост\n");
	}
}
/**
 ...
 * между записью строки и допиской к ней, и суд по настройке нынешней снимал бы не ту
 * последовательность знаков
 *
 * @warning Замерено щупом до правки, и обе стороны были плохи. Смена CRLF на LF: дописка
 *          снимала один перевод строки из двух знаков и оставляла одинокий возврат
 *          каретки ПОСРЕДИ строки - `k = v[CR] ; хвост[LF]`, - отвечая при том УСПЕХОМ.
 *          Смена LF на CRLF: отказ кодом INTERNAL, то есть изъян записи там, где его нет
 *
 * @note Довод «о написанном нельзя судить по настройке» принесён от кодека
 *       JSON, где та же беда склеивала документы потока NDJSON
 *
 */
TEST(CodecIniWriter, TrailingJudgesWrittenNotSettings) {
	/**
	 * Выполняем проверку смены разметки строк с CRLF на LF
	 */
	{
		// Настройки записи текста настроек
		ini::writer_t::settings_t settings;
		...
		settings.newline = ini::newline_t::LF;
		 ...
		 *
		 * @note Без правки текст выходил `k = v[CR] ; хвост[LF]` - возврат каретки стоял
		 *       посреди строки, и разбор такого текста расходился с записью
		 */
		ASSERT_EQ(writer.text(), string("[a]\r\nk = v ; хвост\n"));
	}
	/**
	 * Выполняем проверку смены разметки строк с LF на CRLF
	 */
	{
		// Настройки записи текста настроек
		ini::writer_t::settings_t settings;
		// Устанавливаем признание примечаний в конце строки
		settings.inlineComments = true;
		// Устанавливаем разметку строк переводом строки
		settings.newline = ini::newline_t::LF;
		// Объект записи текста настроек
		ini::writer_t writer(settings);
		// Выполняем запись объявления раздела
		...
		// Выполняем запись свойства со значением
		ASSERT_TRUE(writer.property("k", "v"));
		// Выполняем смену разметки строк на возврат каретки с переводом
		settings.newline = ini::newline_t::CRLF;
		// Выполняем установку сменённых настроек записи
		writer.settings(settings);
		/**
		 * Выполняем дописку примечания к записанной строке
		 *
		 * @note Без правки здесь приходил отказ кодом INTERNAL: снимаемая
		 *       последовательность бралась из настроек нынешних, а текст был записан
		 *       прежними, и совпадения не находилось
		 */
		ASSERT_TRUE(writer.trailing("хвост"));
		// Выполняем проверку собранного текста настроек
		ASSERT_EQ(writer.text(), string("[a]\nk = v ; хвост\r\n"));
...
TEST(CodecIniWriter, BracketedKeyWithoutArrays) {
	...
	ini::document_t collected(arrayed);
...
TEST(CodecIniWriter, RefusalSticksToWriter) {
	...
	const ini::error_t error = writer.error();
	/**
	 * Выполняем перебор всех методов записи открытого договора
	 *
	 * @warning Перебираются ВСЕ они, а не выборка: сторож стоит в каждом теле по
	 *          отдельности, и метод, сторожа не получивший, выпал бы из договора молча.
	 *          Заведётся у записи новый метод - приписать его сюда
	 */
	/**
	 ...
	 *
	 * @note Снятие всех шести сторожей роняет восемь утверждений из девяти: методов
	 *       больше, нежели сторожей, оттого что часть их поручает работу соседям.
	 *       Замерено срывом
	 */
	EXPECT_FALSE(writer.section("иной"));
	EXPECT_FALSE(writer.section("иной", "подраздел"));
	EXPECT_FALSE(writer.property("второй"));
	EXPECT_FALSE(writer.property("второй", "значение"));
	EXPECT_FALSE(writer.property("второй", "значение", true));
	...
	ASSERT_TRUE(writer.section("server") && writer.property("ключ", "значение"));
}

/**
 * @brief Проверка записи имён разделов со скобками при жадном розыске
 *
 * @details Читающий жадный берёт имя раздела до ПОСЛЕДНЕЙ скобки в строке: «[x[]]» есть
 * ему имя «x[]». Запись, скобки отвергавшая безусловно, рвала круг «чтение - запись» на
 * наречии python: разбор текст принимал, а запись дерево отвергала пустотой
 *
 * @note Проверены имена со скобкою на всяком месте, первое включая: догадка о
 *       недопустимости открывающей скобки первым знаком снята замером - «[a]»
 *       записывается объявлением «[[a]]» и читается обратно знак в знак
 *
 * @note Проверка эта доказана порчею: возврат прежнего безусловного запрета скобок в
 *       `Writer::acceptable` роняет её отказом записи на первом же имени
 */
TEST(CodecIniWriter, GreedySectionNamesRoundTrip){
	/**
	 * Проверяемые имена разделов со скобками
	 */
	const vector <string> names = {"x[]", "[a", "[a]", "a]", "]a[", "a[b"};
	/**
	 * Выполняем перебор всех проверяемых имён разделов
	 */
	for(const string & name : names){
		// Настройки дерева настроек, наречию python отвечающие
		ini::document_t::settings_t settings;
		// Устанавливаем настройки разбора наречия python
		settings.reader = ini::reader_t::settings_t::python();
		...
		ASSERT_TRUE(document.parse("")) << name;
		// Выполняем объявление раздела с проверяемым именем
		ASSERT_TRUE(document.create(name)) << name << ": " << ini::message(document.error());
		// Выполняем установку свойства объявленного раздела
		ASSERT_TRUE(document.set("k", "v", name)) << name;
		// Получаем записанный текст настроек наречием python
		const string text = document.text(ini::writer_t::settings_t::python());
		...
		// Выполняем разбор записанного текста настроек
		ASSERT_TRUE(back.parse(text)) << name << ": " << text;
		// Выполняем проверку того, что раздел прочитан с тем же именем
		ASSERT_TRUE(back.has("k", name)) << name << ": " << text;
		// Выполняем проверку значения прочитанного свойства
		ASSERT_EQ(back.get("k", name), "v") << name << ": " << text;
	}
}
/**
 * @brief Проверка отказа записи имени свойства со знаком разделителя читающего
 *
 * @details Пишется разделитель один - знак `separator`, - а признавать читающий вправе
 * оба: наречие языка Python числит разделителем и равенство, и двоеточие. Имя свойства
 * `:elais`, записью принятое, доставалось такому читающему с именем пустым, и круг
 * «чтение - запись - чтение» рвался: разбор текст принимал, запись его принимала, а
 * обратное чтение отвергало пустотою имени
 *
 * @note Нашёл это ворошитель переводом дерева настроек из наречия в наречие
 */
TEST(CodecIniWriter, ColonInNameRefusedForPythonDialect){
	// Настройки разбора текста настроек с разделителем одним
	ini::document_t::settings_t reading;
	...
	ini::document_t document(reading);
	...
	ASSERT_TRUE(document.has(":elais", "b"));
	/**
	 * Выполняем проверку отказа перевода дерева в наречие языка Python
	 *
	 * @note Отказ здесь есть верное поведение: имя это наречием невыразимо, а
	 *       записанное как есть читалось бы обратно пустым
	 */
	ASSERT_TRUE(document.text(ini::writer_t::settings_t::python()).empty());
	/**
	 * Выполняем проверку перевода дерева в наречие MS Windows
	 *
	 * @note Наречие это разделителем признаёт одно равенство, и двоеточие в имени ему
	 *       не мешает: отвергать такое имя не за что
	 */
	const string translated = document.text(ini::writer_t::settings_t::windows());
	// Выполняем проверку того, что перевод состоялся
	ASSERT_FALSE(translated.empty()) << ini::message(document.error());
	// Настройки обратного чтения перевода наречием MS Windows
	ini::document_t::settings_t back_settings;
	...
	ini::document_t back(back_settings);
	...
	ASSERT_TRUE(back.has(":elais", "b")) << translated;
}
/**
 * @brief Проверка отказа записи имени свойства с меткой порядка байтов в начале текста
 *
 * @details Метка порядка байтов, в начале текста стоящая, есть метка, а не знак
 * содержимого: читающий снимает её прежде разбора. Имя свойства, ею начатое и
 * записанное ПЕРВЫМ, теряет её при обратном чтении - свойство возвращается с именем
 * короче на знак. Оградить её нечем: имена управляющими последовательностями не
 * записываются, а кавычками имя свойства не защищается
 *
 * @note Отказ ставится по пустоте собранного текста, а не по имени одному: то же имя,
 *       записанное за разделом либо за свойством другим, круг проходит знак в знак -
 *       метка там знаком содержимого и остаётся. Отвергать его всюду значило бы
 *       отвергать годное, и проверка закрепляет обе стороны правила
 */
TEST(CodecIniWriter, LeadingByteOrderMarkInNameRefusedFirstOnly){
	// Имя свойства, меткою порядка байтов открытое
	const string name("\xEF\xBB\xBF" "n");
	/**
	 * Выполняем проверку отказа записи имени первым свойством текста
	 */
	{
	...
	/**
	 * Выполняем проверку круга, когда имя записывается не первым
	 */
	{
		// Объект дерева настроек, значение принимающего
		ini::document_t document;
		...
		/**
		 * Выполняем проверку того, что имя пережило круг знак в знак
		 *
		 * @note Метка, не первою в тексте стоящая, знаком содержимого и остаётся
		 */
		ASSERT_TRUE(back.has(name)) << written;
	}
}
/**
 * @brief Проверка знака примечания в имени раздела и в имени свойства
 *
 * @details Требование к знаку примечания НЕСИММЕТРИЧНО. У имени свойства он значащ
 * лишь тогда, когда стоит первым либо отделён пробельным знаком: имя `k#v` наречие Git
 * читает целиком. У имени же раздела он обрывает объявление где угодно: `[a#]` то же
 * наречие отвергает отказом незакрытого раздела. Запись требовала пробела в обоих
 * случаях, и имя раздела со знаком примечания внутри проходило записью беспрепятственно
 *
 * @note Круг «чтение - запись - чтение» на этом и рвался: текст `[a#]`, прочтённый
 *       наречием без примечаний в конце строки, записывался как есть, а перевод его в
 *       наречие Git давал текст, тем же наречием уже не читаемый. Нашёл это ворошитель
 *       переводом дерева настроек из наречия в наречие
 */
TEST(CodecIniWriter, CommentMarkerInSectionNameRefusedAnywhere){
	// Настройки разбора наречия MS Windows, примечаний в конце строки не знающего
	ini::document_t::settings_t reading;
	// Устанавливаем настройки разбора наречия MS Windows
	reading.reader = ini::reader_t::settings_t::windows();
	// Объект дерева настроек, текст разбирающего
	...
	ASSERT_TRUE(document.has("k", "a#"));
	/**
	 * Выполняем проверку отказа перевода дерева в наречие Git
	 *
	 * @note Отказ здесь есть верное поведение: имя это наречием невыразимо, а
	 *       записанное как есть обращало бы объявление в незакрытое
	 */
	ASSERT_TRUE(document.text(ini::writer_t::settings_t::git()).empty());
	/**
	 * Выполняем проверку того, что имя СВОЙСТВА со знаком примечания отвергается тоже
	 *
	 * @note Пробела перед знаком читающий не требует: имя `k#v` наречие Git читает
	 *       именем `k`, а `#v = 1` берёт примечанием - значение пропадало бы вовсе
	 */
	{
		// Объект дерева настроек, значение принимающего
		ini::document_t keyed(reading);
		// Выполняем разбор текста со знаком примечания внутри имени свойства
		ASSERT_TRUE(keyed.parse("[s]\r\nk#v=1\r\n")) << ini::message(keyed.error());
		// Выполняем проверку того, что имя свойства прочитано целиком
		ASSERT_TRUE(keyed.has("k#v", "s"));
		// Выполняем проверку отказа перевода дерева в наречие Git
		ASSERT_TRUE(keyed.text(ini::writer_t::settings_t::git()).empty());
	}
	/**
	 * Выполняем проверку того, что наречием без примечаний в конце строки имя годно
	 *
	 * @note Признак `inlineComments` тем и решает: наречие systemd знак примечания
	 *       внутри имени значащим не числит, и круг проходит знак в знак
	 */
	{
		// Объект дерева настроек, значение принимающего
		ini::document_t keyed(reading);
		...
		const string translated = keyed.text(ini::writer_t::settings_t::systemd());
		// Выполняем проверку того, что перевод состоялся
		ASSERT_FALSE(translated.empty()) << ini::message(keyed.error());
		// Настройки обратного чтения перевода наречием systemd
		ini::document_t::settings_t back_settings;
		...
		ASSERT_TRUE(back.parse(translated)) << translated << ": " << ini::message(back.error());
		// Выполняем проверку того, что имя свойства пережило круг знак в знак
		ASSERT_TRUE(back.has("k#v", "s")) << translated;
	}
}
/**
 * @brief Проверка зеркала строгой грамматики имён у записи
 *
 * @details Наречие Git строит имя раздела из букв, цифр, черты да точки, а имя свойства
 * из буквы первым знаком да букв, цифр и черты далее. Читающий, грамматику поверяющий,
 * имена иные отвергает - и запись обязана отвергать их наравне с ним
 *
 * @note Зеркала этой настройки у записи не было вовсе: имя `[раздел]` кириллицей
 *       записывалось как есть, а обратное чтение тем же наречием отвергало его отказом
 *       построения объявления. Дерево выдавало текст, какого своё же чтение не
 *       принимает, и отказа при записи не было - порок молчаливый
 *
 * @note Нашёл это ворошитель, как только набор его научился перебирать саму настройку:
 *       прежде она держалась снятой при всяком проходе, а снятую настройку проверить
 *       нечем
 */
TEST(CodecIniWriter, StrictNamesAreMirroredFromTheReader) {
	/**
	 * Выполняем проверку отказа записи имени раздела, грамматике противного
	 */
	{
		// Настройки записи текста настроек
		ini::writer_t::settings_t settings;
		// Устанавливаем поверку имён строгою грамматикой наречия
		settings.strictNames = true;
		// Объект записи текста настроек
		ini::writer_t writer(settings);
		// Выполняем проверку отказа записи имени раздела кириллицей
		ASSERT_FALSE(writer.section("\xd1\x80\xd0\xb0\xd0\xb7\xd0\xb4\xd0\xb5\xd0\xbb"));
		// Выполняем проверку выданного кода отказа записи
		ASSERT_EQ(writer.error(), ini::error_t::INVALID_SECTION);
	}
	/**
	 * Выполняем проверку отказа записи имени свойства, грамматике противного
	 */
	{
		/**
		 * Проверяемые имена свойств, грамматике наречия противные
		 */
		for(auto & name : {string("1k"), string("k_2"), string("k.2"), string("\xd0\xba\xd0\xbb\xd1\x8e\xd1\x87")}){
			// Настройки записи текста настроек
			ini::writer_t::settings_t settings;
			// Устанавливаем поверку имён строгою грамматикой наречия
			settings.strictNames = true;
			...
			ASSERT_TRUE(writer.section("s"));
			// Выполняем проверку отказа записи имени свойства
			ASSERT_FALSE(writer.property(name, "1")) << name;
			// Выполняем проверку выданного кода отказа записи
			ASSERT_EQ(writer.error(), ini::error_t::INVALID_KEY) << name;
		}
	}
	/**
	 * Выполняем проверку того, что имена, грамматике отвечающие, записываются
	 */
	{
		// Настройки записи текста настроек
		ini::writer_t::settings_t settings;
		...
		ini::writer_t writer(settings);
		// Выполняем запись имени раздела
		ASSERT_TRUE(writer.section("core.sub"));
		// Выполняем запись свойства раздела
		ASSERT_TRUE(writer.property("key-1", "value"));
		// Выполняем проверку собранного текста настроек
		ASSERT_EQ(writer.text(), "[core.sub]\nkey-1 = value\n");
	}
	/**
	 * Выполняем проверку того, что дерево зеркалит настройку сама
	 *
	 * @note Дерево берёт настройки записи у настроек разбора: без зеркала оно выдавало бы
	 *       текст, своим же чтением отвергаемый
	 */
	{
		// Настройки дерева настроек
		ini::document_t::settings_t settings;
		...
		/**
		 * Выполняем проверку того, что перезапись пуста
		 *
		 * @note Пустая перезапись означает отказ записи: имя наречием невыразимо, и отказ
		 *       здесь верен, тогда как текст, чтением отвергаемый, есть порок
		 */
...
/**
 * @brief Проверка знака разделителя, записи назначенного настройкою
 *
 * @details Знак разделителя имени и значения задаётся настройкою и стоит в записи двумя
 * местами: он уходит в текст между именем и значением, и он же запретен внутри самого
 * имени - иначе читающий разрезал бы имя по нему и прочёл половину. Обездвиживание
 * настройки умолчанием набора не роняло вовсе: записывался один лишь знак равенства
 *
 * @note Проверяются ОБЕ стороны правила: знак, разделителем назначенный, имя отвергает, а
 *       знак, разделителем НЕ являющийся, в имени дозволен. Одной стороны мало: запрет,
 *       на знак равенства прибитый намертво, прошёл бы её незамеченным
 *
 */
TEST(CodecIniWriter, SeparatorSettingIsObeyed) {
	/**
	 * Проверяемые знаки разделителя вместе с ожидаемой записью
	 */
	const vector <pair <char, string>> samples = {
		// Разделитель умолчальный
		{'=', "[раздел]\nимя = значение\n"},
		// Разделитель двоеточием
		{':', "[раздел]\nимя : значение\n"},
		// Разделитель знаком, наречиям не свойственным
		{'|', "[раздел]\nимя | значение\n"}
	};
	/**
	 * Выполняем перебор всех проверяемых знаков разделителя
	 */
	for(const auto & item : samples){
		// Настройки записи текста
		ini::writer_t::settings_t settings;
		// Устанавливаем знак, разделяющий имя свойства и его значение
		settings.separator = item.first;
		// Объект потоковой записи текста
		ini::writer_t writer(settings);
		// Выполняем запись объявления раздела
		ASSERT_TRUE(writer.section("раздел")) << item.first;
		// Выполняем запись свойства раздела
		ASSERT_TRUE(writer.property("имя", string("значение"))) << item.first;
		// Выполняем проверку собранной записи текста
		ASSERT_EQ(writer.text(), item.second) << item.first;
	}
	/**
	 * Выполняем проверку запрета разделителя внутри имени свойства
	 */
	{
		// Настройки записи текста
		ini::writer_t::settings_t settings;
		...
		// Выполняем запись объявления раздела
		ASSERT_TRUE(writer.section("раздел"));
		// Выполняем проверку отказа записи имени, разделитель несущего
		ASSERT_FALSE(writer.property("и:мя", string("значение")));
		// Выполняем проверку выданного кода отказа записи
		ASSERT_EQ(writer.error(), ini::error_t::INVALID_KEY);
	}
	/**
	 * Выполняем проверку дозволенности знака, разделителем НЕ являющегося
	 *
	 * @note Проверка эта и отделяет настройку от запрета, знаку равенства прибитого:
	 *       знак равенства при разделителе-двоеточии имени вредить не может
	 */
	{
		// Настройки записи текста
		ini::writer_t::settings_t settings;
		// Устанавливаем знак, разделяющий имя свойства и его значение
		settings.separator = ':';
		// Устанавливаем признание обоих знаков разделителя запретными
		settings.separators = ini::separator_t::COLON;
		// Объект потоковой записи текста
		ini::writer_t writer(settings);
		// Выполняем запись объявления раздела
		ASSERT_TRUE(writer.section("раздел"));
		// Выполняем проверку принятия имени со знаком равенства внутри
		ASSERT_TRUE(writer.property("и=мя", string("значение"))) << ini::message(writer.error());
		// Выполняем проверку собранной записи текста
		ASSERT_EQ(writer.text(), "[раздел]\nи=мя : значение\n") << writer.text();
	}
}
 ...
 *
 */
TEST(CodecIniWriter, EveryLanguageIntegerRecordIsWritten) {
	// Объект записи текста настроек
	ini::writer_t writer;
	// Выполняем запись объявления раздела
	ASSERT_TRUE(writer.section("s"));
