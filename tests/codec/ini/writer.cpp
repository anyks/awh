...
TEST(CodecIniWriter, CommentMarkerInName) {
	...
	options.reader.inlineComments = true;
	{
		// Объект дерева настроек
		ini::document_t document(options);
		// Выполняем установку свойства со знаком примечания в имени
		ASSERT_TRUE(document.set("k ; заметка", "значение", "раздел"));
		// Выполняем проверку того, что запись отвечает отказом
		ASSERT_TRUE(document.text().empty());
	}
	{
		// Объект дерева настроек
		ini::document_t document(options);
		// Выполняем установку свойства в разделе со знаком примечания
		ASSERT_TRUE(document.set("k", "значение", "раздел ; заметка"));
		// Выполняем проверку того, что запись отвечает отказом
		ASSERT_TRUE(document.text().empty());
	}
	{
