 ...
 *
 * @warning Поля sensitiveSections тут НЕТ: ряд событий разбора имена разделов выдаёт
 *          как записаны при обеих настройках, и текст «[Section] … [section]» даёт
 *          слепок знак в знак один и тот же. Дерево настроек их тоже не сличает - оба
 *          раздела остаются раздельными, - и розыск свойства именем иного регистра
 *          отвечает отказом при обеих. Чем поле это наблюдается, я не установил;
 *          заходом его НЕ заводить ни деревом, ни событиями - вынесено владельцу
 *
 */
