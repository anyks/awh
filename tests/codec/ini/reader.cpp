...
TEST(CodecIniReader, Chunked) {
	...
	for(size_t i = 0; i < (sizeof(texts) / sizeof(texts[0])); i++){
		 ...
		 *       все границы, дальше седьмого октета лежащие, - а именно там стоят у этих
		 *       текстов продолжение строки, возврат каретки да отступ перечня. Сосед YAML
		 *       перебирал до длины текста с самого начала
		 */
		for(size_t step = 1; step <= texts[i].size(); step++)
 ...
 *
 */
TEST(CodecIniReader, MalformedSequenceOutcomeIsIndependentOfTheChunking) {
	/**
	...
	for(const string & sequence : {
		...
		for(const string & text : {"[s]\nk = a" + sequence + "b\n", "[s]\nk = a" + sequence}){
			...
			ASSERT_NE(whole, static_cast <uint8_t> (ini::error_t::NONE)) << text.size();
			/**
 ...
 *
 * @note Порода находки указана по кодекам JSON, XML и CSV: у разметки XML стояла
