 ...
 * Имена снимаются лишь на время реализации - возврат в конце файла
 */
...
TEST(CodecIniReader, Subsection) {
	...
	settings.maxDepth = 1;
	/**
	 * Выполняем проверку того, что предел в единицу один уровень ДОПУСКАЕТ
	 *
	 * @note Граница включающая: прежде она была исключающей, и предел в единицу значил
	...
	// Снимаем предел глубины вложенности подразделов нулём
	settings.maxDepth = 0;
	/**
	 * Выполняем проверку того, что ноль предел СНИМАЕТ
	 *
	 * @note Уклад этот один у всех кодеков рамки: прежде ноль подразделы запрещал, и
	 *       настройка, перенесённая с иного кодека, срабатывала наоборот и молча
	 */
	ASSERT_EQ(::dump("[a.b.c]\nk=v\n", settings), "S[a|b.c]\nP[k=v]\n");
	// Выполняем запрет подразделов признаком, а не пределом
	settings.nesting = false;
	// Выполняем проверку отклонения подраздела запретом
	ASSERT_EQ(::dump("[a.b]\nk=v\n", settings), "E[subsection depth exceeded]@1:1\n");
}
/**
...
/**
 * @brief Проверка отклонения записи знака кодовым значением, Юникоду не принадлежащим
 *
 * @details Застава эта есть ЕДИНСТВЕННАЯ защита от суррогата и от значения за пределом
 *          Юникода: разбор записи разряды складывает, ничего сверх шестнадцатеричности
 *          не поверяя, и отвергает такое значение лишь запись знака в UTF-8. Достижимость
 *          заставы доказана печатью 07.09.2026, покрытия же она не имела вовсе
 *
 */
TEST(CodecIniReader, EscapesOutsideTheUnicode) {
	// Собираемые настройки разбора текста настроек
	ini::reader_t::settings_t settings;
	// Устанавливаем разбор управляющих последовательностей в значении
	settings.escapes = true;
	// Выполняем проверку отклонения записи суррогатной парою
	ASSERT_EQ(::dump("[a]\nk=\\uD800\n", settings), "S[a|]\nE[invalid escape sequence]@2:3\n");
	// Выполняем проверку отклонения записи суррогата полным кодовым значением
	ASSERT_EQ(::dump("[a]\nk=\\U0000DFFF\n", settings), "S[a|]\nE[invalid escape sequence]@2:3\n");
	// Выполняем проверку отклонения значения свыше предела Юникода
	ASSERT_EQ(::dump("[a]\nk=\\U00110000\n", settings), "S[a|]\nE[invalid escape sequence]@2:3\n");
	// Выполняем проверку принятия значения предельного, Юникоду принадлежащего
	ASSERT_EQ(::dump("[a]\nk=\\U0010FFFF\n", settings), "S[a|]\nP[k=\xF4\x8F\xBF\xBF]\n");
}
/**
 * @brief Проверка имени раздела с точкой и подразделом в кавычках разом
 *
 * @details Запись «[a.b "origin"]» несёт оба построения имени подраздела разом, и
 * средство `git config` выдаёт её ключом «a.b.origin.имя»: точечная цепь и имя в
 * кавычках складываются в одно имя подраздела. Разбор резал по точке первым и клал в
 * имя подраздела «b "origin"» вместе с кавычками - имя выходило небывалым. Нашло это
 * сличение с эталоном на текстах, под наречие Git писанных
 *
 */
TEST(CodecIniReader, DottedSectionWithQuotedSubsection) {
	// Собираемые настройки разбора текста настроек
	ini::reader_t::settings_t settings = ini::reader_t::settings_t::git();
	// Выполняем проверку разбора точечной цепи вместе с именем в кавычках
	ASSERT_EQ(::dump("[a.b \"origin\"]\n\tname = 1\n", settings), "S[a|b.origin]\nP[name=1]\n");
	// Выполняем проверку разбора имени подраздела одними кавычками
	ASSERT_EQ(::dump("[core \"origin\"]\n\tname = 1\n", settings), "S[core|origin]\nP[name=1]\n");
	// Выполняем проверку разбора имени подраздела одной точечной цепью
	ASSERT_EQ(::dump("[a.b.c]\n\tname = 1\n", settings), "S[a|b.c]\nP[name=1]\n");
	// Выполняем проверку разбора точки внутри имени, кавычками взятого
	ASSERT_EQ(::dump("[core \"a.b\"]\n\tname = 1\n", settings), "S[core|a.b]\nP[name=1]\n");
}
/**
 * @brief Проверка усечённой записи знака кодовым значением
 *
 * @details Запись знака кодовым значением требует ровно двух, четырёх либо восьми
 * разрядов за указателем, и запись, оборванная концом значения, годной не является.
 * Строку эту пересечение трёх прогонов числило слепой: ни набор проверок, ни корпус
 * наречий, ни двадцать тысяч проходов ворошителя усечённой записи не подавали
 *
 */
TEST(CodecIniReader, TruncatedCodepointEscape) {
	// Собираемые настройки разбора текста настроек
	ini::reader_t::settings_t settings;
	// Устанавливаем разбор управляющих последовательностей в значении
	settings.escapes = true;
	// Выполняем проверку отклонения усечённой записи шестнадцатеричным кодовым значением
	ASSERT_EQ(::dump("[a]\nk=x\\x1\n", settings), "S[a|]\nE[invalid escape sequence]@2:4\n");
	// Выполняем проверку отклонения усечённой записи кодовым значением Юникода
	ASSERT_EQ(::dump("[a]\nk=x\\u12\n", settings), "S[a|]\nE[invalid escape sequence]@2:4\n");
	// Выполняем проверку отклонения усечённой записи кодовым значением широким
	ASSERT_EQ(::dump("[a]\nk=x\\U0001\n", settings), "S[a|]\nE[invalid escape sequence]@2:4\n");
	// Выполняем проверку разбора записи кодовым значением полной
	ASSERT_EQ(::dump("[a]\nk=x\\u0041\n", settings), "S[a|]\nP[k=xA]\n");
}
/**
...
TEST(CodecIniReader, FeedAndNextAtBoundaries) {
	 ...
	 */
	{
		...
		/**
		 * @note Прежде здесь утверждалось `NONE`, и утверждалась тем самым НЕМОТА: подача
		 *       после объявленного конца отвергалась без кода, и потребитель, спросивший
		 *       причину, получал отсутствие отказа при отказе
		 */
		// Выполняем проверку того, что отвергнутая подача назвала причину свою
 ...
 *
 */
TEST(CodecIniReader, CopySurvivesFeedUnlikeView) {
	// Объект потокового чтения текста настроек
	ini::reader_t reader(::logger());
	// Первый кусок исходного текста
	const string first = "[раздел]\nключ = значение\n";
	...
	for(uint32_t i = 0; i < 200; i++)
		...
		more.append("к" + std::to_string(i) + " = значение\n");
	// Выполняем подачу второго куска исходного текста
	ASSERT_TRUE(reader.feed(more.data(), more.size(), true));
 ...
 *
 */
TEST(CodecIniReader, CopyViewsPointIntoSource) {
	// Объект потокового чтения текста настроек
	ini::reader_t reader(::logger());
	// Исходный текст настроек
	const string text = "[раздел]\nключ = значение\n";
	 ...
	 */
	ASSERT_EQ(static_cast <const void *> (copy.section().section.data()),
	          static_cast <const void *> (reader.section().section.data()));
}
/**
 ...
 * молчалив, а код уходит потребителю причиною. Записи подобраны так, чтобы каждая
 * доходила до СВОЕГО места отказа, а не до соседнего с тем же кодом
 *
 * @note Заведено щупом слепоты: всякое место `fail(error_t::X)` подменялось порознь на
 *       причину иную, и печатались места, которых набор не заметил. По 29 местам INI
 *       вышло пять без сторожа, из них два - заставы последнего рубежа, к каким пути нет
 *
 * @note Место `INVALID_KEY` у поверки имени осталось незакрытым намеренно: восемь ходов
 *       к нему замерены и ни один не привёл - имя со скобкой в начале разбирается
 *       объявлением раздела прежде, а перевод строки внутрь имени не попадает ни при
 *       склейке строк, ни в ограде. Объявлять его недостижимым без доказательства не
 *       стану: помечено как неразрешённое замером
 *
 */
TEST(CodecIniReader, RefusalsNameTheirOwnCause) {
	/**
	 * Выполняем проверку отказа при имени раздела длиннее дозволенного
	 */
	{
		// Настройки разбора текста настроек
		ini::reader_t::settings_t settings;
		...
		while(reader.next());
		// Выполняем проверку выданного кода отказа разбора
		ASSERT_EQ(reader.error(), ini::error_t::NAME_TOO_LONG);
	}
	/**
	 * Выполняем проверку отказа при негодной отменяющей последовательности
	 *
	 * @note Записи подобраны трёх видов: неведомый знак за чертою, шестнадцатеричная
	 *       запись с негодными разрядами и запись знака Юникода, разрядов недобравшая
	 */
	{
		/**
		 * Выполняем перебор записей с негодными отменяющими последовательностями
		 */
		for(auto & source : {string("a = \"раз\\qдва\"\n"), string("a = \"\\xZZ\"\n"),
		                     string("a = \"\\u00\"\n")}){
			// Настройки разбора текста настроек
			ini::reader_t::settings_t settings;
			// Устанавливаем признание отменяющих последовательностей
			settings.escapes = true;
			// Объект потокового чтения текста настроек
			ini::reader_t reader(::logger(), settings);
			// Выполняем подачу разбираемой записи текста настроек
			...
			while(reader.next());
			// Выполняем проверку выданного кода отказа разбора
			ASSERT_EQ(reader.error(), ini::error_t::INVALID_ESCAPE) << source;
		}
	}
	/**
	 * Выполняем проверку отказа имени свойства, из строк продолжения собранного
	 *
	 * @details Продолжение отступом знак конца строки сохраняет, и имя, собранное из
	 * двух строк, несёт его внутри себя. Записью такое имя не отведено: знак конца
	 * строки в имени свойства недопустим всюду
	 *
	 * @note Место это восьми подборам написаний вслепую не поддалось - нашлось оно
	 *       подменою кода отказа особым и розыском написания по нему. Прежде наречие
	 *       бралось обычное, а продолжение отступом даёт лишь наречие Python
	 */
	{
		// Настройки разбора текста настроек наречия Python
		ini::reader_t::settings_t settings = ini::reader_t::settings_t::python();
		// Объект потокового чтения текста настроек
		ini::reader_t reader(::logger(), settings);
		// Разбираемая запись текста настроек
...
/**
 * @brief Проверка поверки имён строгою грамматикой наречия
 *
 * @details Наречия INI расходятся в построении имён слишком широко, и умолчанием
 *          грамматика не поверяется вовсе. Наречие Git же строит имя раздела из букв,
 *          цифр, черты и точки, а имя свойства - из буквы первым знаком и далее из
 *          букв, цифр и черты; записи «[a b]», «[a_b]», «k_2 = v», «1k = v» и «[ a ]»
 *          средство «git config» отвергает. Признак заведён затем, что чтение принимало
 *          их все, и наречие выходило шире своего образца: из 377 текстов корпуса
 *          сличения, средством не осиленных, чтение принимало 376. Признаком этим
 *          отвергаются ВСЕ 376, а из 257 текстов, средством осиленных, не отвергается
 *          НИ ОДИН: наречие сходится с образцом ровно
 *
 * @note Умолчанием признак снят, и поведение чтения не меняется вовсе. В наречие Git
 *       он НЕ вставлен: включение его одному чтению развело бы чтение с записью
 *
 */
TEST(CodecIniReader, StrictNameGrammar) {
	/**
	 * @brief Функция разбора текста настроек заданным признаком поверки
	 *
	 * @param text   разбираемый текст настроек
	 * @param strict признак поверки имён строгою грамматикой
	 * @return       признак успешности разбора
	 *
	 */
	auto parse = [](const string & text, const bool strict) noexcept -> bool {
		// Собираемые настройки разбора текста настроек
		ini::reader_t::settings_t settings;
		// Устанавливаем признание обоих построений имени подраздела
		settings.subsections = ini::subsection_t::BOTH;
		// Устанавливаем поверку имён строгою грамматикой наречия
		settings.strictNames = strict;
		// Объект потокового чтения текста настроек
		ini::reader_t reader(::logger(), settings);
		// Выполняем передачу текста настроек чтению
		if(!reader.feed(text))
			// Выводим признак неудачного разбора
			return false;
		// Выполняем перебор всех событий разбора
		while(reader.next());
		// Выводим признак успешности разбора
		return (reader.error() == ini::error_t::NONE);
	};
	/**
	 * Записи, грамматике наречия не отвечающие
	 */
	const vector <string> refused = {
		// Имя раздела с пробелом посреди
		"[a b]\nk = v\n",
		// Имя раздела со знаком подчёркивания
		"[a_b]\nk = v\n",
		// Имя свойства со знаком подчёркивания
		"[a]\nk_2 = v\n",
		// Имя свойства, начатое цифрой
		"[a]\n1k = v\n",
		// Объявление раздела с пробельной обвязкой внутри скобок
		"[ a ]\nk = v\n"
	};
	/**
	 * Выполняем перебор записей, грамматике не отвечающих
	 */
	for(const auto & text : refused){
		// Выполняем проверку того, что умолчанием запись принимается
		ASSERT_TRUE(parse(text, false)) << text;
		// Выполняем проверку того, что поверкою запись отвергается
		ASSERT_FALSE(parse(text, true)) << text;
	}
	/**
	 * Записи, грамматике наречия отвечающие
	 */
	const vector <string> accepted = {
		// Имя раздела с чертою
		"[a-b]\nk = v\n",
		// Имя раздела с точкою, подраздел отделяющей
		"[a.b]\nk = v\n",
		// Имя раздела с цифрою
		"[a1]\nk = v\n",
		// Имя свойства с чертою и цифрою
		"[a]\nk-2 = v\n",
		// Имя подраздела в кавычках, грамматике не подлежащее
		"[a \"любое имя\"]\nk = v\n"
	};
	/**
	 * Выполняем перебор записей, грамматике отвечающих
	 */
	for(const auto & text : accepted)
		// Выполняем проверку того, что поверкою запись принимается
		ASSERT_TRUE(parse(text, true)) << text;
}

/**
 * @brief Проверка отключаемости выдачи примечаний
 *
 * @details Настройка `emitComments` решает, выдавать ли примечания событиями. Отключив
 *          её, потребитель получает поток без них - ровно то, чего он и просит, когда
 *          примечания ему не нужны вовсе
 *
 * @warning Проверяются ОБА положения настройки: поверка одного лишь включённого
 *          прошла бы и с настройкой, вовсе не читаемой, - примечания выдавались бы
 *          всегда, и отключить их было бы нечем
 *
 * @note Пробел открыт обездвиживанием настроек: настройка эта - ЕДИНСТВЕННАЯ из
 *       двадцати одной логической у кодека, обездвиживание которой набор не заметил
 *       вовсе. Прочие двадцать ловятся отказами от одного до двадцати
 *
 */
TEST(CodecIniReader, CommentEmissionIsSwitchable){
	// Разбираемый текст настроек с примечанием
	const string text = "; примечание\n[раздел]\nключ = 1\n";
	/**
	 * Выполняем счёт событий примечаний при обоих положениях настройки
	 */
	for(const bool emit : {true, false}){
		...
		// Количество событий примечаний
		size_t comments = 0;
		// Количество событий всех видов
		size_t events = 0;
		/**
		 * Выполняем выборку всех событий разбора
		 */
		while(reader.next()){
			 ...
			 */
			if(reader.event() == ini::event_t::COMMENT)
				// Выполняем учёт события примечания
				comments++;
		}
		/**
		 * Выполняем проверку количества событий примечаний
		 *
		 * @note Ровно это и не стереглось: примечание выдавалось всегда, а настройка
		 *       читалась вхолостую
		 */
		ASSERT_EQ(comments, static_cast <size_t> (emit ? 1 : 0)) << emit;
		/**
		 * Выполняем проверку того, что прочие события отключением не задеты
		 *
		 * @note Половина эта стережёт ширину: чтение, отключением примечаний
		 *       умолкающее вовсе, прошло бы поверку выше
		 */
		ASSERT_EQ(events, static_cast <size_t> (emit ? 3 : 2)) << emit;
		// Выполняем проверку того, что разбор дошёл до конца текста
		ASSERT_EQ(reader.state(), ini::state_t::FINISHED) << emit;
	}
}

/**
 * @brief Проверка того, что за последним куском чтение голода не объявляет
 *
 * @details Состояние `HUNGRY` объявлено значащим «для продолжения разбора требуется
 *          следующий кусок текста», и за последним куском оно ЛОЖНО: продолжения не
 *          требуется, ибо его нет вовсе
 *
 * @warning Ложь эта не была безобидной: потребитель, договору следующий буквально -
 *          «при HUNGRY подавай ещё», - уходил в БЕСКОНЕЧНЫЙ круг, ибо всякая
 *          следующая подача отвечает отказом и состояния не меняет. Проверка эта
 *          круг тот и воспроизводит, отсекая его счётом кругов
 *
 * @note Породу находки указал владелец кодеков JSON, XML и CSV: у разметки XML стояла
 *       та же ложь с иного конца. Из семи кодеков рамки расходился этот один
 *
 */
TEST(CodecIniReader, FinalChunkLeavesNoHunger){
	// Разбираемый текст настроек
	const string text = "[section]\nkey = 1\nother = 2\n";
	/**
	 * Выполняем проверку состояния при подаче текста ЦЕЛИКОМ
	 */
	{
		// Объект чтения текста настроек
		ini::reader_t reader(::logger());
		// Выполняем проверку того, что до подачи чтение продолжения требует
		ASSERT_EQ(reader.state(), ini::state_t::HUNGRY);
		// Выполняем подачу текста настроек целиком с признаком последнего куска
		ASSERT_TRUE(reader.feed(text.data(), text.size(), true));
		/**
		 * Выполняем проверку того, что голода за последним куском чтение не объявляет
		 *
		 * @note Ровно это и лгало: состояние оставалось `HUNGRY` при том, что подавать
		 *       было нечего
		 */
		ASSERT_NE(reader.state(), ini::state_t::HUNGRY);
		// Выполняем проверку того, что чтение объявило наличие разбираемого
		ASSERT_EQ(reader.state(), ini::state_t::READY);
		// Количество выбранных событий разбора
		size_t events = 0;
		// Выполняем выборку всех событий разбора
		while(reader.next()) events++;
		// Выполняем проверку того, что события выбраны
		ASSERT_EQ(events, static_cast <size_t> (3));
		// Выполняем проверку того, что разбор дошёл до конца текста
		ASSERT_EQ(reader.state(), ini::state_t::FINISHED);
	}
	/**
	 * Выполняем проверку того, что круг подачи по договору разрывается
	 *
	 * @details Круг этот и есть беда во плоти: потребитель кормит чтение, покуда оно
	 *          голодно, а оно голодно всегда
	 */
	{
		// Объект чтения текста настроек
		ini::reader_t reader(::logger());
		// Выполняем подачу текста настроек целиком с признаком последнего куска
		ASSERT_TRUE(reader.feed(text.data(), text.size(), true));
		// Количество кругов подачи
		size_t rounds = 0;
		/**
		 * Выполняем подачу до тех пор, покуда чтение голодно
		 */
		while((reader.state() == ini::state_t::HUNGRY) && (rounds < 8)){
			// Выполняем наращивание счёта кругов подачи
			rounds++;
			// Выполняем подачу пустого куска, договором предписанную
			reader.feed("", 0, true);
		}
		// Выполняем проверку того, что круга не случилось вовсе
		ASSERT_EQ(rounds, static_cast <size_t> (0));
	}
	/**
	 * Выполняем проверку того, что подача НЕПОСЛЕДНЕГО куска голод объявляет
	 *
	 * @note Половина эта стережёт ширину правки: чтение, голода не объявляющее
	 *       никогда, прошло бы обе половины выше, а потребитель не узнал бы, что
	 *       текст ещё нужен
	 */
	{
		// Объект чтения текста настроек
		ini::reader_t reader(::logger());
		// Выполняем подачу первого куска текста без признака последнего
		ASSERT_TRUE(reader.feed(text.data(), 4, false));
		// Выполняем проверку того, что чтение продолжения требует
		ASSERT_EQ(reader.state(), ini::state_t::HUNGRY);
		// Выполняем подачу остатка текста с признаком последнего куска
		ASSERT_TRUE(reader.feed(text.data() + 4, text.size() - 4, true));
		// Выполняем проверку того, что голод снят последним куском
		ASSERT_EQ(reader.state(), ini::state_t::READY);
	}
}
 ...
 *
 */
TEST(CodecIniReader, FeedAfterTheDeclaredEndIsRefused) {
	// Объект потокового чтения текста
	ini::reader_t reader(::logger());
	// Собираемая подача исходного текста
	const string first = "[s]\nk=1\n";
	// Выполняем проверку того, что первая подача с признаком конца принимается
	ASSERT_TRUE(reader.feed(first.data(), first.size(), true)) << ini::message(reader.error());
	// Собираемая подача, конец текста переступающая
	const string second = "[t]\nm=2\n";
...
/**
 * @brief Проверка того, что подача следом за отказом приведения называет причину
 *
 * @details Отказ приведения у кодека INI ОТКЛАДЫВАЕТСЯ до вычерпывания событий - там он
 * выдаётся вместе со строкою и местом, - и оттого сама подача отвечает успехом. Кодеки
 * TOML и YAML отвечают отказом сразу. Исход для потребителя, события вычерпывающего, у
 * всех трёх один; разнится миг объявления. Но потребитель, событий НЕ вычерпавший и
 * подавший кусок следом, получал прежде ложь при коде `NONE` - отсутствие отказа при
 * отказе
 *
 */
TEST(CodecIniReader, FeedAfterTheFailedConversionNamesTheCause) {
	// Объект потокового чтения текста настроек
	ini::reader_t reader(::logger());
	// Подача с последовательностью байтов, кодировке не отвечающей
	const string bad = "\xC3\x28негодная\n";
	// Выполняем подачу негодного куска исходного текста
	reader.feed(bad.data(), bad.size(), false);
	/**
	 * @note События НЕ вычерпываются намеренно: ими отказ и объявляется, а поверяется
	 *       здесь именно тот путь, где потребитель их не спросил
	 */
	// Годная подача, за отказом приведения поданная
	const string good = "[s]\nk = 1\n";
	// Выполняем проверку отказа подачи следом за отказом приведения
	ASSERT_FALSE(reader.feed(good.data(), good.size(), true));
	// Выполняем проверку того, что отказ назван поимённо, а не отдан молча
	ASSERT_EQ(reader.error(), ini::error_t::INVALID_ENCODING) << ini::message(reader.error());
}
