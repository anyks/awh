...
/**
 * Подключаем переносимые ходы по временным файлам проверок
 *
 * @note Заведение каталога зовётся посредником `makeDirectory()`, а не `::mkdir()`
 *       напрямую: у POSIX тот берёт путь да права доступа, а у MinGW под MS Windows -
 *       один лишь путь, и прямой вызов сборку там валит. Указано это у кодека CEF:
 *       проверки кодеков собираются ОДНОЙ программой, и три моих вызова
 *       остановили сборку всего набора, включая чужие проверки
 */
 ...
 */
/**
 * @brief Проверка постановки владеющего значения по пути
 *
 * @details Ход заведён ради общего вида кодеков рамки: правка дерева зовётся у одних
 *          кодеков ходом значения, у других ходом документа, и потребитель, два кодека
 *          рядом читающий, разыскивал ход не там, где тот лежал
 *
 * @note Проверка держит и то, что постановка по ключу СТРОКОЮ осталась цела: доводы её
 *       и доводы постановки владеющего значения различаются лишь видом, и заведение
 *       второй сделало первую двусмысленной. Ограждение снято шаблоном, а проверка
 *       закрепляет, что обе зовутся по-прежнему
 *
 */
/**
 ...
 *       объявленных разделов. Проверка держит обе стороны разом - и новый смысл `size()`, и то, что
 ...
 *
 * @note Смена согласована с прочими кодеками 03.09.2026
 *
 */
/**
 ...
 */
/**
 * @brief Проверка выдачи кодировки прочитанного текста
 *
 * @details Ход общий у кодеков рамки, текст читающих, и у INI с TOML его прежде не было
 *          вовсе. Кодировку определяет ЧТЕНИЕ - по метке порядка октетов либо по самому
 *          виду текста, - а дерево лишь несёт её потребителю: своего опознания дерево не
 *          ведёт, ибо два опознания разошлись бы молча
 *
 * @note Проверка сличает состояние ДО разбора и ПОСЛЕ: сличать одно лишь «после» мало -
 *       ход, всегда выдающий UTF-8, дал бы тот же ответ и ничего не читая. Случай
 *       подобран так, чтобы неверное поведение давало ИНОЙ ответ, а не тот же самый
 *
 */
TEST(CodecIniDocument, EncodingIsReportedByTheReader){
	// Объект дерева настроек, ещё ничего не разобравший
	ini::document_t document;
	// Выполняем проверку того, что до разбора кодировка не выставлена
	ASSERT_EQ(document.encoding(), ini::encoding_t::NONE);
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse("a = 1\n")) << ini::message(document.error());
	// Выполняем проверку того, что после разбора кодировка опознана
	ASSERT_EQ(document.encoding(), ini::encoding_t::UTF8);
}
TEST(CodecIniDocument, LoadAndSaveRoundTrip){
	// Путь до временного файла настроек
	const string filename = string(::testing::TempDir()) + ::unique("ini-roundtrip.conf");
	// Объект дерева настроек, текст разбирающий
	ini::document_t source;
	// Выполняем разбор текста настроек
...
TEST(CodecIniDocument, SizeCountsChildrenOfTheRoot){
	...
	ASSERT_EQ(document.size(), ini::value_t(document).size());
	// Выполняем проверку того, что прежний счёт никуда не делся, а лишь переехал
	ASSERT_NE(document.sections().size(), 0u);
}
TEST(CodecIniDocument, SettingOwningValueByPath){
	// Объект дерева настроек, текст разбирающий
	ini::document_t document;
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse("[server]\nhost = old\n")) << ini::message(document.error());
	// Выполняем проверку того, что постановка по ключу строкою цела
	ASSERT_TRUE(document.set("port", "8080", "server")) << ini::message(document.error());
	// Выполняем постановку владеющего значения по пути
	ASSERT_TRUE(document.set("/server/host", ini::value_t("anyks"))) << ini::message(document.error());
	// Снимаем настройки владеющим значением
	const ini::value_t whole(document);
	// Извлекаемое содержимое простого значения
	string text;
	// Выполняем проверку того, что поставленное значение легло по пути
	ASSERT_TRUE(whole.at("/server/host").value(text));
	// Выполняем проверку содержимого поставленного значения
	ASSERT_EQ(text, "anyks");
	// Выполняем проверку того, что постановка по ключу строкою значение сохранила
	ASSERT_TRUE(whole.at("/server/port").value(text));
	// Выполняем проверку содержимого значения, строкою поставленного
	ASSERT_EQ(text, "8080");
}
...
TEST(CodecIniDocument, Global) {
	 ...
	 */
	ASSERT_EQ(document.sections().size(), 1u);
}
/**
...
TEST(CodecIniDocument, GlobalAssembly) {
	 ...
	 * Выполняем проверку того, что раздел без имени объявленным не считается
	 */
	ASSERT_EQ(document.sections().size(), 0u);
}
/**
...
TEST(CodecIniDocument, InvalidNames) {
	...
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse("[a]\n"));
	// Выполняем проверку отклонения имени свойства со знаком возврата каретки
	ASSERT_FALSE(document.set("k\rx", "v", "a"));
...
TEST(CodecIniDocument, ReferenceDepthDiffersFromSubsectionDepth) {
	...
	// Устанавливаем построение имени подраздела разделителем
	settings.reader.subsections = ini::subsection_t::DELIMITED;
	// Выполняем запрет подразделов вовсе признаком читающего
	settings.reader.nesting = false;
	// Выполняем разбор текста настроек с подразделом
	ASSERT_FALSE(document.parse("[раздел.подраздел]\nkey = value\n", settings));
	// Выполняем дозволение подразделов признаком читающего
	settings.reader.nesting = true;
	// Устанавливаем предел глубины вложенности подразделов читающего
...
/**
 * @brief Проверка выдачи пустых перечней при обращении к необъявленному разделу
 *
 * @details Снятие имён свойств, значений, исходных записей и признаков закавыченности
 *          у раздела, которого дерево не несёт, обязано выдавать перечень пустой, а не
 *          перечень чужого раздела: пересечение прогонов числило слепыми все четыре
 *          строки отказа поиска раздела - задеты были лишь снос свойства и удаление
 *          раздела, отвечающие кодом ошибки
 *
 * @note Тот же вызов по разделу объявленному ведётся рядом нарочно: пустой перечень
 *       сам по себе прошёл бы и при дереве, ничего не несущем вовсе
 *
 */
TEST(CodecIniDocument, EmptyListsOnUnknownSection) {
	// Дерево настроек
	ini::document_t document;
	// Выполняем разбор текста настроек
	/**
	 * Свойство раздела без имени носит то же имя нарочно: промах поиска раздела
	 * порядкового номера не трогает, а заведён он нулём - номером раздела без
	 * имени. Оставь тот раздел пустым, и обход сторожа выдал бы тот же пустой
	 * перечень, а проверка прошла бы и без сторожа вовсе
	 */
	ASSERT_TRUE(document.parse("k = \"корень\"\n[a]\nk = \"v\"\n"));
	// Выполняем проверку выдачи пустого перечня имён свойств
	ASSERT_TRUE(document.keys("нет").empty());
	// Выполняем проверку выдачи пустого перечня значений
	ASSERT_TRUE(document.values("k", "нет").empty());
	// Выполняем проверку выдачи пустого перечня исходных записей
	ASSERT_TRUE(document.sources("k", "нет").empty());
	// Выполняем проверку выдачи пустого перечня признаков закавыченности
	ASSERT_TRUE(document.quotes("k", "нет").empty());
	// Выполняем проверку выдачи перечня имён свойств раздела объявленного
	ASSERT_EQ(document.keys("a"), (vector <string_view> {"k"}));
	// Выполняем проверку выдачи перечня значений раздела объявленного
	ASSERT_EQ(document.values("k", "a").size(), 1u);
	// Выполняем проверку выдачи перечня исходных записей раздела объявленного
	ASSERT_EQ(document.sources("k", "a").size(), 1u);
	// Выполняем проверку выдачи перечня признаков закавыченности раздела объявленного
	ASSERT_EQ(document.quotes("k", "a"), (vector <bool> {true}));
}
/**
 ...
 *
 * @note Беда эта найдена у соседних кодеков и передана сюда
 *
 */
TEST(CodecIniDocument, ErrorAnswersForLastOperation) {
	// Объект дерева настроек
	ini::document_t document;
	// Выполняем проверку отказа разбора негодного текста
	ASSERT_FALSE(document.parse("[раздел\n"));
	 ...
	 */
	ASSERT_TRUE(document.create("новый"));
...
/**
 * @brief Проверка отказов работы над деревом, входом достижимых
 *
 * @details Заходы эти пересечение трёх прогонов числило слепыми: объявление раздела с
 * пустым именем не подавалось вовсе, а долив свойства не спрашивался ни с негодным
 * именем, ни в раздел негодный
 *
 */
TEST(CodecIniDocument, RefusalsOnTreeOperations) {
	// Объект дерева настроек
	ini::document_t document;
	/**
	 * Выполняем разбор текста, свойство до первого объявления несущего
	 *
	 * @note Свойство это ложится в раздел безымянный, и раздел тот в дереве заводится
	 */
	ASSERT_TRUE(document.parse("вне = раздела\n[a]\nk=v\n"));
	// Выполняем проверку того, что свойство безымянного раздела читается
	ASSERT_EQ(document.get("вне"), "раздела");
	/**
	 * Выполняем проверку отказа поиска раздела без имени
	 *
	 * @warning Раздел безымянный в дереве ЕСТЬ - он держит свойства до первого
	 *          объявления, - и поиск его нашёл бы. Отказ выдаётся нарочно: спрашивать
	 *          наличие раздела, какой есть всегда, смысла не имеет, а положительный
	 *          ответ на пустое имя уводил бы потребителя в сторону
	 *
	 * @note Разбор здесь стоит ПРЕЖДЕ проверки нарочно: у дерева пустого поиск не нашёл
	 *       бы раздела и без сторожа, и порча его осталась бы незамечена - замерено
	 *       срывом
	 */
	ASSERT_FALSE(document.section("", ""));
	// Выполняем проверку отказа долива свойства с пустым именем
	ASSERT_FALSE(document.push("", "v", "a"));
	// Выполняем проверку кода отказа
	ASSERT_EQ(document.error(), ini::error_t::EMPTY_KEY);
	// Выполняем проверку отказа долива свойства в раздел со знаком конца строки в имени
	ASSERT_FALSE(document.push("k", "v", "a\nb"));
	// Выполняем проверку кода отказа
	ASSERT_EQ(document.error(), ini::error_t::INVALID_SECTION);
	/**
	 * Выполняем проверку того, что долив годный принимается
	 *
	 * @note Заход этот стоит рядом нарочно: отказ, приходящий на всякий долив, отвечал
	 *       бы тем же ожиданиям, и порча поверок осталась бы незамечена
	 */
	ASSERT_TRUE(document.push("second", "w", "a"));
}
/**
 * @brief Проверка признака обращений при вставке свойства в середину дерева
 *
 * @details Дерево держит признак того, что оно несёт обращения к значениям: без него
 * подстановка не ведётся вовсе. Признак этот обязан взводиться и правкой, а не одним
 * лишь разбором, - иначе обращение, заведённое доливом, осталось бы неразрешённым
 *
 * @warning Условие это стоит в теле долива ДВАЖДЫ - у дозаписи в конец и у вставки в
 *          середину, - и заходы эти разные. Пересечение трёх прогонов числило слепым
 *          именно второй: долив спрашивался лишь в раздел последний, где запись ложится
 *          в конец. Нашлось это отладчиком - останов у первого близнеца срабатывал, у
 *          второго нет. Раздел здесь взят СРЕДНИЙ нарочно: за ним стоит другой, и
 *          запись обязана лечь в середину
 *
 */
TEST(CodecIniDocument, ReferenceFlagOnMiddleInsert) {
	// Собираемые настройки дерева настроек
	ini::document_t::settings_t settings;
	// Устанавливаем построение обращений видом оболочки
	settings.references = ini::reference_t::SHELL;
	// Объект дерева настроек
	ini::document_t document(settings);
	// Выполняем разбор текста настроек из двух разделов
	ASSERT_TRUE(document.parse("[a]\nk=v\n[b]\nm=n\n"));
	// Выполняем долив свойства с обращением в раздел средний
	ASSERT_TRUE(document.push("second", "${b:m}", "a"));
	/**
	 * Выполняем проверку того, что обращение разрешилось подстановкой
	 *
	 * @note Без взведения признака дерево считалось бы обращений не несущим, и значение
	 *       осталось бы записью обращения как есть
	 */
	ASSERT_EQ(document.get("second", "a"), "n");
}
/**
 * @brief Проверка извлечения числовых значений в предельных случаях
 *
 * @details Нечисло к целому виду приводится нулём, а не отказом: приведение `NaN` к
 * целому есть неопределённое поведение при любом пределе - x86-64 выдаёт наименьшее
 * целое, ARM64 ноль, - и кодек берёт ноль сам, не полагаясь на устройство машины.
 * Запись же, числом не являющаяся вовсе, даёт отказ извлечения
 *
 * @note Строки эти пересечение трёх прогонов числило слепыми: нечисло не подавалось, а
 *       негодная запись спрашивалась лишь целым видом
 *
 * @warning Сторож нечисла на ЭТОЙ машине равнозначен: ARM64 приводит `NaN` к целому
 *          нулём и без него, и снятие сторожа проверки не роняет - замерено срывом.
 *          На x86-64 равнозначным он не является: там приведение даёт наименьшее целое,
 *          и проверка упала бы. Сторож держится ради независимости итога от устройства
 *          машины, и проверка стережёт его лишь там, где машины расходятся
 *
 */
TEST(CodecIniDocument, NumericExtractionEdges) {
	// Объект дерева настроек
	ini::document_t document;
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse("[a]\nнечисло=nan\nмусор=не число\nдробное=1.5\n"));
	/**
	 * Выполняем проверку приведения нечисла к целому виду
	 */
	{
		// Извлекаемое целое значение
		int64_t value = -1;
		// Выполняем проверку успешности извлечения
		ASSERT_TRUE(document.value(value, "нечисло", "a"));
		/**
		 * Выполняем проверку того, что нечисло приведено нулём
		 *
		 * @note Ноль выдаётся УСПЕХОМ, а не отказом, и это решение кодека: без него
		 *       итог зависел бы от устройства машины
		 */
		ASSERT_EQ(value, static_cast <int64_t> (0));
	}
	/**
	 ...
	 */
	{
		...
		double value = -1.;
		// Выполняем проверку отказа извлечения дробного значения
		ASSERT_FALSE(document.value(value, "мусор", "a"));
		/**
		 * Выполняем проверку того, что запись годная извлекается
		 *
		 * @note Заход этот стоит рядом нарочно: отказ, приходящий на всякую запись,
		 *       отвечал бы тому же ожиданию
		 */
		ASSERT_TRUE(document.value(value, "дробное", "a"));
		// Выполняем проверку извлечённого дробного значения
		ASSERT_DOUBLE_EQ(value, 1.5);
	}
}
/**
 * @brief Проверка пустоты дерева, записи которого снесены
 *
 * @details Дерево, все записи которого сняты, пустым является наравне с деревом, ничего
 * не разобравшим: снесённая запись в выдачу не идёт. Строку эту пересечение трёх
 * прогонов числило слепой - пустота спрашивалась лишь у дерева, ничего не разобравшего
 *
 */
TEST(CodecIniDocument, EmptyAfterEverythingRemoved) {
	// Объект дерева настроек
	ini::document_t document;
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse("[a]\nk=v\n"));
	// Выполняем проверку того, что дерево пустым не является
	ASSERT_FALSE(document.empty());
	// Выполняем снятие свойства
	ASSERT_TRUE(document.erase("k", "a"));
	// Выполняем снос раздела
	ASSERT_TRUE(document.remove("a"));
	// Выполняем проверку того, что дерево стало пустым
	ASSERT_TRUE(document.empty());
}
/**
 * @brief Проверка обращения к свойству, разделу неведомому
 *
 * @details Свойство, разделом не объявленное, снятию не поддаётся: запись выдаётся
 *          пустой, а перечни значений, исходных записей и признаков закавыченности -
 *          пустыми перечнями. Разница с разделом необъявленным тут в том, что раздел
 *          есть, а свойства в нём нет, и строки эти пересечение трёх прогонов числило
 *          слепыми - снимались лишь свойства наличные
 *
 */
TEST(CodecIniDocument, MissingPropertyInKnownSection) {
	// Дерево настроек
	ini::document_t document;
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse("[a]\nk = \"v\"\n"));
	// Выполняем проверку выдачи пустой записи у свойства неведомого
	ASSERT_TRUE(document.get("нет", "a").empty());
	// Выполняем проверку выдачи пустого перечня значений у свойства неведомого
	ASSERT_TRUE(document.values("нет", "a").empty());
	// Выполняем проверку выдачи пустого перечня исходных записей у свойства неведомого
	ASSERT_TRUE(document.sources("нет", "a").empty());
	// Выполняем проверку выдачи пустого перечня закавыченности у свойства неведомого
	ASSERT_TRUE(document.quotes("нет", "a").empty());
	// Выполняем проверку выдачи записи у свойства наличного
	ASSERT_EQ(document.get("k", "a"), "v");
	// Выполняем проверку выдачи перечня значений у свойства наличного
	ASSERT_EQ(document.values("k", "a").size(), 1u);
	// Выполняем проверку выдачи перечня исходных записей у свойства наличного
	ASSERT_EQ(document.sources("k", "a").size(), 1u);
	// Выполняем проверку выдачи перечня закавыченности у свойства наличного
	ASSERT_EQ(document.quotes("k", "a"), (vector <bool> {true}));
}
/**
 * @brief Проверка правки свойства со скобками в имени при чтении без перечней
 *
 * @details Читающий, перечней не признающий, числит «l[]» именем обыкновенным и текст
 *          такой принимает. Правка же дерева скобки отвергала всегда, и круг «чтение -
 *          правка» на них рвался: перенос значения в дерево пустое отказывал именем,
 *          какое сам же разбор и выдал. Со включёнными перечнями скобки значащи -
 *          «l[]» есть долив к перечню, - и отвергать их правке должно
 *
 * @note Правило одно с правилом записи: там скобки ограждены тем же признаком `arrays`
 *
 */
TEST(CodecIniDocument, BracketedKeyEditWithoutArrays) {
	// Дерево настроек, перечней не признающее
	ini::document_t plain;
	// Выполняем разбор текста настроек со скобками в имени свойства
	ASSERT_TRUE(plain.parse("l[] = a\n"));
	// Выполняем проверку того, что имя прочитано целиком со скобками
	ASSERT_EQ(plain.get("l[]"), "a");
	// Выполняем проверку того, что правка такого имени принимается
	ASSERT_TRUE(plain.set("m[]", "b"));
	// Выполняем проверку того, что правленое свойство снимается тем же именем
	ASSERT_EQ(plain.get("m[]"), "b");
	// Выполняем проверку того, что перезапись читается обратно
	ASSERT_NE(plain.text().find("m[] = b"), string::npos);
	// Владеющее значение, с дерева снятое
	const ini::value_t lifted(plain);
	// Дерево настроек, куда переносится значение
	ini::document_t target;
	// Выполняем разбор пустого текста настроек
	ASSERT_TRUE(target.parse(""));
	// Выполняем проверку успешности переноса значения в дерево
	ASSERT_TRUE(lifted.graft(target));
	// Выполняем проверку того, что перенесённое свойство на месте
	ASSERT_EQ(target.get("l[]"), "a");
	// Настройки дерева настроек, перечни признающего
	ini::document_t::settings_t settings;
	// Включаем признание перечней значений
	settings.reader.arrays = true;
	// Дерево настроек, перечни признающее
	ini::document_t arrayed;
	// Выполняем разбор пустого текста настроек с признанием перечней
	ASSERT_TRUE(arrayed.parse("", settings));
	// Выполняем проверку того, что правка имени со скобками отвергается
	ASSERT_FALSE(arrayed.set("m[]", "b"));
}
/**
 * @brief Проверка уклада нуля у числовых пределов настроек
 *
 * @details Пределы `maxLine`, `maxName` и `maxDepth` нулём СНИМАЮТСЯ, а предел
 *          продолжений `maxContinuation` нулём ЗАПРЕЩАЕТ их вовсе. Уклады эти разные, а
 *          соседство четырёх настроек одной записи читается единым, и правку туда внесут
 *          скорее по недоразумению
 *
 * @note Прежде разноуклад шёл пополам: нулём снимались две настройки и две запрещали.
 *       `maxDepth` сведён к общему укладу кодеков рамки, ибо настройка эта есть часть
 *       общего API и переносится потребителем с кодека на кодек. `maxContinuation`
 *       оставлен как был: продолжения строк есть свойство одного лишь этого кодека,
 *       переносить его некуда, и общего уклада на него не распространяется
 *
 * @note Ограждения нуля недоставало правке дерева: разбор при `maxName` в ноль
 *       принимал имя любое, а правка отвергала всякое непустое словами «имя слишком
 *       длинно», и круг «чтение - правка» рвался целиком. Записывающий уклад этот
 *       блюл, дерево - нет
 *
 */
TEST(CodecIniDocument, ZeroConventionOfNumericLimits) {
	{
		// Настройки дерева настроек
		ini::document_t::settings_t settings;
		// Снимаем предел длины имени раздела или свойства
		settings.reader.maxName = 0;
		// Дерево настроек
		ini::document_t document;
		// Выполняем проверку успешности разбора текста настроек
		ASSERT_TRUE(document.parse("[a]\nk = v\n", settings));
		// Выполняем проверку того, что правка снятым пределом проходит
		ASSERT_TRUE(document.set("m", "w", "a"));
		// Выполняем проверку того, что правленое свойство на месте
		ASSERT_EQ(document.get("m", "a"), "w");
		// Выполняем проверку того, что перезапись правку несёт
		ASSERT_NE(document.text().find("m = w"), string::npos);
	}{
		// Настройки дерева настроек
		ini::document_t::settings_t settings;
		// Снимаем предел длины логической строки
		settings.reader.maxLine = 0;
		// Дерево настроек
		ini::document_t document;
		// Выполняем проверку того, что снятый предел длину строки не поверяет
		ASSERT_TRUE(document.parse("k = " + string(100000, 'x') + "\n", settings));
	}{
		// Настройки дерева настроек
		ini::document_t::settings_t settings;
		// Включаем склеивание строк, продолженных обратной косой чертой
		settings.reader.continuations = true;
		// Запрещаем продолжения вовсе
		settings.reader.maxContinuation = 0;
		// Дерево настроек
		ini::document_t document;
		// Выполняем проверку того, что нуль продолжение отвергает, а не снимает предел
		ASSERT_FALSE(document.parse("k = a\\\nb\n", settings));
		// Выполняем проверку того, что запись без продолжения тем же нулём принимается
		ASSERT_TRUE(ini::document_t().parse("k = a\n", settings));
	}{
		// Настройки дерева настроек
		ini::document_t::settings_t settings;
		// Включаем отделение подраздела знаком-разделителем
		settings.reader.subsections = ini::subsection_t::DELIMITED;
		// Снимаем предел вложенности подразделов нулём
		settings.reader.maxDepth = 0;
		/**
		 * Выполняем проверку того, что нуль предел СНИМАЕТ, а не запрещает вложенность
		 *
		 * @note Уклад этот сведён к общему у всех кодеков рамки: прежде нуль подразделы
		 *       запрещал, и настройка, перенесённая с иного кодека, срабатывала наоборот и
		 *       молча. Запрет получил своё выражение - признак `nesting`
		 */
		ASSERT_TRUE(ini::document_t().parse("[a.b.c.d]\nk = v\n", settings));
		// Запрещаем подразделы вовсе признаком, а не пределом
		settings.reader.nesting = false;
		// Выполняем проверку того, что снятый признак подраздел отвергает
		ASSERT_FALSE(ini::document_t().parse("[a.b]\nk = v\n", settings));
		// Выполняем проверку того, что раздел без подраздела тем же признаком принимается
		ASSERT_TRUE(ini::document_t().parse("[a]\nk = v\n", settings));
	}
}
/**
 * @brief Проверка границы открытых посредников, судящих по одному знаку
 *
 * @details Посредники `commented()` и `separated()` возвращают `bool`, выглядят
 *          приговором и молчат о том, чего не видят: на входе у них один знак - ни
 *          места его в строке, ни настроек. Кодек же решает по целому, и приговоры
 *          расходятся. Граница вписана оговорками у самих посредников, а закреплена
 *          здесь: запись без проверки читается обещанием полноты
 *
 * @note Закрепляется и ОТСУТСТВИЕ расхождения там, где его нет: знак, набору не
 *       принадлежащий, ни посредником, ни кодеком примечанием не признаётся
 *
 */
TEST(CodecIniCommon, PredicateScopeAgainstCodec) {
	// Выполняем проверку того, что посредник знак примечания признаёт всегда
	ASSERT_TRUE(ini::commented(';', ini::marker_t::BOTH));
	// Выполняем проверку того, что знак вне набора посредником не признаётся
	ASSERT_FALSE(ini::commented('!', ini::marker_t::BOTH));
	// Выполняем проверку того, что посредник знак разделителя признаёт всегда
	ASSERT_TRUE(ini::separated('=', ini::separator_t::BOTH));
	{
		// Дерево настроек
		ini::document_t document;
		// Выполняем разбор текста настроек умолчанием
		ASSERT_TRUE(document.parse("k = a;b\n"));
		// Выполняем проверку того, что умолчанием знак примечанием не служит
		ASSERT_EQ(document.get("k"), "a;b");
	}{
		// Настройки дерева настроек
		ini::document_t::settings_t settings;
		// Включаем признание примечания в конце строки
		settings.reader.inlineComments = true;
		// Дерево настроек
		ini::document_t document;
		// Выполняем разбор текста настроек
		ASSERT_TRUE(document.parse("k = a;b\n", settings));
		// Выполняем проверку того, что знак без отделения примечанием не служит
		ASSERT_EQ(document.get("k"), "a;b");
		// Дерево настроек со знаком, пробелом отделённым
		ini::document_t spaced;
		// Выполняем разбор текста настроек со знаком, пробелом отделённым
		ASSERT_TRUE(spaced.parse("k = a ;b\n", settings));
		// Выполняем проверку того, что отделённый знак примечанием служит
		ASSERT_EQ(spaced.get("k"), "a");
		// Снимаем требование пробела перед началом примечания
		settings.reader.spacedComments = false;
		// Дерево настроек без требования пробела
		ini::document_t tight;
		// Выполняем разбор текста настроек без требования пробела
		ASSERT_TRUE(tight.parse("k = a;b\n", settings));
		// Выполняем проверку того, что знак посреди значения примечанием служит
		ASSERT_EQ(tight.get("k"), "a");
	}{
		// Дерево настроек
		ini::document_t document;
		// Выполняем разбор текста настроек с двумя знаками разделителя
		ASSERT_TRUE(document.parse("k = a = b\n"));
		// Выполняем проверку того, что разделителем служит лишь первый знак
		ASSERT_EQ(document.get("k"), "a = b");
	}
}
/**
 ...
 *
 * @note Замерено 03.09.2026 до правки: свойство `a/b` в разделе стояло, а `at("/раз/a/b")`
 *       отвечало недействительным значением - обход дерева был не замкнут
 *
 */
TEST(CodecIniDocument, PathTokenCarriesEscapesOfTheStandard){
	// Объект дерева настроек, текст разбирающего
	ini::document_t document;
	// Выполняем разбор текста настроек с именами, отменяющих записей требующими
	 ...
	 */
	ASSERT_TRUE(document.at("/раз/a~1b").valid());
	// Выполняем проверку того, что записанное звено ведёт к тому самому имени
	ASSERT_EQ(document.at("/раз/a~1b").text(), "1");
	/**
	 * Выполняем проверку того, что имя как есть путём не выражается
	 *
	 * @note Половина эта важнее первой: разбирайся звено как есть - путь `/раз/a/b` вёл бы
	 *       к свойству `b` подраздела `a`, какого в дереве нет вовсе, и обход ушёл бы в
	 *       никуда молча
	 */
	ASSERT_FALSE(document.at("/раз/a/b").valid());
	// Выполняем проверку достижимости имени со знаком отмены отменяющей записью
	ASSERT_TRUE(document.at("/раз/тильда~0тут").valid());
	// Выполняем проверку того, что записанное звено ведёт к тому самому имени
	ASSERT_EQ(document.at("/раз/тильда~0тут").text(), "2");
	/**
	 ...
	 */
	ASSERT_FALSE(document.at("/раз/тильда~тут").valid());
	// Выполняем проверку того, что имя отменяющих записей не требующее достижимо просто
	ASSERT_TRUE(document.at("/раз/обычное").valid());
	// Выполняем проверку того, что знак отмены в конце звена отвергается
	ASSERT_FALSE(document.at("/раз/обычное~").valid());
}
/**
 * @brief Проверка огранения поднятого предела вложенности потолком допустимого
 *
 * @details Настройка, поднятая потребителем выше потолка, обходила его вовсе: подразделы
 * глубже допустимого разбирались успешно
 *
 * @warning Уклад нуля здесь ИНОЙ, нежели у контейнера JSON: ноль означает запрет
 *          подразделов вовсе, а не отсутствие предела, - и огранению не подлежит.
 *          Огранись он потолком - запрет обратился бы дозволением молча, оттого вторая
 *          половина проверки его и стережёт
 *
 */
TEST(CodecIniDocument, RaisedDepthLimitIsBoundedByTheCeiling){
	{
		// Настройки дерева настроек с поднятым пределом вложенности
		ini::document_t::settings_t settings;
		/**
		 * Устанавливаем выделение подразделов знаком-разделителем
		 *
		 * @warning Без него проверка мерила бы ПУСТОТУ: по умолчанию подразделы не
		 *          выделяются вовсе, имя раздела берётся целиком, и глубина не считается
		 *          ни разу - проверка зеленела бы и при снятом огранении
		 */
		settings.reader.subsections = ini::subsection_t::DELIMITED;
		// Поднимаем предел вложенности много выше потолка допустимого
		settings.reader.maxDepth = 200000;
		// Объект дерева настроек с поднятым пределом вложенности
		ini::document_t document(settings);
		// Собираемое имя раздела с глубокой вложенностью подразделов
		string name = "[р";
		/**
		 * Выполняем сборку имени раздела с подразделами глубже потолка
		 */
		for(size_t i = 0; i < 100; i++)
			// Выполняем добавление очередного подраздела
			name.append(".п");
		// Выполняем закрытие имени раздела
		name.append("]\nk = 1\n");
		/**
		 * Выполняем проверку отказа разбора по превышению глубины вложенности
		 *
		 * @note Отказ этот и есть починка: прежде настройка потолок обходила молча
		 */
		ASSERT_FALSE(document.parse(name));
		// Выполняем проверку того, что отказано именно по глубине вложенности
		ASSERT_EQ(document.error(), ini::error_t::DEPTH_EXCEEDED);
		// Выполняем проверку того, что вложенность в пределах потолка разбирается
		ASSERT_TRUE(document.parse("[р.п.п]\nk = 1\n")) << ini::message(document.error());
	}
	{
		// Настройки дерева настроек с запретом подразделов вовсе
		ini::document_t::settings_t settings;
		// Устанавливаем выделение подразделов знаком-разделителем
		settings.reader.subsections = ini::subsection_t::DELIMITED;
		// Запрещаем подразделы вовсе признаком дозволения
		settings.reader.nesting = false;
		// Объект дерева настроек с запретом подразделов
		ini::document_t document(settings);
		/**
		 * Выполняем проверку того, что запрет подразделов уцелел
		 *
		 * @note Половина эта стережёт запрет признаком: огранение потолком его не
		 *       касается вовсе, ибо признак не число. Прежде запрет выражался нулём
		 *       предела, и огранение обращало его в дозволение молча
		 */
		ASSERT_FALSE(document.parse("[р.п]\nk = 1\n"));
		// Выполняем проверку того, что раздел без подразделов при том разбирается
		ASSERT_TRUE(document.parse("[р]\nk = 1\n")) << ini::message(document.error());
	}
}
/**
 * @brief Проверка установки пустого значения по пути к нему
 *
 * @details Ход этот общий у кодеков рамки, а у настроек прежде отсутствовал вовсе.
 * Пустое значение от снятия свойства отлично: свойство остаётся налицо, а значения при
 * нём нет
 *
 * @note Запись «ключ =» у настроек законна: замерено 04.09.2026 - разбирается свойством с
 *       пустым значением и переписывается обратно ею же. У кодека TOML хода этого нет
 *       вовсе, и расхождение то законное: запись «a = » отвергается там самим стандартом
 *
 */
TEST(CodecIniDocument, SettingEmptyValueByPath){
	// Объект дерева настроек, текст разбирающего
	ini::document_t document;
	// Выполняем разбор текста настроек с двумя свойствами
	ASSERT_TRUE(document.parse("[р]\nk = значение\nсосед = 2\n")) << ini::message(document.error());
	// Выполняем проверку того, что значение свойства на месте
	ASSERT_EQ(document.at("/р/k").text(), "значение");
	// Выполняем установку пустого значения по пути к свойству
	ASSERT_TRUE(document.reset("/р/k")) << ini::message(document.error());
	/**
	 * Выполняем проверку того, что свойство осталось налицо
	 *
	 * @note Половина эта и есть весь смысл хода: снятие свойства дало бы недействительное
	 *       значение, и отличить одно от другого можно лишь этой проверкою
	 */
	ASSERT_TRUE(document.at("/р/k").valid());
	// Выполняем проверку того, что значения при свойстве не осталось
	ASSERT_TRUE(document.at("/р/k").text().empty());
	// Выполняем проверку того, что соседнее свойство не задето
	ASSERT_EQ(document.at("/р/сосед").text(), "2");
	/**
	 * Выполняем проверку того, что перезапись читается обратно тем же
	 *
	 * @note Половина эта стережёт запись: пустое значение, записанное неверно, обратным
	 *       чтением обратилось бы либо в отказ, либо в свойство пропавшее
	 */
	{
		// Получаем перезапись правленого дерева настроек
		...
		// Выполняем проверку того, что свойство круг пережило
		ASSERT_TRUE(back.at("/р/k").valid()) << written;
		// Выполняем проверку того, что пустота значения круг пережила
		ASSERT_TRUE(back.at("/р/k").text().empty()) << written;
	}
	/**
	 * Выполняем проверку того, что пустое значение от снятия свойства отлично
	 *
	 * @note Сличение это ловит подмену хода: сделай `reset()` то же, что `prune()`, - обе
	 *       половины выше прошли бы у отсутствующего свойства ровно так же
	 */
	ASSERT_TRUE(document.erase("сосед", "р"));
	// Выполняем проверку того, что снятое свойство недействительно
	ASSERT_FALSE(document.at("/р/сосед").valid());
	// Выполняем проверку того, что свойство с пустым значением при том действительно
	ASSERT_TRUE(document.at("/р/k").valid());
}
/**
 ...
 *
 */
TEST(CodecIniDocument, FailedSaveKeepsThePreviousContent){
	/**
	 ...
	 *       программой. Указано это у кодека CEF, где сборка от того и была потеряна
	 */
	#if defined(_WIN32) || defined(_WIN64)
	...
	const string filename(::unique("/tmp/awh-ini-failed-save.txt"));
	// Прежнее содержимое записываемого файла
	const string previous("[раздел]\nпрежнее = содержимое\nважное = очень\n");
	{
	 ...
	 *
	 * @note Заведён он затем, чтобы поверить ОГЛАШЕНИЕ отказа: отказ самой записи -
	 *       третий и последний путь отказа сохранения, и достаётся он одним лишь
	 *       пределом размера файла. Проверка `EveryFailedSaveIsAnnounced` берёт два
	 *       прочих - открытие да подмену цели, - а этот доказать может лишь здесь
	 */
	...
	{
		// Собираемый текст, предел размера заведомо превосходящий
		string bulky("[р]\n");
		/**
		 * Выполняем сборку текста многими строками, а не одною длинной
		 *
		 * @note Одна длинная строка отвергается разбором по пределу длины её, и проверка
		 *       падала бы на собственной подготовке, до записи не дойдя вовсе
		 */
		for(size_t i = 0; i < 20000; i++)
			// Выполняем добавление очередной записи настроек
			bulky.append("k").append(std::to_string(i)).append(" = значение\n");
		// Выполняем разбор собранного текста
		ASSERT_TRUE(document.parse(bulky)) << ini::message(document.error());
	}
	/**
	 ...
	 *          Указана хрупкость у кодеков JSON, XML и CSV, где нашлась та же хрупкость
	 *
	 */
	struct keeper_t {
	 ...
	 *
	 * @warning Прежде отказ этот молчал: запись отвечала ложью, а журнал оставался
	 *          пуст - потребитель не узнавал ни места отказа, ни рода его. Молчали
	 *          так все три кодека разом, тогда как JSON, XML и CSV оглашают всякий
	 *          свой отказ записи
	 */
	 ...
	 *          одно лишь дерево, второе пропустила бы молча. Указано это по кодекам JSON,
	 *          XML и CSV, где найден тот же пробел
	 */
	{
		{
			// Поток записи прежнего содержимого
		...
		ini::value_t lifted(document);
		/**
		 * Выполняем установку журнала собирающего владеющему значению
		 *
		 * @note Без того отказ записи ЗНАЧЕНИЯ поверялся бы одним признаком, а оглашение
		 *       его - ничем: ход записи у значения свой, и журнал ему ставится отдельно
		 */
		...
		ASSERT_FALSE(stored);
		/**
		 * Выполняем проверку того, что отказ записи значения ОГЛАШЁН
		 *
		 * @warning Ход записи у владеющего значения СВОЙ, кодом с записью дерева общего
...
/**
 * @brief Проверка попадания правки по записанному звену в то самое свойство
 *
 * @details Снятие отменяющих записей живёт у разбора пути, и пропуск его означает, что
 * правка заводит свойство НОВОЕ вместо правки существующего: молча и правдоподобно
 *
 * @warning Сличается ЧИСЛО детей раздела, а не одно лишь значение по пути: правка,
 *          заведшая свойство новое, отдала бы по пути верное значение и прошла бы
 *          проверку, сличающую только его
 *
 */
TEST(CodecIniDocument, EditingByAnEscapedTokenReachesThatVeryProperty){
	// Объект дерева настроек, текст разбирающего
	ini::document_t document;
	// Выполняем разбор текста настроек с именами, отменяющих записей требующими
	ASSERT_TRUE(document.parse("[р]\n~ = раз\na/b = два\nобычное = три\n")) << ini::message(document.error());
	// Выполняем проверку количества детей раздела до правки
	ASSERT_EQ(document.at("/р").size(), static_cast <size_t> (3));
	// Выполняем правку свойства по записанному звену знака отмены
	ASSERT_TRUE(document.set("/р/~0", ini::value_t(string("правка")))) << ini::message(document.error());
	// Выполняем проверку того, что правка легла по записанному звену
	ASSERT_EQ(document.at("/р/~0").text(), "правка");
	/**
	 * Выполняем проверку того, что свойства НОВОГО правка не завела
	 *
	 * @note Половина эта и есть весь порок: прежде тут заводилось свойство, буквально
	 *       названное `~0`, а свойство `~` оставалось нетронутым
	 */
	ASSERT_EQ(document.at("/р").size(), static_cast <size_t> (3));
	// Выполняем правку свойства по записанному звену косой черты
	ASSERT_TRUE(document.set("/р/a~1b", ini::value_t(string("вторая")))) << ini::message(document.error());
	// Выполняем проверку того, что правка легла по записанному звену
	ASSERT_EQ(document.at("/р/a~1b").text(), "вторая");
	// Выполняем проверку того, что свойства нового и эта правка не завела
	ASSERT_EQ(document.at("/р").size(), static_cast <size_t> (3));
	// Выполняем установку пустого значения по записанному звену
	ASSERT_TRUE(document.reset("/р/~0")) << ini::message(document.error());
	// Выполняем проверку того, что пустое значение легло по записанному звену
	ASSERT_TRUE(document.at("/р/~0").text().empty());
	// Выполняем проверку того, что и здесь свойства нового не завелось
	ASSERT_EQ(document.at("/р").size(), static_cast <size_t> (3));
}

/**
 * @brief Проверка замкнутости обхода дерева на повторном объявлении свойства
 *
 * @details Повтор имени есть место, где обход мог бы РАЗОМКНУТЬСЯ молча: выдай дерево
 *          двух одноимённых потомков - и одно звено вело бы лишь к одному из них, а
 *          второе значение стало бы недостижимым ничем. Проверка закрепляет, что дерево
 *          собирает повтор ОДНИМ потомком-перечнем, а внутрь ведут числовые звенья -
 *          тем же способом, каким держится последовательность у прочих кодеков
 *
 * @note Своей грамматики звена наречию отсюда не требуется: звено остаётся ИМЕНЕМ, и
 *       записи вида `port[1]` не заводится. Мерено щупом по всем четырём видам
 *       обращения с повтором, а не выведено чтением
 *
 * @note Половина вторая закрепляет, что плоский ход и путь несут РАЗНЫЕ обещания, а не
 *       противоречат друг другу: `get()` выдаёт объявление действующее, политикой
 *       назначенное, а `at()` выдаёт все объявления перечнем, ничего не теряя. Правка,
 *       сводившая снятие с политикой, была отменена именно по этой причине
 */
TEST(CodecIniDocument, RepeatedPropertyKeepsTraversalClosed){
	// Виды обращения с повторным объявлением свойства, обход сохраняющие
	const ini::duplicate_t modes[] = {
		ini::duplicate_t::FIRST, ini::duplicate_t::LAST, ini::duplicate_t::MERGE
	};
	/**
	 * Выполняем перебор всех видов обращения с повторным объявлением свойства
	 */
	for(const ini::duplicate_t mode : modes){
		// Объект дерева настроек, текст разбирающего
		ini::document_t document;
		// Настройки разбора текста настроек
		ini::document_t::settings_t settings;
		// Назначаем очередной вид обращения с повторным объявлением свойства
		settings.reader.duplicates = mode;
		// Выполняем разбор текста настроек с повторным объявлением свойства
		ASSERT_TRUE(document.parse("[net]\nport = 80\nport = 443\n", settings)) << ini::message(document.error());
		// Перечень имён свойств раздела
		const vector <string_view> keys = document.keys("net");
		/**
		 * Выполняем проверку того, что повтор выдан именем ОДНАЖДЫ
		 *
		 * @note Выдача имени дважды была бы ложью: потомок там один
		 */
		ASSERT_EQ(keys.size(), static_cast <size_t> (1));
		// Выполняем проверку выданного имени свойства раздела
		ASSERT_EQ(keys.front(), "port");
		// Выполняем проверку того, что звено имени ведёт к перечню
		ASSERT_TRUE(document.at("/net/port").is(ini::type_t::ARRAY));
		// Выполняем проверку количества значений перечня
		ASSERT_EQ(document.at("/net/port").size(), static_cast <size_t> (2));
		// Выполняем проверку достижимости первого объявления числовым звеном
		ASSERT_EQ(document.at("/net/port/0").text(), "80");
		// Выполняем проверку достижимости второго объявления числовым звеном
		ASSERT_EQ(document.at("/net/port/1").text(), "443");
	}
	// Объект дерева настроек, повтор отвергающего
	ini::document_t document;
	// Настройки разбора текста настроек
	ini::document_t::settings_t settings;
	// Назначаем прекращение разбора на повторном объявлении свойства
	settings.reader.duplicates = ini::duplicate_t::ERROR;
	/**
	 * Выполняем проверку отказа разбора текста настроек с повтором
	 *
	 * @note Вид этот обход не размыкает по той причине, что узлов повтора при нём не
	 *       заводится вовсе: разбор прекращается на самом повторе
	 */
	ASSERT_FALSE(document.parse("[net]\nport = 80\nport = 443\n", settings));
	// Объект дерева настроек, обещания ходов разводящего
	ini::document_t plain;
	// Выполняем разбор текста настроек с повторным объявлением свойства
	ASSERT_TRUE(plain.parse("[net]\nport = 80\nport = 443\n")) << ini::message(plain.error());
	/**
	 * Выполняем проверку того, что плоский ход выдаёт объявление действующее
	 *
	 * @note Умолчание есть `LAST`, и действующим объявлением им назначено последнее
	 */
	ASSERT_EQ(plain.get("port", "net"), "443");
	/**
	 * Выполняем проверку того, что путь выдаёт все объявления перечнем
	 *
	 * @note Расхождение это НЕ есть противоречие: ходы несут разные обещания, и
	 *       владеющее значение держит запись без потерь намеренно
	 */
	ASSERT_EQ(plain.at("/net/port").size(), static_cast <size_t> (2));
}

/**
 * @brief Проверка полного обхода дерева общим перечнем звеньев и розыском
 *
 * @details Перечень звеньев вместе с розыском обязан давать ЗАМКНУТЫЙ обход: всякое
 *          выданное звено годно для розыска, и обойденных узлов ровно столько, сколько
 *          их в дереве. Замкнутость держится отменяющими записями RFC 6901
 */
TEST(CodecIniDocument, EnumerationAndLookupFormAClosedTraversal){
	// Объект дерева настроек, текст разбирающего
	ini::document_t document;
	// Выполняем разбор текста настроек с разделом, повтором и трудными именами
	ASSERT_TRUE(document.parse(
		"простое = 1\n"
		"имя/с/чертой = 4\n"
		"тильда~тут = 5\n"
	...
	// Количество разысканных обходом узлов дерева настроек
	size_t walked = 0;
	/**
	 * @brief Метод обхода дерева настроек вглубь по звеньям
	 *
	...
	// Выполняем обход дерева настроек от корня его
	walk(walk, string());
	/**
	 * Выполняем проверку того, что обойдено ровно столько узлов, сколько их в дереве
	 *
	 * @note Счёт берётся с корнем: четыре пары корня, перечень повтора с двумя
	 *       значениями его да сам корень - восемь
	 */
	ASSERT_EQ(walked, static_cast <size_t> (8));
	// Перечень звеньев пути к детям корня дерева настроек
	const vector <string> keys = document.keys(string());
	// Выполняем проверку количества звеньев корня дерева настроек
	 ...
	 */
	ASSERT_EQ(keys.at(1), "имя~1с~1чертой");
	// Выполняем проверку записи знака отмены отменяющей записью
	ASSERT_EQ(keys.at(2), "тильда~0тут");
	// Выполняем проверку достижимости имени с косой чертой записанным звеном
	ASSERT_TRUE(document.at("/имя~1с~1чертой").valid());
	// Выполняем проверку недостижимости того же имени звеном сырым
	ASSERT_FALSE(document.at("/имя/с/чертой").valid());
}

/**
 * @brief Проверка того, что путь принимается только точным видом строки
 *
 * @details Застава общего перечня звеньев стоит по ТОЧНОМУ виду `string`, а не по
 *          преобразуемости, и проверка эта сторожит саму заставу, а не поведение хода:
 *          ослабь её до преобразуемости - и вызов `keys("srv")`, свойства раздела
 *          означающий, МОЛЧА стал бы означать путь
 *
 * @warning Проверка сторожит ЦЕНУ заставы наравне с её пользой: литерал уходит в ход
 *          прежний, и запись `keys("/net")` означает раздел с таким именем, а не путь, -
 *          в отличие от YAML и TOML, где литерал означает путь
 */
TEST(CodecIniDocument, PathEnumerationTakesOnlyTheStringType){
	// Объект дерева настроек, текст разбирающего
	ini::document_t document;
	// Выполняем разбор текста настроек с разделом
	ASSERT_TRUE(document.parse("[net]\nhost = a\nport = 80\n")) << ini::message(document.error());
	/**
	 * Выполняем проверку того, что литерал уходит в ход ПРЕЖНИЙ
	 *
	 * @note Вид возврата и есть доказательство: у прежнего хода он `string_view`
	 */
	static_assert(std::is_same <decltype(document.keys("net")), vector <string_view>>::value, "литерал ушёл в общий ход");
	// Выполняем проверку того, что вызов без довода ушёл в ход прежний
	static_assert(std::is_same <decltype(document.keys()), vector <string_view>>::value, "вызов без довода подменён");
	// Выполняем проверку того, что вызов с двумя доводами ушёл в ход прежний
	static_assert(std::is_same <decltype(document.keys("net", "")), vector <string_view>>::value, "вызов с подразделом подменён");
	// Выполняем проверку того, что точный вид строки выбирает ход общий
	static_assert(std::is_same <decltype(document.keys(string("/net"))), vector <string>>::value, "строка не выбрала общий ход");
	// Перечень имён свойств раздела, ходом прежним выданный
	const vector <string_view> named = document.keys("net");
	// Выполняем проверку количества имён свойств раздела
	ASSERT_EQ(named.size(), static_cast <size_t> (2));
	// Выполняем проверку первого выданного имени свойства раздела
	ASSERT_EQ(named.front(), "host");
	// Перечень звеньев пути, ходом общим выданный
	const vector <string> linked = document.keys(string("/net"));
	// Выполняем проверку того, что общий ход выдал те же звенья именами
	ASSERT_EQ(linked.size(), static_cast <size_t> (2));
	// Выполняем проверку первого выданного звена пути
	ASSERT_EQ(linked.front(), "host");
	// Выполняем проверку достижимости ребёнка выданным звеном
	ASSERT_EQ(document.at("/net/" + linked.front()).text(), "a");
}

/**
 * @brief Проверка спроса наличия значения по пути
 *
 * @details Спрос по пути поставлен ПЕРЕГРУЗКОЙ рядом с прежним спросом свойства в
 *          разделе, и застава его стоит по точному виду `string`: литерал уходит в ход
 *          прежний, и смысл существующих вызовов правкою не меняется
 *
 * @note Половина последняя закрепляет замкнутость: всякое звено, перечнем выданное,
 *       спрос проходит
 */
TEST(CodecIniDocument, PresenceByPath){
	// Объект дерева настроек, текст разбирающего
	ini::document_t document;
	// Выполняем разбор текста настроек с разделом и повтором
	ASSERT_TRUE(document.parse("[net]\nport = 80\nport = 443\n")) << ini::message(document.error());
	/**
	 * Выполняем проверку того, что литерал уходит в спрос ПРЕЖНИЙ
	 *
	 * @note Вид довода и есть доказательство: прежний ход спрашивает свойство в разделе,
	 *       и `has("port")` означает свойство `port` раздела без имени, а не путь `port`
	 */
	ASSERT_FALSE(document.has("port"));
	// Выполняем проверку прежнего спроса свойства в разделе именованном
	ASSERT_TRUE(document.has("port", "net"));
	// Выполняем проверку общего спроса наличия раздела по пути
	ASSERT_TRUE(document.has(string("/net")));
	// Выполняем проверку общего спроса наличия свойства по пути
	ASSERT_TRUE(document.has(string("/net/port")));
	// Выполняем проверку общего спроса наличия значения перечня по числовому звену
	ASSERT_TRUE(document.has(string("/net/port/1")));
	// Выполняем проверку отказа спроса по пути в никуда
	ASSERT_FALSE(document.has(string("/net/нет")));
	// Выполняем проверку отказа спроса по числовому звену за краем перечня
	ASSERT_FALSE(document.has(string("/net/port/2")));
	/**
	 * Выполняем проверку того, что всякое выданное звено спрос проходит
	 */
	for(const string & key : document.keys(string("/net")))
		// Выполняем проверку наличия значения по собранному пути
		ASSERT_TRUE(document.has("/net/" + key)) << key;
}

/**
 * @brief Проверка снятия значения по пути
 *
 * @details Снос по пути поставлен ПЕРЕГРУЗКОЙ рядом с прежним снятием свойства в
 *          разделе, и застава его стоит по точному виду `string`
 *
 * @note Числовое звено принимается наравне с именем: повтор имени ложится перечнем, и
 *       не прими снос номера - обход был бы замкнут для чтения и разомкнут для правки
 */
TEST(CodecIniDocument, RemovalByPath){
	// Объект дерева настроек, текст разбирающего
	ini::document_t document;
	// Выполняем разбор текста настроек с разделом, повтором и трудным именем
	ASSERT_TRUE(document.parse(
		"[net]\n"
		"host = anyks\n"
		"port = 80\n"
	...
	// Выполняем проверку снятия свойства по пути
	ASSERT_TRUE(document.erase(string("/net/host"))) << ini::message(document.error());
	// Выполняем проверку того, что снятое свойство более недостижимо
	ASSERT_FALSE(document.has(string("/net/host")));
	// Выполняем проверку того, что соседнее свойство сносом не задето
	ASSERT_TRUE(document.has(string("/net/port")));
	/**
	 * Выполняем проверку снятия значения перечня по числовому звену
	 *
	 * @note Половина эта и есть замкнутость правки: звено выдано перечнем звеньев, и оно
	 *       же принимается сносом
	 */
	ASSERT_TRUE(document.erase(string("/net/port/0"))) << ini::message(document.error());
	// Выполняем проверку того, что у перечня осталось одно значение
	ASSERT_EQ(document.at("/net/port").text(), "443");
	/**
	 * Выполняем проверку снятия свойства по записанному звену
	 *
	 * @note Хвост пути проходит снятие отменяющих записей: возьми его сырым - и снос бил
	 *       бы мимо, молча и правдоподобно
	 */
	ASSERT_TRUE(document.erase(string("/net/имя~1с~1чертой"))) << ini::message(document.error());
	// Выполняем проверку того, что свойство с трудным именем снято
	ASSERT_FALSE(document.has(string("/net/имя~1с~1чертой")));
	// Выполняем проверку отказа снятия по пути в никуда
	ASSERT_FALSE(document.erase(string("/net/нет")));
	/**
	 * Выполняем проверку снятия всего дерева пустым путём
	 *
	 * @note Пустой путь по описанию RFC 6901 означает ВЕСЬ документ, и пустой файл
	 *       настроек INI законен вполне
	 */
	ASSERT_TRUE(document.erase(string())) << ini::message(document.error());
	// Выполняем проверку того, что дерево опустело
	ASSERT_TRUE(document.empty());
}

/**
 * @brief Проверка заведения нового свойства правкою по пути
 *
 * @details Правка по пути обязана ЗАВОДИТЬ свойство, которого в дереве ещё нет, а не
 *          одну лишь перезаписывать существующее: ход этот общий у кодеков рамки, и
 *          YAML с TOML тем же вызовом новое заводят
 *
 * @warning Здесь стоял порок: успех правки судился по ГОДНОСТИ правимого узла, а
 *          заведение отдаёт свежий узел пустым - и пустой узел негоден. Оттого правка
 *          по пути, которого в дереве нет, отвергалась ВСЕГДА, и дерево умело лишь
 *          перезаписывать. Один и тот же вызов на трёх кодеках давал разный исход, и
 *          расхождение это молчало. Судьёю ныне служит наличие ВМЕСТИЛИЩА
 *
 * @note Половина последняя закрепляет, что звенья пути, кроме последнего, обязаны быть
 *       налицо: вместилищ по дороге не заводится, ибо опечатка в пути обращалась бы в
 *       новую ветвь дерева молча
 */
TEST(CodecIniDocument, SettingByPathCreatesTheMissingProperty){
	// Объект дерева настроек, текст разбирающего
	ini::document_t document;
	// Выполняем разбор текста настроек с разделом
	ASSERT_TRUE(document.parse("a = 1\n[net]\nport = 80\n")) << ini::message(document.error());
	// Выполняем проверку заведения нового свойства в корне дерева
	ASSERT_TRUE(document.set("/новое", ini::value_t(string("значение")))) << ini::message(document.error());
	// Выполняем проверку достижимости заведённого свойства
	ASSERT_EQ(document.at("/новое").text(), "значение");
	// Выполняем проверку того, что заведение прежнего свойства не задело
	ASSERT_EQ(document.at("/a").text(), "1");
	// Выполняем проверку заведения нового свойства в разделе
	ASSERT_TRUE(document.set("/net/host", ini::value_t(string("anyks")))) << ini::message(document.error());
	// Выполняем проверку достижимости заведённого свойства раздела
	ASSERT_EQ(document.at("/net/host").text(), "anyks");
	// Выполняем проверку того, что заведение соседа по разделу не задело
	ASSERT_EQ(document.at("/net/port").text(), "80");
	/**
	 * Выполняем проверку того, что успешная правка код прежнего отказа сбрасывает
	 */
	ASSERT_EQ(document.error(), ini::error_t::NONE);
	// Выполняем проверку перезаписи существующего свойства
	ASSERT_TRUE(document.set("/a", ini::value_t(string("иное")))) << ini::message(document.error());
	// Выполняем проверку того, что перезапись легла
	ASSERT_EQ(document.at("/a").text(), "иное");
	/**
	 * Выполняем проверку отказа правки по недостающему промежуточному звену
	 */
	ASSERT_FALSE(document.set("/нет/внутри", ini::value_t(string("значение"))));
	// Выполняем проверку того, что отказ дерева не задел
	ASSERT_FALSE(document.has(string("/нет")));
	/**
	 * Выполняем проверку того, что удачная правка следом код отказа сбрасывает
	 */
	ASSERT_TRUE(document.set("/a", ini::value_t(string("третье")))) << ini::message(document.error());
	// Выполняем проверку сброса кода отказа удачной правкой
	ASSERT_EQ(document.error(), ini::error_t::NONE);
}

/**
 * @brief Проверка того, что счёт детей корня копии дерева не строит
 *
 * @details Счёт вёлся снятием владеющего значения, то есть строил ПОЛНУЮ копию дерева
 *          ради одного числа. Ныне имена детей корня собираются по самому дереву
 *
 * @warning Имена собираются РАЗЛИЧНЫМИ намеренно: свойство верхнего уровня и раздел того
 *          же имени суть ОДИН ребёнок корня, а не два, - снятие сводит их вместе. Простой
 *          счёт свойств и разделов давал бы здесь два, и это единственное построение из
 *          одиннадцати, где счёты расходились
 */
TEST(CodecIniDocument, SizeCountsChildrenWithoutBuildingTheTree){
	// Построения, на которых счёты могли бы разойтись
	const pair <const char *, size_t> cases[] = {
	...
	/**
	 * Выполняем перебор всех проверяемых построений
	 */
	for(auto & one : cases){
		// Объект дерева настроек, текст разбирающего
		ini::document_t document;
		// Выполняем разбор очередного текста настроек
		ASSERT_TRUE(document.parse(one.first)) << one.first << ": " << ini::message(document.error());
		// Выполняем проверку счёта детей корня дерева настроек
		ASSERT_EQ(document.size(), one.second) << one.first;
		/**
		 * Выполняем проверку того, что счёт сходится со снятым значением
		 */
		ASSERT_EQ(document.size(), ini::value_t(document).size()) << one.first;
	}
}

/**
 * @brief Проверка оглашения всякого отказа записи в файл
 *
 * @details Отказ обязан быть ОГЛАШЁН, а не проглочен молча: потребитель, ложь без слова
 *          получивший, не узнает ни места отказа, ни рода его. Отказов у записи три -
 *          открытие временного файла, сама запись да подмена цели переименованием, - и
 *          оглашать себя обязан каждый
 *
 * @warning Поверяются ОБА хода записи - дерева и владеющего значения: пути у них
 *          порознь, и поверка, поставленная на один лишь ход, второй пропустила бы молча
 *
 * @note Прежде оглашал себя один лишь отказ открытия у дерева, а прочие молчали -
 *       и молчали у всех трёх кодеков разом, тогда как JSON, XML и CSV оглашают все.
 *       Кода FILE_NOT_WRITTEN не было вовсе, и он заведён этой правкой
 *
 * @note Пути отказов добываются не пределом размера файла, а самим устройством:
 *       каталога назначения нет вовсе - отказывает открытие; цель есть КАТАЛОГ -
 *       открытие проходит, а подмена цели переименованием отказывает
 *
 * @note Нашло пробел объединённое покрытие набора с ворошителем
 *
 */
TEST(CodecIniDocument, EveryFailedSaveIsAnnounced){
	// Собранные отчёты журнала
	vector <string> reports;
	// Разрешаем отложенный вывод: подписка кормится именно им
	awh::log::mode({awh::log::mode_t::DEFERRED});
	...
	ASSERT_TRUE(value.parse("a = 1\nb = 2\n")) << ini::message(value.error());
	/**
	 * Заводим каталог, целью записи ставимый
	 *
	 * @note Цель-каталог отказывает ПОДМЕНЕ, а не открытию: временный файл ложится
	 *       рядом с нею именем своим, и открытие его проходит
	 */
	const string folder = ::unique("./цель-записи-каталог-ini");
	/**
	 * Выполняем заведение каталога, целью записи ставимого
	 *
	 * @note Зовётся посредник переносимый, а не `::mkdir()` прямо: у MinGW под
	 *       MS Windows тот берёт один лишь путь, без прав доступа
	 */
	/**
	 * Выполняем уборку каталога, прежним прогоном оставленного
	 *
	 * @warning Уборка обязана идти ПЕРЕД заведением: заведение отвечает отказом на каталог
	 *          уже сущий, и остаток от прогона, прерванного либо порченого, красил бы
	 *          проверку в красный у сборки исправной. Зовётся снос ВМЕСТЕ С СОДЕРЖИМЫМ:
	 *          `::rmdir` берёт лишь каталог пустой, а внутри остатка может лежать и файл.
	 *          Замерено наведённой порчею 07.09.2026 - остаток валит проверку у всех трёх
	 *          кодеков. Указан разряд этот по кодекам JSON, XML и CSV
	 */
	removeDirectory(folder);
	ASSERT_TRUE(makeDirectory(folder));
	// Путь в каталоге несуществующем
	const string missing = "./нет-такого-каталога-ini/цель.ini";
	/**
	 * Проверяемые пути записи, отказом отвечающие
	 */
	const vector <pair <string, string>> refusals = {
		{missing, "открытие"}, {folder, "подмена"}
	};
	/**
	 * Выполняем перебор всех путей записи, отказом отвечающих
	 */
	for(auto & refusal : refusals){
		// Количество отчётов журнала до записи дерева
		size_t before = reports.size();
		// Выполняем проверку того, что запись дерева отвергнута
		ASSERT_FALSE(document.save(refusal.first)) << refusal.second;
		/**
		 * Выполняем проверку того, что отказ записи дерева оглашён
		 *
	 ...
	 *
	 * @note Половина эта стережёт уборку: временный файл, отказом брошенный, копился
	 *       бы рядом с целью при всякой неудаче
	 */
	{
		// Поток чтения временного файла
	...
	// Выполняем снос каталога, целью записи ставимого
	removeDirectory(folder);
}
 ...
 *
 */
TEST(CodecIniDocument, TheRefusalOfTheParsingReachesTheLog){
	// Собранные отчёты журнала
	vector <string> reports;
	// Разрешаем отложенный вывод: подписка кормится именно им
	awh::log::mode({awh::log::mode_t::DEFERRED});
	...
	ASSERT_FALSE(broken.parse("[раздел\nключ = 1\n"));
	/**
	...
	ASSERT_NE(broken.error(), ini::error_t::NONE);
}

/**
 * @brief Проверка отказа звена пути с номером, за разрядность вышедшим
 *
 * @details Звено числовое разбирается номером значения перечня, и разбор тот обязан
 *          ОТВЕРГАТЬ запись, в разрядность не вмещающуюся, а не падать на ней
 *
 * @warning Прежде номер брался `std::stoull`, а тот при переполнении КИДАЕТ
 *          `std::out_of_range`. Ходы правки объявлены `noexcept`, и исключение сносило
 *          ПРИЛОЖЕНИЕ ЦЕЛИКОМ - щуп по такому пути падал с кодом 134. Путь этот
 *          приходит от ПОТРЕБИТЕЛЯ, а не из разбора текста: поверка звена дозволяет
 *          одни цифры, но длины им не назначает вовсе, и повалить работу мог всякий,
 *          кто строит путь из чужой строки
 *
 * @note Нашло это объединённое покрытие набора с ворошителем: заход лежал непокрытым,
 *       а щуп, по нему пущенный, уронил приложение
 *
 */
TEST(CodecIniDocument, HugeIndexInThePathIsRefusedNotFatal){
	// Объект дерева настроек, текст разбирающего
	ini::document_t document;
	// Выполняем разбор текста с перечнем одноимённых объявлений
	ASSERT_TRUE(document.parse("[раздел]\nключ = 1\nключ = 2\n")) << ini::message(document.error());
	// Записанный текст дерева до всякой правки
	const string before = document.dump();
	/**
	 * Проверяемые звенья с номерами, за разрядность вышедшими
	 */
	const vector <string> huge = {
		"/раздел/ключ/99999999999999999999999999",
		"/раздел/ключ/18446744073709551616",
		"/раздел/ключ/999999999999999999999999999999999999999999"
	};
	/**
	 * Выполняем перебор всех звеньев с номерами непомерными
	 */
	for(auto & path : huge){
		/**
		 * Выполняем проверку того, что снятие по такому пути ОТВЕРГНУТО
		 *
		 * @note Ровно здесь и падало: до отказа дело не доходило вовсе
		 */
		ASSERT_FALSE(document.prune(path)) << path;
		// Выполняем проверку того, что перенос по такому пути отвергнут
		ASSERT_FALSE(document.graft(path, ini::value_t(string("x")))) << path;
		// Выполняем проверку того, что извлечение по такому пути отвергнуто
		ASSERT_FALSE(document.at(path).valid()) << path;
	}
	/**
	 * Выполняем проверку того, что дерево отказами не тронуто
	 *
	 * @note Половина эта стережёт целость: правка, до отказа успевшая тронуть дерево,
	 *       оставила бы его урезанным
	 */
	ASSERT_EQ(document.dump(), before);
	/**
	 * Выполняем проверку того, что номер годный отказом не задет
	 *
	 * @note Половина эта стережёт ширину отказа: разбор, всякий номер отвергающий,
	 *       прошёл бы перебор выше целиком
	 */
	ASSERT_TRUE(document.at("/раздел/ключ/0").valid());
}

/**
 * @brief Проверка отказа звена пути с негодной отменяющей записью
 *
 * @details Записей отменяющих у пути ровно две - `~0` знаком отмены да `~1` косой
 *          чертою, - и всякая иная отвергается, а не берётся дословно: имя `~x`,
 *          принятое как есть, стало бы ВТОРЫМ написанием того же места, и обход
 *          перестал бы быть замкнутым
 *
 * @warning Отвергать обязаны ВСЕ ходы разом - извлечение, обрезка и перенос: путь
 *          разбирают они порознь, своим снятием каждый, и поверка, на один лишь ход
 *          поставленная, прочие пропустила бы молча. У кодека TOML это подтвердилось
 *          порчею: отказ, у дерева снятый, поверки через `at()` не валил вовсе
 *
 * @note Пробел открыло объединённое покрытие набора с ворошителем: успешные записи
 *       `~0` и `~1` проверены были обе, а отказы снятия - тильда висячая да запись
...
TEST(CodecIniDocument, MalformedEscapeInThePathIsRefused){
	 ...
	 * Выполняем разбор текста настроек с именами, отменяющих записей требующими
	 *
	 * @warning Имя `а~x` здесь ОБЯЗАТЕЛЬНО: без него отказ снятия ничем себя не выдаёт -
	 *          путь `/раздел/а~x`, дословно взятый, ведёт в никуда так же, как и
	 *          отвергнутый, и порча снятия проверки не валит вовсе. С этим же именем
	 *          дословный путь НАХОДИТ свойство, и разница становится видна. Замерено
	 *          порчею: без имени порча значения давала 0 отказов, с ним - 3
	 */
	ASSERT_TRUE(document.parse("[раздел]\nа~x = 1\n~ = 2\nа = 3\n")) << ini::message(document.error());
	// Записанный текст дерева до всяких отказов
	const string before = document.dump();
	/**
	 * Проверяемые негодные звенья пути
	 */
	const vector <string> broken = {
		"/раздел/а~", "/раздел/~", "/раздел/а~x", "/раздел/~9", "/раздел~/а", "/раздел/а~b~"
	};
	/**
	 * Выполняем перебор всех негодных звеньев пути
	 */
	for(auto & path : broken){
		// Выполняем проверку того, что извлечение по негодному звену отвергнуто
		ASSERT_FALSE(document.at(path).valid()) << path;
		// Выполняем проверку того, что обрезка по негодному звену отвергнута
		ASSERT_FALSE(document.prune(path)) << path;
		// Выполняем проверку того, что отказ обрезки причину называет
		ASSERT_EQ(document.error(), ini::error_t::INVALID_KEY) << path;
		// Выполняем проверку того, что перенос по негодному звену отвергнут
		ASSERT_FALSE(document.graft(path, ini::value_t(string("x")))) << path;
		// Выполняем проверку того, что отказ переноса причину называет
		ASSERT_EQ(document.error(), ini::error_t::INVALID_KEY) << path;
	}
	/**
	 * Выполняем проверку того, что дерево отказами не тронуто
	 *
	 * @note Половина эта стережёт целость: обрезка, до отказа успевшая снять свойство,
	 *       оставила бы дерево урезанным
	 */
	ASSERT_EQ(document.dump(), before);
	/**
	 * Выполняем проверку того, что годные записи отказом не задеты
	 *
	 * @note Половина эта стережёт ширину отказа: поверка, тильду отвергающая без
	 *       разбора, прошла бы перебор выше целиком
	 */
	ASSERT_TRUE(document.at("/раздел/~0").valid());
	// Владеющее значение, тот же текст разбирающее
	ini::value_t value;
	// Выполняем разбор текста настроек владеющим значением
	ASSERT_TRUE(value.parse("[раздел]\nа~x = 1\n~ = 2\nа = 3\n")) << ini::message(value.error());
	/**
	 * Выполняем перебор всех негодных звеньев пути у владеющего значения
	 *
	 * @warning Ход у значения СВОЙ, кодом с ходом дерева общего не имеющий
	 */
	for(auto & path : broken)
		// Выполняем проверку того, что розыск владеющего значения отвергнут
		ASSERT_FALSE(value.at(path).valid()) << path;
	// Выполняем проверку того, что годная запись отказом не задета и здесь
	ASSERT_TRUE(value.at("/раздел/~0").valid());
	/**
	 * Выполняем проверку замкнутости обхода на имени, тильду несущем
	 *
	 * @details Имя `а~x` выдаётся обходом ЗАПИСАННЫМ - `а~0x`, - и путь тот к нему и
	 *          ведёт, а путь дословный отвергается. Тем обход и замкнут: у места одно
	 *          написание, а не два
	 */
	ASSERT_TRUE(document.at("/раздел/а~0x").valid());
	ASSERT_TRUE(value.at("/раздел/а~0x").valid());
	// Выполняем проверку того, что обход выдаёт имя записанным, а не дословным
	{
		// Перечень звеньев пути к детям раздела
		const vector <string> names = document.keys(string("/раздел"));
		// Выполняем проверку того, что дословного имени обход не выдаёт
		ASSERT_EQ(::std::find(names.begin(), names.end(), "а~x"), names.end());
		// Выполняем проверку того, что записанное имя обход выдаёт
		ASSERT_NE(::std::find(names.begin(), names.end(), "а~0x"), names.end());
	}
}

/**
 * @brief Проверка постановки значения перечня по номеру звена
 *
 * @details Свойство, объявленное в разделе дважды, есть перечень одноимённых
 *          объявлений, и звеном пути к его значениям служит НОМЕР. Постановка по
 *          номеру годному правит именно то значение, а по номеру за краем отвергается
 *
 * @warning Половины эти неразделимы: поверка, одну лишь годную постановку берущая,
 *          прошла бы и с заставою края снесённой, а поверка одного отказа - с
 *          постановкой, отвергающей всякий номер
 *
 * @note Пробел открыло объединённое покрытие набора с ворошителем: ни постановка по
 *       номеру, ни отказ её по краю не были задеты ни разу
 *
 */
TEST(CodecIniDocument, SetsByArrayIndexWithinBounds){
	// Объект дерева настроек, текст разбирающего
	ini::document_t document;
	// Выполняем разбор текста настроек с перечнем одноимённых объявлений
	ASSERT_TRUE(document.parse("[раздел]\nключ = 1\nключ = 2\n")) << ini::message(document.error());
	// Выполняем проверку количества значений перечня
	ASSERT_EQ(document.keys(string("/раздел/ключ")).size(), static_cast <size_t> (2));
	// Выполняем постановку значения перечня по номеру годному
	ASSERT_TRUE(document.graft("/раздел/ключ/0", ini::value_t(string("новое")))) << ini::message(document.error());
	/**
	 * Выполняем проверку того, что правлено именно ТО значение перечня
	 *
	 * @note Половина эта отделяет правку по номеру от правки по имени: постановка,
	 *       номер за имя принявшая, завела бы третье объявление либо затёрла оба
	 */
	ASSERT_EQ(document.dump(), "[раздел]\nключ = новое\nключ = 2\n");
	// Записанный текст дерева до отказов
	const string before = document.dump();
	/**
	 * Выполняем перебор всех номеров, за край перечня выходящих
	 */
	for(auto & path : {"/раздел/ключ/2", "/раздел/ключ/5", "/раздел/ключ/100"}){
		// Выполняем проверку того, что постановка по номеру за краем отвергнута
		ASSERT_FALSE(document.graft(path, ini::value_t(string("х")))) << path;
		// Выполняем проверку того, что отказ причину называет
		ASSERT_EQ(document.error(), ini::error_t::INVALID_KEY) << path;
	}
	// Выполняем проверку того, что дерево отказами не тронуто
	ASSERT_EQ(document.dump(), before);
...
/**
 * @brief Проверка предела глубины обращений, настройкою заданного
 *
 * @details Значение свойства вправе ссылаться на другое свойство, а то на третье - и
 * цепь эта ограничена настройкою. Обездвиживание настройки умолчанием набора не роняло
 * вовсе: цепи глубиной свыше умолчания набор не подавал, а иного предела не ставил никто,
 * и подмена настройки числом 32 проходила незамеченной
 *
 * @note Образцы подобраны РАЗЛИЧАЮЩИЕ: одна и та же цепь под тремя пределами даёт три
 *       разных итога, и предел, к числу прибитый намертво, ловится любым из них
 *
 */
TEST(CodecIniDocument, ReferenceDepthLimitIsObeyed) {
	/**
	 * @brief Сборка цепи обращений заданной глубины
	 *
	 * @param depth глубина собираемой цепи обращений
	 * @return      собранный текст настроек
	 *
	 */
	const auto chain = [](const uint32_t depth) noexcept -> string {
		// Собираемый текст настроек
		string result = "[s]\na0 = дно\n";
		/**
		 * Выполняем перебор звеньев собираемой цепи
		 */
		for(uint32_t i = 1; i <= depth; i++)
			// Выполняем добавление очередного звена цепи обращений
			result.append("a" + std::to_string(i) + " = %(a" + std::to_string(i - 1) + ")s\n");
		// Выводим собранный текст настроек
		return result;
	};
	/**
	 * Проверяемые пределы вместе с глубиной цепи и ожидаемым итогом разбора
	 */
	const vector <tuple <uint32_t, uint32_t, bool>> samples = {
		/**
		 * Предел, цепи РОВНО по мерке
		 *
		 * @note Мерка эта в таблице и отсутствовала, оттого смещение предела на единицу
		 *       жило незамеченным: цепь из трёх обращений требовала предела четыре, а
		 *       поле зовётся «наибольшей глубиной вложенности обращений». Границу берёт
		 *       ровно этот заход, и без него соседние двa лишь охватывают её с боков
		 */
		{3,  3, true},
		// Предел, на единицу цепи не достающий
		{2,  3, false},
		// Предел, цепи из одного обращения ровно достаточный
		{1,  1, true},
		// Предел, цепи из одного обращения не достающий
		{0,  1, false},
		// Предел, цепи достаточный
		{4,  3, true},
		// Тот же предел на цепи, ему не по мерке
		{4,  5, false},
		// Предел умолчальный
		{32, 5, true}
	};
	/**
	 * Выполняем перебор всех проверяемых пределов
	 */
	for(const auto & item : samples){
		// Настройки дерева настроек
		ini::document_t::settings_t settings;
		// Устанавливаем вид записи обращения к значению
		settings.references = ini::reference_t::PYTHON;
		// Устанавливаем предел глубины обращений
		settings.maxReferenceDepth = std::get <0> (item);
		// Собираемое дерево настроек
		ini::document_t document(settings);
		// Собранный текст настроек проверяемой глубины
		const string source = chain(std::get <1> (item));
		// Выполняем проверку итога разбора текста настроек
		ASSERT_EQ(document.parse(source), std::get <2> (item))
			<< "предел " << std::get <0> (item) << " глубина " << std::get <1> (item);
		/**
		 * Если разбор текста настроек удался
		 */
		if(std::get <2> (item))
			// Выполняем проверку разрешённого обращения последнего звена цепи
			ASSERT_EQ(document.get("a" + std::to_string(std::get <1> (item)), "s"), "дно")
				<< "предел " << std::get <0> (item);
		/**
		 * Если разбор текста настроек отвергнут
		 */
		else
...
/**
 * @brief Проверка передачи знака-разделителя и признака глобальных свойств
 *
 * @details Знак, разделяющий имя раздела от имени подраздела, и признак свойств, до
 * первого раздела стоящих, берутся настройками чтения, а служат ОБОИМ путям - и разбору
 * текста, и правке дерева с записью его. Замер обездвиживания по исходникам нашёл дерево
 * не задетым: настройки эти проверены у чтения и у записи порознь, а путь через дерево не
 * ходил ни разу
 *
 * @note Знак-разделитель сличается ОБЕИМИ сторонами: имя раздела, знак этот несущее,
 *       отвергается, а имя с иным знаком препинания - принимается. Одной стороны мало:
 *       запрет, к точке прибитый намертво, прошёл бы её незамеченным
 *
 */
TEST(CodecIniDocument, DelimiterAndGlobalReachTheTreeWork) {
	/**
	 * Выполняем проверку знака-разделителя имени подраздела
	 */
	{
		/**
		 * Проверяемые знаки-разделители вместе со знаком, разделителем НЕ являющимся
		 */
		const vector <pair <char, char>> samples = {
			// Разделитель точкою, двоеточие дозволено
			{'.', ':'},
			// Разделитель двоеточием, точка дозволена
			{':', '.'}
		};
		/**
		 * Выполняем перебор всех проверяемых знаков-разделителей
		 */
		for(const auto & item : samples){
			// Настройки дерева настроек
			ini::document_t::settings_t settings;
			// Устанавливаем знак-разделитель имени подраздела
			settings.reader.delimiter = item.first;
			...
			// Выполняем разбор текста настроек
			ASSERT_TRUE(document.parse("[раздел]\na = 1\n")) << ini::message(document.error());
			// Выполняем проверку отказа правки именем раздела со знаком-разделителем
			ASSERT_FALSE(document.set("b", "2", string("раз") + item.first + "дел")) << item.first;
			// Выполняем проверку выданного кода отказа правки
			ASSERT_EQ(document.error(), ini::error_t::INVALID_SECTION) << item.first;
			// Выполняем проверку принятия имени раздела со знаком, разделителем не являющимся
			ASSERT_TRUE(document.set("c", "3", string("раз") + item.second + "дел"))
				<< item.second << ": " << ini::message(document.error());
		}
	}
	/**
	 * Выполняем проверку признака свойств, до первого раздела стоящих
	 */
	{
		/**
		 * Выполняем проверку записи глобального свойства при дозволении их
		 */
		{
			// Настройки дерева настроек
			ini::document_t::settings_t settings;
			// Устанавливаем дозволение свойств, до первого раздела стоящих
			settings.reader.global = true;
			// Собираемое дерево настроек
			ini::document_t document(settings);
			// Выполняем разбор текста настроек
			ASSERT_TRUE(document.parse("[s]\nb = 2\n")) << ini::message(document.error());
			// Выполняем постановку свойства вне разделов
			ASSERT_TRUE(document.set("a", "1", "")) << ini::message(document.error());
			// Выполняем проверку собранной перезаписи дерева настроек
			ASSERT_EQ(document.dump(), "a = 1\n[s]\nb = 2\n") << document.dump();
		}
		/**
		 * Выполняем проверку отказа ПРАВКИ глобальным свойством при запрете их
		 *
		 * @details Отказ приходит правкою, а не записью: записать такое свойство нечем, и
		 *          дерево, его принявшее, обращалось бы в незаписываемое целиком - выдача
		 *          текста отвечала пустотою, а прежнее содержимое уходило вместе с нею.
		 *          Правка же отвечала успехом, и потребитель узнавал о потере при записи
		 *
		 * @note Закрепляется и СОХРАННОСТЬ дерева: отвергнутая правка обязана оставить
		 *       прежнее содержимое нетронутым и записываемым
		 */
		{
			// Настройки дерева настроек
			ini::document_t::settings_t settings;
			// Устанавливаем запрет свойств, до первого раздела стоящих
			settings.reader.global = false;
			// Собираемое дерево настроек
			ini::document_t document(settings);
			// Выполняем разбор текста настроек
			ASSERT_TRUE(document.parse("[s]\nb = 2\n")) << ini::message(document.error());
			// Выполняем проверку отказа постановки свойства вне разделов
			ASSERT_FALSE(document.set("a", "1", ""));
			// Выполняем проверку выданного кода отказа правки
			ASSERT_EQ(document.error(), ini::error_t::KEY_OUTSIDE_SECTION);
			// Выполняем проверку того, что дерево отказом не тронуто и записывается
			ASSERT_EQ(document.dump(), "[s]\nb = 2\n") << document.dump();
		}
	}
}

/**
 * @brief Проверка знаков разделителя, правкой дерева признаваемых запретными
 *
 * @details Знак, разделяющий имя свойства от значения, запретен внутри самого имени:
 * читающий разрезал бы пару по нему и прочёл половину именем. Набор знаков этих задаётся
 * настройкою, и правка дерева обязана держаться её, а не знака равенства прибитого. Замер
 * обездвиживания по исходникам нашёл путь этот не задетым
 *
 * @note Наречия перебираются ОБОИМИ ладами: знак, разделителем назначенный, имя отвергает,
 *       а знак, разделителем не являющийся, - дозволен. Проверка тем и отделяет настройку
 *       от запрета, к знаку прибитого намертво
 *
 */
TEST(CodecIniDocument, SeparatorSetReachesTheEditing) {
	/**
	 * Проверяемые наборы разделителей вместе с текстом наречия их
	 */
	const vector <tuple <ini::separator_t, string, string, string>> samples = {
		// Разделителем служит знак равенства
		{ini::separator_t::EQUALS, "[s]\na = 1\n", "и=мя", "и:мя"},
		// Разделителем служит двоеточие
		{ini::separator_t::COLON,  "[s]\na : 1\n", "и:мя", "и=мя"}
	};
	/**
	 * Выполняем перебор всех проверяемых наборов разделителей
	 */
	for(const auto & item : samples){
		// Настройки дерева настроек
		ini::document_t::settings_t settings;
		// Устанавливаем признаваемые знаки разделителя имени и значения
		settings.reader.separators = std::get <0> (item);
		// Собираемое дерево настроек
		ini::document_t document(settings);
		// Выполняем разбор текста настроек наречия
		ASSERT_TRUE(document.parse(std::get <1> (item)))
			<< std::get <1> (item) << ": " << ini::message(document.error());
		// Выполняем проверку отказа правки именем, разделитель несущим
		ASSERT_FALSE(document.set(std::get <2> (item), "1", "s")) << std::get <2> (item);
		// Выполняем проверку выданного кода отказа правки
		ASSERT_EQ(document.error(), ini::error_t::INVALID_KEY) << std::get <2> (item);
		// Выполняем проверку принятия имени со знаком, разделителем не являющимся
		ASSERT_TRUE(document.set(std::get <3> (item), "2", "s"))
			<< std::get <3> (item) << ": " << ini::message(document.error());
	}
}

/**
 * @brief Проверка передачи признака обвязки значения от дерева к записи
 *
 * @details Признак отбрасывания пробельной обвязки читающим служит записи защитою: зная,
 * что читающий обвязку отбросит, запись обязана её оградить - кавычками либо управляющей
 * последовательностью, - а не зная, вправе писать значение как есть. Замер обездвиживания
 * по исходникам нашёл передачу этого признака через дерево не задетой
 *
 * @note Расхождение видно лишь при читающем, кавычек не снимающем и последовательностей не
 *       признающем: оградить обвязку тогда нечем, и защита оборачивается ОТКАЗОМ записи,
 *       тогда как без защиты то же значение уходит в текст дословно
 *
 */
TEST(CodecIniDocument, TrimFlagReachesTheWriter) {
	/**
	 * Выполняем проверку записи значения с пробельной обвязкой при отбрасывании её
	 */
	{
		// Настройки дерева настроек
		ini::document_t::settings_t settings;
		// Устанавливаем отбрасывание пробельной обвязки значения читающим
		settings.reader.trim = true;
		// Устанавливаем удержание кавычек частью значения
		settings.reader.quotes = ini::quote_t::KEEP;
		// Снимаем признание управляющих последовательностей
		settings.reader.escapes = false;
		// Собираемое дерево настроек
		ini::document_t document(settings);
		// Выполняем разбор текста настроек
		ASSERT_TRUE(document.parse("[s]\na = 1\n")) << ini::message(document.error());
		// Выполняем постановку значения с пробелом на конце
		ASSERT_TRUE(document.set("b", "пробелы ", "s")) << ini::message(document.error());
		/**
		 * Выполняем проверку того, что запись такого дерева отвергнута
		 *
		 * @note Отказ верен: обвязку читающий отбросит, оградить её нечем - молчаливая
		 *       запись потеряла бы пробел
		 */
		ASSERT_TRUE(document.dump().empty()) << document.dump();
	}
	/**
	 * Выполняем проверку записи того же значения при сохранении обвязки читающим
	 */
	{
		// Настройки дерева настроек
		ini::document_t::settings_t settings;
		// Снимаем отбрасывание пробельной обвязки значения читающим
		settings.reader.trim = false;
		// Устанавливаем удержание кавычек частью значения
		settings.reader.quotes = ini::quote_t::KEEP;
		// Снимаем признание управляющих последовательностей
		...
		// Выполняем постановку значения с пробелом на конце
		ASSERT_TRUE(document.set("b", "пробелы ", "s")) << ini::message(document.error());
		// Выполняем проверку того, что значение ушло в текст дословно
		ASSERT_EQ(document.dump(), "[s]\na = 1\nb = пробелы \n") << document.dump();
	}
}

/**
 * @brief Проверка извлечения числа видами ЯЗЫКА
 *
 * @details Виды языка и виды разрядные равными не являются: у macOS `int64_t` есть
 *          `long long`, а у Linux - `long`. Перечень порождений одними разрядными
 *          видами негоден на ОБЕИХ системах, только по-разному, и потребитель,
 *          извлекающий число видом `long` либо `size_t`, оставался ни с чем
 *
 * @note Проверка эта - зеркало проверки записи: там та же болезнь была починена
 *       перечнем видов языка, а слой извлечения остался при разрядных
 *
 * @warning Отказ здесь бывает ДВУХ видов, и оба сборкою: у обобщённого приёма -
 *          отсутствие порождения, у необобщённого - отсутствие подходящего приёма
 *          вовсе. Ни один из них проверкою на исполнении не ловится
 *
 */
TEST(CodecIniDocument, LanguageKindsExtractTheNumber){
	...
	long wide = 0;
	// Выполняем проверку извлечения числа видом языка со знаком
	ASSERT_TRUE(document.value(wide, "port", "server"));
	// Выполняем проверку извлечённого числа
	ASSERT_EQ(wide, 8080);
	// Извлекаемое число видом размера
	size_t size = 0;
	// Выполняем проверку извлечения числа видом размера
	ASSERT_TRUE(document.value(size, "port", "server"));
	// Выполняем проверку извлечённого числа
	ASSERT_EQ(size, static_cast <size_t> (8080));
	// Извлекаемое число видом языка широчайшим
	long long widest = 0;
	// Выполняем проверку извлечения числа видом языка широчайшим
	ASSERT_TRUE(document.value(widest, "port", "server"));
	// Выполняем проверку извлечённого числа
	ASSERT_EQ(widest, 8080);
	// Извлекаемое число видом языка без знака
	unsigned long unsigned_ = 0;
	// Выполняем проверку извлечения числа видом языка без знака
	ASSERT_TRUE(document.value(unsigned_, "port", "server"));
	// Выполняем проверку извлечённого числа
	ASSERT_EQ(unsigned_, 8080UL);
}

/**
 * @brief Проверка отказа разбора числа видом ЯЗЫКА
 *
 * @details Отказ разбора у видов языка идёт своею дорогою: разбор ведётся видом
 *          разрядным, и отказ его надлежит выдать наружу, а не подменить нулём
 *
 * @note Проверка держит и то, что приёмник при отказе НЕ ЗАТИРАЕТСЯ: перенос
 *       разобранного идёт лишь при успехе, и прежнее содержимое приёмника отказ
 *       оставляет нетронутым
 *
 */
TEST(CodecIniDocument, LanguageKindsRefuseTheNonNumber){
	// Выполняем создание объекта документа
	ini::document_t document;
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse("[server]\nhost = локальный\n")) << static_cast <uint32_t> (document.error());
	...
	long wide = 1250;
	// Выполняем проверку отказа разбора числа видом языка со знаком
	ASSERT_FALSE(document.value(wide, "host", "server"));
	// Выполняем проверку сохранности прежнего содержимого приёмника
	ASSERT_EQ(wide, 1250);
	// Приёмник вида размера с прежним содержимым
	size_t size = 77;
	// Выполняем проверку отказа разбора числа видом размера
	ASSERT_FALSE(document.value(size, "host", "server"));
	// Выполняем проверку сохранности прежнего содержимого приёмника
	ASSERT_EQ(size, static_cast <size_t> (77));
}

/**
 * @brief Проверка снятия пробельной обвязки при разборе числа
 *
 * @details Разбор числа снимает пробелы с ОБЕИХ сторон значения. Обвязка ведущая
 *          снимается и у значения голого, а хвостовая до разбора числа доходит лишь у
 *          значения в кавычках: у голого её снимает читающий прежде
 *
 * @note Место это было слепо для карты общего набора: приём `trim` одноимён приёмам
 *       иных кодеков, и записи покрытия его уходили к соседу. Найдено картою СТЕНДА,
 *       где соседей по имени нет
 *
 */
TEST(CodecIniDocument, NumberParsingTrimsBothSides) {
	// Выполняем создание объекта документа
	ini::document_t document;
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse("[s]\nголое = 42   \nв_кавычках = \"  7  \"\n"))
	 << static_cast <uint32_t> (document.error());
	// Разбираемое число значения голого
	int64_t bare = 0;
	// Выполняем проверку разбора числа значения голого
	ASSERT_TRUE(document.value(bare, "голое", "s"));
	// Выполняем проверку разобранного числа
	ASSERT_EQ(bare, 42);
	// Разбираемое число значения в кавычках
	int64_t quoted = 0;
	// Выполняем проверку разбора числа значения в кавычках
	ASSERT_TRUE(document.value(quoted, "в_кавычках", "s"));
	// Выполняем проверку разобранного числа
	ASSERT_EQ(quoted, 7);
}

/**
 * @brief Проверка отказа чтения по пути, на каталог указывающему
 *
 * @details Каталог ОТКРЫВАЕТСЯ успешно, а читается признаками конца и отказа - теми же,
 *          какими отзывается файл пустой. Без распознавания чтение отвечало УСПЕХОМ и
 *          отдавало документ без записей: потребитель, каталог подавший по ошибке,
 *          получал «загружено» и пустоту
 *
 * @note Замерено 07.09.2026 подачею пути `/tmp`: YAML, TOML и INI отвечали признаком
 *       успеха при коде отказа НУЛЕВОМ, тогда как JSON, XML и CSV отвергали
 *
 ...
 *
 * @note Проверка честна на ОБЕИХ системах: открытие каталога законно успешно и у macOS,
 *       и у Linux, а отказ приходит лишь на чтении. Замерено 07.09.2026 одной программой
 *       на macOS 15 arm64 и на Astra glibc 2.36: `fopen("/tmp","rb")` успех, `fread` даёт
 *       нуль при `errno = 21` (Is a directory) и признаке отказа потока
 *
 * @warning Прежде здесь стояло, будто у Linux `fopen` отказывает сразу кодом EISDIR и
 *          проверка там зеленеет по чужой причине. Довод тот был назван ПО ПАМЯТИ и
 *          опровергнут замером. Пустое чтение и вправду неотличимо от файла пустого -
 *          оттого распознавание и стоит `stat` с `S_ISDIR` ДО открытия, а не отдачею
 *          потока: у MS Windows каталог не открывается вовсе, и проверка после открытия
 *          там мертва
 *
 */
TEST(CodecIniDocument, DirectoryIsRefusedNotLoaded){
	// Выполняем создание объекта документа
	ini::document_t document;
 ...
 *
 * @note Замерено 09.09.2026 на стенде MSYS2 при ACP=1251: до правки запись «настройки.ini»
 *       заводила на диске «РЅР°СЃС‚СЂРѕР№РєРё.ini», задуманного имени не было вовсе, а
 ...
 *
 */
TEST(CodecIniDocument, TheCyrillicFileNameLandsOnTheDiskAsIntended) {
	// Собираемое дерево настроек
	ini::document_t document;
	// Выполняем разбор текста настроек
	ASSERT_TRUE(document.parse("[раздел]\nключ=значение\n"));
	// Путь записи с кириллическим именем
	const string filename = string(::testing::TempDir()) + ::unique("настройки-кириллица.ini");
	// Выполняем запись дерева настроек в файл
	ASSERT_TRUE(document.save(filename)) << ini::message(document.error());
	/**
	 * Для операционной системы MS Windows
	 */
	#if defined(_WIN32) || defined(_WIN64)
	...
	ASSERT_TRUE(back.load(filename)) << ini::message(back.error());
	// Выполняем проверку того, что содержимое пережило круг
	ASSERT_EQ(back.get("ключ", "раздел"), "значение");
	// Выполняем снятие записанного файла
	dropFile(filename);
}

/**
 * @brief Проверка того, что выразимость значения задаётся наречием записи
 *
 * @details Значение с переводом строки, возвратом каретки либо нулём записывается
 *          управляющею последовательностью при взведённом `escapes` и записано быть не
 *          может при выключенном. Наречие решает, а не значение само
 *
 * @warning При невыразимости сборка отвергается ЦЕЛИКОМ: выдача пуста, а причина названа
 *          кодом. Частичной выдачи быть не может - текст настроек либо читается обратно
 *          без потерь, либо не годен вовсе. Половина проверки стережёт именно это: без
 *          неё «пусто» и «отказ» были бы неразличимы
 *
 * @note Правка значения наречием НЕ поверяется, в отличие от правки имени: наречие вольно
 *       смениться между правкой и сборкой. Проверка держит и это - `set()` принимает
 *       невыразимое ныне значение, и оно становится выразимым от одной настройки
 *
 * @note Проверка эта СОСЕДСТВУЕТ с `CodecIniDocument.WriteFailure`, а не заменяет её:
 *       та меряет один знак и путь через явные настройки записи `text(settings)`, эта -
 *       три знака и путь через настройки самого дерева, да ещё называет код отказа
 *       поимённо, тогда как соседка довольствуется «не NONE»
 *
 * @warning Заведена она по донесению о «дефекте», пришедшему трижды от трёх разных
 *          читателей кода. Все трое, и я в их числе, соседку НЕ НАШЛИ: розыск шёл по
 *          `INVALID_CHARACTER` и по именам со словами escape, newline, invalid, - а
 *          соседка зовётся `WriteFailure` и кода не называет. Отсюда правило: разыскивая
 *          закрепление, искать надлежит по СОДЕРЖАНИЮ утверждений, а не по написанию
 *          имени, - иначе отсутствие находки выдаст себя за отсутствие проверки
 *
 */
TEST(CodecIniDocument, TheExpressibilityOfAValueIsSetByTheDialect) {
	// Знаки, записать которые без управляющей последовательности нечем
	const vector <string> values = {string("a\nb"), string("a\rb"), string("a\0b", 3)};
	/**
	 * Выполняем перебор всех невыразимых значений
	 */
	for(const auto & value : values){
		/**
		 * Выполняем проверку отказа сборки при выключенной записи последовательностей
		 */
		{
			// Настройки дерева с выключенной записью управляющих последовательностей
			ini::document_t::settings_t settings;
			// Выполняем выключение записи управляющих последовательностей
			settings.reader.escapes = false;
			// Дерево настроек, наречием без управляющих последовательностей собираемое
			ini::document_t document(settings);
			// Выполняем проверку того, что правка значение ПРИНИМАЕТ
			ASSERT_TRUE(document.set("ключ", value, "")) << ini::message(document.error());
			// Выполняем проверку того, что сборка отвергнута целиком
		...
		/**
		 * Выполняем проверку успеха сборки при взведённой записи последовательностей
		 */
		{
			// Настройки дерева с взведённой записью управляющих последовательностей
			ini::document_t::settings_t settings;
			// Выполняем взведение записи управляющих последовательностей
			settings.reader.escapes = true;
			// Дерево настроек, наречием с управляющими последовательностями собираемое
			ini::document_t document(settings);
			// Выполняем проверку того, что правка значение принимает
			ASSERT_TRUE(document.set("ключ", value, "")) << ini::message(document.error());
			// Выполняем проверку того, что сборка удалась
			ASSERT_FALSE(document.dump().empty()) << ini::message(document.error());
			// Выполняем проверку того, что отказа не случилось
			ASSERT_EQ(document.error(), ini::error_t::NONE);
		}
	}
}
