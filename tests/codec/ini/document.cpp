 ...
 *
 * @details Пределы длины - `maxLine` и `maxName` - нулём СНИМАЮТСЯ, а пределы
 *          вложенности - `maxDepth` и `maxContinuation` - нулём ЗАПРЕЩАЮТ её вовсе.
 *          Уклады эти разные, а соседство четырёх настроек одной записи читается
 *          единым, и правку туда внесут скорее по недоразумению
 *
...
TEST(CodecIniDocument, ZeroConventionOfNumericLimits) {
	{
		...
		settings.reader.subsections = ini::subsection_t::DELIMITED;
		// Запрещаем вложенность подразделов вовсе
		settings.reader.maxDepth = 0;
		// Выполняем проверку того, что нуль подраздел отвергает, а не снимает предел
		ASSERT_FALSE(ini::document_t(::logger()).parse("[a.b]\nk = v\n", settings));
		// Выполняем проверку того, что раздел без подраздела тем же нулём принимается
