...
PRESENT=$(ls "$ROOT/src/codec/yaml"/*.cpp | while read -r FILE; do basename "$FILE" .cpp; done | sort | tr '\n' ' ')
...
#
# @note Часть «replace» изъята 11.09.2026: модуль «codec/replace» удалён, а
...
#       повтором имён
$COMPILER $OPTIONS $OBJECTS -L"$GTEST/lib" -lgtest -lgtest_main -pthread $SYSTEM_LIBS -lz -o "$OUTPUT/yaml-tests"
