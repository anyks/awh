...
TEST(CodecYamlCommon, Messages) {
	...
	for(uint32_t code = 0; code <= static_cast <uint32_t> (yaml::error_t::SETTINGS_ALREADY_APPLIED); code++){
		 ...
		 *
		 * @warning Строка сличения обязана слово в слово равняться на ту, какую отдаёт
		 *          `message()` отказом неизвестным. Прежде здесь стояло «неизвестный код
		 *          отказа» - описания перевели на английский, а сличение осталось русским,
		 *          и утверждение это ОТКАЗАТЬ не могло вовсе: кодек такой строки не отдаёт
		 *          ни при каком коде
		 */
		ASSERT_STRNE(message, "unknown error");
		/**
	 ...
	 *
	 * @details Поверка эта обращает предупреждение о пределе в правило, само себя
	 *          блюдущее: заведи кто код за пределом - описание ему будет отведено, и
	 *          «unknown error» здесь не выйдет, а проверка отказом потребует предел
	 *          подвинуть. Без неё предупреждение остаётся одними словами и гниёт молча
	 *
	 * @note Приём взят у кодеков INI и TOML, где он уже стоял. У TOML он сработал
	 *       15.09.2026 на заведении `UNREPRESENTABLE_VALUE` и потребовал подвинуть предел
	 *       в ту же правку; у соседа по мосту тем же приёмом вскрылись два кода, не
	 *       проверявшихся вовсе. Здесь его недоставало
	 */
	ASSERT_STREQ(yaml::message(static_cast <yaml::error_t> (
	 static_cast <uint32_t> (yaml::error_t::SETTINGS_ALREADY_APPLIED) + 1)), "unknown error");
}
/**
...
TEST(CodecYamlCommon, ResolveLegacy) {
	...
	/**
	 * Выполняем проверку того, что записи восьмеричной наречия 1.2 наречие 1.1 не знает
	 *
	 * @note Описание 1.1 указателя `0o` не имеет вовсе - восьмеричное пишется там ведущим
	 *       нулём, `017`, - и запись `0o1_7` наречием тем есть строка. Прежде тут стояло
	 *       утверждение обратное, и оно закрепляло чтение, описанию противное
	 */
	ASSERT_EQ(yaml::resolve("0o1_7", schema), yaml::type_t::STRING);
	// Выполняем проверку того, что восьмеричное наречия 1.1 пишется ведущим нулём
	ASSERT_EQ(yaml::resolve("01_7", schema), yaml::type_t::NUMBER);
...
TEST(CodecYamlCommon, MessagesAndUnknownNames) {
	 ...
	 */
	{
		...
		for(const yaml::error_t error : errors){
			...
			ASSERT_NE(description, "unknown error") << static_cast <unsigned> (error);
		}
	}
	/**
...
/**
 * @brief Проверка вида записи значения с байтом, знака не составляющим
 *
 * @details Вид записи значения выбирается по его содержимому, и значение с байтом,
 *          последовательности Юникода не составляющим, обязано записываться двойною
 *          оградой: лишь она несёт управляющие последовательности, какими такой байт
 *          и записывается. Оградой одинарной либо вовсе без ограды он лёг бы в текст
 *          сырым, и читающий отверг бы запись собственного кодека
 *
 * @note Заход даётся прямым вызовом вывозимой связки: между проверкой и целью слоёв
 *       нет, и порча бьёт ровно в задуманное место
 *
 */
TEST(CodecYamlCommon, QuotingBrokenSequence) {
	// Значение с байтом, знака Юникода не составляющим
	const string broken = string("a\xFF" "b");
	// Выполняем проверку того, что значение записывается двойною оградой
