...
TEST(CodecYamlValue, ExtendedNumbers) {
	...
	/**
	 * Выполняем проверку того, что поднормальные числа родным видом признаны
	 *
	 * @note Признак выхода за предел стоит и у них, а представимы они точностью
	 *       урезанной: сличается оттого сам исход разбора, а не один признак
	 */
	ASSERT_EQ(doc.root().at("/a").type(), yaml::type_t::DOUBLE);
	// Выполняем проверку вида второго поднормального числа
	ASSERT_EQ(doc.root().at("/b").type(), yaml::type_t::DOUBLE);
	// Выполняем проверку того, что потеря значимости родным видом не признана
	ASSERT_EQ(doc.root().at("/c").type(), yaml::type_t::EXTENDED);
}
