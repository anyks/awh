 ...
 */
/**
 * @brief Проверка независимости исхода на негодной подаче от нарезки её на куски
 *
 */
TEST(CodecYamlReader, MalformedSequenceOutcomeIsIndependentOfTheChunking) {
	/**
	 * @note Отсев ведущих байт `C0`, `C1` и свыше `F4`, заведённый 08.09.2026, меняет МЕСТО
	 *       отказа у потокового приведения: прежде такой байт удерживался до следующего
	 *       куска и отвергался чтением значения, ныне отвергается сразу. Исход обязан
	 *       остаться тем же при всякой нарезке, и проверка эта то и сторожит
	 *
	 * @note Проверка независимости, стоявшая прежде, брала лишь ГОДНЫЕ тексты, и путь
	 *       удержания негодной последовательности не поверялся ею вовсе
	 *
	 * @note Судится ИСХОД разбора, а не возврат подачи: отказ по кодировке бывает отложен
	 *       до извлечения событий, и щуп, смотревший возврат `feed`, показывал семнадцать
	 *       ложных расхождений из восьмидесяти восьми
	 */
	/**
	 * @brief Способ снятия исхода разбора при заданной нарезке подачи
	 *
	 * @param text подаваемый текст настроек
	 * @param cut  место разреза подачи надвое, нулевое при подаче целиком
	 * @return     код исхода разбора
	 *
	 */
	const auto outcome = [](const string & text, const size_t cut) noexcept -> uint8_t {
		...
		if(cut == 0){
			...
			while(reader.next());
		/**
		 * Если первый кусок подачи принят
		 */
		} else if(reader.feed(text.data(), cut, false)) {
			// Выполняем вычерпывание событий первого куска
			while(reader.next());
			// Выполняем подачу второго куска
			reader.feed(text.data() + cut, (text.size() - cut), true);
			// Выполняем вычерпывание событий второго куска
			while(reader.next());
		}
		// Выводим код исхода разбора
		return static_cast <uint8_t> (reader.error());
	};
	/**
	 * Выполняем перебор негодных последовательностей
	 */
	for(const string & sequence : {
		string("\xC0\x80"), string("\xC1\xBF"), string("\xF5\x80\x80\x80"),
		string("\xE2\x82"), string("\xE1\x41"), string("\xE0\x80\x80"), string("\xED\xA0\x80")
	}){
		/**
		 * Выполняем перебор подач: негодное в середине да негодное на самом конце
		 *
		 * @note Вторая подача отдельна от первой намеренно: у первой негодная
		 *       последовательность к концу куска всегда достраивается, отчего путь
		 *       удержания «оборвано, а подача кончилась» ею не задет ВОВСЕ. Доказано
		 *       порчею: обращение условия `end` в ложь на том пути проверку НЕ роняло,
		 *       покуда второй подачи здесь не было
		 */
		for(const string & text : {"k: a" + sequence + "b\n", "k: a" + sequence}){
			// Исход разбора подачею целиком
			const uint8_t whole = outcome(text, 0);
			// Выполняем проверку того, что негодная подача отвергнута
			ASSERT_NE(whole, static_cast <uint8_t> (yaml::error_t::NONE)) << text.size();
			/**
			 * Выполняем перебор всех мест разреза подачи надвое
			 */
			for(size_t cut = 1; cut < text.size(); cut++)
				// Выполняем проверку того, что исход нарезкою не изменился
				ASSERT_EQ(outcome(text, cut), whole) << cut;
		}
	}
}
...
TEST(CodecYamlReader, QuotedEscapeTable) {
	...
	yaml::document_t refused(::framework(), ::logger());
	/**
...
TEST(CodecYamlReader, SettingsRefusedWhileParsing) {
	...
	/**
	 * @note Прежде здесь утверждалось `NONE`, и утверждалась тем самым НЕМОТА: отвергнутая
	 *       смена настроек кода не ставила, и потребитель получал отсутствие отказа при
	 *       отказе. Ныне код назван: смена КОДИРОВКИ берёт общий код семьи, смена прочих
	 *       полей - свой
	 */
	// Выполняем проверку того, что отвергнутая смена настроек назвала причину свою
	ASSERT_EQ(reader.error(), yaml::error_t::ENCODING_ALREADY_CHOSEN) << yaml::message(reader.error());
...
TEST(CodecYamlReader, FeedRefusedAfterFailureAndEnd) {
	 ...
	 */
	{
		...
		ASSERT_FALSE(reader.feed(second.data(), second.size(), true));
		/**
		 * @note Код `TEXT_ALREADY_ENDED`, а не `TRAILING_CHARACTERS`: лишних знаков в тексте
		 *       не было вовсе - лишним был ВЫЗОВ у потребителя, уже объявившего конец
		 */
		// Выполняем проверку выданного кода отказа разбора
...
TEST(CodecYamlReader, EmptyContentUnderTag) {
	// Объект дерева документа
	yaml::document_t standard(::framework(), ::logger());
	...
	yaml::document_t flowing(::framework(), ::logger());
	...
	yaml::document_t custom(::framework(), ::logger());
...
TEST(CodecYamlReader, TrailingBreakDoesNotChangePlainScalar) {
	...
	for(const string & text : texts){
		...
		yaml::document_t tailed(::framework(), ::logger());
...
TEST(CodecYamlReader, BlockScalarIsNotAnImplicitKey){
	...
	/**
	 * Написания, где заголовок блока стоит НЕ ГЛУБЖЕ отступа вместилища
	 *
	 * @details Отказ им выносит правило отступа, а не правило имени пары, и выносит его
	 * РАНЬШЕ - на самой строке заголовка, а не на строке разделителя ниже. Причина тому
	 * та же, по какой отвергается всякое значение на отступе открытого вместилища:
	 * значение пары стоит либо в строке имени её, либо глубже имени, и заголовок на
	 * отступе самого вместилища ни значением не является, ни записью его
	 *
	 * @note Прежде написания эти стояли выше, среди отвергаемых правилом имени пары.
	 *       Разведены они замером: правило имени осталось живым и ловит заголовок,
	 *       глубже отступа стоящий, а отступ ловит тот, что стоит не глубже
	 */
	const vector <string> shallow = {"x:\n! |\n a\n: b\n", "x: 1\n|\n a\n: b\n", "x:\n>\n a\n: b\n"};
	/**
	 * Выполняем перебор всех написаний с заголовком на отступе вместилища
	 */
	for(const string & text : shallow){
		// Объект дерева документа, написание разбирающего
		yaml::document_t document(::framework(), ::logger());
		// Выполняем проверку отказа разбора написания
		ASSERT_FALSE(document.parse(text)) << text << " -> " << document.dump();
		// Выполняем проверку названной причины отказа
		ASSERT_EQ(document.error(), yaml::error_t::INVALID_INDENTATION) << text;
	}
	/**
...
/**
 * @brief Проверка полноты сброса состояния чтения
 *
 * @details Чтение, один текст уже прочитавшее, обязано по сбросу прочесть следующий ровно
 * так же, как прочло бы его чтение свежее: стопа открытых построений, метки, наречие,
 * опознанная кодировка - остаток всего этого обязан быть сброшен целиком
 *
 * @warning Дефект такого рода прогоном свежего чтения не воспроизводится НИ РАЗУ: он виден
 *          лишь тогда, когда одно и то же чтение читает череду текстов. Оттого проверка
 *          гоняет всякий текст дважды - свежим чтением и чтением, всеми прежними текстами
 *          занятым, - и сличает исход, код отказа и перечень событий
 *
 * @warning Проверка эта порчею НЕ ДОКАЗАНА, и вот почему: остаток требует ЧЕРЕДЫ текстов,
 *          а не пары. Сличение всех пар из 351 текста набора соответствия при снятых
 *          шестнадцати сбросах, какие `clear()` обходил стороною, расхождений не дало ни
 *          одного - парою его не поймать. Ворошитель же ловит его чтением, живущим от
 *          прохода к проходу: без сбросов правило «сброшенное чтение равно свежему»
 *          валится, со сбросами проходит, а поток тот же самый - обе кампании прошли
 *          ровно 500 000 ходов с одинаковым числом событий
 *
 * @note Проверка оттого стережёт заставою: череду, ворошителем найденную, ею не
 *       воспроизвести, а согласие свежего чтения со сброшенным она держит на текстах,
 *       состояние чтения нагружающих
 */
TEST(CodecYamlReader, ResetClearsEveryTraceOfPreviousText){
	/**
	 * Проверяемые написания текста, состояние чтения нагружающие
	 */
	const vector <string> texts = {
		"a: |2\n   x\n", "? имя\n: значение\n", "- !t &m x\n", "---\n", "a:\n\t- x\n",
		"\"с оградой\": [ { b: {}, имя: b#c}\t, 92", "%YAML 1.1\n---\non: 1\n",
		">-\n  сложенное\n", "- - - глубоко\n", "{a: [1, {b: 2}]}\n", "a: 1\n...\n",
		"? |\n  block key\n: value\n", "!t: v\n", "a:\n- x\n: 1\n"
	};
	// Объект потокового чтения, всеми прежними текстами занятого
	yaml::reader_t reused(::logger());
	/**
	 * Выполняем перебор всех проверяемых написаний текста
	 */
	for(const string & text : texts){
		// Перечень событий, свежим чтением выданных
		vector <yaml::event_t> first;
		// Перечень событий, чтением сброшенным выданных
		vector <yaml::event_t> second;
		// Объект потокового чтения свежего
		yaml::reader_t fresh(::logger());
		// Выполняем подачу текста свежему чтению
		const bool passed = fresh.feed(text.data(), text.size(), true);
		/**
		 * Выполняем перебор всех событий свежего чтения
		 */
		while(fresh.next())
			// Выполняем набор очередного выданного события
			first.push_back(fresh.event());
		// Выполняем сброс состояния чтения, прежними текстами занятого
		reused.clear();
		// Выполняем подачу текста чтению сброшенному
		const bool repeated = reused.feed(text.data(), text.size(), true);
		/**
		 * Выполняем перебор всех событий чтения сброшенного
		 */
		while(reused.next())
			// Выполняем набор очередного выданного события
			second.push_back(reused.event());
		// Выполняем проверку согласия исходов подачи
		ASSERT_EQ(passed, repeated) << text;
		// Выполняем проверку согласия кодов отказа
		ASSERT_EQ(fresh.error(), reused.error()) << text;
		// Выполняем проверку согласия перечней выданных событий
		ASSERT_EQ(first, second) << text;
	}
}

/**
 * @brief Проверка продолжения простого значения, корнем документа стоящего
 *
 * @details Продолжение простого значения обязано стоять глубже построения, значение
 * объемлющего. Значение же, корнем документа стоящее, построений над собою не имеет, и
 * продолжению его дозволен отступ нулевой - тот же, что у самого значения. Склейка строк
 * сличала отступ строго и там, отчего числила продолжение строкою чужой
 *
 * @warning Всплывало это хвостовым переводом строки: строка последняя, перевода не имеющая,
 *          склейке не подлежит вовсе. Написание `b\n"в #\n-` без перевода отвергалось
 *          верно, а с переводом склейка сводила вторую строку с третьей, примечание рубило
 *          сведённое, и строка `-` пропадала молча - текст принимался
 *
 * @note Проверка эта доказана порчею: возврат строгого сличения отступа роняет её
 *       расхождением исходов
 */
TEST(CodecYamlReader, RootPlainScalarContinuesAtZeroIndent){
	/**
	 * Проверяемые написания текста с продолжением на нулевом отступе
	 */
	const vector <string> texts = {"b\n\"в #\n-", "b\n\"в\n-", "b\n'x #\n- y"};
	/**
	 * Выполняем перебор всех проверяемых написаний текста
	 */
	for(const string & text : texts){
		...
		// Выполняем проверку согласия исходов разбора обоих написаний
		ASSERT_EQ(first, second) << text;
		// Выполняем проверку согласия кодов отказа обоих написаний
		ASSERT_EQ(bare.error(), tailed.error()) << text;
	}
}

/**
 * @brief Проверка свёртки логической строки, склейкой собранной
 *
 * @details Склейка сводит несколько строк исходных в одну логическую, когда первая из них
 * ограду значения открывает и не закрывает. Свёртка собранного обязана дать то же, что даёт
 * путь непрерывный: перевод один обращается пробелом, каждый следующий остаётся переводом,
 * а хвостовые содержимым не являются вовсе
 *
 * @warning Всякий порок этого рода виден лишь ХВОСТОВЫМ переводом строки: строка последняя,
 *          перевода не имеющая, склейке не подлежит вовсе, и один и тот же текст читался
 *          по-разному в зависимости от того, окончен он переводом или нет
 *
 * @note Проверка эта доказана порчею поштучно: снятие свёртки отделителя роняет её на
 *       написании с подачею, возврат сброса счётчика за свёртку - на написании с пустою
 *       строкой, а сброс признака начатого значения скобкою безусловно - на написании
 *       `: o["`
 */
TEST(CodecYamlReader, FoldedLogicalLineMatchesContinuousPath){
	/**
	 * Проверяемые написания текста, склейке подлежащие
	 */
	const vector <string> texts = {
	...
	for(const string & text : texts){
		...
		if(first)
			// Выполняем проверку согласия деревьев обоих написаний
			ASSERT_EQ(bare.dump(), tailed.dump()) << text;
		// Если разбор обоих написаний отвергнут
		else ASSERT_EQ(bare.error(), tailed.error()) << text;
	}
}
/**
 * @brief Проверка запятой, местом значения вне поточного построения не являющейся
 *
 * @details Склейка строк опознаёт ограду значения местом кавычки: оградою кавычка
 * является, лишь стоя НА МЕСТЕ значения. Запятая место значения открывает только
 * внутри поточного построения - вне его она есть знак содержимого простого значения.
 * Признак начатого простого значения сбрасывался ею безусловно, и кавычка в записи
 * `a: ,"` числилась оградою: склейка глотала строку следующую, обращая её в
 * содержимое значения
 *
 * @note Порок опознаётся зависимостью исхода от хвостового перевода строки: строка
 *       последняя, перевода не имеющая, склейке не подлежит вовсе, и написание без
 *       перевода отвергалось, а с переводом принималось. Нашёл это ворошитель
 */
/**
 * @brief Проверка склейки строк внутри построения, строкою прежней открытого
 *
 * @note Предпросмотр склейки видит одну телесную строку, и глубину поточных построений
 *       обязан брать от стопы читающего, а не с нуля: скобка, оставшаяся строкою выше,
 *       иначе теряется, запятая числится знаком содержимого, и ограда за нею оградою не
 *       признаётся вовсе. Случай 8UDB набора yaml-test-suite
 *
 */
TEST(CodecYamlReader, FlowOpenedOnEarlierLineKeepsCommaAValuePlace){
	// Объект дерева документа, текст разбирающего
	yaml::document_t document(::framework(), ::logger());
	// Выполняем разбор поточного перечня, скобкою на своей строке открытого
	ASSERT_TRUE(document.parse("[\nx, \"a\nb\"\n]\n")) << yaml::message(document.error());
	// Значение, склейкою собранное
	string value;
	// Выполняем чтение значения второго члена поточного перечня
	ASSERT_TRUE(document.root()[1].value(value));
	// Выполняем проверку того, что склейка перевод строки свернула пробелом
	ASSERT_EQ(value, string("a b"));
}
/**
 * @brief Проверка свойств узла, узла своего не дождавшихся
 *
 * @note Свойства держат ожидание значения пары ради узла, что придёт строкою глубже.
 *       Строка отступом имени пары пару эту закрывает, и значением её остаётся пустота,
 *       свойства несущая: переживая пару свою, свойства прилипали к имени пары следующей,
 *       а значение той пропадало молча
 *
 */
TEST(CodecYamlReader, PropertiesWithoutNodeYieldEmptyValue){
	// Объект дерева документа, текст разбирающего
	yaml::document_t document(::framework(), ::logger());
	// Выполняем разбор отображения, где свойства узла своего не дождались
	ASSERT_TRUE(document.parse("k:\n  !!str\nj: v\n")) << yaml::message(document.error());
	// Выполняем проверку количества пар отображения
	ASSERT_EQ(document.root().size(), 2u);
	// Значение пары, свойствами исчерпанной
	string value;
	// Выполняем чтение значения пары, свойствами исчерпанной
	ASSERT_TRUE(document.root()["k"].value(value));
	// Выполняем проверку пустоты значения пары
	ASSERT_TRUE(value.empty());
	// Выполняем чтение значения пары следующей
	ASSERT_TRUE(document.root()["j"].value(value));
	// Выполняем проверку сохранности значения пары следующей
	ASSERT_EQ(value, string("v"));
}
/**
 * @brief Проверка свойств узла, стоящих глубже отступа перечня
 *
 * @note Правило случая GT5M отвергает свойства, СТРОКУ ПЕРЕЧНЯ занявшие: записи своей у
 *       них там нет. Свойства же, стоящие глубже, принадлежат узлу члена предыдущего по
 *       праву, и сличать правилу надлежит отступ строки, свойства несшей, а не отступ
 *       строки черты
 *
 */
TEST(CodecYamlReader, NodePropertiesDeeperThanSequenceBelongToTheItem){
	// Объект дерева документа, текст разбирающего
	yaml::document_t document(::framework(), ::logger());
	// Выполняем разбор перечня, где свойства узла стоят глубже отступа его
	ASSERT_TRUE(document.parse("- item1\n  &node\n- item2\n")) << yaml::message(document.error());
	// Выполняем проверку количества членов перечня
	ASSERT_EQ(document.root().size(), 2u);
	// Объект дерева документа, узел одними свойствами несущего
	yaml::document_t empty(::framework(), ::logger());
	/**
	 * Выполняем разбор перечня, где член первый одними свойствами исчерпан
	 *
	 * @note Форма эта и нашлась ворошителем: без соседа тот же текст читался, а с
	 *       соседом отвергался
	 */
	ASSERT_TRUE(empty.parse("-\n  !!str\n- item2\n")) << yaml::message(empty.error());
	// Выполняем проверку количества членов перечня
	ASSERT_EQ(empty.root().size(), 2u);
	// Объект дерева документа, свойства на отступе перечня несущего
	yaml::document_t refused(::framework(), ::logger());
	/**
	 * Выполняем проверку отказа разбора свойств, строку перечня занявших
	 *
	 * @note Случай GT5M набора yaml-test-suite: правило снятию не подлежит
	 */
	ASSERT_FALSE(refused.parse("- item1\n&node\n- item2\n"));
}
/**
 * @brief Проверка директивы, отступом предварённой
 *
 * @details Отступ директиве описанием НЕ ДОЗВОЛЕН: правило `l-directive` ставит знак `%`
 * в начало строки. Принимая директиву с отступом, чтение расходилось с удержанием
 * исходного текста, где начало документа берётся вместе с директивами его и только с
 * начала строки, - и перезапись правленого дерева отвергалась своим же чтением
 *
 * @note Строка с отступом уходит разбору содержимого: наречие ею не назначается, и
 *       поток выходит о двух документах, а не об одном
 *
 */
TEST(CodecYamlReader, IndentedDirectiveIsNotDirective){
	// Объект дерева документа, текст разбирающего
	yaml::document_t document(::framework(), ::logger());
	// Выполняем разбор потока, где директива предварена пробелом
	ASSERT_TRUE(document.parse(" %YAML 1.2\n---\na: 1\n")) << yaml::message(document.error());
	/**
	 * Выполняем проверку количества документов потока
	 *
	 * @note Директива документа своего не открывает, и поток об одном документе выходит
	 *       лишь тогда, когда она директивой и признана. Здесь их два: скалярное значение
	 *       да отображение за чертою
	 */
	ASSERT_EQ(document.documents(), static_cast <size_t> (2));
	// Значение первого документа потока
	string value;
	// Выполняем чтение значения первого документа потока
	ASSERT_TRUE(document.root(0).value(value));
	// Выполняем проверку того, что запись директивы стала содержимым
	ASSERT_EQ(value, string("%YAML 1.2"));
	// Объект дерева документа, директиву с подачей разбирающего
	yaml::document_t tabbed(::framework(), ::logger());
	// Выполняем разбор потока, где директива предварена подачей
	ASSERT_TRUE(tabbed.parse("\t%YAML 1.2\n---\na: 1\n")) << yaml::message(tabbed.error());
	// Выполняем проверку количества документов потока
	ASSERT_EQ(tabbed.documents(), static_cast <size_t> (2));
	// Объект дерева документа, директиву с начала строки разбирающего
	yaml::document_t plain(::framework(), ::logger());
	/**
	 * Выполняем разбор потока, где директива стоит с начала строки
	 *
	 * @note Правило снятию не подлежит: директива с начала строки директивой и остаётся,
	 *       и поток выходит об ОДНОМ документе
	 */
	ASSERT_TRUE(plain.parse("%YAML 1.2\n---\na: 1\n")) << yaml::message(plain.error());
	// Выполняем проверку количества документов потока
	ASSERT_EQ(plain.documents(), static_cast <size_t> (1));
}
TEST(CodecYamlReader, CommaOutsideFlowIsNotValuePlace){
	// Объект дерева документа, текст разбирающего
	yaml::document_t document(::framework(), ::logger());
	/**
	 * Выполняем проверку отказа разбора текста с хвостовым переводом строки
	 *
	 * @note Строка `b` стоит отступом нуль, а продолжению простого значения описание
	 *       языка велит стоять глубже своей пары: текст этот негоден
	 */
	ASSERT_FALSE(document.parse("a: ,\"\nb\n"));
	// Объект дерева документа, текст без хвостового перевода разбирающего
	yaml::document_t tailless(::framework(), ::logger());
	// Выполняем проверку того, что исход не зависит от хвостового перевода строки
	ASSERT_FALSE(tailless.parse("a: ,\"\nb"));
	/**
	 * Выполняем проверку склейки строк внутри поточного построения
	 *
	 * @note Внутри построения запятая место значения открывает по праву, и значение
	 *       огранённое вправе тянуться на строки ниже: склейку здесь снимать нельзя
	 */
	{
		// Объект дерева документа, поточное построение разбирающего
		yaml::document_t flowing(::framework(), ::logger());
		// Выполняем разбор поточного перечня с огранённым значением в две строки
		ASSERT_TRUE(flowing.parse("a: [1, \"x\ny\"]\n")) << yaml::message(flowing.error());
		// Значение, склейкою собранное
		string value;
		// Выполняем чтение значения второго члена поточного перечня
		ASSERT_TRUE(flowing.root()["a"][1].value(value));
		// Выполняем проверку того, что склейка перевод строки свернула пробелом
		ASSERT_EQ(value, string("x y"));
	}
}
/**
 * @brief Проверка заголовка блочного значения на отступе открытого вместилища
 *
 * @details Значение пары стоит либо в строке имени её, либо ГЛУБЖЕ имени. Заголовок
 * блочного значения, на отступе самого вместилища стоящий, ни значением пары не
 * является, ни записью его - разобрать его нечем. Развилка заголовка, однако, уходила
 * возвратом ПРЕЖДЕ правила, отвергающего всякое значение на отступе вместилища, и
 * заслон её не касался вовсе
 *
 * @note Написание `a:` со строкою `>` под ним давало пару с именем ПУСТЫМ, содержимым
 *       какой становилась строка следующая: текст `a:\n>\nb:` читался деревом о двух
 *       парах, где второй в тексте нет вовсе. Написание же `a:\n>\n b` роняло
 *       содержимое молча. Нашёл это ворошитель правкой дерева: правка ставила новую
 *       пару за строкою заголовка, а обратное чтение обращало её содержимым блока
 */
TEST(CodecYamlReader, BlockHeaderAtCollectionIndentIsRefused){
	/**
	 * Проверяемые написания с заголовком блока на отступе вместилища
	 */
	const vector <string> refused = {"a:\n>\nb:\n", "a:\n>\n b\n", "a:\n|\nb:\n", "- x\n>\n y\n"};
	/**
	 * Выполняем перебор всех отвергаемых написаний текста
	 */
	for(const string & text : refused){
	...
	/**
	 * Проверяемые написания годные, отказу не подлежащие
	 *
	 * @note Заголовок в строке имени пары, заголовок глубже имени, заголовок за чертою
	 *       записи перечня и заголовок корневой - все четыре стоят по праву, и заслон
	 *       их касаться не должен
	 */
	const vector <string> taken = {"a: >\n b\n", "a:\n  >\n  b\n", "- >\n  b\n", ">\nb\n"};
	/**
	 * Выполняем перебор всех годных написаний текста
	 */
	for(const string & text : taken){
		// Объект дерева документа, написание разбирающего
		yaml::document_t document(::framework(), ::logger());
		// Выполняем проверку успешного разбора написания
 ...
 *
 * @details Значение огранённое вправе стоять в несколько строк, и строки эти склейка
 * сводит в одну логическую прежде разбора. Строка последняя перевода за собою не имеет,
 * и розыск конца её отвечает отсутствием - тем же, чем отвечает отсутствие строк вовсе.
 * Склейка обрывалась на ней, и ограда значения оставалась незакрытой
 *
 * @note Порок опознаётся зависимостью исхода от хвостового перевода строки: текст
 *       `[[],"` со строкою `"]` под ним без перевода отвергался, а с переводом
 *       принимался. Нашёл это ворошитель
 */
TEST(CodecYamlReader, TaillessLastLineJoinsToOpenQuote){
	/**
	 * Проверяемые написания, огранённое значение на строке последней несущие
	 */
	const vector <string> texts = {"[[],\"\n\"]", "a: \"x\n y\"", "- \"x\n  y\"", "{a: \"x\ny\"}"};
	/**
	 * Выполняем перебор всех проверяемых написаний текста
	...
	for(const string & text : texts){
		// Объект дерева документа, написание без хвостового перевода разбирающего
		yaml::document_t tailless(::framework(), ::logger());
		// Выполняем проверку успешного разбора написания без хвостового перевода
		ASSERT_TRUE(tailless.parse(text)) << text << ": " << yaml::message(tailless.error());
		// Объект дерева документа, написание с хвостовым переводом разбирающего
		yaml::document_t tailed(::framework(), ::logger());
		// Выполняем проверку успешного разбора написания с хвостовым переводом
		ASSERT_TRUE(tailed.parse(text + "\n")) << text << ": " << yaml::message(tailed.error());
		/**
		 * Выполняем проверку согласия деревьев обоих написаний
		 *
		 * @note Хвостовой перевод строки содержимого не несёт, и деревья обязаны совпасть
		 */
		ASSERT_TRUE(yaml::value_t(tailless.root()) == yaml::value_t(tailed.root())) << text;
	}
	/**
	 * Выполняем проверку того, что ограда незакрытая отказом и остаётся
	 *
	 * @note Склейка строки последней ограду не закрывает сама по себе: текст, кавычки
	 *       закрывающей не имеющий вовсе, отвергается по-прежнему
	 */
	{
		// Объект дерева документа, написание с незакрытой оградою разбирающего
		yaml::document_t document(::framework(), ::logger());
...
/**
 * @brief Проверка того, что вид блочного значения решает предпосланная метка типа
 *
 * @details Содержимое блочного значения есть строка по построению своему, и вид его
 * назначается строковым: без этого запись `true` под заголовком `|` обратилась бы
 * логическим значением. Метка же типа, ЯВНО предпосланная, вид решает сама, и
 * назначение это её решение отменяло: запись `!!null |` с пустым содержимым выдавалась
 * СТРОКОЮ, тогда как то же пустое содержимое под оградой - `!!null ""` - выдавалось
 * пустотою
 *
 * @note Порок опознаётся сличением двух записей одной пустоты, а не глазами по одной:
 *       перезапись поточным построением обращала первую во вторую, и значение при
 *       перезаписи менялось молча - строка обращалась пустотой. Нашёл это ворошитель
 *       сличением деревьев перезаписи иного оформления
 *
 * @note Строгость к меткам, содержимому противным, правкою не задета: запись `!!int |`
 *       с содержимым `5` отвергается разрешением вида прежде - обе половины проверки
 *       то и стерегут
 */
TEST(CodecYamlReader, BlockValueTakesItsKindFromTheTag){
	/**
	 * Проверяемые записи пустоты и виды, каким они отвечают
	 */
	const vector <pair <string, yaml::type_t>> probes = {
		{"a: !!null |\n", yaml::type_t::NUL},
		{"a: !!null \"\"\n", yaml::type_t::NUL},
		{"a: !!null ''\n", yaml::type_t::NUL},
		{"a: !!null >\n", yaml::type_t::NUL},
	...
	for(auto & probe : probes){
		...
		ASSERT_EQ(value.at(string("a")).type(), probe.second) << probe.first;
	}
	/**
	 * Выполняем проверку отказа метки, содержимому блочного значения противной
	 */
	for(auto & source : {string("a: !!int |\n  5\n"), string("a: !!bool |\n  true\n"),
	                     string("a: !!null |\n  x\n"), string("a: !!float |\n  1.5\n")}){
		// Объект потокового чтения текста
		yaml::reader_t reader(::logger());
		// Выполняем подачу разбираемой записи текста документа
		...
		while(reader.next());
		// Выполняем проверку выданного кода отказа разбора
		ASSERT_EQ(reader.error(), yaml::error_t::TAG_MISMATCH) << source;
	}
	/**
	 * Выполняем проверку неподвижности перезаписи поточным построением
	 *
	 * @note Ею порок и вскрылся: перезапись обращала строку пустотою, и дерево
	 *       перезаписи с исходным расходилось
	 */
	{
		// Объект дерева документа
		yaml::document_t doc(::framework(), ::logger());
		// Выполняем проверку успешности разбора записи
		ASSERT_TRUE(doc.parse("a: !!null |\nb: 1\n"));
		// Настройки записи текста поточным построением
		yaml::writer_t::settings_t writing;
		// Устанавливаем поточное построение записи
		writing.layout = yaml::layout_t::FLOW;
		// Получаем перезапись дерева поточным построением
		const string restyled = doc.dump(writing);
		// Объект дерева перезаписанного документа
		yaml::document_t back(::framework(), ::logger());
		// Выполняем проверку читаемости перезаписи
		ASSERT_TRUE(back.parse(restyled)) << restyled;
		// Выполняем проверку совпадения деревьев перезаписи и исходного
		ASSERT_TRUE(yaml::value_t(back.root()) == yaml::value_t(doc.root())) << restyled;
	}
}
/**
 * @brief Проверка свёртки пустых строк внутри склеенной логической строки
 *
 * @details Строка логическая собирается из нескольких исходных, когда первая из них
 *          ограду значения открывает и не закрывает: ограда та внутри простого значения
 *          оградою не является, но склейка происходит прежде разбора. Свёртка обязана
 *          обойтись со склеенным ровно так же, как со строками, склейке не подвергшимися:
 *          перевод один обращается пробелом, а пустая строка переводом и остаётся
 *
 * @note Замерено покрытием: ветви свёртки, пустую строку разбирающие, не проходил ни
 *       набор проверок, ни ворошитель - склейка достигалась лишь однострочной записью
 */
TEST(CodecYamlReader, GluedLineFoldsItsBlanksLikeAnyOther){
	/**
	 * Проверяемые написания и перезаписи, каким они отвечают
	 */
	const vector <pair <string, string>> probes = {
...
/**
 * @brief Проверка снятия примечания с записи указания
 *
 * @details Указание разбирается остатком строки за именем своим, и примечание, за
 *          указанием стоящее, в остаток этот входит. Снять его надлежит прежде разбора
 *          доводов: иначе `%YAML 1.2 # слово` числилось бы наречием `1.2 # слово`
 *
 * @note Замерено покрытием: снятие примечания с записи указания не проходил ни набор
 *       проверок, ни ворошитель - указания писались ими без примечаний
 */
TEST(CodecYamlReader, DirectiveDropsTheRemarkBehindIt){
	/**
	 * Проверяемые написания и перезаписи, каким они отвечают
	 */
	const vector <pair <string, string>> probes = {
...
/**
 * @brief Проверка закрытия построений имени, вопросом объявленного, прежде пустоты значения
 *
 * @details Имя, вопросом объявленное, вправе быть построением, и построение это стоит
 *          отступом глубже вопроса своего. Пустота значения принадлежит отображению
 *          вопроса, а не построению имени: выданная прежде закрытия уровней, она легла бы
 *          записью перечня имени либо парою отображения его. Случаи KK5P и M2N8 набора
 *          yaml-test-suite
 *
 * @note Проверяется потоковое чтение, а не дерево: дерево имя-построение держать не умеет
 *       и отвечает отказом COMPLEX_KEY, оттого набор проверок сюда и не доходил. Замерено
 *       покрытием: закрытие уровней не проходил ни набор проверок, ни ворошитель
 */
TEST(CodecYamlReader, AskedKeyClosesItsLevelsBeforeTheEmptyValue){
	/**
	 * Проверяемые написания и череда событий, каким они отвечают
	 */
	const vector <pair <string, string>> probes = {
		{"? - a\n  - b\n: v\n", "MAPPING_START,SEQUENCE_START,SCALAR«a»,SCALAR«b»,SEQUENCE_END,SCALAR«v»,MAPPING_END,"},
		{"? - a\n", "MAPPING_START,SEQUENCE_START,SCALAR«a»,SEQUENCE_END,SCALAR«»,MAPPING_END,"},
		{"? a: 1\n  b: 2\n", "MAPPING_START,MAPPING_START,SCALAR«a»,SCALAR«1»,SCALAR«b»,SCALAR«2»,MAPPING_END,SCALAR«»,MAPPING_END,"},
		{"? - a\nb: 1\n", "MAPPING_START,SEQUENCE_START,SCALAR«a»,SEQUENCE_END,SCALAR«»,SCALAR«b»,SCALAR«1»,MAPPING_END,"},
		{"? - a\n? - b\n", "MAPPING_START,SEQUENCE_START,SCALAR«a»,SEQUENCE_END,SCALAR«»,SEQUENCE_START,SCALAR«b»,SEQUENCE_END,SCALAR«»,MAPPING_END,"},
		{"? a: 1\nb: 2\n", "MAPPING_START,MAPPING_START,SCALAR«a»,SCALAR«1»,MAPPING_END,SCALAR«»,SCALAR«b»,SCALAR«2»,MAPPING_END,"}
	};
	/**
	 * Выполняем перебор всех проверяемых написаний
	 */
	for(auto & probe : probes){
		// Объект потокового чтения текста документа
		yaml::reader_t reader(::logger());
		// Собираемое описание череды событий разбора
		string collected;
		// Выполняем подачу разбираемой записи текста документа
		ASSERT_TRUE(reader.feed(probe.first.data(), probe.first.size(), true)) << probe.first;
		/**
		 * Выполняем перебор всех накопленных событий разбора
		 */
		while(reader.next()){
			/**
			 * Если событие обрамляет поток либо документ
			 *
			 * @note Обрамление у всех написаний одно и то же, и сличению не подлежит
			 */
			if((reader.event() == yaml::event_t::STREAM_START) || (reader.event() == yaml::event_t::STREAM_END) ||
			   (reader.event() == yaml::event_t::DOCUMENT_START) || (reader.event() == yaml::event_t::DOCUMENT_END))
				// Выполняем переход к следующему событию разбора
				continue;
			// Выполняем запись названия очередного события
			collected.append(yaml::name(reader.event()));
			/**
			 * Если событие несёт скалярное значение
			 */
			if(reader.event() == yaml::event_t::SCALAR)
				// Выполняем запись содержимого события
				collected.append("«").append(reader.value().text).append("»");
			// Выполняем запись разделителя событий
			collected.append(",");
		}
		// Выполняем проверку собранного описания череды событий
		ASSERT_EQ(collected, probe.second) << probe.first;
	}
}
/**
 * @brief Проверка разбора последней строки, возвратом каретки закрытой
 *
 * @details Строка последняя перевода за собою не имеет, и конец её есть конец текста.
 *          Возврат каретки, за нею стоящий, снять надлежит там же, где он снимается у
 *          строк прочих: иначе он ушёл бы в содержимое значения молча
 *
 * @note Замерено покрытием: снятие возврата каретки с последней строки не проходил ни
 *       набор проверок, ни ворошитель - оба закрывали текст переводом строки
 */
TEST(CodecYamlReader, LastLineDropsItsCarriageReturn){
	/**
	 * Проверяемые написания и перезаписи, каким они отвечают
	 */
	const vector <pair <string, string>> probes = {
...
/**
 * @brief Проверка отказа негодных свойств узла внутри поточного построения
 *
 * @details Свойства снимаются разбором построения, а не разбором значения: за ними вправе
 *          стоять построение вложенное. Отказ разбора свойств обязан выйти отказом
 *          построения, а не быть проглоченным
 *
 * @note Имя метки пустым быть не вправе, а вот двоеточие в имени её ДОЗВОЛЕНО: правило
 *       `ns-anchor-char` изымает лишь указатели построения `,[]{}`, и запись `{&: 1}`
 *       есть метка с именем `:` - она законна и отказом здесь не поверяется
 *
 * @note Замерено покрытием: выход отказа свойств у поточного построения не проходил ни
 *       набор проверок, ни ворошитель
 */
TEST(CodecYamlReader, FlowRefusesTheMalformedProperties){
	/**
	 * @brief Описание проверяемого отказа
	 *
	 */
	...
	for(auto & probe : probes){
		...
		const string source(probe.source);
		// Выполняем проверку отказа разбора написания
		ASSERT_FALSE(reader.feed(source.data(), source.size(), true)) << probe.source;
		// Выполняем проверку выданного кода отказа разбора
		ASSERT_EQ(reader.error(), probe.error) << probe.source;
	}
}
 ...
 *
 */
TEST(CodecYamlReader, FeedAfterTheDeclaredEndIsRefused) {
	// Объект потокового чтения текста
	yaml::reader_t reader(::logger());
	...
	ASSERT_TRUE(reader.feed(first.data(), first.size(), true)) << yaml::message(reader.error());
...
/**
 * @brief Проверка поверки содержимого меток `!!binary` и `!!timestamp`
 *
 * @details Поверок этих у кодека не было ВОВСЕ: содержимое обеих меток принималось
 * дословно, и коды `INVALID_BINARY` да `INVALID_STAMP` обещали сверку, какой нет. Довод
 * при месте применения метки гласил, будто запись base64 сверяет извлечение содержимого,
 * - и то была ложь первого разряда: разбора base64 в кодеке не нашлось ни одного места
 *
 */
TEST(CodecYamlReader, TaggedContentIsVerifiedAgainstItsTag) {
	/**
	 * @brief Способ разбора текста наречием 1.1
	 *
	 * @param text разбираемый текст документа
	 * @param code ожидаемый код отказа разбора
	 *
	 */
	const auto probe = [](const string & text, const yaml::error_t code) noexcept -> void {
		// Настройки разбора наречием, отметки времени признающим
		...
		// Выполняем проверку исхода разбора текста документа
		ASSERT_EQ(document.parse(text), (code == yaml::error_t::NONE)) << text;
		// Выполняем проверку выданного кода отказа разбора
		ASSERT_EQ(document.error(), code) << text << " -> " << yaml::message(document.error());
	};
	// Выполняем проверку того, что годная запись base64 принимается
	probe("k: !!binary QUJD\n", yaml::error_t::NONE);
	// Выполняем проверку отказа записи, словарю base64 не отвечающей
	probe("k: !!binary не-base64!!\n", yaml::error_t::INVALID_BINARY);
	/**
	 * @note Длина записи base64 обязана быть кратной четырём: три знака виду отвечают,
	 *       а записью не являются
	 */
	// Выполняем проверку отказа записи base64 некратной длины
	probe("k: !!binary QUJ\n", yaml::error_t::INVALID_BINARY);
	// Выполняем проверку того, что годная отметка времени принимается
	probe("k: !!timestamp 2020-01-02\n", yaml::error_t::NONE);
	// Выполняем проверку отказа записи, отметкою времени не являющейся
	probe("k: !!timestamp не-дата\n", yaml::error_t::INVALID_STAMP);
	/**
	 * @note Числа месяца и дня сверяются ПО СУЩЕСТВУ, а не одним лишь видом: запись
	 *       `2020-99-99` виду отвечает вполне
	 */
	// Выполняем проверку отказа отметки времени с числами за пределами возможного
	probe("k: !!timestamp 2020-99-99\n", yaml::error_t::INVALID_STAMP);
	/**
	 * @note Написания эти взяты ИЗ ОПИСАНИЯ наречия 1.1 дословно. Первый мой поверщик был
	 *       СТРОЖЕ описания - требовал двузначных месяца, дня и часа, - и отвергал
	 *       `2020-1-2`, описанием дозволенное. Строгость сверх описания есть та же
	 *       неверность, что и попустительство
	 */
	// Выполняем проверку того, что однозначные месяц и день принимаются
	probe("k: !!timestamp 2020-1-2\n", yaml::error_t::NONE);
	// Выполняем проверку того, что полная запись с долею секунды и поясом принимается
	probe("k: !!timestamp 2001-12-14 21:59:43.10 -5\n", yaml::error_t::NONE);
	// Выполняем проверку того, что запись с разделителем `t` и поясом принимается
	probe("k: !!timestamp 2001-12-14t21:59:43.10-05:00\n", yaml::error_t::NONE);
	// Выполняем проверку того, что запись со всемирным поясом принимается
	probe("k: !!timestamp 2001-12-14 21:59:43.10 Z\n", yaml::error_t::NONE);
	/**
	 * @note Одно время без даты отметкою времени НЕ является: описание требует части даты
	 */
	// Выполняем проверку отказа записи одного лишь времени
	probe("k: !!timestamp 12:30\n", yaml::error_t::INVALID_STAMP);
	// Выполняем проверку отказа записи с часами за пределами возможного
	probe("k: !!timestamp 2020-01-02T25:00:00\n", yaml::error_t::INVALID_STAMP);
}

/**
 * @brief Проверка того, что состояние окончания наступает по вычерпывании очереди
 *
 * @details Прежде состояние это ставилось при ПОДАЧЕ, тогда как у прочих кодеков рамки
 * оно ставится по вычерпывании очереди событий. Потребитель, спросивший состояние между
 * подачею последнего куска и вычерпыванием, получал «разобрано до конца» при непустой
 * очереди. Событий при том не терялось - очередь вычерпывалась по-прежнему, - но ответ
 * был ложен по существу
 *
 */
TEST(CodecYamlReader, FinishedStateFollowsTheDrainedQueue) {
	// Объект потокового чтения текста документа
	yaml::reader_t reader(::logger());
	// Разбираемый текст документа
	const string text = "ключ: значение\nиное: 2\n";
	// Выполняем подачу текста документа целиком
	ASSERT_TRUE(reader.feed(text.data(), text.size(), true)) << yaml::message(reader.error());
	// Выполняем проверку того, что окончание разбора ещё не объявлено
	ASSERT_NE(reader.state(), yaml::state_t::FINISHED);
	// Количество снятых событий разбора
	size_t events = 0;
	/**
	 * Выполняем вычерпывание очереди событий разбора
	 */
	while(reader.next())
		// Выполняем учёт снятого события разбора
		events++;
	// Выполняем проверку того, что события вправду были
	ASSERT_GT(events, static_cast <size_t> (0));
	// Выполняем проверку того, что окончание разбора объявлено по вычерпывании
	ASSERT_EQ(reader.state(), yaml::state_t::FINISHED);
	/**
	 * @note Заслон подачи после объявленного конца поверяется РЯДОМ: он стоял на состоянии,
	 *       а состояние ныне наступает позже, - останься заслон на нём, подача после конца
	 *       снова стала бы приниматься
	 */
	// Объект потокового чтения текста документа для поверки заслона
	yaml::reader_t guarded(::logger());
	// Выполняем подачу текста документа целиком
	ASSERT_TRUE(guarded.feed(text.data(), text.size(), true)) << yaml::message(guarded.error());
	// Выполняем проверку отказа подачи следом, очереди не вычерпав
	ASSERT_FALSE(guarded.feed(text.data(), text.size(), true));
	// Выполняем проверку того, что отказ назван поимённо
	ASSERT_EQ(guarded.error(), yaml::error_t::TEXT_ALREADY_ENDED) << yaml::message(guarded.error());
}
