...
TEST(CodecYamlReader, FailureReachesTheLog) {
	...
	ASSERT_EQ(flag, awh::log::flag_t::CRITICAL);
...
TEST(CodecYamlReader, TaggedContentIsVerifiedAgainstItsTag) {
	...
	const auto probe = [](const string & text, const yaml::error_t code) noexcept -> void {
		...
		yaml::document_t document(settings);
