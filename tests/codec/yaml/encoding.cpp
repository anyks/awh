 ...
 */
/**
 * @brief Проверка того, что негодной последовательности отмеряется наибольшая её часть
 *
 */
TEST(CodecYamlEncoding, MalformedSequenceIsMeasuredByItsLongestPart) {
	/**
	 * @note Договор этот заведён 08.09.2026 вместе со сведением тел разбора UTF-8 у трёх
	 *       кодеков: длина при отказе есть НАИБОЛЬШАЯ ЧАСТЬ последовательности, а не нуль
	 *       и не длина её целиком. Без него зовущий, желавший перешагнуть негодную
	 *       последовательность, шагал по одному байту, и подмена знаком замены выдавала бы
	 *       на хвосте `E2 82` ДВА знака вместо одного
	 *
	 * @note Прежде застава нехватки байт стояла ДО перебора продолжающих, и запись `E1 41`
	 *       числилась оборванной, тогда как байт `41` продолжающим не бывает и никакое
	 *       продолжение её уже не выправит
	 *
	 * @note Три тела сличены исчерпывающе: все записи до трёх байт да четырёхбайтные с
	 *       ведущим `F0..F7` - 151 060 736 записей, ноль расхождений
	 */
	...
	size_t length = 0;
	/**
	 * Выполняем перебор негодных последовательностей вместе с наибольшею их частью
	 */
	for(const auto & item : {
		std::make_pair(string("\xD0"), size_t(1)),
		std::make_pair(string("\xE2\x82"), size_t(2)),
		std::make_pair(string("\xE1\x41"), size_t(1)),
		std::make_pair(string("\xC0\x80"), size_t(1)),
		std::make_pair(string("\xE0\x80\x80"), size_t(1)),
		std::make_pair(string("\xED\xA0\x80"), size_t(1)),
		std::make_pair(string("\xF5\x80\x80\x80"), size_t(1))
	}){
		// Выполняем сброс длины прочитанной последовательности
		length = 0;
		// Выполняем проверку отказа разбора негодной последовательности
		ASSERT_NE(yaml::inspect(item.first, 0, code, length), yaml::utf8_t::VALID) << item.first.size();
		// Выполняем проверку длины наибольшей части негодной последовательности
		ASSERT_EQ(length, item.second) << item.first.size();
	}
	// Выполняем сброс длины прочитанной последовательности
	length = 0;
	// Выполняем проверку того, что годная последовательность читается целиком
	ASSERT_EQ(yaml::inspect(string_view("\xD0\x94", 2), 0, code, length), yaml::utf8_t::VALID);
	// Выполняем проверку прочитанного знака Юникода
	ASSERT_EQ(code, 0x414U);
	// Выполняем проверку длины прочитанной последовательности
	ASSERT_EQ(length, 2U);
}
...
TEST(CodecYamlEncoding, Forbidden) {
	...
	/**
	 * Выполняем проверку запрета управляющего знака в кодировке UTF-32
	 *
	 * @note Приведение четвёрками байтов ведётся своим телом, отдельным от приведения
	 *       парами: запрет знака держался проверкою лишь у UTF-8 и UTF-16, а у UTF-32
	 *       отказ не проверялся ничем - ни набором проверок, ни корпусом yaml-test-suite,
	 *       ни ворошителем. Оба порядка байтов берутся нарочно: они разбираются разными
	 *       ветвями сборки знака из четвёрки
	 */
	{
		// Порядок байтов записи и текст с управляющим знаком
		const pair <const char *, size_t> samples[] = {
			// Метка UTF-32 с обратным порядком байтов и знак U+0001 за нею
			{"\xFF\xFE\x00\x00" "\x01\x00\x00\x00", 8},
			// Метка UTF-32 с прямым порядком байтов и знак U+0001 за нею
			{"\x00\x00\xFE\xFF" "\x00\x00\x00\x01", 8}
		};
		/**
		 * Выполняем перебор порядков байтов записи
		 */
		for(auto & sample : samples){
			...
			// Приведённый к UTF-8 текст
			string result;
			// Выполняем проверку отказа приведения управляющего знака четвёрками байтов
			ASSERT_FALSE(decoder.convert(sample.first, sample.second, true, result));
			// Выполняем проверку кода ошибки приведения
			ASSERT_EQ(decoder.error(), yaml::error_t::INVALID_CHARACTER);
		}
	}
...
/**
 * @brief Проверка отказа приведения текста, кусок оборвавшего единицу записи
 *
 * @details Кодировки UTF-16 и UTF-32 читаются единицами о двух да о четырёх байтах,
 * и текст, оборванный посреди такой единицы, годным не является. Пути отказа эти
 * покрытием не задевались вовсе: отличить отказ от молчаливого отбрасывания хвоста
 * было нечем, а отбрасывание это отняло бы у разбора последний знак текста
 *
 * @note Недописанная пара суррогатов поверяется отдельно от недописанной единицы:
 *       пара удерживается своим полем и своим же путём отвергается по окончании
 *       текста
 *
 */
TEST(CodecYamlEncoding, TruncatedUnit) {
	// Приведённый текст
	string result;
	/**
	 * Оборванные тексты, отказ приведения обязанные вызвать
	 */
	const vector <tuple <yaml::encoding_t, string, string>> broken = {
		// Единица кодировки UTF-16 о двух байтах, оборванная концом текста
		{yaml::encoding_t::UTF16BE, string("\x00\x41\x00", 3),         "UTF-16BE, единица о двух байтах"},
		// Единица кодировки UTF-16 с обратным порядком байтов, оборванная концом текста
		{yaml::encoding_t::UTF16LE, string("\x41\x00\x41", 3),         "UTF-16LE, единица о двух байтах"},
		// Единица кодировки UTF-32 о четырёх байтах, оборванная концом текста
		{yaml::encoding_t::UTF32BE, string("\x00\x00\x00\x41\x00", 5), "UTF-32BE, единица о четырёх байтах"},
		// Единица кодировки UTF-32 с обратным порядком байтов, оборванная концом текста
		{yaml::encoding_t::UTF32LE, string("\x41\x00\x00\x00\x41\x00", 6), "UTF-32LE, единица о четырёх байтах"}
	};
	/**
	 * Выполняем перебор оборванных текстов
	 */
	for(const auto & item : broken){
		...
		ASSERT_TRUE(decoder.encoding(std::get <0> (item))) << std::get <2> (item);
		// Выполняем проверку отказа приведения оборванного текста
		ASSERT_FALSE(decoder.convert(std::get <1> (item).data(), std::get <1> (item).size(), true, result))
			<< std::get <2> (item);
		// Выполняем проверку выданного кода ошибки приведения
		ASSERT_EQ(decoder.error(), yaml::error_t::INVALID_ENCODING) << std::get <2> (item);
	}
	/**
	 * Выполняем проверку того, что оборванная единица отказа до конца текста не даёт
	 *
	 * @note Отказ обязан ждать конца текста: единица, границей куска разорванная,
	 *       дописывается следующим куском, и отказ по ней сразу отнял бы у потокового
	 *       разбора всякую подачу кусками
	 */
	{
		// Объект приведения исходного текста к кодировке UTF-8
		yaml::decoder_t decoder;
		...
		// Выполняем навязывание кодировки исходного текста
		ASSERT_TRUE(decoder.encoding(yaml::encoding_t::UTF16BE));
		// Выполняем проверку того, что приведение оборванного куска отказа не дало
		ASSERT_TRUE(decoder.convert("\x00\x41\x00", 3, false, result));
		// Выполняем проверку того, что приведение дописанного куска удалось
		ASSERT_TRUE(decoder.convert("\x42", 1, true, result));
