 ...
 *
 * Имя error_t уточняется пространством имён, хотя оно и внесено объявлением выше
 *
 * Библиотека glibc объявляет свой error_t в глобальном пространстве имён, и голое
 * обращение становится двусмысленным: сборка отвечает отказом «reference to error_t
