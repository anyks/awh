 ...
 *
 * @brief Общий набор сценариев эталонных стендов сравнения протокола HTTP/2 —
 ...
 *
 * @details Учёт возможен там, где есть точка подмены аллокатора: операторы
...
namespace {
	/**
	 * @brief Функция выполнения сценария кодирования блоков заголовков
	 *
	...
	static void encodeScenario(const char * mask) noexcept {
		...
		static constexpr const char * NAME = "http2/hpack/encode";
		...
		// Создаём объект сжатия заголовков сравниваемой реализации
		Codec codec;
		// Если блоки заголовков закодированы с ошибкой
		if(!rival::encoding(codec, outcome)){
			// Выводим сообщение о невыполненном сценарии
			rival::skip(NAME, "прогон не выполнен: блок заголовков не закодирован");
			// Выходим из сценария
			return;
		}
		...
		::snprintf(
			...
			((outcome.original > 0) ? ((static_cast <double> (outcome.bytes) / static_cast <double> (outcome.original)) * 100.0) : 0.0),
			(static_cast <double> (outcome.bytes) / static_cast <double> (outcome.operations))
		);
		// Выводим результат прогона сценария
		rival::report(NAME, "блоков/с", rival::perSecond(outcome), details);
	}
	/**
	 * @brief Функция выполнения сценария декодирования блоков заголовков
	 *
	...
	static void decodeScenario(const bool measured, const char * mask) noexcept {
		// Название выполняемого сценария
		const char * name = (measured ? "http2/allocations/per-decoded-block" : "http2/hpack/decode");
		// Если название сценария не соответствует фильтру
		if(!rival::selected(name, mask))
			// Выходим без выполнения сценария
			return;
		...
		// Создаём объект сжатия заголовков сравниваемой реализации
		Codec codec;
		// Если блоки заголовков декодированы с ошибкой
		if(!rival::decoding(codec, measured, outcome)){
			// Выводим сообщение о невыполненном сценарии
			rival::skip(name, "прогон не выполнен: блок заголовков не декодирован");
			// Выходим из сценария
			return;
		}
		// Буфер сведений о прогоне
		...
		// Если измерялась скорость декодирования
			...
			// Формируем сведения о прогоне
			::snprintf(
				details, sizeof(details), "блоков: %zu, объём потока: %.1f МБ (%.1f октетов на блок), %.1f МБ/с",
				outcome.operations, (static_cast <double> (outcome.bytes) / 1048576.0),
				(static_cast <double> (outcome.bytes) / static_cast <double> (outcome.operations)),
				rival::megabytes(outcome.bytes, outcome)
			);
			// Выводим результат прогона сценария
			rival::report(name, "блоков/с", rival::perSecond(outcome), details);
		}
	...
	/**
	 * @brief Функция выполнения сценария измерения степени сжатия заголовков
	 *
	 * @param mask фильтр названий сценариев
	 *
	 */
	static void compressionScenario(const char * mask) noexcept {
		// Название выполняемого сценария
		static constexpr const char * NAME = "http2/hpack/compression";
		// Если название сценария не соответствует фильтру
		if(!rival::selected(NAME, mask))
			// Выходим без выполнения сценария
			return;
		...
		rival::report(
			NAME, "% от исходного",
			((outcome.original > 0) ? ((static_cast <double> (outcome.bytes) / static_cast <double> (outcome.original)) * 100.0) : 100.0),
			details
		);
	}
	/**
	 ...
	 */
		...
		static void requestScenario(const bool measured, const char * mask) noexcept {
			// Название выполняемого сценария
			const char * name = (measured ? "http2/allocations/per-request" : "http2/parse/request-stream");
			// Если название сценария не соответствует фильтру
			if(!rival::selected(name, mask))
				// Выходим без выполнения сценария
				return;
			...
			const std::string & stream = rival::requestStream();
			// Итоги прогона сценария
			rival::outcome_t outcome;
			...
			// Если измерялась скорость разбора потока запросов
				...
				::snprintf(
					...
					outcome.operations, (static_cast <double> (stream.size()) / 1048576.0),
					(static_cast <double> (stream.size()) / static_cast <double> (outcome.operations)),
					rival::megabytes(stream.size(), outcome)
				);
				// Выводим результат прогона сценария
		...
		static void bodyScenario(const bool measured, const char * mask) noexcept {
			// Название выполняемого сценария
			const char * name = (measured ? "http2/allocations/per-data-frame" : "http2/parse/data-body");
			// Если название сценария не соответствует фильтру
			if(!rival::selected(name, mask))
				// Выходим без выполнения сценария
				return;
			...
			const std::string & stream = rival::bodyStream();
			/**
		...
		static void exchangeScenario(const size_t streams, const char * mask) noexcept {
			// Название выполняемого сценария
			const char * name = ((streams > 1) ? "http2/session/multiplexed" : "http2/session/round-trip");
			// Если название сценария не соответствует фильтру
			if(!rival::selected(name, mask))
				// Выходим без выполнения сценария
				return;
...
int32_t main(int32_t argc, char ** argv) noexcept {
	...
	rival::attach();
	// Выполняем сценарий кодирования блоков заголовков
	::encodeScenario(mask);
	// Выполняем сценарий декодирования блоков заголовков
	::decodeScenario(false, mask);
	// Выполняем сценарий измерения степени сжатия заголовков
	::compressionScenario(mask);
	// Выполняем сценарий количества выделений памяти на декодированный блок
	 ...
	 */
		 ...
		 */
		for(const char * name : {
			"http2/parse/request-stream", "http2/parse/data-body",
			"http2/session/round-trip", "http2/session/multiplexed",
			"http2/allocations/per-request", "http2/allocations/per-data-frame"
		}){
