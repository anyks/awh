/*
 * Удельный расход памяти на одно наблюдаемое подключение, libuv.
 *
...
// Пустые обработчики выделения буфера и готовности к чтению
static void onAlloc(uv_handle_t *, size_t, uv_buf_t * buf){ buf->base = nullptr; buf->len = 0; }
...
int main(int argc, char ** argv){
	...
	for(size_t i = 0; i < count; i++){
		...
		for(const int fd : {client, peer}){
			...
			uv_tcp_t * w = new uv_tcp_t;
			uv_tcp_init(loop, w);
			uv_tcp_open(w, fd);
			uv_read_start(reinterpret_cast <uv_stream_t *> (w), onAlloc, onRead);
