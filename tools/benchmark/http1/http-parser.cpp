 ...
 *
 * @brief Эталонный стенд сравнения парсера протокола HTTP/1.x с парсером http-parser
 *        проекта Node.js — потоковым конечным автоматом, написанным вручную
 *
...
/**
 * @brief Обвязка парсера http-parser под интерфейс прогона сценариев
 *
 */
namespace {
	/**
	 * @brief Класс разбора сообщений парсером http-parser
	 *
	 * @details Развитие парсера прекращено в пользу llhttp, но он остаётся в
	 *          эксплуатации у множества проектов и представляет в сравнении
	 *          класс написанных вручную конечных автоматов, к которому
	 *          относится и парсер nginx: именно из парсера nginx он и вырос
	 *
	 */
	class Engine {
		private:
		...
		private:
			 ...
			 *
			 */
			static int32_t field([[maybe_unused]] http_parser * parser, [[maybe_unused]] const char * at, size_t length) noexcept {
			 ...
			 *
			 */
			static int32_t value([[maybe_unused]] http_parser * parser, [[maybe_unused]] const char * at, size_t length) noexcept {
			 ...
			 *
			 */
			static int32_t body([[maybe_unused]] http_parser * parser, const char * at, size_t length) noexcept {
			 ...
			 *
			 */
			static int32_t finish(http_parser * parser) noexcept {
		...
		public:
			 ...
			 *
			 * @note Отдельной операции сброса парсер не предоставляет: состояние
			 *       возвращается в исходное повторной инициализацией, которая
			 *       сохраняет пользовательские данные объекта
			 *
			 */
			void reset() noexcept {
			...
			bool feed(const char * data, const size_t size) noexcept {
				...
				return (::http_parser_execute(&this->_parser, &this->_settings, data, size) == size);
			}
			/**
