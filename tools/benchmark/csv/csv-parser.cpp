 ...
 *
 * @brief Эталонный стенд сравнения контейнера CSV на реализации Vince's CSV Parser
 *
 ...
 *
 * @details Разбор ведётся без объявления заголовка: реализация умолчанием берёт
 * первую запись именами столбцов, а прочие стенды заголовка не выделяют, и сравнивать
 * следует одинаковую работу
 *
 * @note Многопоточный разбор отключён намеренно: сравниваются однопоточные
 * реализации, и оставить его значило бы сравнивать число ядер, а не разбор
 *
...
static bool parse(const std::string & text) noexcept {
	// Собираемые настройки разбора текста таблицы
	csv::CSVFormat format;
	// Устанавливаем знак-разделитель полей
	format.delimiter(',');
	// Устанавливаем знак кавычек, обрамляющих поле
	format.quote('"');
	// Отключаем выделение заголовка из первой записи
	format.no_header();
	// Отключаем разбор в отдельном потоке
	format.threading(false);
	// Отключаем умозрительный разбор в нескольких потоках
	format.speculative_parallel_threads(1);
	// Дозволяем записи с разным числом полей
	...
	try {
		...
		csv::CSVReader reader = csv::parse(csv::string_view(text.data(), text.size()), format);
		/**
		...
		for(csv::CSVRow & row : reader){
			...
			for(csv::CSVField & field : row){
				// Получаем содержимое очередного поля записи
				const csv::string_view value = field.get_sv();
