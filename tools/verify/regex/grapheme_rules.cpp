...
static const uint32_t table[] = {
	(1u<<LF), 0, 0, ESZ,
	ESZ|(1u<<EPI)|(1u<<PRE)|(1u<<HL)|(1u<<HV)|(1u<<HT)|(1u<<HLV)|(1u<<HLVT)|(1u<<OTH)|(1u<<RI),
	ESZ, ESZ|(1u<<HL)|(1u<<HV)|(1u<<HLV)|(1u<<HLVT), ESZ|(1u<<HV)|(1u<<HT), ESZ|(1u<<HT),
	ESZ|(1u<<HV)|(1u<<HT), ESZ|(1u<<HT), ESZ|(1u<<RI), ESZ, ESZ|(1u<<EPI), ESZ
};
...
static void probe(const vector <uint32_t> & codes){
	...
	if(bt.exec(prog, string_view(buffer, length), 0, out)) theirs = (out[0].second - out[0].first);
	...
	if(ours != theirs){
		...
		if(first.empty()){
			...
			char tmp[256];
			snprintf(tmp, sizeof(tmp), "последовательность %s: правила=%zu, модуль=%zu", text.c_str(), ours, theirs);
			first = tmp;
		}
	}
}
int main(){
	...
	printf("сличений правил разбиения: %zu, расхождений: %zu\n", checked, diverged);
