...
int main(int argc, char ** argv){
	...
	const auto dashed = [&](const char * mark) noexcept -> std::string {
		 ...
		 */
		/**
		 * @warning Возврат каретки в перечне знаков ОБЯЗАТЕЛЕН: описание языка числит
		 *          `CR LF` законным концом строки наравне с `LF`, и при оконных концах
		 *          строк за чертою стоит именно он. Без него щуп объявлял чертою лишь
		 *          строки с концом `LF`, а признаки начала и конца документа при `CRLF`
		 *          пропадали из выдачи - и выглядело это пороком РАЗБОРА, каким не
		 *          являлось: события чтения в обоих случаях одинаковы, что и доказано
		 *          щупом по самому разбору. Нашло это сличение на стенде Windows
		 */
