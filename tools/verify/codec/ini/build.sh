...
#
# @brief Сборка стенда сличения разбора INI с эталоном вместе с получением корпуса
#
# @details Кодек INI опирается лишь на заголовочные файлы разбора чисел, и собрать
#          стенд можно шестью вызовами собирателя, не собирая библиотеки целиком
#
# @note Корпус не забирается со стороны, а порождается: общепризнанного набора сверки у
#       записи INI нет и быть не может, ибо единого описания у неё нет вовсе. Порождается
#       он воспроизводимо - зерно закреплено, - и потому одинаков на всякой машине
#
# @copyright Copyright © 2026
#
# Вызов:
#   tools/verify/codec/ini/build.sh [каталог сборки]
#
...
if [ -z "$(ls -A "$OUTPUT/corpus" 2>/dev/null)" ]; then
	...
	# Выводим сообщение о составлении корпуса
	echo "Составляем корпус текстов настроек"
	# Выполняем составление корпуса
	python3 "$ROOT/tools/verify/codec/ini/corpus.py" "$OUTPUT/corpus"
fi
...
echo "Собираем стенд сличения INI: $COMPILER"
...
$COMPILER $OPTIONS "$ROOT/tools/verify/codec/ini/dump.cpp" $OBJECTS $SYSTEM_LIBS -lz -o "$OUTPUT/dump"
