...
#          по нулю сличённых текстов
#
# @warning Непустоты каталога тоже НЕДОВОЛЬНО: уборка временного хранилища сносит файлы,
#          оставляя дерево каталогов, - забор корпуса такой остаток отменял, а поверка
#          валилась после четырёх пройденных шагов сообщением «корпус пуст либо не
#          забран». Признаком служит наличие самих случаев, то есть файлов «.xml»
if [ "$(find "$OUTPUT/corpus" -name '*.xml' 2>/dev/null | head -1 | wc -l)" -eq 0 ]; then
