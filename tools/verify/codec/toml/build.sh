...
#
# @brief Сборка стенда сличения разбора TOML с эталоном вместе с получением корпуса
#
# @details Кодек TOML опирается лишь на заголовочные файлы разбора чисел, и собрать
#          стенд можно шестью вызовами собирателя, не собирая библиотеки целиком
#
# @note Корпус забирается из набора toml-test - общепризнанного собрания текстов, где для
#       всякого годного дано ожидаемое дерево записью JSON, а негодный обязан быть
#       отвергнут. Забирается он единожды: при наличии каталога загрузка пропускается
#
# @copyright Copyright © 2026
#
# Вызов:
#   tools/verify/codec/toml/build.sh [каталог сборки]
#
...
if [ -z "$(ls -A "$OUTPUT/corpus" 2>/dev/null)" ]; then
	...
	git clone --depth 1 -q https://github.com/toml-lang/toml-test "$OUTPUT/suite"
	# Выполняем заведение каталога корпуса
	mkdir -p "$OUTPUT/corpus"
	# Выполняем перенос годных и негодных текстов в каталог корпуса
	cp -r "$OUTPUT/suite/tests/valid" "$OUTPUT/suite/tests/invalid" "$OUTPUT/corpus/"
	#
	# Выполняем перенос перечня случаев описания версии 1.0.0
	#
	# @note Набор несёт случаи двух описаний разом, а кодек ведёт разбор по 1.0.0 - так
	#       и сказано в заголовке его. Случаи 1.1.0 сличать им значило бы вменять кодеку
	#       в отказ то, чего он не обещал, и перечень этот их отсеивает
	#
	cp "$OUTPUT/suite/tests/files-toml-1.0.0" "$OUTPUT/corpus/"
fi
...
#       правило это несёт с самого начала, а сборщик сличения не нёс. Вскрылось это
