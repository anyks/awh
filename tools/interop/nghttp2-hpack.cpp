 ...
 *          распаковщику эталона: расхождение хотя бы в одном октете значения
 *          означает, что таблицы разошлись. На обычной нагрузке такое расхождение
 *          не всплывает - оно проявляется только тогда, когда пир сошлётся именно
 *          на разошедшуюся запись, - поэтому сверяется вся таблица целиком,
 *          а не те записи, что попали в набор
 *
 * @param entries количество сверенных записей
 *
 */
static void staticTableAgainstReference(size_t & entries) noexcept {
	// Объект распаковщика эталонной реализации
	nghttp2_hd_inflater * inflater = nullptr;
	// Создаём объект распаковщика эталонной реализации
	if(::nghttp2_hd_inflate_new(&inflater) != 0){
		// Фиксируем расхождение
		::mismatch("статическая таблица", 0, "распаковщик эталона не создан");
		// Выходим из функции
		return;
	}
	/**
	 ...
	 */
	for(size_t index = 1; index <= h2::hpack::STATIC_TABLE_SIZE; index++){
		// Получаем сверяемую запись нашей статической таблицы
		const h2::hpack::static_entry_t * entry = h2::hpack::staticTable(index);
		// Собираемый блок заголовков из единственной ссылки на запись
		std::string block;
		// Дописываем ссылку на запись статической таблицы
		h2::hpack::prefixed::encode(block, index, 7, 0x80);
		// Декодированные эталоном заголовки блока
		std::vector <field_t> decoded;
		// Если эталон не смог разобрать ссылку на статическую таблицу
		if(!::inflate(inflater, block, decoded) || (decoded.size() != 1)){
			// Фиксируем расхождение
			::mismatch("статическая таблица", index, "эталон не смог разобрать ссылку");
			// Переходим к следующей записи
			continue;
		}
		// Если название либо значение записи разошлись
		if((decoded[0].name != entry->name) || (decoded[0].value != entry->value)){
			// Фиксируем расхождение
			::mismatch(
				"статическая таблица", index,
				("у эталона [" + decoded[0].name + ": " + decoded[0].value + "], у нас [" + std::string(entry->name) + ": " + std::string(entry->value) + "]")
			);
		}
		// Считаем сверенную запись
		entries++;
	}
	// Удаляем объект распаковщика эталонной реализации
...
int main(){
	...
	::staticTableAgainstReference(entries);
	/**
	...
	std::cout << "сверено записей таблицы: " << entries << ", блоков: " << (total * 2) << ", расхождений: " << failures << std::endl;
	// Выводим результат
