...
#
# Путь к библиотеке языка C++ того собирателя, каким собран ворошитель
#
# @note Путь этот прописывается в двоичный файл: у DragonFly рядом стоят несколько
#       собирателей, и ворошитель, собранный `g++14`, при запуске подхватывал
#       `libstdc++` от gcc11 и отваливался с «version GLIBCXX_3.4.32 not found».
#       Сборка при этом зелёная, а прогон отвечает отказом, и находку пойдут искать
#       в кодеке, какого там нет
#
...
case "$(uname -s)" in
	...
	#          «backtrace» лежит в libc, оттого дефект и не всплывал до 31.08.2026
	##
	##
	...
	##
	# @warning Довод «-pthread» нужен и САМОЙ FreeBSD: замер 05.09.2026 целью «bridge»
	#          отвечает «undefined symbol: pthread_create». Прежние заметки здесь
	#          дважды утверждали обратное («у FreeBSD нехватки не видно») — утверждение
	#          это опиралось на цели, тела фреймворка НЕ тянущие, и потому неверно.
	#          Правило проще исключений: библиотека потоков подключается собирателем
	#          не всюду, и молчание одной машины о переносимости не говорит ничего
	##
	FreeBSD) SYSTEM_LIBS="-pthread" ;;
	*) SYSTEM_LIBS="" ;;
esac
...
#
# @details «src/codec/replace.cpp» несёт переносимую подмену: у MS Windows «rename»
#          существующий файл НЕ заменяет, и вторая запись по тому же пути не удавалась
#          вовсе. Тело её сперва стояло в заголовке, и обход о ней не знал; перенос
#          тела в исходник оборвал связывание ворошителей ВСЕХ СЕМИ кодеков разом -
#          «awh::codec::replace» не находилось ни у одной цели
#
# @note Проверено на целях «toml», «yaml», «ini» и «json»: до правки не собиралась ни
#       одна, после - собираются все. Судить по одной цели нельзя, как и указано выше
##
