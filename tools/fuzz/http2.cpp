...
int32_t main(int32_t argc, char * argv[]) noexcept {
	...
	for(int32_t iteration = 0; iteration < count; ++iteration){
		...
		for(int32_t direct = 0; direct < 2; ++direct){
			...
			/**
			 * Функции обратного вызова анонсов обязательны, а не декоративны: без них
			 * парсер кадр не разбирает вовсе, и весь разбор длин остался бы непройденным
			 */
			parser->on(parser_http2_t::altsvc_callback_t([&](const uint32_t, const string_view origin, const string_view value) noexcept {
				// Обращаемся к представлениям анонса: они ссылаются во входной буфер
				volatile size_t length = (origin.size() + value.size());
				// Не используемый результат
				(void) length;
				// Изредка приложение сбрасывает состояние соединения прямо из обработчика
				if((rng() % 256) == 0)
			...
			// Устанавливаем функцию обратного вызова для обработки набора origin
			parser->on(parser_http2_t::origin_callback_t([&](const string_view origin) noexcept {
				// Обращаемся к представлению origin: оно ссылается во входной буфер
				volatile size_t length = origin.size();
				// Не используемый результат
				(void) length;
				/**
				 * Изредка приложение сбрасывает состояние соединения прямо из обработчика:
				 * набор доставляется циклом, и сброс посреди него обязан цикл прервать
				 */
				if((rng() % 128) == 0)
					// Выполняем сброс состояния соединения
					parser->reset();
			...
			for(int32_t i = 0; i < total; ++i){
				...
				switch(rng() % 14){
					...
					case 10: {
						/**
						 * PRIORITY_UPDATE шлёт только клиент: серверу такие кадры слать
						 * не вправе, а клиент при их получении рвёт соединение
						 * (RFC 9218 §7.1). Когда испытуемый парсер - клиент, пир здесь
						...
						const uint32_t target = (1 + 2 * (rng() % 3));
					...
					// Анонс альтернативного сервиса
					case 12: {
						/**
						 * Анонс отправляет только сервер: присланный клиентом кадр
						 * получатель обязан игнорировать (RFC 7838 §4). Когда испытуемый
						 * парсер - сервер, пир здесь клиент, и кадр формировать нельзя
						 */
						if(server)
							// Прекращаем формирование кадра
							break;
						// Буфер полезной нагрузки кадра
						string payload;
						// Таблица проверяемых origin анонса
						static const char * origins[] = {"https://example.com", "", "https://[::1]:443", "https://\xd0\xbf\xd1\x80\xd0\xb8\xd0\xbc\xd0\xb5\xd1\x80"};
						// Выбираем origin анонса
						const string origin(origins[rng() % 4]);
						/**
						 * Нулевой поток означает анонс для соединения и требует origin,
						 * ненулевой - наоборот. Выбор ведётся независимо от origin:
						 * запрещённые сочетания парсер обязан игнорировать, а не падать
						 */
						const uint32_t sid = ((((rng() & 1) != 0) && !open.empty()) ? open[rng() % open.size()] : 0);
						// Дописываем старший байт длины origin
						payload.push_back(static_cast <char> ((origin.size() >> 8) & 0xFF));
						// Дописываем младший байт длины origin
						payload.push_back(static_cast <char> (origin.size() & 0xFF));
						// Дописываем origin анонса
						payload += origin;
						// Таблица проверяемых значений поля Alt-Svc
						static const char * values[] = {"h3=\":443\"; ma=3600", "clear", "", "h2=\"alt:443\", h3=\":443\"; persist=1"};
						// Дописываем значение поля Alt-Svc
						payload += values[rng() % 4];
						/**
						 * Изредка объявленная длина origin выводится за пределы нагрузки:
						 * это ровно тот путь разбора, где ошибка в арифметике даёт чтение
						 * за границей буфера
						 */
						if(((rng() % 8) == 0) && (payload.size() >= 2))
							// Задираем старший байт длины origin
							payload[0] = static_cast <char> (0x7F);
						/**
						 * Изредка нагрузка обрезается короче двух октетов длины: без этого
						 * ветвь усечённого кадра остаётся непройденной, а вычитание
						 * из беззнаковой длины там даёт огромное число
						 */
						if((rng() % 16) == 0)
							// Обрезаем нагрузку до нуля либо одного октета
							payload.resize(rng() % 2);
						// Формируем кадр анонса альтернативного сервиса
						current = ::frame(0x0A, 0x00, sid, payload);
					} break;
					// Набор origin, обслуживаемых соединением
					case 13: {
						/**
						 * Набор объявляет сервер: присланный клиентом кадр игнорируется
						 * (RFC 8336 §2.1), поэтому пир здесь обязан быть сервером
						 */
						if(server)
							// Прекращаем формирование кадра
							break;
						// Буфер полезной нагрузки кадра
						string payload;
						// Таблица проверяемых записей набора
						static const char * entries[] = {"https://example.com", "https://cdn.example.com", "", "https://a.example.com:8443"};
						// Выбираем количество записей набора
						const size_t count = (rng() % 5);
						/**
						 * Выполняем формирование всех записей набора
						 */
						for(size_t i = 0; i < count; i++){
							// Выбираем очередную запись набора
							const string origin(entries[rng() % 4]);
							// Дописываем старший байт длины записи
							payload.push_back(static_cast <char> ((origin.size() >> 8) & 0xFF));
							// Дописываем младший байт длины записи
							payload.push_back(static_cast <char> (origin.size() & 0xFF));
							// Дописываем очередную запись набора
							payload += origin;
						}
						/**
						 * Изредка длина первой записи выводится за пределы нагрузки:
						 * набор разбирается циклом, и выход за границу обязан отвергнуть
						 * кадр целиком, а не отдать половину записей
						 */
						if(((rng() % 8) == 0) && (payload.size() >= 2))
							// Задираем старший байт длины первой записи
							payload[0] = static_cast <char> (0x7F);
						/**
						 * Изредка в хвосте набора остаётся лишний октет: запись короче
						 * двух октетов длины - отдельная ветвь разбора, и без такого
						 * хвоста она остаётся непройденной
						 */
						if((rng() % 16) == 0)
							// Дописываем лишний октет в хвост набора
							payload.push_back(static_cast <char> (rng() & 0xFF));
						/**
						 * Изредка кадр уходит в поток: кадр относится к соединению целиком,
						 * и в потоке он подлежит игнорированию
						 */
						const uint32_t sid = ((((rng() % 8) == 0) && !open.empty()) ? open[rng() % open.size()] : 0);
						// Формируем кадр набора origin
						current = ::frame(0x0C, 0x00, sid, payload);
					} break;
				}
