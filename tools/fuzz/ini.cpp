 ...
 *
 * @details Кодировка текста опознаётся меткою порядка байтов, а без неё - расположением
 * нулевых байтов первых знаков. Длина такого текста обязана быть кратной размеру знака,
 * и снятие либо приписка одного байта исход разбора меняет ПО ПРАВУ. Проверка
 * независимости от хвостового перевода строки на таком тексте неприменима вовсе
 *
 * @note Взято у ворошителя YAML, где правило это стоит с самого заведения оси:
 *       расходиться двум ворошителям в столь общем вопросе не в чем
 */
...
namespace {
		...
		for(uint32_t i = 0; i < count; i++){
			...
			switch(engine() % 10){
				...
				case 5: {
					 ...
					 *
					 * @warning Изредка берётся имя, строгой грамматике наречия ПРОТИВНОЕ:
					 *          наречие Git строит имя из буквы первым знаком, а далее из
					 *          букв, цифр и черты - и ничего более. Замер покрытием показал,
					 *          что до обоих мест отказа по этой грамматике ворошитель не
					 *          доходил вовсе: набор строил одни лишь имена в одну букву,
					 *          годные при всяком наречии
					 */
					if((engine() % 10) == 0){
						// Набор имён, строгой грамматике наречия противных
						static const char * const strange[] = {"1k", "k_2", "k.2", "_k", "k:2"};
						// Дописываем имя свойства, грамматике противное
						result.append(strange[engine() % (sizeof(strange) / sizeof(strange[0]))]);
					// Дописываем имя свойства
					} else result.append(1, static_cast <char> ('k' + (engine() % 4)));
					/**
					 * Если требуется растянуть ИМЯ свойства на строки продолжения
					 *
					 * @details Продолжение отступом знак конца строки сохраняет, и имя, из
					 *          двух строк собранное, несёт его внутри себя. Записью такое имя
					 *          не отведено, и чтение отвергает его кодом `INVALID_KEY`
					 *
					 * @warning Строил ворошитель продолжения одним лишь ЗНАЧЕНИЯМ, и место
					 *          отказа по имени, из строк продолжения собранному, не задевалось
					 *          вовсе: восемь подборов написаний вслепую его не нашли, нашла
					 *          подмена кода отказа особым. Зелёный ворошитель отчитывается лишь
					 *          о том, что он строит
					 */
					if((engine() % 12) == 0){
						// Дописываем перевод строки и отступ продолжения имени
						result.append("\n  ");
						// Дописываем продолжение имени свойства
						result.append(1, static_cast <char> ('m' + (engine() % 3)));
					}
					/**
	...
	bool tree(const string & text, const ini::reader_t::settings_t & reading, mt19937 & engine) noexcept {
		 ...
		 */
		/**
		 * @warning Правило это ведётся по БАЙТАМ, а байт хвостовой у кодировок
		 *          многобайтовых переводом строки не является: текст UTF-16BE, байтами
		 *          `0D 0A` оканчивающийся, несёт знаком последним букву U+0D0A, и
		 *          снятие их сносит СОДЕРЖИМОЕ, а не перевод. Записи расходились тогда
		 *          по праву, и ворошитель подавал на то ложное расхождение. Мерить
		 *          надлежит по знакам, а не по байтам, - до того проверка таким текстам
		 *          не подлежит. Ограда взята у ворошителя YAML, где правило это стоит с
		 *          самого заведения оси: расходиться двум ворошителям тут не в чем
		 */
		if(!::multibyte(text) && !text.empty() && (text.back() != '\r')){
			/**
			 ...
			 *
			 * @warning Строка разбивается ОБОИМИ знаками конца: читающий INI признаёт
			 *          переводом строки и возврат каретки одинокий, - проверено разбором
			 *          текста `[c]\r[d]\r\n` на два раздела. Ища один лишь перевод,
			 *          проверка не видела строки пустой, возвратом одиноким открытой:
			 *          написание `  [c]\r\r\n` оканчивается строкою пустою, снятие
			 *          хвостового перевода снимает её ПО ПРАВУ, а ворошитель подавал на
			 *          то ложное расхождение записей
			 */
		 ...
		 */
		{
			...
			if(target.parse("")){
				 ...
				 */
				const bool planted = lifted.graft(target);
				/**
				 * Если перенос отказал, причины не назвав
				 *
				 * @note Поверяется не сам отказ, а НАЗВАНИЕ причины его: отказ без
				 *       причины отдаёт потребителю ложь - `error()` отвечает «ошибок не
				 *       обнаружено» при истинном отказе. Правило заведено по находке у
				 *       YAML, где причины не назначала нижняя ступень укладки
				 */
				if(!planted && (target.error() == ini::error_t::NONE)){
					// Выводим сообщение об отказе переноса без названной причины
					::fprintf(stderr, "ini fuzz: graft refused without a named cause\n");
					// Выводим исходный текст настроек
					dump(text);
					// Выводим результат проверки дерева настроек
					return false;
...
int32_t main(int32_t argc, char * argv[]) noexcept {
	...
	for(uint64_t i = 0; i < count; i++){
		...
		if((engine() % 2) == 0){
			...
			/**
			 * Выполняем перебор строгой поверки имён грамматикой наречия
			 *
			 * @warning Настройка эта ворошителем не перебиралась вовсе, и оба места отказа
			 *          по грамматике имени стояли непройденными: замер покрытием это и
			 *          показал. Ставится она изредка - грамматика наречия Git отвергает
			 *          имена с подчёркиванием да с точкой, а те строятся набором часто, - и
			 *          держи её всякий проход, прочие пути охвата лишились бы
			 */
			settings.strictNames = ((engine() % 6) == 0);
