...
namespace {
			...
			switch(engine() % 10){
				...
				case 6: {
					...
					switch(engine() % 4){
						...
						case 1: result.append(" ; примечание"); break;
						// Дописываем примечание, несущее знак разделителя
						case 2: result.append(" # см. далее: текст"); break;
					}
				} break;
	...
	bool sound(const vector <Event> & events, const ini::reader_t::settings_t & settings, const string & text) noexcept {
		 ...
		 *
		 */
		const auto clean = [&settings](const string & name) noexcept -> bool {
			/**
			 * Выполняем перебор знаков проверяемого имени
			 */
			for(size_t i = 0; i < name.length(); i++){
				/**
				 * Если знак начинает примечание, стоя в начале имени либо за пробельным знаком
				 */
				if(ini::commented(name[i], settings.comments) &&
				   ((i == 0) || (name[i - 1] == ' ') || (name[i - 1] == '\t') || (name[i - 1] == '\n') || (name[i - 1] == '\r')))
		...
		for(size_t i = 0; i < events.size(); i++){
			 ...
			 */
			if(!clean(event.key) || !clean(event.section) || !clean(event.subsection)){
