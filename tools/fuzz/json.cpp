...
namespace {
	...
	string numeric(mt19937_64 & engine) noexcept {
		...
		switch(engine() % 8){
			...
			case 2:
				...
				::snprintf(buffer, sizeof(buffer), "%d.%d", static_cast <int32_t> (engine() % 1000), static_cast <int32_t> (engine() % 1000));
			break;
			// Если число записано с порядком
			case 3:
				// Собираем запись числа с порядком
				::snprintf(buffer, sizeof(buffer), "%d.%de%s%d",
					static_cast <int32_t> (engine() % 100), static_cast <int32_t> (engine() % 100),
					((engine() % 2) != 0 ? "-" : "+"), static_cast <int32_t> (engine() % 40));
			break;
	...
	void assemble(mt19937_64 & engine, const json::reader_t::settings_t & settings, const size_t depth, string & result) noexcept {
		 ...
		 */
		switch((depth > 5) ? (engine() % 5) : (engine() % 8)){
	...
	bool streaming(const string & text, const json::document_t::settings_t & settings, Statistic & totals) noexcept {
		 ...
		 *
		 * @note Сборка целиком удерживает последний документ потока, и сличать её с
		 *       потоковой выдачей можно лишь на тексте об одном документе
		 */
		if(streamed.size() == 1){
