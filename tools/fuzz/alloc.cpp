...
/**
 * Подставляем пустое заведение ядра - лишь у отдельной сборки
 *
 * `main` зовёт `awh::fmk::initialize()` первой строкой, а отдельная сборка по описанию
 * (`tools/fuzz/README.md`) связывает один распределитель, без `src/sys/fmk.cpp`: без
 * подпорки она не связывалась вовсе (с 13.09.2026, у всех систем). Захват же ворошитель
 * делает сам, и пустое тело ничего у него не отнимает; предел снимка памяти и локаль
 * ворошению безразличны
 *
 * Сборщик `tools/fuzz/build.sh` связывает НАСТОЯЩИЙ `src/sys/fmk.cpp` и заявляет это
 * признаком `AWH_FUZZ_FRAMEWORK`: подпорка там дала бы задвоенное имя
 */
#if !defined(AWH_FUZZ_FRAMEWORK)
