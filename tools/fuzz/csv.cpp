...
namespace {
	...
	bool hostile(mt19937 & engine) noexcept {
		...
		/**
		 * Задаём знак начала строки примечания и запись метки порядка байтов
		 *
		 * @note Поля эти прежде не ворошились вовсе, а оба правят запись ПЕРВОГО поля
		 *       записи: поле, знаком примечания начатое, разбор числит примечанием, а
		 *       метку, стоящую в начале текста, снимает признаком кодировки. Оба случая
		 *       нашлись прежде щупом и проверкою, но не ворошителем
		 */
		writing.comment = ((engine() % 2) == 0 ? '\0' : '#');
		 ...
		 *
		 * @note Метка порядка байтов, настройкою затребованная, снимается ПРЕЖДЕ сличения:
		 *       разбор её снимает признаком кодировки, и текст «метка плюс конец строки»
		 *       пуст для него ровно так же. Прощение прежде писалось по всему тексту и
		 *       метку не снимало - едва ворошитель стал её порождать, оно перестало
		 *       срабатывать, и случай законный выдавался расхождением
		 */
		{
			// Записанный текст без метки порядка байтов
			string_view written(writer.text());
			// Если текст начат меткой порядка байтов, снимаем её
			if((written.length() >= 3) && (static_cast <uint8_t> (written[0]) == 0xEF) &&
			   (static_cast <uint8_t> (written[1]) == 0xBB) && (static_cast <uint8_t> (written[2]) == 0xBF))
				// Выполняем снятие метки порядка байтов
				written.remove_prefix(3);
			// Если записанное неотличимо от пустой строки
			if(written.find_first_not_of("\r\n") == string_view::npos)
				// Выводим результат проверки кругового хода
				return true;
		}
		 ...
		 *
		 * @note Знак берётся тем же, что и у записи: разбор, знака этого не знающий,
		 *       поле, им начатое, примечанием не сочтёт, и проверка прошла бы мимо
		 */
		reading.comment = writing.comment;
