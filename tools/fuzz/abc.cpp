...
namespace {
	...
	/**
	 * @brief Зерно источника случайных величин по умолчанию
	 *
	 * @details Умолчание закреплено намеренно: прогон обязан повторяться числом в число,
	 *          иначе найденный дефект не воспроизвести, а перепись прогона служит
	 *          сторожем правок - расхождение её означает, что кодек повёл себя иначе
	 *
	 */
	 ...
	 *
	 * @note Зерно сменяемо вторым доводом вызова: с одним зерном ворошитель ходит по
	 *       одним и тем же входам сколько его ни гоняй, и новых дефектов ему не найти.
	 *       Умолчание при том прежнее, и сторож переписи не теряется
	 *
	 */
...
int main(int argc, char * argv[]) noexcept {
	...
	// Зерно источника случайных величин
	uint64_t seed = SEED;
	/**
	 * Если зерно источника случайных величин подано параметром
	 */
	if(argc > 2){
		// Выполняем получение зерна источника случайных величин
		seed = static_cast <uint64_t> (::strtoull(argv[2], nullptr, 0));
		// Выполняем пересев источника случайных величин
		source.seed(seed);
	}
	/**
	 * Если зерно взято не умолчанием, доносим о том отдельной строкой
	 *
	 * @note Строка эта ОТДЕЛЬНАЯ намеренно: перепись прогона сличается с прежними
	 *       знак в знак, и печатай мы зерно в ней самой, прогон умолчанием
