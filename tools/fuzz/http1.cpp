...
namespace {
	...
	static std::string headers(bool & chunked, size_t & length, const std::string & host, traits_t & traits) noexcept {
		...
		if(!host.empty()){
			 ...
			 *
			 * Пробел внутри значения законным не бывает ни в одном написании и является
			 * формой подмены целевого узла: звенья цепочки, режущие значение по пробелу
			 * и не режущие, направят запрос на разные узлы (RFC 9112 §3.2)
			 */
			if(::chance(6))
				// Вносим пробельный символ и второй адрес в значение заголовка
				result.append(::chance(50) ? " " : "\t").append(::token(::pick(8) + 1)).append(".com");
			// Дописываем окончание строки заголовка адреса сервера
			result.append(::eol());
			/**
			 * Изредка дописываем второй заголовок адреса сервера
			 *
		...
		if(::chance(30)){
			...
			static const char * encodings[] = {
				...
				"chunked;ext=1", "gzip;q=1, chunked"
			};
...
namespace {
	...
	typedef struct Outgoing {
		...
		bool unfixable;
		// Признак объявления кадрирующего кодирования с параметрами
		bool parametrized;
	...
	static bool encodingDropped(const struct Outgoing & outgoing) noexcept {
		 ...
		 *
		 * Ответ переключения протокола, успешный ответ на CONNECT и ответы с кодом
		 * 1xx и 204 отправлять его не
	...
	static outgoing_t compose() noexcept {
		...
		if((result.direct == direct_t::RESPONSE) && ::chance(12)){
			...
			static const uint16_t codes[] = {100, 102, 204, 304};
		 ...
		 */
			...
			// Помечаем ответ как передающий соединение другому протоколу
			result.tunnel = true;
			// Выбираем между переключением протокола и туннелем через CONNECT
			if(::chance(50))
				// Собираем ответ переключения протокола
				result.code = static_cast <uint16_t> (101);
			// Собираем успешный ответ на запрос CONNECT
			else {
		...
		/**
		 * Изредка кадрирующее кодирование объявляется с параметрами
		 *
		 * Параметров chunked не определяет, и RFC 9112 §7.1 предписывает считать их
		 * присутствие ошибкой: собственный приёмник такое объявление отвергает, и
	...
	static std::string emit(const awh::fmk_t * fmk, const awh::log_t * log, const outgoing_t & outgoing, const bool pull, const bool source) noexcept {
		...
		if(outgoing.chunked)
			// Дописываем заголовок кодирования тела сообщения
			block.emplace("Transfer-Encoding", (outgoing.unfixable
			 ? "chunked, gzip" : (outgoing.parametrized ? "chunked;foo=bar" : "chunked")));
		...
		if(source && !outgoing.body.empty()){
			...
			sender.dataSource(parser_http_t::data_source_callback_t([&sender, &outgoing, &position, announced](const uint32_t, uint8_t * buffer, const size_t cap, bool & eof) noexcept -> int64_t {
				 ...
				 *
				 * Подготовка снимает признак завершённости, и следующий блок заголовков
				 * прошёл бы мимо проверки незавершённого сообщения - лёг бы строкой размера
				 * чанка прямо в чужое тело. Попытка обязана быть отвергнута без следа
				 */
				if(::chance(15))
					// Пробуем подготовить отправитель посреди незавершённого сообщения
					sender.resetSender();
				/**
				 * Пробуем назначить новый источник поверх исполняющегося
				 *
				 * Пока источник исполняется, объект функции принадлежит своему же вызову,
				 * и его уничтожение освободило бы память под ногами исполняющегося кода.
				 * Попытка обязана быть отвергнута, а выдача - продолжиться прежним источником
				 */
				if(::chance(15))
					// Пробуем назначить новый источник изнутри исполняющегося
					sender.dataSource(parser_http_t::data_source_callback_t(
					 [](const uint32_t, uint8_t *, const size_t, bool & end) noexcept -> int64_t {
						// Объявляем конец тела, ничего не выдавая
						end = true;
						// Выводим отсутствие данных
						return 0;
					 }));
