...
static vector <int32_t> descriptorList() noexcept {
	 ...
	 */
		 ...
		 *          вскрылась на переносе ворошителя под MS Windows: узкий перебор
		 *          прячет держателя ЗА собой, а прибор о том молчит
		 *
 ...
 * Если операционной системой является не MS Windows
 *
 * @note Ограждается САМО ОПРЕДЕЛЕНИЕ, а не одни лишь места вызова. Прежде ограждены были
 *       только вызовы, а определения стояли открытыми, и цель эта у MinGW не собиралась
 *       вовсе: ни `setitimer`, ни `ITIMER_REAL`, ни `suseconds_t` там нет. То есть
 *       ворошитель движка под MS Windows не запускался НИ РАЗУ - отказ сборки о том не
 *       сообщал никому, потому что собрать эту цель там никто не пробовал
 */
#if !defined(_WIN32) && !defined(_WIN64)
/**
...
#else
/**
 * Время начала текущего оборота опроса в миллисекундах, нуль - оборот не идёт
 *
 * @note Сторож у этой системы устроен ИНАЧЕ: ни `SIGALRM`, ни таймеров этого вида здесь
 *       нет, и прервать вызов, ушедший в ядро, нечем. Взамен заводится поток-наблюдатель,
 *       а оборот отмечает своё начало и конец этой меткой. Разница с путём POSIX в том,
 *       что находка сообщается СО СТОРОНЫ, а вставший поток так и остаётся стоять - но
 *       для отчёта это всё равно: работа прекращается с кодом отказа, и зависание
 *       перестаёт быть молчаливым
 */
static std::atomic <uint64_t> __awh_poll_since__{0};
/**
 * @brief Функция получения текущего времени в миллисекундах
 *
 * @return текущее время монотонных часов
 *
 */
static uint64_t __awh_poll_now__() noexcept {
	// Выводим показание монотонных часов в миллисекундах
 ...
 */
static void __awh_poll_guard__(const uint32_t ms) noexcept {
	// Отмечаем начало оборота либо снимаем отметку по его концу
	::__awh_poll_since__.store(((ms > 0) ? ::__awh_poll_now__() : 0), std::memory_order_relaxed);
}
/**
 * @brief Функция потока-наблюдателя за оборотами опроса
 *
 * @details Спит малыми долями и сличает метку начала оборота с текущим временем: оборот,
 *          не вернувший управление за отведённый срок, означает вставший движок
 *
 */
static void __awh_poll_watcher__() noexcept {
	/**
	 * Выполняем присмотр за оборотами опроса
	 */
	while(true){
		// Засыпаем на долю срока: сторож обязан сработать заметно позже находки, а не сразу
		std::this_thread::sleep_for(std::chrono::milliseconds(100));
		// Получаем метку начала текущего оборота
		const uint64_t since = ::__awh_poll_since__.load(std::memory_order_relaxed);
		// Если оборот не идёт, сличать нечего
		if(since == 0)
			// Переходим к следующему кругу присмотра
			continue;
		// Если оборот в срок уложился, находки нет
		if((::__awh_poll_now__() - since) < static_cast <uint64_t> (::__awh_poll_limit__))
			// Переходим к следующему кругу присмотра
			continue;
		// Сообщаем о находке в поток ошибок
		::fprintf(stderr,
			"\nНАХОДКА: движок не вернул управление из одного оборота опроса за %u мс - опрос встал\n",
			::__awh_poll_limit__
		);
		/**
		 * @warning Стека вставшего потока здесь НЕТ, и это не недоработка, а граница
		 *          способа: снимается он у потока, который его снимает, а встал -
		 *          другой. Снять чужой стек тут можно лишь остановкой потока с обходом
		 *          его кадров, и это работа отдельная. Находка сообщает, ЧТО движок
		 *          встал; ГДЕ - смотреть отладчиком по живому процессу, он ещё стоит
		 */
		// Сбрасываем поток ошибок: работа прекращается сразу за этим
		::fflush(stderr);
		// Выходим с кодом ошибки
		::_exit(EXIT_FAILURE);
	}
...
static void turns(awh::engine::io_t & io, mt19937_64 & engine) noexcept {
	...
	for(uint64_t i = 0; (i < count) && (std::chrono::steady_clock::now() < deadline); i++){
		...
		::totals.polls++;
		/**
		 * Взводим сторож на один оборот опроса
		 *
		 * @note Устройство сторожа у систем разное - сигнал у POSIX, поток-наблюдатель
		 *       у MS Windows, - а места взвода и снятия общие: выбор способа сделан
		 *       один раз, при его заведении, и разводить его ещё и здесь незачем
		 */
		__awh_poll_guard__(__awh_poll_limit__);
		// Выполняем оборот опроса
		static_cast <void> (io.poll(static_cast <int32_t> (::pick(engine, 3))));
		// Снимаем сторож: оборот вернул управление
...
int main(int argc, char * argv[]) noexcept {
	...
	#else
		/**
		 * Заводим поток-наблюдатель за оборотами опроса
		 *
		 * @note Поток отвязывается намеренно: работа его бесконечна, и ждать его конца
		 *       при выходе означало бы не выйти вовсе
		 */
