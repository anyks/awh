...
namespace {
	...
	struct Totals {
		...
		uint64_t mirrored;
		// Количество несобранных записей по кодам причины отказа
		uint64_t refused[32];
		// Количество показанных примеров отказа сборки
		uint64_t shown;
		/**
		 * @brief Конструктор
		 *
		 */
		Totals() noexcept :
		 records(0), corrupted(0), events(0), survived(0), trees(0), rewrites(0), mirrored(0), refused{}, shown(0) {}
	};
	...
	bool tree(const string & text, const cef::reader_t::settings_t & given) noexcept {
		...
		if(rewritten.empty()){
			/**
			 * Считаем отказ сборки по коду его причины
			 *
			 * @details Прежде всякий отказ сборки признавался законным исходом без
			 *          разбора причины, и ворошитель молчал бы о новом дефекте записи
			 *          ровно так же, как о законном отказе. Счётчик по кодам показывает,
			 *          ЧЕМ окончились несобранные записи
			 */
			totals.refused[static_cast <size_t> (doc.error()) & 0x1F]++;
			// Если разбор примеров отказа включён и примеров ещё недостаточно
			if(::getenv("AWH_FUZZ_SHOW") != nullptr && (totals.shown < 3)){
				// Наращиваем количество показанных примеров
				totals.shown++;
				// Выводим исходную запись, сборка которой отвергнута
				::fprintf(stderr, "ПРИМЕР отказа «%s»: [%s]\n", cef::message(doc.error()), text.c_str());
			}
...
int32_t main(int32_t argc, char * argv[]) noexcept {
	...
	/**
	 * Если не построено ни одной записи
	 *
	 * @details Прогон без записей отчитывался прежде успехом: счётчики выводились
	 *          нулями, а код выхода был нулевым, и «ничего не проверено» становилось
	 *          неотличимо от «проверено и чисто». Ловится это на вызове с доводом,
	 *          какого разбор числа не понимает: «-runs=200000» обращается в нуль
	 *          проходов молча
	 *
	 * @note Заслон ставится ПОСЛЕ вывода статистики намеренно: числа видеть надобно и
	 *       при отказе, иначе причина остаётся неясной
	 */
	/**
	 * Выполняем перебор кодов причин отказа сборки
	 */
	for(size_t i = 0; i < 32; i++){
		// Если записи с таким кодом отказа встречались
		if(totals.refused[i] > 0)
			// Выводим количество несобранных записей этой причины
			::fprintf(
				stdout, "cef fuzz: отказов сборки «%s»: %llu\n",
				cef::message(static_cast <cef::error_t> (i)),
				static_cast <unsigned long long> (totals.refused[i])
			);
	}
	if(totals.records == 0){
		// Выводим сообщение об отсутствии построенных записей
		::fprintf(stderr, "%s fuzz: не построено ни одной записи - проверять нечего\n", "cef");
		// Выводим подсказку о правильном виде вызова
		::fprintf(stderr, "вызов: %s <число проходов> [зерно]\n", "cef-fuzz");
		// Выходим из приложения с кодом ошибки
		return EXIT_FAILURE;
	}
