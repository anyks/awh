		...
		for(uint32_t i = 0; i < count; i++){
			...
			if((engine() % 10) == 0){
				...
				static const char * const tags[] = {"!!str", "!свой", "!<tag:x,2000:mine>", "!",
				                                    ...
				                                    "!e!x"};
				/**
				 * @warning Метка `!!binary` из набора изъята НАМЕРЕННО: перенос значения в
				 *          чужое дерево свойств узла не берёт, и двоичное содержимое теряет
				 *          вид свой - `!!binary 12:30` наречием 1.1 читается обратно числом
				 *          семьсот пятьдесят. Порок этот местно не чинится: дерево способа
				 *          поставить узлу свойство не имеет вовсе, и завести его - решение
				 *          владельца. Он записан при самом методе `graft` и доложен; вернуть
				 *          метку в набор надлежит вместе с починкой
				 */
	...
	for(uint64_t i = 0; i < count; i++){
		 ...
		 */
		{
			...
			if(!text.empty() && (text.back() != '\r') && !::multibyte(text)){
				 ...
				 *          и требует. Исход разбора правилу тому подвластен ТОЖЕ, но лишь при
				 *          пределе длины значения: снятие перевода удержанного длину значения
				 *          укорачивает, и «- >+\n  2024-02-29\n\n» при пределе 11 отвергается
				 *          целиком, а усечённое - принимается. Оба исхода тут ВЕРНЫ
				 *
				 ...
				 */
				if((taken != parsed) && !(keeping && (settings.maxScalar > 0))){
					// Выводим сообщение о расхождении исхода разбора
