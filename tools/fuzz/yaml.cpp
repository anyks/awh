/**
 * @file yaml.cpp
 * @date 2026-08-17
 *
 * @license{LicenseRef-AWH-1.0}
 *
 * @author Yuriy Lobarev
 *
 * @telegram{forman}
 * @phone{+7 (910) 983-95-90}
 *
 * @email forman@anyks.com
 * @site https://anyks.com
 *
 * @brief Инструмент ворошения кодека YAML — построение текста описания наречия 1.2 с
 *        точечной порчей, подача его чтению целиком и кусками произвольного размера, а
 *        также в кодировках UTF-16 обоих порядков байтов, для поиска аварийных завершений,
 *        выходов за границы буфера и расхождений разбора
 *
 * @copyright Copyright © 2026
 *
 */

/**
 * Стандартные заголовочные файлы
 */
#include <string>
#include <vector>
#include <random>
#include <cstdio>
#include <cstring>
#include <cstdlib>
#include <cstdint>

/**
 * Подключаем заголовочный файл проекта
 */
#include <codec/yaml/yaml.hpp>
#include <sys/log.hpp>
#include <sys/fmk.hpp>

/**
 * @brief Метод проверки текста на опознание кодировкою многобайтовой
 *
 * @param text текст, проверке подлежащий
 * @return     признак опознания текста кодировкою многобайтовой
 *
 * @details Кодировка текста без метки порядка байтов опознаётся по расположению нулевых
 * байтов первых знаков - так велит описание YAML 1.2, - и текст, чей второй байт есть
 * нуль, читается как UTF-16. Длина такого текста обязана быть чётной, и снятие либо
 * приписка одного байта исход разбора меняет ПО ПРАВУ. Проверка независимости от
 * хвостового перевода строки на таком тексте неприменима вовсе
 */
static bool multibyte(const std::string & text) noexcept {
	/**
	 * Если текст меткою порядка байтов многобайтовой кодировки открывается
	 */
	if((text.size() >= 2) && ((static_cast <uint8_t> (text[0]) == 0xFF) || (static_cast <uint8_t> (text[0]) == 0xFE)))
		// Сообщаем об опознании текста кодировкою многобайтовой
		return true;
	/**
	 * Выполняем перебор первых четырёх байтов текста
	 */
	for(size_t i = 0; (i < 4) && (i < text.size()); i++){
		/**
		 * Если байт текста нулевым является
		 */
		if(text[i] == '\0')
			// Сообщаем об опознании текста кодировкою многобайтовой
			return true;
	}
	// Сообщаем о том, что текст многобайтовою кодировкою не опознаётся
	return false;
}

/**
 * @brief Пространство имён проверок этого файла
 *
 * @note Держится оно безымянным намеренно: проверки кодеков собираются одной
 *       программою, и одноимённые построения разных файлов иначе сходятся в
 *       одно, порождая порчу вдали от места её причины
 *
 */
namespace {
	/**
	 * @brief Объект журнала проверок с отключённым выводом
	 *
	 * @details Вывод отключается назначением пустого перечня приёмников: отказы
	 *          разбора проверки наводят намеренно, и журнал их засорял бы выдачу
	 *
	 */
	struct Silent {
		/**
		 * @brief Конструктор
		 *
		 */
		Silent() noexcept {
			// Выполняем отключение вывода логов
			awh::log::mode({});
		}
	};
}

/**
 * Используем стандартное пространство имён
 */
using namespace std;

/**
 * Подписываемся на пространство имён контейнеров данных
 */
using namespace awh::codec;

/**
 * @brief Внутренние вспомогательные средства генератора (внутренняя компоновка)
 *
 */
namespace {
	/**
	 * @brief Учёт проделанной работы
	 *
	 */
	struct Statistic {
		// Количество построенных текстов
		uint64_t texts;
		// Количество испорченных текстов
		uint64_t corrupted;
		// Количество текстов, разобранных чтением до конца
		uint64_t survived;
		// Количество выданных чтением событий
		uint64_t events;
		// Количество сличений подачи кусками
		uint64_t chunked;
		// Количество сличений кодировок
		uint64_t transcoded;
		/**
		 * Количество текстов, к сличению подачи в кодировке UTF-16 не допущенных
		 *
		 * @note Счётчик этот сторожит само сличение: доводы отсева законны, однако
		 *       отсев, ни в один счётчик не попадающий, отнял бы у прогона подачи молча,
		 *       а отчёт остался бы прежним
		 */
		uint64_t unconverted;
		/**
		 * Количество приведений к однобайтовым кодировкам, знаками текста не допущенных
		 *
		 * @note Счётчик этот тем же доводом, что и сосед его выше: тексты с кириллицей ни
		 *       ISO-8859-1, ни Windows-1252 не записываются, и круг их отсеивается законно -
		 *       но отсев молчаливый неотличим от охвата
		 */
		uint64_t unnarrowed;
		// Количество собранных деревьев документов
		uint64_t trees;
		// Количество деревьев, исходный текст удержавших
		uint64_t kept;
		// Количество деревьев, удержавших текст при сносе повторных пар
		uint64_t pruned;
		// Количество правок, чтением перезаписи сличённых
		uint64_t mirrored;
		// Количество круговых ходов снятого значения через перезапись
		uint64_t circling;
		// Количество деревьев, правку принявших
		uint64_t edited;
		// Количество владеющих значений, с деревьев снятых
		uint64_t taken;
		// Количество значений, потоковой сборкой собранных
		uint64_t assembled;
		// Количество круговых ходов перезаписи иным оформлением
		uint64_t restyled;
		// Количество разборов деревом, прежним разбором занятым
		uint64_t recycled;
		// Количество текстов, написанием хвостовым поверенных
		uint64_t tailed;
		// Количество круговых ходов, директивою наречия обойдённых
		uint64_t undirected;
		// Количество сличений записи свежей с записью, прежней подачей занятой
		uint64_t transcribed;
		// Количество попыток кругового переноса значения в дерево
		uint64_t grafts;
		// Количество удавшихся переносов значения в дерево
		uint64_t grafted;
		/**
		 * @brief Конструктор
		 *
		 */
		Statistic() noexcept :
		 texts(0), corrupted(0), survived(0), events(0), chunked(0), transcoded(0), unconverted(0), trees(0), kept(0), pruned(0),
		 mirrored(0), circling(0), edited(0), assembled(0), restyled(0), recycled(0), tailed(0), undirected(0), transcribed(0), taken(0), grafts(0), grafted(0) {}
	} totals;

	/**
	 * @brief Событие разбора, запомненное для сличения
	 *
	 * @details Сличается вся выдача разбора целиком: расхождение хотя бы одного признака
	 * означает зависимость разбора от нарезки исходного текста на куски
	 *
	 */
	struct Event {
		// Разновидность события и вид значения его
		uint8_t event, type;
		// Вид записи значения в исходном тексте
		uint8_t style;
		// Содержимое значения либо примечания
		string text;
		// Имя метки, событию предпосланной
		string anchor;
		// Метка типа, событию предпосланная
		string tag;
		// Место начала события в исходном тексте
		uint32_t line, column;
		// Глубина вложенности, где событие собрано
		uint32_t depth;
		// Смещение начала события от начала исходного текста
		uint64_t position;
		/**
		 * @brief Конструктор
		 *
		 */
		Event() noexcept :
		 event(0), type(0), style(0), line(0), column(0), depth(0), position(0) {}
		/**
		 * @brief Оператор сравнения
		 *
		 * @param other событие для сравнения
		 * @return      результат сравнения
		 *
		 */
		bool operator != (const Event & other) const noexcept {
			// Выполняем сличение всех признаков события
			return (
				(this->event != other.event) || (this->type != other.type) ||
				(this->style != other.style) || (this->text != other.text) ||
				(this->anchor != other.anchor) || (this->tag != other.tag) ||
				(this->line != other.line) || (this->column != other.column) ||
				(this->depth != other.depth) || (this->position != other.position)
			);
		}
	};

	/**
	 * @brief Метод набора путей ко всем узлам дерева документа
	 *
	 * @details Пути набираются для правки: правится узел по пути к нему, и путей этих
	 * надобен весь перечень, чтобы правка приходилась куда попало, а не в один корень
	 *
	 * @param value  ссылка на узел, от какого путь отсчитан
	 * @param path   путь к узлу, ссылкою данному
	 * @param result перечень набранных путей
	 *
	 */
	void collect(const yaml::document_t::value_t & value, const string & path, vector <string> & result) noexcept {
		/**
		 * Если путь к узлу не пуст
		 */
		if(!path.empty())
			// Выполняем добавление пути к узлу в перечень путей
			result.push_back(path);
		/**
		 * Если узел вместилищем не является
		 */
		if((value.kind() != yaml::kind_t::MAPPING) && (value.kind() != yaml::kind_t::SEQUENCE))
			// Выходим из набора путей
			return;
		// Номер очередного разбираемого ребёнка вместилища
		size_t index = 0;
		/**
		 * Выполняем перебор всех детей вместилища
		 */
		for(yaml::document_t::value_t child = value.begin(); child.valid(); child = child.next()){
			/**
			 * Если вместилище является отображением пар
			 */
			if(value.kind() == yaml::kind_t::MAPPING){
				/**
				 * Собираем звено пути к очередной паре отображения
				 *
				 * @details Имя записывается звеном по RFC 6901: косая черта обращается в `~1`,
				 * а сам знак отмены - в `~0`. Прежде имя бралось КАК ЕСТЬ, а имя с косою чертою
				 * пропускалось вовсе с доводом «путь к такому узлу не выразить»
				 *
				 * @warning Довод тот устарел дважды разом. Путь с косою чертою ныне выражается
				 *          отменяющей записью - и пропуск отнимал бы у ворошителя целый разряд
				 *          имён. Хуже того, имя со знаком отмены не пропускалось, а сырой путь
				 *          с ним ныне ОТВЕРГАЕТСЯ разбором: правка молча не пробовалась бы, и
				 *          ворошитель того не заметил бы - отказ приходит с названным кодом, а
				 *          сторож ловит лишь отказ безымянный. Потеря охвата вышла бы немой
				 */
				string token;
				/**
				 * Выполняем перебор всех знаков имени пары отображения
				 */
				for(const char letter : child.name()){
					/**
					 * Определяем очередной знак имени пары
					 */
					switch(letter){
						// Если знаком является тильда
						case '~': token.append("~0"); break;
						// Если знаком является косая черта
						case '/': token.append("~1"); break;
						// Если знак отменяющей записи не требует
						default: token.push_back(letter);
					}
				}
				// Выполняем набор путей к поддереву очередной пары
				collect(child, (path + "/" + token), result);
			/**
			 * Если вместилище является перечнем значений
			 */
			} else collect(child, (path + "/" + to_string(index)), result);
			// Выполняем переход к следующему ребёнку вместилища
			index++;
		}
	}
	/**
	 * @brief Метод вывода разбираемого текста в шестнадцатеричном виде
	 *
	 * @param text выводимый текст
	 *
	 */
	void dump(const string & text) noexcept {
		// Выводим заголовок разбираемого текста
		::fprintf(stderr, "yaml fuzz: text (%zu bytes)\n", text.length());
		/**
		 * Выполняем перебор всех байтов разбираемого текста
		 */
		for(size_t i = 0; i < text.length(); i++){
			/**
			 * Если байт начинает очередную строку вывода
			 */
			if((i % 16) == 0)
				// Выводим смещение начала строки вывода
				::fprintf(stderr, "%s%04zx: ", ((i > 0) ? "\n" : ""), i);
			// Выводим очередной байт разбираемого текста
			::fprintf(stderr, "%02x ", static_cast <uint8_t> (text.at(i)));
		}
		// Выводим окончание вывода разбираемого текста
		::fprintf(stderr, "\n%s\n", text.c_str());
		// Выполняем выталкивание накопленного вывода
		::fflush(stderr);
	}
	/**
	 * @brief Метод приведения текста из UTF-8 в UTF-16
	 *
	 * @details Приведение отвергает всякую последовательность, записью UTF-8 не
	 * являющуюся: иначе оно обратило бы текст, разбором отвергаемый, в текст правильный,
	 * и итоги двух кодировок разошлись бы по вине приведения, а не разбора
	 *
	 * @param source приводимый текст в кодировке UTF-8
	 * @param big    признак приведения прямым порядком байтов
	 * @param result приведённый текст в кодировке UTF-16
	 * @return       результат выполнения операции
	 *
	 */
	bool transcode(const string & source, const bool big, string & result) noexcept {
		// Выполняем сброс приведённого текста
		result.clear();
		// Получаем метку порядка байтов приведённого текста
		const uint32_t signature = 0xFEFF;
		/**
		 * Выполняем добавление метки порядка байтов приведённого текста
		 */
		if(big){
			// Выполняем добавление старшего байта метки порядка байтов
			result.push_back(static_cast <char> ((signature >> 8) & 0xFF));
			// Выполняем добавление младшего байта метки порядка байтов
			result.push_back(static_cast <char> (signature & 0xFF));
		/**
		 * Если приведение ведётся обратным порядком байтов
		 */
		} else {
			// Выполняем добавление младшего байта метки порядка байтов
			result.push_back(static_cast <char> (signature & 0xFF));
			// Выполняем добавление старшего байта метки порядка байтов
			result.push_back(static_cast <char> ((signature >> 8) & 0xFF));
		}
		// Смещение чтения приводимого текста
		size_t i = 0;
		/**
		 * Выполняем перебор всех знаков приводимого текста
		 */
		while(i < source.length()){
			// Получаем начальный байт очередного знака
			const uint8_t letter = static_cast <uint8_t> (source.at(i));
			// Кодовое значение очередного знака
			uint32_t code = 0;
			// Длина последовательности очередного знака
			size_t length = 0;
			/**
			 * Если знак записан одним байтом
			 */
			if(letter < 0x80){
				// Запоминаем кодовое значение знака
				code = letter;
				// Запоминаем длину последовательности знака
				length = 1;
			/**
			 * Если знак записан двумя байтами
			 */
			} else if((letter & 0xE0) == 0xC0) {
				// Запоминаем кодовое значение начала знака
				code = (letter & 0x1F);
				// Запоминаем длину последовательности знака
				length = 2;
			/**
			 * Если знак записан тремя байтами
			 */
			} else if((letter & 0xF0) == 0xE0) {
				// Запоминаем кодовое значение начала знака
				code = (letter & 0x0F);
				// Запоминаем длину последовательности знака
				length = 3;
			/**
			 * Если знак записан четырьмя байтами
			 */
			} else if((letter & 0xF8) == 0xF0) {
				// Запоминаем кодовое значение начала знака
				code = (letter & 0x07);
				// Запоминаем длину последовательности знака
				length = 4;
			/**
			 * Если последовательность знака построена ошибочно
			 */
			} else return false;
			/**
			 * Если последовательность знака в текст не помещается
			 */
			if((i + length) > source.length())
				// Выводим отрицательный результат выполнения операции
				return false;
			/**
			 * Выполняем перебор продолжающих байтов последовательности знака
			 */
			for(size_t j = 1; j < length; j++){
				// Получаем очередной продолжающий байт последовательности
				const uint8_t next = static_cast <uint8_t> (source.at(i + j));
				/**
				 * Если байт продолжающим не является
				 */
				if((next & 0xC0) != 0x80)
					// Выводим отрицательный результат выполнения операции
					return false;
				// Выполняем накопление кодового значения знака
				code = ((code << 6) | (next & 0x3F));
			}
			// Выполняем смещение положения чтения приводимого текста
			i += length;
			/**
			 * Если кодовое значение знака выходит за отведённый отрезок
			 */
			if((code > 0x10FFFF) || ((code >= 0xD800) && (code <= 0xDFFF)))
				// Выводим отрицательный результат выполнения операции
				return false;
			// Наименьшие кодовые значения, записываемые последовательностью каждой длины
			static constexpr uint32_t BOUNDS[] = {0x00000000, 0x00000000, 0x00000080, 0x00000800, 0x00010000};
			/**
			 * Если последовательность знака избыточна
			 */
			if(code < BOUNDS[length])
				// Выводим отрицательный результат выполнения операции
				return false;
			// Части записи знака кодировкой UTF-16
			uint32_t parts[2] = {code, 0};
			// Количество частей записи знака
			size_t count = 1;
			/**
			 * Если знак записывается суррогатной парой
			 */
			if(code > 0xFFFF){
				// Получаем кодовое значение знака за пределами основной плоскости
				const uint32_t value = (code - 0x10000);
				// Запоминаем старшую часть суррогатной пары
				parts[0] = (0xD800 + (value >> 10));
				// Запоминаем младшую часть суррогатной пары
				parts[1] = (0xDC00 + (value & 0x3FF));
				// Запоминаем количество частей записи знака
				count = 2;
			}
			/**
			 * Выполняем перебор всех частей записи знака
			 */
			for(size_t j = 0; j < count; j++){
				// Получаем старший байт очередной части записи знака
				const char high = static_cast <char> ((parts[j] >> 8) & 0xFF);
				// Получаем младший байт очередной части записи знака
				const char low = static_cast <char> (parts[j] & 0xFF);
				/**
				 * Если приведение ведётся прямым порядком байтов
				 */
				if(big){
					// Выполняем добавление старшего байта части записи
					result.push_back(high);
					// Выполняем добавление младшего байта части записи
					result.push_back(low);
				/**
				 * Если приведение ведётся обратным порядком байтов
				 */
				} else {
					// Выполняем добавление младшего байта части записи
					result.push_back(low);
					// Выполняем добавление старшего байта части записи
					result.push_back(high);
				}
			}
		}
		// Выводим положительный результат выполнения операции
		return true;
	}
	/**
	 * @brief Признак прохода, строящегося знаками однобайтовых кодировок
	 *
	 * @details Признак живёт от прохода к проходу и ставится генератором: он решает, из
	 *          какого набора берутся имена пар и записи значений. Без него круг кодировок
	 *          ISO-8859-1 и Windows-1252 проходил одними лишь знаками US-ASCII
	 *
	 */
	static bool latined = false;

	/**
	 * @brief Метод разбора текста UTF-8 на кодовые значения знаков
	 *
	 * @details Разбор отвергает всякую последовательность, записью UTF-8 не являющуюся,
	 *          тем же доводом, что и приведение к UTF-16: испорченный текст, приведением
	 *          выправленный, развёл бы итоги по вине ворошителя, а не разбора
	 *
	 * @param source разбираемый текст в кодировке UTF-8
	 * @param result перечень кодовых значений знаков текста
	 * @return       результат выполнения операции
	 *
	 */
	bool codepoints(const string & source, vector <uint32_t> & result) noexcept {
		// Выполняем сброс перечня кодовых значений
		result.clear();
		// Смещение чтения разбираемого текста
		size_t i = 0;
		/**
		 * Выполняем перебор всех знаков разбираемого текста
		 */
		while(i < source.length()){
			// Получаем начальный байт очередного знака
			const uint8_t letter = static_cast <uint8_t> (source.at(i));
			// Кодовое значение очередного знака
			uint32_t code = 0;
			// Длина последовательности очередного знака
			size_t length = 0;
			/**
			 * Определяем длину последовательности очередного знака
			 */
			if(letter < 0x80){
				// Запоминаем кодовое значение знака
				code = letter;
				// Запоминаем длину последовательности знака
				length = 1;
			} else if((letter & 0xE0) == 0xC0) {
				// Запоминаем кодовое значение начала знака
				code = (letter & 0x1F);
				// Запоминаем длину последовательности знака
				length = 2;
			} else if((letter & 0xF0) == 0xE0) {
				// Запоминаем кодовое значение начала знака
				code = (letter & 0x0F);
				// Запоминаем длину последовательности знака
				length = 3;
			} else if((letter & 0xF8) == 0xF0) {
				// Запоминаем кодовое значение начала знака
				code = (letter & 0x07);
				// Запоминаем длину последовательности знака
				length = 4;
			// Выводим отрицательный результат выполнения операции
			} else return false;
			/**
			 * Если последовательность за конец текста выходит
			 */
			if((i + length) > source.length())
				// Выводим отрицательный результат выполнения операции
				return false;
			/**
			 * Выполняем перебор продолжающих байтов последовательности
			 */
			for(size_t j = 1; j < length; j++){
				// Получаем очередной продолжающий байт последовательности
				const uint8_t next = static_cast <uint8_t> (source.at(i + j));
				/**
				 * Если байт продолжающим не является
				 */
				if((next & 0xC0) != 0x80)
					// Выводим отрицательный результат выполнения операции
					return false;
				// Выполняем накопление кодового значения знака
				code = ((code << 6) | (next & 0x3F));
			}
			// Выполняем смещение положения чтения разбираемого текста
			i += length;
			/**
			 * Если кодовое значение предела выходит либо суррогатом является
			 */
			if((code > 0x10FFFF) || ((code >= 0xD800) && (code <= 0xDFFF)))
				// Выводим отрицательный результат выполнения операции
				return false;
			// Наименьшие кодовые значения, записываемые последовательностью каждой длины
			static constexpr uint32_t BOUNDS[] = {0x00000000, 0x00000000, 0x00000080, 0x00000800, 0x00010000};
			/**
			 * Если знак записан последовательностью длиннее нужной
			 */
			if(code < BOUNDS[length])
				// Выводим отрицательный результат выполнения операции
				return false;
			// Выполняем запись кодового значения знака
			result.push_back(code);
		}
		// Выводим положительный результат выполнения операции
		return true;
	}
	/**
	 * @brief Метод приведения текста из UTF-8 в UTF-32
	 *
	 * @param source приводимый текст в кодировке UTF-8
	 * @param big    признак приведения прямым порядком байтов
	 * @param result приведённый текст в кодировке UTF-32
	 * @return       результат выполнения операции
	 *
	 */
	bool transcode32(const string & source, const bool big, string & result) noexcept {
		// Выполняем сброс приведённого текста
		result.clear();
		// Перечень кодовых значений знаков приводимого текста
		vector <uint32_t> codes;
		/**
		 * Если разобрать приводимый текст не удалось
		 */
		if(!codepoints(source, codes))
			// Выводим отрицательный результат выполнения операции
			return false;
		/**
		 * @brief Функция записи кодового значения четырьмя байтами
		 *
		 * @param code записываемое кодовое значение знака
		 *
		 */
		const auto append = [&result, big](const uint32_t code) noexcept -> void {
			/**
			 * Выполняем перебор всех четырёх байтов кодового значения
			 */
			for(size_t i = 0; i < 4; i++){
				// Получаем сдвиг очередного байта кодового значения
				const uint32_t shift = static_cast <uint32_t> (big ? ((3 - i) * 8) : (i * 8));
				// Выполняем добавление очередного байта кодового значения
				result.push_back(static_cast <char> ((code >> shift) & 0xFF));
			}
		};
		// Выполняем запись метки порядка байтов приведённого текста
		append(0xFEFF);
		// Смещение начала знаков приводимого текста
		size_t start = 0;
		/**
		 * Если приводимый текст меткою порядка байтов открывается
		 *
		 * @note Метка своя ставится приведением всегда, и метку приводимого текста надлежит
		 *       снять: иначе приведённый текст нёс бы две метки подряд, а вторая из них есть
		 *       уже не метка, а знак содержимого - и итоги двух кодировок разошлись бы по
		 *       вине приведения, а не разбора
		 */
		if(!codes.empty() && (codes.front() == 0xFEFF))
			// Выполняем пропуск метки порядка байтов приводимого текста
			start = 1;
		/**
		 * Выполняем перебор всех знаков приводимого текста
		 */
		for(size_t i = start; i < codes.size(); i++)
			// Выполняем запись очередного знака четырьмя байтами
			append(codes.at(i));
		// Выводим положительный результат выполнения операции
		return true;
	}
	/**
	 * @brief Метод приведения текста из UTF-8 в однобайтовую кодировку
	 *
	 * @details Метки порядка байтов однобайтовые кодировки не имеют вовсе, и опознать их
	 *          разбору нечем: кодировка навязывается ему настройками. Приведение отвергает
	 *          всякий знак, кодировкой не записываемый, - иначе оно подменило бы текст
	 *
	 * @param source   приводимый текст в кодировке UTF-8
	 * @param encoding кодировка, к какой приводится текст
	 * @param result   приведённый текст в однобайтовой кодировке
	 * @return         результат выполнения операции
	 *
	 */
	bool narrow(const string & source, const yaml::encoding_t encoding, string & result) noexcept {
		// Выполняем сброс приведённого текста
		result.clear();
		// Перечень кодовых значений знаков приводимого текста
		vector <uint32_t> codes;
		/**
		 * Если разобрать приводимый текст не удалось
		 */
		if(!codepoints(source, codes))
			// Выводим отрицательный результат выполнения операции
			return false;
		// Знаки Юникода, каким кодировка Windows-1252 отводит место управляющих знаков
		static constexpr uint32_t WINDOWS[32] = {
			0x20AC, 0x0000, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021,
			0x02C6, 0x2030, 0x0160, 0x2039, 0x0152, 0x0000, 0x017D, 0x0000,
			0x0000, 0x2018, 0x2019, 0x201C, 0x201D, 0x2022, 0x2013, 0x2014,
			0x02DC, 0x2122, 0x0161, 0x203A, 0x0153, 0x0000, 0x017E, 0x0178
		};
		/**
		 * Выполняем перебор всех знаков приводимого текста
		 */
		for(const uint32_t code : codes){
			/**
			 * Если знак принадлежит к US-ASCII
			 */
			if(code < 0x80){
				// Выполняем запись знака одним байтом
				result.push_back(static_cast <char> (code));
				// Выполняем переход к следующему знаку текста
				continue;
			}
			/**
			 * Если кодировка иных знаков не несёт вовсе
			 */
			if(encoding == yaml::encoding_t::ASCII)
				// Выводим отрицательный результат выполнения операции
				return false;
			/**
			 * Если знак записывается кодировкой напрямую
			 *
			 * @note Место знаков от 0x80 до 0x9F отведено управляющим знакам, а тем в тексте
			 *       YAML места нет: приведение их отвергает у обеих кодировок
			 */
			if((code >= 0xA0) && (code <= 0xFF)){
				// Выполняем запись знака одним байтом
				result.push_back(static_cast <char> (code));
				// Выполняем переход к следующему знаку текста
				continue;
			}
			/**
			 * Если кодировка знаков вне ISO-8859-1 не несёт
			 */
			if(encoding != yaml::encoding_t::CP1252)
				// Выводим отрицательный результат выполнения операции
				return false;
			// Признак того, что знак кодировкой Windows-1252 записан
			bool found = false;
			/**
			 * Выполняем перебор знаков, кодировке Windows-1252 отведённых
			 */
			for(size_t i = 0; i < 32; i++){
				/**
				 * Если очередной знак искомому равен
				 */
				if((WINDOWS[i] != 0) && (WINDOWS[i] == code)){
					// Выполняем запись знака одним байтом
					result.push_back(static_cast <char> (0x80 + i));
					// Запоминаем признак того, что знак записан
					found = true;
					// Выходим из перебора знаков кодировки
					break;
				}
			}
			/**
			 * Если знак кодировкой не записывается
			 */
			if(!found)
				// Выводим отрицательный результат выполнения операции
				return false;
		}
		// Выводим положительный результат выполнения операции
		return true;
	}
	/**
	 * @brief Метод построения имени пары отображения
	 *
	 * @param engine источник псевдослучайных чисел
	 * @return       построенное имя пары отображения
	 *
	 */
	string named(mt19937 & engine) noexcept {
		// Набор имён, из которых строится имя пары
		/**
		 * Если проход строится знаками, однобайтовым кодировкам посильными
		 *
		 * @details Кириллица ни ISO-8859-1, ни Windows-1252 не записывается, и круг этих
		 *          кодировок проходил одними лишь текстами US-ASCII: старшие байты обеих
		 *          разбором не исполнялись вовсе. Оттого часть проходов строится набором,
		 *          обеим кодировкам посильным, - в нём и знаки места управляющих знаков,
		 *          какие Windows-1252 отводит своей таблице
		 */
		if(latined){
			// Набор имён, однобайтовым кодировкам посильных
			static const char * const narrowed[] = {
				"host", "clé", "año", "±допуск", "°угол", "naïve", "€цена", "тире—имя", "a"
			};
			// Выводим построенное имя пары отображения
			return string(narrowed[engine() % (sizeof(narrowed) / sizeof(narrowed[0]))]);
		}
		static const char * const names[] = {
			"host", "port", "name", "value", "a", "b", "имя", "key-1", "key_2",
			"'дословное'", "\"с оградой\"", "0", "true", "no", "null", "~", "с пробелом"
		};
		// Выводим построенное имя пары отображения
		return string(names[engine() % (sizeof(names) / sizeof(names[0]))]);
	}
	/**
	 * @brief Метод построения записи скалярного значения
	 *
	 * @details Записи перебираются по всем видам, какие отводит описание: обе ограды,
	 * все виды чисел обеих схем, пустые значения, логические значения обоих наречий и
	 * записи, схемою за строку почитаемые
	 *
	 * @param engine источник псевдослучайных чисел
	 * @return       построенная запись скалярного значения
	 *
	 */
	string scalared(mt19937 & engine) noexcept {
		/**
		 * Если проход строится знаками, однобайтовым кодировкам посильными
		 */
		if(latined){
			// Набор записей, однобайтовым кодировкам посильных
			static const char * const narrowed[] = {
				"café", "ñandú", "±0.5", "€100", "‰part", "\"—tiret\"", "'à côté'",
				"12", "true", "", "~", "\"с отменой\\tи\\u0041\"", "°C", "Ÿ"
			};
			// Выводим построенную запись скалярного значения
			return string(narrowed[engine() % (sizeof(narrowed) / sizeof(narrowed[0]))]);
		}
		// Набор записей, из которых строится значение
		static const char * const values[] = {
			"простая", "'дословная'", "\"с отменой\\tи\\u0041\"", "\"\"", "''",
			/**
			 * Записи с отменяющими последовательностями редкими
			 *
			 * @note Заходы эти взяты особо: построение выше давало лишь `\\t` и `\\u`,
			 *       и добрая половина таблицы отмен разбором не исполнялась вовсе
			 */
			"\"\\0\\a\\b\\v\\f\\e\"", "\"\\N\\_\\L\\P\"", "\"\\x41\\U0001F600\"",
			"\"\\ \\\"\\/\\\\\"", "\"\\q\"", "\"обрыв\\\"",
			"12", "-1250", "0x1F", "0o755", "0b1010", "0777", "3.14159", "-2.5e-13",
			".inf", "-.inf", ".nan", "true", "false", "yes", "no", "on", "off",
			"null", "~", "", "12:30", "2024-02-29", "a:b", "b#c", "100%", "-5",
			"9223372036854775807", "99999999999999999999999999", "0.1", "текст с пробелами"
		};
		// Выводим построенную запись скалярного значения
		return string(values[engine() % (sizeof(values) / sizeof(values[0]))]);
	}
	/**
	 * @brief Метод построения записи поточного построения
	 *
	 * @param engine источник псевдослучайных чисел
	 * @param depth  остаток допустимой глубины вложенности
	 * @param indent отступ строки, построение несущей
	 * @return       построенная запись поточного построения
	 *
	 */
	/**
	 * @brief Метод построения простого значения, на несколько строк растянутого
	 *
	 * @details Внутри скобок простое значение вправе стоять в несколько строк, и первый
	 * знак строки продолжения описание берёт правилом `ns-plain-char`, а не
	 * `ns-plain-first`: указатели там знаками содержимого и остаются
	 *
	 * @note Строится оно отдельно от прочих значений: продолжение годится лишь простому
	 *       значению, а к огранённому либо блочному строка, за ним стоящая, пристала бы
	 *       содержимым за завершённой записью - и отказ был бы законным, а не находкой.
	 *       Изымаются указатели построения поточного - запятая да скобки, - двоеточие с
	 *       пробелом да знак примечания: они значение обрывают по праву
	 *
	 * @warning Ворошитель таких записей не строил вовсе, и оттого шесть прочтений,
	 *          описанию противных, прожили в кодеке незамеченными: запись `[a`, `&m`,
	 *          `b]` выдавалась значением `a b` с меткою `&m`, а `[a`, `"b"]` - двумя
	 *          значениями в обратном порядке. Зелёный ворошитель отчитывается лишь о том,
	 *          что он СТРОИТ
	 */
	string continued(mt19937 & engine, const uint32_t indent) noexcept {
		// Набор записей продолжения, указателями открывающихся
		static const char * const pieces[] = {
			"&\xd0\xbc\xd0\xb5\xd1\x82\xd0\xba\xd0\xb0", "!!str", "?", "? \xd0\xb4", "-", "- \xd0\xb4",
			"\"\xd0\xb4\"", "'\xd0\xb4'", "\xd0\xb4\xd0\xb2\xd0\xb0", "%YAML 1.2", "@\xd0\xb4", "`\xd0\xb4", ":\xd0\xb4", "\xd0\xb4:\xd0\xb2"
		};
		// Собираемая запись простого значения
		string result(named(engine));
		// Количество строк продолжения записи
		const uint32_t count = (1 + (engine() % 2));
		/**
		 * Выполняем построение всех строк продолжения
		 */
		for(uint32_t i = 0; i < count; i++){
			// Выполняем добавление перевода строки продолжения
			result.append(1, '\n');
			// Выполняем добавление отступа строки продолжения
			result.append((indent + 2), ' ');
			// Выполняем добавление записи продолжения
			result.append(pieces[engine() % (sizeof(pieces) / sizeof(pieces[0]))]);
		}
		// Выводим собранную запись простого значения
		return result;
	}
	string flowed(mt19937 & engine, const uint32_t depth, const uint32_t indent) noexcept {
		// Признак того, что построение является перечнем значений
		const bool sequence = ((engine() % 2) == 0);
		// Собираемая запись поточного построения
		string result(sequence ? "[" : "{");
		// Количество значений собираемого построения
		const uint32_t count = (engine() % 4);
		/**
		 * Выполняем построение всех значений
		 */
		for(uint32_t i = 0; i < count; i++){
			/**
			 * Если значение не первое
			 */
			if(i > 0)
				// Выполняем добавление разделителя значений
				result.append(",");
			/**
			 * Если построение записывается несколькими строками
			 *
			 * @note Внутри скобок отступ не значит ничего, однако записывается он всё же
			 *       осмысленно: разбор обязан пройти и по строке с отступом, и по строке без
			 */
			if((engine() % 3) == 0){
				// Выполняем добавление перевода строки построения
				result.append("\n");
				// Выполняем добавление отступа строки построения
				result.append((indent + 2), ' ');
			// Если значение отделяется одним пробелом
			} else result.append(" ");
			/**
			 * Если построение является отображением пар
			 */
			if(!sequence){
				// Выполняем добавление имени пары отображения
				result.append(named(engine));
				/**
				 * Если пара несёт значение своё
				 *
				 * @note Отображение `{a, b}` описанием дозволено: значения пар пусты
				 */
				if((engine() % 4) != 0)
					// Выполняем добавление разделителя имени пары
					result.append(": ");
				// Если пара значения не несёт
				else continue;
			}
			/**
			 * Если значение является вложенным построением
			 */
			if((depth > 0) && ((engine() % 4) == 0))
				// Выполняем добавление вложенного поточного построения
				result.append(flowed(engine, (depth - 1), indent));
			/**
			 * Если значение является простым, на несколько строк растянутым
			 */
			else if((engine() % 6) == 0)
				// Выполняем добавление простого значения со строками продолжения
				result.append(continued(engine, indent));
			// Если значение является скалярным
			else result.append(scalared(engine));
		}
		/**
		 * Если построение несёт примечание перед закрывающей скобкой
		 */
		if((count > 0) && ((engine() % 8) == 0)){
			// Выполняем добавление примечания построения
			result.append(" # примечание\n");
			// Выполняем добавление отступа закрывающей скобки
			result.append((indent + 1), ' ');
		}
		// Выполняем добавление закрывающей скобки построения
		result.append(sequence ? "]" : "}");
		// Выводим собранную запись поточного построения
		return result;
	}
	/**
	 * @brief Метод построения блочного значения
	 *
	 * @param engine источник псевдослучайных чисел
	 * @param indent отступ строки, заголовок несущей
	 * @return       построенная запись блочного значения
	 *
	 */
	string blocked(mt19937 & engine, const uint32_t indent) noexcept {
		// Собираемая запись блочного значения
		string result((engine() % 2) == 0 ? "|" : ">");
		/**
		 * Если заголовок несёт указатель отступа
		 */
		if((engine() % 6) == 0)
			// Выполняем добавление указателя отступа заголовка
			result.push_back(static_cast <char> ('0' + 1 + (engine() % 3)));
		/**
		 * Если заголовок несёт правило усечения переводов строк
		 */
		if((engine() % 3) == 0)
			// Выполняем добавление правила усечения переводов строк
			result.push_back(((engine() % 2) == 0) ? '-' : '+');
		/**
		 * Если за заголовком стоит примечание
		 */
		if((engine() % 6) == 0)
			// Выполняем добавление примечания заголовка
			result.append(" # заголовок");
		// Количество строк собираемого блочного значения
		const uint32_t lines = (1 + (engine() % 3));
		/**
		 * Выполняем построение всех строк блочного значения
		 */
		for(uint32_t i = 0; i < lines; i++){
			// Выполняем добавление перевода строки блочного значения
			result.append("\n");
			/**
			 * Если строка блочного значения пуста
			 */
			if((engine() % 5) == 0)
				// Выполняем переход к следующей строке блочного значения
				continue;
			// Выполняем добавление отступа строки блочного значения
			result.append((indent + 2 + (((engine() % 4) == 0) ? 2 : 0)), ' ');
			// Выполняем добавление содержимого строки блочного значения
			result.append(scalared(engine));
		}
		// Выводим собранную запись блочного значения
		return result;
	}
	/**
	 * @brief Метод добавления разделителя записей к собранному тексту
	 *
	 * @note Записи, узлу предпосланные, отделяются пробельными знаками и от узла своего,
	 *       и друг от друга: запись `&метка!тип` описанию противна
	 *
	 * @param result собираемый текст
	 *
	 */
	void separate(string & result) noexcept {
		/**
		 * Если собранный текст оканчивается знаком, разделителем не являющимся
		 */
		if(!result.empty() && (result.back() != ' ') && (result.back() != '\n'))
			// Выполняем добавление разделителя записей
			result.push_back(' ');
	}
	/**
	 * @brief Метод построения блочного построения текста
	 *
	 * @details Текст строится деревом, а не строками вперемешку: отступ есть суть
	 * построения YAML, и текст, строками собранный наугад, отвергался бы первой же
	 * строкой, а глубокие пути разбора так не достигаются
	 *
	 * @param engine  источник псевдослучайных чисел
	 * @param depth   остаток допустимой глубины вложенности
	 * @param indent  отступ собираемого построения
	 * @param anchors перечень имён меток, документом объявленных
	 * @param result  собираемый текст
	 *
	 */
	void compose(mt19937 & engine, const uint32_t depth, const uint32_t indent, vector <string> & anchors, string & result) noexcept {
		// Признак того, что построение является перечнем значений
		const bool sequence = ((engine() % 3) == 0);
		// Количество значений собираемого построения
		const uint32_t count = (1 + (engine() % 4));
		/**
		 * Выполняем построение всех значений
		 */
		for(uint32_t i = 0; i < count; i++){
			/**
			 * Если перед значением стоит пустая строка
			 */
			if((engine() % 8) == 0)
				// Выполняем добавление пустой строки
				result.append("\n");
			/**
			 * Если перед значением стоит примечание собственной строкой
			 */
			if((engine() % 8) == 0){
				// Выполняем добавление отступа строки примечания
				result.append(indent, ' ');
				// Выполняем добавление примечания собственной строкой
				result.append("# сверху\n");
			}
			// Выполняем добавление отступа строки значения
			result.append(indent, ' ');
			/**
			 * Если построение является перечнем значений
			 */
			if(sequence)
				// Выполняем добавление объявления значения перечня
				result.append("- ");
			/**
			 * Если построение является отображением пар
			 */
			else {
				/**
				 * Если имени пары предпосылаются свойства
				 *
				 * @details Свойства эти принадлежат ИМЕНИ пары, а не значению её: написание
				 *          `&m !!str a: b` метит запись `a`. Держатся они у дерева особыми
				 *          полями, и путь их - свой, отдельный от пути свойств значения
				 *
				 * @warning Замер покрытием показал: прежде набор свойств имени пары не строил
				 *          ВОВСЕ, и потеря метки типа имени держалась при зелёном ворошителе -
				 *          текст `!!str 1: x` возвращался записью `'1': x`
				 *
				 * @note Метка типа берётся строковой да целочисленной: имя пары дерево держит
				 *       записью, и метка, содержимому противная, отказом не отзывается - в
				 *       перезаписи она стоит рядом с оградою имени
				 */
				if((engine() % 12) == 0){
					/**
					 * Набор свойств, имени пары предпосылаемых
					 */
					static const char * const keyed[] = {"&\xd0\xbc\xd0\xb5\xd1\x82\xd0\xba\xd0\xb0",
					                                     "!!str", "!!int", "!свой%21", "!<tag:x,2000:mine>",
					                                     "&\xd0\xbc\xd0\xb5\xd1\x82\xd0\xba\xd0\xb0 !!str"};
					// Выполняем добавление свойств имени пары отображения
					result.append(keyed[engine() % (sizeof(keyed) / sizeof(keyed[0]))]);
					// Выполняем добавление разделителя записей перед именем пары
					result.append(" ");
				}
				// Выполняем добавление имени пары отображения
				result.append(named(engine));
				// Выполняем добавление разделителя имени пары
				result.append(":");
			}
			// Получаем отступ значения, объявлением занятый
			const uint32_t inner = (indent + 2);
			/**
			 * Если значением является ссылка на объявленную метку
			 */
			if(!anchors.empty() && ((engine() % 10) == 0)){
				// Выполняем добавление разделителя записей перед ссылкой
				separate(result);
				// Выполняем добавление ссылки на объявленную метку
				result.append("*").append(anchors.at(engine() % anchors.size()));
				// Выполняем добавление перевода строки значения
				result.append("\n");
				// Выполняем переход к следующему значению построения
				continue;
			}
			/**
			 * Если значению предпослана метка
			 */
			if((engine() % 8) == 0){
				// Собираемое имя предпосылаемой метки
				string anchor("м");
				// Выполняем добавление порядкового знака имени метки
				anchor.push_back(static_cast <char> ('a' + (engine() % 6)));
				// Выполняем добавление разделителя записей перед меткой
				separate(result);
				// Выполняем добавление метки значения
				result.append("&").append(anchor);
				// Запоминаем имя объявленной метки
				anchors.push_back(anchor);
			}
			/**
			 * Если значению предпослана метка типа
			 */
			if((engine() % 10) == 0){
				/**
				 * Набор меток типов, значению предпосылаемых
				 *
				 * @note Метки с долевыми записями знаков обязательны в наборе: метка `!свой`
				 *       описанию противна - правило `ns-tag-char` знаков незападных не
				 *       принимает, - и читающий отвергает её прежде круга, оттого до
				 *       пишущего дело не доходит вовсе. Годным видом незападного знака в
				 *       метке служит доля, и лишь она круг замыкает. Прежде набор её не
				 *       строил, и разрыв круга на метках держался при зелёном ворошителе:
				 *       пишущий выдавал `!а` там, где прочитано было `!%D0%B0`
				 */
				static const char * const tags[] = {"!!str", "!свой", "!<tag:x,2000:mine>", "!",
				                                    "!%D0%B0", "!%D0%BC%D0%B5%D1%82%D0%BA%D0%B0",
				                                    "!x%21y", "!!%D0%B0",
				                                    /**
				                                     * Метки, содержимому заведомо противные, да сокращение
				                                     * необъявленное
				                                     *
				                                     * @warning Замер покрытием показал: до мест отказа
				                                     *          `TAG_MISMATCH` да `UNKNOWN_TAG_HANDLE`
				                                     *          ворошитель не доходил вовсе - набор строил
				                                     *          одни лишь метки, содержимому отвечающие.
				                                     *          Отказ ворошителю не помеха: он поверяет
				                                     *          независимость разбора от нарезки да
				                                     *          названность причины отказа, а те держатся
				                                     *          и на текстах отвергаемых
				                                     */
				                                    "!!int", "!!bool", "!!float", "!!null", "!!map", "!!seq",
				                                    "!e!x", "!!binary"};
				/**
				 * @note Метка `!!binary` изъята была из набора, покуда перенос значения в чужое
				 *       дерево свойств узла не брал: двоичное содержимое теряло вид свой -
				 *       `!!binary 12:30` наречием 1.1 читалось обратно числом семьсот пятьдесят.
				 *       Дерево получило `endow()`, укладка свойства ставит, и метка возвращена
				 *       в набор вместе с починкой
				 */
				// Выполняем добавление разделителя записей перед меткой типа
				separate(result);
				// Выполняем добавление метки типа значения
				result.append(tags[engine() % (sizeof(tags) / sizeof(tags[0]))]);
			}
			// Выбираемая разновидность записи значения
			const uint32_t choice = (engine() % 12);
			/**
			 * Если значением является вложенное построение
			 */
			if((depth > 0) && (choice < 3)){
				// Выполняем добавление перевода строки объявления
				result.append("\n");
				/**
				 * Если вложенное построение записывается на отступе имени своей пары
				 *
				 * @note Перечень, стоящий на отступе имени своей пары, описанием дозволен, и
				 *       ветвь эта разбором проходится особо
				 */
				const uint32_t nested = ((!sequence && ((engine() % 4) == 0)) ? indent : inner);
				// Выполняем построение вложенного построения
				compose(engine, (depth - 1), nested, anchors, result);
				// Выполняем переход к следующему значению построения
				continue;
			}
			/**
			 * Если значением является блочное значение
			 */
			if(choice < 5){
				// Выполняем добавление разделителя записей перед блочным значением
				separate(result);
				// Выполняем добавление блочного значения
				result.append(blocked(engine, indent));
				// Выполняем добавление перевода строки значения
				result.append("\n");
				// Выполняем переход к следующему значению построения
				continue;
			}
			/**
			 * Если значением является поточное построение
			 */
			if(choice < 7){
				// Выполняем добавление разделителя записей перед поточным построением
				separate(result);
				// Выполняем добавление поточного построения
				result.append(flowed(engine, 2, indent));
				// Выполняем добавление перевода строки значения
				result.append("\n");
				// Выполняем переход к следующему значению построения
				continue;
			}
			/**
			 * Если значение пусто вовсе
			 */
			if(choice == 7){
				// Выполняем добавление перевода строки значения
				result.append("\n");
				// Выполняем переход к следующему значению построения
				continue;
			}
			// Выполняем добавление разделителя записей перед скалярным значением
			separate(result);
			// Выполняем добавление скалярного значения
			result.append(scalared(engine));
			/**
			 * Если значение продолжается строкою ниже
			 *
			 * @note Простое значение, добежавшее до конца строки, вправе продолжиться, и
			 *       ветвь эта разбором проходится особо: выдача его откладывается
			 */
			if((engine() % 8) == 0){
				// Количество строк продолжения значения
				const uint32_t lines = (1 + (engine() % 2));
				/**
				 * Выполняем построение всех строк продолжения значения
				 */
				for(uint32_t j = 0; j < lines; j++){
					// Выполняем добавление перевода строки продолжения
					result.append("\n");
					/**
					 * Если перед продолжением стоит пустая строка
					 */
					if((engine() % 4) == 0)
						// Выполняем добавление пустой строки продолжения
						result.append("\n");
					// Выполняем добавление отступа строки продолжения
					result.append((inner + 1), ' ');
					// Выполняем добавление содержимого строки продолжения
					result.append("продолжение");
				}
			/**
			 * Если за значением стоит примечание
			 */
			} else if((engine() % 6) == 0)
				// Выполняем добавление примечания за значением
				result.append(" # сбоку");
			// Выполняем добавление перевода строки значения
			result.append("\n");
		}
	}
	/**
	 * @brief Метод построения текста
	 *
	 * @param engine источник псевдослучайных чисел
	 * @return       построенный текст
	 *
	 */
	/**
	 * Имена ключей, меток да опор, прямою подачею в запись отдаваемые
	 *
	 * @note Имена подобраны так, чтобы задеть все виды ограды: голое имя, имя со знаком
	 *       двоеточия, имя с кавычкою, имя пустое да имя знаками Юникода
	 */
	static const char * NAMES[] = {"a", "b c", "d:e", "f\"g", "", "ключ", "#h", "- i"};

	/**
	 * Значения, прямою подачею в запись отдаваемые
	 *
	 * @note Значения подобраны так, чтобы задеть подъём ограды: голый текст, текст со
	 *       знаком перевода строки, текст с кавычкою, текст, читаемый числом, да текст,
	 *       читаемый признаком истины
	 */
	static const char * VALUES[] = {"value", "j\nk", "l\"m", "12:30", "true", "", "значение", " n "};

	/**
	 * @brief Метод подачи заготовленной череды вызовов в запись
	 *
	 * @details Запись доселе поверялась одним лишь деревом, а оно на всякую перезапись
	 *          заводит объект записи заново. Прямая подача - построения, ключи, значения,
	 *          опоры да метки - охвата не имела вовсе, а с нею и остаток прежней подачи
	 *          в объекте записи. Череда вызовов заготавливается наперёд числами, а не
	 *          берётся у источника по ходу подачи: два объекта записи обязаны получить
	 *          череду одну и ту же знак в знак
	 *
	 * @param script   череда вызовов, числами заготовленная
	 * @param writer   объект записи, куда ведётся подача
	 * @param traces   исход всякого вызова череды
	 * @param complete признак завершения записи по окончании череды
	 *
	 */
	void replay(const vector <uint32_t> & script, yaml::writer_t & writer, vector <bool> & traces, const bool complete) noexcept {
		// Выполняем очистку перечня исходов вызовов
		traces.clear();
		// Выполняем резервирование памяти под перечень исходов вызовов
		traces.reserve(script.size());
		/**
		 * Выполняем подачу заготовленной череды вызовов
		 */
		for(size_t i = 0; i < script.size(); i++){
			// Число, разновидность вызова несущее
			const uint32_t number = script.at(i);
			// Имя ключа, метки либо опоры, числом выбранное
			const string name = NAMES[number % (sizeof(NAMES) / sizeof(NAMES[0]))];
			// Значение, числом выбранное
			const string value = VALUES[(number / 8) % (sizeof(VALUES) / sizeof(VALUES[0]))];
			// Построение, числом выбранное
			const yaml::layout_t layout = ((((number / 64) % 2) == 0) ? yaml::layout_t::BLOCK : yaml::layout_t::FLOW);
			/**
			 * Выполняем выборку разновидности вызова
			 */
			switch(number % 14){
				// Выполняем открытие отображения
				case 0: traces.push_back(writer.mapping(layout)); break;
				// Выполняем открытие перечня
				case 1: traces.push_back(writer.sequence(layout)); break;
				// Выполняем закрытие построения
				case 2: traces.push_back(writer.close()); break;
				// Выполняем запись имени ключа
				case 3: traces.push_back(writer.key(name)); break;
				// Выполняем запись строкового значения
				case 4: traces.push_back(writer.value(value)); break;
				// Выполняем запись пустого значения
				case 5: traces.push_back(writer.null()); break;
				// Выполняем запись логического значения
				case 6: traces.push_back(writer.value((((number / 4) % 2) == 0))); break;
				// Выполняем запись целого значения
				case 7: traces.push_back(writer.value(static_cast <int64_t> (number) - 2048)); break;
				// Выполняем запись дробного значения
				case 8: traces.push_back(writer.value(static_cast <double> (number) / 64.0)); break;
				// Выполняем установку опоры
				case 9: traces.push_back(writer.anchor(name)); break;
				// Выполняем запись обращения к опоре
				case 10: traces.push_back(writer.alias(name)); break;
				// Выполняем запись примечания
				case 11: traces.push_back(writer.comment(value)); break;
				// Выполняем запись блочного значения
				case 12: traces.push_back(writer.block(value,
				 (((number / 128) % 2) == 0 ? yaml::style_t::LITERAL : yaml::style_t::FOLDED),
				 static_cast <yaml::chomp_t> ((number / 256) % 3))); break;
				// Выполняем открытие нового документа
				case 13: traces.push_back(writer.document()); break;
			}
		}
		/**
		 * Если требуется завершить запись
		 *
		 * @note Завершение зовётся не всегда намеренно: оно закрывает построения, открытыми
		 *       оставшиеся, и стопа уровней к очистке приходит пустою. Брось потребитель
		 *       запись посреди построения - и очистка стопы получит работу; без сего пути
		 *       снятие её ворошителем не ловилось вовсе, проверено опытом
		 */
		if(complete)
			// Выполняем завершение записи
			traces.push_back(writer.finish());
	}

	string generate(mt19937 & engine) noexcept {
		// Собираемый текст
		string result;
		/**
		 * Если текст начинается меткой порядка байтов
		 */
		if((engine() % 8) == 0)
			// Выполняем добавление метки порядка байтов
			result.append("\xEF\xBB\xBF");
		// Количество документов собираемого текста
		const uint32_t documents = (1 + (engine() % 2));
		/**
		 * Выполняем построение всех документов текста
		 */
		for(uint32_t i = 0; i < documents; i++){
			// Признак того, что документу предпосланы директивы
			bool directed = false;
			/**
			 * Если документу предпослана директива наречия
			 */
			if((engine() % 8) == 0){
				/**
				 * Набор директив наречия, документу предпосылаемых
				 *
				 * @warning Замер покрытием показал: до мест отказа по наречию неподдержанному
				 *          да по директиве, построенной ошибочно, ворошитель не доходил вовсе -
				 *          набор строил одни лишь директивы годные. Отказ ворошителю не помеха:
				 *          поверки его - независимость разбора от нарезки текста да названность
				 *          причины отказа - держатся и на текстах отвергаемых
				 */
				static const char * const versions[] = {"%YAML 1.2\n", "%YAML 1.1\n", "%YAML 1.2\n",
				                                        "%YAML 3.0\n", "%YAML 1\n", "%YAML\n",
				                                        "%YAML 1.2.3\n", "%СВОЙ 1.0\n"};
				// Выполняем добавление директивы наречия текста
				result.append(versions[engine() % (sizeof(versions) / sizeof(versions[0]))]);
				// Запоминаем признак предпосланных директив
				directed = true;
			}
			/**
			 * Если документу предпослана директива сокращения метки типа
			 */
			if((engine() % 10) == 0){
				/**
				 * Набор директив сокращения метки типа
				 *
				 * @note Половина набора построена ошибочно: сокращение без восклицательных
				 *       знаков, сокращение без начала да сокращение, объявленное дважды
				 */
				static const char * const handles[] = {"%TAG !e! tag:example.com,2000:\n",
				                                       "%TAG !e! tag:example.com,2000:\n",
				                                       "%TAG !! tag:example.com,2000:\n",
				                                       "%TAG e! tag:example.com,2000:\n",
				                                       "%TAG !e!\n", "%TAG\n",
				                                       "%TAG !e! tag:example.com,2000:\n%TAG !e! tag:other,2000:\n"};
				// Выполняем добавление директивы сокращения метки типа
				result.append(handles[engine() % (sizeof(handles) / sizeof(handles[0]))]);
				// Запоминаем признак предпосланных директив
				directed = true;
			}
			/**
			 * Если документ открывается чертою
			 *
			 * @note Документ с директивами обязан открыться чертою: без неё неведомо, где
			 *       кончаются директивы и начинается содержимое
			 */
			if(directed || (i > 0) || ((engine() % 3) == 0))
				// Выполняем добавление черты начала документа
				result.append("---\n");
			// Перечень имён меток, документом объявленных
			vector <string> anchors;
			// Выполняем построение содержимого документа
			compose(engine, (1 + (engine() % 3)), 0, anchors, result);
			/**
			 * Если документ закрывается чертою
			 */
			if((engine() % 6) == 0)
				// Выполняем добавление черты конца документа
				result.append("...\n");
		}
		/**
		 * Если текст записывается возвратом каретки с переводом строки
		 */
		if((engine() % 6) == 0){
			// Собираемый текст с возвратом каретки
			string carriage;
			/**
			 * Выполняем перебор всех знаков собранного текста
			 */
			for(const char letter : result){
				/**
				 * Если знак является переводом строки
				 */
				if(letter == '\n')
					// Выполняем добавление возврата каретки перед переводом строки
					carriage.push_back('\r');
				// Выполняем добавление очередного знака текста
				carriage.push_back(letter);
			}
			// Запоминаем собранный текст с возвратом каретки
			result.assign(carriage);
		}
		// Выводим собранный текст
		return result;
	}
	/**
	 * @brief Метод порчи построенного текста
	 *
	 * @details Чисто случайный набор байтов отвергался бы первой же строкой, и глубокие
	 * пути разбора так не достигаются: портится готовый правильный текст
	 *
	 * @param text   портимый текст
	 * @param engine источник псевдослучайных чисел
	 *
	 */
	void corrupt(string & text, mt19937 & engine) noexcept {
		/**
		 * Если портимый текст пуст
		 */
		if(text.empty())
			// Выходим из функции
			return;
		// Количество вносимых в текст порч
		const uint32_t count = (1 + (engine() % 3));
		/**
		 * Выполняем внесение всех порч в текст
		 */
		for(uint32_t i = 0; (i < count) && !text.empty(); i++){
			// Выбираемая разновидность вносимой порчи
			const uint32_t choice = (engine() % 5);
			// Место внесения порчи в текст
			const size_t position = (engine() % text.length());
			/**
			 * Выполняем выбор разновидности вносимой порчи
			 */
			switch(choice){
				// Если порчей является замена знака
				case 0: {
					// Выполняем выборку знака замены
					const uint32_t letter = (engine() % 256);
					// Выполняем замену знака текста
					text.at(position) = static_cast <char> (letter);
				} break;
				// Если порчей является вставка знака
				case 1: {
					// Выполняем выборку вставляемого знака
					const uint32_t letter = (engine() % 256);
					// Выполняем вставку знака в текст
					text.insert(position, 1, static_cast <char> (letter));
				} break;
				// Если порчей является удаление знака
				case 2: {
					// Выполняем удаление знака текста
					text.erase(position, 1);
				} break;
				// Если порчей является обрезание текста
				case 3: {
					// Выполняем обрезание текста
					text.erase(position);
				} break;
				/**
				 * Если порчей является вставка знака горизонтальной подачи
				 *
				 * @note Знак этот в отступе описанием запрещён, и порча им ведёт разбор по
				 *       ветви отказа, случайным байтом почти недостижимой
				 */
				case 4: {
					// Выполняем вставку знака горизонтальной подачи в текст
					text.insert(position, 1, '\t');
				} break;
			}
		}
	}
	/**
	 * @brief Метод подачи текста чтению
	 *
	 * @param text     разбираемый текст
	 * @param settings настройки разбора текста
	 * @param chunk    размер куска подачи, ноль - подача целиком
	 * @param events   перечень выданных разбором событий
	 * @param failure    код ошибки разбора по окончании подачи
	 * @param recognised опознанная кодировка исходного текста
	 * @return           состояние потокового чтения по окончании подачи
	 *
	 */
	yaml::state_t consume(const string & text, const yaml::reader_t::settings_t & settings, const size_t chunk, vector <Event> & events, yaml::error_t * failure = nullptr, yaml::encoding_t * recognised = nullptr, const bool recycle = false) noexcept {
		/**
		 * Объект потокового чтения, от подачи к подаче живущий
		 *
		 * @note Держится он тут ради поверки сброса: чтение, один текст уже прочитавшее,
		 *       обязано по сбросу прочесть следующий ровно так же, как прочло бы его
		 *       чтение свежее. Остаток прежней подачи - стопа открытых построений, метки,
		 *       наречие, опознанная кодировка - обязан быть сброшен целиком
		 */
		static yaml::reader_t reused;
		// Создаём объект потокового чтения текста
		yaml::reader_t fresh(settings);
		/**
		 * Если чтение ведётся объектом, прежней подачей занятым
		 */
		if(recycle){
			// Выполняем сброс состояния прежней подачи
			reused.clear();
			// Выполняем установку настроек разбора очередной подачи
			(void) reused.settings(settings);
		}
		// Объект потокового чтения, подачу выполняющий
		yaml::reader_t & reader = (recycle ? reused : fresh);
		// Размер куска подачи текста
		const size_t size = ((chunk > 0) ? chunk : (text.empty() ? 1 : text.length()));
		// Смещение начала очередного куска подачи
		size_t offset = 0;
		/**
		 * Выполняем подачу текста кусками
		 */
		do {
			// Размер подаваемого куска текста
			const size_t length = (((text.length() - offset) < size) ? (text.length() - offset) : size);
			// Признак того, что подаётся последний кусок текста
			const bool end = ((offset + length) >= text.length());
			// Признак успешной подачи куска текста
			const bool passed = reader.feed(text.data() + offset, length, end);
			// Выполняем смещение начала очередного куска подачи
			offset += length;
			/**
			 * Выполняем чтение выданных разбором событий
			 */
			while(reader.next()){
				// Создаём запоминаемое событие разбора
				Event event;
				// Запоминаем разновидность события
				event.event = static_cast <uint8_t> (reader.event());
				// Запоминаем вид значения события
				event.type = static_cast <uint8_t> (reader.value().type);
				// Запоминаем вид записи значения в исходном тексте
				event.style = static_cast <uint8_t> (reader.value().style);
				// Запоминаем содержимое значения события
				event.text.assign(reader.value().text);
				// Запоминаем имя метки, событию предпосланной
				event.anchor.assign(reader.value().anchor);
				// Запоминаем метку типа, событию предпосланную
				event.tag.assign(reader.value().tag);
				// Запоминаем номер строки, где событие собрано
				event.line = reader.value().location.line;
				// Запоминаем положение события в строке
				event.column = reader.value().location.column;
				// Запоминаем глубину вложенности, где событие собрано
				event.depth = reader.value().location.depth;
				// Запоминаем смещение начала события от начала текста
				event.position = reader.value().location.offset;
				// Выполняем добавление события в перечень выданных
				events.push_back(event);
				// Выполняем учёт выданного разбором события
				totals.events++;
			}
			/**
			 * Если подача куска текста не удалась
			 */
			if(!passed)
				// Выходим из цикла подачи текста
				break;
		// Выполняем подачу до исчерпания текста
		} while(offset < text.length());
		/**
		 * Если код ошибки разбора затребован
		 */
		if(failure != nullptr)
			// Запоминаем код ошибки разбора по окончании подачи
			(* failure) = reader.error();
		/**
		 * Если опознанная кодировка затребована
		 */
		if(recognised != nullptr)
			// Запоминаем опознанную кодировку исходного текста
			(* recognised) = reader.encoding();
		// Выводим состояние потокового чтения по окончании подачи
		return reader.state();
	}
	/**
	 * @brief Метод сброса смещений начала событий в перечне
	 *
	 * @note Смещение считается байтами исходного текста и кодировкой разнится по самому
	 *       устройству: один и тот же знак занимает в UTF-16 вдвое больше места.
	 *       Сличению кодировок оно потому не подлежит, в отличие от места события,
	 *       считаемого строками и знаками
	 *
	 * @param events перечень событий, смещения которого сбрасываются
	 *
	 */
	void detach(vector <Event> & events) noexcept {
		/**
		 * Выполняем перебор всех событий перечня
		 */
		for(Event & event : events)
			// Выполняем сброс смещения начала события
			event.position = 0;
	}
	/**
	 * @brief Метод сборки описания настроек разбора текста
	 *
	 * @param settings описываемые настройки разбора
	 * @return         собранное описание настроек разбора
	 *
	 */
	string described(const yaml::reader_t::settings_t & settings) noexcept {
		// Собираемое описание настроек разбора
		string result("schema=");
		// Выполняем добавление схемы разрешения видов
		result.append(to_string(static_cast <unsigned> (settings.schema)));
		// Выполняем добавление признака выдачи примечаний
		result.append(" comments=").append(settings.emitComments ? "1" : "0");
		// Выполняем добавление признака выдачи пустых строк
		result.append(" blanks=").append(settings.emitBlanks ? "1" : "0");
		// Выполняем добавление правила обращения с повторяющимся именем
		result.append(" duplicates=").append(to_string(static_cast <unsigned> (settings.duplicates)));
		// Выполняем добавление предела глубины вложенности
		result.append(" depth=").append(to_string(settings.maxDepth));
		// Выполняем добавление предела длины скалярного значения
		result.append(" scalar=").append(to_string(settings.maxScalar));
		// Выводим собранное описание настроек разбора
		return result;
	}
	/**
	 * @brief Метод сличения перечней выданных разбором событий
	 *
	 * @param first    перечень событий, взятый за образец
	 * @param second   перечень событий, с образцом сличаемый
	 * @param label    название сличаемой подачи
	 * @param text     разбираемый текст
	 * @param settings настройки разбора текста
	 * @return         результат сличения перечней
	 *
	 */
	bool compare(const vector <Event> & first, const vector <Event> & second, const string & label, const string & text, const yaml::reader_t::settings_t & settings) noexcept {
		/**
		 * Если количество выданных событий разошлось
		 */
		if(first.size() != second.size()){
			// Выводим сообщение о расхождении количества выданных событий
			::fprintf(stderr, "yaml fuzz: %s events %zu != %zu, settings %s\n",
				label.c_str(), first.size(), second.size(), described(settings).c_str());
			// Выводим разбираемый текст
			dump(text);
			// Выводим результат сличения перечней
			return false;
		}
		/**
		 * Выполняем перебор всех выданных разбором событий
		 */
		for(size_t i = 0; i < first.size(); i++){
			/**
			 * Если очередное событие разошлось
			 */
			if(first.at(i) != second.at(i)){
				// Выводим сообщение о расхождении выданного события
				::fprintf(stderr, "yaml fuzz: %s event %zu differs (kind %u/%u «%s»/«%s» at %u:%u/%u:%u), settings %s\n",
					label.c_str(), i, first.at(i).event, second.at(i).event,
					first.at(i).text.c_str(), second.at(i).text.c_str(),
					first.at(i).line, first.at(i).column, second.at(i).line, second.at(i).column,
					described(settings).c_str());
				// Выводим разбираемый текст
				dump(text);
				// Выводим результат сличения перечней
				return false;
			}
		}
		// Выводим результат сличения перечней
		return true;
	}
	/**
	 * @brief Функция сличения извлечения владеющего значения с деревом документа
	 *
	 * @details Путей к числу здесь два: узел дерева хранит его разобранным при чтении, а
	 *          владеющее значение разбирает запись заново при снятии. Разойдись они -
	 *          разойдутся и кодеки между собой, ибо договор извлечения у всех один
	 *
	 * @param node      узел дерева документа
	 * @param value     владеющее значение, с узла снятое
	 * @param route     путь к сличаемому узлу
	 * @param diverged  переменная, куда помещается путь разошедшегося узла
	 * @return          признак согласия извлечения
	 *
	 */
	static bool agreeing(const yaml::document_t::value_t & node, const yaml::value_t & value,
	 const string & route, string & diverged) noexcept {
		/**
		 * Если виды значений разошлись
		 */
		if(node.kind() != value.kind()){
			// Запоминаем путь разошедшегося узла
			diverged.assign(route + " (вид)");
			// Выводим признак несогласия извлечения
			return false;
		}
		/**
		 * Если виды хранения значений разошлись
		 */
		if(node.type() != value.type()){
			// Запоминаем путь разошедшегося узла
			diverged.assign(route + " (вид хранения)");
			// Выводим признак несогласия извлечения
			return false;
		}
		/**
		 * Если записи значений разошлись
		 */
		if(node.text() != value.text()){
			// Запоминаем путь разошедшегося узла
			diverged.assign(route + " (запись)");
			// Выводим признак несогласия извлечения
			return false;
		}
		/**
		 * Если значение вместилищем является
		 */
		if((value.kind() == yaml::kind_t::MAPPING) || (value.kind() == yaml::kind_t::SEQUENCE)){
			// Номер сличаемого значения вместилища
			size_t index = 0;
			/**
			 * Выполняем перебор значений вместилища
			 */
			for(yaml::document_t::value_t item = node.begin(); item.valid(); item = item.next()){
				/**
				 * Если значений у владеющего меньше, чем у дерева
				 */
				if(index >= value.size()){
					// Запоминаем путь разошедшегося узла
					diverged.assign(route + " (количество)");
					// Выводим признак несогласия извлечения
					return false;
				}
				/**
				 * Если сличение вложенного значения согласия не дало
				 */
				if(!agreeing(item, value[index], (route + "/" + to_string(index)), diverged))
					// Выводим признак несогласия извлечения
					return false;
				// Выполняем переход к следующему значению вместилища
				index++;
			}
			/**
			 * Если значений у владеющего больше, чем у дерева
			 */
			if(index != value.size()){
				// Запоминаем путь разошедшегося узла
				diverged.assign(route + " (количество)");
				// Выводим признак несогласия извлечения
				return false;
			}
			// Выводим признак согласия извлечения
			return true;
		}
		// Извлекаемое из узла дерева целое число со знаком
		int64_t integer = 0;
		// Извлекаемое из владеющего значения целое число со знаком
		int64_t number = 1;
		// Признак успешности извлечения целого числа со знаком из узла дерева
		const bool signedness = node.value(integer);
		/**
		 * Если извлечение целого числа со знаком разошлось
		 *
		 * @note Значения сличаются лишь при успешном извлечении: неудачное их не трогает
		 *       вовсе, и сличать оставленное нетронутым значило бы объявлять расхождением
		 *       разность начальных значений. Ворошитель сам на это и попался
		 */
		if((signedness != value.value(number)) || (signedness && (integer != number))){
			// Запоминаем путь разошедшегося узла
			diverged.assign(route + " (целое)");
			// Выводим признак несогласия извлечения
			return false;
		}
		// Извлекаемое из узла дерева целое число без знака
		uint64_t natural = 0;
		// Извлекаемое из владеющего значения целое число без знака
		uint64_t plain = 1;
		// Признак успешности извлечения целого числа без знака из узла дерева
		const bool naturalness = node.value(natural);
		/**
		 * Если извлечение целого числа без знака разошлось
		 */
		if((naturalness != value.value(plain)) || (naturalness && (natural != plain))){
			// Запоминаем путь разошедшегося узла
			diverged.assign(route + " (целое без знака)");
			// Выводим признак несогласия извлечения
			return false;
		}
		// Извлекаемое из узла дерева дробное число
		double real = 0.;
		// Извлекаемое из владеющего значения дробное число
		double fraction = 1.;
		// Признак успешности извлечения дробного числа из узла дерева
		const bool fractional = node.value(real);
		/**
		 * Если успешность извлечения дробного числа разошлась
		 */
		if(fractional != value.value(fraction)){
			// Запоминаем путь разошедшегося узла
			diverged.assign(route + " (дробное)");
			// Выводим признак несогласия извлечения
			return false;
		}
		/**
		 * Если извлечённые дробные числа разошлись
		 *
		 * @note Нечисло сличению обычным порядком не поддаётся: оно не равно даже самому
		 *       себе, и сличать здесь надлежит признак нечисла у обоих
		 */
		if(fractional && ((::isnan(real) != ::isnan(fraction)) || (!::isnan(real) && (real != fraction)))){
			// Запоминаем путь разошедшегося узла
			diverged.assign(route + " (дробное)");
			// Выводим признак несогласия извлечения
			return false;
		}
		// Выводим признак согласия извлечения
		return true;
	}
	/**
	 * @brief Функция проверки того, что дерево значения несёт метку типа
	 *
	 * @details Перенос значения в чужое дерево свойств узла НЕ БЕРЁТ - дерево способа
	 *          поставить узлу свойство не имеет вовсе, - и вид, одною лишь меткою типа
	 *          державшийся, при переносе теряется: `!!binary aGk=` ложится строкою, а
	 *          `!!int -1250` при схеме защитной обращается строкою же. Предел этот записан
	 *          при самом методе `graft` и доложен по принадлежности: лечится он заведением
	 *          у дерева способа установки свойств, а то есть решение по модулю дерева
	 *
	 * @note Сличение перенесённого значения с исходным на деревьях с метками оттого и
	 *       пропускается: оно отчитывалось бы о пороке известном при всяком проходе, и
	 *       ворошитель перестал бы находить прочее. Сам перенос при этом ведётся - поверка
	 *       «отказ называет причину» его и стережёт. Снять пропуск надлежит вместе с
	 *       починкой
	 *
	 * @param value проверяемое владеющее значение
	 * @return      признак того, что дерево значения несёт метку типа
	 *
	 */
	static bool tagged(const yaml::value_t & value) noexcept {
		/**
		 * Если метка типа значению предпослана
		 */
		if(!value.tag().empty())
			// Выводим признак наличия метки типа
			return true;
		/**
		 * Выполняем перебор всех значений вместилища
		 */
		for(size_t i = 0; i < value.size(); i++){
			/**
			 * Если дерево очередного значения метку типа несёт
			 */
			if(::tagged(value[i]))
				// Выводим признак наличия метки типа
				return true;
		}
		// Выводим признак отсутствия метки типа
		return false;
	}
	/**
	 * @brief Функция сличения якорей и меток двух владеющих значений
	 *
	 * @details Сличение значений равенством ведётся по содержимому: якорь и метка в него
	 *          не входят намеренно, ибо два поля одного содержимого равны и при разных
	 *          якорях. Оттого перенос якорей потоковой сборкой равенством НЕ проверяется
	 *          вовсе, и без этого обхода он был бы правкой, ничем не поверяемой
	 *
	 * @param first  первое сличаемое значение
	 * @param second второе сличаемое значение
	 * @param route  путь сличаемого узла
	 * @param diverged путь узла, где обнаружено расхождение
	 * @return       признак совпадения якорей и меток
	 *
	 */
	static bool decorated(const yaml::value_t & first, const yaml::value_t & second, const string & route, string & diverged) noexcept {
		/**
		 * Если якоря сличаемых значений разошлись
		 */
		if(first.anchor() != second.anchor()){
			// Запоминаем путь разошедшегося узла
			diverged.assign(route + " (якорь)");
			// Выводим признак расхождения
			return false;
		}
		/**
		 * Если метки сличаемых значений разошлись
		 */
		if(first.tag() != second.tag()){
			// Запоминаем путь разошедшегося узла
			diverged.assign(route + " (метка)");
			// Выводим признак расхождения
			return false;
		}
		/**
		 * Если размеры сличаемых вместилищ разошлись
		 */
		if(first.size() != second.size()){
			// Запоминаем путь разошедшегося узла
			diverged.assign(route + " (размер)");
			// Выводим признак расхождения
			return false;
		}
		/**
		 * Выполняем перебор членов сличаемого вместилища
		 */
		for(size_t i = 0; i < first.size(); i++){
			/**
			 * Если якоря членов вместилища разошлись
			 */
			if(!decorated(first[i], second[i], route + "/" + to_string(i), diverged))
				// Выводим признак расхождения
				return false;
		}
		// Выводим признак совпадения
		return true;
	}
	/**
	 * @brief Функция сборки владеющего значения потоковой сборкой
	 *
	 * @details Сборка идёт обходом готового значения: тем проверяется, что договор
	 *          сборки покрывает всякое дерево, какое кодек способен построить разбором
	 *
	 * @param value    собираемое значение
	 * @param builder  потоковая сборка
	 * @return         признак успешности сборки
	 *
	 */
	static bool assemble(const yaml::value_t & value, yaml::builder_t & builder) noexcept {
		/**
		 * Если собираемому значению предпослан якорь
		 *
		 * @note Якорь и метка суть свойства YAML, каких прочие кодеки не знают вовсе, и
		 *       предпосылаются они ДО открытия вместилища либо записи простого значения:
		 *       сборка держит их у себя до ближайшего занесения
		 */
		if(!value.anchor().empty() && !builder.anchor(value.anchor()))
			// Выводим признак неудачной сборки
			return false;
		/**
		 * Если собираемому значению предпослана метка
		 */
		if(!value.tag().empty() && !builder.tag(value.tag()))
			// Выводим признак неудачной сборки
			return false;
		/**
		 * Определяем вид собираемого значения
		 */
		switch(static_cast <uint8_t> (value.kind())){
			/**
			 * Если значение является отображением пар
			 */
			case static_cast <uint8_t> (yaml::kind_t::MAPPING): {
				/**
				 * Если открыть отображение пар не удалось
				 */
				if(!builder.mapping(value.layout()))
					// Выводим признак неудачной сборки
					return false;
				/**
				 * Выполняем перебор полей отображения
				 */
				for(size_t i = 0; i < value.size(); i++){
					/**
					 * Если назначить имя поля отображения не удалось
					 *
					 * @note Имя назначается добавлением, а не установкой: отображение вправе
					 *       нести повторяющиеся имена - их даёт правило `duplicate_t::KEEP`, -
					 *       и установка слила бы два поля одного имени в одно. Первому
					 *       вхождению добавление не вредит: отображение к тому мигу пусто
					 */
					if(!builder.append(value.key(i)))
						// Выводим признак неудачной сборки
						return false;
					/**
					 * Если собрать значение поля отображения не удалось
					 */
					if(!assemble(value[i], builder))
						// Выводим признак неудачной сборки
						return false;
				}
				// Выводим признак успешности закрытия отображения пар
				return builder.close();
			}
			/**
			 * Если значение является перечнем значений
			 */
			case static_cast <uint8_t> (yaml::kind_t::SEQUENCE): {
				/**
				 * Если открыть перечень значений не удалось
				 */
				if(!builder.sequence(value.layout()))
					// Выводим признак неудачной сборки
					return false;
				/**
				 * Выполняем перебор значений перечня
				 */
				for(size_t i = 0; i < value.size(); i++){
					/**
					 * Если собрать значение перечня не удалось
					 */
					if(!assemble(value[i], builder))
						// Выводим признак неудачной сборки
						return false;
				}
				// Выводим признак успешности закрытия перечня значений
				return builder.close();
			}
		}
		// Выводим признак успешности записи простого значения
		return builder.value(value);
	}

}

/**
 * @brief Признак сборки под надзирателями памяти
 *
 * @details Надзиратели есть не всюду: у OpenWRT с musl тел их нет вовсе, и сборщик
 *          ворошителя, пробою это установив, собирает без них - о том он и говорит
 *          при сборке. Но отчёт ПРОГОНА о том молчал, а именно отчёт и переносится в
 *          записи: чистая кампания без надзирателей доказывает равенство поведения, а
 *          не чистоту памяти, и разницу эту надо видеть в самой строке отчёта
 *
 * @note Признаки эти ставит сам собиратель: `__SANITIZE_ADDRESS__` у GCC,
 *       `__has_feature(address_sanitizer)` у Clang
 */
#if defined(__has_feature)
	#if __has_feature(address_sanitizer)
		#define AWH_FUZZ_SANITIZED 1
	#endif
#endif
#if !defined(AWH_FUZZ_SANITIZED) && defined(__SANITIZE_ADDRESS__)
	#define AWH_FUZZ_SANITIZED 1
#endif
#if !defined(AWH_FUZZ_SANITIZED)
	#define AWH_FUZZ_SANITIZED 0
#endif

/**
 * @brief Функция запуска приложения
 *
 * @param argc длина массива параметров
 * @param argv массив параметров
 * @return     код выхода из приложения
 *
 */
int32_t main(int32_t argc, char * argv[]) noexcept {
	/**
	 * Выполняем заведение модуля ядра первым делом
	 *
	 * @note Заведение захватывает выдачу памяти процесса и обязано идти
	 *       ДО всякой выдачи и ДО порождения потоков
	 */
	awh::fmk::initialize();
	// Количество выполняемых проходов генератора
	uint64_t count = 3000;
	/**
	 * Если количество проходов задано параметром командной строки
	 */
	if(argc > 1)
		// Выполняем чтение количества проходов из параметра командной строки
		count = static_cast <uint64_t> (::strtoull(argv[1], nullptr, 10));
	// Зерно источника псевдослучайных чисел
	uint32_t seed = 0x7A11;
	/**
	 * Если зерно задано вторым параметром командной строки
	 *
	 * @note Зерно закреплено по умолчанию: прогон обязан воспроизводиться. Задавать его
	 *       иным стоит затем, что построения одного зерна складываются в один и тот же
	 *       набор, сколько проходов ни задай
	 */
	if(argc > 2)
		// Выполняем чтение зерна из параметра командной строки
		seed = static_cast <uint32_t> (::strtoul(argv[2], nullptr, 10));
	// Создаём источник псевдослучайных чисел с закреплённым зерном
	mt19937 engine(seed);
	// Шаг вывода отчёта о ходе работы генератора
	const uint64_t report = ((count > 20) ? (count / 20) : 0);
	/**
	 * Выполняем проходы генератора
	 */
	for(uint64_t i = 0; i < count; i++){
		// Запоминаем признак прохода, однобайтовым кодировкам посильного
		latined = ((i % 4) == 0);
		/**
		 * Если очередным проходом пришёл черёд отчёта о ходе работы
		 *
		 * @note Прогон на многих миллионах проходов идёт часами, и молчание его
		 *       неотличимо от зависания: отчёт этот показывает, что работа движется, и
		 *       даёт снять пробу на том проходе, на котором прогон встал
		 */
		if((report > 0) && (i > 0) && ((i % report) == 0)){
			// Выводим отчёт о ходе работы генератора
			::fprintf(stderr, "yaml fuzz: %llu/%llu passes, %llu events, %llu chunked, %llu transcoded\n",
				static_cast <unsigned long long> (i), static_cast <unsigned long long> (count),
				static_cast <unsigned long long> (totals.events), static_cast <unsigned long long> (totals.chunked),
				static_cast <unsigned long long> (totals.transcoded));
			// Выполняем выталкивание накопленного вывода
			::fflush(stderr);
		}
		// Собираемые настройки разбора текста
		yaml::reader_t::settings_t settings;
		// Задаём схему разрешения видов скалярных значений
		settings.schema = static_cast <yaml::schema_t> (engine() % 4);
		// Задаём выдачу событий примечаний
		settings.emitComments = ((engine() % 4) != 0);
		// Задаём выдачу событий пустых строк
		settings.emitBlanks = ((engine() % 4) != 0);
		// Задаём правило обращения с повторяющимся именем пары
		settings.duplicates = static_cast <yaml::duplicate_t> (engine() % 4);
		/**
		 * Если требуется поджать пределы разбора
		 *
		 * @details Пределы меряются по ходу накопления одной величиной, а по готовой
		 * записи - другой, и расхождения их проступают лишь на поджатых значениях: при
		 * умолчаниях накопление до предела не доходит вовсе
		 */
		if((engine() % 4) == 0){
			// Устанавливаем наибольшую допустимую глубину вложенности
			settings.maxDepth = (1 + (engine() % 4));
			// Устанавливаем наибольшую допустимую длину скалярного значения
			settings.maxScalar = (1 + (engine() % 32));
		}
		// Выполняем построение текста
		string text = generate(engine);
		// Выполняем учёт построенного текста
		totals.texts++;
		/**
		 * Если требуется испортить построенный текст
		 */
		if((engine() % 3) == 0){
			// Выполняем порчу построенного текста
			corrupt(text, engine);
			// Выполняем учёт испорченного текста
			totals.corrupted++;
		}
		// Перечень событий подачи текста целиком
		vector <Event> whole;
		// Код ошибки разбора текста, поданного целиком
		yaml::error_t failure = yaml::error_t::NONE;
		// Выполняем подачу текста целиком
		const yaml::state_t state = consume(text, settings, 0, whole, &failure);
		/**
		 * Если текст разобран до конца
		 */
		if(state == yaml::state_t::FINISHED)
			// Выполняем учёт разобранного до конца текста
			totals.survived++;
		/**
		 * Выполняем сличение подачи свежим чтением с подачей чтением, сброшенным после
		 * прежней подачи
		 *
		 * @details Чтение, один текст уже прочитавшее, обязано по сбросу прочесть
		 *          следующий ровно так же, как прочло бы его чтение свежее: стопа
		 *          открытых построений, метки, наречие, опознанная кодировка - остаток
		 *          всего этого обязан быть сброшен целиком. Дефект такого рода прогоном
		 *          свежего чтения не воспроизводится ни разу
		 *
		 * @note Чтение это живёт от прохода к проходу намеренно: чем длиннее череда
		 *       прочитанных им текстов, тем вернее вылезет остаток
		 */
		{
			// Перечень событий подачи текста чтением, прежней подачей занятым
			vector <Event> reused;
			// Код ошибки разбора текста чтением, прежней подачей занятым
			yaml::error_t repeated = yaml::error_t::NONE;
			// Выполняем подачу текста целиком чтением, прежней подачей занятым
			const yaml::state_t achieved = consume(text, settings, 0, reused, &repeated, nullptr, true);
			/**
			 * Если состояние по окончании подачи со свежим чтением разошлось
			 */
			if(achieved != state){
				// Выводим сообщение о расхождении состояния подачи
				::fprintf(stderr, "yaml fuzz: reused reader state differs: %u vs %u, settings %s\n",
				 static_cast <uint32_t> (achieved), static_cast <uint32_t> (state), described(settings).c_str());
				// Выводим разбираемый текст
				dump(text);
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
			}
			/**
			 * Если код ошибки со свежим чтением разошёлся
			 */
			if(repeated != failure){
				// Выводим сообщение о расхождении кода ошибки
				::fprintf(stderr, "yaml fuzz: reused reader error differs: %s vs %s, settings %s\n",
				 yaml::message(repeated), yaml::message(failure), described(settings).c_str());
				// Выводим разбираемый текст
				dump(text);
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
			}
			// Выполняем сличение перечней выданных событий
			if(!compare(whole, reused, "сброшенное чтение", text, settings))
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
		}
		/**
		 * Выполняем сличение подачи целиком с подачей кусками нескольких размеров
		 *
		 * @note Размеров берётся несколько, а не один, и первый из них всегда
		 *       однобайтовый: расхождения проступают лишь при таких размерах, где кусок
		 *       рвёт запись посреди неё
		 */
		for(uint32_t attempt = 0; attempt < 3; attempt++){
			// Размер куска подачи текста
			const size_t chunk = ((attempt == 0) ? 1 : (1 + (engine() % 24)));
			// Перечень событий подачи текста кусками
			vector <Event> chunked;
			// Код ошибки разбора текста, поданного кусками
			yaml::error_t reached = yaml::error_t::NONE;
			// Выполняем подачу текста кусками
			const yaml::state_t outcome = consume(text, settings, chunk, chunked, &reached);
			// Выполняем учёт сличения подачи кусками
			totals.chunked++;
			/**
			 * Если исход разбора нарезкою переменился
			 *
			 * @note Сличается не одна лишь выдача событий, но и состояние с кодом отказа:
			 *       разбор, отказавший целиком и прошедший кусками, событиями разошёлся бы
			 *       не всегда, а исход его переменился бы всё равно
			 */
			if((outcome != state) || (reached != failure)){
				// Выводим сообщение о расхождении исхода разбора
				::fprintf(stderr, "yaml fuzz: chunk=%zu outcome %u/%u error %u/%u, settings %s\n",
					chunk, static_cast <unsigned> (state), static_cast <unsigned> (outcome),
					static_cast <unsigned> (failure), static_cast <unsigned> (reached),
					described(settings).c_str());
				// Выводим разбираемый текст
				dump(text);
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
			}
			/**
			 * Если перечни выданных разбором событий разошлись
			 */
			if(!compare(whole, chunked, ("chunk=" + to_string(chunk)), text, settings))
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
		}
		/**
		 * Если разбираемый текст записью UTF-8 является
		 *
		 * @note Метка порядка байтов снимается прежде приведения: приведение ставит свою,
		 *       и без снятия текст нёс бы две метки, вторая из которых стала бы содержимым
		 */
		string source(text);
		/**
		 * Если текст открывается меткой порядка байтов
		 */
		if((source.length() >= 3) && (source.compare(0, 3, "\xEF\xBB\xBF") == 0))
			// Выполняем снятие метки порядка байтов с начала текста
			source.erase(0, 3);
		// Перечень событий подачи текста в кодировке UTF-8 без метки
		vector <Event> native;
		// Опознанная кодировка текста, поданного без метки
		yaml::encoding_t recognised = yaml::encoding_t::NONE;
		// Выполняем подачу текста в кодировке UTF-8 без метки
		consume(source, settings, 0, native, nullptr, &recognised);
		// Выполняем сброс смещений начала событий
		detach(native);
		/**
		 * Выполняем постройку дерева документа по разобранному тексту
		 *
		 * @details Дерево строится и на текстах испорченных: разбор их прекращается
		 *          отказом, а дерево собирается настолько, насколько текст прочитан - путь
		 *          этот проходится чаще всякого иного, и оставить его без пробы нельзя
		 */
		{
			// Настройки разбора дерева документа
			yaml::document_t::settings_t tree;
			// Устанавливаем схему разрешения видов скалярных значений
			tree.schema = settings.schema;
			// Устанавливаем правило обращения с повторяющимся именем пары
			tree.duplicates = settings.duplicates;
			// Устанавливаем наибольшую допустимую глубину вложенности
			tree.maxDepth = settings.maxDepth;
			// Устанавливаем наибольшую допустимую длину скалярного значения
			tree.maxScalar = settings.maxScalar;
			// Устанавливаем наибольшее допустимое количество узлов раскрытия ссылок
			tree.maxExpansion = 4096;
			// Объект дерева документа
			yaml::document_t document(tree);
			/**
			 * Если разобрать текст в дерево документа удалось
			 */
			// Выполняем разбор текста в дерево документа
			const bool parsed = document.parse(text);
			/**
			 * Если разбор текста документа отвергнут без названной причины
			 *
			 * @details Отказ разбора обязан называть причину наравне с отказом правки:
			 *          потребитель, получивший ложь без кода ошибки, не узнает ни места
			 *          отказа, ни рода его
			 */
			if(!parsed && (document.error() == yaml::error_t::NONE)){
				// Выводим сообщение об отказе без названной причины
				::fprintf(stderr, "yaml fuzz: parsing refused without a named cause\n");
				// Выводим отрицательный результат работы генератора
				return EXIT_FAILURE;
			}
			/**
			 * Выполняем проверку разбора на дереве, прежним разбором уже занятом
			 *
			 * @details Дерево, разобравшее один текст, обязано разобрать следующий ровно
			 *          так же, как разобрало бы его дерево свежее: остаток прежнего разбора
			 *          - метки, наречия, схема, удержанный текст - обязан быть сброшен
			 *          целиком. Утечка такого остатка даёт дефект, ни одним прогоном
			 *          свежего дерева не воспроизводимый: он показывается лишь у
			 *          потребителя, разбирающего одним объектом много текстов подряд
			 *
			 * @note Дерево это живёт от прохода к проходу намеренно: чем длиннее череда
			 *       разобранных им текстов, тем вернее вылезет остаток
			 */
			{
				// Дерево документа, от прохода к проходу живущее
				static yaml::document_t recycled;
				// Устанавливаем настройки разбора очередного прохода
				recycled.settings(tree);
				// Выполняем разбор того же текста деревом, прежним разбором занятым
				const bool again = recycled.parse(text);
				/**
				 * Если исход разбора с исходом свежего дерева разошёлся
				 */
				if(again != parsed){
					// Выводим сообщение о расхождении исхода разбора
					::fprintf(stderr, "yaml fuzz: reused tree parse differs: %s vs %s, settings %s\n",
					 (again ? "удался" : "отвергнут"), (parsed ? "удался" : "отвергнут"),
					 described(settings).c_str());
					// Выводим разбираемый текст
					dump(text);
					// Выходим из приложения с ошибкой
					return EXIT_FAILURE;
				}
				/**
				 * Если разбор удался, а перезаписи деревьев разошлись
				 */
				if(parsed && (recycled.dump() != document.dump())){
					// Выводим сообщение о расхождении перезаписей
					::fprintf(stderr, "yaml fuzz: reused tree differs, settings %s\n", described(settings).c_str());
					// Выводим разбираемый текст
					dump(text);
					// Выводим перезапись свежего дерева
					dump(document.dump());
					// Выводим перезапись дерева, прежним разбором занятого
					dump(recycled.dump());
					// Выходим из приложения с ошибкой
					return EXIT_FAILURE;
				}
				// Выполняем учёт разбора деревом, прежним разбором занятым
				totals.recycled++;
			}
			/**
			 * Выполняем проверку независимости исхода разбора от хвостового перевода строки
			 *
			 * @details Описание языка о переводе строки в конце текста не говорит ничего:
			 *          текст, разбором принятый, обязан приниматься и с переводом, и без
			 *          него, отдавая то же дерево. Разница здесь есть порок сама по себе
			 *
			 * @note Правило это завелось находкою и тут же нашло три порока склейки строк:
			 *       кавычку внутри продолжения простого значения, кавычку за свойством
			 *       значения и написание `-\"`. Строка последняя, перевода не имеющая,
			 *       склейке не подлежит вовсе, и пороки эти на ней не проявлялись
			 */
			/**
			 * Признак того, что текст возвратом каретки одиноким оканчивается
			 *
			 * @warning Возврат такой записи негоден сам по себе, и перевод строки, к нему
			 *          приписанный, обращает окончание в годное «\r\n». Отказ, правкою
			 *          снятый, есть тут исход ВЕРНЫЙ, а не расхождение
			 */
			if(!text.empty() && (text.back() != '\r') && !::multibyte(text)){
				/**
				 * Собираем написание текста иное - с переводом строки хвостовым либо без него
				 *
				 * @note Снимается перевод ЦЕЛИКОМ, вместе с возвратом каретки: снятие одного
				 *       перевода у окончания «\r\n» оставляет возврат одинокий, а он записи
				 *       негоден вовсе, и ворошитель подал на то ложное расхождение
				 */
				const string written = ((text.back() == '\n') ?
				 text.substr(0, (text.size() - (((text.size() > 1) && (text.at(text.size() - 2) == '\r')) ? 2 : 1))) :
				 (text + "\n"));
				// Дерево документа, написание иное разбирающее
				yaml::document_t tailed(tree);
				// Выполняем разбор написания иного
				const bool taken = tailed.parse(written);
				// Признак удержания переводов блочным значением
				bool keeping = false;
				/**
				 * Выполняем розыск указателя удержания переводов у блочных значений
				 *
				 * @warning Правилу `chomp_t::KEEP` перевод хвостовой есть СОДЕРЖИМОЕ, и
				 *          снятие его значение меняет по праву. Сличать деревья тут нельзя:
				 *          ворошитель назвал бы расхождением ровно то, чего описание языка
				 *          и требует. Исход разбора правилу тому подвластен ТОЖЕ - и не одним
				 *          лишь пределом длины значения, как думалось прежде. Пределом:
				 *          снятие перевода удержанного длину значения укорачивает, и
				 *          «- >+\n  2024-02-29\n\n» при пределе 11 отвергается целиком, а
				 *          усечённое - принимается. Меткою типа: содержимое удержанного
				 *          значения перевод этот НЕСЁТ, и «- !!null >3+\n\n» отвергается
				 *          несоответствием виду, а усечённое - пустотою метке отвечает.
				 *          Оба исхода тут ВЕРНЫ, и оттого сличение снимается у всякого
				 *          удержания, а не у одного лишь предела
				 *
				 * @note Прежняя оговорка стояла условием `keeping && (settings.maxScalar > 0)`
				 *       и второй случай пропускала: ворошитель подал на него ложное
				 *       расхождение исходов разбора
				 *
				 * @note Розыск ведётся перебором, а не поиском записи «|+»: между знаком
				 *       блока и указателем удержания вправе стоять указатель отступа - и
				 *       написание `>1+` поиск записью пропускал, чем и подал ложное
				 *       расхождение на первом же прогоне
				 */
				for(size_t i = 0; !keeping && (i < text.size()); i++){
					/**
					 * Если знак блочное значение открывает
					 */
					if((text.at(i) == '|') || (text.at(i) == '>')){
						// Смещение знака, за знаком блока стоящего
						size_t position = (i + 1);
						/**
						 * Выполняем пропуск указателя отступа блочного значения
						 */
						while((position < text.size()) && (text.at(position) >= '0') && (text.at(position) <= '9'))
							// Выполняем переход к следующему знаку текста
							position++;
						// Запоминаем признак удержания переводов блочным значением
						keeping = ((position < text.size()) && (text.at(position) == '+'));
					}
				}
				/**
				 * Если исход разбора написания иного с исходом исходного разошёлся
				 */
				if((taken != parsed) && !keeping){
					// Выводим сообщение о расхождении исхода разбора
					::fprintf(stderr, "yaml fuzz: trailing newline changes verdict: %s vs %s, settings %s\n",
					 (taken ? "принят" : "отвергнут"), (parsed ? "принят" : "отвергнут"),
					 described(settings).c_str());
					// Выводим разбираемый текст
					dump(text);
					// Выходим из приложения с ошибкой
					return EXIT_FAILURE;
				}
				/**
				 * Если разбор удался, а деревья написаний разошлись
				 *
				 * @note Сличаются ДЕРЕВЬЯ, а не перезаписи их: перевод хвостовой удержанный
				 *       исходный текст меняет, и сличение перезаписей звало бы это
				 *       расхождением при настройках, оформление сохраняющих
				 */
				if(parsed && !keeping && !(yaml::value_t(tailed.root()) == yaml::value_t(document.root()))){
					// Выводим сообщение о расхождении деревьев написаний
					::fprintf(stderr, "yaml fuzz: trailing newline changes tree, settings %s\n",
					 described(settings).c_str());
					// Выводим разбираемый текст
					dump(text);
					// Выводим перезапись дерева написания иного
					dump(tailed.dump());
					// Выводим перезапись дерева исходного написания
					dump(document.dump());
					// Выходим из приложения с ошибкой
					return EXIT_FAILURE;
				}
				// Выполняем учёт разбора написания иного
				totals.tailed++;
			}
			if(parsed){
				// Выполняем учёт собранного дерева документа
				totals.trees++;
				/**
				 * Выполняем проверку владеющего значения, с дерева снятого
				 *
				 * @details Проверяется трижды: согласие извлечения с деревом узел за узлом,
				 *          круговой ход перезаписи и потоковая сборка того же дерева заново.
				 *          Первая из трёх нашла дефект чтения целого без знака у самого
				 *          дерева, оттого она здесь и первая
				 */
				if(document.root().valid()){
					// Владеющее значение, с дерева документа снятое
					const yaml::value_t taken(document.root());
					/**
					 * Выполняем круговой ход снятого значения через собственную перезапись
					 *
					 * @details Значение, с дерева снятое, обязано пережить свою же перезапись
					 * без потерь: перезапись его читается тем же наречием, и снятое с
					 * прочтённого обязано совпасть с исходным. Путь этот - снятие, перезапись,
					 * чтение, снятие - именно тот, каким пользуется всякий, кто держит
					 * настройки владеющим значением
					 *
					 * @note Сличается ЗНАЧЕНИЕ, а не текст: владеющее значение примечаний не
					 *       держит вовсе, и требовать от его перезаписи дословного совпадения
					 *       с перезаписью дерева значило бы требовать невозможного
					 */
					{
						// Перезапись снятого значения
						const string written = taken.dump();
						/**
						 * Если перезапись снятого значения не пуста
						 *
						 * @note Пустая перезапись означает отказ записи, а он поверяется своими
						 *       проверками: сличать нечего, и круг пропускается
						 */
						if(!written.empty()){
							/**
							 * Собираемые настройки разбора перезаписи со снятыми пределами
							 *
							 * @note Пределы снимаются намеренно: ссылки деревом раскрываются, и
							 *       перезапись раскрытого законно глубже исходного текста. Отказ
							 *       по пределу означал бы вину ворошителя, а не записи
							 */
							yaml::document_t::settings_t unbound = tree;
							// Выполняем снятие предела глубины вложенности
							unbound.maxDepth = 0;
							// Выполняем снятие предела длины скалярного значения
							unbound.maxScalar = 0;
							// Собираемое дерево документа перезаписи снятого значения
							yaml::document_t rebuilt(unbound);
							/**
							 * Если перезапись снятого значения разобрана
							 */
							if(rebuilt.parse(written)){
								// Значение, снятое с дерева перезаписи
								const yaml::value_t back(rebuilt.root());
								/**
								 * Если снятое с перезаписи значение с исходным разошлось
								 */
								if(!(back == taken)){
									// Выводим сообщение о расхождении кругового хода
									::fprintf(stderr, "yaml fuzz: round trip of the taken value through a rewrite ended in divergence\n");
									// Выводим разбираемый текст
									dump(text);
									// Выводим перезапись дерева документа
									dump(document.dump());
									// Выводим перезапись снятого значения
									dump(written);
									// Выводим перезапись значения, с перезаписи снятого
									dump(back.dump());
									// Выходим из приложения с ошибкой
									return EXIT_FAILURE;
								}
								// Выполняем учёт кругового хода снятого значения
								totals.circling++;
							}
						}
					}
					// Разошедшийся путь извлечения, розыском выданный
					string diverged;
					/**
					 * Если извлечение владеющего значения с деревом разошлось
					 */
					if(!agreeing(document.root(), taken, string(), diverged)){
						// Выводим сообщение о расхождении извлечения
						::fprintf(stderr, "yaml fuzz: taken value disagrees with tree at «%s», settings %s\n",
							diverged.c_str(), described(settings).c_str());
						// Выводим разбираемый текст
						dump(text);
						// Выходим из приложения с ошибкой
						return EXIT_FAILURE;
					}
					// Выполняем учёт снятого владеющего значения
					totals.taken++;
					// Прочитанное обратно владеющее значение
					yaml::value_t circled;
					/**
					 * Если круговой ход перезаписи владеющего значения нарушен
					 *
					 * @note Перезапись снятого значения обязана читаться обратно и давать
					 *       значение то же самое: оформления оно не удерживает, и всё, что
					 *       переживает круг, есть суть значения
					 */
					// Настройки разбора перезаписи снятого значения
					yaml::document_t::settings_t unbound = tree;
					/**
					 * Выполняем снятие предела глубины вложенности
					 *
					 * @note Пределы перезаписи не ставятся: ссылки деревом раскрываются, и
					 *       перезапись раскрытого дерева законно глубже исходного текста.
					 *       Отказ по пределу означал бы вину ворошителя, а не записи
					 */
					unbound.maxDepth = 0;
					// Выполняем снятие предела длины скалярного значения
					unbound.maxScalar = 0;
					if(!circled.parse(taken.dump(), unbound) || !(circled == taken)){
						// Выводим сообщение о нарушении кругового хода
						::fprintf(stderr, "yaml fuzz: taken value round trip broken, settings %s\n",
							described(settings).c_str());
						// Выводим разбираемый текст
						dump(text);
						// Выводим перезапись снятого значения
						dump(taken.dump());
						// Выходим из приложения с ошибкой
						return EXIT_FAILURE;
					}
					// Потоковая сборка владеющего значения
					yaml::builder_t builder;
					/**
					 * Если собрать значение потоковой сборкой удалось
					 */
					if(assemble(taken, builder)){
						// Получаем собранное потоковой сборкой значение
						const yaml::value_t built = builder.finish();
						/**
						 * Если собранное потоковой сборкой значение с исходным разошлось
						 */
						if(!(built == taken)){
							// Выводим сообщение о расхождении потоковой сборки
							::fprintf(stderr, "yaml fuzz: streamed assembly differs, settings %s\n",
								described(settings).c_str());
							// Выводим разбираемый текст
							dump(text);
							// Выводим перезапись собранного значения
							dump(built.dump());
							// Выходим из приложения с ошибкой
							return EXIT_FAILURE;
						}
						// Путь узла, где якоря либо метки разошлись
						string diverged;
						/**
						 * Если якоря либо метки собранного значения с исходным разошлись
						 */
						if(!decorated(built, taken, "", diverged)){
							// Выводим сообщение о расхождении якорей потоковой сборки
							::fprintf(stderr, "yaml fuzz: streamed assembly loses decoration at %s, settings %s\n",
								diverged.c_str(), described(settings).c_str());
							// Выводим разбираемый текст
							dump(text);
							// Выводим перезапись собранного значения
							dump(built.dump());
							// Выходим из приложения с ошибкой
							return EXIT_FAILURE;
						}
						// Выполняем учёт собранного потоковой сборкой значения
						totals.assembled++;
					}
					/**
					 * Выполняем круговой перенос снятого значения в дерево документа
					 *
					 * @note Перенос вправе и отказать: имя пары, косую черту несущее, путь
					 *       выразить не может, и отказ там намеренный. Отказ расхождением не
					 *       считается, но считается счётчиком - молчаливый пропуск обратил бы
					 *       проверку в бездействующую
					 */
					{
						// Собираемое дерево документа, куда переносится значение
						yaml::document_t target;
						/**
						 * Если разобрать пустое отображение удалось
						 *
						 * @note Настройки берутся те же, какими разобрано исходное дерево:
						 *       перенос кладёт числа записью своею, и наречие дерева решает,
						 *       чем они прочтутся обратно. Запись `0777` наречием 1.1 есть
						 *       число восьмеричное, а наречием 1.2 - десятичное, и дерево
						 *       иных настроек выдало бы значение иное по праву, а не по
						 *       дефекту
						 */
						/**
						 * Выполняем назначение дереву переноса настроек исходного дерева
						 *
						 * @note Настройки берутся у самого дерева, а не у ворошителя: указание
						 *       `%YAML 1.1` в тексте пересиливает заданную схему, и дерево
						 *       держит уже схему действующую. Перенос кладёт числа записью
						 *       своею, и наречие дерева решает, чем они прочтутся обратно:
						 *       `0777` наречием 1.1 есть число восьмеричное, а ядровою схемой -
						 *       десятичное, и расхождение то законно
						 */
						yaml::document_t::settings_t carried = document.settings();
						// Выполняем снятие предела глубины вложенности
						carried.maxDepth = 0;
						// Выполняем снятие предела длины скалярного значения
						carried.maxScalar = 0;
						/**
						 * Выполняем назначение схемы, деревом действительно применяемой
						 *
						 * @note Указание `%YAML 1.1` в тексте пересиливает заданную схему, а
						 *       настройки держат заданную: схему действующую выдаёт сам узел
						 */
						carried.schema = document.root().schema();
						// Выполняем назначение дереву переноса настроек исходного дерева
						target.settings(carried);
						/**
						 * Если разобрать пустое отображение удалось
						 */
						if(target.parse("{}")){
							// Выполняем учёт попытки кругового переноса
							totals.grafts++;
							/**
							 * Если перенести снятое значение в дерево удалось
							 */
							/**
							 * Если перенести снятое значение в дерево не удалось
							 *
							 * @details Оба отказа переноса намеренны и описаны при самом методе:
							 *          имя, косую черту несущее, путём указывало бы на узел чужой,
							 *          а имя, вторично встреченное, путём не выражается вовсе.
							 *          Правило поверяет не сам отказ, а НАЗВАНИЕ причины его:
							 *          отказ без причины отдаёт потребителю ложь - `error()`
							 *          отвечает «ошибок не обнаружено» при истинном отказе, и
							 *          потребитель по ней заключил бы об удаче
							 *
							 * @note Правило это ловит ступень, причину не назначившую, а не
							 *       наличие самого способа передачи её: способ был верен и
							 *       описан, а нижняя ступень укладки причины не ставила
							 */
							const bool planted = taken.graft(target);
							if(!planted && (target.error() == yaml::error_t::NONE)){
								// Выводим сообщение об отказе переноса без названной причины
								::fprintf(stderr, "yaml fuzz: graft refused without a named cause, settings %s\n",
									described(settings).c_str());
								// Выводим разбираемый текст
								dump(text);
								// Выводим перезапись снятого значения
								dump(taken.dump());
								// Выходим из приложения с ошибкой
								return EXIT_FAILURE;
							}
							if(planted){
								// Выполняем учёт удавшегося переноса
								totals.grafted++;
								// Выполняем снятие владеющего значения с дерева переноса
								const yaml::value_t back(target.root());
								/**
								 * Если снятое с дерева переноса значение с исходным разошлось
								 */
								if(!(back == taken)){
									// Выводим сообщение о расхождении перенесённого значения
									::fprintf(stderr, "yaml fuzz: grafted value differs from the taken one, settings %s\n",
										described(settings).c_str());
									// Выводим разбираемый текст
									dump(text);
									// Выводим перезапись снятого значения
									dump(taken.dump());
									// Выводим перезапись перенесённого значения
									dump(back.dump());
									// Выходим из приложения с ошибкой
									return EXIT_FAILURE;
								}
							}
						}
					}
				}
				/**
				 * Настройки разбора перезаписанного документа
				 *
				 * @details Пределы глубины и длины значения перезаписи не ставятся: ссылки
				 *          деревом раскрываются, и перезапись раскрытого дерева законно глубже
				 *          исходного текста. Отказ по пределу означал бы вину ворошителя, а не
				 *          записи, и находкою считаться не вправе
				 */
				yaml::document_t::settings_t limitless = tree;
				// Выполняем снятие предела глубины вложенности
				limitless.maxDepth = 0;
				// Выполняем снятие предела длины скалярного значения
				limitless.maxScalar = 0;
				// Объект дерева перезаписанного документа
				yaml::document_t rewritten(limitless);
				/**
				 * Если разобрать перезаписанный текст не удалось
				 *
				 * @note Перезапись обязана читаться обратно всегда: текст, деревом собранный
				 *       и разбором отвергнутый, есть дефект записи, а не разбора
				 */
				if(!rewritten.parse(document.dump())){
					// Выводим сообщение об отказе чтения перезаписанного текста
					::fprintf(stderr, "yaml fuzz: rewrite refused %s, settings %s\n",
						yaml::message(rewritten.error()), described(settings).c_str());
					// Выводим перезаписанный текст
					dump(document.dump());
					// Выводим разбираемый текст
					dump(text);
					// Выходим из приложения с ошибкой
					return EXIT_FAILURE;
				}
				/**
				 * Если перезапись перезаписанного с первою разошлась
				 *
				 * @note Перезапись обязана быть устойчивой: второй проход её менять уже
				 *       нечему, и расхождение означает, что дерево первого разбора и дерево
				 *       второго - разные деревья
				 */
				if(rewritten.dump() != document.dump()){
					// Выводим сообщение о расхождении перезаписей
					::fprintf(stderr, "yaml fuzz: rewrite unstable, settings %s\n", described(settings).c_str());
					// Выводим разбираемый текст
					dump(text);
					// Выводим перезапись первого дерева
					dump(document.dump());
					// Выводим перезапись второго дерева
					dump(rewritten.dump());
					// Выходим из приложения с ошибкой
					return EXIT_FAILURE;
				}
				/**
				 * Выполняем круговой ход перезаписи иным оформлением
				 *
				 * @details Оформление записи задаётся девятью настройками, а круговой ход
				 *          поверял доселе одно лишь оформление по умолчанию. Текст, записанный
				 *          скобками либо иною оградою, обязан читаться обратно и давать то же
				 *          самое дерево: оформление сути значений не несёт, и всё, что круг
				 *          переживает, есть суть
				 *
				 * @note Схема записи берётся схемою разбора: разойдись они, разошлось бы и
				 *       разрешение видов скалярных значений, а это расхождение законное и к
				 *       записи отношения не имеющее
				 *
				 * @note Ограда блочная в выборку не берётся: записывающий зовёт её наудачу и,
				 *       ложь получив, законно отступает к ограде двойной - выбор её ничего к
				 *       поверке не прибавляет
				 *
				 * @note Обращение с негодной кодировкой остаётся умолчанием: уклад REFUSE
				 *       отдаёт пустоту намеренно, и круговой ход поверять там нечего
				 */
				{
					/**
					 * Если текст наречие своё директивою объявляет
					 *
					 * @details Проверка эта обходится стороною: директива `%YAML` переводит
					 *          разбор на схему объявленного наречия, и схема действовавшая
					 *          расходится тогда со схемою, настройками заданной. Записывающему
					 *          же настройками передаётся именно заданная - выдачи схемы
					 *          действовавшей дерево не имеет вовсе, - и перезапись выходила бы
					 *          записанной по одной схеме, а прочтённой по другой. Расхождение
					 *          то принадлежит ворошителю, а не записи
					 *
					 * @note Обход считается счётчиком: молчаливый пропуск обратил бы проверку
					 *       в бездействующую там, где директивы часты
					 */
					if(document.dump().compare(0, 5, "%YAML") == 0)
						// Выполняем учёт обойдённого кругового хода
						totals.undirected++;
					// Если наречие директивою не объявлено
					else {
					// Настройки записи текста, наудачу выбранные
					yaml::writer_t::settings_t writing;
					// Устанавливаем схему записи схемою разбора
					writing.schema = tree.schema;
					/**
					 * Если действует схема защитная, пустоты не знающая
					 *
					 * @note Схема эта всякое скалярное значение числит строкою, и пустоты вида
					 *       своего при ней нет вовсе. Внутри скобок пустота записи не имеет, и
					 *       записывающему остаётся ограда пустая - строка, а не пустота.
					 *       Круговой ход тут теряет вид значения не по дефекту записи, а по
					 *       самой схеме, и требовать от него равенства деревьев нельзя
					 */
					if(writing.schema == yaml::schema_t::FAILSAFE)
						// Выполняем учёт обойдённого кругового хода
						totals.undirected++;
					// Если схема пустоту выразить способна
					else {
					// Устанавливаем построение вместилищ наудачу
					writing.layout = (((engine() % 2) == 0) ? yaml::layout_t::BLOCK : yaml::layout_t::FLOW);
					// Порядковый номер ограды, значениям назначаемой
					const uint32_t quoting = (engine() % 3);
					// Устанавливаем ограду значений наудачу
					writing.quoting = ((quoting == 0) ? yaml::style_t::PLAIN :
					 ((quoting == 1) ? yaml::style_t::SINGLE : yaml::style_t::DOUBLE));
					// Устанавливаем ширину отступа наудачу, нуля не допуская
					writing.indent = static_cast <uint8_t> (1 + (engine() % 8));
					// Устанавливаем отступ перечней наудачу
					writing.sequenceIndent = ((engine() % 2) == 0);
					// Устанавливаем запись черты начала документа наудачу
					writing.explicitStart = ((engine() % 2) == 0);
					// Устанавливаем запись черты конца документа наудачу
					writing.explicitEnd = ((engine() % 2) == 0);
					/**
					 * Устанавливаем запись директивы наречия невыполняемой
					 *
					 * @note Директива наречия схему разбора переводит, и навязывать её тексту,
					 *       её не имевшему, значило бы читать перезапись схемою иной, нежели
					 *       та, какою она писана
					 */
					writing.version = false;
					// Выполняем перезапись дерева выбранным оформлением
					const string restyled = document.dump(writing);

					/**
					 * Если перезапись оформлением иным отдала пустоту при непустом дереве
					 */
					if(restyled.empty() && !document.empty()){
						// Выводим сообщение о пустой перезаписи
						::fprintf(stderr, "yaml fuzz: restyled rewrite is empty, layout %u quoting %u indent %u\n",
						 static_cast <uint32_t> (writing.layout), quoting, static_cast <uint32_t> (writing.indent));
						// Выводим разбираемый текст
						dump(text);
						// Выходим из приложения с ошибкой
						return EXIT_FAILURE;
					}
					// Объект дерева, перезаписью иного оформления собранного
					yaml::document_t reshaped(limitless);
					/**
					 * Если разобрать перезапись иного оформления не удалось
					 */
					if(!reshaped.parse(restyled)){
						// Выводим сообщение об отказе чтения перезаписи иного оформления
						::fprintf(stderr, "yaml fuzz: restyled rewrite refused %s, layout %u quoting %u indent %u seq %u start %u end %u version %u\n",
						 yaml::message(reshaped.error()), static_cast <uint32_t> (writing.layout), quoting,
						 static_cast <uint32_t> (writing.indent), static_cast <uint32_t> (writing.sequenceIndent),
						 static_cast <uint32_t> (writing.explicitStart), static_cast <uint32_t> (writing.explicitEnd),
						 static_cast <uint32_t> (writing.version));
						// Выводим перезапись иного оформления
						dump(restyled);
						// Выводим разбираемый текст
						dump(text);
						// Выходим из приложения с ошибкой
						return EXIT_FAILURE;
					}
					/**
					 * Если дерево перезаписи иного оформления с исходным разошлось
					 *
					 * @note Сличаются ДЕРЕВЬЯ, а не перезаписи их. Оформление узла ядро
					 *       запоминает, и текст, оградою двойною записанный, перезапишется
					 *       ею же: «12:30» против «"12:30"» - та же строка, записанная
					 *       двояко. Сличение перезаписей звало бы это расхождением, чем и
					 *       обмануло меня при первом же прогоне
					 */
					if(!(yaml::value_t(reshaped.root()) == yaml::value_t(document.root()))){
						// Выводим сообщение о расхождении деревьев
						::fprintf(stderr, "yaml fuzz: restyled rewrite differs, layout %u quoting %u indent %u seq %u start %u end %u version %u\n",
						 static_cast <uint32_t> (writing.layout), quoting, static_cast <uint32_t> (writing.indent),
						 static_cast <uint32_t> (writing.sequenceIndent), static_cast <uint32_t> (writing.explicitStart),
						 static_cast <uint32_t> (writing.explicitEnd), static_cast <uint32_t> (writing.version));
						// Выводим разбираемый текст
						dump(text);
						// Выводим перезапись иного оформления
						dump(restyled);
						// Выводим перезапись исходного дерева
						dump(document.dump());
						// Выводим перезапись дерева иного оформления
						dump(reshaped.dump());
						// Выходим из приложения с ошибкой
						return EXIT_FAILURE;
					}
					// Выполняем учёт кругового хода перезаписи иным оформлением
					totals.restyled++;
					}
					}
				}
			}
			/**
			 * Выполняем проверку правки дерева документа
			 *
			 * @details Правка обязана давать текст, обратным чтением принимаемый: узел
			 *          собирается заново, соседи его переносятся дословно, и стык двух этих
			 *          кусков есть место, где отступ разъезжается легче всего
			 */
			{
				// Настройки разбора дерева с удержанием исходного текста
				yaml::document_t::settings_t held = tree;
				// Устанавливаем удержание исходного текста
				held.retain = true;
				// Снимаем предел глубины вложенности, ссылками раскрываемый
				held.maxDepth = 0;
				// Снимаем предел длины скалярного значения
				held.maxScalar = 0;
				// Объект дерева документа, правке подлежащего
				yaml::document_t edited(held);
				/**
				 * Если разобрать текст в дерево документа удалось
				 */
				if(edited.parse(text) && (edited.encoding() == yaml::encoding_t::UTF8)){
					// Перечень путей ко всем узлам дерева документа
					vector <string> paths;
					/**
					 * Выполняем перебор всех документов текста
					 */
					for(size_t i = 0; i < edited.documents(); i++)
						// Выполняем набор путей ко всем узлам очередного документа
						collect(edited.root(i), string(), paths);
					/**
					 * Если пути к узлам дерева набраны
					 */
					if(!paths.empty()){
						// Получаем путь к правимому узлу дерева
						const string & path = paths.at(engine() % paths.size());
						// Признак успешной правки дерева документа
						bool changed = false;
						// Вид совершаемой правки
						const uint8_t kind = static_cast <uint8_t> (engine() % 6);
						/**
						 * Определяем вид совершаемой правки
						 */
						switch(kind){
							// Если правкой ставится строковое значение
							case 0: changed = edited.set(path, string_view("правка")); break;
							// Если правкой ставится значение, числом читаемое
							case 1: changed = edited.set(path, string_view("12")); break;
							// Если правкой ставится целое значение
							case 2: changed = edited.set(path, static_cast <int64_t> (-1250)); break;
							// Если правкой ставится дробное значение
							case 3: changed = edited.set(path, 3.14159); break;
							// Если правкой ставится пустое значение
							case 4: changed = edited.reset(path); break;
							// Если правкой снимается узел
							case 5: changed = edited.erase(path); break;
						}
						/**
						 * Если правка отвергнута, а причина отказа не названа
						 *
						 * @details Отказ, кода ошибки не назначающий, оставляет потребителя
						 *          ни с чем: отличить отсутствие узла от изъяна пути он
						 *          тогда не может вовсе. Правило это поверяется здесь, а не
						 *          глазами по исходному тексту: путей отказа у правки дерева
						 *          десятки
						 */
						if(!changed && (edited.error() == yaml::error_t::NONE)){
							// Выводим сообщение об отказе без названной причины
							::fprintf(stderr, "yaml fuzz: edit refused without a named cause, kind %u, path [%s]\n",
							 static_cast <uint32_t> (kind), path.c_str());
							// Выводим отрицательный результат работы генератора
							return EXIT_FAILURE;
						}
						/**
						 * Если правка дерева удалась
						 */
						if(changed){
							// Выполняем учёт правленого дерева документа
							totals.edited++;
							// Объект дерева правленого документа
							yaml::document_t back(held);
							/**
							 * Если разобрать правленый текст не удалось
							 */
							if(!back.parse(edited.dump())){
								// Выводим сообщение об отказе чтения правленого текста
								::fprintf(stderr, "yaml fuzz: edited rewrite refused %s at «%s», settings %s\n",
									yaml::message(back.error()), path.c_str(), described(settings).c_str());
								// Выводим разбираемый текст
								dump(text);
								// Выводим правленый текст
								dump(edited.dump());
								// Выходим из приложения с ошибкой
								return EXIT_FAILURE;
							}
							/**
							 * Выполняем проверку состоявшегося сноса узла
							 *
							 * @details Обратное чтение правленого текста доказывает лишь то, что
							 *          текст читается, - но не то, что снос в него попал. Узел,
							 *          удержанием дословно перенесённый, остаётся в тексте и
							 *          после снятия своего из дерева: сосед его накрывает байты
							 *          снятого хвостом записи своей. Снос, успехом ответивший,
							 *          записанное изменить обязан
							 *
							 * @note Обе стороны идут одним и тем же путём записи с удержанием, и
							 *       потому вид записи из сличения выпадает: сохранение вида есть
							 *       смысл удержания, и сличать по нему нечего. Прочие правки
							 *       сличению не подлежат - постановка значения вправе записанного
							 *       и не изменить, коли значение то уже стояло
							 */
							/**
							 * Выполняем проверку устойчивости правленой перезаписи
							 *
							 * @details Запись обязана быть точкою неподвижною: перезапись её
							 *          обязана совпасть с нею октет в октет. Вид записи,
							 *          зависящий от собственного разбора её, означает, что
							 *          круговой ход не сходится - и всякий следующий ход
							 *          менял бы текст заново
							 *
							 * @note Поверку эту несут ворошители TOML да INI с самого начала,
							 *       а YAML нёс её лишь для дерева неправленого
							 */
							if(back.dump() != edited.dump()){
								// Выводим сообщение о неустойчивости правленой перезаписи
								::fprintf(stderr, "yaml fuzz: edited rewrite unstable at «%s», settings %s\n",
									path.c_str(), described(settings).c_str());
								// Выводим разбираемый текст
								dump(text);
								// Выводим правленый текст
								dump(edited.dump());
								// Выводим перезапись правленого текста
								dump(back.dump());
								// Выходим из приложения с ошибкой
								return EXIT_FAILURE;
							}
							/**
							 * Выполняем проверку правки, в текст попавшей
							 *
							 * @details Обратное чтение доказывает лишь то, что текст читается.
							 *          Сличается оттого чтение правленого дерева с чтением
							 *          перезаписи его: правка, до текста не дошедшая, расходит
							 *          их немедля. Ворошители TOML да INI поверку эту несли, а
							 *          YAML - нет, и снос узла, из записи пропадавший, проходил
							 *          ими незамеченным
							 *
							 * @note Тексты со ссылками сличению не подлежат: ссылка
							 *       раскрывается при разборе, и копия её правку метки уже не
							 *       видит, тогда как перезапись с обратным чтением раскрывает
							 *       ссылку заново - значением правленым. Расхождение это
							 *       ссылкам присуще и дефектом не является
							 */
							if((text.find('*') == string::npos) && (text.find('&') == string::npos)){
								// Выполняем учёт правки, чтением сличаемой
								totals.mirrored++;
								/**
								 * Если чтение правленого дерева с чтением записи его разошлось
								 */
								if(!(yaml::value_t(edited.root()) == yaml::value_t(back.root()))){
									// Выводим сообщение о правке, в текст не попавшей
									::fprintf(stderr, "yaml fuzz: edit %u lost at «%s», settings %s\n",
										static_cast <unsigned> (kind), path.c_str(), described(settings).c_str());
									// Выводим разбираемый текст
									dump(text);
									// Выводим правленый текст
									dump(edited.dump());
									// Выводим запись обратного чтения
									dump(back.dump());
									// Выходим из приложения с ошибкой
									return EXIT_FAILURE;
								}
							}
							/**
							 * Если правкой снимался узел
							 */
							if(kind == 5){
								// Объект дерева, исходный текст удержанием читающего
								yaml::document_t before(held);
								/**
								 * Если исходный текст удержанием прочитан
								 */
								if(before.parse(text)){
									/**
									 * Если запись после сноса записи до него равна
									 */
									if(before.dump() == edited.dump()){
										// Выводим сообщение о сносе, в текст не попавшем
										::fprintf(stderr, "yaml fuzz: erase lost at «%s», settings %s\n",
											path.c_str(), described(settings).c_str());
										// Выводим разбираемый текст
										dump(text);
										// Выводим запись, сносу подвергнутую
										dump(edited.dump());
										// Выходим из приложения с ошибкой
										return EXIT_FAILURE;
									}
								}
							}
						}
					}
				}
			}
			/**
			 * Выполняем проверку удержания исходного текста при сносе повторных пар
			 *
			 * @details Проверка дословная ниже правило повторов сама ставит `KEEP`, и потому
			 *          сочетание «удержание плюс снос пары» ей недоступно вовсе. Между тем
			 *          снос пары двигает границы записей соседних узлов, и узел, к снесённой
			 *          паре примыкающий, накрывает её байты своим хвостом при дословном
			 *          переносе - пара, деревом снесённая, возвращается в текст. Правки для
			 *          этого не требуется никакой: хватает разбора и обратной записи
			 *
			 * @note Сличать здесь с исходным текстом побайтово нельзя: пару снесли нарочно, и
			 *       расхождение байтов законно. Потому запись читается обратно, и сличаются
			 *       два дерева - записанное обязано нести ровно то, что дерево держит
			 */
			{
				// Настройки разбора дерева с удержанием исходного текста
				yaml::document_t::settings_t held = tree;
				// Устанавливаем удержание исходного текста
				held.retain = true;
				// Объект дерева документа, текст удерживающего
				yaml::document_t pruned(held);
				/**
				 * Если разобрать текст в дерево документа удалось
				 */
				if(pruned.parse(text) && (pruned.encoding() == yaml::encoding_t::UTF8)){
					// Выполняем учёт собранного дерева с удержанием
					totals.pruned++;
					// Выполняем перезапись удержанного дерева
					const string & written = pruned.dump();
					// Объект дерева документа, перезапись обратно читающего
					yaml::document_t reread(held);
					/**
					 * Если перезапись обратным чтением принята
					 */
					if(reread.parse(written)){
						/**
						 * Если дерево перезаписи с деревом исходным разошлось
						 */
						if(reread.dump() != written){
							// Выводим сообщение о расхождении удержания при сносе пар
							::fprintf(stderr, "yaml fuzz: pruned retention unstable, settings %s\n", described(settings).c_str());
							// Выводим разбираемый текст
							dump(text);
							// Выводим перезапись удержанного дерева
							dump(written);
							// Выводим перезапись обратного чтения
							dump(reread.dump());
							// Выходим из приложения с ошибкой
							return EXIT_FAILURE;
						}
					}
				}
			}
			/**
			 * Выполняем проверку дословной перезаписи при удержании исходного текста
			 *
			 * @details Удержание обязано давать перезапись, исходному тексту побайтово
			 *          равную: правки не было, и собирать заново нечего. Проверка эта сильнее
			 *          всех прочих - расходится она на всяком примечании, всякой пустой строке
			 *          и всякой ширине отступа, дереву неведомых
			 *
			 * @note Проверяется она лишь у текста в кодировке UTF-8: удержание работает
			 *       только там, а текст иной кодировки перезаписью приводится к UTF-8, и
			 *       равенства байтов с ним быть не может по устройству
			 */
			{
				// Настройки разбора дерева с удержанием исходного текста
				yaml::document_t::settings_t held = tree;
				// Устанавливаем удержание исходного текста
				held.retain = true;
				/**
				 * Устанавливаем удержание всех пар отображения
				 *
				 * @note Правило иное сносит пару повторную, и текст обязан от исходного
				 *       расходиться: сличается здесь дословность переноса, а не правило
				 *       обхождения с повторами, и снос пары сличению этому не подлежит
				 */
				held.duplicates = yaml::duplicate_t::KEEP;
				// Объект дерева документа, текст удерживающего
				yaml::document_t kept(held);
				/**
				 * Если разобрать текст в дерево документа удалось
				 */
				if(kept.parse(text) && (kept.encoding() == yaml::encoding_t::UTF8)){
					// Выполняем учёт собранного дерева с удержанием
					totals.kept++;
					/**
					 * Если перезапись с исходным текстом побайтово разошлась
					 */
					if(kept.dump() != text){
						// Выводим сообщение о расхождении дословной перезаписи
						::fprintf(stderr, "yaml fuzz: retained rewrite differs, settings %s\n", described(settings).c_str());
						// Выводим разбираемый текст
						dump(text);
						// Выводим перезапись удержанного дерева
						dump(kept.dump());
						// Выходим из приложения с ошибкой
						return EXIT_FAILURE;
					}
				}
			}
		}
		/**
		 * Если текст без метки порядка байтов опознан не как UTF-8
		 *
		 * @details Опознание описанием задано прямо: пара байтов `xx 00` в начале текста
		 *          без метки есть UTF-16LE. Испорченный текст, пустой байт вторым знаком
		 *          несущий, под правило это и попадает - и текстом UTF-8 он уже не читается.
		 *          Сличать его с приведением к UTF-16 нечестно: это два разных текста, и
		 *          расхождение их будет виною ворошителя, а не разбора
		 */
		if(recognised != yaml::encoding_t::UTF8){
			// Выполняем учёт текста, к сличению не допущенного
			totals.unconverted++;
			// Выполняем переход к следующему проходу генератора
			continue;
		}
		/**
		 * Если текст несёт пустой байт
		 *
		 * @details Приведение пустого знака к UTF-16 даёт `00 00` следом за меткой порядка
		 *          байтов, а `FF FE 00 00` есть метка UTF-32LE - и текст, приведённый к
		 *          UTF-16, опознаётся как UTF-32. Двусмысленность эта заложена в самом
		 *          описании, и разбор тут ни при чём: пустой знак тексту YAML не принадлежит
		 *          вовсе, и приводить его к иной кодировке незачем
		 */
		if(source.find('\0') != string::npos){
			// Выполняем учёт текста, к сличению не допущенного
			totals.unconverted++;
			// Выполняем переход к следующему проходу генератора
			continue;
		}
		/**
		 * Выполняем перебор обоих порядков байтов кодировки UTF-16
		 */
		for(const bool big : {false, true}){
			// Приведённый к кодировке UTF-16 текст
			string converted;
			/**
			 * Если привести текст к кодировке UTF-16 не удалось
			 */
			if(!transcode(source, big, converted))
				// Выходим из перебора порядков байтов
				break;
			// Перечень событий подачи приведённого текста
			vector <Event> foreign;
			// Выполняем подачу приведённого текста целиком
			consume(converted, settings, 0, foreign);
			// Выполняем сброс смещений начала событий
			detach(foreign);
			// Выполняем учёт сличения кодировок
			totals.transcoded++;
			/**
			 * Если перечни выданных разбором событий разошлись
			 */
			if(!compare(native, foreign, (big ? "utf16be" : "utf16le"), text, settings))
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
		}
		/**
		 * Выполняем перебор обоих порядков байтов кодировки UTF-32
		 *
		 * @details Кодировка эта опознаётся меткой порядка байтов так же, как и UTF-16, и
		 *          круг её обязан сойтись с кругом UTF-8 знак в знак: расхождение здесь есть
		 *          дефект приведения кодировки, а не разбора текста
		 */
		for(const bool big : {false, true}){
			// Приведённый к кодировке UTF-32 текст
			string converted;
			/**
			 * Если привести текст к кодировке UTF-32 не удалось
			 */
			if(!transcode32(source, big, converted))
				// Выходим из перебора порядков байтов
				break;
			// Перечень событий подачи приведённого текста
			vector <Event> foreign;
			// Выполняем подачу приведённого текста целиком
			consume(converted, settings, 0, foreign);
			// Выполняем сброс смещений начала событий
			detach(foreign);
			// Выполняем учёт сличения кодировок
			totals.transcoded++;
			/**
			 * Если перечни выданных разбором событий разошлись
			 */
			if(!compare(native, foreign, (big ? "utf32be" : "utf32le"), text, settings))
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
		}
		/**
		 * Выполняем перебор однобайтовых кодировок текста
		 *
		 * @details Метки порядка байтов такие кодировки не имеют, и разбор опознать их не
		 *          может вовсе: кодировка навязывается ему настройками. Оттого круг этот
		 *          держит не опознание, а само приведение знаков - и берёт он путь разбора,
		 *          какого метка порядка байтов не открывает
		 */
		for(const yaml::encoding_t encoding : {yaml::encoding_t::LATIN1, yaml::encoding_t::ASCII, yaml::encoding_t::CP1252}){
			// Приведённый к однобайтовой кодировке текст
			string converted;
			/**
			 * Если привести текст к однобайтовой кодировке не удалось
			 */
			if(!narrow(source, encoding, converted)){
				// Выполняем учёт приведения, знаками текста не допущенного
				totals.unnarrowed++;
				// Выполняем переход к следующей однобайтовой кодировке
				continue;
			}
			// Настройки разбора с навязанной кодировкой текста
			yaml::reader_t::settings_t forced = settings;
			// Устанавливаем навязанную разбору кодировку текста
			forced.encoding = encoding;
			// Перечень событий подачи приведённого текста
			vector <Event> foreign;
			// Выполняем подачу приведённого текста целиком
			consume(converted, forced, 0, foreign);
			// Выполняем сброс смещений начала событий
			detach(foreign);
			// Выполняем учёт сличения кодировок
			totals.transcoded++;
			// Получаем название кодировки, разбору навязанной
			const char * naming = ((encoding == yaml::encoding_t::LATIN1) ? "latin1" :
			 ((encoding == yaml::encoding_t::ASCII) ? "ascii" : "cp1252"));
			/**
			 * Если перечни выданных разбором событий разошлись
			 */
			if(!compare(native, foreign, naming, text, settings))
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
		}
		/**
		 * Выполняем сличение записи свежей с записью, прежней подачей уже занятой
		 *
		 * @details Объект записи потребитель вправе держать один на многие тексты,
		 * очищая его между ними. Остаток прежней подачи - накопленный текст, стопа
		 * открытых построений, назначенный отступ, опора с меткою, ждущие своего
		 * значения, да код прежнего отказа - обязан пропадать без следа: запись,
		 * очищенная от прежнего, обязана собрать следующий текст ровно так же, как
		 * собрала бы его запись свежая. Дефект такого рода прогоном свежей записи не
		 * показывается ни разу: вылезает он лишь у потребителя, собирающего одним
		 * объектом много текстов подряд
		 *
		 * @note Череда вызовов заготавливается наперёд, а не берётся у источника по
		 *       ходу подачи: возьми её всякий объект записи сам, и сличались бы две
		 *       разные череды, а расхождение их за дефект принять было бы нельзя
		 */
		{
			// Череда вызовов, наудачу заготовленная
			vector <uint32_t> script(1 + (engine() % 24));
			/**
			 * Выполняем заготовку череды вызовов
			 */
			for(size_t j = 0; j < script.size(); j++)
				// Выполняем выборку очередного вызова череды
				script.at(j) = static_cast <uint32_t> (engine());
			// Настройки записи текста, наудачу выбранные
			yaml::writer_t::settings_t writing;
			// Устанавливаем построение по умолчанию наудачу
			writing.layout = (((engine() % 2) == 0) ? yaml::layout_t::BLOCK : yaml::layout_t::FLOW);
			// Устанавливаем ширину отступа наудачу
			writing.indent = (1 + (engine() % 8));
			// Устанавливаем отступ записей перечня наудачу
			writing.sequenceIndent = ((engine() % 2) == 0);
			// Устанавливаем запись явного начала документа наудачу
			writing.explicitStart = ((engine() % 2) == 0);
			// Устанавливаем запись явного конца документа наудачу
			writing.explicitEnd = ((engine() % 2) == 0);
			// Объект записи, живущий от прохода к проходу
			static yaml::writer_t reused;
			// Выполняем очистку объекта записи от прежней подачи
			reused.clear();
			// Устанавливаем настройки записи объекту, прежней подачей занятому
			reused.settings(writing);
			// Создаём свежий объект записи для той же самой череды вызовов
			yaml::writer_t fresh(writing);
			// Исходы вызовов у свежего объекта записи
			vector <bool> first;
			// Исходы вызовов у объекта записи, прежней подачей занятого
			vector <bool> second;
			// Признак завершения записи по окончании череды, наудачу выбранный
			const bool complete = ((engine() % 4) != 0);
			// Выполняем подачу череды вызовов свежему объекту записи
			replay(script, fresh, first, complete);
			// Выполняем подачу той же череды объекту, прежней подачей занятому
			replay(script, reused, second, complete);
			/**
			 * Если исходы вызовов у двух объектов записи разошлись
			 */
			if(first != second){
				// Выводим сообщение о расхождении исходов вызовов
				::fprintf(stderr, "yaml fuzz: feeding outcomes differ between a fresh writer and a reused one\n");
				// Выводим текст, собранный свежим объектом записи
				dump(fresh.text());
				// Выводим текст, собранный объектом, прежней подачей занятым
				dump(reused.text());
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
			}
			/**
			 * Если собранный двумя объектами записи текст разошёлся
			 */
			if(fresh.text() != reused.text()){
				// Выводим сообщение о расхождении собранного текста
				::fprintf(stderr, "yaml fuzz: a writer reused after a previous feed assembled a different text\n");
				// Выводим текст, собранный свежим объектом записи
				dump(fresh.text());
				// Выводим текст, собранный объектом, прежней подачей занятым
				dump(reused.text());
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
			}
			/**
			 * Если код отказа у двух объектов записи разошёлся
			 */
			if(fresh.error() != reused.error()){
				// Выводим сообщение о расхождении кода отказа
				::fprintf(stderr, "yaml fuzz: refusal code differs between a fresh writer and a reused one: %u vs %u\n",
				 static_cast <uint32_t> (fresh.error()), static_cast <uint32_t> (reused.error()));
				// Выходим из приложения с ошибкой
				return EXIT_FAILURE;
			}
			// Выполняем учёт сличения записи свежей с записью занятой
			totals.transcribed++;
		}
	}
	// Выводим итог работы генератора
	::fprintf(stderr, "yaml fuzz: %llu texts (%llu corrupted, %llu survived), %llu events, %llu chunked, %llu transcoded (%llu rejected, %llu single-byte filtered), %llu trees, %llu kept, %llu pruned, %llu edited (%llu verified), %llu taken, %llu assembled, %llu restyled (%llu skipped), %llu recycled (%llu by tail), %llu grafts (%llu refused), %llu transcribed, %llu circling%s\n",
		static_cast <unsigned long long> (totals.texts), static_cast <unsigned long long> (totals.corrupted),
		static_cast <unsigned long long> (totals.survived), static_cast <unsigned long long> (totals.events),
		static_cast <unsigned long long> (totals.chunked), static_cast <unsigned long long> (totals.transcoded),
		static_cast <unsigned long long> (totals.unconverted),
		static_cast <unsigned long long> (totals.unnarrowed),
		static_cast <unsigned long long> (totals.trees), static_cast <unsigned long long> (totals.kept),
		static_cast <unsigned long long> (totals.pruned),
		static_cast <unsigned long long> (totals.edited), static_cast <unsigned long long> (totals.mirrored), static_cast <unsigned long long> (totals.taken),
		static_cast <unsigned long long> (totals.assembled),
		static_cast <unsigned long long> (totals.restyled),
		static_cast <unsigned long long> (totals.undirected),
		static_cast <unsigned long long> (totals.recycled),
		static_cast <unsigned long long> (totals.tailed),
		static_cast <unsigned long long> (totals.grafted),
		static_cast <unsigned long long> (totals.grafts - totals.grafted),
		static_cast <unsigned long long> (totals.transcribed),
		static_cast <unsigned long long> (totals.circling),
		(AWH_FUZZ_SANITIZED ? "" : " [БЕЗ НАДЗИРАТЕЛЕЙ]"));
	// Выводим код успешного выхода из приложения
	return EXIT_SUCCESS;
}
