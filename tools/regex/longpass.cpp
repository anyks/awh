 ...
 *
 * @brief Щуп разыскания цены прохода по тексту на ДЛИННОМ тексте —
 *        то же деление стоимости, что и у «passes», на тексте в четверть мегабайта
 *
 * @details Текст сложен тем же порядком, что и у набора замеров: связная
 *          проза до заданной длины, а искомые последовательности — у самого
 *          конца, отчего текст приходится пройти весь. Длина текста меняет
 *          выбор пути исполнения, и щуп короткого текста ответа за длинный
 *          не даёт: пороги «MAX_BACKTRACK» и «MIN_REVERSE» разводят их
 *          по разным путям
 *
 *          Щуп собирается и запускается стендом своим - признак
 *          «AWH_REGEX_PROBING» и состав исходных текстов ведутся там:
 *          @code
 *          sh tools/regex/probe.sh longpass
 *          @endcode
 *
