...
#   PROBING — «нет» собирает щуп без учёта путей и мер работы
...
cd "$ROOT"
##
# Учёт путей и мер работы снимается признаком «PROBING=нет»
#
# Учёт стоит времени: всякое сопоставление вносит меры работы сложением
# атомарным, семь раз на вызов. Щуп, плату за вызов разлагающий, мерил бы
# тогда и её, а цель его - та плата, какую несёт сборка штатная
##
DEFINES="-DAWH_REGEX_PROBING"
if [ "$PROBING" = "нет" ] || [ "$PROBING" = "no" ]; then
	DEFINES=""
fi
echo "учёт: $([ -n "$DEFINES" ] && echo заведён || echo снят)"
##
...
$CXX -std=c++17 -O3 -DNDEBUG $DEFINES -Wno-c++11-narrowing $FLAGS \
