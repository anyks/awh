...
ssh -p "$PORT" "$TARGET" '
	...
	#
	# Порядок перечисления обратный порядку в пути: приписка ведётся в начало,
	# и каталог, перечисленный последним, оказывается в пути первым. Каталог
	# связки вооружения обязан опережать /usr/bin: последний несёт у MSYS2
	# компилятор с целью «x86_64-pc-cygwin», и стенд, его взявший, проверял бы
	# сборку под Cygwin, называя её сборкой под Windows. Набор POSIX при этом
	# не теряется: tar и gzip лежат в /usr/bin, а в каталогах связки их нет,
	# и поиск доходит до /usr/bin сам
