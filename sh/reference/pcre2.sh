...
##
 # Подмодуль, развёрнутый не на закреплённом коммите, - отказ
 #
 # Указатель подмодуля передвигает коммит дерева, а развёрнутое содержимое - лишь
 # `git submodule update`. На стенде Эльбруса 24.09.2026 указатель стоял на 10.49-DEV,
 # а развёрнут был 10.48-DEV, и свежая сборка честно собирала эталон не тот. Знак «+»
 # в начале строки состояния и есть это расхождение. Дерево без git - копия
 # исходников - проверку пропускает: сверять там не с чем
 ##
STATE=$(git -C "$ROOT" submodule status submodules/pcre2 2> /dev/null)
case "$STATE" in
	+*)
		echo "ОТКАЗ: подмодуль PCRE2 развёрнут не на закреплённом коммите: $STATE; выполните:" >&2
		echo "  git submodule update --init --recursive submodules/pcre2" >&2
		exit 1
		;;
esac
...
##
 # Сборка метится источником и переиспользуется лишь при метке той же
 #
 # Прежде она переиспользовалась, пока в каталоге лежала библиотека: «меняться ей
 # нечего, подмодуль закреплён». Закреплённое, однако, передвигается коммитом:
 # 23.09.2026 подмодуль перешёл с 10.48-DEV на 10.49-DEV, а стенды сличали и дальше
 # с 10.48 - каталог сборки его и держал. Ныне всякий источник собирается в свой
 # подкаталог: сборка иного источника прежней не подменяется, а та же занимает
 # минуту лишь однажды. Метка - коммит развёрнутого подмодуля и признак порождения
 # кода; у копии исходников без git - сумма заголовка версии и списка сборки.
 # Сборки прежних источников остаются рядом и не мешают: удалить их можно вместе
 # с каталогом сборки целиком
 ##
...
##
 ...
 # Ищется он в сборке ЭТОГО источника: в корне каталога может лежать сборка прежних
 # вызовов, и поиск по всему каталогу вывел бы её заголовок
 ##
dirname "$(find "$BUILD" -name pcre2.h | head -1)"
echo "$BUILD/libpcre2-8.a"
exit 0
