...
# и оно меняется от версии к версии. Между 1.66.0 и 1.69.90 nghttp2 сменил реакцию
# на переполнение окна потока и ускорился на 13-26% - и то и другое выглядело бы
# как регрессия у нас. Поэтому версия сверяется, а не просто печатается
#
readonly NGHTTP2_EXPECTED="1.70.90"

##
# Ожидаемое количество известных расхождений в сверке обработки ошибок
#
# Расхождение не означает дефекта: RFC во многих местах допускает и потоковую
# ошибку, и ошибку соединения. Но изменение их числа означает, что поведение
...
NGHTTP2_VERSION=$(grep -hoE '"[0-9][0-9.]*"' "$NGHTTP2_BUILD/lib/includes/nghttp2/nghttp2ver.h" 2>/dev/null | head -1 | tr -d '"')
# Если версия не определена - помечаем её как системную
[ -n "$NGHTTP2_VERSION" ] || NGHTTP2_VERSION="системная"
announce "Сверки с эталонной реализацией nghttp2 $NGHTTP2_VERSION"
# Если версия эталона разошлась с той, на которой снята документация
if [ "$NGHTTP2_VERSION" != "$NGHTTP2_EXPECTED" ]; then
	# Выводим предупреждение о смене версии эталона
	printf "\033[33m  вним\033[0m  документация снята на %s: перечень известных расхождений и числа\n" "$NGHTTP2_EXPECTED"
	printf "        в benchmark/proto/http2/COMPARISON.md подлежат пересмотру\n"
fi
...
printf "\n\033[32mПРОВЕРКА ПРОЙДЕНА\033[0m: шагов %d, эталон nghttp2 %s, журналы в %s\n" "$STEPS" "${NGHTTP2_VERSION:-нет}" "$WORK"
