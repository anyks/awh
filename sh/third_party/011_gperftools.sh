...
if [ -n "$1" ]; then
	...
	# Если необходимо собрать проект
		...
		if [ ! -f "$src/.stamp_done" ]; then
			...
			##
			 # Каталог сборки очищается целиком, а не одним файлом кэша
			 #
			 # Настройка порождает в нём gperftools/tcmalloc.h, а путь -I на каталог
			 # сборки стоит впереди пути на исходники. Заголовок от прошлого выпуска
			 # переживает смену версии и заслоняет настоящий - собиратель отвечает
			 # отказом на имена, каких в старом выпуске ещё не было, притом указывая
			 # на исходники нового. Пробел всплывает лишь при смене версии подмодуля
			 #
			 # Полная очистка ничего не удорожает: до этого места управление доходит
			 # только при отсутствии отметки готовности, то есть при намеренной пересборке
			 ##
			rm -rf "$src/build_awh"
			...
			if [[ $OS = "Windows" ]]; then
				...
				# Если операционной системой является FreeBSD на архитектуре ARM
				#
				# Заголовок src/getpc-inl.h разбирает ucontext_t по парам «система и
				# архитектура», и для FreeBSD там расписаны i386, ppc и x86_64 - записи
				# для aarch64 нет вовсе. Сборка измерителя отвечает тем же отказом, что и
				# на illumos: "no member named 'Get' in ...". Это пробел самой gperftools
				# на неподдержанной ею связке, и правке с нашей стороны он не подлежит
				#
				# Гасится точечно лишь измеритель: tcmalloc и tcmalloc_minimal, ради
				# которых gperftools и собирается, встают целыми. FreeBSD на x86_64
				# измеритель собирает, и там он остаётся - проверено опытом на стенде
				# FreeBSD 14.1 amd64
				if [ $OS = "FreeBSD" ] && [ $ARCHITECTURE = "arm" ]; then
					# Гасим сборку измерителя нагрузки
			...
			##
			 # Каталог заводится с -p, а не без него
			 #
			 # Без признака повторная сборка отвечает отказом «File exists» и валит скрипт
			 # целиком: библиотека к тому моменту уже собрана, а перенести её наружу
			 # управление не успевает. Всплывает лишь при пересборке поверх готового
			 # дерева зависимостей - при чистой сборке каталога ещё нет, и пробел не виден
			 ##
			mkdir -p "$PREFIX/include/tcmalloc" || exit 1
