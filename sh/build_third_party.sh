...
##
 ...
 # -march=core2 - команды, которых у ARM64 нет вовсе
 ##
...
##
 # Средство сборки выбирается по наличию, а не по умолчанию
 #
 # Ninja собирает то же дерево заметно быстрее: задания он раздаёт по готовности, а не
 # по слоям, и на машине с многими вычислителями разница выходит ощутимой. Требовать
 # его нельзя - ставится он отдельно и есть не везде, а у того, кто его не ставил,
 # сборка обязана идти по-прежнему.
 #
 # Отсюда и проверка наличием: есть ninja - берётся он, нет - остаётся GNU make и
 # прежнее описание сборки. Выбранное называется вслух: молчаливая смена средства
 # сборки потом разыскивается как чужая беда.
 #
 # Имён у него два: часть систем зовёт его ninja, часть - ninja-build (там имя ninja
 # занято другой программой). Спрашиваются оба, иначе на этих системах он считался бы
 # отсутствующим при том, что установлен.
 #
 # Зависимости, собираемые СВОИМ описанием, остаются на GNU make при любом выборе:
 # density и lizard несут Makefile, а описания для CMake не имеют вовсе, и zlib у
 # систем POSIX настраивается своим configure
##
BUILDER="$MAKE"

# Описание сборки по умолчанию: у оболочки MSYS2 оно своё
if [ $OS = "Windows" ]; then
...
# Если Ninja установлен в системе
...
echo "Build tool: $BUILDER (CMake generator: $GENERATOR)"
