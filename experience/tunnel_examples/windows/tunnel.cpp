...
inline bool is_keepalive(const uint8_t* d, size_t l) {
    return l == KEEPALIVE_SIZE && memcmp(d, KEEPALIVE_MARKER, KEEPALIVE_SIZE) == 0;
}

void run_tunnel(SOCKET tun_fd, SOCKET net_fd, const struct sockaddr* remote, socklen_t rlen) {
    ...
    while (true) {
        ...
        FD_SET(tun_fd, &rfds);
        FD_SET(net_fd, &rfds);
        SOCKET max_fd = (tun_fd > net_fd ? tun_fd : net_fd) + 1;
